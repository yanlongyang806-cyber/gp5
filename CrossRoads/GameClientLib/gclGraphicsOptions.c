...
//////////////////////////////////////////////////////////////////////////
// Transgaming specific

AUTO_COMMAND ACMD_ACCESSLEVEL(0);
void gfxTransgamingSetFullscreen(bool fs) {
	TGSetFullscreen(fs);
}

bool gfxTransgamingGetFullscreen(void) {
	return TGIsFullscreen();
}
...
static int adapter_index_plus_one;
static int limitInactiveFPS;
static GfxAntialiasingMode antialiasing_Mode_Lookup[GAA_MAX_MODES];
static int antialiasing_Mode_Count;
static int antialiasing_Mode;
static int antialiasing_Quality;
static int texAniso;
...
static OptionSetting* settingAntialiasing_Quality = NULL;
...
static void uiScaleRevert(OptionSetting *setting)
{
	if (1.f != setting->fFloatValue)
	{
		setting->fFloatValue = 1.f;
		GamePrefStoreFloat("UIScale", setting->fFloatValue);
	}
}
...
static int calcDesiredMatScore(F32 slowUglyScaleVal)
{
	if (gConf.bEnableShaderQualitySlider == SHADER_QUALITY_SLIDER_LABEL)
	{
		if (showAdvanced)
		{
			return CLAMP(matDesiredScore * SGRAPH_QUALITY_INTERVAL_SIZE,0,SGRAPH_QUALITY_MAX_VALUE);
		}
		else
		{
			...
			switch (scaleCase)
			{
			case 0: return SGRAPH_QUALITY_LOW;
				break;
			case 1: return CLAMP(gcl_gfx_settings.recommended_Mat_quality - 1,0,SGRAPH_QUALITY_MAX_VALUE);
				break;
			case 2: return gcl_gfx_settings.recommended_Mat_quality;
				break;
			case 3: return SGRAPH_QUALITY_MAX;
				break;
			default: return SGRAPH_QUALITY_LOW;
			}
		}
	}
	return SGRAPH_QUALITY_LOW;
}

static void preApply(void)
{
	const RdrDeviceInfo * const * devices = rdrEnumerateDevices();
	int adapter_index = adapter_index_plus_one ? adapter_index_plus_one - 1 : 0;
	// Takes local settings and puts them into gcl_gfx_settings to be passed to gfxApplySettings()

    gcl_gfx_settings.antialiasingMode = antialiasing_Mode_Lookup[antialiasing_Mode];
    gcl_gfx_settings.antialiasing = 1 << (antialiasing_Quality + 1);
	...
	if (adapter_index_plus_one)
	{
		if (adapter_index < eaSize(&devices))
		{
			gcl_gfx_settings.device_type = devices[adapter_index]->type;
			gcl_gfx_settings.monitor_index_plus_one = devices[adapter_index]->monitor_index + 1;
		}
		else
		{
			// back to default
			adapter_index_plus_one = 0;
			adapter_index = 0;
			gcl_gfx_settings.device_type = NULL;
			gcl_gfx_settings.monitor_index_plus_one = 0;
		}
	}
	else
	{
	...
	if(getIsTransgaming()) {
		...
		// Tell Transgaming's API about whatever was selected for fullscreen mode.
		if(fullscreen_mode == 0) {
			gfxTransgamingSetFullscreen(false);
		} else {
			gfxTransgamingSetFullscreen(true);
		}

		// Override actual fullscreen setting.
...
static void postGet(void)
{
    ...
    antialiasing_Mode_Count = 0;
	antialiasing_Mode_Lookup[antialiasing_Mode_Count++] = GAA_NONE;
	if (rdrSupportsSurfaceType(gfxGetPrimaryDevice(), SBT_RGBA) >= 2) {
		antialiasing_Mode_Lookup[antialiasing_Mode_Count++] = GAA_MSAA;
	}
	if (rdrSupportsFeature(gfxGetPrimaryDevice(), FEATURE_TXAA)) {
		antialiasing_Mode_Lookup[antialiasing_Mode_Count++] = GAA_TXAA;
	}

	// find the combobox index corresponding to the selected AA mode
	antialiasing_Mode = 0;
	for (i = 1; i < antialiasing_Mode_Count; ++i) {
		if (antialiasing_Mode_Lookup[i] == gcl_gfx_settings.antialiasingMode) {
			antialiasing_Mode = i;
			break;
		}
	}

	if (gcl_gfx_settings.antialiasingMode != GAA_NONE) {
        antialiasing_Quality = log2(gcl_gfx_settings.antialiasing) - 1;
	} else {
		antialiasing_Quality = 0;
	}
	...
	FOR_EACH_IN_EARRAY(supported_resolutions, GfxResolution, res)
	{
		if (((res->width == gcl_gfx_settings.fullscreenWidth &&
			...
			eaiFind(&res->refreshRates, gcl_gfx_settings.refreshRate) != -1)
		{
	...
	// search adapter list for (renderer device type, monitor) combination
	adapter_index_plus_one = -1;
	if (!rdrIsDeviceTypeAuto(gcl_gfx_settings.device_type))
	{
		FOR_EACH_IN_CONST_EARRAY_FORWARDS(device_infos, RdrDeviceInfo, device)
		{
			if (device->monitor_index + 1 == gcl_gfx_settings.monitor_index_plus_one &&
				!strcmp(device->type, gcl_gfx_settings.device_type))
			{
				adapter_index_plus_one = ideviceIndex + 1;
				break;
			}
		}
		FOR_EACH_END;
	...
	if (adapter_index_plus_one < 0 || adapter_index_plus_one >= eaSize(&device_infos) + 1 || 
		(adapter_index_plus_one >= 1 &&
		(device_infos[adapter_index_plus_one - 1]->monitor_index + 1 != gcl_gfx_settings.monitor_index_plus_one ||
		strcmp(device_infos[adapter_index_plus_one - 1]->type, gcl_gfx_settings.device_type))))
	{
		// reset to Auto
		adapter_index_plus_one = 0;
		gcl_gfx_settings.device_type = NULL;
	...
	if(getIsTransgaming()) {
		...
		fullscreen_mode = gfxTransgamingGetFullscreen();
	...
	if (gConf.bEnableShaderQualitySlider)
	{
		...
		matDesiredScore = gcl_gfx_settings.desired_Mat_quality / SGRAPH_QUALITY_INTERVAL_SIZE;

		wlSetDesiredMaterialScore(gcl_gfx_settings.desired_Mat_quality);
	}

	if (gcl_gfx_settings.antialiasingEnabled)
	{
		const char **options=NULL;
		options = NULL;
		eaPush(&options, allocAddCaseSensitiveString("None"));
		if (rdrSupportsSurfaceType(gfxGetPrimaryDevice(), SBT_RGBA) >= 2) {
			eaPush(&options, allocAddCaseSensitiveString("MSAA"));
		}
		if (rdrSupportsFeature(gfxGetPrimaryDevice(), FEATURE_TXAA)) {
			eaPush(&options, allocAddCaseSensitiveString("TXAA"));
		}

		OptionSettingUpdateComboBoxOptions(settingAntialiasing, GRAPHICS_CATEGORY, (char***)&options, false);
		OptionSettingSetActive(settingAntialiasing, true);
	}
	else
	{
		OptionSettingSetActive(settingAntialiasing, false);
	}

	// Update UI
	if (gcl_gfx_settings.antialiasing <= 1) {
		OptionSettingSetActive(settingAntialiasing_Quality, false);
	} else {
		const char **options=NULL;
		...
		if (gcl_gfx_settings.antialiasingMode == GAA_MSAA) {
		...
		OptionSettingSetActive(settingAntialiasing_Quality, true);
		OptionSettingUpdateComboBoxOptions(settingAntialiasing_Quality, GRAPHICS_CATEGORY, (char***)&options, false);
	}

	OptionSettingSetActive(settingResolution, gcl_gfx_settings.fullscreen != 0);
	OptionSettingSetActive(settingRefreshRate, gcl_gfx_settings.fullscreen != 0);
	OptionSettingSetActive(settingGamma, true);
	OptionSettingSetActive(settingMax_Shadowed_Lights, shadow_type > 1);
	{
...
void gfxSettingsChangedCallback(OptionSetting *setting_UNUSED)
{
...
void gfxSettingsNeedUpdate(void)
{
	g_ui_State.graphicsUINeedsUpdate = 1;
}
...
void gclGraphicsOptions_Init(void)
{
	...
	// Antialiasing selection
	{
		...
		settingAntialiasing = autoSettingsAddComboBox(GRAPHICS_CATEGORY, "Antialiasing_Mode", &options, false, &antialiasing_Mode, gfxSettingsChangedCallback, gfxSettingsCommittedCallback, gfxSettingsUpdateCallback, gfxSettingsRestoreDefaultsCallback, NULL, rdrSupportsSurfaceType(gfxGetPrimaryDevice(), SBT_RGBA) >= 2);
		...
		if (antialiasing_Mode_Lookup[antialiasing_Mode] != GAA_TXAA) {
			if (rdrSupportsSurfaceType(gfxGetPrimaryDevice(), SBT_RGBA) >= 8)
				eaPush(&options, allocAddCaseSensitiveString("8x"));
			if (rdrSupportsSurfaceType(gfxGetPrimaryDevice(), SBT_RGBA) >= 16)
				eaPush(&options, allocAddCaseSensitiveString("16x"));
		}
		settingAntialiasing_Quality = autoSettingsAddComboBox(GRAPHICS_CATEGORY, "Antialiasing_Quality", &options, false, &antialiasing_Quality, gfxSettingsChangedCallback, gfxSettingsCommittedCallback, gfxSettingsUpdateCallback, gfxSettingsRestoreDefaultsCallback, NULL, rdrSupportsSurfaceType(gfxGetPrimaryDevice(), SBT_RGBA) >= 2);
		OptionSettingSetDisabledTooltip(settingAntialiasing_Quality, "Options_Setting_Graphics_Disabled_Tooltip_GPU");
	}
#endif

	uiScaleDefault(NULL);
	ui_scale_setting = OptionSettingAddSlider("Basic", "UIScale", 0.5f, 2.f, 1.f, NULL, uiScaleChanged, uiScaleUpdate, uiScaleRevert, NULL, NULL, NULL);
	...
	// Device selection
	{
		...
		FOR_EACH_IN_EARRAY_FORWARDS(devices, const RdrDeviceInfo, device)
		{
			...
			snprintf(buf, _TRUNCATE, "%s #%d - %s", TranslateMessageKey(key), device->monitor_index + 1, device->name);
			eaPush(&options, allocAddCaseSensitiveString(buf));
		}
		FOR_EACH_END;
		if (eaSize(&options) > 2)
		{
			...
			settingDevice = autoSettingsAddComboBox(GRAPHICS_CATEGORY, "Device", &options, false, &adapter_index_plus_one, gfxSettingsChangedCallback, gfxSettingsCommittedCallback, gfxSettingsUpdateCallback, gfxSettingsRestoreDefaultsCallback, NULL, true);
		}
	}
#endif
	...
	if (gConf.bEnableShaderQualitySlider) {
		...
		for (i = 0; i <= SGRAPH_QUALITY_MAX_VALUE; i += SGRAPH_QUALITY_INTERVAL_SIZE)
			eaPush(&options,StaticDefineIntRevLookup(ShaderGraphQualityEnum, i));

		settingMaterialDesiredScore = autoSettingsAddComboBox(GRAPHICS_CATEGORY, "Material_Quality_Maximum", &options, true, &matDesiredScore, gfxSettingsChangedCallback, gfxSettingsCommittedCallback, gfxSettingsUpdateCallback, gfxSettingsRestoreDefaultsCallback, NULL, true);
	}
	...
	gfxSetUIUpdateCB(gfxSettingsNeedUpdate);
}
