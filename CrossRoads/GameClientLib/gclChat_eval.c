...
// Send trade chat to other players in the same zone.
AUTO_COMMAND ACMD_NAME(trade) ACMD_ACCESSLEVEL(0) ACMD_CATEGORY(Chat, Social);
void cmdClientChat_SendTradeChat(Entity *pEnt, const ACMD_SENTENCE msg) {
	ClientChat_SendTradeChat(pEnt, msg);
}

// Send LFG chat to other players in the same zone.
AUTO_COMMAND ACMD_NAME(LookingForGroup) ACMD_ACCESSLEVEL(0) ACMD_CATEGORY(Chat, Social);
void cmdClientChat_SendLFGChat(Entity *pEnt, const ACMD_SENTENCE msg) {
	ClientChat_SendLFGChat(pEnt, msg);
}
