...
// --------------------------------------------------------------------------------------------------------------------
static F32 CombatReactivePower_GetActivationDelay(Character *pChar, CombatReactivePowerInfo *pInfo)
{
	F32 fTimeAdjust = 0.25f;
	
	if (pInfo->iLastQueuedFailReason == ECombatReactiveActivateFailReason_KNOCKED)
		return fTimeAdjust;

	// if we're not currently activating a power and just finished one, 
	// check if the power had a hitframe that was nearly at the end of the activation, 
	// if it was, we need to delay the reactive activation by a short time so we don't get a mispredict on the server
	// who will probably still be in the power activation.
	if (!pChar->pPowActCurrent && pChar->pPowActFinished && pChar->pPowActFinished->fTimeFinished < fTimeAdjust)
	{
		PowerDef *pPowerDef = GET_REF(pChar->pPowActFinished->hdef);
		if (pPowerDef && pPowerDef->eType == kPowerType_Click)
		{
			...
			if (pAnimFX)
			{
				...
				if (fHitTime + fTimeAdjust <= pPowerDef->fTimeActivate)
					return 0.f;

				return fTimeAdjust - pChar->pPowActFinished->fTimeFinished;
			}
		}
	}
...
void gclCombatReactivePower_UpdateNoneState(Character *pChar, CombatReactivePowerInfo *pInfo, CombatReactivePowerDef *pDef)
{
	if(pInfo->uiQueuedActivateTime)
	{
		...
		{
			F32 fActivationDelay = CombatReactivePower_GetActivationDelay(pChar, pInfo);
			if (fActivationDelay)
			{
				pInfo->iLastQueuedFailReason = 0;
				pInfo->uiQueuedActivateTime = pmTimestamp(fActivationDelay);
			}
			else 
			{
...
void gclCombatReactivePower_Activate(Character *pChar)
{
	...
	else
	{
		...
		if (eFail == ECombatReactiveActivateFailReason_NONE || 
			...
			(!pDef->pRoll && (eFail == ECombatReactiveActivateFailReason_KNOCKED || 
								eFail == ECombatReactiveActivateFailReason_DISABLED)))
		{
			pInfo->uiQueuedActivateTime = pmTimestamp(0);
			pInfo->iLastQueuedFailReason = eFail;
			if (pDef->pRoll)
			{
				_GetCurrentInputIfQueuedInputNotSet(pInfo);
			}
...
void gclCombatReactivePower_ForceDeactivate()
{
	Entity* e = entActivePlayerPtr();
	...
	if (e && e->pChar && e->pChar->pCombatReactivePowerInfo)
	{
		CombatReactivePowerDef *pDef = GET_REF(e->pChar->pCombatReactivePowerInfo->hCombatBlockDef);
		if (pDef)
			CombatReactivePower_Stop(e->pChar, e->pChar->pCombatReactivePowerInfo, pDef, pmTimestamp(0), true);
	}
}