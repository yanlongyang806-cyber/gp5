...
#define DEBUGWRITELINE(a) gfxfont_PrintEx(&g_font_Sans, 30, iFirstLine + iPixPerLine*(iCurrentLine++), 0, 1.1, 1.1, 0, a, (int)strlen(a), colors, NULL)
									...
									bool bRecordDepth)
{
	...
	if ( s_iTargetsListSize )
	{
		...
		for (i = 0; i < s_iTargetsListSize; ++i)
		{
			...
			if (gGCLState.bUseFreeCamera || !e)
			{
				pBestEnt = pTargetPriority->pEntity;
				break;
			}
			else if(!gConf.bTargetSelectUnderMouseUsesPowersLOSCheck)
			{
				...
				if(combat_CheckLoS(PARTITION_CLIENT, vCursorStart, vEnd, e, pTargetPriority->pEntity, 0, 0, true, NULL))
				{
					pBestEnt = pTargetPriority->pEntity;
					targetDistanceSQ = lengthVec3Squared(vEnd);
					break;
				}
			}
			else
			{
				...
				if(combat_CheckLoS(PARTITION_CLIENT, vPlayerEnt, vTargetEntPos, e, pTargetPriority->pEntity, NULL, false, true, NULL))
				{
					pBestEnt = pTargetPriority->pEntity;
					targetDistanceSQ = distance3Squared(vCursorStart,pTargetPriority->vCollisionPoint);
					break;
				}
			}
		}
	...
	if (bRecordDepth)
	{
		if (pBestEnt)
		{
			gfxSetTargetEntityDepth(sqrt(targetDistanceSQ));
		} else {
			gfxSetTargetEntityDepth(0);
		}
	}
...
Entity * target_SelectUnderMouse(	Entity *e,
									...
									bool bRecordDepth)
{
	return target_SelectUnderMouseEx(e,NULL,target_type_req,target_type_exc,worldPos,bSelectable,bIsClick,preferInteractables,bRecordDepth);
}
...
void runCmdOnCursor(ACMD_SENTENCE pcmd)
{
	...
	if(e && !mouseIsLocked())
	{
		...
		target = target_SelectUnderMouse(e,0,kTargetType_Self,NULL,false,false, false, false);
...
void cursorModeDefault_OnClickEx(bool bDown, Entity** pTargetOut, WorldInteractionNode** pNodeOut, Vec3 vLocOut)
{
	...
	if (e && !mouseIsLocked())//devassertmsg(e, "No player found (are you outside of gclGameplay?"))
	{
		...
		target = target_SelectUnderMouse(e, iTypeReq, iTypeExclude, vLocOut, true, true, false, false);
...
void targetCursorOrAutoAttack(void )
{
	...
	if (e && !mouseIsLocked())
	{
		target = target_SelectUnderMouse(e,kTargetType_Alive,kTargetType_Self, NULL, true, false, false, false);
		if(target)
		{
	...
	if(pEnt->pChar)
	{
		...
		for(i=eaSize(&pEnt->pChar->ppPowers)-1; i>=0; i--)
		{
			...
			if(pPowerTarget && pPowerTarget->bAllowNearDeath)
			{
				...
				if (bConfineToCursor)
				{
					pEntTarget = target_SelectUnderMouseEx(pEnt, pPowerTarget, 0, 0, NULL, true, false, false, false);
				}
				else
				{
...
static void gclAutoAttack_Enable(void)
{
...

	{
		...
		if (pPlayerEnt && pPlayerEnt->pChar)
			gclCharacter_ForceDismount(pPlayerEnt->pChar, NULL);
	}
...
static void target_debugRaycastMaterials(void)
{
	...
	gfxfont_PrintEx(&g_font_Sans, 30, 800, 0, 1.1, 1.1, 0, cMaterialOutput, (int)strlen(cMaterialOutput), colors, NULL);
}
...
ClientTargetDef *clientTarget_SelectBestTargetForPowerEx(Entity *pEntPlayer, Power *ppow, PowerTarget *pPowTargetOverride, bool *pShouldSetHard)
{
	...
	else
	{
		...
		{
			Entity *pEnt = target_SelectUnderMouseEx(pEntPlayer, ppowTarget, 0, 0, NULL, true, false, false, false);
		...
		else
		{
			...
			else if(gclPlayerGetTargetingAssist() == CombatTargetingAssist_Full)
			{
		...
		else
		{
				...
				&& pchar->eChargeMode == kChargeMode_None)
			{
				...
				for(i=0; i<s; i++)
				{
					...
					{
						Power *pSwitchedPower = CombatPowerStateSwitching_GetSwitchedStatePower(pchar, ppow);
						if (pSwitchedPower)
							ppow = pSwitchedPower;
					}
