											...
											CombatPowerStateDef **pFailedStateOut)
{
	...
	i = iStartIdx;
	do {
		if (CombatPowerStateSwitching_CanEnterState(pChar, pChar->pCombatPowerStateInfo, pDef->eaStates[i], peFailReason))
		{
						...
						bool bNotifyServer)
{
	...
	// exit the current state
	{
		...
		if (pInfo->pchCurrentState)
			pOldState = CombatPowerStateSwitching_GetStateByName(pInfo->pchCurrentState, pDef);

		if (pOldState == pNewState)
		{	// entering the same state, exit now
			return;
		}

		if (pDef->bDismountOnStateSwitch)
		{
			gclCharacter_ForceDismount(pChar, NULL);
		}

		CombatPowerStateSwitching_ExitState(pChar, pDef, pInfo, pOldState, uiTime);
	}

	if (pNewState)
	{
		pInfo->iCurActIdOffset++;
		if (pInfo->iCurActIdOffset > 10)
			pInfo->iCurActIdOffset = 0;
	}

	if (bNotifyServer)
	{
		ServerCmd_gslCombatPowerStateSwitching_ActivateServer((pNewState ? pNewState->pchName : NULL), uiTime, pInfo->iCurActIdOffset);
	}

	CombatPowerStateSwitching_EnterState(pChar, pDef, pInfo, pNewState, uiTime);
...
void gclCombatPowerStateSwitching_CycleNextState(Character *pChar)
{
	...
	if (!CombatPowerStateSwitching_CanStateSwitch(pChar, pInfo, pDef, &eFailReason))
	{
	...
	if (pInfo->fCooldown <= 0.f)
	{
		...
		pNewState = _GetNextMode(pChar, pInfo->pchCurrentState, pDef, &eFailReason, &pFailedState);
		...
		_EnterState(pChar, pInfo, pDef, pNewState, true);
	}
}
...
void gclCombatPowerStateSwitching_EnterStateByName(Character *pChar, const char *pchMode)
{
	if (pChar->pCombatPowerStateInfo)
	{
		...
		if (!CombatPowerStateSwitching_CanStateSwitch(pChar, pChar->pCombatPowerStateInfo, pDef, NULL))
			return;

		pNewState = CombatPowerStateSwitching_GetStateByName(pchMode, pDef);
		
		_EnterState(pChar, pChar->pCombatPowerStateInfo, pDef, pNewState, true);
	}
...
void gclCombatPowerStateSwitching_ExitCurrentState(Character *pChar)
{
	if (pChar->pCombatPowerStateInfo)
	{
		...
		_EnterState(pChar, pChar->pCombatPowerStateInfo, pDef, NULL, true);
	}
											...
											CombatPowerStateDef *pModeDef)
{
	...
	pInfo->uAutoAttackLastPowerID = 0;
	...
	if (gclAutoAttack_IsEnabled())
	{
		...
		{
			pInfo->uAutoAttackLastPowerID = powID;
											...
											CombatPowerStateSwitchingInfo *pInfo)
{
	...
	if (pInfo->uAutoAttackLastPowerID)
	{
		S32 iLastAutoAttackSlot = character_PowerIDSlot(pChar, pInfo->uAutoAttackLastPowerID);
		if (iLastAutoAttackSlot >= 0)
		{
			S32 iActSlot = CombatPowerStateSwitching_GetCorrespondingStatePowerSlot(pChar, 
																			pInfo->pchCurrentState,
																			pInfo->pchLastState, 
																			iLastAutoAttackSlot);
			if (iActSlot >= 0)
			{
...
void gclCombatPowerStateSwitching_ExitState(const char *pchOldState, const char *pchEnterState)
{
	Entity* e = entActivePlayerPtr();
	...
	{
		CombatPowerStateSwitchingDef *pDef = NULL;
		CombatPowerStateSwitchingInfo *pInfo = e->pChar->pCombatPowerStateInfo;
		...
		if (pchEnterState)
		{
			pchEnterState = allocFindString(pchEnterState);
			devassertmsg(pchEnterState, "CombatPowerStateSwitching_CancelState: Invalid state sent from server.");
		}
		...
		if (pInfo->pchCurrentState == pchOldState)
		{

			pMode = CombatPowerStateSwitching_GetStateByName(pInfo->pchCurrentState, pDef);

			CombatPowerStateSwitching_ExitState(e->pChar, pDef, pInfo, pMode, pmTimestamp(0));
		}

		{
			pMode = CombatPowerStateSwitching_GetStateByName(pchEnterState, pDef);
			_EnterState(e->pChar, pInfo, pDef, pMode, false);
		}
