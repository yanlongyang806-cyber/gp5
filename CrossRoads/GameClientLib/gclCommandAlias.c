...
static KeyBindProfile s_AliasProfile = { "Aliases", __FILE__, KeyboardLocale_EnglishUS, NULL, true, true, NULL, &g_AliasList,  InputBindPriorityUser };
...
void gclCommandAliasAddEx(const char *pchAlias, const ACMD_SENTENCE pchCommand, S32 iAccessLevel, bool bHidden, const char *pchHelp)
{
	...
	if (!s_bInit)
	{
		...
		s_bInit = true;
		s_stTable = stashTableCreateWithStringKeys(16, StashDeepCopyKeys_NeverRelease);
	}
