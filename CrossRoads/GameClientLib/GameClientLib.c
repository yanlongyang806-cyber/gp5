...
bool g_GCL_GFX_Shutdown_Always = true;
AUTO_CMD_INT(g_GCL_GFX_Shutdown_Always, GCL_GFX_Shutdown_Always) ACMD_CMDLINE ACMD_ACCESSLEVEL(0);
...
#define CLIENT_PROD_STRINGCACHE_SIZE 1280*1024
#define CLIENT_DEV_STRINGCACHE_SIZE 1280*1024*2
...
void gclMain(HINSTANCE hInstance, LPSTR lpCmdLine, const void *logo_jpg_data, int logo_data_size)
{
	...
	// Fixes ClientBinner GameClient crashes due to corrupt rdrShaderCache.hogg. IF check can be removed once we permanently move gclGfxShutdown
	// from gclBase_Leave to here at the end of gclMain.
...
void gclSetGameCameraActive(void)
{
	...
	gclPlayerControl_UnSuspendMouseLook();
...
void gclSetProductionEdit(int enable)
{
	...
	// Enable skin
	ugcEditorToggleSkin(enable);
	MEFieldSetDisableContextMenu( enable );
...
AUTO_COMMAND ACMD_CMDLINE ACMD_ACCESSLEVEL(0) ACMD_NAME(EnableStereoscopic);
void gclEnableStereoscopic(S32 i)
{
	gGCLState.bEnableStereoscopic = !!i;
}
...
void gclGfxShutdown(void)
{
	if(!gbNoGraphics && gGCLState.pPrimaryDevice) {
