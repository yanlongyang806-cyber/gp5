...
// Gets the contact indicator of the my current contact.
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("ContactGetIndicator");
const char* exprContactGetIndicator(SA_PARAM_OP_VALID Entity *pPlayerEnt)
{
	...
	if (pContactDialog)
	{
		ret = StaticDefineIntRevLookup(ContactIndicatorEnum, pContactDialog->eIndicator);
	}

	return NULL_TO_EMPTY(ret);
}

// Gets the level of the next mission offer from a given entity for the given player
...
void contactUI_ToggleVisibilityOfPlayersIfHeadshotEntExists(SA_PARAM_OP_VALID Entity *pEntity, bool bVisible)
{
	...
	if (pInfo &&
		...
		entFromEntityRefAnyPartition(pInfo->pContactDialog->headshot.headshotEnt))
	{
...
bool contactCutSceneCameraGetPosPyr(F32 timestep, Vec3 cameraPos /*out*/, Vec3 cameraPYR /*out*/, GfxSkyData *sky_data)
{
	...
	if (pCutSceneDef)
	{
		...
		if (pContactDialog)
		{
			Entity * pHeadshotEnt = entFromEntityRefAnyPartition(pContactDialog->headshot.headshotEnt);
...
void exprContactDialogSnapCameraToContactsHead(SA_PARAM_OP_VALID Entity *pEntity)
{
	...
	if ((pEntContact = entFromEntityRefAnyPartition(pInfo->pContactDialog->headshot.headshotEnt)) == NULL && vec3IsZero(pInfo->pContactDialog->vecCameraSourcePos))
	{
		return;
	}
...
void exprClientPlayContactDialogSound(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity)
{
	...
	if(pContactDialog)
	{
		...
		{
			EntityRef entRef = pContactDialog->cameraSourceEnt == 0 ? pContactDialog->headshot.headshotEnt : pContactDialog->cameraSourceEnt;
	...
	if (s_ContactSoundData.pchPreviousSound &&
		...
		sndKillSourceIfPlaying(s_ContactSoundData.pPreviousSource, true); // killing source here should also clear out effects from any prior calls to sndAddContactDialogSource below
	...
	if (!s_ContactSoundData.pchPreviousSound && pContactDialog && pContactDialog->pchSoundToPlay) {
		...
		if (g_ContactConfig.bEnableLipSync) {
			sndAddContactDialogSource(s_ContactSoundData.pPreviousSource);
		}
	}
}
...
// Returns the StickerBookCollection name to show in the StickerBook UIGen when the player clicks Zone Rewwards option on a ZoneRewards-flagged Contact.
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("ContactStickerBookCollection");
const char *exprContactStickerBookCollection(SA_PARAM_OP_VALID Entity *pEntity)
{
	ContactDialog *pDialog = SAFE_MEMBER3(pEntity, pPlayer, pInteractInfo, pContactDialog);
	if(pDialog)
	{
		StickerBookCollection *pStickerBookCollection = GET_REF(pDialog->hStickerBookCollection);
		if(pStickerBookCollection)
			return pStickerBookCollection->pchName;
	}
...
static void ContactImageMenuDialogOptionAddUIDetails(ContactDialogOption* dialogOption)
{
	...
	{
		if(pMissionInfo->waypointList[i]->pchMissionRefString 
		  &&(   pMissionInfo->waypointList[i]->pchDestinationMap && pMissionInfo->waypointList[i]->pchDestinationMap == dialogOption->pchMapName
		    || !pMissionInfo->waypointList[i]->pchDestinationMap && dialogOption->bYouAreHere
			))
		{
			//a mission is here.
...
void exprContactDialogFillCategorizedChoosableRewardList(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pPlayerEnt)
{
	...
	{
		S32 iCount = GetChoosableItemList(pInfo->pContactDialog->eaRewardBags, peaData);
...
bool exprGenRemoteContact_IsRemoteTurnInAvailableForMission(ExprContext *pContext, const char* pchMission)
{
	...
	return (RemoteContact_SetRemoteContactDisplayRowForMission(pPlayerEnt, pchMission, ContactIndicator_MissionCompleted, NULL) ||
			RemoteContact_SetRemoteContactDisplayRowForMission(pPlayerEnt, pchMission, ContactIndicator_MissionRepeatableAvailable, NULL));
}
...
void exprGenRemoteContact_LaunchRemoteTurnInForMission(ExprContext *pContext, const char* pchMission)
{
	...
	if(RemoteContact_SetRemoteContactDisplayRowForMission(pPlayerEnt, pchMission, ContactIndicator_MissionCompleted, pRow) ||
		RemoteContact_SetRemoteContactDisplayRowForMission(pPlayerEnt, pchMission, ContactIndicator_MissionRepeatableAvailable, pRow))
	{
...
int exprGenContactOptionCooldownTimeLeft(SA_PARAM_OP_VALID ContactDialogOption* pOption) {
	if(pOption)
	{
		// Option cooldown (Which is mission cooldown) now uses the ShardClock
		return pOption->uCooldownExpireTime - ShardClock_SecondsSince2000();
	}
...
const char* exprGenContactOptionCooldownTimeLeftString( ExprContext* pContext, SA_PARAM_OP_VALID ContactDialogOption* pOption, ACMD_EXPR_DICT(Message) const char* pchMessageKey) {
	...
	if(pchMessageKey && pchMessageKey[0])
	{
		...
		{
			// Option cooldown (Which is mission cooldown) now uses the ShardClock
			iTimeLeft = pOption->uCooldownExpireTime - ShardClock_SecondsSince2000();
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ContactDialogGetHeadshotCount);
S32 exprContactDialogGetHeadshotCount(void)
{
	Entity *pEntity = entActivePlayerPtr();
	ContactDialog *pDialog = SAFE_MEMBER3(pEntity, pPlayer, pInteractInfo, pContactDialog);
	if (pDialog)
		return 1 + eaSize(&pDialog->eaExtraHeadshots);
	return 0;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ContactDialogGetHeadshotBorderFrame);
const char *exprContactDialogGetHeadshotBorderFrame(S32 iIndex)
{
	Entity *pEntity = entActivePlayerPtr();
	ContactDialog *pDialog;
	ContactDialogCostume *pCostume;

	if (iIndex == 0)
		CONTACT_DIALOG_SIMPLE_GETTER(pEntity, headshot.pchHeadshotBorderFrame, NULL);

	pDialog = SAFE_MEMBER3(pEntity, pPlayer, pInteractInfo, pContactDialog);
	pCostume = pDialog ? eaGet(&pDialog->eaExtraHeadshots, iIndex - 1) : NULL;
	return SAFE_MEMBER(pCostume, pchHeadshotBorderFrame);
}
