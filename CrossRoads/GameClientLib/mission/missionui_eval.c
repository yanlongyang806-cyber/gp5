...
static char *s_estrOpenMissionRewardMessage = NULL;
...
const char* missiondef_GetLogicalCategoryName(const MissionDef *pDef)
{
	if (pDef)
	{
		if (IS_HANDLE_ACTIVE(pDef->hCategory))
...
{
	return missiondef_GetLogicalCategoryName(mission_GetDef(mission));
}
...
bool missiondef_gclHasObjectiveOnCurrentMap(SA_PARAM_OP_VALID Entity* pEnt, MissionDef *pDef) {
	...
	if(pDef->bObjectiveMapsIncludesAllStaticMaps && zmapInfoGetMapType(NULL) == ZMTYPE_STATIC)
		return true;
...
U32 g_MissionObjectiveMinRecursiveDepth = 0;
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MissionObjectiveSetMinRecursionDepth);
void gclExprMissionObjectiveSetMinRecursionDepth(U32 depth)
{
	g_MissionObjectiveMinRecursiveDepth = depth;
}
...
static void mission_AppendObjectiveStringRecursive(Entity *pEnt, Mission *mission, char** estrBuffer, const char *pchRowHeader, const char *pchIndentSpacer, const char *pchObjectiveHeader, const char *pchCompletedHeader, const char *pchFailedHeader, const char *pchSummaryHeader, const char *pchRowFooter, int depth, bool withCollapse, bool includeSummary, bool bExpandCompletedMissions)
{
	...
	if (mission && (mission_HasUIString(mission) || mission->depth <= g_MissionObjectiveMinRecursiveDepth))
	{
...
char *exprGetMissionDetailString(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Mission *mission)
{
	...
	if (def) {
		...
		missiondef_GetDetails(def, pEnt, &detailStringMsg);
		missionsystem_ClientFormatMessagePtr("MissionUI", pEnt, def, player_GetPrimaryNemesisID(pEnt), &estrBuffer, GET_REF(detailStringMsg.hMessage));
	}
...
char *exprGetCompletedMissionDetailString(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID CompletedMission *pCompletedMission)
{
	...
	if (def) {
		DisplayMessage detailStringMsg;
		missiondef_GetDetails(def, pEnt, &detailStringMsg);
		missionsystem_ClientFormatMessagePtr("MissionUI", pEnt, def, iNemesisID, &estrBuffer, GET_REF(detailStringMsg.hMessage));
	}
...
char *exprGetMissionSummary(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Mission *mission)
{
	...
	if (def) {
		...
		missiondef_GetSummary(def, pEnt, &summaryMsg);
		missionsystem_ClientFormatMessagePtr("MissionUI", pEnt, def, player_GetPrimaryNemesisID(pEnt), &estrBuffer, GET_REF(summaryMsg.hMessage));
	}
...
char *exprGetCompletedMissionSummary(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID CompletedMission *pCompletedMission)
{
	...
	if (def) {
		DisplayMessage summaryMsg;
		missiondef_GetSummary(def, pEnt, &summaryMsg);
		missionsystem_ClientFormatMessagePtr("MissionUI", pEnt, def, iNemesisID, &estrBuffer, GET_REF(summaryMsg.hMessage));
	}
		...
		if(pCompleted)
		{
			...
			if(pMissionDef)
			{
				...
				FormatGameMessageKey(&estrBuffer,
					...
					STRFMT_INT("MaxCount", pMissionDef->uRepeatCooldownCount),
					STRFMT_INT("CountLeft", pMissionDef->uRepeatCooldownCount - missionInfo->uRepeatCount),
...
static void AddMissionRequestToQueue(const char ***peachQueue, Entity* pEnt, MissionInfo *pMissionInfo, SA_PARAM_OP_STR const char* pchMissionDefs)
{
	...
	if (peachQueue == &s_ppchUpdateQueue)
	{
		...
		U32 bCurrent = pData && CachedMissionIsCurrent(pEnt, pData) ? true : false;
		U32 bCachedCurrent = pData && pData->bCurrent ? true : false;
		if (!pData || 
			(!pData->bUpdate && 
				(pData->eState != eState ||
					pData->uTime + PERIODIC_PROGRESSION_MISSION_INFO_REQUEST_TIME <= uCurrentTime || 
					bCachedCurrent != bCurrent)))
		{
...
void PopulateMissionJournalRowStrings(ExprContext *pContext, SA_PARAM_NN_VALID MissionJournalRow *pRow, SA_PARAM_OP_VALID Entity *pEntity)
{
	...
	if(pDef)
	{
		...
		missiondef_GetSummaryAndDetails(pDef, pEntity, &summaryMsg, &detailStringMsg);
		...
		if(!pRow->estrDescription || !pRow->estrDescription[0])
		{
			...
			{
				missionsystem_ClientFormatMessagePtr("MissionUI", pEntity, pDef, idNemesis,
					&pRow->estrDescription, GET_REF(summaryMsg.hMessage));
			}
			else
			{
				missionsystem_ClientFormatMessagePtr("MissionUI", pEntity, pDef, idNemesis,
		...
		if(!pRow->estrDetail || !*pRow->estrDetail)
		{
			...
			missionsystem_ClientFormatMessagePtr("MissionUI", pEntity, pDef, idNemesis,
				&pRow->estrDetail, GET_REF(detailStringMsg.hMessage));
		}
	}
}
...
// bUseLogicalName switches the behavior between comparing pchCategoryName to the mission category's TRANSLATED name (if false) and the mission category's LOGICAL name (if true)
void exprFillSelectedPerksListEx(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEntity, SA_PARAM_OP_STR const char *pchCategoryName, S32 bShowCompleted, S32 bShowInProgress, bool bUseLogicalName, int (*comparitorFunc)(const MissionJournalRow**, const MissionJournalRow**))
{
	MissionInfo *pInfo = mission_GetInfoFromPlayer(pEntity);
	MissionJournalRow*** peaMissionRowList = ui_GenGetManagedListSafe(pGen, MissionJournalRow);
	int i, numValidRows = 0;
	...
	if (pInfo)
	{
		for (i = 0; i < eaSize(&pInfo->missions); i++)
		{
			...
			const char *pchDefCategory = bUseLogicalName ? missiondef_GetLogicalCategoryName(pDef) : missiondef_GetTranslatedCategoryName(pDef);
...
{
	exprFillSelectedPerksListEx(pContext, pGen, pEntity, pchCategoryName, bShowCompleted, bShowInProgress, false, missionjournalrow_SortByDisplayName);
}

AUTO_EXPR_FUNC(entityutil) ACMD_NAME("FillPerksListByLogicalName");
void exprFillSelectedPerksListByLogicalName(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEntity, SA_PARAM_OP_STR const char *pchLogicalCategoryName, S32 bShowCompleted, S32 bShowInProgress)
{
	exprFillSelectedPerksListEx(pContext, pGen, pEntity, pchLogicalCategoryName, bShowCompleted, bShowInProgress, true, missionjournalrow_SortByDisplayName);
}
...
void exprFillSelectedPerksListSorted(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEntity, SA_PARAM_OP_STR const char *pchCategoryName, S32 bShowCompleted, S32 bShowInProgress, const char* pchSortMethod)
{
	...
	if (comparitorFunc)
		exprFillSelectedPerksListEx(pContext, pGen, pEntity, pchCategoryName, bShowCompleted, bShowInProgress, false, comparitorFunc);
}

// Fill the gen's model with completed and uncompleted Perks.
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("FillSortedPerksListByLogicalName");
void exprFillSelectedPerksListSortedByLogicalName(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEntity, SA_PARAM_OP_STR const char *pchLogicalCategoryName, S32 bShowCompleted, S32 bShowInProgress, const char* pchSortMethod)
{
	int (*comparitorFunc)(const MissionJournalRow**, const MissionJournalRow**);
	if (stricmp("DisplayName", pchSortMethod) == 0)
		comparitorFunc = missionjournalrow_SortByDisplayName;
	...
	if (comparitorFunc)
		exprFillSelectedPerksListEx(pContext, pGen, pEntity, pchLogicalCategoryName, bShowCompleted, bShowInProgress, true, comparitorFunc);
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GetOpenMissionRewardMessage");
const char *exprGetOpenMissionRewardMessage(SA_PARAM_OP_VALID Entity* pEnt)
{
	return s_estrOpenMissionRewardMessage;
}
...
void exprClearOpenMissionRewardList(ExprContext *pContext, SA_PARAM_OP_VALID Entity* pEnt)
{
	...
	estrDestroy(&s_estrOpenMissionRewardMessage);
}
...
AUTO_COMMAND ACMD_CLIENTCMD ACMD_PRIVATE ACMD_ACCESSLEVEL(0);
void SetOpenMissionRewardMessage(const char *pchRewardMessage)
{
	estrCopy2(&s_estrOpenMissionRewardMessage, pchRewardMessage ? pchRewardMessage : "");
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("OpenMissionEventHasTag");
bool exprOpenMissionEventHasTag(SA_PARAM_OP_VALID Entity *pPlayerEnt, SA_PARAM_OP_VALID OpenMission *pOpenMission, SA_PARAM_NN_STR const char *pchDisplayTag)
{
	if (pPlayerEnt && pPlayerEnt->pPlayer)
	{
		MissionInfo *pMissionInfo = mission_GetInfoFromPlayer(pPlayerEnt);
		...
		if (pMissionInfo)
		{
			...
			if (pLeaderboardInfo && pOpenMission && pOpenMission->pMission)
			{
				...
				if (pDef)
				{
					...
					if(pEventDef)
					{
						U32 uDisplayTag = StaticDefineIntGetInt(ActivityDisplayTagsEnum, pchDisplayTag);
						return (ea32Find(&pEventDef->uDisplayTags, uDisplayTag) != -1);
					}
				}
			}
		}
...
{
	const MissionCooldownInfo *pMissionInfo = mission_GetCooldownInfo(pEnt, pMission->missionNameOrig);
	if (pMissionInfo!=NULL)
	{
		if (pMissionInfo->bIsInCooldown)
		{
			return(pMissionInfo->uCooldownSecondsLeft);
		}
	}
	return(0);
}
