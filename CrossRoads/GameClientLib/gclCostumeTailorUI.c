...
// Get the active costume index based off a costume storage type and pet number
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_GetActiveCostumeSlotFromPet");
int CostumeCreator_GetActiveCostumeSlotFromPet(/* PCCostumeStorageType */ int eStorageType, int iPetNum)
{
	Entity *pStoreEnt = CostumeCreator_GetStoreCostumeEntityFromPet(eStorageType, iPetNum, 0);
	return SAFE_MEMBER2(pStoreEnt, pSaved, costumeData.iActiveCostume);
}

// Get the active costume index based off a costume storage type and container id
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_GetActiveCostumeSlotFromContainer");
int CostumeCreator_GetActiveCostumeSlotFromContainer(/* PCCostumeStorageType */ int eStorageType, U32 uContainerID)
{
	Entity *pStoreEnt = CostumeCreator_GetStoreCostumeEntityFromContainer(eStorageType, uContainerID, 0);
	return SAFE_MEMBER2(pStoreEnt, pSaved, costumeData.iActiveCostume);
}
...
bool CostumeCreator_CanPlayerChangeCostume(SA_PARAM_OP_VALID Entity *pEnt)
{
	...
	return pCostume && !pCostume->bPlayerCantChange;
}

enum {
	GET_COSTUME_SLOTS_SHOW_ENTITY_HEADERS	= 1,
	GET_COSTUME_SLOTS_SHOW_ACTIVE			= 2,
	GET_COSTUME_SLOTS_COUNT_ACTIVE			= 4,
	GET_COSTUME_SLOTS_SHOW_INACTIVE			= 8,
	GET_COSTUME_SLOTS_COUNT_INACTIVE		= 16,
	GET_COSTUME_SLOTS_SHOW_LOCKED			= 32,
	GET_COSTUME_SLOTS_COUNT_LOCKED			= 64,
};
...
static S32 CostumeCreator_GetCostumeSlots(UICostumeSlot ***peaSlots, PCCostumeStorageType eStorageType, S32 iPetNum, U32 uFlags)
{
	...
	if (eStorageType < 65536)
	{
		...
		else if (iPetNum < 0)
		{
			...
			bEntityHeaders = (uFlags & GET_COSTUME_SLOTS_SHOW_ENTITY_HEADERS) != 0;
		}
	}
	else
	{
	...
	for (iEnt = 0; iEnt < eaSize(&s_eaFillEntities); iEnt++)
	{
		...
		// Determine how many slots to fill
		if (uFlags & (GET_COSTUME_SLOTS_COUNT_INACTIVE | GET_COSTUME_SLOTS_SHOW_INACTIVE))
			iCostumeSlots = costumeEntity_trh_GetNumCostumeSlots(CONTAINER_NOCONST(Entity, pEnt), CONTAINER_NOCONST(GameAccountData, entity_GetGameAccount(pEnt)));
		...
		for (iSlotIndex = 0; iSlotIndex < iCostumeSlots; iSlotIndex++)
		{
			...
			bool bLocked = false;
			UICostumeSlot *pUISlot;

			if (pSlot && pSlot->pCostume)
			{
				if (uFlags & GET_COSTUME_SLOTS_COUNT_ACTIVE)
					iTotalCount++;
				if ((uFlags & GET_COSTUME_SLOTS_SHOW_ACTIVE) == 0)
					continue;

				bLocked = pSlot->pCostume->bPlayerCantChange;
			}
			else
			{
				// Ignore hidden slots
				if (costumeEntity_IsSlotHidden(pPlayerEnt, pEnt, pSlotDef))
					continue;

				bLocked = !costumeEntity_IsSlotUnlocked(pPlayerEnt, pEnt, pSlotDef);
				if (bLocked)
				{
					if (uFlags & GET_COSTUME_SLOTS_COUNT_LOCKED)
						iTotalCount++;					
					if ((uFlags & GET_COSTUME_SLOTS_SHOW_LOCKED) == 0)
						continue;
				}
				else
				{
					if (uFlags & GET_COSTUME_SLOTS_COUNT_INACTIVE)
						iTotalCount++;
					if ((uFlags & GET_COSTUME_SLOTS_SHOW_INACTIVE) == 0)
						continue;
				}
			}

			pUISlot = peaSlots ? eaGetStruct(peaSlots, parse_UICostumeSlot, iSlotCount++) : NULL;
			if (pUISlot)
			{
				// Determine slot type
...
void CostumeCreator_GetPlayerCostumeSlotList(SA_PARAM_NN_VALID UIGen *pGen, /* PCCostumeStorageType */ int eCostumeType, int iPetNum)
{
	...
	COSTUME_UI_TRACE_FUNC();
	CostumeCreator_GetCostumeSlots(peaCostumeSlots, eCostumeType, iPetNum, GET_COSTUME_SLOTS_SHOW_ENTITY_HEADERS | GET_COSTUME_SLOTS_SHOW_ACTIVE | GET_COSTUME_SLOTS_COUNT_ACTIVE | GET_COSTUME_SLOTS_COUNT_LOCKED | GET_COSTUME_SLOTS_COUNT_LOCKED | GET_COSTUME_SLOTS_SHOW_INACTIVE);
...
void CostumeCreator_GetPlayerActiveCostumeSlotList(SA_PARAM_NN_VALID UIGen *pGen, /* PCCostumeStorageType */ int eCostumeType, int iPetNum)
{
	UICostumeSlot ***peaCostumeSlots = ui_GenGetManagedListSafe(pGen, UICostumeSlot);
	COSTUME_UI_TRACE_FUNC();
	CostumeCreator_GetCostumeSlots(peaCostumeSlots, eCostumeType, iPetNum, GET_COSTUME_SLOTS_SHOW_ENTITY_HEADERS | GET_COSTUME_SLOTS_SHOW_ACTIVE | GET_COSTUME_SLOTS_COUNT_ACTIVE);
	ui_GenSetManagedListSafe(pGen, peaCostumeSlots, UICostumeSlot, true);
}
...
{
	COSTUME_UI_TRACE_FUNC();
	return CostumeCreator_GetCostumeSlots(NULL, eCostumeType, iPetNum, GET_COSTUME_SLOTS_SHOW_ENTITY_HEADERS | GET_COSTUME_SLOTS_SHOW_ACTIVE | GET_COSTUME_SLOTS_COUNT_ACTIVE | GET_COSTUME_SLOTS_COUNT_LOCKED | GET_COSTUME_SLOTS_COUNT_LOCKED | GET_COSTUME_SLOTS_SHOW_INACTIVE);
}
...
{
	COSTUME_UI_TRACE_FUNC();
	return CostumeCreator_GetCostumeSlots(NULL, eCostumeType, iPetNum, GET_COSTUME_SLOTS_SHOW_ENTITY_HEADERS | GET_COSTUME_SLOTS_SHOW_ACTIVE | GET_COSTUME_SLOTS_COUNT_ACTIVE);
}

// StarTrek
// Get the list of player costumes available
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_GetPlayerSelectedCostumeSlotList");
void CostumeCreator_GetPlayerSelectedCostumeSlotList(SA_PARAM_NN_VALID UIGen *pGen, /* PCCostumeStorageType */ int eCostumeType, int iPetNum, U32 uFlags)
{
	UICostumeSlot ***peaCostumeSlots = ui_GenGetManagedListSafe(pGen, UICostumeSlot);
	COSTUME_UI_TRACE_FUNC();
	CostumeCreator_GetCostumeSlots(peaCostumeSlots, eCostumeType, iPetNum, uFlags);
	ui_GenSetManagedListSafe(pGen, peaCostumeSlots, UICostumeSlot, true);
}

// StarTrek
// Get the size of the list of empty costumes available
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_GetPlayerSelectedCostumeSlotListSize");
int CostumeCreator_GetPlayerSelectedCostumeSlotListSize(/* PCCostumeStorageType */ int eCostumeType, int iPetNum, U32 uFlags)
{
	COSTUME_UI_TRACE_FUNC();
	return CostumeCreator_GetCostumeSlots(NULL, eCostumeType, iPetNum, uFlags);
}
...
// Start replacing (or creating) a costume from the current state
// Night, Bronze, FightClub, StarTrek
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_StartReplaceCostume");
void CostumeCreator_StartReplaceCostumeExpr(S32 iCostumeIdx)
{
	CostumeCreator_StartReplaceCostume(iCostumeIdx);
}
...
void CostumeCreator_CopyCostumeFromEnt(PCCostumeStorageType eStorageType, Entity *pEnt, S32 iIndex)
{
	...
	// STO only code in next if block
	if(!pCostume && pEnt->myEntityType != GLOBALTYPE_ENTITYPLAYER && pPlayerEnt)
	{
		// STO the puppet chosen doesn't have a costume which is the case for the captain. Use the players first costume
		if(pPlayerEnt && pPlayerEnt->pSaved && eaSize(&pPlayerEnt->pSaved->costumeData.eaCostumeSlots) > 0)
		{
			// choose the active costume for this player
			// the slot type and slot defs should be fine

			if(iIndex < eaSize(&pPlayerEnt->pSaved->costumeData.eaCostumeSlots))
			{
				PlayerCostume *pTempCostume = pPlayerEnt->pSaved->costumeData.eaCostumeSlots[iIndex]->pCostume;
				if(pTempCostume)
				{
					pCostume = StructCloneDeConst(parse_PlayerCostume, pTempCostume);
				}
			}

			// try the first costume
			if(!pCostume && pPlayerEnt->pSaved->costumeData.eaCostumeSlots[0]->pCostume)
			{
				pCostume = StructCloneDeConst(parse_PlayerCostume, pPlayerEnt->pSaved->costumeData.eaCostumeSlots[0]->pCostume);
			}
		}
	}
...
bool CostumeCreator_BeginCostumeEditing(NOCONST(PlayerCostume) *pCostume)
{
	...
	if (pCostume && pCostume->eCostumeType != kPCCostumeType_Player)
	{
		...
		costumeTailor_FillAllBones(pCostume, GET_REF(g_CostumeEditState.hSpecies), g_CostumeEditState.eaPowerFXBones, g_CostumeEditState.pSlotType, true, false, true);
		costumeTailor_MakeCostumeValid(pCostume, GET_REF(g_CostumeEditState.hSpecies), g_CostumeEditState.eaUnlockedCostumes, g_CostumeEditState.pSlotType, true, g_CostumeEditState.bUnlockAll, false, guild_GetGuild(pEnt), false, pExtract, false, g_CostumeEditState.eaPowerFXBones);
		costumeTailor_FillAllBones(pCostume, GET_REF(g_CostumeEditState.hSpecies), g_CostumeEditState.eaPowerFXBones, g_CostumeEditState.pSlotType, true, false, true);
...
void CostumeCreator_StartReplaceCostume(S32 iCostumeIndex)
{
	COSTUME_UI_TRACE_FUNC();
	if (CostumeCreator_BeginCostumeEditing(g_CostumeEditState.pCostume))
	{
		...
		if (g_CostumeEditState.iCostumeIndex != iCostumeIndex && pEnt)
		{
			PCSlotDef *pSlotDef = NULL;
			PCSlotSet *pSlotSet = NULL;

			costumeEntity_GetStoreCostumeSlot(pPlayerEnt, pEnt, iCostumeIndex, &pSlotDef, NULL);
			pSlotSet = costumeEntity_GetSlotSet(pEnt, CostumeCreator_EntIsPet(pPlayerEnt, pEnt));

			// Inherit slot type from current costume, use SlotDef from new slot
			// slot type will be fixed by the later CostumeUI_RegenCostume if it's
			// not valid in the new SlotDef.
			g_CostumeEditState.pSlotDef = pSlotDef;
			g_CostumeEditState.pcSlotSet = SAFE_MEMBER(pSlotSet, pcName);
			g_CostumeEditState.iSlotID = SAFE_MEMBER(pSlotDef, iSlotID);
			g_CostumeEditState.bExtraSlot = SAFE_MEMBER(pSlotSet, pExtraSlotDef) == pSlotDef;
			...
			g_CostumeEditState.iCostumeIndex = iCostumeIndex;
		}
