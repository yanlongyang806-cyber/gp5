...
void gclCamera_UseNearOffset(bool bEnable)
{
	...
	if (pCamera)
	{
		gclCamera_GetSettings(pCamera)->bUseNearOffset = g_CurrentScheme.bEnableFirstPersonCamera ? false : bEnable;
	}
}
...
static void gclCamera_UpdateHeightMultiplier(Entity* pEnt, GfxCameraController* pCamera, F32 fElapsedTime)
{
	...
	else if(pEnt->costumeRef.pMountCostume)
	{
		if(!fMountHeight)
		{
			...
			DynSkeleton *pMountSkeleton = dynSkeletonFromGuid(pEnt->dyn.guidSkeleton);
			DynSkeleton *pRiderSkeleton = dynSkeletonGetRider(pMountSkeleton);
			...
			if (pRiderSkeleton) {
				fMountHeight = MAX(pMountSkeleton->vCurrentExtentsMax[1], pRiderSkeleton->vCurrentExtentsMax[1]) - pMountSkeleton->vCurrentExtentsMin[1] + pMountSkeleton->vCurrentExtentsMax[2] / 2;
			} else {
				fMountHeight = pMountSkeleton->vCurrentExtentsMax[1] - pMountSkeleton->vCurrentExtentsMin[1] + pMountSkeleton->vCurrentExtentsMax[2] / 2;
			}
		}
...
static F32 gclCamera_GetClosestDistance(CameraSettings *pSettings)
{
	...
	if(pSettings->bIsInInspectMode || g_CurrentScheme.bEnableFirstPersonCamera)
		fDistance = 0;
...
void mouseBufferAddSample(MouseBuffer * pBuffer,Vec2 const vVal,F32 fTime)
{
	...
	devassert(pBuffer->aValues[iSlot].fTimeRemaining == 0.0f);
	pBuffer->aValues[iSlot].fTimeRemaining = 0.0f;
...
static void gclShooterCameraFunc(GfxCameraController *pCamera, GfxCameraView *pCameraView, F32 fElapsedTime, F32 fRealElapsedTime)
{
	...
	if (pSettings->bIsInAimMode)
	{
		if (!pShooterSettings->bAimModeInit)
		{
			...
			pSettings->fDistance = g_CurrentScheme.bEnableFirstPersonCamera ? pShooterSettings->fSavedDistance : pShooterSettings->fAimDist;
	...
	{
		F32 fShooterMinDist = g_CurrentScheme.bEnableFirstPersonCamera ? 0 : pShooterSettings->fMinDist;
		F32 fMinDist = fShooterMinDist * pSettings->fZoomMult;
		F32 fMaxDist = fMinDist + (pShooterSettings->fMaxDist - fShooterMinDist);
