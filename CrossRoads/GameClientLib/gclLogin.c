...
bool gbAllowVersionMismatch = true;
...
static char *s_chosenCharacterName = NULL;
...
static void
ClearChosenCharacterName(void)
{
    if ( s_chosenCharacterName )
    {
        free(s_chosenCharacterName);
        s_chosenCharacterName = NULL;
    }
}

static void
SetChosenCharacterName(const char *characterName)
{
    ClearChosenCharacterName();
    s_chosenCharacterName = strdup(characterName);
}
...
AUTO_COMMAND ACMD_NAME(AuthTicketNew) ACMD_CLIENTCMD ACMD_CMDLINEORPUBLIC ACMD_ACCESSLEVEL(0) ACMD_HIDE;
...
static void gclLoginCheckLoading(F32 fFrameTime)
{
	...
	{
		if (!gfxLoadingIsStillLoadingEx(0.25, 5, 0, false) || GSM_IsStateActive(GCL_LOGIN_USER_CHOOSING_CHARACTER))
		{
...
static int cmpLastPlayedCharacterChoices(const Login2CharacterChoice** ppA, const Login2CharacterChoice** ppB)
{
	...
	int retval = pB->isOffline - pB->isOffline;
	if (retval)
		return retval;
	else 
...
void gclLoginHandleInput(Packet* pak, int cmd, NetLink* link,void *user_data)
{
	...
	PACKET_CASE(LOGIN2_TO_CLIENT_CHARACTER_SELECTION_DATA, pak);
		{
			...
			if (GET_REF(g_characterSelectionData->hGameAccountData))
			{
    ...
    PACKET_CASE(LOGIN2_TO_CLIENT_CHARACTER_DETAIL, pak);
    {
		...
		if(g_characterSelectionData && g_characterSelectionData->characterChoices && characterDetail && characterDetail->playerEnt)
		{
			S32 i;
			for(i = 0; i < eaSize(&g_characterSelectionData->characterChoices->characterChoices); ++i)
			{
				if(characterDetail->playerEnt->myContainerID == g_characterSelectionData->characterChoices->characterChoices[i]->containerID)
				{
					// if we have detail we are online
					g_characterSelectionData->characterChoices->characterChoices[i]->isOffline = false;
				}
			}
		}
    }
    ...
    PACKET_CASE(LOGIN2_TO_CLIENT_LOGIN_FAILED, pak);
    {
        if (!GSM_IsStateActive(GCL_LOGIN_FAILED))
        {
            ...
            translatedError = pktGetStringTemp(pak);
            gclLoginFail(translatedError);
        }
    }
    PACKET_CASE_END(pak);

	PACKET_CASE(LOGIN2_TO_CLIENT_SUPPORTED_LANGUAGES, pak);
	{
		const int* eaPreviousSupportedLanguages = langGetSupportedLanguagesThisShard();
		int* eaSupportedLanguages = NULL;
		int numSupportedLanguages = pktGetU32( pak );
		int it;

		for( it = 0; it < numSupportedLanguages; ++it ) {
			Language lang = pktGetU32( pak );
			eaiPush( &eaSupportedLanguages, lang );
		}

		assert( eaPreviousSupportedLanguages == NULL
				|| eaiCompare( &eaSupportedLanguages, &eaPreviousSupportedLanguages ) == 0);
...
void gclLogin_Enter(void)
{
	...
	sndSetIsLoginCallback(gclSndIsLoginCB);
...
void gclLogin_Leave(void)
{
	...
	sndSetIsLoginCallback(NULL);
}


//--------------stuff for GCL_ACCOUNT_SERVER_LOGIN

static void gclAccountLogin(const char *pchUser, const char *pchPassword, bool bEditLogin, AccountLoginType eLoginType)
{
	...
	else
	{
		...
		strcpy(gGCLState.loginField, pchUser);
...
{
	gclAccountLogin(pchUser, pchPassword, bEditLogin, ACCOUNTLOGINTYPE_Default);
}
...
{
	gclAccountLogin(pchUser, pchPassword, sbForceEditMode, ACCOUNTLOGINTYPE_CrypticAndPW);
}
...
{
	gclAccountLogin(pchUser, pchPassword, sbForceEditMode, ACCOUNTLOGINTYPE_PerfectWorld);
}
...
{
	gclAccountLogin(pchUser, pchPassword, sbForceEditMode, ACCOUNTLOGINTYPE_Cryptic);
}
...
void gclAccountServerLogin_Enter(void)
{
	...
	// Clear any character selection data if it exists
	StructDestroySafe(parse_Login2CharacterSelectionData, &g_characterSelectionData);
	...
	if (gbClickThroughLoginProcess)
	{
		if (gGCLState.loginName[0])
		{
			gclAccountLogin(gGCLState.loginName, "", false, gGCLState.eLoginType);
		}
		else
		{
...
bool gclLoginChooseCharacter(Login2CharacterChoice *chosenCharacter)
{
    ...
    SetChosenCharacterName(s_chosenCharacter->savedName);
...
bool gclLoginCreateCharacter(Login2CharacterCreationData *characterCreationData)
{
    ...
    SetChosenCharacterName(s_characterCreationData->name);
...
// Make a purchase using numeric for a gad key value
void gclLoginRequestGADPurchase(ContainerID playerID, const char *gadPurchaseDef)
{
	Packet *pPak;

	pPak = pktCreate(gpLoginLink, TOLOGIN_LOGIN2_GAD_PURCHASE);
	pktSendU32(pPak, playerID);
	pktSendString(pPak, gadPurchaseDef);
	pktSend(&pPak);
}
...
// Generate the filename for a localized version of the video of the form:
//  basename-XX.bik, where XX is the short language code.
static char *LocalizeVideoFilename(const char *basenameIn)
{
    static char *filename = NULL;
    const char *langCode = locGetCrypticSpecific2LetterIdentifier(getCurrentLocale());
    char *extension;
    char *basename = strdup(basenameIn);

    extension = strstri(basename, ".bik");
    if ( extension == NULL )
    {
        return NULL;
    }
    ...
    *extension = '\0';
    ...
    estrPrintf(&filename, "%s-%s.bik", basename, langCode);
...
static void gclLoginCreateCharacter_Enter(void)
{
    ...
    {
        char *localizedFilename = LocalizeVideoFilename(gConf.pchCharCreateVideoPath);
        const char *videoFilename = gConf.pchCharCreateVideoPath;

        // Will play a fil
        if ( localizedFilename && fileExists(localizedFilename) )
        {
            videoFilename = localizedFilename;
        }
        gclPlayVideo_Start(videoFilename, 0, true);
    }
}
...
// Find a character that can make a purchase, if they all fail then make sure best one is in cache
bool gclLogin_GameAccountCanMakeNumericPurchaseWithAnyCharacter(GameAccountDataNumericPurchaseDef* pDef)
{
    ...
    if (pDef && g_characterSelectionData)
	{
        ...
        for ( i = eaSize(&g_characterSelectionData->characterChoices->characterChoices) - 1; i >= 0; i-- )
        {
			...
			if (!characterChoice->virtualShardID && !characterChoice->isOffline)
			{
				if (GAD_Login2CanMakeNumericPurchase(gclLogin_GetNumericValueFromCharacterDetail(characterChoice, pDef), characterChoice->virtualShardID, NULL, pDef, false))
				{
					return true;
				}
			}
		...
		if(timeSecondsSince2000() > s_uTm)
		{
			// There are characters but none succeeded, are they in the cache? if they have the needed numeric and no the get them into the cache
			for ( i = eaSize(&g_characterSelectionData->characterChoices->characterChoices) - 1; i >= 0; i-- )
			{
				Login2CharacterChoice *characterChoice = g_characterSelectionData->characterChoices->characterChoices[i];

				if (!characterChoice->virtualShardID && !characterChoice->isOffline && gclLogin2_CharacterDetailCache_Get(characterChoice->containerID) == NULL)
				{
					const char *pcVal = gclLogin_GetExtraHeaderForPurchaseDef(characterChoice, pDef);
					if(pcVal != NULL)
					{
						if(GAD_Login2CanMakeNumericPurchase(atoi(pcVal), characterChoice->virtualShardID, NULL, pDef, false))
						{
							// this character might have what we need, get details for it
							gclLogin2_CharacterDetailCache_Fetch(characterChoice->containerID, NULL, NULL);
							break;
						}
					}
				}
			}
			s_uTm = timeSecondsSince2000() + 5; // 5 seconds between trying to see if there is a character we can fetch
		}
	}
...
static int SortByShardOrder(void *context, const Login2ShardStatus **shardStatusA, const Login2ShardStatus **shardStatusB)
{
    U32 orderA = (*shardStatusA)->displayOrder;
    U32 orderB = (*shardStatusB)->displayOrder;

    if ( orderA == orderB )
    {
        return strcmp((*shardStatusA)->shardName, (*shardStatusB)->shardName);
    }
...
void gclLogin_ExprLoginGetClusterNormalShards(SA_PARAM_NN_VALID UIGen *pGen)
{
    ...
    if ( s_ShardStatusVersion != s_ClusterStatusVersion )
    {
        ...
        if ( s_ClusterStatus )
        {
            ...
            eaQSort_s(s_ShardStatusList, SortByShardOrder, NULL);
        }
