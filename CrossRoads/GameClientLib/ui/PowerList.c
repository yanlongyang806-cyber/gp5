...
typedef struct PowerListWarpData
{
	...
	const char* pchPowerDisplayName;		AST(NAME(PowerDisplayName) UNOWNED)
												...
												SA_PARAM_NN_VALID const char *pchMessageKey)
{
	...
	if(pEntity && pEntity->pChar && pPower)
	{
		...
		if(pDef)
		{
			...
			char *pchTemp = NULL;
			power_AutoDesc(entGetPartitionIdx(pEntity),langGetCurrent(),pPower,pEntity->pChar,&pchTemp,NULL,"<br>","<bsp><bsp>","- ",false,0,entGetPowerAutoDescDetail(pEntity,false), pExtract,NULL);
			...
			if (pPowerDef && pPowerDef->bHasWarpAttrib)
			{
				...
				if (pWarpModDef)
				{
					...
					if (!pData)
					{
						...
						pData->pchPowerDisplayName = TranslateDisplayMessage(pPowerDef->msgDisplayName);
				...
				if (pPower->pSourceItem)
				{
					...
					if (!pSlot || !pSlot->pItem || pSlot->pItem->id != pPower->pSourceItem->id)
					{
						...
						if (inv_trh_FindItemByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), pPower->pSourceItem->id, pIter))
						{
