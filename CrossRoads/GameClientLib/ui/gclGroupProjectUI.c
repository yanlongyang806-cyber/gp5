...
GroupProjectState *gclGroupProject_ResolveState(GroupProjectType eProjectType, const char *pchGroupName)
{
	...
	if (!pGroupProjectDef)
	{
		...
		{
			char *pchCopy;
			strdup_alloca(pchCopy, pchGroupName);
			*strchr(pchGroupName, ',') = '\0';
			pGroupProjectDef = RefSystem_ReferentFromString("GroupProjectDef", pchCopy);
		}
	}
...
int gclGroupProject_DonationTask_SortCategoryName(const DonationTaskUI **ppTaskL, const DonationTaskUI **ppTaskR, const DonationTaskFilter *pContext)
{
	...
	// Low numbers go first
	if ((*ppTaskL)->iUISortOrder != (*ppTaskR)->iUISortOrder)
		return (*ppTaskL)->iUISortOrder - (*ppTaskR)->iUISortOrder;
...
void gclGroupProjectUI_UpdateTask(GroupProjectDef *pGroupProject, DonationTaskUI *pDonationUI, DonationTaskDef *pDef, DonationTaskSlot *pSlot, bool bAvailable)
{
	...
	pDonationUI->iUISortOrder = pDef->iUISortOrder;
	pDonationUI->iUIMaxRepeatCount = pDef->iUIMaxRepeatCount;
	pDonationUI->bAvailable = bAvailable;
	...
	pDonationUI->bCancelable = pDef->cancelable;

	if (pSlot)
	{
		GroupProject_UpdateBucketQuantities(gclGroupProject_ResolveContainer(pGroupProject->type), gclGroupProject_ResolveState(pGroupProject->type, pGroupProject->name), pGroupProject, pDef, pSlot, &pDonationUI->iCurrentBucketQuantity, &pDonationUI->iTotalBucketQuantity);
		...
		xcase DonationTaskState_Finalized:
			{
				pDonationUI->iRemainingCompletionTime = pSlot->completionTime >= timeServerSecondsSince2000() ? pSlot->completionTime - timeServerSecondsSince2000() : 0;
...
void gclGroupProjectUI_UpdateTaskCategoryHeader(GroupProjectDef *pGroupProject, DonationTaskUI *pDonationUI, DonationTaskCategoryType eCategory)
{
	...
	pDonationUI->iUISortOrder = 0;
	pDonationUI->iUIMaxRepeatCount = 0;
	pDonationUI->bAvailable = false;
...
void gclGroupProject_UpdateBucket(Entity *pEnt, DonationTaskBucketUI *pBucketUI, GroupProjectState *pState, DonationTaskSlot *pSlot, DonationTaskDef *pTask, GroupProjectDonationRequirement *pTaskBucket, DonationTaskBucketData *pFilledBucket)
{
	...
	pBucketUI->iRequiredQuantity = GroupProject_GetDiscountedTaskBucketRequirement((pGroupProject ? gclGroupProject_ResolveContainer(pGroupProject->type) : NULL), pState, pTask, pTaskBucket->name);
			...
			if (bRefreshSlow && pEnt && pEnt->pInventoryV2)
			{
				...
				for (i = eaSize(&pEnt->pInventoryV2->ppInventoryBags) - 1; i >= 0; i--)
				{
					...
					for (j = eaSize(&pBag->ppIndexedInventorySlots) - 1; j >= 0; j--)
					{
                        ...
                        if ( !DonationTask_ItemMatchesExpressionRequirement(entGetPartitionIdx(pEnt), pEnt, pTaskBucket, pInvSlot->pItem) )
							continue;
...
S32 gclGroupProjectUI_ApplyBonus(GroupProjectContainer *pContainer, GroupProjectNumericDef *pNumericDef, GroupProjectConstant *pConstant)
{
	GroupProjectBonusDef *pBonusDef;
	GroupProjectNumericDef *pBonusNumericDef;
	F32 iBonusPercent = 0;
	F32 iBonusAmount = 0;
	S32 iAmount = 0;
	S32 i;
	...
	for ( i = 0; i < eaSize(&pContainer->bonusList); i++ )
	{
		pBonusDef = GET_REF(pContainer->bonusList[i]->bonusDef);
		if ( pBonusDef )
		{
			pBonusNumericDef = GET_REF(pBonusDef->numericDef);
			if ( pBonusNumericDef == pNumericDef )
			{
				switch ( pBonusDef->bonusType )
				{
				case BonusType_Percentage:
					iBonusPercent += pBonusDef->value;
					break;

				case BonusType_Absolute:
					iBonusAmount += pBonusDef->value;
					break;
				}
	...
	if (bFoundBonus)
	{
		iAmount += iAmount * iBonusPercent;
		iAmount += iBonusAmount;
	}
	...
	xcase DonationTaskRewardType_NumericAdd:
		{
			...
			{
				GroupProjectConstant *pConstant = pGroupProject ? GroupProject_FindConstant(pGroupProject, pReward->rewardConstant) : NULL;
				GroupProjectContainer *pContainer = gclGroupProject_ResolveContainer(pGroupProject->type);
				S32 iValue = gclGroupProjectUI_ApplyBonus(pContainer, pNumeric, pConstant);

				gclGroupProjectUI_UpdateNumeric(pRewardUI->pNumeric, pNumeric, iValue, pState);
			}
...
{
	static DonationTaskCategoryType *peCategories = NULL;
	...
	if (pGroupProjectDef)
	{
		for (i = 0; i < eaSize(&pGroupProjectDef->donationTaskDefs); i++)
		{
			...
			if (GroupProject_TaskFilterCheck(entGetPartitionIdx(pEnt), pTask, pEnt, pState, pFilter, slotNum, &bAvailable, &peCategories))
			{
				...
				gclGroupProjectUI_UpdateTask(pGroupProjectDef, pTaskUI, pTask, NULL, bAvailable);
			}
		}
	}
	...
	{
		DonationTaskUI *pTaskUI = NULL;
		DonationTaskCategoryType eCategory = peCategories[i];

		// Attempt to keep the task list instances stable
		for (j = iCount; j < eaSize(peaTaskUI); j++)
		{
			if (!IS_HANDLE_ACTIVE((*peaTaskUI)[j]->hDonationTask) && (*peaTaskUI)[j]->eCategory == eCategory)
			{
				if (j != iCount)
					eaSwap(peaTaskUI, j, iCount);
				pTaskUI = (*peaTaskUI)[iCount++];
		...
		gclGroupProjectUI_UpdateTaskCategoryHeader(pGroupProjectDef, pTaskUI, eCategory);
	}
	...
	if (pState)
	{
		...
		for (i = 0; i < (U32)eaiSize(&pGroupProjectDef->slotTypes); i++)
		{
			...
			if (pSlot && GET_REF(pSlot->taskDef))
				gclGroupProjectUI_UpdateTask(pGroupProjectDef, pSlotUI->pActiveTask, GET_REF(pSlot->taskDef), pSlot, false);
			...
			if (pSlot && GET_REF(pSlot->nextTaskDef))
				gclGroupProjectUI_UpdateTask(pGroupProjectDef, pSlotUI->pNextTask, GET_REF(pSlot->nextTaskDef), NULL, false);
			...
			for (j = eaSize(&pGroupProjectDef->donationTaskDefs) - 1; j >= 0; j--)
			{
				...
				xcase GroupProjectType_Guild:
					if (!GuildProject_DonationTaskAllowed(PARTITION_CLIENT, pPlayer, pState, pTask, i))
						continue;
				xcase GroupProjectType_Player:
					if (!PlayerProject_DonationTaskAllowed(PARTITION_CLIENT, pPlayer, pState, pTask, i))
						continue;
				}
...
void gclGroupProjectExprGetTaskList(SA_PARAM_NN_VALID UIGen *pGen, U32 eProjectType, const char *pchGroupName, S32 eSlotType, int slotNum)
{
	...
	Filter.eSlotType = eSlotType;
	if (!gclGroupProject_ResolveContainer(eProjectType))
		Filter.bRequireAvailableForNewProject = true;
	gclGroupProjectUI_GetTaskList(pGen, pState, &Filter, slotNum);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetTaskListIncludeUnavailable");
void gclGroupProjectExprGetTaskListIncludeUnavailable(SA_PARAM_NN_VALID UIGen *pGen, U32 eProjectType, const char *pchGroupName, S32 eSlotType, int slotNum)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	DonationTaskFilter Filter = {0};
	Filter.eSlotType = eSlotType;
	Filter.bIncludeUnavailableTasks = true;
	gclGroupProjectUI_GetTaskList(pGen, pState, &Filter, slotNum);
}
...
void gclGroupProjectExprGetMaskedCategorySortedTaskList(SA_PARAM_NN_VALID UIGen *pGen, U32 eProjectType, const char *pchGroupName, S32 eSlotType, int slotNum, U32 uCategoryMask)
{
	...
	eaiStackCreate(&Filter.peCategoryMask, 32);
	for (i = 0; i < 32; i++)
	{
		if (uCategoryMask & (1 << i))
			eaiPush(&Filter.peCategoryMask, i);
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetMaskedCategorySortedTaskListEx");
void gclGroupProjectExprGetMaskedCategorySortedTaskListEx(SA_PARAM_NN_VALID UIGen *pGen, U32 eProjectType, const char *pchGroupName, S32 eSlotType, int slotNum, const char *pchCategoryMask)
{
	static DonationTaskCategoryType *s_peCategoryMask = NULL;
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	DonationTaskFilter Filter = {0};
	Filter.eSlotType = eSlotType;
	Filter.bSortByCategories = true;
	...
	eaiClearFast(&s_peCategoryMask);
	if (pchCategoryMask && *pchCategoryMask)
	{
		const char *pchDelims = " \r\n\t,%|";
		char *pchMaskCopy, *pchContext, *pchToken;
		strdup_alloca(pchMaskCopy, pchCategoryMask);
		if (pchToken = strtok_r(pchMaskCopy, pchDelims, &pchContext))
		{
			do 
			{
				DonationTaskCategoryType eType = StaticDefineIntGetInt(DonationTaskCategoryTypeEnum, pchToken);
				if (eType != -1)
					eaiPushUnique(&s_peCategoryMask, eType);
			} while (pchToken = strtok_r(NULL, pchDelims, &pchContext));
		}
	}
	Filter.peCategoryMask = s_peCategoryMask;
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetDisplayName");
SA_RET_NN_STR const char * gclGroupProjectExprGetDisplayName(GroupProjectType eProjectType, const char *pchGroupName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	GroupProjectDef *pGroupProject = pState ? GET_REF(pState->projectDef) : RefSystem_ReferentFromString("GroupProjectDef", pchGroupName);
	...
	return pGroupProject ? TranslateDisplayMessage(pGroupProject->displayNameMsg) : "";
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetFirstActiveSlot");
S32 gclGroupProjectExprGetFirstActiveSlot(GroupProjectType eProjectType, const char *pchGroupName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	U32 iBest = UINT_MAX;
	S32 i;
...
// This will return 0 if the pState is NULL or pState->taskSlots is empty
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetFirstAvailableSlotForTask");
S32 gclGroupProjectExprGetFirstAvailableSlotForTask(U32 eProjectType, const char *pchGroupName, const char *pchTaskName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	DonationTaskDef *pTask = RefSystem_ReferentFromString("DonationTaskDef", pchTaskName);
	U32 iBest = UINT_MAX;
	...
	if (pState)
	{
		for (i = eaSize(&pState->taskSlots) - 1; i >= 0; i--)
		{
			if ( !IS_HANDLE_ACTIVE(pState->taskSlots[i]->taskDef) && (pState->taskSlots[i]->taskSlotType == pTask->slotType) && (pState->taskSlots[i]->taskSlotNum < iBest) )
				iBest = pState->taskSlots[i]->taskSlotNum;
		}
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectIsAnyTaskSlotInState");
bool gclGroupProjectExprIsAnyTaskSlotInState(U32 eProjectType, const char *pchGroupName, DonationTaskState eSlotState)
{
	int i,j;
	if( pchGroupName && strchr(pchGroupName, '*') )
	{
		...
		for (i = eaSize(&pContainer->projectList) - 1; i >= 0; i--)
		{
			...
			for (j = 0; j < eaSize(&pState->taskSlots); j++)
			{
				DonationTaskSlot *pSlot = pState->taskSlots[j];
				if( pSlot->state == eSlotState )
					return true;
			}
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectIsTaskAlreadyCompleted");
bool gclGroupProjectExprIsTaskAlreadyCompleted(U32 eProjectType, const char *pchGroupName, const char *pchTaskName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	if ( pState )
	{
		return ( eaIndexedGetUsingString(&pState->completedTasks, pchTaskName) != NULL );
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectSetTaskAndDonateAllItems");
bool gclGroupProjectExprSetTaskAndDonateAllItems(U32 eProjectType, const char *pchGroupName, S32 iSlot, const char *pchDonationTask)
{
    // start the task if there is no active task, queue the task if there is an active task
    ...
    case GroupProjectType_Player:
        ServerCmd_gslPlayerProject_SetTaskAndDonateAllItems(pchGroupName, pchDonationTask, iSlot);
        return true;
    }
...
S32 gclGroupProjectExprTaskRewardQuantity(SA_PARAM_OP_VALID DonationTaskBucketUI *pBucket, S32 iQuantity)
{
	...
	pConstant = GroupProject_FindConstant(pGroupProject, pTaskBucket->contributionConstant);
	if (!pConstant)
		return 0;
...
// Return -1 if the task isn't active, or 0+ as the slotnum
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetActiveTaskSlotIndex");
S32 gclGroupProjectExprGetActiveTaskSlotIndex(U32 eProjectType, const char *pchGroupName, const char *pchTaskName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	DonationTaskDef *pTask = RefSystem_ReferentFromString("DonationTaskDef", pchTaskName);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectSetGenDataToActiveTask");
void gclGroupProjectExprSetGenDataToActiveTask(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, U32 eProjectType, const char *pchGroupName, const char *pchTaskName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	DonationTaskDef *pTask = RefSystem_ReferentFromString("DonationTaskDef", pchTaskName);
	...
	if (pState && pTask)
	{
		...
		for (i = eaSize(&pState->taskSlots) - 1; i >= 0; i--)
		{
			if (GET_REF(pState->taskSlots[i]->taskDef) == pTask)
			{
				DonationTaskUI *pTaskUI = ui_GenGetManagedPointer(pGen, parse_DonationTaskUI);
				bool bAvailable = false;
				GroupProjectDef *pGroupProjectDef = GET_REF(pState->projectDef);
				Entity *pEnt = entActivePlayerPtr();
				...
				if( pGroupProjectDef )
				{
					...
					{
					xcase GroupProjectType_Guild:
						bAvailable = GuildProject_DonationTaskAllowed(entGetPartitionIdx(pEnt), pEnt, pState, pTask, i);
					xcase GroupProjectType_Player:
						bAvailable = PlayerProject_DonationTaskAllowed(entGetPartitionIdx(pEnt), pEnt, pState, pTask, i);
					}
				}
				
				// Because UpdateTask updates the DonationTaskUI's time fields, make sure to call it every frame.
				gclGroupProjectUI_UpdateTask(GET_REF(pState->projectDef), pTaskUI, pTask, pState->taskSlots[i], bAvailable);
				
				ui_GenSetManagedPointer(pGen, pTaskUI, parse_DonationTaskUI, true);
				return;
			}
		}
	}

	ui_GenSetManagedPointer(pGen, NULL, parse_DonationTaskUI, true);	
}
...
S32 gclGroupProjectExprGetNumericValue(U32 eProjectType, const char *pchGroupName, const char *pchNumericName)
{
	...
	if (pNumeric || pchNumericName && strchr(pchNumericName, '*'))
	{
		GroupProjectContainer *pContainer = gclGroupProject_ResolveContainer(eProjectType);
		S32 i, j, iTotal = 0;
		if (!pContainer)
	...
	else
	{
		...
		return pNumericUI ? pNumericUI->iCurrentValue : 0;
	}
}


AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetNumericMaximum");
S32 gclGroupProjectExprGetNumericMaximum(U32 eProjectType, const char *pchGroupName, const char *pchNumericName)
{
	GroupProjectNumericDef *pNumeric = RefSystem_ReferentFromString("GroupProjectNumericDef", pchNumericName);
	...
	if (pNumeric || pchNumericName && strchr(pchNumericName, '*'))
	{
		...
		if (pchGroupName && strchr(pchGroupName, '*'))
		{
			for (i = eaSize(&pContainer->projectList) - 1; i >= 0; i--)
			{
				...
				for (j = eaSize(&pGroupProject->validNumerics) - 1; j >= 0; j--)
				{
					GroupProjectNumericDef *pDataDef = GET_REF(pGroupProject->validNumerics[j]->numericDef);
					if (pNumeric && pDataDef == pNumeric || !pNumeric && pDataDef && isWildcardMatch(pchNumericName, pDataDef->name, false, true))
						iTotal += pDataDef->maxValue;
				}
			}
		}
		else
		{
			GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
			if (pState && GET_REF(pState->projectDef))
			{
				GroupProjectDef *pGroupProject = GET_REF(pState->projectDef);
				for (j = eaSize(&pGroupProject->validNumerics) - 1; j >= 0; j--)
				{
	...
	else
	{
		GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
		GroupProjectNumericUI *pNumericUI = gclGroupProjectUI_GetNumericValue(NULL, pState, pNumeric);
		return pNumericUI ? pNumericUI->iMaximumValue : 0;
	}
}

// Takes a comma-separated list of numerics (with wildcards), returns the sum of GetNumericValue/GetNumericMaximum for each, but scaled so that each numeric is given the same weight TOTAL.
// For example, if you complete:
//    1/3 + 0/1 + 0/3 + 0/1 = 1/12
//    3/3 + 0/1 + 0/3 + 0/1 = 1/4
//    3/3 + 1/1 + 0/3 + 0/1 = 2/4
//    3/3 + 1/1 + 3/3 + 0/1 = 3/4
//    3/3 + 1/1 + 2/3 + 0/1 = 8/12
//    3/3 + 1/1 + 3/3 + 1/1 = 4/4
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectGetNumericProgress");
F32 gclGroupProjectExprGetNumericProgress(U32 eProjectType, const char *pchGroupName, const char *pchNumericNames)
{
	float fScaledSum = 0.0f;
	float fNumNumerics = 0.0f;
	char *pchBuffer, *pchContext, *pchToken;

	strdup_alloca(pchBuffer, pchNumericNames);
	if (pchToken = strtok_r(pchBuffer, " ,", &pchContext))
	{
		do
		{
			S32 iMax = gclGroupProjectExprGetNumericMaximum(eProjectType, pchGroupName, pchToken);
			if( iMax != 0 )
			{
				S32 iValue = gclGroupProjectExprGetNumericValue(eProjectType, pchGroupName, pchToken);
				fScaledSum += ((float)iValue)/((float)iMax);
				fNumNumerics += 1.0f;
			}
		} while (pchToken = strtok_r(NULL, " ,", &pchContext));
	}

	return (fNumNumerics) ? (fScaledSum/fNumNumerics) : 0;
}
...
const char *gclGroupProjectExprFindItem(U32 eProjectType, const char *pchGroupName, const char *pchNumericPattern)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	GroupProjectDef *pGroupProject = pState ? GET_REF(pState->projectDef) : NULL;
	ItemDef *pItemDef = GroupProject_FindItemDef(pGroupProject, pchNumericPattern);
	return SAFE_MEMBER(pItemDef, pchName);
}

// Note: this doesn't handle active tasks very well, since it will check against the total requirement
// of the bucket, instead of the remaining requirement.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectTaskCanFillAllBucketsFromBag");
bool gclGroupProjectExprCanFillAllBucketsFromBag(U32 eProjectType, const char *pchGroupName, const char *pchTaskName, InvBagIDs iBagID)
{
	DonationTaskDef *pTask = RefSystem_ReferentFromString("DonationTaskDef", pchTaskName);
	if (pTask)
	{
		...
		for(i=0; i<eaSize(&pTask->buckets); ++i)
		{
			...
			{
				ItemDef *pItemDef = GET_REF(pTaskBucket->requiredItem);
				int iCurrentQuantity = 0; //SAFE_MEMBER(pFilledBucket, donationCount); // TODO
				int iRequiredQuantity = GroupProject_GetDiscountedTaskBucketRequirement((pGroupProject ? gclGroupProject_ResolveContainer(pGroupProject->type) : NULL), pState, pTask, pTaskBucket->name);
				int iInventoryQuantity = (pItemDef->eType == kItemType_Numeric) ? inv_GetNumericItemValue(pEnt, pItemDef->pchName) : inv_ent_CountItems(pEnt, iBagID, pItemDef->pchName, pExtract);
				
				if( iInventoryQuantity + iCurrentQuantity < iRequiredQuantity )
					return false;
			}
		}
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectHasRemoteContact");
bool gclGroupProjectExprHasRemoteContact(U32 eProjectType, const char *pchGroupName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	GroupProjectDef *pGroupProjectDef = pState ? GET_REF(pState->projectDef) : NULL;
	return pGroupProjectDef && eaSize(&pGroupProjectDef->remoteContacts) > 0;
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectHasPowerTrees");
bool gclGroupProjectExprHasPowerTrees(U32 eProjectType, const char *pchGroupName)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	GroupProjectDef *pGroupProjectDef = pState ? GET_REF(pState->projectDef) : NULL;
	return pGroupProjectDef && eaSize(&pGroupProjectDef->powerTrees) > 0;
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectCancelNextTask");
void gclGroupProjectExprCancelNextTask(U32 eProjectType, const char *pchGroupName, S32 iSlot)
{
	GroupProjectState *pState = gclGroupProject_ResolveState(eProjectType, pchGroupName);
	if (pState && eaIndexedGetUsingInt(&pState->taskSlots, iSlot))
		ServerCmd_gslPlayerProject_CancelNextProject(pchGroupName, iSlot);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GroupProjectHasBonus");
bool gclGroupProjectExprHasBonus(U32 eProjectType)
{
	GroupProjectContainer *pContainer = gclGroupProject_ResolveContainer(eProjectType);
	if (!pContainer)
	{
	...
	return (eaSize(&pContainer->bonusList) > 0);
}
...
void GroupProjectUIStartup(void)
{
	...
	ui_GenInitStaticDefineVars(DonationTaskStateEnum, "DonationTaskState");
