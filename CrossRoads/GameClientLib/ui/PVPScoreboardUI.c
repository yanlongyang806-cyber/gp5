...
void exprGetScoreboardDomPoints(SA_PARAM_NN_VALID UIGen *pGen, F32 width, F32 height)
{
	...
	{
		ui_GenSetManagedListSafe(pGen,NULL, DOMControlPoint, true);
	}
}
		...
		if(pState)
		{
			...
			for (i = 0; i < eaSize(&eaScores); ++i)
			{
				...
				if(pScore && pState && pState->pPlayerValueData)
				{
					...
					// WOLF[8Aug2013] We are getting crashes on live becaue pPlayerValues is NULL. we have a score but are not
					//  represented in pState->pPlayerValueData->eaPlayerValues. For now, patch over the crash with a NULL check,
					//  and we need to investigate why this inconsistent state has happened.
					if (pPlayerValues!=NULL)
					{
...
NOCONST(InventoryBag) **g_eaRewardBags = NULL;

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(Scoreboard_GetRewardList);
void exprScoreboard_GetRewardList(SA_PARAM_NN_VALID UIGen *pGen)
{
	static InventorySlot **eaSlots = NULL;
	S32 i;

	if (eaSize(&eaSlots))
	{
		eaClearStruct(&eaSlots, parse_InventorySlot);
	}

	for (i = 0; i < eaSize(&g_eaRewardBags); ++i)
	{
		S32 j;

		for (j = 0; j < eaSize(&g_eaRewardBags[i]->ppIndexedInventorySlots); ++ j)
		{
			InventorySlot *pSlot = StructCloneFromNoConst(parse_InventorySlot, g_eaRewardBags[i]->ppIndexedInventorySlots[j]);

			eaPush(&eaSlots, pSlot);
		}
	}

	if(g_eaRewardBags && eaSize(&eaSlots))
		ui_GenSetManagedListSafe(pGen, (InventorySlot***)&eaSlots, InventorySlot, false);
	else
...
void cmd_setReward(InventoryBagGroup *pBags)
{
	if (pBags)
	{
		NOCONST(InventoryBag) **eaNewBags = NULL;
		eaCopyStructsDeConst(&pBags->eaBags, &eaNewBags, parse_InventoryBag);
		eaPushEArray(&g_eaRewardBags, &eaNewBags);
	}
}
...
void exprScoreboard_ClearReward()
{
	if(g_eaRewardBags)
	{
		eaDestroyStructNoConst(&g_eaRewardBags, parse_InventoryBag);
	}
}
