...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_ResetEditLinesByHeader");
void CostumeCreator_ResetEditLinesByHeader( SA_PARAM_OP_VALID CostumeEditLine *el )
{
	CostumeEditLine **eaModify = NULL;
	PlayerCostume *pStartCostume = CONTAINER_RECONST(PlayerCostume, g_CostumeEditState.pStartCostume);
	NOCONST(PlayerCostume) *pTargetCostume = g_CostumeEditState.pCostume;
	Entity *pEnt = CostumeCreator_GetEditPlayerEntity();
	int iStart, iEnd;

	COSTUME_UI_TRACE_FUNC();

	if (!pTargetCostume || !pStartCostume)
		return;
	if (el && el->iType != kCostumeEditLineType_Region && el->iType != kCostumeEditLineType_Divider)
		return;

	iStart = eaFind(&g_CostumeEditState.eaCostumeEditLine, el);
	if (iStart < 0)
		return;

	for (iEnd = iStart + 1; iEnd < eaSize(&g_CostumeEditState.eaCostumeEditLine); iEnd++)
	{
		if (g_CostumeEditState.eaCostumeEditLine[iEnd]->iType == kCostumeEditLineType_Region ||
			g_CostumeEditState.eaCostumeEditLine[iEnd]->iType == kCostumeEditLineType_Divider)
		{
			break;
		}
		eaPush(&eaModify, g_CostumeEditState.eaCostumeEditLine[iEnd]);
	}

	costumeLineUI_SetCostumeEditLinesGeneral(pStartCostume, pTargetCostume, !el ? g_CostumeEditState.eaCostumeEditLine : eaModify,
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_ResetEditLines");
void costumeLineUI_ResetEditLines( void )
{
	CostumeCreator_ResetEditLinesByHeader(NULL);
}
