...
bool gclCostumeEditListExpr_AddRandomPreset(const char *pcList, U32 uSeed, const char *pchSpecies, const char *pchSlotType, const char *pchPresetGroups, U32 uFlags)
{
	...
	do
	{
		...
		for (i = 0; i < eaSize(&eapchGroups); i++)
		{
			...
			CostumeCreator_ApplyPresetOverlay(pRandomCostume, eaGroupPresets[iRandomPreset], false, false);
...
SA_RET_OP_VALID PaperdollHeadshotData *gclCostumeEditListExpr_GetPaperdollCostume(ExprContext *pContext, const char *pcList, S32 iIndex)
{
	...
	return gclPaperdoll_CreateHeadshotData(pContext, NULL, pCostume, NULL, NULL, NULL, NULL, false, false);
}
...
void gclCostumeEditListExpr_AddBackgroundCostume(const char *pcList, S32 iIndex, const char *pchCostumeName)
{
	...
	if (pCostume)
	{
		...
		if (!pViewCostume && g_pCostumeView)
		{
		...
		costumeView_RegenViewCostume(pViewCostume, pCostume, NULL, NULL, NULL, NULL, NULL);
	}
}
...
static int s_bCostumeTailor_RefreshPresets;
AUTO_CMD_INT(s_bCostumeTailor_RefreshPresets, CostumeTailor_RefreshPresets) ACMD_ACCESSLEVEL(9) ACMD_CATEGORY(Costume);
...
void gclCostumeEditListExpr_AddCostumeTailorPresets(const char *pcList, const char *pcSkeleton)
{
	...
	CostumeSourceList *pSourceList;

	if (strcspn(pcSkeleton, "?*") != strlen(pcSkeleton))
		pcSkeleton = "*";

	FOR_EACH_IN_REFDICT("SpeciesDef", SpeciesDef, pSpecies);
	{
		if (pSpecies->eRestriction & (kPCRestriction_Player | kPCRestriction_Player_Initial))
			eaPush(&eaSpecies, pSpecies);
	}
	FOR_EACH_END;
	...
	FOR_EACH_IN_REFDICT("PCCostumeSet", PCCostumeSet, pSet);
	{
		...
		for (i = 0; i < eaSize(&pSet->eaPlayerCostumes); i++)
		{
			...
			if (!CostumeEditList_GetCostume(pcList, pchCostumeName) || s_bCostumeTailor_RefreshPresets)
			{
				CostumeSource *pSource = CostumeEditList_AddNamedCostumeRef(pcList, pchCostumeName, pchCostumeName);
				NOCONST(PlayerCostume) *pCostumeCopy = StructCloneDeConst(parse_PlayerCostume, pCostume);
				PCGeometryDef **eaGeos = NULL;
				PCMaterialDef **eaMats = NULL;
				PCTextureDef **eaTexs = NULL;
				S32 iPart, iProduct, iProductID, iSpecies;

				SET_HANDLE_FROM_REFERENT("PCCostumeSet", pSet, pSource->hCostumeSetRef);
				eaCopy(&pSource->eapchSlotTypes, &pSet->eaPlayerCostumes[i]->eapcSlotTypes);
				pSource->fOrder = pSet->eaPlayerCostumes[i]->fOrder;

				// Test against a player costume
				...
				// Determine which species can validly use this preset
				for (iSpecies = eaSize(&eaSpecies) - 1; iSpecies >= 0; iSpecies--)
				{
					SpeciesDef *pSpecies = eaSpecies[iSpecies];

					if (!REF_COMPARE_HANDLES(pSpecies->hSkeleton, pCostumeCopy->hSkeleton))
						continue;

					// Validate geos on parts
					for (iPart = eaSize(&pCostumeCopy->eaParts) - 1; iPart >= 0; iPart--)
					{
						PCSkeletonDef *pSkeleton = GET_REF(pCostumeCopy->hSkeleton);
						NOCONST(PCPart) *pPart = pCostumeCopy->eaParts[iPart];
						PCBoneDef *pBone = GET_REF(pPart->hBoneDef);
						PCRegion *pRegion = pBone ? GET_REF(pBone->hRegion) : NULL;
						PCCategory *pCategory = pRegion ? costumeTailor_GetCategoryForRegion(CONTAINER_RECONST(PlayerCostume, pCostumeCopy), pRegion) : NULL;
						...
						if (!pSkeleton || !pBone || !pRegion || !pCategory || !pGeo || !pMat)
							break;

						if (stricmp(pGeo->pcName, "None"))
						{
							costumeTailor_GetValidGeos(pCostumeCopy, pSkeleton, pBone, pCategory, pSpecies, NULL, &eaGeos, false, false, false, true);
							if (eaFind(&eaGeos, pGeo) < 0)
								break;
						}

						if (pMat && stricmp(pMat->pcName, "None"))
						{
							costumeTailor_GetValidMaterials(pCostumeCopy, pGeo, pSpecies, NULL, NULL, NULL, &eaMats, false, false, true);
							if (eaFind(&eaMats, pMat) < 0)
								break;
						}

						if (pPattern && stricmp(pPattern->pcName, "None"))
						{
							costumeTailor_GetValidTextures(pCostumeCopy, pMat, pSpecies, NULL, NULL, pGeo, NULL, NULL, kPCTextureType_Pattern, &eaTexs, false, false, true);
							if (eaFind(&eaTexs, pPattern) < 0)
								break;
						}

						if (pDetail && stricmp(pDetail->pcName, "None"))
						{
							costumeTailor_GetValidTextures(pCostumeCopy, pMat, pSpecies, NULL, NULL, pGeo, NULL, NULL, kPCTextureType_Detail, &eaTexs, false, false, true);
							if (eaFind(&eaTexs, pDetail) < 0)
								break;
						}

						if (pDiffuse && stricmp(pDiffuse->pcName, "None"))
						{
							costumeTailor_GetValidTextures(pCostumeCopy, pMat, pSpecies, NULL, NULL, pGeo, NULL, NULL, kPCTextureType_Diffuse, &eaTexs, false, false, true);
							if (eaFind(&eaTexs, pDiffuse) < 0)
								break;
						}

						if (pSpecular && stricmp(pSpecular->pcName, "None"))
						{
							costumeTailor_GetValidTextures(pCostumeCopy, pMat, pSpecies, NULL, NULL, pGeo, NULL, NULL, kPCTextureType_Specular, &eaTexs, false, false, true);
							if (eaFind(&eaTexs, pSpecular) < 0)
								break;
						}

						if (pMovable && stricmp(pMovable->pcName, "None"))
						{
							costumeTailor_GetValidTextures(pCostumeCopy, pMat, pSpecies, NULL, NULL, pGeo, NULL, NULL, kPCTextureType_Movable, &eaTexs, false, false, true);
							if (eaFind(&eaTexs, pMovable) < 0)
								break;
						}
					}

					// If no parts were rejected, then it's a valid species
					if (iPart < 0)
					{
						SpeciesDefRef *pRef = StructCreate(parse_SpeciesDefRef);
						SET_HANDLE_FROM_REFERENT("SpeciesDef", eaSpecies[iSpecies], pRef->hSpecies);
						eaPush(&pSource->eaValidSpecies, pRef);
					}
				}
	...
	// Sort list of costumes by display name and order
	pSourceList = CostumeEditList_GetSourceList(pcList, false);
	if (pSourceList)
		eaQSort(pSourceList->eaCostumes, SortByDisplayName);

	s_bCostumeTailor_RefreshPresets = false;
...
static int SortByDisplayName(const CostumeSource **ppLeft, const CostumeSource **ppRight)
{
	if (!nearf((*ppLeft)->fOrder, (*ppRight)->fOrder))
		return (*ppLeft)->fOrder - (*ppRight)->fOrder;
	return stricmp_safe((*ppLeft)->pchDisplayName, (*ppRight)->pchDisplayName);
}

static bool gclCostumeEditList_IsOwned(PCRestriction eRestriction, const char *pcName, StashTable stashUnlockData)
{
	UnlockMetaData *pUnlock = NULL;

	if (eRestriction & kPCRestriction_Player_Initial)
		return true;

	if (!stashFindPointer(stashUnlockData, pcName, &pUnlock))
		return false;

	if (!pUnlock)
		return false;

	return pUnlock->bOwned;
}

static void gclCostumeEditList_GetCostumes(CostumeSource ***peaCostumes, const char *pcList, const char *pcSkeleton, const char *pcSpecies, const char *pcSlotType)
{
	CostumeSourceList *pList = CostumeEditList_GetSourceList(pcList, false);
	S32 i, j;
	...
	if (!pcSkeleton || strcspn(pcSkeleton, "?*") != strlen(pcSkeleton))
		pcSkeleton = "*";

	if (!pcSpecies || strcspn(pcSpecies, "?*") != strlen(pcSpecies))
		pcSpecies = "*";

	if (!pcSlotType || strcspn(pcSlotType, "?*") != strlen(pcSlotType))
		pcSlotType = "*";
	...
	if (pList)
	{
		for (i = 0; i < eaSize(&pList->eaCostumes); i++)
		{
			...
			if (stricmp(pcSkeleton, "*"))
			{
				if (!pSkeleton || !isWildcardMatch(pcSkeleton, pSkeleton->pcName, false, true))
					continue;
			}

			if (stricmp(pcSpecies, "*"))
			{
				for (j = eaSize(&pSource->eaValidSpecies) - 1; j >= 0; j--)
				{
					SpeciesDef *pSpecies = GET_REF(pSource->eaValidSpecies[j]->hSpecies);
					if (pSpecies && isWildcardMatch(pcSpecies, pSpecies->pcName, false, true))
						break;
				}
				if (j < 0)
					continue;
			...
			if (stricmp(pcSlotType, "*"))
			{
				for (j = eaSize(&pSource->eapchSlotTypes) - 1; j >= 0; j--)
				{
					if (pSource->eapchSlotTypes[j] && isWildcardMatch(pcSlotType, pSource->eapchSlotTypes[j], false, true))
						break;
				}
				if (j < 0)
					continue;
			...
			if (g_bHideUnownedCostumes)
			{
				for (j = eaSize(&pCostume->eaParts) - 1; j >= 0; j--)
				{
					...
					PCGeometryDef *pGeo = GET_REF(pPart->hGeoDef);
					PCMaterialDef *pMat = GET_REF(pPart->hMatDef);
					PCTextureDef *apTex[5] = {
						GET_REF(pPart->hPatternTexture),
						GET_REF(pPart->hDetailTexture),
						GET_REF(pPart->hSpecularTexture),
						GET_REF(pPart->hDiffuseTexture),
						SAFE_GET_REF2(pPart, pMovableTexture, hMovableTexture)
					};

					if (pGeo ? !gclCostumeEditList_IsOwned(pGeo->eRestriction, pGeo->pcName, g_CostumeEditState.stashGeoUnlockMeta) : IS_HANDLE_ACTIVE(pPart->hGeoDef))
						break;
					if (pMat ? !gclCostumeEditList_IsOwned(pMat->eRestriction, pMat->pcName, g_CostumeEditState.stashMatUnlockMeta) : IS_HANDLE_ACTIVE(pPart->hMatDef))
						break;
					if (apTex[0] ? !gclCostumeEditList_IsOwned(apTex[0]->eRestriction, apTex[0]->pcName, g_CostumeEditState.stashTexUnlockMeta) : IS_HANDLE_ACTIVE(pPart->hPatternTexture))
						break;
					if (apTex[1] ? !gclCostumeEditList_IsOwned(apTex[1]->eRestriction, apTex[1]->pcName, g_CostumeEditState.stashTexUnlockMeta) : IS_HANDLE_ACTIVE(pPart->hDetailTexture))
						break;
					if (apTex[2] ? !gclCostumeEditList_IsOwned(apTex[2]->eRestriction, apTex[2]->pcName, g_CostumeEditState.stashTexUnlockMeta) : IS_HANDLE_ACTIVE(pPart->hSpecularTexture))
						break;
					if (apTex[3] ? !gclCostumeEditList_IsOwned(apTex[3]->eRestriction, apTex[3]->pcName, g_CostumeEditState.stashTexUnlockMeta) : IS_HANDLE_ACTIVE(pPart->hDiffuseTexture))
						break;
					if (apTex[4] ? !gclCostumeEditList_IsOwned(apTex[4]->eRestriction, apTex[4]->pcName, g_CostumeEditState.stashTexUnlockMeta) : pPart->pMovableTexture && IS_HANDLE_ACTIVE(pPart->pMovableTexture->hMovableTexture))
						break;
				}
			...
			eaPush(peaCostumes, pSource);
		}

		eaQSort(*peaCostumes, SortByDisplayName);
	}
}
...
void gclCostumeEditListExpr_GenGetCostumes(SA_PARAM_NN_VALID UIGen *pGen, const char *pcList, const char *pcSkeleton)
{
	...
	gclCostumeEditList_GetCostumes(&s_eaCostumes, pcList, pcSkeleton, NULL, NULL);
	ui_GenSetListSafe(pGen, &s_eaCostumes, CostumeSource);
	eaClear(&s_eaCostumes);
}

// Get a costume from the costume list for a particular slot type
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(CostumeCreatorList_GetCostumeForSlot);
SA_RET_OP_VALID CostumeSource *gclCostumeEditListExpr_GetCostumeForSlot(const char *pcList, const char *pcSkeleton, const char *pcSlotType)
{
	static CostumeSource **s_eaCostumes;
	CostumeSource *pSource;
	gclCostumeEditList_GetCostumes(&s_eaCostumes, pcList, pcSkeleton, NULL, pcSlotType);
	pSource = eaGet(&s_eaCostumes, 0);
	eaClear(&s_eaCostumes);
	return pSource;
}

// Get a costume name from the costume list for a particular slot type
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(CostumeCreatorList_GetCostumeNameForSlot);
SA_RET_OP_VALID CostumeSource *gclCostumeEditListExpr_GetCostumeNameForSlot(const char *pcList, const char *pcSkeleton, const char *pcSlotType)
{
	static CostumeSource **s_eaCostumes;
	CostumeSource *pSource;
	gclCostumeEditList_GetCostumes(&s_eaCostumes, pcList, pcSkeleton, NULL, pcSlotType);
...
// Get the list of costumes from the costume list
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(CostumeCreatorList_GenGetSpeciesCostumes);
void gclCostumeEditListExpr_GenGetSpeciesCostumes(SA_PARAM_NN_VALID UIGen *pGen, const char *pcList, const char *pcSpecies)
{
	static CostumeSource **s_eaCostumes;
	gclCostumeEditList_GetCostumes(&s_eaCostumes, pcList, NULL, pcSpecies, NULL);
	ui_GenSetListSafe(pGen, &s_eaCostumes, CostumeSource);
	eaClear(&s_eaCostumes);
}

// Get a costume from the costume list for a particular slot type
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(CostumeCreatorList_GetSpeciesCostumeForSlot);
SA_RET_OP_VALID CostumeSource *gclCostumeEditListExpr_GetSpeciesCostumeForSlot(const char *pcList, const char *pcSpecies, const char *pcSlotType)
{
	static CostumeSource **s_eaCostumes;
	CostumeSource *pSource;
	gclCostumeEditList_GetCostumes(&s_eaCostumes, pcList, NULL, pcSpecies, pcSlotType);
	pSource = eaGet(&s_eaCostumes, 0);
	eaClear(&s_eaCostumes);
	return pSource;
}

// Apply a costume source as an overlay to the editor costume
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(CostumeCreatorList_ApplySourceBoneGroupOverlay);
void gclCostumeEditListExpr_ApplySourceBoneGroupOverlay(SA_PARAM_OP_VALID CostumeSource *pSource, const char *pcBoneGroup, bool bHover)
{
	if (pSource)
	{
...
{
	gclCostumeEditListExpr_ApplySourceBoneGroupOverlay(CostumeEditList_GetCostume(pcList, pcName), pcBoneGroup, bHover);
}
