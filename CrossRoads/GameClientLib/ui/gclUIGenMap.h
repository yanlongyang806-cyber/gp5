...
typedef struct UIGenMapIcon UIGenMapIcon;
...
AUTO_STRUCT;
typedef struct UIGenMapIconLayerDef
{
	...
	U32 uiColor;					AST(SUBTABLE(ColorEnum) NAME(Color))
		// The icon has this color. If zero, then it'll inherit the color from the base layer of the IconDef

	F32 fFrameDuration;				AST(DEFAULT(-1))
	S32 iFrameCount;				AST(DEFAULT(1))

	F32 fPercentX;					AST(NAME(PercentX))
	F32 fPercentY;					AST(NAME(PercentY))
		// The percentage of the base layer to offset this icon from

	UIDirection eOffsetFrom;		AST(NAME(OffsetFrom) DEFAULT(UINoDirection))
		// The offset from the base layer to position this icon from

	F32 fAbovePlayer;				AST(DEFAULT(-FLT_MAX))
		// Display the icon when the waypoint is the given number of units above the player (may be negative)
	F32 fBelowPlayer;				AST(DEFAULT(FLT_MAX))
		// Display the icon when the waypoint is the given number of units below the player (may be positive)

	WorldRegionType eRegion;		AST(DEFAULT(WRT_None))
		// The region that this layer is allowed to appear in

	S16 iZ;							AST(NAME(Z))
		// This icon appears here in the Z order (larger = on top).

	F32 fLayerScale;				AST(DEFAULT(1))
		// Additional scaling this layer has over the base
	...
	bool bIgnoreWorldSize : 1;
		// Ignore the world size of this thing and treat it as 0x0x0.

	bool bSkinningOverride : 1;
		// Perform skinning on the texture

	bool bShowUpwardsAndDownwards : 1;
		// Show upwards / downwards icon next to this icon

	bool bShrinkToFit : 1;
		// Shrink this icon down to fit inside the the volume (or base layer)

	AtlasTex *pTexture; NO_AST
		// Cached copy of the texture for this IconLayer.
} UIGenMapIconLayerDef;

AUTO_STRUCT;
typedef struct UIGenMapIconDef
{
	UIGenMapIconLayerDef BaseLayer; AST(EMBEDDED_FLAT)
		// The base layer of the icon

	UIGenMapIconLayerDef **eaLayers; AST(NAME(Layer))
	...
	const char *pchDistanceScaleGroup; AST(POOL_STRING)
		// The group of map icons that this performs distance scaling on
...
typedef struct UIGenMapKeyIcon
{
	...
	S32 iDominationPoint;
		//For PvP domination points, this is the point's number. This should be 0 for other icon types
} UIGenMapKeyIcon;
...
typedef struct UIGenMapIcon
{
	...
	S32 iLayer;
	UIGenMapIcon *pParentIcon; NO_AST
