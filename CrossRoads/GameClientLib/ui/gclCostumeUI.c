...
void CostumeUI_UpdateLists(NOCONST(PlayerCostume) *pCostume, bool bUGC, bool bValidateSafeMode)
{
	...
	if (g_CostumeEditState.pcSlotSet) {
		...
		if (g_CostumeEditState.pSlotDef) {
			costumeTailor_GetValidSlotTypes(pCostume, g_CostumeEditState.pSlotDef, pSpecies, eaFlatList, g_CostumeEditState.eaPowerFXBones, &g_CostumeEditState.eaSlotTypes, CGVF_OMIT_EMPTY);
			if (eaFind(&g_CostumeEditState.eaSlotTypes, g_CostumeEditState.pSlotType) < 0) {
				g_CostumeEditState.pSlotType = costumeLoad_GetSlotType(g_CostumeEditState.pSlotDef->pcSlotType);
			}
		}
	} else {
	...
	if (pGeo && pGeo->pClothData && pGeo->pClothData->bIsCloth && pGeo->pClothData->bHasClothBack && pPart->pClothLayer && GET_REF(pBone->hMainLayerFront) && GET_REF(pBone->hMainLayerBack)) {
		...
		if (pBothLayer && pFrontLayer && pBackLayer) {
			...
			if (pPart->eEditMode == kPCEditMode_Both) {
				...
				if (devassert(pPart->pClothLayer)) {
	...
	for (i = eaSize(&g_CostumeEditState.eaBoneValidValues) - 1; i >= 0; i--) {
		NOCONST(PCPart) *pValidPart = costumeTailor_GetPartByBone(pCostume, GET_REF(g_CostumeEditState.eaBoneValidValues[i]->hBone), NULL);
		if (pValidPart) {
			costumeTailor_GetValidTextures(pCostume, GET_REF(pValidPart->hMatDef), pSpecies, NULL, NULL /*pBoneGroup*/, GET_REF(pValidPart->hGeoDef), NULL, g_CostumeEditState.eaUnlockedCostumes, kPCTextureType_Pattern, &g_CostumeEditState.eaBoneValidValues[i]->eaPatterns, false, true, g_CostumeEditState.bUnlockAll);
			costumeTailor_GetValidTextures(pCostume, GET_REF(pValidPart->hMatDef), pSpecies, NULL, NULL /*pBoneGroup*/, GET_REF(pValidPart->hGeoDef), NULL, g_CostumeEditState.eaUnlockedCostumes, kPCTextureType_Detail, &g_CostumeEditState.eaBoneValidValues[i]->eaDetails, false, true, g_CostumeEditState.bUnlockAll);
			costumeTailor_GetValidTextures(pCostume, GET_REF(pValidPart->hMatDef), pSpecies, NULL, NULL /*pBoneGroup*/, GET_REF(pValidPart->hGeoDef), NULL, g_CostumeEditState.eaUnlockedCostumes, kPCTextureType_Specular, &g_CostumeEditState.eaBoneValidValues[i]->eaSpeculars, false, true, g_CostumeEditState.bUnlockAll);
			costumeTailor_GetValidTextures(pCostume, GET_REF(pValidPart->hMatDef), pSpecies, NULL, NULL /*pBoneGroup*/, GET_REF(pValidPart->hGeoDef), NULL, g_CostumeEditState.eaUnlockedCostumes, kPCTextureType_Diffuse, &g_CostumeEditState.eaBoneValidValues[i]->eaDiffuses, false, true, g_CostumeEditState.bUnlockAll);
			costumeTailor_GetValidTextures(pCostume, GET_REF(pValidPart->hMatDef), pSpecies, NULL, NULL /*pBoneGroup*/, GET_REF(pValidPart->hGeoDef), NULL, g_CostumeEditState.eaUnlockedCostumes, kPCTextureType_Movable, &g_CostumeEditState.eaBoneValidValues[i]->eaMovables, false, true, g_CostumeEditState.bUnlockAll);
		} else {
			CostumeBoneValidValues_ResetLists(g_CostumeEditState.eaBoneValidValues[i]);
		}
	}
...
SA_RET_NN_VALID static UIColor *CostumeUI_GetColor(int iColor)
{
	...
	if (!(kPCEditColor_SharedColor0 <= iColor && iColor <= kPCEditColor_SharedColor3)) {
		...
		if (pGeo && pGeo->pClothData && pGeo->pClothData->bIsCloth && pGeo->pClothData->bHasClothBack && pRealPart->pClothLayer && (pRealPart->eEditMode == kPCEditMode_Both)) {
			bBothMode = true;
		}
...
void CostumeUI_costumeView_RegenCostume(CostumeViewGraphics *pGraphics, PlayerCostume *pCostume, const PCSlotType *pSlotType, PCMood *pMood, CharacterClass* pClass, ItemDefRef **eaShowItems)
{
	...
	if (pNewCostume)
	{
		costumeView_RegenCostumeEx(pGraphics, (PlayerCostume*)pNewCostume, pSlotType, pMood, pClass, NULL, NULL);
		StructDestroyNoConst(parse_PlayerCostume, pNewCostume);
	}
	else
	{
		costumeView_RegenCostumeEx(pGraphics, pCostume, pSlotType, pMood, pClass, NULL, NULL);
	}
...
static bool CostumeCreator_ApplyPresetOverlayBones(PCBoneDef *pBone, NOCONST(PlayerCostume) *pSrc, NOCONST(PlayerCostume) *pDst, bool bStrict) 
{
	...
	if (pBone) {
		...
		} else if (bStrict && pDstPart) {
			eaFindAndRemove(&pDst->eaParts, pDstPart);
...
void CostumeCreator_ApplyPresetOverlay(NOCONST(PlayerCostume) *pDst, CostumePreset *pPreset, bool bForceIgnoreHeight, bool bStrict)
{
#define COSTUME_PRESET_VALIDATE_VALUE(value) (pPreset->bOverrideValidateValues || !pPresetCat ? pPreset->validatePlayerValues.value : pPresetCat->validatePlayerValues.value)
	...
	if (COSTUME_PRESET_VALIDATE_VALUE(bTestBones)) {
		...
		for (i = eaSize(&eaBones) - 1; i >= 0; i--) {
			bValidate |= CostumeCreator_ApplyPresetOverlayBones(GET_REF(eaBones[i]->hBone), pSrc, pDst, bStrict);
		}
	}
	...
	if (COSTUME_PRESET_VALIDATE_VALUE(bTestBoneScales)) {
		...
		for (i = eaSize(&eaScales) - 1; i >= 0; i--) {
			...
			if(!costumeTailor_GetOverrideBoneScale(pDstSkel, NULL, eaScales[i]->pcName, GET_REF(pDst->hSpecies), g_CostumeEditState.pSlotType, &fMin, &fMax)) {
				continue;
			}
			CostumeCreator_CommonSetBoneScale(pDst, fMin, fMax, eaScales[i]->pcName, fValue - fMin);
		}
	}
	...
	if (bValidate) {
		...
		Entity *pPlayerEnt = CostumeCreator_GetEditPlayerEntity();
		GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pPlayerEnt);
		costumeTailor_MakeCostumeValid(pDst, GET_REF(pDst->hSpecies), g_CostumeEditState.eaUnlockedCostumes, pSlotType, true, false, false, guild_GetGuild(pPlayerEnt), false, pExtract, false, g_CostumeEditState.eaPowerFXBones);
	}
...
bool CostumeCreator_SetHoverPresetOverlay(const char *pchPreset, U32 uFlags)
{
	PCSkeletonDef *pSkel = GET_REF(g_CostumeEditState.hSkeleton);
	SpeciesDef *pSpecies = GET_REF(g_CostumeEditState.hSpecies);
	...
	if (pSkel && pSpecies && g_CostumeEditState.pCostume) {
		...
		for (i = 0; i < eaSize(&pSpecies->eaPresets); i++) {
			...
			if (!stricmp(name, pchPreset)) {
				...
				g_CostumeEditState.pHoverCostume = StructCloneNoConst(parse_PlayerCostume, g_CostumeEditState.pCostume);
				CostumeCreator_ApplyPresetOverlay(g_CostumeEditState.pHoverCostume, pPreset, bIgnoreHeight, bStrictMode);
				costumeEntity_ApplyEntityInfoToCostume(CostumeUI_GetSourceEnt(),g_CostumeEditState.pConstHoverCostume);
...
bool CostumeCreator_SetPresetOverlay(const char *pchPreset, U32 uFlags)
{
	PCSkeletonDef *pSkel = GET_REF(g_CostumeEditState.hSkeleton);
	SpeciesDef *pSpecies = GET_REF(g_CostumeEditState.hSpecies);
	bool bIgnoreHeight = (uFlags & COSTUME_CREATOR_APPLY_PRESET_FLAG_IGNORE_HEIGHT) != 0;
	...
	if (pSkel && pSpecies && g_CostumeEditState.pCostume) {
		...
		for (i = 0; i < eaSize(&pSpecies->eaPresets); i++) {
			...
			if (!stricmp(name, pchPreset)) {
				...
				CostumeCreator_ApplyPresetOverlay(g_CostumeEditState.pCostume, pPreset, bIgnoreHeight, bStrictMode);
				CostumeUI_RegenCostume(true);
				return true;
			}
		}
...
void CostumeCreator_SetAllColorLink(int /*PCColorLink*/ eLinkType)
{
	...
	for (i = eaSize(&g_CostumeEditState.pCostume->eaParts)-1; i >=0; --i)
	{
		pRealPart = g_CostumeEditState.pCostume->eaParts[i];
...
void CostumeCreator_SetAllMaterialLink(int /*PCColorLink*/ eLinkType)
{
	...
	for (i = eaSize(&g_CostumeEditState.pCostume->eaParts)-1; i >=0; --i)
	{
		...
		pBone = GET_REF(pRealPart->hBoneDef);
		if (!pBone)
			continue;
...
SA_RET_NN_VALID PCRegion *CostumeCreator_GetEditRegion(void)
{
	...
	// TODO: this needs to support being able to return NULL
...
SA_RET_NN_VALID PCCategory *CostumeCreator_GetEditCategory(void)
{
	...
	COSTUME_UI_TRACE_FUNC();
	if (!category && g_CostumeEditState.pConstCostume) {
		category = costumeTailor_GetCategoryForRegion(g_CostumeEditState.pConstCostume, GET_REF(g_CostumeEditState.hRegion));
	}
	// TODO: this needs to support returning NULL
	assert(category);
	return category;
}
...
const char *CostumeCreator_GetGeoSysNameInternal(PCBoneDef *pBone)
{
	...
	pGeo = GET_REF(pRealPart->hGeoDef);
	return SAFE_MEMBER(pGeo, pcName);
}
...
const char *CostumeCreator_GetMatSysNameInternal(SA_PARAM_OP_VALID PCBoneDef *pBone)
{
	...
	if (!pBone || !stricmp(pBone->pcName,"None")) {
		PCSkeletonDef *pSkel = GET_REF(g_CostumeEditState.hSkeleton);
		const PCBoneGroup *pBoneGroup = pSkel ? costumeTailor_FindBoneGroupByName(pSkel, "CommonMaterialsDef") : NULL;
		PCBoneRef *pBoneRef = pBoneGroup ? eaGet(&pBoneGroup->eaBoneInGroup, 0) : NULL;
		pBone = SAFE_GET_REF(pBoneRef, hBone);
	}
	...
	pMat = GET_REF(pRealPart->hMatDef);
	return SAFE_MEMBER(pMat, pcName);
}
...
const char *CostumeCreator_GetMatNameInternal(SA_PARAM_OP_VALID PCBoneDef *pBone)
{
	...
	if (g_MirrorSelectMode && (pRealPart->eEditMode == kPCEditMode_Both) && bHaveMirrorBone) {
		...
		if (pMirrorPart) {
			...
			if ((pMirrorMat && ((pMat == pMirrorMat) || (pMat && stricmp(TranslateDisplayMessage(pMat->displayNameMsg), TranslateDisplayMessage(pMirrorMat->displayNameMsg)) == 0))) ||
				(!pMirrorMat && (pMat && stricmp(pMat->pcName,"None") == 0))){
	...
	return pMat ? TranslateDisplayMessage(pMat->displayNameMsg) : "";
}
...
const char *CostumeCreator_GetTexSysNameInternal(PCBoneDef *pBone, int id)
{
	...
	return SAFE_MEMBER(pTex, pcName);
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(CostumeCreator_ResetCostumeSaved);
void CostumeCreator_ResetCostumeSaved(void)
{
	COSTUME_UI_TRACE_FUNC();
	s_bStorePlayerCostumeSuccessful = false;
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_ResetCostumeSkinColor");
void CostumeCreator_ResetCostumeSkinColor(void)
{
	COSTUME_UI_TRACE_FUNC();
	CostumeUI_ResetCostumeSkinColor(g_CostumeEditState.pCostume, CONTAINER_RECONST(PlayerCostume, g_CostumeEditState.pStartCostume));
}
...
void CostumeCreator_FlashFX(const char* pchFXName)
{
	COSTUME_UI_TRACE_FUNC();
	if (g_pCostumeView
		&& g_pCostumeView->costume.pFxManager
		&& g_pCostumeView->costume.pSkel
		&& pchFXName
		&& pchFXName[0])
	{
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CostumeCreator_StartFXWithHue");
void CostumeCreator_StartFXWithHue(const char* pchFXName, float fHue)
{
	COSTUME_UI_TRACE_FUNC();
	if (g_pCostumeView && g_pCostumeView->costume.pSkel && pchFXName && pchFXName[0] && g_pCostumeView->costume.pFxManager)
	{
		dynFxManAddMaintainedFX(g_pCostumeView->costume.pFxManager, pchFXName, NULL, fHue, 0, eDynFxSource_UI);
	}
}
...
{
	CostumeCreator_StartFXWithHue(pchFXName, 0.0f);
}
...
void CostumeUI_GetRadiusAndHeight(DynSkeleton *pSkel, F32 fFOVy, bool bAutoCenter, bool bUpdateExtents, Vec3 v3ExtentsMinOut, Vec3 v3ExtentsMaxOut, Vec3 v3AutoCenter, F32 *fRadiusOut, F32 *fHeightOut)
{
	Vec3 v3ExtentsMin;
	Vec3 v3ExtentsMax;
	Vec3 v3ExtentDif;
	...
	if (bUpdateExtents)
	{
		dynSkeletonGetVisibilityExtents(pSkel, v3ExtentsMin, v3ExtentsMax, true);

		for (i = 0; v3ExtentsMinOut && i < 3; i++)
			v3ExtentsMin[i] = MIN(v3ExtentsMinOut[i], v3ExtentsMin[i]);
		for (i = 0; v3ExtentsMaxOut && i < 3; i++)
			v3ExtentsMax[i] = MAX(v3ExtentsMaxOut[i], v3ExtentsMax[i]);
	}
	else
	{
		if (v3ExtentsMinOut)
			copyVec3(v3ExtentsMinOut, v3ExtentsMin);
		if (v3ExtentsMaxOut)
			copyVec3(v3ExtentsMaxOut, v3ExtentsMax);
	}

	// Calculate the radius and height to use
	subVec3(v3ExtentsMax, v3ExtentsMin, v3ExtentDif);
	if (fRadiusOut)
	{
		*fRadiusOut = lengthVec3(v3ExtentDif);
		if(*fRadiusOut == 0) {
		...
		*fRadiusOut = tanf(RAD(fFOVy / 2)) * 2.0f / (*fRadiusOut) + 1.1f * (*fRadiusOut);
	}

	if (fHeightOut)
		*fHeightOut = vecY(v3ExtentDif);

	// If centering based off the skeleton, calculate the center here
	if (bAutoCenter && v3AutoCenter)
	{
		scaleVec3(v3ExtentDif, .5f, v3ExtentDif);
		subVec3(v3ExtentsMax, v3ExtentDif, v3AutoCenter);
	}

	if (v3ExtentsMinOut)
		copyVec3(v3ExtentsMin, v3ExtentsMinOut);
	if (v3ExtentsMaxOut)
		copyVec3(v3ExtentsMax, v3ExtentsMaxOut);
}
