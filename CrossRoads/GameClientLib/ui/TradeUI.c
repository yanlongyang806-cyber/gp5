...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenIsTradeInEscrow");
bool trade_GenExprIsTradeInEscrow(SA_PARAM_OP_VALID Entity* pEnt)
{
	return pEnt && pEnt->pPlayer && pEnt->pPlayer->pTradeBag && pEnt->pPlayer->pTradeBag->escrowed;
}
...
static void gclTrade_GetTradeableItems(SA_PARAM_NN_VALID UIGen* pGen, SA_PARAM_OP_VALID Entity* pEnt, S32 iBag, bool bIncludePets)
{
	...
	static S32 *s_BagIDs = NULL;

	if(s_BagIDs == NULL || s_BagIDs[0] != iBag)
	{
		ea32Push(&s_BagIDs,iBag);
	}
	
	Trade_GetTradeableItems(peaSlots,pEnt,&s_BagIDs,bIncludePets,true);
