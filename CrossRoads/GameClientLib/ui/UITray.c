								  ...
								  const char *cpchDefaultIcon)
{
	...
	if(pelem->pTrayElem && pelem->pTrayElem->eType == kTrayElemType_SavedPetPower && !pPetInfo)
	{
		pelem->bActivatable = false;
	}
	else
										 ...
										 bool bReferenceTrayElems)
{
	...
	if (curEnt)
	{
		...
		for ( i = 0; i < iNumPowers; ++i )
		{
			...
			{
				GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEntity);
				power_AutoDesc(entGetPartitionIdx(pEntity),langGetCurrent(),pPow,pChar,&pPowerElem->pchToolTip,NULL,"<br>","<bsp><bsp>","- ",false,0,entGetPowerAutoDescDetail(entActivePlayerPtr(),true),pExtract,NULL);
			}
		}
										...
										int iKey )
{
	if (pElem)
	{
		...
		else if (pEntity && pEntity->pSaved)
		{
			...
			AlwaysPropSlot* pPropSlot = AlwaysPropSlot_FindByPetID(pEntity, uPetID, pEntity->pSaved->pPuppetMaster->curID, kAlwaysPropSlotCategory_Default);
			if (pPropSlot)
			{
...
// internal function for exprEntTrayElemTooltip, exprEntTrayElemTooltipEx to get the tooltip for the given UITrayElem
// optional pchPowerAutoDescMessageKey to change default formatting
static const char *EntTrayElemTooltipInternal(	SA_PARAM_OP_VALID Entity *pEntity, 
												SA_PARAM_NN_VALID UITrayElem *pelem,
												SA_PARAM_OP_VALID const char* pchPowerAutoDescMessageKey)
{
	...
	else
	{
		...
		if(pEntity && pEntity->pChar && pelem->pTrayElem)
		{
			...
			{
				GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEntity);
				power_AutoDesc(entGetPartitionIdx(pEntity),langGetCurrent(),ppow,pchar,&s_pchTooltip,
								NULL,"<br>","<bsp><bsp>","- ",false,0,
								entGetPowerAutoDescDetail(entActivePlayerPtr(),true),pExtract, pchPowerAutoDescMessageKey);
			}
		}
		else if (pEntity && pEntity->pChar && pelem->pPetData && pelem->pPetData->pchPower)
		{
			...
			{
				GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEntity);
				powerdef_AutoDesc(entGetPartitionIdx(pEntity),langGetCurrent(),pdef,&s_pchTooltip,
									NULL,"<br>","<bsp><bsp>","- ",pchar, NULL, NULL, pchar->iLevelCombat, 0, 
									entGetPowerAutoDescDetail(pEntity,true), pExtract, pchPowerAutoDescMessageKey);
			}
		}
	}
...
const char *exprEntTrayElemTooltip(SA_PARAM_OP_VALID Entity *pEntity,
								   SA_PARAM_NN_VALID UITrayElem *pelem)
{
	return EntTrayElemTooltipInternal(pEntity, pelem, NULL);
}

AUTO_EXPR_FUNC(entityutil) ACMD_NAME("EntTrayElemTooltipEx");
const char *exprEntTrayElemTooltipEx(SA_PARAM_OP_VALID Entity *pEntity,
										SA_PARAM_NN_VALID UITrayElem *pelem,
										SA_PARAM_OP_VALID const char* pchPowerAutoDescMessageKey)
{
	return EntTrayElemTooltipInternal(pEntity, pelem, pchPowerAutoDescMessageKey);
}

// Given the UITrayElem, returns the PowerDef for the power that would be activated
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("EntTrayElemGetPowerDef");
SA_RET_OP_VALID PowerDef* exprEntTrayElemGetPowerDef(SA_PARAM_OP_VALID Entity *pEntity, SA_PARAM_NN_VALID UITrayElem *pelem)
{
	if (pEntity)
	{
		Power *ppow = EntTrayGetActivatedPower(pEntity, pelem->pTrayElem, pelem->bUseBaseReplacePower, NULL);
...
// Returns the time to recharge on a given item, similar to how it would on a tray slot.
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("ItemPowerCanBeUsed");
bool exprItemPowerCanBeUsed(SA_PARAM_OP_VALID Item *pItem)
{
	Entity *e = entActivePlayerPtr();
	if(e && e->pChar)
	{
		...
		for(i=0; i<iPowers; i++)
		{
			...
			if (pPowItem)
			{
				return !!character_CanQueuePower(entGetPartitionIdx(e), e->pChar, pPowItem, NULL, NULL, NULL, NULL, NULL, NULL, false, pmTimestamp(0), -1, NULL, false, true, false, NULL);
			}
		}
	}
	return false;
