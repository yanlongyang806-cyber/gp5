...
static UIGenMapIconLayerDef *ui_GenMapGetIconLayer(UIGenMapIcon *pIcon)
{
	...
	iLayer = pIcon->iLayer;
	if (iLayer < 0 || !pDef->eaLayers || iLayer >= eaSize(&pDef->eaLayers))
		return &pDef->BaseLayer;

	return pDef->eaLayers[iLayer];
}
...
{
	UIGenMapIconDef *pDef = pIcon->pDef;
	UIGenMapIconLayerDef *pLayer = ui_GenMapGetIconLayer(pIcon);
	const char *pchIcon = pLayer->pchIcon;
	bool bPreferIconTexture;
	if (pLayer->bSkinningOverride)
		pchIcon = ui_GenGetTextureSkinOverride(pchIcon);
	UI_GEN_LOAD_TEXTURE(pchIcon, pLayer->pTexture);

	// For the base layer, prefer using the texture on the icon
	// For other layers, prefer using the texture on the def
	bPreferIconTexture = pIcon->iLayer < 0 ? !!pIcon->pchTexture : !pLayer->pTexture;

	if (!pLayer->pchFormatIcon)
	{
		AtlasTex *pIconTex = NULL;
		// Use the per-icon override if set and present, otherwise use the one from the def.
		if (bPreferIconTexture)
			pIconTex = atlasLoadTexture(!pLayer->bSkinningOverride ? pIcon->pchTexture : ui_GenGetTextureSkinOverride(pIcon->pchTexture));
		if ((!pIconTex || pIconTex == white_tex_atlas)
			&& pLayer->pTexture
			&& pLayer->pTexture != white_tex_atlas)
			return pLayer->pTexture;
		else
			return pIconTex;
	}
	else
	{
		...
		if (pLayer->fFrameDuration > 0 && pLayer->iFrameCount > 1)
		{
			iFrame = ((int)((g_ui_State.totalTimeInMs / 1000.0) / pLayer->fFrameDuration) % pLayer->iFrameCount) + 1;
		}
		else
		{
			iFrame = 1;
		...
		if (s_pch && *s_pch && (pTex = atlasLoadTexture(!pLayer->bSkinningOverride ? s_pch : ui_GenGetTextureSkinOverride(s_pch))) != white_tex_atlas)
			return pTex;
		// Otherwise try the unformatted texture override.
		else if (pIcon->pchTexture && (pTex = atlasLoadTexture(!pLayer->bSkinningOverride ? pIcon->pchTexture : ui_GenGetTextureSkinOverride(pIcon->pchTexture))) != white_tex_atlas)
			return pTex;
...
static bool GenMapCheckLayer(UIGenMapState *pState, UIGenMapIconLayerDef *pLayer, Vec3 v3Pos, WorldRegionType eRegionType)
{
	F32 fY = pState->v3Target[1] - v3Pos[1];
	if (fY < pLayer->fAbovePlayer)
		return false;
	if (fY > pLayer->fBelowPlayer)
		return false;
	if (pLayer->eRegion != WRT_None && eRegionType != pLayer->eRegion)
		return false;
	return true;
}
...
static UIGenMapIcon *GenAddMapIcon(UIGenMapIcon ***peaIcons, S32 *piCount)
{
	UIGenMapIcon *pNewIcon = eaGetStruct(peaIcons, parse_UIGenMapIcon, (*piCount)++);
	S32 *eaIndices = pNewIcon->eaIndices;
	char *pchLabel = pNewIcon->pchLabel;
	char *pchIndexLabel = pNewIcon->pchIndexLabel;
	bool bMouseOver = pNewIcon->bMouseOver;

	memset(pNewIcon, 0, sizeof(UIGenMapIcon));
	eaiClear(&eaIndices);
	estrClear(&pchLabel);
	estrClear(&pchIndexLabel);

	pNewIcon->iKeyIndex = 0;
	pNewIcon->iLayer = -1;

	pNewIcon->eaIndices = eaIndices;
	pNewIcon->pchLabel = pchLabel;
	pNewIcon->pchIndexLabel = pchIndexLabel;
	pNewIcon->bMouseOver = bMouseOver;

	return pNewIcon;
}

static UIGenMapIcon *GenAddMapIconClone(UIGenMapIcon ***peaIcons, S32 *piCount, UIGenMapIcon *pClone)
{
	UIGenMapIcon *pNewIcon = eaGetStruct(peaIcons, parse_UIGenMapIcon, (*piCount)++);
	S32 *eaIndices = pNewIcon->eaIndices;
	char *pchLabel = pNewIcon->pchLabel;
	...
	memcpy(pNewIcon, pClone, sizeof(UIGenMapIcon));
	eaiClear(&eaIndices);
	estrClear(&pchLabel);
	estrClear(&pchIndexLabel);
...
static void GenMapIconLayersFrom(UIGenMapState *pState, S32 *piCount, UIGenMapIcon *pParentIcon, WorldRegionType eRegionType)
{
	UIGenMapIconDef *pDef = pParentIcon->pDef;
	S32 i, n = eaSize(&pDef->eaLayers);
	for (i = 0; i < n; i++)
	{
		UIGenMapIconLayerDef *pLayer = pDef->eaLayers[i];
		if (GenMapCheckLayer(pState, pLayer, pParentIcon->v3WorldPos, eRegionType))
		{
			UIGenMapIcon *pLayerIcon = GenAddMapIconClone(&pState->eaIcons, piCount, pParentIcon);
			pLayerIcon->iLayer = i;
			pLayerIcon->pParentIcon = pParentIcon;
			pLayerIcon->iLayerZ = pParentIcon->iLayerZ + pLayer->iZ;
		}
	}
}
...
static UIGenMapEntityIconDef *GenMapMatchEntity(UIGen *pGen, CONST_EARRAY_OF(UIGenMapEntityIconDef) eaEntityIcons, Entity *pPlayer, Entity *pTarget, const CBox *pClampBox, float fClampRadius, Vec2 v2RadiusCenter)
{
	...
	Vec3 v3TargetPos;
	Vec2 v2ScreenPos;
	WorldRegionType eRegionType = pPlayer ? entGetWorldRegionTypeOfEnt(pPlayer) : WRT_None;
	...
	for (i = eaSize(&eaEntityIcons) - 1; i >= 0; i--)
	{
		...
		if (!GenMapCheckLayer(pState, &pDef->Def.BaseLayer, v3TargetPos, eRegionType))
			continue;
...
static void GenMapIconForEntity(Entity *pPlayer, Entity *pEntity, UIGenMapIconDef *pIconDef, UIGenMapIcon *pIcon, Vec3 v3Center)
{
	Vec3 pyr;
	pIcon->pDef = pIconDef;
	pIcon->iLayerZ = pIconDef->BaseLayer.iZ;
	...
	pIcon->fCenterDistance = 0;
	pIcon->fDistanceScale = 1;
	pIcon->pchDistanceScaleGroup = pIconDef->pchDistanceScaleGroup;
...
static UIGenMapNodeIconDef *GenMapMatchNode(UIGen *pGen, CONST_EARRAY_OF(UIGenMapNodeIconDef) eaNodeIcons, TargetableNode *pTarget, Entity *pPlayer, const CBox *pClampBox, float fClampRadius, Vec2 v2RadiusCenter)
{
	...
	WorldRegionType eRegionType = pPlayer ? entGetWorldRegionTypeOfEnt(pPlayer) : WRT_None;
	TargetType eTargetType = 0;
	Vec3 v3PlayerPos, v3Pos;
	S32 i;
	Vec2 v2ScreenPos;
	F32 fRadius, fDistance;
	...
	fRadius = wlInteractionNodeGetSphereBounds(pNode, v3Pos);
	fDistance = distance3Squared(v3PlayerPos, v3Pos);
	...
	for (i = eaSize(&eaNodeIcons) - 1; i >= 0; i--)
	{
		...
		if (!GenMapCheckLayer(pState, &pDef->Def.BaseLayer, v3Pos, eRegionType))
			continue;
...
static UIGenMapWaypointIconDef *GenMapMatchWaypoint(UIGen *pGen, UIGenMapState *pState, CONST_EARRAY_OF(UIGenMapWaypointIconDef) eaWaypointIcons, MinimapWaypoint *pWaypoint, const CBox *pClampBox, float fClampRadius, Vec2 v2RadiusCenter, bool bCheckOutOfBounds)
{
	...
	for (i = eaSize(&eaWaypointIcons) - 1; i >= 0; i--)
	{
		...
		if (pDef->Def.bOutOfBounds && (!(bCheckOutOfBounds && !bAreaOnScreen) || !GenMapCheckOutOfBounds(v2ScreenPos, pClampBox, fClampRadius, v2RadiusCenter)))
			continue;
		if (!GenMapCheckLayer(pState, &pDef->Def.BaseLayer, pWaypoint->pos, eRegionType))
			continue;
		...
		if (pDef->bPrimary && pWaypoint->pchMissionRefString)
		{
			...
			const char* pchPrimaryMission = entGetPrimaryMission(pPlayer);
...
static void GenMapIconForWaypoint(MinimapWaypoint *pWaypoint, UIGenMapIconDef *pIconDef, UIGenMapIcon *pIcon, Vec3 v3Center)
{
	...
	else
	{
		...
		// Sphere volumes for some reason appear upside down, rotate them by 180 degrees.
		if (pWaypoint->eShape == WL_VOLUME_SPHERE)
			pIcon->fRotation += -PI;
	}
...
{
	F32 fDistA, fDistB;
	if ((*ppA)->iZOrder != (*ppB)->iZOrder)
		// Sort into layers
		return (*ppA)->iZOrder - (*ppB)->iZOrder;
	fDistA = distance3Squared((*ppA)->pos, v3Center);
	fDistB = distance3Squared((*ppB)->pos, v3Center);
	return (fDistA < fDistB) ? -1 : (fDistA > fDistB) ? 1 : (*ppA - *ppB);
}
...
static int SortMapKeyIcons(const UIGenMapKeyIcon **ppIconA, const UIGenMapKeyIcon **ppIconB)
{
	if(ppIconA && ppIconB)
	{
		...
		if (pIconA && pIconB)
		{
			...
			if (pIconA->iDominationPoint != 0)
			{
				if (pIconB->iDominationPoint != 0)
				{
					iComparison = pIconA->iDominationPoint - pIconB->iDominationPoint;
				}
				else
				{
					iComparison = -1;
			...
			else if (pIconB->iDominationPoint != 0)
			{
				iComparison = 1;
			}
			else if (pIconA->eWaypointType != MinimapWaypointType_None)
			{
				if (pIconA->eWaypointType != MinimapWaypointType_Landmark && pIconA->eWaypointType != MinimapWaypointType_AreaLandmark)
				{
				...
				{
					if (pIconB->eWaypointType == MinimapWaypointType_Landmark || pIconB->eWaypointType == MinimapWaypointType_AreaLandmark)
					{
						iComparison = pIconB->eWaypointType - pIconA->eWaypointType;
					} 
					else
...
static const char *ui_GetHeaderNameForKeyIcon(UIGenMapKeyIcon *pIcon)
{
	...
	if (pIcon->iDominationPoint != 0)
	{
		pchHeader = allocAddString(TranslateMessageKey("Map.Domination_Point_Key_Header"));
	}
	else if (pIcon->eWaypointType != MinimapWaypointType_None)
	{
...
static void _CreateIconsForPvPDomination(Entity *pEnt, UIGenMapState *pState, S32 *piCountInOut, 
											UIGenMapPvPDominationIconDef *pPvPDominationIconDef, S32 *piKeyIconCount, UIGenMap *pMap)
{
	WorldRegionType eRegionType = pEnt ? entGetWorldRegionTypeOfEnt(pEnt) : WRT_None;
	...
	if (peaControlPoints)
	{
		...
		{
			UIGenMapIcon *pIcon = GenAddMapIcon(&pState->eaIcons, piCountInOut);
			...
			if(pMap->bShowMapKey)
			{
				UIGenMapKeyIcon *pKeyIcon = eaGetStruct(&s_eaMapKeyIcons, parse_UIGenMapKeyIcon, (*piKeyIconCount)++);
				const char *pchLabel = allocAddString(TranslateMessagePtr(pControlPoint->pMessage));
				pKeyIcon->eContactIndicator = ContactIndicator_NoInfo;
				pKeyIcon->eWaypointType = MinimapWaypointType_None;
				pKeyIcon->pchLabel = (pchLabel && pchLabel[0] ? pchLabel : allocAddString(pIcon->pchLabel));
				pKeyIcon->iKeyIndex = *piCountInOut - 1;
				pKeyIcon->bIsHeader = false;
				pKeyIcon->pchTexture = NULL;
				pKeyIcon->iDominationPoint = pControlPoint->iPointNumber;
			}

			GenMapIconLayersFrom(pState, piCountInOut, pIcon, eRegionType);
		}
		FOR_EACH_END
	}
}
...
void ui_GenUpdateMap(UIGen *pGen)
{
		...
		|| (pHighlight && pHighlight->bShowIcons))
	{
		...
		// Always set the target Y value to the player's Y value, so
		// that when selecting for icons that are above or below the
		// player have some reasonable value to compare against.
		//
		// If a target Y value that isn't the player's Y value isn't
		// necessary, this probably will need to change, but since
		// most likely the minimap needs to be considering the
		// player's Y value when doing anything 3D related, it's
		// convenient to just set this here.
		pState->v3Target[1] = vEntPos[1];
		...
		for (i = 0; i < eaSize(&s_eaEnts); i++)
		{
			...
			{
				UIGenMapIcon *pCurIcon = GenAddMapIcon(&pState->eaIcons, &iCount);
				iMapKeyIndex = iCount - 1;
				GenMapIconForEntity(pEnt, s_eaEnts[i], &pEntityIconDef->Def, pCurIcon, pState->v3WorldCenter);
				GenMapIconLayersFrom(pState, &iCount, pCurIcon, eRegionType);
				...
				if(pEntityIconDef && pEntityIconDef->eContactIndicator != ContactIndicator_NoInfo && pMap->bShowMapKey)
				{
					...
					pKeyIcon->iKeyIndex = iMapKeyIndex;
					pKeyIcon->bIsHeader = false;
					pKeyIcon->pchTexture = NULL;
					pKeyIcon->iDominationPoint = 0;
				}
			}
		}
		...
		for (i = 0; i < eaSize(&s_eaWaypoints); i++)
		{
			...
			if (bShouldDraw)
			{
				...
				if (pWaypointIconDef = GenMapMatchWaypoint(pGen, pState, pMap->eaWaypointIcons, s_eaWaypoints[i], &ClampBox, fClampRadius, v2RadiusCenter, true))
				{
					UIGenMapIcon *pCurIcon = GenAddMapIcon(&pState->eaIcons, &iCount);
					iMapKeyIndex = iCount - 1;
					GenMapIconForWaypoint(s_eaWaypoints[i], &pWaypointIconDef->Def, pCurIcon, pState->v3WorldCenter);
					GenMapIconLayersFrom(pState, &iCount, pCurIcon, eRegionType);

					// Add non-out of bounds icon
					...
					{
						pCurIcon = GenAddMapIcon(&pState->eaIcons, &iCount);
						iMapKeyIndex = iCount - 1;
						GenMapIconForWaypoint(s_eaWaypoints[i], &pWaypointIconDef->Def, pCurIcon, pState->v3WorldCenter);
						GenMapIconLayersFrom(pState, &iCount, pCurIcon, eRegionType);
					}
					...
					if(pWaypointIconDef->eType != MinimapWaypointType_None && pMap->bShowMapKey) 
					{
						...
						pNewIcon->iKeyIndex = iMapKeyIndex;
						pNewIcon->bIsHeader = false;
						pNewIcon->pchTexture = NULL;
						pNewIcon->iDominationPoint = 0;
		...
		for (i = 0; i < eaSize(&s_eaNodes); i++)
		{
			...
			{
				UIGenMapIcon *pIcon = GenAddMapIcon(&pState->eaIcons, &iCount);
				iMapKeyIndex = iCount;
				GenMapIconForNode(s_eaNodes[i], &pNodeIconDef->Def, pIcon, pState->v3WorldCenter);
				GenMapIconLayersFrom(pState, &iCount, pIcon, eRegionType);
		...
		if (pMap->pPlayerIcon)
		{
			UIGenMapIcon *pPlayerIcon = GenAddMapIcon(&pState->eaIcons, &iCount);
			GenMapIconForEntity(pEnt, pEnt, pMap->pPlayerIcon, pPlayerIcon, pState->v3WorldCenter);
			GenMapIconLayersFrom(pState, &iCount, pPlayerIcon, eRegionType);
		}
		...
		{
			Vec3 pyr;
			UIGenMapIcon *pCameraIcon = GenAddMapIcon(&pState->eaIcons, &iCount);
			gfxGetActiveCameraYPR(pyr);
			GenMapIconForEntity(pEnt, pEnt, pMap->pCameraIcon, pCameraIcon, pState->v3WorldCenter);
			GenMapIconLayersFrom(pState, &iCount, pCameraIcon, eRegionType);
		...
		{
			_CreateIconsForPvPDomination(pEnt, pState, &iCount, pMap->pPvPDominationIcons, &iKeyIconCount, pMap);
		}
	...
	if(pMap->bShowMapKey)
	{
		...
		while (i < eaSize(&s_eaMapKeyIcons))
		{
			...
			if(pCurIcon)
			{
				...
				UIGenMapIconLayerDef *pLayer = ui_GenMapGetIconLayer(pMapIcon);
				...
				{
					pCurIcon->pchTexture = pLayer->pchIcon;
				}
				//If the two icons are the same, remove the second one and update its waypoint to match the number index
				if (pOldIcon && pCurIcon->iDominationPoint == 0
							 && pOldIcon->iDominationPoint == 0
							 && pCurIcon->pchLabel == pOldIcon->pchLabel
							 && pCurIcon->eContactIndicator == pOldIcon->eContactIndicator)
				{
					pMapIcon->iKeyIndex = pOldIcon->iKeyIndex;
...
static F32 GenMapShrinkToFit(UIGenMapIcon *pIcon, UIGenMapIconLayerDef *pLayer, F32 fBoundWidth, F32 fBoundHeight, bool bRotateBound)
{
	AtlasTex *pTex = pIcon->pTex;
	F32 fScale = 1;
	int iRotation = (int)(pIcon->fRotation / 90.0f);

	if (fBoundWidth == 0 || fBoundHeight == 0 || !pTex || pTex->width == 0 || pTex->height == 0)
		return fScale;

	if (!nearf(iRotation * 90, pIcon->fRotation) && (bRotateBound || pLayer->bRotate))
	{
		Vec2 vCorners[8] = {
			{ -fBoundWidth * 0.5f,  fBoundHeight * 0.5f }, // TL
			{  fBoundWidth * 0.5f,  fBoundHeight * 0.5f }, // TR
			{  fBoundWidth * 0.5f, -fBoundHeight * 0.5f }, // BR
			{ -fBoundWidth * 0.5f, -fBoundHeight * 0.5f }, // BL

			{ -pTex->width * 0.5f,  pTex->height * 0.5f }, // TL
			{  pTex->width * 0.5f,  pTex->height * 0.5f }, // TR
			{  pTex->width * 0.5f, -pTex->height * 0.5f }, // BR
			{ -pTex->width * 0.5f, -pTex->height * 0.5f }, // BL
		};
		Vec3 vLines[8] = {0};
		Vec2 v2Transform = {
			cosf(RAD(pIcon->fRotation)),
			sinf(RAD(pIcon->fRotation))
		};
		S32 i, j, iStart = 0, iStop = 8;
		F32 fTextureSize = SQR(vCorners[4][0]) + SQR(vCorners[4][1]);
		F32 fBoundSize = SQR(vCorners[0][0]) + SQR(vCorners[0][1]);

		// Rotate coordinates of the corners
		if (!bRotateBound)
			iStart += 4;
		if (!pLayer->bRotate)
			iStop -= 4;
		for (iStart = 0; iStart < iStop; iStart++)
		{
			Vec2 v2 = { vCorners[iStart][0], vCorners[iStart][1] };
			vCorners[iStart][0] = v2[0] * v2Transform[0] - v2[1] * v2Transform[1];
			vCorners[iStart][1] = v2[0] * v2Transform[1] + v2[1] * v2Transform[0];
		}

		// Convert coordinates to line equations
		for (i = 0; i < 8; i++)
		{
			F32 *vP = vCorners[((i + 0) % 4) + (i / 4) * 4];
			F32 *vQ = vCorners[((i + 1) % 4) + (i / 4) * 4];
			F32 fdX = vQ[0] - vP[0];
			F32 fdY = vQ[1] - vP[1];
			...
			// ==> B * (y1 - y0) = A * (x1 - x0)
			vLines[i][0] = vQ[1] - vP[1]; // A = (y1 - y0)
			vLines[i][1] = vQ[0] - vP[0]; // B = (x1 - x0)
			vLines[i][2] = -(vLines[i][0] * vP[0] - vLines[i][1] * vP[1]);
		}

		// - Perform inscribing by finding the intersection point between the
		//   bound lines and the texture lines.
		// - Compute the distance between the intersection and the origin.
		// - If the distance is smaller than the texture size, calculate how
		//   much the smaller the distance is compared to the texture size.
		for (i = 0; i < 4; i++)
		{
			for (j = 4; j < 8; j++)
			{
				if (nearf(vLines[i][0], vLines[j][0]) && nearf(vLines[i][1], vLines[j][1]))
				{
					// When the lines are parallel, only the C constant matters
					if (vLines[i][2] - vLines[j][2] < 0)
					{
						F32 fMinScale = vLines[i][2] / vLines[j][2];
						fScale = min(fScale, fMinScale);
					}
				}
				else
				{
					...
					// A1 * x + B1 * y + C1 = 0
					//
					// A0 * x + B0 * y + C0 - (A1 * x + B1 * y + C1) * A0 / A1 = 0
					// B0 * y + C0 - B1 * y * A0 / A1 - C1 * A0 / A1 = 0
					// y * (B0 - B1 * A0 / A1) + (C0 - C1 * A0 / A1) = 0
					// y * (B0 - B1 * A0 / A1) = -(C0 - C1 * A0 / A1)
					// y = -(A1 * C0 - C1 * A0) / (A1 * B0 - B1 * A0)
					//
					//   x = -(B1 * C0 - C1 * B0) / (B1 * A0 - A1 * B0)
					//   y = -(A1 * C0 - C1 * A0) / (A1 * B0 - B1 * A0)
					//
					Vec2 vIntersection = {
						-(vLines[j][1] * vLines[i][2] - vLines[j][2] * vLines[i][1]) / (vLines[j][1] * vLines[i][0] - vLines[j][0] * vLines[i][1]),
						-(vLines[j][0] * vLines[i][2] - vLines[j][2] * vLines[i][0]) / (vLines[j][0] * vLines[i][1] - vLines[j][1] * vLines[i][0])
					};
					F32 fIntersectionSize = SQR(vIntersection[0]) + SQR(vIntersection[1]);
					if (fIntersectionSize < fTextureSize)
					{
						F32 fMinScale = sqrtf(fBoundSize / fIntersectionSize);
						fScale = min(fScale, fMinScale);
					}
				}
			}
	...
	{
		// Simple scaling... the angle is either a multiple of 180 or 90 degrees.
		// When it's not a multiple of 180 degrees, swap width and height.
		bool bRotate = (iRotation % 2) == 0 && (bRotateBound || pLayer->bRotate);
		F32 fScaleX = bRotate ? fBoundWidth : /* rotated by 90 degrees */ fBoundHeight;
		F32 fScaleY = bRotate ? fBoundHeight : /* rotated by 90 degrees */ fBoundWidth;
		fScaleX /= pTex->width;
		fScaleY /= pTex->height;

		// Pick the smaller of the scales
		fScale = min(fScale, fScaleX);
		fScale = min(fScale, fScaleY);
	}
...
void ui_GenLayoutLateMap(UIGen *pGen)
{
	...
	{
		const char *pchDistanceScaleGroup = pState->eaIcons[i]->pchDistanceScaleGroup;
		F32 fMin = 0, fMax = 0;
		bool bSet = false;
		UIGenMapIcon *pIcon;
		int j, n;
		...
		if (!pchDistanceScaleGroup)
			continue;

		n = eaSize(&pState->eaIcons);

		// Determine origin
		if (pMap->bDistanceScaleFromPlayer)
			entGetPos(pEnt, v3Origin);
		else
			copyVec3(pState->v3WorldCenter, v3Origin);

		// Determine min & max distances for the group
		for (j = i; j < n; j++)
		{
			pIcon = pState->eaIcons[j];
			if (pchDistanceScaleGroup == pIcon->pchDistanceScaleGroup)
			{
				// Calculate X/Z distance from the center of the map gen
				F32 dx = pIcon->v3WorldPos[0] - v3Origin[0];
				F32 dz = pIcon->v3WorldPos[2] - v3Origin[2];
				pIcon->fCenterDistance = sqrtf(dx * dx + dz * dz);

				if (bSet)
				{
					MIN1(fMin, pIcon->fCenterDistance);
					MAX1(fMax, pIcon->fCenterDistance);
				}
				else
				{
					fMin = fMax = pIcon->fCenterDistance;
					bSet = true;
				}
			}
		}

		// Scale icons if there are multiple icons in the scale group
		for (j = i; bSet && j < n; j++)
		{
			pIcon = pState->eaIcons[j];
			if (pchDistanceScaleGroup == pIcon->pchDistanceScaleGroup)
			{
				F32 fBaseDistanceScale = 2 * fMin / (pIcon->fCenterDistance + fMin);
				pIcon->fDistanceScale = MAX(fBaseDistanceScale, pIcon->pDef->fMinScale);

				// Don't process icon again
				pIcon->pchDistanceScaleGroup = NULL;
			}
		}
	}
	...
	for (i = 0; i < eaSize(&pState->eaIcons); i++)
	{
		...
		{
			if (pLayer->bIgnoreWorldSize || (pIcon->v3WorldSize[0] == 0 && pIcon->v3WorldSize[2] == 0))
			{
				fScale = pLayer->bScaleToWorld ? pState->fPixelsPerWorldUnit / pIcon->pDef->fPixelsPerWorldUnit : pGen->fScale;
				fScale *= pIcon->pDef->fBaseScale * pLayer->fLayerScale * pIcon->fDistanceScale;

				if (pLayer->bScale)
				{
					F32 fZoomScale = pIcon->pDef->fScaleMultiplier * (1 - (pMap->eScaleMode == UIGenMapScalePixelsPerWorldUnit ? pState->fPixelsPerWorldUnit : pState->fMapScale) / pMap->fScaleMax);
					fScale *= MAX(pIcon->pDef->fMinScale, fZoomScale);
				}
				else if (pLayer->bShrinkToFit && (pIcon->pParentIcon || pIcon->v3WorldSize[0] != 0 && pIcon->v3WorldSize[2] != 0))
				{
					UIGenMapIconLayerDef *pParentLayer = pIcon->pParentIcon ? ui_GenMapGetIconLayer(pIcon->pParentIcon) : NULL;
					F32 fBoundWidth = pIcon->pParentIcon ? CBoxWidth(&pIcon->pParentIcon->ScreenBox) : pIcon->v3WorldSize[0];
					F32 fBoundHeight = pIcon->pParentIcon ? CBoxHeight(&pIcon->pParentIcon->ScreenBox) : pIcon->v3WorldSize[2];
					fScale *= GenMapShrinkToFit(pIcon, pLayer, fBoundWidth, fBoundHeight, !pParentLayer || pParentLayer->bRotate);
				}
			...
			{
				fScale = pState->fPixelsPerWorldUnit * pIcon->pDef->fBaseScale * pLayer->fLayerScale;
				fWidth = pIcon->v3WorldSize[0] * fScale;
				fHeight = pIcon->v3WorldSize[2] * fScale;
			}
		}

		if (pIcon->pParentIcon)
		{
			CBox *pParentBox = &pIcon->pParentIcon->ScreenBox;
			F32 fOffsetX = CBoxWidth(pParentBox) * pLayer->fPercentX;
			F32 fOffsetY = CBoxHeight(pParentBox) * pLayer->fPercentY;

			if (pLayer->eOffsetFrom & UILeft)
				v2ScreenPos[0] = pParentBox->lx + (fOffsetX + fWidth * 0.5f);
			else if (pLayer->eOffsetFrom & UIRight)
				v2ScreenPos[0] = pParentBox->hx - (fOffsetX + fWidth * 0.5f);
			else
				v2ScreenPos[0] = ((pParentBox->lx + pParentBox->hx) - fOffsetX) / 2;

			if (pLayer->eOffsetFrom & UITop)
				v2ScreenPos[1] = pParentBox->ly + (fOffsetY + fHeight * 0.5f);
			else if (pLayer->eOffsetFrom & UIBottom)
				v2ScreenPos[1] = pParentBox->hy - (fOffsetY + fHeight * 0.5f);
			else
				v2ScreenPos[1] = ((pParentBox->ly + pParentBox->hy) - fOffsetY) / 2;
		}
		else
		{
...
static void GenMiniMapDrawFakeZone(UIGen *pGen, UIGenMap *pMap, UIGenMapState *pState, UIGenMapFakeZone *pFakeZone)
{
	...
	if(pHighlight)
	{
		...
		if(pFakeZone->pHighlights->Marker.BaseLayer.pchIcon)
		{
			BasicTexture *pTexMarker;

			pTexMarker = texFindAndFlag(pFakeZone->pHighlights->Marker.BaseLayer.pchIcon, 1, WL_FOR_UI);
			...
			if(pTexMarker)
			{
				...
				{
					if (pFakeZone->pHighlights->Marker.BaseLayer.bRotate)
						display_sprite_mask_boxes_rotated(NULL, pTexMarker, NULL, NULL, &TexBox, &TexBox, UI_GET_Z(), ui_StyleColorPaletteIndex(pFakeZone->pHighlights->Marker.BaseLayer.uiColor), fRot);
					else
						display_sprite_box2(pTexMarker, &TexBox, UI_GET_Z(), ui_StyleColorPaletteIndex(pFakeZone->pHighlights->Marker.BaseLayer.uiColor));
				}
			}
		}
	}
...
static void GenMapSetIconIndexLabels(UIGenMapIcon **eaIcons)
{
	...
	for(i = 0; i < eaSize(&eaIcons); ++i)
	{
		...
		if(pCurrentIcon)
		{
			if(pCurrentIcon->iKeyIndex <= 0 || !pCurrentIcon->pchLabel || !pCurrentIcon->pchLabel[0] || pCurrentIcon->iLayer >= 0)
				continue;
...
{
	int dz = (*ppLeft)->iLayerZ - (*ppRight)->iLayerZ;
	if (dz != 0)
		return dz;

	// Sort any icons on the same player by distance scale (smaller icons behind larger icons)
	if ((*ppLeft)->fDistanceScale < (*ppRight)->fDistanceScale)
		return -1;
	if ((*ppLeft)->fDistanceScale > (*ppRight)->fDistanceScale)
		return 1;
	return 0;
...
void ui_GenDrawEarlyMap(UIGen *pGen)
{
	...
	if (!pState->pFakeZone || gConf.bShowMapIconsOnFakeZones)
	{
		...
		for (i = 0; i < eaSize(&pState->eaIcons); i++)
		{
			...
			if (pLayer->uiColor == 0)
			{
				if (pIcon->iLayer >= 0 && pIcon->pDef->BaseLayer.uiColor != 0)
					uiColor = ColorRGBAMultiplyAlpha(ui_StyleColorPaletteIndex(pIcon->pDef->BaseLayer.uiColor), pGen->chAlpha);
				else
					uiColor = ColorRGBAMultiplyAlpha(0xffffffff, pGen->chAlpha);
			}
			else
			{
				uiColor = ColorRGBAMultiplyAlpha(ui_StyleColorPaletteIndex(pLayer->uiColor), pGen->chAlpha);
			}
			...
			if (CBoxIntersects(&pGen->UnpaddedScreenBox, &pIcon->ScreenBox))
			{
				...
				if(bRevealed)
				{
					...
					F32 fZ = pLayer->iZ * fIconStepZ + fIconMinZ;
					bool bCenter = false;

					clipperPushRestrict(pLayer->bClip ? &pGen->ScreenBox : NULL);
					display_sprite_mask_boxes_rotated(
						pIcon->pTex, NULL, pLayer->bClip ? pState->pMask : NULL, NULL,
						&pIcon->ScreenBox, &pGen->ScreenBox, fZ,
						uiColor, pLayer->bRotate ? pIcon->fRotation : 0);

					// If this is an area rather than a point, put the annotation icon
					// into the center of it.
					if (!pLayer->bIgnoreWorldSize && pIcon->v3WorldSize[0] && pIcon->v3WorldSize[2])
					{
					...
					if (pLayer->bShowUpwardsAndDownwards)
					{
	...
	if (pTexAs)
	{
		ui_TextureAssemblyDraw(
			...
			fIconStepZ ? (pMap->pMapAssembly->iZ + 1) * fIconStepZ + fIconMinZ : fIconMaxZ, 
			pGen->chAlpha, 
