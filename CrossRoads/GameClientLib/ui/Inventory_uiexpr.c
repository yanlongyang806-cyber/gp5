...
#include "FCInventoryUI_h_ast.h"
#include "Inventory_uiexpr_h_ast.h"
...
typedef struct RewardPackDisplayData
{
	...
	//InventorySlot* pSlot; AST(UNOWNED) // inventory slots really serve no purpose here.  They just have items in them.
	Item* pItem; AST(UNOWNED)
	int iSpecialRewardStatus;
} RewardPackDisplayData;

// This structure is ONLY for reward packs
AUTO_STRUCT;
typedef struct UIItemRewardPackData
{
	REF_TO(ItemDef) hRewardPackItem;
	ItemQuality ePackResultQuality;
	UIItemRewardGrantData* pRewards;
	ItemRewardPackType eRewardPackType;
} UIItemRewardPackData;

//////////////////////////////////////////////////////////////////////////////////////////////////////
//////////

static SavedPetDisplayData **s_eaSavedPetDisplayData;
static UIItemRewardPackData **s_eaRewardPackData = NULL;
static ItemRewardPackRequest *s_RewardPackRequest = NULL;
...
// For debugging untranslated item names.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GenGetItemNameMessageKey);
const char *GenExprGetItemNameMessageKey(ExprContext* pContext, SA_PARAM_OP_VALID Item *pItem)
{
	Entity *entPlayer = entActiveOrSelectedPlayer();
	const char *pcName = "";
	if (pItem && entPlayer && GET_REF(pItem->hItem))
	{
		ItemDef *pItemDef = GET_REF(pItem->hItem);
		Message *pMessage = GET_REF(pItemDef->displayNameMsg.hMessage);
		if( pMessage )
		{
			pcName = pMessage->pcMessageKey;
		}
	}
	return pcName;
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(RequestItemPackRewardData);
void RequestItemPackRewardData(SA_PARAM_OP_VALID Item *pItem)
{
	StructDestroy(parse_ItemRewardPackRequest, s_RewardPackRequest);
	s_RewardPackRequest = NULL;

	if(pItem)
	{
		ServerCmd_RequestItemPackRewardData(pItem->id);
	}
}
	...
	for (i = 0; i < eaSize(&eaRewardBags); i++)
	{
			...
			inv_bag_CountItems(pRewardBag, NULL) > 0 && pRewardBag->pRewardBagInfo->NumPicks > 0)
		{
			...
			if (eaSize(&s_eaItems) > 0)
			{
				...
				StructDestroySafe(parse_Item, &pData->pItemCopy);
				pData->pItem = NULL;
				pData->iBagIdx = i;
			...
			for (j = 0; j < eaSize(&s_eaItems); j++)
			{
				...
				if (StructCompare(parse_Item, pData->pItemCopy, s_eaItems[j], 0, 0, 0) != 0)
				{
					StructDestroySafe(parse_Item, &pData->pItemCopy);
					pData->pItemCopy = StructClone(parse_Item, s_eaItems[j]);
				}
				pData->pItem = pData->pItemCopy;
				pData->iBagIdx = i;
				pData->iNumPicks = pRewardBag->pRewardBagInfo->NumPicks;
				pData->bSelected = false;
			}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GetItemChoicesFromRewardPack);
void GetItemChoicesFromRewardPack(SA_PARAM_NN_VALID UIGen *pGen)
{
	...
	GetSelectedChoosableItems(peaData, &s_eaChosenItemDefs);

	if(s_RewardPackRequest)
		iCount = GetChoosableItemList(s_RewardPackRequest->eaRewardBags, peaData);
	else
		iCount = 0;
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ClaimRewardPackWithChoices);
void ClaimrewardPackWithChoices(SA_PARAM_NN_VALID UIGen *pGen,const char *pchItemKey)
{
	Entity *pPlayer = entActivePlayerPtr();
	ChoosableItem ***peaData = ui_GenGetManagedListSafe(pGen, ChoosableItem);
	ContactRewardChoices *pChoices = StructCreate(parse_ContactRewardChoices);
	BagIterator *pIter = NULL;
	int i;

	UIInventoryKey Key = {0};
	gclInventoryParseKey(pchItemKey, &Key);
	...
	for(i=0;i<eaSize(peaData);i++)
	{
		if((*peaData)[i]->bSelected == true)
		{
			eaPush(&pChoices->ppItemNames,StructAllocString(REF_STRING_FROM_HANDLE((*peaData)[i]->hItemDef)));
		}
	}

	ServerCmd_ItemRewardPack_OpenWithChoices(pPlayer->myEntityType,pPlayer->myContainerID,Key.eBag,Key.iSlot,pChoices);
}

AUTO_COMMAND ACMD_CLIENTCMD ACMD_PRIVATE ACMD_ACCESSLEVEL(0);
void ReceiveItemPackRewardData(ItemRewardPackRequest *pData)
{
	if(s_RewardPackRequest)
		StructDestroy(parse_ItemRewardPackRequest, s_RewardPackRequest);

	s_RewardPackRequest = StructClone(parse_ItemRewardPackRequest,pData);
}
									...
									bool* pbAddedTitle, char** pestrResult)
{
	...
	if(pPowerDef) {
		...
		powerdef_AutoDesc(entGetPartitionIdx(pEnt), langGetCurrent(), pPowerDef, NULL, pAutoDesc, NULL, NULL, NULL, NULL, NULL, NULL, 1, true, kAutoDescDetail_Normal, pExtract, NULL);
		powerdef_AutoDescCustom(pEnt, langGetCurrent(), &s_estrPower, pPowerDef, pAutoDesc, pchPowerMessageKey, pchAttribMessageKey);
				...
				if(pNode && pNode->ppRanks && powertree_NodeHasPropagationPowers(pNode) == bPropagated) {
					...
					if(pPowerDef) {
						...
						if(pEnt) {
							...
							powerdef_AutoDesc(entGetPartitionIdx(pEnt), langGetCurrent(), pPowerDef, NULL, pAutoDesc, NULL, NULL, NULL, NULL, NULL, NULL, 1, true, kAutoDescDetail_Normal, pExtract, NULL);
							powerdef_AutoDescCustom(pEnt, langGetCurrent(), pestrResult, pPowerDef, pAutoDesc, pchPowerMessageKey, pchAttribMessageKey);
						}
						if(pAutoDesc) {
	...
	for(iPower=0; iPower< iNumPowers; iPower++)
	{
		...
		if(pItemPower && !bRollFailed)
		{
			...
			if (pPowerDef && (pPowerDef->eType == kPowerType_Click || pPowerDef->eType == kPowerType_Toggle || pPowerDef->eType == kPowerType_Instant))
			{
				...
				(pItemDef->eType == kItemType_Device || pItemDef->eType == kItemType_Upgrade || pItemDef->eType == kItemType_Weapon || pItemDef->eType == kItemType_Gem))
			{
...
const char* GenExprGetItemInspect(ExprContext* context, SA_PARAM_OP_VALID Item *pItem, int iMaxPowers)
{
	...
	if ( pItem && pItemDef )
	{
		...
		estrClear(&tmpS);
		estrPrintf(&tmpS, "<font style=Game_HUD color=white outline=0>" );
		Item_PowerAutoDescCustom(entActiveOrSelectedPlayer(), langGetCurrent(), pItem, &tmpS, "", "", 0);
		estrAppend2(&InspectText,tmpS);
		...
		if (iDstBagIdx == InvBagIDs_None)
		{
			...
			else 
			{
				...
				else
				{
					...
					iDstBagIdx = pDef ? GetBestBagForItemDef(pEnt, pDef, pItem, iCount, false, pExtract) : InvBagIDs_None;
...
const char* GenExprGetFormattedItemInfo(ExprContext* pContext, SA_PARAM_OP_VALID Item *pItem, SA_PARAM_OP_VALID Entity *pEnt, ACMD_EXPR_DICT(Message) const char *pchDescriptionKey, S32 eActiveGemSlotType)
{
	...
	if ( pItem && pItemDef && pChar )
	{
		...
		if (pItem)
		{
			...
			Item_InnatePowerAutoDesc(entActiveOrSelectedPlayer(), langGetCurrent(), pItem, &estrItemInnatePowerAutoDesc, eActiveGemSlotType);
			Item_PowerAutoDescCustom(entActiveOrSelectedPlayer(), langGetCurrent(), pItem, &estrItemPowerAutoDesc, "", "", 0);
		...
		{
			Item_GetItemCategoriesString(langGetCurrent(), pItemDef, NULL, &estrItemCategories);
		}
...
// DEPRECATED.  See gclItemExprGetCompareKeys() and gclItemExprGetCompareKeysSimilarPowerCategories().
	...
	if ( iSrcBagID != InvBagIDs_Loot )
	{
		...
		if ( pItemDef )
		{
			...
			if ( iDstBagID <= InvBagIDs_None )
				iDstBagID = GetBestBagForItemDef(pEntDst, pItemDef, pItem, tempCount, false, pExtractDst);
			if(bNoSwap) {
...
SA_RET_OP_VALID Entity *STOFindDescriptionEntity_GetGivenEnt(PuppetMaster *pPuppetMaster, GlobalType type, ContainerID id, InvBagIDs eBag, GameAccountDataExtract *pExtract)
{
	int i;
	for(i=eaSize(&pPuppetMaster->ppPuppets)-1; i>=0; i--)
	{
		Entity *pEntity = GET_REF(pPuppetMaster->ppPuppets[i]->hEntityRef);
		if (pEntity 
			&& entGetType(pEntity) == type 
			&& entGetContainerID(pEntity) == id
			&& inv_GetBag(CONTAINER_NOCONST(Entity, pEntity), eBag, pExtract))
		{
			return pEntity;
		}
	}
...
SA_RET_OP_VALID Entity *STOFindDescriptionEntity_GetEntWithItem(PuppetMaster *pPuppetMaster, U64 uiItemID)
{
	int i;
	if (uiItemID)
	{
		for(i=eaSize(&pPuppetMaster->ppPuppets)-1; i>=0; i--)
		{
			Entity *pEntity = GET_REF(pPuppetMaster->ppPuppets[i]->hEntityRef);
			if (pEntity 
				&& !(entGetType(pEntity) == pPuppetMaster->curType && entGetContainerID(pEntity) == pPuppetMaster->curID)
				&& inv_GetItemByID(pEntity, uiItemID))
			{
				return pEntity;
			}
		}
...
Entity *STOFindDescriptionEntity_GetBestEnt(PuppetMaster *pPuppetMaster, InvBagIDs eBag, CharClassCategorySet *pSet, GameAccountDataExtract *pExtract)
{
	Entity *pBackupEntity = NULL;
	int i;
	for(i = eaSize(&pPuppetMaster->ppPuppets)-1; i >= 0; i--)
	{
		PuppetEntity *pPuppet = pPuppetMaster->ppPuppets[i];
		Entity *pEntity = GET_REF(pPuppet->hEntityRef);
		if (pEntity 
			&& !(entGetType(pEntity) == pPuppetMaster->curType && entGetContainerID(pEntity) == pPuppetMaster->curID)
			...
			&& inv_GetBag(CONTAINER_NOCONST(Entity, pEntity), eBag, pExtract))
		{
			...
			{
				pBackupEntity = pEntity;
			}
		}
	}
	return pBackupEntity;
}
...
//  for now.
// This fucntion has gotten kind of messy, so here's what it's supposed to check
//  1) If there is a preferred puppet, use that one. This is for when you have 
//  the ship status window open, you want the description to reflect the ship
//  you're looking at
//  2) Next check the active puppet. If you're a ship, for example, you want the
//  tooltip to reflect the stats if it applied to your current ship.
//  3) If the active puppet can't slot this item, look for the puppet holding the
//  item. If one is holding it, use that puppet
//  4) Finally, loop through the all puppets and look for anyone who can slot it
//  at all, preferring the one that is in the preferred CharClassCategorySet
SA_RET_NN_VALID static Entity *STOFindDescriptionEntityEx(SA_PARAM_NN_VALID Entity *pMaster, bool bPreferGivenPuppet, GlobalType iPuppetType, ContainerID iPuppetID, SA_PARAM_OP_VALID ItemDef *pItemDef, U64 uiItemID)
{
	if(pMaster->pSaved && pMaster->pSaved->pPuppetMaster && pItemDef && eaiGet(&pItemDef->peRestrictBagIDs, 0))
	{
		...
		if (bPreferGivenPuppet)
		{
			if (pPuppetMaster->curType == iPuppetType && pPuppetMaster->curID == iPuppetID)
				pActivePuppet = pMaster;
			else
				pActivePuppet = STOFindDescriptionEntity_GetGivenEnt(pPuppetMaster, iPuppetType, iPuppetID, eBag, pExtract);
		}

		if (!pActivePuppet && inv_GetBag(CONTAINER_NOCONST(Entity, pMaster),eBag,pExtract))
		{
			pActivePuppet = pMaster;
		}
		...
		if (!pActivePuppet)
		{
			pActivePuppet = STOFindDescriptionEntity_GetEntWithItem(pPuppetMaster, uiItemID);
		}

		if (!pActivePuppet)
		{
			pActivePuppet = STOFindDescriptionEntity_GetBestEnt(pPuppetMaster, eBag, CharClassCategorySet_getPreferredSet(pMaster), pExtract);
		}
...
{
	return STOFindDescriptionEntityEx(pMaster, 0, 0, 0, pItemDef, uiItemID);
}
									...
									const char* pchAttribModsMessageKey)
{
	...
	for(i = 0; i < len; i++) {
		...
		StructReset(parse_AutoDescPower, pAutoDesc);
		power_AutoDesc(entGetPartitionIdx(pEnt), langGetCurrent(), pPow, pEnt->pChar, NULL, pAutoDesc, NULL, NULL, NULL, false, -1, entGetPowerAutoDescDetail(entActiveOrSelectedPlayer(), true), pExtract,NULL);
		powerdef_AutoDescCustom(pEnt, langGetCurrent(), &estrPowerDesc, pPowerDef, pAutoDesc, pchPowerMessageKey, pchAttribModsMessageKey);
...
char* GenExprGetItemPowerDescByType(ExprContext *pContext,
									SA_PARAM_OP_VALID Entity* pEnt,
									bool bPreferGivenPuppet,
									int iPuppetType,
									int iPuppetID,
									...
									bool bAllExceptType)
{
	...
	if (pItem)
	{
		...
		if(eaPowers) {
			ItemDef *pItemDef = GET_REF(pItem->hItem);
			Entity *pentDesc = STOFindDescriptionEntityEx(pEnt, bPreferGivenPuppet, iPuppetType, iPuppetID, pItemDef, pItem->id); //pEnt;
		...
		*bIsAlgoItemRecipe = *bIsItemRecipe && SAFE_MEMBER(pItemDef, Group) != 0;
		if (*bIsItemRecipe)
		{
			...
			else if (pItemDef->eType == kItemType_ItemPowerRecipe)
			{
				...
				{
					AutoDescPower* pAutoDesc = StructCreate(parse_AutoDescPower);
					powerdef_AutoDesc(entGetPartitionIdx(pEntity), langGetCurrent(), pResultPower, NULL, pAutoDesc, NULL, NULL, NULL, NULL, NULL, NULL, 1, true, kAutoDescDetail_Normal, pExtract, NULL);
					powerdef_AutoDescCustom(pEntity, langGetCurrent(), pestrItemPowerRecipe, pResultPower, pAutoDesc, pchPowerKey, pchModKey);
					StructDestroy(parse_AutoDescPower, pAutoDesc);
				}
			}
		}
...
//		char *estrInfuseDesc = NULL;
		...
		Item_InnatePowerAutoDesc(entActiveOrSelectedPlayer(), langGetCurrent(), pItem, &estrItemInnatePowerAutoDesc, 0);
		Item_PowerAutoDescCustom(entActiveOrSelectedPlayer(), langGetCurrent(), pItem, &estrItemPowerAutoDesc, pchPowerKey, pchModKey, 0);

//		estrStackCreate(&estrInfuseDesc);
//		GetItemDescriptionInfuse(pItem,pItemDef,&estrInfuseDesc);

		estrStackCreate(&estrItemSetDesc);
		GetItemDescriptionItemSet(&estrItemSetDesc,langGetCurrent(),pItemDef,pItem,pEntity,pchPowerKey,pchModKey,"Inventory_ItemInfo_ItemSetFormat",pExtract);
											...
											ACMD_EXPR_DICT(Message) const char* pchItemSetFormatKey)
{
	...
	if (pItemSetDef && eaSize(&pItemSetDef->ppItemSetMembers) > 0)
	{
		GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEntity);
		estrStackCreate(&estrItemSetDesc);
		GetItemDescriptionFromItemSetDef(&estrItemSetDesc,langGetCurrent(),pItemSetDef,pEntity,pchPowerKey,pchModKey,pchItemSetFormatKey,pExtract);
	}
										 ...
										 ACMD_EXPR_DICT(Message) const char* pchItemSetFormatKey)
{
	...
	if (pItemDef)
	{
		...
		estrStackCreate(&estrItemSetDesc);
		GetItemDescriptionItemSet(&estrItemSetDesc, langGetCurrent(), pItemDef, pItem, pEntity, pchPowerKey, pchModKey, pchItemSetFormatKey, pExtract);
...
bool gclInvExprInventoryFindItemMoveToDest(S32 eSrcBag, S32 eDstBag, SA_PARAM_NN_VALID Item *pSearchItem, S32 count)
{
	...
	if (pEnt && pSearchItem)
	{
		...
		if (pchItemDefName)
		{
			...
			if (inv_bag_trh_FindItemByDefName(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, (pEnt)), eSrcBag, pchItemDefName, 0, pIter))
			{
				if (eDstBag == InvBagIDs_None)
				{
					...
					//equip on items in inventory has behavior based on item type
					Item *pItem = CONTAINER_RECONST(Item, bagiterator_GetItem(pIter));
					ItemDef *pDef = pItem ? GET_REF(pSearchItem->hItem) : NULL;
					eDstBag = pDef ? GetBestBagForItemDef(pEnt, pDef, pItem, count, false, pExtract) : InvBagIDs_None;
...
const char* GenExprItemDefGetCategoryDisplayNamesByPrefix(SA_PARAM_OP_VALID ItemDef* pItemDef, const char* pchPrefix)
{
	...
	Item_GetItemCategoriesString(langGetCurrent(), pItemDef, pchPrefix, &s_estrItemCategories);
	return s_estrItemCategories;
}

AUTO_COMMAND ACMD_CLIENTCMD ACMD_PRIVATE ACMD_ACCESSLEVEL(0);
void gclReceiveRewardPackData(ItemRewardPackTransferData* pRewardPackData)
{
	Entity* pEnt = entActiveOrSelectedPlayer();
	if (pEnt && pRewardPackData && pRewardPackData->pRewards)
	{
		UIItemRewardPackData* pData = StructCreate(parse_UIItemRewardPackData);
		COPY_HANDLE(pData->hRewardPackItem, pRewardPackData->hRewardPackItem);
		pData->pRewards = StructCreate(parse_UIItemRewardGrantData);
		gclInvFillUIItemRewardGrantData(pEnt, pRewardPackData->pRewards, pData->pRewards, false);
		pData->ePackResultQuality = pRewardPackData->ePackResultQuality;
		pData->eRewardPackType = pRewardPackData->eRewardPackType;

		eaPush(&s_eaRewardPackData, pData);
	}
}

static int compareRewardPackDisplayData(const RewardPackDisplayData **ppFirst, const RewardPackDisplayData **ppSecond)
{
	const RewardPackDisplayData *pFirst = (ppFirst ? *ppFirst : NULL);
	const RewardPackDisplayData *pSecond = (ppSecond ? *ppSecond : NULL);

	if(pFirst && pSecond)
	{
		int iResult;

		// header should remain first (That's the only reason I currently know why an item would be NULL)
		if (pFirst->pItem == NULL)
			return -1;
		if (pSecond->pItem == NULL)
			return 1;

		// sort by "special" class first (for now, higher numbers go first)
		iResult = pSecond->iSpecialRewardStatus-pFirst->iSpecialRewardStatus;
		if (iResult != 0)
		{
			return iResult;
		}
		...
		if(pFirst->pItem && pSecond->pItem)
		{
			...
			{
				iResult = pSecondDef->Quality - pFirstDef->Quality;
		...
		// not sure what meaningful thing this accomplishes (was previously checking slot name which should always have been blank)
		return stricmp(pFirst->pchItemDisplayName, pSecond->pchItemDisplayName);
	}
...
static void gclFillRewardPackData(UIItemRewardPackData* pRewardPackData, 
								  RewardPackDisplayData*** peaData, 
								  S32* piCount, 
								  bool bShowRewardPackHeaders,
								  bool bSort)
{
	ItemDef* pRewardPackDef = GET_REF(pRewardPackData->hRewardPackItem);
	UIItemRewardGrantData* pRewards = pRewardPackData->pRewards;
	...
	if (pRewards && eaSize(&pRewards->eaRewards))
	{
		...
		for (i = 0; i < eaSize(&pRewards->eaRewards); i++)
		{
			...
			pData->pItem = pRewards->eaRewards[i]->pItem;
			pData->iSpecialRewardStatus = pRewards->eaRewards[i]->GrantData.iSpecialRewardStatus;
		}

		if(bSort)
			eaQSort(*peaData, compareRewardPackDisplayData);
	}
}
...
void GenExprGetRewardPackDisplayData(SA_PARAM_NN_VALID UIGen* pGen, S32 iRewardPackIndex)
{
	...
	{
		UIItemRewardPackData* pRequestData = eaGet(&s_eaRewardPackData, iRewardPackIndex);
		if (pRequestData)
		{
	...
	else
	{
		...
		{
			UIItemRewardPackData* pRequestData = s_eaRewardPackData[iRewardPackIndex];
...
S32 GenExprRewardPackListCountAndRemoveItem(SA_PARAM_NN_VALID UIGen* pGen, const char* pchName)
{
	...
	{
		if ((*peaData)[i]->pItem && REF_STRING_FROM_HANDLE((*peaData)[i]->pItem->hItem) == allocFindString(pchName))
		{
			iRet += (*peaData)[i]->pItem->count;
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GetRewardPackType);
S32 GenExprGetRewardPackItemDefType(S32 iRewardPackIndex)
{
	UIItemRewardPackData* pRequestData = eaGet(&s_eaRewardPackData, iRewardPackIndex);
	if (pRequestData)
	{
		return pRequestData->eRewardPackType;
	}

	return kItemRewardPackType_Undefined;
}
...
{
	eaDestroyStruct(&s_eaRewardPackData, parse_UIItemRewardPackData);
}
...
{
	ItemDef *pDef = pItem ? GET_REF(pItem->hItem) : NULL;
	return pDef && item_CanDiscard(entActiveOrSelectedPlayer(),pDef,false) && item_CanRemoveItem(pItem);
}
...
bool exprItemCanTransmutateTo(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Item* pMainItem, SA_PARAM_OP_VALID Item* pTransmutateToItem)
{
	...
	return pMainItemDef && pTransmutateToItemDef && item_trh_CanTransMutateTo(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEnt), CONTAINER_NOCONST(Item, pMainItem), pMainItemDef, CONTAINER_NOCONST(Item, pTransmutateToItem), pTransmutateToItemDef);
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenGetItemTransmuteSourceListForItem");
void exprGetItemTransmuteSourceListForItem(SA_PARAM_NN_VALID UIGen* pGen, SA_PARAM_OP_VALID Entity* pEnt, SA_PARAM_OP_VALID Item* pMainItem)
{
	TradeSlotLite ***peaSlots = ui_GenGetManagedListSafe(pGen, TradeSlotLite); // TODO: Change the type, maybe?

	S32 iCount = 0;
	if (pEnt && pEnt->pInventoryV2 && pMainItem && GET_REF(pMainItem->hItem))
	{
		ItemDef *pMainItemDef = GET_REF(pMainItem->hItem);
		S32 j;
		for (j = 0; j < eaSize(&pEnt->pInventoryV2->ppInventoryBags); j++)
		{
			...
			for (i = 0; i < eaSize(&pBag->ppIndexedInventorySlots); i++)
			{
				...
				if (pSlot->pItem)
				{
					...
					if (exprItemCanTransmutateTo(pEnt, pMainItem, pSlot->pItem))
					{
						TradeSlotLite *pTrade = eaGetStruct(peaSlots, parse_TradeSlotLite, iCount++); // TODO: change the type
						pTrade->count = pSlot->pItem->count;
						pTrade->SrcSlot = i;
	...
	while (eaSize(peaSlots) > iCount)
		StructDestroy(parse_TradeSlotLite, eaPop(peaSlots)); // TODO: Change the type

	ui_GenSetManagedListSafe(pGen, peaSlots, TradeSlotLite, true); // TODO: Change the type
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ItemHasRewardChoices);
bool ItemExprHasRewardChoice(SA_PARAM_OP_VALID Item* pItem)
{
	if(pItem)
	{
		ItemDef *pDef = GET_REF(pItem->hItem);

		if(pDef && item_HasRewardPackChoices(pDef))
		{
			return true;
		}
	}
...
bool exprUseNextItemOfName(SA_PARAM_OP_VALID Entity *pEntity, S32 eBagID, const char *pchName)
{
	...
	if(pEntity && pchName)
	{
		...
		if (eBagID == InvBagIDs_Inventory)//Search overflow + player bags too if we're looking in "inventory"
			bFound = inv_bag_FindItem(pEntity, eBagID, pIter, kFindItem_ByName, pchName, true, false);
		else
			bFound = inv_bag_FindItem(pEntity, eBagID, pIter, kFindItem_ByName, pchName, false, false);
		if (bFound)
		{
			bRet = gclInvExprInventoryIndexBestAction(bagiterator_GetCurrentBagID(pIter), bagiterator_GetSlotID(pIter));
		}
		bagiterator_Destroy(pIter);
	}
	return bRet;
...
const char* exprFindInventoryKeyByItemName(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity, S32 eBagID, SA_PARAM_OP_STR const char *pchItemName)
{
	...
	if(pEntity && pchItemName)
	{
		...
		bool bFound = false;
		if (eBagID == InvBagIDs_Inventory)//Search overflow + player bags too if we're looking in "inventory"
			bFound = inv_bag_FindItem(pEntity, eBagID, pIter, kFindItem_ByName, pchItemName, true, false);
		else
			bFound = inv_bag_FindItem(pEntity, eBagID, pIter, kFindItem_ByName, pchItemName, false, false);
		if (bFound)
		{
			UIInventoryKey Key = {0};
			...
			{
				pchRet = gclInventoryMakeKeyString(pContext, &Key);
			}
		}
		bagiterator_Destroy(pIter);
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(FindInventoryKeyByItemType);
const char* exprFindInventoryKeyByItemType(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity, S32 eBagID, SA_PARAM_OP_STR const char *pchItemType)
{
	const char* pchKey = "";
	if(pEntity && pchItemType)
	{
		ItemType eItemType = StaticDefineInt_FastStringToInt(ItemTypeEnum, pchItemType, -1);
		BagIterator *pIter = bagiterator_Create();
		bool bFound = false;
		if (eBagID == InvBagIDs_Inventory)//Search overflow + player bags too if we're looking in "inventory"
			bFound = inv_bag_FindItem(pEntity, eBagID, pIter, kFindItem_ByType, (void*)eItemType, true, false);
		else
			bFound = inv_bag_FindItem(pEntity, eBagID, pIter, kFindItem_ByType, (void*)eItemType, false, false);
		if (bFound)
		{
			UIInventoryKey Key = {0};
			if( gclInventoryMakeSlotKey(pEntity, CONTAINER_RECONST(InventoryBag, bagiterator_GetCurrentBag(pIter)), bagiterator_GetSlot(pIter), &Key) )
...
#include "Inventory_uiexpr_h_ast.c"
