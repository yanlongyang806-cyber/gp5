												...
												const char* pchTrainNode, bool bCheckCost)
{
	...
	if ( eReturn = Officer_GetTrainReturnValue(PARTITION_CLIENT,pPlayerEnt,pTrainer,entGetContainerID(pOfficer),pchOldNode,pchTrainNode,bCheckCost,false,
												allegiance_GetOfficerPreference(GET_REF(pPlayerEnt->hAllegiance),GET_REF(pPlayerEnt->hSubAllegiance)), NULL) )
	{
		U32 uiSize;
		estrCreate( &pcMessageKey );
												...
												const char* pchPowerTrees)
{
	...
	for ( i = 0; i < eaSize(&s_eaCategories); i++ )
	{
		...
		{
			if(pCategory->eaNodeList[j] && pCategory->eaNodeList[j] != &EmptyListNode)
			{
				// If these aren't destroyed when we null then we will leak memory
				StructDestroySafe(parse_PowerListNode, &pCategory->eaNodeList[j]);
			}
