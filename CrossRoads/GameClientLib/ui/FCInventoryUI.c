...
AUTO_STRUCT;
typedef struct UIItemBagCount
{
	InvBagIDs eBagID; AST(KEY)
	InventorySlot **eaSlots; AST(UNOWNED)
	InventorySlotLite **eaSlotLites; AST(UNOWNED)
	S32 iCount;
} UIItemBagCount;

AUTO_STRUCT;
typedef struct UIItemCount
{
	REF_TO(ItemDef) hItemDef;
	S32 iCount;
	UIItemBagCount **eaBagCounts;
} UIItemCount;

AUTO_STRUCT;
typedef struct UIEntityItemCounts
{
	U32 uLastUpdateTime;
...
static StashTable s_stEntityItemCounts;
...
void FCInventoryUI_Startup(void)
{
	if(!gbNoGraphics)
	{
		...
		ui_GenInitStaticDefineVars(ItemRewardPackTypeEnum, "ItemRewardPack_");
...
void gclInventoryOncePerFrame(void)
{
	...
	if (s_stEntityItemCounts)
	{
		stashGetIterator(s_stEntityItemCounts, &it);

		while (stashGetNextElement(&it, &pElem))
		{
			UIEntityItemCounts *pEntry = stashElementGetPointer(pElem);
			if (!pEntry || pEntry->uLastUpdateTime != gGCLState.totalElapsedTimeMs)
				eaPush(&s_eaRemoved, stashElementGetKey(pElem));
		}
		...
		if (s_eaRemoved)
		{
			while (eaSize(&s_eaRemoved) > 0)
			{
				UIEntityItemCounts *pEntry;
				if (stashRemovePointer(s_stEntityItemCounts, eaPop(&s_eaRemoved), &pEntry) && pEntry)
					StructDestroy(parse_UIEntityItemCounts, pEntry);
			}
			eaDestroy(&s_eaRemoved);
		}
	}
...
static void gclInventoryCountUpdate(Entity *pEntity)
{
	UIEntityItemCounts *pEntityItemCounts;
	UIItemBagCount *pBagCount;
	UIItemCount *pItemCount;
	InventoryBagLite *pBagLite;
	InventoryBag *pBag;
	StashTableIterator itItem;
	StashElement pElemItem;
	const char **eapchRemove = NULL;
	const char *pchItemDef;
	S32 i, j;
	...
	if (!s_stEntityItemCounts)
		s_stEntityItemCounts = stashTableCreateAddress(16);

	if (!stashAddressFindPointer(s_stEntityItemCounts, pEntity, &pEntityItemCounts))
	{
		pEntityItemCounts = StructCreate(parse_UIEntityItemCounts);
		pEntityItemCounts->pEnt = pEntity;
		pEntityItemCounts->stItems = stashTableCreateAddress(256);
		stashAddressAddPointer(s_stEntityItemCounts, pEntity, pEntityItemCounts, false);
	}

	if (pEntityItemCounts->uLastUpdateTime == gGCLState.totalElapsedTimeMs)
		return;

	pEntityItemCounts->pEnt = pEntity;

	// Clear current slot lists
	stashGetIterator(pEntityItemCounts->stItems, &itItem);
	while (stashGetNextElement(&itItem, &pElemItem))
	{
		pItemCount = stashElementGetPointer(pElemItem);
		for (i = eaSize(&pItemCount->eaBagCounts) - 1; i >= 0; i--)
		{
			pBagCount = pItemCount->eaBagCounts[i];
			eaClearFast(&pBagCount->eaSlotLites);
			eaClearFast(&pBagCount->eaSlots);
		}
	}

	if (pEntity->pInventoryV2)
	{
		// Add items from inventory bags
		for (i = eaSize(&pEntity->pInventoryV2->ppInventoryBags) - 1; i >= 0; i--)
		{
			pBag = pEntity->pInventoryV2->ppInventoryBags[i];
			for (j = eaSize(&pBag->ppIndexedInventorySlots) - 1; j >= 0; j--)
			{
				if (!pBag->ppIndexedInventorySlots[j]->pItem)
					continue;

				pchItemDef = REF_STRING_FROM_HANDLE(pBag->ppIndexedInventorySlots[j]->pItem->hItem);
				if (!pchItemDef)
					// a sign of item corruption
					continue;

				if (!stashAddressFindPointer(pEntityItemCounts->stItems, pchItemDef, &pItemCount))
				{
					pItemCount = StructCreate(parse_UIItemCount);
					eaIndexedEnable(&pItemCount->eaBagCounts, parse_UIItemBagCount);
					SET_HANDLE_FROM_STRING("ItemDef", pchItemDef, pItemCount->hItemDef);
					stashAddressAddPointer(pEntityItemCounts->stItems, pchItemDef, pItemCount, false);
				}

				pBagCount = eaIndexedGetUsingInt(&pItemCount->eaBagCounts, pBag->BagID);
				if (!pBagCount)
				{
					pBagCount = StructCreate(parse_UIItemBagCount);
					pBagCount->eBagID = pBag->BagID;
					eaIndexedEnable(&pBagCount->eaSlots, parse_InventorySlot);
					eaIndexedEnable(&pBagCount->eaSlotLites, parse_InventorySlotLite);
					eaIndexedAdd(&pItemCount->eaBagCounts, pBagCount);
				}

				eaIndexedAdd(&pBagCount->eaSlots, pBag->ppIndexedInventorySlots[j]);
			}
		}

		// Add items from lite bags
		for (i = eaSize(&pEntity->pInventoryV2->ppLiteBags) - 1; i >= 0; i--)
		{
			pBagLite = pEntity->pInventoryV2->ppLiteBags[i];
			for (j = eaSize(&pBagLite->ppIndexedLiteSlots) - 1; j >= 0; j--)
			{
				pchItemDef = REF_STRING_FROM_HANDLE(pBagLite->ppIndexedLiteSlots[j]->hItemDef);
				if (!pchItemDef)
					// a sign of item corruption
					continue;
				...
				pBagCount = eaIndexedGetUsingInt(&pItemCount->eaBagCounts, pBagLite->BagID);
				if (!pBagCount)
				{
					pBagCount = StructCreate(parse_UIItemBagCount);
					pBagCount->eBagID = pBagLite->BagID;
					eaIndexedEnable(&pBagCount->eaSlots, parse_InventorySlot);
					eaIndexedEnable(&pBagCount->eaSlotLites, parse_InventorySlotLite);
					eaIndexedAdd(&pItemCount->eaBagCounts, pBagCount);
				}

				eaIndexedAdd(&pBagCount->eaSlotLites, pBagLite->ppIndexedLiteSlots[j]);
			}
		}
	}
	...
	while (stashGetNextElement(&itItem, &pElemItem))
	{
		...
		for (i = eaSize(&pItemCount->eaBagCounts) - 1; i >= 0; i--)
		{
			pBagCount = pItemCount->eaBagCounts[i];
			pBagCount->iCount = 0;
			for (j = eaSize(&pBagCount->eaSlots) - 1; j >= 0; j--)
				pBagCount->iCount += pBagCount->eaSlots[j]->pItem ? pBagCount->eaSlots[j]->pItem->count : 0;
			for (j = eaSize(&pBagCount->eaSlotLites) - 1; j >= 0; j--)
				pBagCount->iCount += pBagCount->eaSlotLites[j] ? pBagCount->eaSlotLites[j]->count : 0;
			if (!pBagCount->iCount)
				StructDestroy(parse_UIItemBagCount, eaRemove(&pItemCount->eaBagCounts, i));
			else
				pItemCount->iCount += pBagCount->iCount;
		}
		if (!pItemCount->iCount)
			eaPush(&eapchRemove, stashElementGetKey(pElemItem));
	}
	for (i = eaSize(&eapchRemove) - 1; i >= 0; i--)
	{
		if (stashAddressRemovePointer(pEntityItemCounts->stItems, eapchRemove[i], &pItemCount))
			StructDestroy(parse_UIItemCount, pItemCount);
	}
	eaDestroy(&eapchRemove);

	pEntityItemCounts->uLastUpdateTime = gGCLState.totalElapsedTimeMs;
}

S32 gclInventoryItemCount(SA_PARAM_OP_VALID Entity *pEntity, const char *pchItemDef)
{
	UIEntityItemCounts *pEntityItemCounts;
	UIItemCount *pItemCount;

	gclInventoryCountUpdate(pEntity);
	if (!s_stEntityItemCounts)
		return 0;
	...
	if (!stashAddressFindPointer(pEntityItemCounts->stItems, allocAddString(pchItemDef), &pItemCount))
		return 0;

	return pItemCount->iCount;
}

AUTO_FIXUPFUNC;
TextParserResult UIItemBagCountFixup(UIItemBagCount *pItemBagCount, enumTextParserFixupType eType, void *pExtraData)
{
	switch (eType)
	{
	case FIXUPTYPE_DESTRUCTOR:
		if (pItemBagCount)
		{
			eaDestroy(&pItemBagCount->eaSlots);
			eaDestroy(&pItemBagCount->eaSlotLites);
		}
		break;
	}
...
AUTO_FIXUPFUNC;
TextParserResult UIEntityItemCountsFixup(UIEntityItemCounts *pEntityItemCounts, enumTextParserFixupType eType, void *pExtraData)
{
	switch (eType)
	{
	case FIXUPTYPE_DESTRUCTOR:
		if (pEntityItemCounts)
			stashTableDestroyStructSafe(&pEntityItemCounts->stItems, NULL, parse_UIItemCount);
		break;
	}
	return PARSERESULT_SUCCESS;
}
...
AUTO_EXPR_FUNC(entityutil, UIGen) ACMD_NAME(InventoryGetBagMaxSlots);
...
bool gclInvExprInventoryKeyMove(const char *pchKeyA, S32 eBagB, S32 iSlotB, S32 iCount)
{
	...
	if (eBagB == InvBagIDs_None)
	{
		...
		if (pDef)
			eBagB = GetBestBagForItemDef(KeyA.pEntity, pDef, pItem, iCount, false, KeyA.pExtract);
	}
...
bool gclInvExprInventoryKeyMoveKey(const char *pchKeyA, const char *pchKeyB, S32 iCount)
{
	...
	if (KeyB.eBag == InvBagIDs_None)
	{
		...
		ItemDef *pDef = pItem ? GET_REF(pItem->hItem) : KeyA.pSlotLite ? GET_REF(KeyA.pSlotLite->hItemDef) : NULL;
		if (pDef)
			KeyB.eBag = GetBestBagForItemDef(KeyA.pEntity, pDef, pItem, iCount, false, KeyA.pExtract);
	}
...
bool gclInvExprInventoryKeyEquipEnt(const char *pchKeyA, SA_PARAM_OP_VALID Entity *pEntB)
{
	...
	pDef = KeyA.pSlotLite ? GET_REF(KeyA.pSlotLite->hItemDef) : KeyA.pSlot && KeyA.pSlot->pItem ? GET_REF(KeyA.pSlot->pItem->hItem) : NULL;
	pItem = KeyA.pSlotLite ? NULL : KeyA.pSlot ? KeyA.pSlot->pItem : NULL;
	if (!pDef)
		return false;
	if (KeyA.pOwner != pPlayerEnt || gclInventoryGetOwner(pEntB) != KeyA.pOwner)
		return false;

	eBagB = GetBestBagForItemDef(pEntB, pDef, pItem, -1, false, KeyA.pExtract);
...
const char *gclInvExprItemGetPowerDesc(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pPlayer, SA_PARAM_OP_VALID Item *pItem, ACMD_EXPR_DICT(Message) const char *pchPowerMessageKey, ACMD_EXPR_DICT(Message) const char *pchAttribMessageKey)
{
	...
	else if (pItemDef->eType == kItemType_ItemPowerRecipe)
	{
		...
		if (pPowerDef)
		{
			...
			powerdef_AutoDesc(entGetPartitionIdx(pPlayer),langGetCurrent(),pPowerDef,NULL,&s_AutoDesc,NULL,NULL,NULL,pChar,NULL,NULL,iLevel,true,entGetPowerAutoDescDetail(pOwner,false),pExtract,NULL);
			powerdef_AutoDescCustom(pPlayer, langGetCurrent(), &s_pch, pPowerDef, &s_AutoDesc, pchPowerMessageKey, pchAttribMessageKey);
	...
	else
	{
		...
		for(iPower=0; iPower< iNumPowers; iPower++)
		{
			...
			{
				GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pOwner);
				power_AutoDesc(entGetPartitionIdx(pPlayer),langGetCurrent(),pPower,pChar,NULL,&s_AutoDesc,NULL,NULL,NULL,true,0,entGetPowerAutoDescDetail(pOwner,false),pExtract,NULL);
				if (pPowerDef)
					powerdef_AutoDescCustom(pPlayer, langGetCurrent(), &s_pch, pPowerDef, &s_AutoDesc, pchPowerMessageKey, pchAttribMessageKey);
			}
			else
...
const char *gclInvExprItemGemGetInnatePowerDescFiltered(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pPlayer, SA_PARAM_OP_VALID Item *pHolder, SA_PARAM_OP_VALID Item *pItem, ACMD_EXPR_DICT(Message) const char *pchAttribMessageKey, int eGemType)
{
	...
	{
		powerdefs_GetAutoDescInnateMods(entGetPartitionIdx(pPlayer), langGetCurrent(), pItem,s_ppDefsInnate, s_pfScales, NULL, &s_eaInnateMods, NULL, pChar, s_eaSlotRequired, iGemPowers ? iNumPowers - iGemPowers : -1, iLevel, true, 0, entGetPowerAutoDescDetail(pOwner,false));
	}
	else
	{
...
const char *gclInvExprItemGetInnatePowerCompareDesc(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pPlayer, SA_PARAM_OP_VALID Item *pItem, SA_PARAM_OP_VALID Item *pNewItem, ACMD_EXPR_DICT(Message) const char *pchAttribMessageKey)
{
	...
	if(eaSize(&s_ppDefsInnate) > 0)
	{
		powerdefs_GetAutoDescInnateMods(entGetPartitionIdx(pPlayer), langGetCurrent(), pItem, s_ppDefsInnate, s_pfScales, NULL, &s_eaInnateMods, NULL, pChar, NULL, -1, iLevel, true, 0, entGetPowerAutoDescDetail(pOwner,false));
	}
	else
	{
		eaClearStruct(&s_eaInnateMods, parse_AutoDescInnateModDetails);
	...
	{
		powerdefs_GetAutoDescInnateMods(entGetPartitionIdx(pPlayer), langGetCurrent(), pNewItem, s_ppNewDefsInnate, s_pfNewScales, NULL, &s_eaNewInnateMods, NULL, pChar, NULL, -1, iNewLevel, true, 0, entGetPowerAutoDescDetail(pOwner,false));
	}
	else
	{
...
bool gclInvExprInventoryKeyMoveValid(const char *pchKeyA, S32 iDestBag, S32 iDestSlot)
{
	...
	if (iDestBag <= InvBagIDs_None)
		iDestBag = GetBestBagForItemDef(KeyA.pEntity, pDef, pItem, -1, false, KeyA.pExtract);
...
bool gclInvExprInventoryKeyMoveValidEnt(const char *pchKeyA, SA_PARAM_OP_VALID Entity *pEntB, S32 iDestBag, S32 iDestSlot)
{
	...
	if (iDestBag <= InvBagIDs_None)
		iDestBag = GetBestBagForItemDef(pEntB, pDef, pItem, -1, false, KeyA.pExtract);
...
bool gclInvExprItemCanEquip(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Item *pItem)
{
	...
	if (pEnt && pItem)
	{
		...
		InvBagIDs iDestBag = GetBestBagForItemDef(pEnt, pDef, pItem, -1, false, pExtract);
...
F32 gclInvExprInventoryGetInnatePowerAttribFromItemsInBag(SA_PARAM_NN_VALID Entity *pEnt, S32 eSrcBag, SA_PARAM_NN_VALID const char *pchAttribTypeName) 
{
	if (pEnt && pEnt->pChar && pchAttribTypeName)
	{
		...
		// it's in the bag
		pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
		pBagIt = bagiterator_Create();
		bagiterator_SetBagByID(pEnt, eSrcBag, pBagIt, pExtract);
		if (!pBagIt)
			return 0.f;
...
static const char *gclInvBagFindItemKey(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity, GameAccountDataExtract *pExtract, InvBagIDs eBagID, const char *pchName)
{
	...
	if (inv_bag_trh_FindItemByDefName(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pEntity), eBagID, pchName, 0, pIter))
	{
...
bool gclInventoryKeyExprIsEquippable(const char *pchKey, SA_PARAM_OP_VALID Entity *pEnt)
{
	...
	if (pEnt && pItem)
	{
		InvBagIDs iDestBag = GetBestBagForItemDef(pEnt, pItemDef, pItem, -1, false, Key.pExtract);
...
void ui_GenExprLayoutBoxSetDataItemFromAnything(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen)
{
	...
	void ***peaData = ui_GenGetList(pGen, NULL, &pTable); // NOTE: GenGetList() can return a 0 size list, if the list wasn't set yet this frame.
	int i;
	ui_GenGetItemFromStructCB cbGetItemFromStruct = GetItemStructCallback(pTable);
	if (cbGetItemFromStruct)
	{
		for (i = 0; i < eaSize(peaInstances) && i < eaSize(peaData); i++)
		{
			ui_GenSetPointer((*peaInstances)[i], cbGetItemFromStruct((*peaData)[i]), parse_Item);
		}
		for ( ; i < eaSize(peaInstances); i++)
		{
...
void gclInvGetComparableSlots(SA_PARAM_NN_VALID InventorySlot ***peaSlots, SA_PARAM_OP_VALID Item *pItem, SA_PARAM_OP_VALID Entity *pEntity)
{
...
// Looks for equipped items to compare to pItem, and puts the found items' keys in the gen vars specified in pchVars.
// pchVars is a comma delineated list of var names in pGen, ex. "ItemCompareKey1,ItemCompareKey2"
// looks for as many items as it gets vars.
...
{
	static InventorySlot **s_eaSlots=NULL;
	gclInvGetComparableSlots(&s_eaSlots, pItem, pEntity);
	gclSetCompareKeys(pContext, pGen, pchVars, &s_eaSlots);
	eaClearFast(&s_eaSlots);
}

// Like gclItemExprGetCompareKeys(), except prefers items that have powerCategories in pchPowerCategories.
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("ItemGetCount");
S32 gclItemExprGetCount(SA_PARAM_OP_VALID Entity *pEnt, const char *pchItemDef)
{
	return gclInventoryItemCount(pEnt, pchItemDef);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenGetItemsWithCategories");
void gclInvExprGetItemsWithCategories(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEntity, const char *pchItemCategories)
{
	static ItemDef **s_eaDefs = NULL;
	static ItemCategory *s_eaiCategories;
	UIEntityItemCounts *pEntityItemCounts;
	UIItemCount *pItemCount;
	StashTableIterator itItem;
	StashElement pElemItem;
	char *achBuffer, *pchToken, *pchContext;
	S32 i;

	eaiClearFast(&s_eaiCategories);
	strdup_alloca(achBuffer, pchItemCategories);
	if (pchToken = strtok_r(achBuffer, " \r\n\t%|,", &pchContext))
	{
		do
		{
			ItemCategory iCategory = StaticDefineIntGetInt(ItemCategoryEnum, pchToken);
			if (iCategory != -1)
				eaiPush(&s_eaiCategories, iCategory);
		} while (pchToken = strtok_r(NULL, " \r\n\t%|,", &pchContext));
	}

	if (!eaiSize(&s_eaiCategories))
		return;

	gclInventoryCountUpdate(pEntity);
	if (!stashAddressFindPointer(s_stEntityItemCounts, pEntity, &pEntityItemCounts))
		pEntityItemCounts = NULL;

	eaClearFast(&s_eaDefs);
	stashGetIterator(pEntityItemCounts->stItems, &itItem);
	while (stashGetNextElement(&itItem, &pElemItem))
	{
		ItemDef *pItemDef;
		pItemCount = stashElementGetPointer(pElemItem);
		pItemDef = GET_REF(pItemCount->hItemDef);
		if (!pItemDef)
			continue;
		for (i = eaiSize(&s_eaiCategories) - 1; i >= 0; i--)
		{
			if (eaiFind(&pItemDef->peCategories, s_eaiCategories[i]) >= 0)
				break;
		}
		if (i < 0)
			continue;
		eaPush(&s_eaDefs, pItemDef);
	}
	ui_GenSetListSafe(pGen, &s_eaDefs, ItemDef);
}
...
static void gclInvFillUINumericRewardGrantData(Entity* pEnt, ItemNumericData* pNumericData, UIItemRewardGrantEntry* pEntry)
{
	...
	if (pItem == NULL)
	{
		...
		COPY_HANDLE(pItem->hItem, pNumericData->hDef);
	}

	pItem->count = pNumericData->iNumericValue;
	pEntry->pItem = CONTAINER_RECONST(Item, pItem);
}

// Takes a InvRewardRequest and processes the eaNumericRewards and eaItemRewards into the client struct UIItemRewardGrantData
// bPlaceNumericsAfterItems determines whether the numerics are placed at the beginning or end of the list
void gclInvFillUIItemRewardGrantData(	Entity* pEnt, 
									InvRewardRequest* pTransferData, 
									UIItemRewardGrantData* pUIData, 
									bool bPlaceNumericsAfterItems)
{
	S32 i, twice = 0;
	bool bProcessNumerics = !bPlaceNumericsAfterItems;
	const int iNumNumerics = eaSize(&pTransferData->eaNumericRewards);
	eaClearStruct(&pUIData->eaRewards, parse_UIItemRewardGrantEntry);
	...
	do {
		if (bProcessNumerics)
		{
			for (i = 0; i < iNumNumerics; i++)
			{
				UIItemRewardGrantEntry* pEntry = StructCreate(parse_UIItemRewardGrantEntry);
				gclInvFillUINumericRewardGrantData(pEnt, pTransferData->eaNumericRewards[i], pEntry);
				pEntry->GrantData = *pTransferData->eaGrantData[i];
				eaPush(&pUIData->eaRewards, pEntry);
			}
		}
		else
		{
			for (i = 0; i < eaSize(&pTransferData->eaItemRewards); i++)
			{
				UIItemRewardGrantEntry* pEntry = StructCreate(parse_UIItemRewardGrantEntry);
				Item* pItemCopy = StructClone(parse_Item, pTransferData->eaItemRewards[i]);
				item_trh_FixupPowers(CONTAINER_NOCONST(Item, pItemCopy));
				pEntry->pItem = pItemCopy;

				pEntry->GrantData = *pTransferData->eaGrantData[i+iNumNumerics];
				eaPush(&pUIData->eaRewards, pEntry);
			}
		}
