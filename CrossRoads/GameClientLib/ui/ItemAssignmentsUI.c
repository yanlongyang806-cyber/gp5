												...
												ItemAssignmentSlottedItem** eaSlottedItems)
{
	...
	ItemAssignment_GetUIOutcomes(langGetCurrent(), pEnt, pDef, eaSlottedItems, peaData);
...
static ItemAssignmentOutcomeUI* gclItemAssignment_GetOutcome(const char* pchAssignmentDef, const char *pchOutcome)
{
		...
		s_cachedData.pchAssignmentDef != pchAssignmentDef)
	{
		...
		ItemAssignment_GetUIOutcomes(langGetCurrent(), pEnt, pDef, eaSlottedItems, &s_cachedData.eaOutcomeUI);
...
void GenExprGetSlottedItemsForActiveItemAssignment(SA_PARAM_NN_VALID UIGen* pGen, U32 uAssignmentID)
{
	...
	if (pData)
	{
		...
		if (pCompleteDetails)
		{
			ItemAssignment_FillInFromCompleteDetails(langGetCurrent(), pEnt, pPlayerData, peaData, pCompleteDetails);
		}
	}
...
void GenExprGetItemAssignmentSlots(SA_PARAM_NN_VALID UIGen* pGen, U32 uFlags)
{
	...
	ItemAssignment_FillItemAssignmentSlots(langGetCurrent(), pEnt, peaItemAssignmentUIData);
...
void GenExprGetActiveItemAssignments(SA_PARAM_NN_VALID UIGen* pGen, U32 uFlags)
{
	...
	if (pPlayerData)
	{
		for (i = 0; i < eaSize(&pPlayerData->eaActiveAssignments); i++)
		{
			...
			if (!(uFlags & kGetActiveAssignmentFlags_DontFill))
			{
				...
				FillActiveItemAssignment(langGetCurrent(), pData, pAssignment, pEnt, pPlayerData, uTimeCurrent);
			}
			else
			{
				pData = NULL;
		...
		if ((uFlags & kGetActiveAssignmentFlags_IncludeCompleted))
		{
			for (i = 0; i < eaSize(&pPlayerData->eaRecentlyCompletedAssignments); i++)
			{
				...
				if (!(uFlags & kGetActiveAssignmentFlags_DontFill))
				{
					...
					FillCompletedItemAssignment(langGetCurrent(), pEnt, pData, pAssignment, pActiveAssignment, uTimeCurrent);
	...
	if (bStatusHeaders)
	{
		if (bHasInProgress)
		{
			pData = ItemAssignment_CreateActiveHeader(langGetCurrent(), peaData, &iCount, "ItemAssignment_InProgressHeader");
			pData->bHasRewards = false;
			pData->uSortCategory = 0;
		}
		if (bHasReadyToTurnIn)
		{
			pData = ItemAssignment_CreateActiveHeader(langGetCurrent(), peaData, &iCount, "ItemAssignment_ReadyToTurnInHeader");
			pData->bHasRewards = true;
			pData->uSortCategory = 1;
		}
		if (bHasCompleted)
		{
			pData = ItemAssignment_CreateActiveHeader(langGetCurrent(), peaData, &iCount, "ItemAssignment_CompletedHeader");
			pData->bHasRewards = false;
			pData->uSortCategory = 2;
		}
	}
	else if (bCategoryHeaders)
	{
		for (i = 0; i < 255; ++i)
		{
			if (uHeaders == kGetActiveAssignmentFlags_AddAllCategoryHeaders || TSTB(uValidCategories, i))
			{
				...
				{
					pData = ItemAssignment_CreateActiveHeader(langGetCurrent(), peaData, &iCount, REF_STRING_FROM_HANDLE(pCategorySettings->msgDisplayName.hMessage));
													...
													U32 uFlags)
{
	Entity* pEnt = entActivePlayerPtr();
	ItemAssignmentUI*** peaData = ui_GenGetManagedListSafe(pGen, ItemAssignmentUI);
	GetAvailableItemAssignmentsByCategoryInternal(langGetCurrent(), pEnt, peaData, pchBagIDs, pchItemCategoryFilters, NULL, uFlags, -1, -1, 0, NULL);
	ui_GenSetManagedListSafe(pGen, peaData, ItemAssignmentUI, true);
}
													...
													const char *pchStringSearch)
{
	...
	GetAvailableItemAssignmentsByCategoryInternal(langGetCurrent(), pEnt, peaData, pchBagIDs, pchItemCategoryFilters, pchWeightFilters, 
...
void GenExprItemAssignmentsClearSlottedItems(void)
{
	...
	// RE: NNO-13518
	//
	// STO-43597: Destroying structs that the UIGens may reference is a very bad idea.
	//
	// The only time it's safe to modify the existence of a struct that the UI may reference is
	// during PointerUpdate in a strict top-down fashion (e.g. a child can't mutate the existence
	// of a parent or a sibling). While it is generally safe to mutate pointers outside of the UI
	// ticking, there are still events that can cause UIGens to evaluate outside of the normal
	// UI ticking (e.g. a command that sends a UIGen message).
}
...
bool GenExprItemAssignmentsSlotItem(S32 iAssignmentSlot, S32 eBagID, S32 iBagSlot)
{
	...
	if (pEnt && pSlotUI)
	{
		...
		if (ItemAssignmentsCanSlotItem(pEnt, iAssignmentSlot, eBagID, iBagSlot, pExtract))
		{
			...
			if (pItem)
			{
				ItemAssignmentsSlotItemCheckSwap(langGetCurrent(), pEnt, pSlotUI, eBagID, iBagSlot, pItem);
				bSuccess = true;
			}
		}
	}
...
// Clears all slotted items and then slots all items
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("ItemAssignmentsAutoSlotBestItems");
void GenExprItemAssignmentsAutoSlotBestItems(const char* pchAssignmentDef)
{
	Entity* pEnt = entActivePlayerPtr();
	
	if (pEnt)
	{
		// get the def, make sure we have all the slots 
		ItemAssignmentDef *pDef =  ItemAssignment_DefFromName(pchAssignmentDef);

		if (pDef)
		{
			S32 iCount = 0;
			eaSetSizeStruct(&pIACache->eaSlots, parse_ItemAssignmentSlotUI, eaSize(&pDef->eaSlots));
			buildSlotsForItemAssignment(langGetCurrent(), pEnt, pDef, true, &pIACache->eaSlots, &iCount);
			ItemAssignments_AutoSlotBestItems(langGetCurrent(), pEnt);
		}
	}
}
...
bool GenExprGenGetSlotsForItemAssignmentWithOptions(SA_PARAM_NN_VALID UIGen* pGen, const char* pchAssignmentDef, U32 uOptionFlags)
{
	...
	bFull = SetGenSlotsForItemAssignment(langGetCurrent(), pEnt, &pIACache->eaSlots, &iCount, false, peaGenData, pchAssignmentDef, uOptionFlags);
...
bool GenExprItemAssignmentGetSlotsForItemAssignmentDef(SA_PARAM_NN_VALID UIGen* pGen, const char* pchAssignmentDef, U32 uOptionFlags)
{
	...
	bReturn = SetGenSlotsForItemAssignment(langGetCurrent(), pEnt, &s_eaItemAssignmentSlotUIs, &s_iCacheSlotCount, true, peaGenData, pchAssignmentDef, uOptionFlags);
...
SA_RET_OP_VALID ItemAssignmentUI *GenExprItemGetActiveAssignment(SA_PARAM_OP_VALID Item* pItem)
{
	...
	if (pPlayerData)
	{
		for (i = 0; i < eaSize(&pPlayerData->eaActiveAssignments); i++)
		{
			...
			if (j >= 0)
			{
				FillActiveItemAssignment(langGetCurrent(), &s_Assignment, pAssignment, pEnt, pPlayerData, uTimeCurrent);
...
SA_RET_OP_VALID ItemAssignmentUI *GenExprItemAssignmentFindNextToComplete(void)
{
	...
	if (pBestAssignment)
	{
		FillActiveItemAssignment(langGetCurrent(), &s_Assignment, pBestAssignment, pEnt, pPlayerData, uTimeCurrent);
...
SA_RET_OP_VALID ItemAssignmentUI *GenExprItemAssignment(const char *pchItemAssignment)
{
	...
	if (GET_REF(s_Data.hDef) == pDef)
	{
		...
		{
			ItemAssignment_UpdateRequirementsInfo(langGetCurrent(), &s_Data, pEnt, pDef, g_ItemAssignmentSettings.bGetItemAssignmentUIChecksSlottedNotInventory);
	...
	s_uRequirementUpdateTimeMs = gGCLState.totalElapsedTimeMs;
	SetItemAssignmentData(langGetCurrent(), pEnt, &s_Data, pDef, bPersonal, false, g_ItemAssignmentSettings.bGetItemAssignmentUIChecksSlottedNotInventory);
	return &s_Data;
}
		...
		if (pPlayerData)
		{
			for (i = eaSize(&pPlayerData->eaCompletedAssignments) - 1; i >= 0; i--)
			{
				...
				if (pchFilter && *pchFilter && stricmp(pchFilter, pDef->pchName))
					continue;
				ItemAssignment_AddChainToList(langGetCurrent(), pEnt, peaChains, &iCount, pDef, uFlags, false);
			}
		}
	}
	else
...
void gclItemAssignment_GetAssignmentCategories(SA_PARAM_NN_VALID UIGen *pGen, U32 uFlags)
{
	...
	ItemAssignment_FillAssignmentCategories(langGetCurrent(), pEnt, &s_eaCategories, uFlags);
...
S32 gclItemAssignment_GetMetaXPForNextRank()
{
		...
		g_ItemAssignmentSettings.pMetaRankingSchedule->pchNumericRank)
	{
		...
		return ItemAssignments_GetExperienceThresholdForRank(NULL, g_ItemAssignmentSettings.pMetaRankingSchedule, iCurrentRank+1);
	}
	return 0;
}
...
F32 gclItemAssignment_MetaXPPercentThroughRank()
{
		...
		g_ItemAssignmentSettings.pMetaRankingSchedule->pchNumericRank)
	{
		...
		iXPForCurrentRank = ItemAssignments_GetExperienceThresholdForRank(NULL, g_ItemAssignmentSettings.pMetaRankingSchedule, iCurrentRank);
		iXPForNextRank = ItemAssignments_GetExperienceThresholdForRank(NULL, g_ItemAssignmentSettings.pMetaRankingSchedule, iNextRank);
			...
			if (pCategorizedSlot->pSlot && pCategorizedSlot->pSlot->pItem)
			{
				...
				{	
					...
					if (pFoundAssignment)
					{
						...
						FillActiveItemAssignment(	langGetCurrent(),
							&pEquippedAssignment->assignmentUI, 
							pFoundAssignment, 
							pEnt, 
							pPlayerData, 
							uTimeCurrent);
				...
				{	// no pEquippedAssignment, we need to make one if we can find an assignment for this item
					...
					if (pFoundAssignment)
					{
						...
						FillActiveItemAssignment(langGetCurrent(), &pEquippedAssignment->assignmentUI, pFoundAssignment, pEnt, pPlayerData, uTimeCurrent);
									...
									S32 iMaxToFind)
{
	...
	if (pDef->eType == kItemType_Numeric) {
		...
		return inv_FindItemCountByDefNameEx(pEnt, eSearchBags, eExcludeSearchBags, pDef->pchName, kItemFlag_SlottedOnAssignment, iMaxToFind, false);
	}
	return 0;
}
