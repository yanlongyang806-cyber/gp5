...
static ExprContext *DamageFloatGetContext(void)
{
	...
	{
		s_stFuncTable = exprContextCreateFunctionTable("DamageFloaters");
 ...
 *
 */
void gclDrawDamageFloaters(Entity *pEnt, F32 fScale, SpriteProperties *spriteProps)
{
	F32 fZ;
	S32 i;
	int iGroup;
		...
		for (i = 0; i < eaSize(&pFloatGroup->eaDamageFloats); i++)
		{
			...
			if (pInfo->pchMessage && FloatIsVisible(pInfo))
			{
				...
				if (pInfo->pIcon)
				{
					...
					if (spriteProps->is_3D)
					{
						display_3D_sprite_box(pInfo->pIcon, &spriteBox, spriteProps->screen_distance, pInfo->iIconColor | iIconAlpha);
					}
					else
					{
				...
				gfxfont_PrintfEx(v2Pos[0], v2Pos[1], spriteProps->is_3D ? spriteProps->screen_distance : fZ,
					fNumScale, fNumScale, CENTER_X, spriteProps->is_3D ? spriteProps : NULL, "%s", pInfo->pchMessage);
