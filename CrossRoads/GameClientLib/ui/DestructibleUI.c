...
const char *gclNodeGetName(SA_PARAM_OP_VALID WorldInteractionNode *pNode)
{
	...
	// Check for any display text overrides on the player first
	if (pEnt && pEnt->pPlayer)
	{
		int i;
		Player* pPlayer = pEnt->pPlayer;

		for (i = 0; i < eaSize(&(pPlayer->InteractStatus.ppOverrideDisplayTextNodes)); i++)
		{
			if (GET_REF(pPlayer->InteractStatus.ppOverrideDisplayTextNodes[i]->hNode) == pNode)
			{
				if (IS_HANDLE_ACTIVE(pPlayer->InteractStatus.ppOverrideDisplayTextNodes[i]->hOverrideDisplayTextMsg))
				{
					holder = TranslateMessagePtr(GET_REF(pPlayer->InteractStatus.ppOverrideDisplayTextNodes[i]->hOverrideDisplayTextMsg));
					return holder;
				}
			}
		}
