...
void EntGetTitlesFiltered(SA_PARAM_NN_VALID ExprContext *pContext, SA_PARAM_OP_VALID ItemDef ***peaFilteredItemList, SA_PARAM_OP_VALID Entity *pEntity, const char* pchFilter)
{
...

	{
		...
		bagiterator_SetLiteBagByID(pEntity, InvBagIDs_Titles, pIter, pExtract);
...
// Fill in a very limited number of stats for exp bar
AUTO_EXPR_FUNC(UIGen);
void PlayerStats_BeforeTickXPOnly()
{
	Entity *pEnt = entActivePlayerPtr();
...
static ForceFieldStruct *UpdateFragileModHealth(Entity *pEntity, const char* pchTag)
{
	...
	eTag = StaticDefineIntGetInt(PowerTagsEnum, pchTag);
...
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("EntGetFragileModHealthMax");
S32 exprEntGetFragileModHealthMax(SA_PARAM_OP_VALID Entity *pEntity, const char* pchTag)
{
	S32 iHealthMax = 0;
	if(pEntity && pEntity->pChar)
	{
		ForceFieldStruct *pHealth = UpdateFragileModHealth(pEntity, pchTag);
		if(pHealth)
...
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("EntGetFragileModHealth");
S32 exprEntGetFragileModHealth(SA_PARAM_OP_VALID Entity *pEntity, const char* pchTag)
{
	S32 iHealth = 0;
	if(pEntity && pEntity->pChar)
	{
...
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("EntGetFragileModHealthPct");
F32 exprEntGetFragileModHealthPct(SA_PARAM_OP_VALID Entity *pEntity, const char* pchTag)
{
	F32 fHealthPct = 0;
	if(pEntity && pEntity->pChar)
	{
...
{
	return exprEntGetFragileModHealthMax(pEntity, "ForceField");
}
...
{
	return exprEntGetFragileModHealth(pEntity, "ForceField");
}
...
{
	return exprEntGetFragileModHealthPct(pEntity, "ForceField");
}
