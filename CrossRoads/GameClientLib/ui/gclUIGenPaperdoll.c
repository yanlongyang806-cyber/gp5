...
STATIC_ASSERT_MESSAGE(PAPERDOLL_MAX_ANIMATED_COUNT <= HEADSHOT_MAX_ANIMATED, "The UIGenPaperdoll is attempting to use more animated headshots than are recommended.");
...
static bool gclPaperdoll_UpdateCostumeDataChangedTime(UIGenPaperdollState* pState, Entity* pEntity)
{
	if (pState->EntityContainerRef.containerType && pState->EntityContainerRef.containerID)
	{
		...
		if (i >= 0)
		{
			WLCostume* playerWLCostume = GET_REF(pEntity->hWLCostume);
			PaperdollEntityRef* pEntRef = s_eaPaperdollEntities[i];
			if (playerWLCostume && pState->uLastUpdateTime < playerWLCostume->uChangedTime || 
				pEntRef->uChangedTime > pState->uLastUpdateTime || 
				pState->uDisplayDataTime > pState->uLastUpdateTime)
			{
				pState->uLastUpdateTime = MAX(pState->uDisplayDataTime, pEntRef->uChangedTime);
				if (playerWLCostume)
				{
					pState->uLastUpdateTime = MAX(pState->uLastUpdateTime, playerWLCostume->uChangedTime);
				}
				return true;
			}
		}
...
static void gclPaperdoll_UpdateCamera(UIGen* pGen, DynSkeleton *pSkel, GroupDef* ignored, Vec3 vCamPos, Vec3 vCamDir)
{
	...
	{
		CostumeUI_GetRadiusAndHeight(pSkel, 
			contact_HeadshotStyleDefGetFOV(pStyle, pPaperdoll->fFOVy), 
			pPaperdoll->bAutoCenter, 
			!pState->bExtentsInitialized || !pPaperdoll->bUpdateExtentsOnce,
			pState->v3ExtentsMin, pState->v3ExtentsMax, v3AutoCenter, &fRadius, &fHeight);
...
// This is called just to add a timestamp to player's WLCostumes,
// which paperdolls check to see if they should update (e.g. when a player
// equips an item.
static void gclPaperdoll_CostumeDictChanged(enumResourceEventType eType, 
		...
		case RESEVENT_RESOURCE_MODIFIED: 
			{
				...
				costume->uChangedTime = g_ui_State.totalTimeInMs;
			}
		}
	}
}
...
static void gclPaperdoll_Init(void)
{
	...
	if (!s_bPaperdollInit)
	{
		...
		resDictRegisterEventCallback( "Costume", gclPaperdoll_CostumeDictChanged, NULL );
	}
}
...
static void gclPaperdoll_UpdateHeadshotAnimBits(UIGenPaperdollState* pState)
{
	gfxHeadshotUpdateAnimBits(pState->pTexture, &pState->BitFieldGroup);
}
...
static void gclPaperdoll_HeadshotRelease(UIGenPaperdoll *pPaperdoll, UIGenPaperdollState* pState)
{
	...
	if (pPaperdoll->bRenderStaleHeadshots
		&& !pState->bCreated && pState->pTexture && gfxHeadshotIsFinishedCostume(pState->pTexture))
	{
		if (pState->pStaleTexture)
			texGenFreeNextFrame(pState->pStaleTexture);
		pState->pStaleTexture = pState->pTexture;
		pState->pTexture = NULL;

		// Detach stale texture from headshot system. Cleanup is
		// performed after new headshot is rendered, or this gen goes away.
		gfxHeadshotReleaseFreeTexture(pState->pStaleTexture, false);
	}
...
static void gclPaperdoll_ResetCostumeData(UIGenPaperdollState* pState)
{
	...
	StructDestroySafe(parse_ContactCostumeStyle, &pState->pStyleOverride);
...
static WLCostume *gclPaperdoll_CreateWLCostume(UIGenPaperdollState *pState, PlayerCostume *pCostume, SpeciesDef *pSpecies)
{
	...
	pWLCostume = costumeGenerate_CreateWLCostume(pCostume, pSpecies, NULL, NULL, NULL, NULL, NULL, NULL, NULL, PAPERDOLL_WLCOSTUME_PREFIX, GLOBALTYPE_NONE, pState->uWLCostumeId, false, &eaSubCostumes);
	if (pWLCostume)
	{
...
static PlayerCostume* gclPaperdoll_GetPlayerCostumeFromEntity(UIGenPaperdollState* pState, Entity* pEntity, bool* pbCreatedNewCostume, bool bForceActiveSlot)
{
	PlayerCostume* pPlayerCostume = costumeEntity_GetEffectiveCostume(pEntity);  // Live entities
	if (!pPlayerCostume || pState->pDisplayData || bForceActiveSlot)
	{
		GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEntity);
		NOCONST(PlayerCostume)* pNewCostume = NULL;
		CostumeDisplayData** eaData = NULL;
		PCSlotType* pSlotType = NULL;
		bool bUseDisplayData = true;
		...
		if (bForceActiveSlot && pEntity && entGetType(pEntity) == GLOBALTYPE_ENTITYPLAYER
			&& GetCharacterClassEnum(pEntity) != StaticDefineIntGetInt(CharClassTypesEnum, "Ground"))
		{
			// If forcing the active slot on the player entity, and the player entity isn't a ground
			// puppet, don't include the display data from powers or items.
			bUseDisplayData = false;
		}

		if (bUseDisplayData && (!pPlayerCostume || !pPlayerCostume->bPlayerCantChange))
		{
			item_GetItemCostumeDataToShow(PARTITION_CLIENT, pEntity, &eaData, pExtract);
		}

		if(!pPlayerCostume && pEntity->myEntityType == GLOBALTYPE_ENTITYSAVEDPET && pEntity->pSaved &&
			pEntity->pSaved->conOwner.containerID && pEntity->pSaved->conOwner.containerType == GLOBALTYPE_ENTITYPLAYER)
		{
			// This is ground a puppet, get the costume from the owning ent
			Entity* pOwner = entFromContainerIDAnyPartition(pEntity->pSaved->conOwner.containerType, pEntity->pSaved->conOwner.containerID);
			if(pOwner && pOwner->pSaved && pOwner->pSaved->pPuppetMaster)
			{
				pPlayerCostume = costumeEntity_GetActiveSavedCostume(pOwner); 
			}
		}
		...
		if (eaSize(&eaData)) 
		{
			...
			for (i = eaSize(&eaData)-1; i >= 0; --i)
			{
				...
				{
					costume_FreeCostumeDisplayData(eaData[i]);
				}
			}
			eaDestroy(&eaData);
		}
...
// bForceActiveSlot: look at the player's costume slot and use those instead of using the entity's costume.
					//( needed for STO ships for some reason )
static void gclPaperdoll_UpdateEntity(UIGenPaperdollState* pState, Entity* pEntity, bool bForceActiveSlot)
{
	...
	if (!pEntity)
	{
		gclPaperdoll_ResetCostumeData(pState);
			...
			gclPaperdoll_UpdateCostumeDataChangedTime(pState, pEntity))
	{
		WLCostume* pWLCostume;
		...
		pPlayerCostume = gclPaperdoll_GetPlayerCostumeFromEntity(pState, pEntity, &bCleanupCostume, bForceActiveSlot);
												  ...
												  bool bTalking)
{
	...
	if (bTalking &&
		FALSE_THEN_SET(pState->bTalking))
	{
		dynBitFieldGroupSetBit(&pState->BitFieldGroup, allocAddString("Talking"));
	}
	else if(!bTalking &&
			TRUE_THEN_RESET(pState->bTalking))
	{
		dynBitFieldGroupClearBit(&pState->BitFieldGroup, allocAddString("Talking"));
	}
														...
														AIAnimList			*pAnimList)
{
	...
	if (pAnimList && pAnimList->animKeyword)
		pcAnimKeyword = allocAddString(pAnimList->animKeyword);
	else if (SAFE_MEMBER(pStyle, pcAnimKeyword))
		pcAnimKeyword = allocAddString(pStyle->pcAnimKeyword);
	else
		pcAnimKeyword = allocAddString(pDoll->pcAnimKeyword);

	if (pState->astrAnimKeyword != pcAnimKeyword)
	{
		pState->astrAnimKeyword = pcAnimKeyword;
		gclPaperdoll_ResetCostumeState(pState);
	}
}

static const char* gclPaperdoll_HeadshotStyleGetFrame(UIGenPaperdoll *pPaperdoll, UIGenPaperdollState *pState, HeadshotStyleDef* pStyle)
{
	if (pState && pState->pStyleOverride && pState->pStyleOverride->pchFrame)
	{
		return pState->pStyleOverride->pchFrame;
	}
...
static void gclui_GenPaperdollUpdate(UIGen* pGen)
{
	...
	bool bWasTalking = pState->bTalking;
	bool bTalking = false;
	...
	if (pPaperdoll->pHeadshotExpr)
	{
		...
		if (pData && iValidate >= 0 && iValidate < pState->iUsedHeadshotData)
		{
			...
			{
				gclPaperdoll_UpdateEntity(pState, pData->pEntity, pData->bForceActiveSlot);
			}
			else
			{
				gclPaperdoll_ResetCostumeData(pState);
			...
			if (pData->pStyleOverride && pState->pStyleOverride)
				StructCopyAll(parse_ContactCostumeStyle, pData->pStyleOverride, pState->pStyleOverride);
			else if (pData->pStyleOverride && !pState->pStyleOverride)
				pState->pStyleOverride = StructClone(parse_ContactCostumeStyle, pData->pStyleOverride);
			else if (!pData->pStyleOverride && pState->pStyleOverride)
				StructDestroySafe(parse_ContactCostumeStyle, &pState->pStyleOverride);
		} 
		else
	...
	if (pState->pStyleOverride && pState->pStyleOverride->pchBackground)
		pchBackground = pState->pStyleOverride->pchBackground;
	else if (pStyle && pStyle->pchBackground)
	...
	// Apply updates every frame (slight lie by original author)
	if (!gConf.bNewAnimationSystem)
		gclPaperdoll_HeadshotStyleSetAnimBits(pState, pStyle, pPaperdoll->pchAnimBits, pAnimList, bTalking);
	else
		gclPaperdoll_HeadshotStyleSetAnimKeyword(pPaperdoll, pState, pStyle, pAnimList);

	// when the talking bit has changed, apply the update
	if (bWasTalking != bTalking) {
		gclPaperdoll_UpdateHeadshotAnimBits(pState);
	}
...
static void gclui_GenPaperdoll_CreateFXFromStyle(UIGenPaperdollState* pState, HeadshotStyleFX** eaHeadshotFX)
{
	int i;
	for (i = eaSize(&eaHeadshotFX)-1; i >= 0; i--)
	{
		HeadshotStyleFX* pStyleFX = eaHeadshotFX[i];
...
static void gclui_GenPaperdollCapture(UIGen *pGen, UIGenPaperdoll* pPaperdoll, UIGenPaperdollState* pState, bool bAnimate)
{
	...
	const char* pchSky = pState->pStyleOverride && pState->pStyleOverride->pchSky ? pState->pStyleOverride->pchSky : pStyle ? pStyle->pchSky : pPaperdoll->pchSky;
	const char* pchFrame = gclPaperdoll_HeadshotStyleGetFrame(pPaperdoll, pState, pStyle);
	...
	{
		gclui_GenPaperdoll_CreateFXFromStyle(pState, pStyle->eaHeadshotFX);
	}
	if (bAnimate && pState->pStyleOverride && eaSize(&pState->pStyleOverride->eaHeadshotFX))
	{
		gclui_GenPaperdoll_CreateFXFromStyle(pState, pState->pStyleOverride->eaHeadshotFX);
	}
...
static void gclui_GenPaperdollLayoutLate(UIGen* pGen)
{
	...
	if (bUpdateRadius)
	{
		...
		if (pEntity && !pPaperdoll->bAutoCenter)
		{
			...
			{
				// The entity may have a poor height, calculate
				// the radius based on the skeleton instead.
				pState->bUseSkeletonRadius = true;
			}
		}
		else
...
static void gclui_GenPaperdollDrawEarly(UIGen *pGen)
{
	...
	if (pCostume && !bBackgroundOnly)
	{
		...
		{
			if (eaFind(&s_ppHeadshotFocusHeadshots, pGen) >= 0 && eaSize(&s_ppHeadshotFocusHeadshots)- PAPERDOLL_MAX_ANIMATED_COUNT <= eaFind(&s_ppHeadshotFocusHeadshots, pGen))
				bAnimate = true;
			else if (eaFind(&s_ppFocusedGenHeadshots, pGen) >= 0 && eaSize(&s_ppFocusedGenHeadshots) - PAPERDOLL_MAX_ANIMATED_COUNT + eaSize(&s_ppHeadshotFocusHeadshots) <= eaFind(&s_ppFocusedGenHeadshots, pGen))
				bAnimate = true;
			else if (eaFind(&s_ppAnimatedHeadshots, pGen) >= 0 && eaSize(&s_ppAnimatedHeadshots) - PAPERDOLL_MAX_ANIMATED_COUNT + eaSize(&s_ppFocusedGenHeadshots) + eaSize(&s_ppHeadshotFocusHeadshots) <= eaFind(&s_ppAnimatedHeadshots, pGen))
				bAnimate = true;
		}
		...
		else if (!pState->bAnimating && bAnimate)
		{
			if (pState->pTexture)
				gclPaperdoll_HeadshotRelease(pPaperdoll, pState);
			pState->bAnimating = true;
	...
	if ((pState->pTexture && bHeadshotReady) || pState->pStaleTexture || pState->pBackground)
	{
		...
		CBox TextureBox = pGen->ScreenBox;
		CBox* pBox = &TextureBox;
		U32 uColor;
		...
		xcase kUIGenHeadshotMode_FilledUnclipped:
		...
		xcase kUIGenHeadshotMode_Filled:
			CBoxGetCenter(pBox, &fCenterX, &fCenterY);
			{
				F32 fScale = max(fScaleX, fScaleY);
				CBox texBox;
				BuildCBox(&texBox, fCenterX - fWidth * fScale / 2, fCenterY - fHeight * fScale / 2, fWidth * fScale, fHeight * fScale);
				pfTexU[0] = (pBox->lx - texBox.lx) / CBoxWidth(&texBox);
				pfTexV[0] = (pBox->ly - texBox.ly) / CBoxHeight(&texBox);
				pfTexU[1] = (pBox->hx - texBox.lx) / CBoxWidth(&texBox);
				pfTexV[1] = (pBox->hy - texBox.ly) / CBoxHeight(&texBox);
			}
		xdefault:
			{
		...
		display_sprite_effect_ex(
			...
			RdrSpriteEffect_None, 1.0f, SPRITE_2D);
	}
													   ...
													   bool bTalking)
{
	...
	pData->pItemArt = pArt;
	pData->bForceActiveSlot = bForceActiveSlot;
	pData->bTalking = bTalking;
	if (ppchAddedFX)
...
static bool gclPaperdoll_IsValidRenderSize(U32 uWidth, U32 uHeight)
{
	...
	if( uWidth == 512 && uHeight == 256 ) {
		return true;
	}
	if( uWidth == 512 && uHeight == 1024 ) {
		return true;
	}
	if( uWidth == 1024 && uHeight == 512 ) {
		return true;
	}
...
static bool gclui_GenPaperdollValidate(UIGen *pGen, UIGenInternal *pInt, const char *pchDescriptor)
{
	...
	{
		InvalidDataErrorFilenamefInternal(__FILE__, __LINE__, pGen->pchFilename, "%s: %dx%d is not a valid UIGenPaperdoll render size. Valid sizes are 128x128, 256x256, 256x512, 512x256, 512x512, 512x1024, 1024x512, and 1024x1024.", pGen->pchName, pPaperdoll->uRenderWidth, pPaperdoll->uRenderHeight);
		return false;
	}
	return true;
}
