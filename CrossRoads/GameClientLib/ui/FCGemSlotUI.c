		...
		if (pEnt)
		{
			...
			if (pCurrentItem)
			{
				...
				for (i = 0; i < iSize; i++)
				{
					...
					pGemSlot->uProgressionLevel = item_GetSlottedGemProgressionLevel(pCurrentItem, i);
					pGemSlot->uProgressionXP = item_GetSlottedGemProgressionXP(pCurrentItem, i);
				}
			}
			else
			{
...
void GemSlotUI_AttachGemToItemAnywhere(SA_PARAM_OP_VALID UIGen* pGen, SA_PARAM_OP_VALID Entity* pEnt, const char *pchGemKey)
{
	...
	if (pGen)
	{
		...
		for (i = 0; i < n; i++)
		{
			if (gclInventoryParseKey(pchGemKey, &Key) && Key.pSlot && inv_CanGemSlot(pEnt, pGemState->pCurrentItem, Key.pSlot->pItem, i))
			{
...
bool GemSlotUI_CanPlaceGemAnywhere(SA_PARAM_OP_VALID Entity* pEnt, SA_PARAM_OP_VALID Item* pHolder, SA_PARAM_OP_VALID Item* pGem)
{
	if (pHolder && pGem)
	{
		...
		for (i = 0; i < n; i++)
		{
			if (inv_CanGemSlot(pEnt, pHolder, pGem, i))
			{
				return true;
			}
		}
...
bool GemSlotUI_CanPlaceGemInSlot(SA_PARAM_OP_VALID Entity* pEnt, SA_PARAM_OP_VALID Item* pHolder, SA_PARAM_OP_VALID Item* pGem, int iDestGemSlot)
{
	return inv_CanGemSlot(pEnt, pHolder, pGem, iDestGemSlot);
}
