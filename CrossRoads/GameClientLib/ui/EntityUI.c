...
// If the distance is less than 4, objects look like they are coming out of the screen which feels jarring.
#define MINIMUM_SCREEN_DISTANCE 4.0f
...
// returns false if the CombatTrackerNet should be ignored 
static bool gclCheckCombatTrackerAgainstClientFilters(CombatTrackerNet* pTracker)
{
	...
	FOR_EACH_IN_EARRAY(g_CombatLogFilters.ppFilters, CombatLogClientFilter, pFilter)
	{
		if (attrib_Matches(pTracker->eType, pFilter->eAttrib))
		{
			if (pTracker->fMagnitude == 0.f)
				return false;

			if (pTracker->fMagnitude > 0.f)
			{
				if (pFilter->bFilterPositiveMagnitude)
					return false;
			}
			else
			{
				if (pFilter->bFilterNegativeMagnitude)
					return false;
			}
		}
	}
...
// returns true if the player was the target and 
// if the source of the CombatTrackerNet is hostile to the entity
static int gclWasSourceHostileToPlayer(CombatTrackerNet *pNet, Entity *pEnt)
{
	Entity *pSource = pNet->erSource ? entFromEntityRefAnyPartition(pNet->erSource) : NULL;
	EntityRef erPlayer = entPlayerRef(0);

	if (pSource && pSource != pEnt && erPlayer == entGetRef(pEnt))
	{
		return character_TargetIsFoe(PARTITION_CLIENT, pEnt->pChar, pSource->pChar);
	}
...
__forceinline static void gclEntityProcessCombatTracker(Entity *pent)
{
	...
	if(pent->pChar && 0!=(s = eaSize(&pent->pChar->combatTrackerNetList.ppEventsBuffer)))
	{
		...
		for (i=0; i<s; i++)
		{
			...
			// handling sending notifications have to do with power change
			if (pNet->eType == kAttribType_Power && pNet->fMagnitude != 0)
			{
				if (pNet->fMagnitude < 0.f)
				{
					if (gclWasSourceFromPlayer(pNet) && 
				...
				else if (gclWasSourceHostileToPlayer(pNet, pent) && 
							gclNotifyIsHandled(kNotifyType_PowerAttribLost, NULL))
				{
					// send a notification kNotifyType_PowerAttribLost is the player was targeted by a hostile entity
					// and we lost power
					gclNotifyReceiveWithEntityRef(kNotifyType_PowerAttribLost, "PowerAttribLost", NULL, 
														(S32)floorf(pNet->fMagnitude), pNet->erSource);
				}
			}
...
void gclDrawStuffOverEntities(void)
{
	...
	{
		bool is_3D = false;
		bool popSpriteProps = false;
		F32 screenDist = 0.0;
		UIGenEntityState *p3DState = NULL;
		SpriteProperties spriteProps = {0};

		pEnt = s_eaEnts[k];

		if (pEnt->pEntUI && pEnt->pEntUI->pGen && gfxStereoscopicActive())
		{
			p3DState = UI_GEN_STATE(pEnt->pEntUI->pGen,Entity);
			spriteProps.is_3D = true;
			spriteProps.ignore_depth_test = true;
			spriteProps.screen_distance = p3DState->fScreenDist;
			spriteProps.screen_distance = max(spriteProps.screen_distance, MINIMUM_SCREEN_DISTANCE);
			GenSpritePropPush(&spriteProps);
			popSpriteProps = true;
		}
		...
		if (!gConf.bDamageFloatsDrawOverEntityGen)
		{
			...
			if (pEnt->pChar && ShowEntityHUD())
				gclDrawDamageFloaters(pEnt, 1.f, &spriteProps);
...
F32 gclExprEntGetAttribModResist(SA_PARAM_OP_VALID Entity* pEntity, const char* pchAttrib)
{
	...
	return (1.0f - AttribResist( pEntity->pChar, pchAttrib )) * 100.0f;
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("EntGetAttribResistRating");
F32 gclExprEntGetAttribResistRating(SA_PARAM_OP_VALID Entity* pEntity, const char* pchAttrib)
{
	if (pEntity && pEntity->pChar)
	{
		F32 fResistTrue = 1.0f, fResist;
		fResist = character_GetResistGeneric(entGetPartitionIdx(pEntity), pEntity->pChar, StaticDefineIntGetInt(AttribTypeEnum, pchAttrib), &fResistTrue, NULL);
		return (fResist - 1) * 100.0f;
	}
	return 0;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("EntGetOfflineAttribResistRating");
F32 gclExprEntGetOfflineAttribResistRating(SA_PARAM_OP_VALID Entity* pEntity, const char* pchAttrib)
{
	if (pEntity && entGetType(pEntity) == GLOBALTYPE_ENTITYSAVEDPET)
		pEntity = savedpet_GetOfflineOrNotCopy(entGetContainerID(pEntity));
	return gclExprEntGetAttribResistRating(pEntity, pchAttrib);
}
...
S32 gclExprEntRegionGetNumAllowedPets(SA_PARAM_OP_VALID Entity* pEntity)
{
	if (pEntity)
	{
		...
		if (pRegionRules && pRegionRules->iAllowedPetsPerPlayer > 0 && pRegionRules->iUnteamedPetsPerPlayer > 0)
			return pRegionRules->iAllowedPetsPerPlayer + pRegionRules->iUnteamedPetsPerPlayer;
		else if (pRegionRules && pRegionRules->iAllowedPetsPerPlayer > 0)
			return pRegionRules->iAllowedPetsPerPlayer;
		else if (pRegionRules && pRegionRules->iUnteamedPetsPerPlayer > 0)
			return pRegionRules->iUnteamedPetsPerPlayer;
...
static void UpdateUserEntityPointer(SA_PARAM_NN_VALID UserEntity *pUserEnt)
{
	...
	else if (pUserEnt->eSubEntType && pUserEnt->eSubEntID)
	{
		...
		{
			pEnt = entity_GetSubEntity(PARTITION_CLIENT, pPlayer, pUserEnt->eSubEntType, pUserEnt->eSubEntID);

			// Make a request for the puppet or pet if we don't have it
			if(!pEnt && pPlayer && pPlayer->pSaved && pPlayer->pSaved->pPuppetMaster && pUserEnt->eSubEntType == GLOBALTYPE_ENTITYSAVEDPET)
			{
				PetRelationship *pPetR = SavedPet_GetPetFromContainerID(pPlayer, pUserEnt->eSubEntID, false);
				if(pPetR)
				{
					PuppetEntity *pPuppet = SavedPet_GetPuppetFromPet(pPlayer,pPetR);
					// The following call will make the correct subscription
					pEnt = Entity_GetEntFromPuppetOrPet(pPlayer, pPuppet, pPetR);
				}
			}
		}
	}
