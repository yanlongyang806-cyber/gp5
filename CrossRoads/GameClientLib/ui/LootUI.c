...
static LootUIData **s_eaCompareList = NULL;
static LootUIData **s_pLootUIDataList = NULL;
static bool s_bOverflow = false;

AUTO_STRUCT;
typedef struct LootCompareSlot
{
	LootUIData Slot; AST(EMBEDDED_FLAT)
	S32 iDiffCount;
} LootCompareSlot;
...
{
	eaDestroyStruct(&s_pLootUIDataList, parse_LootUIData);
}

static bool gclLootCreateLootUIDataListFromBag(InventoryBag* pBag, LootUIData*** peaList)
{
...
static bool gclLootCreateLootUIDataListFromBags(InventoryBag ** eaBags, LootUIData*** peaList)
{
	int iNumBags = eaSize(&eaBags);
	int i;
	bool bResult = false;

	for (i=0;i<iNumBags;i++)
	{
		if (gclLootCreateLootUIDataListFromBag(eaBags[i],peaList))
			bResult = true;
	}
...
void LootInteraction(Entity *pEnt, LootToClient * pLootInfo)
{
	eaDestroyStruct(&s_pLootUIDataList, parse_LootUIData);

	if (pLootInfo->eaLootBags)
		assert(pLootInfo->eaLootBags[0]->BagID != InvBagIDs_Overflow);
	
	s_bOverflow = false;

	// get a list of all items in the specified bag
	gclLootCreateLootUIDataListFromBags(pLootInfo->eaLootBags, &s_pLootUIDataList);
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_CLIENTCMD ACMD_PRIVATE;
void LootUpdateList(Entity *pEnt, LootToClient * pLootInfo)
{
	// ignore if this is trying to update the overflow bag with non-overflow bag data (or vice versa)
//	if ((pLootInfo->eaLootBags[0]->BagID == InvBagIDs_Overflow) != s_bOverflow)
//		return;
	if (pLootInfo->eaLootBags)
		assert(pLootInfo->eaLootBags[0]->BagID != InvBagIDs_Overflow);
	assert(!s_bOverflow);

	eaClearStruct(&s_pLootUIDataList, parse_LootUIData);
...
{
	eaDestroyStruct(&s_eaCompareList, parse_LootUIData);

	if (pCompareBag)
	{
		gclLootCreateLootUIDataListFromBag(pCompareBag, &s_eaCompareList);
	}
}
...
static bool lootui_UpdateOverflowList(void)
{
	if (s_bOverflow)
	{
		...
		if (pEnt && pOverflowBag)
		{
			...
			gclLootCreateLootUIDataListFromBag(pOverflowBag, &s_pLootUIDataList);
		}
		return true;
	}
	return false;
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GenGetLootInteractionLootUIDataList,GenGetLootInteractionTradeSlotList);
void lootui_GetLootInteractionLootUIDataList(SA_PARAM_NN_VALID UIGen *pGen)
{
	Entity *pPlayer = entActivePlayerPtr();
	int i;
	LootUIData ***ppSlots = ui_GenGetManagedListSafe(pGen, LootUIData);
	...
	if(eaSize(ppSlots))
	{
		eaClearStruct(ppSlots,parse_LootUIData);
	}
	...
	for(i=0;i<eaSize(&pPlayer->pPlayer->InteractStatus.interactOptions.eaOptions);i++)
	{
		if(pPlayer->pPlayer->InteractStatus.interactOptions.eaOptions[i]->entRef == pPlayer->pPlayer->InteractStatus.overrideRef
			&& eaSize(&pPlayer->pPlayer->InteractStatus.interactOptions.eaOptions[i]->eaLootBags) > 0)
		{
			gclLootCreateLootUIDataListFromBags(pPlayer->pPlayer->InteractStatus.interactOptions.eaOptions[i]->eaLootBags,ppSlots);
		}
	}
	ui_GenSetManagedListSafe(pGen,ppSlots, LootUIData,true);
}

// Get a list of currently available loot.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GenGetLootUIDataList,GenGetLootTradeSlotList);
void lootui_GetLootUIDataList(SA_PARAM_NN_VALID UIGen *pGen)
{
	...
	// This is bad for performance, but protects against s_pLootUIDataList being modified
	// at the wrong time and crashing the client.
	{
		LootUIData ***peaSlots = ui_GenGetManagedListSafe(pGen, LootUIData);
		eaCopyStructs(&s_pLootUIDataList, peaSlots, parse_LootUIData);
		ui_GenSetManagedListSafe(pGen, peaSlots, LootUIData, true);
	}
}

// Get a list of currently available loot, and compare it to the items in s_eaCompareList if possible.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GenGetLootUIDataListCompare,GenGetLootTradeSlotListCompare);
void lootui_GetLootUIDataListCompare(SA_PARAM_NN_VALID UIGen *pGen)
{
	...
	{
		S32 i, iSize = eaSize(&s_pLootUIDataList);
		eaSetSizeStruct(peaSlots, parse_LootCompareSlot, iSize);
		for (i = 0; i < iSize; i++)
		{
			LootUIData* pSlot = s_pLootUIDataList[i];
			LootCompareSlot* pCompareSlot = eaGetStruct(peaSlots, parse_LootCompareSlot, i);
			pCompareSlot->iDiffCount = 0;
			StructCopyAll(parse_LootUIData, pSlot, &pCompareSlot->Slot);
		}
	}
	else
	{
		S32 i, j, iSize = eaSize(&s_pLootUIDataList);
		eaSetSizeStruct(peaSlots, parse_LootCompareSlot, iSize);
		for (i = 0; i < iSize; i++)
		{
			...
			StructCopyAll(parse_LootUIData, pSlot, &pNewSlot->Slot);
		}
	}
...
void LootOverflow(Entity *pEnt)
{
	...
	gclLootCreateLootUIDataListFromBag(pBag, &s_pLootUIDataList);
...
{
	LootUIData *pSlot = eaGet(&s_pLootUIDataList, iSlot);
	if (pSlot)
		return pSlot->pItem;
	return NULL;
}
