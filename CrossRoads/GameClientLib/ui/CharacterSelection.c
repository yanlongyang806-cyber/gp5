...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterSelection_IsChoiceActive");
bool CharacterSelection_IsChoiceActive(Login2CharacterChoice *pChoice)
{
	return !!SAFE_MEMBER(pChoice, privAccountName);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterSelection_HasAnyInactiveCharacters");
bool CharacterSelection_HasAnyInactiveCharacters(UIGen *pGen)
{
	Login2CharacterChoice ***peaChoices = ui_GenGetManagedListSafe(pGen, Login2CharacterChoice);
	int i;
	for (i = 0; i < eaSize(peaChoices); i++)
	{
		Login2CharacterChoice *pChoice = (*peaChoices)[i];
		if (!pChoice->privAccountName)
		{
			return true;
		}
	}
...
const char *CharacterSelection_GetChoiceAllegianceUntranslated(Login2CharacterChoice *pChoice)
{
	if (pChoice) 
    {
		const char *subAllegiance = Login2_GetSubAllegianceFromCharacterChoice(pChoice);
		if (subAllegiance && *subAllegiance != 0)
...
{
	return SAFE_MEMBER( g_characterSelectionData, UGCLoginOnly );
}
...
{
	if(GSM_IsStateActive(GCL_LOGIN) && g_characterSelectionData)
    {
...
void CharacterSelectionExpr_ChangeNameByContainerID(ContainerID iContainerID, bool bBadName, const char *pcNewName)
{
    ...
    if (GSM_IsStateActive(GCL_LOGIN_USER_CHOOSING_EXISTING) && g_characterSelectionData)
    {
        ...
        if ( characterChoice && !conflictFound )
        {
            ...
            {
                Entity *playerEnt = gclLogin2_CharacterDetailCache_GetEntity(iContainerID);
                bool bCharacterBadName = false;

                if ( playerEnt && playerEnt->pSaved )
                {
                    bCharacterBadName = playerEnt->pSaved->bBadName;
                }

                if ( bCharacterBadName == bBadName )
                {
...
bool exprIsBadNameById(ContainerID id)
{
	if (GSM_IsStateActive(GCL_LOGIN_USER_CHOOSING_EXISTING))
	{
        ...
        if ( playerEnt && playerEnt->pSaved )
        {
            return playerEnt->pSaved->bBadName;
        }
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterSelection_GetPuppetNameByClassTypeAndSet");
const char *CharacterSelection_GetPuppetNameByClassTypeAndSet(U32 uContainerID, const char *pchClassType, const char *pchSet)
{
	Entity *playerEnt = gclLogin2_CharacterDetailCache_GetEntity(uContainerID);
	Entity *subEnt = gclLogin2_CharacterDetailCache_GetPuppetWithSetName(uContainerID, StaticDefineIntGetInt(CharClassTypesEnum, pchClassType), pchSet);
...
{
	return CharacterSelection_GetPuppetNameByClassTypeAndSet(uContainerID, pchClassType, NULL);
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterSelection_GetPuppetClassNameByClassTypeAndSet");
const char *CharacterSelection_GetPuppetClassByClassTypeAndSet(U32 uContainerID, const char *pchClassType, const char *pchSet)
{
	Entity *playerEnt = gclLogin2_CharacterDetailCache_GetEntity(uContainerID);
	Entity *subEnt = gclLogin2_CharacterDetailCache_GetPuppetWithSetName(uContainerID, StaticDefineIntGetInt(CharClassTypesEnum, pchClassType), pchSet);
	CharacterClass *charClass = NULL;
...
static bool CharacterSelection_IsInvalidEntity(Entity *pEnt)
{
	if (!pEnt)
		return true;

	if (entGetType(pEnt) != GLOBALTYPE_ENTITYPLAYER && entGetType(pEnt) != GLOBALTYPE_ENTITYSAVEDPET)
		return true;

	if (!pEnt->pChar || !pEnt->pSaved)
		return true;

	if (entGetType(pEnt) == GLOBALTYPE_ENTITYPLAYER && !pEnt->pPlayer)
		return true;

	return false;
}

// If a character id returns true for IsInvalidCharacter, the player shouldn't
// be able to login with it.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterSelection_IsInvalidCharacter");
bool CharacterSelectionExpr_IsInvalidCharacter(U32 uCharacterID)
{
	Login2CharacterDetail *pDetails = gclLogin2_CharacterDetailCache_Get(uCharacterID);
	S32 i, n;

	if (!pDetails)
		return false;

	if (!pDetails->playerEnt)
		return true;

	if (CharacterSelection_IsInvalidEntity(pDetails->playerEnt))
		return true;

	n = eaSize(&pDetails->activePuppetEnts);
	for (i = 0; i < n; i++)
		if (CharacterSelection_IsInvalidEntity(pDetails->activePuppetEnts[i]))
			return true;

	return false;
}

// Generic is it possible to login using the character & virtual shard combination.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterSelection_CanLoginWithCharacter");
bool CharacterSelectionExpr_CanLoginWithCharacter(U32 uVirtualShard, U32 uCharacterID)
{
	if (CharacterSelection_IsVirtualShardDisabled(uVirtualShard))
		return false;

	if (CharacterSelectionExpr_IsInvalidCharacter(uCharacterID))
		return false;
...
void CharacterSelection_InvalidateCharacter(U32 uCharacterID)
{
	Login2CharacterDetail *pDetails = gclLogin2_CharacterDetailCache_Get(uCharacterID);
	S32 i, n;
	NOCONST(Entity) *pInvalid;
	const U32 s_uInvalidID = 0xFFFFFFFD; // SPECIAL_CONTAINERID_RANDOM, except using it as a marker for the bad character (no container should be created with that ID)

	if (!pDetails)
	{
		conPrintf("Character '%d' not found", uCharacterID);
		return;
	}
	...
	for (i = 0; i < n; i++)
	{
		if (pDetails->activePuppetEnts[i]->myContainerID == s_uInvalidID)
		{
			conPrintf("Character '%d' already invalidated", uCharacterID);
			return;
		}
	}

	pInvalid = StructCreateNoConst(parse_Entity);
	pInvalid->myContainerID = s_uInvalidID;
	eaPush(&pDetails->activePuppetEnts, CONTAINER_RECONST(Entity, pInvalid));
	conPrintf("Character '%d' is now invalid", uCharacterID);
}
