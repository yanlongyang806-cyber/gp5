...
typedef struct ContactCostumeStyle ContactCostumeStyle;

#define PAPERDOLL_MAX_ANIMATED_COUNT 2
...
typedef enum UIGenHeadshotMode
{
	...
	kUIGenHeadshotMode_FilledUnclipped,
} UIGenHeadshotMode;
...
typedef struct UIGenPaperdollState
{
	...
	const char *astrAnimKeyword; AST(POOL_STRING)
	char *pchLastBits;
...
typedef struct PaperdollHeadshotData
{
	...
	ContactCostumeStyle *pStyleOverride; AST(UNOWNED)
	bool bForceActiveSlot;
	bool bTalking;
} PaperdollHeadshotData;

extern PaperdollHeadshotData* gclPaperdoll_CreateHeadshotData(ExprContext* pContext, WLCostume* pWLCostume, PlayerCostume* pPlayerCostume, Entity* pEntity, ItemArt* pArt, const char* pchHeadshotStyle, const char*** ppchAddedFX, bool bForceActiveSlot, bool bTalking);
