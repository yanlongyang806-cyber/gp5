...
#define TYPE_parse_ItemDef Item
...
static int gclui_SMFHover(const char *pch, UIGen* pGen)
{
    Item *def = (Item*) gclInvExprItemGetDummy(pch);
	if(def)
	{
		ui_GenSetPointer(pGen, def, parse_Item);
		return true;
	}
	else
	{
...
void gclLoadUI(void)
{
	...
	gclBasicOptions_InitColorPalettes();
}
...
AUTO_EXPR_FUNC(UIGen);
F32 AttribModShieldMagnitudeTag(SA_PARAM_OP_VALID Entity *pent, ACMD_EXPR_ENUM(AttribType) const char* attribName, const char *tags)
{
	if (pent && pent->pChar)
	{
		S32 iTag = StaticDefineIntGetInt(PowerTagsEnum, tags);
		int i, n = eaSize(&pent->pChar->ppModsNet);
		F32 fResult = 0;

		for (i = 0; i < n; i++)
		{
			AttribModDef *pmoddef;
			AttribModNet *pNet = pent->pChar->ppModsNet[i];
			...
			{
				if (eaiFind(&pmoddef->tags.piTags, iTag) >= 0)
				{
					fResult += pNet->iHealth;
				}
			}
		}
...
AUTO_EXPR_FUNC(UIGen);
F32 AttribModShieldMagnitudeMaxTag(SA_PARAM_OP_VALID Entity *pent, ACMD_EXPR_ENUM(AttribType) const char* attribName, const char *tags)
{
	if (pent && pent->pChar)
	{
		S32 iTag = StaticDefineIntGetInt(PowerTagsEnum, tags);
		...
		for (i = 0; i < n; i++)
		{
			...
			if (pmoddef && pmoddef->offAttrib == kAttribType_Shield)
			{
				if (eaiFind(&pmoddef->tags.piTags, iTag) >= 0)
				{
					fResult += pNet->iHealthMax ? pNet->iHealthMax : pNet->iHealth;
				}
			}
		}
...
AUTO_EXPR_FUNC(UIGen);
F32 AttribModShieldMagnitudePctTag(SA_PARAM_OP_VALID Entity *pent, ACMD_EXPR_ENUM(AttribType) const char* attribName, const char *tags)
{
	if (pent && pent->pChar)
	{
		S32 iTag = StaticDefineIntGetInt(PowerTagsEnum, tags);
		...
		for (i = 0; i < n; i++)
		{
			...
			if (pmoddef && pmoddef->offAttrib == kAttribType_Shield)
			{
				...
				{
					fValue += pNet->iHealth;
					fMax += pNet->iHealthMax ? pNet->iHealthMax : pNet->iHealth;
				}
			}
		}

		return fMax > 0 ? fValue / fMax : fValue > 0 ? 1 : 0;
	}
	return -1;
}
...
static void gclEntBuffCleanup(StashTable stBuffStash, UIGenEntityBuff **eaBuffs, S32 iUpdateIndex, S32 bUpdatePowerDefBuffList)
{
	...
	while (stashGetNextElement(&iter, &elem))
	{
		...
		{
			// clear the UIGenEntityBuff's data pointers that use this 
			FOR_EACH_IN_EARRAY(eaBuffs, UIGenEntityBuff, pBuff)
			{
				if (pBuff->pData == pData)
				{
					pBuff->pData = NULL;
				}
			}
...
static S32 EntGetBuffsByPower(UIGen *pGen, Entity *pEntity, S32* eaTags, StashTable stBuffStash)
{
	...
	gclEntBuffCleanup(stBuffStash, (peaBuffs ? *peaBuffs : NULL), s_iUpdateIndex, true);
	...
	for (i = 0; i < eaSize(peaBuffs); i++)
	{
		...
		for (j=eaSize(&pChar->ppModsNet)-1; j>=0; j--)
		{
			...
			if (ATTRIBMODNET_VALID(pModNet) && GET_REF(pModNet->hPowerDef) == pPowerDef) 
			{
				...
				{
					bFound = false;
					FOR_EACH_IN_EARRAY(pBuff->pData->PowerDefData.eaModStack, AttribModStack, pModStack)
					{
						if (pModStack->pModNet == pModNet)
						{
							bFound = true;
							break;
						}
		...
		{
			powerdef_UseNNOFormattingForBuff(lang, pPowerDef, &pBuff->pchAutoDesc);
		}
		else
		{
			// AutoDesc
...
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("EntBuffPowerModeGetDuration");
F32 exprEntBuffPowerModeGetDuration(SA_PARAM_OP_VALID Entity *pEntity, const char* pchMode, U32 uFlags)
{
	enum Flags {
		kFlag_MaxRemaining = 1,
		kFlag_MaxDuration = 2,
	};

	Character *pChar = SAFE_MEMBER(pEntity, pChar);
	S32 iPowerMode = StaticDefineIntGetInt(PowerModeEnum, pchMode);
	AttribModNet *pBestModNet = NULL;
	U32 uBestDuration = 0;
	S32 i;

	if (!pChar || iPowerMode == -1)
		return 0;
	...
	for(i=0; i < eaSize(&pChar->ppModsNet); i++)
	{
		...
		if(pModDef)
		{
			PowerModeParams *pParams = ATTRIBMODNET_VALID(pModNet) && pModDef->offAttrib == kAttribType_PowerMode ? (PowerModeParams *)pModDef->pParams : NULL;
			if (pParams && pParams->iPowerMode == iPowerMode)
			{
				if (uFlags & kFlag_MaxRemaining)
				{
					U32 uDuration = character_ModNetGetPredictedDuration(pChar, pModNet);
					if (uDuration >= uBestDuration)
					{
						pBestModNet = pModNet;
						uBestDuration = uDuration;
					}
				}
				else
				{
	...
	if (pBestModNet)
	{
		if (uFlags & kFlag_MaxDuration)
			return pBestModNet->uiDurationOriginal;
		return character_ModNetGetPredictedDuration(pChar, pBestModNet);
	}
	return 0;
}
...
char *gclGenExprPowerEnhDescription(ExprContext *pContext, const char *pchNode, const char *pchEnh, const char *pchMessage)
{
	...
	if(pNode && pEnh)
	{
		...
		powerdef_AutoDesc(PARTITION_CLIENT,langGetCurrent(),pEnh,&pchTemp,NULL,"<br>","<bsp><bsp>","* ",pChar,NULL,NULL,iLevel,true,entGetPowerAutoDescDetail(e,false),pExtract,NULL);
...
char *gclGenExprPowerDefDescription(ExprContext *pContext, SA_PARAM_OP_VALID PowerDef *pDef) {
	...
	if (pDef) {
		...
		powerdef_AutoDesc(PARTITION_CLIENT,langGetCurrent(),pDef,&s_pchDescription,NULL,"<br>","<bsp><bsp>","- ",
		...
		for (i = iRank; i >= 0; i--)
		{
			...
			if (pDef)
			{
				...
				if (pchCustomAutoDesc && *pchCustomAutoDesc)
				{
					...
					powerdef_AutoDesc(PARTITION_CLIENT,langGetCurrent(),pDef,NULL,pAutoDescPower,"<br>","<bsp><bsp>","* ",pChar,NULL,NULL,iLevel,true,entGetPowerAutoDescDetail(e,false),pExtract,NULL);
					powerdef_AutoDescCustom(e, langGetCurrent(), &pchTemp, pDef, pAutoDescPower, pchCustomAutoDesc, pchCustomAttribMod);
					StructDestroy(parse_AutoDescPower, pAutoDescPower);
				}
				else
				{
					powerdef_AutoDesc(PARTITION_CLIENT,langGetCurrent(),pDef,&pchTemp,NULL,"<br>","<bsp><bsp>","* ",pChar,NULL,NULL,iLevel,true,entGetPowerAutoDescDetail(e,false),pExtract,NULL);
				}
...
static void gcluigen_TutorialStartStop(UIGenTutorial *pTutorial, bool bStarted)
{
	if (bStarted)
	{
		char *estr = NULL;
		estrStackCreate(&estr);
		estrPrintf(&estr, "Tutorial.%s", pTutorial->pchName);
		if (!GamePrefGetInt(estr, 0))
			GamePrefStoreInt(estr, timeSecondsSince2000());
		estrDestroy(&estr);
	}
}
...
void gclui_GenRegister(void)
{
	...
	ui_GenSetSMFHoverCallback(gclui_SMFHover);
	ui_GenTutorialSetStartStopHandler(gcluigen_TutorialStartStop);
}
...
{	
	...
	{	
		...
		else if (!pEntPlayer->pSaved->bClientWantsAllPets && !pEntPlayer->pSaved->bSubscribeAllPets)
		{
			pEntPlayer->pSaved->bClientWantsAllPets = true;
			ServerCmd_SubscribeAllPets();
		}
	}
...
bool gclSavedPetGetPowerPropSlotsForActivePuppet(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, S32 ePropCategory, bool bReverseSort)
{
	Entity *pEntPlayer = entActivePlayerPtr();
	U32 iCurID = 0;
	PuppetMaster *pMaster = SAFE_MEMBER2(pEntPlayer, pSaved, pPuppetMaster);
	if (pMaster)
		iCurID = pMaster->curID ? pMaster->curID : pMaster->curTempID;
	return gclSavedPetGetAllPowerPropSlotsEx(pContext, pGen, pEntPlayer, ePropCategory, bReverseSort, iCurID);
}
	...
	if (i >= 0) 
	{
		...
		if (pSlotDef = GET_REF(pSlot->hDef))
		{
			for (i = 0; i < eaSize(&pEntPlayer->pSaved->ppOwnedContainers); i++)
			{
				...
				PuppetEntity *pPuppet = SavedPet_GetPuppetFromPet(pEntPlayer, pPet);
				Entity* pPetEnt;

				// skip puppets, looking for boffs
				if(pPuppet)
				{
					continue;
				}

				pPetEnt = Entity_GetEntFromPuppetOrPet(pEntPlayer, pPuppet, pPet);
				
				if (!pPetEnt || eaSize(&pPetEnt->pChar->ppTraining))
					continue;
...
{	
	...
	{	
		...
		else if (!pEntPlayer->pSaved->pPuppetMaster->bPuppetClientWantsAll && !pEntPlayer->pSaved->pPuppetMaster->bPuppetSubscribeAll)
		{
			ServerCmd_SubscribeAllPuppets();
			pEntPlayer->pSaved->pPuppetMaster->bPuppetClientWantsAll = true;
		}
	}
...
void exprGenOpenWebBrowser(const char *pchUrl)
{
	...
	{
#ifdef UNICODE
		wchar_t *pchURLUnicode;
		int size = MultiByteToWideChar(CP_UTF8, 0, pchUrl, -1, NULL, 0);
		pchURLUnicode = malloc(sizeof(wchar_t) * (size + 1));
		MultiByteToWideChar(CP_UTF8, 0, pchUrl, -1, pchURLUnicode, size + 1);
		ShellExecute(NULL, L"open", pchURLUnicode, NULL, NULL, SW_SHOWNORMAL);
		free(pchURLUnicode);
#else
		STATIC_ASSERT(0);
		char *pchURLACP;
		int size = UTF8ToACP(pchUrl, NULL, 0);
		pchURLACP = malloc(size);
		UTF8ToACP(pchUrl, pchURLACP, size);
		ShellExecute(NULL, "open", pchURLACP, NULL, NULL, SW_SHOWNORMAL);
		free(pchURLACP);
#endif
	}
#endif
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenOpenWebBrowserInOverlay");
bool exprGenOpenWebBrowserInOverlay(const char *pchUrl, bool bFallbackToExternal)
{
	if (bFallbackToExternal)
	{
		exprGenOpenWebBrowser(pchUrl);
		return true;
	}
													...
													HeadshotNotifyBytesF notifyBytesF, UserData notifyBytesData )
{
	...
	if (pCostume)
	{
		...
		if (!pWLCostume)
		{
			...
			pWLCostume = costumeGenerate_CreateWLCostume(pCostume, NULL, NULL, NULL, NULL, NULL, 0, NULL, NULL, "Headshot_", 0, 0, false, &eaSubCostumes);
...
S32 gclExprGetGimmeVersion(void)
{
	...
	if (pch && (pch[iShortNameLen] == '.' || pch[iShortNameLen] == ' ' || pch[iShortNameLen] == '_') && isdigit(pch[iShortNameLen + 1] & 0xff))
	{
		// e.g. "ST.<branch>.<date>.<incr>" or "<svn> (ST <branch>)" or "ST_<branch>_<date>_<time>"
...
// Check to see if a tutorial has already been started
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenTutorialIsStarted");
bool gclExprGenTutorialIsStarted(const char *pchTutorial)
{
	char *estr = NULL;
	S32 iValue;
	estrStackCreate(&estr);
	estrPrintf(&estr, "Tutorial.%s", pchTutorial);
	iValue = GamePrefGetInt(estr, 0);
	estrDestroy(&estr);
	return iValue != 0;
}

// Start a tutorial only if it has not previously been started
AUTO_COMMAND ACMD_NAME("GenTutorialStartOnce") ACMD_ACCESSLEVEL(0) ACMD_HIDE;
bool gclExprGenTutorialTutorialStartOnce(const char *pchTutorial);

// Start a tutorial only if it has not previously been started
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenTutorialStartOnce");
bool gclExprGenTutorialTutorialStartOnce(ACMD_NAMELIST("UIGenTutorial", REFDICTIONARY) const char *pchTutorial)
{
	UIGenTutorial *pTutorial = RefSystem_ReferentFromString("UIGenTutorial", pchTutorial);
	if (pTutorial && !gclExprGenTutorialIsStarted(pchTutorial))
	{
...
// Determine if the client is in the really early loading phase
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ClientIsPreLogin);
bool gclExprClientIsPreLogin(void)
{
	// For a single frame GCL_BASE is all alone, the next frame
	// either GCL_PLAY_VIDEO or GCL_LOGIN is entered. GCL_LOGIN
	// in it's enter will wait for graphics related data to finish
	// loading.
	return !GSM_DoesStateHaveChildStates(GCL_BASE);
}

AUTO_STRUCT;
typedef struct MapDisplayBarVariable
{
	REF_TO(UIStyleBar) hBar;        AST(NAME(Bar) STRUCTPARAM)
	REF_TO(Message) hLowMessage;    AST(NAME(LowMessage))
	REF_TO(Message) hHighMessage;   AST(NAME(HighMessage))
	F32 fLowValue;
	F32 fHighValue;
} MapDisplayBarVariable;

AUTO_STRUCT;
typedef struct MapDisplayVariableValid
{
	F32 fLowValue;
	F32 fHighValue;
} MapDisplayVariableValid;

AUTO_STRUCT;
typedef struct MapDisplayVariable
{
	const char *pchZoneMap;                 AST(POOL_STRING)
	const char *pchVariableName;            AST(POOL_STRING)
	const char *pchVolume;                  AST(POOL_STRING)

	const char *pchGroup;                   AST(POOL_STRING)

	REF_TO(Message) hDisplayName;           AST(NAME(DisplayName))
	REF_TO(Message) hInfo;                  AST(NAME(Info))

	int iDisplayOrder;                      AST(NAME(displayOrder))

	MapDisplayVariableValid *pValid;

	MapDisplayBarVariable *pBar;

	bool bTimer;
} MapDisplayVariable;

AUTO_STRUCT;
typedef struct MapDisplayVariableList
{
	MapDisplayVariable **eaMapVariable;     AST(NAME(DisplayVariable))
} MapDisplayVariableList;

static MapDisplayVariableList s_VariableList;
static MapDisplayVariableList s_VariableListFiles;

void gclMapDisplayVariable_Reload(const char *pchPath, int when)
{
	int col = ParserFindColumnFromOffset(parse_MapDisplayVariable, offsetof(MapDisplayVariable, iDisplayOrder));

	loadstart_printf("Loading MapDisplayVariables...");

	StructReset(parse_MapDisplayVariableList, &s_VariableList);
	StructReset(parse_MapDisplayVariableList, &s_VariableListFiles);

	// TODO(jm): Remove the single def file after STO stops using it
	ParserLoadFiles(NULL, "defs/config/MapDisplayVariables.def", "MapDisplayVariables.bin", PARSER_OPTIONALFLAG, parse_MapDisplayVariableList, &s_VariableList);
	ParserLoadFiles("ui/mapvariables", ".mapdisplayvariables", "MapDisplayVariableFiles.bin", PARSER_OPTIONALFLAG, parse_MapDisplayVariableList, &s_VariableListFiles);

	if (devassertmsg(col >= 0, "iDisplayOrder not in MapDisplayVariable?"))
	{
		eaStableSortUsingColumn(&s_VariableList.eaMapVariable, parse_MapDisplayVariable, col);
		eaStableSortUsingColumn(&s_VariableListFiles.eaMapVariable, parse_MapDisplayVariable, col);
	}

	loadend_printf(" done (%d displayable variables)\n", eaSize(&s_VariableList.eaMapVariable) + eaSize(&s_VariableListFiles.eaMapVariable));
}

AUTO_STARTUP(MapDisplayVariableList, 1) ASTRT_DEPS(UILib);
void gclMapDisplayVariable_Load(void)
{
	gclMapDisplayVariable_Reload(NULL, 0);

	// TODO(jm): Remove the single def file after STO stops using it
	FolderCacheSetCallback(FOLDER_CACHE_CALLBACK_UPDATE, "defs/config/MapDisplayVariables.def", gclMapDisplayVariable_Reload);
	FolderCacheSetCallback(FOLDER_CACHE_CALLBACK_UPDATE, "ui/mapvariables/*.mapdisplayvariables", gclMapDisplayVariable_Reload);
}

AUTO_STRUCT;
typedef struct MapDisplayValue
{
	S32 iInt;                               AST(NAME(Int))
	F32 fFloat;                             AST(NAME(Float))
	char *estrString;                       AST(NAME(String) ESTRING)

	bool bIsInt : 1;                        AST(NAME(IsInt))
	bool bIsFloat : 1;                      AST(NAME(IsFloat))
	bool bIsString : 1;                     AST(NAME(IsString))

	MapDisplayVariable *pDisplay;           AST(NAME(Display) UNOWNED)
} MapDisplayValue;

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GenGetMapDisplayValues);
void ui_GenExprGetMapDisplayValues(SA_PARAM_NN_VALID UIGen *pGen, const char *pchGroup)
{
	static const char **s_eapchVolumeNames;
	static Entity *s_pVolumeEntity;
	static const char *s_pchVolumeMap;
	static WorldVariable **s_eaWorldVariables;
	Entity *pEnt = entActivePlayerPtr();
	MapDisplayValue ***peaVars = ui_GenGetManagedListSafe(pGen, MapDisplayValue);
	MapState *pMapState;
	const char *pchZoneMap;
	S32 i, n, j, o, iCount = 0;

	pchGroup = allocFindString(EMPTY_TO_NULL(pchGroup));

	if (pEnt && pEnt->pPlayer)
	{
		pMapState = mapState_FromEnt(pEnt);
		pchZoneMap = zmapGetName(NULL);

		mapState_GetAllPublicVars(pMapState, &s_eaWorldVariables);
		eaIndexedEnable(&s_eaWorldVariables, parse_WorldVariable);

		n = eaSize(&s_VariableList.eaMapVariable);
		o = eaSize(&s_VariableListFiles.eaMapVariable);
		i = j = 0;
		while (i < n || j < o)
		{
			MapDisplayVariable *pDisplay = NULL;
			WorldVariable *pVar;
			MapDisplayValue *pVal;
			bool bIsNumber;
			F32 fNumber;

			if (i < n && j < o)
			{
				if (s_VariableList.eaMapVariable[i]->iDisplayOrder <= s_VariableListFiles.eaMapVariable[j]->iDisplayOrder)
					pDisplay = s_VariableList.eaMapVariable[i++];
				else
					pDisplay = s_VariableListFiles.eaMapVariable[j++];
			}
			else if (i < n)
			{
				pDisplay = s_VariableList.eaMapVariable[i++];
			...
			if (pchGroup && pDisplay->pchGroup != pchGroup)
				continue;

			if (pDisplay->pchZoneMap != pchZoneMap)
				continue;
			// Volumes are not sent to the client, server code will be needed to check the following condition
			if (pDisplay->pchVolume)
			{
				InteractVolumeStatus *pVolumeStatus = eaIndexedGetUsingString(&pEnt->pPlayer->InteractStatus.ppInteractVolumeStatus, pDisplay->pchVolume);
				if (!pVolumeStatus)
				{
					// Check to see if the list of requested volume names is still valid
					if (s_pVolumeEntity != pEnt || s_pchVolumeMap != pchZoneMap)
					{
						eaClearFast(&s_eapchVolumeNames);
						s_pVolumeEntity = pEnt;
						s_pchVolumeMap = pchZoneMap;
					}

					// If tracking for the volume hasn't been requested yet, make the request for it now
					if (eaFind(&s_eapchVolumeNames, pDisplay->pchVolume) < 0)
					{
						eaPush(&s_eapchVolumeNames, pDisplay->pchVolume);
						ServerCmd_gslEntity_TrackVolumeStatus(pDisplay->pchVolume);
					}
				}

				if (!pVolumeStatus || !pVolumeStatus->bInVolume)
					continue;
			}

			pVar = mapState_GetPublicVarByName(pMapState, pDisplay->pchVariableName);
			if (!pVar)
				continue;

			bIsNumber = pVar->eType == WVAR_INT || pVar->eType == WVAR_FLOAT;
			fNumber = pVar->eType == WVAR_INT ? pVar->iIntVal : pVar->eType == WVAR_FLOAT ? pVar->fFloatVal : 0;

			if (pDisplay->pValid)
			{
				if (bIsNumber && (fNumber < pDisplay->pValid->fLowValue || pDisplay->pValid->fHighValue < fNumber))
					continue;
			}
			else
			{
				if (pDisplay->pBar && (!bIsNumber || fNumber < pDisplay->pBar->fLowValue || pDisplay->pBar->fHighValue < fNumber))
					continue;
				if (pDisplay->bTimer && (!bIsNumber || fNumber < 0))
					continue;
			}

			pVal = eaGetStruct(peaVars, parse_MapDisplayValue, iCount++);

			pVal->bIsInt = pVar->eType == WVAR_INT;
			pVal->bIsFloat = pVar->eType == WVAR_FLOAT;
			pVal->bIsString = pVar->eType == WVAR_STRING || pVar->eType == WVAR_MESSAGE;

			switch (pVar->eType)
			{
			xcase WVAR_INT:
				pVal->iInt = pVar->iIntVal;
				pVal->fFloat = pVal->iInt;
				estrPrintf(&pVal->estrString, "%d", pVal->iInt);

			xcase WVAR_FLOAT:
				pVal->fFloat = pVar->fFloatVal;
				pVal->iInt = pVal->fFloat;
				estrPrintf(&pVal->estrString, "%f", pVal->fFloat);

			xcase WVAR_STRING:
			acase WVAR_MESSAGE:
				if (pVar->eType == WVAR_STRING)
					estrCopy2(&pVal->estrString, pVar->pcStringVal);
				else if (pVar->eType == WVAR_MESSAGE)
					estrCopy2(&pVal->estrString, TranslateDisplayMessage(pVar->messageVal));
				pVal->iInt = atoi(pVal->estrString);
				pVal->fFloat = atof(pVal->estrString);
			}

			pVal->pDisplay = pDisplay;
		}

		eaIndexedDisable(&s_eaWorldVariables);
		eaClear(&s_eaWorldVariables);
	}

	eaSetSizeStruct(peaVars, parse_MapDisplayValue, iCount);
	ui_GenSetManagedListSafe(pGen, peaVars, MapDisplayValue, true);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueFormat);
const char *ui_GenExprMapDisplayValueFormat(ExprContext *pContext, SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	static char *s_estr = NULL;
	...
	if (pValue && pValue->pDisplay)
	{
		const char *pchDisplayName = TranslateMessageRef(pValue->pDisplay->hDisplayName);
		if (pchDisplayName)
		{
			if (pValue->bIsInt)
			{
				FormatGameString(&s_estr, pchDisplayName,
					STRFMT_INT("Value", pValue->iInt),
					STRFMT_INT("Value1", pValue->iInt)
				);
			}
			else if (pValue->bIsFloat)
			{
				FormatGameString(&s_estr, pchDisplayName,
					STRFMT_FLOAT("Value", pValue->fFloat),
					STRFMT_FLOAT("Value1", pValue->fFloat)
				);
			}
			else
			{
				FormatGameString(&s_estr, pchDisplayName,
					STRFMT_STRING("Value", pValue->bIsString ? pValue->estrString : ""),
					STRFMT_STRING("Value1", pValue->bIsString ? pValue->estrString : "")
				);
			}
		}
	}

	return exprContextAllocString(pContext, NULL_TO_EMPTY(s_estr));
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueLabel);
const char *ui_GenExprMapDisplayValueLabel(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	if (pValue && pValue->pDisplay)
	{
		return TranslateMessageRef(pValue->pDisplay->hDisplayName);
	}
	return NULL;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueInt);
S32 ui_GenExprMapDisplayValueInt(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	return SAFE_MEMBER(pValue, iInt);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueFloat);
F32 ui_GenExprMapDisplayValueFloat(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	return SAFE_MEMBER(pValue, fFloat);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueString);
const char *ui_GenExprMapDisplayValueString(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	return SAFE_MEMBER(pValue, estrString);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueIsBar);
bool ui_GenExprMapDisplayValueIsBar(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	return pValue && pValue->pDisplay && pValue->pDisplay->pBar;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueBarPct);
F32 ui_GenExprMapDisplayValueBarPct(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	if (pValue && pValue->pDisplay && pValue->pDisplay->pBar)
	{
		return (pValue->fFloat - pValue->pDisplay->pBar->fLowValue) / (pValue->pDisplay->pBar->fHighValue - pValue->pDisplay->pBar->fLowValue);
	}
	return -1;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueBarLowLabel);
const char *ui_GenExprMapDisplayValueBarLowLabel(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	if (pValue && pValue->pDisplay && pValue->pDisplay->pBar)
	{
		return TranslateMessageRef(pValue->pDisplay->pBar->hLowMessage);
	}
	return NULL;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueBarHighLabel);
const char *ui_GenExprMapDisplayValueBarHighLabel(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	if (pValue && pValue->pDisplay && pValue->pDisplay->pBar)
	{
		return TranslateMessageRef(pValue->pDisplay->pBar->hHighMessage);
	}
	return NULL;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueBarName);
const char *ui_GenExprMapDisplayValueBarName(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	if (pValue && pValue->pDisplay && pValue->pDisplay->pBar)
	{
		return REF_STRING_FROM_HANDLE(pValue->pDisplay->pBar->hBar);
	}
	return NULL;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueIsTimer);
bool ui_GenExprMapDisplayValueIsCountdown(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	return pValue && pValue->pDisplay && pValue->pDisplay->bTimer;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MapDisplayValueInfo);
const char *ui_GenExprMapDisplayValueInfo(SA_PARAM_OP_VALID MapDisplayValue *pValue)
{
	if (pValue && pValue->pDisplay)
	{
		return TranslateMessageRef(pValue->pDisplay->hInfo);
	}
	return NULL;
}
