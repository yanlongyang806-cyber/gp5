...
typedef struct PetData
{
	...
	U32					uiCreatePowerID;
	U32					uiCreateAttribModIdx;
	U32					uiCreateAttribModCount;
...
SA_RET_OP_VALID Entity* PuppetExpr_GetPuppetEntityBySetNameAndType(SA_PARAM_OP_VALID Entity* pEntity, const char *pchSet, const char *pchType)
{
	...
	return (pEntity && pSet) ? entity_GetPuppetEntityByType( pEntity, pchType, pSet, false, true ) : NULL;
}
...
{
	CharClassCategorySet *pSet = RefSystem_ReferentFromString(g_hCharacterClassCategorySetDict, pchSet);
	return (pEnt && pSet) ? entity_GetPuppetEntityByType( pEnt, pchClassType, pSet , true, true ) : NULL;
}
							  ...
							  PetRelationship *pPet)
{
	...
	else if(pPet && pPet->pcName)
	{
		pchPetName = pPet->pcName;
	}
	else
	{
		return;
	...
	if (pOwner)
	{
		...
		if(pEntity)
		{
			iID = uCritterPetID ? entGetContainerID(pOwner) : entGetContainerID(pEntity);
			iType = uCritterPetID ? GLOBALTYPE_ENTITYCRITTER : entGetType(pEntity);
		}
		else if(pPet)
		{
			iID = pPet->conID;
			iType = GLOBALTYPE_ENTITYSAVEDPET;	//must be this
		}
		else if(pOwner && uCritterPetID)
		{
			iID = entGetContainerID(pOwner);
			iType = GLOBALTYPE_ENTITYCRITTER;
		}
					 ...
					 bool bAllowUnsubscribed)
{
	...
	if (ePetFlags & (kUIPetListFlag_AllPets|kUIPetListFlag_AllPuppets))
	{
			...
			SAFE_MEMBER(pEntity->pSaved->pPuppetMaster, curTempID))
		{
			...
			{
				gclPetList_AddPet(peaPets, &iSize, pEntity, pEntity, NULL, pClass, 0, pEntity->pSaved->pPuppetMaster->curTempID, false, false, true, NULL);
			}
		}
		for (i = 0;i < eaSize(&pEntity->pSaved->ppOwnedContainers); i++)
		{
			...
			if(pPuppet)
			{
				if((ePetFlags & kUIPetListFlag_InactivePuppets) == 0 && pPuppet->eState != PUPPETSTATE_ACTIVE)
				{
					// unless we want in active puppets skip the inactive ones
					continue;
				}

				if(!piIncludeClassTypes || eaiFind(&piIncludeClassTypes, pPuppet->eType) < 0)
				{
					// wrong type, skip it
					// doing the check here so a skipping a space puppet won't cause a subscription by Entity_GetEntFromPuppetOrPet
					continue;
				}
			}

			if(bAllowUnsubscribed)
			{
				pPetEnt = GET_REF(pPet->hPetRef);
			}
			else
			{
				pPetEnt = Entity_GetEntFromPuppetOrPet(pEntity, pPuppet, pPet);
				if(!pPetEnt)
				{
					continue;
				}
...

			{
				...
				if(pPetEnt)
				{
					pClass = GET_REF(pPetEnt->pChar->hClass);
				}
				else
				{
					pClass = GET_REF(pPet->hCharacterClassRef);
				}

				if(!pClass)
				{
					// missing data, subscribe all pets
					(void)Entity_GetEntFromPuppetOrPet(pEntity, pPuppet, pPet);
					continue;
				}
				...
				gclPetList_AddPet(peaPets, &iSize, pEntity, pPetEnt, NULL, pClass, 0, SAFE_MEMBER(pPuppet, curID), bPuppetActive, bActive, bCurrent, pPet);
	...
	if (ePetFlags & kUIPetListFlag_Critters)
	{
		for(i=0;i<eaSize(&pEntity->pSaved->ppAllowedCritterPets);i++)
		{
			...
			{
				gclPetList_AddPet(peaPets, &iSize, pEntity, NULL, pDef, pClass, uCritterID, 0, false, false, false, NULL);
			}
		}
	}
							  ...
							  bool bAllowUnsubscribed)
{
	...
	iSize = gclGenGetPetList(pGen,pEntity,piClassTypes,ePetFlags,pSortData,bExcludeAlreadySummoned, bAllowUnsubscribed); 
					   ...
					   const char *pchSortFirst, const char* pchSortLast)
{
	...
	return gclGenGetPetListEx(pGen,pEnt,pchClassTypeList,ePetFlags,pchSortFirst,pchSortLast,false,false);
}


AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GetPetListUnsubscribed");
S32 PetExpr_GetPetListUnsubscribed(SA_PARAM_NN_VALID UIGen *pGen,
	const char* pchClassTypeList,
	S32 ePetFlags, 
	bool bUseLastName, int iMaxLen,
	const char *pchSortFirst, const char* pchSortLast ,bool bAllowUnsubscribed)
{
	Entity* pEnt = entActivePlayerPtr();
	return gclGenGetPetListEx(pGen,pEnt,pchClassTypeList,ePetFlags,pchSortFirst,pchSortLast,false,bAllowUnsubscribed);
}
								 ...
								 const char *pchSortFirst, const char* pchSortLast)
{
	...
	return gclGenGetPetListEx(pGen,pEntity,pchClassTypeList,ePetFlag|ePuppetFlag,pchSortFirst,pchSortLast,true,false);
}
...
static int SortSummons(const void **ppLeft, const void **ppRight, const void *pContext)
{
	const PetData *pLeft = *ppLeft;
	const PetData *pRight = *ppRight;
	int diff;

	diff = pLeft->iStackIdx - pRight->iStackIdx;
	if (diff != 0)
		return diff;

	diff = pLeft->uiCreateAttribModIdx - pRight->uiCreateAttribModIdx;
	if (diff != 0)
		return diff;
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ItemGetSummons);
void gclPetExpr_ItemGetSummons(SA_PARAM_NN_VALID UIGen *pGen, const char *pchItemKey, S32 iSummonsPerStack, S32 iStacks, bool bInsertEmpty)
{
	static Power **s_eaPowers;
	static AttribMod **s_eaMods;
	static PowerCategory *s_pCarrier_UI_2up;
	static PowerCategory *s_pCarrier_UI_4up;
	static PowerCategory *s_pCarrier_UI_6up;
	static PetData **s_eaCache;
	UIInventoryKey Key = {0};
	S32 i, j, n, p, iCount = 0, iMaxStack = iStacks;
	PetData ***peaPetData = ui_GenGetManagedListSafe(pGen, PetData);

	if (gclInventoryParseKey(pchItemKey, &Key) && Key.pSlot && Key.pSlot->pItem && Key.pEntity && Key.pEntity->pChar)
	{
		n = eaSize(&Key.pSlot->pItem->ppPowers);
		for (i = 0; i < n; i++)
		{
			if (Key.pSlot->pItem->ppPowers[i] && GET_REF(Key.pSlot->pItem->ppPowers[i]->hDef))
			{
				eaPush(&s_eaPowers, Key.pSlot->pItem->ppPowers[i]);
			}
		}

		n = eaSize(&Key.pEntity->pChar->modArray.ppMods);
		p = eaSize(&s_eaPowers);
		for (i = 0; i < n; i++)
		{
			AttribMod *pMod = Key.pEntity->pChar->modArray.ppMods[i];
			PowerDef *pPowerDef = pMod ? GET_REF(pMod->hPowerDef) : NULL;

			if (!pPowerDef || !pMod->erCreated)
				continue;

			for (j = 0; j < p; j++)
			{
				if (GET_REF(s_eaPowers[j]->hDef) == pPowerDef)
					break;
			}

			if (j < p)
				eaPush(&s_eaMods, pMod);
		}

		n = eaSize(&s_eaMods);
		for (i = 0; i < n; i++)
		{
			Entity *pPetEnt = entFromEntityRef(PARTITION_CLIENT, s_eaMods[i]->erCreated);
			PetData *pPetData = NULL;
			Power *pPower = NULL;
			PowerDef *pPowerDef = NULL;
			S32 iStack = 0;

			if (!pPetEnt || !pPetEnt->pCritter || !pPetEnt->pCritter->uiEntCreatePowerID)
			{
				eaRemove(&s_eaMods, i);
				i--;
				n--;
			...
			for (j = 0; j < p; j++)
			{
				if (s_eaPowers[j]->uiID == pPetEnt->pCritter->uiEntCreatePowerID)
					break;
			}
			if (j >= p)
			{
			...
			for (j = iCount; j < p; j++)
			{
				if ((*peaPetData)[j]->pEntity == pPetEnt)
				{
					if (j != iCount)
						eaMove(peaPetData, j, iCount);
					pPetData = (*peaPetData)[iCount++];
					break;
				}
			}
			...
			if (!pPetData)
			{
				pPetData = StructCreate(parse_PetData);
				eaInsert(peaPetData, pPetData, iCount++);
			}

			// Figure out what stack index it is, this is completely
			// arbitrary since it won't actually reflect the order
			// in which it was created.
			for (j = 0; j < i; j++)
			{
				if (GET_REF(s_eaMods[j]->hPowerDef) == GET_REF(s_eaMods[i]->hPowerDef)
					&& s_eaMods[j]->uiDefIdx == s_eaMods[i]->uiDefIdx)
				{
					iStack++;
				}
			}
			...
			pPetData->pEntity = pPetEnt;
			pPetData->pchAiStance = NULL;
			pPetData->pchAiState = NULL;
			pPetData->pchName = entGetLocalName(pPetEnt);
			pPetData->iLevel = entity_GetSavedExpLevel(pPetEnt);
			pPetData->iCombatLevel = entity_GetCombatLevel(pPetEnt);

			pPetData->uiCreatePowerID = pPetEnt->pCritter->uiEntCreatePowerID;
			pPetData->uiCreateAttribModIdx = pPetEnt->pCritter->uiEntCreateAttribModDefIdx;
			pPetData->uiCreateAttribModCount = eaSize(&pPowerDef->ppOrderedMods);
			pPetData->iStackIdx = iStack;

			pPetData->pPower = pPower;
			pPetData->pPowerDef = pPowerDef;
			pPetData->pchIcon = gclGetBestIconName(pPowerDef->pchIconName, "Power_Generic");

			pPetData->bSummon = true;
			pPetData->bDead = false;
			pPetData->bPet = false;

			MAX1(iMaxStack, iStack);
		}

		eaClearFast(&s_eaPowers);
		eaClearFast(&s_eaMods);
	}

	// Move extra elements to cache
	while (eaSize(peaPetData) > iCount)
	{
		PetData *pStale = eaPop(peaPetData);
		StructReset(parse_PetData, pStale);
		eaPush(&s_eaCache, pStale);
	}

	// Sort actual elements
	eaStableSort(*peaPetData, NULL, SortSummons);

	// Pad stacks to summons per stack
	iCount = 0;
	for (j = 0; j < iMaxStack; j++)
	{
		PetData *pData;
		U32 uiCreateAttribModCount = 0;

		for (i = 0; i < iSummonsPerStack; i++)
		{
			pData = eaGet(peaPetData, iCount);
			if (!pData || pData->iStackIdx != j)
				break;
			pData->iSummonIdx = i;
			MAX1(uiCreateAttribModCount, pData->uiCreateAttribModCount);
			iCount++;
		}

		for (; bInsertEmpty && i < iSummonsPerStack; i++)
		{
			// Insert dummy stack elements
			pData = eaSize(&s_eaCache) ? eaPop(&s_eaCache) : StructCreate(parse_PetData);
			pData->iStackIdx = j;
			pData->uiCreateAttribModIdx = uiCreateAttribModCount + i;
			pData->iSummonIdx = i;
			eaInsert(peaPetData, pData, iCount++);
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(PetCommand_SendMessage);
void PetCommandExpr_SendMessage(SA_PARAM_OP_VALID Entity *pPet, const char *pchMessage)
{
	Entity *pPlayer = entActivePlayerPtr();

	if (pPlayer && pPet && pPet->erOwner == entGetRef(pPlayer))
	{
		ServerCmd_PetCommands_SendMessage(entGetRef(pPet), pchMessage);
	}
}
