...
AUTO_EXPR_FUNC(entityutil) ACMD_NAME("Team_OpenPetTeamSlots");
S32 TeamExpr_OpenPetTeamSlots(SA_PARAM_OP_VALID Entity *pEnt)
{
	int partitionIdx = pEnt ? entGetPartitionIdx(pEnt) : PARTITION_CLIENT;
	RegionRules *pRules = getRegionRulesFromZoneMap(zmapGetInfo(NULL));
	Team *pTeam = pEnt ? team_GetTeam(pEnt) : NULL;
	Entity **eaTeamMembers = NULL;
	S32 iPetsPerPlayer = SAFE_MEMBER(pRules, iAllowedPetsPerPlayer);
	S32 iNonTeamPetsPerPlayer = SAFE_MEMBER(pRules, iUnteamedPetsPerPlayer);
	S32 iMembers = 0, iOpenSlots = 0;
	S32 i, n, iTeamSizeLimit;

	iPetsPerPlayer = MAX(iPetsPerPlayer, 0);
	iNonTeamPetsPerPlayer = MAX(iNonTeamPetsPerPlayer, 0);

	eaStackCreate(&eaTeamMembers, TEAM_MAX_SIZE);

	if (pTeam && eaSize(&pTeam->eaMembers) > 0)
	{
		n = eaSize(&pTeam->eaMembers);
		for (i = 0; i < n; i++)
		{
			Entity *pPlayerEnt = entFromContainerID(partitionIdx, GLOBALTYPE_ENTITYPLAYER, pTeam->eaMembers[i]->iEntID);
			if (pPlayerEnt)
				eaPush(&eaTeamMembers, pPlayerEnt);
		}
	}
	else if (pEnt)
	{
		eaPush(&eaTeamMembers, pEnt);
	}

	n = eaSize(&eaTeamMembers);
	iTeamSizeLimit = MIN((iPetsPerPlayer + 1) * n, TEAM_MAX_SIZE);
	for (i = 0; i < n; i++)
	{
		Entity *pPlayerEnt = eaTeamMembers[i];
		S32 iActivePets = 0;
		...
		if (pPlayerEnt->pSaved)
		{
			...
			// Count saved pets
			o = ea32Size(&pPlayerEnt->pSaved->ppAwayTeamPetID);
			for (j = 0; j < o; j++)
			{
				Entity *pPetEnt = entFromContainerID(partitionIdx, GLOBALTYPE_ENTITYSAVEDPET, pPlayerEnt->pSaved->ppAwayTeamPetID[j]);
				if (!pPetEnt)
					continue;
			...
			// Count critter pets
			o = eaSize(&pPlayerEnt->pSaved->ppCritterPets);
			for (j = 0; j < o; j++)
			{
				Entity *pCritterEnt = entFromEntityRef(partitionIdx, pPlayerEnt->pSaved->ppCritterPets[j]->erPet);
				if (!pCritterEnt)
					continue;
			...
			iActivePets = MAX(0, iActivePets - iNonTeamPetsPerPlayer);

			iMembers += iActivePets + 1;
			iOpenSlots += MAX(iPetsPerPlayer, iActivePets) - iActivePets;
		}
	}

	eaDestroy(&eaTeamMembers);
	return MIN(iTeamSizeLimit - iMembers, iOpenSlots);
}
...
//
// Vote Kick
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(team_IsVoteKick);
bool teamExpr_IsVoteKick(void)
{
	Entity *pPlayer = entActivePlayerPtr();
	if (pPlayer) {
		Team *pTeam = team_GetTeam(pPlayer);
		if (pTeam)
		{
			return(pTeam->bVoteRequiredForKick);
//			return true;
		}
	}
	return false;
...
// Silly wrapper to Vote Kick command on the server since we cannot set up the proper double quote structure in the uigen. Bleh.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(team_StartVoteKick);
void teamExpr_StartVoteKick(const char* pchTarget, const char* pchReason)
{
	ServerCmd_Team_VoteKick(pchTarget, pchReason); // gslTeam_cmd_VoteKick in TeamCommands.c
}



AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_PRIVATE ACMD_CLIENTCMD;
void TeamUI_VoteKick_StartSingleVote(const char* pTargetDisplayName, const char* pKickReason)
{
	UIGen *pUIGen;
	const char* pchGenName = "TeamVoteKick_Window";
