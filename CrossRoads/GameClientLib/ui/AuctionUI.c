...
static void AuctionListPageCopy(AuctionLot ***peaLots, AuctionLotList **ppAuctionList, int iStart, int iPageSize)
{
	...
	if (s_bClearSearchResultsNextTick && ppAuctionList == &s_pAuctionLotList)
	{
		...
		StructDestroySafe(parse_AuctionLotList, &s_FilteredLots);
		pAuctionList = NULL;
...
{
	Entity *pEnt = entActivePlayerPtr();
	return pEnt && entGetContainerID(pEnt) != pLot->ownerID && Auction_AcceptsBids(pLot) && pLot->uExpireTime > timeServerSecondsSince2000();
}
...
{
	Entity *pEnt = entActivePlayerPtr();
	return pEnt && entGetContainerID(pEnt) != pLot->ownerID && Auction_AcceptsBuyout(pLot) && pLot->uExpireTime > timeServerSecondsSince2000();
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(AuctionCanBeCanceled);
bool exprAuctionCanBeCanceled(SA_PARAM_NN_VALID AuctionLot *pLot)
{
	return Auction_CanBeCanceled(pLot) && pLot->uExpireTime > timeServerSecondsSince2000();
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(PlayerIsAtAuctionHouse);
bool exprPlayerIsAtAuctionHouse()
{
	Entity *pEnt = entActivePlayerPtr();
	ContactDialog* pDialog = SAFE_MEMBER3(pEnt, pPlayer, pInteractInfo, pContactDialog);
	bool bPlayerIsAtAuctionHouse = (pDialog && pDialog->screenType == ContactScreenType_Market);
	
	return bPlayerIsAtAuctionHouse;
}
