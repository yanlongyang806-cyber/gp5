...
typedef struct QueueDesc
{
	...
	U32 iQueuedPlayerCount;
	U32 iOfferedPlayerCount;
	U32 iAcceptedPlayerCount;
	U32 iInMapPlayerCount;
	S32 iPlayerCountMax;
	S32 iPlayerCountMin;
	...
	int iMinLevel;
	int iMaxLevel;
	int iCompositeLevel;	// A combination of Min and Max level so we have a single TPI field for sorting.
	REF_TO(AllegianceDef) hGroup1Allegiance;
	REF_TO(AllegianceDef) hGroup2Allegiance;
	int  iRequiredGearRating;
	bool bInQueue : 1;
	bool bBolster : 1;
...
typedef enum QueueGetInstanceListFlag {
	...
	// Flags to distinguish PvE vs. PvP maps. Not used for GenGetQueueInstances (which uses mapType instead)
	kGetInstanceListFlag_PvE = 0x00001000,
	kGetInstanceListFlag_PvP = 0x00002000,
	// Flags to distinguish In queue from not.
	// We are not in the queue (state_None or no instance)
	kGetInstanceListFlag_NotInQueue = kGetInstanceListFlag_ExcludeQueued,
	// We are in the queue and waiting for it to process us (state_InQueue)
	kGetInstanceListFlag_InQueueWaiting = 0x00004000,
	// We are in the queue and no longer waiting for it to process us (any other state)
	kGetInstanceListFlag_InQueueOther = 0x00008000,
...
static QueueIntArray* gclQueue_CreateInviteListFromMatch(SA_PARAM_NN_VALID QueueMatch* pMatch)
{
	...
	for (i = eaSize(&s_pInviteMatch->eaGroups)-1; i >= 0; i--)
	{
		...
		for (j = eaSize(&pGroup->eaMatchMembers)-1; j >= 0; j--)
		{
			ea32Push(&pList->piArray, pGroup->eaMatchMembers[j]->uEntID);
		}
	}
	return pList;
}
...
// If the queue is not on the player, this will return QueueCannotUseReason_Other. 
...
// The client will not know about a named queue if the name is bad, or if the game server has not sent data about the queue to the client.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(queue_ClientKnowsAboutNamedQueue);
bool exprQueue_ClientKnowsAboutNamedQueue(SA_PARAM_OP_VALID Entity *pEnt, const char* pchQueueDefName)
{
	PlayerQueueInfo* pQueueInfo = SAFE_MEMBER2(pEnt, pPlayer, pPlayerQueueInfo);
	if (pQueueInfo)
	{
...
{	
	...
	pDesc->iRequiredGearRating = (int)pDef->Requirements.uRequiredGearRating;
	...
	if (pInstance && pInstance->pParams)
	{
		...
		pDesc->iLevelBandIndex = pInstance->pParams->iLevelBandIndex;
		pDesc->iQueuedPlayerCount = pInstance->iQueuedPlayerCount;
		pDesc->iOfferedPlayerCount = pInstance->iOfferedPlayerCount;
		pDesc->iAcceptedPlayerCount = pInstance->iAcceptedPlayerCount;
		pDesc->iInMapPlayerCount = pInstance->iInMapPlayerCount;
		...
		{
			pDesc->iTotalPlayerCount = pDesc->iQueuedPlayerCount + pDesc->iOfferedPlayerCount + pDesc->iAcceptedPlayerCount + pDesc->iInMapPlayerCount;
			pDesc->iMapKey = 0;
			pDesc->uElapsedMapTime = 0;
		}
	}
	...
	{
		pDesc->iLevelBandIndex = queue_GetQueueLevelBandIndex(pEnt, pDef);
		pDesc->uiInstanceID = 0;
		pDesc->iMapKey = 0;
		pDesc->uAvgWaitTime = 0;
		pDesc->uElapsedMapTime = 0;
		pDesc->uiOwnerID = 0;
		pDesc->iQueuedPlayerCount = 0;
		pDesc->iOfferedPlayerCount = 0;
		pDesc->iAcceptedPlayerCount = 0;
		pDesc->iInMapPlayerCount = 0;
	...
	queue_GetMinMaxLevel(pDef, pDesc->iLevelBandIndex, &(pDesc->iMinLevel), &(pDesc->iMaxLevel));
	pDesc->iCompositeLevel = 1000 * pDesc->iMinLevel + pDesc->iMaxLevel; // A combination of Min and Max level so we have a single TPI field for sorting.
									 ...
									 )
{
		...
		)
	{
		...
		for (i = 0; i < n; i++)
		{
			...
			if (!(eOptions & kGetInstanceListFlag_PrivateGames))
			{
			...
			if (eOptions & kGetInstanceListFlag_Templates)
			{
				...
				if (eOptions & kGetInstanceListFlag_TemplatesIncludeInstance)
				{
					...
					FOR_EACH_IN_EARRAY(pQueue->eaInstances, PlayerQueueInstance, pInstance)
					{
						if (pInstance->eQueueState != PlayerQueueState_None || !pIncludeInstance)
						{
							...
							else if (!(kGetInstanceListFlag_PublicGames & eOptions))
							{
								continue;
							}

							if (!pIncludeInstance && pInstance->eQueueState == PlayerQueueState_None)
							{
	...
	if (pEntity && pEntity->pPlayer && pQueueInfo)
	{
		...
		if (s_pInviteMatch)
		{
			for (i = eaSize(&s_pInviteMatch->eaGroups)-1; i >= 0; i--)
			{
				...
				for (j = eaSize(&pGroup->eaMatchMembers)-1; j >= 0; j--)
				{
					U32 uiMemberID = pGroup->eaMatchMembers[j]->uEntID;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//  Simple queue model calls for Neverwinter without all the bells and whistles needed for STO with its private queues,
//  filtering, ground/space, etc. etc. 
//
//  We only use the following eOptions:
//		kGetInstanceListFlag_PvP
//		kGetInstanceListFlag_PvE
//		kGetInstanceListFlag_InQueueWaiting
//		kGetInstanceListFlag_InQueueOther
//		kGetInstanceListFlag_NotInQueue
//
//  For now sort first by Min Level, then Max Level, then gear rating, then alphabetically. This is currently hard-coded. We could open it up
//    with some more options on the uigen call.

static int SortFillQueueDescs(const QueueDesc **ppQueueDescA, const QueueDesc **ppQueueDescB)
{
	const QueueDesc* pA = (*ppQueueDescA);
	const QueueDesc* pB = (*ppQueueDescB);

	if (pA->iMinLevel!=pB->iMinLevel)
	{
		return(pA->iMinLevel-pB->iMinLevel);
	}

	if (pA->iMaxLevel!=pB->iMaxLevel)
	{
		return(pA->iMaxLevel-pB->iMaxLevel);
	}
	if (pA->iRequiredGearRating!=pB->iRequiredGearRating)
	{
		return(pA->iRequiredGearRating-pB->iRequiredGearRating);
	}
	return stricmp(pA->pchQueueName,pB->pchQueueName);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("FillQueueList");
void gclQueue_FillQueueList(SA_PARAM_NN_VALID UIGen *pGen,
									 SA_PARAM_OP_VALID Entity *pEntity,
									 U32 eOptions)
{
	QueueDesc ***peaQueues = ui_GenGetManagedListSafe(pGen, QueueDesc);
	...
	bool bPvPQueues = (eOptions & kGetInstanceListFlag_PvP)!=0;
	bool bPvEQueues = (eOptions & kGetInstanceListFlag_PvE)!=0;
	bool bQueuedWaiting = (eOptions & kGetInstanceListFlag_InQueueWaiting)!=0;
	bool bQueuedOther = (eOptions & kGetInstanceListFlag_InQueueOther)!=0;
	bool bUnqueued = (eOptions & kGetInstanceListFlag_NotInQueue)!=0;

	int iCount = 0;
	int i;

	if(pPlayerQueueInfo!=NULL)
	{
		for (i = 0; i < eaSize(&pPlayerQueueInfo->eaQueues); i++)
		{
			...
			if (pDef!=NULL &&
				(((pDef->MapSettings.eMapType == ZMTYPE_PVP) && bPvPQueues) ||
				 ((pDef->MapSettings.eMapType == ZMTYPE_QUEUED_PVE) && bPvEQueues)) )
			{
				PlayerQueueInstance *pIncludeInstance = NULL;
				int j;
				int iNumInstances = eaSize(&pQueue->eaInstances);
	
				if (iNumInstances==0)
				{
					// We have no instances at this point. The queue is available, but we haven't created an insntance for it yet.
					//   This will be the case before we have ever joined the queue.
					if (bUnqueued)
					{
						pDesc = eaGetStruct(peaQueues, parse_QueueDesc, iCount++);
						gclQueue_FillDesc(pEntity, pDesc, pDef, pQueue, NULL, NULL, kControlSchemeRegionType_None);
					}
				}
				else
				{
					// Look through the instances. Depending on the state and request flags, create a desc for it.
					for (j=0;j<eaSize(&pQueue->eaInstances);j++)
					{
						bool bInclude=false;
						PlayerQueueInstance *pInstance = pQueue->eaInstances[j];
						...
						else if (pInstance->eQueueState == PlayerQueueState_InQueue)
						{
							bInclude = bQueuedWaiting;
						}
						else
						{
							bInclude = bQueuedOther;
						}
		
						if (bInclude)
						{
							pDesc = eaGetStruct(peaQueues, parse_QueueDesc, iCount++);
							gclQueue_FillDesc(pEntity, pDesc, pDef, pQueue, pInstance, NULL, kControlSchemeRegionType_None);
						}
					}
				}
			}
	...
	eaSetSizeStruct(peaQueues, parse_QueueDesc, iCount);
	eaQSort(*peaQueues, SortFillQueueDescs);		
	ui_GenSetManagedListSafe(pGen, peaQueues, QueueDesc, true);
}
...
///////
//
//  Removed several get filtered queue list functions from here.
//  They appear to have been used for STO once upon a time but
//  have been replaced by GenGetQueueInstances with appropriate flags
//////
										...
										const char* pchDescFormat, const char* pchGroupCountFormat)
{
	...
	estrStackCreate(&estrResult);
	if (pDesc->bAllGroupsSameAffiliation || !pDesc->piGroupPlayerCount)
	{
									 ...
									 QueueRulesData*** peaLevelBands)
{
	...
	if (pDef && peaLevelBands)
	{
		if (uiThisFrame != s_uiLastFrame)
		{
			...
			if (pInstance && pInstance->pParams)
			{
				iLevelBandIndex = pInstance->pParams->iLevelBandIndex;
			}
			else
			{
				iLevelBandIndex = queue_GetQueueLevelBandIndex(pEntity, pDef);
			}

			if (iLevelBandIndex > -1)
			{
				QueueLevelBand* pLevelBand = eaGet(&pDef->eaLevelBands,iLevelBandIndex);
				if (pLevelBand)
				{
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("queue_IsLeaverPenaltyForAFK");
bool exprQueueIsPenaltyAFK(S32 eQueueCategory)
{
	Entity* pEnt = entActivePlayerPtr();
	...
	if (pPenaltyData)
	{
		if (eQueueCategory >= 0)
		{
			...
			if (pCategoryData)
			{
				bAnswer = pCategoryData->bAFKPenalty;
			}
		}
		else
		{
