...
#define CacheNewUse(pContext, pCache) (CacheUpdate(pCache), gclPaperdoll_CreateHeadshotData((pContext), NULL, (pCache)->pResult, NULL, NULL, NULL, NULL, false, false))
...
SA_RET_OP_VALID PlayerCostume *gclCreateGuildEmblemCostume(ContainerID iGuildID, const char *pchSkeleton, const char *pchEmblemBone, GuildEmblemData *pEmblemData, bool bNoBackground, bool bNoDetail)
{
	...
	{
		if (pCachedCostume->pSource1 == &s_GuildEmblem && pCachedCostume->pvKey == (void*)iGuildID && pCachedCostume->iKeyInt == pEmblemData->iVersion)
		{
	...
	if (pEmblemPart)
	{
		...
		if (pEmblemData->pcEmblem3 && !bNoDetail)
		{
	...
	pCache->iKeyInt = pEmblemData->iVersion;
...
static Entity *gclExprPaperdollEntityOldPuppetCostumeBehavior(SA_PARAM_OP_VALID Entity* pEntity, SA_PARAM_OP_VALID SA_POST_VALID bool *pbForceActiveSlot)
{
	// STO: Compatibility behavior after moving costumes from ground puppet to
	// primary entity. Since I doubt other games in the future will be using
	// puppets, this will probably not cause another game any problems.
	if (pEntity && entGetType(pEntity) != GLOBALTYPE_ENTITYPLAYER)
	{
		Entity *pPlayer = entActivePlayerPtr();
		Entity *pSubEntity = entity_GetSubEntity(PARTITION_CLIENT, pPlayer, entGetType(pEntity), entGetContainerID(pEntity));
		if (pSubEntity == pEntity && pSubEntity->pSaved
			&& pSubEntity->pSaved->conOwner.containerType == entGetType(pPlayer)
			&& pSubEntity->pSaved->conOwner.containerID == entGetContainerID(pPlayer))
		{
			bool bActiveEnt = false;

			if (pPlayer->pSaved && pPlayer->pSaved->pPuppetMaster)
			{
				if (pPlayer->pSaved->pPuppetMaster->curTempID == entGetContainerID(pEntity))
					bActiveEnt = true;
				else if (pPlayer->pSaved->pPuppetMaster->curID == entGetContainerID(pEntity))
					bActiveEnt = true;
			}

			// If the requested entity is the active entity, use the active costume
			// on the player with all powers + items and anything else attached to it.
			if (bActiveEnt)
			{
				if (pbForceActiveSlot)
					*pbForceActiveSlot = false;
				return pPlayer;
			}

			// If the requested entity is the ground puppet, but the ground puppet
			// isn't the active puppet. Build it from the active slot on the player
			// entity.
			if (GetCharacterClassEnum(pEntity) == StaticDefineIntGetInt(CharClassTypesEnum, "Ground"))
			{
				if (pbForceActiveSlot)
					*pbForceActiveSlot = true;
				return pPlayer;
			}
		}
	}

	// Build the costume from entity.
	return pEntity;
}
...
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromCostumeSlotCompat(ExprContext* pContext,
																						SA_PARAM_OP_VALID Entity* pEntity,
																						S32 iSlotIndex)
{
	bool bForceActiveSlot = false;
	pEntity = gclExprPaperdollEntityOldPuppetCostumeBehavior(pEntity, &bForceActiveSlot);
	if (pEntity && pEntity->pSaved)
	{
		PlayerCostumeSlot* pSlot = eaGet(&pEntity->pSaved->costumeData.eaCostumeSlots, iSlotIndex);
		if (pSlot)
		{
			return gclPaperdoll_CreateHeadshotData(pContext, NULL, pSlot->pCostume, NULL, NULL, NULL, NULL, bForceActiveSlot, false);
		}
	}
	return NULL;
}
...
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromEntCompat(ExprContext* pContext,
																				SA_PARAM_OP_VALID Entity* pEntity)
{
	bool bForceActiveSlot = false;
	pEntity = gclExprPaperdollEntityOldPuppetCostumeBehavior(pEntity, &bForceActiveSlot);
	if (pEntity)
	{
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, NULL, pEntity, NULL,  NULL, NULL, bForceActiveSlot, false);
	}
	return NULL;
}

// Gets the Entity's PlayerCostume stored on the costume slot at the specified index
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("PaperdollGetHeadshotFromPuppetCostumeSlot");
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromPuppetCostumeSlot(ExprContext* pContext,
																						SA_PARAM_OP_VALID Entity* pEntity, 
																						const char* pchPuppetType,
																						S32 iSlotIndex)
{
	CharClassCategorySet *pSet = SAFE_GET_REF3(pEntity, pSaved, pPuppetMaster, hPreferredCategorySet);
	Entity *pPuppetEnt = entity_GetPuppetEntityByType(pEntity, pchPuppetType, pSet, false, true);
	if (pPuppetEnt && pPuppetEnt->pSaved)
	{
		PlayerCostumeSlot* pSlot = eaGet(&pPuppetEnt->pSaved->costumeData.eaCostumeSlots, iSlotIndex);
		if (pSlot)
		{
			return gclPaperdoll_CreateHeadshotData(pContext, NULL, pSlot->pCostume, NULL, NULL, NULL, NULL, false, false);
		}
	}
	return NULL;
}
...
// Gets the appropriate costume stored on the Entity
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("PaperdollGetHeadshotFromEntPuppet");
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromEntPuppet(ExprContext* pContext,
																				SA_PARAM_OP_VALID Entity* pEntity,
																				const char* pchPuppetType)
{
	CharClassCategorySet *pSet = SAFE_GET_REF3(pEntity, pSaved, pPuppetMaster, hPreferredCategorySet);
	...
	if (pPuppetEnt)
	{
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, NULL, pPuppetEnt, NULL,  NULL, NULL, false, false);
	}
	return NULL;
}

// Gets the Entity's PlayerCostume stored on the costume slot at the specified index
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("PaperdollGetHeadshotFromDirectCostumeSlot");
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromDirectCostumeSlot(ExprContext* pContext,
																						SA_PARAM_OP_VALID Entity* pEntity, 
																						S32 iSlotIndex)
{
	if (pEntity && pEntity->pSaved)
...
// Gets the appropriate costume stored on the Entity
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("PaperdollGetHeadshotFromDirectEnt");
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromDirectEnt(ExprContext* pContext,
																				SA_PARAM_OP_VALID Entity* pEntity)
{
	if (pEntity)
	{
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, NULL, pEntity, NULL,  NULL, NULL, false, false);
	}
	return NULL;
}
	...
	SA_PARAM_OP_VALID PossibleCharacterChoice* pChoice, S32 iIndex)
{
	...
	{		
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, pChoice->eaCostumes[iIndex]->pConstCostume, NULL, NULL,  NULL, NULL, false, false);
	}
	return NULL;
}
	...
	SA_PARAM_OP_VALID Item* pItem, const char* pchInvisCostumeName, bool bOverrideHeadshotStyle)
{
	...
	if (pItem && pItemDef)
	{
		...
		for(iCostume=eaSize(&pItemDef->ppCostumes)-1; iCostume>=0; iCostume--)
		{
			...
			if(bValid || iCostume == 0)
			{
				...
				{
					return gclPaperdoll_CreateHeadshotData(pContext, NULL, pCostume, NULL, NULL, pchHeadshotStyle, NULL, false, false);
				}
			}
		}
	}
	...
	{
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, NULL, NULL, NULL, NULL, NULL, false, false);
	}
	return NULL;
}
																					 ...
																					 const char* pchPlayerCostume)
{
	if (pchPlayerCostume && pchPlayerCostume[0])
	{
		...
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, pPlayerCostume, NULL, NULL, NULL, NULL, false, false);
	}
	return NULL;
}
																			   ...
																			   const char* pchCostumeName)
{
	if (pchCostumeName && pchCostumeName[0])
	{
		...
		return gclPaperdoll_CreateHeadshotData(pContext, pWLCostume, NULL, NULL, NULL, NULL, NULL, false, false);
	}
	return NULL;
}
...
static PaperdollHeadshotData* gclPaperdoll_GetHeadshotFromContactData(ExprContext* pContext,
																	  ...
																	  ContactCostumeStyle* pStyleOverride,
																	  ...
																	  bool bTalking)
{
	...
	pHeadshotData = gclPaperdoll_CreateHeadshotData(pContext, pWLCostume, pPlayerCostume, pCostumeEnt, NULL, pchHeadshotStyle, NULL, false, bTalking);
	pHeadshotData->pAnimList = pAnimList;
	pHeadshotData->pStyleOverride = pStyleOverride;
	return pHeadshotData;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("PaperdollGetHeadshotFromContactDialogIndexWithAnimList");
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromContactDialogIndexWithAnimList(ExprContext* pContext, S32 iIndex, bool bAnimList)
{
	Entity* pEntity = entActivePlayerPtr();
	ContactDialog* pContactDialog = SAFE_MEMBER3(pEntity, pPlayer, pInteractInfo, pContactDialog);
	ContactDialogCostume* pCostume = NULL;
	if (pContactDialog)
	{
		if (iIndex == 0)
			pCostume = &pContactDialog->headshot;
		else if (iIndex >= 1)
			pCostume = eaGet(&pContactDialog->eaExtraHeadshots, iIndex - 1);
	}
	if (pCostume)
	{
		return gclPaperdoll_GetHeadshotFromContactData(pContext,
													   pCostume->pHeadshotOverride,
													   GET_REF(pCostume->hHeadshotOverride),
													   pCostume->iHeadshotOverridePetID,
													   entFromEntityRefAnyPartition(pCostume->headshotEnt),
													   pCostume->pchHeadshotStyleDef,
													   pCostume->pStyleOverride,
													   bAnimList ? GET_REF(pContactDialog->hAnimListToPlayForActiveEntity) : NULL,
													   sndIsContactDialogSourceAudible()); // I believe the index will always be 0 here & that multiple actors are not allowed
	}
	return NULL;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("PaperdollGetHeadshotFromContactDialogIndex");
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromContactDialogIndex(ExprContext* pContext, S32 iIndex)
{
	return gclExprPaperdollGetHeadshotFromContactDialogIndexWithAnimList(pContext, iIndex, iIndex == 0);
}
...
{
	return gclExprPaperdollGetHeadshotFromContactDialogIndexWithAnimList(pContext, 0, true);
}
...
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromGuildUniform(ExprContext* pContext, S32 iUniform)
{
	...
	if (pGuild)
	{
		...
		{
			return gclPaperdoll_CreateHeadshotData(pContext, NULL, pCostume->pCostume, NULL, NULL, NULL, NULL, false, false);
		}
	}
	return NULL;
}
...
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetHeadshotFromSpecies(ExprContext* pContext, const char* pchSpecies)
{
	...
	{
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, GET_REF(pSpecies->eaPresets[0]->hCostume), NULL, NULL, NULL, NULL, false, false);
	}
	return NULL;
}
...
{
	return gclPaperdoll_CreateHeadshotData(pContext, NULL, gclPaperdollLoadCostume(pchFilename), NULL, NULL, NULL, NULL, false, false);
}
...
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetEmblemFromGuild(ExprContext* pContext, SA_PARAM_OP_VALID Entity *pEnt, const char *pchSkeleton, const char *pchEmblemBone)
{
	...
	if (pGuild)
	{
		...
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, gclCreateGuildEmblemCostume(iGuildID, pchSkeleton, pchEmblemBone, &EmblemData, false, false), NULL, NULL, NULL, NULL, false, false);
	}
	return NULL;
}
...
SA_RET_OP_VALID PaperdollHeadshotData* gclExprPaperdollGetEmblemFromGuildWithoutBackground(ExprContext* pContext, SA_PARAM_OP_VALID Entity *pEnt, const char *pchSkeleton, const char *pchEmblemBone)
{
	...
	if (pGuild)
	{
		...
		EmblemData.fEmblem2ScaleY = pGuild->fEmblem2ScaleY;
		EmblemData.pcEmblem3 = pGuild->pcEmblem3;
		return gclPaperdoll_CreateHeadshotData(pContext, NULL, gclCreateGuildEmblemCostume(iGuildID, pchSkeleton, pchEmblemBone, &EmblemData, true, false), NULL, NULL, NULL, NULL, false, false);
	}
	return NULL;
}
...
SA_RET_OP_VALID PaperdollHeadshotData* gclPaperdollTailorCostume(ExprContext* pContext, bool bHover)
{
	...
	if (pTailorCostume)
	{
		...
		pEnt = CostumeCreator_GetEditEntity();
		pPlayerEnt = CostumeCreator_GetEditPlayerEntity();
		pExtract = entity_GetCachedGameAccountDataExtract(pPlayerEnt);
		costumeTailor_MakeCostumeValid(CONTAINER_NOCONST(PlayerCostume, pCache->pResult), GET_REF(g_CostumeEditState.hSpecies), g_CostumeEditState.eaUnlockedCostumes, g_CostumeEditState.pSlotType, true, false, false, guild_GetGuild(pPlayerEnt), false, pExtract, false, g_CostumeEditState.eaPowerFXBones);
																		...
																		U32 uFlags)
{
	...
	{
		PaperdollCostumeGenCache* pCache;
		bool bIgnoreHeight = (uFlags & COSTUME_CREATOR_APPLY_PRESET_FLAG_IGNORE_HEIGHT) != 0;
		bool bStrictMode = (uFlags & COSTUME_CREATOR_APPLY_PRESET_FLAG_STRICT_MODE) != 0;
		...
		CostumeCreator_ApplyPresetOverlay(CONTAINER_NOCONST(PlayerCostume, pCache->pResult), pPreset, bIgnoreHeight, bStrictMode);
		return CacheUse(pBase, pCache);
	}
	return pBase;
}
																	  ...
																	  const char* pcGeometryDef)
{
	...
	if (pBase && pBoneDef && pGeometryDef && (GET_REF(pGeometryDef->hBone) == pBoneDef || pcGeometryDef && stricmp(pcGeometryDef, "None") == 0))
	{
		PaperdollCostumeGenCache* pCache;
...
#include "AutoGen/gclUIGenPaperdollExpr_h_ast.c"
