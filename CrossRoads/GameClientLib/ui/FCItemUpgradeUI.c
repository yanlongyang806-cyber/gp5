...
void ItemUpgradeUI_Tick(Entity *pEnt, F32 fTick)
{
	if(pEnt && pEnt->pPlayer->ItemUpgradeInfo.fUpgradeTime)
	{
		pEnt->pPlayer->ItemUpgradeInfo.fUpgradeTime = CLAMP(pEnt->pPlayer->ItemUpgradeInfo.fUpgradeTime - fTick,0,pEnt->pPlayer->ItemUpgradeInfo.fFullUpgradeTime);
	}
}

AUTO_COMMAND ACMD_CLIENTCMD ACMD_ACCESSLEVEL(0);
void ItemUpgradeUI_SetUpgradeTime(F32 fUpgradeTime)
{
	Entity *pEnt = entActivePlayerPtr();
	if(SAFE_MEMBER(pEnt,pPlayer))
	{
		pEnt->pPlayer->ItemUpgradeInfo.fUpgradeTime = fUpgradeTime;
	}
}
...
void ItemUpgradeUI_Update(SA_PARAM_OP_VALID Entity *pEnt)
{
	...
	if (pUpgradeState)
	{
		...
		REF_HANDLE_SET_FROM_STRING(g_hItemDict,itemUpgrade_GetUpgradeString((NOCONST(Entity)*)pEnt,pBaseDef, &pUpgradeState->iWillConsume, pModifier),pUpgradeState->hResultItemRef);
		pResultDef = GET_REF(pUpgradeState->hResultItemRef);
