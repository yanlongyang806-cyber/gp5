...
// Not really an inventory thing
AUTO_STRUCT;
typedef struct UIItemRewardGrantEntry
{
	InvRewardGrantData GrantData; // has to be caps to avoid making our engine angry
	Item * pItem;
} UIItemRewardGrantEntry;

AUTO_STRUCT;
typedef struct UIItemRewardGrantData
{
	EARRAY_OF(UIItemRewardGrantEntry) eaRewards;
} UIItemRewardGrantData;
...
extern S32 gclInventoryItemCount(SA_PARAM_OP_VALID Entity *pEnt, const char *pchItemDef);
...
// Gets a list of slots that contain items that are comparable to the item you pass in
...
// This function doesn't really have anything to do with inventory, and so could be moved somewhere more appropriate
...
void gclInvFillUIItemRewardGrantData(Entity* pEnt, InvRewardRequest* pTransferData, UIItemRewardGrantData* pUIData, bool bPlaceNumericsAfterItems);
