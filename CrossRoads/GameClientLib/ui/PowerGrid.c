						...
						PTNode *pNode, PTNodeDef *pNodeDef)
{
	...
	if (pNodeDef)
	{
		...
		if(!bSetLevel && pGroupDef && pGroupDef->pRequires)
		{
													...
													const char *pchNodeAttribMessageKey)
{
	...
	if (pDescEnt && pNode)
	{
		...
		else if (pDescEnt->pChar)
		{
			...
			if (!pPower || (pPowerDef && pPowerDef->powerProp.bPropPower && pEntity))
			{
				// For STO:
				...
				else if (pPowerDef && pPowerDef->powerProp.bPropPower && pEntity)
				{
					...
					for (iType = eaiSize(&pPowerDef->powerProp.eCharacterTypes) - 1; iType >= 0; iType--)
					{
						...
						if (pSubEnt)
						{
							...
							// If we have a power, try to attach enhancements to it so that it displays
							// accurate values in the autodescription
							if (pPower && pDescEnt->pChar)
							{
								power_AttachEnhancements(entGetPartitionIdx(pEntity),pDescEnt->pChar,pPower);
							}
						}
					}
				}
			...
			if (pPower)
			{
				...
				power_AutoDesc(entGetPartitionIdx(pEntity), langGetCurrent(), pPower, pDescEnt->pChar, &s_pchAutoDesc, NULL, pchLine, pchIndent, pchModPrefix, false, 0, entGetPowerAutoDescDetail(pDescEnt,false), pExtract,NULL);
			}
			else if (pPowerDef)
			{
				GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEntity);
				...
				powerdef_AutoDesc(entGetPartitionIdx(pEntity), langGetCurrent(), pPowerDef, &s_pchAutoDesc, NULL, pchLine, pchIndent, pchModPrefix,
...
const char* gclAutoDescPower(Entity *pEnt, SA_PARAM_OP_VALID Power *pPower, PowerDef *pPowerDef, const char* pchPowerMessageKey, const char* pchAttribModsMessageKey, bool bTooltip)
{
	...
	if (pEnt && pEnt->pChar && pchPowerMessageKey && pchAttribModsMessageKey)
	{
		...
		if (pPower)
		{
			...
			power_AutoDesc(entGetPartitionIdx(pEnt), langGetCurrent(), pPower, pEnt->pChar, NULL, pAutoDescPower, NULL, NULL, NULL, false, 0, entGetPowerAutoDescDetail(pEnt, bTooltip), pExtract,NULL);
		}
		else if (pPowerDef)
		{
			GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
			powerdef_AutoDesc(entGetPartitionIdx(pEnt), langGetCurrent(), pPowerDef, NULL, pAutoDescPower, NULL, NULL, NULL,
		...
		if (pPower || pPowerDef)
		{
			...
			powerdef_AutoDescCustom(pEnt, langGetCurrent(), &s_pchDescription, pPowerDef, pAutoDescPower, pchPowerMessageKey, pchAttribModsMessageKey);
		}
...
void gclGenExprBuyPowerCartItems(SA_PARAM_OP_VALID Entity *pEnt)
{
	...
	if ( pEnt == pPlayer )
	{
		if(g_bCartRespecRequest)
			ServerCmd_gslBuyPowerCartItemsWithRespec(kPTRespecGroup_ALL);
		else
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenBuyPowerCartItemsType");
void gclGenExprBuyPowerCartItemsType(SA_PARAM_OP_VALID Entity *pEnt, PTRespecGroupType eRespecType)
{
	Entity* pPlayer = entActivePlayerPtr();
	...
	{
		if(g_bCartRespecRequest)
			ServerCmd_gslBuyPowerCartItemsWithRespec(eRespecType);
		else
			ServerCmd_gslBuyPowerCartItems();
	}
	else
...
typedef enum GetPowerListNodeFlags {
	...
	GPLN_CAN_IGNORE_GROUP_REQUIREMENTS = 8192,
	GPLN_VALIDATE_GROUP_EXPRESSION_REQUIREMENTS = 16384,
						 ...
						 S32 iCategory, GetPowerListNodeFlags eFlags)
{
		...
		)
	{
		...
		do
		{
			...
			for (i = 0; i < eaSize(&pTreeDef->ppGroups); i++)
			{
				...
				if (pGroupDef->bUIIgnoreGroupRequirements && (eFlags & GPLN_CAN_IGNORE_GROUP_REQUIREMENTS))
				{
					if ((eFlags & GPLN_VALIDATE_GROUP_EXPRESSION_REQUIREMENTS) && pGroupDef->pRequires && pGroupDef->pRequires->pExprPurchase)
					{
						ExprContext *pPurchaseContext = ptpurchaserequirements_GetContext();
						MultiVal mVal = {0};

						// If ignoring the group requirements, but explicitly validating expression requirements
						// manually perform the expression requirements here.
						exprContextSetPointerVar(pPurchaseContext, "Source", pCheckEnt, parse_Entity, true, true);
						exprContextSetPointerVar(pPurchaseContext, "GameAccount", entity_GetGameAccount(pCheckEnt), parse_GameAccountData, false, true);
						exprContextSetSelfPtr(pPurchaseContext, pEnt);
						exprContextSetPartition(pPurchaseContext, PARTITION_IN_TRANSACTION);
						exprEvaluateTolerateInvalidUsage(pGroupDef->pRequires->pExprPurchase, pPurchaseContext, &mVal);

						if (MultiValGetInt(&mVal, NULL) <= 0)
							continue;
					}
				}
				else if (bCanBuyGroup ? !(eFlags & GPLN_BUYABLE_GROUPS) : !(eFlags & GPLN_UNBUYABLE_GROUPS))
				{
					continue;
				}
				...
				for (j = 0; j < eaSize(&pGroupDef->ppNodes); j++)
				{
					...
					if (pFakeEnt)
					{
						...
						pEntNode = character_FindPowerTreeNodeHelper(CONTAINER_NOCONST(Entity, pEnt)->pChar, NULL, pNodeDef->pchNameFull);
						if (pEntNode)
							iEntRank = pEntNode->iRank + 1;
					}
					else
					{
						...
						pEntNode = pNode;
						if (pEntNode)
							iEntRank = pNode->iRank + 1;
					}

					if (pEnt && iEntRank < pListNode->iMaxRank)
					{
					...
					pListNode->bIsFakeEntDifferent = (!pNode) != (!pEntNode) || pNode && pEntNode && StructCompare(parse_PTNode, pNode, pEntNode, 0, 0, 0) != 0;
	...
	if (eFlags & GPLN_COSTTABLE_NULL_NODES)
	{
		...
		{
			S32 iSpentPoints = entity_PointsSpent(CONTAINER_NOCONST(Entity, pCheckEnt), s_eaCostTables[i]);
			S32 iMaxPoints;
			if (powertable_Find(s_eaCostTables[i]))
				...
				iMaxPoints = entity_PowerTableLookupAtHelper(CONTAINER_NOCONST(Entity, pCheckEnt), s_eaCostTables[i], 100000);
			else
				iMaxPoints = entity_PointsEarned(CONTAINER_NOCONST(Entity, pCheckEnt), s_eaCostTables[i]);
...
void gclGenExprGetPowerNodeListWithFlagsEx(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Entity *pFakeEnt, const char *pchTrees, const char *pchCategory, const char *pchFlags)
{
	...
	if ((pcToken = strtok_r(pcBuffer, " \r\n\t,%", &pcContext)) != NULL)
	{
		do
		{
			...
			if (stricmp(pcToken, "-GroupRequirements") == 0)
				uAddFlag = GPLN_CAN_IGNORE_GROUP_REQUIREMENTS;
			if (stricmp(pcToken, "+GroupRequirements") == 0 || stricmp(pcToken, "GroupRequirements") == 0)
				uRemoveFlag = GPLN_CAN_IGNORE_GROUP_REQUIREMENTS;

			if (stricmp(pcToken, "+ValidateExpressionRequirements") == 0 || stricmp(pcToken, "ValidateExpressionRequirements") == 0)
				uAddFlag = GPLN_VALIDATE_GROUP_EXPRESSION_REQUIREMENTS;
			if (stricmp(pcToken, "-ValidateExpressionRequirements") == 0)
				uRemoveFlag = GPLN_VALIDATE_GROUP_EXPRESSION_REQUIREMENTS;
...
static bool gclGenPowerTreeDecreaseNodeAndValidate( UIGen* pGen, Entity* pEnt, Entity* pFakeEnt, PowerTreeDef* pTreeDef, PTNodeDef* pNodeDef, S32 iDesiredRank, const char* pchNoCopyNumeric)
{
	...
	if (i >= 0)
	{
		...
		if (bSuccess && g_bCartRespecRequest)
		{
			...
			if (!entity_PowerTreeStepsRespecSteps(PARTITION_CLIENT,CONTAINER_RECONST(Entity,pValidateEnt),NULL,pSteps,NULL,NULL,NULL,kPTRespecGroup_ALL,true,false))
			{
				bSuccess = false;
			}
		}
...
static S32 gclPowerCartSetRespec(SA_PARAM_NN_VALID UIGen *pGen,
								 SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Entity* pFakeEnt, S32 eRespecType, bool bFillPowerCart)
{
	Entity* pPlayer = entActivePlayerPtr();
	...
	if ( pPlayer && pEnt && pFakeEnt )
	{
		...
		character_GetPowerTreeSteps(CONTAINER_NOCONST(Character, pFakeEnt->pChar),pSteps, false, eRespecType);
		character_PowerTreeStepsCostRespec(PARTITION_CLIENT,pEnt->pChar,pSteps,0);

		if(entity_PowerTreeStepsRespecSteps(PARTITION_CLIENT,pFakeEnt,NULL,pSteps,NULL,NULL,NULL,eRespecType,true,false))
		{
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenPowerCartRespecEx");
S32 gclGenExprPowerCartRespecEx(SA_PARAM_NN_VALID UIGen *pGen,
							  SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Entity * pFakeEnt, S32 eRespecType, U32 bFillPowerCart)
{
	return gclPowerCartSetRespec(pGen,pEnt,pFakeEnt,eRespecType,bFillPowerCart);
}
...
{
	return gclGenExprPowerCartRespecEx(pGen,pEnt,pFakeEnt,kPTRespecGroup_ALL,bFillPowerCart);
}
...
//Returns the number of account wide respecs the character has available.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GetGameAccountRespecsOfType");
S32 gclGenExprGetGameAccountRespecsOfType(SA_PARAM_OP_VALID Entity *pEnt, PTRespecGroupType eRespecType)
{
	if(pEnt && pEnt->pPlayer)
	{
		GameAccountData *pData = GET_REF(pEnt->pPlayer->pPlayerAccountData->hData);
		const char *pchRespecTokenGAD = PowerTree_GetRespecTokensPurchaseGAD(eRespecType);
		if(pData)
		{
			if (gConf.bDontAllowGADModification)
				return gad_GetAccountValueInt(pData, pchRespecTokenGAD);
			else
				return gad_GetAttribInt(pData, pchRespecTokenGAD);
		}
	}
