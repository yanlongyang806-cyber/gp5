...
static S32 drawEntryLayout(UIGenChatLog *pChatLog, ChatConfig *pConfig, ChatLogEntry *pEntry, UIGenChatLogEntryLayout *pLayout, F32 fX, F32 fY, F32 fZ, U8 chAlpha, bool bHideHandle, S32 iSelectionStart, S32 iSelectionEnd, ChatLink *pActiveLink)
{
	...
	for (i = 0; i < n; ++i)
	{
		...
		do
		{
			...
			gfxfont_PrintEx(g_font_Active, x, y + v2Size[1], fZ, fScale, fScale, 0, pchText + ch, iCodepoints, NULL, NULL);
		...
		if( *pFullText == '/' )
		{
			...
			if( *channelNameEnd == ' ' && channelNameEnd > channelNameBegin )
			{
				...
				else if( !stricmp(channelName, "replylast") )
				{
					...
					strStartsWith(channelName, CHAT_ZONE_SHORTCUT) || 
					strStartsWith(channelName, CHAT_TRADE_SHORTCUT) ||
					strStartsWith(channelName, CHAT_LFG_SHORTCUT) ||
				...
				{
					if( strStartsWith(channelName, CHAT_TRADE_SHORTCUT) && pchCommandToExecute )
					{
						// The /Trade aliases are special, in that we want to use the command name as the initial text.
						TextBuffer_SetText(pTextEntryState->pBuffer, channelNameBegin);
						TextBuffer_InsertText(pTextEntryState->pBuffer, channelNameEnd-channelNameBegin, " ");
						TextBuffer_SetCursor(pTextEntryState->pBuffer, channelNameEnd-channelNameBegin + 1); // +1 for the space
					}
					else
					{
						S32 iCursorOld = TextBuffer_GetCursor(pTextEntryState->pBuffer);
