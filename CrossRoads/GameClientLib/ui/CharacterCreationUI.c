...
static KeyBindProfile s_Profile = {"Character Creation Commands", __FILE__, KeyboardLocale_EnglishUS, NULL, true, true, NULL, &g_CharacterCreationCmds};

NOCONST(Entity) *g_pFakePlayer = NULL;
NOCONST(Entity) *g_eaFakeOwnedEnts = NULL;

AUTO_COMMAND ACMD_NAME("CharacterCreation_FakeAccessLevel") ACMD_ACCESSLEVEL(9);
void CharacterCreation_FakeAccessLevel(S32 iAccessLevel)
{
	if (g_pFakePlayer && g_pFakePlayer->pPlayer)
	{
		g_pFakePlayer->pPlayer->accessLevel = iAccessLevel;
		g_pFakePlayer->pPlayer->accountAccessLevel = iAccessLevel;
	}
}
...
void CharacterCreation_SetAllegiance(const char *faction)
{
	...
	{
		if (g_pFakePlayer)
			REMOVE_HANDLE(g_pFakePlayer->hAllegiance);
	...
	if (g_pFakePlayer)
		SET_HANDLE_FROM_STRING("Allegiance",g_pcAllegianceName,g_pFakePlayer->hAllegiance);
}
...
static void CharacterCreation_Enter(void)
{
	...
	if (!g_pFakePlayer)
	{
		...
		g_pFakePlayer->bFakeEntity = true;
		...
		g_pFakePlayer->pPlayer->accountID = SAFE_MEMBER(g_characterSelectionData, accountID);
		g_pFakePlayer->pPlayer->pPlayerAccountData = StructCreateNoConst(parse_PlayerAccountData);
		g_pFakePlayer->pPlayer->pPlayerAccountData->iAccountID = g_pFakePlayer->pPlayer->accountID;
...
static bool CharacterCreation_CreateCharacter(bool bFinish, const char *pchName, const char *pchSubName, const char *pchDescription, bool bGoToLobby)
{
    ...
    else if ( g_pFakePlayer && g_pFakePlayer->pSaved )
    {
...
static bool CanPlayerUseSpecies(SpeciesDef *pSpecies, bool bUGC, bool bUnlockAll)
{
	...
	// Too soon
	if( (pSpecies->uUnlockTimestamp != 0) && (pSpecies->uUnlockTimestamp > timeServerSecondsSince2000()) && !gad_GetAccountValueInt(GET_REF(g_characterSelectionData->hGameAccountData), "Test.IgnoreSpeciesUnlockTimestamps") )
		return false;
										 ...
										 U32 uFlags)
{
	...
	for (i = 0; i < eaSize(&pArray->ppReferents); i++) {
		...
		if (!CanPlayerUseSpecies(pSpecies, !!(uFlags & kFillSpeciesListFlag_UGC), false)) {
			...
			// If we are not in UGC, check to see if we want to allow unowned microtransacted species
			// While in UGC, we do not care about microtransacted species though
			if (!(uFlags & kFillSpeciesListFlag_UGC))
			{
...
PlayerCostume* CharacterCreation_GetDefaultCostumeForSpecies(SpeciesDef* pSpecies, bool bIsPet)
{
	...
	if (!g_pFakePlayer) {
		iPreset = 0;
	} else if (bIsPet && g_CharacterCreationData && eaSize(&g_CharacterCreationData->petInfo) > 0) {
...
const char *CharacterCreation_GetPrimaryPowerTree(SA_PARAM_OP_VALID Entity *pEntity)
{
	return g_CharacterCreationData && g_CharacterCreationData->powerTreeName ? g_CharacterCreationData->powerTreeName : "";
}
...
{
	if ( pEntity->pChar && g_pFakePlayer && g_pFakePlayer->pChar && g_CharacterCreationData )
	{
...
void CharacterCreation_GenEntSetName(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity, const char *pchName)
{
...
void CharacterCreation_GenEntSetDescription(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity, const char *pchDescription)
{
...
void CharacterCreation_BuyPowerTreeNode(const char *pchTree, const char *pchNodeFull)
{
	...
	if (pchNodeName && g_CharacterCreationData)
	{
		PTNodeDef *pNode;
		PTGroupDef *pGroup;
...
void CharacterCreation_AutoBuyRecommendedPowersByCostTable(ExprContext *pContext, SA_PARAM_NN_STR const char *pchCostTable)
{
	...
	if (pchCostTable == NULL || pchCostTable[0] == '\0' || !g_CharacterCreationData)
		return;
...
{
	if (g_CharacterCreationData && eaSize(&g_CharacterCreationData->powerNodes))
	{
...
{
	while (g_CharacterCreationData && eaSize(&g_CharacterCreationData->powerNodes))
	{
		char *pchNode = eaPop(&g_CharacterCreationData->powerNodes);
		S32 i, j;
...
void CharacterCreation_InitPowers(const char *pchTreeName)
{
	...
	if(g_CharacterCreationData && eaSize(&ppNodes) == 1)
	{
...
void CharacterCreation_RemovePowers(const char *pchTreeName)
{
	...
	if (g_CharacterCreationData)
	{
		...
		{
			PTNodeDef *pNode = powertreenodedef_Find(g_CharacterCreationData->powerNodes[i]);

			estrClear(&estrTree);
			if (!powertree_TreeNameFromNodeDef(pNode, &estrTree))
				continue;

			if (powertreedef_Find(estrTree) == pTreeDef)
				StructFreeString(eaRemove(&g_CharacterCreationData->powerNodes, i));
		}
		estrDestroy(&estrTree);
	}
}
...
void CharacterCreation_ResetStatPoints()
{
	...
	CharacterPath *pCharacterPath = SAFE_GET_REF2(g_pFakePlayer, pChar, hPath);

	// Remove all assigned stat points
	if (g_CharacterCreationData && g_CharacterCreationData->assignedStats)
	{
	...
	else // Use the defaults
	{
		...
		{
			if (g_CharacterCreationData && pCharacterPath->ppAssignedStats[i]->iPoints != 0)
			{
...
F32 CharacterCreation_GetBaseAttribValue(const char *pszAttribName)
{
	...
	{
		CharacterClass* pClass = SAFE_GET_REF2(g_pFakePlayer, pChar, hClass);
		if (pClass)
		{
...
{
	if ( !g_CharacterCreationData || iPetIndex >= eaUSize( &g_CharacterCreationData->petInfo ) )
		return false;
...
bool CharacterCreation_SaveCostumePresetForPet( U32 iPetIndex, SA_PARAM_NN_VALID UIGen* pParentGen, SA_PARAM_OP_VALID CostumePreset* pPreset )
{
	...
	if ( !g_CharacterCreationData || pPreset==NULL || iPetIndex >= eaUSize( &g_CharacterCreationData->petInfo ) )
		return false;
...
{
	if (g_CharacterCreationData && eaGet(&g_CharacterCreationData->petInfo, iPetIndex))
...
{
	if ( !g_CharacterCreationData || iPupIndex >= eaUSize( &g_CharacterCreationData->puppetInfo ) )
		return false;
...
{
	if (g_CharacterCreationData && iPupIndex < eaUSize(&g_CharacterCreationData->puppetInfo) && pchName)
	{
...
bool CharacterCreation_SavePlayerCostumePreset(SA_PARAM_NN_VALID UIGen* pParentGen, CostumePreset* pPreset)
{
	...
	if ( g_CharacterCreationData && pSpecies )
	{
		S32 i;
		for ( i = 0; i < eaSize( &pSpecies->eaPresets ); i++ )
		{
...
void CharacterCreation_RandomizeUnusedPetCostumes()
{
	...
	U32 i, c;
	U32 iPetArraySize = g_CharacterCreationData ? eaSize(&g_CharacterCreationData->petInfo) : 0;
	U32 iSpeciesArraySize = eaSize( &pArray->ppReferents );

	U32* eaUsed = NULL;
    SpeciesDef *speciesDef = SAFE_GET_REF2(g_pFakePlayer, pChar, hSpecies);
...
void CharacterCreation_GenGetSpeciesListGender( SA_PARAM_NN_VALID UIGen *pGen, int iGender )
{
	...
	{
		ui_GenSetListSafe(pGen, NULL, SpeciesDef);
		return;
	}
...
{
	if (g_CharacterCreationData && g_CharacterCreationData->characterPathName)
	{
...
void CharacterCreation_FillSpeciesGroupListEx(SpeciesGroupUI ***peaSpeciesGroups, SpeciesDef **eaSpecies)
{
	int iGroup, iSpecies;
	SpeciesGroupUI *pSpeciesGroup;

	eaClear(peaSpeciesGroups);
	// For each Species
	for( iSpecies = 0; iSpecies < eaSize(&eaSpecies); ++iSpecies )
	{
		...
		for( iGroup = 0; iGroup < eaSize(peaSpeciesGroups); ++iGroup )
		{
			if( (*peaSpeciesGroups)[iGroup]->pcSpeciesGroup == pSpeciesDef->pcSpeciesGroup )
			{
				pSpeciesGroup = (*peaSpeciesGroups)[iGroup];
				break;
			}
		}
		...
		if( pSpeciesGroup == NULL )
		{
			...
			iGroup = eaPush(peaSpeciesGroups, pSpeciesGroup);
			pSpeciesGroup = (*peaSpeciesGroups)[iGroup];
		}
...
void CharacterCreation_GenSpeciesGroupGetListWithDummy(SA_PARAM_NN_VALID UIGen *pGen, U32 uFlags)
{
	...
	CharacterCreation_FillSpeciesGroupListEx(&eaSpeciesGroups, eaSpecies);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenSpeciesGroupGetList");
void CharacterCreation_GenSpeciesGroupGetList(SA_PARAM_NN_VALID UIGen *pGen, U32 uFlags)
{
	SpeciesDef **eaSpecies = NULL;
	SpeciesGroupUI ** eaSpeciesGroups = NULL;
	// Get the list of species
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(SpeciesGroupUI_GetSpeciesList);
void SpeciesGroupUI_GetSpeciesList(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_NN_VALID SpeciesGroupUI *pSpeciesGroupUI)
{
	SpeciesDef ** eaSubSpeciesDefs = NULL;
	int iSpeciesIndex = 0;
	eaClear(&eaSubSpeciesDefs);
	for(iSpeciesIndex = 0; iSpeciesIndex < eaSize(&pSpeciesGroupUI->eaSpeciesList); ++iSpeciesIndex)
	{
		SpeciesDef *pSpeciesDef = pSpeciesGroupUI->eaSpeciesList[iSpeciesIndex];
		eaPush(&eaSubSpeciesDefs, pSpeciesDef);
	}
	ui_GenSetListSafe(pGen, &eaSubSpeciesDefs, SpeciesDef);
	eaClearFast(&eaSubSpeciesDefs);
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenUpdateFakePlayerLiteBag");
void gclGenExprUpdateFakePlayerLiteBag(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Entity *pFakeEnt, S32 iBagID)
{
	GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
 	InventoryBagLite *pInvBag = (InventoryBagLite *)inv_GetLiteBag(pEnt, iBagID, pExtract);
 	NOCONST(Entity) *pFakePlayer = CONTAINER_NOCONST(Entity, pFakeEnt);

 	if (pFakePlayer && pInvBag)
 	{
 		int iIndex = eaIndexedFindUsingInt(&pFakePlayer->pInventoryV2->ppLiteBags, iBagID);

		if (iIndex != -1)
		{
			InventoryBagLite *pFakeInvBag = (InventoryBagLite *)eaRemove(&pFakePlayer->pInventoryV2->ppLiteBags, iIndex);
			InventoryBagLite *pCopyBag = StructCreate(parse_InventoryBagLite);

		 	StructDestroySafe(parse_InventoryBagLite, &pFakeInvBag);
		 	StructCopyAll(parse_InventoryBagLite, pInvBag, pCopyBag);
		 	eaIndexedAdd(&pFakePlayer->pInventoryV2->ppLiteBags, pCopyBag);
		}
	}
}
...
void CharacterCreation_gclGenExprAddPowerTreesByType(SA_PARAM_OP_VALID Entity* pEnt, ACMD_EXPR_DICT(PowerTreeTypeDef) const char *pchPowerTreeType)
{
	...
	if (g_CharacterCreationData) {
		...
		for (i = eaSize(&g_CharacterCreationData->powerNodes) - 1; i >= 0; i--) {
			PTNodeDef *pNode = powertreenodedef_Find(g_CharacterCreationData->powerNodes[i]);
			if (powertree_TreeNameFromNodeDef(pNode, &estrTree)) {
				PowerTreeDef *pTreeDef = powertreedef_Find(estrTree);
				if (pTreeDef && GET_REF(pTreeDef->hTreeType) == pType) {
					CharacterCreation_RemovePowers(pTreeDef->pchName);
					MIN1(i, eaSize(&g_CharacterCreationData->powerNodes));
				}
			}
		}
		estrDestroy(&estrTree);
	...
	while (pEnt && (pTree = RefSystem_GetNextReferentFromIterator(&iter)))
	{
...
//Adds the power trees of the given type to the player, and based on the character's current species & class will attempt to buy the suggested nodes.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenAddSuggestedPowerTreesByType");
void CharacterCreation_gclGenExprAddSuggestedPowerTreesByType(SA_PARAM_OP_VALID Entity* pEnt, ACMD_EXPR_DICT(PowerTreeTypeDef) const char *pchPowerTreeType)
{
	PTTypeDef *pType = RefSystem_ReferentFromString("PowerTreeTypeDef", pchPowerTreeType);
	SpeciesDef *pSpecies = SAFE_GET_REF2(pEnt, pChar, hSpecies);
	CharacterClass *pClass = SAFE_GET_REF2(pEnt, pChar, hClass);
	PowerTree *pPrimaryTree = pEnt && pEnt->pChar ? eaGet(&pEnt->pChar->ppPowerTrees, 0) : NULL;
	S32 i;

	CharacterCreation_gclGenExprAddPowerTreesByType(pEnt, pchPowerTreeType);

	if (pSpecies && (pClass || pPrimaryTree))
	{
		SpeciesPowerSuggestion *pSuggestion = NULL;
		if (!pSuggestion && pClass)
			pSuggestion = eaIndexedGetUsingString(&pSpecies->eaPowerSuggestions, pClass->pchName);
		if (!pSuggestion && pPrimaryTree && IS_HANDLE_ACTIVE(pPrimaryTree->hDef))
			pSuggestion = eaIndexedGetUsingString(&pSpecies->eaPowerSuggestions, REF_STRING_FROM_HANDLE(pPrimaryTree->hDef));
		if (pSuggestion)
		{
			char *pchTree = NULL;
			estrStackCreate(&pchTree);

			for (i = eaSize(&pSuggestion->eaNodes) - 1; i >= 0; i--)
			{
				PTNodeDef *pNode = GET_REF(pSuggestion->eaNodes[i]->hNodeDef);
				PowerTreeDef *pTree;

				estrClear(&pchTree);
				if (!pNode || !powertree_TreeNameFromNodeDef(pNode, &pchTree))
					continue;

				pTree = powertreedef_Find(pchTree);
				if (!pTree)
					continue;
				...
				if (GET_REF(pTree->hTreeType) == pType)
				{
					CharacterCreation_BuyPowerTreeNode(pTree->pchName, pNode->pchNameFull);
				}
			}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterCreation_IsAllegianceUnlocked");
bool CharacterCreation_IsAllegianceUnlocked(SA_PARAM_NN_STR const char *pchAllegianceDef)
{
	if (pchAllegianceDef)
	{
		AllegianceDef *pAllegianceDef = allegiance_FindByName(pchAllegianceDef);
		...
		{
			GameAccountData *pData = entity_GetGameAccount(NULL);
			return GamePermission_GADHasToken(pData, pAllegianceDef->pcRequiredPermission);
		}
	}
	return false;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterCreation_IsAllegianceUnlockedForVirtualShard");
bool CharacterCreation_IsAllegianceUnlockedForVirtualShard(SA_PARAM_NN_STR const char *pchAllegianceDef, ContainerID iVirtualShard)
{
	if (pchAllegianceDef)
	{
		AllegianceDef *pAllegianceDef = allegiance_FindByName(pchAllegianceDef);
		if (pAllegianceDef)
		{
			...
			GameAccountData *pData = entity_GetGameAccount(NULL);
			if(bUGC && pAllegianceDef->bCannotPlayUGC)
			{
				return false;
			}
...
bool CharacterCreation_IsCharCreateTypeUnlocked(const char *unlockable)
{
	if (unlockable)
	{
		if (g_characterSelectionData && (StaticDefineIntGetInt(UnlockedCreateFlagsEnum, unlockable) & g_characterSelectionData->unlockCreateFlags))
		{
			return true;
		}
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(CharacterCreation_CountBoughtPowerNodes);
S32 CharacterCreation_CountBoughtPowerNodes(void)
{
	return g_CharacterCreationData ? eaSize(&g_CharacterCreationData->powerNodes) : 0;
}
...
static const char *CharacterCreation_CheckNameError(const char *pcName, int iMinLength, int iMaxLength, bool bCharacterName)
{
	...
	if ((err = StringIsInvalidNameGeneric(pcName, "-._ '", iMinLength, iMaxLength, false, true, true, true, true, true, LoginGetAccessLevel())) != STRINGERR_NONE)
...
void CharacterCreation_UIAllegiance_FillSpeciesDef(CharacterCreationAllegianceUI *pAllegianceUI, CharacterCreationSpeciesUI *pSpeciesUI, SpeciesDef *pSpeciesDef)
{
	if (pSpeciesUI)
	{
		...
		pSpeciesUI->bCreatable = CanPlayerUseSpecies(pSpeciesDef, false, false) && SAFE_MEMBER(pAllegianceUI, bCreatable);
	}
}
...
void CharacterCreation_FillUIAllegiances(U32 iVirtualShardID)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (SAFE_MEMBER2(g_pFakePlayer, pPlayer, accessLevel) < ACCESS_DEBUG &&
			SAFE_MEMBER2(g_pFakePlayer, pPlayer, accountAccessLevel) < ACCESS_DEBUG &&
			pAllegianceDef->pcRequiredPermission && !CharacterSelection_IsVirtualShardUGCShard(iVirtualShardID))
		{
			GameAccountData *pData = entity_GetGameAccount(NULL);
			if (!GamePermission_GADHasToken(pData, pAllegianceDef->pcRequiredPermission))
		...
		pAllegianceUI->bSelected = pAllegianceUI->bCreatable && g_pFakePlayer && GET_REF(g_pFakePlayer->hAllegiance) == pAllegianceDef;
...
void CharacterCreation_UIAllegiances_SetSpeciesDef(const char *pchAllegiance, const char *pchSpecies, const char *pchSpeciesDef)
{
	...
	if (pAllegianceUI->bSelected && pSpeciesUI->bSelected)
		CharacterCreation_SavePlayerSpeciesByName(pSpeciesUI->pchCurrentSpecies);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("CharacterCreation_UIAllegiances_SetAllSkeleton");
void CharacterCreation_UIAllegiances_SetAllSkeleton(const char *pchSkeleton)
{
	PCSkeletonDef *pSkeleton = RefSystem_ReferentFromString(g_hCostumeSkeletonDict, pchSkeleton);
	S32 i, j, k;
	...
	for (i = 0; i < eaSize(&s_eaUIAllegiances); i++)
	{
		CharacterCreationAllegianceUI *pAllegianceUI = s_eaUIAllegiances[i];

		for (j = 0; j < eaSize(&pAllegianceUI->eaSpecies); j++)
		{
			CharacterCreationSpeciesUI *pSpeciesUI = pAllegianceUI->eaSpecies[j];

			for (k = 0; k < eaSize(&pSpeciesUI->eapchRelatedSpecies); k++)
			{
				SpeciesDef *pSpeciesDef = RefSystem_ReferentFromString(g_hSpeciesDict, pSpeciesUI->eapchRelatedSpecies[k]);

				if (pSpeciesDef && GET_REF(pSpeciesDef->hSkeleton) == pSkeleton)
				{
					if (pSpeciesUI->pchCurrentSpecies != pSpeciesUI->eapchRelatedSpecies[k])
						CharacterCreation_UIAllegiances_SetSpeciesDef(pAllegianceUI->pchAllegiance, pSpeciesUI->pchSpeciesName, pSpeciesUI->eapchRelatedSpecies[k]);
					break;
				}
			}
		}
