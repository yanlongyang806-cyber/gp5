...
typedef struct DonationTaskUI
{
	...
	// The order the tasks should appear in the UI, if their category is equal. Low numbers go first.
	U32 iUISortOrder;

	// A shortcut for the UI to check the TaskAvailableExpr repeat count.
	// But we can't actually look at the values in that expr, so use this instead.
	U32 iUIMaxRepeatCount;
	...
	// The flag that indicates if this task may be canceled
	U32 bCancelable : 1;
	...
	// If the TaskAvailableExpr returned true
	U32 bAvailable : 1;
} DonationTaskUI;
