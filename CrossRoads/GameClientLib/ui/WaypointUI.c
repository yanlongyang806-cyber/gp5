							   ...
							   S32 iIconMinRange, S32 iMaxRange)
{
	...
	for (i = 0; i < eaSize(&s_eaWaypoints); i++)
	{
		...
		if ((bGetLandmarks && (pWaypoint->type == MinimapWaypointType_Landmark || pWaypoint->type == MinimapWaypointType_AreaLandmark))
			||
			(!bGetLandmarks && pWaypoint->type != MinimapWaypointType_Landmark && pWaypoint->type != MinimapWaypointType_AreaLandmark))
		{
			F32 fScaledSize;
