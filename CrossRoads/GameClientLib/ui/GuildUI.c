...
#include "AutoGen/gclUIGenPaperdollExpr_h_ast.h"
...
bool gclGuild_expr_HasPermissionToKick(S32 iRank, SA_PARAM_OP_VALID Entity* pEnt)
{
	...
	if ( pGuildMember != NULL )
	{
		...
		if (pGuild && guild_HasPermission(pGuildMember->iRank, pGuild, GuildPermission_Remove))
		{
			//we must be above the other member's rank to kick
			if ( iRank - 1 < pGuildMember->iRank )
			{
				return true;
			}
		}
...
bool gclGuild_expr_HasPermissionToDemote(S32 iRankToDemote, SA_PARAM_OP_VALID Entity* pEnt)
{
	...
	if ( pGuildMember != NULL )
	{
		...
		if ( pGuild != NULL )
		{
			...
			{
				bool bIsLeader = aslGuild_trh_IsLeader(CONTAINER_NOCONST(Guild, pGuild), pEnt->myContainerID);
				if
				( 
					(!pGuild->uDemoteLeaderID || !bIsLeader) &&
					(timeServerSecondsSince2000() >= pGuildMember->uProvisionalTimeSeconds || !bIsLeader) &&
					guild_HasPermission( pGuildMember->iRank, pGuild, GuildPermission_DemoteAtRank )
				)
				{
					return true;
				}
...
bool gclGuild_expr_HasPermissionSelf(SA_PARAM_OP_VALID Entity* pEnt, const char* pcPermission)
{
	...
	if (pGuildMember && pGuild)
	{
		...
		// leaders being demoted don't have permission to use guild commands ... no common code for this, duplicated in aslGuild_trh_HasPermission
		if(pGuild->uDemoteLeaderID == pEnt->myContainerID)
		{
			return false;
		}
...
typedef struct GuildMemberData {
	...
	U32			uProvisionalTimeSeconds;
} GuildMemberData;

// Fills eaSortedList (unsorted) with guild members.
// You can strip out most of the effort by passing bNameHandlesOnly true.
void BuildMemberList(SA_PARAM_NN_VALID GuildMemberData ***peaSortedList, SA_PARAM_NN_VALID Guild* pGuild, bool bNameHandlesOnly, bool bShowOffline, SA_PARAM_OP_VALID StashTable st_GroupProjectCounts, SA_PARAM_OP_VALID PlayerGuild *pPlayerGuildData )
{
	int i;
	int iOldSize = eaSize(peaSortedList);
	int iGuildSize = eaSize(&pGuild->eaMembers);
	int iCurSize = 0;
	char temp[1024];
	...
	for (i = 0; i < iGuildSize; i++) {
		if (bShowOffline || pGuild->eaMembers[i]->bOnline) {
			...
			if (iCurSize >= iOldSize) {
				eaPush(peaSortedList, StructCreate(parse_GuildMemberData));
			} else {
				StructReset(parse_GuildMemberData, eaGet(peaSortedList, iCurSize));
			}
			pData = eaGet(peaSortedList, iCurSize);
			iCurSize++;
			...
			pData->uProvisionalTimeSeconds = pGuild->eaMembers[i]->uProvisionalTimeSeconds;
			if (pData->bOnline) {
	...
	eaSetSizeStruct(peaSortedList, parse_GuildMemberData, iCurSize);
}

void SortMemberList(SA_PARAM_NN_VALID GuildMemberData ***peaSortedList, SA_PARAM_NN_VALID UIGenListState *pState)
{
	...
	if (pColState) {
		...
		eaStableSortUsingColumn(peaSortedList, parse_GuildMemberData, pColState->iTPICol);
	}
}

// Get the members of this entity's guild.
// Use a Filter to do the slice instead.
// 
// If you add anything to this function, make sure it stays synchronized with gclGuild_expr_GetMemberList_GetMemberPageIndex below.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_getmemberlist);
void gclGuild_expr_GetMemberList(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt, bool bShowOffline)
{
	...
	if (pGuild && pState && pEnt) {
		...
		BuildMemberList(peaGuildMembers, pGuild, false, bShowOffline, st_GroupProjectCounts, pPlayerGuildData);
		PERFINFO_AUTO_STOP();
	}

	ui_GenSetManagedListSafe(pGen, peaGuildMembers, GuildMemberData, false);
}

// Find the membername@handle in the gen's unfiltered list. This function is designed to be called in the Model expression, after guild_getmemberlist().
// Pass true to bGetPage to get the page the member appears on, or false to get the row on that page where they'll appear.
S32 gclGuild_expr_GetMemberList_GetMemberPageIndexHelper(SA_PARAM_NN_VALID UIGen *pGen, S32 iCount, SA_PARAM_NN_STR const char* pMemberName, SA_PARAM_NN_STR const char* pMemberAtHandle, bool bGetPage)
{
	UIGenListState *pState = UI_GEN_STATE(pGen, List);
	GuildMemberData ***peaSortedList = ui_GenGetManagedListSafe(pGen, GuildMemberData);
	int i, iResult = -1;

	if (pState)
	{
		// Find the requested user in the member array
		for (i = 0; i < eaSize(peaSortedList); i++)
		{
			if( !strcmp((*peaSortedList)[i]->pcName, pMemberName) && !strcmp((*peaSortedList)[i]->pcAtName, pMemberAtHandle) )
			{
				int iIndexFromStart = (pState->eSortMode == UISortDescending) ? eaSize(peaSortedList) - 1 - i : i;
				iResult = bGetPage ? (iIndexFromStart / iCount) : (iIndexFromStart % iCount);
				break;
			}
		}
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_memberlist_GetMemberPageIndex);
S32 gclGuild_expr_GetMemberList_GetMemberPageIndex(SA_PARAM_NN_VALID UIGen *pGen, S32 iCount, SA_PARAM_NN_STR const char* pMemberName, SA_PARAM_NN_STR const char* pMemberAtHandle)
{
	return gclGuild_expr_GetMemberList_GetMemberPageIndexHelper(pGen, iCount, pMemberName, pMemberAtHandle, true);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_memberlist_GetMemberRowIndex);
S32 gclGuild_expr_GetMemberList_GetMemberRowIndex(SA_PARAM_NN_VALID UIGen *pGen, S32 iCount, SA_PARAM_NN_STR const char* pMemberName, SA_PARAM_NN_STR const char* pMemberAtHandle)
{
	return gclGuild_expr_GetMemberList_GetMemberPageIndexHelper(pGen, iCount, pMemberName, pMemberAtHandle, false);
}
...
bool gclGuild_expr_MayPromote(SA_PARAM_OP_VALID Entity *pEnt, const char *pchOtherAccountName)
{
	...
	if (pEnt && pGuild) {
		...
		ResolveKnownEntityID(pEnt, pchOtherAccountName, NULL, NULL, &iEntID);
		pOtherMember = guild_FindMemberInGuildEntID(iEntID, pGuild);
		if (pOtherMember && gclGuild_expr_HasPermissionToPromote(pOtherMember->iRank+1, pEnt)) {
			if (pGuild->uDemoteLeaderID) {
				S32 uiMaxRank = eaSize(&pGuild->eaRanks) - 1;
				return pOtherMember->iRank+1 < uiMaxRank;
			}
			else {
				return true;
			}
...
{
	// This wants AD year, And 1-12 month. We are passed in 0-11 month
	return dayOfWeek(iYear, iMonth+1, iDay);
}
...
S32 gclGuildGetItemWithdrawLeft(ExprContext *pContext, S32 iBagID)
{
	...
	if(pGuildBankInfo && pGuildMember)
	{
		...
		iWithdrawTotal=0;
		if (NONNULL(pWithdrawLimit))
		{
			if (inv_guildbank_withdrawals_in_current_interval(pWithdrawLimit->iTimestamp))
			{
				iWithdrawTotal = pWithdrawLimit->iItemsWithdrawn;
			}
		}
...
static struct {
	U32 uGuildID;
	GuildEmblemData EmblemData;

	const char *pcEmblemColors[2];
	const char *pcEmblem2Colors[2];

	const char **eaValidEmblems;
	const char **eaValidEmblems2;
	const char **eaValidEmblems3;
} s_GuildEditEmblem;

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_FindEmblemTextures);
void gclExprGuildEmblem_FindEmblemTextures(const char *pcSkeleton, const char *pcBone)
{
	NOCONST(PlayerCostume) *pCostume = StructCreateNoConst(parse_PlayerCostume);
	S32 i, j;

	pcBone = allocAddString(pcBone);
	s_GuildEditEmblem.uGuildID = 0;

	// Clear existing lists
	eaDestroy(&s_GuildEditEmblem.eaValidEmblems);
	eaDestroy(&s_GuildEditEmblem.eaValidEmblems2);
	eaDestroy(&s_GuildEditEmblem.eaValidEmblems3);

	// Initialize the emblem costume
	SET_HANDLE_FROM_STRING("CostumeSkeleton", pcSkeleton, pCostume->hSkeleton);
	...
	// Get textures from costume bone
	for (i = 0; i < eaSize(&pCostume->eaParts); i++)
	{
		...
		if (!pBone || pBone->pcName != pcBone)
			continue;

		// Get valid textures for pcEmblem
		costumeTailor_GetValidTextures(pCostume, GET_REF(pPart->hMatDef), NULL, NULL, NULL, GET_REF(pPart->hGeoDef), NULL, NULL, kPCTextureType_Pattern, &eaTextures, false, true, true);
		for (j = 0; j < eaSize(&eaTextures); j++)
		{
			if (!stricmp(eaTextures[j]->pcName, "None") && !gConf.bAllowGuildstoHaveNoEmblems)
				continue;
			eaPush(&s_GuildEditEmblem.eaValidEmblems, eaTextures[j]->pcName);
		}

		// Get valid textures for pcEmblem2
		costumeTailor_GetValidTextures(pCostume, GET_REF(pPart->hMatDef), NULL, NULL, NULL, GET_REF(pPart->hGeoDef), NULL, NULL, kPCTextureType_Movable, &eaTextures, false, true, true);
		for (j = 0; j < eaSize(&eaTextures); j++)
		{
			eaPush(&s_GuildEditEmblem.eaValidEmblems2, eaTextures[j]->pcName);
		}

		// Get valid textures for pcEmblem3
		costumeTailor_GetValidTextures(pCostume, GET_REF(pPart->hMatDef), NULL, NULL, NULL, GET_REF(pPart->hGeoDef), NULL, NULL, kPCTextureType_Detail, &eaTextures, false, true, true);
		for (j = 0; j < eaSize(&eaTextures); j++)
		{
			eaPush(&s_GuildEditEmblem.eaValidEmblems3, eaTextures[j]->pcName);
		}

		// Get the colors for pcEmblem
		pColors = costumeTailor_GetColorSetForPart(pCostume, NULL, NULL, pPart, 2);
		s_GuildEditEmblem.pcEmblemColors[0] = SAFE_MEMBER(pColors, pcName);
		pColors = costumeTailor_GetColorSetForPart(pCostume, NULL, NULL, pPart, 3);
		s_GuildEditEmblem.pcEmblemColors[1] = SAFE_MEMBER(pColors, pcName);

		// Get the colors for pcEmblem2
		pColors = costumeTailor_GetColorSetForPart(pCostume, NULL, NULL, pPart, 0);
		s_GuildEditEmblem.pcEmblem2Colors[0] = SAFE_MEMBER(pColors, pcName);
		pColors = costumeTailor_GetColorSetForPart(pCostume, NULL, NULL, pPart, 1);
		s_GuildEditEmblem.pcEmblem2Colors[1] = SAFE_MEMBER(pColors, pcName);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_GenSetEditPointer);
void gclExprGuildEmblem_GenSetEditPointer(ExprContext* pContext, SA_PARAM_NN_VALID UIGen *pGen)
{
	ui_GenSetPointer(pGen, &s_GuildEditEmblem.EmblemData, parse_GuildEmblemData);
}

static const char *gclGuildEmblem_FixTexture(const char **eaTextures, const char *pcTexture)
{
	if (!eaSize(&eaTextures))
		return pcTexture;

	if (!pcTexture)
		pcTexture = allocAddString("None");

	if (eaFind(&eaTextures, pcTexture) >= 0)
		return pcTexture;

	return eaGet(&eaTextures, 0);
}

static U32 gclGuildEmblem_FixColor(const char *pcColorSet, U32 uColor)
{
	UIColorSet *pColorSet = RefSystem_ReferentFromString("CostumeColors", pcColorSet);
	Vec4 v4Color = { (uColor >> 0) & 0xff, (uColor >> 8) & 0xff, (uColor >> 16) & 0xff, (uColor >> 24) & 0xff };
	U32 uBestColor = uColor;
	...
	for (i = 0; i < eaSize(&pColorSet->eaColors); i++)
	{
		...
		subVec4(pColor->color, v4Color, v4Tmp);
		fLength = lengthVec4Squared(v4Tmp);
		if (fLength < fBestDistance)
		{
			uBestColor = ((U32)pColor->color[0] << 0) | ((U32)pColor->color[1] << 8) | ((U32)pColor->color[2] << 16) | ((U32)pColor->color[3] << 24);
			fBestDistance = fLength;

			if (uBestColor == uColor)
				return uColor;
		}
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_ResetPaperdollEditEmblem);
void gclExprGuildEmblem_ResetPaperdollEditEmblem(SA_PARAM_OP_VALID Entity *pEnt)
{
	Guild *pGuild = guild_GetGuild(pEnt);
	if (pGuild)
	{
		const char *pcNone = allocAddString("None");
		s_GuildEditEmblem.uGuildID = pGuild->iContainerID;
		s_GuildEditEmblem.EmblemData.iVersion = 0;

		s_GuildEditEmblem.EmblemData.pcEmblem = gclGuildEmblem_FixTexture(s_GuildEditEmblem.eaValidEmblems, pGuild->pcEmblem);
		s_GuildEditEmblem.EmblemData.iEmblemColor0 = gclGuildEmblem_FixColor(s_GuildEditEmblem.pcEmblemColors[0], pGuild->iEmblemColor0);
		s_GuildEditEmblem.EmblemData.iEmblemColor1 = gclGuildEmblem_FixColor(s_GuildEditEmblem.pcEmblemColors[1], pGuild->iEmblemColor1);
		s_GuildEditEmblem.EmblemData.fEmblemRotation = CLAMPF(pGuild->fEmblemRotation, -100, 100);

		s_GuildEditEmblem.EmblemData.pcEmblem2 = gclGuildEmblem_FixTexture(s_GuildEditEmblem.eaValidEmblems2, pGuild->pcEmblem2);
		s_GuildEditEmblem.EmblemData.iEmblem2Color0 = gclGuildEmblem_FixColor(s_GuildEditEmblem.pcEmblem2Colors[0], pGuild->iEmblem2Color0);
		s_GuildEditEmblem.EmblemData.iEmblem2Color1 = gclGuildEmblem_FixColor(s_GuildEditEmblem.pcEmblem2Colors[1], pGuild->iEmblem2Color1);
		s_GuildEditEmblem.EmblemData.fEmblem2Rotation = CLAMPF(pGuild->fEmblem2Rotation, 0, 100);
		s_GuildEditEmblem.EmblemData.fEmblem2X = CLAMPF(pGuild->fEmblem2X, -100, 100);
		s_GuildEditEmblem.EmblemData.fEmblem2Y = CLAMPF(pGuild->fEmblem2Y, -100, 100);
		s_GuildEditEmblem.EmblemData.fEmblem2ScaleX = CLAMPF(pGuild->fEmblem2ScaleX, 0, 100);
		s_GuildEditEmblem.EmblemData.fEmblem2ScaleY = CLAMPF(pGuild->fEmblem2ScaleY, 0, 100);

		s_GuildEditEmblem.EmblemData.pcEmblem3 = gclGuildEmblem_FixTexture(s_GuildEditEmblem.eaValidEmblems3, pGuild->pcEmblem3);
	}
	else
		ZeroStruct(&s_GuildEditEmblem);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_SavePaperdollEditEmblem);
bool gclExprGuildEmblem_SavePaperdollEditEmblem(SA_PARAM_OP_VALID Entity *pEnt)
{
	Guild *pGuild = guild_GetGuild(pEnt);
	if (pGuild && s_GuildEditEmblem.EmblemData.iVersion > 0)
	{
		ServerCmd_Guild_SetAdvancedEmblem(
			0 == stricmp_safe(s_GuildEditEmblem.EmblemData.pcEmblem, "None") ? NULL : s_GuildEditEmblem.EmblemData.pcEmblem,
			s_GuildEditEmblem.EmblemData.iEmblemColor0,
			s_GuildEditEmblem.EmblemData.iEmblemColor1,
			s_GuildEditEmblem.EmblemData.fEmblemRotation,
			true
		);

		ServerCmd_Guild_SetAdvancedEmblem2(
			0 == stricmp_safe(s_GuildEditEmblem.EmblemData.pcEmblem2, "None") ? NULL : s_GuildEditEmblem.EmblemData.pcEmblem2,
			s_GuildEditEmblem.EmblemData.iEmblem2Color0,
			s_GuildEditEmblem.EmblemData.iEmblem2Color1,
			s_GuildEditEmblem.EmblemData.fEmblem2Rotation,
			s_GuildEditEmblem.EmblemData.fEmblem2X,
			s_GuildEditEmblem.EmblemData.fEmblem2Y,
			s_GuildEditEmblem.EmblemData.fEmblem2ScaleX,
			s_GuildEditEmblem.EmblemData.fEmblem2ScaleY
		);

		ServerCmd_Guild_SetAdvancedEmblem3(
			0 == stricmp_safe(s_GuildEditEmblem.EmblemData.pcEmblem3, "None") ? NULL : s_GuildEditEmblem.EmblemData.pcEmblem3,
			true
		);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_GetPaperdollEditEmblem);
SA_RET_OP_VALID PaperdollHeadshotData* gclExprGuildEmblem_GetPaperdollEditEmblem(ExprContext* pContext, SA_PARAM_OP_VALID Entity *pEnt, const char *pchSkeleton, const char *pchEmblemBone, bool bBackground)
{
	Guild *pGuild = guild_GetGuild(pEnt);
	U32 uGuildID = SAFE_MEMBER(pGuild, iContainerID);
	if (s_GuildEditEmblem.uGuildID != uGuildID)
		gclExprGuildEmblem_ResetPaperdollEditEmblem(pEnt);
	return gclPaperdoll_CreateHeadshotData(pContext, NULL, gclCreateGuildEmblemCostume(uGuildID, pchSkeleton, pchEmblemBone, &s_GuildEditEmblem.EmblemData, !bBackground, false), NULL, NULL, NULL, NULL, false, false);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_Emblem_GenGetValidTextures);
void gclExprGuildEmblem_Emblem_GenGetValidTextures(SA_PARAM_NN_VALID UIGen *pGen)
{
	static PCTextureDef **s_eaTextures;

	eaClearFast(&s_eaTextures);
	FOR_EACH_IN_EARRAY_FORWARDS(s_GuildEditEmblem.eaValidEmblems, const char, pcTexture);
	{
		PCTextureDef *pTexture = RefSystem_ReferentFromString("CostumeTexture", pcTexture);
		if (pTexture)
			eaPush(&s_eaTextures, pTexture);
	}
	FOR_EACH_END;

	ui_GenSetListSafe(pGen, &s_eaTextures, PCTextureDef);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_Emblem_GenColorChooserGetValidColors);
void gclExprGuildEmblem_Emblem_GenColorChooserGetValidColors(SA_PARAM_NN_VALID UIGen *pGen, S32 iColor)
{
	UIColorSet *pColorSet = NULL;

	if (iColor >= 0 && iColor < ARRAY_SIZE(s_GuildEditEmblem.pcEmblemColors))
		pColorSet = RefSystem_ReferentFromString("CostumeColors", s_GuildEditEmblem.pcEmblemColors[iColor]);

	if (UI_GEN_IS_TYPE(pGen, kUIGenTypeColorChooser))
	{
		int **peaiColors = ui_GenGetColorList(pGen);
		eaiClearFast(peaiColors);
		if (pColorSet)
		{
			S32 i, n = eaSize(&pColorSet->eaColors);
			for (i = 0; i < n; i++)
			{
				UIColor *pColor = pColorSet->eaColors[i];
				U32 uRGBA[4] = { pColor->color[0], pColor->color[1], pColor->color[2], pColor->color[3] };
				eaiPush(peaiColors, (uRGBA[0] << 24) | (uRGBA[1] << 16) | (uRGBA[2] << 8) | (uRGBA[3] << 0));
			}
		}
	}
	else
	{
		// Set list model
		ui_GenSetListSafe(pGen, pColorSet ? &pColorSet->eaColors : NULL, UIColor);
	}
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_SetEmblem);
void gclExprGuildEmblem_SetEmblem(const char *pchTexture, U32 iColor0, U32 iColor1, F32 fRotation)
{
	// Keep rotation in valid bounds
	fRotation += 100;
	fRotation -= floorf(fRotation / 200) * 200;
	fRotation -= 100;

	pchTexture = allocAddString(pchTexture);

	if (s_GuildEditEmblem.EmblemData.pcEmblem != pchTexture)
	{
		s_GuildEditEmblem.EmblemData.pcEmblem = pchTexture;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.iEmblemColor0 != iColor0)
	{
		s_GuildEditEmblem.EmblemData.iEmblemColor0 = iColor0;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.iEmblemColor1 != iColor1)
	{
		s_GuildEditEmblem.EmblemData.iEmblemColor1 = iColor1;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.fEmblemRotation != fRotation)
	{
		s_GuildEditEmblem.EmblemData.fEmblemRotation = fRotation;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_Emblem2_GenGetValidTextures);
void gclExprGuildEmblem_Emblem2_GenGetValidTextures(SA_PARAM_NN_VALID UIGen *pGen)
{
	static PCTextureDef **s_eaTextures;

	eaClearFast(&s_eaTextures);
	FOR_EACH_IN_EARRAY_FORWARDS(s_GuildEditEmblem.eaValidEmblems2, const char, pcTexture);
	{
		PCTextureDef *pTexture = RefSystem_ReferentFromString("CostumeTexture", pcTexture);
		if (pTexture)
			eaPush(&s_eaTextures, pTexture);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_Emblem2_GenColorChooserGetValidColors);
void gclExprGuildEmblem_Emblem2_GenColorChooserGetValidColors(SA_PARAM_NN_VALID UIGen *pGen, S32 iColor)
{
	UIColorSet *pColorSet = NULL;

	if (iColor >= 0 && iColor < ARRAY_SIZE(s_GuildEditEmblem.pcEmblem2Colors))
		pColorSet = RefSystem_ReferentFromString("CostumeColors", s_GuildEditEmblem.pcEmblem2Colors[iColor]);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_SetEmblem2);
void gclExprGuildEmblem_SetEmblem2(const char *pchTexture, U32 iColor0, U32 iColor1, F32 fRotation, F32 fX, F32 fY, F32 fScaleX, F32 fScaleY)
{
	// Keep rotation in valid bounds
	fRotation -= floorf(fRotation / 100) * 100;

	// Ensure values are in valid bounds
	fX = CLAMPF(fX, -100, 100);
	fY = CLAMPF(fY, -100, 100);
	fScaleX = CLAMPF(fScaleX, 0, 100);
	fScaleY = CLAMPF(fScaleY, 0, 100);

	pchTexture = allocAddString(pchTexture);

	if (s_GuildEditEmblem.EmblemData.pcEmblem2 != pchTexture)
	{
		s_GuildEditEmblem.EmblemData.pcEmblem2 = pchTexture;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.iEmblem2Color0 != iColor0)
	{
		s_GuildEditEmblem.EmblemData.iEmblem2Color0 = iColor0;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.iEmblem2Color1 != iColor1)
	{
		s_GuildEditEmblem.EmblemData.iEmblem2Color1 = iColor1;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.fEmblem2X != fX)
	{
		s_GuildEditEmblem.EmblemData.fEmblem2X = fX;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.fEmblem2Y != fY)
	{
		s_GuildEditEmblem.EmblemData.fEmblem2Y = fY;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.fEmblem2ScaleX != fScaleX)
	{
		s_GuildEditEmblem.EmblemData.fEmblem2ScaleX = fScaleX;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.fEmblem2ScaleY != fScaleY)
	{
		s_GuildEditEmblem.EmblemData.fEmblem2ScaleY = fScaleY;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
	if (s_GuildEditEmblem.EmblemData.fEmblem2Rotation != fRotation)
	{
		s_GuildEditEmblem.EmblemData.fEmblem2Rotation = fRotation;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_Emblem3_GenGetValidTextures);
void gclExprGuildEmblem_Emblem3_GenGetValidTextures(SA_PARAM_NN_VALID UIGen *pGen)
{
	static PCTextureDef **s_eaTextures;

	eaClearFast(&s_eaTextures);
	FOR_EACH_IN_EARRAY_FORWARDS(s_GuildEditEmblem.eaValidEmblems3, const char, pcTexture);
	{
		PCTextureDef *pTexture = RefSystem_ReferentFromString("CostumeTexture", pcTexture);
		if (pTexture)
			eaPush(&s_eaTextures, pTexture);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GuildEmblem_SetEmblem3);
void gclExprGuildEmblem_SetEmblem3(const char *pchTexture)
{
	pchTexture = allocAddString(pchTexture);

	if (s_GuildEditEmblem.EmblemData.pcEmblem3 != pchTexture)
	{
		s_GuildEditEmblem.EmblemData.pcEmblem3 = pchTexture;
		s_GuildEditEmblem.EmblemData.iVersion++;
	}
}

// Can this leader rescue the guild leader being demoted. note that a leader must be in the process of demoting a leader and this rescuing leader can't be provisional
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_CanRescueLeader);
bool gclGuild_expr_CanRescueLeader(SA_PARAM_OP_VALID Entity* pEnt)
{
	GuildMember* pGuildMember = guild_FindMember( pEnt );
	...
	if(pGuildMember != NULL)
	{
		...
		// is the guild demoting leader? Also can't rescue yourself
		if(pGuild != NULL && pGuild->uDemoteLeaderID && pGuild->uDemoteLeaderID != pEnt->myContainerID)
		{
			if(aslGuild_trh_IsLeader(CONTAINER_NOCONST(Guild, pGuild), pEnt->myContainerID) && timeServerSecondsSince2000() > pGuildMember->uProvisionalTimeSeconds)
			{
				return true;
			}
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_GetDemotionPlayerName);
const char* gclGuild_expr_GetDemotionPlayerName(SA_PARAM_OP_VALID Entity* pEnt)
{
	Guild *pGuild = guild_GetGuild(pEnt);
	int i;
	static char *estrName = NULL;
	estrClear(&estrName);
	if (pGuild->uDemoteLeaderID)
	{
		for (i = 0; i < eaSize(&pGuild->eaMembers); i++)
		{
			...
			if (pGuild->uDemoteLeaderID == pMember->iEntID)
			{
				estrCopy2(&estrName, pMember->pcName);
			}
		}
	}
	return estrName;
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_GetDemotionTime);
U32 gclGuild_expr_GetDemotionTime(SA_PARAM_OP_VALID Entity* pEnt)
{
	Guild *pGuild = guild_GetGuild(pEnt);
	return SAFE_MEMBER(pGuild, uDemotionTimeSeconds);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_GetDemotionTimeRemaining);
U32 gclGuild_expr_GetDemotionTimeRemaining(SA_PARAM_OP_VALID Entity* pEnt)
{
	Guild *pGuild = guild_GetGuild(pEnt);
	U32 uiTime = SAFE_MEMBER(pGuild, uDemotionTimeSeconds);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_GetProvisionalTime);
U32 gclGuild_expr_GetProvisionalTime(SA_PARAM_OP_VALID GuildMemberData *pData)
{
	return SAFE_MEMBER(pData, uProvisionalTimeSeconds);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(guild_GetProvisionalTimeRemaining);
U32 gclGuild_expr_GetProvisionalTimeRemaining(SA_PARAM_OP_VALID GuildMemberData *pData)
{
	U32 uiTime = SAFE_MEMBER(pData, uProvisionalTimeSeconds);
	U32 uiNow = timeServerSecondsSince2000();
	if (uiTime > uiNow)
		return uiTime - uiNow;
	else
