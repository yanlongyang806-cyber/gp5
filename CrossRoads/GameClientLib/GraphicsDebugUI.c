...
static F32 addHDRSliders(UIScrollArea *scrollArea, const char *title, F32 x, F32 y)
{
	...
	char buf[128];
	int is_d3d11 = gfx_state.allRenderersFeatures & FEATURE_DX11_RENDERER;
...
static UITab* gfxDebugAddFXTab(void)
{
	...
	y = addCmdParseCheck(scrollArea, "Show Costume Setup", "costumeShowSkeletonFiles", x, y);
	y+=LINE_HEIGHT/4;
...
static UITab* gfxDebugAddCharacterTab(void)
{
	UITab *pTab = ui_TabCreate("Character");
	UIScrollArea *scrollArea;
	F32 x=0, y=2;
...
static UITab* gfxDebugAddDynamicsTab(void)
{
	...
	y = addCmdParseCheck(scrollArea, "Disable Height Fixup Fix (req. costume change)", "danimDisableHeightFixupFix", x, y);
...
void gfxDebugUI(void)
{
	if (!gfx_debug_data.window) {
		...
		eaPush(&gfx_debug_data.tab_infos, gfxDebugUICreateTabInfo(gfxDebugAddCharacterTab));
