...
void gclMicroTrans_ClearLists()
{
	...
	gclMicroTrans_UpdateNewProductsHash();
...
static void gclMicroTrans_PurchaseMicroTransactionEx(Entity *pEntity, const char *pchCategory, U32 uID, const char *pchCurrency, bool bUseItemAfterPurchase)
{
	if(pchCategory &&
		uID &&
		g_pMTList != NULL)
	{
		...
		else if(gclServerIsConnected())
		{
			if(pEntity)
			{
				ServerCmd_PurchaseProductEx(pchCategory, uID, pPrice, NULL, ui_GenExprGetCurrentCouponItemID(uID), bUseItemAfterPurchase);
			}
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(PurchaseAndUseMicroTransaction);
void gclMicroTrans_expr_PurchaseAndUseMicroTransaction(SA_PARAM_OP_VALID Entity *pEntity, SA_PARAM_OP_STR const char *pchCategory, U32 uID, SA_PARAM_OP_STR const char *pchCurrency)
{
	gclMicroTrans_PurchaseMicroTransactionEx(pEntity, pchCategory, uID, pchCurrency, true);
}
...
{
	gclMicroTrans_PurchaseMicroTransactionEx(pEntity, pchCategory, uID, pchCurrency, false);
}
...
{
	const char *tag = locGetCrypticSpecific2LetterIdentifier(getCurrentLocale());
	int backupIndex = -1;
...
__forceinline void gclMicroTrans_UpdateFlags(MicroTransactionCostume *pMTCostume, MicroTransactionCostumeSource *pMTCostumeSource)
{
	...
	pMTCostume->bOwned = pMTCostume->bOwned || (pMTCostumeSource->bOwned && !pMTCostumeSource->bHidden);
...
void gclMicroTrans_UpdateNewProductsHash()
{
	int iNewHash = 0;
	if (g_pMTList)
	{
		...
		{
			MicroTransactionProduct *pMTProduct = g_pMTList->ppProducts[iIdx];
			if( pMTProduct && pMTProduct->pProduct && pMTProduct->pProduct->pName )
			{
				bool bNew = microtrans_HasNewCategory(pMTProduct);
				if( bNew )
				{
					const char *pDefName = pMTProduct->pProduct->pName;
					iNewHash ^= hashString(pDefName, false); // Use XOR to combine the hashes, so the order of the products list doesn't matter.
				}
			}
		}
	}
	g_hashNewProducts = iNewHash;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MicroTrans_StoreHasAnyNewProducts);
bool exprMicroTrans_StoreHasAnyNewProducts()
{
	// Compare the current NewCategory hash with the last one the player saw.
	// If they're different, there's new stuff.
	return g_hashNewProducts != GamePrefGetInt("CStore.NewProductsHash", 0);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MicroTrans_StoreSeeNewProducts);
void exprMicroTrans_StoreSeeNewProducts()
{
	// Store the current NewCategory hash into the last one the player saw.
	GamePrefStoreInt("CStore.NewProductsHash", g_hashNewProducts);
}
