...
// These function are all problematic with regards to them depending on EventDef_Find
//  which will only work if the dictionary has actually sent the reference data to the client
//  already.
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("Event_GetBackground");
const char * exprEvent_GetBackground(SA_PARAM_OP_STR const char *pchEventName)
{
	EventDef *pEventDef = EventDef_Find(pchEventName);

	if (pEventDef)
	{
		return pEventDef->pchBackground ? pEventDef->pchBackground : "";
	}
	return "";
}
