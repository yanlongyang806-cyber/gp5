...
LATELINK;
void gameSpecific_EnterGameplay(void);

void DEFAULT_LATELINK_gameSpecific_EnterGameplay(void)
{
}
...
static void gclGamePlay_clientTimerConnected(bool bConnected)
{
	ui_GenConnectTimer(bConnected);
}

AUTO_STARTUP( AS_SupportedLanguages );
void gclSupportedLanguages(void)
{
	// this should never get called, but is needed for the graph
	// calculation to work.
	assert( 0 );
}
...
AUTO_STARTUP(GameClient) ASTRT_DEPS(GraphicsLib,
									...
									AS_ItemSalvageDefMinimal,
									AutoStart_ZoneRewards,
									AutoStart_StickerBook
								);
void gclStartup(void)
{
	...
	if (isDevelopmentMode()) {
		...
		ui_GenInitTimer();
		timerSetConnectedCallback(gclGamePlay_clientTimerConnected);
	}
}
...
void gclInit_EndFrame(void)
{
	...
	// There are a couple frames that render while waiting for the
	// graphics settings to load. Instead of rendering a blue screen
	// of nothing, render a black screen.
	display_sprite_tex(white_tex, 0, 0, -1, 1500, 1500, 0x000000FF);
...
void gclInit_Enter(void)
{
	...
	dynFxSetSetAlienColorFunc(gfxAlienChangeColor);
	...
	{
		eaPush(&gGCLState.ppLoginServerNames, ORGANIZATION_DOMAIN);
	}
...
{
	// Fixes ClientBinner GameClient crashes due to corrupt rdrShaderCache.hogg. This can be removed once we permanently move gclGfxShutdown
	// from here in gclBase_Leave to the end of gclMain.
	if(!g_GCL_GFX_Shutdown_Always)
		gclGfxShutdown();
...
void gclGamePlay_Enter(void)
{
	...
	gclBasicOptions_EnterGameplay();

	gameSpecific_EnterGameplay();
...
void gclGamePlay_Leave(void)
{
	...
	mapState_ResetAllNodeVisibleOverrides();
