...
Entity *
gclLogin2_CharacterDetailCache_GetPuppetWithSet(ContainerID playerID, CharClassTypes puppetClassType, CharClassCategorySet *pSet)
{
	...
	if ( characterDetail )
	{
		for ( i = eaSize(&characterDetail->activePuppetEnts) - 1; i >= 0; i-- )
		{
			...
			if ( tmpPuppetEnt->pChar && CharClassCategorySet_checkIfPassEntity(pSet, tmpPuppetEnt) )
			{
...
// Find a puppet entity with matching type in the character details for the given character.
Entity *
gclLogin2_CharacterDetailCache_GetPuppetWithSetName(ContainerID playerID, CharClassTypes puppetClassType, const char* pchSet)
{
	bool bStrExists = pchSet || *pchSet;
	CharClassCategorySet *pSet = bStrExists ? CharClassCategorySet_find(pchSet) : NULL;
	return (!bStrExists || pSet) ? gclLogin2_CharacterDetailCache_GetPuppetWithSet(playerID, puppetClassType, pSet) : NULL;
}
...
{
	return gclLogin2_CharacterDetailCache_GetPuppetWithSet(playerID, puppetClassType, NULL);
}
...
const char *gclLogin_GetExtraHeaderForPurchaseDef(Login2CharacterChoice *pChoice, GameAccountDataNumericPurchaseDef* pDef)
{
	if(pChoice && pDef)
	{
		return GetExtraStringFromCharacterChoice(pChoice, pDef->iExtraHeaderID);
	}
...
{
	Login2CharacterDetail *pCharDetail = gclLogin2_CharacterDetailCache_Get(pChoice->containerID);
	if(pCharDetail && pCharDetail->playerEnt)
	{
		return inv_GetNumericItemValue(pCharDetail->playerEnt, pDef->pchNumericItemDef);
	}
