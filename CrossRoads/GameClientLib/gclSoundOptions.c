...
OptionSetting *g_SoundOutputDevice;
...
void audioDriverUpdateCallback(OptionSetting *option)
{
	...
	option->iIntValue = g_audio_state.curDriver;
	OptionSettingChanged(option, false);
}
...
void gclSoundUpdateOutputDevice(int iUseDevice)
{
	if(g_SoundOutputDevice)
	{
		g_SoundOutputDevice->iIntValue = iUseDevice;
		OptionSettingChanged(g_SoundOutputDevice, false);
	}
}

void gclSoundUpdateOutputDevices(void)
{	
	static char** outputDriverOptions = NULL;
	eaClear(&outputDriverOptions);
	sndGetDriverNames(&outputDriverOptions);
	OptionSettingUpdateComboBoxOptions(g_SoundOutputDevice, AUDIO_CATEGORY, &outputDriverOptions, false);
}
...
static void gclSoundOptions_Init(void)
{
	...
	g_SoundOutputDevice = autoSettingsAddComboBox(	AUDIO_CATEGORY, "OutputDevice", &outputDriverOptions, false, (int*) &g_audio_state.curDriver,
	...
	sndOptionsSetCallbacks(gclSoundUpdateOutputDevice);
	sndOptionsDataSetCallbacks(gclSoundUpdateOutputDevices);
}
