...
// Hides the entire demo UI. Use the tilde console command quit to terminate the demo playback, 
// or the client window's close button.
static bool s_demoHideUI = false;
AUTO_CMD_INT( s_demoHideUI, demo_hide_ui ) ACMD_COMMANDLINE ACMD_ACCESSLEVEL(0);

// Prevents display of the performance test results dialog at the end of demo playback.
static bool s_demoNoResultsDialog = false;
AUTO_CMD_INT( s_demoNoResultsDialog, demo_no_results_dialog ) ACMD_COMMANDLINE ACMD_ACCESSLEVEL(0);
...
static void demo_FatalErrorf( const char* fmt, ... )
{
	if( g_isContinuousBuilder ) {
		...
		char* estr = NULL;
		estrGetVarArgs( &estr, fmt );
		
		errorDialogInternal( NULL, estr, "Error", "Error", 1 );
		estrDestroy( &estr );
	}
...
void demo_startMapPatching(void)
{
	if ( s_Demo.zoneName ){
		...
		if (!worldZoneMapStartPatchingByName(nameToUse))
		{
			...
			demo_FatalErrorf("Unable to load zone map \"%s\"", nameToUse);
		}
	} else if( s_Demo.zmInfo ) {
		if (!worldZoneMapStartPatching(s_Demo.zmInfo))
		{
			demo_FatalErrorf("Unable to load zone map \"%s\"", zmapInfoGetPublicName(s_Demo.zmInfo));
		}
	} else {
		demo_FatalErrorf( "Internal Error: Demo appears to have no relevant zone map." );
	}
}
...
void demo_RecordMapStateDiff(MapState* mapState, U32 pakID)
{
	...
	pUpdate = StructCreate( parse_RecordedMapStateUpdate );
	pUpdate->pakID = pakID;
	StructWriteTextDiff( &pUpdate->diffUpdate, parse_MapState, s_pMapStateBefore, mapState, 0, 0, 0, TEXTDIFFFLAG_SKIP_POLYMORPHIC_CHILDREN );
...
void gclDemoPlayback_Enter(void)
{
	...
	if( !s_demoScreenshotFileNamePrefix && !s_demoHideUI ) {
		demo_AddUI();
    }
}
...
void demo_LoadReplay(void)
{
	...
	if (!fileExists(fname_buf)) {
		demo_FatalErrorf("Could not find demo %s.", s_demo_filename);
	}
	...
	if (!ParserReadTextFile(fname_buf, parse_DemoRecording, &s_Demo, PARSER_NOERRORFSONPARSE)) {
		...
		if (!ParserReadTextFile(fname_buf, parse_DemoRecording, &s_Demo, 0)) {
			demo_FatalErrorf("Demo in old format or corrupt.  Failed to load demo file %s", s_demo_filename);
		}
	}
...
static void demoEnsureLegacyMode( void )
{
...

	{
		...
		if( !ParserFindColumn( parse_RecordedEntity, "entityAttach", &col )) {
			demo_FatalErrorf( "Could not find column %s.", "entityAttach" );
		}

		parse_RecordedEntity[ col ].subtable = parse_SavedEntityDataDemo;
	}
	
	ParserSetTableInfo(parse_SavedEntityDataDemo, sizeof(SavedEntityData), "SavedEntityDataDemo", NULL, "gclDemo.c", SETTABLEINFO_ALLOW_CRC_CACHING);
