...
void gclUGC_InitUI(void)
{
	...
	ui_GenInitStaticDefineVars(UGCTagListTypeEnum, "UGCTagListType_");
}
...
static bool s_bDefaultFeaturedIncludeArchives = false;

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchDefaultFeaturedIncludeArchives);
void gclUGC_SearchSetDefaultFeaturedIncludeArchives(bool bDefaultFeaturedIncludeArchives)
{
	s_bDefaultFeaturedIncludeArchives = bDefaultFeaturedIncludeArchives;
}
...
void gclUGC_SearchSetOptionRanges(F32 fRatingMin, F32 fRatingMax, F32 fDurationMin, F32 fDurationMax)
{
...
AUTO_ENUM;
typedef enum UGCTagListType
{
	UGCTagListType_All, // Used to store tags selected for search combo menu associated with "Include all of these tags:"
	UGCTagListType_Any, // Used to store tags selected for search combo menu associated with "Include any of these tags:"
	UGCTagListType_None, // Used to store tags selected for search combo menu associated with "Include none of these tags:"
	UGCTagListType_Submit, // Used to store tags selected when submitting a review
	UGCTagListType_Max, EIGNORE
} UGCTagListType;
extern StaticDefineInt UGCTagListTypeEnum[];

static int *s_eaiUGCTags_ByListType[UGCTagListType_Max] = { NULL, NULL, NULL };

static bool s_bEnableAL = false;

static UGCProjectSearchInfo* gclUGC_CreateSearchInfo(const char *pSimple, 
													 int iSpecialType, 
													 const char *pAuthorName, bool bAuthorExact,
													 const char *pTitle, bool bTitleExact,
													 const char *pSummary,
													 const char *pShortCode, bool bShortCodeExact,
													 S32 eLanguage,
													 ...
													 bool bFeaturedIncludeArchives)
{
	...
	eaiCopy(&pSearchInfo->eaiIncludeAllTags, &s_eaiUGCTags_ByListType[UGCTagListType_All]);
	eaiCopy(&pSearchInfo->eaiIncludeAnyTags, &s_eaiUGCTags_ByListType[UGCTagListType_Any]);
	eaiCopy(&pSearchInfo->eaiIncludeNoneTags, &s_eaiUGCTags_ByListType[UGCTagListType_None]);
	...
	// Filter by ID string
	if (pShortCode && pShortCode[0])
	{
		pFilter = StructCreate(parse_UGCProjectSearchFilter);
		pFilter->eType = UGCFILTER_STRING;
		pFilter->pField = StructAllocString("ID");
		pFilter->eComparison = bShortCodeExact ? UGCCOMPARISON_EXACT : UGCCOMPARISON_CONTAINS;
		pFilter->pStrValue = estrCreateFromStr(pShortCode);
	...
	if (pTitle && pTitle[0])
	{
		pFilter = StructCreate(parse_UGCProjectSearchFilter);
		pFilter->eType = UGCFILTER_STRING;
		pFilter->pField = StructAllocString("Name");
		pFilter->eComparison = bTitleExact ? UGCCOMPARISON_EXACT : UGCCOMPARISON_CONTAINS;
		pFilter->pStrValue = estrCreateFromStr(pTitle);
		eaPush(&pSearchInfo->ppFilters, pFilter);
	}
	...
	if (pAuthorName && pAuthorName[0])
	{
		...
		pFilter->eComparison = bAuthorExact ? UGCCOMPARISON_EXACT : UGCCOMPARISON_CONTAINS;
		pFilter->pStrValue = estrCreateFromStr(pAuthorName);
		eaPush(&pSearchInfo->ppFilters, pFilter);
	}

	// Filter by summary
	if (pSummary && pSummary[0])
	{
		pFilter = StructCreate(parse_UGCProjectSearchFilter);
		pFilter->eType = UGCFILTER_STRING;
		pFilter->pField = StructAllocString("Description");
		pFilter->eComparison = UGCCOMPARISON_CONTAINS;
		pFilter->pStrValue = estrCreateFromStr(pSummary);
		eaPush(&pSearchInfo->ppFilters, pFilter);
	}
	...
	pSearchInfo->iAccessLevel = s_bEnableAL;
								  ...
								  )
{
	UGCProjectSearchInfo* pSearchInfo = gclUGC_CreateSearchInfo(pSimple, 
																iSpecialType, 
																pAuthorName, bAuthorExact,
																pTitle, bTitleExact,
																pSummary,
																pShortCode, bShortCodeExact,
																eLanguage,
																pchLocation,
...
void gclUGC_SearchAdvanced(const char *pSimple,
	int iSpecialType, 
	const char *pAuthorName, int bAuthorExact,
	const char *pTitle, int bTitleExact,
	const char *pSummary,
	const char *pShortCode, int bShortCodeExact,
	int eLanguage,
	const char *pchLocation)
{
	gclUGC_SearchInternal(pSimple, iSpecialType, pAuthorName, bAuthorExact, pTitle, bTitleExact, pSummary, pShortCode, bShortCodeExact, eLanguage, pchLocation, s_bDefaultFeaturedIncludeArchives);
}
...
void gclUGC_SearchEx(
					...
					F32 fDurationMax)
{
	gclUGC_SearchSetOptionRanges(fRatingMin, fRatingMax, fDurationMin, fDurationMax);
	gclUGC_SearchInternal(pSimple, iSpecialType, pAuthorName, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, eLanguage, "", s_bDefaultFeaturedIncludeArchives);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchAgent);
void gclUGC_SearchContactUGCSearchAgent(int eLanguage)
{
	const char* location = entActivePlayerPtr()->pPlayer->pInteractInfo->pContactDialog->pUGCSearchAgentData->location;
	gclUGC_SearchInternal(/*pSimple=*/NULL, SPECIALSEARCH_FEATURED_AND_MATCHING, /*pAuthorName=*/NULL, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, eLanguage, location, s_bDefaultFeaturedIncludeArchives);
...
{
	gclUGC_SearchInternal(pSimple, SPECIALSEARCH_NONE, /*pAuthorName=*/NULL, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, -1, NULL, s_bDefaultFeaturedIncludeArchives);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_GetBrowseContent);
void gclUGC_GetBrowseContent(S32 eLanguage)
{
	gclUGC_SearchInternal(NULL, SPECIALSEARCH_BROWSE, /*pAuthorName=*/NULL, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, eLanguage, NULL, s_bDefaultFeaturedIncludeArchives);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_GetNewContent);
void gclUGC_SearchNewContent(S32 eLanguage)
{
	gclUGC_SearchInternal(NULL, SPECIALSEARCH_NEW, /*pAuthorName=*/NULL, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, eLanguage, NULL, s_bDefaultFeaturedIncludeArchives);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_GetFeaturedContent);
void gclUGC_GetFeaturedContent(S32 eLanguage, bool bIncludeArchives)
{
	gclUGC_SearchInternal(NULL, SPECIALSEARCH_FEATURED, /*pAuthorName=*/NULL, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, eLanguage, NULL, bIncludeArchives);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_GetSubscribedContent);
void gclUGC_SearchSubcribedContent(void)
{
	gclUGC_SearchInternal(NULL, SPECIALSEARCH_SUBCRIBED, /*pAuthorName=*/NULL, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, -1, NULL, s_bDefaultFeaturedIncludeArchives);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_GetReviewContent);
void gclUGC_SearchReviewContent(void)
{
	gclUGC_SearchInternal(NULL, SPECIALSEARCH_REVIEWER, /*pAuthorName=*/NULL, /*bAuthorExact=*/false, /*pTitle=*/NULL, /*bTitleExact=*/false, /*pSummary=*/NULL, /*pShortCode=*/NULL, /*bShortCodeExact=*/false, -1, NULL, s_bDefaultFeaturedIncludeArchives);
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_ContentNumRatingsAtValue);
float gclUGC_ContentNumRatingsAtValue(SA_PARAM_OP_VALID UGCContentInfo* pInfo, float fValue)
{
	if( pInfo ) {
		if( pInfo->iUGCProjectID ) {
			return gclUGC_NumRatingsAtValue( gclUGC_CacheGetProject( pInfo->iUGCProjectID ), fValue);
		} else if( pInfo->iUGCProjectSeriesID ) {
			return gclUGC_NumRatingsAtValueSeries( gclUGC_CacheGetProjectSeries( pInfo->iUGCProjectSeriesID ), fValue);
		}
	}
	return 0;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_NumTagsAtValue);
int gclUGC_NumTagsAtValue(SA_PARAM_OP_VALID UGCProject* pProject, S32 iTag)
{
	int iCount = 0;
	if(pProject)
	{
		UGCTagData *pUGCTagData = eaIndexedGetUsingInt(&pProject->ugcReviews.eaTagData, iTag);
		if(pUGCTagData)
			iCount = pUGCTagData->iCount;
	}
	return iCount;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_NumTagsAtValueSeries);
int gclUGC_NumTagsAtValueSeries(SA_PARAM_OP_VALID UGCProjectSeries* pSeries, S32 iTag)
{
	int iCount = 0;
	if(pSeries)
	{
		UGCTagData *pUGCTagData = eaIndexedGetUsingInt(&pSeries->ugcReviews.eaTagData, iTag);
		if(pUGCTagData)
			iCount = pUGCTagData->iCount;
	}
	return iCount;
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_ContentNumTagsAtValue);
float gclUGC_ContentNumTagsAtValue(SA_PARAM_OP_VALID UGCContentInfo* pInfo, S32 iTag)
{
	if( pInfo ) {
		if( pInfo->iUGCProjectID ) {
			return gclUGC_NumTagsAtValue( gclUGC_CacheGetProject( pInfo->iUGCProjectID ), iTag);
		} else if( pInfo->iUGCProjectSeriesID ) {
			return gclUGC_NumTagsAtValueSeries( gclUGC_CacheGetProjectSeries( pInfo->iUGCProjectSeriesID ), iTag);
		}
	}
	return 0;
}
...
void gclUGC_AddReview(SA_PARAM_OP_VALID UGCProject *pProject, float fRating, char *pComment)
{
	...
	if (pProject && gclUGC_CanReview(pProject))
	{
		...
		ugcReviewData.eaiUGCTags = s_eaiUGCTags_ByListType[UGCTagListType_Submit];
...
void gclUGC_AddReviewSeries(SA_PARAM_OP_VALID UGCProjectSeries* pSeries, float fRating, char* pComment)
{
	if( pSeries && gclUGC_CanReviewSeries( pSeries )) {
		...
		ServerCmd_gslUGC_AddReviewSeries( pSeries->id, pVersion->strName, &ugcReviewData);
	}
}
...
{
	return UGCProject_IsFeatured(CONTAINER_NOCONST(UGCProject, pProject), gConf.bUGCPreviouslyFeaturedMissionsQualifyForRewards, false);
}
...
U32 ugcCacheRetainTimeInSeconds = 30;
AUTO_CMD_INT( ugcCacheRetainTimeInSeconds, ugcCacheRetainTimeInSeconds );

// How long do we require between cache requests.
U32 ugcCacheThrottleTimeInSeconds = 1;
AUTO_CMD_INT( ugcCacheThrottleTimeInSeconds, ugcCacheThrottleTimeInSeconds );

static StashTable s_stProjectCache = NULL;
static StashTable s_stProjectSeriesCache = NULL;
static UGCIDList s_RequestedIDs;
static int s_RequestedLastTime = 0;
...
void gclUGC_CacheOncePerFrame( void )
{
	...
	if( gGCLState.totalElapsedTimeMs - s_RequestedLastTime > ugcCacheThrottleTimeInSeconds * 1000 ) {
		if(ea32Size(&s_RequestedIDs.eaProjectIDs) || ea32Size(&s_RequestedIDs.eaProjectSeriesIDs)) {
			// Only make 500 requests of each type per request, to
			// avoid destroying the server.
			const int maxRequestPerType = 500;
			
			if(linkConnected(gServerLink)) {
				UGCIDList searchIDList = { 0 };
				StructCopy( parse_UGCIDList, &s_RequestedIDs, &searchIDList, 0, 0, 0 );
				if( eaiSize( &searchIDList.eaProjectIDs ) > maxRequestPerType ) {
					eaiSetSize( &searchIDList.eaProjectIDs, maxRequestPerType );
				}
				if( eaiSize( &searchIDList.eaProjectSeriesIDs ) > maxRequestPerType ) {
					eaiSetSize( &searchIDList. eaProjectSeriesIDs, maxRequestPerType );
				}

				ServerCmd_gslUGC_CacheSearchByID(&searchIDList);
				s_RequestedLastTime = gGCLState.totalElapsedTimeMs;
				StructReset( parse_UGCIDList, &searchIDList );
			}

			eaiRemoveRange( &s_RequestedIDs.eaProjectIDs, 0, maxRequestPerType );
			eaiRemoveRange( &s_RequestedIDs.eaProjectSeriesIDs, 0, maxRequestPerType );
		}
	}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchEnableAL);
void gclUGC_SearchEnableAL(bool bEnable)
{
	s_bEnableAL = bEnable;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchQueryAL);
bool gclUGC_SearchQueryAL()
{
	return s_bEnableAL;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchSetTag);
void gclUGC_SearchSetTag(S32 ugcTagListType, S32 ugcTag, bool bSet)
{
	if(bSet)
		eaiPushUnique(&s_eaiUGCTags_ByListType[ugcTagListType], ugcTag);
	else
		eaiFindAndRemoveFast(&s_eaiUGCTags_ByListType[ugcTagListType], ugcTag);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchGetTag);
bool gclUGC_SearchGetTag(S32 ugcTagListType, S32 ugcTag)
{
	return 0 <= eaiFind(&s_eaiUGCTags_ByListType[ugcTagListType], ugcTag);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchSetAllTags);
void gclUGC_SearchSetAllTags(S32 ugcTagListType)
{
	S32 *eaValues = NULL;
	UGCTagFillAllKeysAndValues(NULL, &eaValues);
	FOR_EACH_IN_EARRAY_INT(eaValues, S32, ugcTag)
	{
		gclUGC_SearchSetTag(ugcTagListType, ugcTag, true);
	}
	FOR_EACH_END;
	eaiDestroy(&eaValues);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchClearAllTags);
void gclUGC_SearchClearAllTags(S32 ugcTagListType)
{
	S32 *eaValues = NULL;
	UGCTagFillAllKeysAndValues(NULL, &eaValues);
	FOR_EACH_IN_EARRAY_INT(eaValues, S32, ugcTag)
	{
		gclUGC_SearchSetTag(ugcTagListType, ugcTag, false);
	}
	FOR_EACH_END;
	eaiDestroy(&eaValues);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchAllTagsSet);
bool gclUGC_SearchAllTagsSet(S32 ugcTagListType)
{
	bool bResult = true;
	S32 *eaValues = NULL;
	UGCTagFillAllKeysAndValues(NULL, &eaValues);
	FOR_EACH_IN_EARRAY_INT(eaValues, S32, ugcTag)
	{
		if(!gclUGC_SearchGetTag(ugcTagListType, ugcTag))
		{
			bResult = false;
			break;
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchAllTagsClear);
bool gclUGC_SearchAllTagsClear(S32 ugcTagListType)
{
	bool bResult = true;
	S32 *eaValues = NULL;
	UGCTagFillAllKeysAndValues(NULL, &eaValues);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchCountTagsSet);
S32 gclUGC_SearchCountTagsSet(S32 ugcTagListType)
{
	S32 iCount = 0;
	S32 *eaValues = NULL;
	UGCTagFillAllKeysAndValues(NULL, &eaValues);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchTagsSetFromReviewableInfo);
void gclUGC_SearchTagsSetFromReviewableInfo(S32 ugcTagListType)
{
	const UGCSingleReview *pReview = SAFE_MEMBER2(spReviewableProject, pExtraDetailData, pReviewForCurAccount);
	if(pReview)
	{
		S32 *eaValues = NULL;
		UGCTagFillAllKeysAndValues(NULL, &eaValues);
		FOR_EACH_IN_EARRAY_INT(eaValues, S32, ugcTag)
		{
			if(eaiFind(&pReview->eaiUGCTags, ugcTag) >= 0)
				gclUGC_SearchSetTag(ugcTagListType, ugcTag, true);
		}
		FOR_EACH_END;
		eaiDestroy(&eaValues);
		...
		gclUGC_SearchAllTagsClear(ugcTagListType);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchTagsSetFromReviewableSeriesInfo);
void gclUGC_SearchTagsSetFromReviewableSeriesInfo(S32 ugcTagListType)
{
	const UGCSingleReview *pReview = SAFE_MEMBER2(spReviewableProjectSeries, pExtraDetailData, pReviewForCurAccount);
	if(pReview)
	{
		S32 *eaValues = NULL;
		UGCTagFillAllKeysAndValues(NULL, &eaValues);
...
static int ugcTagComparator(const UGCTag *a, const UGCTag *b)
{
	return *b - *a;
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(ugc_SearchTagsString);
char *gclUGC_SearchTagsString(S32 ugcTagListType)
{
	static char *estrTags = NULL;

	estrClear(&estrTags);
	eaiQSort(s_eaiUGCTags_ByListType[ugcTagListType], ugcTagComparator);
	FOR_EACH_IN_EARRAY_INT(s_eaiUGCTags_ByListType[ugcTagListType], S32, ugcTag)
	{
		const char *pchName = StaticDefineGetTranslatedMessage(UGCTagEnum, ugcTag);
		if(estrLength(&estrTags) == 0)
			estrConcatf(&estrTags, "%s", pchName);
		else
			estrConcatf(&estrTags, ", %s", pchName);
	}
	FOR_EACH_END;
...
AUTO_COMMAND ACMD_HIDE ACMD_ACCESSLEVEL(4);
void ugcClearSearchCache()
{
	// free projects
	{
		StashTableIterator it = { 0 };
	...
	ServerCmd_ugcSearchCacheClear();
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME("ugc_GetSupportedLanguagesThisShard");
ExprFuncReturnVal gclUG_GetSupportedLanguagesThisShard( SA_PARAM_NN_VALID UIGen *pGen )
{
	const int* eaSupportedLanguages = langGetSupportedLanguagesThisShard();
	
	// MJF Oct/21/2013 -- StarTrek has a hard-coded list of languages
	// that are supported and is a superset of the languages supported
	// by the shard.
	if( isDevelopmentMode() ) {
		assert( stricmp( "Night", GetProductName() ) == 0 );
	}

	{
		...
		eaSetSizeStruct( peaList, parse_GenEnum, eaiSize( &eaSupportedLanguages ));
		for( it = 0; it != eaiSize( &eaSupportedLanguages ); ++it ) {
			Language lang = eaSupportedLanguages[ it ];
			LocaleID loc = locGetIDByLanguage( lang );

			(*peaList)[ it ]->key = locGetDisplayName( loc );
			(*peaList)[ it ]->value = lang;
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("GenGetUGCTagList");
ExprFuncReturnVal ui_GenGetUGCTagList(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, ACMD_EXPR_ERRSTRING errEstr)
{
	char** eaKeys = NULL;
	S32* eaValues = NULL;
	GenEnum ***peaList = ui_GenGetManagedListSafe(pGen, GenEnum);
	...
	UGCTagFillAllKeysAndValues(&eaKeys, &eaValues);
		
	while (i < eaSize(&eaKeys))
	{
		if(eaValues[i] >= INT_MIN && eaValues[i] <= INT_MAX)
		{
			if (j >= eaSize(peaList)) 
			{
