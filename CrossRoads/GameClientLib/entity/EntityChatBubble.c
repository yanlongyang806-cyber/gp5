...
// 
void ChatBubble_PerformMessageTagsForEntity(const char **ppchMessage, EntityRef hEntity, bool bPlayer)
{
	...
	if( pchMessage && pchMessage[0] == '<')
	{
		...
		{
			// check for sound tag
			if(!bPlayer && // prevent grief
				!strnicmp(pchMessage, pchSoundTagOpen, iSoundTagLen))
			{
...
void ChatBubble_Say(EntityRef hEntity, const char *pchMessage, F32 fDuration, const char *pchBubble)
{
	...
	if (eaSize(peaBubbleStack) < ciQueueLimit)
	{
		...
		ChatBubble_PerformMessageTagsForEntity(&pchMessage, hEntity, pEnt->myEntityType == GLOBALTYPE_ENTITYPLAYER);
