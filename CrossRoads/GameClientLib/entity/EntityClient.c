...
static S32 gclEntityTickDoFade(const Vec3 vSourcePos, Entity* e, ClientOnlyEntity* coe, F32 elapsed)
{
	...
	if (bImperceptible || e->bFadeOutAndThenRemove || e->bForceFadeOut || e->bInCutscene || e->bDeadBodyFaded || (e->pChar && e->pChar->bDoNotDraw))
		fAlphaGoal = 0.0f;
...
static void gclEntityTickUpdateSkeleton(SA_PARAM_NN_VALID Entity* e)
{
	...
	if (pActivePlayer)
	{
		if (pActivePlayer->myRef == e->myRef && pContactDialog) // The current player is in a contact dialog
		{
			...
			else if ((pCurrentContact = entFromEntityRefAnyPartition(pContactDialog->headshot.headshotEnt)) != NULL)
			{
				eaiPushUnique(&s_eaiContactedEntRefs, pContactDialog->headshot.headshotEnt);
			}
...
static S32 entSkeletonGetAudioDebugInfo(DynSkeleton *pSkeleton)
{
	...
	pSkeleton->debugCostumeInfo.pcDebugCostume			= NULL;
	pSkeleton->debugCostumeInfo.pcDebugCostumeFilename	= NULL;
	pSkeleton->debugCostumeInfo.pcDebugCSkel			= NULL;
	pSkeleton->debugCostumeInfo.pcDebugCSkelFilename	= NULL;
	pSkeleton->debugCostumeInfo.pcDebugSpecies			= NULL;
	pSkeleton->debugCostumeInfo.pcDebugSpeciesFilename	= NULL;
	eaClear(&pSkeleton->debugCostumeInfo.eaDebugRequiredClickBoneNames);
	eaClear(&pSkeleton->debugCostumeInfo.eaDebugOptionalClickBoneNames);
	...
	if (e)
	{
		...
		if (c)
		{
			pSkeleton->debugCostumeInfo.pcDebugCostume			= c->pcName;
			pSkeleton->debugCostumeInfo.pcDebugCostumeFilename	= c->pcFileName;

			if (s = GET_REF(c->hSkeleton))
			{
				pSkeleton->debugCostumeInfo.pcDebugCSkel			= s->pcName;
				pSkeleton->debugCostumeInfo.pcDebugCSkelFilename	= s->pcFileName;

				FOR_EACH_IN_EARRAY(s->eaRequiredBoneDefs, PCBoneRef, pBoneRef) {
					PCBoneDef* pBoneDef = GET_REF(pBoneRef->hBone);
					if (pBoneDef) {
						eaPushUnique(&pSkeleton->debugCostumeInfo.eaDebugRequiredClickBoneNames,FIRST_IF_SET(pBoneDef->pcClickBoneName,pBoneDef->pcBoneName));
					}
				} FOR_EACH_END;

				FOR_EACH_IN_EARRAY(s->eaOptionalBoneDefs, PCBoneRef, pBoneRef) {
					PCBoneDef* pBoneDef = GET_REF(pBoneRef->hBone);
					if (pBoneDef) {
						eaPushUnique(&pSkeleton->debugCostumeInfo.eaDebugOptionalClickBoneNames,FIRST_IF_SET(pBoneDef->pcClickBoneName,pBoneDef->pcBoneName));
					}
				} FOR_EACH_END;
			}
			...
			{
				pSkeleton->debugCostumeInfo.pcDebugSpecies			= r->pcName;
				pSkeleton->debugCostumeInfo.pcDebugSpeciesFilename	= r->pcFileName;
			}
		}
	}
