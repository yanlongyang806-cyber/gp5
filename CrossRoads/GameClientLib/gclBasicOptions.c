...
#include "AutoGen/gclOptions_h_ast.h"
...
static const char **s_ppchColorPalettes;
static S32 s_iColorPaletteSelection = 0;
...
static bool gclBasicOptions_HideColorPaletteOptions(OptionSetting* pSetting)
{
	return entActivePlayerPtr() == NULL;
}

static void gclBasicOptions_ColorPaletteChanged(OptionSetting* pSetting)
{
	OptionSettingComboChoice *pChoice = eaGet(&pSetting->eaComboBoxOptions, pSetting->iIntValue);
	const char *pchName = pChoice ? eaGet(&s_ppchColorPalettes, pChoice->iIndex) : NULL;
	Entity *pEnt = entActivePlayerPtr();
	ui_StyleColorPaletteSwitchPriority(pchName, kUIStyleColorPalettePriority_User);
	if (pEnt)
		ServerCmd_gslSetUserColorPalette(pchName);
}

static void gclBasicOptions_ColorPaletteUpdate(OptionSetting* pSetting)
{
	OptionSettingComboChoice *pChoice;
	UIStyleColorPaletteDef *pPalette;
	S32 i, iCount = 0, iVisible = 0;
	const char *pchCurrent = ui_StyleColorPalettePriorityCurrent(kUIStyleColorPalettePriority_User);

	// Create the default option
	pChoice = eaGetStruct(&pSetting->eaComboBoxOptions, parse_OptionSettingComboChoice, iCount++);
	pChoice->pchName = NULL;
	SET_HANDLE_FROM_STRING("Message", "Options_Setting_Basic_ColorPalette_Default", pChoice->hDisplayName);
	pChoice->iIndex = 0;
	pChoice->bHideChoice = false;
	eaSet(&s_ppchColorPalettes, NULL, iCount - 1);

	// Add the palettes
	for (i = 0; i < eaSize(&g_ColorPalettes.eaPalettes); i++)
	{
		bool bHideChoice = false;
		pPalette = g_ColorPalettes.eaPalettes[i];

		if (!IS_HANDLE_ACTIVE(pPalette->BasePalette.hDisplayName))
			bHideChoice = true;
		...
		pChoice = eaGetStruct(&pSetting->eaComboBoxOptions, parse_OptionSettingComboChoice, iCount++);
		pChoice->pchName = pPalette->pchName;
		COPY_HANDLE(pChoice->hDisplayName, pPalette->BasePalette.hDisplayName);
		pChoice->iIndex = i;
		eaSet(&s_ppchColorPalettes, pPalette->pchName, iCount - 1);

		iVisible++;
	}

	eaSetSizeStruct(&pSetting->eaComboBoxOptions, parse_OptionSettingComboChoice, iCount);
	eaSetSize(&s_ppchColorPalettes, iCount);

	s_iColorPaletteSelection = eaFind(&s_ppchColorPalettes, pchCurrent);
	pSetting->bHide = iVisible <= 1;
}

static U32 s_iColorPaletteStates;

static U32 gclBasicOptions_GetColorPaletteDisables(void)
{
	return ~s_iColorPaletteStates;
}

static void gclBasicOptions_ColorPaletteStateDisablesChanged(OptionSetting* pSetting)
{
	Entity *pEnt = entActivePlayerPtr();
	if (pEnt)
		ServerCmd_gslSetColorPaletteStateDisables(~s_iColorPaletteStates);
}
...
static void gclBasicOptions_Revert(OptionSetting *setting)
{
	...
	else if (stricmp(setting->pchName,"ColorPalette")==0)
	{
		setting->iIntValue = -1;
		gclBasicOptions_ColorPaletteChanged(setting);
	}
	else if (strStartsWith(setting->pchName,"ColorPaletteStateDisable"))
	{
		setting->iIntValue = 0;
		gclBasicOptions_ColorPaletteStateDisablesChanged(setting);
	}
...
void gclBasicOptions_InitColorPalettes(void)
{
	...
	for (i = eaSize(&g_ColorPalettes.eaPalettes) - 1; i >= 0; i--)
	{
		if (IS_HANDLE_ACTIVE(g_ColorPalettes.eaPalettes[i]->BasePalette.hDisplayName))
			iCount++;
	}

	s_iColorPaletteSelection = -1;
	if (iCount >= 2 || isDevelopmentMode() && eaSize(&g_ColorPalettes.eaPalettes) > 0)
	{
		OptionSetting *pSetting = autoSettingsAddComboBox(BASIC_CATEGORY, "ColorPalette", &s_ppchColorPalettes, false, &s_iColorPaletteSelection, gclBasicOptions_ColorPaletteChanged, NULL, gclBasicOptions_ColorPaletteUpdate, gclBasicOptions_Revert, NULL, true);
		gclBasicOptions_ColorPaletteUpdate(pSetting);
		options_HideOption(BASIC_CATEGORY, "ColorPalette", gclBasicOptions_HideColorPaletteOptions);
	}

	for (i = eaSize(&g_ColorPalettes.eaStateNames) - 1; i >= 0; i--)
	{
		if (g_ColorPalettes.eaStateNames[i]->iDisableFlag)
			uiDisables |= 1 << (g_ColorPalettes.eaStateNames[i]->iDisableFlag - 1);
	}
	...
	if (uiDisables)
	{
		for (i = eaSize(&g_ColorPalettes.eaStateNames) - 1; i >= 0; i--)
		{
			if (g_ColorPalettes.eaStateNames[i]->iDisableFlag)
			{
				U32 uBit = 1 << (g_ColorPalettes.eaStateNames[i]->iDisableFlag - 1);
				if (uiDisables & uBit)
				{
					OptionSetting *pSetting;
					char achNameBuffer[256];

					sprintf(achNameBuffer, "ColorPaletteStateDisable_%s", g_ColorPalettes.eaStateNames[i]->pchName);

					pSetting = autoSettingsAddBit(BASIC_CATEGORY, achNameBuffer, &s_iColorPaletteStates, uBit, gclBasicOptions_ColorPaletteStateDisablesChanged, NULL, NULL, gclBasicOptions_Revert, NULL, true);
					options_HideOption(BASIC_CATEGORY, achNameBuffer, gclBasicOptions_HideColorPaletteOptions);

					uiDisables = uiDisables & ~uBit;
				}
			}
		}

		ui_StyleColorPaletteSetDisableFlags(gclBasicOptions_GetColorPaletteDisables);
	}
}
...
void gclBasicOptions_EnterGameplay(void)
{
	...
	s_iColorPaletteStates = ~SAFE_MEMBER3(pEnt, pPlayer, pUI, ubfColorPaletteStateDisables);
	ui_StyleColorPaletteSwitchPriority(SAFE_MEMBER3(pEnt, pPlayer, pUI, pchUserColorPalette), kUIStyleColorPalettePriority_User);
}

void gclBasicOptions_ExitGameplay(void)
{
	ui_StyleColorPaletteSwitchPriority(NULL, kUIStyleColorPalettePriority_User);
}
