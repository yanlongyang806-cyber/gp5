...
void ClientChat_FillInputChannelInput(ChatConfigInputChannel *pInputChannel, const char *pchSystemName, bool bForceSubscribed)
{
	...
	if (!pchSystemName || !stricmp_safe(pchSystemName, LOCAL_CHANNEL_NAME))
	{
		...
		pInputChannel->bPreventItemLinks = false;
	...
	else if (bBuiltIn)
	{
		...
		pInputChannel->bPreventItemLinks = (gConf.bPreventItemLinksInZoneLikeChatChannels && (kType == kChatLogEntryType_LookingForGroup || kType == kChatLogEntryType_Zone));
...
static void ClientChat_SetCurrentChannelByType(ChatLogEntryType eType)
{
	...
	xcase kChatLogEntryType_Trade:
		ClientChat_SetCurrentChannelByName(CHAT_TRADE_SHORTCUT);
	xcase kChatLogEntryType_LookingForGroup:
		ClientChat_SetCurrentChannelByName(CHAT_LFG_SHORTCUT);
...
static void ClientChat_SendMessageInternal(SA_PARAM_OP_VALID const ChatUserInfo *pTo, ChatLogEntryType eType, SA_PARAM_OP_VALID const char *pchChannel, SA_PARAM_OP_VALID const char *pchText)
{
	...
	if (pMsg)
	{
		...
		ChatConfig *pCurrentConfig = ClientChatConfig_GetChatConfig(pEnt);
		...
		{
		case kChatLogEntryType_Zone:
		case kChatLogEntryType_Trade:
		case kChatLogEntryType_LookingForGroup:
		...
		if (bChangeCurrentChannel && !(pCurrentConfig && pCurrentConfig->bDisableStickyChannels))
		{
...
static void ClientChat_SendMessageChannel(ChatConfigInputChannel *pInputChannel, SA_PARAM_OP_VALID const char *msg, SA_PARAM_OP_VALID ChatData *pData)
{
	...
	if( pInputChannel->bPreventItemLinks && pData && eaSize(&pData->eaLinkInfos) )
		notify_NotifySend(NULL, kNotifyType_ChatLinkForbiddenInChannel, TranslateMessageKey("Chat_LinksForbiddenInChannel"), NULL, NULL);
	else
	{
...
void ClientChat_SendTradeChat(Entity *pEnt, const char *pchText) {
	// Channel will be filled in by the server
	ClientChat_SendMessageInternal(NULL, kChatLogEntryType_Trade, NULL, pchText);
}

void ClientChat_SendLFGChat(Entity *pEnt, const char *pchText) {
	// Channel will be filled in by the server
	ClientChat_SendMessageInternal(NULL, kChatLogEntryType_LookingForGroup, NULL, pchText);
}
...
// Get the system name of the current input channel.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(Chat_CurrentChannelAllowsItemLinks);
bool ClientChat_CurrentChannelAllowsItemLinks(void)
{
	ChatConfigInputChannel *pInputChannel = ClientChatConfig_GetCurrentInputChannel();
	return !pInputChannel->bPreventItemLinks;
}
