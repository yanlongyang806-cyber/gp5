...
void gclCutsceneEndOnClient(bool bEndNow)
{
	...
	ui_GameUIShow(UI_GAME_HIDER_CUTSCENE);
...
static Entity* gclCutsceneGetNamedPet(const char *pPetName)
{
	Entity * pEnt = NULL, *pPlayerEnt, *pBackupPet = NULL;

	if(demo_playingBack())
	{
		pPlayerEnt = demo_GetActivePlayer();
	}
	else
	{
	...
	// See if we can find a team. If team find leader, if leader then find named pet there first
	// if none then show pet from leader first
	if(pPlayerEnt)
	{
		S32 i;
		...
		if(pTeam && eaSize(&pTeam->eaMembers) > 0)
		{
			// Get the team leader
			Entity *pLeader = team_GetTeamLeaderAnyPartition(pTeam);
			if(pLeader)
			{
				eaPush(&eaPlayers, pLeader);
			}
			...
			for(i = 0; i < eaSize(&pTeam->eaMembers); ++i)
			{
				...
				if(pTeamEnt && (!pLeader || pTeamEnt->myContainerID != pLeader->myContainerID))
				{
					eaPush(&eaPlayers, pTeamEnt);
				}
			}
		}
		
		if(eaSize(&eaPlayers) < 1)
		{
			// solo or team info missing
			eaPush(&eaPlayers, pPlayerEnt);
		}

		// We have the list of players, see if we can find the named pet on one of them starting with the leader
		for(i = 0; i < eaSize(&eaPlayers); ++i)
		{
			Entity *pCurEnt = eaPlayers[i];
			if(pCurEnt && pCurEnt->pSaved)
			{
				// go through pets
				S32 j;

				for(j = 0; j < eaSize(&pCurEnt->pSaved->ppOwnedContainers); ++j)
				{
					PetRelationship* pPet = pCurEnt->pSaved->ppOwnedContainers[j];
					Entity* ePet = pPet ? SavedPet_GetEntity(PARTITION_CLIENT, pPet) : NULL;

					// No pet or puppet skip it
					if(!ePet || !ePet->pSaved || SavedPet_IsPetAPuppet(pCurEnt, pPet))
					{
						continue;
					}

					if(Entity_IsPetDefNamed(ePet, pPetName))
					{
						// early out destroy the players earray
						eaDestroy(&eaPlayers);
						return ePet;
					}

					// This will be the fallback pet
					if(!pBackupPet)
					{
						pBackupPet = ePet;
					}
		...
		// Use the backup pet we found during our search
		if(pBackupPet)
		{
			pEnt = pBackupPet;
		}
	...
	case CutsceneOffsetType_Contact:
		{
			...
			if (pContactDialog) {
				...
				if (pCameraSourceEnt) {
					...
					pEntity = entFromEntityRefAnyPartition(pContactDialog->headshot.headshotEnt);
				} else {
					// Use specific source
			...
			{
				// We are a contact cutscene, but there is no contact dialog.  That's a failure scenario.
				return false;
			}
			break;
		}
...
static Entity* gclCutsceneGetEntityFromType(CutsceneEntityList *pList)
{
	...
	switch(pList->entityType) {
		...
		xcase CutsceneEntityType_NamedPet:
		{
			pRetEnt = gclCutsceneGetNamedPet(pList->pchPetName);
		}
	}
	return pRetEnt;
}
...
static Entity* gclCutsceneInitClientEntity(CutsceneEntityList *pList)
{
	...
	if(pList->entityType == CutsceneEntityType_Custom) {
		...
		if(pParentEnt) 
		{
			...
			//Copy Costume
			// Ground costumes are always on the player now
			if(pList->charClassType != CharClassTypes_None && pPuppetEnt)
			{
				PlayerCostume *pCostume = costumeEntity_GetActiveSavedCostume(pPuppetEnt);

				// If there isn't costume then we want to view the ground costume
				if(!pCostume)
				{
					// If the effective costume is the ground costume use it, otherwise use the slotted costume
					PlayerCostume *pPlayerCostume = NULL;
					pCostume = costumeEntity_GetEffectiveCostume(pParentEnt);

					if(pParentEnt->pSaved->costumeData.iActiveCostume < eaSize(&pParentEnt->pSaved->costumeData.eaCostumeSlots))
					{
						pPlayerCostume = pParentEnt->pSaved->costumeData.eaCostumeSlots[pParentEnt->pSaved->costumeData.iActiveCostume]->pCostume;
					}

					if(pPlayerCostume && GET_REF(pCostume->hSkeleton) != GET_REF(pPlayerCostume->hSkeleton))
					{
						// Use the slotted one
						pCostume = pPlayerCostume;
					}
				}
				...
				if (pNewCostume)
				{
					...
					if(pPuppetEnt)	// Added if as compiler safe pointer checking was confused
					{
				...
				if(!pCostume && pParentEnt && pParentEnt->pSaved && pParentEnt->pSaved->costumeData.iActiveCostume < eaSize(&pParentEnt->pSaved->costumeData.eaCostumeSlots))
				{
					// Boffs that aren't on a team won't have an effective costume, try to get it now
					if(!pCostume)
					{
						pCostume = pParentEnt->pSaved->costumeData.eaCostumeSlots[pParentEnt->pSaved->costumeData.iActiveCostume]->pCostume;
					}
				}
				if(pCostume)
		...
		{
			// set costume for named pet to this one as a backup the player doesn't have any pets
			if(pList->entityType == CutsceneEntityType_NamedPet)
			{
				REF_HANDLE_COPY(pEnt->costumeRef.hReferencedCostume, pList->hCostume);
			}
	...
	{
		costume_FreeCostumeDisplayData(eaDisplayData[i]);
	}
...
static void gclCutsceneShowSubtitle(SMFBlock *pText, F32 fStart, F32 fEnd, F32 elapsedTime)
{
	...
	smf_ParseAndDisplay(pText, NULL, iWindowWidth*0.05, iWindowHeight - iLetterBox - iTextHeight - iSubtitleOffset, UI_UI2LIB_Z-1, iWindowWidth*0.9, 0, false, false, false, pSubtitleAttribs, fAlpha, NULL, NULL);
}
...
static void gclCutscene_UpdateSubtitles(SMFBlock *pSubtitleBlock, F32 elapsedTime)
{
	...
	for (i = 0; i < eaSize(&pSubtitleBlock->pBlock->ppBlocks); i++)
	{
			...
			&& (elapsedTime > fStart && elapsedTime < fEnd))
		{
			...
			smf_Render(pBlock, s_pAttribs, iX, iY - pBlock->pos.iY, UI_UI2LIB_Z-1, CLAMP(fAlpha * 255, 0, 255), 1.0f, NULL, NULL, NULL);
...
static void gclCutscene_UpdateTransitionText(S32 iWindowWidth, S32 iWindowHeight, S32 iLetterBox)
{
	...
	if ( pBlock && smf_GetTime(pBlock, &fStartTime, &fEndTime) )
	{
		...
		smf_ParseAndDisplay(s_pTransTextBlock, NULL, iX, iY - pBlock->pos.iY, 1, iWindowWidth * 0.7f, 0, false, false, false, s_pTransTextAttribs, CLAMP(fAlpha * 255, 0, 255), NULL, NULL);
	}
}
...
void CutsceneStartOnClient(F32 fStartTime, DoorTransitionType eTransitionType, CutsceneWorldVars *pUpdatedCutsceneVars)
{
	...
	ui_GameUIHide(UI_GAME_HIDER_CUTSCENE);
	if(!demo_playingBack())
	{
		UIGen *pCutsceneRoot = ui_GenFind("Cutscene_Root", kUIGenTypeNone);
