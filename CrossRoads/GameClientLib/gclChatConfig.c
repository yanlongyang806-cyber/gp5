...
// Sets whether chat log entries should be filtered for profanity
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("Chat_SetStickyChannels");
void exprClientChatSetStickyChannels(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity, bool bStickyChannels)
{
	if (ClientChatConfig_UsingEntityConfig())
		ServerCmd_Chat_SetStickyChannels(bStickyChannels);
	else
	{
		NOCONST(ChatConfig) *pConfig = ClientChatConfig_GetEditableConfig();
		if (pConfig)
			pConfig->bDisableStickyChannels = !bStickyChannels;
		GServerCmd_crChatConfig_SetStickyChannels(GLOBALTYPE_CHATRELAY, bStickyChannels);
	}
}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(chat_getstickychannels);
bool gclChatConfig_GetStickyChannels(SA_PARAM_OP_VALID Entity* pEntity)
{
	ChatConfig *pConfig = ClientChatConfig_GetChatConfig(pEntity);
	return !( pConfig && pConfig->bDisableStickyChannels );
}
...
S32 exprClientChatCountInputChannels(SA_PARAM_OP_VALID ChatTabGroupConfig *pWindowSettings, const char *pchTitle, U32 uShowFlags)
{
	...
	if (pTab)
	{
		...
		// Treat private messages as a valid input channel
		if ((uShowFlags & 1) != 0 &&
			(ChatCommon_IsLogEntryVisibleInTab(pTab, kChatLogEntryType_Private, NULL)
				|| ChatCommon_IsLogEntryVisibleInTab(pTab, kChatLogEntryType_Private_Sent, NULL)))
		{
			iUsedChannels++;
		}
	}
...
void exprClientChatGetInputChannels(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID ChatTabGroupConfig *pWindowSettings, const char *pchTitle, U32 uShowFlags)
{
	...
	if (pTab)
	{
		...
		if ((uShowFlags & 1) != 0 &&
			...
			|| ChatCommon_IsLogEntryVisibleInTab(pTab, kChatLogEntryType_Private_Sent, NULL)))
		{
			pchTypeName = ChatCommon_GetMessageTypeName(kChatLogEntryType_Private);
			pchSystemName = PRIVATE_CHANNEL_NAME;

			if (pchSystemName)
			{
				// Add the fake channel
				pChannel = eaGetStruct(peaChannels, parse_ChatConfigInputChannel, iUsedChannels++);
				ClientChat_FillInputChannelInput(pChannel, pchSystemName, false);
			}
		}
