...
char gLocalHost[32] = ORGANIZATION_DOMAIN;
...
void gclFatalErrorfCallback(ErrorMessage* errMsg, void *userdata)
{
	...
	if (!g_isContinuousBuilder && !gfxIsHeadshotServer())
	{
		...
		MessageBox_UTF8(NULL, buffer, "Fatal Error", MB_ICONEXCLAMATION | MB_OK | MB_SYSTEMMODAL);
	}
#endif
...
bool gclCreatePrimaryDevice(HINSTANCE hInstance)
{
	...
	int deviceCount = eaSize(&device_infos);
	...
	params.bStereoEnable = gGCLState.bEnableStereoscopic;

	params.window_title = regGetAppName(); // Haven't loaded any data yet, use the registry app name

	virtualMemoryAnalyzeAndMemlogStats(NULL, "After Startup", NULL);

	// First attempt specified device if it's not Auto (NULL or "")
	gGCLState.pPrimaryDevice->device = NULL;
	if (!rdrIsDeviceTypeAuto(params.device_type))
	{
        loadupdate_printf("Trying (%s)...", params.device_type);
		gGCLState.pPrimaryDevice->device = rdrCreateDevice(&params, hInstance, THREADINDEX_RENDER);
	}
	// Failover: If the device indicated by default or saved settings fails, or is on Auto (NULL or ""),
	// attempt every other device in order.
	if (!gGCLState.pPrimaryDevice->device)
	{
		params.display.preferred_adapter = 0;
		do
		{
			params.device_type = device_infos[params.display.preferred_adapter]->type;
			params.bNotifyUserOnCreateFail = false;
			loadupdate_printf("Trying (%s)...", params.device_type);
			gGCLState.pPrimaryDevice->device = rdrCreateDevice(&params, hInstance, THREADINDEX_RENDER);
			if (!gGCLState.pPrimaryDevice->device)
				++params.display.preferred_adapter;
		}
		while (!gGCLState.pPrimaryDevice->device && params.display.preferred_adapter < deviceCount);
	}
	loadend_printf(" done.");

	virtualMemoryAnalyzeAndMemlogStats(NULL, "After Device Init", NULL);
...
LRESULT CALLBACK DlgSystemSpecsWarning (HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowText_UTF8(hDlg, TranslateMessageKeySafe("GameClientLib.FailedSystemSpecsTitle"));

		SetWindowText_UTF8(GetDlgItem(hDlg, IDC_MESSAGE), g_systemspecswarning_data->message);

		SetWindowText_UTF8(GetDlgItem(hDlg, IDC_PROMPT), TranslateMessageKeySafe("GameClientLib.ContinueAnyway"));

		SetWindowText_UTF8(GetDlgItem(hDlg, ID_YES_LOWERED), TranslateMessageKeySafe("GameClientLib.YesLowered"));

		SetWindowText_UTF8(GetDlgItem(hDlg, ID_YES_DEFAULT), TranslateMessageKeySafe("GameClientLib.YesDefault"));

		SetWindowText_UTF8(GetDlgItem(hDlg, IDC_LOWERED_HELP), TranslateMessageKeySafe("GameClientLib.LoweredHelp"));

		SetWindowText_UTF8(GetDlgItem(hDlg, IDC_DEFAULT_HELP), TranslateMessageKeySafe("GameClientLib.DefaultHelp"));

		SetWindowText_UTF8(GetDlgItem(hDlg, IDC_QUIT_BUGGING_ME), TranslateMessageKeySafe("GameClientLib.QuitBuggingMe"));

		SetWindowText_UTF8(GetDlgItem(hDlg, IDCANCEL), TranslateMessageKeySafe("GameClientLib.Cancel"));
...
void gclSvNotifyJoin(S32 showFirstConnectTutorial, S32 isOpenMic)
{
	notify_NotifySend(NULL, kNotifyType_VoiceChannelJoin, TranslateMessageKey("VoiceChannel.Join"), NULL, NULL);

	if (showFirstConnectTutorial)
	{
		if (isOpenMic)
		{
			notify_NotifySend(NULL, kNotifyType_Tip_General, TranslateMessageKeySafe("Voice.FirstConnectTutorialOpenMic"), NULL, NULL);
		}
		else
		{
			S32 iPush2Talk = gclKeyBindExprFindKeyBindIndexByCommand("+svPushToTalk");
			const char *pcPush2Talk = gclKeyBindExprGetPrimaryKeyBindDisplayNameByIndex(iPush2Talk, false);

			if (strlen(pcPush2Talk))
			{
				char *pchTemp = NULL;
				estrStackCreate(&pchTemp);
				FormatMessageKey(&pchTemp, "Voice.FirstConnectTutorialShowKey", STRFMT_STRING("KeyBind", pcPush2Talk), STRFMT_END);
				notify_NotifySend(NULL, kNotifyType_Tip_General, allocAddString(pchTemp), NULL, NULL);
				estrDestroy(&pchTemp);
			}
			else
			{
				notify_NotifySend(NULL, kNotifyType_Tip_General, TranslateMessageKeySafe("Voice.FirstConnectTutorialNeedsKey"), NULL, NULL);
			}
		}
	}
}
		...
		bool bCostumeV0)
{
	...
	pWLCostume = costumeGenerate_CreateWLCostume(pHandle->pPlayerCostume, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "Entity.", eContainerType, iContainerID, true, &eaSubCostumes);
	if (pWLCostume)
	{
	...
	for (i=eaSize(&sppTextureRequestHandles) - 1; i >= 0; i--)
	{
		...
		if (pHandle->pHeader->tex_is_loaded & RAW_DATA_BITMASK)
		{
			...
			if (rawInfo->tex_format == RTEX_BGRA_U8)
			{
				...
				{
					PngPixel4 *pPixel = (PngPixel4*) (rawInfo->texture_data + pixelNum * sizeof(PngPixel4));
					U8 temp = pPixel->r;
					pPixel->r = pPixel->b;
					pPixel->b = temp;		
				}

				bResult = WritePNG_File(rawInfo->texture_data, pHandle->pHeader->width, pHandle->pHeader->height, rawInfo->width, 4, pHandle->pFileName);
			}
			else if (rawInfo->tex_format == RTEX_BGR_U8)
			{
				...
				for (pixelNum = 0; pixelNum < rawInfo->width * rawInfo->height; pixelNum++)
				{
					PngPixel3 *pPixel = (PngPixel3*) (rawInfo->texture_data + pixelNum * sizeof(PngPixel3));
					U8 temp = pPixel->r;
					pPixel->r = pPixel->b;
					pPixel->b = temp;		
				}

				bResult = WritePNG_File(rawInfo->texture_data, pHandle->pHeader->width, pHandle->pHeader->height, rawInfo->width, 3, pHandle->pFileName);
			}
			else
			{
				bResult = false;
...
void OVERRIDE_LATELINK_GetCommandsForGenericServerMonitoringPage(GenericServerInfoForHttpXpath *pInfo)
{
	...
	{
		estrPrintf(&pInfo->pCommand1, "BeginExpressionServerMonitoring $NORETURN $NOCONFIRM $COMMANDNAME(Begin monitoring expression funcs) $REDIRECT_AND_EXEC(/viewxpath?xpath=Client[0].globObj.Exprfuncs)");
	}
}

void OVERRIDE_LATELINK_RequestAdditionalExprFuncs(void)
{
	ServerCmd_RequestExprFuncsForServerMonitoring();
}

AUTO_COMMAND ACMD_CLIENTCMD;
void HereAreExprFuncs(ExpressionFuncForServerMonitorList *pList)
{
	AddListOfExpressionFuncsFromOtherLocation(pList);
}

void RequestOpenMonitoringURL(char *pURL)
{
	NetLink *pLink = gclGetDebugLinkToLauncher(false);
	if (pLink)
	{
		Packet *pPak = pktCreate(pLink, TO_LAUNCHER_CLIENT_REQUESTS_OPEN_MONITORING_URL);
		pktSendString(pPak, pURL);
		pktSend(&pPak);
	}
}

//open a webpage viewing all expression funcs
AUTO_COMMAND;
char *ExprFuncs(void)
{
	if (GSM_StateContains("gclGameplay"))
	{
		BeginExpressionServerMonitoring();
		RequestOpenMonitoringURL("http://localhost/viewxpath?xpath=client[0].globObj.Exprfuncs");
		return "succeeded";
	}
	else
	{
		return "You must be in gclGameplay, connected to a gameserver";
	}
}