...
// Returns whether or not we should show the voice UI
// Very Important Note : this won't be true unless
//		(a) it's been enabled by the chat server AND
//		(b) the game client is connected to a chat relay (which also requires them to have at some point connected to a game server)
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(VoiceShowUI);
int exprFuncShowVoiceUI(void)
{
	return g_VoiceState.enabled_region;
}
