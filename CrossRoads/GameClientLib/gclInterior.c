...
static MicroTransactionUIProduct **s_eaRemoveProductList;
static MicroTransactionUIProduct **s_eaAddProductList;
static U32 s_uSelectedProductListLastUpdateFrame;
...
{
	eaClearStruct(&s_eaAddProductList, parse_MicroTransactionUIProduct);
	eaClearFast(&s_eaRemoveProductList);
	...
	if (pInteriorSelect)
	{
		...
		if (eaSize(&pInteriorSelect->eaFullProductList))
		{
			o = eaSize(&pInteriorSelect->eaFullProductList);
			n = eaSize(&s_eaSelectedProductList);

			for (j = 0; j < o; j++)
			{
				uID = pInteriorSelect->eaFullProductList[j]->uID;
				for (i = 0; i < n; i++)
				{
					if (s_eaSelectedProductList[i]->uID == uID)
						break;
				}
				if (i >= n)
					eaPush(&s_eaAddProductList, StructClone(parse_MicroTransactionUIProduct, pInteriorSelect->eaFullProductList[j]));
			}

			for (i = 0; i < n; i++)
			{
				uID = s_eaSelectedProductList[i]->uID;
				for (j = 0; j < o; j++)
				{
					if (pInteriorSelect->eaFullProductList[j]->uID == uID)
						break;
				}
				if (j >= o)
					eaPush(&s_eaRemoveProductList, s_eaSelectedProductList[i]);
				else if (StructCompare(parse_MicroTransactionUIProduct, pInteriorSelect->eaFullProductList[j], s_eaSelectedProductList[i], 0, 0, 0) != 0)
					// This is safe as long as struct pointers don't change
					StructCopyAll(parse_MicroTransactionUIProduct, pInteriorSelect->eaFullProductList[j], s_eaSelectedProductList[i]);
			}
		...
		{
			n = eaSize(&s_eaSelectedProductList);
			uID = pInteriorSelect->pProduct->uID;
			for (i = 0; i < n; i++)
			{
				if (s_eaSelectedProductList[i]->uID != uID)
					eaPush(&s_eaRemoveProductList, s_eaSelectedProductList[i]);
		...
		else if (eaSize(&s_eaSelectedProductList) > 0)
		{
			eaPushEArray(&s_eaRemoveProductList, &s_eaSelectedProductList);
		}
	}
	else if (eaSize(&s_eaSelectedProductList) > 0)
	{
...
static void
InteriorSelectRow_UpdateProductList(void)
{
	if (s_uSelectedProductListLastUpdateFrame != g_ui_State.uiFrameCount)
	{
		S32 i, n;

		n = eaSize(&s_eaRemoveProductList);
		for (i = 0; i < n; i++)
			eaFindAndRemove(&s_eaSelectedProductList, s_eaRemoveProductList[i]);
		eaClearStruct(&s_eaRemoveProductList, parse_MicroTransactionUIProduct);

		n = eaSize(&s_eaAddProductList);
		for (i = 0; i < n; i++)
			eaPush(&s_eaSelectedProductList, s_eaAddProductList[i]);
		eaClearFast(&s_eaAddProductList);

		s_uSelectedProductListLastUpdateFrame = g_ui_State.uiFrameCount;
	}
}
...
{
	InteriorSelectRow_UpdateProductList();
