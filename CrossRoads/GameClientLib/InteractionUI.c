...
{
	Entity *pEnt = NULL;
	return target_SelectUnderMouse(entActivePlayerPtr(),0,0,NULL,false,false,false,false);
}
...
{		
	...
	if(e)
	{
		...
		if (!e->pPlayer->InteractStatus.bLockIntoCursorOverrides)
		{
			...
			target = target_SelectUnderMouse(e, 0, kTargetType_Player, NULL, false, false, true, false);		
			pTargetPlayer = target_SelectUnderMouse(e, kTargetType_Player, 0, NULL, false, false, false, false);
...
bool interaction_ExprInteractWithTargetUnderMouse(void)
{
	...
	if (pPlayerEnt)
	{
		Entity* pTargetEnt = target_SelectUnderMouse(pPlayerEnt, 0, kTargetType_Self, NULL, true, false, true, false);
		if (pTargetEnt)
		{
