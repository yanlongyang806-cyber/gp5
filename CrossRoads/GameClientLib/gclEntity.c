...
const char *gclEntGetDistanceFromPointAsString(SA_PARAM_OP_VALID Entity *pPlayerEnt, F32 fX, F32 fY, F32 fZ)
{
	Vec3 vPoint;
...
{
	ExprFuncTable* funcTable = exprContextCreateFunctionTable("EntityFSMGroups");
...
const char *entGetPuppetName(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity)
{
...
SA_RET_OP_VALID Entity *entExprGetPrimaryPet(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity)
{
...
SA_RET_OP_VALID Entity *entExprGetRider(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity)
{
...
bool gclEntIsPrimaryMission(SA_PARAM_OP_VALID Entity *pEntity, const char *pcMission)
{
...
bool teamExpr_IsPrimaryMission(SA_PARAM_OP_VALID Entity *pEntity, const char *pcMission)
{
...
bool teamExpr_IsPrimaryMissionOrSubmission(SA_PARAM_OP_VALID Entity *pEntity, const char *pcMission)
{
	if(pcMission && pcMission[0] != 0)
	{
...
bool Mission_CanChangePrimaryMission(SA_PARAM_OP_VALID Entity *pEntity)
{
...
bool MissionExpr_IsMissionHidden(SA_PARAM_OP_VALID Entity *pEntity, const char *pcMission)
{
	if(pcMission && pcMission[0] != 0)
	{
...
{
	return CharClassCategorySet_find(pchSet);
}
...
{
	CharClassCategorySet *pSet = CharClassCategorySet_find(pchSet);
	if (pSet)
	{
...
WLCostume* gclEntity_CreateWLCostumeFromPlayerCostume(Entity* pEnt, PlayerCostume* pCostume, bool bForceUpdate)
{
	...
	if (!pWLCostume || bForceUpdate) {
		...
		AllegianceDef* pAllegiance = pEnt ? GET_REF(pEnt->hAllegiance) : NULL;
		AllegianceDef* pSubAllegiance = pEnt ? GET_REF(pEnt->hSubAllegiance) : NULL;
...
void gclGenExprGetPlayerMacroList(SA_PARAM_OP_VALID UIGen* pGen)
{
	Entity* pEnt = entActivePlayerPtr();
...
void gclExprGenGetMacroEditCommandList(SA_PARAM_OP_VALID UIGen* pGen)
{
	if (s_pMacroEditData)
	{
...
bool gclEntExprAllegianceCanTranswarp(SA_PARAM_OP_VALID Entity *pEnt)
{
	...
	return allegiance_CanPlayerUseWarpAllegiance(pEnt, pAllegiance);
}
