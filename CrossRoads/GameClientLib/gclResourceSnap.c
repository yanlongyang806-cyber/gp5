...
#define RESOURCE_SNAP_VERSION 12
...
bool gclResourceSnapTakePhotos( ResourceSnapDesc** eaResources )
{
	...
	PERFINFO_AUTO_START_FUNC();
	{
		...
		state.timestamps = stashTableCreateWithStringKeys( 64, StashDeepCopyKeys_NeverRelease | StashCaseInsensitive );
...
void gclSnapPhotoFinishedCB( ResourceSnapState* state, U8* cap_data, int w, int h, char** ignored )
{
	...
	{
		RdrTexFormat texFormat = state->curDesc->objectIsTopDownView ? RTEX_DXT5 : RTEX_DXT1;
		size_t compressed_size = imgByteCount(RTEX_2D, texFormat, w, h, 1, 1);
		size_t fileSize = compressed_size + 4 + sizeof( DDSURFACEDESC2 );
		...
		//Compress Data
        nvdxtCompress( cap_data, fileIt, w, h, texFormat, 1, 0 );
