...
void cmdDisableCursorMode(bool bDisable)
{
	if(bDisable)
	{
		...
		{
			//Entering action combat should also exit any active contacts.  Not certain this is 
			//the best place for this, but it's better than in the command alias, where it was before.
			Entity* pEnt = entActivePlayerPtr();
...
{
	Entity* e = entActivePlayerPtr();
	Entity* target = target_SelectUnderMouse(e, 0, 0, NULL, false, true, true, false);
...
void gclPlayerControl_UnSuspendMouseLook()
{
	gPlayerControlData.bSuspendForcedMouselook = false;
}