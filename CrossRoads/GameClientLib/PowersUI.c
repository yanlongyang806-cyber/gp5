...
static void PowerUI_CreateCombatStateListNodes(ExprContext *pContext, UIGen *pGen, Entity *pEntity);
...
// fills in the ErrorDetailsf for a bad PowersUIState 
static void PowersUI_GenerateErrorDetailsf(ExprContext *pContext, UIGen *pGen, const char* pchFunction)
{
	const char *pchGen = pGen ? pGen->pchName : NULL;
	char *estrExpression = NULL;

	if (pContext->curExpr)
	{
		exprGetCompleteStringEstr(pContext->curExpr, &estrExpression);
		if (estrLength(&estrExpression) > 31)
		{
			estrSetSize(&estrExpression, 31);
		}
	}

	ErrorDetailsf("UIGen %s, Function %s, Expression %s", 
					pchGen ? pchGen : "None", 
					pchFunction ? pchFunction : "None",
					estrExpression ? estrExpression : "None");
...
SA_RET_OP_VALID PowersUIState *PowersUI_GetPowersUIStateEx(ExprContext *pContext, UIGen *pGen, bool bCheckTimestamp, const char* pchFunction)
{
	...
	if (!pPowersUI)
	{
		if (!s_bErrorDisplayed)
		{
			...
			PowersUI_GenerateErrorDetailsf(pContext, pGen, pchFunction);
			Errorf("No PowersUI Data. Either you forget to call PowersUI_Init, you opened the editor with the powers UI open.");
		}
	}
	else if (bCheckTimestamp && pPowersUI->uiFrameLastUpdate < pGen->uiFrameLastUpdate)
	{
		...
		if (!s_bErrorDisplayed)
		{
			...
			PowersUI_GenerateErrorDetailsf(pContext, pGen, pchFunction);
			Errorf("Stale PowersUI Data. You didn't update with PowersUI_Update or PowersUI_UpdateEx in the PointerUpdate phase.");
		}
	}
...
// ------------------------------------------------------------------------------------------------------------------
static void PowersUI_DestroyPowerStateStashList(PowerListNode **eaNodes)
{
	eaDestroy(&eaNodes);
}
...
void PowersUI_Deinit(ExprContext *pContext, SA_PARAM_OP_VALID UIGen *pGen)
{
	...
	if (stashAddressRemovePointer(s_stPowersUIStates, pGen, &pPowersUI))
	{
		...
		if (pPowersUI->statePowerListNodesHash)
		{
			stashTableDestroyEx(pPowersUI->statePowerListNodesHash, NULL, PowersUI_DestroyPowerStateStashList);
			pPowersUI->statePowerListNodesHash = NULL;
		}
	}
...
bool PowersUI_AddPowersInPowerGroup(PowersUIState *pPowersUI, Entity *pEnt, SA_PARAM_OP_VALID Entity *pEntCompare, PowerTreeDef *pTreeDef, PTGroupDef *pGroupDef)
{
	...
	for(i = 0; i < eaSize(&pGroupDef->ppNodes); i++)
	{
			...
			(pPowersUI->bShowUnavailable && !(bIsAvailable || bIsOwned))))
		{
			...
			if (!pPowersUI->bTalentTreesUsePowersPurpose && pTreeDef->bIsTalentTree) // Talent trees ignore the power purpose
			...
			pListNode->bIsConditionalNode = powertree_IsConditionalNode(pNodeDef);
			pListNode->bCanAccessConditionalNode = bIsOwned || powertree_CanAccessConditionalNode(pEnt, pNodeDef);
...
static PowerListNode *PowersUI_FindPowerListNodeByTalentTreePos(SA_PARAM_NN_VALID Entity *pEnt, 
																SA_PARAM_NN_VALID PowersUIState *pPowersUI, 
																S8 iRow, S8 iColumn, 
																SA_PARAM_NN_STR const char *pchTalentTreeName)
{
	S32 i, j;
	// if we're trying to find a node that has buying requirements, 
	// if we can't buy any of them, save the first one we come across
	PowerListNode *pNonPurchasedPowerListNode = NULL;
	...
	for (i = 0; i < eaSize(&pPowersUI->eaPurposeListNodes); i++)
	{
		...
		if (stricmp(pPowersUI->eaPurposeListNodes[i]->pchName, pchTalentTreeName) == 0)
		{
			...
			{
				PTNodeDef *pNodeDef = GET_REF(pPowersUI->eaPurposeListNodes[i]->eaPowerListNodes[j]->hNodeDef);
				if (pNodeDef && pNodeDef->iUIGridRow == iRow && pNodeDef->iUIGridColumn == iColumn)
				{
					// there is no node stack here, return this node
					if (pPowersUI->eaPurposeListNodes[i]->eaPowerListNodes[j]->pNode 
						|| !powertree_IsConditionalNode(pNodeDef))
					{
						return pPowersUI->eaPurposeListNodes[i]->eaPowerListNodes[j];
					}
					else  
					{
						// evaluate if we can buy this node
						if (!pNonPurchasedPowerListNode)
						{
							pNonPurchasedPowerListNode = pPowersUI->eaPurposeListNodes[i]->eaPowerListNodes[j];
						}

						if (powertree_CanAccessConditionalNode(pEnt, pNodeDef))
						{
							return pPowersUI->eaPurposeListNodes[i]->eaPowerListNodes[j];
						}
					}
	...
	return pNonPurchasedPowerListNode;
}
	...
	if (iTotalNodeCount > 0)
	{
		...
		for (i = iLowY; i <= iHighY; i++)
		{
			for (j = iLowX; j <= iHighX; j++)
			{
				...
				pTalentTreeNode->pPowerListNode = PowersUI_FindPowerListNodeByTalentTreePos(pEnt, pPowersUI, i, j, pTalentTree->pchTreeName);
...
void PowersUI_UpdateEx(ExprContext *pContext, SA_PARAM_OP_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Entity *pEntCompare, bool bAutoExpand)
{
	...
	if (pEnt)
	{
		...
		// create all the associated combat power state list nodes
		// 
		PowerUI_CreateCombatStateListNodes(pContext, pGen, pEnt);
...
// This is the list of powers within a purpose. These are the powers themselves.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME("PowersUI_GetPowerListEx");
void PowersUI_GetPowerListEx(ExprContext *pContext, SA_PARAM_OP_VALID UIGen *pGen, 
							SA_PARAM_NN_VALID PowersUIPurposeNode *pPurposeNode, 
							ACMD_EXPR_ENUM(PTNodeUICategory) const char *pchPTNodeUICategories)
{
	if (pGen)
	{
		S32 *eaiCategories = NULL;
		PowerListNode*** peaPowerListNodes = (PowerListNode***)ui_GenGetManagedList(pGen, parse_PowerListNode);
		int iCount = 0;

		StaticDefineIntParseStringForInts(PTNodeUICategoryEnum, pchPTNodeUICategories, &eaiCategories, NULL);

		eaClear(peaPowerListNodes);

		if (eaiSize(&eaiCategories) > 0)
		{
			FOR_EACH_IN_EARRAY_FORWARDS(pPurposeNode->eaPowerListNodes, PowerListNode, pPowerListNode)
			{
				PTNodeDef *pNode = GET_REF(pPowerListNode->hNodeDef);
				if (pNode && eaiFind(&eaiCategories, pNode->eUICategory) >= 0)
				{
					eaPush(peaPowerListNodes, pPowerListNode);
				}
			}
			FOR_EACH_END
		...
		ui_GenSetManagedList(pGen, peaPowerListNodes, parse_PowerListNode, false);
	}
}
...
void PowersUI_SetFiltersEx(ExprContext *pContext, SA_PARAM_OP_VALID UIGen *pGen,
						   ...
						   bool bTalentTreesUsePowersPurpose)
{
	PowersUIState *pPowersUI = PowersUI_GetPowersUIState(pContext, pGen);
	if (pPowersUI)
	{
		...
		pPowersUI->bTalentTreesUsePowersPurpose = bTalentTreesUsePowersPurpose;
	}
}
...
void PowersUI_SetFilters(ExprContext *pContext, SA_PARAM_OP_VALID UIGen *pGen, bool bShowOwned, bool bShowAvailable, bool bShowUnavailable)
{
	...
	if (pPowersUI)
	{
		PowersUI_SetFiltersEx(pContext, pGen, bShowOwned, bShowAvailable, bShowUnavailable, 
								pPowersUI->bFilterOwnedByComparisonEntity, 
								pPowersUI->bAddAvailableForComparisonEntity,
								pPowersUI->bTalentTreesUsePowersPurpose);
	}
}
									  ...
									  SA_PARAM_OP_VALID PowerListNode *pNode)
{
	...
	if (SAFE_MEMBER(pEntity, pChar) && pNode)
	{
		...
		if (pPower)
		{
			...
			power_AutoDesc(entGetPartitionIdx(pEntity), langGetCurrent(), pPower, pEntity->pChar, &s_pchDescription, pAutoDescPower, NULL, NULL, NULL, false, 0, entGetPowerAutoDescDetail(pEntity,false), pExtract,NULL);
		}
		else if (pPowerDef)
		{
			// NNO HACK: Only passing &s_pchDescription here because NNO hack breaks without it
			powerdef_AutoDesc(entGetPartitionIdx(pEntity), langGetCurrent(), pPowerDef, &s_pchDescription, pAutoDescPower, NULL, NULL, NULL,
...
#define FormatRangedVar(varname) \
	...
	if (pEntity && pNode)
	{
		...
		{
			// NNO HACK: Only passing &s_pchDescription here because NNO hack breaks without it
			power_AutoDesc(entGetPartitionIdx(pEntity), langGetCurrent(), pPower, pEntity->pChar, &s_pchDescription, pAutoDescPower, NULL, NULL, NULL, false, 0, entGetPowerAutoDescDetail(pEntity,false), pExtract, pchPowerAutoDescMessageKey);
		}
		else if (pPowerDef)
		{
			// NNO HACK: Only passing &s_pchDescription here because NNO hack breaks without it
...
// given a powerStaeSwitching def and a powerDef, create PowerListNodes for each of the states
// and then stash the list of PowerListNodes based on the POwerDef name
static void PowerUI_CreateCombatStatePowerListNodesForPowerDef(PowersUIState *pPowersUI, PowerListNode *pListNode, CombatPowerStateSwitchingDef* pPowerStateDef, PowerDef *pPowerDef)
{
	CombatPowerStatePowerSet* pPowerStateSet = NULL;
	
	if (!pPowerDef)
		return;

	// see if we have a powerSet for this powerDef
	pPowerStateSet = CombatPowerStateSwitching_FindPowerStateSetDef(pPowerStateDef, pPowerDef);
	if (pPowerStateSet)
	{
		PowerListNode **eaNewPowerListNodes = NULL;

		if (pPowersUI->statePowerListNodesHash)
		{
			if (stashFindPointer(pPowersUI->statePowerListNodesHash, pPowerDef->pchName, (void**)&eaNewPowerListNodes))
			{
				FOR_EACH_IN_EARRAY_FORWARDS(eaNewPowerListNodes, PowerListNode, pNode)
				{
					pNode->iRank = pListNode->iRank;
					pNode->bIsOwned = pListNode->bIsOwned;
					pNode->bCanIncrement = pListNode->bCanIncrement;
					pNode->bIsAvailable = pListNode->bIsAvailable;
					pNode->bIsAvailableForFakeEnt = pListNode->bIsAvailableForFakeEnt;
					pNode->bIsTraining = pListNode->bIsTraining;
				}
				FOR_EACH_END
		...
		// we found a valid set for this powerDef
		// go through all the powers in this set and create a PowerListNode for each
		FOR_EACH_IN_EARRAY(pPowerStateSet->eaPowers, CombatPowerStatePower, pCombatStateDef)
		{
			PowerDef *pStatePowerDef = GET_REF(pCombatStateDef->hPowerDef);
			if (pStatePowerDef)
			{
				PowerListNode* pNewPowerListNode = NULL;

				pNewPowerListNode = StructClone(parse_PowerListNode, pListNode);
				
				if (pNewPowerListNode)
				{
					REF_HANDLE_REMOVE(pNewPowerListNode->hPowerDef);

					REF_HANDLE_SET_FROM_STRING(g_hPowerDefDict, pStatePowerDef->pchName, pNewPowerListNode->hPowerDef);
					if (pStatePowerDef->pchIconName)
					{
						pNewPowerListNode->pchPowerIcon = allocAddString(pStatePowerDef->pchIconName);
					}
					// do we need to get the power?
					pNewPowerListNode->pPower = NULL;

					eaPush(&pPowersUI->eaCombatPowerStatePowerListNodes, pNewPowerListNode);
					eaPush(&eaNewPowerListNodes, pNewPowerListNode);
				}
			}
		}
		FOR_EACH_END

		// we haven't created the hash yet, create it now
		if (!pPowersUI->statePowerListNodesHash)
		{
			pPowersUI->statePowerListNodesHash = stashTableCreateWithStringKeys(64, StashDefault);
		}

		if (!stashAddPointer(pPowersUI->statePowerListNodesHash, pPowerDef->pchName, eaNewPowerListNodes, false))
		{
			// we already have this?
			eaDestroy(&eaNewPowerListNodes);
		}
	}
}

// Creates a list of PowerListNode from the PowersUIState's eaPurposeListNodes for powers that have a combat switched state
// puts the PowerListNode on individual lists in a hash by the associated PowerDef's name
static void PowerUI_CreateCombatStateListNodes(ExprContext *pContext, UIGen *pGen, Entity *pEntity)
{
	PowersUIState *pPowersUI = PowersUI_GetUpdatedPowersUIState(pContext, pGen);
	CombatPowerStateSwitchingDef* pPowerStateDef = NULL;
	CharacterClass *pClass = NULL;


	if (!pPowersUI || !pEntity || !pEntity->pChar)
		return;

	pClass = GET_REF(pEntity->pChar->hClass);
	if (!pClass)
		return;

	pPowerStateDef = CombatPowerStateSwitching_GetDefByName(pClass->pchCombatPowerStateSwitchingDef);
	if (!pPowerStateDef || pPowerStateDef->bHideFromPowerTreeUI)
		return;
		

	FOR_EACH_IN_EARRAY_FORWARDS(pPowersUI->eaPurposeListNodes, PowersUIPurposeNode, pPowersPurpose)
	{
		FOR_EACH_IN_EARRAY_FORWARDS(pPowersPurpose->eaPowerListNodes, PowerListNode, pListNode)
		{
			CombatPowerStatePowerSet* pPowerStateSet = NULL;
			PTNodeDef *pNodeDef = GET_REF(pListNode->hNodeDef);
			if (pNodeDef)
			{
				FOR_EACH_IN_EARRAY_FORWARDS(pNodeDef->ppRanks, PTNodeRankDef, pRanks)
				{
					PowerDef *pPowerDef = GET_REF(pRanks->hPowerDef);
					if (pPowerDef)
						PowerUI_CreateCombatStatePowerListNodesForPowerDef(pPowersUI, pListNode, pPowerStateDef, pPowerDef);
				}
				FOR_EACH_END
...
// Given a PowerListNode and the rank, return the Nth combat power state PowerDef associated with it
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(PowerUI_GetCombatStatePowerDefFromPowerListNode);
SA_RET_OP_VALID PowerDef* PowerUI_GetCombatStatePowerDefFromPowerListNode(ExprContext *pContext, 
																			SA_PARAM_OP_VALID UIGen *pGen, 
																			SA_PARAM_OP_VALID PowerListNode *pPowerListNode, 
																			S32 iRank, S32 iIndex)
{
	PowersUIState *pPowersUI = PowersUI_GetUpdatedPowersUIState(pContext, pGen);
	if (pPowerListNode && pPowersUI && pPowersUI->statePowerListNodesHash)
	{
		PTNodeDef *pNodeDef = GET_REF(pPowerListNode->hNodeDef);
		PTNodeRankDef *pRankDef = pNodeDef ? eaGet(&pNodeDef->ppRanks, iRank) : NULL;

		if (pRankDef)
		{
			PowerDef *pRankPowerDef = GET_REF(pRankDef->hPowerDef);

			if (pRankPowerDef)
			{
				PowerListNode **eaNewPowerListNodes = NULL;

				if (stashFindPointer(pPowersUI->statePowerListNodesHash, pRankPowerDef->pchName, (void**)&eaNewPowerListNodes))
				{
					PowerListNode *pStateListNode = eaGet(&eaNewPowerListNodes, iIndex);
					if (pStateListNode)
					{
						return GET_REF(pStateListNode->hPowerDef);
					}
				}
			}
		}
...
// UIGen Model function
// given a PowerListNode, populates a UIGen with a list of PowerListNode's that are powers associated
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(PowersUI_GetPowerStateSetPowersForPowerListNode);
void exprPowersUI_GetPowerStateSetPowersForPowerListNode(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, 
															SA_PARAM_OP_VALID Entity *pEntity, 
															SA_PARAM_OP_VALID PowerListNode *pListNode)
{
	PowersUIState *pPowersUI = PowersUI_GetUpdatedPowersUIState(pContext, pGen);
	PowerListNode*** peaGenData = ui_GenGetManagedListSafe(pGen, PowerListNode);
	
	eaClear(peaGenData);

	if (pListNode && pPowersUI && pPowersUI->statePowerListNodesHash)
	{
		...
		if (pPowerDef)
		{
			...
			if (stashFindPointer(pPowersUI->statePowerListNodesHash, pPowerDef->pchName, (void**)&eaNewPowerListNodes))
			{
				eaPushEArray(peaGenData, &eaNewPowerListNodes);
			}
		}
	}

	ui_GenSetManagedListSafe(pGen, peaGenData, PowerListNode, false);
}
