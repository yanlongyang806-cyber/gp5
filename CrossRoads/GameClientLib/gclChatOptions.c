...
// Option: StickyChannels
static bool s_bStickyChannels = true; // Current value
bool gclChatConfig_GetStickyChannels(SA_PARAM_OP_VALID Entity* pEntity);
void exprClientChatSetStickyChannels(ExprContext *pContext, SA_PARAM_OP_VALID Entity *pEntity, bool bStickyChannels);

static void gclChatOptions_StickyChannelsChanged(OptionSetting *pSetting)
{
	s_bStickyChannels = !!pSetting->iIntValue;
	exprClientChatSetStickyChannels(NULL, entActivePlayerPtr(), pSetting->iIntValue);
}

static void gclChatOptions_StickyChannelsCommitted(OptionSetting *pSetting)
{
	s_bStickyChannels = !!pSetting->iIntValue;
	exprClientChatSetStickyChannels(NULL, entActivePlayerPtr(), pSetting->iIntValue);
}

static void gclChatOptions_StickyChannelsUpdate(OptionSetting *pSetting)
{
	pSetting->iIntValue = !!s_bStickyChannels;
}

static void gclChatOptions_StickyChannelsRevert(OptionSetting *pSetting)
{
	ChatConfigDefaults * pConfigDefaults = ChatCommon_GetConfigDefaults(ChatCommon_GetChatConfigSourceForEntity(entActivePlayerPtr()));
	pSetting->iIntValue = (pConfigDefaults ? !pConfigDefaults->bDisableStickyChannels : true); // Default
	gclChatOptions_ShowChannelNamesCommitted(pSetting);
}
...
static void setupChatOptions(void) 
{
	...
	s_bStickyChannels = gclChatConfig_GetStickyChannels(pEnt);
	OptionSettingAddBool(CHAT_CATEGORY, "ChatStickyChannels", s_bStickyChannels, gclChatOptions_StickyChannelsChanged, gclChatOptions_StickyChannelsCommitted, gclChatOptions_StickyChannelsUpdate, gclChatOptions_StickyChannelsRevert, NULL, NULL);
