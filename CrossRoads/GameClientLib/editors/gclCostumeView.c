...
static void costumeView_DeInitViewCostume(CostumeViewCostume *pViewCostume);
		...
		if(pPart)
		{
			...
			for(i=eaSize(&pStance->eaParams)-1; i>=0; i--)
			{
				...
				if (!s_pContext)
				{
					...
					exprContextSetFuncTable(s_pContext, exprContextCreateFunctionTable("CostumeView_StanceParams"));
				}
				pParam = eaGetStruct(&pGraphics->pWeaponFxParams->eaDefineParams, parse_DynDefineParam, iParamIndex++);
...
void costumeView_ReinitDrawSkeleton(CostumeViewCostume* pGraphics)
{
	...
	DynAnimChartStack	**swapChartStacks			= NULL;
	DynAnimGraphUpdater	**swapAGUpdater				= NULL;
	DynAnimChartStack	 *swapMovementChartStack	= NULL;
	...
	if (!bKeepSkeleton && pGraphics->pSkel)
	{
		if (bKeepOldSequencers)
		{
			...
			{
				swapChartStacks				= pGraphics->pSkel->eaAnimChartStacks;
				swapAGUpdater				= pGraphics->pSkel->eaAGUpdaterMutable;
				swapMovementChartStack		= pGraphics->pSkel->movement.pChartStack;
				...
				pGraphics->pSkel->eaAnimChartStacks				= NULL;
				pGraphics->pSkel->eaAGUpdaterMutable			= NULL;
				pGraphics->pSkel->movement.pChartStack			= NULL;
	...
	if (pCostume)
	{
		...
		if (!bKeepSkeleton && bKeepOldSequencers)
		{
			...
			{
				pGraphics->pSkel->eaAnimChartStacks				= swapChartStacks;
				pGraphics->pSkel->eaAGUpdaterMutable			= swapAGUpdater;
				pGraphics->pSkel->movement.pChartStack			= swapMovementChartStack;
...
void costumeView_DrawGhostsViewCostume(CostumeViewCostume *pViewCostume, bool bForceEditorRegion)
{
	...
	if( pViewCostume->bModColorIsSet ) {
		pViewCostume->pDrawSkel->fEntityAlpha = pViewCostume->v4ModColor[ 3 ];
		gfxQueueSingleDynDrawSkeleton(pViewCostume->pDrawSkel, bForceEditorRegion ? worldGetEditorWorldRegion() : NULL, true, true, pViewCostume->v4ModColor);
		FOR_EACH_IN_EARRAY(pViewCostume->pDrawSkel->eaSubDrawSkeletons, DynDrawSkeleton, pSub)
			pSub->fEntityAlpha = pViewCostume->v4ModColor[ 3 ];
			gfxQueueSingleDynDrawSkeleton(pSub, bForceEditorRegion ? worldGetEditorWorldRegion() : NULL, false, true, pViewCostume->v4ModColor);
		FOR_EACH_END;
	} else {
		pViewCostume->pDrawSkel->fEntityAlpha = 1;
		gfxQueueSingleDynDrawSkeleton(pViewCostume->pDrawSkel, bForceEditorRegion ? worldGetEditorWorldRegion() : NULL, true, true, NULL);
		FOR_EACH_IN_EARRAY(pViewCostume->pDrawSkel->eaSubDrawSkeletons, DynDrawSkeleton, pSub)
			pSub->fEntityAlpha = 1;
			gfxQueueSingleDynDrawSkeleton(pSub, bForceEditorRegion ? worldGetEditorWorldRegion() : NULL, false, true, NULL);
		FOR_EACH_END;
	}
}
...
CostumeViewGraphics *costumeView_CreateGraphics(void)
{
	...
	costumeView_ClearModColor(pGraphics);
	return pGraphics;
}

CostumeViewCostume *costumeView_CreateViewCostume(void)
{
	CostumeViewCostume *pViewCostume = calloc(1,sizeof(CostumeViewCostume));
	Vec3 pyr = {0, 0, 0};
	PYRToQuat(pyr, pViewCostume->qSkelRot);
	costumeView_SetViewCostumeModColor(pViewCostume, onevec3, 1);
	return pViewCostume;
}
...
void costumeView_RegenViewCostume(CostumeViewCostume *pViewCostume, PlayerCostume *pCostume, const PCSlotType *pSlotType, PCMood *pMood, CharacterClass* pClass,
	AllegianceDef *pAllegiance, AllegianceDef *pSubAllegiance)
{
	...
	pWLCostume = (WLCostume*)costumeGenerate_CreateWLCostumeEx(pCostume, pSpecies, pClass, pAllegiance, pSubAllegiance, NULL, pSlotType, pMood, &g_CostumeEditState.eaFXArray, "CostumeEditor.", 0, 0, false, !pViewCostume->bIgnoreSkelFX, &eaSubCostumes);
...
void costumeView_RegenCostumeEx(CostumeViewGraphics *pGraphics, PlayerCostume *pCostume, const PCSlotType *pSlotType, PCMood *pMood, CharacterClass* pClass,
	AllegianceDef *pAllegianceDef, AllegianceDef *pSubAllegianceDef)
{
	// For backwards compatibility, regen the "primary" costume
	costumeView_RegenViewCostume(&pGraphics->costume, pCostume, pSlotType, pMood, pClass, pAllegianceDef, pSubAllegianceDef);
}
...
void costumeView_FreeViewCostume(CostumeViewCostume *pViewCostume)
{
	costumeView_DeInitViewCostume(pViewCostume);
	free(pViewCostume);
}

void costumeView_DeInitViewCostume(CostumeViewCostume *pViewCostume)
{
	if (!pViewCostume)
		return;
...
void costumeView_FreeGraphics(CostumeViewGraphics *pGraphics)
{
	...
	costumeView_DeInitViewCostume(&pGraphics->costume);

	while (eaSize(&pGraphics->eaExtraCostumes) > 0) {
		costumeView_FreeViewCostume(eaPop(&pGraphics->eaExtraCostumes));
	}
...
void costumeView_SetModColor(CostumeViewGraphics *pGraphics, const Vec3 color, float alpha)
{
	costumeView_SetViewCostumeModColor(&pGraphics->costume, color, alpha);
}

void costumeView_ClearModColor(CostumeViewGraphics *pGraphics)
{
	costumeView_ClearViewCostumeModColor(&pGraphics->costume);
}
...
void costumeView_SetViewCostumeModColor(CostumeViewCostume *pViewCostume, const Vec3 color, float alpha)
{
	copyVec3(color, pViewCostume->v4ModColor);
	pViewCostume->v4ModColor[3] = alpha;
	pViewCostume->bModColorIsSet = true;
}

void costumeView_ClearViewCostumeModColor(CostumeViewCostume *pViewCostume)
{
	setVec4(pViewCostume->v4ModColor, 0, 0, 0, 0);
	pViewCostume->bModColorIsSet = false;
}
