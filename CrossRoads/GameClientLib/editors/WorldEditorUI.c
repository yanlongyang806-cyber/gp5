...
bool wleUISetLayerModeCallback(ServerResponseStatus status, ZoneMapLayer *layer, int step, int total_steps)
{
	if (status == StatusSuccess)
	{
		...
		if (!layer->grouptree.def_lib)
		{
			...
			{
				filelog_printf("resourceSend.log","Layer not locked (missing def_lib, layer probably deleted mid-lock): %s", layer->filename);
			}
			return false;
		}
...
static void wleUITrackerTreeAddEdObjNodeAssoc(EditorObject *edObj, UITreeNode *node)
{
	...
	if (!wleUIEdObjNodeAssocs)
		wleUIEdObjNodeAssocs = stashTableCreateWithStringKeys(32, StashDeepCopyKeys_NeverRelease);
...
// show the object library search window.
AUTO_COMMAND ACMD_CATEGORY(World, Interface) ACMD_NAME("Editor.ObjectLibraryShowSearch");
void wleUIObjectLibraryShowSearch()
{
...
static int wleLayerComparator(const UITreeNode** child_a, const UITreeNode** child_b)
{
	const char* layer_a;
	const char* layer_b;

	assert((*child_a)->displayF == wleUILockMngrLayerNodeDisplay &&
		(*child_b)->displayF == wleUILockMngrLayerNodeDisplay);

	layer_a = layerGetFilename((ZoneMapLayer*)((*child_a)->displayData));
	layer_b = layerGetFilename((ZoneMapLayer*)((*child_b)->displayData));
	return strcmp(layer_a, layer_b);
}
...
static void wleUILockMngrFillLayers(UITreeNode *node, UserData unused)
{
	...
	ui_TreeNodeSortChildren(node, wleLayerComparator);
}
...
static void wleUIRegionMngrTreeSelected(UITree *tree, UIList *sky_list)
{
	...
	if (selectedNode && selectedNode->displayF == wleUIRegionMngrRegionNodeDisplay)
	{
		...
		sprintf(tempStr, "%i", region ? worldRegionGetUnteamedPetsPerPlayer(region) : 0);
		ui_TextEntrySetText(editorUIState->regionMngrUI.maxNonTeamPetsEntry, region ? tempStr : "");
	...
	else
	{
		...
		ui_TextEntrySetText(editorUIState->regionMngrUI.maxNonTeamPetsEntry, "");
...
static void wleUIRegionMngrMaxTeamPetsChanged(UITextEntry *entry, void *unused)
{
	UITreeNode *selectedNode = ui_TreeGetSelected(editorUIState->regionMngrUI.tree);
	...
	if (selectedNode && selectedNode->displayF == wleUIRegionMngrRegionNodeDisplay)
	{
		...
		if (region)
			wleOpSetRegionMaxNonTeamPets(region, atoi(ui_TextEntryGetText(entry)));
	}
}
...
static void wleUIMapUsedInUGCChanged(UIComboBox *combo, int val, UserData unused)
{
	wleOpSetMapUsedInUGC(val);
	wleUIMapPropertiesRefresh();
}
...
void wleUIMapPropertiesRefresh(void)
{
	...
	ZoneMapUGCUsage eUsedInUGC = zmapInfoGetUsedInUGC(zminfo);
	...
	ui_ComboBoxSetSelectedEnum(editorUIState->mapPropertiesUI.usedInUgcCombo, eUsedInUGC);
...
{
	ui_ModalDialog("Version Info", "World Editor\nv0000.00\n", ColorBlack, UIOk);
}
...
void wleUIInit(EMEditorDoc *doc)
{
	...
	label = wleCreateLabel(panel, UI_WIDGET(check), "Used in UGC");
	combo = ui_ComboBoxCreate(0, label->widget.y, 1, NULL, NULL, NULL);
	ui_WidgetSetTooltipString(UI_WIDGET(combo), "If set, this generates extra data needed to use this map in UGC." );
	ui_ComboBoxSetEnum(combo, ZoneMapUGCUsageEnum, wleUIMapUsedInUGCChanged, NULL);
	ui_WidgetSetWidthEx(UI_WIDGET(combo), 1, UIUnitPercentage);
	leftPad = MAX(leftPad, elUINextX(label) + 5);
	editorUIState->mapPropertiesUI.usedInUgcCombo = combo;
	emPanelAddChild(panel, combo, true);

	label = wleCreateLabel(panel, UI_WIDGET(combo), "Private to");
	leftPad = MAX(leftPad, elUINextX(label) + 5);
	...
	editorUIState->mapPropertiesUI.usedInUgcCombo->widget.leftPad = leftPad;
	...
	label = ui_LabelCreate("Max Non-Team Pets", 5, elUINextY(entry) + 5);
	emPanelAddChild(panel, label, false);
	entry = ui_TextEntryCreate("", elUINextX(label) + 5, label->widget.y);
	editorUIState->regionMngrUI.maxNonTeamPetsEntry = entry;
	ui_TextEntrySetIntegerOnly(entry);
	ui_TextEntrySetFinishedCallback(entry, wleUIRegionMngrMaxTeamPetsChanged, NULL);
	emPanelAddChild(panel, entry, false);
