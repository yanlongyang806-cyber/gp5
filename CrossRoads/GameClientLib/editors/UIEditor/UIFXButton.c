...
static void SetToDefault(DynDefineParam *param, UIFXWindow *pWindow)
{
	MultiValCopy(&param->mvVal, dynFxInfoGetParamValue(pWindow->pButton->pFX->pcDynName, param->pcParamName));
}

static void FXWindowSetParamBlock(DynParamBlock *dst, DynParamBlock *src, FxParamGroup **eaGroup, UIFXWindow *pWindow)
{
	if (src && dst)
	{
		int i, j, k;
		...
		for (i = 0; i < eaSize(&dst->eaDefineParams); i++)
		{
			for (j = 0; j < eaSize(&src->eaDefineParams); j++)
			{
				if (strcmp(dst->eaDefineParams[i]->pcParamName, src->eaDefineParams[j]->pcParamName) == 0)
				{
					...
					if (eaGroup)
					{
						for (k = 0; k < eaSize(&eaGroup); k++)
						{
							if (strcmp(dst->eaDefineParams[i]->pcParamName, eaGroup[k]->pcName) == 0)
							{
								if (eaGroup[k]->pParamCheckButton->state)
								{
			...
			if (!found)
			{
				SetToDefault(dst->eaDefineParams[i], pWindow);
			}
		}
	}
}
...
static void FXWindowSelectButtonCallback(UIButton *button, UIFXWindow *pWindow)
{
	...
	FXWindowSetParamBlock(&(pWindow->pButton->pFX->paramBlock), pWindow->newData->FXDataBlock, pWindow->eaParamGroups, pWindow);
	FXWindowSetParamBlock(pWindow->pButton->pSetBlock, pWindow->newData->FXDataBlock, pWindow->eaParamGroups, pWindow);
...
void ui_FXWindowFreeInternal(UIFXWindow *pWindow)
{
	...
	if ((pWindow->oldData->FXDataBlock) && ((!pWindow->newData->FXDataBlock) || (pWindow->oldData->FXDataBlock != pWindow->newData->FXDataBlock)))
	{
