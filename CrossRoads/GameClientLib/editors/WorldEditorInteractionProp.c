...
typedef struct WleAEInteractionPropUI
{
	...
	struct 
	{
		...
		WleAEParamExpression overrideDisplayNameExpr;
		WleAEParamMessage overrideDisplayName;
	} data;
...
static void wleAEInteractionPropDoorHasVariablesApply(WleAEParamBool *param, void *unused, EditorObject **objs)
{
	...
	for (i = 0; i < eaSize(&objs); i++)
	{
		...
		{
			char* pcZoneMap = NULL;
			bool bIsMissionReturn = false;
			ZoneMapInfo *pZoneMap = NULL;

			if (properties->eaEntries[param->index]->pDoorProperties->doorDest.pSpecificValue)
			{
				pcZoneMap = properties->eaEntries[param->index]->pDoorProperties->doorDest.pSpecificValue->pcZoneMap;
				bIsMissionReturn = (stricmp(properties->eaEntries[param->index]->pDoorProperties->doorDest.pSpecificValue->pcStringVal, "MissionReturn") == 0);
			}
...
static void wleAEInteractionPropOverrideDisplayNameUpdate(WleAEParamMessage *param, void *unused, EditorObject *obj)
{
	if (obj->type->objType == EDTYPE_TRACKER || obj->type->objType == EDTYPE_ENCOUNTER_ACTOR)
	{
		wleAEInteractionPropUpdateInit();
		if (properties)
		{
			DisplayMessage* display_name_msg = &properties->overrideDisplayNameMsg;
			langMakeEditorCopy(parse_DisplayMessage, display_name_msg, true);
			groupDefFixupMessageKey( &display_name_msg->pEditorCopy->pcMessageKey, def, param->source_key, NULL );
			if( !display_name_msg->pEditorCopy->pcScope || !display_name_msg->pEditorCopy->pcScope[ 0 ]) {
				display_name_msg->pEditorCopy->pcScope = allocAddString(param->source_key);
...
static void wleAEInteractionPropOverrideDisplayNameApply(WleAEParamMessage *param, void *unused, EditorObject **objs)
{
	int i;
	for (i = 0; i < eaSize(&objs); i++)
	{
...
static void wleAEInteractionPropOverrideDisplayNameExprUpdate(WleAEParamExpression *param, void *unused, EditorObject *obj)
{
	if (obj->type->objType == EDTYPE_TRACKER || obj->type->objType == EDTYPE_ENCOUNTER_ACTOR)
	{
		wleAEInteractionPropUpdateInit();
		if (properties)
		{
			param->exprvalue = exprClone(properties->pOverrideDisplayNameExpr);
			return;
		}
	}
	param->exprvalue = NULL;
...
static void wleAEInteractionPropOverrideDisplayNameExprApply(WleAEParamExpression *param, void *unused, EditorObject **objs)
{
	int i;
	for (i = 0; i < eaSize(&objs); i++)
	{
		...
		if (properties)
		{
			exprDestroy(properties->pOverrideDisplayNameExpr);
			properties->pOverrideDisplayNameExpr = exprClone(param->exprvalue);
		}
		wleOpPropsEndNoUIUpdate();
	}
	wleOpRefreshUI();
...
int wleAEInteractionPropReload(EMPanel *panel, EditorObject *edObj)
{
	...
	wleAEExpressionUpdate(&wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr);
	wleAEMessageUpdate(&wleAEGlobalInteractionPropUI.data.overrideDisplayName);
		...
		if (entry->interactionClass.is_specified && entry->interactionClass.stringvalue && entry->interactionClass.stringvalue[0])
		{
				...
				))	)
			{
				...
				if (entry->isAction.boolvalue)
				{
					...
					wleAEExpressionAddWidget(wleAEGlobalInteractionPropUI.autoWidget, "Fail Expr", "Expression run on failure. (optional)", "pFailureExpr", &entry->failExpr);
		...
		else if (!entry->interactionClass.diff && !entry->interactionClass.spec_diff)
		{
			if (index != 0)
			{
				...
				if (!bActorObjects && stricmp(entry->interactionClass.stringvalue, "Destructible") != 0)
				{
					...
					wleAEExpressionAddWidget(wleAEGlobalInteractionPropUI.autoWidget, "Override Display Expr", "If this expression evaluates to true, use the override display expression instead", "pOverrideDisplayNameExpression", &wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr);

					wleAEMessageAddWidget(wleAEGlobalInteractionPropUI.autoWidget, "Override Display Text", "If the override display text expression is true, this replaces the text displayed above the interact. (optional)", "override_display_name", &wleAEGlobalInteractionPropUI.data.overrideDisplayName);
				}
...
#define wleAEInteractionSetupMoveParam(param, funcPrefix) \
	...
	wleAEGlobalInteractionPropUI.data.overrideDisplayName.entry_align = WLE_AE_INTERACTION_ALIGN_WIDTH;
	wleAEGlobalInteractionPropUI.data.overrideDisplayName.left_pad = WLE_AE_INTERACTION_INDENT;
	wleAEGlobalInteractionPropUI.data.overrideDisplayName.entry_width = WLE_AE_INTERACTION_TEXT_ENTRY_WIDTH;
	wleAEGlobalInteractionPropUI.data.overrideDisplayName.update_func = wleAEInteractionPropOverrideDisplayNameUpdate;
	wleAEGlobalInteractionPropUI.data.overrideDisplayName.apply_func = wleAEInteractionPropOverrideDisplayNameApply;
	sprintf(buf, "overrideDisplayName");
	wleAEGlobalInteractionPropUI.data.overrideDisplayName.source_key = strdup(buf);

	wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr.entry_align = WLE_AE_INTERACTION_ALIGN_WIDTH;
	wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr.left_pad = WLE_AE_INTERACTION_INDENT;
	wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr.entry_width = WLE_AE_INTERACTION_TEXT_ENTRY_WIDTH;
	wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr.context = g_pInteractionContext;
	wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr.update_func = wleAEInteractionPropOverrideDisplayNameExprUpdate;
	wleAEGlobalInteractionPropUI.data.overrideDisplayNameExpr.apply_func = wleAEInteractionPropOverrideDisplayNameExprApply;
