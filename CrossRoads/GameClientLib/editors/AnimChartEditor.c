...
#define ACE_IDX2_TO_PTR(idx1, idx2)			(idx1 < 1000 && idx2 < 1000) ?				\
												U32_TO_PTR((U32)((idx1)+(idx2)*1000)) :	\
												assert(0)
#define ACE_PTR_TO_IDX2(idx1, idx2, ptr)	(idx1) =  ((U32)PTR_TO_U32(ptr));	\
											(idx2) =  ((U32)(idx1)/1000);		\
											(idx1) -= ((U32)(idx2)*1000);

#define ACE_IDX3_TO_PTR(idx1, idx2, idx3)		(idx1 < 1000 && idx2 < 1000 && idx3 < 1000) ?				\
													U32_TO_PTR((U32)((idx1)+(idx2)*1000+(idx3)*1000000)) :	\
													assert(0)
...
#define ACE_IDX4_TO_PTR(idx1, idx2, idx3, idx4)			(idx1 < 1000 && idx2 < 1000 && idx3 < 1000 && idx4 < 2) ?						\
															U32_TO_PTR((U32)((idx1)+(idx2)*1000+(idx3)*1000000+(idx4)*1000000000)) :	\
															assert(0)
#define ACE_PTR_TO_IDX4(idx1, idx2, idx3, idx4, ptr)	(idx1) =  ((U32)PTR_TO_U32(ptr));				\
														(idx4) =  (U32)((idx1)/1000000000);				\
														(idx1) -= (U32)((idx4)*1000000000);				\
														(idx3) =  (U32)((idx1)/1000000);				\
														(idx1) -= (U32)((idx3)*1000000);				\
														(idx2) =  (U32)((idx1)/1000);					\
														(idx1) -= (U32)((idx2)*1000);
...
static void ACERefreshMoveGraphsExpander(AnimChartDoc* pDoc);
...
{
	assert(!pField->uiMoveMode && !pField->pComboBox);
	ui_WidgetQueueFree(UI_WIDGET(pField->pEntry));
	free(pField);
}

void ACEMoveGraphRefFieldDestroy(ACEGraphRefField* pField)
{
	assert(pField->uiMoveMode && !pField->pEntry);
	ui_WidgetQueueFree(UI_WIDGET(pField->pComboBox));
	free(pField);
}
...
static void ACEIsSubchartCB(MEField* pField, bool bFinished, AnimChartDoc* pDoc)
{
	if(pDoc->pObject->bIsSubChart) {
		...
		eaDestroy(&pObject->eaValidMoveKeywords);
		eaDestroyStruct(&pObject->eaMoveGraphRefs, parse_DynAnimChartGraphRefLoadTime);
...
static void ACEFormatValidKeywords(DynAnimChartLoadTime *pObject, U32 uiMoveMode)
{
	const char*** MODE_peaValidKeywords = uiMoveMode ? &pObject->eaValidMoveKeywords : &pObject->eaValidKeywords;
	int idx = eaFind(MODE_peaValidKeywords, allocAddString("Default"));
	if(idx >= 0) {
		eaInsert(MODE_peaValidKeywords, eaRemove(MODE_peaValidKeywords, idx), 0);
	}
}
...
static void ACEUpdateDisplay(AnimChartDoc* pDoc)
{
	...
	ACERefreshMoveGraphsExpander(pDoc);
	...
	*/
	...
	FOR_EACH_IN_EARRAY(pDoc->eaMoveGraphRefFields, ACEGraphRefField, pField)
	{
		const char *pcOrigKeyword = pDoc->pOrigObject ? eaGet(&pDoc->pOrigObject->eaValidMoveKeywords, pField->iIndex) : NULL;
		const char *pcKeyword = pDoc->pObject ? eaGet(&pDoc->pObject->eaValidMoveKeywords, pField->iIndex) : NULL;
		assert(pcKeyword);
		ui_ComboBoxSetSelected(pField->pComboBox, dynAnimMovementStanceIndex(pcKeyword));
		ui_SetChanged(UI_WIDGET(pField->pComboBox), (!pcOrigKeyword || pcOrigKeyword != pcKeyword));
	}
	FOR_EACH_END;
...
{
	AnimChartDoc* pDoc = (AnimChartDoc*)emGetActiveEditorDoc();
	DynAnimChartGraphRefLoadTime*** MODE_peaGraphRefs;
	DynAnimChartGraphRefLoadTime* pRef;
	U32 uiRefIndex, uiRandIdx, uiMoveMode;

	ACE_PTR_TO_IDX3(uiRefIndex, uiRandIdx, uiMoveMode, pFakePtr);

	MODE_peaGraphRefs = uiMoveMode ? &pDoc->pObject->eaMoveGraphRefs : &pDoc->pObject->eaGraphRefs;

	pRef = eaGet(MODE_peaGraphRefs, uiRefIndex);
	assert(pRef);

	if (eaSize(&pRef->eaGraphChances)==0) {
		eaFindAndRemove(MODE_peaGraphRefs, pRef);
...
static void ACEChooseMoveClosedCallback(EMPicker *picker, void* pFakePtr)
{
	...
	ACE_PTR_TO_IDX2(uiRefIndex, uiRandIdx, pFakePtr);
...
static bool ACEChooseGraphCallback(EMPicker *picker, EMPickerSelection **selections, void* pFakePtr)
{
	...
	const char*** MODE_peaValidKeywords;
	U32 uiRefIndex, uiRandIdx, uiMoveMode;
	...
	MODE_peaGraphRefs     = uiMoveMode ? &pDoc->pObject->eaMoveGraphRefs     : &pDoc->pObject->eaGraphRefs;
	MODE_peaValidKeywords = uiMoveMode ? &pDoc->pObject->eaValidMoveKeywords : &pDoc->pObject->eaValidKeywords;
	...
	if (!pRef->pcKeyword)
	{
		...
		while(eaFind(MODE_peaValidKeywords, pcNewKeyword) >= 0) {
			char pcNext[256];
			sprintf(pcNext, "%s_%d", selections[0]->doc_name, ++idx);
			pcNewKeyword = allocAddString(pcNext);
		}
		pRef->pcKeyword = pcNewKeyword;
		eaPush(MODE_peaValidKeywords, pcNewKeyword);
		ACEFormatValidKeywords(pDoc->pObject, uiMoveMode);
	}
...
static void ACERandomGraphWindowFinished(UIWindow *pUnused, WleUIRandomGraphWin *pUI)
{
	...
	if (pDoc)
	{
		if (pUI->pDoc->pRandomGraphWindow) {
			...
			ACE_PTR_TO_IDX3(uiRefIndex, uiRandIdx, uiMoveMode, pUI->pFakePtr);
...
#define ACE_RAND_DATA_SIZE 100
		...
		for ( i=0; i < eaSize(&pRef->eaGraphChances); i++ ) {
			...
			ACECreateGraphEntryRandWindow(pChance, 0, y, bHighlight, ACE_IDX3_TO_PTR((U32)uiRefIndex, (U32)i, uiMoveMode), ui_WindowAddChild, pWindow);	

			{
				...
				ui_TextEntrySetFinishedCallback(pEntry, ACEGraphChanceChanged, ACE_IDX3_TO_PTR((U32)uiRefIndex, (U32)i, uiMoveMode));
...
#define ACE_RAND_DATA_SIZE 100
		...
		for ( i=0; i < eaSize(&pRef->eaMoveChances); i++ ) {
			...
			ACECreateMoveEntryRandWindow(pChance, 0, y, bHighlight, ACE_IDX2_TO_PTR((U32)uiRefIndex, (U32)i), ui_WindowAddChild, pWindow, eaSize(&pRef->eaMoveChances) > 1);	

			{
				...
				sprintf(buf, "%g%%", pChance->fChance*100.0f);
				pEntry = ui_TextEntryCreate(buf, REF_COL_SIZE+5, y);
				ui_TextEntrySetFinishedCallback(pEntry, ACEMoveChanceChanged, ACE_IDX2_TO_PTR((U32)uiRefIndex, (U32)i));
				ui_WidgetSetWidth(UI_WIDGET(pEntry), ACE_RAND_DATA_SIZE);
				ui_WindowAddChild(pWindow, pEntry);
			}
...
static void ACEAddKeyword(UIWidget* pWidget, SA_PARAM_NN_VALID AnimChartDoc* pDoc)
{
	if (pDoc) {
		...
		ACEChooseGraph(pWidget, ACE_IDX3_TO_PTR(uiRefIndex, 0, 0));
	}
}

static void ACEAddMoveKeyword(UIWidget* pWidget, SA_PARAM_NN_VALID AnimChartDoc* pDoc)
{
	if (pDoc) {
		DynAnimChartGraphRefLoadTime* pRef = StructCreate(parse_DynAnimChartGraphRefLoadTime);
		U32 uiRefIndex = eaPush(&pDoc->pObject->eaMoveGraphRefs, pRef);
		ACEChooseGraph(pWidget, ACE_IDX3_TO_PTR(uiRefIndex, 0, 1));
	}
}
...
static void ACEAddDefaultKeyword(UIWidget* pWidget, SA_PARAM_NN_VALID AnimChartDoc* pDoc)
{
	if (pDoc) {
		...
		ACEFormatValidKeywords(pDoc->pObject, 0);

		ACEChooseGraph(pWidget, ACE_IDX3_TO_PTR(uiRefIndex, 0, 0));
	}
}

static void ACEAddDefaultMoveKeyword(UIWidget* pWidget, SA_PARAM_NN_VALID AnimChartDoc* pDoc)
{
	if (pDoc) {
		DynAnimChartGraphRefLoadTime* pRef = StructCreate(parse_DynAnimChartGraphRefLoadTime);
		U32 uiRefIndex = eaPush(&pDoc->pObject->eaMoveGraphRefs, pRef);
		pRef->pcKeyword = allocAddString("Default");
		eaPush(&pDoc->pObject->eaValidMoveKeywords, pRef->pcKeyword);
		ACEFormatValidKeywords(pDoc->pObject, 1);
...
static void ACERemoveGraphHelper(AnimChartDoc *pDoc, UIWidget *pWidget, void *pFakePtr)
{
	...
	pRef = eaRemove(MODE_peaGraphRefs, uiRefIndex);
	assert(pRef);
	if (pRef)
		StructDestroy(parse_DynAnimChartGraphRefLoadTime, pRef);
}
...
static void ACEAddGraphHelper(AnimChartDoc *pDoc, UIWidget *pWidget, void *pFakePtr, bool bBlank)
{
	...
	U32 uiMoveMode, uiRefIndex;

	ACE_PTR_TO_IDX3(keyword_idx, stance_idx, uiMoveMode, pFakePtr);
	stance_idx--;

	MODE_peaGraphRefs = uiMoveMode ? &pDoc->pObject->eaMoveGraphRefs : &pDoc->pObject->eaGraphRefs;
	uiRefIndex = eaPush(MODE_peaGraphRefs, pRef);

	MODE_peaValidKeywords = uiMoveMode ? &pDoc->pObject->eaValidMoveKeywords : &pDoc->pObject->eaValidKeywords;

	assert(keyword_idx >= 0 && keyword_idx < eaSize(MODE_peaValidKeywords));
	pRef->pcKeyword = (*MODE_peaValidKeywords)[keyword_idx];
	...
	pRef->bBlank = bBlank;
	if (!bBlank)
		ACEChooseGraph(pWidget, ACE_IDX3_TO_PTR(uiRefIndex, 0, uiMoveMode));
	else
		ACEAnimChartChanged(pDoc, true);
}
...
static void ACERemoveAndAddGraphRef(UIWidget *pWidget, void *pFakePtr)
{
	AnimChartDoc *pDoc = (AnimChartDoc*)emGetActiveEditorDoc();
	if (pDoc) {
		U32 uiRefIdx, uiKeywordIdx, uiStanceIdx, uiMoveMode;
		ACE_PTR_TO_IDX4(uiRefIdx, uiKeywordIdx, uiStanceIdx, uiMoveMode, pFakePtr);
		ACERemoveGraphHelper(pDoc, pWidget, ACE_IDX3_TO_PTR(uiRefIdx, 0, uiMoveMode));
		ACEAddGraphHelper(pDoc, pWidget, ACE_IDX3_TO_PTR(uiKeywordIdx, uiStanceIdx, uiMoveMode), false);
	}
}
...
static void ACEAddMoveRefHelper(UIWidget *pWidget, void *pFakePtr, bool bBlank)
{
	...
	if (pDoc) {
		...
		if (bBlank) {
			...
			pRef->bBlank = false;
			ACEChooseMove(pWidget, ACE_IDX2_TO_PTR(uiRefIndex, eaSize(&pRef->eaMoveChances)));
		}
	}
}
...
static void ACEMoveKeywordUp(UIWidget* pWidget, void* pFakePtr)
{
	...
	if (pDoc) {
		...
		keyword_idx = ACE_PTR_TO_IDX2(keyword_idx, uiMoveMode, pFakePtr);
		...
		if (keyword_idx > 0)
		{
			if (keyword_idx-1 != 0 || (*MODE_peaValidKeywords)[0] != ANIM_CHART_DEFAULT_KEY)
			{
				pcKeyword = eaRemove(MODE_peaValidKeywords, keyword_idx);
				eaInsert(MODE_peaValidKeywords, pcKeyword, keyword_idx-1);
				ACEAnimChartChanged(pDoc, true);
			}
		}
	}
...
static void ACEMoveKeywordDown(UIWidget* pWidget, void* pFakePtr)
{
	...
	if (pDoc) {
		...
		assert(keyword_idx >= 0 && keyword_idx < eaSize(MODE_peaValidKeywords));
		if (keyword_idx < eaSize(MODE_peaValidKeywords)-1)
		{
			pcKeyword = eaRemove(MODE_peaValidKeywords, keyword_idx);
			eaInsert(MODE_peaValidKeywords, pcKeyword, keyword_idx+1);
			ACEAnimChartChanged(pDoc, true);
...
static void ACEDeleteKeyword(UIWidget* pWidget, void* pFakePtr)
{
	...
	if (pDoc) {
		...
		ACE_PTR_TO_IDX2(keyword_idx, uiMoveMode, pFakePtr);
		...
		for ( i=0; i < eaSize(MODE_peaGraphRefs); i++ ) {
			DynAnimChartGraphRefLoadTime *pRef = (*MODE_peaGraphRefs)[i];
			if(pRef->pcKeyword == pcKeyword) {
				eaRemove(MODE_peaGraphRefs, i);
				i--;
				StructDestroy(parse_DynAnimChartGraphRefLoadTime, pRef);
			}
		}
...
static void ACEGraphAddAdditional(UIWidget* pWidget, void* pFakePtr)
{
	...
	if (pDoc) {
		...
		if (pMovePicker)
			emPickerShowEx(pMovePicker, NULL, false, ACEChooseGraphCallback, ACEChooseGraphClosedCallback, ACE_IDX3_TO_PTR(uiRefIndex, uiRandIdx, uiMoveMode));
	}
}
...
static void ACEMoveAddAdditional(UIWidget* pWidget, void* pFakePtr)
{
	...
	if (pDoc) {
		...
		if (pMovePicker)
			emPickerShowEx(pMovePicker, NULL, false, ACEChooseMoveCallback, ACEChooseMoveClosedCallback, ACE_IDX2_TO_PTR(uiRefIndex, uiRandIdx));
	}
}
...
static void ACEDeleteStanceKey(UIWidget* pWidget, void* pFakePtr)
{
	...
	if (pDoc) {
		...
		for (i=0; i < eaSize(&pDoc->pObject->eaMoveGraphRefs); i++) {
			DynAnimChartGraphRefLoadTime* pRef = pDoc->pObject->eaMoveGraphRefs[i];
			if(dynAnimKeyFromStanceWords(pRef->eaStanceWords) == pcOrigStanceKey) {
				eaRemove(&pDoc->pObject->eaMoveGraphRefs, i);
				i--;
				StructDestroy(parse_DynAnimChartGraphRefLoadTime, pRef);
			}
		}
...
static void ACERefStanceWordChanged(UIComboBox *pComboBox, void* pFakePtr)
{
	...
	if (pDoc) {
		...
		ACE_PTR_TO_IDX2(stance_key_idx, stance_idx, pFakePtr);
		stance_key_idx--;
...
static void ACEKeywordChangedCB(void *pVoid, ACEGraphRefField *pField)
{
	if (pField->pDoc) {
		int i;
		DynAnimChartLoadTime *pObject = pField->pDoc->pObject;
		DynAnimChartGraphRefLoadTime*** MODE_peaGraphRefs = pField->uiMoveMode ? &pObject->eaMoveGraphRefs : &pObject->eaGraphRefs;
		const char*** MODE_peaValidKeywords = pField->uiMoveMode ? &pObject->eaValidMoveKeywords : &pObject->eaValidKeywords;
		const char* pcDefaultString = allocAddString("Default");
		const char* pcNewString;
		const char* pcOrigString;

		if (pField->uiMoveMode) {
			S32 iSelectedIndex = ui_ComboBoxGetSelected(pField->pComboBox);
			char* pcGetString = NULL;
			ui_ComboBoxGetSelectedAsString(pField->pComboBox, &pcGetString);
			pcNewString = allocAddString(pcGetString);
		} else {
			pcNewString = allocAddString(ui_TextEntryGetText(pField->pEntry));
		}
		
		assert(pField->iIndex >= 0 && pField->iIndex < eaSize(MODE_peaValidKeywords));
		pcOrigString = (*MODE_peaValidKeywords)[pField->iIndex];

		if(pcNewString == pcDefaultString) {
			Alertf("Default is a reserved name.");
			if (pField->uiMoveMode) {
				ui_ComboBoxSetSelected(pField->pComboBox, dynAnimMovementStanceIndex(pcOrigString));
			} else {
				ui_TextEntrySetText(pField->pEntry, pcOrigString);
			}
			return;
		}
		...
		if(eaFind(MODE_peaValidKeywords, pcNewString) >= 0) {
		...
		(*MODE_peaValidKeywords)[pField->iIndex] = pcNewString;

		ACEFormatValidKeywords(pObject, pField->uiMoveMode);
...
static void ACECreateStanceWordsExpander(AnimChartDoc* pDoc)
{
	...
	for ( i=-1; i < eaSize(&pDoc->pObject->eaValidStances); i++ ) {
		...
		for ( j=0; j < eaSize(&eaStanceWords); j++ ) {
			...
			ui_ComboBoxSetSelectedCallback((UIComboBox*)pFilterCombo, ACERefStanceWordChanged, ACE_IDX2_TO_PTR(i+1, j));
			...
			pButton = ui_ButtonCreate("X", local_x, local_y, ACERefRemoveStanceWord, ACE_IDX2_TO_PTR(i+1, j));
...
static void ACECreateGraphEntryBlank(DynAnimChartGraphRefLoadTime *pRef, F32 local_x, F32 local_y, void *pUserData, UIAddWidgetFunc addChildF, UIAnyWidget *pParent)
{
	...
	ACE_PTR_TO_IDX4(uiRefIdx, uiKeywordIdx, uiStanceIdx, uiMoveMode, pUserData);

	pButton = ui_ButtonCreate("SKIP", local_x+10, local_y, ACERemoveGraph, ACE_IDX3_TO_PTR(uiRefIdx, 0, uiMoveMode));
...
static void ACECreateGenericGraphsExpander(AnimChartDoc* pDoc, U32 uiMoveMode)
{
	UIExpander* MODE_pExpander						= uiMoveMode ? pDoc->pMoveGraphsExpander			: pDoc->pGraphsExpander;
	const char*** MODE_peaValidKeywords				= uiMoveMode ? &pDoc->pObject->eaValidMoveKeywords	: &pDoc->pObject->eaValidKeywords;
	const char*** MODE_peaOrigValidKeywords			= pDoc->pOrigObject ? (uiMoveMode ? &pDoc->pOrigObject->eaValidMoveKeywords : &pDoc->pOrigObject->eaValidKeywords) : NULL;
	DynAnimChartGraphRefLoadTime*** MODE_peaGraphRef= uiMoveMode ? &pDoc->pObject->eaMoveGraphRefs		: &pDoc->pObject->eaGraphRefs;
	ACEGraphRefField*** MODE_peaGraphRefField		= uiMoveMode ? &pDoc->eaMoveGraphRefFields			: &pDoc->eaGraphRefFields;
	...
	UIPane *pScrollingPane;
	bool *pMatrix = NULL;
	int iKeywordCount = eaSize(MODE_peaValidKeywords);
	int iStanceCount = eaSize(&pDoc->pObject->eaValidStances);
	const char *pcDefaultString = allocAddString("Default");
...

	{
		UISeparator* pSeparator = ui_SeparatorCreate(UIHorizontal);
		ui_WidgetSetPosition(UI_WIDGET(pSeparator), 0, y);
		ui_ExpanderAddChild(MODE_pExpander, pSeparator);
		y += SEPARATOR_HEIGHT;
	}

	{
		...
		pScrollingPane->widget.drawF = ACEPaneDraw;
		pScrollingPane->invisible = true;
		ui_ExpanderAddChild(MODE_pExpander, pScrollingPane);
	}

	{
		ACECreateStanceWordsHeader(pDoc, MODE_pExpander, pScrollingPane, y, "Graph");
		y += STANDARD_ROW_HEIGHT;
	}

	{
		...
		pLabel = ui_LabelCreate(uiMoveMode ? "Movement Stance" : "Keyword", GRAPHS_LEFT_ADJUST, y);
		ui_ExpanderAddChild(MODE_pExpander, pLabel);
		y += STANDARD_ROW_HEIGHT;
	...
	if (eaFind(MODE_peaValidKeywords, pcDefaultString) >= 0)
	{
		F32 local_y = y + y_default_offset;
		UISeparator *pSeparator;

		pButton = ui_ButtonCreate("X", GRAPHS_LEFT_ADJUST, local_y, ACEDeleteKeyword, ACE_IDX2_TO_PTR(0, uiMoveMode)); 
		ui_WidgetSetWidth(UI_WIDGET(pButton), 16);
		ui_ExpanderAddChild(MODE_pExpander, pButton);

		pEntry = ui_TextEntryCreate(pcDefaultString, GRAPHS_LEFT_ADJUST+21, local_y);
		if (aceHighlightText && strstri(pcDefaultString,aceHighlightText))
			ui_WidgetSkin(UI_WIDGET(pEntry),&aceHighlightSkin);
		ui_WidgetSetWidth(UI_WIDGET(pEntry), 179);
		ui_SetActive(UI_WIDGET(pEntry), false);
		ui_ExpanderAddChild(MODE_pExpander, pEntry);
	...
	for ( i=0; i < eaSize(MODE_peaValidKeywords); i++ ) {
		const char *pcKeyword = (*MODE_peaValidKeywords)[i];
		const char *pcOrigKeyword = pDoc->pOrigObject ? eaGet(MODE_peaOrigValidKeywords, i) : NULL;
		UISeparator *pSeparator;
		...
		{
			pButton = ui_ButtonCreate("X", GRAPHS_LEFT_ADJUST, y, ACEDeleteKeyword, ACE_IDX2_TO_PTR(((U32)i), uiMoveMode)); 
			ui_WidgetSetWidth(UI_WIDGET(pButton), 16);
			ui_ExpanderAddChild(MODE_pExpander, pButton);

			pButton = ui_ButtonCreate("^", GRAPHS_LEFT_ADJUST+21, y, ACEMoveKeywordUp, ACE_IDX2_TO_PTR(((U32)i), uiMoveMode));
			ui_WidgetSetWidth(UI_WIDGET(pButton), 16);
			ui_ExpanderAddChild(MODE_pExpander, pButton);

			pButton = ui_ButtonCreate("v", GRAPHS_LEFT_ADJUST+42, y, ACEMoveKeywordDown, ACE_IDX2_TO_PTR(((U32)i), uiMoveMode));
			ui_WidgetSetWidth(UI_WIDGET(pButton), 16);
			ui_ExpanderAddChild(MODE_pExpander, pButton);
		}

		{
			...
			{
				pEntry = NULL;
				pComboBox = ui_FilteredComboBoxCreate(GRAPHS_LEFT_ADJUST+63, y, 229, parse_DynAnimStanceData, &stance_list.eaMovementStances, "Name");
				if (pcKeyword && aceHighlightText && strstri(pcKeyword,aceHighlightText))
					ui_WidgetSkin(UI_WIDGET(pComboBox),&aceHighlightSkin);
				ui_ComboBoxSetSelectedCallback(pComboBox, ACEKeywordChangedCB, pGraphRefField);
				ui_ComboBoxSetSelected(pComboBox, dynAnimMovementStanceIndex(pcKeyword));
				ui_ExpanderAddChild(MODE_pExpander, pComboBox);
				ui_WidgetSetWidth(UI_WIDGET(pComboBox), 179);
				ui_SetChanged(UI_WIDGET(pComboBox), (!pcOrigKeyword || pcOrigKeyword != pcKeyword));
			}
			else
			{
				pComboBox = NULL;
			...
			pGraphRefField->pComboBox = pComboBox;
			pGraphRefField->iIndex = i;
			pGraphRefField->uiMoveMode = uiMoveMode;
			eaPush(MODE_peaGraphRefField, pGraphRefField);
		}
	...
	FOR_EACH_IN_EARRAY_FORWARDS((*MODE_peaGraphRef), DynAnimChartGraphRefLoadTime, pRef)
	{
		F32 local_x, local_y;
		int iKeywordIdx = eaFind(MODE_peaValidKeywords, pRef->pcKeyword); // this might not be valid for movement graphs ? Or to say why isn't after I choose a move ?
...

		{
			bool bHighlight = false;
			if (pRef->bBlank) {
				ACECreateGraphEntryBlank(pRef, local_x, local_y, ACE_IDX4_TO_PTR((U32)ipRefIndex, (U32)iKeywordIdx, (U32)iStanceIdx, uiMoveMode), addF, pParent);
				...
				FOR_EACH_IN_EARRAY(pRef->eaGraphChances, DynAnimGraphChanceRef, pChanceRef)
					if (GET_REF(pChanceRef->hGraph) && aceHighlightText && strstri(REF_HANDLE_GET_STRING(pChanceRef->hGraph),aceHighlightText))
						bHighlight = true;
				FOR_EACH_END;
				ACECreateGraphEntryRandom(pRef, local_x, local_y, bHighlight, ACE_IDX3_TO_PTR((U32)ipRefIndex, 0, uiMoveMode), addF, pParent);
			} else {
				if (eaSize(&pRef->eaGraphChances) == 1 &&
					...
					bHighlight = true;
				ACECreateGraphEntryNormal(pRef, local_x, local_y, bHighlight, ACE_IDX3_TO_PTR((U32)ipRefIndex, 0, uiMoveMode), addF, pParent);
			}
		}
	}
	FOR_EACH_END;
	...
	for ( j=0; j < iStanceCount+1; j++ ) {
		for ( i=0; i < iKeywordCount; i++ ) {
			if(!pMatrix[i+(j*iKeywordCount)]) {
				...
				pButton = ui_ButtonCreate("FALLBACK", local_x+10, local_y, ACEAddBlankGraphRef, ACE_IDX3_TO_PTR(i, j, uiMoveMode));
				...
				pButton = ui_ButtonCreate("Add", local_x+205, local_y, ACEAddGraphRef, ACE_IDX3_TO_PTR(i, j, uiMoveMode)); 
				ui_WidgetSetWidth(UI_WIDGET(pButton), 39);
				ui_WidgetSetTooltipString(UI_WIDGET(pButton), "Assign an animation graph"); 
				if(j == 0)
					ui_ExpanderAddChild(MODE_pExpander, pButton);
	...
	if (!pDoc->pObject->bIsSubChart && eaFind(MODE_peaValidKeywords, pcDefaultString) < 0)
	{
		pButton = ui_ButtonCreate("Add Default", x, y, uiMoveMode ? ACEAddDefaultMoveKeyword : ACEAddDefaultKeyword, pDoc);
		ui_ExpanderAddChild(MODE_pExpander, pButton);
		x += ui_WidgetGetWidth(UI_WIDGET(pButton)) + 5;
	}
	else {
		pButton = ui_ButtonCreate(uiMoveMode ? "Add Stance" : "Add Keyword", x, y, uiMoveMode ? ACEAddMoveKeyword : ACEAddKeyword, pDoc);
		ui_ExpanderAddChild(MODE_pExpander, pButton);
		x += ui_WidgetGetWidth(UI_WIDGET(pButton)) + 5;
	}
	...
	ui_ExpanderSetHeight(MODE_pExpander, y);
}

static void ACECreateMoveGraphsExpander(AnimChartDoc* pDoc)
{
	assert(!pDoc->pMoveGraphsExpander);

	if (!pDoc->pObject->bIsSubChart) {
		pDoc->pMoveGraphsExpander = ACECreateExpander(pDoc->pExpanderGroup, "Movement Sequencer");
		ACECreateGenericGraphsExpander(pDoc, 1);
	}
}
...
static void ACECreateGraphsExpander(AnimChartDoc* pDoc)
{
	...
	pDoc->pGraphsExpander = ACECreateExpander(pDoc->pExpanderGroup, "Default Sequencer");
	ACECreateGenericGraphsExpander(pDoc, 0);
}
...
static void ACECreateSubChartExpander(AnimChartDoc* pDoc)
{
	...
	FOR_EACH_IN_EARRAY_FORWARDS(pDoc->pObject->eaSubCharts, DynAnimSubChartRef, pSubChartRef)
	{
		...
		if(pChart) {
			...
			// there isn't a need to show the movegraph info here since sub-charts can only include graphs for the default sequencer and not the movement sequencer
		}
	}
	FOR_EACH_END;
...
static void ACECreateMovesExpander(AnimChartDoc* pDoc)
{
	...
	assert(!gConf.bUseMovementGraphs && !pDoc->pMovesExpander);
	...
	FOR_EACH_IN_EARRAY_FORWARDS(pDoc->pObject->eaMoveRefs, DynAnimChartMoveRefLoadTime, pRef)
	{
		...
		else if (eaSize(&pRef->eaMoveChances) == 1)
		{
			...
			pButton = ui_ButtonCreate(pcButtonText, local_x+31, local_y, ACEChooseMove, ACE_IDX2_TO_PTR(ipRefIndex,0));
			if (pcButtonText && aceHighlightText && strstri(pcButtonText,aceHighlightText))
				ui_WidgetSkin(UI_WIDGET(pButton), &aceHighlightSkin);
			ui_WidgetSetTooltipString(UI_WIDGET(pButton), pcButtonText);
			ui_WidgetSetWidth(UI_WIDGET(pButton), 125);
...
static void ACERefreshMoveGraphsExpander(AnimChartDoc* pDoc)
{
	bool bWasOpen = pDoc->pMoveGraphsExpander ? ui_ExpanderIsOpened(pDoc->pMoveGraphsExpander) : true;

	eaDestroyEx(&pDoc->eaMoveGraphRefFields, ACEMoveGraphRefFieldDestroy);
	ui_ExpanderGroupRemoveExpander(pDoc->pExpanderGroup, pDoc->pMoveGraphsExpander);
	ui_WidgetForceQueueFree(UI_WIDGET(pDoc->pMoveGraphsExpander));
	pDoc->pMoveGraphsExpander = NULL;
	ACECreateMoveGraphsExpander(pDoc);
	if(pDoc->pMoveGraphsExpander)
	{
		ANALYSIS_ASSUME(pDoc->pMoveGraphsExpander != NULL);
		ui_ExpanderSetOpened(pDoc->pMoveGraphsExpander, bWasOpen);
	}
}
...
void ACERevertAnimChart(AnimChartDoc* pDoc)
{
	...
	if (pChart) {
		...
		// BUG!! We Get lots of calls to here when reloading data from the server!
		// This messing this up if the user starts to choose a move, the chart reverts, then the user finishes choosing their move!!
		pDoc->pObject = StructCloneFields(parse_DynAnimChartLoadTime, pChart);
		ACEChartPostOpenFixup(pDoc->pObject);
		pDoc->pOrigObject = StructCloneFields(parse_DynAnimChartLoadTime, pDoc->pObject);
