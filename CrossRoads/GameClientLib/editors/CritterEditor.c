	...
	else
	{
		if (pCritterDef->oldInteractProps.interactGameActions)
		{
			...
			for (i = 0; i < n; i++)
			{
				...
				{
					Message *pMessage = pAction->pSendNotificationProperties->notifyMsg.msg.pEditorCopy;
					if (pMessage)
					{
						if (resExtractNameSpace(pCritterDef->pchName,nameSpace, baseObjectName))
						{
...
static void ce_initCritterCostumes(METable *pTable)
{
	...
	METableAddSimpleSubColumn(pTable, id, "Asset Set",  "AssetSetIndex", 75, CE_SUBGROUP_CHANCE, kMEFieldType_TextEntry);
