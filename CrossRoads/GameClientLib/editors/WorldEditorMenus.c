...
static void editModelOpenSrcDirectory(UIMenuItem *item, Model *model)
{
	emuOpenContainingDirectoryEx(model->header->filename, SRC);
}

static void editModelOpenDataDirectory(UIMenuItem *item, Model *model)
{
	emuOpenContainingDirectoryEx(model->header->filename, DATA);
}
...
static void editMaterialOpenDataDirectory(UIMenuItem *item, MaterialData *materialData)
{
	emuOpenContainingDirectoryEx(materialData->filename, DATA);
}
...
static void editTextureOpenSrcDirectory(UIMenuItem *item, BasicTexture *texture)
{
	char texFileName[CRYPTIC_MAX_PATH];
	sprintf(texFileName, "%s/%s", fileSrcDir(), texFindFullPath(texture));
	changeFileExt(texFileName, ".tga", texFileName);
	emuOpenContainingDirectoryEx(texFileName, SRC);
}

static void editTextureOpenDataDirectory(UIMenuItem *item, BasicTexture *texture)
{
	char texFileName[CRYPTIC_MAX_PATH];
	sprintf(texFileName, "%s/%s", fileSrcDir(), texFindFullPath(texture));
	changeFileExt(texFileName, ".tga", texFileName);
	emuOpenContainingDirectoryEx(texFileName, DATA);
}
...
void wleTrackerContextMenuCreateForModel(Model *model, UIMenuItem ***outItems)
{
	...
	sprintf(itemText, "Open src \"%s\"...", model->header->filename);
	d = NULL;
	if (c = strrchr(itemText, '/'))
	{
		...
		*c = '/';
		if (d)
		{
			sprintf(itemText, "Open src \"%s", d+1);
		}
	}
	item = ui_MenuItemCreate(itemText, UIMenuCallback, editModelOpenSrcDirectory, model, NULL);
	ui_MenuAppendItem(openSubMenu, item);

	sprintf(itemText, "Open data \"%s\"...", model->header->filename);
	d = NULL;
	if (c = strrchr(itemText, '/'))
	{
		*c = '\0';
		...
		*c = '/';
		if (d)
		{
			sprintf(itemText, "Open data \"%s", d+1);
		}
	}
	item = ui_MenuItemCreate(itemText, UIMenuCallback, editModelOpenDataDirectory, model, NULL);
	ui_MenuAppendItem(openSubMenu, item);
...
void wleTrackerContextMenuCreateForMaterial(const Material *material, UIMenuItem ***outItems)
{
	...
	sprintf(itemText, "Open data \"%s\"...", materialData->filename);
	d = NULL;
	if (c = strrchr(itemText, '/'))
	{
		*c = '\0';
	...
	item = ui_MenuItemCreate(itemText, UIMenuCallback, editMaterialOpenDataDirectory, (void*) materialData, NULL);
	ui_MenuAppendItem(openSubMenu, item);
...
void wleTrackerContextMenuCreateForTexture(BasicTexture *texture, UIMenuItem ***outItems)
{
	...
	sprintf(itemText, "Open src \"%s\"...", texture->fullname);
	d = NULL;
	if (c = strrchr(itemText, '/'))
	{
		*c = '\0';
	...
	item = ui_MenuItemCreate(itemText, UIMenuCallback, editTextureOpenSrcDirectory, texture, NULL);
	ui_MenuAppendItem(openSubMenu, item);

	sprintf(itemText, "Open data \"%s\"...", texture->fullname);
	d = NULL;
	if (c = strrchr(itemText, '/'))
	{
		*c = '\0';
	...
	item = ui_MenuItemCreate(itemText, UIMenuCallback, editTextureOpenDataDirectory, texture, NULL);
	ui_MenuAppendItem(openSubMenu, item);
