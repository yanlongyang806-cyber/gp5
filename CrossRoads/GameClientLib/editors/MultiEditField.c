...
static int mef_textFieldUpdateRefStructArray(MEField *pField, UIWidget *pWidget, const char *pchText, bool bFinished)
{
	...
	// Cull out any entries that were in our list but are not anymore.
	for (i = eaSize(peaTarget) - 1; i >= 0; i--)
	{
		char *achCompareCopy = estrCreateFromStr(achCompare);
		pchEntry = achCompareCopy;
		pchFind = strchr(achCompareCopy, ' ');
		bFoundRef = false;
		...
		while (pchFind) {
			...
			if (REF_COMPARE_HANDLES((*peaTarget)[i]->hRef, pRefStruct->hRef))
			{
				bFoundRef = true;
			}

			StructDestroyVoid(pStructTable, pRefStruct);
		...
		if (!bFoundRef && pchEntry && pchEntry[0] != '\0') {
		...
		if (!bFoundRef)
			eaRemove(peaTarget, i);

		estrDestroy(&achCompareCopy);
	}
	// End of culling of old, non-existant entries
	...
	// End of addition of new entries
