...
static void *ie_createObject(METable *pTable, ItemDef *pObjectToClone, char *pcNewName, bool bCloneKeepsKeys)
{
	...
	const char *pcEventTooltip = NULL;
	...
	if (pObjectToClone) {
		...
		pcEventTooltip = pObjectToClone->eventTooltipMsg.pEditorCopy ? pObjectToClone->eventTooltipMsg.pEditorCopy->pcDefaultString : NULL;
	...
	// Create event tooltip message
	pMessage = langCreateMessage("", "", "", pcEventTooltip);
	pNewDef->eventTooltipMsg.pEditorCopy = pMessage;
...
static void ie_typeChangeCallback(METable *pTable, ItemDef *pItemDef, void *pUserData, bool bInitNotify)
{
	...
	METableSetFieldNotApplicable(pTable, pItemDef, "Prompt Reward Choice", (pItemDef->eType!=kItemType_RewardPack));
	METableSetFieldNotApplicable(pTable, pItemDef, "Allow Scaling Reward Items", (pItemDef->eType!=kItemType_RewardPack));
	...
	METableSetFieldNotApplicable(pTable, pItemDef, "Item Progression Def", pItemDef->eType!=kItemType_Gem && pItemDef->eType!=kItemType_Device);
	METableSetFieldNotApplicable(pTable, pItemDef, "Progression Evo Added Success Chance", pItemDef->eType!=kItemType_UpgradeModifier);
	METableSetFieldNotApplicable(pTable, pItemDef, "Progression Evo Catalyst Insurance", pItemDef->eType!=kItemType_UpgradeModifier);
	...
	METableSetFieldNotApplicable(pTable, pItemDef, "Progression Food XP", pItemDef->eType!=kItemType_Gem && pItemDef->eType!=kItemType_Device && pItemDef->eType!=kItemType_ProgressionFood);
...
//		METableSetFieldNotApplicable(pTable, pItemDef, "StackLimit", 1);
	...
	case kItemType_ProgressionFood:
		{
			METableSetFieldNotApplicable(pTable, pItemDef, "Flags", 0);
			METableSetFieldNotApplicable(pTable, pItemDef, "Description", 0);
			METableSetFieldNotApplicable(pTable, pItemDef, "Icon", 0);
...
static void ie_initItemColumns(METable *pTable)
{
	...
	METableAddSimpleColumn(pTable,   "Event-based Tooltip",  ".eventTooltipMsg.EditorCopy", 160, IE_GROUP_DISPLAY, kMEFieldType_Message);
	METableAddSimpleColumn(pTable,	"Event Tooltip Event",		"TooltipEvent",		120, IE_GROUP_DISPLAY, kMEFieldType_TextEntry);
	...
	METableAddGlobalDictColumn(pTable,	"Added Costume FX",		"AddedCostumeFX",		120, IE_GROUP_DISPLAY, kMEFieldType_ValidatedTextEntry, "DynFXInfo", "resourceName");
	METableAddSimpleColumn(pTable,	"Added Costume Bone",		"AddedCostumeBone",		120, IE_GROUP_DISPLAY, kMEFieldType_TextEntry);
	...
	METableAddGlobalDictColumn(pTable, "Require All Allegiances", ".Restriction.RequireAllAllegiances", 100, IE_GROUP_RESTRICTION, kMEFieldType_BooleanCombo, "Allegiance", "resourceName");
	...
	METableAddSimpleColumn(pTable, "Prompt Reward Choice", ".RewardPackInfo.PromptRewardChoices",100, IE_GROUP_VALUES, kMEFieldType_BooleanCombo);
	METableAddSimpleColumn(pTable, "Allow Scaling Reward Items", ".RewardPackInfo.CanScaleRewards",100,IE_GROUP_VALUES, kMEFieldType_BooleanCombo);
	...
	METableAddSimpleColumn(pTable, "ItemSet Members Use Categories", "ItemSetMembersUseCategories", 100, IE_GROUP_VALUES, kMEFieldType_BooleanCombo);
	...
	METableAddSimpleColumn(pTable, "No Transmutation Cost", "TransmutationHasNoCost", 170, IE_GROUP_VALUES, kMEFieldType_BooleanCombo);
	METableAddSimpleColumn(pTable, "Transmutation Enabled", "TransmutationEnabled", 170, IE_GROUP_VALUES, kMEFieldType_BooleanCombo);

	//Item Progression info
	METableAddGlobalDictColumn(pTable, "Item Progression Def", "ProgressionDef", 160, IE_GROUP_VALUES, kMEFieldType_ValidatedTextEntry, "ItemProgressionDef", "resourceName");
	METableAddSimpleColumn(pTable, "Progression Food XP", "uProgressionFoodXP", 170, IE_GROUP_VALUES, kMEFieldType_TextEntry);
	METableAddSimpleColumn(pTable, "Progression Evo Added Success Chance", "uProgressionEvoSuccessBonus", 200, IE_GROUP_VALUES, kMEFieldType_TextEntry);
	METableAddSimpleColumn(pTable, "Progression Evo Catalyst Insurance", "ProgressionEvoInsurance", 200, IE_GROUP_VALUES, kMEFieldType_BooleanCombo);
	estrDestroy(&pcPath);
}
...
static void ie_initGemHolderColumns(METable *pTable)
{
	...
	METableAddSimpleSubColumn(pTable, id, "Req Min Item Level", "MinLevel", 100, IE_GROUP_RESTRICTION, kMEFieldType_TextEntry);
	METableAddSimpleSubColumn(pTable, id, "Req Max Item Level", "MaxLevel", 100, IE_GROUP_RESTRICTION, kMEFieldType_TextEntry);
}
