...
typedef struct WleAEVolumeUI
{
	...
	struct
	{
		...
		WleAEParamExpression landmarkVisibleCond;
		WleAEParamBool landmarkScaleToArea;
		WleAEParamInt landmarkZOrder;
...
int wleAEVolumeReload(EMPanel *panel, EditorObject *edObj)
{
	...
	wleAEExpressionUpdate(&wleAEGlobalVolumeUI.data.landmarkVisibleCond);
	wleAEBoolUpdate(&wleAEGlobalVolumeUI.data.landmarkScaleToArea);
	wleAEIntUpdate(&wleAEGlobalVolumeUI.data.landmarkZOrder);
		...
		if (isRoom || stricmp(wleAEGlobalVolumeUI.data.volumeStyle.stringvalue, "Yes")==0)
		{
			...
			if (!isRoom)
			{
				...
				if (wleAEGlobalVolumeUI.data.isLandmark.boolvalue)
				{
					...
					wleAEExpressionAddWidget(wleAEGlobalVolumeUI.autoWidget, "Visible Condition", "If expression is blank or evaluates to true, this landmark is visible to the player", "VisibleCondition", &wleAEGlobalVolumeUI.data.landmarkVisibleCond);
					wleAEBoolAddWidget(wleAEGlobalVolumeUI.autoWidget, "Scale to Volume", "Scale the landmark texture to the area of the volume instead of remaining at a fixed size regardless of map zoom level", "landmarkScaleToArea", &wleAEGlobalVolumeUI.data.landmarkScaleToArea);
					wleAEIntAddWidget(wleAEGlobalVolumeUI.autoWidget, "Z Order", "Override the Z-Order of the landmark icon, a positive value increase the draw order, a negative value decreases the draw order", "landmarkZOrder", &wleAEGlobalVolumeUI.data.landmarkZOrder, -10, 10, 1);
				}
			}
...
void wleAEVolumeCreate(EMPanel *panel)
{
	...
	wleAEVolumeSetupStructParam(wleAEGlobalVolumeUI.data.landmarkVisibleCond, server_volume.landmark_volume_properties, parse_WorldLandmarkVolumeProperties, "VisibleCondition");
	wleAEGlobalVolumeUI.data.landmarkVisibleCond.entry_width = 1.0;
	wleAEGlobalVolumeUI.data.landmarkVisibleCond.left_pad += 20;
	wleAEGlobalVolumeUI.data.landmarkVisibleCond.context = exprContextCreate();
	wleAEVolumeSetupStructParam(wleAEGlobalVolumeUI.data.landmarkScaleToArea, server_volume.landmark_volume_properties, parse_WorldLandmarkVolumeProperties, "ScaleToArea");
	wleAEGlobalVolumeUI.data.landmarkScaleToArea.left_pad += 20;
	wleAEVolumeSetupStructParam(wleAEGlobalVolumeUI.data.landmarkZOrder, server_volume.landmark_volume_properties, parse_WorldLandmarkVolumeProperties, "ZOrder");
	wleAEGlobalVolumeUI.data.landmarkZOrder.left_pad += 20;
