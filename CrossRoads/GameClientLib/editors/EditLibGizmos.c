...
******/
int findPlaneLineIntersection(Vec3 pointOnPlane, Vec3 planeNormal,Vec3 p1,Vec3 p2,Vec3 intersection)
{
	Vec3 p3mp1,p2mp1;
...
RotateGizmo *RotateGizmoCreate(void)
{
	...
	newGizmo->enabledFreeRotation = true;
 ...
 * rotation.
 */
bool RotateGizmoHandleInput(RotateGizmo* rotGizmo)
{
	...
	if (rotGizmo->rotationAxis == -1)
	{
		...
		if(!rotGizmo->enabledFreeRotation)
			return false;
	...
	else
	{
		...
		if( !rotGizmo->enabledAxis[ rotGizmo->rotationAxis ])
			return false;
...
void RotateGizmoUpdate(RotateGizmo* rotGizmo)
{
	...
	if (mouseDown(MS_LEFT) && !rotGizmo->activated)
	{
		if (swallow_input && RotateGizmoHandleInput(rotGizmo))
		{
	...
	if( rotGizmo->enabledFreeRotation ) {
		rotGizmo->mousedOver = swallow_input;
	} else {
		rotGizmo->mousedOver = rotGizmo->rotationAxis >= 0;
	}
}
...
bool RotateGizmoIsHover(RotateGizmo* rotGizmo)
{
	return rotGizmo->mousedOver;
}

float RotateGizmoCursorAngle(RotateGizmo* rotGizmo)
{
	int mouse[ 2 ];
	Vec2 screenPos;
	editLibGetScreenPos( rotGizmo->activeMatrix[ 3 ], screenPos );
	mousePos( &mouse[ 0 ], &mouse[ 1 ]);
	return atan2( mouse[ 0 ] - screenPos[ 0 ], screenPos[ 1 ] - mouse[ 1 ]);
}
...
void RotateGizmoEnableAxis(RotateGizmo* rotGizmo, bool pitchEnabled, bool yawEnabled, bool rollEnabled, bool freeRotateEnabled)
{
	rotGizmo->enabledAxis[ 0 ] = pitchEnabled;
	rotGizmo->enabledAxis[ 1 ] = yawEnabled;
	rotGizmo->enabledAxis[ 2 ] = rollEnabled;
	rotGizmo->enabledFreeRotation = freeRotateEnabled;
}
...
void RotateGizmoForceActive(RotateGizmo *rotGizmo, bool active)
{
	rotGizmo->activated = active;
}
...
void TranslateGizmoUpdate(TranslateGizmo* transGizmo)
{
	...
	if (mouseDown(MS_LEFT) && !transGizmo->activated)
	{
...
bool TranslateGizmoIsHover(TranslateGizmo* transGizmo)
{
	return transGizmo->mousedOver;
}

void TranslateGizmoForceActive( TranslateGizmo* transGizmo, bool active )
{
	if( active ) {
		Vec3 start;
		Vec3 end;
		...
		// TranslateGizmoGetIntersection depends on the draw matrix
		// being up to date.
		TranslateGizmoSetDrawMat( transGizmo );
		TranslateGizmoGetIntersection( transGizmo, start, end, transGizmo->startIntersect );
		transGizmo->activated = true;
...
/*             SLIDE GIZMO               */
/*****************************************/
const float SLIDE_GIZMO_CENTER_LINE_SCALE = 10;
const float SLIDE_GIZMO_CIRCLE_SCALE = 0.03;

// MJF Jun/19/2013 -- This gizmo has a lot less features than the
// other gizmos, because as of this writing the only user of this
// gizmo is the UGCPlayingEditor.
//
// Feel free to finish up this gizmo.
typedef struct SlideGizmo
{
	// data
	Mat4 startMat;							// original matrix before translation
	Mat4 activeMat;							// holds correctly-transformed location of the gizmo
	...
	// drawing
	Vec3 activeNormal;						// normal at the current point being dragged over
...
SlideGizmo *SlideGizmoCreate( void )
{
	SlideGizmo* newGizmo = calloc( 1, sizeof( SlideGizmo ));
	return newGizmo;
}

void SlideGizmoDestroy( SlideGizmo* slideGizmo )
{
	free( slideGizmo );
}

void SlideGizmoApplyChanges( SlideGizmo* slideGizmo, Mat4 cam, Vec3 start, Vec3 end )
{
	Mat4 origMat;
	Vec3 origNormal;

	copyMat4( slideGizmo->activeMat, origMat );
	copyVec3( slideGizmo->activeNormal, origNormal );
	// ultimately, we need to calculate the updated matrix and store it
	// if a snap is enabled, apply a snap
	{
		// suspend motion of the object if camera is rotating
		if (!gfxIsActiveCameraControllerRotating())
		{
			...
			if( wcRayCollide( worldGetActiveColl( PARTITION_CLIENT ), start, end, WC_QUERY_BITS_WORLD_ALL, &results )) {
				copyVec3( results.posWorldImpact, slideGizmo->activeMat[ 3 ]);
				copyVec3( results.normalWorld, slideGizmo->activeNormal );
			}
		}
	}

	// if matrix goes out of bounds, reset it to old location
	if( checkMatOutOfBounds( slideGizmo->activeMat )) {
		copyMat4( origMat, slideGizmo->activeMat );
		copyVec3( origNormal, slideGizmo->activeNormal );
	}
}

void SlideGizmoUpdate( SlideGizmo* slideGizmo )
{
	bool mousedOver = false;
	Vec3 start;
	Vec3 end;
	...
	// HANDLE INPUTS
	// check closeness of the mouse to the object
	if( !slideGizmo->activated ) {
		float camDist = distance3( cam[ 3 ], slideGizmo->startMat[ 3 ]);
		float centerLineLength = SLIDE_GIZMO_CENTER_LINE_SCALE * camDist;
		float circleRadius = SLIDE_GIZMO_CIRCLE_SCALE * camDist;
		if( findPointLineIntersection( start, end, slideGizmo->activeMat[ 3 ], NULL ) < circleRadius ) {
			mousedOver = true;
		}
	}

	// determine whether to begin/end sliding by activating/deactivating the gizmo
	if( mouseDown( MS_LEFT ) && !slideGizmo->activated ) {
		if( mousedOver ) {
			slideGizmo->activated = true;
			if( slideGizmo->activateFunc ) {
				slideGizmo->activateFunc( slideGizmo->activeMat, NULL );
			}
		}
	} else if( !mouseIsDown( MS_LEFT ) && slideGizmo->activated ) {
		slideGizmo->activated = false;
		copyMat4( slideGizmo->activeMat, slideGizmo->startMat );
		if( slideGizmo->deactivateFunc ) {
			slideGizmo->deactivateFunc( slideGizmo->activeMat, NULL );
		}
		inpHandled();
	}
	...
	// APPLY CHANGES
	if( slideGizmo->activated ) {
		SlideGizmoApplyChanges( slideGizmo, cam, start, end );
		ui_SoftwareCursorThisFrame();
	}

	// SET DRAW PARAMETERS
	slideGizmo->mousedOver = mousedOver;
}

void SlideGizmoDraw( SlideGizmo* slideGizmo )
{
	const float axisTailBoxScale = 0.5;
	const float axisOriginBoxScale = 0.2;
	Mat4 cam;
	...
	// draw the activated gizmo (not currently implemented)
	if( slideGizmo->activated && false ) {
	}
	...
	// draw gizmo normally
		...
		float centerLineLength = SLIDE_GIZMO_CENTER_LINE_SCALE * camDist;
		float circleRadius = SLIDE_GIZMO_CIRCLE_SCALE * camDist;
		int color = (slideGizmo->mousedOver ? ARGBFromColor( ColorYellow ) : axisColor( 1 ));
		int it;

		// draw the center line
		{
			Vec3 centerLineTop;
			scaleVec3( upvec, centerLineLength, centerLineTop );
			addVec3( centerLineTop, slideGizmo->activeMat[ 3 ], centerLineTop );

			gfxSetPrimZTest( 0 );
			gfxDrawLine3DWidthARGB( slideGizmo->activeMat[ 3 ], centerLineTop, color, color, 2 );
			gfxSetPrimZTest( 1 );
		}
		...
		// draw the circle lines
		{
			Vec3 axis1;
			Vec3 axis2;
			crossVec3( cam[ 0 ], slideGizmo->activeNormal, axis2 );
			crossVec3( slideGizmo->activeNormal, axis2, axis1 );
			normalVec3( axis1 );
			scaleVec3( axis1, circleRadius, axis1 );
			normalVec3( axis2 );
			scaleVec3( axis2, circleRadius, axis2 );

			gfxSetPrimZTest( 0 );
			for( it = 0; it != 180; ++it ) {
				float angleStart = it / 180.0f * TWOPI;
				float angleEnd = (it + 1) / 180.0f * TWOPI;
				Vec3 lineStart;
				Vec3 lineEnd;

				lineStart[ 0 ] = slideGizmo->activeMat[ 3 ][ 0 ] + cosf( angleStart ) * axis1[ 0 ] + sin( angleStart ) * axis2[ 0 ];
				lineStart[ 1 ] = slideGizmo->activeMat[ 3 ][ 1 ] + cosf( angleStart ) * axis1[ 1 ] + sin( angleStart ) * axis2[ 1 ];
				lineStart[ 2 ] = slideGizmo->activeMat[ 3 ][ 2 ] + cosf( angleStart ) * axis1[ 2 ] + sin( angleStart ) * axis2[ 2 ];

				lineEnd[ 0 ] = slideGizmo->activeMat[ 3 ][ 0 ] + cosf( angleEnd ) * axis1[ 0 ] + sin( angleEnd ) * axis2[ 0 ];
				lineEnd[ 1 ] = slideGizmo->activeMat[ 3 ][ 1 ] + cosf( angleEnd ) * axis1[ 1 ] + sin( angleEnd ) * axis2[ 1 ];
				lineEnd[ 2 ] = slideGizmo->activeMat[ 3 ][ 2 ] + cosf( angleEnd ) * axis1[ 2 ] + sin( angleEnd ) * axis2[ 2 ];

				gfxDrawLine3DWidthARGB( lineStart, lineEnd, color, color, 2 );
			}
			gfxSetPrimZTest( 1 );
		}
	}
...
void SlideGizmoSetActivateCallback( SlideGizmo* slideGizmo, GizmoActivateFn activateFunc )
{
	slideGizmo->activateFunc = activateFunc;
}

void SlideGizmoSetDeactivateCallback( SlideGizmo* slideGizmo, GizmoDeactivateFn deactivateFunc )
{
	slideGizmo->deactivateFunc = deactivateFunc;
}

void SlideGizmoSetMatrix( SlideGizmo* slideGizmo, const Mat4 mat )
{
	copyMat4( mat, slideGizmo->startMat );
	copyMat4( mat, slideGizmo->activeMat );
}

void SlideGizmoGetMatrix( SlideGizmo* slideGizmo, Mat4 mat )
{
	copyMat4( slideGizmo->activeMat, mat );
}

bool SlideGizmoIsActive( SlideGizmo* slideGizmo )
{
	return slideGizmo->activated;
}

bool SlideGizmoIsHover( SlideGizmo* slideGizmo )
{
	return slideGizmo->mousedOver;
}

void SlideGizmoForceActive( SlideGizmo* slideGizmo, bool active )
{
	slideGizmo->activated = active;
}
...
void ScaleMinMaxGizmoUpdate(ScaleMinMaxGizmo* scaleGizmo)
{
	...
	if (mouseDown(MS_LEFT) && !scaleGizmo->activated)
	{
		if (overAxis)
		{
