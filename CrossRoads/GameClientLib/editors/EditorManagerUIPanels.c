...
void emPanelsInit(void)
{
	...
	if (!em_data.file_manager.panel)
	{
		...
		list_column = ui_ListColumnCreate(UIListTextCallback, "Filename", (intptr_t) emFileManagerDrawText, NULL);
		ui_ListColumnSetSortable(list_column, true);
		ui_ListAppendColumn(em_data.file_manager.file_list, list_column);
...
{
	int i, j;
	StashTable tab_stash = stashTableCreateWithStringKeys(16, StashDeepCopyKeys_NeverRelease);
