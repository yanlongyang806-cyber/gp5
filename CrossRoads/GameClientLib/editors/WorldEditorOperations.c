...
{
	wl_state.editor_UI_needs_update = true;
}

void wleOpRefreshUIActual(void)
{
	if (!wl_state.editor_UI_needs_update)
	{
		return;
	}
	...
	wl_state.editor_UI_needs_update = false;
}
...
void wleOpReplaceEach(const TrackerHandle **parentHandles, TrackerHandle **handles, GroupDef **defs, Mat4 **mats, TrackerHandle ***newHandles, bool performSelection)
{
	...
	for (i = 0; i < eaSize(&handles); i++)
	{
		...
		if ((tracker->parent) && (!groupIsPublic(tracker->parent->def) && !editState.editOriginal))
		{
...
static bool wleOpSetRegionMaxNonTeamPetsFinish(ZoneMapInfo *zminfo, bool success, WleOpSetRegionAsyncData *data)
{
	WorldRegion *region = zmapGetWorldRegionByNameEx(NULL, data->regionName, false);

	if (success && region)
	{
		zmapRegionSetUnteamedPetsPerPlayer(NULL, region, data->maxTeamPets);
...
void wleOpSetRegionMaxNonTeamPets(SA_PARAM_NN_VALID WorldRegion *region, int maxTeamPets)
{
	WleOpSetRegionAsyncData *data = StructCreate(parse_WleOpSetRegionAsyncData);
	data->regionName = StructAllocString(worldRegionGetRegionName(region));
	data->maxTeamPets = maxTeamPets;

	wleOpLockZoneMap(wleOpSetRegionMaxNonTeamPetsFinish, data);
}
...
bool wleOpSetMapUsedInUGCFinish(ZoneMapInfo *zminfo, bool success, void * eUsedInUGC)
{
	if (success)
	{
		zmapInfoSetMapUsedInUGC(zminfo, (ZoneMapUGCUsage)eUsedInUGC);
...
void wleOpSetMapUsedInUGC(ZoneMapUGCUsage eUsedInUGC)
{
#pragma warning(suppress:4306) // 'type cast' : conversion from 'bool' to 'void *' of greater size
	wleOpLockZoneMap(wleOpSetMapUsedInUGCFinish, (void *)eUsedInUGC);
}
...
bool IsTrackerCollided(GroupTracker *tracker, const WorldCollCollideResults *results)
{
	int i;
	for (i = 0; i < eaSize(&tracker->cell_entries); i++)
	{
		if (tracker->cell_entries[i]->type == WCENT_COLLISION)
		{
			if (eaFind(&(((WorldCollisionEntry*)(tracker->cell_entries[i]))->eaCollObjects), results->wco) != -1)
			{
				return true;
			}
		}
		else if (tracker->cell_entries[i]->type == WCENT_VOLUME)
		{
			if (eaFind(&(((WorldVolumeEntry*)(tracker->cell_entries[i]))->eaCollObjects), results->wco) != -1)
			{
				return true;
			}
		}
	...
	for (i = 0; i < tracker->child_count; i++)
	{
		if (IsTrackerCollided(tracker->children[i], results))
		{
			return true;
		}
	}
...
S32 SnapDownRayCastCallback(void *userPointer, const WorldCollCollideResults *results)
{
	TrackerHandle *parentHandle = NULL;
	GroupTracker *parentTracker = NULL, *tracker = (GroupTracker*)userPointer;
	Mat4 inverse, parent, parentInverse, result, world;
	EditUndoStack *stack = NULL;
	...
	if (IsTrackerCollided(tracker, results))
	{
		return 1;
	}
...
void wleOpSnapDown(const TrackerHandle *handle)
{
...

	{
		...
		if(region)
		{
			...
			wcRayCollideMultiResult(worldGetActiveColl(PARTITION_CLIENT), vStart, vEnd, WC_QUERY_BITS_WORLD_ALL, SnapDownRayCastCallback, tracker, NULL);
		}
	}
