...
void GEFreeActionGroup(GEActionGroup *pGroup)
{
	...
	ui_WidgetQueueFreeAndNull(&pGroup->pLabelSoundOverride);
	...
	ui_WidgetQueueFreeAndNull(&pGroup->pSinglePlayer);
	...
	MEFieldSafeDestroy(&pGroup->pNotifySoundOverrideField);
...
static F32 GEUpdateSendNotificationAction(GEActionGroup *pGroup, WorldGameActionProperties *pAction, WorldGameActionProperties *pOrigAction, int index, F32 y)
{
	...
	// Message
	pDispMessage = &pAction->pSendNotificationProperties->notifyMsg.msg;
		
	if (pOrigAction && (pOrigAction->eActionType == pAction->eActionType)) {
		pOrigDispMessage = &pOrigAction->pSendNotificationProperties->notifyMsg.msg;
	}
	if (pDispMessage) {
		if (!pGroup->pMessageField) {
			pGroup->pMessageField = MEFieldCreateSimple(kMEFieldType_Message, pOrigDispMessage, pDispMessage, parse_DisplayMessage, "EditorCopy");
	...
	// The label
	pGroup->pLabel2 = GERefreshLabel(pGroup->pLabel2, "Needs VO", NULL, X_OFFSET_INDENT, 0, y, pGroup->pWidgetParent);

	if (!pGroup->pNotifySoundField) {
		pGroup->pNotifySoundField = MEFieldCreateSimple(
				kMEFieldType_BooleanCombo,
				SAFE_MEMBER_ADDR2( pOrigAction, pSendNotificationProperties, notifyMsg ),
				&pAction->pSendNotificationProperties->notifyMsg,
				parse_DisplayMessageWithVO,
				"HasVO");
		...
		MEFieldSetAndRefreshFromData(pGroup->pNotifySoundField,
									 SAFE_MEMBER_ADDR2( pOrigAction, pSendNotificationProperties, notifyMsg ),
									 &pAction->pSendNotificationProperties->notifyMsg);
	}
	...
	// The label
	pGroup->pLabelSoundOverride = GERefreshLabel(pGroup->pLabelSoundOverride, "Sound Override", NULL, X_OFFSET_INDENT, 0, y, pGroup->pWidgetParent);

	if (!pGroup->pNotifySoundOverrideField) {
		pGroup->pNotifySoundOverrideField = MEFieldCreateSimpleDataProvided(kMEFieldType_ValidatedTextEntry, pOrigAction ? pOrigAction->pSendNotificationProperties : NULL, pAction->pSendNotificationProperties, parse_WorldSendNotificationActionProperties, "SoundOverride", NULL, sndGetEventListStatic(), NULL);
		GEGameActionAddFieldToParent(pGroup->pNotifySoundOverrideField, pGroup->pWidgetParent, X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 3, pGroup);
	} else {
		ui_WidgetSetPosition(pGroup->pNotifySoundOverrideField->pUIWidget, X_OFFSET_CONTROL, y);
		MEFieldSetAndRefreshFromData(pGroup->pNotifySoundOverrideField,
									 SAFE_MEMBER( pOrigAction, pSendNotificationProperties ),
									 pAction->pSendNotificationProperties);
	}
	...
	if(eType == kNotifyType_MiniContact || eType == kNotifyType_MiniContact_Right)
	{
		WorldGameActionHeadshotProperties *pHeadshot = NULL;
		WorldGameActionHeadshotProperties *pOrigHeadshot = NULL;
	...
	if (eType == kNotifyType_Tip_General) {
		...
		if (pGroup->pNotifyLogicalStringField && pGroup->pNotifyLogicalStringField->eType == kMEFieldType_TextEntry) {
			MEFieldSafeDestroy(&pGroup->pNotifyLogicalStringField);
		...
		y += Y_OFFSET_ROW;
	} else if (eType == kNotifyType_RunCommand) {
		pGroup->pLabel9 = GERefreshLabel(pGroup->pLabel9, "Command", NULL, X_OFFSET_INDENT, 0, y, pGroup->pWidgetParent);

		if (pGroup->pNotifyLogicalStringField && pGroup->pNotifyLogicalStringField->eType == kMEFieldType_ValidatedTextEntry) {
			MEFieldSafeDestroy(&pGroup->pNotifyLogicalStringField);
		...
		if (!pGroup->pNotifyLogicalStringField) {
			pGroup->pNotifyLogicalStringField = MEFieldCreateSimple(kMEFieldType_TextEntry, pOrigAction ? pOrigAction->pSendNotificationProperties : NULL, pAction->pSendNotificationProperties, parse_WorldSendNotificationActionProperties, "LogicalString");
			GEGameActionAddFieldToParent(pGroup->pNotifyLogicalStringField, pGroup->pWidgetParent, X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 3, pGroup);
		} else {
			ui_WidgetSetPosition(pGroup->pNotifyLogicalStringField->pUIWidget, X_OFFSET_CONTROL, y);
			MEFieldSetAndRefreshFromData(pGroup->pNotifyLogicalStringField, pOrigAction ? pOrigAction->pSendNotificationProperties : NULL, pAction->pSendNotificationProperties);
...
static void GESinglePlayerCheckToggle(UICheckButton *pButton, GEActionGroup *pGroup) 
{
	WorldGameActionProperties* pProps = (pGroup->peaActions && *pGroup->peaActions) ? (*pGroup->peaActions)[pGroup->index] : NULL;
	if (pProps && pProps->pWarpProperties)
		pProps->pWarpProperties->bSinglePlayer = ui_CheckButtonGetState(pButton);
	pGroup->dataChangedCB( pGroup->pUserData, true ); 
}
...
static F32 GEUpdateWarpAction(GEActionGroup *pGroup, const char *pcSrcZoneMap, WorldGameActionProperties *pAction, WorldGameActionProperties *pOrigAction, int index, F32 y)
{
	...
	if( !pGroup->pSinglePlayer ) {
		pGroup->pSinglePlayer = ui_CheckButtonCreate( X_OFFSET_CONTROL, y, "Single Player", pAction->pWarpProperties->bSinglePlayer );
		ui_WidgetAddChild(pGroup->pWidgetParent, UI_WIDGET(pGroup->pSinglePlayer));
		ui_CheckButtonSetToggledCallback(pGroup->pSinglePlayer, GESinglePlayerCheckToggle, pGroup);
	} else {
		ui_WidgetSetPosition(UI_WIDGET(pGroup->pSinglePlayer), X_OFFSET_CONTROL, y);
	}
...
static F32 GEUpdateGuildStatUpdateAction(GEActionGroup *pGroup, WorldGameActionProperties *pAction, WorldGameActionProperties *pOrigAction, int index, F32 y)
{
	...
	{
		pGroup->pGuildStatNameField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pOrigAction ? pOrigAction->pGuildStatUpdateProperties : NULL, pAction->pGuildStatUpdateProperties, parse_WorldGuildStatUpdateActionProperties, "StatName", "GuildStatDef", "ResourceName");
...
static F32 GEUpdateItemAssignmentAction(GEActionGroup *pGroup, WorldGameActionProperties *pAction, WorldGameActionProperties *pOrigAction, int index, F32 y)
{
	...
	{
		pGroup->pItemAssignmentNameField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pOrigAction ? pOrigAction->pItemAssignmentProperties : NULL, pAction->pItemAssignmentProperties, parse_WorldItemAssignmentActionProperties, "AssignmentName", "ItemAssignmentDef", "ResourceName");
