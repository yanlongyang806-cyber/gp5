...
static void undo_clear_stack(EditUndoStack *stack, int offset)
{
	...
	for (i = offset; i < eaSize(&stack->op_stack);)
	{
		...
		if( stack->op_stack[i]->extra_data ) {
			if( stack->op_stack[i]->extra_pti ) {
				StructDestroyVoid( stack->op_stack[i]->extra_pti, stack->op_stack[i]->extra_data );
			} else {
				free( stack->op_stack[i]->extra_data );
			}
		}
...
void EditUndoSetExtraData(EditUndoStack *stack, ParseTable* pti, void* data)
{
	EditUndoOperation *op = eaGet( &stack->op_stack, stack->current_index );
	if( op ) {
		assert( !op->extra_data );
		op->extra_data = data;
		op->extra_pti = pti;
	}
}

void* EditUndoLastExtraData(EditUndoStack *stack)
{
	EditUndoOperation* op = eaGet( &stack->op_stack, stack->current_index );
	return SAFE_MEMBER( op, extra_data );
}

void* EditRedoLastExtraData(EditUndoStack *stack)
{
	EditUndoOperation* op = eaGet( &stack->op_stack, stack->current_index + 1 );
	return SAFE_MEMBER( op, extra_data );
}
