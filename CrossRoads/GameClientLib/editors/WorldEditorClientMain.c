...
void gfxfont_Printf(float x, float y, float z, float xsc, float ysc, int flags, SpriteProperties *pProps, const char *fmt, ...);
...
static void wleObjectPlaceEx(WleCopyBuffer **buffer, bool noReplace)
{
	...
	if (wleSelectionGetCount() > 0 && !noReplace)
	{
		...
		if (editState.repeatCreateAcrossSelection)
		{
			...
			EditCreateUndoCustom(edObjGetUndoStack(), wleOpUndoFunction, wleOpUndoFunction, wleOpFreeFunction, groupdbTransactionEnd());
			...
			if (!!EditorPrefGetInt(WLE_PREF_EDITOR_NAME, WLE_PREF_CAT_OPTIONS, "ShowContextMenu", 1))
			{
				...
				if (editState.rayCollideInfo.entry && editState.rayCollideInfo.model)
				{
					...
					if (drawable)
					{
						...
						if (drawableList && drawableList->lod_count > 0)
						{
...
AUTO_COMMAND ACMD_CATEGORY(World, Interface) ACMD_NAME("Editor.IncSnap");
void wleCmdIncSnap(void)
{
	switch (editState.transformMode)	
	{
	case EditTranslateGizmo:
		wleCmdIncTransSnap();
		break;
	case EditRotateGizmo:
		wleCmdIncRotSnap();
		break;
	default:
		// Ok to ignore for gizmos without a "snap" property
		break;
	}
}

AUTO_COMMAND ACMD_CATEGORY(World, Interface) ACMD_NAME("Editor.DecSnap");
void wleCmdDecSnap(void)
{
	switch (editState.transformMode)	
	{
	case EditTranslateGizmo:
...
*   drawModel - bool indicating whether to draw any associated models in the group def
******/
static void wleEdObjTrackerDrawRecursive(GroupDef *def, GroupTracker *tracker, bool drawModel)
{
	GroupChild **defChildren;
	Mat4 mat;
	bool drawWireframes = !!EditorPrefGetInt(WLE_PREF_EDITOR_NAME, WLE_PREF_CAT_OPTIONS, "ShowSelectedWireframe", 1);
	int numWireframeSegments = EditorPrefGetInt(WLE_PREF_EDITOR_NAME, WLE_PREF_CAT_OPTIONS, "NumSegments", -1);
	int i;
	...
	if ((drawModel && def->model) || 
		def->property_structs.sound_sphere_properties || 
		groupHasLight(def) || 
		def->property_structs.wind_source_properties)
	{
		trackerGetMat(tracker, mat);
	}

	if (drawModel && def->model)
		wlDrawModel(def->model, mat);
	...
	if (drawWireframes && wlSoundRadiusFunc && def->property_structs.sound_sphere_properties)
	{
		...
		if (name)
			gfxDrawSphere3D(mat[3], wlSoundRadiusFunc(name), numWireframeSegments, ColorWhite, 1);
	...
	if (drawWireframes && groupHasLight(def) )
	{
		...
		gfxDrawLightWireframe(light_data, numWireframeSegments);
	...
	{
		gfxDrawSphere3D(mat[3], def->property_structs.wind_source_properties->radius, numWireframeSegments, ColorWhite, 1);
		
		if (def->property_structs.wind_source_properties->radius_inner)
			gfxDrawSphere3D(mat[3], def->property_structs.wind_source_properties->radius_inner, numWireframeSegments, ColorWhite, 1);
	}
	...
	for (i = 0; i < eaSize(&defChildren); ++i)
	{
		...
		if (child_def)
			wleEdObjTrackerDrawRecursive(child_def, childTracker, drawModel);
	}
}
	...
	for (i = 0; i < eaSize(&selection); i++)
	{
		...
		if (editState.drawGhosts)
		{
			if (editState.inputData.moveCopyKey)
				wleEdObjTrackerDrawRecursive(tracker->def, tracker, false);
...
static void wlePlaceObject(TrackerHandle* parent)
{
...
static void wleUpdateObjectPlacement(void)
{
	int mouseX, mouseY;
			...
			for (i = 0; i < eaSize(editState.quickPlaceState.buffer); i++)
			{
				...
				if (buffer->pti == parse_WleTrackerBuffer)
				{
					...
					if (ghostDef)
					{
						...
						if (tracker)
							wleEdObjTrackerDrawRecursive(tracker->def, tracker, true);
					}
				}
				else if (buffer->pti == parse_WlePatrolPointBuffer)
				{
...
static void wleExportSelectedToVrmlSub(GroupTracker *tracker, Mat4 mat)
{
	if (tracker)
	{
		...
		if (tracker->def)
		{
			...
			{
				mulMat4(mat, tracker->def->children[i]->mat, childMat);
				wleExportSelectedToVrmlSub(tracker->children[i], childMat);
			}
		}
	}
}
