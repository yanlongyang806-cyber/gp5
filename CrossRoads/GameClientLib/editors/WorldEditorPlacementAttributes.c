...
static void wleAEPlacementPosApply(WleAEParamVec3 *param, void *unused, EditorObject **objs)
{
	...
	for (i = 0; i < eaSize(&objs); i++)
	{
		...
		for (j = 0; j < 3; j++)
		{
			if(-g_worldBoundsMaxDist > param->vecvalue[j] || param->vecvalue[j] > g_worldBoundsMaxDist)
				Errorf("Object %s out of playable area.  Place objects' pivots between -15000 and 15000 in each dimension (30000 in space).", objs[i]->name);
		}
	}
