...
static void pe_powerTypeChangeCallback(METable *pTable, PowerDef *pPowerDef, void *pUserData, bool bInitNotify)
{
	...
	METableSetFieldNotApplicable(pTable, pPowerDef, "Deactivates On Disable", pPowerDef->eType!=kPowerType_Toggle);
	METableSetFieldNotApplicable(pTable, pPowerDef, "Effect Area Ignore Source Capsule", 
									!(pPowerDef->eEffectArea==kEffectArea_Cylinder || 
										pPowerDef->eEffectArea==kEffectArea_Cone || 
										pPowerDef->eEffectArea==kEffectArea_Sphere));
...
{
	RegionRules *pRegionRules = regionRules_GetAnyRegionRulesValidForPower(pPowerDef);  // This is really problematic as there may be multiple regionrules 
																					// that support this power and they may have different measurements.
...
static void pe_stackTypeChangeCallback(METable *pTable, PowerDef *pPowerDef, AttribModDef *pModDef, void *pUserData, bool bInitNotify)
{
	if (pModDef->eStack==kStackType_Discard
		...
		|| pModDef->eStack==kStackType_RefreshDuration)
	{
...
static void pe_initPowerColumns(METable *pTable)
{
	...
	METableAddGlobalDictColumn(pTable, "Pre-Activate ApplyPower", "PreActivatePowerDef", 210, PE_GROUP_ACTIVATION, kMEFieldType_ValidatedTextEntry, "PowerDef", "resourceName");
	METableAddSimpleColumn(pTable, "Auto-Cancel Pre-Activate ApplyPower", "CancelPreActivatePower", 120, PE_GROUP_ACTIVATION, kMEFieldType_BooleanCombo);
	METableAddSimpleColumn(pTable, "No Deactivate On Death", "NoDeactivateOnDeath", 120, PE_GROUP_ACTIVATION, kMEFieldType_BooleanCombo);
	...
	METableAddSimpleColumn(pTable, "Effect Area Ignore Source Capsule", "EffectAreaIgnoreSourceCapsule", 140, PE_GROUP_EFFECT, kMEFieldType_BooleanCombo);
	...
	METableAddSimpleColumn(pTable, "Deactivates On Disable","deactivatesPowerOnDisable", 120, PE_GROUP_CHARGE, kMEFieldType_BooleanCombo);
	...
	METableAddExprColumn(pTable, "Hide AutoDesc Expression", "ExprHideAutoDesc", 140, PE_GROUP_UI, NULL);
...
static void pe_initAttribColumns(METable *pTable)
{
	...
	METableAddSimpleSubColumn(pTable, id, "Leave Mod On Deactivate",	"deactivationLeavesMod", 160, PE_SUBGROUP_TIMING, kMEFieldType_BooleanCombo);
	...
	METableAddExprSubColumn(pTable, id,   "Ignores Expression",     "ExprBlockIgnores",     140, PE_SUBGROUP_REQUIRES, combateval_ContextGet(kCombatEvalContext_Affects));
	...
	METableAddSimpleSubColumn(pTable, id, "AutoDesc Order","AutoDescOrder",       100, PE_SUBGROUP_UI, kMEFieldType_TextEntry);
	...
	METableAddSimpleSubColumn(pTable, id, "AutoDesc MagnitudeScale", "AutoDescMagnitudeScale", 100, PE_SUBGROUP_UI, kMEFieldType_TextEntry);
