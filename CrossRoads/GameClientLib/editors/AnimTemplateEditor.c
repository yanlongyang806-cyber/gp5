...
static void ATEFlowChartNodeAddSwitchs(AnimTemplateDoc* pDoc, DynAnimTemplateNode* pNode, UIFlowchartButton*** peaOutputButtons, DynMovementDirection eAllowedDirection, U32 uiShowDirection)
{
	DynAnimGraphNode* pGraphNode = NULL;
	UIFlowchartButton* outputButton;

	if (pDoc->pObject->pDefaultsGraph) {
		FOR_EACH_IN_EARRAY(pDoc->pObject->pDefaultsGraph->eaNodes, DynAnimGraphNode, pTestNode) {
			if (pTestNode->pcName == pNode->pcName) {
				pGraphNode = pTestNode;
			}
		} FOR_EACH_END;
	}
	...
	{
		if (eAllowedDirection == pSwitch->eDirection)
		{
			char label[MAX_PATH];
			F32 fRequiredPlaytime = 0.f;

			if (pGraphNode && ipSwitchIndex < eaSize(&pGraphNode->eaSwitch)) {
				fRequiredPlaytime = pGraphNode->eaSwitch[ipSwitchIndex]->fRequiredPlaytime;
			}
			...
			outputButton = ui_FlowchartButtonCreate(pDoc->pFlowchart, UIFlowchartNormal, ui_LabelCreate(label, 0, 0), &pSwitch->next);
...
static UIFlowchartNode* ATEFlowchartNodeCreateFromDynAnimTemplateNode(DynAnimTemplateNode* pNode, AnimTemplateDoc* pDoc, int iNodeIndex)
{
	...
	if (pNode->eType != eAnimTemplateNodeType_End)
	{
			...
			pNode->eType == eAnimTemplateNodeType_Normal)
		{
			...
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_Default,		0);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_Forward,		1);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_ForwardRight,	1);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_Right,			1);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_BackwardRight,	1);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_Backward,		1);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_BackwardLeft,	1);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_Left,			1);
			ATEFlowChartNodeAddSwitchs(pDoc, pNode, &eaOutputButtons, eDynMovementDirection_ForwardLeft,	1);
		}
...
static void ATEFieldChangedCB(MEField* pField, bool bFinished, AnimTemplateDoc* pDoc)
{
	// we need to refresh all panels whenever the template type changes since
	// it can change the visibility of the template's individual data fields
	bool bRefresh = pField &&
					pField->pTable && pField->pTable->name && !strcmpi(pField->pTable->name, "DynAnimTemplate") &&
					pField->pchFieldName && !strcmpi(pField->pchFieldName,"Type");
	ATEAnimTemplateChanged(pDoc, bFinished, true, bRefresh, bRefresh, bRefresh);
}
...
static void ATERemoveSwitch(UIButton* pButton, DynAnimGraphSwitch* pRemoveSwitch)
{
	AnimTemplateDoc* pDoc = (AnimTemplateDoc*)emGetActiveEditorDoc();
	if (pDoc && pRemoveSwitch)
	{
		bool bFound = false;
		...
		FOR_EACH_IN_EARRAY(pDoc->pObject->pDefaultsGraph->eaNodes, DynAnimGraphNode, pNode)
		{
			...
			{
				if (pRemoveSwitch == pSwitch)
				{
					DynAnimTemplateSwitch* pFreeMeT;
					DynAnimGraphSwitch*    pFreeMeG;
					assert(eaSize(&pNode->eaSwitch) == eaSize(&pNode->pTemplateNode->eaSwitch));
					pFreeMeT = eaRemove(&pNode->pTemplateNode->eaSwitch, ipSwitchIndex);
					pFreeMeG = eaRemove(&pNode->eaSwitch,                ipSwitchIndex);
					StructDestroy(parse_DynAnimTemplateSwitch, pFreeMeT);
					StructDestroy(parse_DynAnimGraphSwitch,    pFreeMeG);
					bFound = true;
					break;
				}
			}
...
static void ATEAddSwitch(UIButton* pButton, DynAnimGraphNode* pGraphNode)
{
	AnimTemplateDoc* pDoc = (AnimTemplateDoc*)emGetActiveEditorDoc();
	if (pDoc) {
		DynAnimTemplateNode* pTemplateNode = pGraphNode->pTemplateNode;
		DynAnimTemplateSwitch* pTemplateSwitch = StructCreate(parse_DynAnimTemplateSwitch);
		DynAnimGraphSwitch*    pGraphSwitch    = StructCreate(parse_DynAnimGraphSwitch);

		pTemplateSwitch->pcFlag = allocAddString("NewSwitch");
		pTemplateSwitch->eDirection = eDynMovementDirection_Default;
		eaPush(&pTemplateNode->eaSwitch, pTemplateSwitch);

		pGraphSwitch->fRequiredPlaytime = 0.f;
		pGraphSwitch->bInterrupt = true;
...
static void ATESortSwitch(UIButton* pButton, AnimTemplateDoc* pDoc)
{
	// we can't easily do this any more if we have graph switches whose order depends on the template switches
	//if (pDoc) {
	//	DynAnimTemplateNode* pEditNode = pDoc->pObject->eaNodes[pDoc->iFocusIndex];
	//	eaQSort(pEditNode->eaSwitch, dynAnimTemplateCompareSwitchDisplayOrder);
	//	ATEAnimTemplateChanged(pDoc, true, true, false, true, false);
	//}
}
...
static void ATESortMove(UIButton* pButton, DynAnimGraphNode* pNode)
{
	AnimTemplateDoc* pDoc = (AnimTemplateDoc*)emGetActiveEditorDoc();
	if (pDoc) {
...
static EMPanel* ATEInitPropertiesPanel(AnimTemplateDoc* pDoc)
{
	bool bMovementTyped = pDoc->pObject->eType == eAnimTemplateType_Movement;
	EMPanel* pPanel;
	UILabel* pLabel;
	MEField* pField;
...
static F32 ATENodeEditReflow(AnimTemplateDoc* pDoc, EMPanel* pPanel, F32 y)
{
	...
	{
		if (!bMovementTyped ||
			pEditTemplateNode->bInterruptible) // show bad data so artist can fix it
		{
			...
			ATEAddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), 160, y, 0, 50.0, UIUnitFixed, 0, pDoc);
			eaPush(&pDoc->eaNodeFields, pField);
		...
		pEditTemplateNode->eType == eAnimTemplateNodeType_Normal)
	{
		...
		{
			DynAnimGraphSwitch*    pSwitchData     = (pEditGraphNode    && ipSwitchIndex < eaSize(&pEditGraphNode->eaSwitch))    ? pEditGraphNode->eaSwitch[ipSwitchIndex]    : NULL;
			...
			DynAnimGraphSwitch*    pOrigSwitchData = (pOrigGraphNode    && ipSwitchIndex < eaSize(&pOrigGraphNode->eaSwitch))    ? pOrigGraphNode->eaSwitch[ipSwitchIndex]    : NULL;
			bool bErrored = !bMovementTyped &&
							(	pSwitch->eDirection > 0  ||
								pSwitchData &&
								(	pSwitchData->fRequiredPlaytime > 0.f ||
									pSwitchData->fRequiredPlaytime < 0.f));
			...
			pButton = ui_ButtonCreate("X", x, y, ATERemoveSwitch, pSwitchData);
			emPanelAddChild(pPanel, pButton, false);
			x += ui_WidgetGetWidth( UI_WIDGET(pButton) ) + 20;

			if (pEditTemplateNode->eType != eAnimTemplateNodeType_Start)
			{
				pLabel = ui_LabelCreate("Interrupt", x, y);
				emPanelAddChild(pPanel, pLabel, false);
				x += ui_WidgetGetWidth( UI_WIDGET(pLabel) ) + 5;

				// IMPORTANT : notice that I'm reading the bInterrupt data from the anim. graph's switch data here and not the template's switch
				pField = MEFieldCreateSimple(kMEFieldType_Check, pOrigSwitchData, pSwitchData, parse_DynAnimGraphSwitch, "Interrupt");
				ATEAddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 50.0, UIUnitFixed, 0, pDoc);
				eaPush(&pDoc->eaNodeFields, pField);
			}

			if (bMovementTyped || bErrored)
			{
				x = 20;
				y += STANDARD_ROW_HEIGHT;
				...
				emPanelAddChild(pPanel, pLabel, false);
				x += ui_WidgetGetWidth(UI_WIDGET(pLabel)) + 10;
				pField = MEFieldCreateSimpleEnum(kMEFieldType_Combo, pOrigSwitch, pSwitch, parse_DynAnimTemplateSwitch, "Direction", DynMovementDirectionEnum);
				ATEAddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 0, pDoc);
				eaPush(&pDoc->eaNodeFields, pField);

				x = 20;
				y += STANDARD_ROW_HEIGHT;

				// IMPORTANT : notice that I'm reading the fRequiredPlaytime data from the anim. graph's switch data here and not the template's switch
				pLabel = ui_LabelCreate("Required Playtime (seconds)", x, y);
				emPanelAddChild(pPanel, pLabel, false);
				x += ui_WidgetGetWidth(UI_WIDGET(pLabel)) + 10;
				pField = MEFieldCreateSimple(kMEFieldType_TextEntry, pOrigSwitchData, pSwitchData, parse_DynAnimGraphSwitch, "RequiredPlaytime");
				ATEAddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 0, pDoc);
				eaPush(&pDoc->eaNodeFields, pField);
			}
		...
		pButton = ui_ButtonCreate("Add Switch", 0, y, ATEAddSwitch, pEditGraphNode);
		emPanelAddChild(pPanel, pButton, false);
		x = ui_WidgetGetWidth(UI_WIDGET(pButton)) + 10;

		// We can't do this if we have graphs that depend on templates
		// pButton = ui_ButtonCreate("Sort", x, y, ATESortSwitch, pDoc);
		// emPanelAddChild(pPanel, pButton, false);
	...
	if (pEditTemplateNode->eType == eAnimTemplateNodeType_Normal)
	{
		...
		y = AnimEditor_ReflowNormalNode(
			...
			ATEAddMove, ATESortMove, ATEChooseMove, ATEMoveChangeChance, ATEOpenMove, ATERemoveMove,
			&ateNodeSkin[eATEUnconnected1],
			bMovementTyped,
			false,
			y
