...
static bool mmeDrawSkeleton = false;
...
static UICheckButton *mmeDrawSkeletonCheckButton;
...
static void MMEDrawSkeletonCheckboxToggle(UICheckButton *pButton, UserData pData)
{
	if (mmeDrawSkeleton)
		dynDebugState.pDebugSkeleton = NULL;
	
	mmeDrawSkeleton = dynDebugState.bDrawSkeleton = mmeDrawSkeletonCheckButton->state;
	...
	FOR_EACH_IN_EARRAY(pEMDoc->eaFilters, MMEMFilter, pFilter)
	{
		...
		FOR_EACH_IN_EARRAY(pMove->eaDynMoveSeqs, DynMoveSeq, pMoveSeq)
		{
			...
			FOR_EACH_IN_EARRAY(pMoveSeq->dynMoveAnimTrack.eaBoneRotation, DynMoveBoneRotOffset, pBoneRotation)
			{
				...
				if (pFilter->eMoveFilterType == mmeFilterType_Number) {
					if      (pBoneRotation->rotOffset[0] == pFilter->fMoveFilterValue) foundIt = true;
					else if (pBoneRotation->rotOffset[1] == pFilter->fMoveFilterValue) foundIt = true;
					else if (pBoneRotation->rotOffset[2] == pFilter->fMoveFilterValue) foundIt = true;
				}
		...
		//Anim track data
		{
			...
			if (mmeBoneCheckButton->state)
			{
				...
				EARRAY_FOREACH_BEGIN(pNewSeq->dynMoveAnimTrack.eaBoneRotation, j);
				{
					...
					MEContextStepBackUp();
					brContextEntry = MEContextAddMinMax(kMEFieldType_MultiSpinner, -360, 360, 0.1, "rotOffset", "", "Rotation to offset the bone by");
					ui_MultiSpinnerEntrySetValue(brContextEntry->ppFields[0]->pUIMultiSpinner, pNewSeq->dynMoveAnimTrack.eaBoneRotation[j]->rotOffset, 3);
					brContext->iXDataStart = 0;
...
void MMEDrawGhost(MoveMultiDoc *pDoc)
{
	...
	{
		if (mmeDrawSkeleton) {
			dynDebugState.pDebugSkeleton = pDoc->costumeData.pGraphics->costume.pSkel;
			dynDebugState.bDrawSkeleton = true;
		}
...
void MMEInitCustomToolbars(EMEditor *pEditor)
{
	...
	mmeDrawSkeletonCheckButton = ui_CheckButtonCreate(fX, 0, "Draw Skeleton", false);
	ui_CheckButtonSetToggledCallback(mmeDrawSkeletonCheckButton, MMEDrawSkeletonCheckboxToggle, NULL);
	emToolbarAddChild(pToolbar, mmeDrawSkeletonCheckButton, true);
	fX += ui_WidgetGetWidth(UI_WIDGET(mmeDrawSkeletonCheckButton)) + 5.0;
