...
bool exprEdForceAscii(UIEditable *pEditable, unsigned char **estrOld, unsigned char **estrNew, UserData pDummy)
{
	static S32 s_iSize = 0;
	static unsigned short *s_pwch = NULL;
	const size_t szMaxStackAlloc = 4096;

	// An ASCII representation of something generally can't be longer than its UTF-8 representation.
	// Even if it is, we're just going to truncate, so it's still safe, just sucky.
	size_t iLength = strlen(*estrNew) + 1;
	unsigned char *pchClean = iLength <= szMaxStackAlloc ? _alloca(iLength) : malloc(iLength);

	s_pwch = UTF8ToWideStrConvertAndRealloc(*estrNew, s_pwch, &s_iSize);
	// Magic number 20127 is the code for US ASCII.
	WideToEncodingConvert(s_pwch, pchClean, (S32)iLength, 20127);
	estrCopy2(estrNew, pchClean);
	if (iLength > szMaxStackAlloc)
		free(pchClean);
	return true;
}
...
static ExprEdEntryUI *exprEdEntryUICreateText(MultiValType type, ExprEdLineUI *line, bool bMakeButton)
{
	...
	ui_TextEntrySetValidateCallback(entryUI->textEntry, exprEdForceAscii, NULL);
...
void exprEdRegisterValidationFunc(const char *type, ExprEdValidationClicked clickFunc, void *clickData)
{
	...
	if (!exprEdTypeValidations)
		exprEdTypeValidations = stashTableCreateWithStringKeys(16, StashDeepCopyKeys_NeverRelease);
