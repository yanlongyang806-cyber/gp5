...
#define TALENT_TREE_MULTINODE_LABEL_NAME "label"
...
typedef struct TalentTreeNodeData
{
	...
	PTNodeEdit **eaNodeEdits;
	PTNodeEdit *pCurrentNode;
...
EMTaskStatus PTEditorSaveDoc(EMEditorDoc *pAEdoc)
{
	...
	if(status == EM_TASK_SUCCEEDED)
	{
		...
		for(i=eaSize(&pdoc->ppDirty)-1; i>=0; i--)
		{
			...
			if(pdoc->ppDirty[i]->pUICheck)
				ui_SetChanged(UI_WIDGET(pdoc->ppDirty[i]->pUICheck), false);
			if(pdoc->ppDirty[i]->pUIFlagBox)
				ui_SetChanged(UI_WIDGET(pdoc->ppDirty[i]->pUIFlagBox), false);
			if(pdoc->ppDirty[i]->pUIMessage)
				ui_SetChanged(UI_WIDGET(pdoc->ppDirty[i]->pUIMessage), false);
			if(pdoc->ppDirty[i]->pUIExpression)
				ui_SetChanged(UI_WIDGET(pdoc->ppDirty[i]->pUIExpression), false);
		}
...
static bool PTTextFieldUpdateString(SA_PARAM_NN_VALID PTField *pfield, SA_PARAM_NN_STR const char* pchText)
{
	...
	if(pfield->bRevert)
	{
		...
		if(pfield->pUIExpression)
			ui_ExpressionEntrySetText(pfield->pUIExpression, pchText);
...
static bool PTTextFieldUpdateExpression(SA_PARAM_NN_VALID PTField *pfield, SA_PARAM_NN_STR const char *pchText)
{
	...
	if(pfield->bRevert)
	{
		...
		if (pfield->pUIExpression)
			ui_ExpressionEntrySetText(pfield->pUIExpression, pchConstText);
		pfield->bRevert = false;
	}
...
static void PTUpdateFieldDirty(SA_PARAM_NN_VALID PTField *pfield,bool bDirty)
{
	...
	if(bDirty!=pfield->bDirty)
	{
		...
		if(pfield->pUITextArea)
			ui_SetChanged(UI_WIDGET(pfield->pUITextArea),bDirty);
		if(pfield->pUICombo)
			ui_SetChanged(UI_WIDGET(pfield->pUICombo), bDirty);
		if(pfield->pUICheck)
			ui_SetChanged(UI_WIDGET(pfield->pUICheck),bDirty);
		if(pfield->pUIFlagBox)
			ui_SetChanged(UI_WIDGET(pfield->pUIFlagBox),bDirty);
		if(pfield->pUIMessage)
			ui_SetChanged(UI_WIDGET(pfield->pUIMessage),bDirty);
		if(pfield->pUIExpression)
			ui_SetChanged(UI_WIDGET(pfield->pUIExpression),bDirty);
	}
...
static void PTExpressionFieldChanged(SA_PARAM_NN_VALID UIExpressionEntry *pexpression, SA_PARAM_NN_VALID PTField *pfield)
{
	bool bDirty = false;
	char *pchOld = NULL;

	// Reduced functionality PTTextFieldChanged
	assert(!pfield->bisName);
	assert(!pfield->bisTitle);

	bDirty = PTTextFieldUpdateExpression(pfield,ui_ExpressionEntryGetText(pexpression));
	PTUpdateFieldDirty(pfield,bDirty);

	if(pfield->fChangeFunc)
		pfield->fChangeFunc(pfield,pfield->pChangeFuncData);

	ui_WidgetSetTooltipString((UIWidget*)pfield->pUIExpression, ui_ExpressionEntryGetText(pfield->pUIExpression));
}
...
static void PTUpdateField(PTField *pfield,bool brevert)
{
	...
	if(pfield->pUIExpression)
		PTExpressionFieldChanged(pfield->pUIExpression,pfield);
}
...
static PTField *PTAddExpressionField(UIPane *pwin, UIExpander *pexp, PTEditDoc *pdoc, void *pOld, void *pNew, 
									 ParseTable *ptable, char *pchField, const char *pchDictName, char *pchAlias, bool bLabel, UIComboBox *pSubCombo, UIButton *pBtn,
									 int x, int y, int width, bool brevert, bool bVertical, bool bisTitle, UIMenu *pMenu)
{
	PTField *pfield = PTCreateBasicField(pwin,pexp,pdoc,pOld,pNew,ptable,pchField,pchDictName,pchAlias,bLabel,pBtn,&x,&y,width,bVertical,bisTitle,pMenu);
	...
	// Creates the expression field, sets up its callback, and then uses the callback to init
	if(brevert)
		pfield->pUIExpression = ui_ExpressionEntryCreate("", pcontext);
	else
	{
		Expression **ppExprTarget = (Expression**)((char*)(pfield->pNew) + pfield->offset);
		if (!exprIsEmpty(*ppExprTarget))
			pfield->pUIExpression = ui_ExpressionEntryCreate(exprGetCompleteString(*ppExprTarget), pcontext);
		else
			pfield->pUIExpression = ui_ExpressionEntryCreate("", pcontext);
	}

	ui_WidgetSetPosition(UI_WIDGET(pfield->pUIExpression), x, y);

	pfield->pUIExpression->widget.width = width;
	ui_ExpressionEntrySetChangedCallback(pfield->pUIExpression,PTExpressionFieldChanged,pfield);

	PTUpdateField(pfield,brevert);

	if(pwin)
		ui_PaneAddChild(pwin,pfield->pUIExpression);
	if(pexp)
		ui_ExpanderAddChild(pexp,(UIWidget *)pfield->pUIExpression);
...
static void PTCreateRankExp(PTNodeRankDef *pNew, PTNodeRankDef *pOld, PTNodeEdit *pNode)
{
	...
	pNewRank->pExp = ui_ExpanderCreate(pPower ? pPower->pchName : "No Power",PTEditorIsInPowerTreeMode(pNode->pDefNodeNew) ? 310 : 250);
	...
	pTempField = PTAddExpressionField(NULL,pNewRank->pExp,pNode->pdoc,pOld? pOld->pRequires : NULL,pNew->pRequires,parse_PTPurchaseRequirements,"PurchaseExpression",NULL,"Requirements",true,NULL,NULL,10,y,150,true,false,false,NULL);
	eaPush(&pNode->pdoc->ppFields,pTempField);
	//if(IS_HANDLE_ACTIVE(pNode->pDefNodeNew->hNodeClone)) pTempField->pUIText->widget.state = kWidgetModifier_Inactive;
...
static void PTCreateNodeExp(PTNodeDef *pNew,PTNodeDef *pOld, PTGroupEdit *pGroup)
{
	...
	pTempField = PTAddExpressionField(NULL,pNewNode->pExp,pGroup->pdoc,pOld,pNew,parse_PTNodeDef,"AccessCondition",NULL,"Can Access Node",true,NULL,NULL,5,y,100,true,false,false,NULL);
	if(IS_HANDLE_ACTIVE(pNew->hNodeClone)) pTempField->pUIText->widget.state = kWidgetModifier_Inactive;
	eaPush(&pGroup->pdoc->ppFields,pTempField);
	y += 25;
	...
	pCombo = ui_ComboBoxCreateWithDictionary(0,0,100,g_hPowerTreeNodeDefDict,parse_PTNodeDef,"NameFull");
	pCombo->openedWidth = 400;
	pTempField = PTAddTextField(NULL,pNewNode->pExp,pGroup->pdoc,pOld,pNew,parse_PTNodeDef,"AltTrainerNode","PowerTreeNodeDef","Alt Trainer Node",true,pCombo,NULL,215,y,100,true,false,false,NULL);
	PTField_AddFieldChangeCallback(pTempField,PTCloneFieldChange,pNewNode);
	eaPush(&pGroup->pdoc->ppFields,pTempField);
...
static PTNodeEdit* PTNewNode(PTGroupEdit *pGroup, S8 iRow, S8 iCol)
{
	PTNodeDef *pNewNode;
	char achName[MAX_POWER_NAME_LEN];
	...
	if (iCol > 0)
	{
		...
		do {
			...
			sprintf(achName,"_Node %d%d", iCol, iID);

			
			FOR_EACH_IN_EARRAY_FORWARDS(pGroup->ppNodeEdits, PTNodeEdit, pNodeEdit)
			{
				if (pNodeEdit->pDefNodeNew && 
					pNodeEdit->pDefNodeNew->iUIGridRow == iRow &&
					pNodeEdit->pDefNodeNew->iUIGridColumn == iCol &&
					!stricmp(pNodeEdit->pDefNodeNew->pchName, achName))
				{
					bDuped = true;
					break;
				}
...
static void PTCreateGroupWindow(PTGroupDef *pgroupNewDef, PTGroupDef *pgroupOrigDef, PTEditDoc *pdoc)
{
	...
	if (PTEditorIsInPowerTreeMode(pgroupNewDef))
	{
		...
		pTempField = PTAddExpressionField(NULL,pgroupEdit->pExp,pdoc,pgroupOrigDef? pgroupOrigDef->pRequires : NULL,pgroupNewDef->pRequires,parse_PTPurchaseRequirements,"PurchaseExpression",NULL,"Requirements",true,NULL,NULL,5,y,150,true,false,false,NULL);
		eaPush(&pdoc->ppFields,pTempField);
		...
		pTempField = PTAddCheckField(NULL,pgroupEdit->pExp,pdoc,pgroupOrigDef,pgroupNewDef,parse_PTGroupDef,"UIIgnoreGroupRequirements","UI Ignore Group Requirements",true,5,y,150,false,NULL,"Cause some UI's to not filter out powers nodes from the group when the player fails group requirements.");
		eaPush(&pdoc->ppFields,pTempField);
...
static PTNodeEdit *PTTalentTreeFindNodeEditByRowAndColumn(PTEditDoc *pDoc, PTNodeEdit *pCurrentSelected, S8 iRow, S8 iCol)
{
	S32 i, j;
	static PTNodeEdit **eaNodeEdits = NULL;
	...
	for (i = 0; i < eaSize(&pDoc->ppGroupWindows); i++)
	{
		for(j = 0; j < eaSize(&pDoc->ppGroupWindows[i]->ppNodeEdits); j++)
		{
			...
			if(	pNodeDef && pNodeDef->iUIGridRow == iRow && pNodeDef->iUIGridColumn == iCol)
			{
				eaPush(&eaNodeEdits, pDoc->ppGroupWindows[i]->ppNodeEdits[j]);
			}
		}
	}

	// 
	if (eaSize(&eaNodeEdits) > 0)
	{
		if (!pCurrentSelected || eaSize(&eaNodeEdits) == 1)
		{
			return eaNodeEdits[0];
		}
		
		// loop through the list (excluding the last one) to find which one we have selected currently
		// so we can return the next in the list
		for (i = 0; i < eaSize(&eaNodeEdits) - 1; i++)
		{
			if (eaNodeEdits[i] == pCurrentSelected)
			{
				return eaNodeEdits[i + 1];
			}
		}
...
// Fills the list of peaEditsOut with all nodes that occupy the Row & Column
static void PTTalentTreeGetAllNodeEditByRowAndColumn(PTEditDoc *pDoc, S8 iRow, S8 iCol, PTNodeEdit ***peaEditsOut)
{
	S32 i, j;
	...
	for (i = 0; i < eaSize(&pDoc->ppGroupWindows); i++)
	{
		for(j = 0; j < eaSize(&pDoc->ppGroupWindows[i]->ppNodeEdits); j++)
		{
			...
			if(pNodeDef && pNodeDef->iUIGridRow == iRow && pNodeDef->iUIGridColumn == iCol)
			{
				eaPush(peaEditsOut, pDoc->ppGroupWindows[i]->ppNodeEdits[j]);
			}
		}
	}
}
...
static void PTOnNodeContextMenuNewNode(UIMenuItem *pMenuItem, UIButton *pTalentTreeButton)
{
	...
	if (eaSize(&pTalentTreeData->eaNodeEdits) == 0)
	{
		...
		if (pGroupEdit)
		{
			...
			// Create a new node
			pNodeEdit = PTNewNode(pGroupEdit, pTalentTreeData->iRow, pTalentTreeData->iCol);

			pNodeEdit = PTTalentTreeFindNodeEditByRowAndColumn(pTalentTreeData->pDoc, NULL, pTalentTreeData->iRow, pTalentTreeData->iCol);
			...
			if (pNodeEdit)
			{
				...
				eaPush(&pTalentTreeData->eaNodeEdits, pNodeEdit);
				pTalentTreeData->pCurrentNode = pNodeEdit;
			}
		}
...
// creates if needed, then updates, or destroys depending on how many nodes are present 
static void PTSiblingNodeUpdateLabel(UIButton *pTalentTreeButton, TalentTreeNodeData *pTalentTreeData) 
{
	if (pTalentTreeData && eaSize(&pTalentTreeData->eaNodeEdits) > 1)
	{
		UILabel *pLabel = NULL;
		
		
		pLabel = (UILabel*)ui_WidgetFindChild(UI_WIDGET(pTalentTreeButton), TALENT_TREE_MULTINODE_LABEL_NAME);
		if (!pLabel)
		{
			...
			{
				pLabel->bOpaque = true;
				ui_WidgetSetName(UI_WIDGET(pLabel), TALENT_TREE_MULTINODE_LABEL_NAME);
				ui_WidgetSetPositionEx(UI_WIDGET(pLabel), 0, 0, 0, 0, UIBottom);
				ui_WidgetAddChild(UI_WIDGET(pTalentTreeButton), UI_WIDGET(pLabel));
			}
		}
		...
		// set the text
		{
			...
			size = eaSize(&pTalentTreeData->eaNodeEdits);
			count = 1 + eaFind(&pTalentTreeData->eaNodeEdits, pTalentTreeData->pCurrentNode);
			sprintf_s(SAFESTR2(text), "%d/%d", count, size);
			ui_LabelSetText(pLabel, text);
		}

	}
	else
	{
		UIWidget *pLabel = NULL;
		pLabel = ui_WidgetFindChild(UI_WIDGET(pTalentTreeButton), TALENT_TREE_MULTINODE_LABEL_NAME);
		if (pLabel)
		{
			ui_WidgetDestroy(&pLabel);
		}
	}
}

// Creates
static void PTOnNodeContextMenuCreateSiblingNode(UIMenuItem *pMenuItem, UIButton *pTalentTreeButton)
{
	TalentTreeNodeData *pTalentTreeData;
	...
	if (eaSize(&pTalentTreeData->eaNodeEdits) > 0)
	{
		...
		if (pGroupEdit)
		{
			...
			if (pNodeEdit)
			{
				...
				PTSiblingNodeUpdateLabel(pTalentTreeButton, pTalentTreeData);
			}
		}
...
static void PTOnNodeContextMenuRemoveDependency(UIMenuItem *pMenuItem, UIButton *pTalentTreeButton)
{
	...
	if (pTalentTreeData->pCurrentNode)
	{
		REMOVE_HANDLE(pTalentTreeData->pCurrentNode->pDefNodeNew->hNodeRequire);
...
static void PTOnNodeContextMenuDeleteNode(UIMenuItem *pMenuItem, UIButton *pTalentTreeButton)
{
	...
	if (pTalentTreeData->pCurrentNode)
	{
		...
		eaFindAndRemoveFast(&pTalentTreeData->eaNodeEdits, pTalentTreeData->pCurrentNode);

		// Delete the node
		PTDeleteNode(pTalentTreeData->pCurrentNode);

		pTalentTreeData->pCurrentNode = NULL;
		...
		{
			pTalentTreeData->pCurrentNode = eaGet(&pTalentTreeData->eaNodeEdits, 0);
		}
...
static void PTOnTalentTreeButtonContext(UIButton *pTalentTreeButton, TalentTreeNodeData *pTalentTreeData)
{
	...
	if (s_pNodeContextMenu == NULL)
	{
		s_pNodeContextMenu = ui_MenuCreateWithItems("Node Menu",
			...
			ui_MenuItemCreate("Delete node", UIMenuCallback, PTOnNodeContextMenuDeleteNode, NULL, NULL),
	...
	s_pNodeContextMenu->items[2]->clickedData = pTalentTreeButton;
	...
	if (eaSize(&pTalentTreeData->eaNodeEdits) == 0)
	{
		...
		s_pNodeContextMenu->items[2]->active = false;
	}
	else
	{
		if (!pTalentTreeData->pCurrentNode)
		...
		s_pNodeContextMenu->items[0]->clickedF = PTOnNodeContextMenuCreateSiblingNode;
		ui_MenuItemSetTextString( s_pNodeContextMenu->items[0], "Create sibling node" );
		
		// 
		s_pNodeContextMenu->items[1]->active = true;
		s_pNodeContextMenu->items[2]->active = IS_HANDLE_ACTIVE(pTalentTreeData->pCurrentNode->pDefNodeNew->hNodeRequire);
	}
...
static void PTOnAssignedTalentTreeButtonClick(UIButton *pTalentTreeButton, TalentTreeNodeData *pTalentTreeData)
{
	...
	pNodeEdit = PTTalentTreeFindNodeEditByRowAndColumn(pTalentTreeData->pDoc, pTalentTreeData->pCurrentNode, 
														pTalentTreeData->iRow, pTalentTreeData->iCol);

	if (pNodeEdit)
	{
		if (pTalentTreeData->pCurrentNode != pNodeEdit)
		{
			pTalentTreeData->pCurrentNode = pNodeEdit;
			// Set the tooltip for the button
			if (PTTalentTreeGetNodeName(pTalentTreeData->pCurrentNode->pDefNodeNew))
			{
				ui_WidgetSetTooltipString(	UI_WIDGET(pTalentTreeButton), 
											PTTalentTreeGetNodeName(pTalentTreeData->pCurrentNode->pDefNodeNew));
			}
		}
...
static void PTTalentTreeDrag(UIButton *pButton, UserData data)
{
	...
	if (eaSize(&pTalentTreeData->eaNodeEdits) == 1 && pTalentTreeData->pCurrentNode)
	{
		//ui_DragStart(UI_WIDGET(pButton), UI_DND_TALENT_TREE_NODE, pTalentTreeData->pNodeEdit, atlasLoadTexture("A_Button_38.tga"));
		ui_DragStart(UI_WIDGET(pButton), UI_DND_TALENT_TREE_NODE, pTalentTreeData->pCurrentNode, atlasLoadTexture("A_Button_38.tga"));
	}
}
...
static void PTTalentTreeDrop(UIButton *pSourceTalentTreeButton, UIButton *pDestinationTalentTreeButton, UIDnDPayload *pPayload, char *pchDummy)
{
	...
	if (pPayload->type == UI_DND_TALENT_TREE_NODE)
	{
		...
		if (eaSize(&pSourceTalentTreeData->eaNodeEdits) != 1 ||  
			eaSize(&pDestinationTalentTreeData->eaNodeEdits) != 1)
		{
			return;
		}


		pSourceNodeDef = pSourceTalentTreeData->eaNodeEdits[0]->pDefNodeNew;
		pDestinationNodeDef = pDestinationTalentTreeData->eaNodeEdits[0]->pDefNodeNew;
		...
		if (pSourceTalentTreeData->iCol != pDestinationTalentTreeData->iCol)
		{
				...
				iColIt += iColStep)
			{
				if ((
					...
					PTTalentTreeFindNodeEditByRowAndColumn(pSourceTalentTreeData->pDoc, NULL, pSourceTalentTreeData->iRow, iColIt))
				{
		...
		if (pSourceTalentTreeData->iRow != pDestinationTalentTreeData->iRow)
		{
			...
			for (iRowIt = pSourceTalentTreeData->iRow + iRowStep; 
				...
				iRowIt += iRowStep)
			{
				if (PTTalentTreeFindNodeEditByRowAndColumn(pSourceTalentTreeData->pDoc, NULL, iRowIt, pDestinationTalentTreeData->iCol))
				{
					ui_DialogPopup(s_pTalentTreeValidationErrorString, "Cannot create dependency while there are nodes in the path");
					return;
				}
...
static void PTTalentTreeButtonTick(UIButton *pButton, UI_PARENT_ARGS)
{
	...
	if (eaSize(&pTalentTreeData->eaNodeEdits) != 0)
	{
		...
		if (!pTalentTreeData->pCurrentNode)
		{
			pTalentTreeData->pCurrentNode = pTalentTreeData->eaNodeEdits[0];
		}
	
		pchIconName = PTTalentTreeGetNodeIconName(pTalentTreeData->pCurrentNode->pDefNodeNew);

		PTTalentTreeButtonUpdateIcon(pTalentTreeData->pDoc, pTalentTreeData->pCurrentNode->pDefNodeNew, pchIconName);
	}
									 ...
									 UIActivationFunc pClickCallback, TalentTreeNodeData *pTalentTreeData)
{
	...
	{
		pchIconName = PTTalentTreeGetNodeIconName(pTalentTreeData->pCurrentNode->pDefNodeNew);
		pchTooltip = PTTalentTreeGetNodeName(pTalentTreeData->pCurrentNode->pDefNodeNew);
	}
										  ...
										  U32 uiSpacingHorizontal, U32 uiSpacingVertical)
{
	...
	for (i = 1; i <= uiNumRows; i++)
	{
		...
		for (j = 1; j <= uiNumCols; j++)
		{
			...
			eaClear(&pTalentTreeNodeData->eaNodeEdits);
			PTTalentTreeGetAllNodeEditByRowAndColumn(pEditDoc, i, j, &pTalentTreeNodeData->eaNodeEdits);
			pTalentTreeNodeData->pCurrentNode = eaGet(&pTalentTreeNodeData->eaNodeEdits, 0);
			...
			if (pButton && eaSize(&pTalentTreeNodeData->eaNodeEdits) > 1)
			{
				PTSiblingNodeUpdateLabel(pButton, pTalentTreeNodeData);
			}
