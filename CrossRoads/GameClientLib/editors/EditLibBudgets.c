...
#define WARN_START_CUTOFF 0.75f
...
#define BudgetsCategory_Initialized 1<<31 // this can never be turned off, to avoid the stupid bug where turning all alerts off turns them all on
...
void editLibBudgetsShow(bool bAutoOpen);
...
void editLibBudgetsStartup(void)
{
	...
	if (!budget_data.alert_category)
	{
		...
		// this is a marker to say "We're not uninitialized"
		budget_data.alert_category |= BudgetsCategory_Initialized;
	}
}
...
static Color colorFromRatio(F32 ratio, bool squishy)
{
	...
	if (ratio < WARN_CUTOFF) {
		F32 r = MAX(0.0f,ratio-WARN_START_CUTOFF) / (WARN_CUTOFF-WARN_START_CUTOFF); // below WARN_START_CUTOFF is happy green
		ret.r = r*255.f;
		ret.g = 140.f + r*115.f;
...
static bool updateBar(BudgetBar *bar, size_t recent, size_t total, size_t budget, size_t trafficDelta, bool squishy, const UnitSpec *spec)
{
	...
	PERFINFO_AUTO_START(__FUNCTION__, 1);
	{
		...
		bar->last.bUpdatedOnce = true;
		bar->last.bOverBudget = (budget==NO_BUDGET)?false:ret;
	}
	PERFINFO_AUTO_STOP();
	return bar->last.bOverBudget;
}

static bool updateMsBar(BudgetBar *bar, F32 iCurrent, F32 budget, F32 max, bool bTick, char const * pchLabel)
{
	bool ret=false;
	PERFINFO_AUTO_START(__FUNCTION__, 1);
	{
...
//		char buf1[64], buf2[64];
		...
		if (bar) {
			...
			if (bTick || !bar->last.bUpdatedOnce)
			{

				//something more like this
				sprintf(buf, "%1.1f ms %s", iTotal*0.001f,pchLabel);
...
void editLibBudgetsUpdate(void)
{
...

	{
...

		{
			...
			budget_data.art_over_performance |= updateMsBar(&budget_data.bar.cpu_us, counts.cpu_ms*1000.f, CPU_BUDGET_US, MAX_CPU_US, bTick, "Total (Real)");
			budget_data.art_over_performance |= updateMsBar(&budget_data.bar.cpu_dual_core_effective_us, fDualCoreEffectiveCpu, CPU_BUDGET_US, MAX_CPU_US, bTick, "(Dual-core effective)");
			...
			budget_data.art_over_performance |= updateMsBar(&budget_data.bar.gpu_us, counts.gpu_ms*1000.f, CPU_BUDGET_US, MAX_CPU_US, bTick, "Total");
		...
		sprintf(buf, "%d Total Object Draws",counts.opaque_objects_drawn + counts.alpha_objects_drawn);
		ui_LabelSetText(budget_data.bar.object_total_count.label_text, buf); 
...
static BudgetBar addBar(UIRTNode *group, bool toExpander, const char *name_key, EBarType eBarType, DoubleTextType doubleText, size_t budget)
{
	...
	pane->invisible = true;
	pane->widget.leftPad = toExpander?ALIGN_SPOT+1:0; // why +1?  Don't know
	pane->widget.rightPad = 5;
...
static void showDetails(UIAnyWidget *widget_UNUSED, const char *module)
{
	...
	if (subwindow->table == parse_ModuleMemOperationStats) {
		...
		ui_ListAppendColumn(list, ui_ListColumnCreate(UIListPTName, "log UV Density", (intptr_t)"uv_density", NULL));
		ui_ListAppendColumn(list, ui_ListColumnCreate(UIListPTName, "Recent?", (intptr_t)"recent", NULL));
...
static void editLibBudgetsOpenCloseExpanders(void)
{
	...
	bMemoryOver = bArtOver || budget_data.software_over_mem;
	ui_ExpanderSetOpened(expander, bMemoryOver);
	...
	if (bMemoryOver)
	{
		...
		FOR_EACH_IN_EARRAY_FORWARDS(budgets, MemoryBudget, budget)
		{
			BudgetBar * bar = getDynamicBudgetBar(budget->module);
			if (bar->last.bOverBudget)
			{
				// open all my parents
				/*UIRTNode *node = bar->groupnode;
				while (node)
				{
					if (node->expander)
						ui_ExpanderSetOpened(node->expander, true);
					node = node->parent;
				}*/

				// Can't seem to find a way to walk up the tree, so I'm going to try just opening the parent expander.
				ui_ExpanderSetOpened(bar->groupnode->expander, true);
			}
		}
		FOR_EACH_END;
	}
...
void editLibBudgetsShow(bool bAutoOpen)
{
	...
	group = ui_RebuildableTreeAddGroup(budget_data.uirt->root, "Performance", "Performance", true, NULL);
	{
		...
		//ui_RebuildableTreeAddLabel(group, "GPU", NULL, true);
		{
			...
			ui_RebuildableTreeAddLabel(pGPUGroupNode, "Shadow Buffer (ssao)", NULL, true /*newline*/);
			...
			ui_RebuildableTreeAddLabel(pGPUGroupNode, "Opaque", NULL, true /*newline*/);
			budget_data.bar.gpu_opaque_us = addBar(pGPUGroupNode, false, "Opaque", EBarType_Double, DoubleText_None, MAX_CPU_US);
			ui_RebuildableTreeAddLabel(pGPUGroupNode, "Alpha", NULL, true /*newline*/);
			budget_data.bar.gpu_alpha_us = addBar(pGPUGroupNode, false, "Alpha", EBarType_Double, DoubleText_None, MAX_CPU_US);
			ui_RebuildableTreeAddLabel(pGPUGroupNode, "PostProcess", NULL, true /*newline*/);
			budget_data.bar.gpu_postprocess_us = addBar(pGPUGroupNode, false, "PostProcess", EBarType_Double, DoubleText_None, MAX_CPU_US);
			ui_RebuildableTreeAddLabel(pGPUGroupNode, "2D", NULL, true /*newline*/);
			budget_data.bar.gpu_2d_us = addBar(pGPUGroupNode, false, "2D", EBarType_Double, DoubleText_None, MAX_CPU_US);
			ui_RebuildableTreeAddLabel(pGPUGroupNode, "Other", NULL, true /*newline*/);
			budget_data.bar.gpu_other_us = addBar(pGPUGroupNode, false, "Other", EBarType_Double, DoubleText_None, MAX_CPU_US);
		}

		ui_RebuildableTreeAddLabel(group, "                          ---------------------------------------Counts----------------------------------------", NULL, true);

		// Object counts group
		{
			UIRTNode * pObjectNode;
			pObjectNode = ui_RebuildableTreeAddGroup(group, "Objects", "Objects", false, NULL);
			budget_data.bar.object_total_count = addBar(pObjectNode, true, "Objects", EBarType_Single, DoubleText_None, OBJECT_TOTAL_BUDGET);
			//pObjectNode = ui_RebuildableTreeAddLabel(group, "Objects", NULL, true);
			...
			ui_AutoWidgetAddButton(pObjectNode, "?", showDetails, (UserData)"Scene Objs", true, "Show detailed information for this module", NULL);
			ui_RebuildableTreeAddLabel(pObjectNode, "Scene Objs", NULL, false /*newline*/);
			budget_data.bar.object_scene_count = addBar(pObjectNode, false, "Scene Objs", EBarType_Single, DoubleText_None, OBJECT_SCENE_BUDGET);
			ui_RebuildableTreeAddLabel(pObjectNode, "World Objs", NULL, true /*newline*/);
			budget_data.bar.object_world_count = addBar(pObjectNode, false, "World Objs", EBarType_Single, DoubleText_None, OBJECT_WORLD_BUDGET);
			ui_RebuildableTreeAddLabel(pObjectNode, "Character Objs", NULL, true /*newline*/);
			budget_data.bar.object_character_count = addBar(pObjectNode, false, "Character Objs", EBarType_Single, DoubleText_None, OBJECT_CHARACTER_BUDGET);
			ui_RebuildableTreeAddLabel(pObjectNode, "FX Objs", NULL, true /*newline*/);
			budget_data.bar.object_fx_count = addBar(pObjectNode, false, "FX Objs", EBarType_Single, DoubleText_None, OBJECT_FX_BUDGET);
		}
...
#define ADD_BAR(parent, prefix, cat) ADD_BAR_NAMED(parent, prefix, cat, prefix)
	...
	// Dynamic budgets
	{
		...
		{
			BudgetBar *bar;
			ui_AutoWidgetAddButton(other, "?", showDetails, (UserData)budget->module, true, "Show detailed information for module", NULL);
			if (stricmp(budget->module, "Textures:Misc")==0)
				ui_AutoWidgetAddButton(other, "?", showDetails, (UserData)"Textures:Util", false, "Show detailed information for Utility textures", NULL);
