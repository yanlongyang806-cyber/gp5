...
static void sortByPhysicsObjectsReverse(UIListColumn* column, void* userData);
static void sortByPhysicsObjects(UIListColumn* column, void* userData)
{
	dynFxDebugSort(eDebugSortMode_PhysicsObjects);
	ui_ListColumnSetClickedCallback(column, sortByPhysicsObjectsReverse, NULL);
}

static void sortByPhysicsObjectsReverse(UIListColumn* column, void* userData)
{
	dynFxDebugSort(eDebugSortMode_PhysicsObjectsReverse);
	ui_ListColumnSetClickedCallback(column, sortByPhysicsObjects, NULL);
}
...
static void displayDynFxPhysicsObjects(UIList *pList, UIListColumn *col, UI_MY_ARGS, F32 z, CBox *pBox, int index, void *drawData)
{
	gfxfont_Printf(x, y + h/2, z, scale, scale, CENTER_Y, "%d", ((DynFxTracker*)(*pList->peaModel)[index])->uiNumPhysicsObjects);
}
	...
	*/
	...
	// Update Group
	{
		...
		// List
		{
			...
			pCol = ui_ListColumnCreateCallback("Physics", displayDynFxPhysicsObjects, NULL);
			pCol->fWidth = 50;
			ui_ListColumnSetClickedCallback(pCol, sortByPhysicsObjects, NULL);
			ui_ListAppendColumn(fxTracker.pDebugUpdateList, pCol);
