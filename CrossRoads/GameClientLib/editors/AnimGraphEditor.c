...
static const Color ageNodeTypeColor[eAGECount] = {
	...
	{ 192, 192, 192, 170 },		// eAGENormalSkip
	{ 192, 192, 192,  85 },		// eAGENormalInvalid
	{ 240, 240, 128, 255 },     // eAGERandomizer
	{ 240, 240, 128,  85 },     // eAGERandomizerInvalid
...
static void AGENodeDataFree(AGENodeData* pNodeData)
{
	...
	eaDestroy(&pNodeData->eaDirectionLabels);
...
static void AGEFlowChartNodeAddSwitchs(AnimGraphDoc* pDoc, DynAnimGraphNode* pGraphNode, DynAnimTemplateNode* pTemplateNode, UIFlowchartButton*** peaOutputButtons, DynMovementDirection eAllowedDirection, U32 uiShowDirection)
{
	UIFlowchartButton* outputButton;

	FOR_EACH_IN_EARRAY_FORWARDS(pTemplateNode->eaSwitch, DynAnimTemplateSwitch, pTemplateSwitch)
	{
		if (eAllowedDirection == pTemplateSwitch->eDirection)
		{	
			F32 fRequiredPlaytime = (ipTemplateSwitchIndex < eaSize(&pGraphNode->eaSwitch)) ? pGraphNode->eaSwitch[ipTemplateSwitchIndex]->fRequiredPlaytime : 0.f;
			char label[MAX_PATH];
			
			if (uiShowDirection) {
				if (fRequiredPlaytime > 0.f) {
					sprintf(label, "%s : %s (%.2f seconds)",
							StaticDefineIntRevLookup(DynMovementDirectionEnum,eAllowedDirection),
							NULL_TO_EMPTY(pTemplateSwitch->pcFlag),
							fRequiredPlaytime);
				} else {
					sprintf(label, "%s : %s",
							StaticDefineIntRevLookup(DynMovementDirectionEnum,eAllowedDirection),
			...
			outputButton = ui_FlowchartButtonCreate(pDoc->pFlowchart, UIFlowchartNormal, ui_LabelCreate(label, 0, 0), &pTemplateSwitch->next);
			ui_FlowchartButtonSetSingleConnection(outputButton, true);
			eaPush(peaOutputButtons, outputButton);
		}
	}
	FOR_EACH_END;
}

static void AGEFlowChartNodeAddMoves(AnimGraphDoc* pDoc, DynAnimGraphNode* pNode, UIFlowchartNode* pUINode, AGENodeData* pNodeData, int iNodeIndex, int* iMaxX, int* iX, int* iY, DynMovementDirection eAllowedDirection)
{
	int numMoves = 0;

	FOR_EACH_IN_EARRAY(pNode->eaMove, DynAnimGraphMove, pMove) {
		if (eAllowedDirection == pMove->eDirection)
			numMoves++;
	} FOR_EACH_END;
	...
	if (numMoves)
	{
		FOR_EACH_IN_EARRAY_FORWARDS(pNode->eaMove, DynAnimGraphMove, pMove)
		{
			if (eAllowedDirection == pMove->eDirection)
			{
				...
				if (0 < eAllowedDirection)
				{
					if (eAllowedDirection < DYNMOVEMENT_NUMDIRECTIONS)
						sprintf(cMove, " %s:", StaticDefineIntRevLookup(DynMovementDirectionEnum,eAllowedDirection));
					else
						sprintf(cMove, " Bad Direction:");
					...
					eaPush(&pNodeData->eaDirectionLabels, pLabel);
				}
				
				sprintf(cMove, "%s - %.2f%%", REF_STRING_FROM_HANDLE(pMove->hMove), pMove->fChance * 100.0f);
				pLabel = ui_LabelCreate(cMove, (numMoves>1)?*iX+145:*iX+100, *iY);
				pLabel->widget.widthUnit = UIUnitFitContents;
				ui_FlowchartNodeAddChild(pUINode, UI_WIDGET(pLabel), false);
				...
				if (numMoves > 1) {
					...
					ui_CheckButtonSetToggledCallback(pBiasButton, AGEMoveBiasToggled, U32_TO_PTR(iNodeIndex*1000+ipMoveIndex));
					ui_FlowchartNodeAddChild(pUINode, UI_WIDGET(pBiasButton), false);
					eaPush(&pNodeData->eaBiasButtons, pBiasButton);
				}

				*iY += MAX(numMoves>1?ui_WidgetGetHeight(UI_WIDGET(pBiasButton)):0 , ui_WidgetGetHeight(UI_WIDGET(pLabel)));
				MAX1(*iMaxX, (numMoves>1) ? ui_WidgetGetWidth(UI_WIDGET(pLabel))+145 : ui_WidgetGetWidth(UI_WIDGET(pLabel))+100);
			}
		}
		FOR_EACH_END;
	}
...
static UIFlowchartNode* AGEFlowchartNodeCreate(DynAnimGraphNode* pNode, AnimGraphDoc* pDoc, int iNodeIndex)
{
	...
	if (pTemplateNode->eType != eAnimTemplateNodeType_End)
	{
			...
			pTemplateNode->eType == eAnimTemplateNodeType_Normal)
		{
			...
			// trying to get the order to match what the animators are accustom to
			// showing directions even when invalid to help animators remove bad data & to make reflow process smoother
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_Default,		0);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_Forward,		1);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_ForwardRight,	1);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_Right,			1);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_BackwardRight,	1);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_Backward,		1);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_BackwardLeft,	1);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_Left,			1);
			AGEFlowChartNodeAddSwitchs(pDoc, pNode, pTemplateNode, &eaOutputButtons, eDynMovementDirection_ForwardLeft,	1);
		}
		...
		if (pTemplateNode->eType == eAnimTemplateNodeType_Randomizer)
		{
			...
			{
				char cPathName[16];
				if (ipPathIndex < eaSize(&pNode->eaPath))
					sprintf(cPathName, "Path %d (%s%s%.0f%%)",
								ipPathIndex+1,
								pNode->eaPath[ipPathIndex]->fChance < 1.0f ? "  " : "",
								pNode->eaPath[ipPathIndex]->fChance < 0.1f ? " " : "",
								100*pNode->eaPath[ipPathIndex]->fChance); // currently, our UI increments the fChance in +/- 5% steps
				else
...

	{
		...
		switch (pTemplateNode->eType)
		{
			...
			{
				if (pNode->bInvalidForPlayback)		eType = eAGENormalInvalid;
				else if (!eaSize(&pNode->eaMove))	eType = eAGENormalSkip;
				else								eType = eAGENormal;
			}
			xcase eAnimTemplateNodeType_Randomizer:
			{
				if (pNode->bInvalidForPlayback)	eType = eAGERandomizerInvalid;
				else							eType = eAGERandomizer;
			}
		}
...

	{
		...
		if (pTemplateNode->eType == eAnimTemplateNodeType_Normal)
		{
			// trying to get the order to match what the animators are accustom to
			// showing directions even when invalid to help animators remove bad data & to make reflow process smoother
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_Default		);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_Forward		);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_ForwardRight	);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_Right			);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_BackwardRight	);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_Backward		);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_BackwardLeft	);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_Left			);
			AGEFlowChartNodeAddMoves(pDoc, pNode, pUINode, pNodeData, iNodeIndex, &iMaxX, &iX, &iY, eDynMovementDirection_ForwardLeft	);
		}
		...
		if (pNode->bInvalidForPlayback)
		{
			UILabel* pLabel;
			iX = NODE_MARGINS;
			pLabel = ui_LabelCreate(" Skipped (Invalid Path)", iX, iY);
			pLabel->widget.widthUnit = UIUnitFitContents;
			ui_FlowchartNodeAddChild(pUINode, UI_WIDGET(pLabel), false );
			pNodeData->pInvalidLabel = pLabel;
			iY += ui_WidgetGetHeight(UI_WIDGET(pLabel));
			MAX1(iMaxX, ui_WidgetGetWidth(UI_WIDGET(pLabel)));
		}
		else if(pTemplateNode->eType == eAnimTemplateNodeType_Normal &&
				!eaSize(&pNode->eaMove))
		{
			UILabel* pLabel;
			iX = NODE_MARGINS;
			pLabel = ui_LabelCreate(" Skipped (Optional Node)", iX, iY);
			pLabel->widget.widthUnit = UIUnitFitContents;
			ui_FlowchartNodeAddChild(pUINode, UI_WIDGET(pLabel), false );
			pNodeData->pInvalidLabel = pLabel;
			iY += ui_WidgetGetHeight(UI_WIDGET(pLabel));
...
static void AGEFieldChangedCB(MEField* pField, bool bFinished, AnimGraphDoc *pDoc)
{
	bool bMoveTable  = pField && pField->pTable && pField->pTable->name && !strcmpi(pField->pTable->name, "DynAnimGraphMove");
	bool bPathTable  = pField && pField->pTable && pField->pTable->name && !strcmpi(pField->pTable->name, "DynAnimGraphPath");
	bool bSwitchTable= pField && pField->pTable && pField->pTable->name && !strcmpi(pField->pTable->name, "DynAnimGraphSwitch");
	bool bRefreshOp1 = !pDoc->bIgnoreFieldChanges && bMoveTable   && pField->pchFieldName && !strcmpi(pField->pchFieldName, "Direction");
	bool bRefreshOp2 = !pDoc->bIgnoreFieldChanges && bMoveTable   && pField->pchFieldName && !strcmpi(pField->pchFieldName, "Chance");
	bool bRefreshOp3 = !pDoc->bIgnoreFieldChanges && bPathTable   && pField->pchFieldName && !strcmpi(pField->pchFieldName, "Chance");
	bool bRefreshOp4 = !pDoc->bIgnoreFieldChanges && bSwitchTable && pField->pchFieldName && !strcmpi(pField->pchFieldName, "RequiredPlaytime");
	...
	AGEAnimGraphChanged(pDoc, bFinished, bRefreshOp1||bRefreshOp2||bRefreshOp3||bRefreshOp4, bRefreshOp1, false, true);
}
		...
		else
		{
			FOR_EACH_IN_EARRAY(pDoc->eaMoveFields, MoveMEField, pMoveField)
			{
				...
				if (GET_REF(pMove->hMove) &&
					!dynAnimGraphGroupMoveVerifyChance(pDoc->pObject, pDoc->pObject->eaNodes[pMoveField->iNodeIndex], pMove->eDirection, false))
...
static void AGEAddMove(UIButton* pButton, DynAnimGraphNode* pNode)
{
	...
	if (pDoc) {
		...
		pMove->eDirection = eDynMovementDirection_Default;
		eaPush(&pNode->eaMove, pMove);

		FOR_EACH_IN_EARRAY(pNode->eaMove, DynAnimGraphMove, pWalk) {
			if (pWalk->eDirection == pMove->eDirection)
				fNumMoves += 1.f;
		} FOR_EACH_END;
		...
		dynAnimGraphFixupValidPlaybackPaths(pDoc->pObject);
		AGEAnimGraphChanged(pDoc, false, true, true, false, false);
	}
}

static void AGESortMove(UIButton* pButton, DynAnimGraphNode* pNode)
{
	AnimGraphDoc* pDoc = (AnimGraphDoc*)emGetActiveEditorDoc();
	if (pDoc) {
		eaQSort(pNode->eaMove, dynAnimGraphCompareMoveDisplayOrder);
		AGEAnimGraphChanged(pDoc, true, true, true, false, false);
	}
}
...
void AGESetCurrentNodeAndMove(AnimGraphDoc* pDoc, const DynAnimGraphNode* pNode, const DynAnimGraphMove* pMove, DynAnimGraphUpdater* pUpdater)
{
	FOR_EACH_IN_EARRAY(pDoc->pFlowchart->nodeWindows, UIFlowchartNode, pUINode)
	{
		...
		if (pNodeData->pNode == pNode)
		{
			...
			// this needs to be updated to work with directions
					...
					dynAnimGraphVerify(pDoc->pObject, false))
		{
			...
			if (pSkeleton && pSkeleton->eaAGUpdater)
			{
				...
				if(	!pDoc->bCostumeAnimating			||
					...
					(	pCurrentNode->pGraphNode->bInvalidForPlayback ||
						!eaSize(&pCurrentNode->pGraphNode->eaMove)))
				{
					pUpdater->pEditorFxManager = NULL;
...
static EMPanel* AGEInitPropertiesPanel(AnimGraphDoc* pDoc)
{
	...
	if (REF_HANDLE_IS_ACTIVE(pDoc->pObject->hTemplate)) {
		bMovementTyped = ((DynAnimTemplate*)GET_REF(pDoc->pObject->hTemplate))->eType == eAnimTemplateType_Movement;
	} else {
		bMovementTyped = false;
	}
...
static F32 AGENodeEditReflow(AnimGraphDoc* pDoc, EMPanel* pPanel, F32 y)
{
	...
	if (editNodeType == eAnimTemplateNodeType_Normal)
	{
		...
		y = AnimEditor_ReflowNormalNode(
			...
			AGEAddMove, AGESortMove, AGEChooseMove, AGEMoveChangeChance, AGEOpenMove, AGERemoveMove,
			&ageBadDataButton,
			bMovementTyped,
			true,
			y
...
static EMPanel* AGEInitChartsPanel(AnimGraphDoc* pDoc)
{
...

	{
		...
		FOR_EACH_IN_EARRAY(pAnimChartArray->ppReferents, DynAnimChartLoadTime, pChart)
		{
			...
			FOR_EACH_IN_EARRAY(pChart->eaMoveGraphRefs, DynAnimChartGraphRefLoadTime, pGraphRef)
			{
				if (GET_REF(pGraphRef->hGraph) && REF_HANDLE_GET_STRING(pGraphRef->hGraph) == pDoc->pObject->pcName)
					count++;
...
AnimGraphDoc* AGEOpenAnimGraph(EMEditor* pEditor, char* pcName, DynAnimGraph *pGraphIn)
{
	...
	assert(!pGraph || pGraph->bReversedInheritBitPolarity);
