...
void emuOpenContainingDirectoryEx(const char *filename, directoryType type)
{
	...
	*directory = '\0';
	...
	if (type != NONE)
	{
		...
		if (type == SRC)
		{
			if (strstri(fullfilename, "/src/") <= 0)
			{
				replacementPtr = strstri(fullfilename, "/data/");
				if (replacementPtr <= 0)
				{
					return;
				}
				//I really wish there was a better way to make this swap
				replacementPtr[1] = 's';
				replacementPtr[2] = 'r';
				replacementPtr[3] = 'c';
				for (i = 4; replacementPtr[i] != '\0'; i++)
				{
					replacementPtr[i] = replacementPtr[i+1];
				}
			}
		}
		else if (type == DATA)
		{
			if (strstri(fullfilename, "/data/") <= 0)
			{
				char holder = '\0';
				char temp;
				replacementPtr = strstri(fullfilename, "/src/");
				if (replacementPtr <= 0)
				{
					return;
				}
				...
				replacementPtr[2] = 'a';
				replacementPtr[3] = 't';
				holder = replacementPtr[4];
				replacementPtr[4] = 'a';
				for (i = 5; replacementPtr[i-1] != '\0'; i++)
				{
					temp = replacementPtr[i];
					replacementPtr[i] = holder;
					holder = temp;
				}
			}
		}
