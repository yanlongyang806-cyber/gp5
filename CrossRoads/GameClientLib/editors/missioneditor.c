...
static ExprContext * MDEGetMissionExprContext(void)
{
	...
	{
		ExprFuncTable* s_pFuncTable = exprContextCreateFunctionTable("MissionEditor");
...
static ExprContext * MDEGetMissionRequiresExprContext(void)
{
	...
	if(s_pExprContext == NULL)
	{
		ExprFuncTable* s_pFuncTable = exprContextCreateFunctionTable("MissionEditor_Requires");
...
static void MDEFixupMessages(MissionDef *pRootMission, MissionDef *pMission)
{
	...
	langFixupMessageWithTerseKey(pMission->detailStringMsg.msg.pEditorCopy,
	...
	// fix mission display overrides
	n = eaSize(&pMission->eaMissionDisplayOverride);
	for (i=0; i<n; i++) {
		sprintf(buf1, "%s.eaMissionDisplayOverride[%d].DetailString", baseMessageKey, i);
		sprintf(buf2, "This is override detail text %d for %s.", i, pMission->name);
		langFixupMessageWithTerseKey(pMission->eaMissionDisplayOverride[i]->detailStringMsg.msg.pEditorCopy,
			MKP_MISSIONDETAIL, buf1, buf2, scope);

		sprintf(buf1, "%s.eaMissionDisplayOverride[%d].Summary", baseMessageKey, i);
		sprintf(buf2, "This is override Mission Summary text %d for %s.", i, pMission->name);
		langFixupMessageWithTerseKey(pMission->eaMissionDisplayOverride[i]->summaryMsg.pEditorCopy, 
			MKP_MISSIONSUMMARY, buf1, buf2, scope);
	}
	...
	if ( pMission->params ) {
		sprintf(buf1, "%s.OpenMissionRewardMessage.Gold", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing the open mission %s on success.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgGold.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);

		sprintf(buf1, "%s.OpenMissionRewardMessage.Silver", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing open mission %s on success.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgSilver.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);

		sprintf(buf1, "%s.OpenMissionRewardMessage.Bronze", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing open mission %s on success.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgBronze.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);

		sprintf(buf1, "%s.OpenMissionRewardMessage.Default", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing open mission %s on success.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgDefault.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);

		sprintf(buf1, "%s.OpenMissionRewardMessage.FailureGold", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing open mission %s on failure.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgFailureGold.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);

		sprintf(buf1, "%s.OpenMissionRewardMessage.FailureSilver", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing open mission %s on failure.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgFailureSilver.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);

		sprintf(buf1, "%s.OpenMissionRewardMessage.FailureBronze", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing open mission %s on failure.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgFailureBronze.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);

		sprintf(buf1, "%s.OpenMissionRewardMessage.FailureDefault", baseMessageKey);
		sprintf(buf2, "This is text that will be used when the player is rewarded for completing open mission %s on failure.", pMission->name);
		langFixupMessageWithTerseKey(pMission->params->RewardMsgFailureDefault.pEditorCopy,
			MKP_MISSIONPARAM, buf1, buf2, scope);
	}
}
...
static void MDESanitizeMissionForVersionCompareRecursive(MissionDef *pDef)
{
	...
	StructDestroySafe(parse_Message, &pDef->detailStringMsg.msg.pEditorCopy);
	...
	pDef->eMissionRepeatIntervalMode = kShardIntervalTimingMode_Off;
	pDef->fRepeatCooldownHours = 0;
	pDef->fRepeatCooldownHoursFromStart = 0;
	pDef->uRepeatCooldownCount = 0;
	...
	for (i = eaSize(&pDef->eaMissionDisplayOverride)-1; i >= 0; --i){
		StructDestroy(parse_MissionDisplayOverride, pDef->eaMissionDisplayOverride[i]);
		eaRemove(&pDef->eaMissionDisplayOverride, i);
	}
...
static void MDEFreeDisplayOverride(MDEDisplayOverrideGroup *pGroup)
{
	ui_WidgetQueueFree((UIWidget*)pGroup->pSummaryLabel);
	ui_WidgetQueueFree((UIWidget*)pGroup->pDetailLabel);
	ui_WidgetQueueFree((UIWidget*)pGroup->pSoundLabel);
	ui_WidgetQueueFree((UIWidget*)pGroup->pExpressionLabel);
	ui_WidgetQueueFree((UIWidget*)pGroup->pRemoveButton);
	...
	MEFieldSafeDestroy(&pGroup->pSoundField);
...
static void MDEFreeMissionGroupFields(MDEMissionGroup *pGroup)
{
	...
	eaDestroyEx(&pGroup->eaDetailMessageFields, MEFieldDestroy);
	...
	eaDestroyEx(&pGroup->eaDisplayOverrideGroups, MDEFreeDisplayOverride);
	...
	pGroup->pAddDisplayOverrideButton = NULL;
...
static void MDEAddOverrideCB(UIButton *pButton, MDEMissionGroup *pGroup)
{
	MissionDisplayOverride *pDisplayOverride;
	...
	// Add a new group
	pDisplayOverride = StructCreate(parse_MissionDisplayOverride);
	eaPush(&pGroup->pMission->eaMissionDisplayOverride, pDisplayOverride);

	// Update the UI
	MDEMissionChanged(pGroup->pDoc, true);
}

static void MDERemoveOverrideCB(UIButton *pButton, MDEDisplayOverrideGroup *pGroup)
{
	// Make sure the resource is checked out of Gimme
	if (!emDocIsEditable(&pGroup->pMissionGroup->pDoc->emDoc, true)) {
		return;
	...
	// Remove the group
	StructDestroy(parse_MissionDisplayOverride, (*pGroup->peaDisplayOverrides)[pGroup->index]);
	eaRemove(pGroup->peaDisplayOverrides, pGroup->index);
...
static F32 MDEUpdateDisplayOverrideGroup(MDEDisplayOverrideGroup *pGroup, MissionDisplayOverride *pDisplayOverride, MissionDisplayOverride *pOrigDisplayOverride, int index, F32 y, bool bIsRoot)
{
	...
	if (pGroup->pSummaryField) {
		MEFieldSetAndRefreshFromData(pGroup->pSummaryField, pOrigDisplayOverride ? &pOrigDisplayOverride->summaryMsg : NULL, &pDisplayOverride->summaryMsg);
	} else {
		pGroup->pSummaryLabel = MDECreateLabel("Summary", "Short description shown in game.  Appears below Detail in contact dialog, above Detail in mission journal.", X_OFFSET_BASE, 0, y, pGroup->pExpander);
		pGroup->pSummaryField = MEFieldCreateSimple(kMEFieldType_Message, pOrigDisplayOverride ? &pOrigDisplayOverride->summaryMsg : NULL, &pDisplayOverride->summaryMsg, parse_DisplayMessage, "EditorCopy");
		MDEAddFieldToParent(pGroup->pSummaryField, UI_WIDGET(pGroup->pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 3, pGroup->pMissionGroup->pDoc);
	}
	...
	if (bIsRoot) {
		...
		if (pGroup->pDetailField) {
			MEFieldSetAndRefreshFromData(pGroup->pDetailField, pOrigDisplayOverride ? &pOrigDisplayOverride->detailStringMsg.msg : NULL, &pDisplayOverride->detailStringMsg.msg);
		} else {
			pGroup->pDetailLabel = MDECreateLabel("Detail", "Long description shown in game.  Appears above summary in contact dialog, below summary in mission journal.", X_OFFSET_BASE, 0, y, pGroup->pExpander);
			pGroup->pDetailField = MEFieldCreateSimple(kMEFieldType_Message, pOrigDisplayOverride ? &pOrigDisplayOverride->detailStringMsg.msg : NULL, &pDisplayOverride->detailStringMsg.msg, parse_DisplayMessage, "EditorCopy");
			MDEAddFieldToParent(pGroup->pDetailField, UI_WIDGET(pGroup->pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 3, pGroup->pMissionGroup->pDoc);
		}

		// Detail sound
		if (pGroup->pSoundField) {
			MEFieldSetAndRefreshFromData(pGroup->pSoundField, SAFE_MEMBER_ADDR( pOrigDisplayOverride, detailStringMsg ), &pDisplayOverride->detailStringMsg);
		} else {
			pGroup->pSoundField = MEFieldCreateSimple(kMEFieldType_BooleanCombo, SAFE_MEMBER_ADDR( pOrigDisplayOverride, detailStringMsg ), &pDisplayOverride->detailStringMsg, parse_DisplayMessageWithVO, "HasVO");
			MDEAddFieldToParent(pGroup->pSoundField, UI_WIDGET(pGroup->pExpander), 3, y, 0, 80, UIUnitFixed, 0, pGroup->pMissionGroup->pDoc);
			pGroup->pSoundField->pUIWidget->offsetFrom = UITopRight;
			pGroup->pSoundLabel = MDECreateLabel("Needs VO", "Set if this text should be VO'd", 83 , 0, y, pGroup->pExpander);
			pGroup->pSoundLabel->widget.offsetFrom = UITopRight;
			pGroup->pDetailField->pUIWidget->rightPad = pGroup->pSoundLabel->widget.x + pGroup->pSoundLabel->widget.width + 3;
		}
	...
	if (pGroup->pExpressionField) {
		MEFieldSetAndRefreshFromData(pGroup->pExpressionField, pOrigDisplayOverride, pDisplayOverride);
	} else {
		pGroup->pExpressionLabel = MDECreateLabel("Condition", "Condition expression. If true, this detail/summary is shown instead of the default.", X_OFFSET_BASE, 0, y, pGroup->pExpander);
		pGroup->pExpressionField = MEFieldCreateSimpleExpression(kMEFieldTypeEx_Expression, pOrigDisplayOverride, pDisplayOverride, parse_MissionDisplayOverride, "Condition", MDEGetMissionExprContext());
		MDEAddFieldToParent(pGroup->pExpressionField, UI_WIDGET(pGroup->pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 3, pGroup->pMissionGroup->pDoc);
	}
	...
	// Update button
	if (!pGroup->pRemoveButton) {
		pGroup->pRemoveButton = ui_ButtonCreate("Remove", 5, y, MDERemoveOverrideCB, pGroup);
...
static void MDEUpdateDisplayOverrideExpander(MDEMissionGroup *pMissionGroup, bool bIsRoot)
{
	F32 y = Y_OFFSET_ROW;
	int i, n, m;

	if (!pMissionGroup->pDisplayOverrideExpander)
		return;

	// Put in numeric scale entries
	n = eaSize(&pMissionGroup->pMission->eaMissionDisplayOverride);
	m = eaSize(&pMissionGroup->eaDisplayOverrideGroups);
	for(i=0; i<n; ++i) {
		MDEDisplayOverrideGroup *pGroup;
		MissionDisplayOverride *pDisplayOverride;
		MissionDisplayOverride *pOrigDisplayOverride = NULL;

		if (i >= m) {
			pGroup = calloc(1, sizeof(MDEDisplayOverrideGroup));
			pGroup->pMissionGroup = pMissionGroup;
			pGroup->pExpander = pMissionGroup->pDisplayOverrideExpander;
			eaPush(&pMissionGroup->eaDisplayOverrideGroups, pGroup);
		} else {
			pGroup = pMissionGroup->eaDisplayOverrideGroups[i];
		}

		pGroup->peaDisplayOverrides = &pMissionGroup->pMission->eaMissionDisplayOverride;
		pGroup->peaOrigDisplayOverrides = (pMissionGroup->pOrigMission ? &pMissionGroup->pOrigMission->eaMissionDisplayOverride : NULL);
		pGroup->index = i;
		pDisplayOverride = pMissionGroup->pMission->eaMissionDisplayOverride[i];
		if (pMissionGroup->pOrigMission && (i < eaSize(&pMissionGroup->pOrigMission->eaMissionDisplayOverride))) {
			pOrigDisplayOverride = pMissionGroup->pOrigMission->eaMissionDisplayOverride[i];
		}

		y = MDEUpdateDisplayOverrideGroup(pMissionGroup->eaDisplayOverrideGroups[i], pDisplayOverride, pOrigDisplayOverride, i, y, bIsRoot);
	}

	// Clean up extra groups
	while(i < eaSize(&pMissionGroup->eaDisplayOverrideGroups)) {
		MDEFreeDisplayOverride(pMissionGroup->eaDisplayOverrideGroups[i]);
		eaRemove(&pMissionGroup->eaDisplayOverrideGroups, i);
	}

	// Set the expander height
	ui_ExpanderSetHeight(pMissionGroup->pDisplayOverrideExpander, y);	
}
...
static F32 MDEUpdateDropGroup(MDEDropGroup *pGroup, MissionDrop *pDrop, MissionDrop *pOrigDrop, int index, F32 y, bool bIsRoot)
{
	...
	switch(pDrop->type) {
	  ...
	  xcase MissionDropTargetType_EncounterGroup:
				...
				pcDict = NULL;
	  xcase MissionDropTargetType_EncounterGroup_AllCombatantsDead:
				pcText = "Encounter Group - All Combatants Dead";
				pcDict = NULL;
	  xdefault:
				pcText = NULL;
				pcDict = NULL;
...
static void MDEUpdateSpecialDialogOverrideGroup(MDESpecialDialogOverrideGroup *pGroup, SpecialDialogOverride *pOverride, SpecialDialogOverride *pOrigOverride)
{
	...
	if (!pGroup->pNameField) {
		pGroup->pNameField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pOrigOverride, pOverride, parse_SpecialDialogOverride, "ContactName", "Contact", "ResourceName");
...
static void MDEUpdateSpecialActionBlockOverrideGroup(MDESpecialActionBlockOverrideGroup *pGroup, ActionBlockOverride *pOverride, ActionBlockOverride *pOrigOverride)
{
	...
	if (!pGroup->pNameField) {
		pGroup->pNameField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pOrigOverride, pOverride, parse_ActionBlockOverride, "ContactName", "Contact", "ResourceName");
...
static void MDEUpdateMissionOfferOverrideGroup(MDEMissionOfferOverrideGroup *pGroup, MissionOfferOverride *pOverride, MissionOfferOverride *pOrigOverride)
{
	...
	if (!pGroup->pNameField) {
		pGroup->pNameField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pOrigOverride, pOverride, parse_MissionOfferOverride, "ContactName", "Contact", "ResourceName");
...
static void MDEFreeWaypointGroup(MDEWaypointGroup *pGroup)
{
	...
	ui_WidgetQueueFree((UIWidget*)pGroup->pMapLabel);
	ui_WidgetQueueFree((UIWidget*)pGroup->pAlternateMapLabel);
	...
	MEFieldSafeDestroy(&pGroup->pMapField);
	MEFieldSafeDestroy(&pGroup->pAlternateMapField);
	MEFieldSafeDestroy(&pGroup->pTypeField);
...
static F32 MDEUpdateWaypointGroup(MDEWaypointGroup *pGroup, MissionWaypoint *pWaypoint, MissionWaypoint *pOrigWaypoint, F32 y)
{
	...
	if (pWaypoint->type != MissionWaypointType_None)
	{
		...
		pGroup->pAlternateMapLabel = MDERefreshLabel(pGroup->pAlternateMapLabel, "Alternate Map", "If the waypoint map is unreachable, use this map", X_OFFSET_BASE, 0, y, pGroup->pExpander);
		if (!pGroup->pAlternateMapField) {
			pGroup->pAlternateMapField = MEFieldCreateSimpleDataProvided(kMEFieldType_TextEntry, pOrigWaypoint, pWaypoint, parse_MissionWaypoint, "AlternateMapName", NULL, &g_GEMapDispNames, NULL);
			MDEAddFieldToParent(pGroup->pAlternateMapField, UI_WIDGET(pGroup->pExpander), X_OFFSET_CONTROL, y, 0, 200, UIUnitFixed, 3, pGroup->pMissionGroup->pDoc);
		} else {
			ui_WidgetSetPosition(pGroup->pAlternateMapField->pUIWidget, X_OFFSET_CONTROL, y);
			MEFieldSetAndRefreshFromData(pGroup->pAlternateMapField, pOrigWaypoint, pWaypoint);
		}
		y += Y_OFFSET_ROW;
	}
...
static void MDEUpdateMissionGroup(MDEMissionGroup *pGroup, MissionDef *pMission, MissionDef *pOrigMission, bool bIsRoot)
{
	...
	for(i=eaSize(&pGroup->eaDetailMessageFields)-1; i>=0; --i) {
		MEFieldSetAndRefreshFromData(pGroup->eaDetailMessageFields[i], SAFE_MEMBER_ADDR(pOrigMission, detailStringMsg), &pMission->detailStringMsg);
	}
	...
	if (pGroup->pDetailField) {
		MEFieldSetAndRefreshFromData(pGroup->pDetailField, pOrigMission ? &pOrigMission->detailStringMsg.msg : NULL, &pMission->detailStringMsg.msg);
	}
	...
	MDEUpdateDisplayOverrideExpander(pGroup, bIsRoot);
}
...
static void MDEInitDisplayExpander(MissionEditDoc *pDoc, MDEMissionGroup *pGroup, bool bIsRoot)
{
	...
	if (bIsRoot) {
		...
		pField = MEFieldCreateSimple(kMEFieldType_Message, pGroup->pOrigMission ? &pGroup->pOrigMission->detailStringMsg.msg : NULL, &pGroup->pMission->detailStringMsg.msg, parse_DisplayMessage, "EditorCopy");
		...
		// Detail sound
		pField = MEFieldCreateSimple(kMEFieldType_BooleanCombo, SAFE_MEMBER_ADDR( pGroup->pOrigMission, detailStringMsg ), &pGroup->pMission->detailStringMsg, parse_DisplayMessageWithVO, "HasVO");
		MDEAddFieldToParent(pField, UI_WIDGET(pExpander), 3, y, 0, 80, UIUnitFixed, 0, pDoc);
		pField->pUIWidget->offsetFrom = UITopRight;
		eaPush(&pGroup->eaDetailMessageFields, pField);
		pLabel = MDECreateLabel("Needs VO", "Set if this text should be VO'd", 83 , 0, y, pExpander);
		pLabel->widget.offsetFrom = UITopRight;
		pGroup->pDetailField->pUIWidget->rightPad = pLabel->widget.x + pLabel->widget.width + 3;
		y += Y_OFFSET_ROW;
		
		// ContactForOfferVO
		pLabel = MDECreateLabel("Contact for Offer VO (optional)", "If there are multiple contacts that can offer this mission, specify which one to use for the VO.", X_OFFSET_BASE, 0, y, pExpander);
		pField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pGroup->pOrigMission, pGroup->pMission, parse_MissionDef, "ContactForOfferVO", "ContactDef", "ResourceName");
		MDEAddFieldToParent(pField, UI_WIDGET(pExpander), ui_WidgetGetNextX( UI_WIDGET( pLabel )) + 3, y, 0, 1.0, UIUnitPercentage, 3, pDoc);
		eaPush(&pGroup->eaDocFields, pField);
		y += Y_OFFSET_ROW;
	}
...
static void MDEInitDisplayOverrideExpander(MissionEditDoc *pDoc, MDEMissionGroup *pGroup)
{
	UIExpander *pExpander;
	F32 y = 0;

	// Create the expander
	pExpander = MDECreateExpander(pGroup->pExGroup, "Mission Display Overrides", "Mission Display Overrides", true);
	pGroup->pDisplayOverrideExpander = pExpander;

	// Put in add button
	pGroup->pAddDisplayOverrideButton = ui_ButtonCreate("Add Override", 15, y, MDEAddOverrideCB, pGroup);
	ui_WidgetSetWidth(UI_WIDGET(pGroup->pAddDisplayOverrideButton), 90);
	ui_ExpanderAddChild(pExpander, pGroup->pAddDisplayOverrideButton);
...
static void MDEInitOptionsExpander(MissionEditDoc *pDoc, MDEMissionGroup *pGroup, bool bIsRoot)
{
	...
	if (bIsRoot) {
		...
		pLabel = MDECreateLabel("Shard Interval Mode", "Shard Interval Mode for repeating. Set to 'Off' to use explicit hourly fields", X_OFFSET_BASE+15, 0, y, pExpander);
		pField = MEFieldCreateSimpleEnum(kMEFieldType_Combo, pGroup->pOrigMission, pGroup->pMission, parse_MissionDef, "IntervalMode", ShardIntervalTimingModeEnum);
		MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL2, y, 0, 70, UIUnitFixed, 3, pDoc);
		eaPush(&pGroup->eaDocFields, pField);
		y += Y_OFFSET_ROW;
	...
	pLabel = MDECreateLabel("Send completion notify", "If false, this will never send an Objective Complete notify.  If true and there is a Display Name, it will send a normal Objective Complete notify.", X_OFFSET_BASE, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_BooleanCombo, pGroup->pOrigMission, pGroup->pMission, parse_MissionDef, "SendCompletionNotify");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL2, y, 0, 100, UIUnitFixed, 3, pDoc);
	eaPush(&pGroup->eaDocFields, pField);
	y += Y_OFFSET_ROW;
...
static void MDEInitOpenMissionSuccessRewardsExpander(MissionEditDoc *pDoc, MDEMissionGroup *pGroup, bool bIsRoot)
{
	...
	pLabel = MDECreateLabel("Gold Message", "The message to include when the open mission rewards are displayed in a popup. If this message is unspecified, then it will use the Default Message.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgGold");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	eaPush(&pGroup->eaParamFields, pField);
	y += Y_OFFSET_ROW;
	...
	pLabel = MDECreateLabel("Silver Message", "The message to include when the open mission rewards are displayed in a popup. If this message is unspecified, then it will use the Default Message.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgSilver");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	eaPush(&pGroup->eaParamFields, pField);
	y += Y_OFFSET_ROW;
	...
	pLabel = MDECreateLabel("Bronze Message", "The message to include when the open mission rewards are displayed in a popup. If this message is unspecified, then it will use the Default Message.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgBronze");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	eaPush(&pGroup->eaParamFields, pField);
	y += Y_OFFSET_ROW;
	...
	pLabel = MDECreateLabel("Default Message", "The message to include when the open mission rewards are displayed in a popup.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgDefault");
	eaPush(&pGroup->eaParamFields, pField);
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	y += Y_OFFSET_ROW;
...
static void MDEInitOpenMissionFailureRewardsExpander(MissionEditDoc *pDoc, MDEMissionGroup *pGroup, bool bIsRoot)
{
	...
	pLabel = MDECreateLabel("Gold Message", "The message to include when the open mission rewards are displayed in a popup. If this message is unspecified, then it will use the Default Message.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgFailureGold");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	eaPush(&pGroup->eaParamFields, pField);
	y += Y_OFFSET_ROW;
	...
	pLabel = MDECreateLabel("Silver Message", "The message to include when the open mission rewards are displayed in a popup. If this message is unspecified, then it will use the Default Message.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgFailureSilver");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	eaPush(&pGroup->eaParamFields, pField);
	y += Y_OFFSET_ROW;
	...
	pLabel = MDECreateLabel("Bronze Message", "The message to include when the open mission rewards are displayed in a popup. If this message is unspecified, then it will use the Default Message.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgFailureBronze");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	eaPush(&pGroup->eaParamFields, pField);
	y += Y_OFFSET_ROW;
	...
	pLabel = MDECreateLabel("Default Message", "The message to include when the open mission rewards are displayed in a popup.", X_OFFSET_BASE2, 0, y, pExpander);
	pField = MEFieldCreateSimple(kMEFieldType_DisplayMessage, pGroup->pOrigMission ? pGroup->pOrigMission->params : NULL, pGroup->pMission->params, parse_MissionDefParams, "RewardMsgFailureDefault");
	MDEAddFieldToParent(pField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 75, pDoc);
	eaPush(&pGroup->eaParamFields, pField);
	y += Y_OFFSET_ROW;
...
static void MDEInitMissionGroupDisplay(MDEMissionGroup *pGroup, bool bIsRoot)
{
	...
	MDEInitDisplayOverrideExpander(pDoc, pGroup);
	if(bIsRoot) {
...
static void MDEInitToolbarsAndMenus(EMEditor *pEditor)
{
	...
	pTextEntry = ui_TextEntryCreateWithGlobalDictionaryCombo("", 0, 0, "Contact", "ResourceName", true, true, true, true);
...
void MDEInitData(EMEditor *pEditor)
{
	if (!gInitializedEditor) {
		...
		resSubscribeToInfoIndex("MissionDef", true);
		resSubscribeToInfoIndex("GuildStatDef", true);
		resSubscribeToInfoIndex("Contact", true);
		resSubscribeToInfoIndex("ItemAssignmentDef", true);
...
static void MDEMissionPostOpenFixup(MissionDef* pRootMission, MissionDef *pMission)
{
	...
	for(i = 0; i < eaSize(&pMission->eaMissionDisplayOverride); i++)
	{
		MissionDisplayOverride *pMissionDisplayOverride = pMission->eaMissionDisplayOverride[i];
		if(pMissionDisplayOverride)
		{
			if (pMissionDisplayOverride->condition)
			{
				exprClean(pMissionDisplayOverride->condition);
			}
		}
	}
