...
static CECostumeGroup *CECreateCostumeGroup(CECallbackFunc pDocIsEditableFunc, void* pDocIsEditableData, 
	CECallbackFunc pDialogChangedFunc, void* pDialogChangedData, 
	MEFieldChangeCallback pFieldChangeFunc, MEFieldPreChangeCallback pFieldPreChangeFunc, void* pFieldChangeData)
{
	CECostumeGroup *pGroup;

	pGroup = calloc(1, sizeof(CECostumeGroup));
	pGroup->pDocIsEditableFunc = pDocIsEditableFunc;
	pGroup->pDocIsEditableData = pDocIsEditableData;
	pGroup->pDialogChangedFunc = pDialogChangedFunc;
	pGroup->pDialogChangedData = pDialogChangedData;
...
static void CEFreeCostumeGroup(CECostumeGroup *pGroup)
{
	// Free labels
	...
	ui_WidgetQueueFreeAndNull(&pGroup->pHeadshotBorderFrameLabel);
	ui_WidgetQueueFreeAndNull(&pGroup->pHeadshotStyleLabel);
	...
	MEFieldSafeDestroy(&pGroup->pHeadshotBorderFrameField);
	MEFieldSafeDestroy(&pGroup->pHeadshotStyleField);

	ui_WidgetQueueFreeAndNull(&pGroup->pStyleOverridesCheckButton);
	ui_WidgetQueueFreeAndNull(&pGroup->pFrameOverrideCheckButton);

	eaDestroyStruct(&pGroup->eaFrames, parse_HeadShotFrame);

	ui_WidgetQueueFreeAndNull(&pGroup->pOverrideBackground);
	ui_WidgetQueueFreeAndNull(&pGroup->pOverrideSky);
	ui_WidgetQueueFreeAndNull(&pGroup->pOverrideFX);
	ui_WidgetQueueFreeAndNull(&pGroup->pOverrideFrame);

	MEFieldSafeDestroy(&pGroup->pOverrideBackgroundField);
	MEFieldSafeDestroy(&pGroup->pOverrideSkyField);
	MEFieldSafeDestroy(&pGroup->pOverrideFXField);
	MEFieldSafeDestroy(&pGroup->pOverrideFrameField);
...
void CEFreeSpecialDialogGroup(CESpecialDialogGroup *pGroup)
{
	...
	for(i=eaSize(&pGroup->eaCostumeGroups)-1; i>=0; --i) {
		CEFreeCostumeGroup(pGroup->eaCostumeGroups[i]);
	}
	eaDestroy(&pGroup->eaCostumeGroups);
...
void CEFreeOfferGroup(CEOfferGroup *pGroup)
{
	...
	ui_WidgetQueueFree((UIWidget*)pGroup->pRequireAllAllegianceLabel);
	...
	if (pGroup->pRequireAllAllegianceField) {
		MEFieldDestroy(pGroup->pRequireAllAllegianceField);
	}
...
void CEStripEmptyDialogBlocks(DialogBlock ***peaBlocks)
{
	...
	for(i=eaSize(peaBlocks)-1; i>=0; --i) {
		...
		if ((!pBlock->displayTextMesg.msg.pEditorCopy || !pBlock->displayTextMesg.msg.pEditorCopy->pcDefaultString || !pBlock->displayTextMesg.msg.pEditorCopy->pcDefaultString[0]) &&
...
static void CEFixupDialogBlockMessages(DialogBlock*** peaBlocks, const char* pcKey, const char* pcDesc)
{
	...
	for(i=eaSize(peaBlocks)-1; i>=0; --i) {
		...
		if (pBlock) {
			...
			langFixupMessage(pBlock->displayTextMesg.msg.pEditorCopy, keyBuf, pcDesc, "ContactDef");
	...
	if (pBlock) {
		if(pBlock->dialogBlock) {
			for(k = eaSize(&pBlock->dialogBlock)-1; k>=0; --k)
			{
				...
				langFixupMessage(pBlock->dialogBlock[k]->displayTextMesg.msg.pEditorCopy, keyBuf, pcDesc, "ContactDef");
...
static void CEAddCostumeCB(UIButton *pButton, CECostumeGroup *pGroup)
{
	// Make sure the resource is checked out of Gimme
	if (pGroup->pDocIsEditableFunc && !pGroup->pDocIsEditableFunc(pGroup->pDocIsEditableData)) {
		return;
	...
	// Add a new group
	eaPush(pGroup->peaExtraCostumes, StructCreate(parse_ContactCostume));
...
static void CERemoveCostumeCB(UIButton *pButton, CECostumeGroup *pGroup)
{
	// Make sure the resource is checked out of Gimme
	if (pGroup->pDocIsEditableFunc && !pGroup->pDocIsEditableFunc(pGroup->pDocIsEditableData)) {
		return;
	...
	// Remove the group
	StructDestroy(parse_ContactCostume, (*pGroup->peaExtraCostumes)[pGroup->index]);
	eaRemove(pGroup->peaExtraCostumes, pGroup->index);
...
static void CEToggleCostumeStyleOverrides(UICheckButton *pCheckButton, CECostumeGroup *pGroup)
{
	ContactCostume *pCostume;
	...
	pCostume = pGroup->index < 0 ? pGroup->pCostumePrefs : (*pGroup->peaExtraCostumes)[pGroup->index];
	if (pCostume)
	{
		if (pCheckButton->state && !pCostume->pCostumeStyleOverrides)
			pCostume->pCostumeStyleOverrides = StructCreate(parse_ContactCostumeStyle);
		else if (!pCheckButton->state && pCostume->pCostumeStyleOverrides)
			StructDestroySafe(parse_ContactCostumeStyle, &pCostume->pCostumeStyleOverrides);
	}
...
static void CEToggleCostumeFrameOverride(UICheckButton *pCheckButton, CECostumeGroup *pGroup)
{
	ContactCostume *pCostume;
	...
	if (pCostume)
	{
		if (!pCheckButton->state)
			pCostume->pchHeadshotBorderFrame = NULL;
	}
...
static F32 CERefreshDialogGroup(CEDialogGroup *pGroup, UIExpander *pExpander, F32 y, F32 xPercentWidth, int index, DialogBlock ***peaBlocks, char *pcLabel, char *pcTooltip, DialogBlock *pBlock, DialogBlock *pOldBlock, bool bShowAnimList, bool bShowCondition, bool bPrependLabelToCondition, bool bSplitView)
{
	...
	if (!pGroup->pMessageField) {
		pGroup->pMessageField = MEFieldCreateSimple(kMEFieldType_Message, pOldBlock ? &pOldBlock->displayTextMesg.msg : NULL, &pBlock->displayTextMesg.msg, parse_DisplayMessage, "editorCopy");
		...
		MEFieldSetAndRefreshFromData(pGroup->pMessageField, pOldBlock ? &pOldBlock->displayTextMesg.msg : NULL, &pBlock->displayTextMesg.msg);
	}
	...
	if(bSplitView)
	{
		...
		if (!pGroup->pAudioField) {
			pGroup->pAudioField = MEFieldCreateSimple(kMEFieldType_BooleanCombo, SAFE_MEMBER_ADDR( pOldBlock, displayTextMesg ), &pBlock->displayTextMesg, parse_DisplayMessageWithVO, "HasVO");
			...
			MEFieldSetAndRefreshFromData(pGroup->pAudioField, SAFE_MEMBER_ADDR( pOldBlock, displayTextMesg ), &pBlock->displayTextMesg);
		}
	...
	else
	{
		...
		// Update audio
		pGroup->pSoundLabel = CERefreshLabel(pGroup->pSoundLabel, "Needs VO", "Set if this text should be VO'd", X_OFFSET_BASE+15, 0, y, pExpander);
		if (!pGroup->pAudioField) {
			pGroup->pAudioField = MEFieldCreateSimple(kMEFieldType_BooleanCombo, SAFE_MEMBER_ADDR( pOldBlock, displayTextMesg ), &pBlock->displayTextMesg, parse_DisplayMessageWithVO, "HasVO");
...
static F32 CERefreshCostumeGroup(CECostumeGroup *pGroup, UIExpander *pExpander, F32 y, F32 xPercentWidth, int index, ContactCostume ***peaExtraCostumes, char *pcLabel, char *pcTooltip, ContactCostume *pCostume, ContactCostume *pOldCostume, char ***peaVarNames)
{
	bool bHaveOverrideCostume = false;
	bool bHaveExtraCostumeButton = false;

	pGroup->index = index;
	pGroup->peaExtraCostumes = peaExtraCostumes;

	if (peaExtraCostumes && eaSize(peaExtraCostumes) > 0 && 0) {
		y += STANDARD_ROW_HEIGHT * 0.5;
	}

	// Costume Section
	pGroup->pCostumeTypeLabel = CERefreshLabel(pGroup->pCostumeTypeLabel, index >= 0 ? "Extra Costume From" : "Costume From", "Where the extra contact costume should be pulled from", X_OFFSET_BASE, 0, y, pExpander);
	if(!pGroup->pCostumeTypeField) {
		pGroup->pCostumeTypeField = MEFieldCreateSimpleEnum(kMEFieldType_Combo, pOldCostume, pCostume, parse_ContactCostume, "CostumeType", ContactCostumeTypeEnum);
		GEAddFieldToParent(pGroup->pCostumeTypeField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, xPercentWidth, UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
	} else {
		ui_WidgetSetPosition(pGroup->pCostumeTypeField->pUIWidget, X_OFFSET_CONTROL, y);
		MEFieldSetAndRefreshFromData(pGroup->pCostumeTypeField, pOldCostume, pCostume);
	}

	// Update button
	// Arbitrarily limited to adding 1 extra costume
	if (peaExtraCostumes && (index >= 0 || eaSize(peaExtraCostumes) < 1)) {
		if (!pGroup->pRemoveButton) {
			pGroup->pRemoveButton = ui_ButtonCreate((index == -1) ? "Add" : "Remove", 0, y, (index == -1) ? CEAddCostumeCB : CERemoveCostumeCB, pGroup);
			ui_WidgetSetWidth(UI_WIDGET(pGroup->pRemoveButton), 80);
			ui_WidgetSetPositionEx(UI_WIDGET(pGroup->pRemoveButton), 10, y, xPercentWidth, 0, UITopLeft);
			ui_ExpanderAddChild(pExpander, pGroup->pRemoveButton);
		} else {
			ui_WidgetSetPositionEx(UI_WIDGET(pGroup->pRemoveButton), 10, y, xPercentWidth, 0, UITopLeft);
			ui_ButtonSetText(pGroup->pRemoveButton, (index == -1) ? "Add" : "Remove");
			ui_ButtonSetCallback(pGroup->pRemoveButton, (index == -1) ? CEAddCostumeCB : CERemoveCostumeCB, pGroup);
		}
		bHaveExtraCostumeButton = true;
	} else {
		ui_WidgetQueueFreeAndNull(&pGroup->pRemoveButton);
		bHaveExtraCostumeButton = false;
	}

	if (!pGroup->pFrameOverrideCheckButton) {
		pGroup->pFrameOverrideCheckButton = ui_CheckButtonCreate(0, y, "Add Border", pCostume->pchHeadshotBorderFrame && *pCostume->pchHeadshotBorderFrame);
		pGroup->pFrameOverrideCheckButton->toggledF = CEToggleCostumeFrameOverride;
		pGroup->pFrameOverrideCheckButton->toggledData = pGroup;
		ui_ExpanderAddChild(pExpander, pGroup->pFrameOverrideCheckButton);
	}
	ui_WidgetSetPositionEx(UI_WIDGET(pGroup->pFrameOverrideCheckButton), bHaveExtraCostumeButton ? 100 : 10, y, xPercentWidth, 0, UITopLeft);

	// Specified Costume
	if(pCostume && pCostume->eCostumeType == ContactCostumeType_Specified)
	{
		bHaveOverrideCostume = true;
		y += STANDARD_ROW_HEIGHT;
		...
		if(!pGroup->pCostumeField) {
			pGroup->pCostumeField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pOldCostume, pCostume, parse_ContactCostume, "CostumeOverride", "PlayerCostume", "ResourceName");
			GEAddFieldToParent(pGroup->pCostumeField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT, UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pCostumeField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
			MEFieldSetAndRefreshFromData(pGroup->pCostumeField, pOldCostume, pCostume);
		}
	} else {
		MEFieldSafeDestroy(&pGroup->pCostumeField);
	}
	...
	// PetContactList
	if(pCostume && pCostume->eCostumeType == ContactCostumeType_PetContactList)
	{
		bHaveOverrideCostume = true;
		y += STANDARD_ROW_HEIGHT;
		...
		if(!pGroup->pPetContactField) {
			pGroup->pPetContactField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pOldCostume, pCostume, parse_ContactCostume, "UsePetCostume", "PetContactList", "ResourceName");
			GEAddFieldToParent(pGroup->pPetContactField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pPetContactField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
			MEFieldSetAndRefreshFromData(pGroup->pPetContactField, pOldCostume, pCostume);
		}
	} else {
		MEFieldSafeDestroy(&pGroup->pPetContactField);
	}
	...
	// CritterGroup
	if(pCostume && pCostume->eCostumeType == ContactCostumeType_CritterGroup)
	{
		y += STANDARD_ROW_HEIGHT;
		// Critter Group type
		...
		if(!pGroup->pCritterGroupTypeField) {
			pGroup->pCritterGroupTypeField = MEFieldCreateSimpleEnum(kMEFieldType_Combo, pOldCostume, pCostume, parse_ContactCostume, "CostumeCritterGroupType", ContactMapVarOverrideTypeEnum);
			GEAddFieldToParent(pGroup->pCritterGroupTypeField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pCritterGroupTypeField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
			MEFieldSetAndRefreshFromData(pGroup->pCritterGroupTypeField, pOldCostume, pCostume);
		}

		y += STANDARD_ROW_HEIGHT;

		// Specified Critter Group
		if(pCostume && pCostume->eCostumeCritterGroupType == ContactMapVarOverrideType_Specified) {
			bHaveOverrideCostume = true;
			...
			if(!pGroup->pCritterGroupField) {
				pGroup->pCritterGroupField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pOldCostume, pCostume, parse_ContactCostume, "CostumeCritterGroup", "CritterGroup", "resourceName");
				GEAddFieldToParent(pGroup->pCritterGroupField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
			} else {
				ui_WidgetSetPosition(pGroup->pCritterGroupField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
				MEFieldSetAndRefreshFromData(pGroup->pCritterGroupField, pOldCostume, pCostume);
			}
		} else {
			MEFieldSafeDestroy(&pGroup->pCritterGroupField);
		}
		...
		if(pCostume && pCostume->eCostumeCritterGroupType == ContactMapVarOverrideType_MapVar) {
			bHaveOverrideCostume = true;
			...
			if(!pGroup->pCritterMapVarField) {
				pGroup->pCritterMapVarField = MEFieldCreateSimpleDataProvided(kMEFieldType_TextEntry, pOldCostume, pCostume, parse_ContactCostume, "CostumeMapVar", NULL, peaVarNames, NULL);
				GEAddFieldToParent(pGroup->pCritterMapVarField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
			} else {
				ui_WidgetSetPosition(pGroup->pCritterMapVarField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
				MEFieldSetAndRefreshFromData(pGroup->pCritterMapVarField, pOldCostume, pCostume);
			}
		} else {
			MEFieldSafeDestroy(&pGroup->pCritterMapVarField);
		}
		...
		pGroup->pCritterGroupIdentifierLabel = CERefreshLabel(pGroup->pCritterGroupIdentifierLabel, "Name", "A name for this costume.  To use this same costume again, use the same critter group and same name.", X_OFFSET_AUDIO, X_PERCENT_SPLIT, y, pExpander); 
		if (!pGroup->pCritterGroupIdentifierField) {
			pGroup->pCritterGroupIdentifierField = MEFieldCreateSimple(kMEFieldType_TextEntry, pOldCostume, pCostume, parse_ContactCostume, "CostumeIdentifier");
			GEAddFieldToParent(pGroup->pCritterGroupIdentifierField, UI_WIDGET(pExpander), X_OFFSET_AUDIO+45, y, X_PERCENT_SPLIT, 0.20, UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPositionEx(pGroup->pCritterGroupIdentifierField->pUIWidget, X_OFFSET_AUDIO+45, y, X_PERCENT_SPLIT, 0, UITopLeft);
			MEFieldSetAndRefreshFromData(pGroup->pCritterGroupIdentifierField, pOldCostume, pCostume);
		}
	} else {
		ui_WidgetQueueFreeAndNull(&pGroup->pCritterGroupTypeLabel);
		ui_WidgetQueueFreeAndNull(&pGroup->pCritterGroupIdentifierLabel);
	...
	// Only display the border frame option if the check button is set
	if (pGroup->pFrameOverrideCheckButton->state) {
		y += STANDARD_ROW_HEIGHT;
		pGroup->pHeadshotBorderFrameLabel = CERefreshLabel(pGroup->pHeadshotBorderFrameLabel, "Headshot Border Frame", "The border frame style to use for this contact.", X_OFFSET_BASE, 0, y, pExpander); 
		if (!pGroup->pHeadshotBorderFrameField) {
			pGroup->pHeadshotBorderFrameField = MEFieldCreateSimple(kMEFieldType_Texture, pOldCostume, pCostume, parse_ContactCostume, "HeadshotBorderFrame");
			GEAddFieldToParent(pGroup->pHeadshotBorderFrameField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, xPercentWidth, UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pHeadshotBorderFrameField->pUIWidget, X_OFFSET_CONTROL, y);
			MEFieldSetAndRefreshFromData(pGroup->pHeadshotBorderFrameField, pOldCostume, pCostume);
		}
		y += 40;
	} else {
		ui_WidgetQueueFreeAndNull(&pGroup->pHeadshotBorderFrameLabel);
	...
	// Put the headshot style after costumePrefs costume
	y += STANDARD_ROW_HEIGHT;
	pGroup->pHeadshotStyleLabel = CERefreshLabel(pGroup->pHeadshotStyleLabel, "Headshot Style", "The headshot style to use for this contact.", X_OFFSET_BASE, 0, y, pExpander); 
	if (!pGroup->pHeadshotStyleField) {
		pGroup->pHeadshotStyleField = MEFieldCreateSimpleDictionary(kMEFieldType_ValidatedTextEntry, pOldCostume, pCostume, parse_ContactCostume, "HeadshotStyleDef", "HeadshotStyleDef", parse_HeadshotStyleDef, "Name");
		GEAddFieldToParent(pGroup->pHeadshotStyleField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, xPercentWidth, UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
	} else {
		ui_WidgetSetPosition(pGroup->pHeadshotStyleField->pUIWidget, X_OFFSET_CONTROL, y);
		MEFieldSetAndRefreshFromData(pGroup->pHeadshotStyleField, pOldCostume, pCostume);
	}

	if (!pGroup->pStyleOverridesCheckButton) {
		pGroup->pStyleOverridesCheckButton = ui_CheckButtonCreate(0, y, "Override Style", pCostume->pCostumeStyleOverrides != NULL);
		pGroup->pStyleOverridesCheckButton->toggledF = CEToggleCostumeStyleOverrides;
		pGroup->pStyleOverridesCheckButton->toggledData = pGroup;
		ui_ExpanderAddChild(pExpander, pGroup->pStyleOverridesCheckButton);
	}
	ui_WidgetSetPositionEx(UI_WIDGET(pGroup->pStyleOverridesCheckButton), 10, y, xPercentWidth, 0, UITopLeft);

	if (pCostume->pCostumeStyleOverrides) {
		ContactCostumeStyle *pOldOverrides = SAFE_MEMBER(pOldCostume, pCostumeStyleOverrides);
		ContactCostumeStyle *pOverrides = pCostume->pCostumeStyleOverrides;

		y += STANDARD_ROW_HEIGHT;
		pGroup->pOverrideBackground = CERefreshLabel(pGroup->pOverrideBackground, "Background Image", "The background texture override on the headshot", X_OFFSET_BASE+X_OFFSET_BASE, 0, y, pExpander); 
		if (!pGroup->pOverrideBackgroundField) {
			pGroup->pOverrideBackgroundField = MEFieldCreateSimple(kMEFieldType_Texture, pOldOverrides, pOverrides, parse_ContactCostumeStyle, "BackgroundImage");
			GEAddFieldToParent(pGroup->pOverrideBackgroundField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pOverrideBackgroundField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
			MEFieldSetAndRefreshFromData(pGroup->pOverrideBackgroundField, pOldOverrides, pOverrides);
		}

		y += STANDARD_ROW_HEIGHT + 40;
		pGroup->pOverrideSky = CERefreshLabel(pGroup->pOverrideSky, "Sky", "The sky override on the headshot", X_OFFSET_BASE+X_OFFSET_BASE, 0, y, pExpander); 
		if (!pGroup->pOverrideSkyField) {
			pGroup->pOverrideSkyField = MEFieldCreateSimpleDictionary(kMEFieldType_ValidatedTextEntry, pOldOverrides, pOverrides, parse_ContactCostumeStyle, "Sky", "SkyInfo", parse_SkyInfo, "FNNoPath");
			GEAddFieldToParent(pGroup->pOverrideSkyField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pOverrideSkyField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
			MEFieldSetAndRefreshFromData(pGroup->pOverrideSkyField, pOldOverrides, pOverrides);
		}

		y += STANDARD_ROW_HEIGHT;
		pGroup->pOverrideFX = CERefreshLabel(pGroup->pOverrideFX, "FX", "The fx overrides on the headshot", X_OFFSET_BASE+X_OFFSET_BASE, 0, y, pExpander); 
		if (!pGroup->pOverrideFXField) {
			pGroup->pOverrideFXField = MEFieldCreateSimpleDictionary(kMEFieldType_MultiText, pOldOverrides, pOverrides, parse_ContactCostumeStyle, "HeadshotFX", "DynFxInfo", parse_DynFxInfo, "InternalName");
			GEAddFieldToParent(pGroup->pOverrideFXField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pOverrideFXField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
			MEFieldSetAndRefreshFromData(pGroup->pOverrideFXField, pOldOverrides, pOverrides);
		}

		// Copy all unique frame names
		eaClearStruct(&pGroup->eaFrames, parse_HeadShotFrame);
		eaIndexedEnable(&pGroup->eaFrames, parse_HeadShotFrame);
		FOR_EACH_IN_REFDICT("SkelHeadshotInfo", SkelHeadshotInfo, pHeadshotInfo)
		{
			S32 i;
			for (i = eaSize(&pHeadshotInfo->eaHeadShotFrame) - 1; i >= 0; i--)
			{
				if (eaIndexedFindUsingString(&pGroup->eaFrames, pHeadshotInfo->eaHeadShotFrame[i]->pcFrameName) < 0)
					eaIndexedAdd(&pGroup->eaFrames, StructClone(parse_HeadShotFrame, pHeadshotInfo->eaHeadShotFrame[i]));
			}
		}
		FOR_EACH_END

		y += STANDARD_ROW_HEIGHT;
		pGroup->pOverrideFrame = CERefreshLabel(pGroup->pOverrideFrame, "Frame", "The camera framing of the costume", X_OFFSET_BASE+X_OFFSET_BASE, 0, y, pExpander); 
		if (!pGroup->pOverrideFrameField) {
			pGroup->pOverrideFrameField = MEFieldCreateSimpleDataProvided(kMEFieldType_ValidatedTextEntry, pOldOverrides, pOverrides, parse_ContactCostumeStyle, "Frame", parse_HeadShotFrame, &pGroup->eaFrames, "FrameName");
			GEAddFieldToParent(pGroup->pOverrideFrameField, UI_WIDGET(pExpander), X_OFFSET_CONTROL+X_OFFSET_BASE, y, 0, X_PERCENT_SPLIT,UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
		} else {
			ui_WidgetSetPosition(pGroup->pOverrideFrameField->pUIWidget, X_OFFSET_CONTROL+X_OFFSET_BASE, y);
			MEFieldSetAndRefreshFromData(pGroup->pOverrideFrameField, pOldOverrides, pOverrides);
		}
	} else {
		ui_WidgetQueueFreeAndNull(&pGroup->pOverrideBackground);
		ui_WidgetQueueFreeAndNull(&pGroup->pOverrideSky);
	...
	// Extra costume 
	if (peaExtraCostumes && eaSize(peaExtraCostumes) > 0 && index == eaSize(peaExtraCostumes) - 1 && 0) {
		y += STANDARD_ROW_HEIGHT * 0.5;
	}
...
static int CERefreshCostumes(CECostumeGroup ***peaGroups, UIExpander *pExpander, F32* y, F32 xPercentWidth, int n, char *pcLabel, char *pcTooltip, ContactCostume *pCostumePref, ContactCostume *pOldCostumePref, ContactCostume ***peaCostumes, ContactCostume ***peaOldCostumes, char ***peaVarNames, bool bCostumeList,
							 CECallbackFunc pDocIsEditableFunc, void* pDocIsEditableData, 
							 CECallbackFunc pCostumeChangedFunc, void* pCostumeChangedData, 
							 MEFieldChangeCallback pFieldChangeFunc, MEFieldPreChangeCallback pFieldPreChangeFunc, void* pFieldChangeData)
{
	ContactCostume *pCostume, *pOldCostume;
	int i, count = 0;

	if (pCostumePref) {
		while (n >= eaSize(peaGroups)) {
			CECostumeGroup *pGroup = CECreateCostumeGroup(pDocIsEditableFunc, pDocIsEditableData, pCostumeChangedFunc, pCostumeChangedData, pFieldChangeFunc, pFieldPreChangeFunc, pFieldChangeData);
			eaPush(peaGroups, pGroup);
		}
		(*peaGroups)[n]->pCostumePrefs = pCostumePref;
		*y = CERefreshCostumeGroup((*peaGroups)[n], pExpander, (*y), xPercentWidth, -1, peaCostumes, pcLabel, pcTooltip, pCostumePref, pOldCostumePref, peaVarNames);
		++n;
		++count;
	}

	if (peaCostumes) {
		for(i=0; i<eaSize(peaCostumes) && bCostumeList; ++i) {
			pCostume = (*peaCostumes)[i];
			if (peaOldCostumes && (eaSize(peaOldCostumes) > i)) {
				pOldCostume = (*peaOldCostumes)[i];
			} else {
				pOldCostume = NULL;
			}
			while (n >= eaSize(peaGroups)) {
			...
			(*peaGroups)[n]->pCostumePrefs = pCostumePref;
			*y = CERefreshCostumeGroup((*peaGroups)[n], pExpander, (*y), xPercentWidth, i, peaCostumes, pcLabel, pcTooltip, pCostume, pOldCostume, peaVarNames);
			++n;
			++count;
		}
	}
...
static void CERefreshInfoExpander(ContactEditDoc *pDoc)
{
	...
	// Costume Section
	nCostumes += CERefreshCostumes(&pDoc->eaCostumeGroups, pDoc->pInfoExpander, &y, X_PERCENT_SPLIT, nCostumes,
								   "Costume From", "Where the contact's costume should be pulled from",
								   &pDoc->pContact->costumePrefs, pDoc->pOrigContact ? &pDoc->pOrigContact->costumePrefs : NULL,
								   NULL, NULL, &pDoc->eaVarNames, false,
								   CEIsDocEditable, pDoc,
								   CEUpdateUI, pDoc,
								   CEFieldChangedCB, CEFieldPreChangeCB, pDoc);

	while (eaSize(&pDoc->eaCostumeGroups) > nCostumes){
		CEFreeCostumeGroup(eaPop(&pDoc->eaCostumeGroups));
	}
	...
	if (!pDoc->pCutSceneField) {
		pDoc->pCutSceneField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pDoc->pOrigContact, pDoc->pContact, parse_ContactDef, "CutSceneDef", "CutScene", "ResourceName");
	...
	// StickerBookCollection field
	if (contact_IsZoneRewards(pDoc->pContact))
	{
		pDoc->pStickerBookCollectionLabel = CERefreshLabel(pDoc->pStickerBookCollectionLabel, "Sticker Book", "Sticker Book Collection name of the Zone Rewards Collection", X_OFFSET_BASE, 0, y, pDoc->pInfoExpander);
		if (!pDoc->pStickerBookCollectionField) {
			pDoc->pStickerBookCollectionField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pDoc->pOrigContact, pDoc->pContact, parse_ContactDef, "StickerBookCollection", "StickerBook", "ResourceName");
			CEAddFieldToParent(pDoc->pStickerBookCollectionField, UI_WIDGET(pDoc->pInfoExpander), X_OFFSET_CONTROL, y, 0, 100, UIUnitFixed, 0, pDoc);
		} else {
			ui_WidgetSetPosition(pDoc->pStickerBookCollectionField->pUIWidget, X_OFFSET_CONTROL, y);
			MEFieldSetAndRefreshFromData(pDoc->pStickerBookCollectionField, pDoc->pOrigContact, pDoc->pContact);
		}
		y += STANDARD_ROW_HEIGHT;
	}
	else
	{
		REMOVE_HANDLE(pDoc->pContact->hStickerBookCollection);
		ui_WidgetQueueFreeAndNull(&pDoc->pStickerBookCollectionLabel);
		MEFieldSafeDestroy(&pDoc->pStickerBookCollectionField);
	}
												...
												F32 x, F32 y)
{
	...
	else
	{
		...
		if (eaSize(&pContactDialogNode->pMissionOffer->completedDialog) > 0 && 
			pContactDialogNode->pMissionOffer->completedDialog[0]->displayTextMesg.msg.pEditorCopy &&
			pContactDialogNode->pMissionOffer->completedDialog[0]->displayTextMesg.msg.pEditorCopy->pcDefaultString)
		{
			estrConcatf(&estrTooltipText, "<b>Completed Text:</b> %s<br><br>", pContactDialogNode->pMissionOffer->completedDialog[0]->displayTextMesg.msg.pEditorCopy->pcDefaultString); 
		}
		...
		if (pDialogNode->bIsDialogRoot)
		{
			...
			FOR_EACH_IN_CONST_EARRAY_FORWARDS(pSpecialDialogBlock->dialogBlock, DialogBlock, pDialogBlock)
			{
				estrConcatf(&estrDisplayText, "<b>Display Text %d:</b> %s<br><br>", 
					...
					pDialogBlock->displayTextMesg.msg.pEditorCopy && pDialogBlock->displayTextMesg.msg.pEditorCopy->pcDefaultString ? CEGetFormattedDialogText(pDialogBlock->displayTextMesg.msg.pEditorCopy->pcDefaultString, GET_REF(pDialogBlock->hDialogFormatter)) : "");
...
{	
	...
	for (i = 0; i < eaSize(peaDialogBlocks); ++i) 
	{
		...
		ui_WidgetSetWidthEx(UI_WIDGET(pGroup->pMessageTextEntry), xPercentWidth, UIUnitPercentage);					
		ui_TextEntrySetText(pGroup->pMessageTextEntry, TranslateDisplayMessage(pBlock->displayTextMesg.msg));
...
int CERefreshSpecialGroup(CESpecialDialogGroup *pGroup, UIExpander *pExpander, F32 y, F32 xPercentWidth, int index, void ***peaSpecialDialogWrappers, SpecialDialogBlock *pBlock, SpecialDialogBlock *pOldBlock, bool bSplitView, MissionDef *pMissionDef)
{
	...
	if(bSplitView)
	{
		pGroup->pSoundLabel = CERefreshLabel(pGroup->pSoundLabel, "Needs VO / Phrase to Say", "Set if this text should be VO'd (top field), or the voice over phrase to say (bottom field) in a voice based on the contact's costume", X_OFFSET_AUDIO, X_PERCENT_SPLIT, y, pExpander);
	...
	// Add costumes costume
	nCostumes += CERefreshCostumes(&pGroup->eaCostumeGroups, pExpander, &y, xPercentWidth, nCostumes, "Costumes", "The primary costume",  
							&pBlock->costumePrefs, pOldBlock ? &pOldBlock->costumePrefs : NULL, &pBlock->extraCostumes, pOldBlock ? &pOldBlock->extraCostumes : NULL, pGroup->peaVarNames, true,
							pGroup->pCommonCallbackParams->pDocIsEditableFunc, pGroup->pCommonCallbackParams->pDocIsEditableData,
							pGroup->pCommonCallbackParams->pDialogChangedFunc, pGroup->pCommonCallbackParams->pDialogChangedData,
							pGroup->pCommonCallbackParams->pFieldChangeFunc, pGroup->pCommonCallbackParams->pFieldPreChangeFunc, pGroup->pCommonCallbackParams->pFieldChangeData);

	while (eaSize(&pGroup->eaCostumeGroups) > nCostumes){
		CEFreeCostumeGroup(eaPop(&pGroup->eaCostumeGroups));
	}
	...
	if (!pGroup->pCutSceneField) {
		pGroup->pCutSceneField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_ValidatedTextEntry, pOldBlock, pBlock, parse_SpecialDialogBlock, "CutSceneDef", "CutScene", "ResourceName");
...
{	
	...
	pGroup->pRequireAllAllegianceLabel = CERefreshLabel(pGroup->pRequireAllAllegianceLabel, "Require All Allegiances", "Set this to True if the player must have all of the required allegiances to get this offer", X_OFFSET_BASE, 0, y, pExpander);
	if (!pGroup->pRequireAllAllegianceField) {
		pGroup->pRequireAllAllegianceField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_BooleanCombo, pOldOffer, pOffer, parse_ContactMissionOffer, "RequireAllAllegiances", "Allegiance", "resourceName");
		GEAddFieldToParent(pGroup->pRequireAllAllegianceField, UI_WIDGET(pExpander), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 0, pGroup->pFieldChangeFunc, pGroup->pFieldPreChangeFunc, pGroup->pFieldChangeData);
	} else {
		ui_WidgetSetPosition(pGroup->pRequireAllAllegianceField->pUIWidget, X_OFFSET_CONTROL, y);
		MEFieldSetAndRefreshFromData(pGroup->pRequireAllAllegianceField, pOldOffer, pOffer);
	}
...
static void CEInitDialogExpander(ContactEditDoc *pDoc)
{
	...
	pLabel = CECreateLabel("Needs VO / Phrase to Say", "Set if this text should be VO'd (top field), or the voice over phrase to say (bottom field) in a voice based on the contact's costume", 0, 0, pDoc->pDialogExpander);
									 ...
									 const char *pchContactName, const char *pchMissionName)
{
	if (pFile && 
		pDialogBlock && 
		pDialogBlock->displayTextMesg.msg.pEditorCopy && 
		pDialogBlock->displayTextMesg.msg.pEditorCopy->pcDefaultString)
	{
		...
		estrAppend2(&estrDialogText, pDialogBlock->displayTextMesg.msg.pEditorCopy->pcDefaultString);
...
static UIWindow *CEInitMainWindow(ContactEditDoc *pDoc)
{
	...
	// Contact VO comments
	pLabel = ui_LabelCreate("VO Comments", 0, y);
	ui_LabelEnableTooltips(pLabel);
	ui_WidgetSetTooltipString(UI_WIDGET(pLabel), "Information for Audio on how to VO this.  Commonly used info: Gender, Voice, Alternate DisplayName" );
	ui_WindowAddChild(pWin, pLabel);
	pField = MEFieldCreateSimple(kMEFieldType_TextEntry, pDoc->pOrigContact, pDoc->pContact, parse_ContactDef, "VOComments");
	CEAddFieldToParent(pField, UI_WIDGET(pWin), X_OFFSET_CONTROL, y, 0, 1.0, UIUnitPercentage, 21, pDoc);
	eaPush(&pDoc->eaDocFields, pField);
	y += STANDARD_ROW_HEIGHT;
...
void CEInitData(EMEditor *pEditor)
{
	if (!gInitializedEditor) {
		...
		// Subscribe to the info indexes this editor requires
		resSubscribeToInfoIndex("GuildStatDef", true);
		resSubscribeToInfoIndex("Cutscene", true);
...
static void CEContactPreSaveFixup(ContactDef *pContact)
{
	...
	FOR_EACH_IN_EARRAY_FORWARDS(pContact->specialDialog, SpecialDialogBlock, pSpecialDialogBlock)
	{
		...
		FOR_EACH_IN_EARRAY_FORWARDS(pSpecialDialogBlock->extraCostumes, ContactCostume, pExtraCostume)
		{
			if(pExtraCostume->eCostumeType != ContactCostumeType_Specified) {
				if(IS_HANDLE_ACTIVE(pExtraCostume->costumeOverride))
					REMOVE_HANDLE(pExtraCostume->costumeOverride);
			}

			if(pExtraCostume->eCostumeType != ContactCostumeType_PetContactList) {
				if(IS_HANDLE_ACTIVE(pExtraCostume->hPetOverride))
					REMOVE_HANDLE(pExtraCostume->hPetOverride);
			}

			if(pExtraCostume->eCostumeType != ContactCostumeType_CritterGroup) {
				if(IS_HANDLE_ACTIVE(pExtraCostume->hCostumeCritterGroup))
					REMOVE_HANDLE(pExtraCostume->hCostumeCritterGroup);
				pExtraCostume->pchCostumeIdentifier = NULL;
				pExtraCostume->pchCostumeMapVar = NULL;
				pExtraCostume->eCostumeCritterGroupType = 0;
			} else if(pExtraCostume->eCostumeCritterGroupType == ContactMapVarOverrideType_Specified) {
				pExtraCostume->pchCostumeMapVar = NULL;
			} else {
				if(IS_HANDLE_ACTIVE(pExtraCostume->hCostumeCritterGroup))
					REMOVE_HANDLE(pExtraCostume->hCostumeCritterGroup);
...
void CECloseContact(ContactEditDoc *pDoc)
{
	...
	eaDestroyEx(&pDoc->eaCostumeGroups, CEFreeCostumeGroup);
