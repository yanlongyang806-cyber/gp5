...
void AnimEditor_DrawCostumeGhosts(CostumeViewGraphics *pGraphics, F32 fDeltaTime)
{
	...
	gfxQueueSingleDynDrawSkeleton(pGraphics->costume.pDrawSkel, pGraphics->bOverrideSky ? worldGetEditorWorldRegion() : NULL, true, true, NULL);
	FOR_EACH_IN_EARRAY(pGraphics->costume.pDrawSkel->eaSubDrawSkeletons, DynDrawSkeleton, pSub)
		gfxQueueSingleDynDrawSkeleton(pSub, pGraphics->bOverrideSky ? worldGetEditorWorldRegion() : NULL, false, true, NULL);
	FOR_EACH_END;
}
...
F32 AnimEditor_ReflowNormalNode(
	...
	UIActivationFunc AddMoveCB, UIActivationFunc SortMoveCB, UIActivationFunc ChooseMoveCB, MEFieldChangeCallback MoveChangeChanceCB, UIActivationFunc OpenMoveCB, UIActivationFunc RemoveMoveCB,
	UISkin *badSkin,
	bool bMovementTyped,
	...
	)
{
	...
	if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritInterpolation_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaNodeFields, pField);

	y += STANDARD_ROW_HEIGHT;

	// interpolation curve-in
	x = 20.0f;
	if (!pGraph->bPartialGraph)
	{
	...
	if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritEaseIn_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaNodeFields, pField);

	y += STANDARD_ROW_HEIGHT;

	// interpolation curve-out
	x = 20.0f;
	if (!pGraph->bPartialGraph)
	{
	...
	AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0f, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
	if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritEaseOut_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaNodeFields, pField);

	y += STANDARD_ROW_HEIGHT;

	// no self interpolation
	x = 0.0f;
	if (!pGraph->bPartialGraph)
	{
	...
	AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
	if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritNoSelfInterp_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaNodeFields, pField);
	...
	AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
	if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritForceEndFreeze_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaNodeFields, pField);

	y += STANDARD_ROW_HEIGHT;

	// override all bones (all sequencers)
	if (!bMovementTyped ||
		pEditNode->bOverrideAllBones || // show bad data so artist can fix it
		pEditNode->bSnapOverrideAllBones)
	{
		x = 0.0f;
		if (!pGraph->bPartialGraph)
		{
		...
		x += 30.0;
		if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritOverrideAllBones_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaNodeFields, pField);
	...
	// override movement (legs)
	if (!bMovementTyped ||
		pEditNode->bOverrideMovement || // show bad data so aritst can fix it
		pEditNode->bSnapOverrideMovement)
	{
		x = 0.0f;
		if (!pGraph->bPartialGraph)
		{
		...
		x += 30.0f;
		if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritOverrideMovement_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaNodeFields, pField);
		pLabel = ui_LabelCreate("Snap", x, y);
		emPanelAddChild(pPanel, pLabel, false);
	...
	// override default movement (torso)
	if (!bMovementTyped ||
		pEditNode->bOverrideDefaultMove || // show bad data so artist can fix it
		pEditNode->bSnapOverrideDefaultMove)
	{
		x = 0.0f;
		if (!pGraph->bPartialGraph)
		{
		...
		x += 30.0f;
		if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritOverrideDefaultMove_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaNodeFields, pField);
		pLabel = ui_LabelCreate("Snap", x, y);
		emPanelAddChild(pPanel, pLabel, false);
	...
	// allow random repeats
	if (!bMovementTyped ||
		pEditNode->bAllowRandomMoveRepeats) // show bad data so artist can fix it
	{
		x = 0.0f;
		if (!pGraph->bPartialGraph)
		{
		...
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
		x += 30.0f;
		if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritAllowRandomMoveRepeats_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaNodeFields, pField);
	...
	// timeout - disable torso pointing
	if (!bMovementTyped ||
		pEditNode->fDisableTorsoPointingTimeout > 0.f) // show bad data so artist can fix it
	{
		x = 0.0f;
		if (!pGraph->bPartialGraph)
		{
		...
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
		x += 30.0f;
		if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritDisableTorsoPointing_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaNodeFields, pField);
	...
	// timeout - disable ground registration
	x = 0.0f;
	if (!pGraph->bPartialGraph)
	{
	...
	// impact triggers
	if (!bMovementTyped ||
		eaSize(&pEditNode->eaGraphImpact) > 0) // show bad data so artist can fix it
	{
		x = 0.0f;
		if (!pGraph->bPartialGraph)
		{
		...
		if (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritGraphImpact_WhenFalse)
	...
	if (!pGraph->bPartialGraph)
	{
		...
		emPanelAddChild(pPanel, pButton, false);
		x += ui_WidgetGetWidth(UI_WIDGET(pButton)) + 10.f;
		pButton = ui_ButtonCreate("Sort", x, y, SortMoveCB, pEditNode);
		emPanelAddChild(pPanel, pButton, false);
		...
		FOR_EACH_IN_EARRAY_FORWARDS(pEditNode->eaMove, DynAnimGraphMove, pMove)
		{
			...
			if (bMovementTyped)
			{
				...
				pLabel = ui_LabelCreate("Direction", x, y);
				emPanelAddChild(pPanel, pLabel, false);
				x += ui_WidgetGetWidth(UI_WIDGET(pLabel)) + 10.0f;

				pField = MEFieldCreateSimpleEnum(kMEFieldType_Combo, pOrigMove, pMove, parse_DynAnimGraphMove, "Direction", DynMovementDirectionEnum);
				AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 31, pDoc, FieldPreChangedCB, FieldChangedCB);
				eaPush(eaNodeFields, pField);
			...
			emPanelAddChild(pPanel, pLabel, false);
			x += ui_WidgetGetWidth(UI_WIDGET(pLabel)) + 18;

			pField = MEFieldCreateSimple(kMEFieldType_TextEntry, pOrigMove, pMove, parse_DynAnimGraphMove, "Chance");
			AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 31, pDoc, FieldPreChangedCB, FieldChangedCB);
			MEFieldSetChangeCallback(pField, MoveChangeChanceCB, pDoc);
			if (eaSize(&pEditNode->eaMove) == 1)
				eaPush(eaInactiveWidgets, pField->pUIWidget);
			if (GET_REF(pMove->hMove) &&
				!dynAnimGraphGroupMoveVerifyChance(pGraph, pEditNode, pMove->eDirection, false))
			{
		...
		y += 20 + SEPARATOR_HEIGHT;
	}

	// switches
	if (!pGraph->bPartialGraph && // partial graphs show data in animTemplateEditor code
		eaSize(&pEditNode->eaSwitch))
	{
		x = 0.0f;
		pField = MEFieldCreateSimple(kMEFieldType_Check, pOrigNode ? pOrigNode->pInheritBits : NULL, pEditNode->pInheritBits, parse_DynAnimGraphNodeInheritBits, "InheritSwitch");
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 20.0f, UIUnitFixed, 0, pDoc, FieldPreChangedCB, InheritFieldChangedCB);
		eaPush(eaNodeFields, pField);
		x += 20.0;
		pLabel = ui_LabelCreate("Switch Data", x, y);
		emPanelAddChild(pPanel, pLabel, false);
		x += ui_WidgetGetWidth(UI_WIDGET(pLabel)) + 10.0f;
		...
		FOR_EACH_IN_EARRAY_FORWARDS(pEditNode->eaSwitch, DynAnimGraphSwitch, pSwitch)
		{
			DynAnimTemplateSwitch* pTemplateSwitch;
			DynAnimGraphSwitch* pOrigSwitch = (pOrigNode && ipSwitchIndex < eaSize(&pOrigNode->eaSwitch)) ? pOrigNode->eaSwitch[ipSwitchIndex] : NULL;
			char pcSwitchName[MAX_PATH];
			F32 x1;

			assert(pEditNode->pTemplateNode && ipSwitchIndex < eaSize(&pEditNode->pTemplateNode->eaSwitch));
			pTemplateSwitch = pEditNode->pTemplateNode->eaSwitch[ipSwitchIndex];
			sprintf(pcSwitchName, "Switch: %s", pTemplateSwitch->pcFlag);

			x1 = 10.0f;
			pLabel = ui_LabelCreate(pcSwitchName, x1, y);
			emPanelAddChild(pPanel, pLabel, false);

			y += STANDARD_ROW_HEIGHT;

			// only show fRequiredPlaytime for movement graphs & non-movement graphs with bad data
			if (bMovementTyped ||
				pSwitch->fRequiredPlaytime < 0.f ||
				pSwitch->fRequiredPlaytime > 0.f)
			{
				x1 = 20.0f;
				pLabel = ui_LabelCreate("Required Playtime (seconds)", x1, y);
				emPanelAddChild(pPanel, pLabel, false);
				x1 += ui_WidgetGetWidth(UI_WIDGET(pLabel)) + 10.f;

				pField = MEFieldCreateSimple(kMEFieldType_TextEntry, pOrigSwitch, pSwitch, parse_DynAnimGraphSwitch, "RequiredPlaytime");
				AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x1, y, 0, 1.0, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
				if (pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritSwitch_WhenFalse)
					eaPush(eaInactiveWidgets, pField->pUIWidget);
				eaPush(eaNodeFields, pField);
			...
			x1 = 20.0f;
			pLabel = ui_LabelCreate("Interrupt", x1, y);
			emPanelAddChild(pPanel, pLabel, false);
			x1 += ui_WidgetGetWidth(UI_WIDGET(pLabel)) + 10.f;

			pField = MEFieldCreateSimple(kMEFieldType_Check, pOrigSwitch, pSwitch, parse_DynAnimGraphSwitch, "Interrupt");
			AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x1, y, 0, 1.0, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
			if (pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritSwitch_WhenFalse)
				eaPush(eaInactiveWidgets, pField->pUIWidget);
			eaPush(eaNodeFields, pField);
	...
	)
{
	...
	FOR_EACH_IN_EARRAY_FORWARDS(pEditNode->eaPath, DynAnimGraphPath, pPath)
	{
		...
		x = 10.0f;
		pField = MEFieldCreateSimple(kMEFieldType_TextEntry, pOrigPath, pPath, parse_DynAnimGraphPath, "Chance");
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 21, pDoc, FieldPreChangedCB, FieldChangedCB);
		ui_WidgetSetPaddingEx(pField->pUIWidget, x1, x2, 0, 0);
		MEFieldSetChangeCallback(pField, PathChangeChanceCB, pDoc);
		if (eaSize(&pEditNode->eaPath) == 1 || (!pGraph->bPartialGraph && pEditNode->pInheritBits && !pEditNode->pInheritBits->bInheritPath_WhenFalse))
			eaPush(eaInactiveWidgets, pField->pUIWidget);
	...
	)
{
	...
	if (!bMovementTyped ||
		pGraph->bResetWhenMovementStops) // show bad data so artist can fix it
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		x += 30.0f;
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritResetWhenMovementStops_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
	...
	// override all bones
	if (!bMovementTyped ||
		pGraph->bOverrideAllBones || // show bad data so artist can fix it
		pGraph->bSnapOverrideAllBones)
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		x += 30.0f;
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritOverrideAllBones_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
		pLabel = ui_LabelCreate("Snap", x, y);
		emPanelAddChild(pPanel, pLabel, false);
	...
	// override movement
	if (!bMovementTyped ||
		pGraph->bOverrideMovement || // show bad data so artist can fix it
		pGraph->bSnapOverrideMovement)
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		x += 30.0;
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritOverrideMovement_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
		pLabel = ui_LabelCreate("Snap", x, y);
		emPanelAddChild(pPanel, pLabel, false);
	...
	if (!bMovementTyped ||
		pGraph->bOverrideDefaultMove || // show bad data so artist can fix it
		pGraph->bSnapOverrideDefaultMove)
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		x += 30.0f;
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritOverrideDefaultMove_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
		pLabel = ui_LabelCreate("Snap", x, y);
		emPanelAddChild(pPanel, pLabel, false);
	...
	AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 20.0f, UIUnitFixed, 0, pDoc, FieldPreChangedCB, FieldChangedCB);
	if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritForceVisible_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaPropertyFields, pField);
	...
	// pitch to target
	if (!bMovementTyped ||
		pGraph->bPitchToTarget) // show bad data so artist can fix it
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 20.0f, UIUnitFixed, 0, pDoc, FieldPreChangedCB, FieldChangedCB);
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritPitchToTarget_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
	...
	if (!bMovementTyped ||
		pGraph->bGeneratePowerMovementInfo) // show bad data so artist can fix it
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 20.0f, UIUnitFixed, 0, pDoc, FieldPreChangedCB, FieldChangedCB);
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritGeneratePowerMovementInfo_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
	...
	// timeout - disable torso pointing
	if (!bMovementTyped ||
		pGraph->fDisableTorsoPointingTimeout > 0.f) // show bad data so artist can fix it
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 0, pDoc, FieldPreChangedCB, FieldChangedCB);
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritDisableTorsoPointing_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
	...
	// disable ground registration - whole body
	x = 0;
	if (!pGraph->bPartialGraph)
	{
	...
	AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 0, pDoc, FieldPreChangedCB, FieldChangedCB);
	if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritDisableGroundReg_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaPropertyFields, pField);

	y += STANDARD_ROW_HEIGHT;

	// disable ground registration - upper body
	x = 0;
	if (!pGraph->bPartialGraph)
	{
	...
	AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 0, pDoc, FieldPreChangedCB, FieldChangedCB);
	if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritDisableUpperBodyGroundReg_WhenFalse)
		eaPush(eaInactiveWidgets, pField->pUIWidget);
	eaPush(eaPropertyFields, pField);
	...
	// timeout - post-idle
	if (!bMovementTyped ||
		pGraph->fTimeout > 0.f) // show bad data so artist can fix it
	{
		x = 0;
		if (!pGraph->bPartialGraph)
		{
		...
		AnimEditor_AddFieldToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), x, y, 0, 1.0, UIUnitPercentage, 0, pDoc, FieldPreChangedCB, FieldChangedCB);
		if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritTimeout_WhenFalse)
			eaPush(eaInactiveWidgets, pField->pUIWidget);
		eaPush(eaPropertyFields, pField);
	}
	...
	emPanelAddChild(pPanel, pButton, false);
	if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritOnEnterFxEvent_WhenFalse)
		eaPush(eaInactiveWidgets, UI_WIDGET(pButton));
	x += ui_WidgetGetWidth(UI_WIDGET(pButton)) + 10.0f;
	...
	emPanelAddChild(pPanel, pButton, false);
	if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritOnExitFxEvent_WhenFalse)
		eaPush(eaInactiveWidgets, UI_WIDGET(pButton));
	x += ui_WidgetGetWidth(UI_WIDGET(pButton)) + 10.0f;
	...
	emPanelAddChild(pPanel, pButton, false);
	if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritSuppress_WhenFalse)
		eaPush(eaInactiveWidgets, UI_WIDGET(pButton));
	...
	emPanelAddChild(pPanel, pButton, false);
	if (!pGraph->bPartialGraph && pGraph->pInheritBits && !pGraph->pInheritBits->bInheritStance_WhenFalse)
		eaPush(eaInactiveWidgets, UI_WIDGET(pButton));
