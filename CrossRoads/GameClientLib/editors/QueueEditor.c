...
static char** qe_GetCustomMapTypes(METable *pTable, QueueDef *pDef)
{
	...
	if (pDef)
	{
		iSize = eaSize(&pDef->QueueMaps.eaCustomMapTypes);
...
static void qe_initQueueColumns(METable *pTable)
{
	...
	METableAddGlobalDictColumn(pTable,   "Req. Mission",	"RequiredMission", 120, QE_GROUP_REQUIRES, kMEFieldType_ValidatedTextEntry, "Mission", "ResourceName");
	...
	METableAddSimpleColumn(pTable, "Gear Rating", "RequiredGearRating", 120, QE_GROUP_REQUIRES, kMEFieldType_TextEntry);
	...
	METableAddEnumColumn(pTable, "Smart Grouping Mode", "SmartGroupingMode", 120, QE_GROUP_MAIN, kMEFieldType_Combo, QueueSmartGroupingModeEnum);
	...
	METableAddSimpleColumn(pTable, "Minimum Points For Reward","MinimumPointsForReward", 120, QE_GROUP_MAIN, kMEFieldType_TextEntry);
