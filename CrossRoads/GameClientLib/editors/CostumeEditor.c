...
static CostumeEditDyePackStruct **s_eaDyePacks = NULL;
...
static void costumeEdit_UpdateColorButtons(CostumeEditDoc *pDoc)
{
	...
	if (pDoc->pCButton3) {
		...
		if (pDoc->pCButton3->activeWindow)
		{
			if ((pDoc->pColorButtonCurrentPart != NULL) && (pDoc->pCostume != NULL))
			{
...
{
	EMEditor *currEditor = emGetActiveEditor();
...
static void costumeEdit_CostumeChanged(CostumeEditDoc *pDoc)
{
	...
	/*ui_ColorButtonAdoptionCheck(pDoc->pCButton0);
	ui_ColorButtonAdoptionCheck(pDoc->pCButton1);
	ui_ColorButtonAdoptionCheck(pDoc->pCButton2);
	ui_ColorButtonAdoptionCheck(pDoc->pCButton3);*/
	//costumeEdit_UpdateColorButtons(pDoc);
}
...
void costumeEdit_CostumeRevert(CostumeEditDoc *pDoc)
{
	...
	if (pDictCostume)
	{
		...
		pDoc->pCostume = StructCloneDeConst(parse_PlayerCostume, pDictCostume);
		bCreated = false;
	}
	else if (pDoc->pOrigCostume)
	{
	...
	// If we destroyed our original OrigCostume to use DictCostume, rebuild it now, after we've modified stuff via SelectBone
	// (Actually, costumeEdit_UpdateLists.  Things happen in a slightly different order when loading a fresh costume.  It would nice to
	// sort this out.) [RMARR - 6/7/13]
	if (pDictCostume)
	{
...
void costumeEdit_UpdateExtraTextures(CostumeEditDoc *pDoc)
{
	...
	for(i=0; i<numTex; ++i) {
		...
		if (!pTexGroup->pField) {
			pTexGroup->pField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pOldTex, pDoc->pColorButtonCurrentPart->pArtistData->eaExtraTextures[i], parse_PCTextureRef, "Name", "CostumeTexture", "ResourceName");
...
void costumeEdit_UpdateFX(CostumeEditDoc *pDoc)
{
	...
	for(i=0; i<numFx; ++i) {
		...
		if (!pFxGroup->pFxField) {
			pFxGroup->pFxField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pOldFx, pDoc->pCostume->pArtistData->eaFX[i], parse_PCFX, "pcName", "DynFxInfo", "ResourceName");
	...
	if (!pFxGroup->pFxField && pDoc->pCostume->pArtistData->pDismountFX) {
		pFxGroup->pFxField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pOldFx, pDoc->pCostume->pArtistData->pDismountFX, parse_PCFX, "pcName", "DynFxInfo", "ResourceName");
		if (!pFxGroup->pFxField)
		{
...
void costumeEdit_UpdateFXSwap(CostumeEditDoc *pDoc)
{
	...
	for(i=0; i<numFxSwap; ++i) {
		...
		if (!pFxSwapGroup->pFxOldField) {
			pFxSwapGroup->pFxOldField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pOldFxSwap, pDoc->pCostume->pArtistData->eaFXSwap[i], parse_PCFXSwap, "pcOldName", "DynFxInfo", "ResourceName");
		...
		if (!pFxSwapGroup->pFxNewField) {
			pFxSwapGroup->pFxNewField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pOldFxSwap, pDoc->pCostume->pArtistData->eaFXSwap[i], parse_PCFXSwap, "pcNewName", "DynFxInfo", "ResourceName");
...
{
	ServerCmd_costumeGetDyePackNames();
}

CostumeEditDyePackStruct ***costumeEdit_GetDyePacks()
{
	return &s_eaDyePacks;
}
...
static void costumeEdit_UpdateLists(CostumeEditDoc *pDoc)
{
	...
	pDoc->pDisableBankingField->pOld = pDoc->pOrigCostume ? pDoc->pOrigCostume->pArtistData->pFlightControls : NULL;
	pDoc->pDisableBankingField->pNew = pDoc->pCostume->pArtistData->pFlightControls;
	MEFieldRefreshFromData(pDoc->pDisableBankingField);

	pDoc->pBankRateFlyingField->pOld = pDoc->pOrigCostume ? pDoc->pOrigCostume->pArtistData->pFlightControls : NULL;
	pDoc->pBankRateFlyingField->pNew = pDoc->pCostume->pArtistData->pFlightControls;
	MEFieldRefreshFromData(pDoc->pBankRateFlyingField);

	pDoc->pBankMaxAngleFlyingField->pOld = pDoc->pOrigCostume ? pDoc->pOrigCostume->pArtistData->pFlightControls : NULL;
	pDoc->pBankMaxAngleFlyingField->pNew = pDoc->pCostume->pArtistData->pFlightControls;
	MEFieldRefreshFromData(pDoc->pBankMaxAngleFlyingField);
...
void costumeEdit_UIColorChanged(UIColorButton *pButton, bool bFinished, CostumeEditDoc *pDoc)
{
	...
	{
		if ((!pDoc->pCButton0->activeWindow) || (!ui_WindowIsVisible(UI_WINDOW(pDoc->pCButton0->activeWindow)) || (pDoc->pCButton0->activeWindow->bOrphaned))) costumeEdit_UpdateColorButtons(pDoc);
	...
	{
		if ((!pDoc->pCButton1->activeWindow) || (!ui_WindowIsVisible(UI_WINDOW(pDoc->pCButton1->activeWindow)) || (pDoc->pCButton1->activeWindow->bOrphaned))) costumeEdit_UpdateColorButtons(pDoc);
		if (pDoc->pColorButtonCurrentPart->pClothLayer)
		{
	...
	{
		if ((!pDoc->pCButton2->activeWindow) || (!ui_WindowIsVisible(UI_WINDOW(pDoc->pCButton2->activeWindow)) || (pDoc->pCButton2->activeWindow->bOrphaned))) costumeEdit_UpdateColorButtons(pDoc);
		if (pDoc->pColorButtonCurrentPart->pClothLayer)
		{
	...
	else if (pButton == pDoc->pCButton3)
	{
		...
		if ((!pDoc->pCButton3->activeWindow) || (!ui_WindowIsVisible(UI_WINDOW(pDoc->pCButton3->activeWindow)) || (pDoc->pCButton3->activeWindow->bOrphaned))) costumeEdit_UpdateColorButtons(pDoc);
	...
	if (pMirror) {
		...
		for(i=eaSize(&pDoc->pCostume->eaParts)-1; i>=0; --i) {
			if (pMirror == GET_REF(pDoc->pCostume->eaParts[i]->hBoneDef)) {
				...
				costumeEdit_CopyColorScale1(gColorSetClipboard.color0, newColor);
				ui_ColorButtonSetColor(pDoc->pCButton0, newColor);
				costumeEdit_UIColorChanged(pDoc->pCButton0, true, pDoc);
				costumeEdit_CopyColorScale1(gColorSetClipboard.color1, newColor);
				ui_ColorButtonSetColor(pDoc->pCButton1, newColor);
				costumeEdit_UIColorChanged(pDoc->pCButton1, true, pDoc);
				costumeEdit_CopyColorScale1(gColorSetClipboard.color2, newColor);
				ui_ColorButtonSetColor(pDoc->pCButton2, newColor);
				costumeEdit_UIColorChanged(pDoc->pCButton2, true, pDoc);
				costumeEdit_CopyColorScale1(gColorSetClipboard.color3, newColor);
				ui_ColorButtonSetColor(pDoc->pCButton3, newColor);
				costumeEdit_UIColorChanged(pDoc->pCButton3, true, pDoc);
			}
		}
		costumeEdit_CostumeChanged(pDoc);
	}
...
static void costumeEdit_UIMenuSelectColorSetRevert(UIMenuItem *pItem, CostumeEditDoc *pDoc)
{
	...
	if (pOrigPart) {
		...
		costumeEdit_CopyColorScale1(pOrigPart->color0, newColor);
		ui_ColorButtonSetColor(pDoc->pCButton0, newColor);
		costumeEdit_UIColorChanged(pDoc->pCButton0, true, pDoc);
		costumeEdit_CopyColorScale1(pOrigPart->color1, newColor);
		ui_ColorButtonSetColor(pDoc->pCButton1, newColor);
		costumeEdit_UIColorChanged(pDoc->pCButton1, true, pDoc);
		costumeEdit_CopyColorScale1(pOrigPart->color2, newColor);
		ui_ColorButtonSetColor(pDoc->pCButton2, newColor);
		costumeEdit_UIColorChanged(pDoc->pCButton2, true, pDoc);
		costumeEdit_CopyColorScale1(pOrigPart->color3, newColor);
		ui_ColorButtonSetColor(pDoc->pCButton3, newColor);
		costumeEdit_UIColorChanged(pDoc->pCButton3, true, pDoc);
	}
}

void costumeEdit_UIApplyDyePackHelper(ItemDef *pItem, CostumeEditDoc *pDoc)
{
	U8 color0[4];
	U8 color1[4];
	U8 color2[4];
	...
	costumeEdit_CopyColorScale1(color0, newColor);
	ui_ColorButtonSetColor(pDoc->pCButton0, newColor);
	costumeEdit_UIColorChanged(pDoc->pCButton0, true, pDoc);
	costumeEdit_CopyColorScale1(color1, newColor);
	ui_ColorButtonSetColor(pDoc->pCButton1, newColor);
	costumeEdit_UIColorChanged(pDoc->pCButton1, true, pDoc);
	costumeEdit_CopyColorScale1(color2, newColor);
	ui_ColorButtonSetColor(pDoc->pCButton2, newColor);
	costumeEdit_UIColorChanged(pDoc->pCButton2, true, pDoc);
	costumeEdit_CopyColorScale1(color3, newColor);
	ui_ColorButtonSetColor(pDoc->pCButton3, newColor);
	costumeEdit_UIColorChanged(pDoc->pCButton3, true, pDoc);
}
...
static void costumeEdit_UIApplyDyePack(UIWidget* pWidget, CostumeEditDoc *pDoc)
{
	...
	if (pItem)
	{
		costumeEdit_UIApplyDyePackHelper(pItem, pDoc);
	}
	else
	{
		ServerCmd_costumeGetAndApplyDyePack(pDoc->pCurrentDyePack->pchDyePack, pDoc->pCostume->pcName);
	}
}
...
EMPanel *costumeEdit_InitPanelCollision(CostumeEditDoc *pDoc)
{
	...
	emPanelAddChild(pPanel, pLabel, false);
	pField = MEFieldCreateSimpleGlobalDictionary(kMEFieldType_TextEntry, pDoc->pOrigCostume ? pDoc->pOrigCostume->pArtistData : NULL, pDoc->pCostume->pArtistData, parse_PCArtistCostumeData, "CollisionGeo", "CostumeGeometry", "ResourceName");
	if (!pField)
	{
...
EMPanel *costumeEdit_InitPanelFlightControls(CostumeEditDoc *pDoc)
{
	EMPanel *pPanel;
	UILabel *pLabel;
	MEField *pField;
	...
	pPanel = emPanelCreate("Artist", "Flight Controls", 90);

	// Disable banking
	pLabel = ui_LabelCreate("Disable Banking", 15, y);
	emPanelAddChild(pPanel, pLabel, false);
	pField = MEFieldCreateSimple(kMEFieldType_BooleanCombo, pDoc->pOrigCostume ? pDoc->pOrigCostume->pArtistData->pFlightControls : NULL, pDoc->pCostume->pArtistData->pFlightControls, parse_PCFlightControls, "DisableBanking");
	MEFieldAddToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), 140, y);
	MEFieldSetChangeCallback(pField, costumeEdit_UIFieldChanged, pDoc);
	ui_WidgetSetWidth(pField->pUIWidget, 80);
	pDoc->pDisableBankingField = pField;
	y += 28;

	// Bank rate flying
	pLabel = ui_LabelCreate("Bank Rate", 15, y);
	emPanelAddChild(pPanel, pLabel, false);
	pField = MEFieldCreateSimple(kMEFieldType_TextEntry, pDoc->pOrigCostume ? pDoc->pOrigCostume->pArtistData->pFlightControls : NULL, pDoc->pCostume->pArtistData->pFlightControls, parse_PCFlightControls, "BankRateFlying");
	pField->arrayIndex = 0;
	MEFieldAddToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), 140, y);
	MEFieldSetChangeCallback(pField, costumeEdit_UIFieldChanged, pDoc);
	ui_WidgetSetWidthEx(pField->pUIWidget, 1.0, UIUnitPercentage);
	pDoc->pBankRateFlyingField = pField;
	y += 28;

	// Bank max angle flying
	pLabel = ui_LabelCreate("Bank Max Angle", 15, y);
	emPanelAddChild(pPanel, pLabel, false);
	pField = MEFieldCreateSimple(kMEFieldType_TextEntry, pDoc->pOrigCostume ? pDoc->pOrigCostume->pArtistData->pFlightControls : NULL, pDoc->pCostume->pArtistData->pFlightControls, parse_PCFlightControls, "BankMaxAngleFlying");
	pField->arrayIndex = 1;
	MEFieldAddToParent(pField, UI_WIDGET(emPanelGetExpander(pPanel)), 140, y);
	MEFieldSetChangeCallback(pField, costumeEdit_UIFieldChanged, pDoc);
	ui_WidgetSetWidthEx(pField->pUIWidget, 1.0, UIUnitPercentage);
	pDoc->pBankMaxAngleFlyingField = pField;
	y += 28;

	pDoc->pFlightControlPanel = pPanel;
...
{	
	...
	resSubscribeToInfoIndex("DynFxInfo", true);
	...
	eaPush(&pDoc->emDoc.em_panels, costumeEdit_InitPanelFlightControls(pDoc));
...
void costumeEdit_InitData(EMEditor *pEditor)
{
	...
	if (!gInitializedEditor) {
		...
		// Need this for dyes for Neverwinter
		if (!gConf.bCostumeEditorDoesntUseItems) {
			resSubscribeToInfoIndex(g_hItemDict, true);
		}
...
EMTaskStatus costumeEdit_SaveCostume(CostumeEditDoc *pDoc, bool bSaveAsNew)
{
	...
	if (bSaveAsNew) {
		// When save as new, replace the original original costume with the new original costume
		StructDestroyNoConstSafe(parse_PlayerCostume, &pDoc->pOrigCostume);
		pDoc->pOrigPart = NULL;
		pDoc->pOrigCostume = StructCloneNoConst(parse_PlayerCostume, pDoc->pCostume);
...
void costumeEdit_CloseCostume(CostumeEditDoc *pDoc)
{
	...
	MEFieldDestroy(pDoc->pDisableBankingField);
	MEFieldDestroy(pDoc->pBankRateFlyingField);
	MEFieldDestroy(pDoc->pBankMaxAngleFlyingField);
	if (pDoc->pHeightField) {
