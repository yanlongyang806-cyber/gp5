					  ...
					  EMMenuItemCheckFunc check_func, UserData check_func_data, const char *command_str)
{
	...
	if (!editor && !em_data.menu_items)
		em_data.menu_items = stashTableCreateWithStringKeys(32, StashDeepCopyKeys_NeverRelease);
	else if (editor && !editor->menu_items)
		editor->menu_items = stashTableCreateWithStringKeys(32, StashDeepCopyKeys_NeverRelease);
...
{
	if (!editor->menus)
		editor->menus = stashTableCreateWithStringKeys(8, StashDeepCopyKeys_NeverRelease);
...
static EMMenuItemDef em_menu_items[] = {
	...
	{"em_zeniwindow"},
	{"em_ExprFuncs", "Expression Funcs", NULL, NULL, "ExprFuncs"},
};
...
void emMenusShow(EMEditorDoc *doc, EMEditor *editor)
{
	...
	emMenuAppendItems(editor, em_data.title.help_menu, "em_exprfuncs", NULL);
