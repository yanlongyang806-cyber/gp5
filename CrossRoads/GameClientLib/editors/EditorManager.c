...
void emRegisterFileTypeEx(const char *type, const char *display_name, const char *editor_name, EMPreviewFunc preview_func)
{
	...
	if (!em_data.registered_file_types)
		em_data.registered_file_types = stashTableCreateWithStringKeys(64, StashDeepCopyKeys_NeverRelease);
...
EMFile *emGetFile(const char *filename, bool create)
{
	...
	if (!em_data.files)
		em_data.files = stashTableCreateWithStringKeys(512, StashDeepCopyKeys_NeverRelease);
...
void emSetEditorMode(bool editor_mode)
{
	...
	if (editor_mode)
	{
		...
		//   WE used to set the active FAMILY explicitly to EDITOR (which would automatically turn off GAME and FAMILY_CUTSCENE)
		// Set has been removed so we only have access to add and remove. So do the right thing for all three families now.
		ui_AddActiveFamilies(UI_FAMILY_EDITOR);
		ui_RemoveActiveFamilies(UI_FAMILY_CUTSCENE);
		ui_GameUIHide(UI_GAME_HIDER_EDITOR);
	...
	else
	{
		...
		//   WE used to set the active FAMILY explicitly to GAME (which would automatically turn off EDITOR and FAMILY_CUTSCENE)
		// Set has been removed so we only have access to add and remove. So do the right thing for all three families now.
		ui_RemoveActiveFamilies(UI_FAMILY_EDITOR);
		ui_RemoveActiveFamilies(UI_FAMILY_CUTSCENE);
		ui_GameUIShow(UI_GAME_HIDER_EDITOR);
