...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_PRIVATE ACMD_CLIENTCMD;
void PowerSetRechargeClient(U32 uiID, S16 iLinkedSub, F32 fTime)
{
	Entity* e = entActivePlayerPtr();
	...
	if(e && e->pChar)
	{
		if(!uiID)
		{
			character_PowerSetRechargeByID(entGetPartitionIdx(e),e->pChar,0,fTime);
		}
		else
		{
			Power *ppow = NULL;
			ppow = character_FindPowerByRefParams(e->pChar, uiID, -1, iLinkedSub);
...
void PowerActivationFailedRecharge(U32 uiActID, U32 uiPowerID, F32 fTime)
{
	...
	if(e && e->pChar)
	{
		...
		character_PowerSetRechargeByID(iPartitionIdx,pchar,uiPowerID,fTime);
	}
}
