...
void gclChat_AutoComplete(SA_PARAM_NN_VALID UIGen *pListGen, const char *pchFullCommand, S32 iPosition, S32 iMaxListSize)
{
	...
	bool bIsEmote = pchFullCommand ? *pchFullCommand == ';' : false;
	char *pchPrefix = NULL;
	...
	if (bIsCommand) {
		...
		else if (iPosition) //ABW I don't fully understand this code at all but I'm pretty sure this stops a weird
			//bug where it constantly spams cmdGetNameListTypeFromPartialCommandString with one-character strings
		{
		...
		if (iPosition > 1) {
			enumNameListType eNameListType = cmdGetNameListTypeFromPartialCommandString("em ");
			if (eNameListType != NAMELISTTYPE_NONE && eNameListType != NAMELISTTYPE_COMMANDLIST) {
				NameList *pEmoteList = cmdGetNameListFromPartialCommandString("em ");
				if (pEmoteList) {
					fillCommandArgCompletionList(pEmoteList, pchPrefix, peaCompletion, iPrefixReplaceFrom, iPrefixReplaceTo);
				}
			}
		}
	} else {
...
void VerifyCommand(const char *pCommandName, int iAccessLevel, bool bOKIfHidden)
{
	...
	if (pCmd)
	{
		...
		if (pCmd->flags & ((bOKIfHidden ? 0 : CMDF_HIDEPRINT) | CMDF_EARLYCOMMANDLINE | CMDF_COMMANDLINEONLY))
		{
...
//if it returns an alias, then looks up that alias in turn, etc. Also strips out everything after the first space
bool gclCommandAliasGetToExecute_RecursingAndSimplifying(char *pCommandIn, char **ppOutEstr)
{
	const char *pAliased = gclCommandAliasGetToExecute(pCommandIn);
	char *pTemp = NULL;

	if (!pAliased)
	{
		estrCopy2(ppOutEstr, pCommandIn);
		return false;
	}

	estrCopy2(&pTemp, pAliased);
	estrTruncateAtFirstOccurrence(&pTemp, ' ');

	gclCommandAliasGetToExecute_RecursingAndSimplifying(pTemp, ppOutEstr);
...
void gclChatAutoComplete_VerifyCommands(void)
{
	...
	FOR_EACH_IN_EARRAY(autoCompleteNameListList.eaLists, ChatAutoCompleteEntryList, pList)
	{
		...
		{
			char *pAliased = NULL;
			if (gclCommandAliasGetToExecute_RecursingAndSimplifying(pEntry->pchCommand, &pAliased))
			{
				VerifyCommand(pAliased, pEntry->iAccessLevel, true);
			}
			else
			{
				VerifyCommand(pEntry->pchCommand, pEntry->iAccessLevel, false);
			}

			estrDestroy(&pAliased);
		}
		FOR_EACH_END;	
	}
	FOR_EACH_END;
