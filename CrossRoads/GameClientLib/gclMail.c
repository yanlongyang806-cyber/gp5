...
{
	ui_GenInitStaticDefineVars(EMailV3SenderTypeEnum, "EmailMessageSenderType");
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GenDeleteMessage);
void EmailV3_DeleteMessage(SA_PARAM_OP_VALID Entity* pEnt, U32 uMessageID, EMailV3SenderType eType)
{
	EmailV3UIMessage* pUIMessage = NULL;
	int i;
	for (i = 0; i < eaSize(&s_eaMessages); i++)
	{
		if (s_eaMessages[i]->uID == uMessageID && s_eaMessages[i]->eSenderType == eType)
		{
...
void EmailV3_ExprGenGetMailList(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_NN_VALID UIGen *pGen)
{
	...
	EmailV3_RebuildMailList(pEnt,pSharedBank,&s_eaMessages,0);
	ui_GenSetList(pGen, &s_eaMessages, parse_EmailV3UIMessage);
}

// Set this gen's model to the list of mail the player has.
// Rebuilds the list every frame to prevent stale-pointer ugliness.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(GenGetMailListLimited);
void EmailV3_ExprGenGetMailListLimited(SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_NN_VALID UIGen *pGen, int iSizeLimit)
{
	Entity* pSharedBank = SAFE_GET_REF2(pEnt, pPlayer, hSharedBank);
	EmailV3_RebuildMailList(pEnt,pSharedBank,&s_eaMessages,iSizeLimit);
	ui_GenSetList(pGen, &s_eaMessages, parse_EmailV3UIMessage);
}
...
void EmailV3_GenExprMailTakeAttachedItems(U32 uID, EMailV3SenderType eType)
{
	EmailV3UIMessage *pUIMessage = NULL;
	int i;
	for (i = 0; i < eaSize(&s_eaMessages); i++)
	{
		if (s_eaMessages[i]->uID == uID && s_eaMessages[i]->eSenderType == eType)
		{
			pUIMessage = s_eaMessages[i];
			break;
		}
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(MailTakeAttachedItemsAndDelete);
void EmailV3_GenExprMailTakeAttachedItemsAndDelete(U32 uID, EMailV3SenderType eType)
{
	EmailV3UIMessage *pUIMessage = NULL;
	int i;
	for (i = 0; i < eaSize(&s_eaMessages); i++)
	...
	{
		ServerCmd_EmailV3_TakeAllAttachedItemsAndDelete(pUIMessage->pMessage->uID);
	}
}
...
bool gclMail_GenMailHasUnreadMail(SA_PARAM_OP_VALID Entity *pEnt)
{
	...
	NPCEMail* pNPCMail = SAFE_MEMBER2(pEnt, pPlayer, pEmailV2);
	U32 uNextDelivery = SAFE_MEMBER(pNPCMail, uNextFutureDelivery);

	if (SAFE_MEMBER(pNPCMail, bUnreadMail))
	{
		// if we have unread NPCMail
		return true;
	}
	else if (uNextDelivery > 0 && uNextDelivery < timeServerSecondsSince2000())
	{
		// if our "next" future NPCMail's timestamp has passed
		return true;
	}
	else if (pMailbox && pMailbox->bUnreadMail)
	{
		// if we have unread player mail
		return true;
	}
	return false;
}
	...
	else
	{
		...
		for (i = 0; i < eaSize(peaSlots); i++)
		{
			...
			if (pSenderItem->pItem && pSenderItem->bDestroyNextFrame)
			{
				...
				pSenderItem->bDestroyNextFrame = false;
			}

			if ((pSenderItem->pchLiteItemName || pSenderItem->uID) && !pSenderItem->pItem)
			{
				bool bFound = false;
				pIter = bagiterator_Create();
				if (pSenderItem->pchLiteItemName)
					//only Numeric lite items supported right now.
					bFound = inv_bag_trh_FindItemByDefName(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pItemEnt), InvBagIDs_Numeric, pSenderItem->pchLiteItemName, 0, pIter);
				else
					bFound = inv_trh_FindItemByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(Entity, pItemEnt), pSenderItem->uID, pIter);
...
void gclMail_GenMailGetFullItemList(SA_PARAM_NN_VALID UIGen *pGen, U32 uID, EMailV3SenderType eType)
{
	...
	if (pUIMessage)
	{
		...
		uiPlayerEmailID = pUIMessage->uID;
	}
	else
	{
	...
	//We have to do a manual lookup of the given ID instead of using the pointer because there's no guarantee that EmailV3_RebuildMailList() has been called this frame.
	else if(iNPCEmailID > 0 && pEnt && pEnt->pPlayer && pEnt->pPlayer->pEmailV2)
	{
		...
		if (i < eaSize(&pEnt->pPlayer->pEmailV2->mail))
			eaCopyStructs(&pEnt->pPlayer->pEmailV2->mail[i]->ppItemSlot, &s_eaSlots, parse_InventorySlot);
	}
	else if (uiPlayerEmailID > 0 && pMailData)
	{
		int iMail;
		for (iMail = 0; iMail < eaSize(&pMailData->eaMessages); iMail++)
		{
			if (pMailData->eaMessages[iMail]->uID == uiPlayerEmailID)
				break;
		}
		if (iMail < eaSize(&pMailData->eaMessages))
		{
			for (i = 0; i < eaSize(&pMailData->eaMessages[iMail]->ppItems); i++)
			{
				NOCONST(InventorySlot)* pSlot = inv_InventorySlotCreate(i);
				pSlot->pItem = StructCloneDeConst(parse_Item, pMailData->eaMessages[iMail]->ppItems[i]);
				eaPush(&s_eaSlots, CONTAINER_RECONST(InventorySlot, pSlot));
			}
		}
	}
	if (eaSize(&s_eaSlots) <= 0)
		ui_GenSetListSafe(pGen, NULL, InventorySlot);
	else
