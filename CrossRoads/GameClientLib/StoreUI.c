							...
							ItemTag eItemTag)
{
	...
	for (i = 0; i < eaSize(peaStoreItemInfo); i++) {
		...
		if (eItemTag != ItemTag_Any && pItemDef && (pItemDef->eTag != eItemTag)) {
			bRemove = true;
		}
		if (bRemove) {
...
void exprStoreGetBuyItemList(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt)
{
	...
	GetBuyItemList(peaStoreItemInfo, false, false, kStoreCategory_None, UsageRestrictionCategory_None, CharClassCategory_None, 0, ItemTag_Any);
...
void exprStoreGetFilteredPetItemList(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt, S32 eItemUsageCategory, S32 eClassCategories, const char* pchItemTag)
{
	StoreItemInfo *** peaStoreItemInfo;
	ItemTag eTag = StaticDefineInt_FastStringToInt(ItemTagEnum, pchItemTag, ItemTag_Any);
	if (ui_GenIsListManagingStructures(pGen)) {
		ui_GenSetList(pGen, NULL, NULL);
	}
	peaStoreItemInfo = ui_GenGetManagedListSafe(pGen, StoreItemInfo);
	GetBuyItemList(peaStoreItemInfo, false, false, kStoreCategory_None, eItemUsageCategory, CharClassCategory_None, eClassCategories, eTag);
	ui_GenSetManagedListSafe(pGen, peaStoreItemInfo, StoreItemInfo, false);
}
...
void exprStoreGetBuyItemListWithCategories(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt, U32 filterCategory)
{
	...
	peaStoreItemInfo = ui_GenGetManagedListSafe(pGen, StoreItemInfo);
	GetBuyItemList(peaStoreItemInfo, true, true, filterCategory, UsageRestrictionCategory_None, CharClassCategory_None, 0, ItemTag_Any);
	ui_GenSetManagedListSafe(pGen, peaStoreItemInfo, StoreItemInfo, false);
}
...
void exprStoreGetBuyItemListWithCategory(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt, const char *pchFilterCategory)
{
	...
	GetBuyItemList(&s_eaStoreItemInfo, true, false, category, UsageRestrictionCategory_None, CharClassCategory_None, 0, ItemTag_Any);
...
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(StoreBuyAndRemoveAllItems);
void exprStoreBuyAndRemoveAllItems(SA_PARAM_NN_VALID UIGen *pGen, SA_PARAM_OP_VALID Entity *pEnt, SA_PARAM_OP_VALID Entity* pTargetEnt)
{
	if (!pEnt || !pTargetEnt) {
		return;
	}

	ServerCmd_store_BuyAndRemoveAll(entGetType(pTargetEnt), entGetContainerID(pTargetEnt));
}
