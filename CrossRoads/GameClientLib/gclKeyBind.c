...
static KeyBindProfile s_Profile = {"EntityBinds", __FILE__, KeyboardLocale_EnglishUS, NULL, true, true, NULL, NULL, InputBindPriorityUser};
...
void gclKeyBindFillBinds(void)
{
	...
	while ((pProfile = keybind_ProfileIteratorNext(&Iter)))
	{
		...
		for (j = 0; j < eaSize(&pProfile->eaBinds); j++)
		{
			...
			keybind_ParseKeyString(pBind->pchKey, &iKey1, &iKey2, pProfile->eKeyboardLocale);
			for (i = iKeys - 1; i >= 0; i--)
			{
...
void gclKeyBindFillFromEntity(Entity *pEnt)
{
	...
	if (pEnt->pPlayer->pUI->iKeybindsVersion == 0)
	{
		Language lang = locGetLanguage(getCurrentLocale());

		if (lang == LANGUAGE_FRENCH || lang == LANGUAGE_GERMAN)
		{
			static ContainerID oldEntID = 0;
			ContainerID entID = entGetContainerID(pEnt);
			gclKeyBindUnbindAll();

			if (entID != oldEntID)
			{
				notify_NotifySend(pEnt, kNotifyType_GameplayAnnounce, TranslateMessageKey("Keybinds_ResetToDefaults"), "", "");
			}
	...
	ServerCmd_gslKeyBindSetVersion(KEYBIND_SETTINGS_VERSION);
}
...
void gclKeyBindSyncToServerEx(KeyBindProfile* pProfile)
{
	...
	PlayerUI *pUI = SAFE_MEMBER2(entActivePlayerPtr(),pPlayer,pUI);
	...
	for (i = 0; i < eaSize(&pProfile->eaBinds); i++)
	{
		...
		const char *pchCommand = pBind->pchCommand;
		const char *pchKey = keybind_GetKeyScancodeStrings(pBind);
		bool bSecondary = pBind->bSecondary;
...
void gclKeyBindEnterGameplay()
{
	if (!gConf.bKeybindsFromControlSchemes)
	{
		...
		const char *pchProfilePrefix = "GameDefault";
		char const * pchProfile = NULL;

		pProfile = keybind_FindProfileForKeyboardLocale(pchProfilePrefix, keybind_GetCurrentKeyboardLocale());

		if (!pProfile)
		{
			pProfile = keybind_FindProfile("GameDefaultFallback");

			if (!pProfile)
			{
				ErrorDetailsf("Keyboard locale 0x%X", keybind_GetCurrentKeyboardLocale());
				Errorf("User has keyboard locale with no defaults, and the fallback profile GameDefaultFallback is not present.");
			}
		}

		if (pProfile)
		{
			pchProfile = pProfile->pchName;
		}
...
static void BindSpecialsReload(const char *pchPath, S32 iWhen)
{
	...
	{
		s_BindSpecials.eaEntry[i]->iKeyCode = keybind_GetKeyScancode(s_BindSpecials.eaEntry[i]->pchKey, KeyboardLocale_Current);
...
static void KeyboardLayoutsReload(const char *pchPath, S32 iWhen)
{
	...
	for (i = 0; i < eaSize(&s_Keyboards.eaKeyboards); i++)
	{
		...
		{
			pLayout->eaKeys[j]->iKeyCode = keybind_GetKeyScancode(pLayout->eaKeys[j]->pchKey, KeyboardLocale_Current);
...
//Retrieve the "KeyBind" index for the first occurrence of a specific command. 
...
static void gclBindEntKey(ExprContext *pContext, S32 iKey, const char *pchCommand, bool bSecondary)
{
	...
	pchKeyString = keybind_GetNames(&Bind, KeyboardLocale_Current);
...
static KeyBindProfile s_Capture = {"CaptureKeys", __FILE__, KeyboardLocale_EnglishUS, NULL, false, true, BindCapture, NULL, InputBindPriorityUI};
...
void gclKeyBindExprGetKeyLayout(ExprContext *pContext, SA_PARAM_NN_VALID UIGen *pGen, const char *pchKeyboardLayout, const char *pchOtherKey)
{
	...
	if (pLayout)
	{
		iOtherKey = keybind_GetKeyScancode(pchOtherKey, KeyboardLocale_Current);
		...
		for (i = 0; i < eaSize(&s_eaKeyLayoutDefs); ++i)
		{
			...
			if (iKey < 0)
			{
				...
				pKey->iKey1 = kb.iKey1;
				pKey->iKey2 = kb.iKey2;
				pKey->pchKey = keybind_GetNames(&kb, KeyboardLocale_Current);
				pKey->pCommand = NULL;
				StructFreeStringSafe(&pKey->pchBindCommand);
			}
		}
