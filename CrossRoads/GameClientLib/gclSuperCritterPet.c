...
int scp_ExprGetPetXPBonusPool()
{
	...
	if (pData)
	{
		...
		iNumBonus = eaSize(&pItemDef->eaBonusNumerics);
...
void scp_ExprGetActivePetList(SA_PARAM_NN_VALID UIGen *pGen, int iMinSlotsToShow)
{
	...
	if (pEnt && pBag)
	{
		...
		for (i = 0; i < max(iMinSlotsToShow, iCount); i++)
		{
			...
			if (pUIPet->bTraining)
				pUIPet->uiTrainingTimeInSeconds = pPet->uiTimeFinishTraining >0 ? pPet->uiTimeFinishTraining - timeServerSecondsSince2000() : 0;
			...
			{
				s_eaFakeSCPEntities[i] = scp_CreateFakeEntity(pEnt, pItem, pPet);
...
void scp_ExprCreateFakePetInspectEnt(SA_PARAM_OP_VALID Item* pItem)
{
	...
	s_pFakeInspectSCPEntity = scp_CreateFakeEntity(entActivePlayerPtr(), pItem, NULL);
...
void scp_ExprGetPetEquipment(SA_PARAM_NN_VALID UIGen *pGen, int idx)
{
	...
	if (pData && eaGet(&pData->ppSuperCritterPets, idx))
	{
		...
		if (pBag && pPetDef)
		{
			int iCount = eaSize(&pPetDef->eaEquipSlots);
...
U32 scp_ExprGetPetTrainingTime(int idx)
{
	...
	if (pData && pData->ppSuperCritterPets)
	{
		...
		{
			time = pData->ppSuperCritterPets[idx]->uiTimeFinishTraining - timeServerSecondsSince2000();
		}
	}
	return time;
}
...
// send request to upgrade quality of active SCP in slot idx.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(scp_UpgradePetQuality);
void scp_ExprUpgradePetQuality(int idx)
{
	ServerCmd_scp_UpgradePetQuality(idx);
}

// returns whether the active SCP in slot idx can be upgraded by a player 
//	(whether it is at max level for this quality and is below max quality)
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(scp_CanUpgradePetQuality);
bool scp_ExprCanUpgradePetQuality(int idx)
{
	Entity* pEnt = entActivePlayerPtr();
	Item* pPetItem = scp_GetActivePetItem(pEnt, idx);
	return scp_GetPetCombatLevel(pPetItem) == scp_MaxLevel(pPetItem) && item_GetQuality(pPetItem) < g_SCPConfig.iMaxUpgradeQuality;
}

// get the cost of upgrading the quality of active SCP in slot idx.  Uses that pet's current quality and the table in the SCPConfig.
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(scp_GetUpgradePetQualityCost);
int scp_ExprGetUpgradePetQualityCost(int idx)
{
	Entity* pEnt = entActivePlayerPtr();
	Item* pPetItem = scp_GetActivePetItem(pEnt, idx);
	int iQuality = item_GetQuality(pPetItem);

	//item_GetQuality can return -1.
	return iQuality - 1 >= 0 ? g_SCPConfig.eafCostToUpgradeQuality[iQuality - 1] : 0;
}

// return the costume that the active SCP in slot idx has been set to use.
...
SA_RET_OP_VALID PlayerCostume* scp_ExprGetPetCostume(int idx)
{
	...
	if (pSCPCostumeDef)
	{
		return GET_REF(pSCPCostumeDef->hCostume);
	}
	else
	{
		// fallback on default critter costume.
		...
		return pCritterDef ? GET_REF(pCritterDef->ppCostume[0]->hCostumeRef) : NULL;
	}
}
...
SA_RET_NN_VALID const char* scp_ExprGetPetCostumeName(int idx)
{
	...
	if (pSCPCostumeDef)
	{
		return TranslateDisplayMessage(pSCPCostumeDef->displayMsg);
	}
	return TranslateMessageKey("SuperCritterPet.DefaultCostumeString");
}


SA_RET_OP_VALID PaperdollHeadshotData* scp_PaperdollGetHeadshotFromSCP(ExprContext* pContext, Item* pPetItem)
{
	Entity *pEnt = entActivePlayerPtr();
	...
	if (pPet)
	{
		...
		if (pSCPCostumeDef)
		{
			pCostume = GET_REF(pSCPCostumeDef->hCostume);
			pchFXName = REF_STRING_FROM_HANDLE(pSCPCostumeDef->hContinuingPlayerFX);
		}
		else if (pPetDef)
		{
			// fall back on critter def costume.
			...
			if (pCritterDef)
			{
				for (i = 0; i < eaSize(&pCritterDef->ppCostume); i++)
				{
					if ((pCritterDef->ppCostume[i]->iMinLevel < 0 || ((int)pPet->uLevel) >= pCritterDef->ppCostume[i]->iMinLevel)
						...
						&& pCritterDef->ppCostume[i]->iAssetSetIndex == -1)
					{
		...
		pData = gclPaperdoll_CreateHeadshotData(pContext, NULL, pCostume, NULL, NULL, pPetDef?pPetDef->pchStyleDef:NULL, &ppchAddedFx, false, false);
...
SA_RET_OP_VALID PaperdollHeadshotData* scp_ExprPaperdollGetHeadshotFromSCPActive(ExprContext* pContext, int idx)
{
	Entity *pEnt = entActivePlayerPtr();
	Item* pPetItem = scp_GetActivePetItem(pEnt, idx);
	return scp_PaperdollGetHeadshotFromSCP(pContext, pPetItem);
}

AUTO_EXPR_FUNC(UIGen) ACMD_NAME(PaperdollGetHeadshotFromSCPItem);
SA_RET_OP_VALID PaperdollHeadshotData* scp_PaperdollGetHeadshotFromSCPItem(ExprContext* pContext, SA_PARAM_OP_VALID Item* pPetItem)
{
	return scp_PaperdollGetHeadshotFromSCP(pContext, pPetItem);
}
...
void scp_ExprGetPetCostumeList(SA_PARAM_NN_VALID UIGen *pGen, int idx)
{
	...
	else
	{
		...
		if (pPetDef)
		{
			int iCount = eaSize(&pPetDef->eaCostumes) + 1;
			...
			{
				(*peaCostumeList)[i+1]->pchName =TranslateDisplayMessage(pPetDef->eaCostumes[i]->displayMsg);
...
SA_RET_OP_VALID Entity* scp_ExprGetFakePetEntity(int idx)
{
	return scp_GetFakePetEntity(idx);
}
...
{
	return GetPetEquipmentSlotTypeTranslate(langGetCurrent(),pPetItem,iSlot);
}
...
bool gclInv_InventoryKeyMoveSCP(const char *pchKeyA, S32 eBagB, S32 iSlotB, S32 iCount, const char* pchBackupBagName, const char* pchNewName)
{
	...
	if (!gclInventoryParseKey(pchKeyA, &KeyA))
		return false;
	if (!KeyA.pSlot || !KeyA.pSlot->pItem || KeyA.pOwner != pEntity)
		return false;
	...
	if (pItem->flags & kItemFlag_Bound && (KeyA.eBag == iBackupBagID || KeyA.eBag == InvBagIDs_SuperCritterPets))
	{
		...
		if (scp_CanMovePet(pEntity, KeyA.eBag, KeyA.iSlot, eBagB, iSlotB))
		{
			ServerCmd_scp_MoveBoundPet(eBagB, iSlotB, KeyA.eBag, KeyA.iSlot);
		}
		else
		{
			notify_NotifySend(pEntity, kNotifyType_DuplicateUniqueItem, TranslateMessageKeySafe("SuperCritterPets.UniqueActive"), NULL, NULL);
			return false;
		}
	}
	else
	{
		if (scp_CanBindPet(pEntity, pItem))
		{
...
bool gclInvExprInventoryKeyEquipPet(const char *pchKeyA, int iPetIdx, S32 iSlot)
{
	...
	{
		if (pDef->peRestrictBagIDs[i] == pPetDef->eaEquipSlots[iSlot]->eID)
		{
			bValid = true;
			break;
		}
...
const char* scp_ExprGetSCPIcon(SA_PARAM_OP_VALID Item* pPetItem)
{
	if (pPetItem && pPetItem->pSpecialProps && pPetItem->pSpecialProps->pSuperCritterPet)
	{
		...
		if(pPetDef && pPetDef->pchIconName)
...
{
	ServerCmd_scp_ChangeCostume(idx, iCostume - 1);
}
...
{
	if (iSlot > 0 && iSlot < eaiSize(&g_SCPConfig.eaiEquipSlotUnlockLevels))
		return g_SCPConfig.eaiEquipSlotUnlockLevels[iSlot];
...
{
	if (iSlot > 0 && iSlot < eaiSize(&g_SCPConfig.eaiGemSlotUnlockLevels))
		return g_SCPConfig.eaiGemSlotUnlockLevels[iSlot];

	return 1;
}

// UIGen Model list call to get a list of SuperCritterPetActivePowerDef based on the Item & SCP def
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(scp_GetSuperCritterPetActivePowers);
void scp_ExprGetSuperCritterPetActivePowers(SA_PARAM_OP_VALID UIGen *pGen, SA_PARAM_OP_VALID Item *pPetItem)
{
	if (pGen)
	{
		SuperCritterPet* pPet = pPetItem ? scp_GetPetFromItem(pPetItem) : NULL;
		SuperCritterPetDef* pSCPDef = pPet ? GET_REF(pPet->hPetDef) : NULL;
		SuperCritterPetActivePowerDef*** peaSCPPowerListNodes = (SuperCritterPetActivePowerDef***)ui_GenGetManagedList(pGen, parse_SuperCritterPetActivePowerDef);

		eaClear(peaSCPPowerListNodes);
		
		if (pSCPDef)
		{
			scp_GetActivePowerDefs(pSCPDef, pPetItem, peaSCPPowerListNodes);
		}

		ui_GenSetManagedList(pGen, peaSCPPowerListNodes, parse_SuperCritterPetActivePowerDef, false);
	}
}

// Returns true if the pet can be moved into the active SuperCritterPets bag
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(scp_CanBindPet);
bool scp_ExprCanBindPet(SA_PARAM_OP_VALID Item *pPetItem)
{
	Entity* pEntity = entActivePlayerPtr();
	return scp_CanBindPet(pEntity, pPetItem);
}

// Returns true if the given source bag/slot SCP can be moved into the destination bag/slot
AUTO_EXPR_FUNC(UIGen) ACMD_NAME(scp_CanMovePet);
bool scp_ExprCanMovePet(S32 eSrcBag, S32 iSrcSlot, S32 eDstBag, S32 iDstSlot)
{
	Entity* pPlayerEnt = entActivePlayerPtr();

	if (pPlayerEnt)
	{
		return scp_CanMovePet(pPlayerEnt, eSrcBag, iSrcSlot, eDstBag, iDstSlot);
	}
