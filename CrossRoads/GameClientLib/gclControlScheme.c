...
static RegionRules* ControlSchemeGetCurrentRegionRules(void)
{
	...
	{
		// This is kind of wonky. Perhaps the camera presets should not be stored in the region rules.
		//  Or at least the controlScheme stuff should not be hooked up like this.
		// The idea seems to be use the entities current region rules, unless the current region rules don't
		//   match the scheme. In which case use ANY regions rules that match the scheme. That doesn't
		//   seem particularly useful. Especially if different regions that match the scheme have different
		//   settings. WOLF[18Oct2013]

		RegionRules* pRules = getRegionRulesFromEnt(pEnt);

		// Use the entities current region if it matches.
		if (pRules && pRules->eSchemeRegionType == eSchemeRegion)
		{
			return pRules;
		}
		// Otherwise get ANY regionRules that match this scheme
		return regionRules_GetAnyRegionRulesValidForSchemeRegionType(eSchemeRegion);
	}
	return NULL;
}
...
static void ControlSchemeFillKeyBindsFromProfile(const char* pchProfile)
{
	...
	assert(gConf.bKeybindsFromControlSchemes);
...
void ControlSchemeUpdateFlightParams(OptionSetting *setting)
{
	ServerCmd_UpdateFlightParams();
}
...
void schemes_initOptions(Entity *pEnt)
{
	...
	{	
		...
		autoSettingsAddFloatSlider(CONTROLSCHEME_CATEGORY, "FlightMaxPitchAngle",
									g_CombatConfig.fFlightPitchClampMin,
									g_CombatConfig.fFlightPitchClamp,
									(F32*)&pStoredScheme->fFlightMaxPitchAngle,
									NULL, ControlSchemeFieldCommitCallback, ControlSchemeUpdateFlightParams,
									ControlSchemeRevertCallback, displayTheFloatValue, NULL, true);
		...
		options_HideOption(CONTROLSCHEME_CATEGORY, "FlightMaxPitchAngle", NULL);
