...
static void gclGameplayChoosingGameServer_Enter(void)
{
	...
	if (devassertmsg(eaSize(&s_MapChoices.ppChoices), "Server sent empty map choice list (or in the wrong state)!"))
	{
		...
		if(pEntity && pEntity->pSaved && pEntity->pSaved->pPuppetMaster && pEntity->pSaved->pPuppetMaster->bPuppetClientWantsAll)
		{
			// going to new server clear the subscription info for puppets
			pEntity->pSaved->pPuppetMaster->bPuppetClientWantsAll = false;
		}
