...
#include "gclCommandParse_c_ast.h"
...
int GameClientParsePrivate(const char *str, CmdContextFlag iFlags, Entity *ent, int iOverrideAccessLevel, enumCmdContextHowCalled eHow, CmdParseStructList *pStructs)
{
	...
	if (!parseResult)
	{
		if (!cmd_context.found_cmd)
		{
			char *pShortCommandName = NULL;
			Cmd *pCmd;

			//we're hitting this error a lot, mysteriously. So let's stick some additional info in the logging that will
			//hopefully help track things down
			estrStackCreate(&pShortCommandName);
			estrCopy2(&pShortCommandName, str);
			estrTruncateAtFirstOccurrence(&pShortCommandName, ' ');

			pCmd = cmdListFind(&gPrivateCmdList, pShortCommandName);

			if (!pCmd)
			{
				Errorf("Private command (%s)(%s) not found at all. AL %d. HowCalled %s. Flags %d.",
					str,pShortCommandName,
					cmd_context.access_level, GetContextHowString(&cmd_context), iFlags);
			}
			else
			{
				Errorf("Private command (%s)(%s) not found initially, but does exist. AL %d. HowCalled %s. Flags %d. Actual AL of command: %d",
					str, pShortCommandName,
					cmd_context.access_level, GetContextHowString(&cmd_context), iFlags, pCmd->access_level);

			}

			estrDestroy(&pShortCommandName);
		}
		else
		{
...
void PowersCancelPowers(U32 uiID, bool bForceCurrent, bool bRecharge, U32 uiIDPower, S32 iIdxSub, S8 iLinkSub)
{
	Entity* e = entActivePlayerPtr();
	if(e && e->pChar)
	{
		...
		else
		{
			...
			{
				Power *ppow = character_FindPowerByRefParams(p, uiIDPower, iIdxSub, iLinkSub);
...
int gclLoadLoginConfig(void)
{
	...
	// If the Launcher ever changes where it stores the saved username, you MUST CHANGE this section to read from the right place
	if (isDevelopmentMode())
	{
		regGetAppString("DevUserName", "", SAFESTR(gGCLState.loginName));
		
		if (!gGCLState.loginName[0])
		{
			tmp = GamePrefGetString("Login.Username", NULL);
			if (tmp)
	...
	{
		regGetAppString_ForceAppName("Cryptic Launcher", "UserName", "", SAFESTR(gGCLState.loginName));
	}
...
{
	// If the Launcher ever changes where it stores the saved username, you MUST CHANGE this section to write to the right place
	if (isDevelopmentMode())
		regPutAppString("DevUserName", gGCLState.loginField);
	else
		regPutAppString_ForceAppName("Cryptic Launcher", "UserName", gGCLState.loginField);
...
//don't request args for the same function from the server more than every 120 seconds
#define ARG_REQUEST_THROTTLING_CUTOFF 120

AUTO_STRUCT;
typedef struct ArgNameRequestThrottlingStruct
{
	char *pCommandName;
	U32 iTimeSubmitted;
} ArgNameRequestThrottlingStruct;

StashTable sThrottlingRequestsByName = NULL;
...
void ResetClientCommandArgsForAutoCompletion(void)
{
	...
	if (sThrottlingRequestsByName)
	{
		stashTableClearStruct(sThrottlingRequestsByName, NULL, parse_ArgNameRequestThrottlingStruct);
	}
}
...
void AddCommandArgsToClientForAutoCompletion(NamesForAutoCompletion *pNames)
{
	...
	if (!bCacheNotFull)
	{
		ArgNameRequestThrottlingStruct *pThrottlingStruct;
		//also remove it from the throttling cache at this point so that a new request for the args will go through
		if (stashRemovePointer(sThrottlingRequestsByName, gArgNameCacheForAutoCompletion[NUM_CACHED_COMMANDS_FOR_ARG_AUTO_COMPLETION-1].pCommandName,
			&pThrottlingStruct))
		{
			StructDestroy(parse_ArgNameRequestThrottlingStruct, pThrottlingStruct);
		}
...
void ThrottlingRequestPurgeCB(TimedCallback *callback, F32 timeSinceLastCallback, UserData userData)
{
	U32 iCutoffTime = timeSecondsSince2000() - ARG_REQUEST_THROTTLING_CUTOFF;
	FOR_EACH_IN_STASHTABLE(sThrottlingRequestsByName, ArgNameRequestThrottlingStruct, pStruct)
	{
		if (pStruct->iTimeSubmitted < iCutoffTime)
		{
			stashRemovePointer(sThrottlingRequestsByName, pStruct->pCommandName, NULL);
			StructDestroy(parse_ArgNameRequestThrottlingStruct, pStruct);
		}
	}
	FOR_EACH_END;
}
...
void RequestAutoCompletionArgNamesFromServer_Throttled(char *pCommandName)
{
	ArgNameRequestThrottlingStruct *pThrottlingStruct = NULL;
	U32 iCurTime = timeSecondsSince2000();

	if (!sThrottlingRequestsByName)
	{
		sThrottlingRequestsByName = stashTableCreateWithStringKeys(5, StashDefault);
		TimedCallback_Add(ThrottlingRequestPurgeCB, NULL, 60.0f);
	}

	if (stashFindPointer(sThrottlingRequestsByName, pCommandName, &pThrottlingStruct))
	{
		if (pThrottlingStruct->iTimeSubmitted > iCurTime - ARG_REQUEST_THROTTLING_CUTOFF)
		{
			//already request recently, do nothing
			return;
		}
		else
		{
			pThrottlingStruct->iTimeSubmitted = iCurTime;
		}
	}

	ServerCmd_RequestAutoCompletionArgNamesFromServer(pCommandName);

	if (!pThrottlingStruct)
	{
		pThrottlingStruct = StructCreate(parse_ArgNameRequestThrottlingStruct);
		pThrottlingStruct->pCommandName = strdup(pCommandName);
		pThrottlingStruct->iTimeSubmitted = iCurTime;
		stashAddPointer(sThrottlingRequestsByName, pThrottlingStruct->pCommandName, pThrottlingStruct, false);
	}
}
...
enumNameListType gclCmdParseGetExtraNameListTypeForArgAutoCompletion(char *pCommandName, int iArgNum) {
	...
	RequestAutoCompletionArgNamesFromServer_Throttled(pCommandNameToUse);
	return NAMELISTTYPE_NONE;
}
...
#include "gclCommandParse_c_ast.c"