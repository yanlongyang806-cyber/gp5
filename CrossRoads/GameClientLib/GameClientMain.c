...
#elif _XBOX
int wmain(int argc_in, const char** argv_in)
#else
...
#endif
{
	...
	{
		HRSRC rsrc = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(gGCLState.logoResource), L"JPG");
		if (rsrc)
		{
...
void gclExternOncePerFrame(F32 elapsed, bool drawWorld)
{
	...
	ItemUpgradeUI_Tick(entActivePlayerPtr(), fTickTime);
