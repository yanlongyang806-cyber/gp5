...
static bool patch_streaming_beta = false;
static int patch_streaming_mac = -1;  // -1 means default
...
// Test the beta filespec, instead of the live one.
AUTO_CMD_INT(patch_streaming_beta, PatchStreamingBeta) ACMD_ACCESSLEVEL(0) ACMD_CATEGORY(CommandLine) ACMD_CMDLINE ACMD_EARLYCOMMANDLINE;

// Test the Mac filespec, instead of the live one.
AUTO_CMD_INT(patch_streaming_mac, PatchStreamingMac) ACMD_ACCESSLEVEL(0) ACMD_CATEGORY(CommandLine) ACMD_CMDLINE ACMD_EARLYCOMMANDLINE;
...
// Check if file needs preprocessing, and if so, preprocess it.
static FileScanAction PatchStreamingPreprocess(const char *fullpath, FolderNode *node, void *pUserData_Proc, void *pUserData_Data)
{
	static const char suffix[] = ".preproc.txt";
	...
	// See if this file should be preprocessed, and if so, preprocess it.
	slash = strrchr(fullpath, '/');			// Don't match against directory.
	if (!slash)
		slash = fullpath;
	if ((pos = strstri(slash, suffix))
		&& pos[sizeof(suffix) - 1] == 0)  // Make sure it's at the end.
	{
		char output[MAX_PATH];
		char output_write[MAX_PATH];
		char output_temp[MAX_PATH];
		char output_temp_write[MAX_PATH];
		int different;
		PCL_ErrorCode error;
		int result;

		// Get the output filenames.
		strncpy(output, fullpath, pos-fullpath);
		strcat(output, ".filespecoverride");
		fileLocateWrite(output, output_write);
		strncpy(output_temp, output, pos-fullpath);
		strcat(output_temp, ".tmp");
		fileLocateWrite(output_temp, output_temp_write);

		// Preprocess the file.
		error = pclProprocessFilespec(fullpath, output_temp);
		if (error)
			FatalErrorFilenamef(fullpath, "Unable to preprocess");

		// If the processed file is different, save it.
		different = fileCompare(output, output_temp);
		if (different)
		{
			GimmeErrorValue gimme_result;
			__time32_t timestamp;
			S64 size;

			// Check out file, if necessary.
			timestamp = fileLastChanged(output);
			size = fileSize64(output);
			gimme_result = gimmeDLLDoOperation(output, GIMME_CHECKOUT, GIMME_QUIET);
			if (gimme_result != GIMME_NO_ERROR)
				FatalErrorFilenamef(fullpath, "Unable to check out output file %s (%s)", output, gimmeDLLGetErrorString(gimme_result));
				
			// Move the old output file out of the way, if Gimme didn't already do that.
			if (timestamp == fileLastChanged(output) && size == fileSize64(output))
			{
				result = fileRenameToBak(output_write);
				if (result && fileExists(output_write))
					FatalErrorFilenamef(fullpath, "Unable to backup output file %s", output);
			}
			else
			{
				result = fileForceRemove(output_write);
				if (result)
					FatalErrorFilenamef(fullpath, "Unable to remove output file %s", output);
			}

			// Move the file into place.
			result = fileMove(output_temp_write, output_write);
			if (result)
				FatalErrorFilenamef(fullpath, "Unable to rename output file %s", output);
		}
		else
		{
			result = fileForceRemove(output_temp);
			if (result)
				ErrorFilenamef(fullpath, "Unable to remove temporary file %s", output_temp);
		}
	}
...
void PatchStreamingAutoEnable(void)
{
	if (g_PatchingStreamingOn)
	{
		...
		if (g_PatchProject[0])
		{
			...
			{
				char patch_flagspec_base[CRYPTIC_MAX_PATH];
				bool found = false;
				const char *language;
				bool use_mac;

				// Preprocess any filespecoverrides that need to be processed.
				fileScanAllDataDirs2("patchfilespec", PatchStreamingPreprocess, g_ExtraFilespec);

				// Search for a filespec.
				// Mimic CrypticLauncher decision-making process.  Try a few names in order of priority.

				// Decide if we're on the Mac or not.
				if (patch_streaming_mac == 0)
					use_mac = false;
				else
					use_mac = patch_streaming_mac || getIsTransgaming();

				// Compose the base name.
				sprintf(patch_flagspec_base, "patchfilespec/%sClient%s%s",
					GetProductName(),
					patch_streaming_beta ? "Beta" : "",
					patch_streaming_mac ? "Mac" : "");
				language = locGetName(getCurrentLocale());

				// First, try language-specific file.
				sprintf(g_ExtraFilespec, "%s%s.filespecoverride", patch_flagspec_base, language);
				found = fileExists(g_ExtraFilespec);

				// Next, try flat non-language specific file.
				if (!found)
				{
					sprintf(g_ExtraFilespec, "%s.filespecoverride", patch_flagspec_base);
					found = fileExists(g_ExtraFilespec);
				}

				// Make sure the filespec override defines optional files.
				if (found)
				{
					found = fileContainsString(g_ExtraFilespec, "NotRequired");
					if (!found)
						ErrorFilenamef(g_ExtraFilespec, "NotRequired");
				}

				// If we didn't find a .filespecoverride file, we're going to have to turn off dev mode patch streaming simulation.
				if (found)
					leaveStreamingOn = true;
				else
					*g_ExtraFilespec = 0;
		...
		{
			if (isProductionMode())
				loadend_printf(" disabled (no appropriate filespec override found)");
			else
				loadend_printf(" disabled (no optional files in filespec)");
			g_PatchingStreamingOn = false;
		}
		else
		{
			...
			loadend_printf(" enabled (simulation %s%s%s)",
				g_PatchStreamingSimulate ? (make_fast ? "\"fast mode\"" : "on") : "off",
				g_ExtraFilespec[0] ? ", " : "",
				g_ExtraFilespec);
		}
	}
