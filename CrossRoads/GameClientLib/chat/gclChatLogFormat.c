...
static void formatUser(char **estrFormattedUser, ChatUserInfo *pUser, ChatLogEntryType eType, const ChatLogEntry *pEntry, SA_PARAM_NN_VALID ChatLink **ppLink) {
	...
	const U32 iEntContainerID = pUser->playerID;
	...
	if (*pchUser && eType == kChatLogEntryType_System) {
		...
		if (*pchHandle) {
			...
			*ppLink = ChatData_CreatePlayerLink(achNameAndHandle, *estrFormattedUser, pUser->bIsGM, pUser->bIsDev, SAFE_MEMBER2(pEntry, pMsg, pchText), iEntContainerID);
		}
	}
}
