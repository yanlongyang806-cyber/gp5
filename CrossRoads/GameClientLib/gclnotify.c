...
static NotifyAction* gclNotifyAction_FindAction(NotifyType eType, const char *pchLogicalStringFilter)
{
	...
	if (eType >= kNotifyType_Default && eType < kNotifyType_COUNT)
	{
		...
		{
			/*
			Notify NULL                                   --> Action 1

			Notify "Bar" (no tutorial gen)                --> Action 1
			Notify "Bar" (is tutorial gen; not on screen) --> Action 2, Action 1
			Notify "Bar" (is tutorial gen; on screen)     --> Action 3, Action 1

			Notify "Foo" (no tutorial gen)                --> Action 4, Action 1
			Notify "Foo" (is tutorial gen; not on screen) --> Action 5, Action 2, Action 1
			Notify "Foo" (is tutorial gen; on screen)     --> Action 6, Action 3, Action 1
			...
			Action 2
			{
				LogicalStringIsTutorial 0
			}

			Action 3
			{
				LogicalStringIsTutorial 1
			}

			Action 4
			{
				LogicalStringFilter Foo
			}
			...
			*/
			...
			{
				// Action 1
				if (!pDefaultAction || !pTargetGen)
					pDefaultAction = pAction;
				continue;
			}
			else if (!pAction->ppchLogicalStringFilters && pTargetGen)
			{
				// Action 2, Action 3
			...
			if (pAction->ppchLogicalStringFilters && 
				(!pchLogicalStringFilter || eaFind(&pAction->ppchLogicalStringFilters, pchLogicalStringFilter) == -1))
			{
				continue;
			}
...
static void gclNotifyInitExpressionContext(void)
{
	if ( s_pNotifyContext==NULL )
	{
		...
		stFuncTable = exprContextCreateFunctionTable("Notify");
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_NAME(NotifySendWithTagAndValue) ACMD_CLIENTCMD ACMD_HIDE;
void gclNotifyReceiveWithTagAndValue(NotifyType eType, const char *pchDisplayString, const char *pchLogicalString, const char *pchTag, S32 iValue)
{
	gclNotifyReceiveInternal(eType, pchDisplayString, pchLogicalString, NULL, NULL, NULL, NULL, pchTag, iValue, NULL, 0, 0);
}
...
{
	memset(s_NotifySettings, 0, kNotifyType_COUNT * sizeof(NotifySettingFlags));
								...
								EntityRef erEntity)
{
	...
	if (pchSound && *pchSound && !gclNotify_CheckSettingFlags(eType, kNotifySettingFlags_DisableNotifySound))
	{
		sndPlayUIAudio(pchSound, s_Actions.pchFilename);
	}

	if (!gclNotify_CheckSettingFlags(eType, kNotifySettingFlags_DisableActionSound))
	{
		for (i = 0; i < eaSize(&pAction->eaSound); i++)
		{
