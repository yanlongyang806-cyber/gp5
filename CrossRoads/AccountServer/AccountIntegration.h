...
AST_PREFIX( PERSIST )
AUTO_STRUCT AST_CONTAINER AST_IGNORE(pLocale);
typedef struct PWCommonAccount
{
	...
	CONST_STRING_MODIFIABLE pRegistrationPage;
...
typedef struct PerfectWorldAccountBatch
{
	...
	AST_COMMAND("Apply Transaction","ServerMonTransactionOnEntity accountbatch $FIELD(UBatchID) $STRING(Transaction String)")
...
PerfectWorldUpdateResult AccountIntegration_CreateOrUpdatePWCommon(
	...
	SA_PARAM_OP_STR const char *pLocale,
...
// Scan through all of the batches on start-up
