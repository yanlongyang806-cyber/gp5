...
AUTO_TRANSACTION
ATR_LOCKS(pAccount, ".Eauindexedlogbatchids, .Eaurebucketlogbatchids, .Pptemporarylogbufferduringrebucketing");
enumTransactionOutcome trAccountLog_CommitRebucketing(ATR_ARGS, NOCONST(AccountInfo) *pAccount)
{
	// Destroy existing indexed batches, and rebucketing buffer
	...
	// The next log ID and rebucketed flag will be set, and indexed buffer destroyed, outside the transaction
	return TRANSACTION_OUTCOME_SUCCESS;
}

AUTO_TRANSACTION
ATR_LOCKS(pAccount, ".Eaulogbatchids, .Eauindexedlogbatchids, .Eaurebucketlogbatchids, .Pplogentriesbuffer, .Pplogentries, .Pptemporarylogbufferduringrebucketing");
enumTransactionOutcome trAccountLog_CommitRebucketing_NonIndexed(ATR_ARGS, NOCONST(AccountInfo) *pAccount)
{
	...
	// The next log ID and rebucketed flag will be set outside the transaction
	return TRANSACTION_OUTCOME_SUCCESS;
}
...
static void makeAccountLogNonTransacted(const char *pFieldName, U32 uIndex, U32 uTimestamp, const char *pLogString, char **ppDiffString)
{
	estrConcatf(ppDiffString, "create %s[\"%u\"]\n", pFieldName, uIndex);
	estrConcatf(ppDiffString, "set %s[\"%u\"].UID = \"%u\"\n", pFieldName, uIndex, uIndex);
	estrConcatf(ppDiffString, "set %s[\"%u\"].Usecondssince2000 = \"%u\"\n", pFieldName, uIndex, uTimestamp ? uTimestamp : timeSecondsSince2000());
	estrConcatf(ppDiffString, "set %s[\"%u\"].Pmessage = \"%s\"\n", pFieldName, uIndex, pLogString);
}

static void updateNextLogIDNonTransacted(U32 uNextLogID, char **ppDiffString)
{
	estrConcatf(ppDiffString, "set .Unextlogid = \"%u\"\n", uNextLogID);
}

static void removeIndexedBufferLogNonTransacted(U32 uLogID, char **ppDiffString)
{
	estrConcatf(ppDiffString, "destroy .Ppindexedlogbuffer[\"%u\"]\n", uLogID);
}
...
static void commitRebucketing(const AccountInfo *pAccount)
{
	...
	{
		assert(sAccountLogRebucketingStatus.uNextLogID == pAccount->uNextLogID + eaSize(&pAccount->ppTemporaryLogBufferDuringRebucketing));
		...
		AutoTrans_trAccountLog_CommitRebucketing(NULL, GLOBALTYPE_ACCOUNTSERVER, GLOBALTYPE_ACCOUNT, pAccount->uID);
	}
	else
	{
		...
		AutoTrans_trAccountLog_CommitRebucketing_NonIndexed(NULL, GLOBALTYPE_ACCOUNTSERVER, GLOBALTYPE_ACCOUNT, pAccount->uID);
	}

	EARRAY_CONST_FOREACH_BEGIN(pAccount->ppIndexedLogBuffer, iEntry, iNumEntries);
	{
		removeIndexedBufferLogNonTransacted(pAccount->ppIndexedLogBuffer[iEntry]->uID, &pDiffString);
	}
	EARRAY_FOREACH_END;

	updateNextLogIDNonTransacted(sAccountLogRebucketingStatus.uNextLogID, &pDiffString);
	accountSetFlagsNonTransacted(pAccount, ACCOUNT_FLAG_LOGS_REBUCKETED, &pDiffString);
	AccountNonTransactedChange(GLOBALTYPE_ACCOUNT, pAccount->uID, pDiffString);
...
static void flushOneBufferedLog(SA_PARAM_NN_VALID const AccountInfo *pAccount)
{
	...
	makeAccountLogNonTransacted(".Eaindexedlogentries", pEntry->uID, pEntry->uSecondsSince2000, pEntry->pMessage, &pDiffString);
	AccountNonTransactedChange(GLOBALTYPE_ACCOUNTSERVER_LOG_BATCH, pBatch->uBatchID, pDiffString);
	estrClear(&pDiffString);

	removeIndexedBufferLogNonTransacted(pEntry->uID, &pDiffString);
	AccountNonTransactedChange(GLOBALTYPE_ACCOUNT, pAccount->uID, pDiffString);
	estrClear(&pDiffString);
}
...
static void addAccountLog(SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pLogString)
{
	...
	if (eaSize(&pAccount->ppIndexedLogBuffer) < giAccountLogBufferSize)
	{
		...
		static char *pDiffString = NULL;
		makeAccountLogNonTransacted(".Ppindexedlogbuffer", uNewLogID, 0, pLogString, &pDiffString);
		updateNextLogIDNonTransacted(pAccount->uNextLogID + 1, &pDiffString);
		AccountNonTransactedChange(GLOBALTYPE_ACCOUNT, pAccount->uID, pDiffString);
		estrClear(&pDiffString);
	}
	else
	{
		...
		makeAccountLogNonTransacted(".Eaindexedlogentries", uNewLogID, 0, pLogString, &pDiffString);
		AccountNonTransactedChange(GLOBALTYPE_ACCOUNTSERVER_LOG_BATCH, pBatch->uBatchID, pDiffString);
		estrClear(&pDiffString);
