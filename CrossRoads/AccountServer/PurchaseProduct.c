...
static void freePurchaseSession(SA_PARAM_NN_VALID PurchaseSession *pPurchaseSession, PurchaseResult eResult)
{
	...
	if (pPurchaseSession->eaProducts)
	{
		...
		if (commit)
		{
			pLog = AccountTransactionOpenNonTransacted(pPurchaseSession->pAccount->uID,
				...
				NULL);
			AccountTransactionRecordKeyValueChangesNonTransacted(pLog, pPurchaseSession->eaKeyValueChanges, NULL);
		}
		...
		for (curProduct = 0; curProduct < eaSize(&pPurchaseSession->eaProducts); curProduct++)
		{
			if (commit)
			{
				...
				AccountTransactionRecordPurchaseNonTransacted(pLog, pProduct->uID, &price);
				moneyDeinit(&price);
								   ...
								   SA_PARAM_OP_VALID void *pUserData)
{
	...
	if (!isRealCurrency(pCurrency))
	{
		...
		// Lock the amount
		eKeyValueResult = AccountKeyValue_Change(pDetails->pProxy, pAccount, getPointCurrency(pCurrency), -moneyCountPoints(&total), &pPurchaseSession->pPurchaseInformation->pPassword);
	...
	case PURCHASE_RESULT_PENDING:
		...
		EARRAY_FOREACH_BEGIN(pPurchaseSession->pPurchaseInformation->eaActivations, i);
		{
			...
			EARRAY_FOREACH_BEGIN(pActivation->ppKeyLocks, j);
			{
				...
				if (AccountKeyValue_LockAgain(pPurchaseSession->pPurchaseInformation->pProxy, pPurchaseSession->pAccount, pPair->pKey, &pPair->pLock) != AKV_SUCCESS)
				{
					PERFINFO_AUTO_STOP();
					PERFINFO_AUTO_STOP();
					return false;
		...
		{
			if (AccountKeyValue_LockAgain(pPurchaseSession->pPurchaseInformation->pProxy, pPurchaseSession->pAccount, getPointCurrency(pPurchaseSession->pPurchaseInformation->pCurrency),
									   ...
									   SA_PARAM_OP_VALID void *pUserData)
{
	...
	if (pPurchaseSession->pPurchaseInformation->uFlags & PURCHASEINFO_POINTS)
	{
		if (bCapture)
		{
			...
			CHECK(AccountKeyValue_Commit(pPurchaseSession->pPurchaseInformation->pProxy, pPurchaseSession->pAccount, getPointCurrency(pPurchaseSession->pPurchaseInformation->pCurrency),
		...
		else
		{
			...
			CHECK(AccountKeyValue_Rollback(pPurchaseSession->pPurchaseInformation->pProxy, pPurchaseSession->pAccount, getPointCurrency(pPurchaseSession->pPurchaseInformation->pCurrency),
