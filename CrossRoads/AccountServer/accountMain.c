...
static BOOL consoleCtrlHandler(DWORD fdwCtrlType)
{
	switch (fdwCtrlType){ 
		...
		case CTRL_C_EVENT: 
			...
			AccountServerSetExitCalled(true);
			while (1)
			{
...
{
	static U32 uLastXperfDump = 0;
	AccountServerConfig *pConfig = GetAccountServerConfig();

	PERFINFO_AUTO_START_FUNC();
	if (fElapsed > pConfig->uSecondsXperfDumpLongFrame)
	{
		...
		if (isProductionMode() && timeSecondsSince2000() - uLastXperfDump >= pConfig->uSecondsXperfDumpReset)
		{
			xperfDump("slowframe");
			uLastXperfDump = timeSecondsSince2000();
		}
...
int wmain(int argc, WCHAR** argv_wide)
{
	...
	SetConsoleTitle(L"Account Server - Starting...");
	...
	SetConsoleTitle_UTF8(GetAccountServerConsoleTitle());
	...
	if (!isAccountServerMode(ASM_Merger) && !isAccountServerMode(ASM_UpdateSchemas))
	{
		...
		U32 loopCount = 0;
		while (!AccountServerGetExitCalled())
		{
