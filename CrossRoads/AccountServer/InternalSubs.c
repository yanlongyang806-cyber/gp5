...
static void addInternalSubToStashTables(SA_PARAM_NN_VALID InternalSubscription *pInternalSub)
{
	...
	if (!stInternalSubsByAccountIDAndSubID)
		stInternalSubsByAccountIDAndSubID = stashTableCreateWithStringKeys(100, StashDeepCopyKeys_NeverRelease);
...
bool internalSubCreate(SA_PARAM_NN_VALID AccountInfo * pAccount, SA_PARAM_NN_VALID const char * pSubInternalName, U32 uExpiration, U32 uProductID)
{
	...
	// Schedule an initial stipend grant for the new internal sub
	scheduleInitialStipendGrant(pAccount, pInternalSub->pSubInternalName, getInternalSubAnniversaryDay(CONTAINER_RECONST(InternalSubscription, pInternalSub)), true);
	
	// Destroy the template object, or else it leaks!
	StructDestroyNoConst(parse_InternalSubscription, pInternalSub);
...
bool isInternalSubBilledThroughTime(const InternalSubscription *pSub, U32 uTime)
{
	if (pSub->uExpiration && pSub->uExpiration < uTime)
		return false;

	return true;
}

int getInternalSubAnniversaryDay(const InternalSubscription *pSub)
{
	struct tm localTime = {0};

	timeMakeLocalTimeStructFromSecondsSince2000(pSub->uCreated, &localTime);
