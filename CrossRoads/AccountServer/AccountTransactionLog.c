...
static StashTable stTransactionLogByPWOrderID = NULL;
...
#define ACCT_TRANS_ID_DUPLICATED_FMT " %s was duplicated, did not add (acct: %u, trans: %u) to stash table"
	...
	if (pLog->pPWOrderID && !stashAddPointer(stTransactionLogByPWOrderID, pLog->pPWOrderID, pLog, false))
		ErrorOrAlert("PWORDERID_DUPLICATED", "PW Order ID" ACCT_TRANS_ID_DUPLICATED_FMT, pLog->pPWOrderID, pLog->uAccountID, pLog->uID);
...
static void AccountTransactionRemoveCB(Container *con, AccountTransactionLogContainer *pLogContainer)
{
	EARRAY_CONST_FOREACH_REVERSE_BEGIN(pLogContainer->eaTransactions, iLog, iNumLogs);
	{
		...
		stashRemovePointer(stTransactionLogByPWOrderID, pLog->pTransactionID, NULL);
	}
	EARRAY_FOREACH_END;
}
...
void InitializeAccountTransactionLog(void)
{
	...
	stTransactionLogByPWOrderID = stashTableCreateWithStringKeys(131072, StashDefault);
}
...
static void AccountTransactionPopulateLog(NOCONST(TransactionLogContainer) *pLog, U32 uLogID, U32 uAccountID, U32 eType, const char *pSource, U32 eProvider, const char *pMerchantTransactionID, const char *pOrderID, const char *pPWOrderID)
{
	char *pUniqueID = AccountTransactionCreateNewID();

	pLog->uID = uLogID;
	pLog->uAccountID = uAccountID;
	...
	pLog->pPWOrderID = pPWOrderID ? StructAllocString(pPWOrderID) : NULL;

	pLog->eTransactionType = eType;
}

NOCONST(TransactionLogContainer) *AccountTransactionOpenNonTransacted(U32 uAccountID, TransactionLogType eType, const char *pSource, TransactionProvider eProvider, const char *pMerchantTransactionID, const char *pOrderID, const char *pPWOrderID)
{
	AccountTransactionLogContainer *pLogContainer = AccountTransactionGetLogContainer(uAccountID, true);
	NOCONST(TransactionLogContainer) *pLog = StructCreateNoConst(parse_TransactionLogContainer);
	AccountTransactionPopulateLog(pLog, pLogContainer->uNextLogID, uAccountID, eType, pSource, eProvider, pMerchantTransactionID, pOrderID, pPWOrderID);
	return pLog;
}

static void AccountTransactionRecordPurchase(NOCONST(TransactionLogContainer) *pLog, U32 uProductID, const Money *pPrice)
{
...
void AccountTransactionRecordPurchaseNonTransacted(NOCONST(TransactionLogContainer) *pLog, U32 uProductID, const Money *pPrice)
{
	AccountTransactionRecordPurchase(pLog, uProductID, pPrice);
}

static void AccountTransactionRecordKeyValueChanges(NOCONST(TransactionLogContainer) *pLog, CONST_EARRAY_OF(TransactionLogKeyValueChange) eaChanges, const char *pReason)
{
	eaCopyStructsDeConst(&eaChanges, &pLog->eaKeyValueChanges, parse_TransactionLogKeyValueChange);
	pLog->pKeyValueChangeReason = pReason ? StructAllocString(pReason) : NULL;
}

void AccountTransactionRecordKeyValueChangesNonTransacted(NOCONST(TransactionLogContainer) *pLog, CONST_EARRAY_OF(TransactionLogKeyValueChange) eaChanges, const char *pReason)
{
	AccountTransactionRecordKeyValueChanges(pLog, eaChanges, pReason);
}

AUTO_TRANSACTION
ATR_LOCKS(pLogContainer, ".eaTransactions[], .uNextLogID");
enumTransactionOutcome trAccountTransactionFinishNonTransacted(ATR_ARGS, NOCONST(AccountTransactionLogContainer) *pLogContainer, NON_CONTAINER const TransactionLogContainer *pLog, U32 uLogID)
{
	if (!eaIndexedPushUsingIntIfPossible(&pLogContainer->eaTransactions, uLogID, StructCloneDeConst(parse_TransactionLogContainer, pLog)))
...
void AccountTransactionFinishNonTransacted(NOCONST(TransactionLogContainer) **ppLog)
{
	TransactionReturnVal returnVal = {0};
	AccountTransactionLogContainer *pAccountLog = NULL;
	TransactionLogContainer *pConstLog = NULL;

	AutoTrans_trAccountTransactionFinishNonTransacted(&returnVal, GLOBALTYPE_ACCOUNTSERVER, GLOBALTYPE_ACCOUNTSERVER_TRANSACTIONLOG, (*ppLog)->uAccountID, CONTAINER_RECONST(TransactionLogContainer, *ppLog), (*ppLog)->uID);

	if (returnVal.eOutcome == TRANSACTION_OUTCOME_SUCCESS) // This works only because AS has local ObjectDB
	{
		pAccountLog = (AccountTransactionLogContainer*) objGetContainerData(GLOBALTYPE_ACCOUNTSERVER_TRANSACTIONLOG, (*ppLog)->uAccountID);
		pConstLog = (TransactionLogContainer*) eaIndexedGetUsingInt(&pAccountLog->eaTransactions, (*ppLog)->uID);
		AccountTransactionStashLog(pConstLog);
	}

	StructDestroyNoConstSafe(parse_TransactionLogContainer, ppLog);
}
...
void AccountTransactionGetKeyValueChanges(AccountInfo *pAccount, CONST_STRING_EARRAY eaKeys, TransactionLogKeyValueChange ***eaChanges)
{
	...
	EARRAY_CONST_FOREACH_BEGIN(eaKeys, iKey, iNumKeys);
	{
		...
		eResult = AccountKeyValue_Get("internal", pAccount, pKey, &pChange->iOldValue);
		if (eResult == AKV_SUCCESS)
			eResult = AccountKeyValue_GetLocked("internal", pAccount, pKey, &pChange->iNewValue);
...
SA_RET_OP_VALID TransactionLogContainer* AccountTransactionGetLogByPWOrderID(SA_PARAM_NN_STR const char *pPWOrderID)
{
	TransactionLogContainer *pLog = NULL;

	if (SAFE_DEREF(pPWOrderID))
	{
		stashFindPointer(stTransactionLogByPWOrderID, pPWOrderID, &pLog);
	}
