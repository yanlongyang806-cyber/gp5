...
static StashTable sAccountNameStash;
static StashTable sDisplayNameStash;
static StashTable sAccountEmailStash;
static StashTable sGUIDStash;
static StashTable sEmployeeStatusStash; // {uID->EmployeeStatus}, only has accounts with non-zero employee status
...
{
	sAccountNameStash = stashTableCreate(131072, StashDeepCopyKeys_NeverRelease, StashKeyTypeStrings, 0);
	sDisplayNameStash = stashTableCreate(131072, StashDeepCopyKeys_NeverRelease, StashKeyTypeStrings, 0);
	sAccountEmailStash = stashTableCreate(131072, StashDeepCopyKeys_NeverRelease, StashKeyTypeStrings, 0);
	sGUIDStash = stashTableCreate(131072, StashDeepCopyKeys_NeverRelease, StashKeyTypeStrings, 0);
	sEmployeeStatusStash = stashTableCreate(131072, StashDeepCopyKeys_NeverRelease, StashKeyTypeInts, SIZEOF2(AccountInfo, uID));
}

static void updateAccountToEmployeeStatusStash(AccountInfo *pAccount)
{
	if (pAccount->employeeStatus)
	{
		stashIntAddInt(sEmployeeStatusStash, (int) pAccount->uID, pAccount->employeeStatus, true);
	}
	else
	{
		stashIntRemoveInt(sEmployeeStatusStash, (int) pAccount->uID, NULL);
	}
}

static void addAccountToStashTables(AccountInfo *account)
{
	if (!stashAddInt(sAccountNameStash, account->accountName, account->uID, false))
	{
		ErrorOrProgrammerAlert("ACCOUNT_DUPLICATE_DETAILS", "Cannot add account '%s' to account name stash table, existing account name '%s'", account->accountName, account->accountName);
	}
	if (SAFE_DEREF(account->displayName))
	{
		if (!stashAddInt(sDisplayNameStash, account->displayName, account->uID, false))
		{
			ErrorOrProgrammerAlert("ACCOUNT_DUPLICATE_DETAILS", "Cannot add account '%s' to display name stash table, existing display name '%s'", account->accountName, account->displayName);
		}
	}
	if (SAFE_DEREF(account->personalInfo.email))
	{
		if (!stashAddInt(sAccountEmailStash, account->personalInfo.email, account->uID, false))
		{
			ErrorOrProgrammerAlert("ACCOUNT_DUPLICATE_DETAILS", "Cannot add account '%s' to email stash table, existing email '%s'", account->accountName, account->personalInfo.email);
		}
	}
	if (SAFE_DEREF(account->globallyUniqueID))
	{
		stashAddInt(sGUIDStash, account->globallyUniqueID, account->uID, false);
	}
	updateAccountToEmployeeStatusStash(account);
}
...
static void removeAccountFromStashTables(AccountInfo *account)
{
	...
	if (SAFE_DEREF(account->displayName))
	{
		stashRemoveInt(sDisplayNameStash, account->displayName, NULL);
	}
	if (SAFE_DEREF(account->personalInfo.email))
	{
	...
	stashIntRemoveInt(sEmployeeStatusStash, (int) account->uID, NULL);
}
...
SA_RET_OP_VALID Container * findAccountContainerByName(SA_PARAM_OP_STR const char * pAccountName)
{
	int containerID = 0;

	if (!SAFE_DEREF(pAccountName))
		return NULL;
	stashFindInt(sAccountNameStash, pAccountName, &containerID);
	if (containerID)
		return objGetContainer(GLOBALTYPE_ACCOUNT, containerID);
	return NULL;
}

SA_RET_OP_VALID Container * findAccountContainerByDisplayName(SA_PARAM_OP_STR const char * pDisplayName)
{
	int containerID = 0;

	if (!SAFE_DEREF(pDisplayName))
		return NULL;
	stashFindInt(sDisplayNameStash, pDisplayName, &containerID);
	if (containerID)
		return objGetContainer(GLOBALTYPE_ACCOUNT, containerID);
	return NULL;
}
...
bool isAccountNameUsed(const char *name)
{
...
bool isDisplayNameUsed(const char* pDisplayName)
{
	if (findAccountContainerByDisplayName(pDisplayName))
		return true;
	if (findPWCommonAccountByForumName(pDisplayName))
		return true;
	return false;
}

bool isDisplayNameAvailableToAccount(AccountInfo *pAccount, const char *pDisplayName)
{
	PWCommonAccount *pPWCommon;

	if (SAFE_DEREF(pAccount->displayName) && stricmp(pAccount->displayName, pDisplayName) == 0)
		return true;
	// check if the name exist in cryptic display name
	if (findAccountContainerByDisplayName(pDisplayName))
		return false;
	// if the display name is used by a PW forum name, it is only available if the cryptic account is linked to such PW account
	pPWCommon = findPWCommonAccountByForumName(pDisplayName);
	if (pPWCommon)
	{
		if (!(pAccount && pAccount->pPWAccountName))
		{
			return false;
		}
		return stricmp(pAccount->pPWAccountName, pPWCommon->pAccountName) == 0;
	}
	return true;
}
...
{
	Container *con = findAccountContainerByDisplayName(pDisplayName);
	if (con)
	{
		AccountInfo *account = (AccountInfo*) con->containerData;
...
// Get a list of employee status accounts -- DO NOT FREE THE CONTENTS; ONLY eaDestroy IT!
SA_RET_OP_VALID EARRAY_OF(AccountInfo) getEmployeeStatusAccountList()
{
	EARRAY_OF(AccountInfo) eaAccounts = NULL;
	AccountInfo *pAccount = NULL;
	StashTableIterator iter;
	StashElement element;
	stashGetIterator(sEmployeeStatusStash, &iter);
	while (stashGetNextElement(&iter, &element))
	{
		U32 uID = (U32) stashElementGetIntKey(element);
		pAccount = findAccountByID(uID);
		if (pAccount)
			eaPush(&eaAccounts, pAccount);
	}
	return eaAccounts;
}
...
void generateAccountGUID(ATH_ARG NOCONST(AccountInfo) *pAccount)
{
	...
	// If calling this function during creating an account, uID might still be 0.
	// Therefore need to add GUID to the stash table a second time in trCreateAccount_CB().
...
void trCreateAccount_CB(TransactionReturnVal *returnVal, TRCreateAccountData *trData)
{
	...
	if (returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		if (uID)
		{
			...
			// Although generateAccountGUID() already added GUID to the stash table,
			// the account->uID at that time was still 0.
			// Therefore need to add the GUID to the stash table a second time here.
...
AccountCreationResult createAutogenPWAccount(SA_PARAM_NN_STR const char *pAccountName, SA_PARAM_NN_STR const char *displayName)
{
	...
	if (findAccountByDisplayName(displayName))
					   ...
					   int iAccessLevel)
{
	...
	if (StringIsInvalidAccountName(pAccountName, iAccessLevel)
		...
		return false;
	if (isAccountNameUsed(pAccountName) || isDisplayNameUsed(displayName))
		return false;
...
bool validateAccountEmail(AccountInfo *account, const char *validateEmailKey)
{
	if (validateEmailKey && stricmp(validateEmailKey, account->validateEmailKey) == 0)
	{
		...
		accountClearFlagsNonTransacted(account, ACCOUNT_FLAG_NOT_ACTIVATED, &pDiffString);
...
bool changeAccountDisplayName(AccountInfo *account, const char *displayName, TransactionReturnCallback transCB, void *data)
{
	...
	if (!account || !displayName || !displayName[0])
		return false;
	if (stricmp(displayName, account->displayName) == 0)
		return true;
	...
	returnVal = objCreateManagedReturnVal(transCB, data);
    AutoTrans_trAccountChangeDisplayName(returnVal, objServerType(), GLOBALTYPE_ACCOUNT, account->uID, displayName);

	if (returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		static char *pDiffString = NULL;
		accountClearFlagsNonTransacted(account, ACCOUNT_FLAG_INVALID_DISPLAYNAME, &pDiffString);
		AccountNonTransactedChange(GLOBALTYPE_ACCOUNT, account->uID, pDiffString);
		estrClear(&pDiffString);
		return true;
	}
...
bool changeAccountName(AccountInfo *account, const char *accountName, TransactionReturnCallback transCB)
{
	...
	if (stricmp(accountName, account->accountName) == 0)
		return true;
	if (isAccountNameUsed(accountName))
		return false;
	...
	returnVal = objCreateManagedReturnVal(transCB, strdup(accountName));
    AutoTrans_trAccountChangeAccountName(returnVal, objServerType(), GLOBALTYPE_ACCOUNT, account->uID, accountName);
	return returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS;
}
...
bool changeAccountEmail(AccountInfo *account, const char *email, bool bGenerateActivator, TransactionReturnCallback transCB)
{
	...
	if (!transCB) transCB = trChangeName_CB;
	returnVal = objCreateManagedReturnVal(transCB, strdup(email));
	AutoTrans_trAccountChangeEmail(returnVal, objServerType(), GLOBALTYPE_ACCOUNT, account->uID, email, bGenerateActivator);
	...
	if (returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		if (bGenerateActivator)
		{
			accountSetFlagsNonTransacted(account, ACCOUNT_FLAG_NOT_ACTIVATED, &pDiffString);
		}
		else
		{
			accountClearFlagsNonTransacted(account, ACCOUNT_FLAG_NOT_ACTIVATED, &pDiffString);
	...
	verifyAccountEmployeeStatus(account);
	return true;
}

static void sendEmployeeStatusChangeEmail(
	SA_PARAM_NN_VALID AccountInfo *account,
	const EmployeeStatus prevStatus,
	const EmployeeStatus newStatus,
	SA_PARAM_OP_STR const char* szManipulator,
	SA_PARAM_OP_STR const char* szVia)
{
	SMTPMessageRequest *pReq = StructCreate(parse_SMTPMessageRequest);
	char *pResultEstr = NULL;
	AccountServerConfig *pConfig = GetAccountServerConfig();
	char *pMailto = pConfig->pEmployeeStatusChangeAlertMailto;

	if (!verify(account))
		return;
	if (!SAFE_DEREF(pMailto))
		return;

	if (SAFE_DEREF(pConfig->pEmployeeStatusChangeAlertServer))
	{
		estrCopy(&pReq->server, &pConfig->pEmployeeStatusChangeAlertServer);
	}

	pReq->timeout = pConfig->iEmployeeStatusChangeAlertTimeout;

	DivideString(pMailto, ",", &pReq->to, DIVIDESTRING_POSTPROCESS_ESTRINGS);

	estrPrintf(&pReq->from, "AccountManagement");

	estrPrintf(&pReq->subject, "Account Server EmployeeStatus changed for account: %s", account->accountName);

	estrPrintf(&pReq->body, "Cryptic Account Name: %s\n", account->accountName);
	estrConcatf(&pReq->body, "Employee Status: %s -> %s\n", StaticDefineIntRevLookup(EmployeeStatusEnum, prevStatus),
		StaticDefineIntRevLookup(EmployeeStatusEnum, newStatus));
	if (account->pPWAccountName)
	{
		estrConcatf(&pReq->body, "PW Email: %s\n", findPWCommonAccountByName(account->pPWAccountName)->pEmail);
	}
	estrConcatf(&pReq->body, "Cryptic Email: %s\n", account->personalInfo.email);
	estrConcatf(&pReq->body, "Manipulator: %s\n", SAFE_DEREF(szManipulator) ? szManipulator : "(none)");
	estrConcatf(&pReq->body, "Via: %s\n", SAFE_DEREF(szVia) ? szVia : "(none)");

	pReq->pResultCBFunc = GenericSendEmailResultCB; // generate a critical netops alert for failure
...
static void changeAccountEmployeeStatusInternal(
	SA_PARAM_NN_VALID AccountInfo *account,
	...
	SA_PARAM_OP_STR const char* szVia)
{
	if (!verify(account))
		return;
	if (account->employeeStatus == employeeStatus)
		return;
	accountLog(account, "Changed employee status from %s to %s (manipulator: %s, via: %s)",
		StaticDefineIntRevLookup(EmployeeStatusEnum, account->employeeStatus),
		StaticDefineIntRevLookup(EmployeeStatusEnum, employeeStatus),
		SAFE_DEREF(szManipulator) ? szManipulator : "(none)",
		SAFE_DEREF(szVia) ? szVia : "(none)"
	);
	SERVLOG_PAIRS(LOG_ACCOUNT_SERVER_GENERAL, "ChangeAccountEmployeeStatus",
		("account", "%s", account->accountName)
		("from", "%s", StaticDefineIntRevLookup(EmployeeStatusEnum, account->employeeStatus))
		("to", "%s", StaticDefineIntRevLookup(EmployeeStatusEnum, employeeStatus))
		("manipulator", "%s", SAFE_DEREF(szManipulator) ? szManipulator : "(none)")
		("via", "%s", SAFE_DEREF(szVia) ? szVia : "(none)")
	);
	sendEmployeeStatusChangeEmail(account, account->employeeStatus, employeeStatus, szManipulator, szVia);
	AutoTrans_trAccountSetEmployeeStatus(NULL, objServerType(), GLOBALTYPE_ACCOUNT, account->uID, employeeStatus);
}

bool changeAccountEmployeeStatus(AccountInfo *account, const EmployeeStatus employeeStatus, const char *szManipulator, const char *szVia)
{
	if (account)
	{
		if (account->employeeStatus != employeeStatus)
		{
			changeAccountEmployeeStatusInternal(account, employeeStatus, szManipulator, szVia);
			verifyAccountEmployeeStatus(account);
			updateAccountToEmployeeStatusStash(account);
...
static bool isValidEmployeeEmail(const char *szEmail)
{
	if (SAFE_DEREF(szEmail))
	{
		char *szDomain = strrchr(szEmail, '@'); // find the last "@"
		if (SAFE_DEREF(szDomain))
		{
			STRING_EARRAY eaDomains = GetAccountServerConfig()->eaEmployeeEmailDomains;
			szDomain += 1; // skip the first "@" character
			EARRAY_CONST_FOREACH_BEGIN(eaDomains, iCurDomain, iNumDomains);
			{
				char *szValidDomain = eaDomains[iCurDomain];
				if (SAFE_DEREF(szValidDomain))
				{
					if (szValidDomain[0] == '@')
						szValidDomain += 1;
					if (strcmpi(szValidDomain, szDomain) == 0)
						return true;
				}
			}
			EARRAY_FOREACH_END;
...
// if linked to a pw account, honor the pw account email setting; otherwise check the cryptic account email.
static bool hasValidEmployeeEmail(SA_PARAM_NN_VALID AccountInfo *pAccount)
{
	if (!verify(pAccount))
		return false;
	if (pAccount->pPWAccountName)
	{
		return isValidEmployeeEmail(findPWCommonAccountByName(pAccount->pPWAccountName)->pEmail);
	}
	else
	{
		return isValidEmployeeEmail(pAccount->personalInfo.email);
	}
}

// verify employee status for every email change and attempted employee status change
// should be called when changing emails and employeeStatus
void verifyAccountEmployeeStatus(SA_PARAM_NN_VALID AccountInfo *pAccount)
{
	if (!verify(pAccount))
		return;
	if (!GetAccountServerConfig()->bVerifyEmployeeStatus)
		return;
	if (pAccount->employeeStatus == EMPLOYEESTATUS_VALID && !hasValidEmployeeEmail(pAccount))
	{
		changeAccountEmployeeStatusInternal(pAccount, EMPLOYEESTATUS_INVALID_EMAIL_DOMAIN, NULL, NULL);
		ErrorOrCriticalAlert("EMPLOYEESTATUS_INVALID_EMAIL_DOMAIN",
			"Account id %d '%s' is using an invalid email for employee status, changing to %s",
			pAccount->uID,
			pAccount->accountName,
			StaticDefineIntRevLookup(EmployeeStatusEnum, pAccount->employeeStatus)
			);
	}
	else if (pAccount->employeeStatus == EMPLOYEESTATUS_INVALID_EMAIL_DOMAIN && hasValidEmployeeEmail(pAccount))
	{
		changeAccountEmployeeStatusInternal(pAccount, EMPLOYEESTATUS_VALID, NULL, NULL);
	}
}
...
AUTO_TRANSACTION
ATR_LOCKS(pAccount, ".Lastloginlocaleid, .Ulastloginlocaleidss2000");
enumTransactionOutcome trAccountUpdateLastLoginLocale(ATR_ARGS,
	NOCONST(AccountInfo) *pAccount, int localeID)
{
	pAccount->lastLoginLocaleID = localeID;
	pAccount->uLastLoginLocaleIDSS2000 = timeSecondsSince2000();
	return TRANSACTION_OUTCOME_SUCCESS;
}

void accountSetLastLoginLocaleID(SA_PARAM_NN_VALID AccountInfo * pAccount, LocaleID localeID)
{
	if (pAccount->lastLoginLocaleID != localeID)
	{
		AutoTrans_trAccountUpdateLastLoginLocale(NULL, objServerType(),
			GLOBALTYPE_ACCOUNT, pAccount->uID, localeID);
	}
}
...
int forceChangePasswordInternal(AccountInfo *account, const char *pNewPassword)
{
	if (account && strlen(pNewPassword) < MAX_PASSWORD_PLAINTEXT)
	{
		...
		accountLog(account, "Cryptic password changed");
		return 1;
	}
	return 0;
}
												 ...
												 AccountProxyProductActivation **ppOutActivation)
{
	...
	*ppOutActivation = StructCreateNoConst(parse_AccountProxyProductActivation);
	...
	EARRAY_CONST_FOREACH_BEGIN(pProduct->ppKeyValueChanges, iCurChange, iNumChanges);
	{
		...
		{
			if (AccountKeyValue_Change(pProxy, pAccount, pKey, change->pValue ? atoi64(change->pValue) : 0, &lock) != AKV_SUCCESS)
			{
				eResult = APR_KeyValueLockFailure;
				goto out;
			}
		}
		else if (AccountKeyValue_Set(pProxy, pAccount, pKey, change->pValue ? atoi(change->pValue) : 0, &lock) != AKV_SUCCESS)
		{
			eResult = APR_KeyValueLockFailure;
			goto out;
		}
	...
	if (keysUsedInRequirements)
	{
		...
		EARRAY_CONST_FOREACH_BEGIN(keysUsedInRequirements, iCurKey, iNumKeys);
		{
			...
			if (!pPair)
			{
				...
				if (AccountKeyValue_Lock(pProxy, pAccount, keyUsed, &lock) != AKV_SUCCESS)
				{
					eResult = APR_KeyValueLockFailure;
					goto out;
				}
	...
	if (eResult != APR_Success)
	{
		...
		if (*ppOutActivation)
		{
			EARRAY_CONST_FOREACH_BEGIN(pProduct->ppKeyValueChanges, iCurChange, iNumChanges);
			{
				...
				AccountKeyValue_Rollback(pProxy, pAccount, pLockPair->pKey, pLockPair->pLock);
			}
			EARRAY_FOREACH_END;
												   ...
												   bool bSuppressFailLog)
{
	...
	EARRAY_CONST_FOREACH_BEGIN(pActivation->ppKeyLocks, iCurLockPair, iNumLocks);
	{
		...
		if (AccountKeyValue_Commit(NULL, pActivation->pAccount, pKeyValueLockPair->pKey, pKeyValueLockPair->pLock) != AKV_SUCCESS)
		{
			eResult = APR_KeyValueCommitFailure;
		}
	}
													 ...
													 bool bSuppressFailLog)
{
	...
	EARRAY_CONST_FOREACH_BEGIN(pActivation->ppKeyLocks, iCurLockPair, s);
	{
		...
		if (AccountKeyValue_Rollback(NULL, pAccount, pKeyValueLockPair->pKey, pKeyValueLockPair->pLock) != AKV_SUCCESS)
		{
			eResult = APR_KeyValueRollbackFailure;
		}
	}
		...
		EARRAY_CONST_FOREACH_BEGIN(pAccount->pCachedSubscriptionList->ppList, iCurCachedSubOld, iNumCachedSubsOld);
		{
			...
			EARRAY_CONST_FOREACH_BEGIN(pNewList->ppList, iCurCachedSubNew, iNumCachedSubsNew);
			{
				...
				if (CachedSubsAreSame(pSubOld, pSubNew))
				{
					...
					// If an existing subscription has changed and is still active, update the corresponding stipend grant's billing day
					if (getCachedSubscriptionStatus(pSubNew) == SUBSCRIPTIONSTATUS_ACTIVE)
					{
						int iBillingDay = getCachedSubBillingDay(pSubNew);

						if (!iBillingDay)
						{
							accountLog(pAccount, "Could not update stipend for %s (VID: %s) because the subscription had no billing day!", pSubNew->internalName, pSubNew->vindiciaID);
						}
						else
						{
							updateStipendBillingDay(pAccount, pSubNew->internalName, iBillingDay, false);
						}
					}
				}
	...
	EARRAY_CONST_FOREACH_BEGIN(pNewList->ppList, iCurCachedSubNew, iNumCachedSubsNew);
	{
		...
		if (pAccount->pCachedSubscriptionList)
		{
			EARRAY_CONST_FOREACH_BEGIN(pAccount->pCachedSubscriptionList->ppList, iCurCachedSubOld, iNumCachedSubsOld);
			{
				...
				if (CachedSubsAreSame(pSubNew, pSubOld))
				{
					bFound = true;
				}
			}
		...
		if (!bFound)
		{
			// In case of a new sub, we have to queue an initial stipend
			int iBillingDay = getCachedSubBillingDay(pSubNew);

			if (!iBillingDay)
			{
				accountLog(pAccount, "Could not queue stipend for %s (VID: %s) because the subscription had no billing day!", pSubNew->internalName, pSubNew->vindiciaID);
				return;
			}

			scheduleInitialStipendGrant(pAccount, pSubNew->internalName, iBillingDay, false);
		}
	}
	EARRAY_FOREACH_END;
...
void accountUpdateCachedSubscription(SA_PARAM_NN_VALID AccountInfo *pAccount, SA_PARAM_NN_VALID NOCONST(CachedAccountSubscription) *pSubscription)
{
	...
	int iBillingDay = getCachedSubBillingDay(CONTAINER_RECONST(CachedAccountSubscription, pSubscription));
	...
	if (pCachedSub && CachedSubsAreSame(pCachedSub, (const CachedAccountSubscription *)pSubscription))
	{
		...
		// If an existing subscription has changed and is still active, update the corresponding stipend grant's billing day
		if (getCachedSubscriptionStatus(CONTAINER_RECONST(CachedAccountSubscription, pSubscription)) == SUBSCRIPTIONSTATUS_ACTIVE)
		{
			if (!iBillingDay)
			{
				accountLog(pAccount, "Could not update stipend for %s (VID: %s) because the subscription had no billing day!", pSubscription->internalName, pSubscription->vindiciaID);
			}
			else
			{
				updateStipendBillingDay(pAccount, pSubscription->internalName, iBillingDay, false);
			}
		}
	}
	else if (!pCachedSub)
	{
		if (!iBillingDay)
		{
			accountLog(pAccount, "Could not queue stipend for %s (VID: %s) because the subscription had no billing day!", pCachedSub->internalName, pCachedSub->vindiciaID);
			return;
		}

		// If a new subscription has appeared, queue up the initial stipend grant
		scheduleInitialStipendGrant(pAccount, pSubscription->internalName, iBillingDay, false);
	}
...
void accountFlagInvalidDisplayName(AccountInfo *account, bool bFlag)
{
	...
	if (bFlag)
	{
		accountSetFlagsNonTransacted(account, ACCOUNT_FLAG_INVALID_DISPLAYNAME, &pDiffString);
	}
	else
	{
		accountClearFlagsNonTransacted(account, ACCOUNT_FLAG_INVALID_DISPLAYNAME, &pDiffString);
...
void accountSetFlagsNonTransacted(const AccountInfo *pAccount, U32 uFlags, char **ppDiffString)
{
	U32 uNewFlags = pAccount->flags | uFlags;

	if (uNewFlags != pAccount->flags)
	{
		estrConcatf(ppDiffString, "set .Flags = \"%u\"\n", uNewFlags);
	}
}

void accountClearFlagsNonTransacted(const AccountInfo *pAccount, U32 uFlags, char **ppDiffString)
{
	U32 uNewFlags = pAccount->flags & ~uFlags;
...
void accountReplaceFlagsNonTransacted(const AccountInfo *pAccount, U32 uFlags, char **ppDiffString)
{
	if (uFlags != pAccount->flags)
	{
		estrConcatf(ppDiffString, "set .Flags = \"%u\"\n", uFlags);
	}
}
...
AUTO_TRANSACTION
ATR_LOCKS(account, ".Binternaluselogin, .Displayname, .Uid");
enumTransactionOutcome trAccountChangeDisplayName (ATR_ARGS, NOCONST(AccountInfo) *account, const char *displayName)
{
	...
	if (stricmp(account->displayName, displayName) == 0)
		return TRANSACTION_OUTCOME_SUCCESS;
	if (isDisplayNameUsed(displayName))
		return TRANSACTION_OUTCOME_FAILURE;
	...
	stashAddInt(sDisplayNameStash, account->displayName, account->uID, false);
	return TRANSACTION_OUTCOME_SUCCESS;
}

AUTO_TRANSACTION
ATR_LOCKS(account, ".Accountname, .Uid");
...
AUTO_TRANSACTION
ATR_LOCKS(account, ".Personalinfo.Email, .Uid, .Validateemailkey");
...
AUTO_TRANSACTION
ATR_LOCKS(account, ".EmployeeStatus");
enumTransactionOutcome trAccountSetEmployeeStatus(ATR_ARGS, NOCONST(AccountInfo) *account, int employeeStatus)
{
	account->employeeStatus = employeeStatus;
	return TRANSACTION_OUTCOME_SUCCESS;
}

AUTO_TRANSACTION
ATR_LOCKS(account, ".Validateemailkey");
...
static bool checkEmployeeStatus(AccountInfo *account, const ProductContainer *product)
{
	return (!GetAccountServerConfig()->bVerifyEmployeeStatus || !product->bRequireEmployee || account->employeeStatus == EMPLOYEESTATUS_VALID);
}
...
static void populateAccountPermsFromProduct(AccountInfo *account, const ProductContainer *product)
{
	...
	if(product && hasOneRequiredSubscription(account, product, REQSUBETC_NOW, NULL) && checkEmployeeStatus(account, product))
	{
...
void AccountManagement_DestroyContainers(void)
{
	...
	stashTableClear(sDisplayNameStash);
...
void accountSendEventEmail_CB(JSONRPCState * pState, void * pUserData)
{
	...
	if (pState->error)
	{
		if (pState->rawResponse)
		{
			ErrorOrCriticalAlert("ACCOUNTSERVER_WEBSRV_BAD_RESPONSE",
				"JSONRPC to WebSrv failed: %s\n\nResponse (%d):\n%s",
				pState->error, pState->responseCode, pState->rawResponse);
		}
		else
		{
			ErrorOrCriticalAlert("ACCOUNTSERVER_WEBSRV_NO_RESPONSE",
				"JSONRPC to WebSrv failed: %s",
				pState->error);
		}
	}
	...
	SA_PARAM_OP_VALID jsonrpcFinishedCB pCallback, SA_PARAM_OP_VALID void * pUserData)
{
	...
	if (!nullStr(pConfig->pWebSrvAddress) && pConfig->iWebSrvPort)
	{
		...
		pLocale = pLocale ? pLocale : accountGetIETFLanguageTag(pAccount);
