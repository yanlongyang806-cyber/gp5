...
static FORCEINLINE void btCompletedOncePerFrame(BillingTransaction *pTrans)
{
	U32 uTicks = 0;
	...
	if((!pTrans->steamTransaction || pTrans->steamTransactionResult != BTR_NONE) && (uTicks - pTrans->stateTime) > (gBillingConfiguration.completedMaxAgeSeconds * 1000))
	{
		// Time to die.
		btDestroy(pTrans);
	}
...
static FORCEINLINE void btActiveOncePerFrame(BillingTransaction *pTrans)
{
	DWORD now;
...
U32 billingGetGracePeriodDays(void)
{
	return gBillingConfiguration.gracePeriodDays;
}
...
bool billingInit()
{
	...
	stBillingTransactionUIDTable = stashTableCreateWithStringKeys(100, StashDeepCopyKeys_NeverRelease);
...
SA_RET_OP_VALID const SteamGameConfig *billingGetSteamGameConfig(SA_PARAM_NN_STR const char *pProduct)
{
	return eaIndexedGetUsingString(&gBillingConfiguration.steamGameConfigs, pProduct);
}

SA_RET_OP_VALID const ShardConfig *billingGetShardConfig(SA_PARAM_NN_STR const char *pProxy)
{
	return eaIndexedGetUsingString(&gBillingConfiguration.shardConfigs, pProxy);
}

SA_RET_OP_VALID const StipendConfig *billingGetStipendConfig(SA_PARAM_NN_STR const char *pInternalSubscription)
{
	return eaIndexedGetUsingString(&gBillingConfiguration.stipendConfigs, pInternalSubscription);
}
...
bool isCachedSubBilledThroughTime(SA_PARAM_NN_VALID const CachedAccountSubscription *pCachedSub, U32 uTime)
{
	struct tm endTime = {0};
	U32 uBilledThroughSS2000 = 0;
	U32 uEndTimeSS2000 = 0;

	uEndTimeSS2000 = pCachedSub->entitlementEndTimeSS2000;

	if (!uEndTimeSS2000)
	{
		uEndTimeSS2000 = pCachedSub->startTimeSS2000;
	}

	// Eliminate the hours, minutes, and seconds, so that if the sub bills on a given day, we don't grant that day unless the end day moves forward
	// We can't guarantee the end timestamp is at any particular time on a given day, so this is the safest thing to do
	timeMakeLocalTimeStructFromSecondsSince2000(uEndTimeSS2000, &endTime);
	endTime.tm_hour = 0;
	endTime.tm_min = 0;
	endTime.tm_sec = 0;

	if (!pCachedSub->gameCard && pCachedSub->vindiciaStatus == SUBSCRIPTIONSTATUS_ACTIVE)
	{
		// Subtract the grace period, then fix up month/day/year as needed
		endTime.tm_mday -= billingGetGracePeriodDays();
		...
		if (endTime.tm_mday < 1)
		{
			...
			endTime.tm_mday += timeDaysInMonth(endTime.tm_mon, endTime.tm_year + 1900);
		}
	}

	uBilledThroughSS2000 = timeGetSecondsSince2000FromLocalTimeStruct(&endTime);

	return uBilledThroughSS2000 >= uTime;
}

int getCachedSubBillingDay(SA_PARAM_NN_VALID const CachedAccountSubscription *pCachedSub)
{
	struct tm billingDay = {0};
	U32 uBillTimestamp = pCachedSub->nextBillingDateSS2000;

	if (!uBillTimestamp) uBillTimestamp = pCachedSub->startTimeSS2000;
	if (!uBillTimestamp) uBillTimestamp = pCachedSub->estimatedCreationTimeSS2000;
	if (!uBillTimestamp) return 0;

	timeMakeLocalTimeStructFromSecondsSince2000(uBillTimestamp, &billingDay);
	return billingDay.tm_mday;
}
