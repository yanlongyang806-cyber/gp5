...
AUTO_STRUCT;
typedef struct StipendConfig
{
	char *internalSubscription; AST(STRUCTPARAM KEY)
	U32 amount; AST(STRUCTPARAM)
	char *currency; AST(STRUCTPARAM)
} StipendConfig;
...
AUTO_STRUCT AST_IGNORE(fetchDeltaEntitlementPageSize);
typedef struct BillingConfiguration
{
	...
	// Number of days for grace period given by Vindicia for billing retries
	U32 gracePeriodDays;							AST(DEF(9))

	S32 entitlementBillingDateDelta;

	BillingNightlyMode nightlyMode;

	U32 fetchDeltaAutobillPageSize;					AST(DEF(40))
	...
	EARRAY_OF(StipendConfig) stipendConfigs; AST(NAME(Stipend))
	//------------------------------------------------------------------------------
