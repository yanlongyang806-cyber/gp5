...
enumTransactionOutcome trProductEdit(ATR_ARGS, ATR_ALLOW_FULL_LOCK NOCONST(ProductContainer) *product, ProductEditRequest *pEdit)
{
	...
	product->bRequireEmployee = pEdit->bRequireEmployee;
	...
	product->uStartDateSS2000 = pEdit->uStartDateSS2000;
	product->uEndDateSS2000 = pEdit->uEndDateSS2000;
				 ...
				 U32 uEndDateSS2000)
{
	...
	if (product)
	{
		...
		pReq->bRequireEmployee = bRequireEmployee;
		pReq->eTaxClassification = eTaxClassification;
		pReq->uExpireDays = uExpireDays;
		pReq->qwOfferID = qwOfferID;
		pReq->uStartDateSS2000 = uStartDateSS2000;
		pReq->uEndDateSS2000 = uEndDateSS2000;
	...
	else
	{
		...
		product->bRequireEmployee = bRequireEmployee;
		...
		product->uStartDateSS2000 = uStartDateSS2000;
		product->uEndDateSS2000 = uEndDateSS2000;
...
// return whether the product is in the range of start date and end date
bool productIsBetweenStartDateEndDate(SA_PARAM_NN_VALID const ProductContainer *pProduct)
{
	U32 timeSS2000 = timeSecondsSince2000();
	if (!verify(pProduct)) return false;
	if (pProduct->uStartDateSS2000 && timeSS2000 < pProduct->uStartDateSS2000) return false;
	if (pProduct->uEndDateSS2000 && timeSS2000 >= pProduct->uEndDateSS2000) return false;
	return true;
}
