...
// Determine if an internal sub is "billed" (i.e. active) through a given time
bool isInternalSubBilledThroughTime(SA_PARAM_NN_VALID const InternalSubscription *pSub, U32 uTime);

// Get the "anniversary day" of the month for an internal sub
int getInternalSubAnniversaryDay(SA_PARAM_NN_VALID const InternalSubscription *pSub);
