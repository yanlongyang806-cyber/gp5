...
#define ACCOUNT_SERVER_VERSION "(Production)"
...
typedef struct AccountLogBatch
{
	...
	CONST_EARRAY_OF(AccountLogEntry) eaIndexedLogEntries; AST(NO_TRANSACT)
} AccountLogBatch;
...
AUTO_STRUCT AST_CONTAINER AST_IGNORE_STRUCT(billingAddress) AST_IGNORE_STRUCT(shippingAddress) AST_IGNORE(dob);
...
{
	U32 uIP;		AST(NO_TRANSACT) // last login IP
	U32 uTime;		AST(NO_TRANSACT) // last login time
} AccountLoginData;
...
{
	EMPLOYEESTATUS_NOT_EMPLOYEE = 0,
	EMPLOYEESTATUS_INVALID_EMAIL_DOMAIN,
	EMPLOYEESTATUS_VALID,
} EmployeeStatus;
...
// Information about recent stipend grants
AUTO_STRUCT AST_CONTAINER;
typedef struct RecentStipendGrant
{
	CONST_STRING_MODIFIABLE pInternalSubName; AST(ESTRING KEY)
	CONST_STRING_MODIFIABLE pCurrency; AST(ESTRING)
	const U32 uAmount;
	const U32 uGrantedSS2000;
} RecentStipendGrant;

typedef struct ScheduledStipendGrant ScheduledStipendGrant;

AUTO_STRUCT AST_CONTAINER
AST_IGNORE(uMasterAccountID) AST_IGNORE(uLogBatchID) AST_IGNORE(xuid)
AST_IGNORE(uNextLogContext) AST_IGNORE(uLogBufferWriteTime) AST_IGNORE(uLastLogID)
AST_IGNORE(uSaveNextMachineExpire)
...
typedef struct AccountInfo
{
	...
	CONST_EARRAY_OF(AccountLogEntry) ppIndexedLogBuffer;	AST(NO_TRANSACT)
	...
	const U32 uNextLogID;									AST(NO_TRANSACT)
	...
	CONST_EARRAY_OF(RecentStipendGrant) eaRecentStipends;
	EARRAY_OF(ScheduledStipendGrant) eaScheduledStipends; NO_AST
	...
	AccountLoginData loginData; AST(ADDNAMES(lastLogin) NO_TRANSACT)
	...
	const U32 flags;							AST(NO_TRANSACT) // ACCOUNT_FLAG_*
	...
	CONST_STRING_MODIFIABLE defaultLocale; AST(ESTRING) // use accountGetIETFLanguageTag
	...
	const LocaleID lastLoginLocaleID; AST(INT DEFAULT(LOCALE_ID_INVALID))	// Last client Cryptic locale ID
	const U32 uLastLoginLocaleIDSS2000;										// Time when last locale was last set

	CONST_EARRAY_OF(SavedMachine) eaSavedClients;		AST(NO_TRANSACT)
	CONST_EARRAY_OF(SavedMachine) eaSavedBrowsers;		AST(NO_TRANSACT)
	const AccountMachineLockState eMachineLockState;	AST(NO_TRANSACT)
	const U32 uSaveNextClientExpire;					AST(NO_TRANSACT)
	const U32 uSaveNextBrowserExpire;					AST(NO_TRANSACT)
...
// Gets the appropriate locale for the account
SA_RET_NN_STR const char *accountGetIETFLanguageTag(SA_PARAM_NN_VALID const AccountInfo *account);
...
typedef struct AccountLoginAttemptData
{
	...
	LocaleID localeID; AST(INT)					// Client Cryptic locale ID (or LOCALE_ID_INVALID)
...
// Process a database update string. Only intended for direct use in limited circumstances; generally,
// you should just let the transaction flow dispatch this appropriately.
void accountdbHandleDatabaseUpdateStringEx(const char *cmd_orig, U64 sequence, U32 timestamp, bool replay);

// Performs a non-transacted update of a field on an account. Only intended for use when you have a
// very well-defined change to apply to a specific field (or fields). Use with caution!
void AccountNonTransactedChange(GlobalType eType, ContainerID conID, SA_PARAM_OP_STR const char *pDiffString);
void AccountNonTransactedChangef(GlobalType eType, ContainerID conID, SA_PARAM_NN_STR FORMAT_STR const char *pFormat, ...);
