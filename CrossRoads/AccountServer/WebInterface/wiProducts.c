...
SA_RET_OP_VALID static ASWIImportExportSession * wiGetImportExportSession(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
...
static bool wiProductSetToExport(SA_PARAM_NN_VALID WICWebRequest *pWebRequest, U32 uProductID)
{
	...
	if (pSession && eaiFind(&pSession->eaProductIDs, uProductID) > -1)
	{
		bFound = true;
	}
...
static bool wiAddProductToExport(SA_PARAM_NN_VALID WICWebRequest *pWebRequest, U32 uProductID)
{
	ASWIImportExportSession * pSession = NULL;
	bool bSuccess = false;
...
static void wiRemoveProductFromExport(SA_PARAM_NN_VALID WICWebRequest *pWebRequest, U32 uProductID)
{
	ASWIImportExportSession * pSession = NULL;
...
SA_RET_OP_VALID static INT_EARRAY wiGetProductExportList(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
	ASWIImportExportSession * pSession = NULL;
...
static void wiClearProductsFromExport(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
	ASWIImportExportSession * pSession = NULL;
...
static void wiAddProductToList(SA_PARAM_NN_VALID WICWebRequest *pWebRequest,
...
static void wiHandleProductsIndex(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
	if (!verify(pWebRequest)) return;
...
static void wiHandleProductsView(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
	if (!verify(pWebRequest)) return;
...
static void wiHandleProductsList(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
	...
	if (wiSubmitted(pWebRequest, "export"))
	{
		...
		if (pProduct)
		{
			if (wiAddProductToExport(pWebRequest, pProduct->uID))
			{
				wiAppendMessageBox(pWebRequest, "Success", STACK_SPRINTF("%s added to export list.", pProduct->pName), 0);
			}
			else
			{
				wiAppendMessageBox(pWebRequest, "Failure", STACK_SPRINTF("%s could not be added.", pProduct->pName), WMBF_Error);
			}
		}
		else
		{
...
static void wiHandleProductsCreate(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
	if (!verify(pWebRequest)) return;
...
static void wiHandleProductsExport(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
...
static void wiHandleProductsImport(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
...
bool wiHandleProducts(SA_PARAM_NN_VALID WICWebRequest *pWebRequest)
{
	bool bHandled = false;
