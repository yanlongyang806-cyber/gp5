...
void wiRedirectToAccount(NetLink *pLink, const char *pAccountName)
{
	char *pFullURL = NULL;
	char *pEscapedAccountName = NULL;
	urlEscape(pAccountName, &pEscapedAccountName, true, false);
	estrPrintf(&pFullURL, "detail?accountname=%s", pEscapedAccountName);
	httpRedirect(pLink, pFullURL);
	estrDestroy(&pEscapedAccountName);
	estrDestroy(&pFullURL);
}

static void wiConcatAccountLink(char **estr, const char *pAccountName)
{
	char *pEscapedAccountName = NULL;
	urlEscape(pAccountName, &pEscapedAccountName, true, false);
	estrConcatf(estr, "<a href=\"detail?accountname=%s\">%s</a>", pEscapedAccountName, pAccountName);
	estrDestroy(&pEscapedAccountName);
}
...
static void wiCreateAccount(DEFAULT_HTTPPOST_PARAMS)
{
	...
	if (isAccountNameUsed(pAccountName))
	{
...
static void wiChangeAddress(DEFAULT_HTTPPOST_PARAMS)
{
	...
	wiRedirectToAccount(link, pAccount->accountName);
}
...
{
	wiRedirectToAccount(holder->pLink, holder->pAccount->accountName);
...
static void wiAppendAccountInfo_KeyValues(char **estr, AccountInfo *pAccount)
{
	...
	appendFancyTableHeader(estr, 5, "Key", "Value", "Unlock", "Commit", "Remove");
	EARRAY_CONST_FOREACH_BEGIN(eaKeys, iCurKey, iNumKeys);
	{
		char buffer[256];
		const char *pKey = eaKeys[iCurKey];
		bool locked = AccountKeyValue_IsLocked("internal", pAccount, pKey);
		S64 iValue = 0;

		if (AccountKeyValue_Get("internal", pAccount, pKey, &iValue) != AKV_SUCCESS)
		{
			continue;
		}
		...
		{
			S64 iLockedValue = 0;
			AccountKeyValue_GetEx("internal", pAccount, pKey, true, &iLockedValue);
			...
			estrConcatf(estr, "<td class=\"summary%std\">", (bOdd)?"even":"odd");
			sprintf(buffer, "commitKeyValuePair_%s", pKey);
			wiFormAppendStart(estr, "/accountCommitKeyValuePair", "POST", buffer, NULL);
			formAppendHiddenInt(estr, "id", pAccount->uID);
			formAppendHidden(estr, "key", pKey);
			formAppendSubmit(estr, "Commit");
			estrConcatf(estr, "new value = %lld", iLockedValue);
			formAppendEnd(estr);
			estrConcatf(estr, "</td>\n");
		}
		else
...
static void wiAppendAccountInfo(char **estr, AccountInfo *pAccount, int iAccountErrors)
{
	...
	if (pAccount)
	{
		...
		urlEscape(pAccount->accountName, &pEscapedAccountName, true, false);
	...
	estrConcatf(estr, "<div class=\"heading\">Employee Status</div>\n%d", pAccount->employeeStatus);
	...
	HTML_HEADING(estr, "Subscription Stipends");
	if (eaSize(&pAccount->eaScheduledStipends) == 0 && eaSize(&pAccount->eaRecentStipends) == 0)
	{
		estrConcatf(estr, "No stipend activity has been recorded for this account.");
	}
	else
	{
		int iRecentCount = eaSize(&pAccount->eaRecentStipends);

		appendFancyTableHeader(estr, 3, "Internal Subscription Name", "Last Grant", "Next Grant");

		EARRAY_FOREACH_BEGIN(pAccount->eaRecentStipends, iRecentStipend);
		{
			RecentStipendGrant *pRecentGrant = pAccount->eaRecentStipends[iRecentStipend];
			ScheduledStipendGrant *pScheduledGrant = findScheduledStipendForAccount(pAccount, pRecentGrant->pInternalSubName);
			char lastGrantDate[512] = {0};
			char nextGrantDate[512] = {0};

			timeMakeLocalDateStringFromSecondsSince2000(lastGrantDate, pRecentGrant->uGrantedSS2000);

			if (pScheduledGrant)
			{
				timeMakeLocalDateStringFromSecondsSince2000(nextGrantDate, pScheduledGrant->uGrantTime);
			}

			appendFancyTableEntry(estr, iRecentStipend%2, 3, pRecentGrant->pInternalSubName, lastGrantDate, nextGrantDate[0] ? nextGrantDate : "None scheduled");
		}
		EARRAY_FOREACH_END;

		EARRAY_FOREACH_BEGIN(pAccount->eaScheduledStipends, iScheduledStipend);
		{
			ScheduledStipendGrant *pScheduledGrant = pAccount->eaScheduledStipends[iScheduledStipend];
			RecentStipendGrant *pRecentGrant = findRecentStipendForAccount(pAccount, pScheduledGrant->pInternalSubName);
			char nextGrantDate[512] = {0};
			...
			timeMakeLocalDateStringFromSecondsSince2000(nextGrantDate, pScheduledGrant->uGrantTime);
			appendFancyTableEntry(estr, (iRecentCount+iScheduledStipend)%2, 3, pScheduledGrant->pInternalSubName, "None granted", nextGrantDate);
		}
		EARRAY_FOREACH_END;
	...
	if (pAccount)
	{
		...
		estrConcatf(estr, "[<a href=\"refreshSubscriptionCache?name=%s\">Refresh</a>] (Last Updated: %s)", 
			pEscapedAccountName, lastUpdatedStr);
		estrDestroy(&pEscapedAccountName);
	...
	{
		char *pEscapedAccountName = NULL;
		urlEscape(pAccount ? pAccount->accountName : "", &pEscapedAccountName, true, false);
	...
	HTML_HEADING(estr, "Locale");
	estrConcatf(estr, "Last log-in locale: %s</br>",
		locIDIsValid(pAccount->lastLoginLocaleID) ? locGetIETFLanguageTag(pAccount->lastLoginLocaleID) : "(unset)");
	estrConcatf(estr, "Drupal locale: %s</br>", pAccount->defaultLocale ? pAccount->defaultLocale : "(unset)");
	estrConcatf(estr, "Effective locale: %s</br>", accountGetIETFLanguageTag(pAccount));
}
...
static void wiRefreshPermissionCache(DEFAULT_HTTPGET_PARAMS)
{
	...
	if(pAccountName)
	{
		wiRedirectToAccount(link, pAccountName);
	}
	else
	{
		wiRedirectf(link, "detail");
							   ...
							   SA_PARAM_NN_VALID const SubscriptionHistoryEntry *pEntry)
{
	...
	wiConcatAccountLink(estr, pAccount->accountName);
...
#define STATS_TABLE_LINE_SUB_S2000(SUBSTRUCT, VARIABLE, DESCRIPTION) do {char strTime[512];													\
	...
	STATS_TABLE_LINE_EXPR(loginQueueSize, "Current log-in queue size", "%d", loginQueueSize());																			\
...
{
	char *pEscapedAccountName = NULL;
	urlEscape(account->accountName, &pEscapedAccountName, true, false);
	appendFancyTableEntry(estr, bOddRow, 6,
		STACK_SPRINTF("%d", account->uID),
		STACK_SPRINTF("<a href=\"detail?accountname=%s\">%s</a>", pEscapedAccountName, account->accountName),
		account->displayName,
	...
	if (keyBatch)
	{
		...
		for (i=0; i<size; i++)
		{
			...
			if (productKeyIsDistributed(eaKeys[i]))
			{
				...
				if (pAccount)
				{
					estrAppend2(&pExtraInfo, "(distributed to ");
					wiConcatAccountLink(&pExtraInfo, pAccount->accountName);
					estrAppend2(&pExtraInfo, ") ");
				}
			}
...
static void wiKeyView(DEFAULT_HTTPGET_PARAMS)
{
	...
	if (account && account->accountName)
		wiConcatAccountLink(&s, account->accountName);
	else
...
static void appendProductKeyBatchCreationForm(char **estr, const char *prefix)
{
	...
	if (isAccountServerMode(ASM_KeyGenerating))
	{
		...
		estrConcatf(estr, "(a list of valid special access tokens</td></tr>\n");
...
static void wiProductKeyActivate(DEFAULT_HTTPPOST_PARAMS)
{
	...
	if (uAccountID && account && pKey)
	{
		...
		{
		case PK_Success:
			estrAppend2(&s, "Successfully activated the key for ");
			wiConcatAccountLink(&s, account->accountName);
			estrAppend2(&s, "!");
...
static void wiProductKeyDistribute(DEFAULT_HTTPPOST_PARAMS)
{
	...
	if (account && pKey)
	{
		...
		case PK_Success:
			...
			estrAppend2(&s, "Successfully distributed the key to ");
			wiConcatAccountLink(&s, account->accountName);
			estrAppend2(&s, "!");
		xcase PK_PrefixInvalid:
			estrConcatf(&s, "Invalid prefix for key");
...
static void wiAppendProductForm(SA_PARAM_NN_VALID char **s, SA_PARAM_OP_VALID const ProductContainer *product)
{
	...
	HTML_HEADING(s, "Require Employee");
	HTML_DESCRIPTION(s, "Whether or not require the EmployeeStatus to be valid to have permission.");
	formAppendCheckBox(s, "requireEmployee", product ? product->bRequireEmployee : false);
	...
	HTML_HEADING(s, "Start Date");
	HTML_DESCRIPTION(s, "If not empty, this is the date the product will become available. The format is 'yyyy-mm-dd hh-mm-ss', e.g. '2000-01-01 00:00:00'");
	timeSS2000 = product ? product->uStartDateSS2000 : 0;
	formAppendEdit(s, 19, "startDate", timeSS2000 ? timeMakeLocalDateStringFromSecondsSince2000_s(NULL, 20, timeSS2000) : "");
	
	HTML_HEADING(s, "End Date");
	HTML_DESCRIPTION(s, "If not empty, this is the date the product will become unavailable. The format is 'yyyy-mm-dd hh-mm-ss', e.g. '2001-12-31 17:29:59'");
	timeSS2000 = product ? product->uEndDateSS2000 : 0;
	formAppendEdit(s, 19, "endDate", timeSS2000 ? timeMakeLocalDateStringFromSecondsSince2000_s(NULL, 20, timeSS2000) : "");
}
...
static void wiProductEdit(DEFAULT_HTTPPOST_PARAMS)
{
	...
	bool requireEmployee = atoi(urlFindSafeValue(args, "requireEmployee"));
	...
	const char *startDateString = urlFindValue(args, "startDate");
	const char *endDateString = urlFindValue(args, "endDate");
	U32 uStartDateSS2000 = 0;
	U32 uEndDateSS2000 = 0;
	...
	{
		uStartDateSS2000 = timeGetSecondsSince2000FromLocalDateString(startDateString);
	}
	if (endDateString && *endDateString)
	{
		uEndDateSS2000 = timeGetSecondsSince2000FromLocalDateString(endDateString);
	}

	if (productSave(name,
		internalName, description, billingstatementidentifier,
		shardString, permissionString, reqsubsString,
		accessLevel, requireEmployee, categoriesString,
...
static void wiProductCreate(DEFAULT_HTTPPOST_PARAMS)
{
	...
	if (name && internalName && *name && *internalName)
	{
		...
		if (productSave(name,
			...
			uStartDateSS2000, uEndDateSS2000))
		{
...
static void wiAccountAddProduct(DEFAULT_HTTPPOST_PARAMS)
{
	...
	wiRedirectToAccount(link, account->accountName);
...
static void wiAccountAddKeyValuePair(DEFAULT_HTTPPOST_PARAMS)
{
	...
	if (account)
	{
		...
		result = AccountKeyValue_Set("internal", account, key, iValue, &lock);
		if (result == AKV_SUCCESS)
			result = AccountKeyValue_Commit("internal", account, key, lock);
		...
		else
		{
			...
			switch (result)
			{
				case AKV_INVALID_KEY:
					estrAppend2(&s, "Invalid key for ");
					wiConcatAccountLink(&s, account->accountName);
					estrAppend2(&s, ".");
					break;
				case AKV_INVALID_RANGE:
					estrConcatf(&s, "Invalid range for the key %s for ", key);
					wiConcatAccountLink(&s, account->accountName);
					estrAppend2(&s, ".");
					break;
				case AKV_LOCKED:
					estrAppend2(&s, "Key/value pair for ");
					wiConcatAccountLink(&s, account->accountName);
					estrAppend2(&s, " is locked.");
					break;
				default:
				case AKV_FAILURE:
					estrAppend2(&s, "Could not add key/value pair for ");
					wiConcatAccountLink(&s, account->accountName);
					estrAppend2(&s, ".");
					break;
			}
...
static void wiAccountRemoveKeyValuePair(DEFAULT_HTTPPOST_PARAMS)
{
	...
	if (account)
	{
		AccountKeyValue_Remove("internal", account, key);
...
static void wiAccountUnlockKeyValuePair(DEFAULT_HTTPPOST_PARAMS)
{
	...
	if (account)
	{
		AccountKeyValue_Unlock("internal", account, key);
...
static void wiAccountCommitKeyValuePair(DEFAULT_HTTPPOST_PARAMS)
{
	U32 uID = atoi(urlFindSafeValue(args, "id"));
	const char *key = urlFindValue(args, "key");
	AccountInfo *account = findAccountByID(uID);
	...
	if (account)
	{
		AccountKeyValue_ForceCommit("internal", account, key);
		log_printf(LOG_ACCOUNT_SERVER_GENERAL, "Web: commitKeyValueFromAccount(%s, %s), %s, %s", 
			account->accountName, key,  
			loggedIn ? loggedIn : "Unknown User", "Success");
...
void httpLegacyHandlePost(NetLink *link, HttpClientStateDefault *pClientState)
{
	...
	// Record this request.
	log_printf(LOG_ACCOUNT_SERVER_WEB, "HTTP POST: URL: \"%s\", Referrer: \"%s\", Auth: \"%s\", Body: \"%s\"",
		NULL_TO_EMPTY(url),
		NULL_TO_EMPTY(pReferer),
		NULL_TO_EMPTY(httpFindAuthenticationUsername(pClientState)),
	...
	else if (strEndsWith(url, "/accountCommitKeyValuePair"))
	{
		wiAccountCommitKeyValuePair(link, pClientState, arglist, pReferer, pCookies);
	}
...
void httpLegacyHandleGet(char *data, NetLink *link, HttpClientStateDefault *pClientState)
{
	...
	// Record this request.
	log_printf(LOG_ACCOUNT_SERVER_WEB, "HTTP GET: URL: \"%s\", Referrer: \"%s\", Auth: \"%s\"",
		NULL_TO_EMPTY(url_esc),
		NULL_TO_EMPTY(pReferer),
		NULL_TO_EMPTY(httpFindAuthenticationUsername(pClientState)));
