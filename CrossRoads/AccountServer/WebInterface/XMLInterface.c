...
AUTO_STRUCT;
typedef struct XMLAccountProduct
{
	NameIDPair name; AST(EMBEDDED_FLAT)
	U32 uAssociatedTimeSS2000;
} XMLAccountProduct;
...
typedef struct XMLAccountResponse
{
	...
	U32 uSaveNextClientExpire;
	U32 uSaveNextBrowserExpire;
	...
	EARRAY_OF(XMLAccountProduct) ppProducts;			AST(NAME(products))
...
static void XMLRPCAddProductsOwned(SA_PARAM_NN_VALID AccountInfo *pAccount, SA_PARAM_NN_VALID XMLAccountResponse *pXML)
{
	...
	EARRAY_CONST_FOREACH_BEGIN(pAccount->ppProducts, i, s);
		...
		if (pProduct)
		{
			XMLAccountProduct *pPair = StructCreate(parse_XMLAccountProduct);

			estrCopy2(&pPair->name.pName, pProduct->pName);
			pPair->name.uID = pProduct->uID;
			pPair->uAssociatedTimeSS2000 = pAccount->ppProducts[i]->uAssociatedTimeSS2000;
...
static void XMLRPCFillFromAccount(AccountInfo *account, XMLAccountResponse *xmlResponse)
{
	...
	xmlResponse->defaultLocale = strdup(accountGetIETFLanguageTag(account));
...
static XMLAccountResponse * XMLRPCGetUserInfo(AccountInfo *account, int iFlags)
{
	...
	if (account)
	{
		...
		if (iFlags & USERINFO_SAVEDMACHINES)
		{
			...
			xmlResponse->uSaveNextMachineExpire = account->uSaveNextClientExpire;
			xmlResponse->uSaveNextClientExpire = account->uSaveNextClientExpire;
			xmlResponse->uSaveNextBrowserExpire = account->uSaveNextBrowserExpire;
...
typedef struct XMLSetKeyValueExRequest
{
	...
	const char *pSource;		AST(NAME(Source) DEFAULT("xmlrpc"))
...
SA_RET_NN_VALID XMLSetKeyValueResponse * XMLRPCSetKeyValueEx(SA_PARAM_NN_VALID const XMLSetKeyValueExRequest *pRequest)
{
	...
	else
	{
		...
		{
			result = AccountKeyValue_Change(pRequest->pSource, pAccount, pRequest->pKey, pRequest->pValue ? atoi64(pRequest->pValue) : 0, &lock);
		}
		else
		{
			result = AccountKeyValue_Set(pRequest->pSource, pAccount, pRequest->pKey, pRequest->pValue ? atoi64(pRequest->pValue) : 0, &lock);
		}
		...
		if (result == AKV_SUCCESS)
		{
			...
			result = AccountKeyValue_Commit(pRequest->pSource, pAccount, pRequest->pKey, lock);
		}
		...
		case AKV_SUCCESS:
			...
			{
				NOCONST(TransactionLogContainer) *pLog = AccountTransactionOpenNonTransacted(pAccount->uID, pRequest->eType, NULL, TPROVIDER_AccountServer, NULL, NULL, NULL);
				AccountTransactionRecordKeyValueChangesNonTransacted(pLog, eaChanges, pRequest->pReason);
...
XMLGetKeyValueResponse * XMLRPCGetKeyValueList(const char *accountName)
{
	...
	else
	{
		...
		EARRAY_CONST_FOREACH_BEGIN(eaKeys, iCurKey, iNumKeys);
		{
			...
			if (AccountKeyValue_Get("xmlrpc", account, key, &iValue) == AKV_SUCCESS)
			{
...
typedef struct XMLGetUserInfoRequest
{
	char *accountName;
	char *email;
	U32 uCrypticID; AST(NAME(CrypticID))
	char *guid;
	int iFlags;
...
XMLAccountResponse * XMLRPCUserInfoEx(XMLGetUserInfoRequest *request)
{
	...
	if (!(request->accountName && request->accountName[0]) &&
		!(request->email && request->email[0]) &&
		!(request->guid && request->guid[0]) &&
		!(request->uCrypticID && request->eType != ACCOUNTLOGINTYPE_PerfectWorld))
	{
	...
	else // eType == ACCOUNTLOGINTYPE_Default || eType == ACCOUNTLOGINTYPE_Cryptic
	{
		...
		else
		{
			AccountInfo *pAccount =
				request->uCrypticID ? findAccountByID(request->uCrypticID) :
				request->email		? findAccountByEmail(request->email) :
				request->guid		? findAccountByGUID(request->guid) :
									  NULL;
...
XMLAccountResponse * XMLRPCCreateNewAccount(SA_PARAM_NN_VALID const XMLRPCCreateAccountRequest *pRequest)
{
	...
	if (pRequest->accountName && pRequest->passwordHash &&  pRequest->email && pRequest->accountName[0] && pRequest->passwordHash[0] && pRequest->email[0]) 
	{
		...
		{
			if (isAccountNameUsed(pRequest->accountName))
				bFailed = true;

			if (isDisplayNameUsed(pRequest->displayName))
			{
				if (bFailed)
...
XMLAccountResponse * XMLRPCUpdateUser(XMLRPCUpdateUserRequest *pRequest)
{
	...
	if (failureCode == LoginFailureCode_Ok || (pRequest->bIsAdmin && account && !account->bInternalUseLogin && !account->bLoginDisabled))
	{
		...
		if (pRequest->displayName &&
			...
			!isDisplayNameAvailableToAccount(account, pRequest->displayName))
		{
			strcpy(xmlResponse->userStatus, ACCOUNT_HTTP_DISPLAYNAME_EXISTS);
			return xmlResponse;
		}
		...
		if (pRequest->employeeStatus)
		{
			changeAccountEmployeeStatus(account, pRequest->employeeStatus, NULL, __FUNCTION__);
		}
		if (pRequest->displayName)
		{
	...
	SA_PARAM_NN_VALID XMLRPCValidateLoginExRequest *pRequest)
{
	...
	*pSlowReturnInfo = pContext->slowReturnInfo;
	...
	loginData.localeID = LOCALE_ID_INVALID;
...
typedef struct XMLRPCMachineLockingSaveNextRequest
{
	const char *accountName;	// Private account name
	bool enable;
	MachineType machineType; AST(DEFAULT(MachineType_CrypticClient))
...
XMLRPCMachineLockingSaveNextResponse *XMLRPCMachineLockingSaveNext(XMLRPCMachineLockingSaveNextRequest *pRequest)
{
	XMLRPCMachineLockingSaveNextResponse *xmlResponse = StructCreate(parse_XMLRPCMachineLockingSaveNextResponse);
	AccountInfo *pAccount = findAccountByName(pRequest->accountName);

	LogXmlrpcf("Request: MachineLockingSaveNext(%s, %d, %s)",
		pRequest->accountName,
		pRequest->enable,
		StaticDefineIntRevLookupNonNull(MachineTypeEnum, pRequest->machineType));
	...
	else
	{
		...
		{
			accountMachineLockingSaveNext(pAccount, pRequest->enable, pRequest->machineType, false);
			xmlResponse->result = ACCOUNT_HTTP_SUCCESS;
		}
		else
...
typedef struct XMLRPCMoveKeyValueRequest
{
	...
	char *pDestination;
	char *pReason;
	char *pRequestSource; AST(DEFAULT("xmlrpc"))
	TransactionLogType eType;	AST(NAME(Type))
...
SA_RET_NN_VALID XMLRPCMoveKeyValueResponse *XMLRPCMoveKeyValue(XMLRPCMoveKeyValueRequest* pRequest)
{
	...
	if (pAccount)
	{
		...
		eResult = AccountKeyValue_Get(pRequest->pRequestSource, pAccount, pRequest->pSource, &amount);
		...
		{
			eResult = AccountKeyValue_Move(pRequest->pRequestSource, pAccount, pRequest->pSource, pAccount, pRequest->pDestination, amount, &pLock);
		}

		if (eResult == AKV_SUCCESS)
		{
			CurrencyPopulateKeyList(pRequest->pSource, &eaKeys);
			CurrencyPopulateKeyList(pRequest->pDestination, &eaKeys);
			AccountTransactionGetKeyValueChanges(pAccount, eaKeys, &eaChanges);
			eaDestroyEx(&eaKeys, NULL);
		}
		...
		if (eResult == AKV_SUCCESS)
		{
			eResult = AccountKeyValue_Commit(pRequest->pRequestSource, pAccount, pRequest->pSource, pLock);
		}
		else
		{
			if (pLock) AccountKeyValue_Rollback(pRequest->pRequestSource, pAccount, pRequest->pSource, pLock);
		}
		...
		case AKV_SUCCESS:
			if (eaSize(&eaChanges) > 0)
			{
				NOCONST(TransactionLogContainer) *pLog = AccountTransactionOpenNonTransacted(pAccount->uID, pRequest->eType, pRequest->pRequestSource, TPROVIDER_AccountServer, NULL, NULL, NULL);
				AccountTransactionRecordKeyValueChangesNonTransacted(pLog, eaChanges, pRequest->pReason);
				AccountTransactionFinishNonTransacted(&pLog);
			}
...
SA_RET_NN_VALID XMLRPCDuplicateKeyValueResponse *XMLRPCDuplicateKeyValue(XMLRPCDuplicateKeyValueRequest* pRequest)
{
	...
	if (pAccount)
	{
		...
		if (eResult == AKV_SUCCESS)
		{
			eResult = AccountKeyValue_Change(pRequest->pRequestSource, pAccount, pRequest->pDestination, amount, &pDestLock);
		}

		if (eResult == AKV_SUCCESS)
		{
			if (pDestLock) eResult = AccountKeyValue_Commit(pRequest->pRequestSource, pAccount, pRequest->pDestination, pDestLock);
			pDestLock = NULL;
		}

		if (eResult != AKV_SUCCESS)
		{
			if (pDestLock) AccountKeyValue_Rollback(pRequest->pRequestSource, pAccount, pRequest->pDestination, pDestLock);
		}
...
// search for partial match of account data
AUTO_COMMAND ACMD_CATEGORY(XMLRPC) ACMD_ACCESSLEVEL(9) ACMD_NAME("AccountSearch");
void XMLRPCAccountSearch(CmdContext *pContext, XMLRPCAccountSearchRequest *pXMLRequest)
{
	...
	*pSlowReturnInfo = pContext->slowReturnInfo;
	...
	accountSearch_Async(
		pXMLRequest->pAccountName,
		pXMLRequest->pDisplayName,
		pXMLRequest->pEmail,
		pXMLRequest->pFirstName,
		pXMLRequest->pLastName,
		pXMLRequest->pProduct,
		pXMLRequest->pKey,
		pXMLRequest->pCardName,
		pXMLRequest->pCardFirstSix,
		pXMLRequest->pCardLastFour,
		pXMLRequest->pStreetAddress,
		pXMLRequest->pCity,
		pXMLRequest->pState,
		pXMLRequest->pZip,
		pXMLRequest->iMaxToReturn,
...
static void XMLRPCRefreshVindiciaAccount_FetchComplete(BillingTransaction * pTrans)
{
	...
	if (pVindiciaResult)
	{
		...
		if (pResponse->_return_->returnCode == VINDICIA_SUCCESS_CODE)
		{
			...
			btUpdateActiveSubscriptions(pAccount->uID, NULL, NULL, pTrans);
		}
		else
		{
...
XMLRPCCollisionQueryResponse *XMLRPCNameCollisionQuery(const char *pAccountOrDisplayName)
{
	XMLRPCCollisionQueryResponse *pResponse = StructCreate(parse_XMLRPCCollisionQueryResponse);

	LogXmlrpcf("Request: XMLRPCNameCollisionQuery(%s)", pAccountOrDisplayName);

	if (isAccountNameUsed(pAccountOrDisplayName) || isDisplayNameUsed(pAccountOrDisplayName))
	{
...
AUTO_STRUCT;
typedef struct XMLRPCCollisionQueryRequest
{
	const char *pAccountName;
	const char *pDisplayName;
} XMLRPCCollisionQueryRequest;

AUTO_COMMAND ACMD_CATEGORY(XMLRPC) ACMD_ACCESSLEVEL(9) ACMD_NAME("NameCollisionQueryEx");
XMLRPCCollisionQueryResponse *XMLRPCNameCollisionQueryEx(XMLRPCCollisionQueryRequest *pRequest)
{
	XMLRPCCollisionQueryResponse *pResponse = StructCreate(parse_XMLRPCCollisionQueryResponse);

	LogXmlrpcf("Request: XMLRPCNameCollisionQueryEx(%s, %s)",
		SAFE_DEREF(pRequest->pAccountName) ? pRequest->pAccountName : "(null)",
		SAFE_DEREF(pRequest->pDisplayName) ? pRequest->pDisplayName : "(null)"
	);

	if (!SAFE_DEREF(pRequest->pAccountName) && !SAFE_DEREF(pRequest->pDisplayName))
	{
		pResponse->pResult = ACCOUNT_HTTP_NO_ARGS;
	}
	else
	{
		if (SAFE_DEREF(pRequest->pAccountName) && isAccountNameUsed(pRequest->pAccountName))
		{
			pResponse->pResult = ACCOUNT_HTTP_USER_EXISTS;
		}
		if (SAFE_DEREF(pRequest->pDisplayName) && isDisplayNameUsed(pRequest->pDisplayName))
		{
			pResponse->pResult = pResponse->pResult ? ACCOUNT_HTTP_ALLNAMES_EXISTS : ACCOUNT_HTTP_DISPLAYNAME_EXISTS;
		}
