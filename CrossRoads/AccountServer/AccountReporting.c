								 ...
								 SA_PARAM_NN_STR const char *pShardCategory, U32 uPlayTime)
{
	...
	asgAddPlayTime(accountGetIETFLanguageTag(pAccount), pProductName, pShardCategory, uPlayTime);
...
void accountReportPurchase(SA_PARAM_NN_VALID AccountInfo *pAccount, SA_PARAM_NN_VALID const ProductContainer *pProduct, SA_PARAM_NN_VALID const Money *pPrice)
{
	...
	asgAddPurchaseStatsItem(accountGetIETFLanguageTag(pAccount), pProduct->pName, pPrice);
								   ...
								   const char *pAvsCode, const char *pCvnCode)
{
	...
	asgAddTransactionCodes(accountGetIETFLanguageTag(pAccount), pCurrency, pAuthCode, pAvsCode, pCvnCode);
...
void ScanAndUpdateStats()
{
	...
	while (con)
	{
		...
		EARRAY_CONST_FOREACH_BEGIN(eaKeys, iCurKey, iNumKeys);
		{
			...
			bool locked = AccountKeyValue_IsLocked("internal", account, pKey);

			if (!locked)
			{
				S64 iValue = 0;
				if (AccountKeyValue_Get("internal", account, pKey, &iValue) == AKV_SUCCESS)
				{
