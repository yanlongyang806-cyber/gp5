...
static AccountProxyProductList *GetProxyProductList(void)
{
	...
	EARRAY_FOREACH_BEGIN(eaProducts, i);
	{
		...
		if (!productIsBetweenStartDateEndDate(pLocalProduct)) continue; // if the product is out of available dates, do not include
...
static void HandleGetRequest(PACKET_PARAMS_PROTOTYPE)
{
	...
	if (pAccount && pSubbedKey)
	{
		...
		packetResponse->eResult = AccountKeyValue_Get(pProxy ? getProxyName(pProxy) : NULL, pAccount, pSubbedKey, &iValue);
...
static void HandleSetRequest(PACKET_PARAMS_PROTOTYPE)
{
	...
	if (pAccount && pSubbedKey)
	{
		...
		AccountKeyValue_Get(pProxy ? getProxyName(pProxy) : NULL, pAccount, pSubbedKey, &iValue);
		packetResponse->pValue = strdupf("%"FORM_LL"d", iValue);
		packetResponse->iValue = iValue;
		...
		{
		xcase AKV_OP_SET:
			result = AccountKeyValue_Set(pProxy ? getProxyName(pProxy) : NULL, pAccount, pSubbedKey, iSetValue, &packetResponse->pLock);
		xcase AKV_OP_INCREMENT:
			result = AccountKeyValue_Change(pProxy ? getProxyName(pProxy) : NULL, pAccount, pSubbedKey, iSetValue, &packetResponse->pLock);
		xdefault:
...
static void HandleCommitRollbackRequest(PACKET_PARAMS_PROTOTYPE)
{
	...
	if (requestPacket.pLock && pSubbedKey && info)
	{
		...
		if (bCommit)
		{
			...
			AccountKeyValue_Get(pProxy ? getProxyName(pProxy) : NULL, info, pSubbedKey, &iPreviousValue);

			// Attempt to commit key values.
			result = AccountKeyValue_Commit(pProxy ? getProxyName(pProxy) : NULL, info, pSubbedKey, requestPacket.pLock);

			if (eaSize(&eaChanges) > 0 && result == AKV_SUCCESS)
			{
				NOCONST(TransactionLogContainer) *pLog = AccountTransactionOpenNonTransacted(info->uID, requestPacket.eTransactionType, pProxy ? getProxyName(pProxy) : NULL, TPROVIDER_AccountServer, NULL, NULL, NULL);
				AccountTransactionRecordKeyValueChangesNonTransacted(pLog, eaChanges, NULL);
				AccountTransactionFinishNonTransacted(&pLog);
			}

			AccountTransactionFreeKeyValueChanges(&eaChanges);

			AccountKeyValue_Get(pProxy ? getProxyName(pProxy) : NULL, info, pSubbedKey, &iNewValue);
		...
		else
		{
			...
			result = AccountKeyValue_Rollback(pProxy ? getProxyName(pProxy) : NULL, info, pSubbedKey, requestPacket.pLock);
		}
	}
...
static void HandleSimpleSet(PACKET_PARAMS_PROTOTYPE)
{
	...
	if (info && pSubbedKey)
	{
		...
		{
		case AKV_OP_SET:
			result = AccountKeyValue_Set(pProxy ? getProxyName(pProxy) : NULL, info, pSubbedKey, iSetValue, &lock);
		xcase AKV_OP_INCREMENT:
			result = AccountKeyValue_Change(pProxy ? getProxyName(pProxy) : NULL, info, pSubbedKey, iSetValue, &lock);
		xdefault:
			devassertmsg(false, "Unsupported account key operation.");
			result = AKV_FAILURE;
		}
		...
		{
			result = AccountKeyValue_Commit(pProxy ? getProxyName(pProxy) : NULL, info, pSubbedKey, lock);
		}
	}
...
static void HandleRequestWebSrvGameEvent(PACKET_PARAMS_PROTOTYPE)
{
	...
	accountSendEventEmail(pAccount, request.pLang, fullEventName, request.keyValueList, NULL, NULL);
	StructDeInit(parse_AccountProxyWebSrvGameEvent, &request);
...
static void HandleMoveRequest(PACKET_PARAMS_PROTOTYPE)
{
	...
	if (pSrcAccount && pDestAccount && request.pSrcKey && request.pDestKey)
	{
		...
		response.eResult = AccountKeyValue_Move(pProxy ? getProxyName(pProxy) : NULL, pSrcAccount, request.pSrcKey, pDestAccount, request.pDestKey, request.iValue, &response.pLock);
	}
...
static void HandleCommitRollbackMoveRequest(PACKET_PARAMS_PROTOTYPE)
{
	...
	if (bCommit && AccountKeyValue_Commit(pProxy ? getProxyName(pProxy) : NULL, pSrcAccount, request.pSrcKey, request.pLock) == AKV_SUCCESS)
	{
		char *pReason = NULL;
		NOCONST(TransactionLogContainer) *pSrcLog = NULL;
		NOCONST(TransactionLogContainer) *pDestLog = NULL;
		U32 uSrcLogID = 0;
		U32 uDestLogID = 0;

		if (eaSrcChanges)
		{
			pSrcLog = AccountTransactionOpenNonTransacted(pSrcAccount->uID, request.eTransactionType, pProxy ? getProxyName(pProxy) : NULL, TPROVIDER_AccountServer, NULL, NULL, NULL);
			uSrcLogID = pSrcLog->uID;
		}

		if (request.uSrcAccountID != request.uDestAccountID && eaDestChanges)
		{
			pDestLog = AccountTransactionOpenNonTransacted(pDestAccount->uID, request.eTransactionType, pProxy ? getProxyName(pProxy) : NULL, TPROVIDER_AccountServer, NULL, NULL, NULL);
			uDestLogID = pDestLog->uID;
		}
		...
		if (pSrcLog)
		{
			...
			AccountTransactionRecordKeyValueChangesNonTransacted(pSrcLog, eaSrcChanges, pReason);
			AccountTransactionFinishNonTransacted(&pSrcLog);
			estrDestroy(&pReason);
		}
		...
		if (pDestLog)
		{
			...
			AccountTransactionRecordKeyValueChangesNonTransacted(pDestLog, eaDestChanges, pReason);
			AccountTransactionFinishNonTransacted(&pDestLog);
			estrDestroy(&pReason);
		}
	}
	else
	{
		AccountKeyValue_Rollback(pProxy ? getProxyName(pProxy) : NULL, pSrcAccount, request.pSrcKey, request.pLock);
	}
