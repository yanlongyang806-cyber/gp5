...
// Merger Container ID to differentiate itself from AS in the logs. Will increase by one each time a merger launches.
static ContainerID suMergerContainerID = 1000;
AUTO_CMD_INT(suMergerContainerID, MergerContainerID) ACMD_CMDLINE;
...
bool gbMachineLockDisable = true;
...
// Maximum number of log-ins to process per second (0 = infinite)
U32 guLoginsPerSecond = 0;
AUTO_CMD_INT(guLoginsPerSecond, LoginsPerSecond) ACMD_CMDLINE ACMD_CATEGORY(Account_Server);
...
/************************************************************************/
/* Account server exit called                                           */
/************************************************************************/
static bool sbAccountSererExitCalled = false;

bool AccountServerGetExitCalled()
{
	return sbAccountSererExitCalled;
}

void AccountServerSetExitCalled(bool bCalled)
{
	sbAccountSererExitCalled = bCalled;
}
...
const char *accountGetIETFLanguageTag(const AccountInfo *account)
{
	if (!account)
	{
		return locGetIETFLanguageTag(DEFAULT_LOCALE_ID);
	}

	if (locIDIsValid(account->lastLoginLocaleID))
	{
		return locGetIETFLanguageTag(account->lastLoginLocaleID);
	}

	if (account->pPWAccountName)
	{
		PWCommonAccount * pwAccount = findPWCommonAccountByName(account->pPWAccountName);
		if (pwAccount && !nullStr(pwAccount->pRegistrationPage))
		{
			// So far, all of the PWE registration page values are IETF
			// language tags
			return pwAccount->pRegistrationPage;
		}
	}

	if (!nullStr(account->defaultLocale))
	{
		// Already an IETF language tag (came from browser from Drupal sites)
		return account->defaultLocale;
	}
...
static bool IPAllowedAccountAutoCreate(U32 uIP)
{
	...
	if (ipfIsTrustedIp(uIP)) return false; // was true = auto created account on web server
...
{
	if (ipfIsTrustedIp(uIP)) return true; // was false = requires no password
...
void AccountNonTransactedChange(GlobalType eType, ContainerID conID, const char *pDiffString)
{
	static char *pUpdateString = NULL;

	if (nullStr(pDiffString))
	{
		return;
	}

	estrPrintf(&pUpdateString, "dbUpdateContainer %u %u %s", eType, conID, pDiffString);
	accountdbHandleDatabaseUpdateStringEx(pUpdateString, 0, 0, false);
	estrClear(&pUpdateString);
}

void AccountNonTransactedChangef(GlobalType eType, ContainerID conID, const char *pFormat, ...)
{
	static char *pUpdateString = NULL;

	estrPrintf(&pUpdateString, "dbUpdateContainer %u %u ", eType, conID);
	estrGetVarArgs(&pUpdateString, pFormat);
	accountdbHandleDatabaseUpdateStringEx(pUpdateString, 0, 0, false);
	estrClear(&pUpdateString);
}
...
static void CreateAccountLoginTicket(SA_PARAM_NN_VALID NetLink *link, AccountLoginType eLoginType, SA_PARAM_NN_VALID AccountInfo *account, const char *pMachineID, U32 uIP)
{
	...
	{
		estrConcatf(&pDiffString, "set .Logindata.Uip = \"%d\"", uIP);
	}

	if (account->loginData.uTime != uNow)
	{
		estrConcatf(&pDiffString, "set .Logindata.Utime = \"%d\"", uNow);
	}
...
static void HandleLoginWithNVPStruct(Packet *pak, int cmd, NetLink* link, AccountLink *accountLink)
{
	...
	HandleLogin_Common(pak, link, accountLink, pLoginField, NULL,
		...
		accountLink->fixedSalt, recvStruct.machineID, recvStruct.localeID);
}
...
static void HandleLoginMachineID(Packet* pak, int cmd, NetLink* link, AccountLink *accountLink)
{
	...
	HandleLogin_Common(pak, link, accountLink, pAccount, pPassword, pPasswordMD5, NULL, NULL, 0, NULL, NULL, pMachineID, LOCALE_ID_INVALID);
	PERFINFO_AUTO_STOP_FUNC();
	...
	void * userData)
{
	...
	if (!pCrypticAccount)
	{
		...
		// Determine if the private name is good
		switch (pLoginData->eLoginType)
		{
		case ACCOUNTLOGINTYPE_CrypticAndPW:
			bGoodPrivateName = !!findPWCommonAccountByLoginField(pLoginData->pPrivateAccountName);
			if (bGoodPrivateName) break;
			// fallthrough

		case ACCOUNTLOGINTYPE_Cryptic:
			pCrypticAccount = findAccountByName(pLoginData->pPrivateAccountName);
			uContainerID = pCrypticAccount ? pCrypticAccount->uID : 0;
			bGoodPrivateName = !!uContainerID;
			break;

		case ACCOUNTLOGINTYPE_PerfectWorld:
			bGoodPrivateName = !!findPWCommonAccountByLoginField(pLoginData->pPrivateAccountName);
			break;
		}
	...
	// Update the user's last log-in locale if the locale is valid and
	// the machine isn't locked
	if (!pLoginData->bMachineLocked &&
		locIDIsValid(pLoginData->localeID))
	{
		accountSetLastLoginLocaleID(pCrypticAccount, pLoginData->localeID);
	}
...
AUTO_STRUCT;
typedef struct LoginQueueEntry
{
	U32 uLinkID;
	AccountSaltRequest request;
} LoginQueueEntry;

static void HandleLoginQueueEntry(LoginQueueEntry * pEntry)
{
	Packet *response = NULL;
	...
	if (!verify(pEntry)) return;

	PERFINFO_AUTO_START_FUNC();

	link = linkFindByID(pEntry->uLinkID);
	if (!link || !linkConnected(link))
	{
		StructDestroy(parse_LoginQueueEntry, pEntry);
		PERFINFO_AUTO_STOP();
		return;
	}

	accountLink = linkGetUserData(link);

	if (pEntry->request.pLoginField[0])
	{
		strcpy(accountLink->loginField, pEntry->request.pLoginField);
		GetFixedSalt(pEntry->request.pLoginField, accountLink->fixedSalt, ARRAY_SIZE(accountLink->fixedSalt));
	}
...
static void processLoginQueue(float fTotalElapsed)
{
	static float fLastProcessTime = 0;
	float fTimePerEntry = guLoginsPerSecond ? 1.0 / guLoginsPerSecond : 0.0;

	PERFINFO_AUTO_START_FUNC();

	if (!fLastProcessTime)
	{
		fLastProcessTime = fTotalElapsed;
	}

	while (!qIsEmpty(gLoginQueue) && fLastProcessTime + fTimePerEntry < fTotalElapsed)
	{
		LoginQueueEntry * pEntry = qDequeue(gLoginQueue);
		HandleLoginQueueEntry(pEntry);

		fLastProcessTime += fTimePerEntry;
	}

	if (qIsEmpty(gLoginQueue))
	{
		if (guLoginsPerSecond)
		{
			fLastProcessTime += (int)((fTotalElapsed - fLastProcessTime) / fTimePerEntry) * fTimePerEntry;
		}
		else
		{
			fLastProcessTime = fTotalElapsed;
...
int loginQueueSize(void)
{
	return qGetSize(gLoginQueue);
}

static void HandleRequestLoginSalt(Packet * pak, NetLink * link, AccountLink * accountLink)
{
	LoginQueueEntry * pEntry = StructCreate(parse_LoginQueueEntry);

	pEntry->uLinkID = linkID(link);

	if (!pktEnd(pak))
	{
		ParserReceiveStructAsCheckedNameValuePairs(pak, parse_AccountSaltRequest, &pEntry->request);
	}

	qEnqueue(gLoginQueue, pEntry);
}

static void AccountServerHandleMessageSwitch(Packet* pak, int cmd, NetLink* link, AccountLink *accountLink, bool bInternalPort)
{
	bool bCmdHandled = false;
	static StaticCmdPerf cmdPerf[TO_ACCOUNTSERVER_MAX] = {0};
	...
	if (!bCmdHandled)
	{
		switch(cmd)
		{
			case TO_ACCOUNTSERVER_LOGINVALIDATE:
				HandleTicketValidateRequest(pak, link, accountLink);
				break;

			case TO_ACCOUNTSERVER_GENERATE_ONETIMECODE:
				HandleGenerateOneTimeCode(pak, link, accountLink);
				break;

			case TO_ACCOUNTSERVER_LOGIN_MACHINEID:
				HandleLoginMachineID(pak, cmd, link, accountLink);
				break;

			case TO_ACCOUNTSERVER_ONETIMECODE:
				HandleOneTimeCode(pak, link, accountLink);
				break;

			case TO_ACCOUNTSERVER_SAVENEXTMACHINE:
				HandleSaveNextMachineInfo(pak, link, accountLink);
				break;

			case TO_ACCOUNTSERVER_REQUEST_LOGINSALT:
			...
			case TO_ACCOUNTSERVER_DISPLAYNAME_CHANGE:
				AssertOrAlert("ACCOUNT_SERVER_NO_DISPLAY_CHANGE",
					"Received packet requesting display name change; not supported.");
				break;

			// Logging of system spects
			case TO_ACCOUNTSERVER_LOGSPECS:
				{
					U32 uAccountID;
					char *systemSpecs;
					AccountInfo *account;
			...
			case TO_ACCOUNTSERVER_NVPSTRUCT_LOGIN  :
...
{
	PERFINFO_AUTO_START_FUNC();
	AccountServerHandleMessageSwitch(pak, cmd, link, accountLink, false);
	PERFINFO_AUTO_STOP();
}
...
{
	PERFINFO_AUTO_START_FUNC();
	AccountServerHandleMessageSwitch(pak, cmd, link, accountLink, true);
	PERFINFO_AUTO_STOP();
}
...
// ONE TIME FIXUP - activate key-value conversion
bool gbDoKeyValueConversion = false;
AUTO_CMD_INT(gbDoKeyValueConversion, DoKeyValueConversion) ACMD_CMDLINE;
...
char gcRecentStipendImportFile[CRYPTIC_MAX_PATH] = {0};
AUTO_CMD_STRING(gcRecentStipendImportFile, ImportRecentStipends) ACMD_CMDLINE ACMD_CATEGORY(Account_Server);
...
static void scanAccounts(void)
{
	...
	StashTable *psKeyValueFixupAccountLists = NULL;
	CONST_EARRAY_OF(KeyValueConversion) eaConversions = GetKeyValueConversions();

	bool bDoingRecentStipendFixup = false;
	StashTable psRecentStipendImportList = NULL;
	...
	if (giOneTimeLockAccountsBeforeCutoff)
	{
		...
		if (fileExists(lockExcludeFile))
		{
			...
			sLockAccountNameExclusions = stashTableCreateWithStringKeys(8, StashDeepCopyKeys_NeverRelease);
	...
	if (gcMassPasswordChangeFile[0] && fileExists(gcMassPasswordChangeFile))
	{
		...
		sMassPasswordChanges = stashTableCreateWithStringKeys(8, StashDeepCopyKeys_ReleaseOnRemove);
	...
	if (gcRecentStipendImportFile[0] && fileExists(gcRecentStipendImportFile))
	{
		char **ppLines = NULL;
		char *pRawText = NULL;
		int len = 0;

		bDoingRecentStipendFixup = true;
		psRecentStipendImportList = stashTableCreateInt(8);

		pRawText = fileAlloc(gcRecentStipendImportFile, &len);
		DivideString(pRawText, "\n", &ppLines, DIVIDESTRING_POSTPROCESS_STRIP_WHITESPACE | DIVIDESTRING_POSTPROCESS_DONT_PUSH_EMPTY_STRINGS | DIVIDESTRING_POSTPROCESS_ESTRINGS);
		fileFree(pRawText);

		EARRAY_FOREACH_BEGIN(ppLines, iLine);
		{
			char **ppFields = NULL;
			NOCONST(RecentStipendGrant) **eaRecents = NULL;
			NOCONST(RecentStipendGrant) *pNewRecent = NULL;
			void *pStashEntry = NULL;
			U32 uAccountID = 0;
			U32 uAmount = 0;
			U32 uGrantTimestamp = 0;

			DivideString(ppLines[iLine], ",", &ppFields, DIVIDESTRING_POSTPROCESS_STRIP_WHITESPACE | DIVIDESTRING_POSTPROCESS_DONT_PUSH_EMPTY_STRINGS | DIVIDESTRING_POSTPROCESS_ESTRINGS);

			if (!devassertmsgf(eaSize(&ppFields) == 5, "Wrong number of fields in recent stipend fixup file, line %d (fields: %d)", iLine, eaSize(&ppFields)))
			{
				eaDestroyEString(&ppFields);
				continue;
			}

			if (!StringToUint(ppFields[0], &uAccountID))
			{
				devassertmsgf(0, "Bad account ID in recent stipend fixup file, line %d (ID: %s)", iLine, ppFields[0]);
				eaDestroyEString(&ppFields);
				continue;
			}

			if (!StringToUint(ppFields[2], &uAmount))
			{
				devassertmsgf(0, "Bad amount in recent stipend fixup file, line %d (amount: %s)", iLine, ppFields[2]);
				eaDestroyEString(&ppFields);
				continue;
			}

			if (!StringToUint(ppFields[4], &uGrantTimestamp))
			{
				devassertmsgf(0, "Bad grant timestamp in recent stipend fixup file, line %d (amount: %s)", iLine, ppFields[4]);
				eaDestroyEString(&ppFields);
				continue;
			}

			pNewRecent = StructCreateNoConst(parse_RecentStipendGrant);
			estrCopy2(&pNewRecent->pInternalSubName, ppFields[1]);
			pNewRecent->uAmount = uAmount;
			estrCopy2(&pNewRecent->pCurrency, ppFields[3]);
			pNewRecent->uGrantedSS2000 = uGrantTimestamp;

			if (stashIntFindPointer(psRecentStipendImportList, uAccountID, &pStashEntry))
			{
				eaRecents = (NOCONST(RecentStipendGrant) **)pStashEntry;
			}
			else
			{
				eaCreate(&eaRecents);
				eaSetCapacity(&eaRecents, 4);
			}

			eaPush(&eaRecents, pNewRecent);
			stashIntAddPointer(psRecentStipendImportList, uAccountID, (void*)eaRecents, true);
	...
	if (gbDoKeyValueConversion)
	{
		psKeyValueFixupAccountLists = calloc(eaSize(&eaConversions), sizeof(StashTable));
		...
		EARRAY_CONST_FOREACH_BEGIN(eaConversions, iConversion, iNumConversions);
		{
			if (eaConversions[iConversion]->pAccountList)
			{
				...
				fileFree(pListText);
				psKeyValueFixupAccountLists[iConversion] = stashTableCreateWithStringKeys(8, StashDeepCopyKeys_NeverRelease);

				EARRAY_FOREACH_BEGIN(ppNames, i);
				{
					char *pName = ppNames[i];
					stashAddInt(psKeyValueFixupAccountLists[iConversion], pName, 1, false);
					estrDestroy(&pName);
				}
				EARRAY_FOREACH_END;
	...
	CONTAINER_FOREACH_BEGIN(GLOBALTYPE_ACCOUNT, accountContainer);
	{
		...
		{
			static char *pDiffString = NULL;
			accountClearFlagsNonTransacted(account, ACCOUNT_FLAG_LOGS_REBUCKETED, &pDiffString);
			AccountNonTransactedChange(GLOBALTYPE_ACCOUNT, account->uID, pDiffString);
			estrClear(&pDiffString);
			bUpdated = true;
		}
		...
		if (gbDoKeyValueConversion)
		{
			EARRAY_CONST_FOREACH_BEGIN(eaConversions, iConversion, iNumConversions);
			{
				KeyValueConversion *pConversion = eaConversions[iConversion];

				if (VirtualCurrency_IsChain(pConversion->pOldKey) || VirtualCurrency_IsChain(pConversion->pNewKey)) return;
				if (pConversion->pAccountList && !stashFindInt(psKeyValueFixupAccountLists[iConversion], account->accountName, NULL)) continue;

				if (AccountKeyValue_ConvertKeyValue(account, pConversion->pOldKey, pConversion->pNewKey))
					bUpdated = true;
			}
			EARRAY_FOREACH_END;
		}
		...
		if (bDoingRecentStipendFixup)
		{
			NOCONST(RecentStipendGrant) **eaRecents = NULL;
			void *pStashEntry = NULL;

			if (stashIntRemovePointer(psRecentStipendImportList, account->uID, &pStashEntry))
			{
				eaRecents = (NOCONST(RecentStipendGrant) **)pStashEntry;

				EARRAY_FOREACH_BEGIN(eaRecents, iRecent);
				{
					fixupRecentStipend(account, eaRecents[iRecent]);
				}
				EARRAY_FOREACH_END;

				eaDestroyStructNoConst(&eaRecents, parse_RecentStipendGrant);

				bUpdated = true;
			}
		}

		// Queue up possible stipends
		{
			const InternalSubscription **eaInternalSubs = findInternalSubsByAccountID(account->uID);

			EARRAY_CONST_FOREACH_BEGIN(eaInternalSubs, iSub, iNumSubs);
			{
				const InternalSubscription *pInternalSub = eaInternalSubs[iSub];
				scheduleInitialStipendGrant(account, pInternalSub->pSubInternalName, getInternalSubAnniversaryDay(pInternalSub), true);
			}
			EARRAY_FOREACH_END;

			eaDestroy(&eaInternalSubs);
		}

		if (account->pCachedSubscriptionList)
		{
			EARRAY_CONST_FOREACH_BEGIN(account->pCachedSubscriptionList->ppList, iSub, iNumSubs);
			{
				const CachedAccountSubscription *pCachedSub = account->pCachedSubscriptionList->ppList[iSub];
				int iBillingDay = getCachedSubBillingDay(pCachedSub);

				if (!iBillingDay)
				{
					accountLog(account, "Could not queue stipend for %s (VID: %s) because the subscription had no billing day!", pCachedSub->internalName, pCachedSub->vindiciaID);
					break;
				}

				scheduleInitialStipendGrant(account, pCachedSub->internalName, iBillingDay, false);
			}
			EARRAY_FOREACH_END;
		}
	...
	if (gbDoKeyValueConversion)
	{
		...
		{
			if (psKeyValueFixupAccountLists[i])
				stashTableDestroy(psKeyValueFixupAccountLists[i]);
		}
		EARRAY_FOREACH_END;

		free(psKeyValueFixupAccountLists);
	}

	if (bDoingRecentStipendFixup)
	{
		stashTableDestroy(psRecentStipendImportList);
	}
...
void accountdbHandleDatabaseUpdateStringEx(const char *cmd_orig, U64 sequence, U32 timestamp, bool replay)
{
	int result = 0;
...
static bool gbMergerMovesBackupToOld = true;
AUTO_CMD_INT(gbMergerMovesBackupToOld, MergerMovesBackupToOld) ACMD_CMDLINE;

static bool gbCompressIncomingAccountLinks = true;
AUTO_CMD_INT(gbCompressIncomingAccountLinks, CompressIncomingAccountLinks) ACMD_CMDLINE;

int AccountInit_2(void)
{
	static U32 uLinkFlags = LINK_FORCE_FLUSH;
	...
	if (isAccountServerMode(ASM_Merger))
	{
		...
		if (gbFastSnapshot)
		{
			objMergeIncrementalHogs(pDatabaseConfig->bBackupSnapshot, gAppendSnapshot, gbMergerMovesBackupToOld);
	...
	AccountIntegration_ScanBatches();
	...
	if (!gbCompressIncomingAccountLinks)
		uLinkFlags |= LINK_NO_COMPRESS;

	val = commListenBoth(commDefault(), LINKTYPE_UNSPEC, uLinkFlags, DEFAULT_ACCOUNTSERVER_UNENCRYPTED_PORT, AccountServerHandleMessage, AccountServerConnectCallback,
	...
	val = commListenBoth(commDefault(), LINKTYPE_UNSPEC, uLinkFlags|LINK_NO_COMPRESS, DEFAULT_ACCOUNTSERVER_TRUSTED_PORT, AccountServerHandleTrustedMessage, AccountServerConnectCallback,
		AccountServerDisconnectCallback, sizeof(AccountLink), &local_link, &public_link);
...
static int gbMergerWaitForDebugger = false;
// command name can't have "waitfordebugger" in it because of cmd line parsing
AUTO_CMD_INT(gbMergerWaitForDebugger,MergerDebug) ACMD_COMMANDLINE;
...
void StartMerger(bool hideMerger, bool defrag)
{
	...
	bool mutexLocked;
	int iMergerPID;
	char *pAdditionalLogServerCommandline = NULL;
	...
	estrConcatf(&estr, " -ContainerID %d", suMergerContainerID);
	GetAdditionalLogServerCommandLineString(&pAdditionalLogServerCommandline);
	if (SAFE_DEREF(pAdditionalLogServerCommandline))
	{
		estrConcatf(&estr, " %s", pAdditionalLogServerCommandline);
	}
	...
	if(gbMergerWaitForDebugger)
		estrConcatf(&estr, " -WaitForDebugger");
	...
	// We don't wrap around the Merger container id. AS restart will reset this.
	++suMergerContainerID;

	estrDestroy(&estr);
	estrDestroy(&pAdditionalLogServerCommandline);
}
...
int AccountServerOncePerFrame(F32 fTotalElapsed, F32 fElapsed)
{
	...
	processLoginQueue(fTotalElapsed);
	stipendTick();
...
AUTO_STARTUP(AccountServer) ASTRT_DEPS( AS_TextFilter );
...
int AccountServerInit(void)
{
	...
	else
	{
		...
		AutoStartup_SetTaskIsOn( "AccountServer", 1 );
		DoAutoStartup();
...
void AccountServer_CleanupDisplayNames (void)
{
	...
	while (con)
	{
		...
		{
			if (!findAccountContainerByDisplayName(pDisplayCopy))
			{
...
// Clear all the rate limit tokens under specific IP
AUTO_COMMAND ACMD_CATEGORY(Account_Server);
bool ClearIPFromRateLimit(SA_PARAM_NN_STR const char * szIP)
{
	bool bResult = false;
	if (gpIPRateLimit)
	{
		bResult = RLRemoveFromRateLimit(gpIPRateLimit, szIP);
	}
	return bResult;
}
