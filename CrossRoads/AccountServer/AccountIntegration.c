...
PWCommonAccount * findPWCommonAccountByName(const char *pAccountName)
{
	...
	if (!devassert(stPWAccountByAccountName))
		stPWAccountByAccountName = stashTableCreateWithStringKeys(PWACCOUNT_DEFAULT_STASH_SIZE, StashDeepCopyKeys_NeverRelease);
...
PWCommonAccount * findPWCommonAccountByForumName(const char *pForumName)
{
	...
	if (!devassert(stPWAccountByForumName))
		stPWAccountByForumName = stashTableCreateWithStringKeys(PWACCOUNT_DEFAULT_STASH_SIZE, StashDeepCopyKeys_NeverRelease);
...
PWCommonAccount * findPWCommonAccountByEmail(const char *pEmail)
{
	...
	if (!devassert(stPWAccountByEmail))
		stPWAccountByEmail = stashTableCreateWithStringKeys(PWACCOUNT_DEFAULT_STASH_SIZE, StashDeepCopyKeys_NeverRelease);
...
static void autoGenerateAccountName(char **estr, SA_PARAM_NN_VALID PWCommonAccount *pAccount)
{
	...
	// No prefix/suffix if account name is available
	if (!findAccountByName(pBaseFixed))
	{
...
AccountIntegrationResult AccountIntegration_LinkAccountToPWCommon(SA_PARAM_NN_VALID AccountInfo *pAccount, SA_PARAM_NN_VALID PWCommonAccount *pCommon)
{
	...
	accountLog(pAccount, "Account linked to PW account %s", pAccount->pPWAccountName);
...
AccountIntegrationResult AccountIntegration_UnlinkAccountFromPWCommon(SA_PARAM_NN_VALID AccountInfo *pAccount)
{
	...
	accountLog(pAccount, "Account unlinked from PW account %s", pAccount->pPWAccountName);
...
AccountIntegrationResult AccountIntegration_CreateAccountFromPWCommon(PWCommonAccount *pCommon, const char *pOverrideDisplayName, AccountInfo **ppAccount, AccountCreationResult *peCreateResult)
{
	...
	if (nullStr(pOverrideDisplayName))
	{
		...
		if (findAccountByDisplayName(pDisplayName))
		{
			PERFINFO_AUTO_STOP();
	...
	else
	{
		...
		if (isDisplayNameUsed(pDisplayName))
		{
			PERFINFO_AUTO_STOP();
			return ACCOUNT_INTEGRATION_DisplayNameConflict;
		}
	...
	account = findAccountByName(pCommon->pAccountName);
	...
	if (returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		if (pPWBatch && trData->pAccountName)
		{
			...
			if (pPWcom)
			{
				...
				if (trData->pPrevEmail && stricmp_safe(trData->pPrevEmail, pPWcom->pEmail))
				{
					stashRemovePointer(stPWAccountByEmail, trData->pPrevEmail, NULL);
				}
...
enumTransactionOutcome trUpdatePWCommonAccount(ATR_ARGS,
	NOCONST(PerfectWorldAccountBatch) *pwBatch,
	const char *pAccountName, 
	const char *pForumName,
	const char *pEmail,
	const char *pRegistrationPage,
	const char *pPasswordHash,
	const char *pPasswordHashFixedSalt,
	...
	int iBanned)
{
	...
	if (strcmp_safe(pwAccount->pRegistrationPage, pRegistrationPage))
	{
		SAFE_FREE(pwAccount->pRegistrationPage);
		pwAccount->pRegistrationPage = StructAllocString(pRegistrationPage);
	}
...
enumTransactionOutcome trAddPWCommonAccount(ATR_ARGS,
	NOCONST(PerfectWorldAccountBatch) *pwBatch,
	const char *pAccountName, 
	const char *pForumName,
	const char *pEmail,
...
static void AccountIntegration_LogPWCommonChange(
	const AccountInfo *pCrypticAccount,
	...
	)
{
	if (pCrypticAccount)
	{
		...
		accountLog(pCrypticAccount, pFullString);
		estrDestroy(&pFullString);
	}
}

static bool AccountIntegration_PWCommonHasChangesWithLogging(
	PWCommonAccount *pPWAccount,
	const char *pAccountName,
	const char *pForumName,
	const char *pEmail,
	const char *pRegistrationPage,
...
{
	bool bResult = false;
	//log on the cryptic account activity if we find the cryptic account
	AccountInfo * pAccount = findAccountByID(pPWAccount->uLinkedID);
	...
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW account name changed (from: '%s' to: '%s')", pPWAccount->pAccountName, pAccountName);
		bResult = true;
	}
	if (strcmp_safe(pPWAccount->pForumName, pForumName) != 0)
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW forum name changed (from: '%s' to: '%s')", pPWAccount->pForumName, pForumName);
		bResult = true;
	}
	if (strcmp_safe(pPWAccount->pEmail, pEmail) != 0)
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW email changed (from: '%s' to: '%s')", pPWAccount->pEmail, pEmail);
		bResult = true;
	}
	if (strcmp_safe(pPWAccount->pRegistrationPage, pRegistrationPage) != 0)
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW registration page changed (from: '%s' to: '%s')", pPWAccount->pRegistrationPage, pRegistrationPage);
		bResult = true;
	}
	...
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW password hash changed");
		bResult = true;
	}
	if (strcmp_safe(pPWAccount->pPasswordHashFixedSalt_RAM, pPasswordHashFixedSalt) != 0)
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW password hash with fixed salt changed");
		bResult = true;
	}
	if (strcmp_safe(pPWAccount->pFixedSalt, pFixedSalt) != 0)
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW fixed salt changed");
		bResult = true;
	}
	
	if (pPWAccount->bBanned != bBanned)
	{
		AccountIntegration_LogPWCommonChange(pAccount, "PW banned status changed (from: '%d' to: '%d')", pPWAccount->bBanned, bBanned);
		bResult = true;
	}
	return bResult;
}
	...
	bool bBanned)
{
	...
	{
		if (AccountIntegration_PWCommonHasChangesWithLogging(
			pExistingCom, pAccountName, pForumName, pEmail, pRegistrationPage,
			pPasswordHash, pPasswordHashFixedSalt, pFixedSalt, bBanned))
		{
			...
			pData->pPrevEmail = StructAllocString(pExistingCom->pEmail);
			AutoTrans_trUpdatePWCommonAccount(objCreateManagedReturnVal(trUpdatePWCommonAccountCB, pData), objServerType(),
				...
				pRegistrationPage,
				pPasswordHash,
				pPasswordHashFixedSalt,
				pFixedSalt,
				bBanned);
			
			// verify employee status on linked cryptic account
			if (pExistingCom->uLinkedID)
				verifyAccountEmployeeStatus(findAccountByID(pExistingCom->uLinkedID));
...
static void pwAddBatchToStashTables(const PerfectWorldAccountBatch *pBatch)
{
	EARRAY_FOREACH_BEGIN(pBatch->eaAccounts, i);
	{
		const PWCommonAccount *pAccount = pBatch->eaAccounts[i];
...
static void pwBatchRemoveCallback(Container *con, PerfectWorldAccountBatch *pBatch)
{
	EARRAY_FOREACH_BEGIN(pBatch->eaAccounts, i);
	{
		...
		if (pAccount->pAccountName)
		{
			stashRemovePointer(stPWAccountByAccountName, pAccount->pAccountName, NULL);
			if (pAccount->pForumName)
				stashRemovePointer(stPWAccountByForumName, pAccount->pForumName, NULL);
			if (pAccount->pEmail)
				stashRemovePointer(stPWAccountByEmail, pAccount->pEmail, NULL);
		}
	}
	EARRAY_FOREACH_END;
}
...
void initializeAccountIntegration(void)
{
	...
	stPWAccountByAccountName = stashTableCreateWithStringKeys(131072, StashDeepCopyKeys_NeverRelease);
	stPWAccountByForumName = stashTableCreateWithStringKeys(131072, StashDeepCopyKeys_NeverRelease);
	stPWAccountByEmail = stashTableCreateWithStringKeys(131072, StashDeepCopyKeys_NeverRelease);
	...
	objRegisterContainerTypeRemoveCallback(GLOBALTYPE_ACCOUNTSERVER_PWCOMMONACCOUNT, pwBatchRemoveCallback);
}

void AccountIntegration_ScanBatches(void)
{
	CONTAINER_FOREACH_BEGIN(GLOBALTYPE_ACCOUNTSERVER_PWCOMMONACCOUNT, pContainer);
	{
		const PerfectWorldAccountBatch * pBatch = pContainer->containerData;

		if (!devassert(pBatch)) continue;

		pwAddBatchToStashTables(pBatch);
	}
...
typedef struct XMLRPCPWUpdateAccountRequest
{
	...
	const char *pRegistrationPage; AST(NAME(RegistrationPage))
...
XMLRPCPWUpdateAccountResponse * XMLRPCPWUpdateAccount(XMLRPCPWUpdateAccountRequest *pRequest)
{
	...
	if (XMLRPCPWUpdateAccountParamsGood(pRequest))
	{
		...
		result = AccountIntegration_CreateOrUpdatePWCommon(
			pRequest->pAccountName,
			pRequest->pForumName,
			pRequest->pEmail,
			pRequest->pRegistrationPage,
			pRequest->pPasswordHash,
			pRequest->pPasswordHashFixedSalt,
			pRequest->pFixedSalt,
			pRequest->bBanned);
...
typedef struct XMLRPCPWTransferCurrencyRequest
{
	...
	const char *pPWOrderID; AST(NAME(PWOrderID))
...
XMLRPCPWTransferCurrencyResponse * XMLRPCPWTransferCurrency(XMLRPCPWTransferCurrencyRequest *pRequest)
{
	...
	if (pRequest->pPWOrderID && AccountTransactionGetLogByPWOrderID(pRequest->pPWOrderID))
	{
		pResponse->pResult = ACCOUNT_HTTP_DUPLICATE_PW_ORDER_ID;
		goto done;
	}
	...
	EARRAY_CONST_FOREACH_BEGIN(eaCurrencies, iCurCurrency, iNumCurrencies);
	{
		...
		eResult = AccountKeyValue_Change("xmlrpc", pAccount, eaCurrencies[iCurCurrency], pRequest->uAmount, &pLock);
	...
	{
		eResult = AccountKeyValue_Commit("xmlrpc", pAccount, eaCurrencies[iCurCurrency], eaLocks[iCurCurrency]);
	...
	pLog = AccountTransactionOpenNonTransacted(pAccount->uID, TransLogType_CashPurchase, NULL, TPROVIDER_PerfectWorld, NULL, NULL, pRequest->pPWOrderID);
	AccountTransactionRecordKeyValueChangesNonTransacted(pLog, eaChanges, "Zen transfer");
	...
	if (eaLocks)
	{
		...
		{
			eResult = AccountKeyValue_Rollback("xmlrpc", pAccount, eaCurrencies[iCurLock], eaLocks[iCurLock]);
			devassert(eResult == AKV_SUCCESS);
...
/***************************************************************************/
/* XML-RPC Method for querying PWOrderID                                   */
/***************************************************************************/

AUTO_STRUCT;
typedef struct XMLRPCPWIsOrderSuccessfulRequest
{
	const char *pPWOrderID; AST(NAME(PWOrderID))
} XMLRPCPWIsOrderSuccessfulRequest;

AUTO_STRUCT;
typedef struct XMLRPCPWIsOrderSuccessfulResponse
{
	const char *pResult; AST(NAME(Result) UNOWNED)
} XMLRPCPWIsOrderSuccessfulResponse;

AUTO_COMMAND ACMD_CATEGORY(XMLRPC) ACMD_ACCESSLEVEL(9) ACMD_NAME("PW::IsOrderSuccessful");
XMLRPCPWIsOrderSuccessfulResponse * XMLRPCPWIsOrderSuccessful(XMLRPCPWIsOrderSuccessfulRequest *pRequest)
{
	XMLRPCPWIsOrderSuccessfulResponse *pResponse = StructCreate(parse_XMLRPCPWIsOrderSuccessfulResponse);
	
	if (!SAFE_DEREF(pRequest->pPWOrderID))
	{
		pResponse->pResult = ACCOUNT_HTTP_NO_ARGS;
	}
	else if (AccountTransactionGetLogByPWOrderID(pRequest->pPWOrderID))
	{
		pResponse->pResult = ACCOUNT_HTTP_SUCCESS;
	}
	else
...
bool AccountIntegration_PWCommonAccountDoEncryptionFixup(SA_PARAM_NN_VALID PWCommonAccount *pAccount)
{
	...
	if (!bRetVal)
	{
		CRITICAL_NETOPS_ALERT("PW_ACCT_FIXUP_FAIL", "While doing encryption fixup on an account, failed because: %s",
...
void CreateTestPWAccount(const char *pwAccountName, const char *forumName)
{
	...
	estrPrintf(&email, "%s@" ORGANIZATION_DOMAIN, pwAccountName);
	accountPWHashPassword(pwAccountName, pwAccountName, pwBuffer);
	ignore = AccountIntegration_CreateOrUpdatePWCommon(pwAccountName, forumName, email, NULL, pwBuffer, NULL, NULL, false);
	estrDestroy(&email);
}
...
void EmailChangePWAccount(const char *pwAccountName, const char *email)
{
	...
	if (!pAccount)
		return;
	ignore = AccountIntegration_CreateOrUpdatePWCommon(pwAccountName, pAccount->pForumName, email, NULL, NULL, NULL, NULL, false);
}

// link a cryptic account to a PW account
AUTO_COMMAND ACMD_ACCESSLEVEL(9) ACMD_CATEGORY(Account_Debug);
SA_RET_NN_STR const char * LinkAccounts(const char *crypticAccount, const char *pwAccountName)
{
	AccountIntegrationResult eResult;
	AccountInfo *pAccount = findAccountByName(crypticAccount);
	PWCommonAccount *pwAccount = findPWCommonAccountByName(pwAccountName);
	if (pAccount == NULL)
	{
		return "Cannot find Cryptic account";
	}
	else if (pwAccount == NULL)
	{
		return "Cannot find PW account";
	}
	else
	{
		eResult = AccountIntegration_LinkAccountToPWCommon(pAccount, pwAccount);
		return getAccountIntegrationResultString(eResult);
	}
}

// unlink a cryptic account to its previously linked PW account
AUTO_COMMAND ACMD_ACCESSLEVEL(9) ACMD_CATEGORY(Account_Debug);
SA_RET_NN_STR const char * UnLinkAccount(const char *crypticAccount)
{
	AccountIntegrationResult eResult;
	AccountInfo *pAccount = findAccountByName(crypticAccount);
	if (pAccount == NULL)
	...
	{
		eResult = AccountIntegration_UnlinkAccountFromPWCommon(pAccount);
		return getAccountIntegrationResultString(eResult);
	}
}
