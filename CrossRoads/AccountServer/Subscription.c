...
{
	if (!stSubscriptionsByName)
		stSubscriptionsByName = stashTableCreateWithStringKeys(100, StashDeepCopyKeys_NeverRelease);

	if (!stSubscriptionsByCategory)
		stSubscriptionsByCategory = stashTableCreateWithStringKeys(100, StashDeepCopyKeys_NeverRelease);
}
