...
static void akvUnlock(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, bool bLazy)
{
	SERVLOG_PAIRS(LOG_ACCOUNT_KEY_VALUE, "AKVUnlock",
		("source", "%s", pProxy ? pProxy : "none")
...
static bool akvLazyUnlock(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_OP_VALID const AccountKeyValuePair *pPair)
{
	if (!pPair)
	{
	...
	{
		akvUnlock(pProxy, pAccount, pKey, true);
		return true;
	}
...
static bool akvPairIsLocked(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_OP_VALID const AccountKeyValuePair *pPair)
{
	if (!pPair)
	{
		pPair = akvGetPair(pAccount, pKey);
	...
	if (sbLazyUnlockOnTest && akvLazyUnlock(pProxy, pAccount, pKey, pPair))
	{
		return false;
	}
...
static void akvLogLock(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_OP_VALID const AccountKeyValuePair *pPair, SA_PARAM_NN_STR const char *pPassword, S64 iValue)
{
...
static void akvLock(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_OP_VALID const AccountKeyValuePair *pPair, SA_PARAM_NN_STR const char *pPassword, S64 iValue)
{
	akvLogLock(pProxy, pAccount, pKey, pPair, pPassword, iValue);
	...
	SA_PARAM_NN_VALID const S64 *piValues)
{
	...
	if (eaSize(&ppKeys) > 6)
	{
		...
		{
			akvLock(pProxy, pAccount, ppKeys[iKey], ppPairs[iKey], pPassword, piValues[iKey]);
		}
		EARRAY_FOREACH_END;
	...
	pKey1 = ppKeys[0];
	iValue1 = piValues[0];
	akvLogLock(pProxy, pAccount, pKey1, ppPairs[0], pPassword, iValue1);

	if (eaSize(&ppKeys) > 1)
	{
		pKey2 = ppKeys[1];
		iValue2 = piValues[1];
		akvLogLock(pProxy, pAccount, pKey2, ppPairs[1], pPassword, iValue2);
	}
	...
	if (eaSize(&ppKeys) > 2)
	{
		pKey3 = ppKeys[2];
		iValue3 = piValues[2];
		akvLogLock(pProxy, pAccount, pKey3, ppPairs[2], pPassword, iValue3);
	}
	...
	if (eaSize(&ppKeys) > 3)
	{
		pKey4 = ppKeys[3];
		iValue4 = piValues[3];
		akvLogLock(pProxy, pAccount, pKey4, ppPairs[3], pPassword, iValue4);
	}
	...
	if (eaSize(&ppKeys) > 4)
	{
		pKey5 = ppKeys[4];
		iValue5 = piValues[4];
		akvLogLock(pProxy, pAccount, pKey5, ppPairs[4], pPassword, iValue5);
	}
	...
	if (eaSize(&ppKeys) > 5)
	{
		pKey6 = ppKeys[5];
		iValue6 = piValues[5];
		akvLogLock(pProxy, pAccount, pKey6, ppPairs[5], pPassword, iValue6);
	}
...
{
	return trhAccountKeyValue_RemoveSix(ATR_PASS_ARGS, pAccount, true, pKey1, pKey2, pKey3, pKey4, pKey5, pKey6);
}
...
static void akvLogCommit(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_NN_VALID const AccountKeyValuePair *pPair)
{
...
static void akvCommit(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_NN_VALID const AccountKeyValuePair *pPair)
{
	akvLogCommit(pProxy, pAccount, pKey, pPair);
	...
	SA_PARAM_NN_NN_VALID const AccountKeyValuePair **ppPairs)
{
	...
	if (eaSize(&ppKeys) > 6)
	{
		EARRAY_FOREACH_BEGIN(ppKeys, iKey);
		{
			akvCommit(pProxy, pAccount, ppKeys[iKey], ppPairs[iKey]);
		}
		EARRAY_FOREACH_END;
	...
	pKey1 = ppKeys[0];
	akvLogCommit(pProxy, pAccount, pKey1, ppPairs[0]);

	if (eaSize(&ppKeys) > 1)
	{
		pKey2 = ppKeys[1];
		akvLogCommit(pProxy, pAccount, pKey2, ppPairs[1]);
	}

	if (eaSize(&ppKeys) > 2)
	{
		pKey3 = ppKeys[2];
		akvLogCommit(pProxy, pAccount, pKey3, ppPairs[2]);
	}

	if (eaSize(&ppKeys) > 3)
	{
		pKey4 = ppKeys[3];
		akvLogCommit(pProxy, pAccount, pKey4, ppPairs[3]);
	}

	if (eaSize(&ppKeys) > 4)
	{
		pKey5 = ppKeys[4];
		akvLogCommit(pProxy, pAccount, pKey5, ppPairs[4]);
	}

	if (eaSize(&ppKeys) > 5)
	{
		pKey6 = ppKeys[5];
		akvLogCommit(pProxy, pAccount, pKey6, ppPairs[5]);
	}
...
static void akvLogRollback(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey)
{
...
static void akvRollback(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey)
{
	akvLogRollback(pProxy, pAccount, pKey);
	AutoTrans_trAccountKeyValue_Unlock(NULL, GLOBALTYPE_ACCOUNTSERVER, GLOBALTYPE_ACCOUNT, pAccount->uID, pKey);
}
	...
	SA_PARAM_NN_NN_VALID const AccountKeyValuePair **ppPairs)
{
	...
	if (eaSize(&ppKeys) > 6)
	{
		EARRAY_FOREACH_BEGIN(ppKeys, iKey);
		{
			akvRollback(pProxy, pAccount, ppKeys[iKey]);
		}
		EARRAY_FOREACH_END;
	...
	pKey1 = ppKeys[0];
	akvLogRollback(pProxy, pAccount, pKey1);

	if (eaSize(&ppKeys) > 1)
	{
		pKey2 = ppKeys[1];
		akvLogRollback(pProxy, pAccount, pKey2);
	}

	if (eaSize(&ppKeys) > 2)
	{
		pKey3 = ppKeys[2];
		akvLogRollback(pProxy, pAccount, pKey3);
	}

	if (eaSize(&ppKeys) > 3)
	{
		pKey4 = ppKeys[3];
		akvLogRollback(pProxy, pAccount, pKey4);
	}

	if (eaSize(&ppKeys) > 4)
	{
		pKey5 = ppKeys[4];
		akvLogRollback(pProxy, pAccount, pKey5);
	}

	if (eaSize(&ppKeys) > 5)
	{
		pKey6 = ppKeys[5];
		akvLogRollback(pProxy, pAccount, pKey6);
	}
...
static void akvRemove(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_NN_VALID const AccountKeyValuePair *pPair)
{
...
AUTO_TRANSACTION
ATR_LOCKS(pAccount, ".Ppkeyvaluepairs[]");
enumTransactionOutcome trAccountConvertKeyValue(ATR_ARGS, NOCONST(AccountInfo) *pAccount, const char *pOldKey, const char *pNewKey)
{
	NOCONST(AccountKeyValuePair) *pOldPair = eaIndexedGetUsingString(&pAccount->ppKeyValuePairs, pOldKey);
	NOCONST(AccountKeyValuePair) *pNewPair = NULL;
	...
	pNewPair = eaIndexedGetUsingString(&pAccount->ppKeyValuePairs, pNewKey);

	if (!pNewPair)
	{
		pNewPair = StructCreateNoConst(parse_AccountKeyValuePair);
		estrCopy2(&pNewPair->key, pNewKey);

		eaIndexedPushUsingStringIfPossible(&pAccount->ppKeyValuePairs, pNewKey, pNewPair);
	}

	pNewPair->iValue += pOldPair->iValue;
	trhAccountKeyValue_RemovePair(ATR_PASS_ARGS, pAccount, pOldKey);
	return TRANSACTION_OUTCOME_SUCCESS;
}

bool AccountKeyValue_ConvertKeyValue(const AccountInfo *account, const char *pOldKey, const char *pNewKey)
{
	const AccountKeyValuePair *pPair = akvGetPair(account, pOldKey);

	if (pPair)
	{
		AutoTrans_trAccountConvertKeyValue(NULL, GLOBALTYPE_ACCOUNTSERVER, GLOBALTYPE_ACCOUNT, account->uID, pOldKey, pNewKey);
		return true;
	}
...
static AccountKeyValueResult akvGetChain(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, bool bAllowLock, SA_PARAM_NN_VALID S64 *piValue)
{
	...
	EARRAY_FOREACH_BEGIN(ppUniqueParts, iPart);
	{
		...
		AccountKeyValue_GetEx(pProxy, pAccount, pPart, bAllowLock, &iPartValue);
		iTotal += iPartValue;
	}
	EARRAY_FOREACH_END;
...
AccountKeyValueResult AccountKeyValue_GetEx(const char *pProxy, const AccountInfo *pAccount, const char *pKey, bool bAllowLock, S64 *piValue)
{
	const AccountKeyValuePair *pPair = NULL;

	if (VirtualCurrency_IsChain(pKey))
	{
		return akvGetChain(pProxy, pAccount, pKey, bAllowLock, piValue);
	}
	...
	if (sbLazyUnlockOnGet && akvLazyUnlock(pProxy, pAccount, pKey, pPair))
	{
		pPair = akvGetPair(pAccount, pKey);
...
bool AccountKeyValue_IsLockedEx(const char *pProxy, const AccountInfo *pAccount, const char *pKey)
{
	return akvPairIsLocked(pProxy, pAccount, pKey, NULL);
}

static AccountKeyValueResult akvPairCanBeLocked(
	SA_PARAM_OP_STR const char *pProxy,
	SA_PARAM_NN_VALID const AccountInfo *pAccount,
	SA_PARAM_NN_STR const char *pKey,
	SA_PARAM_NN_OP_VALID const AccountKeyValuePair **ppPair,
	...
	bool bAllowRelock)
{
	...
	if (*ppPair && akvPairIsLocked(pProxy, pAccount, pKey, *ppPair))
	{
		// If we can't relock, or they didn't provide a password, or the password doesn't match, try lazy unlock
		if (!bAllowRelock || !pPassword || stricmp((*ppPair)->lockData->password, pPassword))
		{
			if (sbLazyUnlockOnLock && akvLazyUnlock(pProxy, pAccount, pKey, *ppPair))
			{
				*ppPair = akvGetPair(pAccount, pKey);
			}
		}

		if (akvPairIsLocked(pProxy, pAccount, pKey, *ppPair))
		{
			...
			if (stricmp((*ppPair)->lockData->password, pPassword))
			{
				return AKV_INVALID_LOCK;
			}
	...
	if (bModify)
	{
		*piValue += akvGetValue(*ppPair, true);
	}
...
AccountKeyValueResult AccountKeyValue_LockEx(const char *pProxy, const AccountInfo *pAccount, const char *pKey, S64 iValue, bool bModify, char **ppInOutPassword, bool bAllowRelock)
{
	...
	eResult = akvPairCanBeLocked(pProxy, pAccount, pKey, &pPair, &iValue, bModify, *ppInOutPassword, bAllowRelock);
	...
	akvLock(pProxy, pAccount, pKey, pPair, *ppInOutPassword, iValue);
	return AKV_SUCCESS;
}

static AccountKeyValueResult akvSetChain(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, S64 iValue, bool bModify, SA_PRE_NN_OP_STR SA_POST_NN_NN_STR char **ppInOutPassword, bool bAllowRelock)
{
	...
	if (!bModify)
	{
		akvGetChain(pProxy, pAccount, pKey, bAllowRelock, &iExistingValue);
	...
	if (iDiff >= 0)
	{
		...
		eResult = akvPairCanBeLocked(pProxy, pAccount, ppUniqueParts[0], &pPair, &piValues[0], true, pPassword, bAllowRelock);
		...
		for (iPart = 1; eResult == AKV_SUCCESS && iPart < eaSize(&ppUniqueParts); ++iPart)
		{
			...
			ea64Push(&piValues, 0);
			eResult = akvPairCanBeLocked(pProxy, pAccount, pPart, &pPair, &piValues[iPart], true, pPassword, bAllowRelock);
			eaPush(&ppPairs, pPair);
		}
	}
	else
	{
		...
		EARRAY_FOREACH_BEGIN(ppUniqueParts, iPart);
		{
			...
			eResult = akvPairCanBeLocked(pProxy, pAccount, pPart, &pPair, &iPartValue, true, pPassword, true);
	...
	if (eResult == AKV_SUCCESS)
	{
		...
		akvLockMultiple(pProxy, pAccount, ppUniqueParts, ppPairs, *ppInOutPassword, piValues);
	}
...
AccountKeyValueResult AccountKeyValue_SetEx(const char *pProxy, const AccountInfo *pAccount, const char *pKey, S64 iValue, bool bModify, char **ppInOutPassword, bool bAllowRelock)
{
	if (VirtualCurrency_IsChain(pKey))
	{
		return akvSetChain(pProxy, pAccount, pKey, iValue, bModify, ppInOutPassword, bAllowRelock);
	}

	return AccountKeyValue_LockEx(pProxy, pAccount, pKey, iValue, bModify, ppInOutPassword, bAllowRelock);
}
	...
	bool bAllowRelock)
{
	...
	if (VirtualCurrency_IsChain(pSrcKey) && VirtualCurrency_IsChain(pDestKey))
	{
		...
		return akvMoveChain(pProxy, pSrcAccount, pSrcKey, pDestAccount, pDestKey, iValue, ppInOutSrcPassword, ppInOutDestPassword, bAllowRelock);
	}

	pSrcPassword = *ppInOutSrcPassword;
	pDestPassword = *ppInOutDestPassword;

	eResult = AccountKeyValue_SetEx(pProxy, pSrcAccount, pSrcKey, -iValue, true, &pSrcPassword, bAllowRelock);
	...
	eResult = AccountKeyValue_SetEx(pProxy, pDestAccount, pDestKey, iValue, true, &pDestPassword, bAllowRelock);

	if (eResult != AKV_SUCCESS)
	{
		akvRollback(pProxy, pSrcAccount, pSrcKey);
	...
	bool bAllowRelock)
{
	...
	EARRAY_FOREACH_BEGIN(ppSrcParts, iPart);
	{
		...
		if (iDupIndex == -1)
		{
			eResult = akvPairCanBeLocked(pProxy, pSrcAccount, pSrcPart, &pPair, &iSrcValue, true, pSrcPassword, true);
		...
		{
			eResult = akvPairCanBeLocked(pProxy, pDestAccount, pDestPart, &pPair, &iSpend, true, pDestPassword, true);
	...
	if (eResult == AKV_SUCCESS)
	{
		...
		akvLockMultiple(pProxy, pSrcAccount, ppSrcParts, ppSrcPairs, *ppInOutSrcPassword, piSrcValues);
		akvLockMultiple(pProxy, pDestAccount, ppDestParts, ppDestPairs, *ppInOutDestPassword, piDestValues);
	}
	...
	bool bAllowRelock)
{
	...
	eResult = akvMove(pProxy, pSrcAccount, pSrcKey, pDestAccount, pDestKey, iValue, &pSrcPassword, &pDestPassword, bAllowRelock);
	eaDestroyEString(&ppPasswordParts);
	...
	SA_PARAM_OP_VALID const AccountKeyValuePair *pPair)
{
	...
	if (!akvPairIsLocked(pProxy, pAccount, pKey, pPair))
	{
...
static AccountKeyValueResult akvFinalizeChain(SA_PARAM_OP_STR const char *pProxy, SA_PARAM_NN_VALID const AccountInfo *pAccount, SA_PARAM_NN_STR const char *pKey, SA_PARAM_NN_STR const char *pPassword, bool bCommit)
{
	const VirtualCurrency *pCurrency = VirtualCurrency_GetByName(pKey);
	char **ppUniqueParts = NULL;
	AccountKeyValueResult eResult = AKV_SUCCESS;
	...
	EARRAY_FOREACH_BEGIN(ppUniqueParts, iPart);
	{
		...
		eResult = akvPairCanBeFinalized(pProxy, pAccount, pKey, pPassword, pPair);
	...
	if (eResult == AKV_SUCCESS)
	{
		if (bCommit)
		{
			...
			akvCommitMultiple(pProxy, pAccount, ppUniqueParts, ppPairs);
		}
		else
		{
			ANALYSIS_ASSUME(ppUniqueParts);
			ANALYSIS_ASSUME(ppPairs);
			akvRollbackMultiple(pProxy, pAccount, ppUniqueParts, ppPairs);
		}
	}
...
AccountKeyValueResult AccountKeyValue_FinalizeEx(const char *pProxy, const AccountInfo *pAccount, const char *pKey, const char *pPassword, bool bCommit)
{
	const AccountKeyValuePair *pPair = akvGetPair(pAccount, pKey);
	AccountKeyValueResult eResult = AKV_FAILURE;
	...
	if (VirtualCurrency_IsChain(pKey))
	{
		return akvFinalizeChain(pProxy, pAccount, pKey, pPassword, bCommit);
	}
	...
	if (bCommit)
	{
		akvCommit(pProxy, pAccount, pKey, pPair);
	}
	else
	{
		akvRollback(pProxy, pAccount, pKey);
	}

	return AKV_SUCCESS;
}

AccountKeyValueResult AccountKeyValue_Finalize(const char *pProxy, const char *pPassword, bool bCommit)
{
	AccountKeyValueResult eResult = AKV_FAILURE;
	...
	if (eResult == AKV_SUCCESS)
	{
		EARRAY_FOREACH_BEGIN(ppPasswordParts, iPart);
		{
			...
			eResult = AccountKeyValue_FinalizeEx(pProxy, pAccount, pKey, pPart, bCommit);
...
AccountKeyValueResult AccountKeyValue_ForceCommit(const char *pProxy, const AccountInfo *pAccount, const char *pKey)
{
	const AccountKeyValuePair *pPair = akvGetPair(pAccount, pKey);
	AccountKeyValueResult eResult = AKV_FAILURE;
	if (!pPair)
	{
		eResult = AKV_NONEXISTANT;
	}
	else if (!akvPairIsLocked(pProxy, pAccount, pKey, pPair))
	{
		eResult = AKV_NOT_LOCKED;
	}
	else
	{
		akvCommit(pProxy, pAccount, pKey, pPair);
...
AccountKeyValueResult AccountKeyValue_RemoveEx(const char *pProxy, const AccountInfo *pAccount, const char *pKey, bool bForce)
{
	const AccountKeyValuePair *pPair = NULL;
	...
	if (akvPairIsLocked(pProxy, pAccount, pKey, pPair) && !bForce)
	{
		return AKV_LOCKED;
	}

	akvRemove(pProxy, pAccount, pKey, pPair);
	return AKV_SUCCESS;
}

AccountKeyValueResult AccountKeyValue_UnlockEx(const char *pProxy, const AccountInfo *pAccount, const char *pKey)
{
	const AccountKeyValuePair *pPair = NULL;
	...
	akvUnlock(pProxy, pAccount, pKey, false);
	return AKV_SUCCESS;
}
