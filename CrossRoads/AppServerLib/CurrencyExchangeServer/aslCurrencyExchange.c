...
CurrencyExchangeNotifyUserOperationResult(CurrencyExchangeResultType result, CurrencyExchangeOperationRequest *operation)
{
	...
	{
		if(operation->uNotifyGatewayServerID == 0)
		{
		...
		{
			RemoteCommand_gslCurrencyExchange_NotifyPlayer(GLOBALTYPE_GATEWAYSERVER, operation->uNotifyGatewayServerID, operation->characterID, result, operation->operationType);
		}
...
HandleCreateOrder(CurrencyExchangeOperationRequest *operation)
{
    ...
    if ( operation->orderType == OrderType_Buy )
    {
        returnVal = LoggedTransactions_CreateManagedReturnValEntInfo("CurrencyExchange", GLOBALTYPE_ENTITYPLAYER, operation->characterID, 
            operation->debugName, operation->gameSpecificLogInfo, HandleCreateOrder_CB, operation);
        AutoTrans_CurrencyExchange_tr_CreateBuyOrder(returnVal, operation->uNotifyGatewayServerID ? GLOBALTYPE_GATEWAYSERVER : GLOBALTYPE_GAMESERVER, GLOBALTYPE_CURRENCYEXCHANGE, operation->accountID, GLOBALTYPE_ENTITYPLAYER, operation->characterID, operation, gCurrencyExchangeConfig.tcNumeric);
    } 
...
HandleClaimTC(CurrencyExchangeOperationRequest *operation)
{
	...
	AutoTrans_CurrencyExchange_tr_ClaimTC(returnVal, operation->uNotifyGatewayServerID ? GLOBALTYPE_GATEWAYSERVER : GLOBALTYPE_GAMESERVER, GLOBALTYPE_CURRENCYEXCHANGE, operation->accountID, GLOBALTYPE_ENTITYPLAYER, 
