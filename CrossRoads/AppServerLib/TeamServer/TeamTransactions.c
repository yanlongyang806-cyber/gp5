...
void aslTeam_trh_CopyToDisconnecteds(TEAM_TRH_ARGS, ATH_ARG NOCONST(Team) *pTeamContainer, ATH_ARG NOCONST(TeamMember) *pTeamMember)
{
	...
	pStubMember->iExpLevel = pTeamMember->iExpLevel;
...
void aslTeam_trh_SetInfo(TEAM_TRH_ARGS, ATH_ARG NOCONST(Team) *pTeamContainer, U32 iEntID, const char *pcMapName, const char *pcMapMsgKey, const char *pcMapVars, U32 iMapInstanceNumber, const char *pcStatus, const char *pchClassName, S32 iLevel, S32 iOfficerRank, U32 uPartitionID, U32 iMapContainerID)
{
	...
	if (pMember) {
		...
		{
			if (pTeamContainer->iGameServerOwnerID!=iMapContainerID || pTeamContainer->iGameServerOwnerPartition!=uPartitionID)
			{
				if (pMember->uOffMapTimeStamp==0)
				{
					pMember->uOffMapTimeStamp=timeSecondsSince2000();	// Possible discrepency between TeamServer time and GameServer time
																	// Not to mention possible 30 sec. delay in being reported off map/on map
				}
			}
			else
			{
				pMember->uOffMapTimeStamp=0; 
			}
		}
...
void aslTeam_DestroyWithReason(U32 iTeamID, const char* pchReason)
{
	...
	// Make sure we interrupt any pending vote kick
...
// ForceLeave is used when updating owned teams. It has no functionality of its own at this point and is just a wrapper on RemoteCommand_CB
//  (which needs to be called to do logging and also to manage chat channel stuff)
void aslTeam_ForceLeave_CB(TransactionReturnVal *pReturn, ASLTeamCBData *pData)
{
	aslTeam_RemoteCommand_CB(pReturn, pData);
}
...
// RemoteCommand_CB gets called to do logging and also to manage chat channel stuff
	aslTeam_RemoteCommand_CB(pReturn, pData);
}
...
AUTO_TRANSACTION
ATR_LOCKS(pTeamContainer, ".Icontainerid, .Eamembers, .IGameServerownerid, .Igameserverownerpartition");
...
AUTO_COMMAND_REMOTE;
void aslTeam_SetRequiresVoteKick(U32 iTeamID, int iRequiresVoteKick)
{
	static char pcActionType[] = "SetRequiresVoteKick";
	TransactionReturnVal *pReturn = NULL;
	Team *pTeam = aslTeam_GetTeam(iTeamID);
	...
	{
		pReturn = objCreateManagedReturnVal(aslTeam_RemoteCommand_CB, aslTeam_MakeCBData(pcActionType));
		AutoTrans_aslTeam_tr_SetRequiresVoteKick(pReturn, GetAppGlobalType(), GLOBALTYPE_TEAM, pTeam->iContainerID, iRequiresVoteKick);
	}
}

// Set the team's requirement for vote kicking
AUTO_TRANSACTION
ATR_LOCKS(pTeamContainer, ".bVoteRequiredForKick");
enumTransactionOutcome aslTeam_tr_SetRequiresVoteKick(ATR_ARGS, NOCONST(Team) *pTeamContainer, int iRequiresVoteKick)
{
	TEAM_TRANSACTION_INIT;
	
	if (iRequiresVoteKick==0)
	{
		pTeamContainer->bVoteRequiredForKick=false;
	}
	else
	{
		pTeamContainer->bVoteRequiredForKick=true;
	}
	TEAM_TRANSACTION_RETURN_SUCCESS;
}
...
AUTO_TRANSACTION
ATR_LOCKS(pTeamContainer, ".Igameserverownerid, .igameserverownerpartition, .Eamembers, .Emode, .Loot_Mode, .Loot_Mode_Quality, .Idifficulty, .pchStatusMessage");
enumTransactionOutcome aslTeam_tr_ReleaseOwned(ATR_ARGS, NOCONST(Team) *pTeamContainer)
{
	...
	if (NONNULL(pTeamContainer)) 
	{
		if (pTeamContainer->iGameServerOwnerID != 0)
		{
			...
			pTeamContainer->iGameServerOwnerPartition = 0;
...
static void aslTeam_Create_Internal(TransactionReturnVal *pReturn, ASLTeamCBData *pData)
{
	...
	pNewTeam->iGameServerOwnerPartition = pData->iPartitionID;	// Should only be being used by owned teams from aslTeam_OwnedTeamCreate.
...
void aslTeam_OwnedTeamUpdate(U32 iTeamID, IntEarrayWrapper *pList, U32 uiRequestID)
{
	...
	for (i=0;i<eaSize(&pTeam->eaMembers);i++)
	{
		...
		if (!bFound)
		{
			...
			pData = aslTeam_MakeCBData("ForceLeave");
			pData->iEntID = iMemberID;
			pData->iTeamID = iTeamID;
			pReturn = objCreateManagedReturnVal(aslTeam_ForceLeave_CB, pData);
			
			AutoTrans_aslTeam_tr_Leave(pReturn, GetAppGlobalType(),
	...
	{
		ContainerID iNewMemberID = (ContainerID)pList->eaInts[j];
		// bhanka originally wrote this code to do a !IsMember check and then call the CheckIsNotMember. This is redundant.
		// Given that this is "force join", lets make it operate silently.
		// (Removed the team size check as well and let the tr handle it so we log the error)
		// The 'real' change here is to add the check for disconnecteds. Otherwise we would attempt to ForceJoin to a team we were already on.

		if (!aslTeam_IsMember(pTeam, iNewMemberID) && !aslTeam_IsOnDisconnecteds(pTeam, iNewMemberID))
		{
			int iWasDisconnected=0;
			U32 iOldTeam=0;
			TransactionReturnVal *pReturn = NULL;
			ASLTeamCBData *pData = aslTeam_MakeCBData("ForceJoin");	// We do not run this through the normal Remote CB. No actions are processed
			...
			if (aslTeam_GetTeam(iOldTeam)==NULL || iOldTeam==iTeamID)
			{
				// Safety checks. Theoretically this should never happen, but just in case
