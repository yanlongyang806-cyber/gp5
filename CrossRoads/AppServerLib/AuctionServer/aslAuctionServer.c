...
static INT_EARRAY s_ea32ForceExpireLotIDs = NULL;
...
static INT_EARRAY sMailedLotIDs = NULL;
static bool s_bMailFixupCanStart = false;
static bool s_bMailFixupDone = false;
...
AUTO_CMD_INT(gLotsExpireCheckPerTick, LotsExpireCheckPerTick) ACMD_COMMANDLINE ACMD_AUTO_SETTING(Auction, AUCTIONSERVER);

U32 gLotsExpireSecondsPerTick = SECONDS_PER_AUCTION_EXPIRE_TICK;
AUTO_CMD_INT(gLotsExpireSecondsPerTick, LotsExpireSecondsPerTick) ACMD_COMMANDLINE ACMD_AUTO_SETTING(Auction, AUCTIONSERVER);

S32 g_AnomalousSaleAlertThreshold = PRICE_HISTORY_ANOMALY_THRESHOLD;
AUTO_CMD_INT(g_AnomalousSaleAlertThreshold, AnomalousSaleAlertThreshold) ACMD_COMMANDLINE ACMD_AUTO_SETTING(Auction, AUCTIONSERVER);
...
typedef struct AuctionExpireCBData
{
	ContainerID lotID;
	bool bForceExpire;
}AuctionExpireCBData;
...
static void
	MailFixupLots()
{
	s_bMailFixupCanStart = true;
	printf("Starting fixup of auction %d mail auction lots.\n", eaiSize(&sMailedLotIDs));
}
...
static void AuctionDeleteLot_CB(TransactionReturnVal *pReturn, ContainerID *piLotID)
{
	AuctionLot *pLot = AuctionServer_GetLot(*piLotID);
...
static bool AuctionMailFixupDone(void)
{
	return s_bMailFixupDone;
}

static void DoAuctionLotMailFixups(void)
{
	U32 uFixupTime = 0; // This doesn't do anything at all. If it were static, on the other hand,
						// this would make it so that this fixup is called with two second intervals
						// from the end of one call to the beginning of another. If we want that,
						// make this static

	if(!s_bMailFixupCanStart)
	{
		return;
	}
	...
	else if(timeSecondsSince2000() >= uFixupTime)
	{
		...
		while(iCount < 1000 && ea32Size(&sMailedLotIDs) > 0)
		{
			U32 lotID = ea32Pop(&sMailedLotIDs);
			AuctionLot *pLot = AuctionServer_GetLot(lotID);
			S32 *piLotID = malloc(sizeof(ContainerID));
			TransactionReturnVal *pReturn;

			*piLotID = lotID;
			pReturn = objCreateManagedReturnVal(AuctionDeleteLot_CB, piLotID);
			...
			AutoTrans_auction_tr_SendAuctionLotThroughMail(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_AUCTIONLOT, lotID, GLOBALTYPE_ENTITYPLAYER, pLot->ownerID);
			++iCount;
		}

		if(ea32Size(&sMailedLotIDs) == 0)
		{
			s_bMailFixupDone = true;
			printf("\nAll mail auction lots deleted\n");
			InvFixupLots();
			return;
		}
		uFixupTime = timeSecondsSince2000() + 2;	// 1000 lots per ~2 seconds
	}
...
AUTO_COMMAND_REMOTE;
void Auction_RecordSalePriceForItem(const char* pchName, S32 iPricePerUnit, U32 iSellerContainerID, U32 iBuyerContainerID)
{
	...
	if (!gAuctionConfig.bEnablePriceHistoryTracking || s_bDisablePriceHistories || !pchName || !pchName[0])
		return;
	...
	if (eaiSize(&pHistory->eaiLastSalePrices) > 0)
	{
		U64 iPreviousAvgPrice = pHistory->uiCachedSum/eaiSize(&pHistory->eaiLastSalePrices);

		if ((U64)iPricePerUnit >= iPreviousAvgPrice*g_AnomalousSaleAlertThreshold)
		{
			//This item sold for so much that we're going to issue a shard alert about it.
			//This is getting replaced with a report.
			//TriggerAlertf("AUCTION_PRICE_ANOMALY", ALERTLEVEL_WARNING, ALERTCATEGORY_GAMEPLAY, 0, GLOBALTYPE_ENTITYPLAYER, iSellerContainerID, GetAppGlobalType(), GetAppGlobalID(), NULL, 0, "An auction for item %s had a per-unit price of %d, which is %llux greater than the recent average price of %llu. Seller container ID: %d. Buyer container ID: %d.", pchName, iPricePerUnit, iPricePerUnit/iPreviousAvgPrice, iPreviousAvgPrice, iSellerContainerID, iBuyerContainerID);
			return;
		}
	}

	pHistory->uiCachedSum -= eaiGet(&pHistory->eaiLastSalePrices, pHistory->uNextWriteIndex);
	pHistory->uiCachedSum += iPricePerUnit;
...
int aslAuction_CheckPriceHistoryForItem(const char* pchName)
{
	...
	if (pHistory && eaiSize(&pHistory->eaiLastSalePrices) > 0)
		return pHistory->uiCachedSum/eaiSize(&pHistory->eaiLastSalePrices);

	return 0;
}

static void ExpireAuctionSuccess(AuctionLot *pLot, bool bForced)
{
    s_TotalLotsExpiredSuccess++;
    if(pLot)
    {
        if (!bForced && pLot->pBiddingInfo && pLot->pBiddingInfo->iCurrentBid > 0 && pLot->pBiddingInfo->iBiddingPlayerContainerID > 0)
        {
            //This expiration was actually a sale
            Auction_RecordSalePriceForItem(pLot->ppItemsV2[0]->pchItemName, ceilf(((F32)pLot->pBiddingInfo->iCurrentBid)/pLot->ppItemsV2[0]->slot.pItem->count), pLot->ownerID, pLot->pBiddingInfo->iBiddingPlayerContainerID);
        }
...
static void
LogAuctionLotAction(AuctionLot *pLot, const char *action, const char *reason)
{
    static char *estrDetail = NULL;
    Item* pItem = pLot->ppItemsV2 && pLot->ppItemsV2[0] ? pLot->ppItemsV2[0]->slot.pItem : NULL;
    S32 count = 0;
    ...
    {
        bidderID = pLot->pBiddingInfo->iBiddingPlayerContainerID;
        lastBid = pLot->pBiddingInfo->iCurrentBid;
    }

    if ( pItem )
    {
        count = pItem->count;
        logicalName = REF_STRING_FROM_HANDLE(pItem->hItem);
    }

    log_printf(LOG_AUCTION, "%s(reason \"%s\" auctionID %u OwnerID %u BidderID %u LastBid %d ItemName %s Count %d)", action, reason, pLot->iContainerID, pLot->ownerID, bidderID, lastBid, logicalName, count);
}

static void ExpireAuctionNoOwner_CB(TransactionReturnVal *pReturn, AuctionExpireCBData *pData)
{
    ContainerID iLotID = pData->lotID;
    AuctionLot *pLot = AuctionServer_GetLot(iLotID);
    s_ExpiredLotCompleteCount++;

    if(pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS && iLotID)
    {
        if ( pLot )
        {
            ExpireAuctionSuccess(pLot, pData->bForceExpire);
            LogAuctionLotAction(pLot, "AuctionExpireRetry", "Auction Expiration Retry for No Owner Succeeded");
        }
    }
    else if(pReturn->eOutcome == TRANSACTION_OUTCOME_FAILURE && pReturn && pLot)
    {
        // If the retry failed, then just destroy the auction lot.
        objRequestContainerDestroy(NULL, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, GLOBALTYPE_AUCTIONSERVER, 0);
        LogAuctionLotAction(pLot, "AuctionContainerDeletion", "Auction Expiration Retry for No Owner Failed");
    }
	free(pData);
}

static void ExpireAuctionNoBuyer_CB(TransactionReturnVal *pReturn, AuctionExpireCBData *pData)
{
    ContainerID iLotID = pData->lotID;
    AuctionLot *pLot = AuctionServer_GetLot(iLotID);
    s_ExpiredLotCompleteCount++;
    ...
    if(pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS && iLotID)
    {
        ...
        {
            ExpireAuctionSuccess(pLot, pData->bForceExpire);
            LogAuctionLotAction(pLot, "AuctionExpireRetry", "Auction Expiration Retry for No Buyer Succeeded");
        }
    }
    else if(pReturn->eOutcome == TRANSACTION_OUTCOME_FAILURE && pReturn && pLot)
    {
        // If the retry failed, then just destroy the auction lot.
        objRequestContainerDestroy(NULL, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, GLOBALTYPE_AUCTIONSERVER, 0);
        LogAuctionLotAction(pLot, "AuctionContainerDeletion", "Auction Expiration Retry for No Buyer Failed");
    }
	free(pData);
}

static void ExpireAuction_CB(TransactionReturnVal *pReturn, AuctionExpireCBData *pData)
{
    ContainerID iLotID = pData->lotID;
	AuctionLot *pLot = AuctionServer_GetLot(iLotID);
	s_ExpiredLotCompleteCount++;
	...
	else if(pReturn->eOutcome == TRANSACTION_OUTCOME_FAILURE && pReturn && pLot)
	{
        ...
        // Don't attempt to re-expire or delete mailed lots
		if(pLot->state != ALS_Mailed)
		{
			...
			{
                const char *returnString = pReturn->pBaseReturnVals[i].returnString;

				if(pReturn->pBaseReturnVals[i].eOutcome == TRANSACTION_OUTCOME_FAILURE && returnString &&
					strstri(returnString, OBJ_TRANS_CONTAINER_DOES_NOT_EXIST))
				{
                    bool unfoundEntity;
                    const char *tmpStr;
                    bool delete = true;

                    // Make sure the container we didn't find was a player.
                    unfoundEntity = ( strstri(returnString, "GlobalType=EntityPlayer") != NULL );

                    // Get the ID of the container we didn't find as a string.
                    tmpStr = strstri(returnString, " ID=");

                    // Transaction failed because one of the containers didn't exists.
                    if ( pLot->pBiddingInfo && pLot->pBiddingInfo->iNumBids > 0 && unfoundEntity && tmpStr)
                    {
                        // There was bidding and one of the entities was not found.
                        ContainerID unfoundEntityID;

                        // Skip past " ID=" to the actual ID
                        tmpStr = tmpStr + 4;

                        // Convert ID string to an int.
                        unfoundEntityID = atoi(tmpStr);
                        if ( unfoundEntityID == pLot->ownerID )
                        {
                            // Owner wasn't found, so expire the lot without him.
                            ItemChangeReason buyerReason = {0};

                            pReturn = LoggedTransactions_CreateManagedReturnVal("AuctionLot", ExpireAuctionNoOwner_CB, pData);

                            AuctionLot_GetGameEconomyDetailString(pLot, &estrDetail);

                            inv_FillItemChangeReason(&buyerReason, NULL, "Auction:Expire:Buyer:NoSeller", estrDetail);

							if (pData->bForceExpire)
								AutoTrans_auction_tr_ForceExpireAuction(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, 0, GLOBALTYPE_ENTITYPLAYER, pLot->pBiddingInfo ? pLot->pBiddingInfo->iBiddingPlayerContainerID : 0, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, &buyerReason, NULL);
							else
								AutoTrans_auction_tr_ExpireAuctionWithLastBidder(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, 0, GLOBALTYPE_ENTITYPLAYER, pLot->pBiddingInfo->iBiddingPlayerContainerID, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, NULL, &buyerReason);

                            estrDestroy(&estrDetail);
                            delete = false;
                            bRetried = true;

                            // Don't count this lot as completed since we are retrying.
                            s_ExpiredLotCompleteCount--;
                        }
                        else if ( unfoundEntityID == pLot->pBiddingInfo->iBiddingPlayerContainerID )
                        {
                            // Bidder wasn't found, so expire the lot without him.
                            ItemChangeReason sellerReason = {0};
                            pReturn = LoggedTransactions_CreateManagedReturnVal("AuctionLot", ExpireAuctionNoBuyer_CB, pData);

                            AuctionLot_GetGameEconomyDetailString(pLot, &estrDetail);

                            inv_FillItemChangeReason(&sellerReason, NULL, "Auction:Expire:Seller:NoBuyer", estrDetail);

							if (pData->bForceExpire)
								AutoTrans_auction_tr_ForceExpireAuction(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, pLot->ownerID, GLOBALTYPE_ENTITYPLAYER, 0, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, NULL, &sellerReason);
							else
								AutoTrans_auction_tr_ExpireAuctionWithLastBidder(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, pLot->ownerID, GLOBALTYPE_ENTITYPLAYER, 0, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, &sellerReason, NULL);
                        ...
                        // If neither of the above cases apply, then we will fall through to the delete code below.
                    }
                    
                    if ( delete )
                    {
                        // If there was no bidding then the seller was not found, so just delete the lot.
					    objRequestContainerDestroy(NULL, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, GLOBALTYPE_AUCTIONSERVER, 0);
                        LogAuctionLotAction(pLot, "AuctionContainerDeletion", "Auction Expiration failed with no owner and no bids");
						bDestroyed = true;
						free(pData);
...
void ExpireAuctionLots(void)
{
	...
	while (iNumLotsChecked < gLotsExpireCheckPerTick && (pLot = objIndexGetNextContainerData(&iter, pStore)))
	{
			...
			)
		{
			...
			if(iCurTime > uExpireTime) 
			{
				...
				ItemChangeReason buyerReason = {0};
				ItemChangeReason sellerReason = {0};
				AuctionExpireCBData* pData = malloc(sizeof(AuctionExpireCBData));

				pData->lotID = pLot->iContainerID;
				pData->bForceExpire = false;
				...
				pReturn = LoggedTransactions_CreateManagedReturnVal("AuctionLot", ExpireAuction_CB, pData);

				AuctionLot_GetGameEconomyDetailString(pLot, &estrDetail);
				
                inv_FillItemChangeReason(&sellerReason, NULL, "Auction:Expire:Seller", estrDetail);
                inv_FillItemChangeReason(&buyerReason, NULL, "Auction:Expire:Buyer", estrDetail);

				if (pLot->pBiddingInfo && pLot->pBiddingInfo->iBiddingPlayerContainerID > 0 && pLot->pBiddingInfo->iCurrentBid <= INT_MAX)
                {
					AutoTrans_auction_tr_ExpireAuctionWithLastBidder(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, pLot->ownerID, GLOBALTYPE_ENTITYPLAYER, pLot->pBiddingInfo->iBiddingPlayerContainerID, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, &sellerReason, &buyerReason);
                }
				else
				{
					AutoTrans_auction_tr_ExpireAuctionNoBids(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, pLot->ownerID, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID);
				}
	...
	while(iNumLotsChecked < gLotsExpireCheckPerTick && ea32Size(&s_ea32ForceExpireLotIDs) > 0)
	{
		pLot = AuctionServer_GetLot(ea32Pop(&s_ea32ForceExpireLotIDs));
		if (pLot && pLot->state == ALS_Open)
		{
			char *estrDetail = NULL;
			ItemChangeReason buyerReason = {0};
			ItemChangeReason sellerReason = {0};
			...
			//Forcibly expire this lot, return the winning bid amount (if any) to its owner.
			AutoTrans_auction_tr_ForceExpireAuction(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, pLot->ownerID, GLOBALTYPE_ENTITYPLAYER, pLot->pBiddingInfo ? pLot->pBiddingInfo->iBiddingPlayerContainerID : 0, GLOBALTYPE_AUCTIONLOT, pLot->iContainerID, &buyerReason, &sellerReason);
...
{	
	...
	// This commented block of code gathers all mailed auction lots and puts them in a list to be mailed out into the
	// new mail system, and then deletes the auction lots. This migration was originally desired, but was subsequently
	// deemed too expensive to run on a live shard. Uncomment this if we ever want to run it again.
	/*else if (pLot->state == ALS_Mailed)
	{
		// collect mailed lots, so that they can be put into the new mail system and then deleted
		ea32Push(&sMailedLotIDs, pLot->iContainerID);
	}*/
	else
	{
	...
	for (iChunk = 0; iChunk < eaSize(&s_pLocalPriceHistories->eaPriceChunks); iChunk++)
	{
		for (j = 0; j < eaSize(&s_pLocalPriceHistories->eaPriceChunks[iChunk]->eaPrices); j++)
		{
			...
			if (iNumPrices > 0)
			{
				...
				{
					s_pLocalPriceHistories->eaPriceChunks[iChunk]->eaPrices[j]->uiCachedSum += s_pLocalPriceHistories->eaPriceChunks[iChunk]->eaPrices[j]->eaiLastSalePrices[k];
				}
			}
		}
	}
...
//		RemoteCommand_InformControllerOfServerState(GLOBALTYPE_CONTROLLER, 0, GetAppGlobalType(), gServerLibState.containerID, "ready");
	...
	{
		if(AuctionFixupsDone() && AuctionMailFixupDone())
		{
			// Expire any auction lots that have gone over time
			// Don't start expiring until all the containers are loaded
			if(timeSecondsSince2000() - iLastExpireTime >= gLotsExpireSecondsPerTick)
			{
		...
		else if (!AuctionMailFixupDone())
		{
			DoAuctionLotMailFixups();
		}
		else
		{
			DoAuctionLotFixups();
...
void AuctionPriceHistoryLoadTesting(int iNumPricesPerItemDef, int minPrice, int maxPrice)
{
	...
	while(pDef = (ItemDef*)RefSystem_GetNextReferentFromIterator(&iter))
	{
		...
		for (i = 0; i < iNumPricesPerItemDef; i++)
			Auction_RecordSalePriceForItem(pDef->pchName, randomIntRange(minPrice, maxPrice), 0, 0);
	}
}
...
//Forcibly expire all auctionlots that belong to a given character.
AUTO_COMMAND ACMD_CATEGORY(Auction) ACMD_ACCESSLEVEL(9);
void Auction_ForceExpireAllLotsOwnedByCharacter(U32 iContainerID)
{
	AuctionLotList searchLotList = {0};
	AuctionSearchRequest request = {0};
	char *estrSearchType = NULL;
	int i;

	request.ownerID = iContainerID;
	request.itemQuality = -1;

	AuctionServer_SearchLots(&searchLotList, &request, Auction_FilterForSearch, &estrSearchType);

	for (i = 0; i < eaSize(&searchLotList.ppAuctionLots); i++)
	{
		ea32Push(&s_ea32ForceExpireLotIDs, searchLotList.ppAuctionLots[i]->iContainerID);
	}

	estrDestroy(&estrSearchType);
}

//Forcibly expire all auctionlots in the shard. This will return the items back to the poster of the auction lot and the currency back to the highest bidder, if any.
AUTO_COMMAND ACMD_CATEGORY(Auction) ACMD_ACCESSLEVEL(9);
void Auction_ForceExpireAllLots()
{
	ContainerIterator iter;
	AuctionLot *pLot;
	...
	while ( pLot = objGetNextObjectFromIterator(&iter) )
	{
		ea32Push(&s_ea32ForceExpireLotIDs, pLot->iContainerID);
	}
	objClearContainerIterator(&iter);
}
