...
// This is the maximum length a search string could possibly be
static U32 s_MaxSearchString = 127;

//__CATEGORY Auction Server settings
// This is the maximum number of auction logs that an auction search will return.
static U32 s_MaxAuctionReturnResults = 400;
AUTO_CMD_INT(s_MaxAuctionReturnResults, MaxAuctionReturnResults) ACMD_AUTO_SETTING(Auction, AUCTIONSERVER);

// This is the maximum number of auction lots that a single search will traverse.
int gMaxElementsToSearch = 200000;
AUTO_CMD_INT(gMaxElementsToSearch, MaxElementsToSearch) ACMD_AUTO_SETTING(Auction, AUCTIONSERVER) ACMD_COMMANDLINE;

int gMaxCachedStringTables = 1000;
AUTO_CMD_INT(gMaxCachedStringTables, MaxCachedStringTables) ACMD_AUTO_SETTING(Auction, AUCTIONSERVER) ACMD_COMMANDLINE;
...
bool Auction_FilterForSearch(AuctionLot *pLot, AuctionSearchRequest *pRequest)
{
	...
	//Don't return lots with invalid buyout/bid values, unless we're doing an ownerID search (which means it's the "Your active auctions" window.)
	if (pRequest->ownerID == 0 && (pLot->price > INT_MAX || (pLot->pBiddingInfo && pLot->pBiddingInfo->iCurrentBid > INT_MAX)))
		return false;
	...
	if(pRequest->pchItemSortTypeCategoryName && pRequest->pchItemSortTypeCategoryName != pSlot->pchItemSortTypeCategoryName)
	{
		return false;
	}
...
typedef struct PendingMailLots
{
	U32 iEntityID;
	U32 iAccountID;
	U32 *uPendingLotIds;
...
static U32 *s_eaActiveMailMigration = NULL;
int g_iMaxLotMigrationBatch = 40;
AUTO_CMD_INT(g_iMaxLotMigrationBatch, MaxLotMigrationBatch) ACMD_COMMANDLINE;

void Auction_Migrate_Some_MailAuctionLotsToEmailV3(PendingMailLots *pPendingLots);

void Auction_Migrate_Some_MailAuctionLotsToEmailV3_CB(TransactionReturnVal *pReturn, PendingMailLots *pPendingMailLots)
{
	if (pReturn->eOutcome == TRANSACTION_OUTCOME_FAILURE)
	{
		pPendingMailLots->bFailed = true;
	}

	// Mail more pending lots or handle completion
	Auction_Migrate_Some_MailAuctionLotsToEmailV3(pPendingMailLots);
}
...
void Auction_Migrate_Some_MailAuctionLotsToEmailV3(PendingMailLots *pPendingLots)
{
	...
	if (ea32Size(&pPendingLots->uPendingLotIds) > 0)
	{
		U32 *uMailBatch = NULL;

		// Convert a limited number of pending lots at a time
		while (ea32Size(&uMailBatch) < g_iMaxLotMigrationBatch && ea32Size(&pPendingLots->uPendingLotIds) > 0)
		{
			ea32Push(&uMailBatch, ea32Pop(&pPendingLots->uPendingLotIds));
		}

		pReturn = LoggedTransactions_CreateManagedReturnVal("EmailV3LostAuctionLots", Auction_Migrate_Some_MailAuctionLotsToEmailV3_CB, pPendingLots);
		AutoTrans_auction_tr_MailLostLots(pReturn, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_AUCTIONLOT, &uMailBatch, GLOBALTYPE_ENTITYSHAREDBANK, pPendingLots->iAccountID);
		ea32Destroy(&uMailBatch);
	}
	else
	{
		if (pPendingLots->uFoundResults >= MAXIMUM_MAIL_LOTS)
		{
			// If the search limit was reached when searching for lots
			// there may be more, so need to perform the search again
			pPendingLots->uFoundResults = AuctionServer_SearchLotsMailID(pPendingLots->iAccountID, &pPendingLots->uPendingLotIds);
			if (pPendingLots->uFoundResults > 0)
			{
				Auction_Migrate_Some_MailAuctionLotsToEmailV3(pPendingLots);
				return;
			}
		}

		if (!pPendingLots->bFailed)
		{
			// Only mark completion if there weren't any failures
			AutoTrans_auction_tr_MailLostLots_Complete(NULL, GLOBALTYPE_AUCTIONSERVER, GLOBALTYPE_ENTITYPLAYER, pPendingLots->iEntityID);
		}

		// Cleanup pending lots
		ea32FindAndRemove(&s_eaActiveMailMigration, pPendingLots->iAccountID);
		free(pPendingLots);
	}
}

AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void Auction_Migrate_MailAuctionLotsToEmailV3(S32 iEntityID, S32 iAccountID)
{
	PendingMailLots *pPending;

	if (ea32Find(&s_eaActiveMailMigration, iAccountID) >= 0)
	{
		// Ignore duplicate requests
		return;
	}

	pPending = callocStruct(PendingMailLots);
	pPending->iEntityID = iEntityID;
	pPending->iAccountID = iAccountID;

	pPending->uFoundResults = AuctionServer_SearchLotsMailID(iAccountID, &pPending->uPendingLotIds);

	ea32Push(&s_eaActiveMailMigration, iAccountID);
	Auction_Migrate_Some_MailAuctionLotsToEmailV3(pPending);
}
...
static int Auction_SortByBuyoutHelper(const AuctionSearchRequest *pRequest, const AuctionLot **pLotA, const AuctionLot **pLotB, bool bDescending)
{
	S32 iPriceComp;
	S32 iItemCountA;
	S32 iItemCountB;
	...
	iItemPriceA = (*pLotA)->price;
	iItemPriceB = (*pLotB)->price;

	if ((*pLotA)->price && !(*pLotB)->price)
	{
		return -1;
	}
	else if (!(*pLotA)->price && (*pLotB)->price)
	{
		return 1;
	}
	...
	{
		return bDescending ? iPriceComp : -iPriceComp;
	}
}


static int Auction_SortByBuyout(const AuctionSearchRequest *pRequest, const AuctionLot **pLotA, const AuctionLot **pLotB)
{
	return Auction_SortByBuyoutHelper(pRequest, pLotA, pLotB, false);
}

static int Auction_SortByBuyoutDesc(const AuctionSearchRequest *pRequest, const AuctionLot **pLotA, const AuctionLot **pLotB)
{
	return Auction_SortByBuyoutHelper(pRequest, pLotA, pLotB, true);
}

static int Auction_SortByBid(const AuctionSearchRequest *pRequest, const AuctionLot **pLotA, const AuctionLot **pLotB)
{
	S32 iPriceComp;
	S32 iItemCountA;
	S32 iItemCountB;
	U32 iItemPriceA = (*pLotA)->pBiddingInfo ? MAX((*pLotA)->pBiddingInfo->iCurrentBid, (*pLotA)->pBiddingInfo->iMinimumBid) : INT_MAX;
	U32 iItemPriceB = (*pLotB)->pBiddingInfo ? MAX((*pLotB)->pBiddingInfo->iCurrentBid, (*pLotB)->pBiddingInfo->iMinimumBid) : INT_MAX;
...
static int Auction_SortByBidDesc(const AuctionSearchRequest *pRequest, const AuctionLot **pLotA, const AuctionLot **pLotB)
{
	return -Auction_SortByBid(pRequest, pLotA, pLotB);
}
...
ACMD_STATIC_RETURN AuctionLotList* Auction_GetLots(AuctionSearchRequest *pRequest)
{
	...
	{	//TODO: We'll need to explode this searchstring for tokens and search on each
		...
		if(strlen(pRequest->stringSearch) > s_MaxSearchString)
		{
			snprintf(buf,s_MaxSearchString,"%s",pRequest->stringSearch);

			StructFreeString(pRequest->stringSearch);

			pRequest->stringSearch = StructAllocString(buf);
		}
	...
	if (s_pSearchLotList->ppAuctionLots && eaSize(&s_pSearchLotList->ppAuctionLots)) {
		...
		case AuctionSort_Buyout:
			eaQSort_s(s_pSearchLotList->ppAuctionLots, Auction_SortByBuyout, pRequest);
			break;
		case AuctionSort_BuyoutDesc:
			eaQSort_s(s_pSearchLotList->ppAuctionLots, Auction_SortByBuyoutDesc, pRequest);
			break;

		case AuctionSort_Bid:
			eaQSort_s(s_pSearchLotList->ppAuctionLots, Auction_SortByBid, pRequest);
			break;
		case AuctionSort_BidDesc:
			eaQSort_s(s_pSearchLotList->ppAuctionLots, Auction_SortByBidDesc, pRequest);
			break;
    ...
    // Only return s_MaxAuctionReturnResults
    if ( s_MaxAuctionReturnResults && ( eaSize(&s_pSearchLotList->ppAuctionLots) > (int)s_MaxAuctionReturnResults ) )
    {
        eaSetSize(&s_pSearchLotList->ppAuctionLots, s_MaxAuctionReturnResults);
    }
	s_pSearchLotList->maxResults = s_MaxAuctionReturnResults;
