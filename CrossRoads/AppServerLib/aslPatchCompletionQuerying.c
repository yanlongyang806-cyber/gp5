...
typedef struct PatchCompletionQueryingNS
{
	...
	PatchCompletionQueryingCB pFailCB; NO_AST
...
void aslPatchConnectionCB(PCL_Client * client, bool updated, PCL_ErrorCode error, const char * error_details, void * userData)
{
	...
	else
	{
		...
		pclSetNamedView(client, spPatchInfo->pResourceProject, spPatchInfo->pViewName, /*getManifest=*/false, /*saveTrivia=*/false, aslPatchViewSetCB, NULL);
	}
}
...
void aslPatchCompletionQuerying_InternalCB(PCL_Client *client, bool synced, bool exists, void *userdata)
{
	...
	if(!exists)
	{
		...
		pNameSpace->pFailCB(pNameSpace->pUserData);
...
void aslPatchCompletionQuerying_Query(char *pNameSpace, PatchCompletionQueryingCB pSuccessCB, PatchCompletionQueryingCB pFailCB, void *pUserData)
{
