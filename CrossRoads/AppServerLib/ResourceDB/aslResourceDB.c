...
// Send all the resource types that are persisted on an entity.
//
// NOTE: Don't call this for more than one resource type.  Otherwise,
// you'll do extra network send calls.
void SendEntityPersistedResources(ResourceDBNameSpace *pNameSpace, GlobalType eServerType, ContainerID iServerID, char *pComment)
{
	int i;
	if( !pNameSpace ) {
		return;
	...
	for( i = 0; i < eaSize( &pNameSpace->ppItemDefs ); ++i ) {
		RemoteCommand_ResourceDB_ReceiveItemDef(eServerType, iServerID, pNameSpace->ppItemDefs[i]->pchName, pNameSpace->ppItemDefs[i], pComment );
	}
}
...
void SendResourceToRequestingServer(ResourceDBNameSpace *pNameSpace, GlobalType eServerType, ContainerID iServerID, eResourceDBResourceType eType, char *pResourceName, void *pResource, char *pComment)
{
	...
	case RESOURCETYPE_MISSIONDEF:
		...
		SendEntityPersistedResources(pNameSpace, eServerType, iServerID, pComment);
...
void BeginResourceCheckVerification(char *pNameSpaceName, GlobalType eServerType, ContainerID iServerID, eResourceDBResourceType eResourceType, char *pResourceName)
{
	...
	RemoteCommand_aslMapManager_IsNameSpacePlayable(objCreateManagedReturnVal(VerificationCB, pRequest), GLOBALTYPE_MAPMANAGER, 0,
		pNameSpaceName, buffer, /*bExpectedTrasferFromShardNS=*/false);
}
