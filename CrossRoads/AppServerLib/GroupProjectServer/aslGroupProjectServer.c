...
static U32 *s_eaFinalizedRewardPending = NULL;
...
static bool
ValidateTaskSlot(GroupProjectContainer *projectContainer, GroupProjectState *projectState, DonationTaskSlot *taskSlot)
{
    ...
    if ( ( taskSlot->state == DonationTaskState_AcceptingDonations ) || ( taskSlot->state == DonationTaskState_Finalized ) )
    {
        ...
        // Only player projects can have finalization reward tables.
        ...
        else if ( taskSlot->state == DonationTaskState_AcceptingDonations )
        {
            ...
            for ( i = eaSize(&taskSlot->buckets) - 1; i >= 0; i-- )
            {
                ...
                if ( bucketData->donationCount >= GroupProject_GetDiscountedBucketRequirement(projectContainer, projectState, taskSlot, bucketData->bucketName) )
                {
...
static bool
ValidateProjectState(GroupProjectContainer *projectContainer, GroupProjectState *projectState)
{
    GroupProjectDef *projectDef;
    int i;
    ...
    for ( i = eaSize(&projectState->taskSlots) - 1; i >= 0; i-- )
    {
        ...
        if ( taskSlot->taskSlotType != projectDef->slotTypes[i] )
        {
            return false;
        }
        if ( ValidateTaskSlot(projectContainer, projectState, taskSlot) == false )
        {
            return false;
        }
    }
...
ValidateProjectContainer(GroupProjectContainer *projectContainer)
{
    ...
    for ( i = eaSize(&projectContainer->projectList) - 1; i >= 0; i-- )
    {
        ...
        if ( ValidateProjectState(projectContainer, projectState) == false )
        {
            return false;
        }
    }

    for ( i = eaSize(&projectContainer->discountList) - 1; i >= 0; i-- )
    {
        DonationTaskDiscountDefRefContainer *discountDefRef = projectContainer->discountList[i];
        if ( REF_IS_SET_BUT_ABSENT(discountDefRef->discountDef) )
        {
            return false;
        }
    }

    for ( i = eaSize(&projectContainer->bonusList) - 1; i >= 0; i-- )
    {
        GroupProjectBonusDefRefContainer *bonusDefRef = projectContainer->bonusList[i];
        if ( REF_IS_SET_BUT_ABSENT(bonusDefRef->bonusDef) )
        {
            return false;
        }
    }
...
ProcessPendingContainers(void)
{
    ...
    for( i = eaSize(&sPendingContainerCreates)-1; i >= 0; i-- )
    {
        ...
        if ( pendingCreateData->state == CreateContainer_WaitingForContainer && aslGroupProject_ProjectContainerExists(pendingCreateData->containerType, pendingCreateData->containerID, false) )
        {
            ...
            {
                if ( pendingCreateData->waiters[j]->cbFunc )
                {
...
//
// Return true if any of the tasks in the project container are in the FinalizedRewardPending state.
//
static bool
GetAnyTasksFinalizedRewardPending(GroupProjectContainer *projectContainer)
{
    // Only player projects can have finalization rewareds.
    if ( projectContainer && ( projectContainer->containerType == GLOBALTYPE_GROUPPROJECTCONTAINERPLAYER ) )
    {
        int i;
        for( i = eaSize(&projectContainer->projectList) - 1; i >= 0; i-- )
...
AddContainerToIndices(GroupProjectContainer *projectContainer)
{
    ...
    // If any of the tasks in this container are in the RewardClaimed state, then add it to the list so that we can complete the task.
    if ( GetAnyTasksFinalizedRewardPending(projectContainer) )
    {
        if ( gDebugLogObjIndex )
        {
            memlog_printf(&s_debugMemLog, "Adding container %d to finalized reward pending list", projectContainer->containerID);
        }
        ea32PushUnique(&s_eaFinalizedRewardPending, projectContainer->containerID);
    }
}
...
RemoveContainerFromIndices(GroupProjectContainer *projectContainer)
{
    ...
    if ( !GetAnyTasksFinalizedRewardPending(projectContainer) )
    {
        if ( gDebugLogObjIndex )
        {
            memlog_printf(&s_debugMemLog, "Removing container %d to finalized reward pending list", projectContainer->containerID);
        }
        ea32FindAndRemoveFast(&s_eaFinalizedRewardPending, projectContainer->containerID);
    }
...
static void
ProcessFinalizedPendingRewardTasks(void)
{
    U32 currentTime = timeSecondsSince2000();
    int i;
    for ( i = ea32Size(&s_eaFinalizedRewardPending) - 1; i >= 0; i-- )
    {
        GroupProjectContainer *projectContainer = (GroupProjectContainer *)objGetContainerData(GLOBALTYPE_GROUPPROJECTCONTAINERPLAYER, s_eaFinalizedRewardPending[i]);
        if ( projectContainer )
        {
            int j;
            for ( j = eaSize(&projectContainer->projectList) - 1; j >= 0; j-- )
            {
                GroupProjectState *projectState = projectContainer->projectList[j];
                GroupProjectDef *projectDef = GET_REF(projectState->projectDef);
                int k;
                for ( k = eaSize(&projectState->taskSlots) - 1; k >= 0; k-- )
                {
                    DonationTaskSlot *taskSlot = projectState->taskSlots[k];
                    if ( taskSlot->state == DonationTaskState_FinalizedRewardPending && 
                         currentTime > taskSlot->finalizationRequestedAt + PROJECT_FINALIZATION_REQUEST_LIMITER )
                    {
                        taskSlot->finalizationRequestedAt = currentTime;
                        RemoteCommand_gslPlayerProject_ClaimFinalizeRewards(GLOBALTYPE_ENTITYPLAYER, projectContainer->containerID,
                            projectContainer->containerID, projectDef->name, taskSlot->taskSlotNum);
                    }
                }
            }
        }
    ...
    if(sContainerLoadingDone)
    {
        ...
        if ( s_GroupProjectFixupDone )
        {
            ...
            if ( lastFrameTimeSeconds != currentTime )
            {
                ...
                ProcessFinalizedPendingRewardTasks();
...
static void GroupProjectDataModified(enumResourceEventType eType, const char *pDictName, const char *pResourceName, void *pResource, void *pUserData)
{
    if ( eType == RESEVENT_RESOURCE_MODIFIED || eType == RESEVENT_RESOURCE_ADDED || eType == RESEVENT_RESOURCE_REMOVED )
    {
        // Perform fixup on the containers when something related to tasks changes, but only while inDevelopmentMode().
        // Doing it on a live game probably won't be pretty.
        if ( isDevelopmentMode() )
        {
            if ( s_GroupProjectFixupStarted )
            {
                memlog_printf(&s_debugMemLog, "Changing GroupProject data %s '%s' restarting fixup", pDictName, pResourceName);
            }

            eaiDestroy(&s_GroupProjectFixupQueue);
            s_GroupProjectFixupsComplete = 0;
            s_ActiveGroupProjectFixups = 0;
            s_GroupProjectFixupDone = false;
            s_GroupProjectFixupStarted = false;
            s_NumFixupsFailed = 0;
            s_CurrentDefIndex = 0;
        }
    }
...
int GroupProjectServerInit(void)
{
    ...
    if ( isDevelopmentMode() )
    {
        // changing buckets / removing task / changing time
        resDictRegisterEventCallback(g_DonationTaskDict, GroupProjectDataModified, NULL);
        // changing discounts / removing discount
        resDictRegisterEventCallback(g_DonationTaskDiscountDict, GroupProjectDataModified, NULL);
        // changing bonuses / removing bonuses
        resDictRegisterEventCallback(g_GroupProjectBonusDict, GroupProjectDataModified, NULL);
    }
