...
// This command is used to force the creation of the group project container.
AUTO_COMMAND_REMOTE ACMD_CATEGORY(GroupProject);
void
aslGroupProject_VerifyProjectContainerExists(int containerType, U32 containerID, int ownerType, U32 ownerID)
{
    if ( !aslGroupProject_ProjectContainerExists(containerType, containerID, false) )
    {
        aslGroupProject_CreateAndInitProjectContainer(containerType, containerID, ownerType, ownerID, NULL, NULL);
    }
}
