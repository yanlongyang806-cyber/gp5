...
typedef struct JobManagerJobState
{
	...
	bool bAlreadyAlertedSlowness; NO_AST //true if the softTimeout alert has already happened
...
AUTO_STRUCT AST_FORMATSTRING(HTML_DEF_FIELDS_TO_SHOW = "Name, OwnerName, Failed, StartTime, CompletionTime, Duration, LogString, Cancel, JumpQueue");
typedef struct JobSummary
{
	...
	U32 iDuration; AST(FORMATSTRING(HTML_SECS_DURATION_SHORT = 1))
