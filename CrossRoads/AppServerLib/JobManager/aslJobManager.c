...
void ReplaceRCGMacros(JobSummary *pSummary, RemoteCommandGroup *pGroup)
{
	...
	for (i=0; i < eaSize(&pGroup->ppCommands); i++)
	{
		...
		if( strstr( pGroup->ppCommands[i]->pCommandString, "__JOB_PERFORMANCE_LOG__" )) {
			if( !detailsSuperEscaped ) {
				estrSuperEscapeString( &detailsSuperEscaped, pSummary->pLogString );
			}
			estrReplaceOccurrences( &pGroup->ppCommands[i]->pCommandString, "__JOB_PERFORMANCE_LOG__", detailsSuperEscaped );
		}
	}

	estrDestroy( &detailsSuperEscaped );
}
...
void CheckJobGroupsForTimeout(TimedCallback *callback, F32 timeSinceLastCallback, UserData userData)
{
	...
	while (stashGetNextElement(&stashIterator, &stashElement))
	{
		...
		if (!pSummary->pQueue)
		{
			...
			{
				if (pGroup->ppJobStates[i]->eResult == JMR_ONGOING && pGroup->ppJobStates[i]->pDef->iSoftTimeout && !pGroup->ppJobStates[i]->bAlreadyAlertedSlowness)
				{
					if (pGroup->ppJobStates[i]->iStartTime + pGroup->ppJobStates[i]->pDef->iSoftTimeout < iCurTime)
					{
						static char *pDurationString1 = NULL;
						static char *pDurationString2 = NULL;

						pGroup->ppJobStates[i]->bAlreadyAlertedSlowness = true;

						timeSecondsDurationToPrettyEString(pGroup->ppJobStates[i]->pDef->iSoftTimeout, &pDurationString1);
						timeSecondsDurationToPrettyEString(pGroup->ppJobStates[i]->pDef->iTimeout, &pDurationString2);


						WARNING_NETOPS_ALERT("SLOW_JOB", "Jobgroup %s step %s has taken longer than %s and is still going. If it takes longer than %s it will fail automatically",
							pGroup->pDef->pJobGroupName, pGroup->ppJobStates[i]->pDef->pJobName, pDurationString1, pDurationString2);
					}
				}
...
void FailJobGroup(JobManagerJobGroupState *pGroup, bool bCancelled, const char *pReason_In, ...)
{
	...
	pGroup->pSummary->iDuration = pGroup->pSummary->iCompletionTime - pGroup->pSummary->iStartTime;
...
void RegisterCompletedSummary(JobSummary *pSummary)
{
	...
	pSummary->iDuration = pSummary->iCompletionTime - pSummary->iStartTime;
...
void QueryPatchCompletion_Success_ForJobCB(void *pUserData)
{
	SlowRemoteCommandReturn_QueryPatchCompletion_ForJob((SlowRemoteCommandID)((intptr_t)pUserData), 1);
}

void QueryPatchCompletion_Fail_ForJobCB(void *pUserData)
{
	SlowRemoteCommandReturn_QueryPatchCompletion_ForJob((SlowRemoteCommandID)((intptr_t)pUserData), 0);
}
...
{
	aslPatchCompletionQuerying_Query(pNameSpaceName, QueryPatchCompletion_Success_ForJobCB, QueryPatchCompletion_Fail_ForJobCB, (void*)(intptr_t)iCmdID);
}
