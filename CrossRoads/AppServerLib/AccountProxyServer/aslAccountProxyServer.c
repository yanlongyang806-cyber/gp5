		...
		if (gbAccountProxyLogging)
		{
			EARRAY_FOREACH_BEGIN(pRequest->eaItems, i);
			{
				...
				SERVLOG_PAIRS(LOG_ACCOUNT_PROXY, "PurchaseAuth",
					...
					("currency", "%s", pRequest->pCurrency)
					("price", "%"FORM_LL"d", isRealCurrency(pRequest->pCurrency) ? 0 : moneyCountPoints(pItem->pPrice))
					("lockid", "%u", pRequest->uLockContainerID));
			}
			EARRAY_FOREACH_END;
		}
...
static void HandleProducts(PACKET_PARAMS_PROTOTYPE)
{
	...
	RemoteCommand_gslAPProductCatalogChanged(GLOBALTYPE_GATEWAYSERVER, 0);

	PERFINFO_AUTO_STOP();
}


static void IsContainerSubscribedCB(TransactionReturnVal *returnVal, PendingAPRemoteCommand *pPending)
{
	char *returnString = NULL;
	int bIsSubscribed;

	if(RemoteCommandCheck_IsContainerSubscribed(returnVal, &bIsSubscribed) == TRANSACTION_OUTCOME_SUCCESS)
	{
		if(bIsSubscribed)
		{
			...
			case APCMD_SET_ALL_KEY_VALUES:
				...
				{
					int i; 
					for(i = 0; i < eaSize(&pPending->pList->ppList); i++)
					{
						AccountProxyKeyValueInfo *pInfo = pPending->pList->ppList[i];
						APUpdateGADCache(GLOBALTYPE_ACCOUNTPROXYSERVER, pPending->uAccountID, pInfo->pKey, pInfo->pValue, NULL, NULL);
					}
				}
				break;
			...
			case APCMD_SET_KEY_VALUE:
				if(pPending->pInfo)
					APUpdateGADCache(GLOBALTYPE_ACCOUNTPROXYSERVER, pPending->uAccountID, pPending->pInfo->pKey, pPending->pInfo->pValue, NULL, NULL);
				break;

			case APCMD_REMOVE_KEY_VALUE:
				APUpdateGADCache(GLOBALTYPE_ACCOUNTPROXYSERVER, pPending->uAccountID, pPending->pKey, NULL, NULL, NULL);
				break;

			case APCMD_STEAM_CAPTURE:
				// Nothing to update in a GameAccountData for a steam capture
				break;
			}
		}
	}
...
static void FindLocation_CB(ContainerRef *pRef, PendingAPRemoteCommand *pPending)
{
	...
	if(pPending)
	{
		...
		{
			bDestroy = false;
			RemoteCommand_IsContainerSubscribed(objCreateManagedReturnVal(IsContainerSubscribedCB, pPending),
				GLOBALTYPE_OBJECTDB, 0, GLOBALTYPE_GAMEACCOUNTDATA, pPending->uAccountID);
		}
	}
