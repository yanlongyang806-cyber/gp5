...
static void SetEventClockDelta(S32 iNewDelta)
{
	...
	RemoteCommand_gslActivity_SetEventClockDelta(GLOBALTYPE_GATEWAYSERVER, SPECIAL_CONTAINERID_ALL, gsiEventClockDelta);
}
...
void aslMapManager_MapInitActivities(GlobalType eServerType, U32 iServerID)
{
	int i;
	U32 uCurrentTime = GetEventClockTime();
	...
	if (pServer==NULL && eServerType == GLOBALTYPE_GAMESERVER)
	{
		return;
	}
	...
	RemoteCommand_gslActivity_SetEventClockDelta(eServerType,iServerID,gsiEventClockDelta);
	...
	{
		RemoteCommand_Event_SetRunMode(eServerType,iServerID, pEventDef->pchEventName, pEventDef->iEventRunMode);
	}
	...
	for(i=0;i<eaSize(&gppActiveEvents);i++)
	{
		...
		RemoteCommand_Event_Start(eServerType,iServerID, pEventDef->pchEventName);

		// Send each of the events activities
		for(iActivity=0; iActivity<eaSize(&(pActiveEvent->ppActiveActivities)); iActivity++)
		{
			ActiveActivityEntry *pActivity = pActiveEvent->ppActiveActivities[iActivity];
			if (!pActivity->bDelayed)
			{
				if ((eServerType == GLOBALTYPE_GAMESERVER && EventDef_MapCheck(pEventDef,pServer->description.pMapDescription)) || EventDef_MapCheck(pEventDef,"EmptyMap"))
				{
					RemoteCommand_Activity_Start(eServerType,iServerID, pActivity->pActivityDef, pActivity->uActivityTimeStart, pActivity->uActivityTimeEnd);
				}
			}
		}
	}
	...
	{
		RemoteCommand_Activity_Start(eServerType,iServerID,gppUnownedActiveActivities[i]->pActivityDef,
...
static void aslMapManager_SendStartActivityToMaps(EventDef *pEventDef, ActiveActivityEntry *pActivity)
{
	...
	if (pEventDef==NULL || EventDef_MapCheck(pEventDef,"EmptyMap"))
	{
		RemoteCommand_Activity_Start(GLOBALTYPE_GATEWAYSERVER,SPECIAL_CONTAINERID_ALL,pActivity->pActivityDef,pActivity->uActivityTimeStart,pActivity->uActivityTimeEnd);
	}
}
...
static ActiveActivityEntry* aslMapManager_FindOrCreateActiveActivity(ActivityDef *pDef, ActiveEventEntry *pOwningEvent)
{
	ActiveActivityEntry *pActiveActivity=NULL;
	EventDef *pEventDef = pOwningEvent ? GET_REF(pOwningEvent->hEvent) : NULL;

	if(pOwningEvent)
	{
		int i;
		for (i=0;i<eaSize(&(pOwningEvent->ppActiveActivities));i++)
		{
			if (pOwningEvent->ppActiveActivities[i]->pchActivityName==pDef->pchActivityName)
			{
				return(pOwningEvent->ppActiveActivities[i]);
			}
		}
	}
	else
	{
		...
		for(i=0;i<eaSize(&gppUnownedActiveActivities);i++)
		{
			if (gppUnownedActiveActivities[i]->pchActivityName==pDef->pchActivityName)
			{
				return(gppUnownedActiveActivities[i]);
			}
		}
	}

	// Set up a new one and put it on the correct list

	pActiveActivity = StructCreate(parse_ActiveActivityEntry);
	pActiveActivity->pActivityDef = pDef;
	pActiveActivity->pchActivityName = pDef->pchActivityName;

	if (pOwningEvent)
	{
		eaPush(&pOwningEvent->ppActiveActivities,pActiveActivity);
	}
	else
	{
		eaPush(&gppUnownedActiveActivities,pActiveActivity);
	}
	return(pActiveActivity);
}
...
{
	EventDef *pEventDef = pOwningEvent ? GET_REF(pOwningEvent->hEvent) : NULL;
	ActiveActivityEntry* pActiveActivity = aslMapManager_FindOrCreateActiveActivity(pDef, pOwningEvent);

	devassert(pActiveActivity!=NULL);
	
	if(pOwningEvent)
	{
		pActiveActivity->uActivityTimeStart = pOwningEvent->uEventTimeStart + pDef->uDefaultDelay;

		if(pDef->uDuration == 0)
			pActiveActivity->uActivityTimeEnd = pOwningEvent->uEventTimeEnd;
		else
			pActiveActivity->uActivityTimeEnd = pActiveActivity->uActivityTimeStart + pDef->uDuration;
	}
	else
	{
		pActiveActivity->uActivityTimeStart = GetEventClockTime();
		pActiveActivity->uActivityTimeEnd = 0xffffffff;			// Unowned activities essentially last forever (until they are manually stopped)
	}

	if (pActiveActivity->uActivityTimeStart > GetEventClockTime())
	{
		// Due to delay we are set to start in the future
		pActiveActivity->bDelayed = true;
	}
	else
	{
		pActiveActivity->bDelayed = false;
		aslMapManager_SendStartActivityToMaps(pEventDef,pActiveActivity);
	}
}
...
static void aslMapManager_StopActivityEx(ActiveActivityEntry **ppActivity, ActiveEventEntry *pOwningEvent)
{
	...
	// This will remove both delayed and not.
	eaFindAndRemove(&pOwningEvent->ppActiveActivities,*ppActivity);
	...
	RemoteCommand_Activity_End(GLOBALTYPE_GATEWAYSERVER,SPECIAL_CONTAINERID_ALL,(*ppActivity)->pchActivityName);
...
static void aslMapManager_StopUnownedActivity(const char *pchActivityName)
{
	...
	//Removes both delayed and not
	
	for(i=eaSize(&gppActiveEvents)-1;i>=0;i--)
	{
		int j;
		for(j=eaSize(&gppActiveEvents[i]->ppActiveActivities)-1;j>=0;j--)
		{
			if (pchActivityName==gppActiveEvents[i]->ppActiveActivities[j]->pchActivityName)
			{
				eaRemove(&gppActiveEvents[i]->ppActiveActivities, j);
			}
		}
	}
	...
	RemoteCommand_Activity_End(GLOBALTYPE_GATEWAYSERVER, SPECIAL_CONTAINERID_ALL, pchActivityName);
...
static void aslMapManager_StartEventEx(EventDef *pEventDef, int iTimingIndex)
{
	if(pEventDef && pEventDef->iEventRunMode!=kEventRunMode_ForceOff)
	{
		...
		// We don't send the start/end times in this data, so we don't need to do this even if we have updated our start/end times.
		if (!bAlreadyActive)
		{
			FOR_EACH_IN_STASHTABLE(sGameServerListsByMapDescription, GameServerList, pList)
			{
			...
			RemoteCommand_Event_Start(GLOBALTYPE_GATEWAYSERVER,SPECIAL_CONTAINERID_ALL,pEventDef->pchEventName);
		}
		
		// Even if we're already active, 'start' the activities. We may need to update their start/end times based on moving to a new
		//   repeat block or something. We run this on both delayed and not delayed activities. _StartActivity will set the flag appropriately.
		// The Events start and end times will be updated according to the ActiveEvent's which are caculated in the above GetUsefulTimes.
...
static void aslMapManager_StopEventEx(EventDef *pEventDef)
{
	...
	if(pEventDef)
	{
		...
		if (pActiveEvent!=NULL)
		{
			...
			RemoteCommand_Event_End(GLOBALTYPE_GATEWAYSERVER,SPECIAL_CONTAINERID_ALL,pEventDef->pchEventName);

			// Stop each of the events activities. Delayed or not.
			for(x=eaSize(&pActiveEvent->ppActiveActivities)-1;x>=0;x--)
			{
				ActiveActivityEntry *pActivity = pActiveEvent->ppActiveActivities[x];
...
static void aslMapManager_SetEventRunMode(EventDef *pEventDef, int iRunMode)
{
	if (pEventDef->iEventRunMode!=iRunMode)
	{
		...
		RemoteCommand_Event_SetRunMode(GLOBALTYPE_GATEWAYSERVER, SPECIAL_CONTAINERID_ALL, pEventDef->pchEventName, pEventDef->iEventRunMode);
	}
}
...
void aslMapManagerEvent_Tick()
{
	...
	while(pEventDef = (EventDef *)RefSystem_GetNextReferentFromIterator(&eventIterator))
	{
		...
		if(pActiveEvent)
		{
			...
			// Check for delay activities

			for(i=eaSize(&pActiveEvent->ppActiveActivities)-1;i>=0;i--)
			{
				ActiveActivityEntry *pActiveActivity = pActiveEvent->ppActiveActivities[i];

				if (pActiveActivity->bDelayed && pActiveActivity->uActivityTimeStart <= uTimeNow)
				{
					pActiveActivity->bDelayed=false;
					aslMapManager_SendStartActivityToMaps(pEventDef,pActiveActivity);	
				}
			...
			if(!aslActiveEvent_ShouldBeOn(pActiveEvent))
			{
				...
				{
					// Start with the new index. Update the DB, Update the activity start times and resend, etc.
			...
			{
				// We know the event thinks it's supposed to be on. Now we have to deal with activities which may need to expire (due to delay tech)
				for(i=eaSize(&pActiveEvent->ppActiveActivities)-1;i>=0;i--)
				{
					ActiveActivityEntry *pActivity = pActiveEvent->ppActiveActivities[i];

					// Turn off any activities that have a duration that has expired
					if(uTimeNow > pActivity->uActivityTimeEnd)
					{
						aslMapManager_StopActivityEx(&pActivity,pActiveEvent);
					}
				}

				// If we're past our ActiveEvent end time, we need to 'restart' the event. We've probably moved into a different
				//   timing cycle that has us as active. Start will update these times, and also update any delayed/durational
				//   activities that are part of this event.
				if (uTimeNow > pActiveEvent->uEventTimeEnd)
				{
					aslMapManager_StartEventEx(pEventDef, pActiveEvent->iTimingIndex);
				}
			}
		}
...
AUTO_COMMAND_REMOTE;
void aslMapManagerUpdateActivites(GlobalType eServerType, int iServerID)
{
	aslMapManager_MapInitActivities(eServerType,iServerID);
}
