...
static int aslContainerAcquireMaxRequests = 1024;
...
int InitializeStringCache(void)
{
	...
	stringCacheSetInitialSize(1024*2048);
...
void OVERRIDE_LATELINK_GetCustomServerInfoStructForHttp(UrlArgumentList *pUrl, ParseTable **ppTPI, void **ppStruct)
{
	...
	xcase GLOBALTYPE_QUEUESERVER:
		QueueServer_GetCustomServerInfoStructForHttp(pUrl, ppTPI, ppStruct);
