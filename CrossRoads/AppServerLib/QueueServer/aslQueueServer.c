											   ...
											   bool bUpdateMap)
{
	...
	// VERBOSE state change logging. We don't want this on live for now. Useful for local stuff.
	//if (peaUpdates!=NULL)
	//{
	//	log_printf(LOG_QUEUE, "MemberChangeStateRequest [%d] State [%d]->[%d]\n", pMember->iEntID, pUpdate->eLastState,pUpdate->eState);
	//}
	return pUpdate;
}
											 ...
											 S32 iStartIndex)
{
	...
	if (iStartIndex < 0)
	{
		iStartIndex = eaSize(&pSrcGroup->eaMatchMembers)-1;
	}
	for (iFindemberIdx = iStartIndex; iFindemberIdx >= 0; iFindemberIdx--)
	{
		QueueMatchMember* pMember = pSrcGroup->eaMatchMembers[iFindemberIdx];

		if (!pMember->bPresetMember && pMember->uTeamID == pTeamMember->iTeamID)
		{
												 ...
												 bool bOvertime)
{
	...
	for (iGroupIdx = 0; iGroupIdx < eaSize(&pMatch->eaGroups); iGroupIdx++)
	{
		...
		for (iMemberIdx = eaSize(&pSrcGroup->eaMatchMembers)-1; iMemberIdx >= 0; iMemberIdx--)
		{
			QueueMatchMember* pMatchMember = pSrcGroup->eaMatchMembers[iMemberIdx];
			QueueMember* pMember = eaIndexedGetUsingInt(&pInstance->eaUnorderedMembers, pMatchMember->uEntID);

			if (pMember && pMatchMember && !pMatchMember->bPresetMember)
			{
											   ...
											   bool* pbTransferFailure)
{
	...
	for (iMemberIdx = eaSize(&pSrcGroup->eaMatchMembers)-1; iMemberIdx >= 0; iMemberIdx--)
	{
		...
		if (pQueueMember && pMatchMember!=NULL && !pMatchMember->bPresetMember)
		{
			if (pMatchMember->uTeamID && !pDef->Settings.bSplitTeams && iTeamSize > 1)
			{
			...
			{
				if (!aslQueue_AutoBalance_TransferMember(pInstance,pMatch,pSrcGroup,pDstGroup,pMatchMember,pDef))
				{
													   ...
													   S32* piMemberIndex)
{
	...
	{
		QueueGroup* pGroup = pMatch->eaGroups[iGroupIdx];
		for (iMemberIdx = eaSize(&pGroup->eaMatchMembers)-1; iMemberIdx >= 0; iMemberIdx--)
		{
			QueueMatchMember* pMatchMember = pGroup->eaMatchMembers[iMemberIdx];
			QueueMember* pMember = eaIndexedGetUsingInt(&pInstance->eaUnorderedMembers, pMatchMember->uEntID);			
			S32 iTeamSize = MAX(pMember->iTeamSize, 1);
			bool bBest = false;

			if (pMember!=NULL && pMatchMember!=NULL && !pMatchMember->bPresetMember)
			{
				if (bLazyBalance)
				{
...
static bool aslQueue_AutoBalance_IsMatchMoreBalanced(QueueMatch* pMatch)
{
	...
	for (iGroupIdxA = eaSize(&pMatch->eaGroups)-1; iGroupIdxA >= 0; iGroupIdxA--)
	{
		...
		{
			// This is probably wrong. eaSize(eaMatchMembers is not a true measure of the team size due to caching)
			...
			S32 iGroupASizeOld = MAX(iGroupASizeNew - eaSize(&pGroupA->eaMatchMembers), 0);
			S32 iGroupBSizeOld = MAX(iGroupBSizeNew - eaSize(&pGroupB->eaMatchMembers), 0);
	...
	if (bSameAffiliationReq)
	{
		...
		while (--iRetryCount > 0)
		{
			...
			while (pMatch->iMatchSize > uMin && (bRetry || (pMatch->iMatchSize % iGroupCount) != 0))
			{
				...
				{
					// GetBestMember checks for presets, so we shouldn't need to check again here.
					QueueGroup* pGroup = pMatch->eaGroups[iGroupIdx];
					QueueMatchMember* pMatchMember = pGroup->eaMatchMembers[iMemberIdx];
					QueueMember* pMember = eaIndexedGetUsingInt(&pInstance->eaUnorderedMembers, pMatchMember->uEntID);					
					devassert(pMember);
...
//But only if they are not presets (which assumably have already been removed)
static void aslQueue_MatchMembersRemoveFromReadyList(QueueMatch *pMatch, QueueMember ***peaReadyMembers)
{
	...
	for (iGroupIdx=0; iGroupIdx<iNumGroups; iGroupIdx++)
	{
		...
		for (iGroupMemberIdx = 0; iGroupMemberIdx < eaSize(&pGroup->eaMatchMembers); iGroupMemberIdx++)
		{
			if (!pGroup->eaMatchMembers[iGroupMemberIdx]->bPresetMember)
			{
				for (iMemberIdx = eaSize(peaReadyMembers)-1; iMemberIdx >= 0; iMemberIdx--)
				{
					if ((*peaReadyMembers)[iMemberIdx]->iEntID == pGroup->eaMatchMembers[iGroupMemberIdx]->uEntID)
					{
										   ...
										   QueueMapState eMapState)
{
	...
	for (iGroupIndex = 0; iGroupIndex < iNumGroups; iGroupIndex++)
	{
		...
		for (iMemberIdx = 0; iMemberIdx < eaSize(&pGroup->eaMatchMembers); iMemberIdx++)
		{
			U32 uEntID = pGroup->eaMatchMembers[iMemberIdx]->uEntID;
			...
			// Don't process presetMembers as they've been dealt with previously.

			if (pMember && !pGroup->eaMatchMembers[iMemberIdx]->bPresetMember)
			{
								   ...
								   SA_PARAM_NN_VALID QueueMember ***peaReadyMembers)
{
	...
	for (iMemberIdx = iNumMembers-1; iMemberIdx >= 0; iMemberIdx--)
	{
		...
		{
			bool bPresetMember = false;	// We're actually adding new people to the group. So we are not preset
			queue_Match_AddMemberToGroup(pMatch, pGroup, pTeamMember->iEntID, pTeamMember->iTeamID, pTeamMember->iGroupRole, pTeamMember->iGroupClass, bPresetMember);
...
static bool aslQueue_IsSmartGroupToAddTo(QueueDef* pQueueDef, QueueGroup* pGroup, int iGroupMaxSize, QueueMember* pMember, int iTeamSize,
														SA_PARAM_NN_VALID QueueMember ***peaReadyMembers)
 {
	if (g_QueueConfig.bRoleClassSmartGroups && pQueueDef->MapRules.eSmartGroupingMode != kQueueSmartGroupingMode_Off)
	{
									   ...
									   SA_PARAM_NN_VALID QueueMember ***peaReadyMembers)
{
	...
	if (iChosenGroup >= 0)
	{
		...
		else
		{
			...
			bool bPresetMember = false;	// We're actually adding new people to the group. So we are not preset
			queue_Match_AddMemberToGroup(pMatch, pMatch->eaGroups[iChosenGroup], pMember->iEntID, pMember->iTeamID, pMember->iGroupRole, pMember->iGroupClass, bPresetMember);
						  ...
						  S32 iStartIdx)
{
	...
	for (iMemberIdx = iStartIdx; iMemberIdx < eaSize(&eaCopyMembers); iMemberIdx++)
	{
		...
		//If the member is on a team, and we tried adding them, then validate that the whole team was added
		if (pMember->iTeamID && iCurrentMembersMatched > 0 && iCurrentMembersMatched != (S32)pMember->iTeamSize)
		{
	...
	if (pQueueMatch->bAllGroupsValid)
	{
		...
		//   (uses only the group and match counts in pMatch)
...
// This is a special case to look for full groups in the queue as we don't want them blocked at all if the earlier members in the queue
//   cannot be matched at a particular instant. Previously the brute force/failure stuff would eventually catch these, but we'd like to speed
//   up the whole thing and always make sure a full group is placed as quickly as possible.
// Note that we do not consider multi-group (PvP-style) groupings in this. A group of 5 would only fill up one side, and it's not fair to jump
//   them in line when we could possibly make groups out of the non-5-teamed members. (Though that would promote pre-made vs. pre-made I guess).
S32 aslQueue_FindFullGroup(SA_PARAM_NN_VALID QueueInfo *pQueue,
						  SA_PARAM_NN_VALID QueueInstance *pInstance,
						  SA_PARAM_NN_VALID QueueDef *pQueueDef,
						  SA_PARAM_OP_VALID QueueMap *pQueueMap,
						  SA_PARAM_NN_VALID QueueMember ***peaReadyMembers,
						   SA_PARAM_NN_VALID QueueMatch *pQueueMatch)
{
	...
	// Only allow if the to-be match is empty and we have a single group to fill.
	if (pQueueMatch->iMatchSize==0 && eaSize(&pQueueMatch->eaGroups)==1)
	{
		// Copy out the ready members to try matching
		QueueMember **eaCopyMembers = NULL;
		
		eaCopy(&eaCopyMembers, peaReadyMembers);
		
		//The size needs to be re-evaluated each iteration because the aslQueue_MatchMember can remove members from the array
		for (iMemberIdx = 0; iMemberIdx < eaSize(&eaCopyMembers); iMemberIdx++)
		{
			QueueMember *pMember = eaGet(&eaCopyMembers, iMemberIdx);
			U32 uTeamSize = MAX(pMember->iTeamSize, 1);
			U32 iCurrentMembersMatched=0;
			bool bIsNewMap = pQueueMap==NULL||(pQueueMap->eMapState == kQueueMapState_Open);  // QueueMap will be NULL if we are queryingto create new maps.

			// Only try if the team is the right size!!
			if (uTeamSize!=pQueue->iMaxMapSize)
				continue;
			...
			// Try to match the whole team. (We will not find a match if affiliation doesn't match or something)
			iCurrentMembersMatched = aslQueue_MatchMember(pInstance, pQueueMatch, pQueueDef, pMember, iMemberIdx, bOvertime, &eaCopyMembers);

			if (iCurrentMembersMatched==uTeamSize)
			{
				iMembersMatched = iCurrentMembersMatched;
				// It worked. Break out of the loop. 
				break;
			}
			// Otherwise we should look for another team.
			
			// We need to guard against the case where a team only partially gets matched. We don't want that team to block searching for
			//  any other whole teams that we could use. Clear out the match so we can try again with another full group.
			if (iCurrentMembersMatched > 0)
			{
				queue_Match_ClearMembers(pQueueMatch);
			}
		}

		// Clean up the copy members.
		eaDestroy(&eaCopyMembers);
	}
							 ...
							 SA_PARAM_NN_VALID QueueMatch *pQueueMatch)
{
	...
	//0) Special case for full groups. We don't want full groups to be blocked in the queue if we are adding to an empty match.
	if (bIsNewMap)
	{
		iMembersMatched = aslQueue_FindFullGroup(pQueue, pInstance, pQueueDef, pQueueMap, peaReadyMembers, pLocalMatch);
	}
	
	//1) Attempt simple match based off of first person in the queue.
	if (iMembersMatched <= 0)
	{
		iMembersMatched = aslQueue_AttemptMatch(pQueue, pInstance, pQueueDef, pQueueMap, peaReadyMembers, pLocalMatch, iAttemptIdx);
	}

	//2) Brute force mode (try later people as seeds) if we have equal or greater failed matches and it's attempting to fill a new map
	if (iMembersMatched <= 0)
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//  Already in map caching.
//  Two versions. One is for getting Offered/Unaccepted members. The other is for generic precaching (most used for joining in-progress queues)
//  In both cases, we want to set as PresetMember any members who are already associated with the map, but not of a particular state

void aslQueue_GroupCache(QueueInstance *pInstance, 
						  QueueDef *pQueueDef,
						  QueueMap *pMap,
	...
	for (iMemberIdx = 0; iMemberIdx < iMemberSize; iMemberIdx++)
	{
		...
		if (pMember->iGroupIndex >= 0)
		{
			...
			if (pGroup!=NULL)
			{
				...
				{
					// We're on the map. We are looking for a particular state and this is not it.
					// Preset (unmoveable) for this member. (They might have been already on the map
					// and we are trying to add a new member for someone who dropped)
					bPresetMember=true;
				}

				pMatchMember = queue_Match_AddMemberToGroup(pMatch, pGroup, pMember->iEntID, pMember->iTeamID,
														pMember->iGroupRole, pMember->iGroupClass,bPresetMember);
			}
		}
	}
}
								...
								bool bIsLaunching)
{
	...
	aslQueue_GroupCache(pInstance, pQueueDef, pMap, PlayerQueueState_Accepted, PlayerQueueState_Offered, pMatch);
		...
		for (iTeamIdx = eaSize(&s_eaCachedTeams)-1; iTeamIdx >= 0; iTeamIdx--)
		{
			...
			if (ea32Find(&pCachedTeam->eaiEntityIds, pMember->iEntID) >= 0)
			{
				...
				if (!pMember->iMapKey && SAFE_MEMBER2(pInstance, pParams, uiOwnerID) > 0)
				{
					// Private queue
					QueueMatch QMatch = {0};
					queue_PrivateGroupCache(pInstance, pDef, &QMatch);  // Fill QMatch with members already assigned to groups
	...
	for (iMemberIdx = iMemberSize-1; iMemberIdx >= 0; iMemberIdx--)
	{
		...
		switch (pMember->eState)
		{
			...
			case PlayerQueueState_Accepted:
			{
				...
				{
					// We used to put members in Limbo here:

					//	PlayerQueueState eState = PlayerQueueState_Limbo;
					//	bDirty |= aslQueue_MemberChangeState(pQueue, pInstance, pMember, eState, NULL, false);

					// This is problematic, as if we are ever restored from limbo, we will immediately be put in the InMap
					// state, likely skipping the actual map transfer. Instead of limbo, let's do the same thing that happens
					// when the offer response time out happens.
					...
					if (pMap==NULL)
					{
						...
						// Something weird happened and we lost our map. We need to be booted
						aslQueue_MemberChangeState(pQueue, pInstance, pMember, eState, QUEUE_OFFER_TIMEOUT_MESG, false);
					}
					else
					{
						aslQueue_HandleOfferValidationMatchFailure(pQueue, 
													   ...
													   pMap, 
													   pQueueDef, 
													   true, // Boot offered. Accepteds are put back in the queue. Is this a problem for teams?
													   &iAccepted, 
													   &iOffered);
						aslQueue_LogAbortMatch(pQueue, pInstance, pMap, iAccepted, iOffered, "Accepted State Timeout Match Abort");
					}
			...
			{
				if (pMember->iLastMapUpdate > 0 && pMember->iLastMapUpdate + g_QueueConfig.uMemberInMapTimeout < iCurrentTime)
				{
					PlayerQueueState eState = PlayerQueueState_Limbo;
					bDirty |= aslQueue_MemberChangeState(pQueue, pInstance, pMember, eState, NULL, false);
				}
									...
									bool bMustMatchAll)
{
	...
	if(!pInstance->bAutoFill && pDef->MapRules.pMatchMakingRules && pDef->MapRules.pMatchMakingRules->pchMatchMakingLeaderboard)
	{
		...
		// WARNING!!!!!  This is broken. (And has been?) We don't do the right thing if the preset caching that makes
		//   the match passed into this function puts presetMembers into the groups. We would not want even
		...
		for(i=0;i<eaSize(&pDef->eaGroupDefs);i++)
		{
			...
			while(eaSize(&pLocalMatch->eaGroups[i]->eaMatchMembers) < fMembersPerGroup && eaSize(&ppLocalMembers))
			{
				int j;
											...
											QueueMember ***peaGroupedMembers)
{
	...
	for(iMapIdx = 0; iMapIdx < iMapSize; iMapIdx++)
	{
		...
		aslQueue_GroupCache(pInstance, pDef, pMap, PlayerQueueState_InQueue, PlayerQueueState_None, pMatch);
								  ...
								  QueueMember ***peaReadyMembers)
{
	...
	for (iMapIdx = 0; iMapIdx < iMapSize; iMapIdx++)
	{
		...
		aslQueue_GroupCache(pInstance, pQueueDef, pMap, PlayerQueueState_InQueue, PlayerQueueState_None, pMatch);
										  ...
										  QueueMember*** peaReadyMembers)
{
	...
	{
		// WARNING!!!!!  This is broken. (And has been?) We don't do the right thing if the preset caching puts presetMembers into the groups.
		//  We would not want even distributions at that point!

		devassert(pDef->MapRules.pMatchMakingRules==NULL);
		...
		if (pInstance)
		{
			...
			{	
				...
				if (pGroupDef)
				{
					...
					{
						char pchLog[512];
						sprintf(pchLog, "Attempt Map Transfer Member(%d) of Group[%d] to Map[%d,%d]", uMemberID, pMemberUpdate->iGroupIndex,uMapID,uPartitionID);
						aslQueue_Log(pDef, pInstance, pchLog);
					}
				}
			}
		}
