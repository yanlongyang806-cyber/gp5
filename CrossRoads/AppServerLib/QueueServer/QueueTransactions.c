...
static void aslQueue_ForcedLeave_CB(TransactionReturnVal *pVal, void *pData)
{
	...
	{
		log_printf(LOG_QUEUE, "EntID [%d] was forced to leave the [%s] queue.\n",
			pCBData->iEntID,
			pQueue->pchName);
		if(pCBData->iMapKey)
		{
...
void aslQueue_PenalizePlayerByID(U32 uEntID, QueueDef* pDef, bool bAFKPenalty)
{
	if (pDef && uEntID)
	{
		...
		if(bAFKPenalty)
		{
			pPenaltyCategory->uPenaltyEndTime = timeSecondsSince2000() + g_QueueConfig.QueueAFKPenaltyTimeSeconds;
			pPenaltyCategory->bAFKPenalty = true;
		}
		else
		{
			pPenaltyCategory->uPenaltyEndTime = timeSecondsSince2000() + queue_GetLeaverPenaltyDuration(pDef);
			pPenaltyCategory->bAFKPenalty = false;
		}
...
void aslQueue_HandleLeaveMap(const char* pchQueueName, S64 iMapKey, ContainerID uEntID, bool bGameIsFinished, bool bLeaverPenaltyEnabled)
{
	...
	if (pDef && pQueue)
	{
		...
		if (pMember)
		{
			...
			{
				aslQueue_PenalizePlayerByID(uEntID, pDef, false);
				bPenalty = true;
			}
...
AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void aslQueue_PenalizePlayer(ContainerID uEntID, const char* pchQueueName, bool bAFKPenalty)
{
	QueueDef* pDef = queue_DefFromName(pchQueueName);
	if (pDef)
	{
		aslQueue_PenalizePlayerByID(uEntID, pDef, bAFKPenalty);
	}
}
									...
									bool bIsPrivateInstance)
{
	...
	// VERBOSE state changes. We don't want this on Live
	// log_printf(LOG_QUEUE, "MemberStateSet ID %d State %d", pMember->iEntID, eNewState);
					   ...
					   U32 uiJoinTimeOverride)
{
	...
	if (NONNULL(pInstance->pParams) && pInstance->pParams->uiOwnerID && iGroupID < 0)
	{
		...
		QueueMatch QMatch = {0};
		queue_trh_PrivateGroupCache(pInstance, pDef, &QMatch); // Fill QMatch with members already assigned to groups
...
enumTransactionOutcome aslQueue_tr_TeamLeave(ATR_ARGS, NOCONST(QueueInfo) *pQueueContainer, U32 iTeamID, U32 uiInstanceID)
{
	...
	if (NONNULL(pInstance))
	{
		...
		for(iMemberIdx=eaSize(&pInstance->eaUnorderedMembers)-1;iMemberIdx>=0;iMemberIdx--)
		{
			...
			{
				// We used to check if each team member CanLeaveQueue. This is problematic as we use this code when
				// a player declines an offer in a TeamJoin. So some members will 'pass' (the decliners) and some members will not (those already accepted)
				// This puts the UI in a state where the declining player has no option but to Accept.
				// (The test for this behaviour is to put 5 people in an instance. Have 2 drop, those two form a team and queue as team.
				// When they are invited, one player accepts, and the other tries to decline)
				//  I am going to make it so that we always remove all members regardless of state. The downside to this
				// is that if somehow a member is on a map already they will be pulled from it.
...
void aslQueue_LeaveForced(ContainerID iEntID, const char* pchQueueName, U32 uiInstanceID)
{
	...
	//   gslQueue_CheckPlayerForceLeaveInstance	(gslQueue.c) and gslQueue_MapTransferMember (gslQueue_cmd.c)
	// We ALWAYS want to leave if the map transfer needs to fail. Otherwise we get stuck in a strange state of being in the queue but not transferring
	// ForceLeaveInstance does the proper state checks that used to be done here. 
	if (pMember!=NULL)
	{
		QueueCBStruct *pCBData = StructCreate(parse_QueueCBStruct);
		TransactionReturnVal *pReturn = objCreateManagedReturnVal(aslQueue_ForcedLeave_CB, pCBData);
...
static void aslQueue_LeaveAllStrictTeaming(ContainerID iEntID)
{
	...
	while (pQueue = objGetNextObjectFromIterator(&queueIter))
	{
		...
		// WOLF[31May13] Added Accepted to prevent being kicked from the queue early. I have seen cases (especially on STO) where the team is formed
		//   before the transition from Accepted to InMap happens. In such cases the player gets removed from the queue and subsequently gets booted
		//   from the map by the team verification code. This could be a problem now with cases where someone is queued for two maps and is allowed
		//   to accept both of them. We should catch that case elsewhere if it happens.
		if ( pMember->iMapKey &&
				(pMember->eState == PlayerQueueState_Countdown ||
				 pMember->eState == PlayerQueueState_Accepted ||			
				 pMember->eState == PlayerQueueState_InMap ||
...
static void AcceptOffer_CB(TransactionReturnVal *pReturn, void *pData)
{
	...
	if(pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		if (pQueue)
		{
			...
			uMapID = queue_GetMapIDFromMapKey(iMapKey);
			uPartitionID = queue_GetPartitionIDFromMapKey(iMapKey);

			log_printf(LOG_QUEUE,"Player [%d] accepted the offer from queue instance [%s][%d] (Map=%d,%d).\n",
										pCBStruct->iEntID, pQueue->pchName, pCBStruct->iInstanceID, uMapID, uPartitionID);
		...
		if (bAMemberWasReallyOnMap)
		{
			...
			for (iMemberIdx = 0; iMemberIdx < eaSize(&pLeaderboard->eaEntities); iMemberIdx++)
			{
				...
				if (pQueueDef!=NULL)
				{
					...
					//    Only do this if they are actually limbo though or we will trigger this if the
					//    player drops the queue then rejoins and gets the same instance (the scoreboard
					//    will be holding on to them from their previous time on the map. (Going straight
					//    to InMap is bad in that case as it will skip the map transfer code, etc.)
					// Note that we eventually (elsewhere) consider if ANY members are really on the map, as we don't want to get
				...
				if (pScore!=NULL)
 				{
					...
					if (pMember!=NULL)
					{
						...
						else if (pMember->eState == PlayerQueueState_Limbo && bCanBeOffMap)
						{
							// See above comment.
							bConsideredOnMap=true;
						}
