...
#include "../objects/objBackupCache.h"
...
static QueueInfo *QueueServerGetBackupCopy(GlobalType eType, ContainerID uID)
{
	Container *con = objGetContainer(eType, uID);
...
static QueueInfo *QueueServerGetModifyCopy(GlobalType eType, ContainerID uID)
{
	Container *con = objGetContainer(eType, uID);
...
int QueueServerStartupOncePerFrame(F32 fElapsed)
{
	...
	if(!bOnce)
	{
		// Register local backup/modify copy callbacks, to avoid having to serialize QueueInfo containers to ourselves every transaction
		BackupCache_RegisterType(GLOBALTYPE_QUEUEINFO, BACKUPCACHE_STASH, 64);
		RegisterFastLocalCopyCB(GLOBALTYPE_QUEUEINFO, QueueServerGetBackupCopy, QueueServerGetModifyCopy);
