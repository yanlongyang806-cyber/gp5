...
AUTO_CMD_INT(siNamespaceUndeleteGracePeriodDays, NamespaceUndeleteGracePeriodDays) ACMD_AUTO_SETTING(Ugc, UGCDATAMANAGER);
...
int s_iNamespacePatchingTimeout = 600;
AUTO_CMD_INT(s_iNamespacePatchingTimeout, NamespacePatchingTimeout);
...
void aslFillInPatchInfoForUGC(DynamicPatchInfo *pPatchInfo, enumFillInPatchInfoFlags eFlags)
{
	...
	{
		//pPatchInfo->pServer = strdup(gUGCPatchserver_ForGetting?gUGCPatchserver_ForGetting:"localhost");//ORGANIZATION_DOMAIN);
		pPatchInfo->pServer = strdup(gUGCPatchserver_ForGetting?gUGCPatchserver_ForGetting:ORGANIZATION_DOMAIN);
	}
	else
	{
		//pPatchInfo->pServer = strdup(gUGCPatchserver?gUGCPatchserver:"localhost");//ORGANIZATION_DOMAIN);
		pPatchInfo->pServer = strdup(gUGCPatchserver?gUGCPatchserver:ORGANIZATION_DOMAIN);
	}
...
bool aslFillInPatchInfo(DynamicPatchInfo *pPatchInfo, const char *pNameSpace, enumFillInPatchInfoFlags eFlags )
{
	...
	{ 
		pPatchInfo->pServer = strdup(gDynamicPatchserver?gDynamicPatchserver:ORGANIZATION_DOMAIN); 
...
#define PCL_ERROR()																									\
	...
	TriggerAutoGroupingAlert("UGCDATAMANAGER_NAMESPACE_DELETE_PATCHING_ERROR", ALERTLEVEL_CRITICAL, ALERTCATEGORY_NETOPS, 600, "PCL error: %s. Client: %s", \
		msg, pclGetUsefulDebugString_Static(client));																\
	ScratchFree(msg);

// Handle a PCL error.
#define PCL_DO_ERROR_WITH_ERROR(err)	\
{										\
...
#define PCL_DO_ERROR()					\
{										\
	PCL_DO_ERROR_WITH_ERROR(return 0);	\
}

// Run PCL commands.
#define PCL_DO_WITH_ERROR(fn, err)	\
{									\
	error = fn;						\
	PCL_DO_ERROR_WITH_ERROR(err);	\
}
#define PCL_DO(fn)						\
{										\
	PCL_DO_WITH_ERROR(fn, return 0);	\
}

// Run PCL commands, and wait for them to finish.
#define PCL_DO_WAIT_WITH_ERROR(fn, err)	\
{										\
	PCL_DO_WITH_ERROR(fn, err);			\
	error = pclWait(client);			\
	PCL_DO_ERROR_WITH_ERROR(err);		\
}
#define PCL_DO_WAIT_WITH_RETRY(fn, attempts, err)									\
{																					\
	int attempt;																	\
	for(attempt = 1; attempt <= attempts; attempt++)								\
	{																				\
		PCL_DO_WAIT_WITH_ERROR(fn, if(PCL_TIMED_OUT == error) continue);			\
		break;																		\
	}																				\
	if(attempt > attempts || PCL_SUCCESS != error)									\
		err;																		\
}
#define PCL_DO_WAIT(fn)						\
{											\
	PCL_DO_WAIT_WITH_ERROR(fn, return 0);	\
}
...
static int NamespaceGettingStateComplete()
{
...
static DWORD WINAPI GetNamespaceListThread(LPVOID lpParam)
{
	...
	// Get manifest.
	PCL_DO_WAIT_WITH_ERROR(pclConnectAndCreate(&client, info->pServer, info->iPort, s_iNamespacePatchingTimeout, patch_comm, "", "aslPatching", "", NULL, NULL),
		return NamespaceGettingStateComplete());

	pclAddFileFlags(client, PCL_METADATA_IN_MEMORY);
	pclSetBadFilesDirectory(client, fileTempDir());

	PCL_DO_WAIT_WITH_RETRY(pclSetNamedView(client, info->pUploadProject, info->pViewName, true, false, NULL, NULL), /*attempts=*/3,
		return NamespaceGettingStateComplete());
	...
	for (i=0; i < eaSize(&ppNamespacesToDelete_BG); i++)
	{
		...
		if(siMSecsToSleepBetweenNSDeletions)
...
static bool aslDeleteNamespace(PCL_Client *client, const char *pName)
{
	...
	if (deleted)
	{
		...
		AssertOrAlert("UGCDATAMANAGER_NAMESPACE_DELETION_ERROR", "Attempt to delete empty directory.  Client: %s",
