...
void aslGuild_ValidateGuilds(void)
{
	...
	while (pGuild = objGetNextObjectFromIterator(&iter)) {
		...
		{
			if(iCurrentTime >= pGuild->uGuildDestroyTimeSeconds)
			{
				// time is set, or there isn't a wait time or there isn't any bank logs
				if(pGuild->uGuildDestroyTimeSeconds || eaSize(&pGuild->eaBankLog) < 1 || gGuildConfig.uDestroyWaitTimeHours == 0)
				{
					RemoteCommand_aslGuild_Destroy(GLOBALTYPE_GUILDSERVER, 0, pGuild->iContainerID, 0);
				}
				else
				{
					// set destroy time for guilds that actually have items in the bank
					pGuild->uGuildDestroyTimeSeconds = iCurrentTime + gGuildConfig.uDestroyWaitTimeHours * SECONDS_PER_HOUR;
				}
			}
		}
		else if(pGuild->uGuildDestroyTimeSeconds)
		{
			// clear the guild destroy time as there is a member
			pGuild->uGuildDestroyTimeSeconds = 0;
		}
		...
		// Check for demoted leader
		aslGuild_DemoteLeader(pGuild);
