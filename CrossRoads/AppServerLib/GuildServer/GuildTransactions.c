...
/*
AUTO_TRANS_HELPER;
bool aslGuild_trh_IsLeader(GUILD_TRH_ARGS, ATH_ARG NOCONST(Guild) *pGuild, U32 EntID)
{
	NOCONST(GuildMember) *pGuildMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuild, EntID);
	if(NONNULL(pGuildMember))
	{
		if(pGuildMember->iRank == eaSize(&pGuild->eaRanks) - 1)
		{
			return true;
		}
	}
...
AUTO_TRANS_HELPER;
bool aslGuild_trh_HasPermission(GUILD_TRH_ARGS, ATH_ARG NOCONST(Guild) *pGuildContainer, ATH_ARG NOCONST(GuildMember) *pMember, GuildRankPermissions ePerms)
{
	S32 iRank = pMember->iRank;

	// leaders being demoted don't have permission to use guild commands
	if(pGuildContainer->uDemoteLeaderID == pMember->iEntID)
	{
		return false;
	}
...
bool aslGuild_trh_RemoveMember(GUILD_TRH_ARGS, ATH_ARG NOCONST(Guild) *pGuildContainer, U32 iEntID, ATH_ARG NOCONST(Entity) *pSubject, U32 iSubjectID)
{
	...
	if (NONNULL(pMember) && iSubjectMemberIdx >= 0)
	{
		...
		// if this is a leader being demoted clear that
		if(iSubjectID == pGuildContainer->uDemoteLeaderID)
		{
			pGuildContainer->uDemoteLeaderID = 0;
			pGuildContainer->uDemotionTimeSeconds = 0;
		}
...
void aslGuild_trh_Promote(GUILD_TRH_ARGS, ATH_ARG NOCONST(Guild) *pGuildContainer, U32 iEntID, U32 iSubjectID)
{
	NOCONST(GuildMember) *pMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuildContainer, iEntID);
	NOCONST(GuildMember) *pSubjectMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuildContainer, iSubjectID);
	int iMaxRank = eaSize(&pGuildContainer->eaRanks) - 1;
	int iNewRank;
	bool bPromotingToLeader;
	
	if (ISNULL(pMember) || ISNULL(pSubjectMember))
		return;

	iNewRank = pSubjectMember->iRank + 1;

	if (iNewRank > iMaxRank)
		return;

	bPromotingToLeader = (iNewRank == iMaxRank);

	if (pGuildContainer->uDemoteLeaderID && bPromotingToLeader)
		return;

	pSubjectMember->iRank = iNewRank;
	if(gGuildConfig.bUseLeaderSlowDemotion && gGuildConfig.uProvisionalTimeHours && bPromotingToLeader)
	{
		pSubjectMember->uProvisionalTimeSeconds = timeSecondsSince2000() + gGuildConfig.uProvisionalTimeHours * SECONDS_PER_HOUR;
	}
	...
	aslGuild_PushMemberAddToChatServer(pGuildContainer->iContainerID, pGuildContainer->pcName, pSubjectMember->iAccountID, pSubjectMember->iRank, (const GuildCustomRank *const *)pGuildContainer->eaRanks);
	
}

// Demote a member
AUTO_TRANS_HELPER;
bool aslGuild_trh_Demote(GUILD_TRH_ARGS, ATH_ARG NOCONST(Guild) *pGuildContainer, U32 iEntID, U32 iSubjectID)
{
	NOCONST(GuildMember) *pMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuildContainer, iEntID);
	NOCONST(GuildMember) *pSubjectMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuildContainer, iSubjectID);
	if (NONNULL(pMember) && NONNULL(pSubjectMember))
	{
		if(!gGuildConfig.bUseLeaderSlowDemotion || iSubjectID == iEntID || timeSecondsSince2000() < pSubjectMember->uProvisionalTimeSeconds || !aslGuild_trh_IsLeader(pGuildContainer, iSubjectID))
		{
			// instant demotion for non-leaders and self-demotion or if slow system is not being used
			pSubjectMember->iRank--;
		}
		else
		{
			// There is a leader being demoted already
			if(pGuildContainer->uDemoteLeaderID)
			{
				// already demoting a leader, this can't be done at this time
				return false;
			}

			pGuildContainer->uDemoteLeaderID = iSubjectID;
			pGuildContainer->uDemotionTimeSeconds = timeSecondsSince2000() + gGuildConfig.uDemotionDurationHours * SECONDS_PER_HOUR;
			aslGuild_trh_AddSuccessFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, iSubjectID, "GuildServer_DemotionStarted", false);
			aslGuild_AddSuccessLogMessage(ATR_GUILD_RETURN, pMember->pcLogName, pGuildContainer->iContainerID, pGuildContainer->pcName, "Leader Demotion of %s started by %s.", pSubjectMember->pcLogName, pMember->pcLogName);
			return true;
		}
...
bool aslGuild_CheckCanPromote(Guild *pGuild, U32 iEntID, U32 iSubjectID, const char *pcActionType) {
	...
	if (pMember && pSubjectMember) {
		...
		} else if (pMember->iRank == (pSubjectMember->iRank+1))
		{
			...
			if(pGuild->uDemoteLeaderID !=0 && aslGuild_trh_IsLeader(CONTAINER_NOCONST(Guild, pGuild), iSubjectID))
			{
				aslGuild_SendFeedback(iEntID, iEntID, iSubjectID, aslGuild_GetName(pGuild), pcActionType, "GuildServer_LeaderDemotionInProgress", false);
				return false;
			}
		} else {
...
bool aslGuild_CheckCanDemote(Guild *pGuild, U32 iEntID, U32 iSubjectID, const char *pcActionType) {
	...
	if (pMember && pSubjectMember) {
		if (pMember->iRank > pSubjectMember->iRank) {
			...
			if(pGuild->uDemoteLeaderID !=0 && aslGuild_trh_IsLeader(CONTAINER_NOCONST(Guild, pGuild), iEntID))
			{
				aslGuild_SendFeedback(iEntID, iEntID, iSubjectID, aslGuild_GetName(pGuild), pcActionType, "GuildServer_LeaderDemotionInProgress", false);
				return false;
			}
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Eamembers, .Pcname, .Earanks, .Pcallegiance, .Iversion, .Icontainerid, .Eainvites, .Udemoteleaderid")
...
enumTransactionOutcome aslGuild_tr_Invite(ATR_ARGS, NOCONST(Guild) *pGuildContainer, NOCONST(Entity) *pEnt, NOCONST(Entity) *pSubject)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_Invite)) {
	...
	// check to make sure leader isn't being demoted
	if(pGuildContainer->uDemoteLeaderID == pEnt->myContainerID)
	{
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, pEnt->myContainerID, pEnt->myContainerID, pSubject->myContainerID, "GuildServer_LeaderDemotionInProgress", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Eamembers, .Pguildstatsinfo, .Icontainerid, .Pcname, .Udemoteleaderid, .Udemotiontimeseconds")
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Eamembers, .Pcname, .Earanks, .Pguildstatsinfo, .Icontainerid, .Udemoteleaderid, .Udemotiontimeseconds")
...
enumTransactionOutcome aslGuild_tr_Kick(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, NOCONST(Entity) *pSubject)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_Remove) || pSubjectMember->iRank >= pMember->iRank) {
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Eamembers, .Pguildstatsinfo, .Icontainerid, .Udemoteleaderid, .Udemotiontimeseconds");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Iocversion, .Earanks, .Icontainerid, eaMembers[], eaMembers[], .Udemoteleaderid");
enumTransactionOutcome aslGuild_tr_Promote(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, U32 iSubjectID)
{
	...
	if (pMember->iRank > pSubjectMember->iRank+1) {
		if (!(aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_PromoteBelowRank) ||
			aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_PromoteToRank))) {
		...
		if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_PromoteToRank)) {
			aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, iSubjectID, "GuildServer_NoPromotePermission", true);
			GUILD_TRANSACTION_RETURN_FAILURE;
		}
	} else {
	...
	// Check to make sure that this player is not being promoted to leader during a leader demotion
	if (pSubjectMember->iRank+1 >= eaSize(&pGuildContainer->eaRanks)-1 && pGuildContainer->uDemoteLeaderID != 0)
	{
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, iSubjectID, "GuildServer_LeaderDemotionInProgress", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}

	// provisional leaders can't promote others to leader
	if(timeSecondsSince2000() < pMember->uProvisionalTimeSeconds && pSubjectMember->iRank+1 >= eaSize(&pGuildContainer->eaRanks)-1)
	{
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, iSubjectID, "GuildServer_CantPromoteToLeader", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}
...
// This is the final demotion of a leader who is being demoted after the wait time has expired
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Iocversion, .Pcname, .Earanks, .Icontainerid, .Udemoteleaderid, .Udemotiontimeseconds, .Eamembers");
enumTransactionOutcome aslGuild_tr_DemoteLeader(ATR_ARGS, NOCONST(Guild) *pGuildContainer)
{
	GUILD_TRANSACTION_INIT;
	NOCONST(GuildMember) *pSubjectMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuildContainer, pGuildContainer->uDemoteLeaderID);
	U32 uSubjectID = 0;

	if (ISNULL(pSubjectMember))
	{
		// can't find the member to demote
		GUILD_TRANSACTION_RETURN_FAILURE;
	}

	if(timeSecondsSince2000() < pGuildContainer->uDemotionTimeSeconds)
	{
		// not enough time yet
		GUILD_TRANSACTION_RETURN_FAILURE;
	}

	pGuildContainer->uDemoteLeaderID = 0;		// leader is now demoted
	pGuildContainer->uDemotionTimeSeconds = 0;	// clear demotion time

	if(pSubjectMember->iRank > 0)
	{	
		if(!aslGuild_trh_Demote(GUILD_TRH_PASS_ARGS, pGuildContainer, pSubjectMember->iEntID, pSubjectMember->iEntID))
		{
			GUILD_TRANSACTION_RETURN_FAILURE;
		}
	}
	...
	GUILD_TRANSACTION_RETURN_SUCCESS("GuildServer_Demoted_Leader", "Demoted ENTITYPLAYER[%u]", pSubjectMember->iEntID);
}


// Check to see if it is time to demote a leader that is demotion state
void aslGuild_DemoteLeader(Guild *pGuild)
{
	if(pGuild && pGuild->uDemoteLeaderID)
	{
		U32 uTm = timeSecondsSince2000();
		if(uTm > pGuild->uDemotionTimeSeconds)
		{
			// demote this leader
			char pcActionType[] = "Demote";
			TransactionReturnVal *pReturn = NULL;
			...
			pReturn = objCreateManagedReturnVal(aslGuild_RemoteCommand_CB, aslGuild_MakeCBData(pcActionType));
			AutoTrans_aslGuild_tr_DemoteLeader(pReturn, GetAppGlobalType(), GLOBALTYPE_GUILD, pGuild->iContainerID);
		}
	}
}

AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Iocversion, .Pcname, .Earanks, .Icontainerid, eaMembers[], eaMembers[], .Udemoteleaderid, .Udemotiontimeseconds");
enumTransactionOutcome aslGuild_tr_Demote(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, U32 iSubjectID)
{
	...
	if (pMember->iRank > pSubjectMember->iRank) {
		if (!(aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_DemoteBelowRank) ||
			aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_DemoteAtRank))) {
		...
		if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_DemoteAtRank)) {
			aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, iSubjectID, "GuildServer_NoDemotePermission", true);
			GUILD_TRANSACTION_RETURN_FAILURE;
		}
	} else {
	...
	if(pMember->iEntID != pSubjectMember->iEntID && timeSecondsSince2000() < pMember->uProvisionalTimeSeconds && pMember->iRank == pSubjectMember->iRank && aslGuild_trh_IsLeader(pGuildContainer, iEntID))
	{
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, iSubjectID, "GuildServer_ProvisionalCantDemoteLeader", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}

	if(!aslGuild_trh_Demote(GUILD_TRH_PASS_ARGS, pGuildContainer, iEntID, iSubjectID))
	{
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, iSubjectID, "GuildServer_FailedToDemote", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}
	pGuildContainer->iOCVersion++;
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Iversion, .Icontainerid, .Eamembers, .Udemoteleaderid");
enumTransactionOutcome aslGuild_tr_Rename(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, char *pcNewName)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_Rename)) {
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Icontainerid, .Eamembers, .Udemoteleaderid");
enumTransactionOutcome aslGuild_tr_RenameRank(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, S32 iRank, char *pcNewName)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_RenameRank)) {
...
enumTransactionOutcome aslGuild_tr_RenameBankTab(ATR_ARGS, NOCONST(Guild) *pGuildContainer, NOCONST(Entity) *pGuildBank, U32 iEntID, S32 iBagID, char *pcNewName)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_RenameBankTab)) {
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Iocversion, .Earanks, .Icontainerid, eaMembers[], .Udemoteleaderid");
enumTransactionOutcome aslGuild_tr_SetPermission(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, S32 iRank, U32 ePerms, U8 bOn)
{
	...
	if (iEntID) {
		...
		if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_SetPermission)) {
		...
		if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, ePerms)) {
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Eauniforms, .Pcname, .Earanks, eaMembers[], .Udemoteleaderid");
enumTransactionOutcome aslGuild_tr_SetUniformPermission(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, U32 iPermissionIndex, U32 iUniformIndex, const char *pcCategory, U8 bOn)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_SetLook)) {
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, 0, "GuildServer_GivingUnownedPermission", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}
...
enumTransactionOutcome aslGuild_tr_SetBankPermission(ATR_ARGS, NOCONST(Guild) *pGuildContainer, NOCONST(Entity) *pGuildBank, U32 iEntID, S32 iBagID, S32 iRank, U32 ePerm, U8 bOn)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_SetBankPermission)) {
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcmotd, .Imotdupdated, .Icontainerid, eaMembers[], .Bisownedbysystem, .Udemoteleaderid");
enumTransactionOutcome aslGuild_tr_SetMotD(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, char *pcMotD)
{
	...
	if(!pGuildContainer->bIsOwnedBySystem)
	{
		...
		if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_SetMotD)) {
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcdescription, .Icontainerid, eaMembers[], .Bisownedbysystem, .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcemblem, .Icontainerid, eaMembers[], .Bisownedbysystem, .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcemblem, .Iemblemcolor0, .Iemblemcolor1, .Femblemrotation, .Icontainerid, eaMembers[], .Bisownedbysystem, .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcemblem2, .Iemblem2color0, .Iemblem2color1, .Femblem2rotation, .Femblem2x, .Femblem2y, .Femblem2scalex, .Femblem2scaley, .Icontainerid, eaMembers[], .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcemblem3, .Icontainerid, eaMembers[], .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pguildbademblem, .Pcemblem, .Iemblemcolor0, .Iemblemcolor1, .Femblemrotation, .Pcemblem2, .Iemblem2color0, .Iemblem2color1, .Femblem2rotation, .Femblem2x, .Femblem2y, .Femblem2scalex, .Femblem2scaley, .Pcemblem3, .Icontainerid, .Pcname, .Earanks, eaMembers[], .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Icolor1, .Icolor2, .Icontainerid, eaMembers[], .Bisownedbysystem, .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Eauniforms, .Icontainerid, .Pcname, .Earanks, eaMembers[], .Udemoteleaderid");
			...
			for (j = eaSize(&pGuild->eaMembers)-1; j >= 0; --j)
			{
				...
				if (!pGuild->bHideMembers)
				{
					...
					if (gm->iRank >= 0 && gm->iRank < eaSize(&pGuild->eaRanks))
					{
						...
						{
							// GuildServer doesn't load messages
							grm->pcDefaultRankName = allocAddString(pGuild->eaRanks[gm->iRank]->pcDefaultNameMsg);
						}
					}
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcrecruitmessage, .Icontainerid, eaMembers[], .Udemoteleaderid");
enumTransactionOutcome aslGuild_tr_SetRecruitMessage(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, char *pcRecruitMessage)
{
	...
	if (!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_SetRecruitInfo)) {
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, 0, "GuildServer_NoRenamePermission", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Pcwebsite, .Icontainerid, eaMembers[], .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Earecruitcat, .Icontainerid, eaMembers[], .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Iminlevelrecruit, .Icontainerid, eaMembers[], .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Bhiderecruitmessage, .Icontainerid, eaMembers[], .Udemoteleaderid");
...
AUTO_TRANSACTION
ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, .Bhidemembers, .Icontainerid, eaMembers[], .Udemoteleaderid");
...
///////////////////////////////////////////////////////////////////////////////////////////
// Rescue a leader that is being demoted
///////////////////////////////////////////////////////////////////////////////////////////

AUTO_COMMAND_REMOTE ACMD_PACKETERRORCALLBACK;
void aslGuild_RescueLeader(U32 iGuildID, U32 iEntID)
{
	char pcActionType[] = "RescueLeader";
	TransactionReturnVal *pReturn = NULL;
	Guild *pGuild = aslGuild_GetGuild(iGuildID);

	if
	(
		pGuild->uDemoteLeaderID && pGuild->uDemoteLeaderID != iEntID &&
		aslGuild_CheckExists(pGuild, iGuildID, iEntID, pcActionType) &&
		aslGuild_CheckIsMember(pGuild, iEntID, pcActionType) &&
		aslGuild_trh_IsLeader(CONTAINER_NOCONST(Guild, pGuild), iEntID) && 
		aslGuild_CheckPermission(pGuild, iEntID, GuildPermission_PromoteToRank, false, pcActionType)
	)
	{
		pReturn = objCreateManagedReturnVal(aslGuild_RemoteCommand_CB, aslGuild_MakeCBData(pcActionType));
		AutoTrans_aslGuild_tr_RescueLeader(pReturn, GetAppGlobalType(), GLOBALTYPE_GUILD, iGuildID, iEntID);
	}
}

AUTO_TRANSACTION
	ATR_LOCKS(pGuildContainer, ".Pcname, .Earanks, eaMembers[], .Udemoteleaderid, .Udemotiontimeseconds");
enumTransactionOutcome aslGuild_tr_RescueLeader(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID)
{
	GUILD_TRANSACTION_INIT;
	NOCONST(GuildMember) *pMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuildContainer, iEntID);
	...
	if(!aslGuild_trh_HasPermission(GUILD_TRH_PASS_ARGS, pGuildContainer, pMember, GuildPermission_PromoteToRank) || 
		!aslGuild_trh_IsLeader(pGuildContainer, iEntID) || timeSecondsSince2000() < pMember->uProvisionalTimeSeconds)
	{
		aslGuild_trh_AddFailureFeedback(ATR_GUILD_RETURN, pGuildContainer, iEntID, iEntID, 0, "GuildServer_NoRescuePermission", true);
		GUILD_TRANSACTION_RETURN_FAILURE;
	}
...
///////////////////////////////////////////////////////////////////////////////////////////
// Test commands for leader demote time and Provisional time
///////////////////////////////////////////////////////////////////////////////////////////

AUTO_COMMAND_REMOTE ACMD_PACKETERRORCALLBACK;
void aslGuild_DemoteTime(U32 iGuildID, U32 uNewTime)
{
	char pcActionType[] = "RescueLeader";
	TransactionReturnVal *pReturn = NULL;
	Guild *pGuild = aslGuild_GetGuild(iGuildID);
	...
	{
		pReturn = objCreateManagedReturnVal(aslGuild_RemoteCommand_CB, aslGuild_MakeCBData(pcActionType));
		AutoTrans_aslGuild_tr_DemoteTime(pReturn, GetAppGlobalType(), GLOBALTYPE_GUILD, iGuildID, uNewTime);
	}
}

AUTO_COMMAND_REMOTE ACMD_PACKETERRORCALLBACK;
void aslGuild_ProvisionalTime(U32 iGuildID, U32 iEntID, U32 uNewTime)
{
	char pcActionType[] = "RescueLeader";
	TransactionReturnVal *pReturn = NULL;
	Guild *pGuild = aslGuild_GetGuild(iGuildID);
	...
	{
		pReturn = objCreateManagedReturnVal(aslGuild_RemoteCommand_CB, aslGuild_MakeCBData(pcActionType));
		AutoTrans_aslGuild_tr_ProvisionalTime(pReturn, GetAppGlobalType(), GLOBALTYPE_GUILD, iGuildID, iEntID, uNewTime);
	}
}

AUTO_TRANSACTION
	ATR_LOCKS(pGuildContainer, ".Udemoteleaderid, .Udemotiontimeseconds");
enumTransactionOutcome aslGuild_tr_DemoteTime(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 uNewTime)
{
	GUILD_TRANSACTION_INIT;

	if(pGuildContainer->uDemoteLeaderID)
	{
		pGuildContainer->uDemotionTimeSeconds = timeSecondsSince2000() + uNewTime;
	}
...
AUTO_TRANSACTION
	ATR_LOCKS(pGuildContainer, "eaMembers[]");
enumTransactionOutcome aslGuild_tr_ProvisionalTime(ATR_ARGS, NOCONST(Guild) *pGuildContainer, U32 iEntID, U32 uNewTime)
{
	GUILD_TRANSACTION_INIT;
	NOCONST(GuildMember) *pMember = aslGuild_trh_FindMember(GUILD_TRH_PASS_ARGS, pGuildContainer, iEntID);
	...
	if(NONNULL(pMember))
	{
		pMember->uProvisionalTimeSeconds = uNewTime;
	}
