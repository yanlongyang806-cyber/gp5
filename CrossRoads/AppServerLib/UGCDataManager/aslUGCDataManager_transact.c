...
AUTO_TRANSACTION
ATR_LOCKS(pUGCProject, ".pFeatured.fAverageDurationInMinutes_Override, .Pidstring, .Id");
enumTransactionOutcome trUGCFeaturedSetAverageDurationInMinutesOverride(ATR_ARGS, NOCONST(UGCProject)* pUGCProject, const char* pchCSRAccount, float averageDurationInMinutes)
{
	if( !ISNULL( pUGCProject->pFeatured )) {
		pUGCProject->pFeatured->fAverageDurationInMinutes_Override = averageDurationInMinutes;
		TRANSACTION_RETURN_LOG_SUCCESS("Project %s(%d) average duration in minutes updated to %f by account %s.",
									   pUGCProject->pIDString, pUGCProject->id,
									   averageDurationInMinutes,
									   pchCSRAccount);
	} else {
		TRANSACTION_RETURN_LOG_FAILURE("Project %s(%d) failed to set average duration in minutes to %f by account %s. Project has never been featured.",
									   pUGCProject->pIDString, pUGCProject->id,
									   averageDurationInMinutes,
									   pchCSRAccount);
	}
...
AUTO_TRANS_HELPER
ATR_LOCKS(pReviews, ".piNumRatings, .fRatingSum, .fAverageRating, .fAdjustedRatingUsingConfidence, .iTimeBecameReviewed, .ppReviews[], .eaTagData");
NOCONST(UGCSingleReview) *aslUGCDataManager_trh_AddReview(ATR_ARGS, ATH_ARG NOCONST(UGCProjectReviews) *pReviews, ContainerID iReviewerAccountID, const char *pReviewerAccountName,
	UGCReviewData *pUGCReviewData)
{
	...
	if(pUGCReviewData->fRating < 0 || pUGCReviewData->fRating > 1.0f)
	{
	...
	if(pUGCSingleReview)
	{
		...
		// Clear the old review data
		SAFE_FREE(pUGCSingleReview->pReviewerAccountName);
		SAFE_FREE(pUGCSingleReview->pComment);

		// Clear/Decrement aggregate data
		{
		...
		{
			NOCONST(UGCTagData) *pUGCTagData = eaIndexedGetUsingInt(&pReviews->eaTagData, eUGCTag);
			if(!pUGCTagData || 0 == pUGCTagData->iCount)
			{
				TRANSACTION_APPEND_LOG_FAILURE("UGC tag data corrupted");
				return NULL;
			}
			pUGCTagData->iCount--;
		}
		FOR_EACH_END;
		eaiClearFast(&pUGCSingleReview->eaiUGCTags);

		pReviews->fRatingSum -= pUGCSingleReview->fRating;
	}
	else
	{
		// Set new review data that does not change even if review is changed by player
	...
	// Set the new review data
	pUGCSingleReview->pReviewerAccountName = StructAllocString(pReviewerAccountName);
	pUGCSingleReview->fRating = pUGCReviewData->fRating;
	pUGCSingleReview->pComment = StructAllocString(pUGCReviewData->pComment);
	eaiCopy(&pUGCSingleReview->eaiUGCTags, &pUGCReviewData->eaiUGCTags);
	pUGCSingleReview->iTimestamp = timeSecondsSince2000();

	// Compute/Increment aggregate data
	pReviews->fRatingSum += pUGCReviewData->fRating;
	...
	{
		S32 iBucket = ugcReviews_FindBucketForRating(pUGCReviewData->fRating);
		if(iBucket >= 0 && iBucket < eaiSize(&pReviews->piNumRatings))
			pReviews->piNumRatings[iBucket]++;
	}
	...
	FOR_EACH_IN_EARRAY_INT(pUGCSingleReview->eaiUGCTags, UGCTag, eUGCTag)
	{
		...
		if(!pUGCTagData)
		{
			...
			eaIndexedPushUsingIntIfPossible(&pReviews->eaTagData, eUGCTag, pUGCTagData);
		}
		pUGCTagData->iCount++;
	}
	FOR_EACH_END;
...
AUTO_TRANSACTION
ATR_LOCKS(pUGCProject, ".id, .iOwnerAccountID, .ugcReviews.piNumRatings, .ugcReviews.fRatingSum, .ugcReviews.iTimeBecameReviewed, .ugcReviews.fAverageRating, .ugcReviews.fAdjustedRatingUsingConfidence, .ugcReviews.ppReviews[], .ugcReviews.eaTagData");
enumTransactionOutcome trReviewUgcProject(ATR_ARGS, NOCONST(UGCProject) *pUGCProject, ContainerID iReviewerAccountID, const char *pReviewerAccountName, S32 bBetaReviewing, UGCReviewData *pUGCReviewData)
{
	...
	pUGCSingleReview = aslUGCDataManager_trh_AddReview(ATR_RECURSE, &pUGCProject->ugcReviews, iReviewerAccountID, pReviewerAccountName, pUGCReviewData);
	if(pUGCSingleReview)
	{
		...
		event->ugcAchievementServerEvent->ugcProjectReviewedEvent->fRating = pUGCReviewData->fRating;
		...
		event->ugcAchievementServerEvent->ugcReviewedProjectEvent->fRating = pUGCReviewData->fRating;
		...
		pUGCSingleReview->fHighestRating = MAX(pUGCReviewData->fRating, pUGCSingleReview->fHighestRating);
...
AUTO_TRANSACTION
ATR_LOCKS(pUGCProjectSeries, ".id, .iOwnerAccountID, .ugcReviews.piNumRatings, .ugcReviews.fRatingSum, .ugcReviews.iTimeBecameReviewed, .ugcReviews.fAverageRating, .ugcReviews.fAdjustedRatingUsingConfidence, .ugcReviews.ppReviews[], .ugcReviews.eaTagData");
enumTransactionOutcome trReviewUgcProjectSeries(ATR_ARGS, NOCONST(UGCProjectSeries)* pUGCProjectSeries, ContainerID iReviewerAccountID, const char *pReviewerAccountName, UGCReviewData *pUGCReviewData)
{
	NOCONST(UGCSingleReview) *pUGCSingleReview = NULL;
	...
	pUGCSingleReview = aslUGCDataManager_trh_AddReview(ATR_RECURSE, &pUGCProjectSeries->ugcReviews, iReviewerAccountID, pReviewerAccountName, pUGCReviewData);
	if(pUGCSingleReview)
	{
		UGCAchievementEvent *event = StructCreate(parse_UGCAchievementEvent);
		...
		event->ugcAchievementServerEvent->ugcSeriesReviewedEvent ->fRating = pUGCReviewData->fRating;
		...
		event->ugcAchievementServerEvent->ugcReviewedSeriesEvent->fRating = pUGCReviewData->fRating;
...
AUTO_TRANSACTION
ATR_LOCKS(pUGCProject, ".Ugcreporting.Inaughtyvalue, .Ugcreporting.Unextnaughtydecaytime, .Ugcreporting.Utemporarybanexpiretime, .Ugcreporting.Eareports, .Ugcreporting.Itemporarybancount, .Ugcreporting.Bdisableautoban, .Bbanned, .Id, .pIDString, .Iowneraccountid, .Powneraccountname, .Iownerlangid, .pFeatured");
enumTransactionOutcome trReportUgcProject(ATR_ARGS, NOCONST(UGCProject) *pUGCProject, U32 iReporterAccountID, const char *pcReporterPublicAccountName, U32 eReason, const char *pchDetails)
{
	...
	else
	{
		...
		if (!pUGCProject->ugcReporting.uTemporaryBanExpireTime &&
			...
			!pUGCProject->ugcReporting.bDisableAutoBan && !pUGCProject->pFeatured)
		{
...
enumTransactionOutcome trSubscribeToAuthor(ATR_ARGS, NOCONST(UGCAccount)* pSubscriberAccount, ContainerID uSubscriberPlayerID, NOCONST(UGCAccount)* pAuthorAccount)
{
	...
	ugc_trh_AuthorSubscribedTo(ATR_PASS_ARGS, pAuthorAccount, uSubscriberPlayerID, pSubscriberAccount->accountID);
...
enumTransactionOutcome trUnsubscribeFromAuthor(ATR_ARGS, NOCONST(UGCAccount)* pSubscriberAccount, ContainerID uSubscriberPlayerID, NOCONST(UGCAccount)* pAuthorAccount)
{
	...
	ugc_trh_AuthorUnsubscribedFrom(ATR_PASS_ARGS, pAuthorAccount, uSubscriberPlayerID);
...
AUTO_TRANSACTION
ATR_LOCKS(pSubscriberAccount, ".accountID, .eaPlayers[], .author.subscribers.eaPlayers[]");
enumTransactionOutcome trSubscribeToAuthorSelf(ATR_ARGS, NOCONST(UGCAccount)* pSubscriberAccount, ContainerID uSubscriberPlayerID)
{
	if(TRANSACTION_OUTCOME_FAILURE == ugc_trh_SubscribeToAuthor(ATR_PASS_ARGS, pSubscriberAccount, uSubscriberPlayerID, pSubscriberAccount->accountID))
		return TRANSACTION_OUTCOME_FAILURE;

	ugc_trh_AuthorSubscribedTo(ATR_PASS_ARGS, pSubscriberAccount, uSubscriberPlayerID, pSubscriberAccount->accountID);

	TRANSACTION_RETURN_LOG_SUCCESS("UGC: Subscriber %u (Player %u) has subscribed to author self", pSubscriberAccount->accountID, uSubscriberPlayerID);
}

AUTO_TRANSACTION
ATR_LOCKS(pSubscriberAccount, ".accountID, .eaPlayers[], .author.subscribers.eaPlayers[]");
enumTransactionOutcome trUnsubscribeFromAuthorSelf( ATR_ARGS, NOCONST(UGCAccount)* pSubscriberAccount, ContainerID uSubscriberPlayerID)
{
	if(TRANSACTION_OUTCOME_FAILURE == ugc_trh_UnsubscribeFromAuthor(ATR_PASS_ARGS, pSubscriberAccount, uSubscriberPlayerID, pSubscriberAccount->accountID))
		return TRANSACTION_OUTCOME_FAILURE;

	ugc_trh_AuthorUnsubscribedFrom(ATR_PASS_ARGS, pSubscriberAccount, uSubscriberPlayerID);

	TRANSACTION_RETURN_LOG_SUCCESS("UGC: Subscriber %u (Player %u) has unsubscribed from author self", pSubscriberAccount->accountID, uSubscriberPlayerID);
}
...
AUTO_TRANS_HELPER;
static F32 aslUGCDataManager_trh_GetProjectDurationBucketAverage(ATH_ARG NOCONST(UGCProjectDurationStats)* pStats, int bucket)
{
	if( g_bUGCDurationAndCompletionStatsResetAfterPublish ) {
		return pStats->eaDurationBuckets[bucket]->uAverage;
	} else {
		int totalSum = pStats->eaDurationBuckets[bucket]->uSum + pStats->eaPrevVersionsDurationBuckets[bucket]->uSum;
		int totalCount = pStats->eaDurationBuckets[bucket]->uCount + pStats->eaPrevVersionsDurationBuckets[bucket]->uCount;
...
AUTO_TRANS_HELPER
ATR_LOCKS(pStats, ".eaDurationBuckets, .eaPrevVersionsDurationBuckets");
static F32 aslUGCDataManager_trh_GetAverageDurationInMinutes_IgnoreOutliers(ATR_ARGS, ATH_ARG NOCONST(UGCProjectDurationStats)* pStats)
{
	...
	if( !g_bUGCDurationAndCompletionStatsResetAfterPublish )
	{
		for (i=0; i < eaSize(&pStats->eaPrevVersionsDurationBuckets); i++) 
		{
			fTotalCount = (fTotalCount + pStats->eaPrevVersionsDurationBuckets[i]->uCount);
		}
	}
	...
	for (i=0; i < eaSize(&pStats->eaDurationBuckets) && iTempCount < iQ1Idx; i++) 
	{
		...
		if( !g_bUGCDurationAndCompletionStatsResetAfterPublish )
		{
			iTempCount = (iTempCount + pStats->eaPrevVersionsDurationBuckets[i]->uCount);
		}
	}
	if(i > 0)
	{
		i--;
		fQ1Val = aslUGCDataManager_trh_GetProjectDurationBucketAverage(pStats, i);
	}
	...
	{
		i++;
		fQ3Val = aslUGCDataManager_trh_GetProjectDurationBucketAverage(pStats, i);
	}
	...
	for (i=0; i < eaSize(&pStats->eaDurationBuckets); i++) 
	{
		int uAverage = aslUGCDataManager_trh_GetProjectDurationBucketAverage(pStats, i);		
		if( uAverage >= fQ1Val && uAverage <= fQ3Val)
		{
			fModifiedCount = (fModifiedCount + pStats->eaDurationBuckets[i]->uCount);
			fModifiedSum = (fModifiedSum + pStats->eaDurationBuckets[i]->uSum);

			if( !g_bUGCDurationAndCompletionStatsResetAfterPublish ) {
				fModifiedCount = (fModifiedCount + pStats->eaPrevVersionsDurationBuckets[i]->uCount);
				fModifiedSum = (fModifiedSum + pStats->eaPrevVersionsDurationBuckets[i]->uSum);
			}
		}
	}
...
AUTO_TRANS_HELPER
ATR_LOCKS(pStats, ".Completionstats.Ucurrentdaytimestamp, .Completionstats.Uremainingcompletedcount, .Completionstats.Eaicompletedcountbyday, .Durationstats.Eadurationbuckets, .durationStats.eaPrevVersionsDurationBuckets");
...
AUTO_TRANS_HELPER
ATR_LOCKS(pDurationStats, ".eaDurationBuckets, .eaPrevVersionsDurationBuckets, .iAverageDurationInMinutes_IgnoreOutliers");
...
// Take the current average play time, distribute that time proportionally over
// the project's maps, and then fill in those play times in each map 5 times
// This ensures that a single play through doesn't totally destroy the existing. 
// The times are distributed proportional to how long the player spent on each map.
// If all times were less than a minute, it just splits it up evenly.
AUTO_TRANS_HELPER;
void aslUGCDataManager_trh_FillInDummyPlaytimes(ATR_ARGS, ATH_ARG NOCONST(UGCProjectStats) *pStats)
{
	int iNewTotal = 0;
	int iOldTotal = pStats->durationStats.iAverageDurationInMinutes_IgnoreOutliers;
	int *eaiMapCompletionTimes = NULL;
	int iTimeAccum = 0;
	int i;

	if(0 == eaSize(&pStats->eaDurationStatsByMap))
		return;

	// Get the total time spent on all maps together
	FOR_EACH_IN_EARRAY_FORWARDS(pStats->eaDurationStatsByMap, NOCONST(UGCProjectMapDurationStats), mapstats)
	{
		iNewTotal += mapstats->durationStats.iAverageDurationInMinutes_IgnoreOutliers;
	}
	FOR_EACH_END;
	...
	FOR_EACH_IN_EARRAY_FORWARDS(pStats->eaDurationStatsByMap, NOCONST(UGCProjectMapDurationStats), mapstats)
	{
		...
		if (iNewTotal)
		{
			int iIndex = FOR_EACH_IDX(pStats->eaDurationStatsByMap, mapstats);
			int iOldTime = mapstats->durationStats.iAverageDurationInMinutes_IgnoreOutliers;
			iNewTime = ((float)iOldTime / iNewTotal) * iOldTotal;
		}
		else
		{
			iNewTime =  iOldTotal / eaSize(&pStats->eaDurationStatsByMap);
		}
		eaiPush(&eaiMapCompletionTimes, iNewTime);
		iTimeAccum += iNewTime;
	}
	FOR_EACH_END;

	// Because of integer truncation, some minutes may have been lost.
	// Distribute them as evenly as possible to preserve the overall time
	for (i = 0; i < (iOldTotal - iTimeAccum); i++)
	{
		int iIndex = i % eaiSize(&eaiMapCompletionTimes);
		eaiMapCompletionTimes[iIndex]++;
	}
	...
	FOR_EACH_IN_EARRAY_FORWARDS(pStats->eaDurationStatsByMap, NOCONST(UGCProjectMapDurationStats), mapstats)
	{
		...
		for (i = 0; i < 5; i++)
		{
			aslUGCDataManager_trh_RecordCompletionDuration(ATR_EMPTY_ARGS, &mapstats->durationStats, eaiMapCompletionTimes[iIndex]);
		}
	}
	FOR_EACH_END;

	eaiDestroy(&eaiMapCompletionTimes);
}

AUTO_TRANSACTION
ATR_LOCKS(pUGCProject, ".Ppprojectversions, .Ugcstats.Completionstats.Eaicompletedcountbyday, .Pidstring, .Ugcstats.Completionstats.Ucurrentdaytimestamp, .Ugcstats.Completionstats.Uremainingcompletedcount, .Ugcstats.Durationstats.Eadurationbuckets, .ugcStats.durationStats.eaPrevVersionsDurationBuckets, .Ugcstats.Durationstats.Iaveragedurationinminutes_Ignoreoutliers, .Ugcstats.Iaveragedurationinminutes_Usingmaps, .Ugcstats.Eadurationstatsbymap, .Bbanned, .Ugcreporting.Utemporarybanexpiretime, .ugcStats.bMapsFilledIn");
enumTransactionOutcome trRecordCompletion(ATR_ARGS, NOCONST(UGCProject)* pUGCProject, U32 uDurationInMinutes, U32 bRecordCompletion)
{
	if(ISNULL(pUGCProject))
		return TRANSACTION_OUTCOME_FAILURE;
	...
	if(gConf.bUGCAveragePlayingTimeUsesCustomMapPlayingTime && gConf.bUGCMigrateToAveragePlayingTimeUsingCustomMaps && !pUGCProject->ugcStats.bMapsFilledIn)
		aslUGCDataManager_trh_FillInDummyPlaytimes(ATR_PASS_ARGS, &pUGCProject->ugcStats);
	...
	// When a completion of an entire UGC mission is recorded, update the average duration using the maps for the most recent playable version
	{
		...
		if(pMostRecentPlayableVersion)
		{
			if(gConf.bUGCAveragePlayingTimeUsesCustomMapPlayingTime && gConf.bUGCMigrateToAveragePlayingTimeUsingCustomMaps && !pUGCProject->ugcStats.bMapsFilledIn)
			{
				...
				{
					eaPush(&pMostRecentPlayableVersion->ppMapNames, strdup(mapstats->pName));
				}
				FOR_EACH_END;

				pUGCProject->ugcStats.bMapsFilledIn = true;
			}
...
AUTO_TRANSACTION
ATR_LOCKS(pUGCProject, ".Ugcstats.Eadurationstatsbymap, .id, .ugcStats.bMapsFilledIn");
enumTransactionOutcome trRecordMapCompletion(ATR_ARGS, NOCONST(UGCProject)* pUGCProject, const char *pcBaseMapName, U32 uDurationInMinutes)
{
	...
	// lazily add map - this assumes all withdrawn projects were flagged as unplayable before the gConf flag goes live being true
	if(gConf.bUGCMigrateToAveragePlayingTimeUsingCustomMaps && !mapstats && !pUGCProject->ugcStats.bMapsFilledIn)
	{
		mapstats = StructCreateNoConst(parse_UGCProjectMapDurationStats);
		mapstats->pName = StructAllocString(pcBaseMapName);
		eaPush(&pUGCProject->ugcStats.eaDurationStatsByMap, mapstats);
	}
...
enumTransactionOutcome trWithdrawProject(ATR_ARGS, NOCONST(UGCProject) *pProject, NOCONST(UGCProjectSeries) *pProjectSeries, char *pComment)
{
	...
	return UGCProject_trh_WithdrawProject(ATR_RECURSE, pProject, pProjectSeries, /*bWithdraw=*/true, pComment);
}

AUTO_TRANSACTION
ATR_LOCKS(pUGCProject, ".ugcReviews.ppReviews, .ugcReviews.eaTagData");
enumTransactionOutcome trComputeAggregateTagDataForUGCProject(ATR_ARGS, NOCONST(UGCProject) *pUGCProject)
{
	return ugc_trh_ComputeAggregateTagData(ATR_RECURSE, &pUGCProject->ugcReviews);
}

AUTO_TRANSACTION
ATR_LOCKS(pUGCProjectSeries, ".ugcReviews.ppReviews, .ugcReviews.eaTagData");
enumTransactionOutcome trComputeAggregateTagDataForUGCProjectSeries(ATR_ARGS, NOCONST(UGCProjectSeries) *pUGCProjectSeries)
{
	return ugc_trh_ComputeAggregateTagData(ATR_RECURSE, &pUGCProjectSeries->ugcReviews);
}
