...
AUTO_COMMAND_REMOTE ACMD_INTERSHARD;
void aslUGCDataManager_FeaturedSetAverageDurationInMinutesOverride( const char* pcShardName, ContainerID entContainerID,
																	ContainerID projectID, const char *pchCSRAccount, float averageDurationInMinutes )
{
	TransactionReturnVal *retVal = aslUGCDataManager_CreateManagedReturnValWithSuccessAndErrorString(__FUNCTION__, pcShardName, GLOBALTYPE_ENTITYPLAYER, entContainerID, RemoteCommand_Intershard_CSR_Return);

	AutoTrans_trUGCFeaturedSetAverageDurationInMinutesOverride(
			retVal, GetAppGlobalType(),
			GLOBALTYPE_UGCPROJECT, projectID, pchCSRAccount, averageDurationInMinutes);
}

AUTO_COMMAND_REMOTE ACMD_INTERSHARD;
void aslUGCDataManager_ReviewProject(const char *pcShardName, ContainerID entContainerID,
	UGCShardReturnProjectReviewed *pUGCShardReturnProjectReviewed, ContainerID iProjectID, ContainerID iReviewerAccountID, const char *pReviewerAccountName, bool bBetaReviewing,
...
void aslUGCDataManager_ReviewProjectSeries(const char *pcShardName, ContainerID entContainerID,
	UGCShardReturnProjectSeriesReviewed *pUGCShardReturnProjectSeriesReviewed, ContainerID iProjectSeriesID, ContainerID iReviewerAccountID, const char *pReviewerAccountName,
	UGCReviewData *pUGCReviewData)
{
	TransactionReturnVal *retVal =
...
typedef struct AuthorSubscriptionData
{
	char *pcShardName;
	ContainerID entAccountID;
	UGCSubscriptionData *pUGCSubscriptionData;
} AuthorSubscriptionData;

static void aslUGCDataManager_SubscribeToAuthor_UGCAccountEnsureExists_CB(UGCAccount* pUGCAccount, AuthorSubscriptionData* pAuthorSubscriptionData)
{
	TransactionReturnVal *retVal =
		aslUGCDataManager_CreateManagedReturnValWithSuccessAndUserData(__FUNCTION__, pAuthorSubscriptionData->pcShardName, GLOBALTYPE_ENTITYPLAYER, pAuthorSubscriptionData->pUGCSubscriptionData->entContainerID, RemoteCommand_Intershard_gslUGC_SubscribeToAuthor_Return,
			parse_UGCSubscriptionData, pAuthorSubscriptionData->pUGCSubscriptionData);

	// We use a separate transaction for subscribing to self to resolve [NNO-17805]. We never want to pass the same container as 2 separate arguments to a transaction.
	if(pAuthorSubscriptionData->pUGCSubscriptionData->iAuthorID != pAuthorSubscriptionData->entAccountID)
		AutoTrans_trSubscribeToAuthor(retVal, GetAppGlobalType(),
			GLOBALTYPE_UGCACCOUNT, pAuthorSubscriptionData->entAccountID,
			pAuthorSubscriptionData->pUGCSubscriptionData->entContainerID,
			GLOBALTYPE_UGCACCOUNT, pAuthorSubscriptionData->pUGCSubscriptionData->iAuthorID);
	else
		AutoTrans_trSubscribeToAuthorSelf(retVal, GetAppGlobalType(),
			GLOBALTYPE_UGCACCOUNT, pAuthorSubscriptionData->entAccountID,
			pAuthorSubscriptionData->pUGCSubscriptionData->entContainerID);

	StructDestroy(parse_UGCSubscriptionData, pAuthorSubscriptionData->pUGCSubscriptionData);
	free(pAuthorSubscriptionData->pcShardName);
	free(pAuthorSubscriptionData);
}
...
{
	AuthorSubscriptionData *pAuthorSubscriptionData = (AuthorSubscriptionData *)calloc(1, sizeof(AuthorSubscriptionData));
	pAuthorSubscriptionData->pcShardName = strdup(pcShardName);
	pAuthorSubscriptionData->entAccountID = entAccountID;
	pAuthorSubscriptionData->pUGCSubscriptionData = StructClone(parse_UGCSubscriptionData, pUGCSubscriptionData);

	UGCAccountEnsureExists(entAccountID, aslUGCDataManager_SubscribeToAuthor_UGCAccountEnsureExists_CB, pAuthorSubscriptionData);
}

static void aslUGCDataManager_UnsubscribeFromAuthor_UGCAccountEnsureExists_CB(UGCAccount* pUGCAccount, AuthorSubscriptionData* pAuthorSubscriptionData)
{
	TransactionReturnVal *retVal =
		aslUGCDataManager_CreateManagedReturnValWithSuccessAndUserData(__FUNCTION__, pAuthorSubscriptionData->pcShardName, GLOBALTYPE_ENTITYPLAYER, pAuthorSubscriptionData->pUGCSubscriptionData->entContainerID, RemoteCommand_Intershard_gslUGC_UnsubscribeFromAuthor_Return,
			parse_UGCSubscriptionData, pAuthorSubscriptionData->pUGCSubscriptionData);

	// We use a separate transaction for unsubscribing from self to resolve [NNO-17805]. We never want to pass the same container as 2 separate arguments to a transaction.
	if(pAuthorSubscriptionData->pUGCSubscriptionData->iAuthorID != pAuthorSubscriptionData->entAccountID)
		...
		AutoTrans_trUnsubscribeFromAuthorSelf(retVal, GetAppGlobalType(),
			GLOBALTYPE_UGCACCOUNT, pAuthorSubscriptionData->entAccountID,
			pAuthorSubscriptionData->pUGCSubscriptionData->entContainerID);
...
void aslUGCDataManager_UnsubscribeFromAuthor(const char *pcShardName, ContainerID entAccountID, UGCSubscriptionData *pUGCSubscriptionData)
{
	...
	UGCAccountEnsureExists(entAccountID, aslUGCDataManager_UnsubscribeFromAuthor_UGCAccountEnsureExists_CB, pAuthorSubscriptionData);
}
...
void aslUGCDataManager_RecordCompletion(ContainerID iUGCProjectID, const char *pcVersionNameSpace, U32 uDurationInMinutes, U32 bRecordCompletion)
{
	...
	AutoTrans_trRecordCompletion(LoggedTransactions_CreateManagedReturnVal(__FUNCTION__, NULL, NULL), GetAppGlobalType(), GLOBALTYPE_UGCPROJECT, iUGCProjectID, uDurationInMinutes, bRecordCompletion);
}
	...
	else
	{
		...
		if(bIncludeSaved)
		{
			...
			while(pUGCProjectSeries = objGetNextObjectFromIterator(&iter))
				if(UGCProjectSeries_IsAccessible(pUGCProjectSeries))
...
void aslUGCDataManager_SendProjectContainerForImport(SlowRemoteCommandID iCmdID, UGCProject *pUGCProject, const char *previousShard, const char *comment, bool forceDelete)
{
	...
	// Do the import
	{
		...
		UGCAccountEnsureExists(pUGCProjectNoVersions->iOwnerAccountID, NULL, NULL);
...
AUTO_TRANSACTION ATR_LOCKS(pUGCProject, ".Iownerlangid, .Ppprojectversions, .Ppublishedversionname");
enumTransactionOutcome trDebugFixupNameAndLanguage(ATR_ARGS, NOCONST(UGCProject) *pUGCProject)
{
	FOR_EACH_IN_EARRAY(pUGCProject->ppProjectVersions, NOCONST(UGCProjectVersion), pUGCProjectVersion)
	{
		if(UGC_PUBLISHED == pUGCProjectVersion->eState_USEACCESSOR)
		{
			if(!pUGCProjectVersion->pName || !pUGCProjectVersion->pName[0] || 0 == strcmp("happy test map", pUGCProjectVersion->pName))
				StructCopyString(&pUGCProjectVersion->pName, pUGCProject->pPublishedVersionName);
			if(LANGUAGE_DEFAULT == pUGCProjectVersion->eLanguage)
				pUGCProjectVersion->eLanguage = pUGCProject->iOwnerLangID;
		}
	}
	FOR_EACH_END;
...
AUTO_COMMAND;
void DebugFixupNameAndLanguage(void)
{
	...
	objInitContainerIteratorFromType(GLOBALTYPE_UGCPROJECT, &iter);
	while(pUGCProject = objGetNextObjectFromIterator(&iter))
		AutoTrans_trDebugFixupNameAndLanguage(NULL, GetAppGlobalType(), GLOBALTYPE_UGCPROJECT, pUGCProject->id);
}

AUTO_TRANS_HELPER
ATR_LOCKS(pUGCProjectReviews, ".ppReviews, .eaTagData");
enumTransactionOutcome ugc_trh_DebugRandomTagsForUGCContent(ATR_ARGS, ATH_ARG NOCONST(UGCProjectReviews) *pUGCProjectReviews)
{
	S32 *eaiValues = NULL;
	S32 *eaiPercentChances = NULL;

	UGCTagFillAllKeysAndValues(/*keys=*/NULL, &eaiValues);

	eaiCreate(&eaiPercentChances);
	FOR_EACH_IN_EARRAY_INT(eaiValues, S32, value)
	{
		if(value >= eaiSize(&eaiPercentChances))
			eaiSetSize(&eaiPercentChances, value + 1);
		if(randomIntRange(0, 100) <= 50) // 50%
			eaiPercentChances[value] = randomIntRange(0, 100);
		else
			eaiPercentChances[value] = 0;
	}
	FOR_EACH_END;
	...
	FOR_EACH_IN_EARRAY(pUGCProjectReviews->ppReviews, NOCONST(UGCSingleReview), pUGCSingleReview)
	{
		...
		if(randomIntRange(0, 100) <= 20) // 20%
		{
			FOR_EACH_IN_EARRAY_INT(eaiValues, S32, value)
			{
				if(eaiPercentChances[value] > 0 && randomIntRange(0, 100) <= eaiPercentChances[value])
					eaiPushUnique(&pUGCSingleReview->eaiUGCTags, value);
			}
			FOR_EACH_END;
		}
	}
	...
	eaiDestroy(&eaiPercentChances);

	return ugc_trh_ComputeAggregateTagData(ATR_RECURSE, pUGCProjectReviews);
}

AUTO_TRANSACTION
ATR_LOCKS(pUGCProject, ".ugcReviews.ppReviews, .ugcReviews.eaTagData");
enumTransactionOutcome trDebugRandomTagsForUGCProject(ATR_ARGS, NOCONST(UGCProject) *pUGCProject)
{
	return ugc_trh_DebugRandomTagsForUGCContent(ATR_RECURSE, &pUGCProject->ugcReviews);
}

AUTO_TRANSACTION
ATR_LOCKS(pUGCProjectSeries, ".ugcReviews.ppReviews, .ugcReviews.eaTagData");
enumTransactionOutcome trDebugRandomTagsForUGCProjectSeries(ATR_ARGS, NOCONST(UGCProjectSeries) *pUGCProjectSeries)
{
	return ugc_trh_DebugRandomTagsForUGCContent(ATR_RECURSE, &pUGCProjectSeries->ugcReviews);
}

AUTO_COMMAND;
void DebugRandomTagsForUGCProject(ContainerID iUGCProjectID)
{
	AutoTrans_trDebugRandomTagsForUGCProject(NULL, GetAppGlobalType(), GLOBALTYPE_UGCPROJECT, iUGCProjectID);
}

AUTO_COMMAND;
void DebugRandomTagsForUGCProjectSeries(ContainerID iUGCProjectSeriesID)
{
	AutoTrans_trDebugRandomTagsForUGCProject(NULL, GetAppGlobalType(), GLOBALTYPE_UGCPROJECTSERIES, iUGCProjectSeriesID);
}

AUTO_COMMAND;
void DebugRandomTagsForAllUGCContent()
{
	UGCProject *pUGCProject;
	...
	objInitContainerIteratorFromType(GLOBALTYPE_UGCPROJECT, &iter);
	while(pUGCProject = objGetNextObjectFromIterator(&iter))
		AutoTrans_trDebugRandomTagsForUGCProject(NULL, GetAppGlobalType(), GLOBALTYPE_UGCPROJECT, pUGCProject->id);

	objInitContainerIteratorFromType(GLOBALTYPE_UGCPROJECTSERIES, &iter);
	while(pUGCProjectSeries = objGetNextObjectFromIterator(&iter))
		AutoTrans_trDebugRandomTagsForUGCProjectSeries(NULL, GetAppGlobalType(), GLOBALTYPE_UGCPROJECTSERIES, pUGCProjectSeries->id);
}
