...
static bool s_bAcquiredSeriesContainers = false;

static void AcquiredSeriesContainers(void)
{
	s_bAcquiredSeriesContainers = true;
}

static bool s_bAcquiredAccountContainers = false;

static void AcquiredAccountContainers(void)
{
	s_bAcquiredAccountContainers = true;
}
...
static int aslUGCDataManagerLibOncePerFrame(F32 fElapsed)
{
	...
	case UDML_STATE_INIT:
		...
		aslAcquireContainerOwnership(GLOBALTYPE_UGCPROJECTSERIES, AcquiredSeriesContainers);
		aslAcquireContainerOwnership(GLOBALTYPE_UGCACCOUNT, AcquiredAccountContainers);
	...
	case UDML_STATE_WHATS_HOT_WAITING:
		if(aslUGCDataManagerWhatsHot_InitComplete() && s_bAcquiredProjectContainers && s_bAcquiredSeriesContainers && s_bAcquiredAccountContainers)
		{
...
AUTO_STARTUP(UGCDataManager) ASTRT_DEPS(AS_TextFilter, UGCAchievements, UGCReporting, UGCTags);
...
void aslUGCDataManager_ServerMonitor_RecordRecentlyChangedPlayableNamespace(UGCPlayableNameSpaceData *pUGCPlayableNameSpaceData)
{
	...
	{
		StructDestroy(parse_UGCPlayableNameSpaceDataOverview, s_UGCDataManagerOverview.eaRecentlyChangedPlayableNamespaces[199]);
...
void aslUGCDataManager_ServerMonitor_RecordMissionRecentlyCompleted(const char *strNameSpace, U32 uDurationInMinutes)
{
	...
	{
		StructDestroy(parse_UGCMissionCompletedData, s_UGCDataManagerOverview.eaMissionsRecentlyCompleted[199]);
