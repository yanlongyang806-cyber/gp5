...
static void
BuildGamePermisionsList(GamePermissionDefs *gamePermissionDefs, PlayerType playerType, U32 daysSubscribed, U32 secondsPlayed, bool hasBeenBilled, STRING_EARRAY extraGamePermissions, bool bIsLifetime)
{
    ...
    for(i = 0; i < eaSize(&g_GamePermissions.eaTimedTokenList); ++i)
    {
        ...
        if ( currentTime >= timedPermission->iStartSeconds )
        {
			...
			else if(timedPermission->bLifetimeSubscriber)
			{
				// This is for a lifetime subscriber
				if(bIsLifetime)
				{
					timedPermissionGranted = true;
				}
...
DoGADRefresh(RefreshGADState *refreshState)
{
    ...
    BuildGamePermisionsList(s_gamePermissions, refreshState->playerType, daysSubscribed, refreshState->accountData->uTotalPlayedSS2000, (refreshState->accountData->bBilled || bHasCurrency), refreshState->extraGamePermissions, refreshState->isLifetime);
    ...
    AutoTrans_aslLogin2_tr_RefreshGAD(returnVal, GLOBALTYPE_LOGINSERVER, GLOBALTYPE_GAMEACCOUNTDATA, refreshState->accountID, s_AttributeList, s_gamePermissions,
        refreshState->isLifetime, refreshState->isPress, refreshState->accountData, &tmpExtraGamePermissions, refreshState->overrideLevel, refreshState->timeStarted);
