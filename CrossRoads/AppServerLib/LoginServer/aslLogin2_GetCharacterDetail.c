...
AUTO_STRUCT;
typedef struct FixupAndGetDetailsState
{
    ContainerID accountID;
    ContainerID playerID;
    bool returnActivePuppets;
...
AUTO_COMMAND_REMOTE ACMD_INTERSHARD ACMD_IFDEF(OBJECTDB) ACMD_IFDEF(APPSERVER);
void
aslLogin2_ReturnCharacterDetail(U32 playerID, Login2CharacterDetailDBReturn *detailReturn, U64 requestToken, bool failed, char *errorString)
{
    ...
    if ( detailReturn )
    {
        ...
        for ( i = eaSize(&detailReturn->activePuppetStrings) - 1; i >= 0; i-- )
        {
            ...
            if ( ParserReadText(detailReturn->activePuppetStrings[i], parse_Entity, puppetEnt, 0) )
			{
		...
		for ( i = eaSize(&detailReturn->teamRequestPetStrings) - 1; i >= 0; i-- )
		{
			Entity *puppetEnt = StructCreateWithComment(parse_Entity, "Login2 character detail puppet entity.");
			if ( ParserReadText(detailReturn->teamRequestPetStrings[i], parse_Entity, puppetEnt, 0) )
			{
				eaPush(&characterDetail->teamRequestEnts, puppetEnt);
			}
			else
			{
				StructDestroy(parse_Entity, puppetEnt);
...
void
aslLogin2_GetCharacterDetail(ContainerID accountID, ContainerID playerID, const char *shardName, bool returnActivePuppets, bool returnTeamRequestPets, bool fixupRequired, GetCharacterDetailCB cbFunc, void *userData)
{
    ...
    if ( fixupRequired )
    {
        RemoteCommand_Intershard_aslLogin2_FixupAndGetCharacterDetail(shardName, GLOBALTYPE_LOGINSERVER, aslLogin2_GetRandomServerOfTypeInShard(shardName, GLOBALTYPE_LOGINSERVER), &myDestination, accountID, playerID, returnActivePuppets, returnTeamRequestPets);
    }
    else
    {
        RemoteCommand_Intershard_dbLogin2_GetCharacterDetail(shardName, GLOBALTYPE_OBJECTDB, 0, &myDestination, accountID, playerID, returnActivePuppets, returnTeamRequestPets);
    }
}

void
EntityFixupCB(ContainerID entityID, bool succeeded, FixupAndGetDetailsState *fixupAndGetDetailsState)
{
    if ( succeeded )
    {
        // Not what fixup is done tell the objectDB to send the character details.
        RemoteCommand_Intershard_dbLogin2_GetCharacterDetail(fixupAndGetDetailsState->returnDestination->shardName, GLOBALTYPE_OBJECTDB, 0, 
            fixupAndGetDetailsState->returnDestination, fixupAndGetDetailsState->accountID, fixupAndGetDetailsState->playerID, fixupAndGetDetailsState->returnActivePuppets, fixupAndGetDetailsState->returnTeamRequestPets);
    }
    else
    {
        RemoteCommand_Intershard_aslLogin2_ReturnCharacterDetail(fixupAndGetDetailsState->returnDestination->shardName, fixupAndGetDetailsState->returnDestination->serverType,
            fixupAndGetDetailsState->returnDestination->serverID, fixupAndGetDetailsState->playerID, NULL, fixupAndGetDetailsState->returnDestination->requestToken, true, "Fixup failed");
    }

    StructDestroy(parse_FixupAndGetDetailsState, fixupAndGetDetailsState);
}

AUTO_COMMAND_REMOTE ACMD_INTERSHARD ACMD_IFDEF(APPSERVER);
void
aslLogin2_FixupAndGetCharacterDetail(ACMD_OWNABLE(Login2InterShardDestination) ppReturnDestination, U32 accountID, U32 playerID, bool returnActivePuppets, bool returnTeamRequestPets)
{
    FixupAndGetDetailsState *fixupAndGetDetailsState;

    fixupAndGetDetailsState = StructCreate(parse_FixupAndGetDetailsState);
    fixupAndGetDetailsState->accountID = accountID;
    fixupAndGetDetailsState->playerID = playerID;
    fixupAndGetDetailsState->returnActivePuppets = returnActivePuppets;
    fixupAndGetDetailsState->returnTeamRequestPets = returnTeamRequestPets;

    // Take ownership of the destination struct.
    fixupAndGetDetailsState->returnDestination = *ppReturnDestination;
    *ppReturnDestination = NULL;

    aslLogin2_RequestPlayerEntityFixup(playerID, accountID, true, EntityFixupCB, fixupAndGetDetailsState);
}
