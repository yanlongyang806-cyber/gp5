...
typedef struct GADPurchasePacketData GADPurchasePacketData;
...
typedef struct Login2State
{
    ...
    // The refresh time that we expect to find in the GameAccountData container after the GAD refresh transaction has run.
    U32 expectedGADRefreshTime;
	...
	//is in a queue, currently
	bool isQueued;

	//if in a queue, it's the VIP queue
    bool isQueueVIP;

	//is all the way through the queue
	bool isThroughQueue;
	...
	bool gameAccountDataFixupComplete;	
	bool entitySharedBankCheckComplete;	
	...
	GADPurchasePacketData *gadPurchasePacketData;
...
#define LOGIN2_STATE_FIXUP_GAME_ACCOUNT_DATA "Login2FixupGameAccountData"
...
#define LOGIN2_STATE_GAD_PURCHASE "Login2GADPurchase"
...
#define LOGIN2_STATE_ENTITY_SHARED_BANK_CHECK "Login2EntitySharedBankCheck"
...
bool aslLogin2_ValidateLoginState(Login2State *loginState);

LATELINK;
void gameSpecific_FixupGameAccountData(Login2State *loginState, TransactionReturnCallback cbFunc, void *cbData);

LATELINK;
U32 gameSpecific_GetGameAccountDataFixupVersion(void);
