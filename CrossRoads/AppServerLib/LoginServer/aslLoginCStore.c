...
static void PurchaseCStoreProduct(U32 iAccountID, SA_PARAM_NN_STR const char *pCategory, U32 uProductID, SA_PARAM_OP_VALID const Money *pExpectedPrice, SA_PARAM_OP_STR const char *pPaymentMethodVID)
{
	...
	if(pMTCategory)
	{
		if(iAccountID && uProductID)
		{
            ...
            APPurchaseProduct(NULL, iAccountID, loginState->clientLanguageID, pcBuffer, uProductID, pExpectedPrice, 0, pPaymentMethodVID, pcIP, 0, false);
		}
	}
	else if(pPaymentMethodVID && iAccountID)
	{
		APPurchaseProduct(NULL, iAccountID, loginState->clientLanguageID, pCategory, uProductID, pExpectedPrice, 0, pPaymentMethodVID, pcIP, 0, false);
	}
}
