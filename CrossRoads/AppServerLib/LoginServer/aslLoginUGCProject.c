...
{
	return (gConf.bUserContent && gbUGCGenerallyEnabled && gbUGCEnabled) || ugc_DevMode;
}
...
void aslLoginUGCProject_SearchPossibleUGCProjects_Return(UGCProjectSearchInfo *pSearchInfo)
{
	...
	if (!aslLogin2_ValidateLoginState(loginState) || !aslLogin2_FailIfNotInState(loginState, LOGIN2_STATE_UGC_PROJECT_SELECT))
    {
		return;
    }
...
U32 aslLoginUGCGetCreateProjectEULACrc(GameAccountData *gameAccountData)
{
    char *strValue = AccountProxyFindValueFromKeyContainer(gameAccountData->eaAccountKeyValues, GetAccountUgcCreateProjectEULAKey());
	return (U32)(strValue ? strtoul(strValue, NULL, 10) : 0);
}
...
static void aslLoginUGCFillInSearchInfo(GameAccountData *gameAccountData, ContainerID accountID, U64 loginCookie, S32 clientAccessLevel, UGCProjectSearchInfo *pSearchInfo)
{
	...
	pSearchInfo->uAccountAcceptedProjectEULACrc = aslLoginUGCGetCreateProjectEULACrc(gameAccountData);
...
void aslLogin_ClearAuthorAllowsFeatured_Return(bool bSuccess, U64 loginCookie)
{
    ...
    // Invalid login link, bail
    if (!aslLogin2_ValidateLoginState(loginState) || !aslLogin2_FailIfNotInState(loginState, LOGIN2_STATE_UGC_CLEAR_AUTHOR_ALLOWS_FEATURED))
    {
        return;
    }
...
void aslLoginUGCProjectSeriesCreate( Packet* pak, Login2State* loginState )
{
	...
	UTF8NormalizeAllStringsInStruct( parse_UGCProjectSeries, series );
