...
void
aslLogin2_GetCharacterChoices(ContainerID accountID, GetCharacterChoicesCB cbFunc, void *userData, bool forcePopulateDeletedChoiceLists)
{
    ...
    for ( i = eaSize(&clusterOverview->ppShards) - 1; i >= 0; i-- )
    {
        ...
        if ( ( shardSummary->eState == CLUSTERSHARDSTATE_CONNECTED ) || ( shardSummary->eState == CLUSTERSHARDSTATE_THATS_ME ) )
        {
            ...
            RemoteCommand_Intershard_dbLogin2_GetCharacterChoices(shardState->shardName, GLOBALTYPE_OBJECTDB, 0, &myDestination, accountID, forcePopulateDeletedChoiceLists);
        }
        else
        {
