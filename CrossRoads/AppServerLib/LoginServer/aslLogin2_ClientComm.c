...
void
aslLogin2_SendSupportedLanguages(Login2State*loginState)
{
	const int* eaSupportedLanguages = langGetSupportedLanguagesThisShard();
	int numSupportedLanguages = eaiSize( &eaSupportedLanguages );
    Packet *pkt;
	int it;

	assert( eaSupportedLanguages );
    pkt = pktCreate(loginState->netLink, LOGIN2_TO_CLIENT_SUPPORTED_LANGUAGES);
	pktSendU32( pkt, eaiSize( &eaSupportedLanguages ));
	for( it = 0; it != eaiSize( &eaSupportedLanguages ); ++it ) {
		pktSendU32( pkt, eaSupportedLanguages[ it ]);
	}
    pktSend(&pkt);
}

static void aslLogin2_FailLoginString(Login2State *loginState, const char *errorMessageEnglish, const char *errorMessageLocalized)
{
	loginState->loginFailed = true;

	if(g_Login2PrintFailures || g_isContinuousBuilder)
	{
	...
	if (loginState->netLink)
	{
		...
		pktSendString(pak, errorMessageLocalized);
		pktSend(&pak);

		aslLogin2_Log("LoginFailure: accountID=%u, error=%s", loginState->accountID, errorMessageLocalized);

		linkFlushAndClose(&loginState->netLink, "Login Failed");
	}
	else
	{
		aslLogin2_Log("LoginFailure", "%s (NO LINK)", errorMessageEnglish);
	}
}

void aslLogin2_FormatFailLogin(Login2State *loginState, const char *errorMessageKey, ...)
{
	char *errorStringEnglish = NULL;
	char *errorStringLocalized = NULL;
	va_list va;

	// Get english version of error string for logging, builder and dev errors.
	estrStackCreate(&errorStringEnglish);
	va_start(va, errorMessageKey);
	langFormatMessageKeyV(LANGUAGE_ENGLISH, &errorStringEnglish, (char*) errorMessageKey, va);
	va_end(va);

	//Get localized version
	estrStackCreate(&errorStringLocalized);
	va_start(va, errorMessageKey);
	langFormatMessageKeyV(loginState->clientLanguageID, &errorStringLocalized, (char*) errorMessageKey, va);
	va_end(va);

	aslLogin2_FailLoginString(loginState, errorStringEnglish, errorStringLocalized);

	estrDestroy(&errorStringEnglish);
	estrDestroy(&errorStringLocalized);
}

void
	aslLogin2_FailLogin(Login2State *loginState, const char *errorMessageKey)
{
	const char *errorMessageEnglish = NULL;
	const char *errorMessageLocalized = NULL;
	errorMessageEnglish = langTranslateMessageKeyDefault(LANGUAGE_ENGLISH, errorMessageKey, errorMessageKey);
	errorMessageLocalized = langTranslateMessageKeyDefault(loginState->clientLanguageID, errorMessageKey, errorMessageKey);

	aslLogin2_FailLoginString(loginState, errorMessageEnglish, errorMessageLocalized);
}
...
GADPurchasePacketData *
	aslLogin2_ReadGADPurchasePacket(Packet *pak)
{
	GADPurchasePacketData *packetData;
	packetData = StructCreate(parse_GADPurchasePacketData);
	packetData->characterID = pktGetU32(pak);
	packetData->purchaseDefName = pktMallocString(pak);
...
aslLogin2_ReadPossibleUGCProject(Packet *pak)
{
	...
	UTF8NormalizeAllStringsInStruct( parse_PossibleUGCProject, possibleUGCProject );
    return possibleUGCProject;
}

static void
RemoteCommandDBImportEntityFromFile_CB(TransactionReturnVal *returnVal, Login2State *loginState)
{
    U32 val;
    enumTransactionOutcome eOutcome;
    eOutcome = RemoteCommandCheck_DBImportEntityFromFile(returnVal, &val);
    ...
    {   
        Packet *pPak;
        pPak = pktCreate(loginState->netLink, LOGINSERVER_TO_CLIENT_RELOAD_CHARACTER_CHOICES);
        pktSend(&pPak);
    }
}

static void
aslLogin2_DBImportCharacter(Packet *pak, Login2State *loginState)
{
    char fileName[MAX_PATH];
    bool overwriteAccountData;
    pktGetString(pak, fileName, MAX_PATH);
    overwriteAccountData = pktGetBool(pak);
    RemoteCommand_DBImportEntityFromFile(
        objCreateManagedReturnVal(RemoteCommandDBImportEntityFromFile_CB, loginState),
        GLOBALTYPE_OBJECTDB, 0,
        fileName, loginState->accountName, loginState->accountDisplayName, loginState->accountID, overwriteAccountData);  
}
...
aslLogin2_HandleInput(Packet* pak, int cmd, NetLink* netLink, Login2State *loginState)
{
    ...
    case TOLOGIN_LOGIN2_RENAME_CHARACTER:
        ...
        break;
	case TOLOGIN_LOGIN2_GAD_PURCHASE:
		if ( aslLogin2_FailIfNotInState(loginState, LOGIN2_STATE_CHARACTER_SELECT) )
		{
			// If a pending rename request already exists, then ignore this one.
			if ( loginState->gadPurchasePacketData == NULL )
			{
				loginState->gadPurchasePacketData = aslLogin2_ReadGADPurchasePacket(pak);
			}
		}
		break;
    ...
    case TOLOGIN_TODB_IMPORT_CONTAINER:
        aslLogin2_DBImportCharacter(pak, loginState);
        break;
