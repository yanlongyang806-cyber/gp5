...
bool g_SkipPermissionsForTrustedClient = true;
...
static U32 s_TimeoutIntershard = 120;
...
static U32 s_TimeoutClient = 120;
...
static U32 s_TimeoutAccount = 120;
...
static U32 s_TimeoutTransaction = 120;
...
static U32 s_TimeoutInshard = 120;
...
static U32 s_TimeoutGameserver = 240;
...
// This setting is a comma separated list of shards that indicates what order they should be shown in the shard selection UI during character creation.
char *s_CreationShardOrderListStr = NULL;
AUTO_CMD_ESTRING(s_CreationShardOrderListStr, CreationShardOrderList) ACMD_AUTO_SETTING(LoginServer, LOGINSERVER) ACMD_CALLBACK(aslLogin2_CreationShardOrderListUpdate);
...
AUTO_CMD_ESTRING(s_AddictionPlaytimeLimitKeyValueName, AddictionPlaytimeLimitKeyValueName) ACMD_AUTO_SETTING(LoginServer, LOGINSERVER);
...
AUTO_CMD_ESTRING(s_NoAddictionPlaytimeLimitKeyValueName, NoAddictionPlaytimeLimitKeyValueName) ACMD_AUTO_SETTING(LoginServer, LOGINSERVER);
...
static char **s_CharCreationOrderShards = NULL;

void
aslLogin2_CreationShardOrderListUpdate(CMDARGS)
{
    if ( s_CharCreationOrderShards != NULL )
    {
        // Clear the old array of shard names.
        eaDestroyEString(&s_CharCreationOrderShards);
        s_CharCreationOrderShards = NULL;
    }

    if ( s_CreationShardOrderListStr != NULL && s_CreationShardOrderListStr[0] != '\0' )
    {
        // Parse the string from the auto setting into the array of shard names.
        estrTokenize(&s_CharCreationOrderShards, ",", s_CreationShardOrderListStr);
    }
}

static U32
GetShardDisplayOrder(const char *shardName)
{
    int i;

    for ( i = eaSize(&s_CharCreationOrderShards) - 1; i >= 0; i-- )
    {
        if ( stricmp(shardName, s_CharCreationOrderShards[i]) == 0 )
        {
            return i;
        }
    }
...
aslLogin2_UpdateClusterStatus(void)
{
    ...
    for ( i = eaSize(&clusterOverview->ppShards) - 1; i >= 0; i-- )
    {
        ...
        if ( shardStatus )
        {
            ...
            // Get the display order for this shard.
            shardStatus->displayOrder = GetShardDisplayOrder(shardSummary->pShardName);
        }
    }
}
...
PushStateForDebug(Login2State *loginState, char *stateName)
{
    ...
    if ( g_Login2PrintStateChanges || g_isContinuousBuilder )
    {
...
bool
aslLogin2_ValidateLoginState(Login2State *loginState)
{
    if ( loginState == NULL )
    {
        return false;
    ...
    if ( !linkConnected(loginState->netLink) )
    {
...
Login2InitialConnection_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	...
	aslLogin2_SendSupportedLanguages(loginState);
}
...
Login2InitialConnection_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    if ( loginState->beginLoginPacketData )
    {
        ...
        aslLogin2_Log("ClientInitialConnection(IP %s%s AccountID %u)", makeIpStr(loginState->clientIP), loginState->isProxy ? "-proxied" : "", loginState->accountID);
    }
    else if ( loginState->redirectLoginPacketData )
    {
        ...
        aslLogin2_Log("ClientRedirect(IP %s%s AccountID %u CrypticAccountName %s LanguageID %u)", makeIpStr(loginState->clientIP), loginState->isProxy ? "-proxied" : "", loginState->accountID, loginState->accountName, loginState->clientLanguageID);
...
Login2ValidateTicket_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    if ( loginState->accountTicket )
    {
        ...
        aslLogin2_Log("ClientTicketValidated(IP %s%s AccountID %u CrypticAccountName %s LanguageID %u)", makeIpStr(loginState->clientIP), loginState->isProxy ? "-proxied" : "", loginState->accountID, loginState->accountName, loginState->clientLanguageID);

        if ( g_SkipPermissionsForTrustedClient && loginState->isClientOnTrustedIP )
        {
        ...
        {
            AccountPermissionStruct* productPermissions = NULL;
            AccountPermissionStruct* specificPermissions;
            AccountPermissionStruct* allPermissions;
            char **tokens = NULL;
            bool tokensAllowPlay;
            bool unrestrictedLogin = false;
            
            specificPermissions = findGamePermissionsByShard(loginState->accountTicket, GetProductName(), GetShardCategoryFromShardInfoString());
            allPermissions = findGamePermissionsByShard(loginState->accountTicket, GetProductName(), "all");
            if (specificPermissions && allPermissions) // get highest access level if both exists
            {
                productPermissions = specificPermissions->iAccessLevel > allPermissions->iAccessLevel ? specificPermissions : allPermissions;
            }
            else
            {
                if (specificPermissions)
                {
                    productPermissions = specificPermissions;
                }
                else if (allPermissions)
                {
                    productPermissions = allPermissions;
                }
            }
			...
			if(loginState->clientAccessLevel < ACCESS_GM && !langIsSupportedThisShard((Language)loginState->clientLanguageID))
			{
				aslLogin2_FailLogin(loginState, "Login2_UnsupportedLanguage");
				return;
			}
...
Login2GenerateOneTimeCode_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    if ( loginState->oneTimeCodePacketData )
    {
        ...
        if ( ( oneTimeCodePacketData->machineName == NULL ) )
        {
...
// return the gameacountdata version 
U32 DEFAULT_LATELINK_gameSpecific_GetGameAccountDataFixupVersion(void)
{
	return 0;
}

// returns true if the new version is greater than the old version
bool aslLogin2_GameAccountDataNeedsFixUp(GameAccountData *gameAccountData)
{
	if(!gConf.bDontAllowGADModification && gameSpecific_GetGameAccountDataFixupVersion() > gameAccountData->uGameSpecficGameAccountDataVersion)
	{
		return true;
	}
	return false;
...
static void
RefreshGameAccountDataCompletionCB(ContainerID accountID, bool succeeded, U32 highestLevel, U32 numCharacters, U32 lastLogoutTime, U32 refreshTime, Login2CookieStruct *cookieStruct)
{
    char idBuf[128];
    Login2State *loginState;
    ...
    loginState->expectedGADRefreshTime = refreshTime;
...
static void
	CheckEntitySharedBankCompletionCB(ContainerID accountID, bool succeeded,Login2CookieStruct *cookieStruct)
{
	Login2State *loginState;
	...
	if ( !succeeded )
	{
		aslLogin2_FailLogin(loginState, "Login2_CheckEntitySharedBankFailed");
		return;
	}

	loginState->entitySharedBankCheckComplete = true;
}

//returns true if account is suspended or banned
static bool 
IsAccountBanned(CONST_EARRAY_OF(AccountProxyKeyValueInfoContainer) accountKeyValues)
{
    ...
    banString = AccountProxyFindValueFromKeyContainer(accountKeyValues, GetAccountBannedKey());
...
static bool 
IsAccountSuspended(CONST_EARRAY_OF(AccountProxyKeyValueInfoContainer) accountKeyValues)
{
    ...
    suspendString = AccountProxyFindValueFromKeyContainer(accountKeyValues, GetAccountSuspendedKey());

    if (suspendString)
    {
        S32 suspendEndTime = atoi(suspendString);
        if(suspendEndTime == 0 || (suspendEndTime > 0 && (U32)suspendEndTime > timeSecondsSince2000()))
        {
            return true;
        }
    }
...
Login2RefreshGameAccountData_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    // Set this to zero, so we can tell when it has been properly set.
    loginState->expectedGADRefreshTime = 0;
...
Login2RefreshGameAccountData_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    if ( gameAccountData && loginState->expectedGADRefreshTime && ( gameAccountData->uLastRefreshTime == loginState->expectedGADRefreshTime ) )
    {
        // Set a flag indicating that the player has UGC project slots.
        loginState->playerHasUGCProjectSlots = ( ( Login2_UGCGetProjectMaxSlots(gameAccountData) > 0 ) || ( Login2_UGCGetSeriesMaxSlots(gameAccountData) > 0 ) );

        if ( IsAccountBanned(gameAccountData->eaAccountKeyValues) )
        {
            aslLogin2_FailLogin(loginState, "Login2_AccountBanned");
            return;
        }

        if ( IsAccountSuspended(gameAccountData->eaAccountKeyValues) )
        {
            aslLogin2_FailLogin(loginState, "Login2_AccountTempSuspended");
            return;
        }

		// Check to see if the game account data needs a fix-up
		// If it does go into the gad fix-up state
		// Note that this check will never succeed if gConf.bDontAllowGADModification = true
		if(aslLogin2_GameAccountDataNeedsFixUp(gameAccountData))
		{
			ISM_SwitchToSibling(LOGIN2_STATE_MACHINE, loginState, LOGIN2_STATE_FIXUP_GAME_ACCOUNT_DATA);
			return;
		}
		...
		// Switch to Entity shared bank check
		ISM_SwitchToSibling(LOGIN2_STATE_MACHINE, loginState, LOGIN2_STATE_ENTITY_SHARED_BANK_CHECK);
...
//////////////////////////////////////////////////////////////////////////
//
// Code for state LOGIN2_STATE_FIXUP_GAME_ACCOUNT_DATA
//
//////////////////////////////////////////////////////////////////////////

void DEFAULT_LATELINK_gameSpecific_FixupGameAccountData(Login2State *loginState, TransactionReturnCallback cbFunc, void *cbData)
{

}

static void
	FixupGameAccountData_CB(TransactionReturnVal *returnVal, void *userData)
{
	Login2State *loginState = aslLogin2_GetActiveLoginStateShortCookiePointer(userData);
	...
	if ( returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS )
	{
		// fix-up was completed successfully.
		loginState->gameAccountDataFixupComplete = true;
	}
	else
	{
		aslLogin2_FailLogin(loginState, "Login2_FixupGameAccountData");
	}
}

// The enter for the gad fix-up
static void 
	Login2FixupGameAccountData_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	if ( !Login2EnterStateCommon(loginState, LOGIN2_STATE_FIXUP_GAME_ACCOUNT_DATA, s_TimeoutTransaction) )
	{
		return;
	}

	loginState->gameAccountDataFixupComplete = false;

	// do fix-up transaction here transaction here
	gameSpecific_FixupGameAccountData(loginState, FixupGameAccountData_CB, aslLogin2_GetShortLoginCookieAsPointer(loginState));
}

static void 
	Login2FixupGameAccountData_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	if ( !Login2BeginFrameCommon(loginState) )
	{
		return;
	...
	if(!loginState->gameAccountDataFixupComplete)	
	{
		return;
	}

	// Send basic info to the client now that we have GameAccountData and its fixed up
	aslLogin2_SendLoginInfo(loginState);
...
//////////////////////////////////////////////////////////////////////////
//
// Code for state LOGIN2_STATE_ENTITY_SHARED_BANK_CHECK
//
//////////////////////////////////////////////////////////////////////////

// The enter for the entity shared bank
static void 
	Login2EntitySharedBankCheck_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	if ( !Login2EnterStateCommon(loginState, LOGIN2_STATE_ENTITY_SHARED_BANK_CHECK, s_TimeoutTransaction) )
	{
		return;
	}

	loginState->entitySharedBankCheckComplete = false;

	// Check to make sure the shared bank is subscribed.
	aslLogin2_CheckEntitySharedBank(loginState->accountID, CheckEntitySharedBankCompletionCB, aslLogin2_GetLoginCookieStruct(loginState));

}

static void 
	Login2EntitySharedBankCheck_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	if ( !Login2BeginFrameCommon(loginState) )
	{
		return;
	...
	if(!loginState->entitySharedBankCheckComplete)	
	{
		return;
	}
...
CreateCharacterSelectionData(Login2CharacterChoices *characterChoices, GameAccountData *gameAccountData, S32 clientAccessLevel, bool UGCLoginOnly, const char *publicAccountName, const char *privateAccountName, U32 numCharactersFromAccountServer)
{
    ...
    for( i = eaSize(&deas->ppReferents) - 1; i >= 0; i-- )
    {
		...
		if(Species_IsUnlocked(species, gameAccountData))
        {
...
Login2GetCharacterList_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    if ( AddictionPlaytimeLimitRequired(gameAccountData->eaAccountKeyValues) )
    {
        ...
        if ( playedTime >= gAddictionMaxPlayTime )
        {
            ...
            aslLogin2_FormatFailLogin(loginState, "Login2_PlayTimeExceeded", STRFMT_INT("TimeLeft", (s_AddictionPlaytimeResetTime - loggedOffTime) / 60) + 1, STRFMT_END);
            return;
        }
    ...
    aslLogin2_GetCharacterChoices(loginState->accountID, GetCharacterListCompletionCB, aslLogin2_GetLoginCookieStruct(loginState), false);
}
...
Login2CharacterSelect_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    if ( loginState->chooseCharacterPacketData || loginState->selectedCharacterChoice )
    {
        ...
        if ( !loginState->selectedCharacterChoice )
        {
			...
			if(!loginState->requestedUGCEdit && characterChoice->virtualShardID != 0)
			{
				aslLogin2_FailLogin(loginState, "Login2_UGCCharacterNotAllowed");
				return;
			}
        ...
        if ( loginState->selectedCharacterChoice->shardName == Login2_GetPooledShardName() )
        {
            ...
            if ( loginState->requestedUGCEdit )
            {
                ...
                if ( !aslLoginIsUGCAllowed() || (loginState->clientAccessLevel < ACCESS_GM && !loginState->selectedCharacterChoice->isUGCEditAllowed))
                {
    ...
    else
    {
        ...
        if ( loginState->requestCharacterDetailPacketData != NULL )
        {
            ...
            if ( loginState->activeCharacterDetailRequest == NULL )
            {
                ...
                fixupRequired = characterChoice->fixupVersion != CURRENT_ENTITY_FIXUP_VERSION;
                aslLogin2_GetCharacterDetail(loginState->accountID, loginState->activeCharacterDetailRequest->characterID, characterChoice->shardName, gConf.bCharacterDetailIncludesPuppets, gConf.bCharacterDetailIncludesTeamRequestPets, fixupRequired, GetCharacterDetailCompletionCB, aslLogin2_GetLoginCookieStruct(loginState));
            }
        }
		...
		else if(loginState->gadPurchasePacketData != NULL)
		{
			devassertmsg(loginState->afterRedirect == false, "Login2CharacterSelect_BeginFrame: Got gad purchase request after redirect!");

			// Switch to gad purchase state if a request has come in.
			ISM_SwitchToSibling(LOGIN2_STATE_MACHINE, loginState, LOGIN2_STATE_GAD_PURCHASE);
		}
...
Login2OnlineAndFixup_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    else
    {
        ...
        fixupRequired = loginState->selectedCharacterChoice->fixupVersion != CURRENT_ENTITY_FIXUP_VERSION;
        aslLogin2_GetCharacterDetail(loginState->accountID, loginState->selectedCharacterChoice->containerID, loginState->selectedCharacterChoice->shardName, false, false, fixupRequired, OnlineAndFixupCB, aslLogin2_GetLoginCookieStruct(loginState));
    }
}
...
Login2GetSelectedCharacterLocation_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	...
	loginState->selectedCharacterOwnedByObjectDB = false;	// [NNO-19651] If this was true from the last time the Foundry was entered, the player's login may fail due to advancing
															// the state too quickly.
...
Login2SendCharacterToGameserver_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
    ...
    if ( loginState->transferToGameserverComplete )
    {
        ...
        // Log for analytics
        aslLogin2_Log("ClientLoginComplete(IP %s%s AccountID %u CrypticAccountName %s CharacterID %u LanguageID %u)", makeIpStr(loginState->clientIP), loginState->isProxy ? "-proxied" : "", loginState->accountID, loginState->accountName, loginState->selectedCharacterID, loginState->clientLanguageID);
...
void
	GADPurchaseCompletionCB(bool success, U64 loginCookie)
{
	Login2State *loginState;
	...
	if ( ( !ISM_IsStateActive(LOGIN2_STATE_MACHINE, loginState, LOGIN2_STATE_GAD_PURCHASE) ) || ( loginState->gadPurchasePacketData == NULL ) )
	{
		aslLogin2_Log("Command %s returned for account %u while in wrong state", __FUNCTION__, loginState->accountID);
		aslLogin2_FailLogin(loginState, "Login2_InternalError");
		return;
	...
	if ( !success )
	{
		aslLogin2_Log("Character gad purchase failed for character %u", loginState->gadPurchasePacketData->characterID);
	}
	else
	{
		aslLogin2_Log("Character gad purchase succeeded for character %u", loginState->gadPurchasePacketData->characterID);
	}

	// Clearing out the rename packet will trigger the next state.
	StructDestroy(parse_GADPurchasePacketData, loginState->gadPurchasePacketData);
	loginState->gadPurchasePacketData = NULL;
}

static void 
	Login2GADPurchase_Enter(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	ContainerID characterID;
	Login2CharacterChoice *characterChoice;

	if ( !Login2EnterStateCommon(loginState, LOGIN2_STATE_GAD_PURCHASE, s_TimeoutIntershard) )
	{
		return;
	}

	characterID = loginState->gadPurchasePacketData->characterID;

	// Make sure the character ID is valid for this account.
	characterChoice = GetCharacterChoiceByID(loginState, characterID);
	if ( characterChoice == NULL || characterChoice->virtualShardID != 0)
	{
		aslLogin2_FailLogin(loginState, "Login2_InvalidGADPurchaseRequest");
		return;
	}

	aslLogin2_GADPurchase(characterID, characterChoice->accountID, loginState->gadPurchasePacketData->purchaseDefName, GADPurchaseCompletionCB, loginState->loginCookie);

}

static void 
	Login2GADPurchase_BeginFrame(InstancedMachineHandleOrName stateMachineHandle, Login2State *loginState, F32 elapsedTime)
{
	if ( !Login2BeginFrameCommon(loginState) )
	{
		return;
	...
	// Wait for rename processing to complete and then re-start character selection.
	if ( loginState->gadPurchasePacketData == NULL )
	{
		// We need to refresh the character list after making a gad purchase
		ISM_SwitchToSibling(LOGIN2_STATE_MACHINE, loginState, LOGIN2_STATE_REFRESH_GAME_ACCOUNT_DATA);
	}
}
...
aslLogin2_InitStateMachine(void)
{
	...
	ISM_AddInstancedState(LOGIN2_STATE_MACHINE, LOGIN2_STATE_FIXUP_GAME_ACCOUNT_DATA, Login2FixupGameAccountData_Enter, Login2FixupGameAccountData_BeginFrame, NULL, NULL);
	...
	ISM_AddInstancedState(LOGIN2_STATE_MACHINE, LOGIN2_STATE_GAD_PURCHASE, Login2GADPurchase_Enter, Login2GADPurchase_BeginFrame, NULL, NULL);
	...
	ISM_AddInstancedState(LOGIN2_STATE_MACHINE, LOGIN2_STATE_ENTITY_SHARED_BANK_CHECK, Login2EntitySharedBankCheck_Enter, Login2EntitySharedBankCheck_BeginFrame, NULL, NULL);
}
