...
{
    ContainerRefArray *dependentContainerRefs = NULL;
...
static void
OnlineCharacterCB(TransactionReturnVal *returnVal, PlayerEntityFixupState *fixupState)
{
    bool online = false;
    enumTransactionOutcome outcome;
    TransactionReturnVal *dependentContainerReturnVal;

    outcome = RemoteCommandCheck_dbLogin2_OnlineCharacter(returnVal, &online);
    if ( outcome == TRANSACTION_OUTCOME_SUCCESS && online )
    {
        // NOTE - we check if the shared bank exists and get the list of dependent containers in parallel.
        if ( fixupState->fixupSharedBank )
        {
            ...
            RemoteCommand_DBCheckSingleContainerExists(sharedBankCheckReturnVal, GLOBALTYPE_OBJECTDB, 0, GLOBALTYPE_ENTITYSHAREDBANK, fixupState->accountID);
        }
        else
        {
        ...
        RemoteCommand_DBReturnDependentContainers(dependentContainerReturnVal, GLOBALTYPE_OBJECTDB, 0, GLOBALTYPE_ENTITYPLAYER, fixupState->playerEntityID);
    }
    else
    {
        fixupState->failed = true;
        estrConcatf(&fixupState->errorString, "aslLogin2_EntityFixup: Onlineing containers failed.  accountID=%d, playerID=%d\n", fixupState->accountID, fixupState->playerEntityID);
        QueueForEntityFixupIfReady(fixupState);
    }
}
...
{
    PlayerEntityFixupState *fixupState;
    TransactionReturnVal *onlineContainerReturnVal;
    ...
    // Make sure the player is online before doing fixup.
    onlineContainerReturnVal = objCreateManagedReturnVal(OnlineCharacterCB, fixupState);
    RemoteCommand_dbLogin2_OnlineCharacter(onlineContainerReturnVal, GLOBALTYPE_OBJECTDB, 0, accountID, playerEntityID);
}
