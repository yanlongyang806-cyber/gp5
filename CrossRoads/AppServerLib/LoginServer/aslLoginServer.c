...
int giNumQueued_Main = 0;
int giNumQueued_VIP = 0;
...
static bool gbLoginServerWritesOutSchemas = false;
AUTO_CMD_INT(gbLoginServerWritesOutSchemas, LoginServerWritesOutSchemas);
...
enumTransactionOutcome SetPlayerLoginUpdateCB(ATR_ARGS, NOCONST(Entity) *newPlayer, NOCONST(Entity) *backupPlayer, GlobalType locationType, ContainerID locationID)
{
	...
	// Set the Dev & GM bits if it's not active on accountID 1 & 2 and give Premium access
	if(newPlayer->pPlayer->accountID <= 2)
	{
		newPlayer->pPlayer->accessLevel = ACCESS_DEBUG;
		newPlayer->pPlayer->accountAccessLevel = ACCESS_DEBUG;
		newPlayer->pPlayer->playerType = kPlayerType_Premium;
		newPlayer->pPlayer->bIsDev = true;
		newPlayer->pPlayer->bIsGM = true;
	}
	else
	{
		newPlayer->pPlayer->accessLevel = ACCESS_USER;
		newPlayer->pPlayer->accountAccessLevel = ACCESS_USER;
		newPlayer->pPlayer->playerType = kPlayerType_Premium;
...
{
	if (!isProductionMode() && gbLoginServerWritesOutSchemas)
	{	
...
void aslLoginServerSendGlobalInfo(TimedCallback *callback, F32 timeSinceLastCallback, UserData userData)
{
	...
	loginServerInfo.bUGCEnabled = aslLoginIsUGCAllowed();
	...
	loginServerInfo.iNumQueued_Main = giNumQueued_Main;
	loginServerInfo.iNumQueued_VIP = giNumQueued_VIP;
...
int aslLoginInit(void)
{
	...
	for (i = 0; i < MAX_LISTEN_TRIES; i++)
	{
		...
		commListenBoth(commDefault(), (isDevelopmentMode() || g_isContinuousBuilder) ?LINKTYPE_TOUNTRUSTED_20MEG:LINKTYPE_TOUNTRUSTED_5MEG, 
			LINK_FORCE_FLUSH, 
	...
	if(local_link)
	{
		listenInitReceiveStats(local_link, "Local login links", ToLoginMsgEnum);
		eaPush(&gLoginServerState.loginLinks, local_link);
	}
	if(public_link)
	{
		listenInitReceiveStats(public_link, "Public login links", ToLoginMsgEnum);
    ...
    if (sbListenForGatewayProxies)
    {
        ...
        if(local_link)
		{
			listenInitReceiveStats(local_link, "Local gateway links", ToLoginMsgEnum);
            eaPush(&gLoginServerState.loginLinks, local_link);
		}
        if(public_link)
		{
			listenInitReceiveStats(local_link, "Public gateway links", ToLoginMsgEnum);
            eaPush(&gLoginServerState.loginLinks, public_link);
		}
    }
	...
	resServerIgnoreDictionaryRequests(GlobalTypeToCopyDictionaryName(GLOBALTYPE_GROUPPROJECTCONTAINERPLAYER));
