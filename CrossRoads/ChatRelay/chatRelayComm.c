...
static void chatRelay_AuthenticateUserCB(TransactionReturnVal *returnVal, ChatRelayAuthStruct *pAuthData)
{
	...
	if (user)
	{
		...
		if (eOutcome == TRANSACTION_OUTCOME_SUCCESS && pData && link)
		{
			...
			user->ePermissionFlags = pData->eGamePermissionInfo;
			user->bAuthed = true;
...
AUTO_COMMAND_REMOTE ACMD_IFDEF(CHATSERVER);
void chatRelay_UpdateVoiceRegionEnabled(U32 uVoiceRegionEnabled)
{
	static int *seaiConnectedUsersList = NULL;
	int i, iSize;

	eaiClear(&seaiConnectedUsersList);
	chatRelayGetUserIDList(&seaiConnectedUsersList, true);
	iSize = eaiSize(&seaiConnectedUsersList);

	for (i = 0; i < iSize; i++)
	{
		GClientCmd_gclChat_UpdateVoiceRegionEnabled(seaiConnectedUsersList[i], uVoiceRegionEnabled);
	}
}
...
AUTO_COMMAND_REMOTE ACMD_IFDEF(CHATSERVER);
void chatRelay_UpdateConfig(ChatRelayConfig *pConfig)
{
	chatRelay_LoadConfig(pConfig);
}

// User logs out without completely disconnecting (eg. Returns to character select)
AUTO_COMMAND_REMOTE ACMD_IFDEF(CHATSERVER);
void chatRelayUserLogout(U32 uAccountID)
{
	ChatRelayUser *relayUser = chatRelayGetUser(uAccountID);
	if (relayUser)
	{
		chatRelayRemoveLink(relayUser, "User logout");
		chatRelayRemoveUser(uAccountID);
	}
}