...
void chatRelayRemoveLink(ChatRelayUser *user, const char *reason)
{
	...
	if (link)
		linkRemove_wReason(&link, reason);
	user->uLinkID = 0;
	user->link = NULL;
}
...
void chatRelayRemoveUnauthenticatedUsers(void)
{
	...
	{
		chatRelayRemoveLink(eaUsersToRemove[i], "Unauthenticated User");
...
ChatRelayConfig gChatRelayConfig = {0};
void chatRelay_LoadConfig(ChatRelayConfig *pConfig)
{
	StructDeInit(parse_ChatRelayConfig, &gChatRelayConfig);
	StructCopy(parse_ChatRelayConfig, pConfig, &gChatRelayConfig, 0, 0, 0);
}
