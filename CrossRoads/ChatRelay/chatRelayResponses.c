...
AUTO_COMMAND_REMOTE ACMD_IFDEF(CHATSERVER);
void crLoginSucceeded(U32 uAccountID, U32 uVoiceEnabledRegion)
{
	ChatRelayUser *relayUser = chatRelayGetUser(uAccountID);
	if (!relayUser)
		return;
	...
	GClientCmd_gclChat_LoginSuccess(relayUser->uAccountID, uVoiceEnabledRegion);
}
...
__forceinline static void crAddMessageCount(ChatLogEntryType eType)
{
	...
	xcase kChatLogEntryType_Trade:
		ADD_MISC_COUNT(1, "TradeMsgReceive");
	xcase kChatLogEntryType_LookingForGroup:
		ADD_MISC_COUNT(1, "LFGMsgReceive");
