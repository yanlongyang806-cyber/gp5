...
extern ChatRelayConfig gChatRelayConfig;
AUTO_COMMAND ACMD_GENERICSERVERCMD ACMD_CATEGORY(ChatRelayCmds) ACMD_ACCESSLEVEL(0) ACMD_PRIVATE;
void crSendMessage(GenericServerCmdData *pCmdData, ChatMessage *pMsg)
{
	...
	// Zone chat Rate Limiting
	if ( (pMsg->eType == kChatLogEntryType_Zone || pMsg->eType == kChatLogEntryType_Trade || pMsg->eType == kChatLogEntryType_LookingForGroup) &&
		gChatRelayConfig.uZoneSilenceTime && 
		gChatRelayConfig.uZoneRateLimit && gChatRelayConfig.uZoneRateLimitPeriod &&
		(relayUser->ePermissionFlags & CHAT_GAME_PERMISSION_INFO_ZONE_RATELIMIT) == 0)
	{
		U32 uCurTime = timeSecondsSince2000();
		int numMsgs;

		if (relayUser->uZoneSilenceTime)
		{
			if (relayUser->uZoneSilenceTime > uCurTime)
			{
				crNotifySend(relayUser, kNotifyType_Failed, langTranslateMessageKey(relayUser->eLanguage, "Zone_RateLimited"));
				return;
			}
			relayUser->uZoneSilenceTime = 0;
		}
		numMsgs = eaiSize(&relayUser->eaiZoneMessageTimes);
		if ((U32) numMsgs >= gChatRelayConfig.uZoneRateLimit)
		{
			int i;
			U32 uLimitTime = uCurTime - gChatRelayConfig.uZoneRateLimitPeriod;
			for (i=0; i<numMsgs; i++)
			{
				if (relayUser->eaiZoneMessageTimes[i] > uLimitTime)
					break;
			}
			eaiRemoveRange(&relayUser->eaiZoneMessageTimes, 0, i);			
			if ((U32) eaiSize(&relayUser->eaiZoneMessageTimes) >= gChatRelayConfig.uZoneRateLimit)
			{
				crNotifySend(relayUser, kNotifyType_Failed, langTranslateMessageKey(relayUser->eLanguage, "Zone_RateLimited"));
				relayUser->uZoneSilenceTime = gChatRelayConfig.uZoneSilenceTime + uCurTime;
				return;
			}
		}
		eaiPush(&relayUser->eaiZoneMessageTimes, uCurTime);
	}
