...
// Sets whether chat log entries should show channel names
AUTO_TRANSACTION ATR_LOCKS(pData, ".chatConfig.bDisableStickyChannels, .chatConfig.iVersion");
enumTransactionOutcome tr_crChatConfig_SetStickyChannels(ATR_ARGS, NOCONST(GameAccountData) *pData, int bStickyChannels)
{
	RETURN_IF_GAD_MODIFICATION_DISALLOWED(TRANSACTION_OUTCOME_FAILURE);
	pData->chatConfig.bDisableStickyChannels = !bStickyChannels;
	pData->chatConfig.iVersion++;
	return TRANSACTION_OUTCOME_SUCCESS;
}
