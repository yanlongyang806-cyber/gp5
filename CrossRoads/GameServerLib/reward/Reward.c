...
static GlobalRewardModifier **eaActiveRewardScalars = NULL;
...
int GetLevelForKilledEnt(Entity * pKilled,int iPlayerLevel)
{
	if (!pKilled)
		return 1;

	if (pKilled->pChar && pKilled->pChar->bLevelAdjusting)
		return iPlayerLevel;

	return GetEntLevel(pKilled);
}
...
AUTO_TRANS_HELPER;
void rewardbag_trh_AddItem(ATH_ARG NOCONST(InventoryBag) *bag, Item *item, bool bSetID)
{
	if (!bag || !item)
	{
	...
	else
	{
		...
		{
			rewardbag_AddNumeric(bag, CONTAINER_RECONST(Item, pItem));
			StructDestroy(parse_Item, item);
		}
		else
		{
			inv_bag_trh_AddItem(ATR_EMPTY_ARGS, NULL, NULL, bag, -1, pItem, NULL, -1, 0, NULL, NULL, NULL);
		}
	}
...
{
	Item *item = CONTAINER_RECONST(Item, inv_bag_trh_RemoveItem(ATR_EMPTY_ARGS, NULL, false,  CONTAINER_NOCONST(InventoryBag, (rw_bag)), index, -1, true, NULL));
...
bool rewardbag_RemoveItemByID(InventoryBag *rw_bag, int id, Item **res_item, int* pCountOut)
{
	...
	int count = 0;
	Item *item = NULL;
	bool bFound = inv_bag_trh_FindItemByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(InventoryBag, (rw_bag)), id, pIter);

	if (bFound)
	{
		count = bagiterator_GetItemCount(pIter);
		item = (Item*)inv_bag_trh_RemoveItem(ATR_EMPTY_ARGS, NULL, false,  CONTAINER_NOCONST(InventoryBag, (rw_bag)), pIter->i_cur, -1, false, NULL);
	}
...
void rewardTableExpr_EvalPlayer(int iPartitionIdx, Entity *pPlayerEnt, Expression *pExpr, RewardContext *pContext, MultiVal * pReturn)
{
	if (pExpr) {
		if (g_pRewardPlayerContext) {
			...
			{
				if (pPlayerEnt->erOwner)
				{
					pPlayerEnt = entFromEntityRef(entGetPartitionIdx(pPlayerEnt),pPlayerEnt->erOwner);
				}
				else if (!pPlayerEnt->myRef && pPlayerEnt->pSaved && pPlayerEnt->pSaved->conOwner.containerID)
				{
					pPlayerEnt = entFromContainerID(iPartitionIdx, pPlayerEnt->pSaved->conOwner.containerType, pPlayerEnt->pSaved->conOwner.containerID);
				}
			}
...
static bool	reward_EntryIsValid(int iPartitionIdx, Entity *pPlayerEnt, RewardContext *ctxt, RewardEntry *pRewardEntry, bool bCheckExp, bool bUsePlayerLevel)
{
	...
	if (ctxt->bIgnoreExpressionEntries && 
		(pRewardEntry->ChoiceType == kRewardChoiceType_Expression || pRewardEntry->ChoiceType == kRewardChoiceType_ExpressionInclude) )
		return false;
...
S32 GetLevelAdjustment(RewardContext *pContext, bool bTeam)
{
	...
	if(pContext)
	{
		...
		if(pContext->pKilled && pContext->pKilled->pChar)
		{
			int iKilledLevel = entity_GetCombatLevel(pContext->pKilled);
			if (pContext->pKilled->pChar && pContext->pKilled->pChar->bLevelAdjusting)
				iKilledLevel = pContext->RewardLevel;
			iAdjustment = CLAMP( ((iKilledLevel - iLevel) + REWARDVALTABLE_ADJUST_ZERO_IDX), REWARDVALTABLE_ADJUST_MIN_IDX, REWARDVALTABLE_ADJUST_MAX_IDX);
		}
		else
		{
...
GlobalRewardModifier **s_ppActiveRewardScalars;

static F32 GetGlobalScalarNumericBonus(const char *pcNumericType)
{
	F32 bonus = 1.0f;
	int i;
	ItemDef *pNumericDef = RefSystem_ReferentFromString(g_hItemDict,pcNumericType);

	for(i=0;i<eaSize(&s_ppActiveRewardScalars);i++)
	{
		if(GET_REF(s_ppActiveRewardScalars[i]->hNumeric) == pNumericDef)
			bonus += s_ppActiveRewardScalars[i]->fBonusValue;
	}
...
F32 GetTotalNumericBonus(RewardContext *pContext, U32 rewardBonusType, const char *pcNumericType)
{
	...
	if((rewardBonusType & RewardBonusType_GlobalModifier) != 0)
	{
		F32 modifier = GetGlobalScalarNumericBonus(pcNumericType);
		value += modifier - 1.0f;
	}
		...
		case RewardContextType_EntKill:
			if ( pContext->killerIsPlayer)
			{
				...
				if (theTable)
				{
					...
					tmpval *= GetTotalNumericBonus(pContext, RewardBonusType_Default, pcType);
		...
		case RewardContextType_EntKill:
			if ( pContext->killerIsPlayer)
			{
				...
				if (theTable)
				{
					...
					tmpval *= GetTotalNumericBonus(pContext, (RewardBonusType_Lifetime|RewardBonusType_Modifier|RewardBonusType_GlobalModifier), pcType);
...
bool reward_GetPlayerOwned(RewardTable* pTable, RewardContext* pCtxt)
{
	if(pTable && (pTable->flags & kRewardFlag_PlayerOwned))
		return true;

	if (pCtxt && pCtxt->bInheritPlayerOwnedFlag)
		return true;
			...
			for(j = 0; j < eaSize(&ppRewardBags[i]->ppIndexedInventorySlots);++j)
			{
				...
				if(pNCGItem)
				{
					...
					if(pGemItemDef && pGemItemDef->eGemType != kItemGemType_None)
					{
						...
						if(inv_trh_GemItem(CONTAINER_NOCONST(Entity, pPlayerEnt), pItem, pNCGItem, -1))
						{
...
NOCONST(Item)* rewarditem_FromCtxt(RewardContext *pContext, char const *pchDefName, U32 *pSeed)
{
	...
	if(pDef && pDef->eType == kItemType_RewardPack && pDef->pRewardPackInfo->bCanScaleRewards)
	{
		if(!pItem->pSpecialProps)
			pItem->pSpecialProps = (NOCONST(SpecialItemProps)*)StructCreate(parse_SpecialItemProps);

		pItem->pSpecialProps->fRewardPackScale = pContext->RewardScale;
	}
...
static void rewarditem_FillRewardDataFromEntry(Item* pItem, RewardEntry* pEntry)
{
	...
	if (pEntry && pEntry->bForceBind)
	{
		pItem->bForceBind = true;
	}
}
...
static bool reward_ShouldGiveNumerics(RewardContext *pContext, RewardTable *pRewardTable)
{
	if(pRewardTable && pContext)
		if(pContext->bGiveNumerics)
			return true;
...
void reward_addentry(int iPartitionIdx, Entity *pPlayerEnt, RewardContext *pContext, RewardEntry *pRewardEntry, RewardTable *pRewardTable, InventoryBag ***bags, InventoryBag *pRewardBag, U32 *pSeed, bool bMissionKillReward, RewardTableOverride *pInRewardTableOverride, RewardGatedDataInOut *pGatedData)
{
	...
	acase kRewardChoiceType_ChoiceVariableCount:
		...
		xcase kRewardType_Numeric:
			{
				if(reward_ShouldGiveNumerics(pContext, pRewardTable))
				{
					...
					if (itemDef)
					{
						...
						if (bGiveItem)
						{
							...
							if(pRewardEntry->numeric_op == NumericOp_Add)
							{
								...
								fValue *= GetTotalNumericBonus(pContext, RewardBonusType_Modifier | RewardBonusType_GlobalModifier, itemDef->pchName);
							}
...
RewardBagInfo* rewardbaginfo_Create()
{
...
InventoryBag * rewardbag_CreateEx(const char* pchRewardTable)
{
	...
	RewardBagInfo*pRewardBagInfo = rewardbaginfo_Create();
...
static InventoryBag *rewardbag_GetFromContext(RewardContext * pContext,RewardTable *table, RewardEntry * pEntry, InventoryBag ***bags,InventoryBag *pParentBag, S32 iNewBagIfNumItems, RewardTableOverride *pInRewardTableOverride)
{
	InventoryBag *pRewardBag = NULL;
	RewardPickupType pickupType;
	bool bUsingCostumes;
	int iSpecialRewardStatus = 0;
	if (pEntry)
	{
		iSpecialRewardStatus = pEntry->iSpecialReward;
	}
	...
	if(iNewBagIfNumItems > 0 || 
		...
		(table->flags & kRewardFlag_SeparateBag) != 0 ||
		reward_GetPlayerOwned(table, pContext) || 
		(pContext->iForceSoloOwnershipContainerID != -1) ||
		(pRewardBag->pRewardBagInfo && (pRewardBag->pRewardBagInfo->flags & (kRewardFlag_SeparateBag | kRewardFlag_PlayerOwned)) != 0))
	{
		...
		for ( BagIdx=0; BagIdx<eaSize(bags); BagIdx++)
		{
			...
			if(pBag)
			{
				...
				(
					...
					((table->flags & kRewardFlag_SeparateBag) == 0 || reward_GetPlayerOwned(table, pContext) || iNumItems == 0) &&
					 pBag->pRewardBagInfo &&
					 ...
					 (!pEntry || pBag->pRewardBagInfo->iSpecialRewardStatus == iSpecialRewardStatus) &&
		...
		if (BagIdx >= eaSize(bags))
		{
			...
			if(iNewBagIfNumItems > 0 || pContext->bInheritPlayerOwnedFlag)
			{
			...
			tmp_bag->pRewardBagInfo->iSpecialRewardStatus = iSpecialRewardStatus;
...
static void reward_GatedRewardGenerated(Entity *pEntity, S32 gatedType)
{
	if(pEntity && pEntity->pPlayer && gatedType > 0)
	{
		...
		U32 uCurrentTimeSeconds = ShardClock_SecondsSince2000();
		...
		if(pPlayerGated)
		{
			...
			if(pRewardGatedInfo->uResetAt > 0 && pPlayerGated->uNumTimes >= pRewardGatedInfo->uResetAt)
			{
				...
				pPlayerGated->uTimeSet = uCurrentTimeSeconds;
			}
		}
		else
		{
			...
			pNewGatedData->uTimeSet = uCurrentTimeSeconds;
...
void reward_generateEx( int iPartitionIdx, Entity *pPlayerEnt, RewardContext *ctxt, RewardTable *table, InventoryBag ***bags, InventoryBag *pParentBag, U32 *pSeed, bool bMissionKillReward, RewardTableOverride *pInRewardTableOverride, RewardGatedDataInOut *pGatedData)
{
	...
	if (!ctxt->bInheritPlayerOwnedFlag && (table->flags & kRewardFlag_PlayerOwned))
	{
		bClearPlayerOwned = true;
		ctxt->bInheritPlayerOwnedFlag = true;
	}
	...
	pRewardBag = rewardbag_GetFromContext(ctxt,table,NULL,bags,pParentBag,iNumItemsForSeparateBag, pInRewardTableOverride);
	...
	case kRewardAlgorithm_Weighted:
		{
			...
			for (iChoice=0; iChoice<table->NumChoices; iChoice++)
			{
				...
				for(ii=0; ii<n_rw_entries; ii++)
				{
					...
					if((table->flags & kRewardFlag_SeparateBag) != 0 || reward_GetPlayerOwned(table, ctxt) || (pRewardEntry->iSpecialReward != pRewardBag->pRewardBagInfo->iSpecialRewardStatus))
					{
						InventoryBag *pNewRewardBag = rewardbag_GetFromContext(ctxt,table,pRewardEntry,bags,pParentBag,iNumItemsForSeparateBag,pInRewardTableOverride);
				...
				if (ii>=n_rw_entries)
					Errorf("reward_generateEx: Weight problem in RewardTable \"%s\". Didn't find a matching entry in %i tries", table->pchName, ii);
			}
...
rewardGenerate_exit:
	...
	if (bClearPlayerOwned)
	{
		ctxt->bInheritPlayerOwnedFlag = false;
	}
}
...
static void reward_GenerateBagFromTable(int iPartitionIdx, Entity *pPlayerEnt, RewardTable *pTable, 
										InventoryBag ***rewardBagList, int iLevel, F32 fScale, 
										RewardNumericScale **eaNumericScales, RewardContextType type, 
										int skillLevel, bool bUseRewardModifiers, bool bIgnoreExpressionEntries, U32* pSeed)
{
	...
	pLocalContext->bIgnoreExpressionEntries = bIgnoreExpressionEntries;
...
{
	reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, rewardBagList, iLevel, 1.0f, NULL, RewardContextType_RandomMissionGen, 0, false, false, NULL);
}
...
void reward_GenerateInteractableBag(int iPartitionIdx, Entity *pPlayerEnt, RewardTable *pTable, InventoryBag ***rewardBagList, int iLevel, int skillLevel, U32* pSeed, Entity* interactingEnt)
{
	...
	reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, rewardBagList, iLevel, 1.0f, NULL, RewardContextType_Clickable, skillLevel, false, false, pSeed);
}
...
{
	reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, peaBags, iLevel, 1.0f, NULL, RewardContextType_Store, 0, false, false, pSeed);
}

// Generates reward bags for item assignments
void reward_GenerateBagsForItemAssignment(	int iPartitionIdx, Entity *pPlayerEnt, SA_PARAM_NN_VALID RewardTable* pTable, 
											S32 iLevel, F32 fScale, RewardNumericScale **eaNumericScales, 
											bool bUseRewardMods, bool bIgnoreExpressionEntries, 
											U32* pSeed, InventoryBag*** peaBags)
{
	reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, peaBags, iLevel, fScale, eaNumericScales, 
								RewardContextType_MissionReward, 0, bUseRewardMods, bIgnoreExpressionEntries, pSeed);
}
...
{
	reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, peaBags, 1, 1.0f, NULL, RewardContextType_MissionReward, 0, false, false, NULL);
}

// Generates reward bags for a reward pack item
void reward_GenerateBagsForRewardPack(int iPartitionIdx, Entity *pPlayerEnt, SA_PARAM_NN_VALID RewardTable* pTable, S32 iLevel, F32 fScale, InventoryBag*** peaBags)
{
	reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, peaBags, iLevel, fScale, NULL, RewardContextType_MissionReward, 0, false, false, NULL);
}
...
{
    reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, peaBags, iLevel, 1.0f, NULL, RewardContextType_MissionReward, 0, false, false, NULL);
}

void reward_GenerateBagsForItemSalvage(int iPartitionIdx, Entity *pPlayerEnt, SA_PARAM_NN_VALID RewardTable* pTable, S32 iLevel, InventoryBag*** peaBags)
{
	reward_GenerateBagFromTable(iPartitionIdx, pPlayerEnt, pTable, peaBags, iLevel, 1.0f, NULL, RewardContextType_MissionReward, 0, false, false, NULL);
}
...
void reward_PowerExec(Entity* e, RewardTable *table, int level, F32 scale, bool bInformClient, const ItemChangeReason *pReason)
{
	...
	if(table && e)
	{
		...
		{
			InventoryBagGroup *pGroup = StructCreate(parse_InventoryBagGroup);
			int i;

			for(i=0;i<eaSize(&bags);i++)
			{
				if(eaSize(&bags[i]->ppIndexedInventorySlots) > 0)
					eaPush(&pGroup->eaBags, bags[i]);
			}

			ClientCmd_PVP_SetReward(e, pGroup);

			eaClear(&pGroup->eaBags);
			StructDestroySafe(parse_InventoryBagGroup, &pGroup);
		}
...
void reward_OpenMissionExec(int iPartitionIdx, Entity* pPlayerEnt, MissionDef* pRootMissionDef, RewardTable *pRewardTable, int level, F32 fRewardScale, Vec3 DropPos, const ItemChangeReason *pReason, const char *pchRewardMessage)
{
	InventoryBag **ppRewardBags = NULL;
	...
	if (bAddedItems)
	{
		ClientCmd_SetOpenMissionRewardMessage(pPlayerEnt, pchRewardMessage);
	}
...
{
	// If this function ever shows up on profiler as a performance problem, there is an easy solution discussed by Jared and Andy A.
	// We will maintain a separate list of unowned pointers to PlayerNumericThreshold on the player for those that have been exceeded.
	// Normative play will rarely have more than 1 or 2 in that separate list.
	PERFINFO_AUTO_START_FUNC();

	if(pEnt && pEnt->pPlayer && zmapIsUGCGeneratedMap(NULL))
	{
		S32 iLevel = entity_GetSavedExpLevel(pEnt);
		U32 uCurrentTime = timeSecondsSince2000();
		FOR_EACH_IN_EARRAY(pEnt->pPlayer->eaPlayerNumericThresholds, PlayerNumericThresholds, pPlayerNumericThresholds)
		{
			ItemDef *pItemDef = RefSystem_ReferentFromString(g_hItemDict, pPlayerNumericThresholds->astrItemDefName);
			if(pItemDef && kItemType_Numeric == pItemDef->eType)
			{
				FOR_EACH_IN_EARRAY(pPlayerNumericThresholds->eaPlayerNumericThreshold, PlayerNumericThreshold, pPlayerNumericThreshold)
				{
					NumericThresholdDef *pNumericThresholdDef = eaIndexedGetUsingString(&pItemDef->eaNumericThresholdDefs, pPlayerNumericThreshold->astrThresholdName);
					if(pNumericThresholdDef && pNumericThresholdDef->bKillCreditLimiter && pNumericThresholdDef->bOnlyUGC && pNumericThresholdDef->bOnlyKillCredit)
					{
						S32 iThreshold = item_GetNumericThresholdForLevel(pNumericThresholdDef, iLevel);
						U32 uTimeDifference = uCurrentTime - pPlayerNumericThreshold->uTimestamp;
						F32 fCooldownRate = item_GetNumericThresholdCooldownRateForLevel(pNumericThresholdDef, iLevel);
						if(pPlayerNumericThreshold->iNumericEarned - (uTimeDifference * fCooldownRate) >= iThreshold)
						{
							PERFINFO_AUTO_STOP();
							return true;
						}
...
F32 reward_CalculateKillCredit(Entity *pKilled, KillCreditTeam ***peaTeams)
{
	...
	for (i = 0; i < eaSize(peaTeams); i++)
	{
		for (j = 0; j < eaSize(&(*peaTeams)[i]->eaMembers); j++)
		{
			...
			else if (pCreditEnt->fPercentCreditSelf)
			{	// hard-rule if assists are enabled, and the player did ANYTHING, give credit
		...
		zmapInfoGetMapType(NULL) == ZMTYPE_STATIC && eaSize(reward_tables) > 0)
	{
		...
		if(Reward_StaticMapRewardTableActive())
		{
			...
			for(teamidx = 0; teamidx < eaSize(peaCreditTeams); ++teamidx)
			{
				...
				{
					S32 iKilledLevel = GetLevelForKilledEnt(pKilled, team_credit->iTeamCombatLevel);
...
void reward_processCreditTeams(Entity *pKilled, RewardTable*** reward_tables, KillCreditTeam*** peaCreditTeams, F32 fTotalDamage, const char *pcRank, const char *pcSubRank, char **logS)
{
	...
	// If we're on a UGC map, and this critter was killed without dealing significant damage to him, no credit.  In Cryptic content, we can assume
	// that this is under the control of the designer, and is intended, but in UGC, we have to assume it's exploitive.
	if (resNamespaceIsUGC(zmapInfoGetPublicName(NULL)))
	{
		if (fTotalDamage < pKilled->pChar->pattrBasic->fHitPointsMax*0.5f)
		{
			return;
		}
	}
	...
	for(teamidx=0; teamidx<eaSize(peaCreditTeams); teamidx++)
	{
		...
		for(memberidx=0; memberidx<eaSize(&team_credit->eaMembers); memberidx++)
		{
			...
			if ( pKilled )
			{
				if (pKilled->pCritter && pKilled->pCritter->eRewardLevelType != kWorldEncounterRewardLevelType_DefaultLevel)
				{
					switch(pKilled->pCritter->eRewardLevelType)
					{
						...
						xdefault:
						{
							reward_level = GetLevelForKilledEnt(pKilled,GetEntLevel(killer));
							Errorf("Bad reward level type specified on critter %s", pKilled->debugName);
						}
					}
				}
				else if (gConf.bRewardTablesUseEncounterLevel)
					reward_level = (pKilled && pKilled->pCritter && pKilled->pCritter->encounterData.activeTeamSize) ? pKilled->pCritter->encounterData.activeTeamLevel : GetLevelForKilledEnt(pKilled,GetEntLevel(killer));
				else
					reward_level = GetLevelForKilledEnt(pKilled,GetEntLevel(killer));
			}
			...
			pLocalContext->bGiveNumerics = false;
			if(reward_EntExceededUGCKillCreditLimit(killer))
				pLocalContext->bWouldGiveItems = false;
			else
			{
				pLocalContext->bGiveNumerics = true;
				if (team_credit->bHighestDamager)
				{
...
void reward_EntLootCB(Entity *pEnt, InventoryBag *pRewardBag, const ItemChangeReason *pReason, TransactionReturnCallback pFunc, void* pData)
{
	...
	if (!bShareNumerics)
	{
		...
		bagiterator_SetBag(CONTAINER_NOCONST(InventoryBag, pRewardBag), iter);
...
void reward_AwardMissionDrops( RewardContext *pContext, Entity *pPlayerEnt, InventoryBag ***bags, U32 inSeed)
{
	...
	if ( pPlayerEnt && pPlayerEnt->pPlayer)
	{
		...
		for (i = eaSize(bags)-1; i>=0; --i){
			...
			iter = bagiterator_Create();
			bagiterator_SetBag(CONTAINER_NOCONST(InventoryBag, bag), iter);
			for(; !bagiterator_Stopped(iter); bagiterator_Next(iter))
			{
...
static void _lootFilterBag(Entity *pEnt, InventoryBag *pLootBag,InventoryBag *** peaLootBagsOut)
{
	...
	if (pLootBagCopy)
	{
		...
		bagiterator_SetBag(CONTAINER_NOCONST(InventoryBag, pLootBagCopy), iter);
		...
		if (rewardbag_Size(pLootBagCopy))
		{
			eaPush(peaLootBagsOut,pLootBagCopy);
		}
		else
		{
...
bool loot_InteractBegin(Entity *pEnt, InventoryBag *pLootBag, bool bClientUpdate, bool bForceSendToClient)
{
	...
	LootToClient * pLootToClient = NULL;

	if (pEnt && pLootBag)
	{
		pLootToClient = StructCreate(parse_LootToClient);
		_lootFilterBag(pEnt,pLootBag,&pLootToClient->eaLootBags);

		// this code is dupped below.  (except for bForceAutoLoot)
		if (bClientUpdate)
		{
			if(bForceSendToClient || !entity_IsAutoLootEnabled(pEnt))
				ClientCmd_LootUpdateList(pEnt, pLootToClient);
			bInteractSuccess = true;
		}
		else if (pLootToClient && eaSize(&pLootToClient->eaLootBags))
		{
			if(bForceSendToClient || !entity_IsAutoLootEnabled(pEnt))
				ClientCmd_LootInteraction(pEnt, pLootToClient);
			bInteractSuccess = true;
		}
		StructDestroy(parse_LootToClient, pLootToClient);
	}
...
bool loot_InteractBeginMultiBags(Entity *pEnt, InventoryBag*** peaLootBags, bool bClientUpdate, bool bForceSendToClient, bool bForceAutoLoot)
{
	...
	if (pEnt && eaSize(peaLootBags) > 0)
	{
		...
		// this code is dupped above. (except for bForceAutoLoot)
		if (bClientUpdate)
		{
...
static bool entity_IsRolloverEntity(Entity *pCritterEnt)
{
	if (pCritterEnt && inv_HasLoot(pCritterEnt)) 
	{
		FOR_EACH_IN_CONST_EARRAY(pCritterEnt->pCritter->eaLootBags, InventoryBag, pLootBag)
		{
			if (pLootBag && (pLootBag->pRewardBagInfo->PickupType == kRewardPickupType_Rollover ))
				return true;
		}
		FOR_EACH_END
	}
...
{
	if (interaction_IsLootEntity(pLootEnt) || entity_IsRolloverEntity(pLootEnt))
	{
...
void loot_InteractCallback(TransactionReturnVal *returnVal, LootInteractCBData *pData)
{
	if ( pData )
	{
		...
		FOR_EACH_IN_EARRAY(eaLootBags, InventoryBag, pLootBag)
		{
			...
			if (!inv_bag_trh_FindItemByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(InventoryBag, pLootBag), pData->iItemID, pIter))
			{
				continue;
			}
			...
			if ( returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS )
			{
				...
				// update the loot bag on the client, but only if not a rollover
				if (!entity_IsRolloverEntity(pLootEnt))
				{
				...
				if (ppLootTracker)
				{
					...
					{
						// An awkward place to put this.
						//  CleanupLootInteraction called interaction_EndInteractionAndDialog so we need to do this.
						// But only if !(*ppLootTracker)) (which is what will cause the interact to close)

						if (!(*ppLootTracker))
						{
							if (pInteractable)
							{
								interaction_AddRecentCompletedInteract(pPlayerEnt, pInteractable, NULL, NULL);
							}
						}
						break;
					}
				}
				else if (interaction_IsLootEntity(pLootEnt) || entity_IsRolloverEntity(pLootEnt))
				{
		...
		if ( returnVal->eOutcome != TRANSACTION_OUTCOME_SUCCESS )
		{
			...
			if (!entity_IsRolloverEntity(pLootEnt) && entity_ShouldAutoLootTarget(pPlayerEnt, pLootEnt))
	...
	if ( interaction_IsPlayerInteracting(pPlayerEnt) )
	{
		...
		if (pLootEnt || pInteractable)
		{
			...
			FOR_EACH_IN_EARRAY(eaLootBags, InventoryBag, pBag)
			{
				...
				{
					// Note. This may not get called when you expect. Since the awarded item may not be removed from pBag until loot_InteractCallback
					//  is called post transaction, the bag may still have the item at this point. (Isn't this a dupe bug posibility?)
					...
					{
						// This cleanup may also happen in loot_InteractCallback after we call interactloot_CleanupLootInteraction
						if (pInteractable)
							interaction_AddRecentCompletedInteract(pPlayerEnt, pInteractable, NULL, NULL);
...
bool LootBag_TakeAll(	Entity* player_ent, S32 iDstBagID, InventoryBag*** peaLootBags, int idx, 
						EntityRef opt_loot_ent, const ItemChangeReason *pReason)
{
	Team *pTeam = team_GetTeam(player_ent);
	BagIterator *iter;
	int iPartitionIdx;
	...
	if (opt_loot_ent)
	{
		pLootEnt = entFromEntityRef(iPartitionIdx, opt_loot_ent);
	}

	iter = bagiterator_Create();
	bagiterator_SetBag(CONTAINER_NOCONST(InventoryBag, pLootBag), iter);
	for (;!bagiterator_Stopped(iter); bagiterator_Next(iter))
	{
		Item *pItem = (Item*)bagiterator_GetItem(iter);
...
void loot_InteractTakeAllOfType(Entity* pPlayerEnt, S32 eItemType, S32 iBagID)
{
	...
	iter = bagiterator_Create();
	bagiterator_SetBagEarray((NOCONST(InventoryBag)***)&eaLootBags, iter);
	for(;!bagiterator_Stopped(iter);bagiterator_Next(iter))
	{
		Item* pItem = (Item*)bagiterator_GetItem(iter);
		ItemDef* pItemDef = bagiterator_GetDef(iter);
...
AUTO_COMMAND ACMD_SERVERONLY ACMD_ACCESSLEVEL(7) ACMD_CATEGORY(Standard, Reward) ACMD_NAME(RewardGrantWithScale);
void GrantRewardTableWithScale(Entity *pEnt, ACMD_NAMELIST("RewardTable", REFDICTIONARY) char *reward_table_name, F32 fScale)
{
	InventoryBag **bags = NULL;
	RewardTable *reward = RefSystem_ReferentFromString(g_hRewardTableDict,reward_table_name);

	if (reward_PowerExec_GenerateBags(pEnt, reward, entity_GetCombatLevel(pEnt), fScale, 0, NULL, &bags))
	{
		ItemChangeReason reason = {0};
		inv_FillItemChangeReason(&reason, pEnt, "Internal:GrantRewardTableEx", NULL);
		reward_GiveBags(pEnt, false, &bags, &reason);
...
static void GiveLootBagToPlayer(int iPartitionIdx, Entity* pEnt, int iLevel, F32 fScale, Vec3 vPos, RewardTable* pTable, PlayerCostume* pYoursCostume, PlayerCostume* pNotYoursCostume, const ItemChangeReason *pReason)
{
	...
	int i;
	reward_GenerateBagFromTable(iPartitionIdx, pEnt, pTable, &eaBags, iLevel, fScale, NULL, RewardContextType_EntKill, 0, false, false, NULL);
...
// check to see if map has a afk threshold, if so make sure everyone has a score that is above the threshold
// If there isn't a threshold then all player will qualify. Players failing the threshold will not get rewards and also have a (leaver) penalty assigned to them.
static void CheckQueueThreshold(S32 iPartitionIdx, Entity  ***pEntsIn, Entity ***pEntsOut)
{
	eaCopy(pEntsOut, pEntsIn);	// default, all qualify

	if(g_QueueConfig.bQueuedRewardThreshold && eaSize(pEntsOut) > 0)
	{
		ShardVariable *pShardVar = shardvariable_GetByName("QueueThresholdOff");
		...
		if(pShardVar)
		{
			...
			if(MultiValGetInt(&val,NULL) > 0)
			{
				// reward thresholds are off
				return;
			}
		}


		if(pInfo && pchQueueDefName)
		{
			S32 iValue = 0;
			S32 i;

			for(i = 0; i < eaSize(&g_QueueConfig.eaQueueThresholds); ++i)
			{
				QueueDef *pQueueDef = GET_REF(g_QueueConfig.eaQueueThresholds[i]->hDef);
				if(pQueueDef && stricmp(pchQueueDefName, pQueueDef->pchName) == 0)
				{
					iValue = g_QueueConfig.eaQueueThresholds[i]->iThreshold * g_QueueConfig.QueueThresholdMultipler;
					break;
				}
			}
			...
			if(iValue > 0)
			{
				...
				if(pState)
				{
					for(i = eaSize(pEntsOut) - 1; i >=0; --i)
					{
						Entity *pEntity = (*pEntsOut)[i];	// for readability recording our entity
						if(pEntity)
						{
							S32 iTotalValue = 0;
							bool bFound = false;
							PlayerMapValues* pPlayerValues = eaIndexedGetUsingInt(&pState->pPlayerValueData->eaPlayerValues, entGetContainerID(pEntity));
							// Only use the values if the player has been on the map for more than 2 minutes. This will prevent late arrivals from being penalized
							if(pPlayerValues && timeSecondsSince2000() > pPlayerValues->uTimeInitialized + SECONDS_PER_MINUTE * 2)
							{
								MapStateValue* pValue;
								S32 k;
								for(k = 0; k < eaSize(&g_QueueConfig.eaQueueLoggedEvents);++k)
								{
									pValue = mapState_FindMapValueInArray(&pPlayerValues->eaValues, g_QueueConfig.eaQueueLoggedEvents[k]->pcEventValue);
									if(pValue)
									{
										iTotalValue += MultiValGetInt(&pValue->mvValue,NULL);
										bFound = true;
									}
								}
							}

							// Penalize players that have a value too low that have been found. If a player is late to a queue they might not have been recorded so don't penalize
							if(iTotalValue < iValue && bFound)
							{
								// AFK player, remove them from reward list
								entLog(LOG_QAFK,pEntity,"QAFK_Removed","%s",pchQueueDefName);	
								RemoteCommand_aslQueue_PenalizePlayer(GLOBALTYPE_QUEUESERVER, 0, entGetContainerID(pEntity), pchQueueDefName, true);
								eaRemove(pEntsOut, i);
							}
						}
					}
...
AUTO_EXPR_FUNC(ai, encounter_action) ACMD_NAME(QueueThresholdGiveLoot);
void exprFunc_QueueThresholdGiveLoot(ExprContext* pContext, ACMD_EXPR_PARTITION iPartitionIdx, ACMD_EXPR_ENTARRAY_IN entsIn, ACMD_NAMELIST("RewardTable", REFDICTIONARY) char *reward_table_name, char* pchNamedPoints, ACMD_NAMELIST("PlayerCostume", REFDICTIONARY) char* pchYoursCostumeOverride, ACMD_NAMELIST("PlayerCostume", REFDICTIONARY) char* pchNotYoursCostumeOverride)
{
	Entity **eaEntsOut = NULL;

	// remove ents from reward list if they
	CheckQueueThreshold(iPartitionIdx, entsIn, &eaEntsOut);

	exprFunc_GiveLootBagToEachPlayerWithScale(pContext, iPartitionIdx, &eaEntsOut, reward_table_name, pchNamedPoints, pchYoursCostumeOverride, pchNotYoursCostumeOverride, NULL);

	eaDestroy(&eaEntsOut);
}

// Generates a loot bag for each player in entsIn, no loot is given if queue threshold check fails ... scale version
AUTO_EXPR_FUNC(ai, encounter_action) ACMD_NAME(QueueThresholdGiveLootScaled);
void exprFunc_QueueThresholdGiveLootScaled(ExprContext* pContext, ACMD_EXPR_PARTITION iPartitionIdx, ACMD_EXPR_ENTARRAY_IN entsIn, ACMD_NAMELIST("RewardTable", REFDICTIONARY) char *reward_table_name, char* pchNamedPoints, ACMD_NAMELIST("PlayerCostume", REFDICTIONARY) char* pchYoursCostumeOverride, ACMD_NAMELIST("PlayerCostume", REFDICTIONARY) char* pchNotYoursCostumeOverride, ACMD_EXPR_SUBEXPR_IN subExpr)
{
	Entity **eaEntsOut = NULL;
	...
	exprFunc_GiveLootBagToEachPlayerWithScale(pContext, iPartitionIdx, entsIn, reward_table_name, pchNamedPoints, pchYoursCostumeOverride, pchNotYoursCostumeOverride, subExpr);
...
void exprFunc_GiveLootBagToTeam(ExprContext* pContext, ACMD_EXPR_PARTITION iPartitionIdx, ACMD_EXPR_ENTARRAY_IN entsIn, ACMD_NAMELIST("RewardTable", REFDICTIONARY) char *reward_table_name, char* pchNamedPoint, ACMD_NAMELIST("PlayerCostume", REFDICTIONARY) char* pchYoursCostumeOverride, ACMD_NAMELIST("PlayerCostume", REFDICTIONARY) char* pchNotYoursCostumeOverride)
{
	...
	if(pTeam){
		...
		reward_GenerateBagFromTable(iPartitionIdx, pEnt, pTable, &eaBags, pCreditTeam->iTeamCombatLevel, 1.0f, NULL, RewardContextType_EntKill, 0, false, false, NULL);
...
void reward_ActivityStart(const char *pchActivity)
{
	int i;

	for(i=0;i<eaSize(&g_RewardConfig.eaRewardModifiers);i++)
	{
		if(g_RewardConfig.eaRewardModifiers[i]->pchRequiredActivity == pchActivity)
		{
			eaPushUnique(&s_ppActiveRewardScalars,g_RewardConfig.eaRewardModifiers[i]);
		}
	}
}

void reward_ActivityEnd(const char *pchActivity)
{
	int i;
	...
	for(i=0;i<eaSize(&g_RewardConfig.eaRewardModifiers);i++)
	{
		if(g_RewardConfig.eaRewardModifiers[i]->pchRequiredActivity == pchActivity)
		{
			eaFindAndRemove(&s_ppActiveRewardScalars,g_RewardConfig.eaRewardModifiers[i]);
		}
	}
}

bool reward_GivePendingAccountReward(Entity *pEnt, const char *pchRewardTable, const char* pchGateType)
{
	RewardTable *pRewardTable = NULL;
	RewardGatedRewardTable *pRewardGatedRewardTable = NULL;

	if (!pEnt || !pEnt->pPlayer || !pchRewardTable || !pchGateType)
	{
		return false;
	}

	pRewardTable = RefSystem_ReferentFromString(g_hRewardTableDict, pchRewardTable);
	if (!pRewardTable)
	{
		return false;
	}

	pRewardGatedRewardTable = StructCreate(parse_RewardGatedRewardTable);
	SET_HANDLE_FROM_REFERENT(g_hRewardTableDict, pRewardTable, pRewardGatedRewardTable->hRewardTable);
	pRewardGatedRewardTable->eRewardGateType = StaticDefineIntGetInt(RewardGatedTypeEnum, pchGateType);

	eaPush(&pEnt->pPlayer->eaPendingAccountRewards, pRewardGatedRewardTable);

	return true;
}

// Process all rewards in the pending account rewards list on the player
void reward_ProcessPendingAccountRewards(Entity *pEnt)
{
	int i;
	...
	// Pull out each pending account reward from the list, process it, and then delete it
	for (i = eaSize(&pEnt->pPlayer->eaPendingAccountRewards) - 1; i >= 0; i--)
	{
		RewardGatedRewardTable *pRewardGatedRewardTable = eaRemove(&pEnt->pPlayer->eaPendingAccountRewards, i);
		RewardGatedInfo *pRewardGatedInfo = eaIndexedGetUsingInt(&g_RewardConfig.eRewardGateInfo, pRewardGatedRewardTable->eRewardGateType);
		if (pRewardGatedRewardTable && pRewardGatedInfo)
		{
			// If there is no existing reward gate data for this account reward, or the data exists but the time block on the data has expired,
			// or the gate has been accessed less than the number of allowed times for this time block, then 
			RewardGatedTypeData *pPlayerGated = eaIndexedGetUsingInt(&pEnt->pPlayer->eaAccountRewardGatedData, pRewardGatedRewardTable->eRewardGateType);
			if (!pPlayerGated || !Reward_StillInPreviousGatedTimeBlock(pPlayerGated->uTimeSet, pRewardGatedInfo) || 
				pPlayerGated->uNumTimes < pRewardGatedInfo->uNumberOfTimesToIncrement)
			{
				RewardTable *pRewardTable = GET_REF(pRewardGatedRewardTable->hRewardTable);
				GiveRewardBagsData *pRewardBagsData = StructCreate(parse_GiveRewardBagsData);
				GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
				TransactionReturnVal* returnVal;
				ItemChangeReason reason = {0};
				U32 eRewardGateType = pRewardGatedRewardTable->eRewardGateType;

				pRewardBagsData->ppRewardBags = NULL;
				pRewardBagsData->ppChoices = NULL;

				// Generate the rewards from the reward table, and then run a transaction
				// to give the player the rewards and the account reward gate
				if (pRewardTable)
				{
					RewardContext *pLocalContext = Reward_CreateOrResetRewardContext(NULL);

					pLocalContext->type = RewardContextType_MissionExpr;
					pLocalContext->pKilled = NULL;
					pLocalContext->RewardLevel = 0;
					pLocalContext->RewardScale = 0;
					SetKillerRewardContext(pEnt, pLocalContext);
					SetBagCostumeContext(pRewardTable, pLocalContext);
					reward_generateEx(entGetPartitionIdx(pEnt), pEnt, pLocalContext, pRewardTable, &pRewardBagsData->ppRewardBags, NULL, NULL, false, NULL, NULL);

					inv_FillItemChangeReason(&reason, pEnt, "GrantAccountGatedRewardTable", pRewardTable->pchName);

					returnVal = LoggedTransactions_CreateManagedReturnValEnt("GiveBags", pEnt, NULL, NULL);
					AutoTrans_mission_tr_GiveAccountGatedReward(returnVal, GetAppGlobalType(),
						entGetType(pEnt), entGetContainerID(pEnt),
						GLOBALTYPE_ENTITYSHAREDBANK, pEnt->pPlayer->accountID,
						eRewardGateType, pRewardBagsData, &reason, pExtract);
				}
			}
		}

		StructDestroy(parse_RewardGatedRewardTable, pRewardGatedRewardTable);
	}
}
