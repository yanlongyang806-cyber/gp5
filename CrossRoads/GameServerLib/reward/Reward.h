...
typedef struct RewardContext
{
	...
	bool bIgnoreExpressionEntries;				// if set, will ignore any entries that are expression based
	bool bInheritPlayerOwnedFlag;				// If set, all tables generated will adopt the behavior of the PlayerOwned flag.
...
void rewardbag_trh_AddItem(NOCONST(InventoryBag) *rw_bag, Item* item, bool bSetID);
#define rewardbag_AddItem(rw_bag, item, bSetID) rewardbag_trh_AddItem(CONTAINER_NOCONST(InventoryBag, rw_bag), item, bSetID)
...
void reward_FixupUGCKillCreditLimit2(Entity* pEnt);
...
void reward_GenerateBagsForItemAssignment(int iPartitionIdx, Entity *pPlayerEnt, SA_PARAM_NN_VALID RewardTable* pTable, S32 iLevel, F32 fScale, RewardNumericScale **eaNumericScales, bool bUseRewardMods, bool bIgnoreExpressionEntries, U32* pSeed, InventoryBag*** peaBags);
...
// Generates reward bags for item salvaging
