...
typedef enum SubscriptionRequestType
{
	...
	REQUESTTYPE_INGAMEMAIL,
	REQUESTTYPE_BADNAME,
	REQUESTTYPE_NAMECHANGE,
...
static void wrCSub_ExecuteRequest(SubscriptionRequest *request, Entity *pEnt, GameAccountData *pGAD)
{
	...
	xcase REQUESTTYPE_INGAMEMAIL:
	{
		NPCMailRequest *mailRequest = (NPCMailRequest*) request->data;
		if (pEnt && mailRequest)
			gslMailNPC_AddMailRequest(pEnt, mailRequest);
		else if(!pEnt)
			DoSlowCmdReturn(1, "Nonexistent player id.", mailRequest->pSlowReturnInfo);

		StructDestroy(parse_NPCMailRequest, mailRequest);
		bComplete = true;
		request->data = NULL;
		request->pti = NULL;
	}
	xcase REQUESTTYPE_BADNAME:
	{
		if (pEnt)
			player_BadName(pEnt);
		bComplete = true;
	}
	xcase REQUESTTYPE_NAMECHANGE:
	{
		char *rename = (char*) request->data;
		if (pEnt && rename && *rename)
			gslEntity_ChangeSavedName(pEnt, rename);
		bComplete = true;
		free (rename);
		bComplete = true;
		request->data = NULL;
...
static bool wrCSub_AddRequest(SubscriptionRequest *pSubRequest, void *data, ParseTable *pti)
{
	Entity *pEnt;
...
void wrCSub_AddDiscountRequest(WebDiscountRequest *pDiscount)
{
	...
	wrCSub_AddRequest(pRequest, NULL, NULL);
	PERFINFO_AUTO_STOP();
}
...
void wrCSub_AddNPCMailRequest(NPCMailRequest *pEmail)
{
	SubscriptionRequest *pRequest = NULL;
	NPCMailRequest *pEmailCopy = StructClone(parse_NPCMailRequest, pEmail);
	PERFINFO_AUTO_START_FUNC();
	pRequest = wrCSub_CreateRequest(0, pEmailCopy->characterID, 0, REQUESTTYPE_INGAMEMAIL);
	wrCSub_AddRequest(pRequest, pEmailCopy, parse_NPCMailRequest);
	PERFINFO_AUTO_STOP();
}

void wrCSub_AddBadNameRequest(U32 uCharacterID)
{
	SubscriptionRequest *pRequest = NULL;
	PERFINFO_AUTO_START_FUNC();
	pRequest = wrCSub_CreateRequest(0, uCharacterID, 0, REQUESTTYPE_BADNAME);
	wrCSub_AddRequest(pRequest, NULL, NULL);
	PERFINFO_AUTO_STOP();
}

void wrCSub_AddNameChangeRequest(U32 uCharacterID, const char *name)
{
	SubscriptionRequest *pRequest = NULL;
	PERFINFO_AUTO_START_FUNC();
	pRequest = wrCSub_CreateRequest(0, uCharacterID, 0, REQUESTTYPE_NAMECHANGE);
	wrCSub_AddRequest(pRequest, StructAllocString(name), NULL);
	PERFINFO_AUTO_STOP();
}
...
void wrCSub_RemoveRequest(U32 uSubRequestID)
{
	...
	{
		if (pRequest->pti)
			StructDestroyVoid(pRequest->pti, pRequest->data);
		else
			free(pRequest->data);
	}
