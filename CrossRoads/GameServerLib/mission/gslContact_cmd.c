				...
				if(pMissionDef && pOption->pOption && pOption->pOption->pData)
				{
					switch (pOption->pOption->pData->targetState)
					{
						xcase ContactDialogState_ViewOfferedMission:
						{
							...
							missiondef_GetSummaryAndDetails(pMissionDef, playerEnt, &summaryMsg, &detailStringMsg);

							if (GET_REF(detailStringMsg.hMessage)) {
								if(gConf.bSwapMissionOfferDialogText) {
									langFormatGameDisplayMessage(entGetLanguage(playerEnt), &estrDescriptionText2, &detailStringMsg, STRFMT_ENTITY(playerEnt), STRFMT_MAPVARS(ppMapVars), STRFMT_END);
								} else {
									langFormatGameDisplayMessage(entGetLanguage(playerEnt), &estrDescriptionText1, &detailStringMsg, STRFMT_ENTITY(playerEnt), STRFMT_MAPVARS(ppMapVars), STRFMT_END);
								}
							}
							if (GET_REF(summaryMsg.hMessage)) {
								if(gConf.bSwapMissionOfferDialogText) {
									langFormatGameDisplayMessage(entGetLanguage(playerEnt), &estrDescriptionText1, &summaryMsg, STRFMT_ENTITY(playerEnt), STRFMT_MAPVARS(ppMapVars), STRFMT_END);
								} else {
									langFormatGameDisplayMessage(entGetLanguage(playerEnt), &estrDescriptionText2, &summaryMsg, STRFMT_ENTITY(playerEnt), STRFMT_MAPVARS(ppMapVars), STRFMT_END);
								}
							}
						}
		...
		if(contactDef) {
			...
			if(pInfo) {
				...
				if(pRemoteContact && !pRemoteContact->pHeadshot)
				{
					...
					contact_CostumeToHeadshotData(playerEnt, &contactDef->costumePrefs, &pRemoteContact->pHeadshot, NULL);
					entity_SetDirtyBit(playerEnt, parse_InteractInfo, pInfo, true);
					entity_SetDirtyBit(playerEnt, parse_Player, playerEnt->pPlayer, false);
				}
			}
