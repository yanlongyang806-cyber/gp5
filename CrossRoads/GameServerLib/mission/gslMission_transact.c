...
RewardGatedDataInOut *mission_trh_CreateRewardGatedData(ATH_ARG NOCONST(Entity)* pEnt)
{
	...
	for(i = 0; i < eaSize(&pEnt->pPlayer->eaRewardGatedData); ++i)
	{
		if(NONNULL(pEnt->pPlayer->eaRewardGatedData) && pEnt->pPlayer->eaRewardGatedData[i]->eType != RewardGatedType_None)
		{
			...
			if (Reward_StillInPreviousGatedTimeBlock(pPlayerGated->uTimeSet, pRewardGatedInfo))
			{
			...
			if(pRewardGatedInfo)
			{
				...
				if(pPlayerGated)
				{
					...
					else
					{
						...
						pPlayerGated->uTimeSet = uNowSeconds;
					}
				}
				else
				{
					...
					pNewGatedData->uTimeSet = uNowSeconds;
...
void mission_trh_GenerateMissionRewards(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt,
										MissionDef *pDef, 
										int eState, 
										const ContactRewardChoices* rewardChoices, 
										U32 *seed, 
										...
										F32 fAverageDurationInMinutes)
{
	...
	if(bUGCProject)
	{
		if (isProductionEditMode() && gConf.bNoUGCRewardsInPreview)
			bMissionQualifiesForUGCRewards = false;
		else
...
void mission_trh_CleanupRecentSecondaryList(ATH_ARG NOCONST(Entity)* pEnt)
{
	...
	for (i = eaSize(&pEnt->pPlayer->missionInfo->eaRecentSecondaryMissions)-1; i>=0; --i){
		...
		if (pCompletedMission->completedTime + missiondef_GetSecondaryCreditLockoutTime(pDef) < ShardClock_SecondsSince2000()){
...
// add Titles to Perks.
AUTO_TRANS_HELPER
ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .pInventoryV2.Pplitebags, .pInventoryV2.Peaowneduniqueitems, .Pplayer.Missioninfo.Completedmissions, .pInventoryV2.Ppinventorybags, .Psaved.Ppallowedcritterpets, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Hallegiance, .Hsuballegiance, .Itemidmax, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Psaved.Ppownedcontainers, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Pplayer.Playertype, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
...
bool mission_trh_UpdateCooldownForMission(ATR_ARGS, ATH_ARG NOCONST(Entity)* ent, const char* missionName, U32 startTime)
{
	...
	//   This function is only for dropping missions. It will not be called if 	bCooldownOnlyOnSuccess is true.

	if(NONNULL(missionDef) && mission_HasCooldownProperties(missionDef))
	{
		...
		if(pCompletedMission)
		{
			...
			if(pStats) {
				...
				bInCooldown = mission_IsInCooldownBlock(missionDef, pStats->lastStartTime, pStats->lastCompletedTime);
				...
				{
					// Reset cooldown count and times.
					pStats->lastCompletedTime = ShardClock_SecondsSince2000();
					pStats->lastStartTime = startTime;
					pCompletedMission->iRepeatCooldownCount = 1;
				}
				...
				pCompletedMission->completedTime = ShardClock_SecondsSince2000();
		...
		else
		{
			...
			if(missionCooldown->startTime == 0)
			{
				...
				missionCooldown->completedTime = ShardClock_SecondsSince2000();
				missionCooldown->startTime = startTime;
				missionCooldown->iRepeatCooldownCount = 1;
			}
			else
			{
				// increase cooldown count
...
AUTO_TRANS_HELPER
ATR_LOCKS(ent, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Pplitebags, .pInventoryV2.Peaowneduniqueitems, .Pplayer.Missioninfo.Missions, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Missioninfo.Eamissionrequests, .pInventoryV2.Ppinventorybags, .Hallegiance, .Hsuballegiance, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Unextrequestid, pPlayer.missionInfo.eaMissionCooldowns[], .Pchar.Ilevelexp");
NOCONST(Mission)* mission_trh_AddMissionInternal(ATR_ARGS, ATH_ARG NOCONST(Entity)* ent, MissionDef *missionDef, const char* missionName, int iMissionLevel, const MissionOfferParams *pParams, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	...
	if (missionDef && missionDef->bSendCompletionNotify && missionDef->missionType != MissionType_Perk) {
	...
	if (missionDef->repeatable && (missionDef->fRepeatCooldownHours > 0 || missionDef->fRepeatCooldownHoursFromStart)){
		...
		bool bInCooldown=false;
		U32 uLastStartedTime = 0;
		U32 uLastCompletedTime = 0;

		if (pCompletedMission){
			uLastStartedTime = completedmission_GetLastStartedTime(pCompletedMission);
			uLastCompletedTime = completedmission_GetLastCompletedTime(pCompletedMission);
		...
		else 
		{
			...
			if(pCooldown) 
			{
				uLastStartedTime = pCooldown->startTime;
				uLastCompletedTime = pCooldown->completedTime;
				iRepeatCooldownCount = pCooldown->iRepeatCooldownCount;
			}
		}

		bInCooldown = mission_IsInCooldownBlock(missionDef, uLastStartedTime, uLastCompletedTime);

		if (bInCooldown && iRepeatCooldownCount >= missionDef->uRepeatCooldownCount)
		{
	...
	newMission->startTime = ShardClock_SecondsSince2000();
...
AUTO_TRANS_HELPER
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Eamissionrequests, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Pplayer.Pprogressioninfo.Eareplaydata, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pplayer.Pprogressioninfo.Pteamdata, .Pplayer.Pprogressioninfo.Ppchcompletednodes, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Unextrequestid, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pplayer.Earewardmods, .Pteam.Lastrecruittype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Pplayer.Missioninfo.Eamissioncooldowns, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.pEmailV2.Mail, .Psaved.Nextactivitylogid, .Psaved.Activitylogentries[AO], .Pteam.Iteamid, .Pteam.Estate, .Pplayer.Pprogressioninfo.Hmostrecentlyplayednode, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
...
AUTO_TRANSACTION
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Eamissionrequests, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Pplayer.Pprogressioninfo.Eareplaydata, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pplayer.Pprogressioninfo.Pteamdata, .Pplayer.Pprogressioninfo.Ppchcompletednodes, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Unextrequestid, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pplayer.Earewardmods, .Pteam.Lastrecruittype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Pplayer.Missioninfo.Eamissioncooldowns, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.pEmailV2.Mail, .Psaved.Nextactivitylogid, .Psaved.Activitylogentries[AO], .Pteam.Iteamid, .Pteam.Estate, .Pplayer.Pprogressioninfo.Hmostrecentlyplayednode, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
	ATR_LOCKS(eaVarContainer, ".id, .Uclock, .Eaworldvars")
	ATR_LOCKS(pGuild, ".Inextactivitylogentryid, .Eaactivityentries, .Eamembers, .Pguildstatsinfo, .Htheme");
...
void missioninfo_AddMissionByName(int iPartitionIdx, MissionInfo* info, const char* missionName, TransactionReturnCallback cb, UserData data)
{
	...
	{
		ErrorOrAlert("ADD_MISSION_BY_NAME_INVALID",
	...
	if(NONNULL(missionDef) && mission_HasCooldownProperties(missionDef))
	{
		...
		else
		{
			...
			if(NONNULL(missionCooldown))
			{
				...
				bInCooldown = mission_IsInCooldownBlock(missionDef, missionCooldown->startTime, missionCooldown->completedTime);
...
AUTO_TRANS_HELPER
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pplayer.Missioninfo.Unextrequestid, .Pplayer.Missioninfo.Missions, .Pplayer.pEmailV2.Mail, .Psaved.Nextactivitylogid, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Activitylogentries[AO], .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pplayer.Earewardmods, .Pteam.Lastrecruittype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Eamissionrequests, .Pplayer.Missioninfo.Eamissioncooldowns, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
	ATR_LOCKS(eaVarContainer, ".id, .Uclock, .Eaworldvars")
	ATR_LOCKS(pGuild, ".Inextactivitylogentryid, .Eaactivityentries, .Eamembers, .Pguildstatsinfo, .Htheme")
...
AUTO_TRANS_HELPER
	ATR_LOCKS(ent, ".Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Psaved.Ppallowedcritterpets, .Pplayer.Earewardmods, .Pteam.Lastrecruittype, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .pInventoryV2.Ppinventorybags, .Psaved.Ppbuilds, .Hallegiance, .Hsuballegiance, .pInventoryV2.Pplitebags, .Pplayer.Playertype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems")
															   ...
															   GameAccountDataExtract *pExtract)
{
	...
	// Grant Rewards
	{
		...
		F32 fAverageDurationInMinutes = isProductionEditMode() ? 15.0f : ugcMissionData_trh_GetPlayingTimeForRewards(ATR_PASS_ARGS, mission->pUGCMissionData);
									 ...
									 GameAccountDataExtract *pExtract)
{
	...
	{
		if(ActivityLog_tr_AddEntityLogEntry(ATR_PASS_ARGS, ent, ActivityLogEntryType_SimpleMissionComplete, missionName, ShardClock_SecondsSince2000(), 0.0f) != TRANSACTION_OUTCOME_SUCCESS)
		{
			bResult = false;
		}
	} 
	else if (mission->eCreditType == MissionCreditType_Primary)
	{
		...
		if (completedMissionStats){
			...
			uLastTm = ShardClock_SecondsSince2000();
			...
			bIncrement = mission_IsInCooldownBlock(missionDef, completedMissionStats->lastStartTime, completedMissionStats->lastCompletedTime);
		...
		else if (missionDef->missionType == MissionType_Nemesis || missionDef->missionType == MissionType_NemesisArc || missionDef->missionType == MissionType_NemesisSubArc || missionDef->repeatable)
		{
			...
			completedMissionStats->firstCompletedTime = ShardClock_SecondsSince2000();
			completedMissionStats->lastCompletedTime =  ShardClock_SecondsSince2000();
			...
			completedMission->completedTime = ShardClock_SecondsSince2000();
		...
		if (missiondef_GetSecondaryCreditLockoutTime(missionDef) > 0){
			...
			if (!completedMission)
			{
				...
				if (mission_HasCooldownProperties(missionDef))
				{
...
AUTO_TRANSACTION
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.Pprogressioninfo.Ppchwindbackmissions, .Pplayer.Pprogressioninfo.Ppchskippedmissions, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .pInventoryV2.Ppinventorybags, .Pplayer.Missioninfo.Eamissioncooldowns, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .pInventoryV2.Pplitebags, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.pEmailV2.Mail, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Nextactivitylogid, .Psaved.Activitylogentries[AO], .Pplayer.Earewardmods, .Pteam.Lastrecruittype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Earecentsecondarymissions[AO], .Pteam.Iteamid, .Pteam.Estate, .Pplayer.Missioninfo.Eamissionrequests, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Pprogressioninfo.Pteamdata.Ppchcompletedmissions, .Pplayer.Pprogressioninfo.Eareplaydata, .Pplayer.Pprogressioninfo.Ppchcompletednodes, pPlayer.missionInfo.eaRecentSecondaryMissions[], .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .pInventoryV2.Peaowneduniqueitems")
	ATR_LOCKS(eaVarContainer, ".id, .Uclock, .Eaworldvars")
	ATR_LOCKS(pGuild, ".Inextactivitylogentryid, .Eaactivityentries, .Eamembers, .Pguildstatsinfo, .Htheme");
...
void mission_TurnInMissionInternal(MissionInfo* info, Mission* mission, ContactRewardChoices* rewardChoices)
{
	...
	if (info->parentEnt && missionDef && missionDef->name && mission->state == MissionState_Succeeded)
	{
		...
		U32 uiCurrentTime = ShardClock_SecondsSince2000();
		ItemChangeReason reason = {0};
...
AUTO_TRANS_HELPER
ATR_LOCKS(ent, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Pplayer.Missioninfo.Eamissionrequests, .pInventoryV2.Pplitebags, .pInventoryV2.Peaowneduniqueitems, .pInventoryV2.Ppinventorybags, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Completedmissions, .Pchar.Ilevelexp");
bool mission_trh_DropMission(ATR_ARGS, ATH_ARG NOCONST(Entity)* ent, const char* missionName, bool bHideNotification, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
...
AUTO_TRANS_HELPER
ATR_LOCKS(ent, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Pplitebags, .pInventoryV2.Peaowneduniqueitems, .Pplayer.Missioninfo.Eamissionrequests, .pInventoryV2.Ppinventorybags, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Completedmissions, .Pchar.Ilevelexp")
ATR_LOCKS(mission, ".Childfullmissions, .Children");
void mission_trh_DropFullChildren(ATR_ARGS, ATH_ARG NOCONST(Entity)* ent, ATH_ARG NOCONST(Mission)* mission, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	int i;
...
AUTO_TRANSACTION
ATR_LOCKS(ent, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .pInventoryV2.Pplitebags, .pInventoryV2.Peaowneduniqueitems, .Pplayer.Missioninfo.Eamissionrequests, .pInventoryV2.Ppinventorybags, .Psaved.Conowner.Containertype, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Completedmissions, .Psaved.Conowner.Containerid, .Pchar.Ilevelexp");
...
static void mission_np_ChangeMissionState(int iPartitionIdx, Mission *pMission, MissionState newState, bool bForcePermanentComplete)
{
	...
	if (  newState == MissionState_Succeeded && pMission->parent == NULL
		...
		if(pOpenMission) {
			...
			pOpenMission->uiSucceededTime = ShardClock_SecondsSince2000();
	...
	if(mission_GetType(pMission) == MissionType_OpenMission)
	{
		...
		U32 completionTime = bIsSucceedingUGCMap ? ShardClock_SecondsSince2000() - pMission->startTime : 0;
		...
		while ((currEnt = EntityIteratorGetNext(iter)))
		{
			...
			if (pInfo && pInfo->pchCurrentOpenMission == pRootMission->missionNameOrig)
			{
				if(bIsSucceedingUGCMap)
				{
					log_printf(LOG_UGC, "EntityPlayer %u contributed to the OpenMission on map %s with duration %u for UGC mission %s", entGetContainerID(currEnt), zmapGetName(NULL), ShardClock_SecondsSince2000() - pMission->startTime, namespacedMissionName);
				...
				// Send notifications
				if (pDef && pDef->bSendCompletionNotify && mission_HasDisplayName(pRootMission)){
		...
		waypoint_FlagWaypointRefreshPlayers(iPartitionIdx);
	}
...
// This changes the state of a SubMission and runs any appropriate actions/grants rewards
AUTO_TRANSACTION
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pplayer.pEmailV2.Mail, .Psaved.Nextactivitylogid, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Activitylogentries[AO], .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pplayer.Missioninfo.Unextrequestid, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Earewardmods, .Pteam.Lastrecruittype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Eamissionrequests, .Pplayer.Missioninfo.Eamissioncooldowns, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
	ATR_LOCKS(eaVarContainer, ".id, .Uclock, .Eaworldvars")
	ATR_LOCKS(pGuild, ".Inextactivitylogentryid, .Eaactivityentries, .Eamembers, .Pguildstatsinfo, .Htheme");
enumTransactionOutcome mission_tr_ChangeSubMissionState(ATR_ARGS, NOCONST(Entity)* ent, 
														...
														const ContactRewardChoices *pRewardChoices,
														const ItemChangeReason *pReason,
														GameAccountDataExtract *pExtract)
{
	NOCONST(Mission)* mission = NULL;
	...
	if (newState == MissionState_Succeeded)
	{
		...
		F32 fAverageDurationInMinutes = isProductionEditMode() ? 15.0f : ugcMissionData_trh_GetPlayingTimeForRewards(ATR_PASS_ARGS, parentMission->pUGCMissionData);
		...
		if (mission_trh_GrantMissionRewards(ATR_PASS_ARGS, ent, eaPets, missionDef, MissionState_Succeeded, pRewardChoices, &seed, 0, iLevel, 0, 0, true, pReason, pExtract, bUGCProject, bCreatedByAccount, bStatsQualifyForUGCRewards, bQualifyForUGCFeaturedRewards, fAverageDurationInMinutes) != TRANSACTION_OUTCOME_SUCCESS)
	...
	else if (newState == MissionState_Failed)
	{
		...
		if (mission_trh_GrantMissionRewards(ATR_PASS_ARGS, ent, eaPets, /*NULL,*/ missionDef, MissionState_Failed, pRewardChoices, &seed, 0, mission->iLevel, 0, 0, false, pReason, pExtract, false, false, false, false, 0) != TRANSACTION_OUTCOME_SUCCESS)
			TRANSACTION_RETURN_LOG_FAILURE("Mission could not grant rewards: %s", pchMissionName);
	}
...
enumTransactionOutcome mission_tr_ChangePerkSubMissionState(ATR_ARGS, NOCONST(Entity)* ent, 
															...
															GameAccountDataExtract *pExtract)
{
	return mission_tr_ChangeSubMissionState(ATR_PASS_ARGS, ent, eaPets, eaVarContainer, pGuild, /*NULL,*/ pchRootMissionName, pchParentMissionName, pchMissionName, newState, bForcePermanentComplete, pRewardChoices, pReason, pExtract);
}
...
enumTransactionOutcome mission_tr_ChangeRootMissionStateNoLocking(ATR_ARGS, NOCONST(Entity)* ent, const char* pchMissionName, int newState, U32 bForcePermanentComplete, int bRecordUGCCompletion)
{
	...
	if (newState == MissionState_Succeeded)
	{
		...
		if(resExtractNameSpace(pchMissionName, pchNameSpace, pchBase) && 0 == stricmp(pchBase, "Mission") && iUGCProjectID)
			QueueRemoteCommand_mission_RecordCompletion(ATR_RESULT_SUCCESS, GetAppGlobalType(), GetAppGlobalID(), iUGCProjectID, pchNameSpace, (ShardClock_SecondsSince2000() - mission->startTime) / 60, bRecordUGCCompletion);
	}
	else if (newState == MissionState_Failed)
	{
...
AUTO_TRANSACTION
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pteam.Lastrecruittype, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Costumedata.Eaunlockedcostumerefs, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Eamissionrequests, .Pplayer.Missioninfo.Eamissioncooldowns, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.pEmailV2.Mail, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Nextactivitylogid, .Psaved.Activitylogentries[AO], .Pplayer.Earewardmods, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
	ATR_LOCKS(eaVarContainer, ".id, .Uclock, .Eaworldvars")
	ATR_LOCKS(pGuild, ".Inextactivitylogentryid, .Eaactivityentries, .Eamembers, .Pguildstatsinfo, .Htheme");
														 ...
														 int bRecordUGCCompletion)
{
	...
	if (newState == MissionState_Succeeded)
	{
		...
		F32 fAverageDurationInMinutes = bUGCProjectInPreviewMode ? 15.0f : ugcMissionData_trh_GetPlayingTimeForRewards(ATR_PASS_ARGS, mission->pUGCMissionData);
	...
	else if (newState == MissionState_Failed)
	{
		...
		if (mission_trh_GrantMissionRewards(ATR_PASS_ARGS, ent, eaPets, /*NULL,*/ missionDef, MissionState_Failed, pRewardChoices, NULL, 0, mission->iLevel, 0, 0, false, pReason, pExtract, false, false, false, false, 0) != TRANSACTION_OUTCOME_SUCCESS)
		{
			StructDestroy(parse_WorldVariableArray, pVariableArray);
			TRANSACTION_RETURN_LOG_FAILURE("Mission could not grant rewards: %s", pchMissionName);
		}
...
enumTransactionOutcome mission_tr_ChangePerkRootMissionState(ATR_ARGS, NOCONST(Entity)* ent, 
															 ...
															 GameAccountDataExtract *pExtract)
{
	return mission_tr_ChangeRootMissionState(ATR_PASS_ARGS, ent, eaPets, eaVarContainer, pGuild, /*NULL, */pchMissionName, newState, bForcePermanentComplete, pRewardChoices, pReason, pExtract, /*bRecordUGCCompletion=*/false);
}
...
static void mission_tr_ChangeMissionState(int iPartitionIdx, Mission* mission, MissionState newState, bool bForcePermanentComplete, ContactRewardChoices* pRewardChoices)
{
	...
	{
		if( pRewardChoices && eaSize( &pRewardChoices->ppItemNames )) {
			devassert( mission_IsPersisted( mission ) && mission->infoOwner && mission->infoOwner->parentEnt
					   && missiondef_MustLockInventoryForStateChange( missionDef, rootMissionDef, newState ));
		}
				...
				if (missiondef_MustLockInventoryForStateChange(missionDef, pRootDef, newState) 
					...
					if (pRootDef->missionType == MissionType_Perk){
						...
						AutoTrans_mission_tr_ChangePerkSubMissionState(returnVal, GetAppGlobalType(), 
							...
							newState, bForcePermanentComplete, pRewardChoices, &reason, pExtract);
						ea32Destroy(&eaPets);
					} else {
					...
					|| bMustLockGuildLog)
				{
					...
					if (pRootDef->missionType == MissionType_Perk){
						...
						AutoTrans_mission_tr_ChangePerkRootMissionState(returnVal, GetAppGlobalType(), 
							...
							GLOBALTYPE_SHARDVARIABLE, bMustLockShardVars ? shardVariable_GetContainerIDList() : &eaEmptyList,
							GLOBALTYPE_GUILD, bMustLockGuildLog ? mission->infoOwner->parentEnt->pPlayer->pGuild->iGuildID : 0, 
							mission->missionNameOrig, newState, bForcePermanentComplete, pRewardChoices, &reason, pExtract);
					} else {
						...
						AutoTrans_mission_tr_ChangeRootMissionState(returnVal, GetAppGlobalType(),
							...
							GLOBALTYPE_SHARDVARIABLE, bMustLockShardVars ? shardVariable_GetContainerIDList() : &eaEmptyList,
							GLOBALTYPE_GUILD, bMustLockGuildLog ? mission->infoOwner->parentEnt->pPlayer->pGuild->iGuildID : 0, 
							mission->missionNameOrig, newState, bForcePermanentComplete, pRewardChoices, &reason, pExtract, bRecordUGCCompletion);
					}
					ea32Destroy(&eaPets);
				} else {
					if (pRootDef->missionType == MissionType_Perk){
...
void mission_tr_CompleteMission(int iPartitionIdx, Mission* mission, bool bForcePermanentComplete, ContactRewardChoices* pRewardChoices)
{
	mission_tr_ChangeMissionState(iPartitionIdx, mission, MissionState_Succeeded, bForcePermanentComplete, pRewardChoices);
}

void mission_tr_FailMission(int iPartitionIdx, Mission* mission)
{
	mission_tr_ChangeMissionState(iPartitionIdx, mission, MissionState_Failed, false, NULL);
}

void mission_tr_UncompleteMission(int iPartitionIdx, Mission* mission)
{
	mission_tr_ChangeMissionState(iPartitionIdx, mission, MissionState_InProgress, false, NULL);
}
...
AUTO_TRANSACTION
ATR_LOCKS(ent, ".Pplayer.Eaplayernumericthresholds, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Pplayer.Missioninfo.Missions, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Missioninfo.Eamissionrequests, .pInventoryV2.Ppinventorybags, .Psaved.Conowner.Containertype, .Hallegiance, .Hsuballegiance, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Unextrequestid, .Psaved.Conowner.Containerid, pPlayer.missionInfo.eaMissionCooldowns[], .Pchar.Ilevelexp, \
...
AUTO_TRANSACTION
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Eamissionrequests, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Pteam.Lastrecruittype, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Costumedata.Eaunlockedcostumerefs, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Unextrequestid, .Pteam.Iteamid, .Pteam.Estate, .Pplayer.Pprogressioninfo.Hmostrecentlyplayednode, .Pplayer.Pprogressioninfo.Eareplaydata, .Pplayer.Pprogressioninfo.Pteamdata, .Pplayer.Pprogressioninfo.Ppchcompletednodes, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pplayer.Missioninfo.Eamissioncooldowns, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.pEmailV2.Mail, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Nextactivitylogid, .Psaved.Activitylogentries[AO], .Pplayer.Earewardmods, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
	ATR_LOCKS(eaVarContainer, ".id, .Uclock, .Eaworldvars")
	ATR_LOCKS(pGuild, ".Inextactivitylogentryid, .Eaactivityentries, .Eamembers, .Pguildstatsinfo, .Htheme");
...
AUTO_TRANSACTION
	ATR_LOCKS(ent, ".Pplayer.Pemailv2.Unextfuturedelivery, .Pplayer.Pemailv2.Bunreadmail, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Egender, .Pchar.Hclass, .Pchar.Hpath, .Pchar.Hspecies, .Pplayer.Missioninfo.Missions, .Pplayer.Missioninfo.Eamissionrequests, .Pinventoryv2.Ppinventorybags, .Pinventoryv2.Pplitebags, .Pplayer.pEmailV2.Mail, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Nextactivitylogid, .Psaved.Activitylogentries[AO], .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Pplayer.Missioninfo.Completedmissions, .Pplayer.Missioninfo.Unextrequestid, .Pteam.Iteamid, .Pteam.Estate, .Pplayer.Pprogressioninfo.Hmostrecentlyplayednode, .Pplayer.Pprogressioninfo.Eareplaydata, .Pplayer.Pprogressioninfo.Pteamdata, .Pplayer.Pprogressioninfo.Ppchcompletednodes, .Pplayer.Playertype, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pplayer.Earewardmods, .Pteam.Lastrecruittype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Pplayer.Missioninfo.Eamissioncooldowns, .Pplayer.Nemesisinfo.Eanemesisstates, .Pplayer.pEmailV2.Ilastusedid, .Pplayer.Langid, .Pplayer.eaRewardGatedData, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
	ATR_LOCKS(eaPets, ".Pcritter.Petdef, .Pinventoryv2.Peaowneduniqueitems")
	ATR_LOCKS(eaVarContainer, ".id, .Uclock, .Eaworldvars")
	ATR_LOCKS(pGuild, ".Inextactivitylogentryid, .Eaactivityentries, .Eamembers, .Pguildstatsinfo, .Htheme");
	...
	U32 iNumberOfPlays, F32 fAverageDurationInMinutes, S32 iProjectIsFeatured, S32 iProjectWasFeatured)
{
	if(NONNULL(pEnt) && NONNULL(pEnt->pPlayer) && NONNULL(pEnt->pPlayer->missionInfo))
	{
		NOCONST(UGCMissionData) *pUGCMissionData = ugc_trh_FindOrCreateMissionDataForNameSpace(ATR_PASS_ARGS, pEnt->pPlayer->missionInfo, strNameSpace);
		if(pUGCMissionData)
		{
			pUGCMissionData->bCreatedByAccount = !!iCreatedByAccount;
			pUGCMissionData->bStatsQualifyForUGCRewards = !!iStatsQualifyForUGCRewards;
			pUGCMissionData->iNumberOfPlays = iNumberOfPlays;
			pUGCMissionData->fAverageDurationInMinutes = fAverageDurationInMinutes;
			pUGCMissionData->bProjectIsFeatured = !!iProjectIsFeatured;
			pUGCMissionData->bProjectWasFeatured = !!iProjectWasFeatured;
...
enumTransactionOutcome mission_tr_SetUGCPlayingAsBetaReviewer(ATR_ARGS, NOCONST(Entity)* pEnt, const char *strNameSpace, int bPlayingAsBetaReviewer)
{
	if(NONNULL(pEnt) && NONNULL(pEnt->pPlayer) && NONNULL(pEnt->pPlayer->missionInfo))
	{
		...
		if(pUGCMissionData)
		{
			pUGCMissionData->bPlayingAsBetaReviewer = bPlayingAsBetaReviewer;
...
AUTO_TRANSACTION
ATR_LOCKS(pEnt, ".Pplayer.Eaaccountrewardgateddata, .Pinventoryv2.Ppinventorybags, .Psaved.Uiindexbuild, .Pchar.Ilevelexp, .Pplayer.Eaplayernumericthresholds, .Psaved.Ppbuilds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pinventoryv2.Pplitebags, .Pplayer.Playertype, .Psaved.Costumedata.Eaunlockedcostumerefs, .Pplayer.Pplayeraccountdata.Eapendingkeys, .Psaved.Ppownedcontainers, .Pinventoryv2.Peaowneduniqueitems, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets")
ATR_LOCKS(pSharedBank, ".Psharedbankdata");
enumTransactionOutcome mission_tr_GiveAccountGatedReward(ATR_ARGS, NOCONST(Entity)* pEnt, NOCONST(Entity)* pSharedBank, U32 eRewardGateType, GiveRewardBagsData *pRewardBagsData, const ItemChangeReason *pReason, GameAccountDataExtract *pExtract)
{
	RewardGatedTypeData *pAccountGated = NULL;
	RewardGatedTypeData *pPlayerGated = NULL;
	RewardGatedInfo *pRewardGatedInfo = NULL;
	U32 uNowSeconds;
	bool bGiveReward = true;

	if(ISNULL(pEnt) || ISNULL(pSharedBank))
	{
		return TRANSACTION_OUTCOME_FAILURE;
	}

	pRewardGatedInfo = eaIndexedGetUsingInt(&g_RewardConfig.eRewardGateInfo, eRewardGateType);
	if (!pRewardGatedInfo)
	{
		return TRANSACTION_OUTCOME_FAILURE;
	}

	// TODO: Remove this later to avoid locking the shared bank data struct if the struct gets any larger.
	// We should be creating this elsewhere. It's fine here for now since it's a brand new struct
	if (ISNULL(pSharedBank->pSharedBankData))
	{
		pSharedBank->pSharedBankData = StructCreateNoConst(parse_SharedBankData);
	}

	uNowSeconds = ShardClock_SecondsSince2000();

	// Check to see if this reward gate exists already in the shared bank. 
	pAccountGated = eaIndexedGetUsingInt(&pSharedBank->pSharedBankData->eaAccountRewardGatedData, eRewardGateType);
	if (pAccountGated)
	{
		if (!Reward_StillInPreviousGatedTimeBlock(pAccountGated->uTimeSet, pRewardGatedInfo) ||
			pAccountGated->uNumTimes < pRewardGatedInfo->uNumberOfTimesToIncrement)
		{
			pAccountGated->uNumTimes++;
			pAccountGated->uTimeSet = uNowSeconds;
		}
		else
		{
			bGiveReward = false;
	...
	// The reward gate does not exist, so create it here
	else
	{
		pAccountGated = StructCreate(parse_RewardGatedTypeData);

		pAccountGated->eType = eRewardGateType;
		pAccountGated->uNumTimes = 1;
		pAccountGated->uTimeSet = uNowSeconds;

		if(!pSharedBank->pSharedBankData->eaAccountRewardGatedData)
		{
			eaIndexedEnableNoConst(&pSharedBank->pSharedBankData->eaAccountRewardGatedData, parse_RewardGatedTypeData);
		}

		eaIndexedAdd(&pSharedBank->pSharedBankData->eaAccountRewardGatedData, pAccountGated);
	}

	// Blindly create or set the gate data on the player. We do no checks on the player data, since
	// the shared bank data is the only authoritative copy
	pPlayerGated = eaIndexedGetUsingInt(&pEnt->pPlayer->eaAccountRewardGatedData, eRewardGateType);
	if (pPlayerGated)
	{
		pPlayerGated->uNumTimes = pAccountGated->uNumTimes;
		pPlayerGated->uTimeSet = pAccountGated->uTimeSet;
	}
	else
	{
		if(ISNULL(pEnt->pPlayer->eaAccountRewardGatedData))
		{
			eaIndexedEnableNoConst(&pEnt->pPlayer->eaAccountRewardGatedData, parse_RewardGatedTypeData);
		}

		eaIndexedAdd(&pEnt->pPlayer->eaAccountRewardGatedData, pAccountGated);
	}

	// Give the reward bags to the player
	if (bGiveReward)
	{
		if (!inv_trh_GiveRewardBags(ATR_PASS_ARGS, pEnt, NULL, pRewardBagsData, kRewardOverflow_AllowOverflowBag, NULL, pReason, pExtract, NULL))
		{
			return TRANSACTION_OUTCOME_FAILURE;
		}
	}
