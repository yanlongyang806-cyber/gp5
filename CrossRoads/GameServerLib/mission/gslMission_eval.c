...
AUTO_EXPR_FUNC_STATIC_CHECK;
int mission_FuncMissionPerformedEmote_LoadVerify(ExprContext *pContext, ACMD_EXPR_PARTITION iPartitionIdx, const char* pchEmoteName, U32 uiTime)
{
	// Track any Mission State Events
	MissionDef *pMissionDef = exprContextGetVarPointerUnsafePooled(pContext, g_MissionDefVarName);
	if (pMissionDef) {
		...
		pEvent->type = EventType_Emote;
		pEvent->pchEmoteName = pchEmoteName;
		pEvent->pchEventName = allocAddString("PerformedEmote");
		pEvent->tMatchSource = TriState_Yes;
...
AUTO_EXPR_FUNC(player, mission) ACMD_NAME(PerformedEmote) ACMD_EXPR_STATIC_CHECK(mission_FuncMissionPerformedEmote_LoadVerify);
int mission_FuncMissionPerformedEmote(ExprContext *pContext, ACMD_EXPR_PARTITION iPartitionIdx, const char* pchEmoteName, U32 uiTime)
{
	Entity *pPlayerEnt = exprContextGetVarPointerUnsafePooled(pContext, g_PlayerVarName);
	if (!pPlayerEnt->pPlayer || !pPlayerEnt->pPlayer->pUI)
	{
		return false;
	}

	if (pPlayerEnt->pPlayer->pUI->uiLastEmoteTime >= uiTime &&
		stricmp(pPlayerEnt->pPlayer->pUI->pchLastEmote, pchEmoteName) == 0)
	{
		return true;
	}
...
AUTO_EXPR_FUNC(player, mission) ACMD_NAME(GiveAccountRewardGate);
int mission_GiveAccountGatedReward(ExprContext *pContext,
...
{
	Entity *pPlayerEnt = exprContextGetVarPointerUnsafePooled(pContext, g_PlayerVarName);
	return reward_GivePendingAccountReward(pPlayerEnt, pchRewardTable, pchGateType);
}

AUTO_EXPR_FUNC(ai, encounter_action) ACMD_NAME(GiveAccountRewardGateToEnts);
void mission_GiveAccountGatedRewardToEnts(ExprContext *pContext, ACMD_EXPR_ENTARRAY_IN entsIn,
	ACMD_NAMELIST("RewardTable", REFDICTIONARY) const char *pchRewardTable,
	ACMD_NAMELIST(RewardGatedTypeEnum, STATICDEFINE) const char* pchGateType)
{
	int i;
	for (i = 0; i < eaSize(entsIn); i++)
	{
		...
		reward_GivePendingAccountReward(pEnt, pchRewardTable, pchGateType);
	}
}