...
typedef struct ContactDialogMessages
{
	...
	REF_TO(Message) hZoneRewardsMsg;
...
void contactsystem_InitContactDialogMessages(void)
{
	...
	SET_HANDLE_FROM_STRING(gMessageDict, "ContactDialog.ZoneRewards", s_ContactMsgs.hZoneRewardsMsg);
...
static bool contact_CanParticipateInTeamDialog(Entity *pEntTeamMember, Entity *pEntTeamTalker, ContactDialog *pDialog)
{
	...
	{
		Entity *pContactEnt = entFromEntityRef(entGetPartitionIdx(pEntTeamMember), pDialog->headshot.headshotEnt);
	...
	if(gConf.bShowContactOffersOnCooldown) {
		for(i=0; i<eaSize(onCooldownMissionDefs); i++){
			...
			if(pNewOption && pInfo) {
				...
				/// ARGGH. The old code seems to have not been updated for all features that have been added to the cooldowns.
				//   Replace the whole thing with something that seems reasonable and cross fingers
				//  This does NOT seem to deal with missions that can be repeated multiple times.
				{
					U32 uSecondsLeft = mission_CooldownSecondsLeft(pMissionDef, uStartTime, uCompletedTime);
					if (uSecondsLeft > 0)
					{
						U32 uStartCooldownExpire = 	ShardClock_SecondsSince2000() + uSecondsLeft;
						if (uStartCooldownExpire > pNewOption->uCooldownExpireTime)
						{
	...
	for (i = 0; i < n; i++)
	{
		...
		if (missionDef && (pMissionOfferToProcess == NULL || pMissionOfferToProcess == missionOffer))
		{
			...
			if (!bRemoteInteract || (missionOffer->eRemoteFlags & ContactMissionRemoteFlag_Grant))
			{
				...
				if(gConf.bShowContactOffersOnCooldown && !activeMission && !bCanBeOffered && !bCanBeOfferedAsFlashback && mission_HasCooldownProperties(missionDef))
				{
	...
	if (pEnt && peaDialogBlocks)
	{
		...
		{			
			...
			if(estrResult){
				...
				langFormatGameDisplayMessage(entGetLanguage(pEnt), estrResult, &pDialog->displayTextMesg.msg, STRFMT_ENTITY(pEnt), STRFMT_ENTITY_KEY("Nemesis", pNemesisEnt), STRFMT_MAPVARS(ppMapVars), STRFMT_END);
				eaDestroy(&ppMapVars);
			}
			...
			if (ppchPooledSound)
			{
				(*ppchPooledSound) = gslVOGetAudioEvent(&pDialog->displayTextMesg);
			}
...
static ContactDialogOption* contactdialog_CreateDialogOptionEx(Entity *pPlayerEnt, ContactDialogOption*** peaOptionsList, const char *pchKey, Message *pDisplayTextMsg, ContactDialogState targetState, ContactActionType action, WorldVariable** eaMapVars, ContactDialogFormatterDef *pDialogFormatter)
{
	...
	switch (targetState) {
		...
		case ContactDialogState_GuildBank:
			...
			break;
		case ContactDialogState_ZoneRewards:
			pOption->eType = ContactIndicator_ZoneRewards;
			break;
		default:
...
static void contactdialog_CreateOptionsList(Entity *pPlayerEnt, ContactDef *pContactDef, ContactDialogOption*** peaOptionsToFill, bool bOnlyShowRemoteOptions)
{
			...
			if( curScreenType == ContactScreenType_List ) {
				...
				for (i = 0; i < eaSize(&eaImageMenuItems); i++){
					if ( (!eaImageMenuItems[i]->visibleCondition || contact_EvaluateAfterManualContextSetup(eaImageMenuItems[i]->visibleCondition))){
						...
						pNewOption->pchEventName = eaImageMenuItems[i]->eventName;
					}
				}
			}
	...
	if(!bOnlyShowRemoteOptions) 
	{
		...
		// Zone Rewards
		if (contact_IsZoneRewards(pContactDef))
		{
			pNewOption = contactdialog_CreateDialogOption(pPlayerEnt, peaOptionsToFill, "OptionsList.ZoneRewards", GET_REF(s_ContactMsgs.hZoneRewardsMsg), ContactDialogState_ZoneRewards, 0, eaMapVars);
		}
	}
...
//************************************************************************************
bool contact_CostumeToHeadshotData(Entity* pPlayerEnt, ContactCostume* pCostumePrefs, ContactHeadshotData** ppReturn, bool *bPlayAudio)
{
	...
	if(pCostumePrefs) {
		if(pCostumePrefs->eCostumeType == ContactCostumeType_Specified && GET_REF(pCostumePrefs->costumeOverride)) {
			...
			PetContactList_GetPetOrCostume(pPlayerEnt, pList, NULL, &pPet, NULL, &pCritterCostume, bPlayAudio);
			...
			if (pPlayerEnt->pSaved && pPlayerEnt->pSaved->pPuppetMaster)
			{
				// This probably ought to be somewhere Star Trek specific, but Star Trek
				// is probably the only game that'll take this code path...
				Entity *pGroundEnt = entity_GetPuppetEntityByType(pPlayerEnt, "Ground", NULL, true, true);
				if (pGroundEnt && pGroundEnt != pPlayerEnt)
					pCostume = costumeEntity_GetActiveSavedCostume(pPlayerEnt);
			}
...
static void contactdialog_SetCostumeFromHeadshotData(SA_PARAM_NN_VALID ContactDialogCostume* pDialogCostume, 
													 SA_PARAM_NN_VALID ContactHeadshotData* pHeadshot)
{
	// Clear the fields
	pDialogCostume->iHeadshotOverridePetID = 0;
	pDialogCostume->headshotEnt = 0;
	if(IS_HANDLE_ACTIVE(pDialogCostume->hHeadshotOverride))
		REMOVE_HANDLE(pDialogCostume->hHeadshotOverride);
	if(pDialogCostume->pHeadshotOverride)
		StructDestroySafe(parse_PlayerCostume, &pDialogCostume->pHeadshotOverride);

	if(pHeadshot->iPetID)
	{
		// Set pet ID
		pDialogCostume->iHeadshotOverridePetID = pHeadshot->iPetID;
	} 
	else if(pHeadshot->pCostume)
	{
		// Set costume
		pDialogCostume->pHeadshotOverride = pHeadshot->pCostume;
		pHeadshot->pCostume = NULL;
	}
	else if(IS_HANDLE_ACTIVE(pHeadshot->hCostume))
	{
		// Set costume ref
		COPY_HANDLE(pDialogCostume->hHeadshotOverride, pHeadshot->hCostume);
	}
}

static void contactdialog_CopyStyleOverrides(ContactCostumeStyle **ppStyleOut, ContactCostumeStyle *pStyle)
{
	S32 i;

	if (!ppStyleOut || !pStyle)
		return;
	if (!pStyle->pchSky && !pStyle->pchFrame && !pStyle->pchBackground && !eaSize(&pStyle->eaHeadshotFX))
		return;

	if (!*ppStyleOut)
		*ppStyleOut = StructCreate(parse_ContactCostumeStyle);

	if (pStyle->pchBackground)
		(*ppStyleOut)->pchBackground = pStyle->pchBackground;
	if (pStyle->pchBackground)
		(*ppStyleOut)->pchFrame = pStyle->pchFrame;
	if (pStyle->pchSky)
		(*ppStyleOut)->pchSky = pStyle->pchSky;

	for (i = 0; i < eaSize(&pStyle->eaHeadshotFX); i++) {
		HeadshotStyleFX *pStyleFX = StructClone(parse_HeadshotStyleFX, pStyle->eaHeadshotFX[i]);
		if (pStyleFX) {
			eaPush(&(*ppStyleOut)->eaHeadshotFX, pStyleFX);
		}
	}
}
											  ...
											  ContactCostume* pCostumePrefs)
{
	...
	if(pCostumePrefs) {
		...
		if(contact_CostumeToHeadshotData(pPlayerEnt, pCostumePrefs, &pHeadshot, NULL))
		{
			contactdialog_SetCostumeFromHeadshotData(&pDialog->headshot, pHeadshot);
		}

		StructDestroy(parse_ContactHeadshotData, pHeadshot);
	} 

	if (SAFE_MEMBER(pCostumePrefs, pchHeadshotStyle))
		pDialog->headshot.pchHeadshotStyleDef = SAFE_MEMBER(pCostumePrefs, pchHeadshotStyle);
	if (SAFE_MEMBER(pCostumePrefs, pchHeadshotBorderFrame))
		pDialog->headshot.pchHeadshotBorderFrame = SAFE_MEMBER(pCostumePrefs, pchHeadshotBorderFrame);
	pDialog->headshot.headshotEnt = contactEntRef;
	if (SAFE_MEMBER(pCostumePrefs, pCostumeStyleOverrides))
		contactdialog_CopyStyleOverrides(&pDialog->headshot.pStyleOverride, pCostumePrefs->pCostumeStyleOverrides);
}

static void contactdialog_SetExtraCostumesFromPrefs(ContactDialog* pDialog, 
													Entity* pPlayerEnt, 
													EntityRef contactEntRef, 
													ContactCostume** eaCostumePrefs,
													S32 iLimit)
{
	S32 i;
	...
	for(i=eaSize(&eaCostumePrefs)-1; i>=0 && iLimit>0; --i, --iLimit) {
		ContactHeadshotData* pHeadshot = StructCreate(parse_ContactHeadshotData);
		ContactDialogCostume* pCostume = StructCreate(parse_ContactDialogCostume);

		if(contact_CostumeToHeadshotData(pPlayerEnt, eaCostumePrefs[i], &pHeadshot, NULL))
		{
			contactdialog_SetCostumeFromHeadshotData(pCostume, pHeadshot);
		}

		if (eaCostumePrefs[i]->pCostumeStyleOverrides && pCostume->pStyleOverride)
			StructCopyAll(parse_ContactCostumeStyle, eaCostumePrefs[i]->pCostumeStyleOverrides, pCostume->pStyleOverride);
		else if (eaCostumePrefs[i]->pCostumeStyleOverrides && !pCostume->pStyleOverride)
			pCostume->pStyleOverride = StructClone(parse_ContactCostumeStyle, eaCostumePrefs[i]->pCostumeStyleOverrides);
		else if (!eaCostumePrefs[i]->pCostumeStyleOverrides && pCostume->pStyleOverride)
			StructDestroySafe(parse_ContactCostumeStyle, &pCostume->pStyleOverride);

		pCostume->headshotEnt = contactEntRef;
		pCostume->pchHeadshotStyleDef = eaCostumePrefs[i]->pchHeadshotStyle;
		pCostume->pchHeadshotBorderFrame = eaCostumePrefs[i]->pchHeadshotBorderFrame;
		eaPush(&pDialog->eaExtraHeadshots, pCostume);
		StructDestroy(parse_ContactHeadshotData, pHeadshot);
	}
}
...
PlayerCostume *contact_PlayerCostumeFromContactDialog(ContactDialog *pContactDialog, Entity *pPlayer)
{
	...
	if(pContactDialog)
	{
		PlayerCostume *pCostume = pContactDialog->headshot.pHeadshotOverride ? pContactDialog->headshot.pHeadshotOverride : GET_REF(pContactDialog->headshot.hHeadshotOverride);
		if(pCostume) // Override
		{
			pPlayerCostume = pCostume;
		} 
		else if(pContactDialog->headshot.iHeadshotOverridePetID && pPlayer) // Pet
		{
			PetRelationship* pPetRelationship = SavedPet_GetPetFromContainerID(pPlayer, pContactDialog->headshot.iHeadshotOverridePetID, true);
			if(pPetRelationship) 
			{
		...
		else if (pContactDialog->headshot.headshotEnt) // Headshot Entity
		{
			// No override so use the entity's costume
			Entity *pEnt = entFromEntityRef(entGetPartitionIdx(pPlayer), pContactDialog->headshot.headshotEnt);
			if(pEnt)
			{
...
{	
	...
	if(pPlayerEnt && !characterIsTransferring(pPlayerEnt) && pDialog) 
	{
		...
		ContactLogEntry *pLog = NULL;
		char* estrChatMessage = NULL;
		char* estrDispName = NULL;

		if (pDialog->pchContactDispName && *pDialog->pchContactDispName)
		{
			estrStackCreate(&estrDispName);
			StringStripTagsPrettyPrint(pDialog->pchContactDispName, &estrDispName);
		}

		pFrom = estrDispName ? ChatCommon_CreateUserInfoFromNameOrHandle(estrDispName) : NULL;
		pLog = StructCreate(parse_ContactLogEntry);
		...
		if (IS_HANDLE_ACTIVE(pDialog->headshot.hHeadshotOverride))
			COPY_HANDLE(pLog->hHeadshotCostumeRef, pDialog->headshot.hHeadshotOverride);
		if (pDialog->headshot.pHeadshotOverride)
			pLog->pHeadshotCostume = StructClone(parse_PlayerCostume, pDialog->headshot.pHeadshotOverride);
		if (pDialog->headshot.headshotEnt)
			pLog->erHeadshotEntity = pDialog->headshot.headshotEnt;
		if (pDialog->headshot.iHeadshotOverridePetID)
			pLog->iHeadshotPetID = pDialog->headshot.iHeadshotOverridePetID;
...
static void contact_ChangeDialogState(Entity* pPlayerEnt, EntityRef contactEnt, ContactDef *pContactDef, const ContactDialogOptionData* pOption)
{
	...
	if(pOption && pOption->uCooldownExpireTime && pOption->uCooldownExpireTime > ShardClock_SecondsSince2000())
	{
		return;
	}
	...
	pDialog->headshot.pchHeadshotStyleDef = NULL;
	pDialog->headshot.pchHeadshotBorderFrame = NULL;
	StructDestroySafe(parse_ContactCostumeStyle, &pDialog->headshot.pStyleOverride);

	// Generate Costume
	if(pContactDef) {
		contactdialog_SetCostumeFromPrefs(pDialog, pPlayerEnt, contactEnt, &pContactDef->costumePrefs);
	} else if (pOption->pHeadshotData) {
		contactdialog_SetCostumeFromHeadshotData(&pDialog->headshot, pOption->pHeadshotData);
		pDialog->headshot.pchHeadshotStyleDef = allocAddString(pOption->pHeadshotData->pchHeadshotStyleDef);
		pDialog->headshot.pchHeadshotBorderFrame = NULL;
	}
	if(!contact_PlayerCostumeFromContactDialog(pDialog, pPlayerEnt) && pCostumeFallback) {
		COPY_HANDLE(pDialog->headshot.hHeadshotOverride, pCostumeFallback->hCostume);
	}

	// Remove extra costumes
	eaDestroyStruct(&pDialog->eaExtraHeadshots, parse_ContactDialogCostume);
	...
	if(pDialog && newState && pOption)
	{
		...
		xcase ContactDialogState_SpecialDialog:			
			...
			if (pSpecialDialog){
				...
				// Display Name
				if (pPlayerEnt && GET_REF(pSpecialDialog->displayNameMesg.hMessage) && gConf.bUseLegacyContactNameOverrideBehavior) {
				...
				// Extra costumes
				if (eaSize(&pSpecialDialog->extraCostumes)) {
					contactdialog_SetExtraCostumesFromPrefs(pDialog, pPlayerEnt, contactEnt, pSpecialDialog->extraCostumes, eaSize(&pSpecialDialog->extraCostumes));
				}
				...
				langFormatGameDisplayMessage(entGetLanguage(pPlayerEnt), &pDialog->pchDialogText1, &pDialogBlock->displayTextMesg.msg, STRFMT_ENTITY(pPlayerEnt), STRFMT_ENTITY_KEY("Nemesis", pNemesisEnt), STRFMT_MAPVARS(eaMapVars), STRFMT_END);

				if (gConf.bNoAudioOnPetContactsWithoutPet)
				{
					// Pet Contact List
					PetContactList* pList = GET_REF(pContactDef->costumePrefs.hPetOverride);
					Entity* pPet = NULL;

					// If there is a pet contact list, try to find an appropriate pet in order to determine if we should be playing audio
					if (pList)
					{
						PetContactList_GetPetOrCostume(pPlayerEnt, pList, NULL, &pPet, NULL, NULL, &bPlayAudio);
					}
				}

				if (bPlayAudio)
				{
					pDialog->pchSoundToPlay = gslVOGetAudioEvent( &pDialogBlock->displayTextMesg );
				}
		...
		xcase ContactDialogState_BridgeOfficerOfferSelfOrTraining:
		{
			...
			if (pCostume)
			{
				...
				{
					SET_HANDLE_FROM_STRING("PlayerCostume",(char*)pCostume->pcName,pDialog->headshot.hHeadshotOverride);
				}
				else
				{
					pDialog->headshot.pHeadshotOverride = StructClone(parse_PlayerCostume, pCostume);
				}
			}
			...
			{
				pDialog->headshot.pchHeadshotStyleDef = allocAddString(g_PetRestrictions.pchPotentialPetHeadshotStyle);
			}
		...
		xcase ContactDialogState_ViewOfferedNamespaceMission:
			{
				...
				if (pData)
				{
					...
					PetContactList_GetPetOrCostume(pPlayerEnt, pList, NULL, &pPet, NULL, &pCritterCostume, NULL);
					
					if (pPet && pPet->myContainerID!=0)
					{
						// Pet found
						pDialog->headshot.iHeadshotOverridePetID = pPet->myContainerID;
					}
					else if (pCritterCostume!=NULL)
					{
						// No pet found, use default critter costume if we found one
						COPY_HANDLE(pDialog->headshot.hHeadshotOverride, pCritterCostume->hCostumeRef);
					}
					else
					{
						COPY_HANDLE(pDialog->headshot.hHeadshotOverride, pData->hContactCostume);
					}
		...
		xcase ContactDialogState_ViewOfferedMission:
			{
				...
				if (pMissionDef)
				{
					...
					const char *pchSoundOnContactOffer;
					MissionDisplayOverride* pOverride;

					// Set the mission UI type information
					contact_FillDialogMissionUITypeInfo(pPlayerEnt, pMissionDef, pDialog);

					pOverride = missiondef_GetApplicableMissionOverride( pMissionDef, pPlayerEnt );
					if( pOverride ) {
						detailStringMsg = pOverride->detailStringMsg.msg;
						summaryMsg = pOverride->summaryMsg;
						pchSoundOnContactOffer = gslVOGetAudioEvent( &pOverride->detailStringMsg );
					} else {
						detailStringMsg = pMissionDef->detailStringMsg.msg;
						summaryMsg = pMissionDef->summaryMsg;
						pchSoundOnContactOffer = gslVOGetAudioEvent( &pMissionDef->detailStringMsg );
					}
					...
					if (GET_REF(detailStringMsg.hMessage)) {
						if(gConf.bSwapMissionOfferDialogText) {
							langFormatGameDisplayMessage(entGetLanguage(pPlayerEnt), &pDialog->pchDialogText2, &detailStringMsg, STRFMT_ENTITY(pPlayerEnt), STRFMT_ENTITY_KEY("Nemesis", pNemesisEnt), STRFMT_MAPVARS(eaMapVars), STRFMT_END);
						} else {
							langFormatGameDisplayMessage(entGetLanguage(pPlayerEnt), &pDialog->pchDialogText1, &detailStringMsg, STRFMT_ENTITY(pPlayerEnt), STRFMT_ENTITY_KEY("Nemesis", pNemesisEnt), STRFMT_MAPVARS(eaMapVars), STRFMT_END);
						}
					}
					...
					if (GET_REF(summaryMsg.hMessage)) {
						if(gConf.bSwapMissionOfferDialogText) {
							langFormatGameDisplayMessage(entGetLanguage(pPlayerEnt), &pDialog->pchDialogText1, &summaryMsg, STRFMT_ENTITY(pPlayerEnt), STRFMT_ENTITY_KEY("Nemesis", pNemesisEnt), STRFMT_MAPVARS(eaMapVars), STRFMT_END);
						} else {
							langFormatGameDisplayMessage(entGetLanguage(pPlayerEnt), &pDialog->pchDialogText2, &summaryMsg, STRFMT_ENTITY(pPlayerEnt), STRFMT_ENTITY_KEY("Nemesis", pNemesisEnt), STRFMT_MAPVARS(eaMapVars), STRFMT_END);
						}
					}
					...
					// Add sound
					pDialog->pchSoundToPlay = allocAddString(pchSoundOnContactOffer);
				}
		...
		xcase ContactDialogState_ViewFailedMission:
			{
				...
				if (pMissionDef)
				{
					...
					if(pMissionOffer){
						...
						missiondef_GetSummaryAndDetails(pMissionDef, pPlayerEnt, &summaryMsg, &detailStringMsg);
		...
		xcase ContactDialogState_ViewSubMission:
			{
				...
				if (pMission)
				{
					...
					bool bMissionQualifiesForUGCReward = bUGCProjectInPreviewMode ? !gConf.bNoUGCRewardsInPreview : (pRootMission->pUGCMissionData ? pRootMission->pUGCMissionData->bStatsQualifyForUGCRewards : false);
					int iLevel = (bUGCProjectInPreviewMode || bUGCProjectInLiveMode) ? entity_GetSavedExpLevel(pPlayerEnt) : pMission->parent->iLevel;
					int iTimeLevel = bUGCProjectInPreviewMode ? 15 : ((bMissionQualifiesForUGCReward && bUGCProjectInLiveMode) ? ugcMissionData_GetPlayingTimeForRewards(pRootMission->pUGCMissionData) : 0);
			...
			pDialog->screenType = ContactScreenType_Buttons;
			{
				...
				if (pOption){
					...
					if (pSearchContact && GET_REF(pSearchContact->costumePrefs.costumeOverride)){
						COPY_HANDLE(pDialog->headshot.hHeadshotOverride, pSearchContact->costumePrefs.costumeOverride);
					}
				}
				if (pSearchContact){
		...
		xcase ContactDialogState_ItemAssignments:
			...
			pNewOption->bIsDefaultBackOption = true;
		xcase ContactDialogState_ZoneRewards:
			pDialog->screenType = ContactScreenType_ZoneRewardsInfo;
			COPY_HANDLE(pDialog->hStickerBookCollection, pContactDef->hStickerBookCollection);
			pNewOption = contactdialog_CreateDialogOption(pPlayerEnt, &pDialog->eaOptions, "ZoneRewards.Back", GET_REF(s_ContactMsgs.hBackMsg), ContactDialogState_OptionListFarewell, 0, eaMapVars);
			pNewOption->bIsDefaultBackOption = true;
...
static void contact_PlayEndDialogAudio(Entity *pEnt, InteractInfo *pInteractInfo, ContactDef *pContactDef)
{
	if (pEnt && pInteractInfo && eaSize(&pContactDef->eaEndDialogAudios))
	{
		...
		if (pEndDialogAudio && pEndDialogAudio->pchAudioName && pEndDialogAudio->pchAudioName[0])
		{
			...
			EntityRef entRef = pInteractInfo->pContactDialog->headshot.headshotEnt;
...
static void contactdialog_CompleteSubMission(SA_PARAM_NN_VALID Entity* playerEnt, SA_PARAM_NN_VALID ContactDialogOptionData* pData, SA_PARAM_OP_VALID ContactRewardChoices* rewardChoices)
{
	MissionDef* missionDef = GET_REF(pData->hRootMission);
	MissionInfo* pMissionInfo = mission_GetInfoFromPlayer(playerEnt);
	...
	if(missionDef && pMissionInfo)
	{
		...
		if(mission && mission->state == MissionState_InProgress){
			mission_CompleteMission(playerEnt, mission, false, rewardChoices);
		}
	}
}
	...
	if (pEnt && pResponse)
	{
		switch(pResponse->pData->action)
		{
			...
			xcase ContactActionType_CompleteSubMission:
				contactdialog_CompleteSubMission(pEnt, pResponse->pData, pRewardChoices);
				...
				if (whichIndicator != ContactIndicator_NoInfo && !contactInfo->calledOutToPlayer)
				{
					...
					if (distance3Squared(playerPos, contactPos) < CONTACT_CALLOUT_DIST_SQ)
					{
						...
						if (dialogToUse)
						{
							...
							mapvariable_GetAllAsWorldVarsNoCopy(iPartitionIdx, &eaMapVars);
							langFormatGameMessage(entGetLanguage(playerEnt), &displayText, GET_REF(dialogToUse->displayTextMesg.msg.hMessage), STRFMT_ENTITY_KEY("Entity", playerEnt), STRFMT_MAPVARS(eaMapVars), STRFMT_END);
							if (displayText)
								ClientCmd_MissionCritterSpeak(playerEnt, pInfo, displayText);
							if (audioName = gslVOGetAudioEvent(&dialogToUse->displayTextMesg))
								ClientCmd_sndPlayRemote3dV2(playerEnt, audioName, contactPos[0], contactPos[1], contactPos[2], "Contact code", -1);
