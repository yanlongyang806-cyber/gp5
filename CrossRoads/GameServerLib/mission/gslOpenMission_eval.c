			...
			if (pTargetEnt){
				...
				if (pRewardDropPoint) {
					...
					for(j = 0; j < eaSize(&eaMissions); j++) {
						...
						const char *pchRewardMessage = NULL;
						...
						if (pRewardMissionDef) {
							switch(iTable) {
								case 0:
									...
									pchRewardMessage = pDef->params?TranslateDisplayMessage(bFailure ? pRewardMissionDef->params->RewardMsgFailureGold : pRewardMissionDef->params->RewardMsgGold):NULL;
								xcase 1:
									...
									pchRewardMessage = pDef->params?TranslateDisplayMessage(bFailure ? pRewardMissionDef->params->RewardMsgFailureSilver : pRewardMissionDef->params->RewardMsgSilver):NULL;
								xcase 2:
									...
									pchRewardMessage = pDef->params?TranslateDisplayMessage(bFailure ? pRewardMissionDef->params->RewardMsgFailureBronze : pRewardMissionDef->params->RewardMsgBronze):NULL;
									break;
								default:
									...
									pchRewardMessage = pDef->params?TranslateDisplayMessage(bFailure ? pRewardMissionDef->params->RewardMsgFailureDefault : pRewardMissionDef->params->RewardMsgDefault):NULL;
									break;
							}
							if(!pchRewardMessage || !*pchRewardMessage) {
								pchRewardMessage = pDef->params?TranslateDisplayMessage(bFailure ? pRewardMissionDef->params->RewardMsgFailureDefault : pRewardMissionDef->params->RewardMsgDefault):NULL;
							}
							if (pTable) {
								ItemChangeReason reason = {0};
								inv_FillItemChangeReason(&reason, pTargetEnt, "Mission:OpenMissionReward", pRewardMissionDef->name);
								reward_OpenMissionExec(iPartitionIdx, pTargetEnt, pDef, pTable, iLevel, fScale, pRewardDropPoint[3], &reason, pchRewardMessage);
							}
						}
					}
				}
					...
					for(j = 0; j < eaSize(&eaMissions); j++) {
						...
						if (pRewardMissionDef) {
							...
							if (pTable) {
								ItemChangeReason reason = {0};
								inv_FillItemChangeReason(&reason, pTargetEnt, "Mission:OpenMissionReward", pRewardMissionDef->name);
								reward_OpenMissionExec(iPartitionIdx, pTargetEnt, pDef, pTable, iLevel, fScale, v3Origin, &reason, pchRewardMessage);
...
void openmission_FuncOpenMissionGrantRewardsNonRecursiveEx(ExprContext *pContext, ACMD_EXPR_PARTITION iPartitionIdx, const char *pcMissionName, ACMD_EXPR_LOC_MAT4_IN pRewardDropPoint, bool bFailure)
{
	...
	if (pDef && pOpenMission) {
		...
		const char *pchGoldMessage = pDef->params?TranslateDisplayMessage(bFailure ? pDef->params->RewardMsgFailureGold : pDef->params->RewardMsgGold):NULL;
		const char *pchSilverMessage = pDef->params?TranslateDisplayMessage(bFailure ? pDef->params->RewardMsgFailureSilver : pDef->params->RewardMsgSilver):NULL;
		const char *pchBronzeMessage = pDef->params?TranslateDisplayMessage(bFailure ? pDef->params->RewardMsgFailureBronze : pDef->params->RewardMsgBronze):NULL;
		const char *pchDefaultMessage = pDef->params?TranslateDisplayMessage(bFailure ? pDef->params->RewardMsgFailureDefault : pDef->params->RewardMsgDefault):NULL;
		...
		for(i = eaSize(&eaSortedScores) - 1; i >= 0; i--) {
			...
			if (pTargetEnt) {
				...
				if (i == 0) {
					pTable = pGoldTable;
					pchRewardMessage = pchGoldMessage && *pchGoldMessage ? pchGoldMessage : pchDefaultMessage;
					fScale *= 1.5;
				} else if (i == 1 || i < eaSize(&eaSortedScores)*.2) {
					pTable = pSilverTable;
					pchRewardMessage = pchSilverMessage && *pchSilverMessage ? pchSilverMessage : pchDefaultMessage;
					fScale *= 1.25;
				} else if (i == 2 || i < eaSize(&eaSortedScores)*.5) {
					pTable = pBronzeTable;
					pchRewardMessage = pchBronzeMessage && *pchBronzeMessage ? pchBronzeMessage : pchDefaultMessage;
					fScale *= 1.1;
				} else {
					pTable = pDefaultTable;
					pchRewardMessage = pchDefaultMessage;
				}
