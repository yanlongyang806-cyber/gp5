	...
	if (pMissionInfo) {
		...
		if (pMission) {
			...
			if (!pMissionDef)
			{
				...
				//   per Jared Finder.
				AssertOrAlertWarning("MISSION_DEF_MISSING", "Mission %s had no MissionDef when a request to DropMission happened", pcMissionName);
			}
			else
			{
								...
								bool bRootMission, CmdContext *pContext)
{
	if (pContext && pContext->clientType == GLOBALTYPE_ENTITYPLAYER) {
		...
		// Send notifications
		if (pDef && pDef->bSendCompletionNotify && bRootMission && missiondef_HasDisplayName(pDef) && missiondef_GetType(pDef) != MissionType_Perk) {
		...
		if (pDef && pDef->bSendCompletionNotify && !bRootMission && eNewState == MissionState_Succeeded)
		{
...
void mission_CmdMissionComplete(Entity *pPlayerEnt, ACMD_NAMELIST("AllMissionsIndex", RESOURCEDICTIONARY) char *pcMissionName)
{
	...
	if (pPlayerEnt && (pInfo = mission_GetInfoFromPlayer(pPlayerEnt))) {
		...
		if (pMission) {
			mission_CompleteMission(pPlayerEnt, pMission, true, NULL);
		}
	}
}
		...
		while (iNumTries++ < 10000){
			...
			for (i = eaSize(&pInfo->missions)-1; i >= 0; --i) {
				...
				if (pDef && pDef->missionType == MissionType_Perk && pInfo->missions[i]->state != MissionState_Succeeded) {
					mission_CompleteMission(pPlayerEnt, pInfo->missions[i], true, NULL);
					bPerkFound = true;
					break;
				}
			}
			if (!bPerkFound) {
				for (i = eaSize(&pInfo->eaNonPersistedMissions)-1; i >= 0; --i) {
					...
					if (pDef && pDef->missionType == MissionType_Perk && pInfo->eaNonPersistedMissions[i]->state != MissionState_Succeeded) {
						mission_CompleteMission(pPlayerEnt, pInfo->eaNonPersistedMissions[i], true, NULL);
						bPerkFound = true;
						break;
					}
				}
			...
			if (!bPerkFound) {
				for (i = eaSize(&pInfo->eaDiscoveredMissions)-1; i >= 0; --i) {
					...
					if (pDef && pDef->missionType == MissionType_Perk && pInfo->eaDiscoveredMissions[i]->state != MissionState_Succeeded) {
						mission_CompleteMission(pPlayerEnt, pInfo->eaDiscoveredMissions[i], true, NULL);
						bPerkFound = true;
						break;
					}
				}
		...
		for(j=0; j<k; j++) {
			...
			if (GET_REF(pOffer->missionDef) && (0 == stricmp(pDef->name, GET_REF(pOffer->missionDef)->name)) && (pOffer->allowGrantOrReturn != ContactMissionAllow_ReturnOnly)) {
				...
				for (iDialog = 0; iDialog < iNumDialogs; iDialog++) {
					...
					if ((pDisplayText = GET_REF(pOffer->offerDialog[iDialog]->displayTextMesg.msg.hMessage)) && pDisplayText->pcDefaultString && pDisplayText->pcDefaultString[0])
					{
...
void mission_CmdRequestCritterDataForLoreJournal(Entity *pPlayerEnt, RequestedCritterAttribs *pAttribs)
{
	...
	BagIterator *pBagIter = bagiterator_Create();
	bagiterator_SetLiteBagByID(pPlayerEnt, InvBagIDs_Lore, pBagIter, pExtract);
...
static MissionOfferOverride *mission_FindBestMissionOfferOverride(Entity *pPlayerEnt, bool bGrant, bool bAlreadyCompleted, MissionOfferOverride **eaOverrides, bool *pbRemote)
{
	...
	for (i = 0; i < eaSize(&eaOverrides); i++)
	{
		...
		if (!contact_CheckPlayerAllegiance(pOverride->pMissionOffer, pPlayerEnt))
			continue;
			...
			if (pDef)
			{
				...
				if (pInfo && !pData->bAvailable && eMissionOfferStatus == MissionOfferStatus_SecondaryCooldown)
				{
					...
					if (pCompletedSecondary) {
						U32 uCurrentTime = ShardClock_SecondsSince2000();
...
char *s_estrMissionKillList = NULL;
AUTO_CMD_ESTRING(s_estrMissionKillList, MissionKillList) ACMD_AUTO_SETTING(Misc, GAMESERVER) ACMD_CALLBACK(mission_UpdateMissionKillList);

char** g_eaMissionKillList = NULL;

void mission_UpdateMissionKillList(CMDARGS)
{
	if (g_eaMissionKillList)
	{
		eaDestroyEString(&g_eaMissionKillList);
		g_eaMissionKillList = NULL;
	}

	if (s_estrMissionKillList && s_estrMissionKillList[0])
	{
		estrTokenize(&g_eaMissionKillList, ",", s_estrMissionKillList);
	}
}

AUTO_COMMAND ACMD_NAME(GiveAccountGatedReward) ACMD_ACCESSLEVEL(9) ACMD_CATEGORY(Debug);
bool mission_CmdGiveAccountGatedReward(Entity *pEnt,
	ACMD_NAMELIST("RewardTable", REFDICTIONARY) const char *pchRewardTable,
	ACMD_NAMELIST(RewardGatedTypeEnum, STATICDEFINE) const char* pchGateType)
{
	return reward_GivePendingAccountReward(pEnt, pchRewardTable, pchGateType);
}
