			...
			case MissionWaypointType_AreaVolume:
				{
					...
					F32 fRot;
					U32 uVolumeShape;
					if (volume_GetVolumeData(entGetPartitionIdx(pEnt), pWaypoint->name, NULL, vVolCenter, vVolPos, vVolLocalMin, vVolLocalMax, &fRot, &uVolumeShape)) {
						pWPData = StructCreate(parse_MinimapWaypoint);

						copyVec3(vVolCenter, pWPData->pos);
						pWPData->fRotation = fRot;
						pWPData->eShape = uVolumeShape;
			...
			FOR_EACH_IN_EARRAY_FORWARDS(pMissionDef->eaWaypoints, MissionWaypoint, pWaypoint) {
				if (pWaypoint)
				{
					...
					else if(bPerformDoorSearch)
					{
						...
						if( !bWaypointCreated && pWaypoint->astrAlternateMapName ) {
							bWaypointCreated |= waypoint_GenerateDoorWaypointsToMap(pInfo->parentEnt, pWaypoint->astrAlternateMapName, g_ppDoorList, pMissionDef, peaWaypointList);
						}
					}
				}
			}
...
void waypoint_UpdateLandmarkWaypoints(Entity *pPlayerEnt)
{
	...
	if (pInfo) {
		...
		for (i = n-1; i >= 0; --i) {
			if (pInfo->waypointList[i]->type == MinimapWaypointType_Landmark || pInfo->waypointList[i]->type == MinimapWaypointType_AreaLandmark) {
		...
		for (i = 0; i < n; i++) {
			...
			if (landmark_GetCenterPoint(eaLandmarks[i], vCenterPos)) {
				...
				if (pLandmarkRegion == pPlayerRegion) {
					...
					WorldVolumeElement *pElement = NULL;
					pWaypoint->type = eaLandmarks[i]->bScaleToArea ? MinimapWaypointType_AreaLandmark : MinimapWaypointType_Landmark;
					...
					pWaypoint->iZOrder = eaLandmarks[i]->iZOrder;

					if (pWaypoint->type == MinimapWaypointType_AreaLandmark) {
						pWaypoint->fRotation = eaLandmarks[i]->fRotation;
						pWaypoint->fXAxisRadius = eaLandmarks[i]->fXAxisRadius;
						pWaypoint->fYAxisRadius = eaLandmarks[i]->fYAxisRadius;
					}

					if (eaLandmarks[i]->pEntry)
						pElement = eaGet(&eaLandmarks[i]->pEntry->elements, 0);
					if (pElement)
						pWaypoint->eShape = pElement->volume_shape;

					if (!exprIsEmpty(eaLandmarks[i]->pVisibleExpr)) {
						pWaypoint->bHidden = !(volume_EvaluateExpr(entGetPartitionIdx(pPlayerEnt), volume_GetByEntry(eaLandmarks[i]->pEntry), pPlayerEnt, eaLandmarks[i]->pVisibleExpr));
					}
...
void waypoint_FlagWaypointRefreshPlayers(int iPartitionIdx)
{
...
AUTO_EXPR_FUNC(gameutil) ACMD_NAME("RefreshPlayerWaypoints");
void waypointexpr_FlagWaypointRefreshPartition(ACMD_EXPR_PARTITION iPartitionIdx)
{
