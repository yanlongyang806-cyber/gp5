...
static EventLoggingCategoryDef sLogCategoryDefs[] = 
{
	...
	{
		EventType_PowerActivation,
		LOG_OTHER_PLAYER_EVENTS,
		LOGGROUP_GAMEPLAYEVENT,
	},
	{
		EventType_PowerActivationEnded,
		LOG_OTHER_PLAYER_EVENTS,
		LOGGROUP_GAMEPLAYEVENT,
	},

	{
		EventType_PvPPlayerPointsAwarded,
			LOG_OTHER_PLAYER_EVENTS,
			LOGGROUP_PVP,
	},
	{
...
static bool eventtracker_MatchEvents(GameEvent *pListeningEvent, GameEvent *pSentEvent, bool bSimpleSourceTarget)
{
	...
	if (pSentEvent->pchPowerName && (pListeningEvent->pchPowerTagName || pListeningEvent->pchPowerCategoryName))
	{
		PowerDef *pPowerDef = powerdef_Find(pSentEvent->pchPowerName);
		if (!pPowerDef)
			return false;

		if (pListeningEvent->pchPowerTagName)
		{
			S32 eTag = StaticDefineIntGetInt(PowerTagsEnum, pListeningEvent->pchPowerTagName);
			if (eTag < 0)
				return false;

			if (eaiFind(&pPowerDef->tags.piTags, eTag) < 0)
				return false;
		}

		if (pListeningEvent->pchPowerCategoryName)
		{
			S32 eCategory = StaticDefineIntGetInt(PowerCategoriesEnum, pListeningEvent->pchPowerCategoryName);
			if (eCategory < 0)
				return false;

			if (eaiFind(&pPowerDef->piCategories, eCategory) < 0)
				return false;
		}
	}
...
void eventtracker_InitLogCategories(void)
{
	...
	for (i=0; i < ARRAY_SIZE(sLogCategoryDefs); i++) {
		...
		assertmsgf(sLogCategoryDefs[i].eEventType == i, "sLogCategoryDef array must be in same order as EventTypeEnum.  Index %d is not in proper order!", i);
