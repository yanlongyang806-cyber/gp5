...
void eventsend_RecordPowerActivation(Entity *pSourceEnt, Entity *pTargetEnt, PowerDef *pPowerDef)
{
	int iPartitionIdx;
	...
	s_event.type = EventType_PowerActivation;
	s_event.iPartitionIdx = iPartitionIdx;
	s_event.pchPowerName = pPowerDef->pchName;
...
void eventsend_RecordPowerActivationEnded(Entity *pSourceEnt, Entity *pTargetEnt, PowerDef *pPowerDef)
{
	int iPartitionIdx;
	...
	s_event.type = EventType_PowerActivationEnded;
	s_event.iPartitionIdx = iPartitionIdx;
	s_event.pchPowerName = pPowerDef->pchName;
...
#define eventsend_AddInteractableVolume(pEvent, pVolume)\
	...
	if (SAFE_MEMBER2(pVolume, pNamedVolume, common_data.parent_group))\
		pEvent.eaClickableGroupScopeNames = worldEncObjGetScopeNames((WorldEncounterObject*) pVolume->pNamedVolume->common_data.parent_group);\
...
// ----------------------------------------------------------------------------------
// Sending PvP Player Points Events
// ----------------------------------------------------------------------------------
void eventsend_RecordPvPPlayerPointsAwarded(int iPartitionIdx, Entity *pEnt, S32 iCount)
{
	PERFINFO_AUTO_START_FUNC();

	eventsend_ClearEvent(&s_event);

	s_event.type = EventType_PvPPlayerPointsAwarded;
	s_event.iPartitionIdx = iPartitionIdx;
