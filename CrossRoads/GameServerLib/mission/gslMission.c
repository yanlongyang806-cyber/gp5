...
extern char** g_eaMissionKillList;

static bool mission_MissionIsOnKillList(MissionDef *pDef)
{
	S32 i;

	if (!g_eaMissionKillList)
	{
		return false;
	}

	for (i = 0; i < eaSize(&g_eaMissionKillList); ++i)
	{
		if (allocFindString(g_eaMissionKillList[i]) == pDef->name)
		{
			return true;
		}
	}
...
bool missiondef_CanBeOfferedAtAll(Entity *pPlayerEnt, MissionDef *pMissionDef, int *piNextOfferLevel, MissionOfferStatus *pOfferStatus, MissionCreditType *pCreditType)
{
	...
	//The mission is on the mission kill list
	if (mission_MissionIsOnKillList(pMissionDef))
	{
		return false;
	}
	...
	if (mission_HasCooldownProperties(pMissionDef))
	{
	...
	if (eCreditType != MissionCreditType_Primary) {
		...
		if (pCompletedSecondary && (pCompletedSecondary->completedTime + missiondef_GetSecondaryCreditLockoutTime(pMissionDef) > ShardClock_SecondsSince2000())) {
			if (pOfferStatus) {
...
static bool mission_RequestNamespaceDef(Entity *pEnt, Mission *pMission)
{
	...
	{	
		...
		RemoteCommand_aslMapManager_IsNameSpacePlayable(pReturnVal, GLOBALTYPE_MAPMANAGER, 0, nameSpace, buffer, bTransferedRecently);
...
bool mission_VerifyEntityMissionData(Entity *pEnt)
{
	...
	if (pMissionInfo) {
		...
		for (i = n-1; i >= 0; --i) {
			...
			else if (pDef && mission_MissionIsOnKillList(pDef))
			{
				entLog(LOG_GSL, pEnt, "Mission-MissionKilled", "Force-dropping mission because it is in the global mission kill list. Mission '%s' will be removed", pMission->missionNameOrig);
				missioninfo_DropMission(pEnt, pMissionInfo, pMission);
				return false;
			}
...
{
	if (pMission) {
		return ShardClock_SecondsSince2000() - mission_GetStartTime(pMission);
	}
	return 0;
}
...
bool mission_HasExpiredOnStartTimersRecursive(Mission *pMission, MissionDef *pDef, MissionDef *pRootDef)
{
	if (pMission && pDef && pRootDef) {
		int i;
		if (pDef->uTimeout && mission_GetEndTime(pDef, pMission) <= ShardClock_SecondsSince2000()) {
			return true;
		}
		for (i = 0; i < eaSize(&pDef->ppOnStartActions); i++) {
...
{
	int iTimeLeft = (int)mission_GetEndTime(pDef, pMission) - (int)ShardClock_SecondsSince2000();
...
bool mission_UpdateState(int iPartitionIdx, Entity *pEnt, Mission *pMission)
{
	...
	if(!pDef->meSuccessCond) {
		...
		if (bSuccess && pMission->state == MissionState_InProgress) {
			if (pMission->infoOwner && pMission->infoOwner->parentEnt) {
				mission_CompleteMission(pMission->infoOwner->parentEnt, pMission, false, NULL);
			} else {
				mission_tr_CompleteMission(iPartitionIdx, pMission, false, NULL);
			}
			PERFINFO_AUTO_STOP();
			return true;
...
void mission_CompleteCurrentPhase(Entity *pPlayerEnt, Mission *pMission)
{
	...
	if (pCurMission) {
		mission_CompleteMission(pPlayerEnt, pCurMission, true, NULL);
	}
}
...
void mission_AddNonPersistedSubMission(int iPartitionIdx, Mission *pParentMission, const MissionDef *pMissionDef, bool bSkipActions)
{
	...
	if (pParentMission && (pNewMission = (Mission*)mission_CreateMission(iPartitionIdx, pMissionDef, 0, 0))) {
		...
		NOCONSTMISSION(pNewMission)->startTime = ShardClock_SecondsSince2000();
...
void mission_PlayerEnteredMap(Entity *pPlayerEnt)
{
	...
	mechanics_PlayerEnteredMap(pPlayerEnt);
...
void mission_CompleteMission(Entity *pEnt, Mission *pMission, bool bForcePermanentComplete, ContactRewardChoices *pRewardChoices)
{
	...
	mission_tr_CompleteMission(iPartitionIdx, pMission, bForcePermanentComplete, pRewardChoices);
}
...
bool Mission_IsPrimaryMissionValid(Entity *pEnt, MissionInfo *pInfo)
{
	if(pEnt)
	{
		Mission *pMission;

		if(entGetPrimaryMission(pEnt) == NULL)
		{
			return false;
		}

		pMission = mission_GetMissionByName(pInfo, entGetPrimaryMission(pEnt));
		if(pMission && (pMission->state == MissionState_InProgress || pMission->state == MissionState_Succeeded))
		{
			return true;
		}
	}
...
static void mission_ProcessEntityMissions(int iPartitionIdx, Entity *pEnt, MissionInfo *pInfo)
{
	...
	// Check to see if the primary mission needs to be set
	if(g_MissionConfig.bAutomaticMakePrimary && !team_IsMember(pEnt) && !Mission_IsPrimaryMissionValid(pEnt, pInfo))
	{
		S32 i;
		for(i = 0; i < eaSize(&pInfo->missions); ++i)
		{
			MissionDef *pDef = mission_GetDef(pInfo->missions[i]);
			if(pDef && pInfo->missions[i]->state == MissionState_InProgress && missiondef_CountsTowardsMaxActive(pDef) && pDef->name)
			{
				mission_SetPrimaryMission(pEnt, pDef->name);
				break;
			}
		}
	}
