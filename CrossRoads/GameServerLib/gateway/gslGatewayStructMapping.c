...
//
void WriteMappedStructJSON(char **pestr, const char *pchDict, const char *pchKey, Language lang)
{
	void *pobj;
	ParseTable *tpi = NULL;
	...
	if(tpi && pobj)
	{
		...
		{
			pobj = pmap->pfnConvert(pmap, pobj, lang);
		}
...
void WriteEmptyMappedStructJSON(char **pestr, const char *pchDict)
{
	...
	if(tpiEmpty && tpiDest)
	{
		...
		if(pmap && pmap->pfnConvert)
		{
			pobj = pmap->pfnConvert(pmap, pEmpty, 0);
		}
...
void *structmap_Message(StructMapping *pmap, void *pvSrc, Language lang)
{
	...
	estrCopy2(&pdest->str, langTranslateMessage(lang, (Message *)psrc));
