...
#include "gslAccountProxy.h" // for gslWebRequestTick()
#include "gslCurrencyExchange.h" // for gslCurrencyExchange_OncePerFrame()
...
void gslGatewayServer_OncePerFrame(void)
{
	...
	wgsFreeDestroyedSessions();
	session_FreeReleasedContainers();
...
/***********************************************************************
 * gslGatewayServer_GameAccountDataModified
 *
 */
void gslGatewayServer_GameAccountDataModified(GameAccountData *pGameAccountdData)
{
	GatewaySession *psess;
	...
	if(psess)
	{
		session_GameAccountDataModified(psess);

		// The most likely reason for a GAD update is a change to Zen, which
		//   happens when buying stuff. So, we'll pre-emptively do a MT update
		//   so that the response time is quicker
		// No major harm is done if this isn't done here, but the client will
		//   potentially update zen quantities 5 seconds before the success
		//   message is received which can be disconcerting to users.
		wgsMicrotransactionUpdate();
