...
void GatewayAuction_RequestCancel(Entity *pEnt, char *pchTrackerID, ContainerID auctionLotID)
{
	...
	if(pLot && pLot->ownerID == entGetContainerID(pEnt) && Auction_CanBeCanceled(pLot))
	{
		...
		if (gslAuction_ValidatePermissions(pEnt, pLot, &eaiContainerItemPets, "Auction.AuctionCancelledFail", "Auction.AuctionCancelledFail"))
		{
			...
			AutoTrans_auction_tr_CancelAuction(LoggedTransactions_CreateManagedReturnValEnt("Auction", pEnt, gslGateway_Auction_CancelRequest_CB, pStruct),
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_NAME(GatewayAuction_RequestPurchase) ACMD_LIST(gGatewayCmdList);
void GatewayAuction_RequestPurchase(Entity *pEnt, char *pchTrackerID, ContainerID auctionLotID)
{
	GatewaySession *psess = wgsFindSessionForAccountId(entGetAccountID(pEnt));
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_NAME(GatewayAuction_RequestBid) ACMD_LIST(gGatewayCmdList);
void GatewayAuction_RequestBid(Entity *pEnt, char *pchTrackerID, ContainerID auctionLotID, U32 iBidValue)
{
	GatewaySession *psess = wgsFindSessionForAccountId(entGetAccountID(pEnt));
	ContainerTracker *ptracker = session_FindContainerTracker(psess,GW_GLOBALTYPE_AUCTION_SEARCH, pchTrackerID);
	AuctionLot *pLot = NULL;
...
void gslGatewayAuction_CreateAuction(Entity *pEnt, U64 iItemID, U32 iCount, U32 iBuyoutPrice, U32 iStartingBid, U32 iAuctionDuration)
{
	...
	if (gAuctionConfig.bBiddingEnabled)
	{
		...
		{
			notify_NotifySend(pEnt, kNotifyType_AuctionFailed, TranslateMessageKey("Auction.MustSetBuyoutPriceOrStartingBid"), NULL, NULL);
			return;
		}
