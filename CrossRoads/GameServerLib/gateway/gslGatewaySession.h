...
typedef struct MappedCStore MappedCStore;
...
typedef struct MicroTransactionUserCatalogResponse MicroTransactionUserCatalogResponse;
typedef struct CurrencyExchangeAccountData CurrencyExchangeAccountData;
typedef struct GatewayGameData GatewayGameData;
typedef struct ActiveActivity ActiveActivity;
...
typedef struct ContainerTracker
{
	...
	REF_TO(GroupProjectContainer) hGroupProjectContainer;
		// (If it's a guild- or player- project) Subscription ref to group project

	REF_TO(CurrencyExchangeAccountData) hExchangeAccountData;
		// (If it's a Exchange Account Data) Subscription ref to a currency exchange account data

	REF_TO(GatewayGameData) hGatewayGameData;
		// (If it's a gateway game data) Subscription ref to a gateway game data
	...
	MicroTransactionUserCatalogResponse *pCatalog;
		// The Cstore as seen by the character

	Entity *pOfflineCopy;
		// The offline entity copy cached

	U32 u32FixupStart;
		// If non-zero, the container is being fixed up. (Used to avoid running
		//   multiple fixups on the same tracker.)
...
void wgsAllContainersModified(void);
