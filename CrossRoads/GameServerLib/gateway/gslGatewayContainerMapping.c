...
void MakeMappedContainer(GatewaySession *psess, ContainerTracker *ptracker)
{
	...
	if(pcmap)
	{
		...
		else
		{
 		...
 		{
			U64 iExtraExcludes = (ptracker->idOwnerAccount != psess->idAccount ? TOK_SELF_ONLY : 0);

			estrCopy2(&ptracker->estrDiff, "");
 			StructWriteTextDiff(&ptracker->estrDiff, pcmap->tpiDest,
				ptracker->pMapped, pmapped,
				NULL, 0, TOK_SERVER_ONLY|TOK_EDIT_ONLY|TOK_NO_NETSEND|iExtraExcludes, 
				TEXTDIFFFLAG_ANNOTATE_REFERENCES | TEXTDIFFFLAG_JSON_SECS_TO_RFC822);
 		}
