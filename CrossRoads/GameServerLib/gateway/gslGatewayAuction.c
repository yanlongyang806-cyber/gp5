...
typedef struct MappedAuctionLot
{
	...
	int iIndex;				AST(NAME(Index) DEFAULT(-1))
	...
	int bCanBid;			AST(NAME(CanBid) DEFAULT(-1))
	int iCurrentBid;		AST(NAME(CurrentBid) DEFAULT(-1))
	int iMinBid;			AST(NAME(MinimumBid) DEFAULT(-1))
	int iNumBids;			AST(NAME(NumBids) DEFAULT(-1))
...
typedef struct MappedAuctionSearch
{
	...
	AuctionLotList *pList;   AST(NAME(AuctionList) SERVER_ONLY)
...
void gslGateway_AuctionRequestSearch(Entity *pEnt, ParamsAuctionSearch *pParams)
{
	...
	pSearchRequest->minLevel = CLAMP(pParams->iMinLevel, 0, MAX_USER_LEVEL);
	pSearchRequest->maxLevel = CLAMP(pParams->iMaxLevel, 0, MAX_USER_LEVEL);
	pSearchRequest->itemQuality = CLAMP(pParams->iQuality, -1, 100);
	...
	pSearchRequest->uiNumGemSlots = CLAMP(pParams->iNumberOfSlots, -1, 100);
...
MappedAuctionSearch *CreateMappedAuctionSearch(GatewaySession *psess, ContainerTracker *ptracker, MappedAuctionSearch *psearch)
{
	...
	for(i=0;i<eaSize(&psearch->pList->ppAuctionLots);i++)
	{
		...
		if(pLot->pBiddingInfo)
		{
			...
			pMappedLot->bCanBid = 1;
		}
		else
		{
			pMappedLot->bCanBid = 0;
		}
