...
// Sets whether sending a chat message to a specific channel should change the current channel to the new channel
void ServerChatConfig_SetStickyChannels(Entity *pEntity, bool bStickyChannels) {
	NOCONST(ChatConfig) *pConfig = ServerChatConfig_GetChatConfig(pEntity);
	if (pConfig) {
