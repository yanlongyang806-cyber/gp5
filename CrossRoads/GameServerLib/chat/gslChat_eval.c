...
static const char * formatChannelName (Entity *pEnt, const char *channel_name)
{
	...
	} else if (stricmp(channel_name, CHAT_TRADE_SHORTCUT) == 0)
	{
		return getTradeChannelName(pEnt);
	} else if (stricmp(channel_name, CHAT_LFG_SHORTCUT) == 0)
	{
		return getLFGChannelName(pEnt);
	} else if (stricmp(channel_name, CHAT_GUILD_SHORTCUT) == 0)
	{
		...
		if (guildID)
			return guild_GetGuildChannelNameFromID(SAFESTR(buffer), guildID, iVirtualShardID);
	} else if (stricmp(channel_name, CHAT_GUILD_OFFICER_SHORTCUT) == 0)
	{
		int guildID = guild_GetGuildID(pEnt);
		ContainerID iVirtualShardID = partition_VirtualShardIDFromIdx(entGetPartitionIdx(pEnt));
		if (guildID)
			return guild_GetOfficerChannelNameFromID(SAFESTR(buffer), guildID, iVirtualShardID);
	}
	return channel_name;
}
...
void cmdServerChat_SendMessage(Entity *pFromEnt, SA_PARAM_NN_VALID ChatMessage *pMsg) {
	if (pMsg->pchChannel)
	{
		...
		if (stricmp(pMsg->pchChannel, "Trade") == 0 || strnicmp(pMsg->pchChannel, TRADE_CHANNEL_PREFIX, strlen(TRADE_CHANNEL_PREFIX)) == 0)
			pMsg->iInstanceIndex = partition_PublicInstanceIndexFromIdx(iPartitionIdx);
		if (stricmp(pMsg->pchChannel, "LookingForGroup") == 0 || strnicmp(pMsg->pchChannel, LFG_CHANNEL_PREFIX, strlen(LFG_CHANNEL_PREFIX)) == 0)
			pMsg->iInstanceIndex = partition_PublicInstanceIndexFromIdx(iPartitionIdx);
