...
static void gslChat_SetChatPermissionFlags(Entity *pEnt, ChatGamePermissionInfoEnum *pInfo)
{
	*pInfo = CHAT_GAME_PERMISSION_INFO_NONE;
	// full rates if not on or has token
	if(!GamePermission_EntHasToken(pEnt, GAME_PERMISSION_CHAT_MAIL_FULL_RATE))
		*pInfo |= CHAT_GAME_PERMISSION_INFO_RESTRICTED;

	if (GamePermission_EntHasToken(pEnt, GAME_PERMISSION_CAN_SEND_TELL))
		*pInfo |= CHAT_GAME_PERMISSION_INFO_TELL;

	if (GamePermission_EntHasToken(pEnt, GAME_PERMISSION_CAN_SEND_ZONE))
		*pInfo |= CHAT_GAME_PERMISSION_INFO_ZONE;

	if (GamePermission_EntHasToken(pEnt, GAME_PERMISSION_BYPASS_ZONE_RATELIMIT))
		*pInfo |= CHAT_GAME_PERMISSION_INFO_ZONE_RATELIMIT;
}
...
void gslChat_RequestChatRelayData(Entity *pEnt)
{
	if (pEnt && pEnt->pPlayer)
	{
		...
		if (authRequest.uAccountID)
		{
			...
			gslChat_SetChatPermissionFlags(pEnt, &authRequest.eGamePermissionInfo);
...
static const char *getZoneChannelNameByPartition(U32 uPartitionID)
{
	...
	ContainerID iVirtualShardID = partition_VirtualShardIDFromID(uPartitionID);
	...
	if (!sZoneChannels.mapName || stricmp(mapName, sZoneChannels.mapName))
	{
		...
		zone_GetZoneChannelNameFromMapName(sZoneChannels.baseZoneChannelName, ARRAY_SIZE_CHECKED(sZoneChannels.baseZoneChannelName), 
			sZoneChannels.shortMapName, 0, eMapType, iVirtualShardID);
	...
	if (!pZoneChannel)
	{
		...
		zone_GetZoneChannelNameFromMapName(pZoneChannel->channelName, ARRAY_SIZE_CHECKED(pZoneChannel->channelName), 
			sZoneChannels.shortMapName, uPartitionID, eMapType, iVirtualShardID);
		if (!sZoneChannels.ppZoneChannels)
...
static GameServerZoneChannels sTradeChannels = {0};
static const char *getTradeChannelNameByPartition(U32 uPartitionID)
{
	const char *mapName = zmapGetFilename(NULL);
	ZoneMapType eMapType = zmapInfoGetMapType(NULL);
	ZoneChannelName *pTradeChannel;
	ContainerID iVirtualShardID = partition_VirtualShardIDFromID(uPartitionID);

	if (!devassert(mapName))
		return NULL;

	if (!sTradeChannels.mapName || stricmp(mapName, sTradeChannels.mapName))
	{
		// Map has changed or is not initialized
		getFileNameNoExtNoDirs(sTradeChannels.shortMapName, mapName);
		zone_GetTradeChannelNameFromMapName(sTradeChannels.baseZoneChannelName, ARRAY_SIZE_CHECKED(sTradeChannels.baseZoneChannelName), 
			sTradeChannels.shortMapName, 0, eMapType, iVirtualShardID);

		if (sTradeChannels.mapName)
			free(sTradeChannels.mapName);
		sTradeChannels.mapName = StructAllocString(mapName);
		if (sTradeChannels.ppZoneChannels)
			eaDestroyStruct(&sTradeChannels.ppZoneChannels, parse_ZoneChannelName);
	}
	if (uPartitionID == 0)
		return sTradeChannels.baseZoneChannelName;
	pTradeChannel = eaIndexedGetUsingInt(&sTradeChannels.ppZoneChannels, uPartitionID);
	if (!pTradeChannel)
	{
		pTradeChannel = calloc(1, sizeof(ZoneChannelName));
		pTradeChannel->uPartitionID = uPartitionID;

		zone_GetTradeChannelNameFromMapName(pTradeChannel->channelName, ARRAY_SIZE_CHECKED(pTradeChannel->channelName), 
			sTradeChannels.shortMapName, uPartitionID, eMapType, iVirtualShardID);
		if (!sTradeChannels.ppZoneChannels)
			eaIndexedEnable(&sTradeChannels.ppZoneChannels, parse_ZoneChannelName);
		eaIndexedAdd(&sTradeChannels.ppZoneChannels, pTradeChannel);
	}
	return pTradeChannel->channelName;
}

const char *getTradeChannelName(Entity *pEnt)
{
	U32 uPartitionID = gslEntGetPartitionID(pEnt);
	return getTradeChannelNameByPartition(uPartitionID);
}

static void GameServer_InitializePartitionTradeChannel(int iPartitionIdx)
{
	U32 uID = partition_IDFromIdx(iPartitionIdx);
	RemoteCommand_ChatServerCreateTradeChannel(GLOBALTYPE_CHATSERVER, 0, getTradeChannelNameByPartition(uID));
}

void ServerChat_InitializeTradeChannel(void)
{
	if( gConf.bUseChatTradeChannel && !GAMESERVER_IS_UGCEDIT)
		partition_ExecuteOnEachPartition(GameServer_InitializePartitionTradeChannel);
}


static GameServerZoneChannels sLFGChannels = {0};
static const char *getLFGChannelNameByPartition(U32 uPartitionID)
{
	const char *mapName = zmapGetFilename(NULL);
	ZoneMapType eMapType = zmapInfoGetMapType(NULL);
	ZoneChannelName *pLFGChannel;
	ContainerID iVirtualShardID = partition_VirtualShardIDFromID(uPartitionID);

	if (!devassert(mapName))
		return NULL;

	if (!sLFGChannels.mapName || stricmp(mapName, sLFGChannels.mapName))
	{
		// Map has changed or is not initialized
		getFileNameNoExtNoDirs(sLFGChannels.shortMapName, mapName);
		zone_GetTradeChannelNameFromMapName(sLFGChannels.baseZoneChannelName, ARRAY_SIZE_CHECKED(sLFGChannels.baseZoneChannelName), 
			sLFGChannels.shortMapName, 0, eMapType, iVirtualShardID);

		if (sLFGChannels.mapName)
			free(sLFGChannels.mapName);
		sLFGChannels.mapName = StructAllocString(mapName);
		if (sLFGChannels.ppZoneChannels)
			eaDestroyStruct(&sLFGChannels.ppZoneChannels, parse_ZoneChannelName);
	}
	if (uPartitionID == 0)
		return sLFGChannels.baseZoneChannelName;
	pLFGChannel = eaIndexedGetUsingInt(&sLFGChannels.ppZoneChannels, uPartitionID);
	if (!pLFGChannel)
	{
		pLFGChannel = calloc(1, sizeof(ZoneChannelName));
		pLFGChannel->uPartitionID = uPartitionID;

		zone_GetLFGChannelNameFromMapName(pLFGChannel->channelName, ARRAY_SIZE_CHECKED(pLFGChannel->channelName), 
			sLFGChannels.shortMapName, uPartitionID, eMapType, iVirtualShardID);
		if (!sLFGChannels.ppZoneChannels)
			eaIndexedEnable(&sLFGChannels.ppZoneChannels, parse_ZoneChannelName);
		eaIndexedAdd(&sLFGChannels.ppZoneChannels, pLFGChannel);
	}
	return pLFGChannel->channelName;
}

const char *getLFGChannelName(Entity *pEnt)
{
	U32 uPartitionID = gslEntGetPartitionID(pEnt);
	return getLFGChannelNameByPartition(uPartitionID);
}

static void GameServer_InitializePartitionLFGChannel(int iPartitionIdx)
{
	U32 uID = partition_IDFromIdx(iPartitionIdx);
	RemoteCommand_ChatServerCreateLFGChannel(GLOBALTYPE_CHATSERVER, 0, getLFGChannelNameByPartition(uID));
}

void ServerChat_InitializeLFGChannel(void)
{
	if( gConf.bUseChatLFGChannel && !GAMESERVER_IS_UGCEDIT)
		partition_ExecuteOnEachPartition(GameServer_InitializePartitionLFGChannel);
}
...
void ServerChat_MapLoad(void)
{
	if (!GAMESERVER_IS_UGCEDIT)
	{
		...
		{
			RemoteCommand_ChatServerJoinChannel(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(currEnt), getZoneChannelName(currEnt), CHANNEL_SPECIAL_ZONE, true);
			if( gConf.bUseChatTradeChannel )
			{
				RemoteCommand_ChatServerJoinChannel(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(currEnt), getTradeChannelName(currEnt), CHANNEL_SPECIAL_TRADE, true);
			}
			if( gConf.bUseChatLFGChannel )
			{
				RemoteCommand_ChatServerJoinChannel(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(currEnt), getLFGChannelName(currEnt), CHANNEL_SPECIAL_LFG, true);
			}
		}
...
void ServerChat_MapUnload(void)
{
	if (!GAMESERVER_IS_UGCEDIT)
	{
		...
		while ((currEnt = EntityIteratorGetNext(iter)))
		{
			...
			if( gConf.bUseChatTradeChannel )
			{
				RemoteCommand_ChatServerLeaveChannel(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(currEnt), getTradeChannelName(currEnt));
			}
			if( gConf.bUseChatLFGChannel )
			{
				RemoteCommand_ChatServerLeaveChannel(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(currEnt), getLFGChannelName(currEnt));
			}
		}
		EntityIteratorRelease(iter);
	}
...
static PlayerInfoStruct *createPlayerInfo(SA_PARAM_NN_VALID Entity *pEnt)
{
	...
	if(pEnt->pPlayer && !entCheckFlag(pEnt, ENTITYFLAG_DESTROY) && (iPartitionIdx != PARTITION_ENT_BEING_DESTROYED))
	{
		...
		ContainerID iVirtualShardID = partition_VirtualShardIDFromIdx(iPartitionIdx);
		...
		pPlayerInfo->uVirtualShardID = iVirtualShardID;
		
		gslChat_SetChatPermissionFlags(pEnt, &pPlayerInfo->eGamePermissionInfo);
		...
		ServerChat_GameSpecific_UpdatePlayerInfoStruct(pEnt, pPlayerInfo);
	}

	return( pPlayerInfo );
}

void DEFAULT_LATELINK_ServerChat_GameSpecific_UpdatePlayerInfoStruct(Entity *pEnt, PlayerInfoStruct *pPlayerInfo)
{
}
...
void ServerChat_ReconnectToChatServer(void)
{
	...
	// Make sure zone-like channels are created
	ServerChat_InitializeZoneChannel();
	ServerChat_InitializeTradeChannel();
	ServerChat_InitializeLFGChannel();
	...
	if( gConf.bUseChatTradeChannel )
	{
		RemoteCommand_ChatServerGroupLogin (GLOBALTYPE_CHATSERVER, 0, getTradeChannelName(), &list);
	}
	if( gConf.bUseChatLFGChannel )
	{
		RemoteCommand_ChatServerGroupLogin (GLOBALTYPE_CHATSERVER, 0, getLFGChannelName(), &list);
	}
...
void ServerChat_Login(Entity *pEnt)
{
	...
	if (pEnt && pEnt->pPlayer)
	{
		...
		if (!GAMESERVER_IS_UGCEDIT)
		{
			...
			if( gConf.bUseChatTradeChannel )
			{
				RemoteCommand_ChatServerJoinChannel(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(pEnt), getTradeChannelName(pEnt), CHANNEL_SPECIAL_TRADE, true);
			}
			if( gConf.bUseChatLFGChannel )
			{
				RemoteCommand_ChatServerJoinChannel(GLOBALTYPE_CHATSERVER, 0, entGetAccountID(pEnt), getLFGChannelName(pEnt), CHANNEL_SPECIAL_LFG, true);
			}
		}
		else
		{
			// Auto connect them to the UGC chat channel.
			ServerChat_JoinSpecialChannel( pEnt, UGCEDIT_CHANNEL_NAME );
			ServerChatConfig_SetCurrentInputChannel( pEnt, "Chatwindow_Tabgroup", 0, UGCEDIT_CHANNEL_NAME );
		}
...
void ServerChat_PlayerLeftMap(Entity *pEnt)
{
	if (!GAMESERVER_IS_UGCEDIT)
	{
		...
		if( gConf.bUseChatTradeChannel )
		{
			RemoteCommand_ChatServerLeaveChannel(GLOBALTYPE_CHATSERVER, 0, pEnt->pPlayer->accountID, getTradeChannelName(pEnt));
		}
		if( gConf.bUseChatLFGChannel )
		{
			RemoteCommand_ChatServerLeaveChannel(GLOBALTYPE_CHATSERVER, 0, pEnt->pPlayer->accountID, getLFGChannelName(pEnt));
		}
	}
...
void ServerChat_SendMessage(Entity *pFromEnt, SA_PARAM_NN_VALID ChatMessage *pMsg) {
	...
	if ( (pMsg->eType == kChatLogEntryType_Zone || pMsg->eType == kChatLogEntryType_Trade || pMsg->eType == kChatLogEntryType_LookingForGroup) && gamePermission_Enabled() && !GamePermission_EntHasToken(pFromEnt, GAME_PERMISSION_CAN_SEND_ZONE)) {
	...
	iVirtualShardID = partition_VirtualShardIDFromIdx(entGetPartitionIdx(pFromEnt));
	...
	switch (pMsg->eType) {
		...
		xcase kChatLogEntryType_Trade: {
			estrCopy2(&pMsg->pchChannel, getTradeChannelName(pFromEnt));
			pMsg->iInstanceIndex =partition_PublicInstanceIndexFromIdx(entGetPartitionIdx(pFromEnt));
		}
		xcase kChatLogEntryType_LookingForGroup: {
			estrCopy2(&pMsg->pchChannel, getLFGChannelName(pFromEnt));
			pMsg->iInstanceIndex =partition_PublicInstanceIndexFromIdx(entGetPartitionIdx(pFromEnt));
		}
		...
		xcase kChatLogEntryType_Guild: {
			if (pFromEnt->pPlayer && pFromEnt->pPlayer->pGuild && pFromEnt->pPlayer->pGuild->iGuildChat) {
				guild_GetGuildChannelNameFromID(SAFESTR(pchDynamicChannel), pFromEnt->pPlayer->pGuild->iGuildChat, iVirtualShardID);
			}
			else
			{
		...
		xcase kChatLogEntryType_Officer: {
			if (pFromEnt->pPlayer && pFromEnt->pPlayer->pGuild && pFromEnt->pPlayer->pGuild->iGuildChat) {
				guild_GetOfficerChannelNameFromID(SAFESTR(pchDynamicChannel), pFromEnt->pPlayer->pGuild->iGuildChat, iVirtualShardID);
			}
			else
			{
...
void ServerChat_JoinSpecialChannel(Entity *pEnt, const char *channel_name)
{
	...
	{
		ContainerID iVirtualShardID = partition_VirtualShardIDFromIdx(entGetPartitionIdx(pEnt));
		char buffer[256];
		buffer[0] = 0;
		if (iVirtualShardID && ServerChat_IsChannelVShardUnique(channel_name))
			sprintf(buffer, "%s%d_%s", VSHARD_PREFIX, iVirtualShardID, channel_name);
		else
