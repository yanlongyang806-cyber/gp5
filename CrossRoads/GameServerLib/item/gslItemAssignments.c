	...
	if (g_ItemAssignmentSettings.bGenerateSampleRewardTable && pSampleRewardOutcome && pSampleRewardOutcome->pResults)
	{
		...
		if (pRewardTable)
		{
			...
			reward_GenerateBagsForItemAssignment(PARTITION_UNINITIALIZED, NULL, pRewardTable, 1, 1.f, 
													NULL, false, true, &uSeed, &eaRewardBags);
		}
...
void gslItemAssignments_NotifySendFailure(Entity* pEnt, ItemAssignmentDef* pDef, const char* pchMsgKey)
{
													 ...
													 InventoryBag*** peaRewardBags)
{
	...
	if (pDef && pRewardTable)
	{
		...
		// Generate reward bags
		reward_GenerateBagsForItemAssignment(iPartIdx, pEnt, pRewardTable, iLevel, fScale, eaNumericScales, 
											 bUseRewardMods, false, &uSeed, peaRewardBags);
		// Cleanup
...
void gslItemAssignments_PlayerUpdateNextProcessTime(Entity* pEnt)
{
	...
	if (pPersistedData)
	{
		...
		if (pPersistedData->uNextUpdateTime == 0 || pPersistedData->uNextUpdateTime > uCurrentTime)
		{
...
bool gslItemAssignments_IsValidAvailableAssignment(Entity* pEnt, ItemAssignmentDef* pDef, ItemAssignmentLocationData* pLocationData)
{
	...
	{
		if(!gslItemAssignments_MaxAssignmentCheck(pEnt, pDef))
		{
			// exceeded allowed maximum running of this assignment
			return false;
		}
	}
...
enumTransactionOutcome ItemAssignmentsUpdateUnlockedSlots(ATR_ARGS, NOCONST(Entity)* pEnt, ItemAssignmentUnlockedSlots *pSlots)
{
	...
	for(i=0;i<ea32Size(&pSlots->eaSlots);i++)
	{
		ea32PushUnique(&pAssignmentData->eaItemAssignmentSlotsUnlocked,pSlots->eaSlots[i]);
	}
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Pplayer.Skilltype, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pplayer.Pitemassignmentpersisteddata, .pInventoryV2.Ppinventorybags, .Psaved.Ppallowedcritterpets, .Psaved.Uiindexbuild, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .pInventoryV2.Pplitebags, .Pplayer.Playertype, .Psaved.Ppbuilds, .Psaved.Ppownedcontainers, .pInventoryV2.Peaowneduniqueitems, .Itemidmax, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems")
														 ...
														 GameAccountDataExtract* pExtract)
{
	...
	if (NONNULL(pAssignment))
	{
		...
		if (pDef && pDef->bIsAbortable && uCurrentTime < uCompleteTime)
		{
			...
			{
				// give back all the items that were saved.
				// 
				...
				for (i = 0; i < eaSize(&pDef->pRequirements->eaItemCosts); i++)
				{
					...
					if (pItemDef)
					{
						...
						else // check that this wasn't an item that was saved off
						{
							S32 iCountFound = 0;
							InvBagIDs eBagID;

							FOR_EACH_IN_EARRAY(pAssignment->eaSavedItems, NOCONST(Item), pSavedItem)
							{
								if (GET_REF(pSavedItem->hItem) == pItemDef)
								{
									Item* pItem = NULL;
									pItem = DECONST(Item*, pSavedItem);
									eaRemoveFast(&pAssignment->eaSavedItems, FOR_EACH_IDX(-, pSavedItem));

									eBagID = inv_trh_GetBestBagForItemDef(pEnt, pItemDef, CONTAINER_NOCONST(Item, (pItem)), iCount, true, pExtract);
									
									if (!inv_ent_trh_AddItem(	ATR_PASS_ARGS, pEnt, eaPets, eBagID, -1, 
																CONTAINER_NOCONST(Item, pItem), ItemAdd_UseOverflow, 
																NULL, pReason, pExtract))
									{
										eResult = TRANSACTION_OUTCOME_FAILURE;
										break;
									}
									iCountFound++;
									if (iCountFound >= iCount)
									{
										break;
									}
								}
							...
							if (iCountFound < iCount)
							{
								...
								iCount = (iCount - iCountFound);
								pItem = pItemDef ? item_FromDefName(pItemDef->pchName) : NULL;
...
static void gslItemAssignments_CheckRankSchedule(	Entity* pEnt,
													SA_PARAM_OP_VALID ItemAssignmentCategorySettings *pCategory,
													...
													DisplayMessage *pMsg)
{
	...
	iNumRanks = ItemAssignments_GetNumRanks(pCategory, pSchedule);
...
void gslItemAssignment_CheckRankSchedules(Entity *pEnt)
{
	if(pEnt && g_ItemAssignmentSettings.pCategoryRankingSchedule)
	{
		int i;
		int iMax;

		DefineGetMinAndMaxInt(ItemAssignmentCategoryEnum,&i,&iMax);

		for(i=i+1;i<iMax;i++)
		{
			ItemAssignmentCategorySettings* pSettings = ItemAssignmentCategory_GetSettings(i);

			if(pSettings)
			{
				gslItemAssignments_CheckRankSchedule(	pEnt, pSettings, g_ItemAssignmentSettings.pCategoryRankingSchedule, 
...
static void gslItemAssignment_AwardCompletionExperience(Entity* pEnt, ItemAssignmentDef* pDef, U32 uItemAssignmentID)
{
	if (pDef && pDef->iCompletionExperience > 0)
	{
		...
		{
			// check the amount of experience we 
			S32 iCurrentXP = inv_GetNumericItemValue(pEnt, pSettings->pchNumericXP1);
			S32 iMaxExperience = ItemAssignments_GetMaxExperience(pSettings, g_ItemAssignmentSettings.pCategoryRankingSchedule);
			S32 iCompletionExperience = pDef->iCompletionExperience;
			if (iCurrentXP + iCompletionExperience > iMaxExperience)
			{
				iCompletionExperience = iMaxExperience - iCurrentXP;
			}

			if (iCompletionExperience > 0)
			{
				ItemChangeReason reason = {0};
				...
				itemtransaction_AddNumeric(pEnt, pSettings->pchNumericXP1, (F32)iCompletionExperience, 
...
static void gslItemAssignment_CollectRewards_CB(TransactionReturnVal* pReturn, ItemAssignmentCBData* pData)
{
	...
	if (pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		{
			gslItemAssignments_CheckRankSchedule(	pEnt, NULL, g_ItemAssignmentSettings.pMetaRankingSchedule, 
...
static void gslItemAssignment_StartNew_CB(TransactionReturnVal* pReturn, ItemAssignmentCBData* pData)
{
	...
	if (pEnt && pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		gslItemAssignments_PlayerUpdateAssignmentsOnStateChange(iPartitionIdx, pEnt, pDef, !g_ItemAssignmentSettings.bAllowDuplicateActiveAssignments);
...
// performs static checks based on the ItemAssignmentDef and ItemDef whether or not the item should potentially be saved on the assignment
// this is for the case that the user aborts the task, that they get their exact item back if has been modified in some way
static S32 gslItemAssignment_ShouldSaveItemStaticCheck(ItemAssignmentDef* pDef, ItemDef* pItemDef)
{
	if (!pDef->bIsAbortable)
		return false;
...
// goes through the items in peaItemCostsInOut and will destroy any that are found to not be needed to save in the case the user decides to abort the task
static void gslItemAssignments_ShouldSaveItemDynamicCheck(ItemAssignmentDef* pDef, ItemDef* pItemDef, Item ***peaItemCostsInOut)
{
	NOCONST(Item)* pCompareItem = NULL;
	int bMatched = false;
	//CONST_EARRAY_OF(NOCONST(Power)) eaSavedPowers = NULL;

	pCompareItem = inv_ItemInstanceFromDefName(pItemDef->pchName, 0, 0, NULL, NULL, NULL, false, NULL);
	if (!pCompareItem)
	{	// couldn't create the item to compare with, we're going to just keep all of them
		return;
	}

	FOR_EACH_IN_EARRAY(*peaItemCostsInOut, Item, pItem)
	{
		int compareResult = 0;
		NOCONST(Item) *pItemNoConst = STRUCT_NOCONST(Item, pItem);
		EARRAY_OF(NOCONST(Power)) eaSavedPowers = (EARRAY_OF(NOCONST(Power)))pItem->ppPowers;

		// set up some data so the compare can work
		pCompareItem->id = pItem->id;
		
		pItemNoConst->ppPowers = NULL;
		compareResult = StructCompare(parse_Item, pItem, pCompareItem, 0,0,0);
		
		// reset anything we changed to make the compare work
		pItemNoConst->ppPowers = eaSavedPowers;

		if (compareResult == 0)
		{	// these items match, we don't need to save the item and can destroy it
			StructDestroy(parse_Item, pItem);
			eaRemove(peaItemCostsInOut, FOR_EACH_IDX(-, pItem));
		}
		
	}
	FOR_EACH_END

	// destroy the item we created to compare with
	StructDestroyNoConst(parse_Item, pCompareItem);

}

// Pays the item costs of the ItemAssignmentDef
// Some Items may need to be saved off if we need to refund the item instead of destroying them
AUTO_TRANS_HELPER;
S32 gslItemAssignments_trh_RemoveItemCosts(ATR_ARGS, 
										   ...
										   const ItemChangeReason *pReason,
										   GameAccountDataExtract* pExtract,
										   Item ***peaItemCostsOut)
{
	static Item **s_eaItemCosts = NULL;
	...
	if (pDef->pRequirements && eaSize(&pDef->pRequirements->eaItemCosts))
	{
		...
		for (i = 0; i < eaSize(&pDef->pRequirements->eaItemCosts); i++)
		{
			...
			{
				if (!gslItemAssignment_ShouldSaveItemStaticCheck(pDef, pItemDef))
				{	// this item should just be removed and deleted
					S32 iNumRemoved = inv_trh_FindItemCountByDefNameEx(ATR_PASS_ARGS, pEnt, eSearchBags, eExcludeSearchBags, pItemDef->pchName, 
						kItemFlag_SlottedOnAssignment, iCount, true, false, 
						pReason, pExtract, NULL);
					if (iNumRemoved < iCount)
					{
						return false;
					}
				}
				else
				{
					...
					eaClear(&s_eaItemCosts);
					iNumRemoved = inv_trh_FindItemCountByDefNameEx(ATR_PASS_ARGS, pEnt, eSearchBags, eExcludeSearchBags, pItemDef->pchName, 
																		kItemFlag_SlottedOnAssignment, iCount, true, false, 
																		pReason, pExtract, &s_eaItemCosts);
					if (iNumRemoved < iCount)
					{
						return false;
					}

					// check to make sure if we really want to save these items off
					gslItemAssignments_ShouldSaveItemDynamicCheck(pDef, pItemDef, &s_eaItemCosts);

					if (eaSize(&s_eaItemCosts) > 0)
					{	// we have some to save, put them in the list to return with
						eaPushEArray(peaItemCostsOut, &s_eaItemCosts);
					}
				}
			}
		}
														 ...
														 GameAccountDataExtract* pExtract)
{
	...
	EARRAY_OF(NOCONST(Item)) eaItemCosts = NULL;
	
	// Verify parameters
	pDef = ItemAssignment_DefFromName(pchAssignmentDef);
	if (!pDef || !pSlots || ISNULL(pEnt) || ISNULL(pEnt->pPlayer)) 
	{
		return NULL;
	}
	
	// Check for requirements and costs
	if (!ItemAssignments_trh_CheckRequirements(ATR_PASS_ARGS, pEnt, pDef, pSlots, pExtract))
	{
		return NULL;
	}

	if (!gslItemAssignments_trh_RemoveItemCosts(ATR_PASS_ARGS, pEnt, pDef, pReason, pExtract, (Item***)&eaItemCosts))
	{
		return NULL;
	}

	// Create persistent data if not present
	...
	// Make sure slot isn't something crazy
	...
	pNewAssignment->eaSavedItems = eaItemCosts;
	SET_HANDLE_FROM_REFERENT(g_hItemAssignmentDict, pDef, pNewAssignment->hDef);

	// Create item slots
	for (i = 0; i < eaSize(&pSlots->eaSlots); i++) 
	{
		NOCONST(ItemAssignmentSlottedItem)* pSlottedItem;
		NOCONST(InventorySlot)* pSlot;

		// Find the item
		pSlottedItem = CONTAINER_NOCONST(ItemAssignmentSlottedItem, pSlots->eaSlots[i]);
		...
		{
			// Item was not found, so destroy the assignment in progress and fail
			StructDestroyNoConstSafe(parse_ItemAssignment, &pNewAssignment);
			return NULL;
		}
	}

	// Put the new assignment onto the active list
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pplayer.Pitemassignmentpersisteddata, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .pInventoryV2.Ppinventorybags, .Pplayer.Skilltype, .Hallegiance, .Hsuballegiance, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, pInventoryV2.ppLiteBags, .Pplayer.Eaplayernumericthresholds");
...
static void gslItemAssignment_Complete_CB(TransactionReturnVal* pReturn, ItemAssignmentCBData* pData)
{
	...
	pEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, pData->eID);
	if(!pEnt && GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER) {
		pEnt = entForClientCmd(pData->eID,pEnt);
	}
	...
	// Find the assignment
	if (pEnt) {
		pAssignment = ItemAssignment_EntityGetActiveAssignmentByID(pEnt, pData->uAssignmentID);
	}

	if (pEnt && pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS) {
		// Update assignments after state change
		iPartitionIdx = entGetPartitionIdx(pEnt);
		pDef = GET_REF(pData->hDef);
		gslItemAssignments_PlayerUpdateAssignmentsOnStateChange(iPartitionIdx, pEnt, pDef, true);
		
		// Clear the read flag and mark it dirty
		pCompletedDetails = ItemAssignment_EntityGetRecentlyCompletedAssignmentByID(pEnt, pData->uAssignmentID);
	...
	// Mark the assignment as no longer pending
	if (pAssignment) {
...
AUTO_TRANS_HELPER;
void gslItemAssignments_trh_CleanupCompletedAssignments(ATR_ARGS, ATH_ARG NOCONST(Entity) *pEnt, NOCONST(ItemAssignmentPersistedData)* pAssignmentData, U32 uCurrentTime)
{
	S32 i;
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pplayer.Pitemassignmentpersisteddata, .pInventoryV2.Ppinventorybags, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Skilltype, .Hallegiance, .Hsuballegiance, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, pInventoryV2.ppLiteBags, .Pplayer.Eaplayernumericthresholds")
...
enumTransactionOutcome gslItemAssignments_tr_CompleteAssignment(ATR_ARGS, 
																...
																GameAccountDataExtract* pExtract)
{
	NOCONST(ItemAssignment)* pAssignment;
	NOCONST(ItemAssignmentPersistedData)* pAssignmentData;
	ItemAssignmentDef* pDef;
	U32 uCurrentTime;
	U32 uCompleteTime;
	ItemAssignmentOutcome* pOutcome;
	bool bCanComplete;
	ItemAssignmentDef* pNewAssignmentDef;
	...
	const char* pchMapMsgKey;
	S32 i;
	U32 uOldestInactiveCompleted;
	U32 uItemAssignmentSlot;
	S32 iLastInactiveCompletedIndex;
	S32 iRecentlyCompletedSize;

	// --- Phase 1: Checking Requirements ----

	// Get the assignment 
	pAssignment = ItemAssignment_trh_EntityGetActiveAssignmentByID(pEnt, uAssignmentID);

	//Fail out if Assignment not found or if it has no reward outcome
	if (ISNULL(pAssignment) || EMPTY_TO_NULL(pAssignment->pchRewardOutcome)) 
	{
		return TRANSACTION_OUTCOME_FAILURE;
	}

	// Check if forcing completion, and if so can the player pay the cost
	if (bForceComplete) 
	{
		if (bUseToken && NONNULL(pData)) 
		{
			// If using a token and have GameAccountData, then consume the token
		...
		{
			// If have a "rush cost" numeric, then spend the cost
			...
			{
				// Failed to spend the cost
				return TRANSACTION_OUTCOME_FAILURE;
			}
		}
	}

	// Get completion status and outcome status
	pDef = GET_REF(pAssignment->hDef);
	uCurrentTime = timeSecondsSince2000();
	uCompleteTime = pAssignment->uTimeStarted + ItemAssignment_GetDuration(pAssignment, pDef);
	bCanComplete = bForceComplete ? true : uCurrentTime >= uCompleteTime;
	pOutcome = ItemAssignment_GetOutcomeByName(pDef, pchOutcomeName);

	// Fail out if no assignment def, no outcome, or can't complete
	if (!pDef || !pOutcome || !bCanComplete) 
	{
		return TRANSACTION_OUTCOME_FAILURE;
	}

	// --- Phase 2: Managing eaCompletedAssignments ----

	// Get assignment persisted data, map message key, and any new assignment
	pAssignmentData = pEnt->pPlayer->pItemAssignmentPersistedData;		
	pchMapMsgKey = pAssignment->pchMapMsgKey;
	...
	{
		pNewAssignmentDef = NULL;
	}

	// Remove old completed assignments that no longer need to be persisted
	// Cleans up "pAssignmentData->eaCompletedAssignments"
	gslItemAssignments_trh_CleanupCompletedAssignments(ATR_PASS_ARGS, pEnt, pAssignmentData, uCurrentTime);

	// Check if this assignment needs to be tracked as a completed assignment, which is
	// necessary if it has a cooldown or otherwise can't simply be repeated.  If so, we need
	// to add or reuse an entry in "pAssignmentData->eaCompletedAssignments"
	if (gslItemAssignments_IsValidCompletedAssignment(pDef, false)) 
	{
		...
		// If it has a cooldown on completion, set the completion time
	...
	// --- Phase 3: Managing eaRecentlyCompletedAssignments ----

	// Look through recently completed assignments to count the number there that are not currently running
	// and to find the least recently completed entry in the list
	iRecentlyCompletedSize = 0;
	iLastInactiveCompletedIndex = -1;
	uOldestInactiveCompleted = 0;
	for (i = eaSize(&pAssignmentData->eaRecentlyCompletedAssignments)-1; i >= 0; i--) 
	{
		...
		{
			// Get here if the assignment is not in progress, so count it
			// It can be in progress and be in this list if the assignment is repeatable
			iRecentlyCompletedSize++;

			// Save the oldest inactive index and ID
	...
	// Add this entry to the recently completed list
	if (iLastInactiveCompletedIndex < 0 || iRecentlyCompletedSize < g_ItemAssignmentSettings.iMaxAssignmentHistoryCount) 
	{
		// Create a new RecentlyCompletedAssignment entry because we're under the max size 
		// or all entries in the array are in progress.
	...
	// First, set the scalar data into the entry
	...
	// bMarkedAsRead is NO_TRANSACT so it not updated here

	// Destroy all the backed up items we had saved in the case of the user canceled the task
	eaDestroyStructNoConst(&pAssignment->eaSavedItems, parse_Item);

	// Next, deal with the array of slotted items on the assignment.
	// We need to consume (destroy) them, put them on the result, or put them on a new assignment
	for (i = 0; i < eaSize(&pAssignment->eaSlottedItems); i++) 
	{
		NOCONST(InventorySlot)* pSlot;
		NOCONST(ItemAssignmentSlottedItemResults)* pSlottedItemResults;
		ItemDef* pItemDef;

		// Find the inventory slot where the items is being kept
		pSlot = ItemAssignments_trh_GetInvSlotFromSlottedItem(ATR_PASS_ARGS, pEnt, pAssignment->eaSlottedItems[i], pExtract);
		if (!pSlot || !pSlot->pItem) 
		{
		...
		// Mark the item as no longer slotted
		pSlot->pItem->flags &= ~kItemFlag_SlottedOnAssignment;

		// If the item has a def, copy item handle and ID to the results
		pItemDef = GET_REF(pSlot->pItem->hItem);
		if (pItemDef) 
		{
		...
		{
			pSlottedItemResults = NULL;
		}

		// Handle completion actions for this item
		if (pOutcome->pResults && pSlottedItemResults) 
		{
			if (pItemDef && gslItemAssignments_ShouldDestroyItem(pItemDef, pOutcome)) 
			{
				// Destroy the item
			...
			else if (pNewAssignmentDef && randomPositiveF32() <= pOutcome->pResults->fNewAssignmentChance) 
			{
				...
				pSlottedItem = StructCloneNoConst(parse_ItemAssignmentSlottedItem, pAssignment->eaSlottedItems[i]);
				pSlottedItem->iAssignmentSlot = eaSize(&eaNewAssignmentSlottedItems);
				eaPush(&eaNewAssignmentSlottedItems, pSlottedItem);
	...
	// --- Phase 4: Managing the Assignment itself ----

	// Save the slot for later use
	uItemAssignmentSlot = pAssignment->uItemAssignmentSlot;

	if (bHasItemRewards || g_ItemAssignmentSettings.bKeepEmptyRewardOutcomeAssignments) 
	{
		// If there are rewards or we're supposed to keep empty assignments, set the outcome
		// onto the assignment.
		pAssignment->pchRewardOutcome = allocAddString(pchOutcomeName);

		// Also delete any slotted item tracking. Anything necessary about the item was done in the previous phase
	...
	else 
	{
		// Remove and destroy the assignment
		eaFindAndRemove(&pAssignmentData->eaActiveAssignments, pAssignment);
		StructDestroyNoConstSafe(parse_ItemAssignment, &pAssignment);
	}
	...
	// --- Phase 5: Starting a new assignment if requested ----

	if (pNewAssignmentDef && eaSize(&eaNewAssignmentSlottedItems)) 
	{
		NOCONST(ItemAssignment)* pNewAssignment;
		ItemAssignmentSlots NewSlots = {0};
		U32 uNewAssignmentStartTime;
		...
		uNewAssignmentStartTime = bForceComplete ? uCurrentTime : uCompleteTime;
		...
		// Clean up slotted items array
		eaDestroyStruct(&NewSlots.eaSlots, parse_ItemAssignmentSlottedItem);

		// Fail completely if the new assignment could not be created
		if (!pNewAssignment) 
		{
			return TRANSACTION_OUTCOME_FAILURE;
		}

		// Put the new assignment tracking in place
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pchar.Ppsecondarypaths, .Pchar.Hpath, .Pplayer.Pitemassignmentpersisteddata, .pInventoryV2.Ppinventorybags, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Skilltype, .Hallegiance, .Hsuballegiance, .Pplayer.Eskillspecialization, .Pchar.Hclass, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .pInventoryV2.Peaowneduniqueitems, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, pInventoryV2.ppLiteBags, .Pplayer.Eaplayernumericthresholds");
enumTransactionOutcome gslItemAssignments_tr_CompleteAssignmentNoToken(ATR_ARGS, 
																NOCONST(Entity)* pEnt, 
																U32 uAssignmentID, 
																const char* pchOutcomeName,
																U32 bHasItemRewards,
																...
																GameAccountDataExtract* pExtract)
{
	return gslItemAssignments_tr_CompleteAssignment(ATR_PASS_ARGS, pEnt, NULL, uAssignmentID, pchOutcomeName, 
				bHasItemRewards, bForceComplete, iForceCompleteCost, false, pReason, pExtract);
}
...
bool gslItemAssignments_CompleteAssignment(Entity* pEnt, ItemAssignment* pAssignment, const char* pchForceOutcome, bool bForce, bool bUseToken)
{
		...
		(pOutcome = gslItemAssignments_ChooseOutcome(pEnt, pAssignment, pchForceOutcome)))
	{
		...
		// Must set this before calling the transaction.  After the transaction, this pointer may be bad.
		pAssignment->bCompletionPending = true;

		// Only pass in game account data if want to use a token, else don't pass it in so it runs in fast local mode
		if (bUseToken) {
			...
			AutoTrans_gslItemAssignments_tr_CompleteAssignmentNoToken(pReturn, objServerType(), 
				GLOBALTYPE_ENTITYPLAYER, uEntID, 
				pAssignment->uAssignmentID, pOutcome->pchName, bHasItemRewards, bForce, iForcedCompletionCost, &reason, pExtract);
		}
	...
	FOR_EACH_IN_EARRAY(pDef->pRequirements->eaItemCosts, ItemAssignmentItemCost, pItemCost)
	{
		...
		if (pItemDef)
		{
			...
			if (pItemDef->eType != kItemType_Numeric)
			{
				...
				eBagID = inv_trh_GetBestBagForItemDef(CONTAINER_NOCONST(Entity, pFakeEnt), pItemDef, CONTAINER_NOCONST(Item, (pItem)), iCount, true, pExtract);
				CONTAINER_NOCONST(Item, pItem)->count = iCount;
...
AUTO_COMMAND ACMD_ACCESSLEVEL(9) ACMD_NAME(ItemAssignmentsForceAddPersonalAssignment);
void gslItemAssignment_ForceAddPersonalAssignment(CmdContext *pCmdContext, Entity* pEnt, ACMD_NAMELIST("ItemAssignmentDef", REFDICTIONARY) const char* pchForceAssignmment)
{
	ItemAssignmentDef *pDef = RefSystem_ReferentFromString(g_hItemAssignmentDict, pchForceAssignmment);
	Player *pPlayer = SAFE_MEMBER(pEnt, pPlayer);
	ItemAssignmentPlayerData *pPlayerData = SAFE_MEMBER(pPlayer, pItemAssignmentData);

	if (!pPlayer)
		return;
	if (!pPlayer->pItemAssignmentData)
		pPlayer->pItemAssignmentData = pPlayerData = StructCreate(parse_ItemAssignmentPlayerData);

	if (pDef)
	{
		if (eaIndexedFindUsingString(&pPlayerData->eaAdditionalPersonalAssignments, pDef->pchName) < 0)
		{
			ItemAssignmentDefRef *pRef = StructCreate(parse_ItemAssignmentDefRef);
			SET_HANDLE_FROM_REFERENT("ItemAssignmentDef", pDef, pRef->hDef);
			eaIndexedAdd(&pPlayerData->eaAdditionalPersonalAssignments, pRef);
			estrConcatf(pCmdContext->output_msg, "Adding %s", pDef->pchName);
		}
		else
		{
			estrConcatf(pCmdContext->output_msg, "Already have %s", pDef->pchName);
		}
	}
	else if (pchForceAssignmment && *pchForceAssignmment)
	{
		FOR_EACH_IN_REFDICT(g_hItemAssignmentDict, ItemAssignmentDef, pAssignment);
		{
			...
			if (pchDisplayName && !stricmp(pchDisplayName, pchForceAssignmment))
			{
				if (eaIndexedFindUsingString(&pPlayerData->eaAdditionalPersonalAssignments, pAssignment->pchName) < 0)
				{
					ItemAssignmentDefRef *pRef = StructCreate(parse_ItemAssignmentDefRef);
					SET_HANDLE_FROM_REFERENT("ItemAssignmentDef", pAssignment, pRef->hDef);
					eaIndexedAdd(&pPlayerData->eaAdditionalPersonalAssignments, pRef);
					estrConcatf(pCmdContext->output_msg, "Adding %s", pAssignment->pchName);
				}
				else
				{
					estrConcatf(pCmdContext->output_msg, "Already have %s", pAssignment->pchName);
				}
			}
		}
		FOR_EACH_END;
