...
static void EquipItemArtOnMoveCB(Entity *pEnt, int iSrcBagID, int iDstBagID, int iDstBagSlot)
{
	GameAccountDataExtract* pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
	Item* pItem = inv_GetItemFromBag(pEnt, iDstBagID, iDstBagSlot, pExtract);
	ItemDef* pDef = SAFE_GET_REF(pItem, hItem);

	if (pItem && pDef && eaSize(&g_ItemConfig.eaBagToFxMaps) > 0)
	{
		ClientCmd_gclPlayChangedCostumePartFX(pEnt, iDstBagID, iDstBagSlot);
	}
	...
	if(pEnt && pEnt->pChar)
	{
		...
		if(gConf.bItemArt)
		{
			InventoryBag *pBagDst = (InventoryBag*)inv_GetBag(CONTAINER_NOCONST(Entity, pEnt), iDstBagID, pExtract);
			InventoryBag *pBagSrc = (InventoryBag*)inv_GetBag(CONTAINER_NOCONST(Entity, pEnt), iSrcBagID, pExtract);
...
static void EntMoveItem_CB(TransactionReturnVal *returnVal, ItemEquipCB *cbData)
{
	...
	{
		EquipItemArtOnMoveCB(pEnt, cbData->iSrcBagID, cbData->iDstBagID, cbData->iDstBagSlot);
	}
	else if (returnVal->eOutcome == TRANSACTION_OUTCOME_FAILURE)
	{
		// if failed to move to general inventory, send inventory full notification
		//   Theoretically there could be other reasons this failed maybe, but the
		//   most likely cause is inventory full. This is what "move from overflow" does.
		if (pEnt && cbData->iDstBagID == InvBagIDs_Inventory)
		{
			ClientCmd_NotifySend(pEnt, kNotifyType_InventoryFull, entTranslateMessageKey(pEnt, INVENTORY_FULL_MSG), NULL, NULL);
		}
	}
...
void ItemUngemItem(Entity *pEnt, S32 iSrcBagID, int iSlotIdx, int iGemIdx, S32 iDestBagID, int iDestIdx, const char *pchCurrency)
{
	...
	if (pSrcItem && pSrcItem->flags & kItemDefFlag_Fused)
		return;
...
static bool ItemMoveInternal(Entity *pEnt, Entity *pSrcEnt, bool bSrcGuild, S32 iSrcBagID, S32 iSrcSlot, Entity *pDstEnt, bool bDstGuild, S32 iDstBagID, S32 iDstSlot, S32 iCount, const char *pcActionName, GameAccountDataExtract *pExtract, TransactionReturnCallback userFunc, void *userData)
{
	...
	if (scp_itemIsSCP(pSrcItem) && pSrcItem->flags & kItemFlag_Bound && invbag_flags(pSrcBag) & InvBagFlag_BoundPetStorage)
	{
	...
	if (bSrcGuild || bDstGuild) {
		...
		returnVal = LoggedTransactions_CreateManagedReturnValEnt("ItemMove", pEnt, EntMoveItem_CB, cbData);
...
void ItemMove(Entity *pEnt, bool bSrcGuild, S32 iSrcBagID, S32 iSrcSlot, bool bDstGuild, S32 iDstBagID, S32 iDstSlot, S32 iCount)
{
	...
	if (iSrcBagID == InvBagIDs_Overflow)
	{
		...
		if (iDstBagID == InvBagIDs_None)
		{
			...
			iDstBagID = GetBestBagForItemDef(pEnt, pSrcItemDef, pSrcItem, iCount, true, pExtract);
			iDstSlot = -1;
		}
...
{
	Entity* pEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, pCBData->entID);
	ItemDef* pRewardPackItemDef = GET_REF(pCBData->hRewardPackItem);

	if(!pEnt && GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
	{
		pEnt = entForClientCmd(pCBData->entID, pEnt);
	}
...
AUTO_COMMAND ACMD_NAME(RequestItemPackRewardData) ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_PRIVATE;
void itemCmd_RequestItemPackRewardData(Entity* pPlayerEnt, U64 uItemID)
{
	Item *pItem = inv_GetItemByID(pPlayerEnt,uItemID);
	ItemDef *pDef = pItem ? GET_REF(pItem->hItem) : NULL;
	S32 iLevel = item_GetLevel(pItem) ? item_GetLevel(pItem) : entity_GetSavedExpLevel(pPlayerEnt);
	RewardTable *pRewardTable = SAFE_GET_REF2(pDef,pRewardPackInfo,hRewardTable);
	ItemRewardPackRequest *pData = StructCreate(parse_ItemRewardPackRequest);
	int j,k;
	
	if(!pDef || !item_IsRewardPack(pDef) || !pRewardTable)
		return;

	reward_GenerateBagsForRewardPack(entGetPartitionIdx(pPlayerEnt), pPlayerEnt, pRewardTable, iLevel, pItem->pSpecialProps ? pItem->pSpecialProps->fRewardPackScale : 1.0f, &pData->eaRewardBags);
	...
	ClientCmd_ReceiveItemPackRewardData(pPlayerEnt, pData);
	StructDestroy(parse_ItemRewardPackRequest, pData);
}

void ItemOpenRewardPackWithChoices(int iPartitionIdx,
											Entity *pEnt,
											Item *pItem,
											ItemDef *pItemDef,
											InvBagIDs eBagID,
											...
											char **ppItemChoices)
{
	...
	if (pEnt && pItem && pRewardTable && ItemCanOpenRewardPack(pEnt, pItem)) {
		...
		pCBData->entID = entGetContainerID(pEnt);
		SET_HANDLE_FROM_REFERENT("ItemDef", pItemDef, pCBData->hRewardPackItem);
		reward_GenerateBagsForRewardPack(iPartitionIdx, pEnt, pRewardTable, iLevel, pItem->pSpecialProps ? pItem->pSpecialProps->fRewardPackScale : 1.0f, &pCBData->eaRewardBags);
		...
		for (i = eaSize(&pCBData->eaRewardBags)-1; i >= 0; i--)
		{
			...
			for (j = eaSize(&pBag->ppIndexedInventorySlots)-1; j >= 0; j--)
			{
				...
				if (pSlot->pItem)
				{
					...
					if(pItemDef->pRewardPackInfo->bPromptRewardChoices && pBag->pRewardBagInfo->PickupType == kRewardPickupType_Choose && ppItemChoices)
					{
						int n;

						for(n=eaSize(&ppItemChoices)-1;n>=0;n--)
						{
							if(stricmp(pRewardItemDef->pchName,ppItemChoices[n]) == 0)
							{
								break;
							}
						}
						...
						if(n==-1)
						{
							eaRemove(&((NOCONST(InventoryBag)*)pBag)->ppIndexedInventorySlots,j);
							StructDestroy(parse_InventorySlot,pSlot);
							continue;
						}
					}
		...
		if(iItemCount == 0)
		{
			ErrorDetailsf("%s\nReward Pack: %s; Reward Table: %s; Allegiance: %s, Level: %d",ENTDEBUGNAME(pEnt),pItemDef->pchName,pRewardTable->pchName,REF_STRING_FROM_HANDLE(pEnt->hAllegiance),entity_GetSavedExpLevel(pEnt));
			Errorf("Reward pack gave no rewards");
			estrStackCreate(&estrError);
			entFormatGameMessageKey(pEnt,&estrError,"Item.FailRewardPackUse",STRFMT_DISPLAYMESSAGE("Itemname",pItemDef->displayNameMsg), STRFMT_END);
			notify_NotifySend(pEnt,kNotifyType_ItemUseFailed,estrError,pItemDef->pchName,NULL);
			estrDestroy(&estrError);
		...
		if (eaSize(&pCBData->eaRewardBags) && eError == kAddSavedPetErrorType_None)
		{
			...
			// Pass in item choices (if any)
			if(eaSize(&ppItemChoices))
			{
				for(j=0;j<eaSize(&ppItemChoices);j++)
				{
					eaPush(&pCBData->ppchItemChoices,StructAllocString(ppItemChoices[j]));
				}

				GiveRewards.ppChoices = pCBData->ppchItemChoices;
			}
...
static void ItemOpenRewardPack(int iPartitionIdx, 
							   ...
							   S32 iSlot)
{
	ItemOpenRewardPackWithChoices(iPartitionIdx,pEnt,pItem,pItemDef,eBagID,iSlot,NULL);
}
							  ...
							  S32 iDstType, U32 iDstID, bool bAutoUse, GameAccountDataExtract *pExtract)
{
	...
	{
		ClientCmd_NotifySend(pEnt, kNotifyType_ItemMoveFailed, entTranslateMessageKey(pEnt, "InventoryUI.CantUseItemInBank"), NULL, NULL);
		return;
	}

	if (pSrcItemDef && pDstEnt)
	{
		iDstBagID = GetBestBagForItemDef(pDstEnt, pSrcItemDef, pSrcItem, -1, false, pExtract);
		if (iDstBagID == InvBagIDs_Inventory && item_IsUpgrade(pSrcItemDef))
		{
			ClientCmd_NotifySend(pEnt, kNotifyType_ItemMoveFailed, entTranslateMessageKey(pEnt, "InventoryUI.CantEquipAnyMore"), NULL, NULL);
			return;
		}
	...
	{
		ClientCmd_NotifySend(pEnt, kNotifyType_ItemMoveFailed, entTranslateMessageKey(pEnt, "InventoryUI.CantEquipUnidentified"), NULL, NULL);
		return;
	}
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_CATEGORY(Standard, Inventory) ACMD_PRIVATE;
void ItemRewardPack_OpenWithChoices(Entity *pEnt,S32 iSrcType, U32 iSrcID, S32 iSrcBagID, S32 iSrcSlot, ContactRewardChoices *pChoices)
{
	GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
	Entity* pSrcEnt = entity_GetSubEntity(entGetPartitionIdx(pEnt), pEnt, iSrcType, iSrcID);
	Item* pSrcItem = inv_GetItemFromBag(pSrcEnt, iSrcBagID, iSrcSlot, pExtract);
	ItemDef *pDef = pSrcItem ? GET_REF(pSrcItem->hItem) : NULL;

	if(pDef && pSrcEnt)
		ItemOpenRewardPackWithChoices(entGetPartitionIdx(pEnt),pSrcEnt,pSrcItem,pDef,iSrcBagID,iSrcSlot,pChoices->ppItemNames);
}
						  ...
						  TransactionReturnCallback cbFunc, void* cbData)
{
	...
	if (bCheckDiscard && !item_CanDiscard(pEnt, pItemDef, true))
		return false;
...
void item_RemoveFromBagForEnt(Entity *pPlayerEnt, S32 iType, U32 iID, S32 BagID, S32 iSlot, S32 iCount)
{
	...
	if (!item_CanDiscard(pPlayerEnt, pItemDef, true))
	{
		return;
	}
...
AUTO_COMMAND ACMD_SERVERONLY ACMD_ACCESSLEVEL(5) ACMD_CATEGORY(Standard, Inventory) ACMD_CACHE_AUTOCOMPLETE;
...
static void EquipItemInternal(Entity* pEnt, ItemDef* pItemDef, U64 uItemID, InvBagIDs iSrcBagID, int iSrcSlot, bool bAutoUse, GameAccountDataExtract *pExtract)
{
	if (pEnt && pItemDef) {
		...
		if (uItemID) {
			pItem = inv_GetItemAndSlotsByID(pEnt, uItemID, &iSrcBagID, &iSrcSlot);
			...
			if (i < iNumBags) {
				pItem = inv_GetItemFromBag(pEnt, iSrcBagID, iSrcSlot, pExtract);
			}
		} else if (iSrcSlot < 0) {
		...
		iDstBagID = GetBestBagForItemDef(pEnt, pItemDef, pItem, -1, false, pExtract);
...
void RemoveItem(Entity *pEnt, ACMD_NAMELIST("ItemDef", REFDICTIONARY) char *pchItemName)
{
	...
	{	
		...
		if (!item_CanDiscard(pEnt,pItemDef, true))
			return;
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_SERVERONLY;
void costumeGetDyePackNames(Entity *e)
{
	RefDictIterator iter = {0};
	ItemDef *pDef;
	CostumeEditDyePackStructList *dyePackNameList;

	dyePackNameList = (CostumeEditDyePackStructList*)malloc(sizeof(CostumeEditDyePackStructList*));
	dyePackNameList->eaDyePack = NULL;

	RefSystem_InitRefDictIterator(g_hItemDict, &iter);

	eaCreate(&(dyePackNameList->eaDyePack));
	...
	while(pDef = (ItemDef*)RefSystem_GetNextReferentFromIterator(&iter))
	{
		if(pDef->eType == kItemType_DyePack)
		{
			...
			eaPush(&dyePackNameList->eaDyePack, pDyePack);
		}
	}
	ClientCmd_costumeGiveDyePackNames(e, dyePackNameList);
	eaDestroy(&dyePackNameList->eaDyePack);
	free(dyePackNameList);
}

AUTO_COMMAND ACMD_SERVERCMD ACMD_SERVERONLY;
void costumeGetAndApplyDyePack(Entity *e, const char *DyePack, char *pName)
{
	RefDictIterator iter = {0};
	ItemDef *pDef;
	RefSystem_InitRefDictIterator(g_hItemDict, &iter);
	while(pDef = (ItemDef*)RefSystem_GetNextReferentFromIterator(&iter))
	{
		if(pDef->eType == kItemType_DyePack)
		{
			if (!strcmp(pDef->pchName, DyePack))
			{
				ClientCmd_costumeApplyDyePack(e, pDef, pName);
			}
		}
	}
}
