...
typedef struct SCPPetStateCBData
{
	...
	int containerID;
	int iPetIdx;
	int bRefresh;	//this means refresh the pets stats in UI.
}SCPPetStateCBData;
...
enumTransactionOutcome scp_trh_SetActivePetLevel(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int iPetIdx, U32 uiNewLevel, int iCostPaid, char bSetXP, GameAccountDataExtract* pExtract)
{
	...
	if (pPetItem && pItemDef && pPetItem->pSpecialProps && pPetItem->pSpecialProps->pSuperCritterPet && scp_LevelIsValid(uiNewLevel, CONTAINER_RECONST(Item, pPetItem)))
	{
		...
		NOCONST(EntitySavedSCPData)* pSCPData = scp_trh_GetOrCreateEntSCPDataStruct(ATR_PASS_ARGS, pEnt, true);
		if (!pPetDef)
		{
		...
		// Pet may have just finished training; clear its timer:
		if (pSCPData && iPetIdx >=0 && iPetIdx < eaSize(&pSCPData->ppSuperCritterPets) )
		{
			pSCPData->ppSuperCritterPets[iPetIdx]->uiTimeFinishTraining = 0;
		}
		else 
		{
			return TRANSACTION_OUTCOME_FAILURE;
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Pinventoryv2.Ppinventorybags[], pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[], .Psaved.Pscpdata, .Pplayer.Playertype");
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Pinventoryv2.Ppinventorybags[], pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Pchar.Ilevelexp, .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pplayer.Eaplayernumericthresholds");
enumTransactionOutcome scp_tr_SetActivePetQuality(ATR_ARGS, NOCONST(Entity)* pEnt, int iPetIdx, int iQuality, int iCost, ItemChangeReason* pReason, GameAccountDataExtract* pExtract)
{
	if (ISNULL(pEnt) || ISNULL(pEnt->pSaved))
		return TRANSACTION_OUTCOME_FAILURE;
	else
	{
		...
		{
			//can't afford it, fail.
			QueueRemoteCommand_notify_RemoteSendNotification(ATR_RESULT_FAIL, 0, 0, "SuperCritterPets.InsufficientCurrencyUpgrade", kNotifyType_ItemRequired);
			TRANSACTION_RETURN_LOG_FAILURE("FAILED to pay pet quality upgrade cost of %i %s", iCost, pDef->pchName);
		}
		else
		{
			NOCONST(Item)* pPetItem = inv_trh_GetItemFromBag(ATR_PASS_ARGS, pEnt, 35 /* Literal InvBagIDs_SuperCritterPets */, iPetIdx, pExtract);
			RewardValTable* pXPTable = GET_REF(g_SCPConfig.hRequiredXPTable);
			int iOldQuality = item_trh_GetQuality(pPetItem);

			if(iOldQuality == iQuality)
			{
				// this does nothing.  Probably means the transaction got spammed.
				return TRANSACTION_OUTCOME_FAILURE;
			}

			// We don't want to count any experience this pet has earned beyond his max level at the old quality.
			// if it has more XP than that, reduce it to that:
			if (pXPTable && iOldQuality >= 0 && iOldQuality < eafSize(&g_SCPConfig.eaiMaxLevelsPerQuality))
			{
				U32 iMaxXPForOldQuality = pXPTable->Val[USER_TO_TAB_LEVEL(g_SCPConfig.eaiMaxLevelsPerQuality[iOldQuality])];

				if(pPetItem->pSpecialProps->pSuperCritterPet->uXP > iMaxXPForOldQuality)
				{
					pPetItem->pSpecialProps->pSuperCritterPet->uXP = iMaxXPForOldQuality;
				}
			}
			else
			{
				Errorf("Super Critter Pet has a bad quality before SetActivePetQuality()");
			}


			item_trh_SetAlgoPropsQuality(pPetItem, iQuality);
			QueueRemoteCommand_notify_RemoteSendNotification(ATR_RESULT_SUCCESS, 0, 0, "SuperCritterPets.UpgradeSuccessful", kNotifyType_SuperCritterPet_Upgraded);
			return TRANSACTION_OUTCOME_SUCCESS;
		}
	}
	return TRANSACTION_OUTCOME_FAILURE;
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Pinventoryv2.Ppinventorybags[], pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics, .Pchar.Ilevelexp, .Hallegiance, .Hsuballegiance, .Pplayer.Eaplayernumericthresholds, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Pscpdata, .Pplayer.Playertype");
...
AUTO_TRANS_HELPER
	ATR_LOCKS(pEnt, ".Pinventoryv2.Ppinventorybags[], .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Hallegiance, .Hsuballegiance, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds");
enumTransactionOutcome scp_trh_RenamePet(ATR_ARGS, ATH_ARG NOCONST(Entity)* pEnt, int iBag, int iPetIdx, const char* pchNewName, ItemChangeReason* pReason, GameAccountDataExtract* pExtract)
{
	...
	else
	{
		...
		if (UTF8GetLength(pchNewName) > 24)	//hard coded pet name character length limit
		{
...
AUTO_TRANSACTION
ATR_LOCKS(pPlayerEnt, ".Pplayer.Playertype, .Psaved.Ppallowedcritterpets, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Pscpdata, pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppuppetmaster.Curtempid, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
enumTransactionOutcome scp_tr_BindPet(ATR_ARGS, NOCONST(Entity)* pPlayerEnt, int iDst, int iDstSlot, int iSrc, int iSrcSlot, int iBackupDstBag, const char* pchNewName)
{
	...
	NOCONST(SuperCritterPet)* pSrcPet = SAFE_MEMBER2(pItem, pSpecialProps, pSuperCritterPet);
	NOCONST(SuperCritterPet)* pDstPet = SAFE_MEMBER2(pDstItem, pSpecialProps, pSuperCritterPet);
	SuperCritterPetDef* pSrcSCPDef = pSrcPet ? GET_REF(pSrcPet->hPetDef) : NULL;
	SuperCritterPetDef* pDstSCPDef = pDstPet ? GET_REF(pDstPet->hPetDef) : NULL;
	...
	int iNumEquipSlots = pSrcSCPDef ? eaSize(&pSrcSCPDef->eaEquipSlots) : 0;
	...
	if (!pSrcPet || !(pItemDef->eType == kItemType_SuperCritterPet))
		TRANSACTION_RETURN_LOG_FAILURE("scp_tr_BindPet called with a non-pet Item %s", REF_STRING_FROM_HANDLE(pItem->hItem));

	if (!pSrcSCPDef)
		TRANSACTION_RETURN_LOG_FAILURE("scp_tr_BindPet called with an SCP that has an invalid or missing def: %s", REF_STRING_FROM_HANDLE(pSrcPet->hPetDef));

	// check if it is valid to move the SCP into this bag due to uniqueness
	if (!scp_trh_CanMoveSCPCheckSwap(ATR_PASS_ARGS, pPlayerEnt, pSrcSCPDef, pDstSCPDef, pSrcBag, pDstBag))
	{
		QueueRemoteCommand_notify_RemoteSendNotification(ATR_RESULT_FAIL, 0, 0, "SuperCritterPets.UniqueActive", kNotifyType_DuplicateUniqueItem);
		TRANSACTION_RETURN_LOG_FAILURE("Bind pet failed because the unique SCP in the active bag check failed.");
		return TRANSACTION_OUTCOME_FAILURE;
	}
	...
	if(pSrcSCPDef->bLevelToPlayer && pSrcPet->uXP == 0)
	{
		...
		pSrcPet->uLevel = newLevel;
		pSrcPet->uXP = scp_GetTotalXPRequiredForLevel(newLevel, CONTAINER_RECONST(Item, pItem));
	}


	// rename the pet
	if(pchNewName && pchNewName[0]){
		if(scp_trh_RenamePet(ATR_PASS_ARGS, pPlayerEnt, iSrc, iSrcSlot, pchNewName, NULL, NULL) == TRANSACTION_OUTCOME_FAILURE)
		{
			//had an invalid name, return failure (but if name was valid keep going, this is because it is hard to get dest slot after the move)
			return TRANSACTION_OUTCOME_FAILURE;
		}
	}
	...
	if (iDstSlot >= 0 && inv_bag_trh_MoveItem(ATR_PASS_ARGS, true, pPlayerEnt, NULL, pDstBag, iDstSlot, pPlayerEnt, NULL, pSrcBag, iSrcSlot, 1, false, false, NULL, NULL, NULL))
	{
		...
		TRANSACTION_APPEND_LOG_TO_CATEGORY_WITH_NAME_SUCCESS(LOG_PETS, "SCPBind", "PetItemDef %s PetLevel %i", pSrcSCPDef->pchName, pSrcPet->uLevel);
		return TRANSACTION_OUTCOME_SUCCESS;
	}
...
AUTO_TRANSACTION
ATR_LOCKS(pPlayerEnt, ".Psaved.pSCPdata, pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], pInventoryV2.ppInventoryBags[], .Pplayer.Playertype, .Psaved.Ppuppetmaster.Curtempid, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds, .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
enumTransactionOutcome scp_tr_UnbindPet(ATR_ARGS, NOCONST(Entity)* pPlayerEnt, int iSrc, int iSrcSlot, int iCost, const ItemChangeReason* pReason)
{
	...
	{
		int iNumEquipSlots = eaSize(&pPetDef->eaEquipSlots);
...
//move an item between a pet's equip slot and a different type of bag.
AUTO_TRANSACTION
ATR_LOCKS(pPlayerEnt, ".pInventoryV2.Ppinventorybags[], .Psaved.pSCPData, .Pplayer.Playertype, .Psaved.Ppuppetmaster.Curtype, .Hallegiance, .Hsuballegiance, .Pchar.Ilevelexp, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Psaved.Uiindexbuild, .Psaved.Ppbuilds, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Psaved.Conowner.Containerid, .Psaved.Conowner.Containertype, .Pinventoryv2.Peaowneduniqueitems, .Psaved.Ppownedcontainers, .Itemidmax, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, pInventoryV2.ppLiteBags[], .Pplayer.Eaastrrecentlyacquiredstickerbookitems");
	...
	if (NONNULL(pPlayerEnt) && NONNULL(pPlayerEnt->pSaved))
	{
		...
		if (NONNULL(pPet) && NONNULL(pPet->pEquipment))
		{
			if (bEquip && !scp_trh_IsEquipSlotLocked(ATR_PASS_ARGS, pPlayerEnt, iPet, iEquipSlot))
			{
				...
				{
					NOCONST(Item)* pItem = inv_bag_trh_GetItem(ATR_PASS_ARGS, pPetEquipBag, iEquipSlot);
					ItemDef* pItemDef = GET_REF(pItem->hItem);
...
// This is called from the transaction that finishes pet training.  It resummons your pet if you
// there isn't one summoned, and builds a notification that tells you what your pet gained.
static void scp_PetTrainingFinishedCB(TransactionReturnVal *pReturnVal, PetTrainingCBData *pData)
{
	//note: There were some crashes here from entFromEntityRefAnyPartition returning NULL.  
	// I think the player logged out/map moved after the transaction was queued.
	Entity *pEnt = entFromContainerIDAnyPartition(pData->containerType, pData->containerID);
	...
	if (pSCPData)
	{
		...
		{
			int i;
			U32 uNewLevel = scp_GetPetCombatLevel(scp_GetActivePetItem(pEnt, pData->iPetIdx));
			...
			{
				scp_SetPetState(pEnt, pData->iPetIdx, true,  0, false);
			}

			for (i = 0; i < eaiSize(&g_SCPConfig.eaiEquipSlotUnlockLevels); i++)
			{
				if (pData->uOldLevel < g_SCPConfig.eaiEquipSlotUnlockLevels[i] &&
					uNewLevel >= g_SCPConfig.eaiEquipSlotUnlockLevels[i])
					iNewEquipSlots++;
			}
			for (i = 0; i < eaiSize(&g_SCPConfig.eaiGemSlotUnlockLevels); i++)
			{
				if (pData->uOldLevel < g_SCPConfig.eaiGemSlotUnlockLevels[i] &&
					uNewLevel >= g_SCPConfig.eaiGemSlotUnlockLevels[i])
					iNewGemSlots++;
			}
			entFormatGameMessageKey(pEnt, &estrNotification, "SuperCritterPets.TrainingComplete",
				...
				STRFMT_INT("LevelDelta", uNewLevel - pData->uOldLevel),
				STRFMT_INT("NewLevel", uNewLevel),
				STRFMT_END);
		...
		{
			Errorf("Failed to train pet %s from level %i to %i!", scp_GetActivePetDefName(pEnt, pData->iPetIdx), pData->uOldLevel, pData->uNewLevel);
		}
...
enumTransactionOutcome scp_tr_MoveBoundPet(ATR_ARGS, NOCONST(Entity)* pPlayerEnt, int iDst, int iDstSlot, int iSrc, int iSrcSlot)
{
	...
	{	
		...
		NOCONST(SuperCritterPet)* pSrcPet = SAFE_MEMBER2(pSrcItem, pSpecialProps, pSuperCritterPet);
		SuperCritterPetDef* pSrcSCPDef = pSrcPet ? GET_REF(pSrcPet->hPetDef) : NULL;
		int iEquipSlots = pSrcSCPDef ? eaSize(&pSrcSCPDef->eaEquipSlots) : 0;
		// Get the dest pet 
		NOCONST(SuperCritterPet)* pDstPet = SAFE_MEMBER2(pDstItem, pSpecialProps, pSuperCritterPet);
		SuperCritterPetDef* pDstSCPDef = pDstPet ? GET_REF(pDstPet->hPetDef) : NULL;
...
Entity* scp_CreateCritterFromPet( Entity *pOwner, ActiveSuperCritterPet* pActivePet, Item* pPetItem, int iPetIdx)
{
	if (pOwner && pPetItem && pPetItem->pSpecialProps && pPetItem->pSpecialProps->pSuperCritterPet)
	{
		...
		SCPAltCostumeDef * pSCPCostumeDef = scp_GetPetCostumeDef(pPetItem);
		...
		createParams.pCostume = pSCPCostumeDef ? GET_REF(pSCPCostumeDef->hCostume) : NULL;
		...
		if (pSCPCostumeDef && IS_HANDLE_ACTIVE(pSCPCostumeDef->hContinuingPlayerFX))
		{
			pchFXName = REF_STRING_FROM_HANDLE(pSCPCostumeDef->hContinuingPlayerFX);
		}
...
void scp_SummonPetInternal(Entity *pPlayerEnt, int idx, bool bNotify)
{
	EntitySavedSCPData* pSCPData = scp_GetEntSCPDataStruct(pPlayerEnt);
	ActiveSuperCritterPet* pPetInfo = pSCPData->ppSuperCritterPets[idx];
	...
	if(entCheckFlag(pPlayerEnt, ENTITYFLAG_PLAYER_LOGGING_OUT) || entCheckFlag(pPlayerEnt, ENTITYFLAG_IGNORE))
	{
		//player is logging in or out.  Do not summon a pet because pSCPData->erSCP will not be reliably destroyed.
		return;
	}


	if(pRegionRules->iAllowedPetsPerPlayer == 0 && pRegionRules->iUnteamedPetsPerPlayer <= 0)
	{
		if(bNotify)
		{
	...
	pSCPData->bIsSummoningPet = true;
	pPetEnt = scp_CreateCritterFromPet(pPlayerEnt, pPetInfo, pPetItem, idx);
	pSCPData->bIsSummoningPet = false;

	if (pPetEnt)
	{
		pSCPData->erSCP = pPetEnt->myRef;
...
void Entity_SuperCritterPetFixup(Entity* pEnt)
{
	...
	else if (pData->iSummonedSCP > -1)
	{
		...
		{
			scp_SummonPetInternal(pEnt, pData->iSummonedSCP, true);
		}
	}
}
...
void gslSCPPlayerRespawn(Entity* pPlayerEnt)
{
	...
	if (pData)
	{
		if (pData->iSummonedSCP >= 0 && pData->erSCP == 0)
		{
			...
			scp_SummonPetInternal(pPlayerEnt, pData->iSummonedSCP, false);
		}
	}
}
...
AUTO_TRANSACTION
	ATR_LOCKS(pEnt, ".Psaved.Pscpdata, .Pplayer.Playertype, pInventoryV2.ppInventoryBags[], pInventoryV2.ppLiteBags[], pPlayer.pPlayerAccountData.eaGamePermissionMaxValueNumerics[]");
enumTransactionOutcome scp_tr_ChangeSummonedPet(ATR_ARGS, NOCONST(Entity)* pEnt, int iPetIdx, U32 uTrainingCompletionTimestamp, int bSummoned, int bRevive)
{
	if (ISNULL(pEnt) || ISNULL(pEnt->pSaved))
		return TRANSACTION_OUTCOME_FAILURE;
	else
	{
		...
		if (bSummoned)
		{
			pSCPData->iSummonedSCP = iPetIdx;
		}
		else
			pSCPData->iSummonedSCP = -1;

		if (uTrainingCompletionTimestamp){
			pSCPData->ppSuperCritterPets[iPetIdx]->uiTimeFinishTraining = uTrainingCompletionTimestamp;
		}
	}
	return TRANSACTION_OUTCOME_SUCCESS;
}
...
// This callback is passed to transactions that can change the state of pets.
// It makes sure the right pet is summoned and refreshes client UI if needed.
void scp_PetStateCB(TransactionReturnVal* returnVal, SCPPetStateCBData* pData)
{
	...
	if(pSCPData)
	{
		if(returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
		{
			...
			if(pData->iPetIdx == pSCPData->iSummonedSCP )
			{
				// This affected the summoned pet, so we should refresh it.  Note that this will reset it's HP, etc.
				scp_SummonPetInternal(pPlayerEnt, pSCPData->iSummonedSCP, true);
			}
			else if (pSCPData->iSummonedSCP == -1 && pSCPData->erSCP)
			{
				// the transaction unsummoned the pet.  Destroy its entity and clear the ref:
				gslQueueEntityDestroy(entFromEntityRef(entGetPartitionIdx(pPlayerEnt), pSCPData->erSCP));
				pSCPData->erSCP = 0;
			}
			// 
			if( pData->bRefresh)
			{
				ClientCmd_scp_InvalidateFakeEntities(pPlayerEnt);
			}
...
//Equip or unequip an item to/from a pet's equip slot.  Moving between slots is scp_MovePetEquipmentToPet().
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(0) ACMD_PRIVATE;
void scp_MovePetEquipment(Entity* pPlayerEnt, int iPet, int iEquipSlot, int iSrcBagID, int iSrcBagSlot, bool bEquip)
{
	TransactionReturnVal* pVal = NULL;
	EntityRef* pRef = NULL;
	GameAccountDataExtract* pExtract = entity_GetCachedGameAccountDataExtract(pPlayerEnt);
	Item* pPetItem = inv_GetItemFromBag(pPlayerEnt, InvBagIDs_SuperCritterPets, iPet, pExtract);
	SuperCritterPet *pPet;
	Item* pItem = inv_GetItemFromBag(pPlayerEnt, iSrcBagID, iSrcBagSlot, pExtract);
	...
	AutoTrans_scp_tr_MovePetEquipment(pVal, GetAppGlobalType(), pPlayerEnt->myEntityType, pPlayerEnt->myContainerID, iPet, iEquipSlot, iSrcBagID, iSrcBagSlot, bEquip);
...
static void scp_resetPetEntInventory(Entity* pPlayerEnt, Entity* pPetEnt, ActiveSuperCritterPet* pActivePet, int idx)
{
	if(pPetEnt && pPetEnt->pChar && IS_HANDLE_ACTIVE(pPetEnt->pChar->hClass))
	{
		...
		inv_bag_trh_AddItem(ATR_EMPTY_ARGS, pNCPetEnt, NULL, pPetBag, -1, StructCloneDeConst(parse_Item, pPetItem), NULL, -1, 0, NULL, NULL, NULL);
		{
			...
			MultiVal mvIntVal = {0};
			MultiValSetInt(&mvIntVal, item_GetQuality(pPetItem));
...
void scp_SetPetState(Entity* pPlayerEnt, int iPetIdx, bool bSummoned, U32 uTrainingCompletionTimestamp, bool bFromClient)
{
	EntitySavedSCPData* pSCPData = scp_GetEntSCPDataStruct(pPlayerEnt);
	...
	else
	{
		...
		AutoTrans_scp_tr_ChangeSummonedPet(pVal, GetAppGlobalType(), pPlayerEnt->myEntityType, pPlayerEnt->myContainerID, iPetIdx, uTrainingCompletionTimestamp, bSummoned, false);
	}
}
...
{
	EntitySavedSCPData* pSCPData = scp_GetEntSCPDataStruct(pPlayerEnt);
	if (pSCPData && pSCPData->iSummonedSCP > -1)
	{
		scp_SetPetState(pPlayerEnt, pSCPData->iSummonedSCP, false, 0, false);
...
{
	scp_SetPetState(pPlayerEnt, iPetIdx, bSummoned, 0, true);
}
...
void scp_PetReviveCB(TransactionReturnVal* returnVal, SCPPetStateCBData* pData)
{
	...
	if(pPlayerEnt)
	{
		if(returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
		{
			...
			AutoTrans_scp_tr_ChangeSummonedPet(pVal, GetAppGlobalType(), pPlayerEnt->myEntityType, pPlayerEnt->myContainerID, pData->iPetIdx, 0, true, true);
			return;
		}
	}
	SAFE_FREE(pData);
...
void scp_SendActivePetForTraining(Entity* pPlayerEnt, int idx)
{
	...
	S32 iLevelDelta = 0;
	EntitySavedSCPData* pData = scp_GetEntSCPDataStruct(pPlayerEnt);
	U32 uTrainingCompletionTimestamp;
	...
	uTrainingCompletionTimestamp = timeSecondsSince2000() + scp_EvalTrainingTime(pPlayerEnt, pPetItem);

	scp_SetPetState(pPlayerEnt, idx, false, uTrainingCompletionTimestamp, false);
...
void scp_CheckForFinishedTraining(Entity* pPlayerEnt)
{
	...
	if (pSCPData && (GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER || pSCPData->bTrainingActive) && eaSize(&pSCPData->ppSuperCritterPets) > 0)
	{
		int i;
		GameAccountDataExtract* pExtract = entity_GetCachedGameAccountDataExtract(pPlayerEnt);
		TransactionReturnVal* pVal = NULL;
		...
		for (i = 0; i < eaSize(&pSCPData->ppSuperCritterPets); i++)
		{
				...
				pSCPData->ppSuperCritterPets[i]->uiLastLevelUpTransactionRequest+300 <= timeSecondsSince2000())
			{
				...
				pData->containerID = pPlayerEnt->myContainerID;
				pData->iPetIdx = i;
				pData->uOldLevel = scp_GetPetCombatLevel(pPetItem);
				pData->uNewLevel = scp_GetPetLevelAfterTraining(pPetItem);
				...
				AutoTrans_scp_tr_SetActivePetLevel(pVal, GetAppGlobalType(), pPlayerEnt->myEntityType, pPlayerEnt->myContainerID, i, pData->uNewLevel, false, pExtract);
			}
		}
	}
...
void scp_RushTraining(Entity* pPlayerEnt, S32 iIndex)
{
	...
	if (pSCPData && eaSize(&pSCPData->ppSuperCritterPets) > iIndex && iIndex >= 0)
	{
		...
		if (pSCPData->ppSuperCritterPets[iIndex]->uiTimeFinishTraining > 0)
		{
			...
			pData->containerType = pPlayerEnt->myEntityType;
			pData->containerID = pPlayerEnt->myContainerID;
			pData->iPetIdx = iIndex;
			pData->uOldLevel = scp_GetPetCombatLevel(pPetItem);
			pData->uNewLevel = scp_GetPetLevelAfterTraining(pPetItem);

			inv_FillItemChangeReason(&reason, pPlayerEnt, "SuperCritterPet rush training", "UseNumeric");
			pVal = LoggedTransactions_CreateManagedReturnValEnt("scp_tr_SetActivePetLevel", pPlayerEnt, scp_PetTrainingFinishedCB, pData);
			AutoTrans_scp_tr_RushSetActivePetLevel(pVal, GetAppGlobalType(), pPlayerEnt->myEntityType, pPlayerEnt->myContainerID, iIndex, pData->uNewLevel, iCost, &reason, pExtract);
		}
	}
}

// Increase a pet's quality.
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(0) ACMD_PRIVATE;
void scp_UpgradePetQuality(Entity* pPlayerEnt, S32 iIndex)
{
	EntitySavedSCPData* pSCPData = scp_GetEntSCPDataStruct(pPlayerEnt);
	if (pSCPData && eaSize(&pSCPData->ppSuperCritterPets) > iIndex && iIndex >= 0)
	{
		Item* pPetItem = scp_GetActivePetItem(pPlayerEnt, iIndex);
		int iCurQuality = item_GetQuality(pPetItem);

		//if upgrading will stay under max quality && pet is at max level for this quality
		if(iCurQuality + 1 <= g_SCPConfig.iMaxUpgradeQuality && g_SCPConfig.eaiMaxLevelsPerQuality[iCurQuality] == scp_GetPetCombatLevel(pPetItem))
		{
			GameAccountDataExtract* pExtract = entity_GetCachedGameAccountDataExtract(pPlayerEnt);
			TransactionReturnVal* pVal = NULL;

			ItemChangeReason reason = {0};
			SCPPetStateCBData* pData = calloc(1, sizeof(SCPPetStateCBData));
			int iCost = scp_GetQualityUpgradeCost(pPlayerEnt, iIndex);
			pData->containerType = pPlayerEnt->myEntityType;
			pData->containerID = pPlayerEnt->myContainerID;
			pData->iPetIdx = iIndex;
			pData->bRefresh = true;

			inv_FillItemChangeReason(&reason, pPlayerEnt, "SuperCritterPet upgrading quality", "UseNumeric");
			pVal = LoggedTransactions_CreateManagedReturnValEnt("scp_UpgradePetQuality", pPlayerEnt, scp_PetStateCB, pData);
			AutoTrans_scp_tr_SetActivePetQuality(pVal, GetAppGlobalType(), pPlayerEnt->myEntityType, pPlayerEnt->myContainerID, iIndex, iCurQuality + 1, iCost, &reason, pExtract);
		}
	}
}

//set summoned pet's quality.  Should be used only for debugging and testing.
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(7) ACMD_NAME(SetPetQuality);
void scp_SetPetQuality(Entity* pPlayerEnt, int iQuality)
{
	...
	if (pSCPData)
	{
		...
		if (iIndex <0)
		{
			notify_NotifySend(pPlayerEnt, kNotifyType_Failed, "This operates on the currently summoned companion.", NULL, NULL);
			return;
		}

		//if upgrading will stay under max quality && pet is at max level for this quality
		if(iQuality <= g_SCPConfig.iMaxUpgradeQuality && iQuality >= 0)
		{
			GameAccountDataExtract* pExtract = entity_GetCachedGameAccountDataExtract(pPlayerEnt);
			TransactionReturnVal* pVal = NULL;
			...
			pVal = LoggedTransactions_CreateManagedReturnValEnt("scp_UpgradePetQuality", pPlayerEnt, scp_PetStateCB, pData);
			AutoTrans_scp_tr_SetActivePetQuality(pVal, GetAppGlobalType(), pPlayerEnt->myEntityType, pPlayerEnt->myContainerID, iIndex, iQuality, 0, NULL, pExtract);
		}
		else
		{
			notify_NotifySend(pPlayerEnt, kNotifyType_Failed, "Bad quality value. Try also: SetPetQuality, PetSetLevel, PetLevelToMe", NULL, NULL);
		}
	}
}
...
void scp_SetPetLevel(Entity* pPlayerEnt, S32 iLevel)
{
	...
	if (pSCPData)
	{
		...
		if (!scp_LevelIsValid(iLevel, pPetItem))
		{
			...
			notify_NotifySend(pPlayerEnt, kNotifyType_Failed, "Bad level, setting to maximum. Try also: SetPetQuality, PetSetLevel, PetLevelToMe", NULL, NULL);
			if (quality >= 0 && quality < eafSize(&g_SCPConfig.eaiMaxLevelsPerQuality))
			{
				iLevel = g_SCPConfig.eaiMaxLevelsPerQuality[quality];	//max level for this pet.
			}
		}

		pVal = LoggedTransactions_CreateManagedReturnValEnt("scp_tr_SetActivePetLevel", pPlayerEnt, scp_PetStateCB, pData);
...
void scp_PetLevelToPlayerLevel(Entity* pPlayerEnt)
{
	...
	if (pPetItem)
	{
		U32 appropriateLevel = scp_GetPetStartLevelForPlayerLevel(pPlayerEnt->pChar->iLevelExp, pPetItem);
		int i = 0;
		while ( i < eafSize(&g_SCPConfig.eaiMaxLevelsPerQuality) && g_SCPConfig.eaiMaxLevelsPerQuality[i] < appropriateLevel)
			i++;

		if (item_GetQuality(pPetItem) < i)
		{
			scp_SetPetQuality(pPlayerEnt, i);
		}
		// This can probably happen before SetQuality transaction resolves, but it's just a dev command...
		scp_SetPetLevel(pPlayerEnt, appropriateLevel);
	}
	else
	{
		notify_NotifySend(pPlayerEnt, kNotifyType_Failed, "This operates on the currently summoned companion.", NULL, NULL);
