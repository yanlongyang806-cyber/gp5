...
static bool store_ApplyDiscountToCostInfo(Entity* pEnt, StoreDef* pStoreDef, ItemDef* pItemDef, StoreItemCostInfo* pCostInfo)
{
	...
	if (pCurrencyDef) {
		int i;
		StoreDef *pDiscountStoreDef = NULL;
		...
		pDiscountStoreDef = GET_REF(pStoreDef->inheritDiscountsStoreDef);
		if (pDiscountStoreDef) {
			for (i = eaSize(&pDiscountStoreDef->eaDiscountDefs)-1; i >= 0; i--) {
				StoreDiscountDef* pDiscountDef = pDiscountStoreDef->eaDiscountDefs[i];
				ItemDef* pDiscountCostItemDef = GET_REF(pDiscountDef->hDiscountCostItem);
...
void Store_SellItem_CB(TransactionReturnVal *pReturn, StoreSellItemCBData *pData)
{
	...
	{
		GatewayVendor_SoldItemToVendor(
			wgsFindOwningSessionForDBContainer(GLOBALTYPE_ENTITYPLAYER, (intptr_t)pData->uEntID),
			pData,
			pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS);

		pEnt = entForClientCmd(pData->uEntID, pEnt);
	}
	...
	if(pEnt && pEnt->pPlayer)
	{
		...
		if (pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS && pEnt && pInteractInfo)
		{
			...
			// Update contact in case we meet requirements for different items now that we've gained money and lost an item
...
static void store_SellItemHelper(Entity* pEnt, S32 iBagID, S32 iSlot, S32 iCount, GlobalType iGlobalType, ContainerID iContainerID, ContactDef *pContactDef, bool bTrySellStore, GameAccountDataExtract *pExtract)
{
...
void store_SellItemNoDialog(Entity* pEnt, S32 iBagID, S32 iSlot, S32 iCount, GlobalType iGlobalType, ContainerID iContainerID, const char* pchContactDef)
{
	GameAccountDataExtract *pExtract;
...
void store_BuybackItem(Entity *pEnt, GlobalType iGlobalType, ContainerID iContainerID, SA_PARAM_OP_VALID StoreItemInfo* pStoreItem)
{
	int iPartitionIdx = entGetPartitionIdx(pEnt);
	Entity *pSource = iGlobalType == GLOBALTYPE_ENTITYSAVEDPET ? entity_GetSubEntity(iPartitionIdx,pEnt,iGlobalType,iContainerID) : NULL;
	InteractInfo *pInteractInfo = SAFE_MEMBER2(pEnt, pPlayer, pInteractInfo);
	...
	bagID = GetBestBagForItemDef(pEnt, pItemDef, pItem, iCount, true, pExtract);
...
void Store_BuyItem_CB(TransactionReturnVal *pReturn, StoreBuyItemCBData *pData)
{
	...
	InteractInfo *pInteractInfo = SAFE_MEMBER2(pEnt, pPlayer, pInteractInfo);
	// Gateway: pEnt and pInteractInfo will be NULL in Gateway. This is OK.

	if(GetAppGlobalType() == GLOBALTYPE_GATEWAYSERVER)
	{
		GatewayVendor_PurchasedVendorItem(
			wgsFindOwningSessionForDBContainer(GLOBALTYPE_ENTITYPLAYER, (intptr_t)pData->uEntID),
			pData,
			pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS);
	}
	...
	if (pReturn->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		if (pEnt)
		{
			...
			// Update contact in case we meet requirements for different items now that we've spent money and have a new item
			if(pInteractInfo)
			{
				pInteractInfo->bUpdateContactDialogOptionsNextTick = true;
			}
...
StoreDef *store_GetStoreDefFromEntAndName(Entity *pEnt, const char *pchStoreName)
{
	ContactDialog *pContactDialog = SAFE_MEMBER3(pEnt, pPlayer, pInteractInfo, pContactDialog);
	ContactDef *pContactDef = pContactDialog?GET_REF(pContactDialog->hContactDef):NULL;
	PersistedStore *pPersistStore = pContactDialog ? GET_REF(pContactDialog->hPersistedStore) : NULL;
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_CATEGORY(Standard, Inventory) ACMD_PRIVATE;
void store_BuyAndRemoveAll(Entity *pEnt, U32 uiTargetEntType, U32 uiTargetEntID)
{
	Entity* pTargetEnt = entity_GetSubEntity(entGetPartitionIdx(pEnt), pEnt, uiTargetEntType, uiTargetEntID);
	ItemDef *pCurrencyDef = NULL;
	const char *pcCurrencyName = NULL;
	...
	if (pEnt && pTargetEnt && pTargetEnt->pSaved)
	{
		TransactionReturnVal *pReturn = LoggedTransactions_CreateManagedReturnValEnt("ItemBuyAndRemoveAll", pEnt, NULL, NULL);
		ItemChangeReason reason = {0};
		GameAccountDataExtract *pExtract = entity_GetCachedGameAccountDataExtract(pEnt);
		ContactDialog *pContactDialog = SAFE_MEMBER3(pEnt, pPlayer, pInteractInfo, pContactDialog);
		StoreDef *pStoreDef = NULL;
		...
		// Calculate discounts
		for (i = eaSize(&pContactDialog->eaStoreItems) - 1; i >= 0; i--)
		{
			StoreItemInfo *pStoreItemInfo = pContactDialog->eaStoreItems[i];

			if (!pStoreDef)
				pStoreDef = store_GetStoreDefFromEntAndName(pEnt, pStoreItemInfo->pchStoreName);
			...
			if (pStoreDef)
			{
				...
				if (pStoreItemDef && pItemDef)
				{
					...
					{
						store_ApplyDiscountToCostInfo(pEnt, pStoreDef, pItemDef, pStoreItemInfo->eaCostInfo[j]);
					}
		...
		if (pEnt == pTargetEnt)
		{
			AutoTrans_tr_ItemBuyAndRemoveAll(pReturn, GLOBALTYPE_GAMESERVER, entGetType(pEnt),
				entGetContainerID(pEnt), GLOBALTYPE_ACCOUNTPROXYSERVER_LOCKS, 0, InvBagIDs_Injuries,
				pStoreDef, pContactDialog, pcCurrencyName, &reason, pExtract);
		}
		else
		{
			AutoTrans_tr_ItemBuyAndRemoveAllAcrossEnts(pReturn, GLOBALTYPE_GAMESERVER, entGetType(pEnt),
				entGetContainerID(pEnt), entGetType(pTargetEnt), entGetContainerID(pTargetEnt),
				GLOBALTYPE_ACCOUNTPROXYSERVER_LOCKS, 0, InvBagIDs_Injuries, pStoreDef,
				pContactDialog, pcCurrencyName, &reason, pExtract);
		}
	}
}
