...
AUTO_STRUCT;
typedef struct PetCommandMessage
{
	const char* messageName; AST(REQUIRED POOL_STRING STRUCTPARAM KEY NAME(Message))
	const char* aiMessage; AST(POOL_STRING NAME(AIMessage))
	F32 value; AST(DEFAULT(1) NAME(Value))
} PetCommandMessage;
...
typedef struct PetCommandConfig
{
	...
	PetCommandMessage** messages;					AST(NAME(Message))
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_PRIVATE ACMD_NAME(SubscribeAllPuppets);
void PetCommands_SubscribeAllPuppets(Entity* pOwner)
{	
	if(pOwner && pOwner->pSaved && pOwner->pSaved->pPuppetMaster && !pOwner->pSaved->pPuppetMaster->bPuppetSubscribeAll)
	{
		pOwner->pSaved->pPuppetMaster->bPuppetCheckPassed = false;
		pOwner->pSaved->pPuppetMaster->bPuppetSubscribeAll = true;
		pOwner->pSaved->bCheckPets = true;

		entity_SetDirtyBit(pOwner, parse_SavedEntityData, pOwner->pSaved, true);

	}
}

AUTO_COMMAND ACMD_ACCESSLEVEL(9) ACMD_SERVERCMD ACMD_NAME(ForceSubscribeAllPuppets);
void PetCommands_ForceSubscribeAllPuppets(Entity* pOwner)
{
	PetCommands_SubscribeAllPuppets(pOwner);
}

AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_PRIVATE ACMD_NAME(SubscribeAllPets);
void PetCommands_SubscribeAllPets(Entity* pOwner)
{	
	if(pOwner && pOwner->pSaved && pOwner->pSaved->pPuppetMaster && !pOwner->pSaved->bSubscribeAllPets)
	{
		pOwner->pSaved->pPuppetMaster->bPuppetCheckPassed = false;
		pOwner->pSaved->bCheckPets = true;
		pOwner->pSaved->bSubscribeAllPets = true;
		entity_SetDirtyBit(pOwner, parse_SavedEntityData, pOwner->pSaved, true);

	}
}

AUTO_COMMAND ACMD_ACCESSLEVEL(9) ACMD_SERVERCMD ACMD_NAME(ForceSubscribeAllPets);
void PetCommands_ForceSubscribeAllPets(Entity* pOwner)
{
	PetCommands_SubscribeAllPets(pOwner);
}

AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_PRIVATE;
void PetCommands_SendMessage(Entity* pOwner, EntityRef erPet, const char *pchMessage)
{
	PetCommandConfig *defaultConfig = GET_REF(defaultConfigRef);
	PetCommandMessage *message;
	Entity *pPetEnt;

	if (!pOwner || !defaultConfig)
		return;

	pPetEnt = entFromEntityRef(entGetPartitionIdx(pOwner), erPet);
	if (!pPetEnt || !pPetEnt->aibase)
		return;

	if (entGetRef(pOwner) != pPetEnt->erOwner)
		return;

	message = eaIndexedGetUsingString(&defaultConfig->messages, pchMessage);
	if (!message)
		return;

	if (message->aiMessage)
		pchMessage = message->aiMessage;

	aiMessageSendEntToEnt(pOwner, pOwner->aibase, erPet, pchMessage, message->value, NULL, 0, NULL, NULL);
}
