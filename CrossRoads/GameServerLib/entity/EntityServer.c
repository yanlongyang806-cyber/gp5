...
void gslExternPlayerLogin(Entity *ent)
{
	...
	if (ent)
	{
		...
		gslItemAssignment_CheckRankSchedules(ent);
	}
...
void entConRecharge(Entity* e, U32 uiID, F32 fTime)
{
	if(e && e->pChar)
	{
		character_PowerSetRechargeByID(entGetPartitionIdx(e),e->pChar,uiID,fTime);
	}
}
...
void entConRefresh(Entity* e)
{
	if(e && e->pChar)
	{
		int iPartitionIdx = entGetPartitionIdx(e);
		int iCategory;
		character_PowerSetRechargeByID(iPartitionIdx,e->pChar, 0, 0);
