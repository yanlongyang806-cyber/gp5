...
void costume_ChangeCostumeCmd(Entity *pEnt, ACMD_NAMELIST("PlayerCostume", REFDICTIONARY)  char *costumeName)
{
	...
	// for sto the space costume must be changed on the puppet
	if(pEnt && pEnt->pSaved && pEnt->pSaved->pPuppetMaster && GetCharacterClassEnum(pEnt) == StaticDefineIntGetInt(CharClassTypesEnum, "Space"))
	{
		S32 i;
		for (i = 0; i < eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets); i++)
		{
			if(pEnt->pSaved->pPuppetMaster->ppPuppets[i]->curID == pEnt->pSaved->pPuppetMaster->curID)
			{
	...
	costumetransaction_ChangePlayerCostume(pEnt, pCostume, pOwner);
}
...
void costume_StorePlayerCostumeCmd(Entity *pEnt, PCCostumeStorageType eCostumeType, U32 uContainerID, int iIndex, PlayerCostume *pCostume, const char *pcSlotType, PCPaymentMethod ePayMethod)
{
	...
	// Ground puppets never have data costume saved to them
	if (pEnt == pOtherEnt && pEnt->pSaved->pPuppetMaster)
	{
		for (i = 0; i < eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets); i++)
		{
			...
			{
				// do not store into captain
				if(GetCharacterClassEnum(GET_REF(pPuppetEnt->hEntityRef))!= StaticDefineIntGetInt(CharClassTypesEnum, "Ground"))
				{
