...
AUTO_TRANS_HELPER;
NOCONST(PuppetEntity) *Entity_FixupFindActiveSpacePuppet(ATH_ARG NOCONST(Entity) *pEnt)
{
	static U32 eSpace = 0;
	int i;
	if (!eSpace)
...
bool Entity_FixupAlwaysProps(ATH_ARG NOCONST(Entity) *pEnt, AlwaysPropSlotDefRefs* pPropSlotRefs)
{
	...
	if (NONNULL(pEnt->pSaved->pPuppetMaster))
	{
		...
		for (i = 0; i < eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets); i++)
		{
			NOCONST(PuppetEntity) *pActivePuppetEntity = pEnt->pSaved->pPuppetMaster->ppPuppets[i];
			if (pActivePuppetEntity->eState == PUPPETSTATE_ACTIVE && pActivePuppetEntity->eType == eSpace)
			{
				S32 iPuppetSlotCount = eaSize(&pActivePuppetEntity->ppSavedPropSlots);

				if (!uiActivePuppetID)
				{
					uiActivePuppetID = pActivePuppetEntity->curID;
				}

				for (j = 0; j < iPuppetSlotCount; j++)
				{
					NOCONST(AlwaysPropSlot)* pSlot = pActivePuppetEntity->ppSavedPropSlots[j];
					bool bFound = (pSlot->iPuppetID == pActivePuppetEntity->curID);

					// Check to see if this prop slot is pointing to an actual puppet
					if (!bFound && pSlot->iPuppetID)
					{
						NOCONST(PuppetEntity)* pPuppet = NULL;
						for (k = 0; k < eaSize(&pEnt->pSaved->pPuppetMaster->ppPuppets); k++)
						{
							pPuppet = pEnt->pSaved->pPuppetMaster->ppPuppets[k];
							if (pPuppet->curID == pSlot->iPuppetID)
							{
								bFound = true;
								break;
							}
					...
					if (!bFound)
					{
						pSlot->iPuppetID = pActivePuppetEntity->curID;
					}
				}
			}
		}
	...
	for (i = 0; i < eaSize(&pPropSlotRefs->eaRefs); i++)
	{
		...
		while (iCount < pRef->iCount)
		{
			...
			pNewSlot = APS_NewAlwaysPropSlot(pDef,uiSlotID,pRef->uiPuppetID);
							  ...
							  GameAccountDataExtract *pExtract)
{
	...
	if(NONNULL(pPuppet))
	{
		...
		if (uiPowerMask)
			entity_ResetPowerIDsHelper(ATR_PASS_ARGS,pMaster,uiPowerMask,pExtract,false,NULL);
	}
								  ...
								  GameAccountDataExtract *pExtract)
{
	...
	if(pTempPuppet)
	{
		...
		if (uiPowerMask)
			entity_ResetPowerIDsHelper(ATR_PASS_ARGS,pEntity,uiPowerMask,pExtract,false,NULL);
		entity_ResetPowerIDsHelper(ATR_PASS_ARGS,pEntity,POWERID_CREATE(0,0,POWERID_TYPE_MAIN),pExtract,false,NULL);
	}
