...
void gslProjectile_StaticInit()
{
	...
	//  Generic, Character, PowerDef, AttribMod
	{
		...
		stTable = exprContextCreateFunctionTable("ProjectileEntity");
...
Entity* gslProjectile_CreateProjectile(S32 iPartitionIdx, ProjCreateParams *pCreateParams)
{
	...
	if (pCreateParams && pCreateParams->pDef && pCreateParams->pvStartPos)
	{
		...
		if (pCreateParams->pOwner)
		{
			...
			if (pCreateParams->pOwner->erOwner)
			{
				pProjectileEnt->erOwner = pCreateParams->pOwner->erOwner;
			}
			else
			{
			...
			pProjectileEnt->erCreator = entGetRef(pCreateParams->pOwner);
...
__forceinline static void gslProjectile_ApplyPower(S32 iPartitionIdx, Entity *pEnt, PowerDef *pPowerDef, EntityRef erTargetEnt)
{
	...
	applyParams.erModOwner = pEnt->erOwner;
	applyParams.pCritical = pEnt->pChar->pSavedCritical;
...
// -------------------------------------------------------------------------------------------------------------------
// Iterates over pProj->eaEntitiesHitAndTime and applies powers to the ents hit by the projectile.  pEnt is the projectile’s entity. 
// Returns true if this projectile can continue hitting more ents.
static bool gslProjectile_HitEntities(Entity *pEnt, ProjectileEntity *pProj)
{
	...
	for (i = 0; i < s; ++i)
	{
		...
		// projectiles can't hit untargetable critters.
		if( e->pCritter)
		{
			CritterDef* pCritterDef = GET_REF(e->pCritter->critterDef);
			if (pCritterDef && pCritterDef->bUntargetable){
				continue;
			}
		}
