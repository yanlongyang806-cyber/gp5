	...
	else 
	{
		...
		else
		{
			...
			else if(ppow && ppow->eSource == kPowerSource_Propagation)
			{
				...
				if (iTray < 0)
				{
					AlwaysPropSlot* pPropSlot = AlwaysPropSlot_FindByPetID(pchar->pEntParent, uiPropPetID, pchar->pEntParent->pSaved->pPuppetMaster->curID, kAlwaysPropSlotCategory_Default);
					if (pPropSlot)
					{
			...
			switch (pElem->eType)
			{
				...
				xcase kTrayElemType_PowerPropSlot:
				{
					...
					if (ppowDef && e->pSaved)
					{
						...
						AlwaysPropSlot* pPropSlot = AlwaysPropSlot_FindByPetID(e, uPropPetID, e->pSaved->pPuppetMaster->curID, kAlwaysPropSlotCategory_Default);
						if (pPropSlot)
						{
							U32 uPropSlotID;
							S32 ePurpose;
							...
							if (uPropSlotID && uPropSlotID != pPropSlot->iSlotID)
							{
								for (n = eaSize(&e->pChar->ppPowersPropagation)-1; n >= 0; n--)
								{
									...
									if (pPropPowDef)
									{
										...
										AlwaysPropSlot* pCheckPropSlot = AlwaysPropSlot_FindByPetID(e, uCheckPropPetID, e->pSaved->pPuppetMaster->curID, kAlwaysPropSlotCategory_Default);
