...
void gslGuild_cmd_CreateEx(SA_PARAM_NN_VALID Entity *pEnt, U32 iColor1, U32 iColor2, const char * pcEmblem, const char * pcDescription, const char * pchThemeName, ACMD_SENTENCE pcName)
{
	...
	if (entGetAccessLevel(pEnt) < ACCESS_DEBUG)
	{
		...
		if (team_NumPresentMembers(pTeam) < GUILD_TEAM_MAX_SIZE)
		{
...
static void gslGuild_cmd_Demote_CB(Entity *pEnt, ContainerID iSubjectID, U32 uiAccountID, U32 uiLoginServerID, GSLGuildCBData *pData)
{
	...
	// check to see if subject is a leader if so check to see if we are prov leader
	if(pGuild && pMember && timeSecondsSince2000() < pMember->uProvisionalTimeSeconds && pMember->iRank == eaSize(&pGuild->eaRanks) - 1)
	{
		gslGuild_ResultMessage(pEnt->myContainerID, pEnt->myContainerID, 0, "", pcErrorName, "GuildServer_ProvisionalCantDemoteLeader", true);
		return;
	}
...
static void gslGuild_GetGuildsSearch_CB(TransactionReturnVal *pReturn, void *pData)
{
	...
	if (RemoteCommandCheck_Guild_GetGuilds(pReturn, &pList) == TRANSACTION_OUTCOME_SUCCESS) 
	{
		...
		if (pEnt)
		{
			FOR_EACH_IN_EARRAY_FORWARDS(pList->eaGuilds, GuildRecruitInfo, pGuildInfo)
			FOR_EACH_IN_EARRAY_FORWARDS(pGuildInfo->eaMembers, GuildRecruitMember, pMember)
			{
				if ((!pMember->pcRankName || !*pMember->pcRankName) && pMember->pcDefaultRankName && *pMember->pcDefaultRankName)
				{
					if (pMember->pcRankName)
						StructFreeString(pMember->pcRankName);
					pMember->pcRankName = StructAllocString(entTranslateMessageKey(pEnt, pMember->pcDefaultRankName));
				}
			}
			FOR_EACH_END
			FOR_EACH_END
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_CATEGORY(XMLRPC, Guild) ACMD_NAME(Guild_RescueLeader) ACMD_HIDE;
void gslGuild_cmd_RescueLeader(SA_PARAM_NN_VALID Entity *pEnt)
{
	Guild *pGuild;
	...
	if(!guild_IsMember(pEnt))
	{
		gslGuild_ResultMessage(pEnt->myContainerID, pEnt->myContainerID, 0, NULL, "GuildServer_ErrorType_RescueLeader", "GuildServer_SelfNotInGuild", true);
		return;
	}

	pGuild = guild_GetGuild(pEnt);
	pGuildMember = guild_FindMember(pEnt);
	if(!pGuild || pGuildMember->iRank < eaSize(&pGuild->eaRanks)-1 || pGuild->uDemoteLeaderID == 0 || pGuild->uDemoteLeaderID == pEnt->myContainerID)
	{
		return;
	}

	VerifyServerTypeExistsInShard(GLOBALTYPE_GUILDSERVER);
	RemoteCommand_aslGuild_RescueLeader(GLOBALTYPE_GUILDSERVER, 0, pEnt->pPlayer->pGuild->iGuildID, pEnt->myContainerID, GUILD_SERVERDOWNHANDLER(pEnt));
}
...
void gslGuild_EntityUpdate(Entity *pEnt)
{
	...
	if (!pEnt->pPlayer->pGuild->iGuildID) {
		...
		if (pEnt->pPlayer->pGuild->iGuildChat) {
			guild_GetGuildChannelNameFromID(pcNameBuff, sizeof(pcNameBuff), pEnt->pPlayer->pGuild->iGuildChat, iVirtualShardID);
		...
		if (pEnt->pPlayer->pGuild->iOfficerChat) {
			guild_GetOfficerChannelNameFromID(pcNameBuff, sizeof(pcNameBuff), pEnt->pPlayer->pGuild->iOfficerChat, iVirtualShardID);
			RemoteCommand_ChatServerLeaveChannel_Special(GLOBALTYPE_CHATSERVER, 0, pEnt->pPlayer->accountID, pEnt->myContainerID, pcNameBuff);
	...
	if (guild_IsMember(pEnt)) {
		...
		if (!pEnt->pPlayer->pGuild->iGuildChat && guild_HasPermission(pMember->iRank, pGuild, GuildPermission_Chat)) {
			guild_GetGuildChannelNameFromID(pcNameBuff, sizeof(pcNameBuff), pGuild->iContainerID, iVirtualShardID);
		...
		if (!pEnt->pPlayer->pGuild->iOfficerChat && guild_HasPermission(pMember->iRank, pGuild, GuildPermission_OfficerChat)) {
			guild_GetOfficerChannelNameFromID(pcNameBuff, sizeof(pcNameBuff), pGuild->iContainerID, iVirtualShardID);
			RemoteCommand_ChatServerJoinOrCreateChannel_Special(GLOBALTYPE_CHATSERVER, 0, pEnt->pPlayer->accountID, 
		...
		if (pEnt->pPlayer->pGuild->iOfficerChat && !guild_HasPermission(pMember->iRank, pGuild, GuildPermission_OfficerChat)) {
			guild_GetOfficerChannelNameFromID(pcNameBuff, sizeof(pcNameBuff), pEnt->pPlayer->pGuild->iGuildChat, iVirtualShardID);
			RemoteCommand_ChatServerLeaveChannel_Special(GLOBALTYPE_CHATSERVER, 0, pEnt->pPlayer->accountID, pEnt->myContainerID, pcNameBuff);
		}
...
ATR_LOCKS(pEnt, ".Pinventoryv2.Ppinventorybags[], .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Psaved.Pscpdata.Isummonedscp, .Psaved.Pscpdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Hallegiance, .Hsuballegiance, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds");
...
AUTO_COMMAND ACMD_ACCESSLEVEL(4) ACMD_SERVERCMD ACMD_CATEGORY(Guild) ACMD_NAME(Guild_DemoteTime);
void gslGuild_DemoteTime(Entity *pEnt, U32 uNewDelay)
{
	GuildMember *pGuildMember = guild_FindMember(pEnt);
	Guild *pGuild = guild_GetGuild(pEnt);
	...
	{
		VerifyServerTypeExistsInShard(GLOBALTYPE_GUILDSERVER);
		RemoteCommand_aslGuild_DemoteTime(GLOBALTYPE_GUILDSERVER, 0, guild_GetGuildID(pEnt), uNewDelay, GUILD_SERVERDOWNHANDLER(pEnt));
	}
}

// for testing guild demote leader. Set time
AUTO_COMMAND ACMD_ACCESSLEVEL(4) ACMD_SERVERCMD ACMD_CATEGORY(Guild) ACMD_NAME(Guild_ProvisionalTime);
void gslGuild_ProvisionalTime(Entity *pEnt, U32 uNewDelay)
{
	GuildMember *pGuildMember = guild_FindMember(pEnt);
	Guild *pGuild = guild_GetGuild(pEnt);
	...
	{
		VerifyServerTypeExistsInShard(GLOBALTYPE_GUILDSERVER);
		RemoteCommand_aslGuild_ProvisionalTime(GLOBALTYPE_GUILDSERVER, 0, guild_GetGuildID(pEnt), pEnt->myContainerID, uNewDelay, GUILD_SERVERDOWNHANDLER(pEnt));
	}
}
