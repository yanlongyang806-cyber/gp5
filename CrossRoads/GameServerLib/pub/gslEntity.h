...
AUTO_STRUCT;
typedef struct ResetPowerIDsRefs
{
...
bool LeaveMapEx(Entity* e, DoorTransitionSequenceDef* pTransOverride, const char* strReason);
...
LATELINK;
void gslGameSpecific_Entity_Login(Entity* e);
...
// checks if the entity has any duplicate powerIDs, and if it does calls a transaction to fixup the duped powerIDs. 
// returns true if there were dupes and ran the transaction to fixup
// note: should only be called from the login sequence before the character has fully logged in!
// note: the entity's player's field eLoginWaiting will receive the flag kLoginSuccess_PowerIDFixup if the transaction runs
bool gslEntity_ResetDupePowerIDs(Entity *pEnt, TransactionReturnCallback pCB, void *userData);
