...
typedef struct SpawnPointPartitionState 
{
	...
	int iUnlockOrderTag; // zero if locked. Otherwise an increasing number. (the latest will have the highest number)
...
// Move a player on the same map to a spawn point (when vEstimateOut is set, the player won't move & that var will be populated with the spawn point's position)
