...
static ExprContext* gslLogoff_CreateAndGetContext(Entity *pEnt)
{
	if (! s_pLogoffContext) {
		...
		stTable = exprContextCreateFunctionTable("Mechanics");
...
static void gslCreateTicketForOnlineAccountCB(TransactionReturnVal *returnStruct, void *userData)
{
	...
	if (iIdx >= 0) 
	{
		...
		if (pLogoff && !pLogoff->bRanPostLogoff && !pLogoff->bDisconnected) 
		{
			...
			// Send the AuthTicket to the client as soon as we know it.
			pEnt = entFromContainerIDAnyPartition(GLOBALTYPE_ENTITYPLAYER, pLogoff->cid);
			if( pEnt && pEnt->pPlayer )
			{
				ClientCmd_AuthTicketNew(pEnt, pEnt->pPlayer->accountID, iAuthTicket);
			}
		}
	}
}
...
void mechanics_PlayerEnteredMap(Entity *pPlayerEnt)
{
	if (pPlayerEnt && pPlayerEnt->pPlayer)
	{
		interaction_PlayerEnteredMap(pPlayerEnt);
	}
}
// ----------------------------------------------------------------------------------
// Entity Cleanup
...
void mechanics_LeaveMapEntityCleanup(Entity *pPlayerEnt)
{
	if (pPlayerEnt && pPlayerEnt->pPlayer) {
		...
		// for the "recently used" stuff
		interaction_PlayerLeftMap(pPlayerEnt);
...
static void mechanics_DismissAllPlayerPets(SA_PARAM_NN_VALID Entity *pOwner)
{
	if (pOwner && pOwner->pSaved)
	{
		...
		FOR_EACH_IN_EARRAY_FORWARDS(pOwner->pSaved->ppCritterPets, CritterPetRelationship, pRelationShip)
		{
			...
			{
				DismissPetEx(pOwner, pRelationShip->pEntity);
			}
		}
		FOR_EACH_END
	}
...
static void mechanics_RestoreAllPlayerPets(SA_PARAM_NN_VALID Entity *pOwner)
{
	if (pOwner && pOwner->pSaved)
	{
		// Summon all dismissed critter pets for this player, if allowed by the region
		FillPetTeamList(pOwner);
	}
}
