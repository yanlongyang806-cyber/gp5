...
// This structure is used to prevent duplicate computation/evaluation of certain things while in
// the notoriously slow interactable_FindAllDoors function.
typedef struct Interactable_FindAllDoors_Cache
{
	StashTable imageMenuCache; // a cache of overworld map image menu items that are usable by the player finding doors to other maps

	WorldInteractionPropertyEntry *pWorldInteractionPropertyEntry; // pointer to the last Interactable Entry that had its Interact condition evaluated
	bool bEntryInteractCondResult; // the result of the Inetract condition evaluation for the above Interactable Entry
} Interactable_FindAllDoors_Cache;
...
static void interactable_GetDestZoneMapsFromWorldGameActionProperties(WorldGameActionProperties ** eaActions, Entity* pUsePlayerEnt, GameInteractable* pInteractable, WorldInteractionPropertyEntry *pEntry, const char ***peaDestZmaps, Interactable_FindAllDoors_Cache *pFindAllDoorsCache);
...
AUTO_COMMAND ACMD_SERVERCMD ACMD_ACCESSLEVEL(8) ACMD_HIDE ACMD_NAME("DebugPrintOveridesSlow");
void interactable_DebugPrintOveridesSlow()
{
	int i;
	char* estrOverrideList = NULL;
	char* estrBuffer = NULL;
	int numInteractables=0, numInteractOverrides=0, numImageMenuOverrides=0, numTotalEntries=0;
	int numEntries;
	estrCreate(&estrOverrideList);
	FOR_EACH_INTERACTABLE(pInteractable)
	{
		// interact overrides:
		numInteractables++;
		if(pInteractable->eaOverrides)
		{
			estrAppend2(&estrOverrideList, pInteractable->pcName);
			estrAppend2(&estrOverrideList, ":\n");
			for (i=0; i<eaSize(&pInteractable->eaOverrides); i++)
			{
				GameInteractableOverride *pOverride = pInteractable->eaOverrides[i];
				numInteractOverrides++;
				estrPrintf(&estrBuffer, "\t\"%s\" from file %s\n", pOverride->pcName, pOverride->pcFilename);
				estrAppend(&estrOverrideList, &estrBuffer);
			}
		}

		// image menu overrides:
		numEntries = interactable_GetNumPropertyEntries(pInteractable, true);
		...
		for(i=0; i<numEntries; i++) 
		{
			...
			if(pEntry->pContactProperties)
			{
				...
				{
					int j;
					ImageMenuItemOverrideData** eaNamespacedOverrides = contact_GetNamespacedImageMenuItemOverrideList(pContactDef);
					if(eaNamespacedOverrides)
					{
						estrAppend2(&estrOverrideList, pInteractable->pcName);
						estrAppend2(&estrOverrideList, ":\n");
						for(j=0; j<eaSize(&eaNamespacedOverrides); j++)
						{
							ImageMenuItemOverrideData* pItem = eaNamespacedOverrides[j];
							numImageMenuOverrides++;
							estrPrintf(&estrBuffer, "\tImageMenu: \"%s\" from mission %s\n", TranslateDisplayMessage(pItem->pItem->name), pItem->pchSourceName);
							estrAppend(&estrOverrideList, &estrBuffer);
						}
					}
				}
	...
	printf("\nOVERRIDES:\n%s\ninteractables:%i\tinteract entries:%i\tinteract overrides:%i\timageMenu overrides:%i\n", 
			estrOverrideList, numInteractables, numTotalEntries, numInteractOverrides, numImageMenuOverrides);
...
// Returns the first detail texture name (pooled string) it finds among the interactable's interaction entries
DisplayMessage *interactable_GetOverrideDisplayText(Entity *pPlayerEnt, GameInteractable *pInteractable)
{
	...
	if (pEntry) {
		...
		if (pProps) {
			if (pProps->pOverrideDisplayNameExpr) {
				if (interactable_EvaluateExpr(entGetPartitionIdx(pPlayerEnt), pInteractable, pPlayerEnt, pProps->pOverrideDisplayNameExpr)) {
					return &pProps->overrideDisplayNameMsg;
				}
			}
		}
	}
...
static bool interactable_FindAllDoors_EntryInteractCond( Entity* pUsePlayerEnt, GameInteractable* pInteractable, WorldInteractionPropertyEntry* pEntry, Interactable_FindAllDoors_Cache *pFindAllDoorsCache )
{
	if (pUsePlayerEnt)
	{
		//Check if the player can use this interaction entry.  Usually call functions for this, but here we want only this entry, 
		// and we already know that the entry has doorProperties, so we can do simpler checks.  This has been a bottleneck so we're optimizing.
		PlayerDebug* pDebug;
		Expression *pInteractCond;
		pDebug = entGetPlayerDebug(pUsePlayerEnt, false);
		pInteractCond = interaction_GetInteractCond(pEntry);

		if((!pDebug || !pDebug->allowAllInteractions) && pInteractCond)
		{
			if(pFindAllDoorsCache->pWorldInteractionPropertyEntry != pEntry)
			{
				pFindAllDoorsCache->bEntryInteractCondResult = interactable_EvaluateExpr(entGetPartitionIdx(pUsePlayerEnt), pInteractable, pUsePlayerEnt, pInteractCond);
				pFindAllDoorsCache->pWorldInteractionPropertyEntry = pEntry;
			}
			return pFindAllDoorsCache->bEntryInteractCondResult;
		}
	}
...
// Helper for interactable_FindAllDoors(), checks that the player can use this interact entry as door and calculates the geographic location the waypoint will use.
static const char* interactable_GetDestZoneMapMaybe( Entity* pUsePlayerEnt, GameInteractable* pInteractable, WorldInteractionPropertyEntry* pEntry, const WorldVariableDef* pDoorDest, Interactable_FindAllDoors_Cache *pFindAllDoorsCache )
{
	PERFINFO_AUTO_START_FUNC();
	if (pDoorDest->eDefaultType == WVARDEF_SPECIFY_DEFAULT && pDoorDest->pSpecificValue) {
		...
		{
			if(interactable_FindAllDoors_EntryInteractCond(pUsePlayerEnt, pInteractable, pEntry, pFindAllDoorsCache))
			{
				PERFINFO_AUTO_STOP();
				return allocAddString(pDoorDest->pSpecificValue->pcZoneMap ? pDoorDest->pSpecificValue->pcZoneMap : pDoorDest->pSpecificValue->pcStringVal);
			}
		}
	}
	PERFINFO_AUTO_STOP();
...
//helper for interactabl_FindAllDoors() to extract doors from a contact
static void interactable_GetDestZoneMapsFromContactDef(ContactDef* pContactDef, Entity* pUsePlayerEnt, GameInteractable* pInteractable, WorldInteractionPropertyEntry *pEntry, const char*** peaDestZmaps, Interactable_FindAllDoors_Cache *pFindAllDoorsCache)
{
	ContactImageMenuData* pCurrentImageMenuData = NULL;

	PERFINFO_AUTO_START("imageMenuWaypointDoors", 1);
	if(pContactDef && pContactDef->pImageMenuData){
		int i;
		if( !stashAddressFindPointer( pFindAllDoorsCache->imageMenuCache, pContactDef->name, NULL )) {
			//this is a new image menu.  Update the cache after getting doors the hard way.
			ContactImageMenuItem **eaImageMenuItems = NULL;
			const char** eaDestZmaps = NULL;
			...
			for (i = 0; i < eaSize(&eaImageMenuItems); i++)
			{
				...
				MissionDef* pDef = GET_REF(item->overridingMissionDef);

				//if this is item is an override from a UGC mission I don't have, skip the expression evaluation and continue.
				if (pUsePlayerEnt && pDef)
				{
					MissionInfo* pMissionInfo = pUsePlayerEnt->pPlayer->missionInfo;
					if (pDef->ugcProjectID										//this item is from a UGC mission override 
						&& (	!pMissionInfo->bHasNamespaceMission				//and (		I have no UGC missions
								||	mission_GetMissionFromDef(pMissionInfo, pDef)))	//		or  I don't have this UGC mission)
					{
						continue;
					}
				}

				//add it if I can use it.
				...
				{
					interactable_GetDestZoneMapsFromWorldGameActionProperties(item->action->eaActions, pUsePlayerEnt, pInteractable, pEntry, &eaDestZmaps, pFindAllDoorsCache);
				}
			}
			eaDestroy(&eaImageMenuItems);

			// Update the cache, transfer ownership of the array into the cache
			stashAddressAddPointer( pFindAllDoorsCache->imageMenuCache, pContactDef->name, (void*)eaDestZmaps, true );
			eaDestZmaps = NULL;
		}

		// Now we are guaranteed to have this in the cache
		{
			const char** eaDestZmaps = NULL;
			stashAddressFindPointer( pFindAllDoorsCache->imageMenuCache, pContactDef->name, (void**)&eaDestZmaps );
			eaPushEArray( peaDestZmaps, &eaDestZmaps );
		}
	}
	PERFINFO_AUTO_STOP();
}
...
static void interactable_GetDestZoneMapsFromWorldGameActionProperties(WorldGameActionProperties ** eaActions, Entity* pUsePlayerEnt, GameInteractable* pInteractable, WorldInteractionPropertyEntry *pEntry, const char ***peaDestZmaps, Interactable_FindAllDoors_Cache *pFindAllDoorsCache)
{
	...
	for( actionIt = 0; actionIt != eaSize( &eaActions ); ++actionIt ) 
	{
		...
		if( pAction->eActionType == WorldGameActionType_Warp && pAction->pWarpProperties ) 
		{
			...
			{
				const char* destZmap = interactable_GetDestZoneMapMaybe( pUsePlayerEnt, pInteractable, pEntry, &pAction->pWarpProperties->warpDest, pFindAllDoorsCache );
				if( destZmap ) 
				{
					eaPush( peaDestZmaps, destZmap );
				}
			}
		}
		if( pAction->eActionType == WorldGameActionType_Contact && pAction->pContactProperties )
		{
			interactable_GetDestZoneMapsFromContactDef( GET_REF( pAction->pContactProperties->hContactDef ), pUsePlayerEnt, pInteractable, pEntry, peaDestZmaps, pFindAllDoorsCache );
		}
	}
}

static void interactable_addDoorFromSingleZoneMap( FoundDoorStruct*** peaDoors, GameInteractable* pInteractable, const char* destZmap )
{
	Vec3 vWorldMin, vWorldMax;
	WorldCellEntry *pCell;
	...
	pDoor->pDoorName = strdup(destZmap);
...
static void interactable_addDoorFromZoneMaps( FoundDoorStruct*** peaDoors, GameInteractable* pInteractable, const char** eaDestZmaps )
{
	int it;
	for( it = 0; it != eaSize( &eaDestZmaps ); ++it ) {
		interactable_addDoorFromSingleZoneMap( peaDoors, pInteractable, eaDestZmaps[ it ]);
	}
}

static void eaDestroyFunc( const char** ea )
{
	eaDestroy( &ea );
}

// find all the doors that the player can use on the current map and put them into peaDoors.
// This looks for doors on interactables and optionally warp actions on interactables and on imageMenus that are on contact-interactables.
// NOTE that this function has been a stall point for game servers because it has to iterate over interactable overrides and contact overrides,
//		which are added dynamically by UGC missions. 
void interactable_FindAllDoors(FoundDoorStruct ***peaDoors, Entity * pUsePlayerEnt, bool includeWarpActions)
{
	Interactable_FindAllDoors_Cache findAllDoorsCache;
	int j;

	PERFINFO_AUTO_START_FUNC();

	findAllDoorsCache.imageMenuCache = stashTableCreateAddress( 16 );
	findAllDoorsCache.pWorldInteractionPropertyEntry = NULL;
	...
	FOR_EACH_INTERACTABLE(pInteractable) {
		int iNumEntries = interactable_GetNumPropertyEntries(pInteractable, SAFE_MEMBER3(pUsePlayerEnt, pPlayer, missionInfo, bHasNamespaceMission));
		...
		for(j=0; j<iNumEntries; ++j) {
			...
			if (pDoorProps) {
				const char* destZmap = interactable_GetDestZoneMapMaybe( pUsePlayerEnt, pInteractable, pEntry, &pDoorProps->doorDest, &findAllDoorsCache );
				if( destZmap ) {
					interactable_addDoorFromSingleZoneMap( peaDoors, pInteractable, destZmap );
				}
			}

			if( includeWarpActions && pEntry && pEntry->pcInteractionClass == pcPooled_Clickable && pEntry->pActionProperties ) {
				const char** eaDestZmap = NULL;
				interactable_GetDestZoneMapsFromWorldGameActionProperties(pEntry->pActionProperties->successActions.eaActions, pUsePlayerEnt, pInteractable, pEntry, &eaDestZmap, &findAllDoorsCache);
				interactable_addDoorFromZoneMaps( peaDoors, pInteractable, eaDestZmap );
				eaDestroy( &eaDestZmap );
			}

			//doors through image menus:
			if(includeWarpActions && pEntry && pEntry->pContactProperties && interactable_FindAllDoors_EntryInteractCond(pUsePlayerEnt, pInteractable, pEntry, &findAllDoorsCache))
			{
				const char** eaDestZmap = NULL;
				interactable_GetDestZoneMapsFromContactDef(GET_REF(pEntry->pContactProperties->hContactDef), pUsePlayerEnt, pInteractable, pEntry, &eaDestZmap, &findAllDoorsCache);
				interactable_addDoorFromZoneMaps( peaDoors, pInteractable, eaDestZmap );
				eaDestroy( &eaDestZmap );
			}
		}
	...
	stashTableDestroyEx( findAllDoorsCache.imageMenuCache, NULL, eaDestroyFunc );
	PERFINFO_AUTO_STOP();
}
...
static void interactable_InitBase(GameInteractable *pInteractable)
{
	...
	if (pEntry && pEntry->full_interaction_properties) {
		...
		if (pProps->pOverrideDisplayNameExpr) {
			if (!exprGenerate(pProps->pOverrideDisplayNameExpr, g_pInteractionContext)) {
				ErrorFilenamef(layerGetFilename(pInteractable->pWorldInteractable->common_data.layer),
					"Interactable %s has an invalid State Expression.", pInteractable->pcName);
			}
		}
...
void interactable_GatherBeaconPositions(void)
{
	...
	for(j = 0; j < eaSize(&s_eaAmbientJobInteractables); j++)
	{
		...
		for(k = 0; k < eaSize(&pGameInteractable->eaInteractLocations); k++)
		{
			...
			{
				beaconAddUsefulPoint(pLoc->pWorldInteractLocationProperties->vPos, 1);
			}			
		}
	}
