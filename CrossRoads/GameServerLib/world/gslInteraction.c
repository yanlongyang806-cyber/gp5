...
void interaction_RetireRecentInteractionLists();
...
static StashTable g_RecentInteractInfoByPlayer;
static StashTable g_RecentInteractRetireTable;
...
bool interaction_IsLootEntity(SA_PARAM_OP_VALID Entity *pCritterEnt)
{
	...
	if (pTracker) {
		...
		if (pCopiedBag) {
			if (pCopiedBag->pRewardBagInfo && 
				...
				bagiterator_SetBag(CONTAINER_NOCONST(InventoryBag, pCopiedBag), iter);
...
static void interaction_OncePerFrameTimerUpdatePartition(PartitionInteracted *partn, F32 fTimeStep, bool bForce)
{
	...
	interaction_RetireRecentInteractionLists();
...
// Not to be confused with interaction_EndInteractionAndDialog below.  This clears the interact bar, but doesn't
...
static ExprContext *interaction_GetCritterInteractContext(Entity *pPlayer, Entity *pTarget)
{
	...
	if (!pContext)
	{
		...
		stTable = exprContextCreateFunctionTable("Interaction");
...
void interaction_DoorAnimationCompleteCB(InteractionMovementRequestData *pData)
{
	...
	{
		LeaveMapEx(pEntity, pData->pTransOverride, __FUNCTION__);
	}
...
void interaction_AddRecentCompletedInteract(Entity *pEntity, GameInteractable *pInteractable, Entity *pEntTarget, GameNamedVolume *pVolume)
{
	PlayerInteractedInfo *pPlayerInfo;
	int i;
	...
	// The usual inefficient double stash-table lookup
	stashIntFindPointer(g_RecentInteractInfoByPlayer,pEntity->myContainerID,&pPlayerInfo);
	if (pPlayerInfo == NULL)
	{
		pPlayerInfo = StructCreate(parse_PlayerInteractedInfo);
		stashIntAddPointer(g_RecentInteractInfoByPlayer,pEntity->myContainerID,pPlayerInfo,false);
	}

	for (i = eaSize(&pPlayerInfo->eaInfo)-1; i >= 0; i--)
	{
		InteractionInfo *pInteractionInfo = pPlayerInfo->eaInfo[i];
	...
	if (i < 0)
	{
		...
		if (eaSize(&pPlayerInfo->eaInfo) >= MAX_CLICKABLES_TRACKED)
		{
			pNewInteractionInfo = eaRemove(&pPlayerInfo->eaInfo, 0);
		...
		// Is allocAddString good here?  pcName might already be token....
		if (pInteractable)
		...
		eaPush(&pPlayerInfo->eaInfo, pNewInteractionInfo);
...
static void interaction_UpdateTargetableNodes(Entity *pEnt, U32 interactClassMask)
{
	...
	for ( i = 0; i < n; i++ ) {
		...
		if ( entity_IsNodeInRange( pEnt, NULL, pNode, fTargetDist, 0, 0, vClose, &fDist, true ) ) {
			...
			OverrideDisplayTextNode *pOverrideDisplayTextNode;
			NodeSummary *pDoorDestStatusNode;
			DisplayMessage *pOverrideDisplayText;
			...
			// Find a usable display text override for this interact
			if (bVisibleForThisEnt || !bHasPerEntVis) {
				pOverrideDisplayText = interactable_GetOverrideDisplayText(pEnt, pInteractable);
				if (pOverrideDisplayText) {
					// Find or allocate a node
					if (iCount < eaSize(&pPlayer->InteractStatus.ppOverrideDisplayTextNodes)) {
						pOverrideDisplayTextNode = pPlayer->InteractStatus.ppOverrideDisplayTextNodes[iCount];
					} else {
						pOverrideDisplayTextNode = StructCreate( parse_OverrideDisplayTextNode );
						eaPush( &pPlayer->InteractStatus.ppOverrideDisplayTextNodes, pOverrideDisplayTextNode );
					}

					SET_HANDLE_FROM_REFERENT(INTERACTION_DICTIONARY, pNode, pOverrideDisplayTextNode->hNode);
					SET_HANDLE_FROM_REFERENT("Message", GET_REF(pOverrideDisplayText->hMessage), pOverrideDisplayTextNode->hOverrideDisplayTextMsg);
				}
			}
...
PlayerInteractedInfo * interaction_GetInteractionInfoForPlayer(Entity * pPlayerEnt)
{
	PlayerInteractedInfo * pPlayerInfo = NULL;
	if (pPlayerEnt && pPlayerEnt->pPlayer)
	{
		stashIntFindPointer(g_RecentInteractInfoByPlayer,pPlayerEnt->myContainerID,&pPlayerInfo);
	}
...
void interaction_RetireRecentInteractionLists()
{
	...
	U32 uTime = timeServerSecondsSince2000();
	if (uTime-uLastCheckTime > 60)
	{
		uLastCheckTime = uTime;
		FOR_EACH_IN_STASHTABLE(g_RecentInteractRetireTable, PlayerInteractedInfo, pPlayerInfo)
		{
			if (uTime-pPlayerInfo->uRetireTime > 15*60) // 15 minutes
			{
				stashRemoveElementByIndex(g_RecentInteractRetireTable,ipPlayerInfoIter.uiIndex);
				StructDestroy(parse_PlayerInteractedInfo,pPlayerInfo);
			}
		}
		FOR_EACH_END;
	}
...
void interaction_PlayerLeftMap(Entity * pPlayerEnt)
{
	PlayerInteractedInfo * pPlayerInfo;
	stashIntRemovePointer(g_RecentInteractInfoByPlayer,pPlayerEnt->myContainerID,&pPlayerInfo);
	if (pPlayerInfo)
	{
		stashIntAddPointer(g_RecentInteractRetireTable,pPlayerEnt->myContainerID,pPlayerInfo,true);
		pPlayerInfo->uRetireTime = timeServerSecondsSince2000();
	}
}

void interaction_PlayerEnteredMap(Entity * pPlayerEnt)
{
	PlayerInteractedInfo * pPlayerInfo;
	stashIntRemovePointer(g_RecentInteractRetireTable,pPlayerEnt->myContainerID,&pPlayerInfo);
	if (pPlayerInfo)
	{
		stashIntAddPointer(g_RecentInteractInfoByPlayer,pPlayerEnt->myContainerID,pPlayerInfo,true);
	}
}
...
void interactionsystem_Init(void)
{
	...
	// create stash tables
	g_RecentInteractInfoByPlayer = stashTableCreateInt(150);
	g_RecentInteractRetireTable = stashTableCreateInt(100);
}
...
void ResetRecentClickableList(Entity *pCaller)
{
	...
	{
		PlayerInteractedInfo * pPlayerInfo;
		stashIntFindPointer(g_RecentInteractInfoByPlayer,pCaller->myContainerID,&pPlayerInfo);
		if (pPlayerInfo)
		{
			stashIntRemovePointer(g_RecentInteractInfoByPlayer,pCaller->myContainerID,NULL);
			StructDestroy(parse_PlayerInteractedInfo,pPlayerInfo);
			return;
		}
		stashIntFindPointer(g_RecentInteractRetireTable,pCaller->myContainerID,&pPlayerInfo);
		if (pPlayerInfo)
		{
			stashIntRemovePointer(g_RecentInteractRetireTable,pCaller->myContainerID,NULL);
			StructDestroy(parse_PlayerInteractedInfo,pPlayerInfo);
		}
	}
}
...
void ListRecentClickables(Entity *pCaller)
{
	if (pCaller && pCaller->pPlayer)
	{
		...
		if (pPlayerInfo)
		{
			...
			for(i = eaSize(&pPlayerInfo->eaInfo)-1; i >= 0; i--)
			{
				InteractionInfo *pInfo = pPlayerInfo->eaInfo[i];
				if(pInfo)
				{
