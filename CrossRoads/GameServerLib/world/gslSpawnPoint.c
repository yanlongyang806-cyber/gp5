...
// Find the 'furthest' (latest) spawn point activated by Anyone on this partition
static GameSpawnPoint* spawnPoint_GetFurthestSpawnPointForPartition(int iPartitionIdx, int* pFurthestOrder)
{
	GameSpawnPoint* pReturnPoint = NULL;
	int iFurthestOrder=0;
	
	FOR_EACH_SPAWN_POINT(pGamePoint)
	{
		SpawnPointPartitionState *pState = eaGet(&pGamePoint->eaPartitionStates, iPartitionIdx);

		if (pState!=NULL && pState->iUnlockOrderTag > iFurthestOrder)
		{
			pReturnPoint = pGamePoint;
			iFurthestOrder = pState->iUnlockOrderTag;
		}
	} FOR_EACH_END;
	
	if (pFurthestOrder!=NULL)
	{
		*pFurthestOrder=iFurthestOrder;
	}
	return(pReturnPoint);
} 
...
static bool spawnpoint_HasPlayerActivatedSpawn(Entity *pPlayerEnt, const char *pcSpawnName, const char *pcSpawnMap)
{
	...
	if (pPlayerEnt->pPlayer) {
		...
		if (pGamePoint)
		{
			// We are partitioned now. Get the state for the right partition to look up our unlocks.
			int iPartitionIdx = entGetPartitionIdx(pPlayerEnt);
			SpawnPointPartitionState *pState = eaGet(&pGamePoint->eaPartitionStates, iPartitionIdx);
			if (pState!=NULL)
			{
				for(i=eaiSize(&pState->eaiPlayerUnlocks)-1; i>=0; --i) {
					ContainerID id = pState->eaiPlayerUnlocks[i];
	...
	if (pEnt && pEnt->pPlayer && pGamePoint) {
		...
		if (pGamePoint->pcName && pcMapName && !spawnpoint_HasPlayerActivatedSpawn(pEnt, pGamePoint->pcName, pcMapName)) {
			...
			if (zmapInfoGetMapType(NULL) == ZMTYPE_STATIC) {
				...
				if (pState!=NULL)
				{
					eaiPushUnique(&pState->eaiPlayerUnlocks, pEnt->myContainerID);

					// Check and Record the overall unlock. (which is tracked for the partition instead of per-player)
					if (pState->iUnlockOrderTag == 0)
					{
						// We haven't unlocked this one yet.
						int iOldFurthestOrder=0;
						GameSpawnPoint* pOldFurthestPoint = spawnPoint_GetFurthestSpawnPointForPartition(iPartitionIdx, &iOldFurthestOrder);

						pState->iUnlockOrderTag = iOldFurthestOrder+1;
					}
				}
			}
		...
		else if (bMissionReturn)
		{
			...
			if (bIncludeTeammates && pTeam)
			{
				for (i = 0; i < eaSize(&pTeam->eaMembers); i++)
				{
					...
					if (pTeamEnt)
						LeaveMapEx(pTeamEnt, pTransOverride, __FUNCTION__ " teamEnt");
				}
			}
			else
			{
				LeaveMapEx(pEnt, pTransOverride, __FUNCTION__);
			}
		}
...
static void spawnpoint_MovePlayerToGameSpawn(Entity *pPlayerEnt, SA_PARAM_NN_VALID GameSpawnPoint *pGamePoint, bool bMovePets)
{
	...
	if( SAFE_MEMBER( pGamePoint->pWorldPoint->properties, y_always_up )) {
		Vec3 pyr;
		quatToPYR( pGamePoint->pWorldPoint->spawn_rot, pyr );
		pyr[ 0 ] = pyr[ 2 ] = 0;
		PYRToQuat( pyr, quatSpawnRot );
	} else {
...
void spawnpoint_MovePlayerToStartSpawn(Entity *pPlayerEnt, Vec3 vEstimateOut, bool bMovePets)
{
	// when you pass a vEstimate into this function, it shouldn't modify
	// the state of the game at all, it should just provide an estimated
	// spawn position back
	...
	if (pGamePoint){
		if (!vEstimateOut) {
			...
			copyVec3(pGamePoint->pWorldPoint->spawn_pos, vEstimateOut);
		}
	} else {
		// Commented out by jpanttaja to reduce log load
		//entLog(LOG_PLAYER, pPlayerEnt, "MovePlayerToSpawn", "No Start Spawn found");
		// I think this should throw an error since there isn't a fallback!
		if (vEstimateOut) {
			zeroVec3(vEstimateOut);
		}
	}
}
...
void spawnpoint_MovePlayerToNearestSpawn(Entity *pPlayerEnt, Vec3 vEstimateOut, bool bActivatedSpawnsOnly, bool bMovePets)
{
	// when you pass a vEstimate into this function, it shouldn't modify
	// the state of the game at all, it should just provide an estimated
	// spawn position back
	...
	// this is horrible if the old location is from a different map
	// hopefully this is only getting called when it's appropriate, and not when changing maps
	...
	if (pGamePoint) {
		...
		spawnpoint_MovePlayerToStartSpawn(pPlayerEnt, vEstimateOut, bMovePets);
	}
}
...
void spawnpoint_MovePlayerToSpawnPointNearTeam(Entity *pPlayerEnt, bool bActivatedSpawnsOnly, bool bMovePets)
{
	...
	{
		spawnpoint_MovePlayerToNearestSpawn(pPlayerEnt,NULL,bActivatedSpawnsOnly,bMovePets);
		return;
	}
	...
	if (pGamePoint) {
		...
		// No spawn point found; go to the start spawn
		spawnpoint_MovePlayerToStartSpawn(pPlayerEnt, NULL, bMovePets);
	}
}

// Find the 'furthest' (latest) spawn point activated by Anyone. 
void spawnpoint_MovePlayerToFurthestActivatedSpawnPoint(Entity *pPlayerEnt, Vec3 vEstimateOut, bool bMovePets)
{
	// when you pass a vEstimate into this function, it shouldn't modify
	// the state of the game at all, it should just provide an estimated
	// spawn position back
	...
	pGamePoint = spawnPoint_GetFurthestSpawnPointForPartition(iPartitionIdx, NULL);
	...
	{
		// No unlocked spawn point found; go to the start spawn
		spawnpoint_MovePlayerToStartSpawn(pPlayerEnt, vEstimateOut, bMovePets);
	}
}


void spawnpoint_MovePlayerToNamedSpawn(Entity *pPlayerEnt, const char *pchNamedSpawnPoint, const WorldScope *pScope, Vec3 vEstimateOut, bool bDefaultToStartSpawn)
{
	// when you pass a vEstimate into this function, it shouldn't modify
	// the state of the game at all, it should just provide an estimated
	// spawn position back
	...
	if(bIsStartSpawn) {
		spawnpoint_MovePlayerToStartSpawn(pPlayerEnt, vEstimateOut, true);
	} else if (bIsRespawnPoint) {
		spawnpoint_MovePlayerToNearestSpawn(pPlayerEnt, vEstimateOut, true, true);
	} else {
...
static void spawnpoint_FreePartitionState(SpawnPointPartitionState *pState)
{
	...
	eaiDestroy(&pState->eaiPlayerUnlocks);
	free(pState);
}
...
void spawnpoint_GatherBeaconPositions(void)
{
	FOR_EACH_SPAWN_POINT(pSpawn)
	{
		beaconAddSpawnLocation(pSpawn->pWorldPoint->spawn_pos, 0, 1);
	} FOR_EACH_END;
}
...
void spawnpoint_PartitionLoad(int iPartitionIdx)
{
	...
	FOR_EACH_SPAWN_POINT(pSpawn) {
		...
		if (!pState) {
			...
			pState->eaiPlayerUnlocks = NULL;
			pState->iUnlockOrderTag = 0; // zero if locked. Otherwise an increasing number. (the latest will have the highest number)
