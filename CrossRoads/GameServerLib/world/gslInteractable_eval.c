...
AUTO_EXPR_FUNC(clickable, player) ACMD_NAME(HasRecentlyCompletedClickable, HasRecentlyCompletedInteractWithObject);
ExprFuncReturnVal exprFuncHasRecentlyCompletedInteractWithObject(ExprContext *pContext, ACMD_EXPR_INT_OUT piRet, const char *pchObjectName)
{
	...
	pInteractable = interactable_GetByName(pchObjectNamePooled, pScope);

	if (pPlayerEnt && pPlayerEnt->pPlayer && pchObjectNamePooled)
	{
		PlayerInteractedInfo * pPlayerInfo = interaction_GetInteractionInfoForPlayer(pPlayerEnt);
		if (pPlayerInfo)
		{
			int i;
			for (i = eaSize(&pPlayerInfo->eaInfo)-1; i >= 0; i--)
			{
				InteractionInfo* pInteract = pPlayerInfo->eaInfo[i];
				...
				if (pInteractable && pInteract->pchInteractableName == pInteractable->pcName)
				{
					(*piRet) = true;
					break;
				}
...
AUTO_EXPR_FUNC(clickable, player) ACMD_NAME(HasRecentlyCompletedInteractWithVolume);
...
AUTO_EXPR_FUNC(clickable, player) ACMD_NAME(HasRecentlyCompletedInteractWithEntityInEncounter);
