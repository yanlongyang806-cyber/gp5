...
ExprFuncTable* encPlayer_CreateExprFuncTable()
{
	...
	if (!s_encPlayerFuncTable) {
		s_encPlayerFuncTable = exprContextCreateFunctionTable("EncounterPlayer");
...
ExprFuncTable* encounter_CreateExprFuncTable()
{
	...
	if (!s_encounterFuncTable) {
		s_encounterFuncTable = exprContextCreateFunctionTable("Encounter");
...
ExprFuncTable* encounter_CreatePlayerExprFuncTable()
{
	...
	if (!s_encounterPlayerFuncTable) {
		s_encounterPlayerFuncTable = exprContextCreateFunctionTable("Encounter_Players");
...
Entity *encounter_GetActorEntity(int iPartitionIdx, GameEncounter *pEncounter, const char *pcActorName)
{
	...
	for(i=eaSize(&pProps->eaActors)-1; i>=0; --i) {
		if (stricmp(pcActorName, pProps->eaActors[i]->pcName) == 0) {
			return encounter_GetActorEntityByIndex(iPartitionIdx, pEncounter, i);
		}
	}
...
Entity *encounter_GetActorEntityByIndex(int iPartitionIdx, GameEncounter* pEncounter, int iActorIndex)
{
	int i;
	...
	for(i=eaSize(&pState->eaEntities)-1; i>=0; --i) {
		...
		if (pEnt->pCritter && (pEnt->pCritter->encounterData.iActorIndex == iActorIndex)) {
			return pEnt;
		}
	}
	return NULL;
...
void encounter_GetEncounterActorPosition(GameEncounter *pEncounter, int iActorIndex, Vec3 vOutVec, Quat qOutQuat)
{
...
bool encounter_AreAllCombatantsDead(GameEncounter *pEncounter, GameEncounterPartitionState *pState)
{
	int i;
								 ...
								 AICombatRolesDef *pCombatRolesDef)
{
	...
	if (pActor && pActor->critterProps.eCritterType == ActorCritterType_PetContactList)
	{
		...
		if (pList!=NULL)
		{
			...
			if (pOwner)
			{
				...
				PetContactList_GetPetOrCostume(pEntPlayer, pList, peaPetContactNameExcludeList, &pPet, &pDefaultCritter, &pPetCostume, NULL);
	...
	if (pNewEnt && pNewEnt->pCritter) {
		...
		// Re-orient the actor to be upright
		vNewEntRot[0] = vNewEntRot[2] = 0.f;
