...
void neighborhood_VolumeEnteredCB(WorldVolumeEntry *pEntry, Entity *pEnt, GameNamedVolume *pGameVolume)
{
	...
	} else if (!pcPooledHoodName)
	{
		if(pGameVolume)
		{
			ErrorDetailsf("Map name %s, Volume name %s", zmapGetFilename(NULL), pGameVolume->pcName);
		}
		Errorf("Player entered neighborhood with no name!  File for this neighborhood is not known. See details for volume name");
	}
}
