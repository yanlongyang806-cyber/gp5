...
bool volume_IsEntityInVolumeByType(const Entity *pEnt, U32 volume_type)
{
	if (pEnt) {
		const WorldVolume **eaVolumes;
		int i, s;
		...
		for(i=0; i<s; i++) {
			const WorldVolume *pVolume = eaVolumes[i];
			if (wlVolumeIsType(pVolume, volume_type))
				return true;
		}
	}
...
{
	InteractVolumeStatus *pClientVolumeStatus;
	...
	pClientVolumeStatus = eaIndexedGetUsingString(&pEnt->pPlayer->InteractStatus.ppInteractVolumeStatus, pGameVolume->pcName);
	if (pClientVolumeStatus) {
		pClientVolumeStatus->bInVolume = true;
...
static void volume_TrackVolumeExitedCB(WorldVolumeEntry *pEntry, Entity *pEnt, GameNamedVolume *pGameVolume)
{
	...
	pClientVolumeStatus = eaIndexedGetUsingString(&pEnt->pPlayer->InteractStatus.ppInteractVolumeStatus, pGameVolume->pcName);
	if (pClientVolumeStatus) {
		pClientVolumeStatus->bInVolume = false;
		entity_SetDirtyBit(pEnt, parse_EntInteractStatus, &pEnt->pPlayer->InteractStatus, true);
		entity_SetDirtyBit(pEnt, parse_Player, pEnt->pPlayer, true);
		entity_SetDirtyBit(pEnt, parse_Entity, pEnt, false);
	}
...
static void volume_WarpVolume_DoWarp(WorldWarpVolumeProperties *pWarpData, GameNamedVolume *pGameVolume, Entity* pEnt)
{
	...
	if (dest && dest->eType == WVAR_MAP_POINT) {
		...
		if (dest->pcZoneMap && pVar && pVar->pVariable && pVar->pVariable->iIntVal != 0){
			LeaveMapEx(pEnt, GET_REF(pWarpData->hTransSequence), __FUNCTION__);
		} else {
...
bool volume_GetVolumeData(int iPartitionIdx, const char *pcVolumeName, const WorldScope* pScope, Vec3 vCenter, Vec3 vPosition, Vec3 vLocalMin, Vec3 vLocalMax, F32 *pfRot, U32 *peVolumeShape)
{
	...
	if (pVolume) {
		WorldVolume *pWorldVolume;
		WorldVolumeElement *pVolumeElement;
		Quat qRot;
		Vec3 vUp;
		...
		pVolumeElement = eaGet(&pVolume->pNamedVolume->entry->elements, 0);
		...
		if (vUp[1] < 0)
			(*pfRot) *= -1;
		(*peVolumeShape) = pVolumeElement ? pVolumeElement->volume_shape : WL_VOLUME_BOX;
		PERFINFO_AUTO_STOP();
		return true;
	}
...
static void volume_VolumeExitedCB(WorldVolume *pVolume, WorldVolumeQueryCache *pQueryCache)
{
	...
	if (pEnt
		&& pEnt->pPlayer
		&& wlVolumeIsType(pVolume, s_ePetsDisabledVolumeType)
		&& !entCheckFlag(pEnt, ENTITYFLAG_PLAYER_LOGGING_OUT))
	{
