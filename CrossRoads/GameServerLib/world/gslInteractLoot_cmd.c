	...
	if (pLeaderEnt->pPlayer->InteractStatus.interactTarget.entRef && stashIntFindPointer(s_LootEventFromLootEntRef, pLeaderEnt->pPlayer->InteractStatus.interactTarget.entRef, &pEvent))
	{
		...
		if (pLootEnt && interaction_IsLootEntity(pLootEnt))
		{
			...
			FOR_EACH_IN_EARRAY(pLootEnt->pCritter->eaLootBags, InventoryBag, pBag)
			{
				...
				if (inv_bag_trh_FindItemByID(ATR_EMPTY_ARGS, CONTAINER_NOCONST(InventoryBag, pBag), itemID, pIter))
				{
					*ppLootBagOut = pBag;
					break;
				}
	...
	for (i = 0; i < eaSize(&pEventItem->peaTeamRolls); i++)
	{
		if (pEventItem->peaTeamRolls[i] && pEventItem->peaTeamRolls[i]->uiEntRef == pPlayerEnt->myRef)
		{
			if (pEventItem->peaTeamRolls[i]->eRollChoice == NeedOrGreedChoice_None)
			{
				...
				const char *pchUsageErrorKey = NULL;
				...
				// Only allow people to roll Need on loot their class can use.
				pItemDef = GET_REF(pItem->hItem);
				if (gConf.bNeedRollRequiresUsageMatch && 
					eChoice == NeedOrGreedChoice_Need && 
					pItemDef &&
					!(itemdef_trh_VerifyUsageRestrictionsClass(ATR_EMPTY_ARGS, pItemDef->pRestriction, CONTAINER_NOCONST(Entity, pPlayerEnt), &pchUsageErrorKey) && 
					itemdef_trh_VerifyUsageRestrictionsCharacterPath(ATR_EMPTY_ARGS, pItemDef->pRestriction, CONTAINER_NOCONST(Entity, pPlayerEnt), &pchUsageErrorKey) && 
					itemdef_trh_VerifyUsageRestrictionsAllegiance(ATR_EMPTY_ARGS, pItemDef->pRestriction, CONTAINER_NOCONST(Entity, pPlayerEnt), CONTAINER_NOCONST(Entity, pPlayerEnt), &pchUsageErrorKey) 
					) )
				{
					char* estrError = NULL;
					estrStackCreate(&estrError);
					entFormatGameMessageKey(pPlayerEnt, &estrError, pchUsageErrorKey, STRFMT_ITEMDEF(pItemDef), STRFMT_END);
					notify_NotifySend(pPlayerEnt, kNotifyType_ItemMoveFailed, estrError, REF_STRING_FROM_HANDLE(pItem->hItem), NULL);
