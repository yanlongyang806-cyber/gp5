...
typedef struct TeamMissionMapTransfer
{
	...
	S32							iSourceUnteamedPetsPerPlayer;
	S32							iAllowedPetsPerPlayer;
	S32							iUnteamedPetsPerPlayer;
...
typedef struct GSLTeamCBData
{
	...
	char pcSubjectString[256];	// We have a 256 limit here. Make sure we copy into it smaller than this
	char pcDataString[256];		// We have a 256 limit here. Make sure we copy into it smaller than this
...
static GSLTeamCBData *gslTeam_MakeCBData(EntityRef iRef, const char *pcErrorName)
{
	...
	pData->pcDataString[0]=0;
	pData->pcSubjectString[0]=0;
	return pData;
}

///////////////////////////////////////////////////////////////////////////////////////////
// Generic Notify for team systems.
///////////////////////////////////////////////////////////////////////////////////////////

AUTO_COMMAND_REMOTE;
void gslTeam_NotifyMessage(U32 uEntID, U32 uTeamID, const char *pcMessageKey, NotifyType eNotifyType)
{
...
void gslTeam_ServerDown(void *pData, void *pData2)
{
	if (pData!=NULL)
	{
		U32 uEntID = (ContainerID)((intptr_t)(pData));
		gslTeam_NotifyMessage(uEntID, 0, "TeamServer_Offline", kNotifyType_TeamError);
	}
}
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_CATEGORY(Team) ACMD_NAME(Team_AcceptInviteSidekick) ACMD_HIDE ACMD_PRODUCTS(StarTrek, FightClub);
...
AUTO_COMMAND ACMD_PRODUCTS(FightClub, StarTrek) ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_CATEGORY(Team) ACMD_NAME(Team_Sidekicking);
...
void gslTeam_cmd_EntReleaseOwned(SA_PARAM_NN_VALID Entity *pEnt)
{
	...
	if (pTeam->iGameServerOwnerID != 0)
	{
		...
		// Not sure if this needs to be here. We're in a debug command here, but it's probably going to be a convenience
		RemoteCommand_aslTeam_SetRequiresVoteKick(GLOBALTYPE_TEAMSERVER, 0, pTeam->iContainerID, 0);
...
static void gslTeam_cmd_Kick_CB(Entity *pEnt, ContainerID iSubjectID, U32 uiAccountID, U32 uiLoginServerID, GSLTeamCBData *pData)
{
	...
	if (pTeam->bVoteRequiredForKick)
	{
		gslTeam_SendResult(pEnt->myContainerID, entGetPersistedName(pEnt), "", pcErrorName, "TeamServer_Error_NotAllowedToKick", true, 0, 0, 0, 0, NULL);
		return;
	}
	

	// This is redundant to the "NotOnTeam check above?
...
// The subject is actually on the team. Do local check
	...
	if (team_IsLocal(pEnt))
	{
		// We should set a mode on the team or the map that means "team leaders are not allowed to kick" rather than checking PvP here.  [RMARR - 5/14/13]
		ZoneMapType eMapType = zmapInfoGetMapType(NULL);
		if (eMapType == ZMTYPE_PVP && !team_FindDisconnectedStubMemberID(pTeam, iSubjectID))
		{
			gslTeam_SendResult(pEnt->myContainerID, entGetPersistedName(pEnt), "", pcErrorName, "TeamServer_Error_NotAllowedToKick", true, 0, 0, 0, 0, NULL);
			return;
		}
...
static void gslTeam_cmd_VoteKick_CB(Entity *pEnt, ContainerID iSubjectID, U32 uiAccountID, U32 uiLoginServerID, GSLTeamCBData *pData)
{
	static char pcErrorName[] = "TeamServer_ErrorType_VoteKick";
	Team* pTeam;
	
	if (pEnt)
	{
		TeamMember* pTargetMember = NULL;
		StubTeamMember* pTargetStub = NULL;
		pTeam = team_GetTeam(pEnt);
		...
		pTargetMember = team_FindMemberID(pTeam, iSubjectID);
		pTargetStub = team_FindDisconnectedStubMemberID(pTeam, iSubjectID);

		if (pTargetMember==NULL && pTargetStub==NULL)
		{
			gslTeam_SendResult(pEnt->myContainerID, entGetPersistedName(pEnt), "", pcErrorName, "TeamServer_Error_NotOnTeam", true, 0, 0, 0, 0, NULL);
			return;
		}
		else if (iSubjectID == pEnt->myContainerID)
		{
			// Can't vote kick self
			// Fail Silently
			return;
		}
		else if (!pTeam->bVoteRequiredForKick)
		{
			// Not a vote kick team. Call regular kick.
			gslTeam_cmd_Kick_CB(pEnt, iSubjectID, uiAccountID, uiLoginServerID, pData);
			return;
		}
		else
		{
			// The subject is actually on the team.
			//
			
			bool bUnrestrictedKickAllowed = false;
			
			// Check eligibility here?
			//
			//		-Can't if already managing a vote kick
			//			-Detected on the TeamServer. (TODO for future? Add flag to team structure to determine here. There would be a delay.)
			//
			//		- If a player DC's, they cannot be kicked for a full minute to give the player a fair chance to reconnect.
			if (pTargetStub!=NULL && timeSecondsSince2000() - pTargetStub->iStubTime < 1.0f * 60.0f)
			{
				gslTeam_SendResult(pEnt->myContainerID, entGetPersistedName(pEnt), "", pcErrorName, "TeamServer_Error_VoteKickInvalid", true, 0, 0, 0, 0, NULL);
				return;
			}
			
			//		-Always allow if Out-of-instance for 3 minutes		
			//		--If a party member remains outside of a dungeon for more than 3 minutes, they become eligible for removal,
			//			regardless of other Vote Kick restrictions.
			//		(Should this operate for AFK?)

			if (pTeam->iGameServerOwnerID!=0 && pTargetMember!=NULL)	
			{
				// We are part of an owned team. See if the target has been off map too long. (Can't check this if they are disconnected)
				// Map status not updated more than once every 15 seconds. We hope partition/map transfers are more infrequent than that.
				if (pTargetMember->uOffMapTimeStamp > 0 && timeSecondsSince2000() - pTargetMember->uOffMapTimeStamp > 3.0f * 60.0f)
				{
					bUnrestrictedKickAllowed = true;
				}
			}

			if (!bUnrestrictedKickAllowed)
			{
				//		-Cooldown for instigator
				//			-TODO (see also penalty application on the TeamServer in VoteKickRegister)
				//
				//		-Start of instance? [5/15 minutes in Wow?]
				//			-Not sure if we really want this. We don't have a good way of timing 'start of instance'.
				//
				//		-In Combat?
				//			-Regular Kick does not do this.
				//
				//		-Only two members may be kicked from a group.
				//
				if (team_NumTotalMembers(pTeam) <= 3)
				{
					// WE ARE ASSUMING TEAMS OF 5!!!!
					// If we ever do raids (HA) this will not work.
			...
			// Tell the team server to start tracking this as a vote kick.
			// It will send out the votes to the appropriate parties.
	
			VerifyServerTypeExistsInShard(GLOBALTYPE_TEAMSERVER);
			RemoteCommand_aslTeam_BeginVoteKick(GLOBALTYPE_TEAMSERVER, 0, pEnt->pTeam->iTeamID, pEnt->myContainerID, iSubjectID,
													pData->pcDataString, pData->pcSubjectString,
													TEAM_SERVERDOWNHANDLER(pEnt));
		}
	}
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_CATEGORY(Team) ACMD_NAME(Team_VoteKick);
void gslTeam_cmd_VoteKick(SA_PARAM_NN_VALID Entity *pEnt, const char* pName, ACMD_SENTENCE pcReason)
{
	static char pcErrorName[] = "TeamServer_ErrorType_VoteKick";
	GSLTeamCBData *pData = gslTeam_MakeCBData(entGetRef(pEnt), "TeamServer_ErrorType_VoteKick");

	if (team_GetTeam(pEnt)==NULL)
	{
		// No team.
		gslTeam_SendResult(pEnt->myContainerID, entGetPersistedName(pEnt), "", pcErrorName, "TeamServer_Error_NotOnTeam", true, 0, 0, 0, 0, NULL);
		return;
	}
	// Theoretically we should querry some sort of thing set on the team to see if a vote is already in progress.
	// It's kind of a pain to get the data though because it's set on the TeamServer and would not be persisted so we can't use SUBSCRIBE
	// This command can essentially spam the TeamServer under user control. We are letting this slide for now.
	
	if (pcReason!=NULL)
	{
		strcpy_s(pData->pcDataString, 256, pcReason);
	}
	if (pName!=NULL)
	{
		strcpy_s(pData->pcSubjectString, 256, pName);
	}
	gslPlayerResolveHandle(pEnt, pName, gslTeam_cmd_VoteKick_CB, gslTeam_ResolveHandleFailure_CB, pData);
}


// The TeamServer is asking us to pass the vote request through to the client.
AUTO_COMMAND_REMOTE ACMD_IFDEF(APPSERVER);
void gslTeam_StartSingleVote(ContainerID uiEntID, const char* pchTargetName, const char* pchKickReason)
{
	Entity* pEntity = entFromContainerIDAnyPartition( GLOBALTYPE_ENTITYPLAYER, uiEntID );
	if (pEntity && pEntity->pPlayer)
	{
		ClientCmd_TeamUI_VoteKick_StartSingleVote(pEntity, pchTargetName, pchKickReason);
	}
}

// Start a vote to kick a player off your team. Called from uigens
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_CATEGORY(Team) ACMD_NAME(Team_VoteKickRegisterOneVote) ACMD_HIDE;
void gslTeam_cmd_VoteKickRegisterOneVote(SA_PARAM_NN_VALID Entity *pEnt, bool bKickConfirm)
{
	static char pcErrorName[] = "TeamServer_ErrorType_VoteKick";
	Team* pTeam;
	...
	if (pEnt)
	{
		...
		else
		{
			...
			VerifyServerTypeExistsInShard(GLOBALTYPE_TEAMSERVER);
			RemoteCommand_aslTeam_VoteKickRegister(GLOBALTYPE_TEAMSERVER, 0, pEnt->pTeam->iTeamID, pEnt->myContainerID, bKickConfirm,
													TEAM_SERVERDOWNHANDLER(pEnt));
		}
	}
}
...
void gslTeam_EntityUpdate(Entity *pEnt)
{
	...
	if (team_IsMember(pEnt)) {
		...
		//  (We can't update this more than once every 15 seconds. We hope partition/map transfers are more infrequent than that)
		if (pMember && iCurTime - pEnt->pTeam->iTimeSinceLastUpdate > TEAM_UPDATE_MIN_INTERVAL &&
			(iInstanceNum != pMember->iMapInstanceNumber ||
			partition_IDFromIdx(entGetPartitionIdx(pEnt))!= pMember->uPartitionID || 
										...
										RegionRules* pNextRules )
{
	...
	if ( (iCurrRegionType == WRT_Space || iCurrRegionType == WRT_SectorSpace) && (iNextRegionType == WRT_Ground) && (iNextZoneMapType != ZMTYPE_OWNED))
		return true;
...
static bool gslTeam_ShouldShowAwayTeamChooser( TeamMissionMapTransfer* pTeamMapTransfer )
{
	if ( pTeamMapTransfer && gConf.bUseAwayTeams)
	{
		...
		{
			if ( eDstRegionType == WRT_Ground && eDstZoneMapType != ZMTYPE_OWNED )
			{	
				return true;
			}
		}
		else if ( eSrcRegionType == WRT_Ground )
		{
			...
			if ( eSrcZoneMapType == ZMTYPE_STATIC && eDstZoneMapType == ZMTYPE_MISSION )
			{
				...
				(pTeamMapTransfer->iAllowedPetsPerPlayer > 0 || pTeamMapTransfer->iUnteamedPetsPerPlayer > 0)	&&
				(pTeamMapTransfer->iSourceAllowedPetsPerPlayer != pTeamMapTransfer->iAllowedPetsPerPlayer ||
				pTeamMapTransfer->iSourceUnteamedPetsPerPlayer != pTeamMapTransfer->iUnteamedPetsPerPlayer) )
			{
				return true;
			}
		}
											...
											DoorTransitionSequenceDef* pTransOverride )
{
	...
	pTeamMapTransfer->iSourceUnteamedPetsPerPlayer = pSrcRules->iUnteamedPetsPerPlayer;
	...
	pTeamMapTransfer->iUnteamedPetsPerPlayer = pDstRules->iUnteamedPetsPerPlayer;
...
static TeamInfoPlayer *gslTeam_GetTeamRequestInfoFromEnt(Entity *pEntity, TeamInfoRequest *pTeamInfoRequest, Language lang)
{
	int i, j, k;
			...
			if (pPetEnt->pChar)
			{
				...
				for ( j = 0; j < eaSize(&ppPowersTemp); ++j )
				{
					...
					if(pEntity->pChar)
					{
						...
						power_AutoDesc(iPartitionIdx,lang,pPow,pEntity->pChar,&tipe->pchToolTip,NULL,"<br>","<bsp><bsp>","- ",false,0,entGetPowerAutoDescDetail(pEntity,true),pExtract,NULL);
					}
...
void gslTeam_cmd_RequestTeamInfo(Entity *pClientEntity, TeamInfoRequest *pTeamInfoRequest)
{
	...
	if (pTeam)
	{
		...
		for (i = eaSize(&pTeam->eaMembers)-1; i >= 0; --i)
		{
			...
			if (!e) continue;
			tip = gslTeam_GetTeamRequestInfoFromEnt(e, pTeamInfoRequest,entGetLanguage(pClientEntity));
			eaPush(&teamInfo.eaPlayerList, tip);
		}
	}
	else
	{
		TeamInfoPlayer *tip = gslTeam_GetTeamRequestInfoFromEnt(pClientEntity, pTeamInfoRequest, entGetLanguage(pClientEntity));
		eaPush(&teamInfo.eaPlayerList, tip);
	}
...
AUTO_EXPR_FUNC(Player, Mission) ACMD_NAME(team_LowestMemberExpLevel);
int gslTeam_expr_LowestTeamMemberExpLevel(ExprContext* context)
{
	Entity* pEntity = exprContextGetVarPointerUnsafePooled(context, g_PlayerVarName);
	int iLowestLevel = 0;	// Returns zero if there is no entity
	int iTestLevel = 0;
	...
	if (pEntity!=NULL)
	{
		...
		{
			int i;
			// Level gets updated periodically on the TeamMember via aslTeam_UpdateInfo
			// It only is updated on the stub when the stub is created (when the disconnect happens)
			// But that is okay since we don't think an entity can level while disconnected

			// Go through all the connected members
			for(i = 0; i < eaSize(&pTeam->eaMembers); i++)
			{
				TeamMember *pTeamMember = pTeam->eaMembers[i];
				if (pTeamMember!=NULL)
				{
					iTestLevel = pTeamMember->iExpLevel;
					if (iLowestLevel==0 || iTestLevel < iLowestLevel)
					{
						iLowestLevel = iTestLevel;
					}
				}
			}

			// Go through all the disconnected members
			for(i = 0; i < eaSize(&pTeam->eaDisconnecteds); i++)
			{
				StubTeamMember *pStubTeamMember = pTeam->eaDisconnecteds[i];
				if (pStubTeamMember!=NULL)
				{
					iTestLevel = pStubTeamMember->iExpLevel;
					if (iLowestLevel==0 || iTestLevel < iLowestLevel)
					{
						iLowestLevel = iTestLevel;
					}
		...
		{
			// If there is no team, use the level of the single entity
			iTestLevel = entity_GetSavedExpLevel(pEntity);
