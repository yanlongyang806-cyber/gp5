...
static bool gslTeam_UpdateMemberList(TeamCorralInfo *pCorral)
{
	Entity **eaTeamEntities = NULL;
	TeamMember **eaTeamMembers = NULL;
	TeamCorralMember ***peaTeamCorralMembers = &pCorral->eaMembers;
	S32 iNumTeamMembers = 0;
	S32 iNumCorralMembers = eaSize(peaTeamCorralMembers);
	...
	if (pEnt && pCorral)
	{
		...
		for (i = 0; i < iNumTeamMembers; ++i)
		{
			...
			if (pCurEnt)
			{
				...
				{
					TeamCorralMember *pCorralMember = (*peaTeamCorralMembers)[j];
		...
		if (eaSize(peaTeamCorralMembers) > iNumTeamMembers)
		{
			for (i = 0; i < eaSize(peaTeamCorralMembers); ++i)
			{
				TeamCorralMember *pCorralMember = (*peaTeamCorralMembers)[i];
				bool bFound = false;
	...
	return eaSize(peaTeamCorralMembers) > 0;
}
...
{
	S32 i;
	for (i = eaSize(&s_eaTeamCorrals) - 1; i >= 0; i--)
	{
		TeamCorralInfo *pInfo = s_eaTeamCorrals[i];

		if (pInfo)
		{
			//This can remove elements from s_eaTeamCorrals
