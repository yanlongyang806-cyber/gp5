...
//  The Local team will no longer be associated with the team of iTeamID, and hTeam, and we will never re-create the team again if players join the map.
void DissociateTeamFromLocalTeam(LocalTeam *pLocalTeam)
{
	if (pLocalTeam!=NULL)
	{
		pLocalTeam->iTeamID = 0;
		pLocalTeam->uNextTeamUpdateRequestTime = 0;
		pLocalTeam->bDissociated = 1;
		REMOVE_HANDLE(pLocalTeam->hTeam);
	}
}

//  The Local team will no longer be connected with the team of iTeamID, and hTeam, but we can re-create the team again if players join the map.
void DisconnectTeamFromLocalTeam(LocalTeam *pLocalTeam)
{
	if (pLocalTeam!=NULL)
	{
		pLocalTeam->iTeamID = 0;
...
void gslTeam_NotifyOwnedTeamDestroyed(U32 iTeamID)
{
	...
	if (pLocalTeam)
	{
		DisconnectTeamFromLocalTeam(pLocalTeam);
	}
}
	...
	if (pInfo)
	{
		...
		if (pLocalTeam)
		{
			...
			if (!pLocalTeam->iTeamID)
			{
				...
				pLocalTeam->bTimesCreatedTeam++; // We created a team for this local team.

				{
					...
					if (pQueueDef!=NULL && pQueueDef->MapRules.bAllowVoteKick)
					{
						RemoteCommand_aslTeam_SetRequiresVoteKick(GLOBALTYPE_TEAMSERVER, 0, iTeamID, 1);
					}
				}
	...
	if (bUpdateGroups)
	{
		...
		for(iTeamIdx = 0; iTeamIdx < iNumTeams; iTeamIdx++)
		{
			...
			for (;iGroupIdx < iNumGroups; iGroupIdx++, iMemberIdx=0)
			{
				...
				for(;//Nothing
				eaiSize(&s_piTeamMembers)<TEAM_MAX_SIZE && iMemberIdx<eaSize(&pGroup->eaMatchMembers);
					iMemberIdx++)
				{
					QueueMatchMember *pMember = pGroup->eaMatchMembers[iMemberIdx];
					
					eaiPush(&s_piTeamMembers, pGroup->eaMatchMembers[iMemberIdx]->uEntID);
				}
			...
			if (pLocalTeam->iTeamID)
			{
				...
				if (pTeam!=NULL && pTeam->iGameServerOwnerID != 0)
				{
					...
					RemoteCommand_aslTeam_SetRequiresVoteKick(GLOBALTYPE_TEAMSERVER, 0, pTeam->iContainerID, 0);
																					// Let's keep control of the vote kick in this file. We could move it
																					// to ReleaseOwned on the server, but that's not symmetrical with us setting
																					// it in this file when we get the team creation callback.
					
					
					DissociateTeamFromLocalTeam(pLocalTeam);
				}
			}
		}
	}
...
void gslTeam_AutoTeamUpdate(QueuePartitionInfo *pInfo)
{
	...
	{	
		...
		if (pLocalTeam->bDissociated)
		{
			// If we've been dissociated we are essentially no longer in need of updating.
			// We keep our structure around so we can still possibly receive queue data or be queried?
			// We will either eventually be deleted
			//   A) When autoTeaming is turned off. (Which will cause gslTeam_CleanupLocalTeams)
			//   B) When the partition goes away.
			continue;
		}
		...
		if (pInfo->bAutoTeamMembersCanBeOnOtherMap)
		{
			...
			// NOTE: bUpdated will be set true here even after the queueServer has stopped sending us updates.
			//  we only know that the list of members from before it stopped updating.
			...
			if (g_QueueConfig.bAutoReleaseOnFinishedMapExit)
			{
				...
				if (pLocalTeam->bDissociated)
				{
					// Do this so we don't try re-creating the team if we actually do an autorelease.
					// In future frames we'll catch this in the 'continue' if above.
					bUpdated=false;
				}
			}
		}
...
//				RemoteCommand_aslTeam_DestroyWithReason(GLOBALTYPE_TEAMSERVER, 0, pLocalTeam->iTeamID, "Local team stopped updating");
				...
				RemoteCommand_aslTeam_SetRequiresVoteKick(GLOBALTYPE_TEAMSERVER, 0, pLocalTeam->iTeamID, 0);// Let's keep control of the vote kick in this file.
																					// We could move it
																					// to ReleaseOwned on the server, but that's not symmetrical with us setting
																					// it in this file when we get the team creation callback.
				DisconnectTeamFromLocalTeam(pLocalTeam);
