...
bool gslUGCTips_trh_RecordTip(ATR_ARGS, ATH_ARG NOCONST(GameAccountData) *pTipperGameAccountData, U32 uAuthorAccountID)
{
	...
	{
		if (!(UGCTipsInCurrentTimeBlock(pTipperGameAccountData->eaTipRecords[i]->uTimeOfTip)))
		{
	...
	pNewRecord->uTimeOfTip = ShardClock_SecondsSince2000();
...
ATR_LOCKS(pTipperEnt, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Hallegiance, .Hsuballegiance, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds");
...
void UGCTipsWithdraw_CB(TransactionReturnVal *pReturn, UGCWithdrawTipsData *pUGCWithdrawTipsData)
{
	...
	{
		pEnt = entForClientCmd(pUGCWithdrawTipsData->uAuthorEntityID, pEnt);
	}
...
static void FoundryWithdrawTips_APChangeKeyValue_CB(AccountKeyValueResult result, U32 accountID, SA_PARAM_NN_STR const char *key, ContainerID containerID, UGCWithdrawTipsData *pUGCWithdrawTipsData)
{
	...
	{
		pAuthorEntity = entForClientCmd(pUGCWithdrawTipsData->uAuthorEntityID, pAuthorEntity);
	}
