...
static bool gslUGC_UploadResources(const char *pNameSpace, bool bPublishData, char **estrErrorMessage, char *pComment_in, ...)
{
	...
	TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": About to call ServerLibPatchUpload, ns=%s", pNameSpace ));
	if (!ServerLibPatchUpload(files, "GameServer", estrErrorMessage, "UploadResources for ns %s(%s)", pNameSpace, pCommentToUse))
	{
		TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": ServerLibPatchUpload failed, ns=%s", pNameSpace ));
		return false;
	}
	loadend_printf("Saving Resources for %s to UGC PatchDB...done.\n", pNameSpace);
	TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": ServerLibPatchUpload succeeded, ns=%s", pNameSpace ));
...
bool gslUGC_ForkNamespace(UGCProjectData *pData, InfoForUGCProjectSaveOrPublish *pInfoForPublish, bool bUpload)
{
	...
	TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": About to generate, ns=%s", pInfoForPublish->pPublishNameSpace ));
	if (!ugcProjectGenerateOnServer(pData))
	{
		...
		TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": Generate failed, ns=%s", pInfoForPublish->pPublishNameSpace ));
		return false;
	}
	TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": Generate succeeded, ns=%s", pInfoForPublish->pPublishNameSpace ));
...
void PublishJobBegunCB(TransactionReturnVal *pReturn, InfoForUGCProjectSaveOrPublish *pInfoForPublish)
{
	...
	{
		SaveAndPublishSuccess( pEntity );
	}
	else
	{
		SaveAndPublishFail( pEntity, "Failed to start publish job", false );
...
enumTransactionOutcome trCancelUGCPublish(ATR_ARGS, int bWithdraw, NOCONST(UGCProject) *pProject, NOCONST(UGCProjectSeries) *pProjectSeries, char *pComment)
{
	...
	return UGCProject_trh_WithdrawProject(ATR_RECURSE, pProject, pProjectSeries, bWithdraw, pComment);
}
...
void gslUGC_DoProjectPublishPostTransaction(Entity *pEntity, InfoForUGCProjectSaveOrPublish *pInfoForPublish)
{
	...
	bool bDoSyncAtEndOfPublishing = !(isDevelopmentMode() && ugc_DevMode);

	if (0 == eaSize(&pInfoForPublish->ppMapNames) && !bDoSyncAtEndOfPublishing)
	{
	...
	if(eaSize(&pInfoForPublish->ppMapNames))
	{
		...
		if (!sbPublishSkipBeaconizing && !ugc_DevMode)
		{
			...
			pBeaconBinJob->iTimeout = gUGCPublish_Beaconizing_Timeout;
			pBeaconBinJob->iSoftTimeout = gUGCPublish_Beaconizing_SoftTimeout;
			eaPush(&pJobGroupDef->ppJobs, pBeaconBinJob);
		}
	}

	if (bDoSyncAtEndOfPublishing)
	{
	...
	AddCommandToRemoteCommandGroup(pJobGroupDef->pWhatToDoOnSuccess, GLOBALTYPE_UGCDATAMANAGER, 0, false,
								   "aslUGCDataManager_UpdateUGCProjectJobPerformance %u __JOB_PERFORMANCE_LOG__", pInfoForPublish->pProjectHeaderCopy->id );
	...
	else
	{
		char *pHeader = NULL;
		char *pBody = NULL;
		ContainerID iVirtualShardID = partition_VirtualShardIDFromIdx(entGetPartitionIdx(pEntity));
		...
		if (isProductionMode())
		{
			AddCommandToRemoteCommandGroup_SentNPCEmail(pJobGroupDef->pWhatToDoOnSuccess, pInfoForPublish->pProjectHeaderCopy->iOwnerAccountID, iVirtualShardID,
	...
	else
	{
		...
		if (isProductionMode())
		{	
			AddCommandToRemoteCommandGroup_SentNPCEmail(pJobGroupDef->pWhatToDoOnFailure, pInfoForPublish->pProjectHeaderCopy->iOwnerAccountID, iVirtualShardID,
													pInfoForPublish->pProjectHeaderCopy->pOwnerAccountName, TranslateMessageKeyDefault("UGC.TheFoundry", "The Foundry"), pHeader, pBody);
		}
	...
	else
	{
		...
		if (isProductionMode())
		{
			AddCommandToRemoteCommandGroup_SentNPCEmail(pJobGroupDef->pWhatToDoOnJobManagerCrash, pInfoForPublish->pProjectHeaderCopy->iOwnerAccountID, iVirtualShardID,
												pInfoForPublish->pProjectHeaderCopy->pOwnerAccountName, TranslateMessageKeyDefault("UGC.TheFoundry", "The Foundry"), pHeader, pBody);
		}
	...
	else
	{
		...
		if (isProductionMode())
		{
			AddCommandToRemoteCommandGroup_SentNPCEmail(pInfoForPublish->pWhatToDoIfPublishJobDoesntStart, pInfoForPublish->pProjectHeaderCopy->iOwnerAccountID, iVirtualShardID,
												pInfoForPublish->pProjectHeaderCopy->pOwnerAccountName, TranslateMessageKeyDefault("UGC.TheFoundry", "The Foundry"), pHeader, pBody);
		}
