...
#include "dynNodeInline.h" // for bounds checking
...
static int iCurrentProjectDataLastAutosaveTime = 0;
...
char* g_strUGCPublishAndQuitNotityEntityShardName;
ContainerID g_UGCPublishAndQuitNotityEntityContainerID;

AUTO_COMMAND ACMD_NAME(UGCPublishAndQuitNotifyEntity) ACMD_CMDLINE;
void UGCPublishAndQuitNotifyEntity( char* shardName, ContainerID ent)
{
	if( shardName && ent ) {
		StructCopyString( &g_strUGCPublishAndQuitNotityEntityShardName, shardName );
		g_UGCPublishAndQuitNotityEntityContainerID = ent;
	}
}

// Expected length of stalls for resource uploading paths
int giUGCLeavePreviewStallySeconds = 2 * 60;
AUTO_CMD_INT(giUGCLeavePreviewStallySeconds, UGCLeavePreviewStallySeconds) ACMD_AUTO_SETTING(Ugc, GAMESERVER);
int giUGCPreviewTransferStallySeconds = 2 * 60;
AUTO_CMD_INT(giUGCPreviewTransferStallySeconds, UGCPreviewTransferStallySeconds) ACMD_AUTO_SETTING(Ugc, GAMESERVER);
int giUGCEnterPreviewStallySeconds = 2 * 60;
AUTO_CMD_INT(giUGCEnterPreviewStallySeconds, UGCEnterPreviewStallySeconds) ACMD_AUTO_SETTING(Ugc, GAMESERVER);
int giUGCSaveStallySeconds = 2 * 60;
AUTO_CMD_INT(giUGCSaveStallySeconds, UGCSaveStallySeconds) ACMD_AUTO_SETTING(Ugc, GAMESERVER);
int giUGCPublishStallySeconds = 2 * 60;
AUTO_CMD_INT(giUGCEnterPreviewStallySeconds, UGCPublishStallySeconds) ACMD_AUTO_SETTING(Ugc, GAMESERVER);
...
int giUGCPreviewMinTimeBetweenAutosavesInSeconds = 5 * 60;
AUTO_CMD_INT(giUGCPreviewMinTimeBetweenAutosavesInSeconds, UGCPreviewMinTimeBetweenAutosavesInSeconds) ACMD_AUTO_SETTING(Ugc, GAMESERVER);
...
{
	return (gConf.bUserContent && gbUGCGenerallyEnabled && s_bUGCPlayingEnabled) || ugc_DevMode;
}
...
{
	bCurrentProjectDataDirty = false;
	iCurrentProjectDataLastAutosaveTime = timeSecondsSince2000();
}
...
void gslUGC_SendAutosaveIfNecessary(bool force)
{
	if (bCurrentProjectDataDirty)
	{
		...
		if (!force && iCurrentProjectDataLastAutosaveTime > 0 && (int)timeSecondsSince2000() - iCurrentProjectDataLastAutosaveTime < giUGCPreviewMinTimeBetweenAutosavesInSeconds)
		{
			log_printf( LOG_UGC, "%s: Skipping autosave -- only %d seconds since last save.", __FUNCTION__, timeSecondsSince2000() - iCurrentProjectDataLastAutosaveTime );
			return;
		}
...
{
	ClientCmd_ugcEditorForceUpdateAutosave( pEntity );
}
...
static void gslUGC_TransferArriveAtMap(Entity *pEntity, const char *map_name, const char *spawn_point, Vec3 spawn_pos, Vec3 spawn_pyr)
{
	...
	// Move the player's entity to an estimate spawn point to prevent crashing on bad coordinates during worldLoadZoneMapByName
	if (spawn_pos && CHECK_DYNPOS_NONFATAL_WITH_TOLERANCE(spawn_pos, 50.f))
	{
		if (spawn_pyr) {
			copyVec3(spawn_pyr, estimatePYR);
			estimatePYR[0] = 0.f;
		}
		PYRToQuat(estimatePYR, estimateQuat);
		entSetPosRotFace(pEntity, spawn_pos, estimateQuat, estimatePYR, true, false, "estimateSpawnLoc");
	} else {
		PYRToQuat(estimatePYR, estimateQuat);
		entSetPosRotFace(pEntity, zerovec3, estimateQuat, estimatePYR, true, false, "estimateSpawnLoc");
	}
	...
	if (spawn_point)
	{
		spawnpoint_MovePlayerToNamedSpawn(pEntity, spawn_point, NULL, NULL, 0);
	}
	else if (spawn_pos)
	{
		if (CHECK_DYNPOS_NONFATAL_WITH_TOLERANCE(spawn_pos, 50.f)) {
			spawnpoint_MovePlayerToLocation(pEntity, spawn_pos, estimateQuat, NULL, true);
		} else {
	...
	if (s_iNumFramesBeforeTransfer > 0)
	{
		...
		if (!s_iNumFramesBeforeTransfer)
		{
			...
			if( entTransfer ) {
				...
				{
					gslUGC_TransferArriveAtMap(entTransfer, s_pcMapName, EMPTY_TO_NULL( s_pcSpawnPoint ), NULL, NULL);				
				}
...
void gslUGC_DoPlayCB( bool succeeded, UserData entityRef )
{
	...
	gslUGC_ProjectAddPlayComponentData( entGetPartitionIdx(pEntity), gGSLState.pLastPlayData, &res.eaComponentData );
	...
	eaDestroyStruct(&res.eaComponentData, parse_UGCPlayComponentData);
}
...
void gslUGC_DoPlay( Entity *pEntity, UGCProjectData* ugc_proj, const char *map_name, U32 objective_id, Vec3 spawn_pos, Vec3 spawn_rot, bool only_reload )
{
	...
	log_printf( LOG_UGC, "%s: Start, ns=%s, map_name=%s, objective_id=%u, spawn_pos=%f,%f,%f, only_reload=%d",
				__FUNCTION__, ugc_proj ? ugcProjectDataGetNamespace( ugc_proj ) : NULL,
				map_name, objective_id, SAFE_AREF( spawn_pos, 0 ),
				SAFE_AREF( spawn_pos, 1 ), SAFE_AREF( spawn_pos, 2 ),
				only_reload );

	if( !ugc_proj ) {
		UGCPlayResult res = { UGC_PLAY_NO_PROJECT };
		ClientCmd_gclUGCProcessPlayResult( pEntity, &res );
		AssertOrAlertWarning( "UGC_PLAY_WITH_NO_PROJECT", "gslUGC_DoPlay was called with a null ugc_proj parameter.  This is supposed to be prevented by the UI.  Entity=%u",
							  entGetContainerID( pEntity ));
		return;
	}
			...
			if (eaSize(&project->pRestrictionProperties->eaFactions) > 0)
			{
				...
				FOR_EACH_IN_EARRAY(project->pRestrictionProperties->eaFactions, WorldUGCFactionRestrictionProperties, faction)
				{
					if (stricmp(faction->pcFaction, allegiance->pcName) == 0 ||
						(subAllegiance && stricmp(faction->pcFaction, subAllegiance->pcName) == 0))
					{
						found = true;
					}
				}
	...
	if( !only_reload ) {
		...
		TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": About to generate, ns=%s", ugcProjectDataGetNamespace( ugc_proj )));
		if (!ugcProjectGenerateOnServerEx(ugc_proj, /*override_spawn_map=*/map_name, /*override_spawn_pos=*/spawn_pos, /*overide_spawn_rot=*/spawn_rot))
		{
			...
			ClientCmd_gclUGCProcessPlayResult(pEntity, &res);
			TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": Generate failed, ns=%s", ugcProjectDataGetNamespace( ugc_proj )));
			return;
		}
		log_printf( LOG_UGC, "%s: After ugcProjectGenerateOnServerEx(), ns=%s",
					__FUNCTION__, ugcProjectDataGetNamespace( ugc_proj ));
		TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": Generate succeeded, ns=%s", ugcProjectDataGetNamespace( ugc_proj )));
		...
		if (spawn_name)
			spawnpoint_MovePlayerToNamedSpawn(pEntity, spawn_name, NULL, NULL, 0);
		else
			spawnpoint_MovePlayerToStartSpawn(pEntity, NULL, true);
...
// UGCSearchConfigStartup is here only to make the bin file used by the UGCSearchManager.
AUTO_STARTUP(UGCServer) ASTRT_DEPS(UGCReporting, UGCSearchCache, UGCTags, UGCSearchConfigStartup);
...
void gslUGC_DeleteNamespaceDataFiles(const char *ns)
{
	...
	TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": About to do system command to delete files, cmd=%s", cmd ));
	...
	TellControllerToLog( STACK_SPRINTF( __FUNCTION__ ": System command finished, cmd=%s", cmd ));
...
static F32 s_fUGCPlayingTimeFractionOfAverageAllowed = 0.5f;
AUTO_CMD_FLOAT(s_fUGCPlayingTimeFractionOfAverageAllowed, UGCPlayingTimeFractionOfAverageAllowed) ACMD_AUTO_SETTING(Ugc, GAMESERVER);

AUTO_TRANS_HELPER
ATR_LOCKS(pUGCMissionData, ".Fplayingtimeinminutes, .Faveragedurationinminutes");
F32 ugcMissionData_trh_GetPlayingTimeForRewards(ATR_ARGS, ATH_ARG NOCONST(UGCMissionData) *pUGCMissionData)
{
	if(NONNULL(pUGCMissionData))
	{
		return (pUGCMissionData->fPlayingTimeInMinutes < (s_fUGCPlayingTimeFractionOfAverageAllowed * pUGCMissionData->fAverageDurationInMinutes))
			? pUGCMissionData->fPlayingTimeInMinutes
			: pUGCMissionData->fAverageDurationInMinutes;
	}
...
{
	NOCONST(UGCMissionData) *pUGCMissionDataNoConst = CONTAINER_NOCONST(UGCMissionData, pUGCMissionData);
	return ugcMissionData_trh_GetPlayingTimeForRewards(ATR_EMPTY_ARGS, pUGCMissionDataNoConst);
}

AUTO_TRANS_HELPER
ATR_LOCKS(pMissionInfo, ".Missions");
NOCONST(UGCMissionData) *ugc_trh_FindOrCreateMissionDataForNameSpace(ATR_ARGS, ATH_ARG NOCONST(MissionInfo) *pMissionInfo, const char *strNameSpace)
{
	if(strNameSpace)
	{
		FOR_EACH_IN_EARRAY(pMissionInfo->missions, NOCONST(Mission), pMission)
		{
			char pchNameSpace[RESOURCE_NAME_MAX_SIZE], pchBase[RESOURCE_NAME_MAX_SIZE];
			if(resExtractNameSpace(pMission->missionNameOrig, pchNameSpace, pchBase) // this is a namespace mission
					&& namespaceIsUGC(pchNameSpace) // this is a UGC namespace mission
					&& 0 == stricmp(strNameSpace, pchNameSpace) // this is the namespace we are looking for
					&& stricmp(pchBase, "Mission") == 0) // this is the main mission (and not the return link or some other piggy backing mission)
			{
				if(!pMission->pUGCMissionData)
					pMission->pUGCMissionData = StructCreateNoConst(parse_UGCMissionData);

				return pMission->pUGCMissionData;
			}
		}
		FOR_EACH_END;
	}
