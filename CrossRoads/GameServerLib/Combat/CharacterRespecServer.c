...
static void CharacterGetFreeRespecSteps(int iPartitionIdx, Character *pchar, PowerTreeSteps *pStepsReusltRespec, PowerTreeSteps *pStepsResultBuy, PTRespecGroupType eRespecType)
{
	...
	entity_PowerTreeStepsRespecSteps(iPartitionIdx,pchar->pEntParent,NULL,pSteps,NULL,pStepsReusltRespec,pStepsResultBuy,kPTRespecGroup_ALL,false,false);
...
enumTransactionOutcome trEntity_UseGameAccountRespec(ATR_ARGS, NOCONST(Entity) *e, NOCONST(GameAccountData) *pData, NOCONST(AccountProxyLockContainer) *pLockContainer, const char *pKey, PowerTreeSteps *pStepsRespec, PowerTreeSteps *pStepsBuy, const ItemChangeReason *pReason, int eRespecType)
{
	enumTransactionOutcome eOutcome = TRANSACTION_OUTCOME_SUCCESS;
...
AUTO_TRANSACTION
ATR_LOCKS(e, ".Pchar.Ppsecondarypaths, .Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.Pscpdata.Erscp, .Pinventoryv2.Ppinventorybags[], .Hallegiance, .Hsuballegiance, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pchar.Pppointspentpowertrees, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Pchar.Uipowertreemodcount, .Pchar.Uilastfreerespectime, .Pchar.Uilastforcedrespectime, .Pplayer.Icreatedtime, .Pchar.Pppowertrees, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds");
...
static void UseGameAccountRespec_CB(AccountKeyValueResult result, U32 accountID, SA_PARAM_NN_STR const char *key, ContainerID containerID, SA_PARAM_NN_VALID GameAccountRespecInfo *pRespecInfo)
{
	...
	if (pEnt && result == AKV_SUCCESS)
	{
		...
		AutoTrans_trEntity_UseGameAccountRespec(pReturnVal, GLOBALTYPE_GAMESERVER,
			...
			key, pRespecInfo->pStepsRespec, pRespecInfo->pStepsBuy, &pRespecInfo->reason, pRespecInfo->eRespecType);
	}
	else
	{
		if (pEnt)
		...
		character_HasRespecContact(e))
	{
		...
		else
		{
			...
			if(iTotalCost > 0)
			{
				...
				pRespecInfo->eRespecType = kPTRespecGroup_ALL;
...
// Respecs your PowerTrees and StatPoints to nothing using a token item
AUTO_COMMAND ACMD_NAME("Character_UseItemOrGameAccountRespec") ACMD_CATEGORY(Powers) ACMD_ACCESSLEVEL(0) ACMD_HIDE;
void UseItemOrGameAccountRespec(Entity *e, PTRespecGroupType eRespecType)
{
	bool bCanUseItem = false;
	bool bCanUseGAD = false;
	PowerTreeSteps *pSteps = NULL;
	const char *pchRespecTokenGAD = PowerTree_GetRespecTokensPurchaseGAD(eRespecType);

	if(!e || !e->pChar || !entIsPlayer(e))
		return;

	bCanUseItem = (inv_GetNumericItemValue(e, PowerTree_GetRespecTokensPurchaseItem(eRespecType)) > 0);
	if (!bCanUseItem && pchRespecTokenGAD)
	{
		bCanUseGAD = slGAD_CanChangeAttribClampedExtract(entity_GetCachedGameAccountDataExtract(e), pchRespecTokenGAD, -1, 0, 100000);
	}

	if ((!bCanUseItem && !bCanUseGAD) || !contact_IsNearRespec(e))
	{
		return;
	}
	...
	else
	{
		...
		if(iTotalCost > 0 || eaSize(&e->pChar->ppSecondaryPaths) > 0)
		{
			if (bCanUseItem)
			{
				...
				TransactionReturnVal *pReturn = LoggedTransactions_CreateManagedReturnValEnt("ItemRespec", e, Respec_CB, pRef);
				PowerTreeSteps *pStepsRespec = StructCreate(parse_PowerTreeSteps);
				PowerTreeSteps *pStepsBuy = StructCreate(parse_PowerTreeSteps);
				ItemChangeReason reason = {0};
			...
			else if (bCanUseGAD)
			{
				...
				pRespecInfo->pStepsRespec = StructCreate(parse_PowerTreeSteps);
				pRespecInfo->pStepsBuy = StructCreate(parse_PowerTreeSteps);
				pRespecInfo->eRespecType = eRespecType;

				CharacterGetFreeRespecSteps(entGetPartitionIdx(e), e->pChar, pRespecInfo->pStepsRespec, pRespecInfo->pStepsBuy, eRespecType);
				*pRespecInfo->pRef = entGetRef(e);
				...
				if (gConf.bDontAllowGADModification)
				{
					APChangeKeyValue(e->pPlayer->accountID, pchRespecTokenGAD, -1, UseGameAccountRespec_CB, pRespecInfo);
				}
				else
				{
					UseGameAccountRespec_CB(AKV_SUCCESS, e->pPlayer->accountID, pchRespecTokenGAD, 0, pRespecInfo);
				}
			}
		}
		else
...
AUTO_TRANSACTION
	ATR_LOCKS(e, ".Psaved.pSCPdata.Isummonedscp, .Psaved.pSCPdata.Fcachedpetbonusxppct, .Psaved.pSCPdata.erSCP, .Pinventoryv2.Ppinventorybags[], .Hallegiance, .Hsuballegiance, .Psaved.Ppuppetmaster.Curtempid, .Psaved.Ppallowedcritterpets, .Pchar.Pppointspentpowertrees, .Pplayer.Pplayeraccountdata.Eagamepermissionmaxvaluenumerics, .Pchar.Ilevelexp, .Pchar.Uipowertreemodcount, .Pchar.Uilastfreerespectime, .Pchar.Uilastforcedrespectime, .Pplayer.Icreatedtime, .Pchar.Pppowertrees, .Pchar.Uipoweridmax, .Psaved.Ipetid, .Psaved.Ppuppetmaster.Curid, .Psaved.Ppuppetmaster.Curtype, .Psaved.Ppownedcontainers, pInventoryV2.ppLiteBags[], .Pplayer.Eaplayernumericthresholds");
...
AUTO_TRANSACTION ATR_LOCKS(e, ".Pchar.Ppsecondarypaths, .Pchar.hClass, .Pchar.hPath, .pchar.Pppowertrees, .pchar.Pppointspentpowertrees");
enumTransactionOutcome trCharacter_AssignArchetype(ATR_ARGS, NOCONST(Entity)* e, const char* pchArchetype)
{
	...
	character_trh_RemoveAllSecondaryCharacterPaths(ATR_PASS_ARGS, e, true);
...
void PlayerRespecAsArchetypeAutobuyCallback(TransactionReturnVal* returnVal, EntityRef *pEntRef)
{
	...
	if (returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		entity_PowerTreeAutoBuy(entGetPartitionIdx(pEnt->pChar->pEntParent), pEnt, NULL);
...
void PlayerRespecAsArchetypeCallback(TransactionReturnVal* returnVal, EntityRef *pEntRef)
{
	...
	{
		PowerTreeSteps* pSteps = StructCreate(parse_PowerTreeSteps);
		if(entity_PowerTreeAutoBuySteps(entGetPartitionIdx(pEnt), pEnt, NULL, pSteps))
		{
			ItemChangeReason reason = {0};
			TransactionReturnVal* returnValNew = NULL;
			
			inv_FillItemChangeReason(&reason, pEnt, "Powers:Respec", "RespecAsArchetype");

			returnValNew = LoggedTransactions_CreateManagedReturnValEnt("AssignArchetypeAutobuyStep", pEnt, PlayerRespecAsArchetypeAutobuyCallback, pEntRef);
			AutoTrans_trEntity_PowerTreeStepsAdd(returnValNew, GetAppGlobalType(), entGetType(pEnt), entGetContainerID(pEnt),
												 0, 0, pSteps, false, &reason);
		}
		StructDestroy(parse_PowerTreeSteps,pSteps);
	}
	else
...
void CharacterPathAddCB(TransactionReturnVal* returnVal, EntityRef *pEntRef)
{
	if (pEntRef && returnVal && returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		entity_PowerTreeAutoBuy(iPartitionIdx,pEnt,NULL);
	}
}
