...
void gslBuyPowerCartItems(Entity* pPlayerEnt)
{
	if (	pPlayerEnt
		&&	pPlayerEnt->pChar
		&&	pPlayerEnt->pPlayer
		&&	(!character_HasMode(pPlayerEnt->pChar,kPowerMode_Combat) || entGetAccessLevel(pPlayerEnt) > ACCESS_GM_FULL)) // AL 7+ people can buy powers anytime, and face the consequences.
	{
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0) ACMD_SERVERCMD ACMD_PRIVATE;
void gslBuyPowerCartItemsWithRespec(Entity *pPlayerEnt, PTRespecGroupType eRespecType)
{
	...
	if (!character_HasMode(pPlayerEnt->pChar,kPowerMode_Combat))
	{
		...
		if(pFakeEntity)
		{
			...
			character_GetPowerTreeSteps(CONTAINER_NOCONST(Character, pPlayerEnt->pChar),pStepsRespec, false, eRespecType);
			character_PowerTreeStepsCostRespec(iPartitionIdx, pPlayerEnt->pChar,pStepsRespec,0);
			bSuccess = entity_PowerTreeStepsRespecSteps(iPartitionIdx,pFakeEntity,NULL,pStepsRespec,NULL,NULL,NULL,eRespecType,true,false);
			...
			if(bSuccess)
			{
				...
				entity_PowerTreeStepsRespec(iPartitionIdx,pPlayerEnt,pStepsRespec,pStepsBuy,LoggedTransactions_CreateManagedReturnValEnt("PowerCartPurchase",pPlayerEnt,trPowerCart_BuyCartItems_PostTransaction,pData),eRespecType,false);
			}
		}
