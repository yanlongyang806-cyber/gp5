...
void CombatReactivePower_ActivateServer(Entity* pEnt, F32 fActivateYaw, U32 uiActivateTime, U8 uActId, F32 fTimeOffset)
{
	if (pEnt && pEnt->pChar)
	{
		...
		if (!CombatReactivePower_CanStartActivation(pEnt->pChar, pInfo, pDef, NULL) || 
			!CombatReactivePower_CanActivate(pEnt->pChar, pDef, NULL))
		{	
			ClientCmd_gclCombatReactivePower_ForceDeactivate(pEnt);
			return;
		}
...
void CombatReactivePower_DeactivateServer(Entity* pEnt, U32 uiActivateTime, U8 eState, F32 fTimer)
{
	if (pEnt && pEnt->pChar)
	{
		...
		{
			uiActivateTime = pmTimestampFrom(uiActivateTime, -gConf.combatUpdateTimer);
