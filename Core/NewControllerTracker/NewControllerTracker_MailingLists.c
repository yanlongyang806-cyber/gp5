...
static void SendEmailToList(char **ppRecipients, char *pSubject, char *pBody, int iBodyLength, bool bHTML)
{
	...
	{
		char emailFileName[CRYPTIC_MAX_PATH];
		static int siCounter = 0;
		FILE *pFile;

		sprintf(emailFileName, "c:\\temp\\email_%d_%u.gz", getpid(), siCounter++);

		pFile = fopen(emailFileName, "wbz");
		...
		if (pFile)
		{
			...
			estrCopy2(&pReq->attachfilename, emailFileName);
			estrCopy2(&pReq->attachsuggestedname, "email.gz");
		}
		else
		{
			estrConcat(&pReq->body, pBody, gZipAndAttachPrefixLen);
	...
	char *pShortBodyString, char **ppOutWhoSentToEString)
{
	...
	bool bDontSendToShort = !pShortBodyString;
	bool bDontSendToFull = !pFullBodyString;
	...
	estrCopy2(&pBodyStringCopy, pFullBodyString ? pFullBodyString : "");
