...
static bool sbDumpStats = false;
AUTO_CMD_INT(sbDumpStats, DumpStats) ACMD_CMDLINE;
...
void DumpStats(TimedCallback *callback, F32 timeSinceLastCallback, UserData userData)
{
	...
	if (siTicksLastTime)
	{
		printf("During last 10 seconds, ran at %I64d fps, processed %I64d tickets per second\n",
			...
			(giNumMCPTicketsProcessed - siTicketsLastTime) / 10);
	}

	siTicksLastTime = gUtilitiesLibTicks;
	siTicketsLastTime = giNumMCPTicketsProcessed;
}

static U32 uCurrentXMLRPCIP = 0;

static void XMLRPCRequest(U32 uIP, const char *pXML)
{
	uCurrentXMLRPCIP = uIP;
}

U32 CurrentXMLRPCIP(void)
{
	return uCurrentXMLRPCIP;
}
