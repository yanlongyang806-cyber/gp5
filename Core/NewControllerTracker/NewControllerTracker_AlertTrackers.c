...
AUTO_FIXUPFUNC;
TextParserResult fixupAlertTrackerThrottle_PerSystem(AlertTrackerThrottle_PerSystem *pThrottle, enumTextParserFixupType eType, void *pExtraData)
{
	switch(eType)
	{
	case FIXUPTYPE_DESTRUCTOR:
		if (pThrottle->pCounter)
		{
			SimpleEventCounter_Destroy(&pThrottle->pCounter);
		}
	}
...
static bool ThrottleMatchesSystem(AlertTrackerThrottle *pThrottle, CriticalSystem_Status *pSystem)
{
	if (eaFindString(&pThrottle->ppSystems, "all") != -1)
	{
		return true;
	}

	if (eaFindString(&pThrottle->ppSystems, pSystem->pConfig->pName) != -1)
	{
		return true;
	}

	if (eaFindString(&pThrottle->ppSystems, pSystem->pProductName) != -1)
	{
		return true;
	}
...
static  AlertTrackerThrottle *FindAlertTrackerThrottleForSystem(AlertTracker *pTracker, CriticalSystem_Status *pSystem)
{
	FOR_EACH_IN_EARRAY_FORWARDS(pTracker->ppThrottles, AlertTrackerThrottle, pThrottle)
	{
		if (ThrottleMatchesSystem(pThrottle, pSystem))
		{
			return pThrottle;
		}
	}
...
enumAlertTrackerThrottlingState AlertTrackers_TrackAlert(Alert *pAlert, CriticalSystem_Status *pSystem, char **ppThrottlingComment)
{
	...
	AlertTrackerThrottle *pThrottle;
	...
	pThrottle = FindAlertTrackerThrottleForSystem(pTracker, pSystem);

	if (pThrottle)
	{
		AlertTrackerThrottle_PerSystem *pPerSystemThrottle = eaIndexedGetUsingString(&pThrottle->ppPerSystemThrottles, pSystem->pConfig->pName);
		if (!pPerSystemThrottle)
		{
			pPerSystemThrottle = StructCreate(parse_AlertTrackerThrottle_PerSystem);
			pPerSystemThrottle->pSystemName = strdup(pSystem->pConfig->pName);
			pPerSystemThrottle->pCounter = SimpleEventCounter_Create(pThrottle->iHowMany, pThrottle->iWithin, pThrottle->iResetLength);
			eaPush(&pThrottle->ppPerSystemThrottles, pPerSystemThrottle);
		}

		//should never happen, but just to be thorough
		if (!pPerSystemThrottle->pCounter)
		{
			pPerSystemThrottle->pCounter = SimpleEventCounter_Create(pThrottle->iHowMany, pThrottle->iWithin, pThrottle->iResetLength);
		}

		if (SimpleEventCounter_ItHappened(pPerSystemThrottle->pCounter, timeSecondsSince2000()))
		{
			char *pDuration1 = NULL;
			char *pDuration2 = NULL;
			estrStackCreate(&pDuration1);
			estrStackCreate(&pDuration2);

			timeSecondsDurationToPrettyEString(pThrottle->iWithin, &pDuration1);
			timeSecondsDurationToPrettyEString(pThrottle->iResetLength, &pDuration2);

			estrPrintf(ppThrottlingComment, "!!!A throttled alert happened %d times within the past %s.\nYou won't see this again for %s\n\n",
				pThrottle->iHowMany, pDuration1, 
				pDuration2);

			return ALERTTHROTTLING_THROTTLING_ON_AND_MET;
		}
		
		return ALERTTHROTTLING_THROTTLING_ON_NOT_MET;
	}
	else
	{
		return ALERTTHROTTLING_NOT_THROTTLED;
	}
}
...
static void UpdateAlertTrackerTimeouts(AlertTracker *pTracker)
{
	...
	for (i = eaSize(&pTracker->ppThrottles) - 1; i >= 0; i--)
	{
		AlertTrackerThrottle *pThrottle = pTracker->ppThrottles[i];
		if (pThrottle->iTurnoffTime && pThrottle->iTurnoffTime < timeSecondsSince2000())
		{
			StructDestroy(parse_AlertTrackerThrottle, pThrottle);
			eaRemove(&pTracker->ppThrottles, i);
		}
	}
}
...
void AlertTrackers_InitSystem(void)
{
	...
	if (fileExists(ALERTTRACKERS_FILE))
	{
		...
		for (i=eaSize(&spAlertTrackerList->ppTrackers)-1; i >=0; i--)
		{
			...
			UpdateAlertTrackerTimeouts(pTracker);
			if (eaSize(&pTracker->ppNetopsComments) == 0 && eaSize(&pTracker->ppThrottles) == 0)
			{
...
AlertTrackerThrottle *FindThrottleByID(AlertTracker *pTracker, int iID)
{
	int i;

	for (i = 0; i < eaSize(&pTracker->ppThrottles); i++)
	{
		if (pTracker->ppThrottles[i]->iID == iID)
		{
			return pTracker->ppThrottles[i];
		}
	}
...
int FindNextThrottleID(AlertTracker *pTracker)
{
	int iRet = 1;

	while (FindThrottleByID(pTracker, iRet))
	{
		iRet++;
	}
...
AUTO_COMMAND;
char *AddThrottleForAlertTracker(const char *pAlertKey, char *pSystems, int iHowMany, int iWithin, int iReset, int iLifespan)
{
	const char *pKey = allocAddString(pAlertKey);
	AlertTracker *pTracker;
	AlertTrackerThrottle *pThrottle;
	...
	if (iWithin < 1)
	{
		return "Need positive # seconds for Within";
	}

	if (iHowMany < 2 || iHowMany > MAX_ALERTTRACKER_THROTTLE_HOWMANY)
	{
		return "HowMany out of range, must be less than " STRINGIFY(MAX_ALERTTRACKER_THROTTLE_HOWMANY);
	}

	pThrottle = StructCreate(parse_AlertTrackerThrottle);
	pThrottle->pParentKey = pKey;
	DivideString(pSystems, " ,", &pThrottle->ppSystems, DIVIDESTRING_POSTPROCESS_STRIP_WHITESPACE | DIVIDESTRING_POSTPROCESS_DONT_PUSH_EMPTY_STRINGS);
	pThrottle->iID = FindNextThrottleID(pTracker);
	pThrottle->iHowMany = iHowMany;
	pThrottle->iWithin = iWithin;
	pThrottle->iResetLength = iReset * 60;
	if (iLifespan)
	{
		pThrottle->iTurnoffTime = timeSecondsSince2000() + iLifespan * 3600;
	}

	eaPush(&pTracker->ppThrottles, pThrottle);
...
AUTO_COMMAND;
char *RemoveThrottle(const char *pParentName, int iID, char *pConfirm)
{
	const char *pKey = allocAddString(pParentName);
	AlertTracker *pTracker;
	int i;
	...
	for (i = 0; i < eaSize(&pTracker->ppThrottles); i++)
	{
		if (pTracker->ppThrottles[i]->iID == iID)
		{
			StructDestroy(parse_AlertTrackerThrottle, pTracker->ppThrottles[i]);
			eaRemove(&pTracker->ppThrottles, i);
