...
typedef struct ShardInfo_Basic
{
	...
	char **ppLanguages; //"english", "french", etc, list of all languages this shard supports.
		//to be precise, these are LocaleTable[foo].name
	...
	//actually set every time a packet is received by getting the IP from the link
