...
#include "NEwControllerTracker_ShardCom.h"
...
int GetNextIssueNumber(void)
{
	...
	initRegReader(reader, "HKEY_LOCAL_MACHINE\\Software\\RaGEZONE\\ControllerTracker");
...
void CriticalSystems_PeriodicUpdate(void)
{
	...
	for (i=0; i < eaSize(&ppCriticalSystems); i++)
	{
		...
		case CRITSYSTEMSTATUS_RUNNING:
			...
			if (iInactivityTime > STATUS_REPORTING_INTERVAL_NORMAL * 2)
			{
...
void HandleGenericStatusInternal(StatusReporting_Wrapper *pWrapper, U32 uIP, NetLink *pLink)
{
	CriticalSystem_Status *pSystem;
	int i;
	static char *pMachineNameOrIPString = NULL;

	if (!stashFindPointer(sCriticalSystemsByName, pWrapper->pMyName, &pSystem))
	{
		ContactAttempted(pWrapper->pMyName);
		return;
	}

	estrClear(&pMachineNameOrIPString);

	pSystem->pNetLink = pLink;

	assert(pSystem);

	StructCopyAll(parse_StatusReporting_GenericServerStatus, &pWrapper->status, &pSystem->lastStatus);

	pSystem->iLastContactTime = timeSecondsSince2000();
	pSystem->IP = uIP;

	estrCopy2(&pSystem->pMachineName, pWrapper->pMyMachineName ? pWrapper->pMyMachineName : "");

	pSystem->iMyID = pWrapper->iMyID;
	pSystem->eMyType = pWrapper->eMyType;
	pSystem->iMyGenericMonitoringPort = pWrapper->iMyGenericMonitoringPort;
	pSystem->iMyMainMonitoringPort = pWrapper->iMyMainMonitoringPort;
	estrCopy2(&pSystem->pVersion, pWrapper->pVersion);

	pSystem->pProductName = pWrapper->pMyProduct;
	pSystem->pShortProductName = pWrapper->pMyShortProduct;

	if (pWrapper->pMyShardName)
	{
		estrCopy2(&pSystem->pShardName, pWrapper->pMyShardName);
	}
	...
	if (estrLength(&pSystem->pMachineName))
	{
		estrCopy2(&pMachineNameOrIPString, pSystem->pMachineName);
	}
	else
	{
		estrCopy2(&pMachineNameOrIPString, makeIpStr(uIP));
	}

	if (pWrapper->iMyMainMonitoringPort)
	{
		estrPrintf(&pSystem->pLink1, "<a href=\"http://%s:%d\">Main Monitoring Link</a>",
			pMachineNameOrIPString,pWrapper->iMyMainMonitoringPort);

		if (pWrapper->iMyGenericMonitoringPort)
		{
			estrPrintf(&pSystem->pLink2, "<a href=\"http://%s:%d\">Generic Monitoring Link</a>",
				pMachineNameOrIPString,pWrapper->iMyGenericMonitoringPort);
		}
	}
	else if (pWrapper->iMyGenericMonitoringPort)
	{
		estrPrintf(&pSystem->pLink1, "<a href=\"http://%s:%d\">Main Monitoring Link</a>",
			pMachineNameOrIPString,pWrapper->iMyGenericMonitoringPort);
	}
	...
	estrPrintf(&pSystem->pVNC, "<a href=\"cryptic://vnc/%s\">VNC</a>", pMachineNameOrIPString);
	...
	for (i = 0; i < ALERTLEVEL_COUNT; i++)
	{
		...
		if (ppEString)
		{
			...
			if (pWrapper->status.iNumAlerts[i])
			{
				estrPrintf(ppEString, "<a href=\"http://%s:%d/viewxpath?xpath=%s[%u].globobj.alerts&svrfilter=me.level%%3D%d\">%d %s Alerts</a>",
					pMachineNameOrIPString,
					pWrapper->iMyGenericMonitoringPort ? pWrapper->iMyGenericMonitoringPort : pWrapper->iMyMainMonitoringPort,
					GlobalTypeToName(pWrapper->eMyType), pWrapper->iMyID, i, pWrapper->status.iNumAlerts[i], StaticDefineIntRevLookup(enumAlertLevelEnum, i));
			}
			else
			{
				estrCopy2(ppEString, "");
	...
	objLogWithStruct(LOG_CRITICALSYSTEMS, pWrapper->eMyType, pWrapper->iMyID, 0, pWrapper->pMyName, NULL, NULL, "StatusReport",
		NULL, pWrapper, parse_StatusReporting_Wrapper);

	if (pLink && !linkGetUserData(pLink))
	{
		linkSetUserData(pLink, (void*)allocAddString(pWrapper->pMyName));
	}
}
...
void HandleGenericStatus(Packet *pak, NetLink *pLink)
{
	...
	HandleGenericStatusInternal(&wrapper, uIP, pLink);
}

AUTO_COMMAND ACMD_CATEGORY(XMLRPC);
void ProcessGenericStatus(StatusReporting_Wrapper *pWrapper)
{
	U32 uIP = CurrentXMLRPCIP();

	HandleGenericStatusInternal(pWrapper, uIP, NULL);
}
	...
	*pBar = '|';
	...
	if (bNewEmailSystem)
	{
		SendEmailWithMailingLists(pSystem, EMAILTYPE_ALERT, pThrottler->eCategory, pThrottler->eLevel, pThrottler->pAlertKey, pThrottler->eAlertServerType, pThrottler->eAlertObjType, pThrottler->iCurCount, pNewLongSubject, pNewShortSubject,
			pNewLongBody, pThrottler->bForceNoShortSend ? NULL : pNewShortBody, NULL);
	}
...
void DoAlertThrottling(CriticalSystem_Status *pSystem, Alert *pAlert, 
	char *pShortSubject, char *pShortBody, char *pLongSubject, char *pLongBody, bool bForceNoShortSend, bool bForceShortSend)
{
	char key[256];
	CriticalSystem_AlertThrottler *pThrottler;
	char *pTypeName = GlobalTypeToName_OrNULL(pAlert->eContainerTypeOfServer);

	if (pTypeName)
	{
		sprintf(key, "%s_%s_%u", pAlert->pKey, pTypeName, pAlert->iErrorID);
	}
	else
	{
		sprintf(key, "%s_%u", pAlert->pKey, pAlert->iErrorID);
	}
	...
	if (stashFindPointer(pSystem->alertThrottlesTable, key, &pThrottler))
	{
		...
		if (bForceShortSend)
		{
			SendEmailWithMailingLists(pSystem, EMAILTYPE_ALERT,
				pAlert->eCategory, pAlert->eLevel, pAlert->pKey, pAlert->eContainerTypeOfServer, pAlert->eContainerTypeOfObject, 1, pLongSubject, pShortSubject, NULL,
				pShortBody, NULL);
		}
	}
	else
	{
		...
		if (bNewEmailSystem)
		{
			SendEmailWithMailingLists(pSystem, EMAILTYPE_ALERT,
				...
				bForceNoShortSend ? NULL : pShortBody, NULL);
		}
		...
		pThrottler->bForceNoShortSend = bForceNoShortSend;
...
//when a shard is reporting as both a shard and a critical system, there are certain cases involving
//sendalert when we want to be able to recognize the shard name and treat it as if it used
//the critical system name. Note that non-clustered shards usually have the same shard name and
//critical system name, but clustered shards are named something like "Dungeon" as a shard but
//"Neverwinter_Dungeon" as a critical system.
CriticalSystem_Status *FindCriticalSystemFromAlternateName(const char *pName)
{
	U32 iShardIP = GetMostRecentIPFromShardName(pName);


	FOR_EACH_IN_STASHTABLE(sCriticalSystemsByName, CriticalSystem_Status, pSystem)
	{
		if (stricmp_safe(pSystem->pShardName, pName) == 0)
		{
			return pSystem;
		}

		if (iShardIP
			&& pSystem->eMyType == GLOBALTYPE_CONTROLLER 
			&& pSystem->pNetLink && linkGetIp(pSystem->pNetLink) == iShardIP)
		{
			return pSystem;
		}
	}
...
void HandleAlertInternal(const char *pName_in, Alert *pAlert, U32 uIP, NetLink *link)
{
	CriticalSystem_Status *pSystem;
	...
	const char * pIP = makeIpStr(uIP);

	CriticalSystem_AlertIssue *pIssue;

	char *pThrottlingComment = NULL;

	enumAlertTrackerThrottlingState eAlertTrackerThrottlingState;

//	char emailListName[256];

	static char *pMachineName = NULL;

	if (!stashFindPointer(sCriticalSystemsByName, pName_in, &pSystem))
	{
		if (!(pSystem = FindCriticalSystemFromAlternateName(pName_in)))
		{
			return;
		}
	}

	if (AlertIsSuppressed(pSystem, pAlert))
	{
		ReportAlertToMailingLists(pSystem, pAlert->pKey, pAlert->eLevel, true);	
		return;
	}
	...
	if (pSystem->pNetLink && pSystem->pNetLink != link && !linkGetUserData(link))
	{
		...
		ParserSendStructSafe(parse_Alert, pOutPack, pAlert);
		pktSend(&pOutPack);
	}

	ReportAlertToMailingLists(pSystem, pAlert->pKey, pAlert->eLevel, false);	

	FindMachineNameFromAlert(pAlert, &pMachineName);


	estrStackCreate(&pThrottlingComment);
	eAlertTrackerThrottlingState = AlertTrackers_TrackAlert(pAlert, pSystem, &pThrottlingComment);

	pIssue = GetAlertIssue(pSystem, pAlert);
	...
	estrPrintf(&pShortBodyString, "\"%s\": (%s %s)", 
		pAlert->pString, StaticDefineIntRevLookup(enumAlertLevelEnum, pAlert->eLevel),
		StaticDefineIntRevLookup(enumAlertCategoryEnum, pAlert->eCategory));
	...
	if (pAlert->pMapName && pAlert->pMapName[0])
	{
	...
	estrConcatf(&pLongBodyString, "Direct link to alert page: http://%s:%d/viewxpath?xpath=%s[%u].globobj.alerts&svrfilter=me.level=%d\n\n",
		...
		GlobalTypeToName(pSystem->eMyType), pSystem->iMyID, pAlert->eLevel);


	if (pAlert->pVNC)
	{
		estrConcatf(&pLongBodyString, "VNC: %s\n\n", DeHtmlIfyLink(pAlert->pVNC));
	}

	if (pAlert->pErrorLink)
	{
		estrConcatf(&pLongBodyString, "Errortracker: %s\n\n", DeHtmlIfyLink(pAlert->pErrorLink));
	}
	...
	if (pSystem->eMyType == GLOBALTYPE_CONTROLLER)
	{
		...
		strcpy_trunc(shortAlertKey, pAlert->pKey);
		estrPrintf(&pLongSubjectString, "%s %s: %s", pShortSubjectString, GlobalTypeToName(pAlert->eContainerTypeOfServer), shortAlertKey);
	}
	else
	{

		estrPrintf(&pLongSubjectString, "%s %s", pShortSubjectString, pAlert->pKey);
	}
	...
	ParserWriteText(&pFullAlertStructString, parse_Alert, pAlert, 0, 0, 0);
	...
	if (strStartsWith(pAlert->pString, "AUTO_EMAIL:"))
	{
		char *pAddress = NULL;
	
		estrCopy2(&pAddress, pAlert->pString + 11);
	...
	{
		if (eAlertTrackerThrottlingState == ALERTTHROTTLING_THROTTLING_ON_AND_MET)
		{
			estrInsertf(&pShortSubjectString, 0, "THROTTLED! ");
			estrInsertf(&pShortBodyString, 0, "%s\n", pThrottlingComment);
		}
...
//the function DoAlertThrottling and its associated functions and stuff, which group emails together so that when
//an alert comes in a bunch of times quickly, the 2nd-through-nth of them get stuck together into one long email
		DoAlertThrottling(pSystem, pAlert, pShortSubjectString, pShortBodyString, pLongSubjectString, pLongBodyString,
			eAlertTrackerThrottlingState == ALERTTHROTTLING_THROTTLING_ON_NOT_MET, 
			eAlertTrackerThrottlingState == ALERTTHROTTLING_THROTTLING_ON_AND_MET);
	}
	...
	estrDestroy(&pThrottlingComment);
}
...
void HandleAlert(Packet *pak, NetLink *link)
{
	...
	HandleAlertInternal(pName, &alert, uIP, link);

	StructDeInit(parse_Alert, &alert);
}

AUTO_COMMAND ACMD_CATEGORY(XMLRPC);
void ProcessAlert(const char *pName, Alert *pAlert)
{
	U32 uIP = CurrentXMLRPCIP();
	HandleAlertInternal(pName, pAlert, uIP, NULL);
}

void HandleCrashReport(Packet *pPack)
{
	char *pCriticalSystemName = pktGetStringTemp(pPack);
	char *pCallStack = pktGetStringTemp(pPack);
	char *pErrorTracker = pktGetStringTemp(pPack);
	U32 iErrorTrackerID = pktGetBits(pPack, 32);
	CriticalSystem_Status *pSystem;
	char *pSubjectString = NULL;
	char *pBodyString = NULL;
	char *pLongBodyString = NULL;
	
	if (!stashFindPointer(sCriticalSystemsByName, pCriticalSystemName, &pSystem))
	{
		return;
	}
	...
	estrPrintf(&pSubjectString, "%s has crashed! ET: %d", pSystem->pConfig->pName, iErrorTrackerID);
	estrPrintf(&pBodyString, "CrypticError reports that %s has crashed or asserted\nError tracker ID: %d\nUniqueID: %c%d\n", 
		pSystem->pConfig->pName, iErrorTrackerID,
		GetIssueNumPrefixChar(), pSystem->iMainIssue);

	estrPrintf(&pLongBodyString, "CrypticError reports that %s has crashed or asserted\r\n\r\nError tracker ID: http://%s/detail?id=%d\r\n\r\nUniqueID: %c%d\r\nCallstack:\r\n%s\r\n", 
		pSystem->pConfig->pName, pErrorTracker, iErrorTrackerID,
		GetIssueNumPrefixChar(), pSystem->iMainIssue, pCallStack);



	if (bNewEmailSystem)
	{
		SendEmailWithMailingLists(pSystem, EMAILTYPE_SYSTEMCRASHED, 0, 0, NULL, GLOBALTYPE_NONE, GLOBALTYPE_NONE, 0, pSubjectString, pSubjectString, pLongBodyString, pBodyString, NULL);
	}
...
bool CriticalSystems_CustomXpathHttpFunc(char *pLocalXPath, UrlArgumentList *pArgList, int iAccessLevel, StructInfoForHttpXpath *pStructInfo, GetHttpFlags eFlags)
{
	...
	else
	{
		...
		estrPrintf(&spHtmlStruct->pBeginDowntimeAll, "BeginCategoryDownTime %s $CONFIRM(Begin downtime for all systems in category %s?) $INT(For how many minutes) $NORETURN", pCategory, pCategory);
...
void CriticalSystemDisconnectCB(NetLink* link,char *pCritSysName)
{
	...
	//if  we don't have a critSysName, then this connection was presumably from sendAlert or crypticError
	//reproting an alert or error directly... and we don't care
...
void CriticalSystems_InitSystem(void)
{
	...
	sCriticalSystemsByName = stashTableCreateWithStringKeys(8, StashDeepCopyKeys_NeverRelease);
...
AUTO_COMMAND;
void BeginDownTime(char *pSystemName, int iNumMinutes, CmdContext *pContext)
{
	CriticalSystem_Status *pSystem;
	...
	if (iNumMinutes == 0 && pContext)
	{
		CRITICAL_NETOPS_ALERT("UNBOUNDED_SYSTEM_DOWNTIME", "%s, via %s, is trying to put system %s into unbounded downtime, this is dangerous",
			pContext->pAuthNameAndIP, GetContextHowString(pContext), pSystemName);
	}
...
AUTO_COMMAND;
void BeginCategoryDowntime(char *pCatName, int iNumMinutes, CmdContext *pContext)
{
	int i;
	...
	if (iNumMinutes == 0 && pContext)
	{
		CRITICAL_NETOPS_ALERT("UNBOUNDED_CATEGORY_DOWNTIME", "%s, via %s, is trying to put category %s into unbounded downtime, this is dangerous",
			pContext->pAuthNameAndIP, GetContextHowString(pContext), pCatName);
	}
	...
	for (i=0; i < eaSize(&ppCriticalSystems); i++)
	{
		...
		{
			BeginDownTime(ppCriticalSystems[i]->pConfig->pName, iNumMinutes, NULL);
		}
	}
}
