...
static void getProcessName(HANDLE p,char *str, int strSize)
{
	...
	if (0==GetModuleFileNameEx_UTF8( p, hMod[0],&pExeFilePath)) {
		estrCopy2(&pExeFilePath, "");
	}
...
static BOOL CALLBACK EnumProcCheckIfCrashed(HWND hwnd, LPARAM lParam)
{
	...
	char *pText = NULL;
	if(GetWindowText_UTF8(hwnd, &pText)) {
		if (0==stricmp_safe(pText, "&Send Error Report") ||
			0==stricmp_safe(pText, "De&bug") ||
			0==stricmp_safe(pText, "Debug") ||
			0==stricmp_safe(pText, "&Abort"))
		{
			*crashed=1;
		}
	}
...
static BOOL CALLBACK EnumProcFindCrashedServers(HWND hwnd, LPARAM lParam)
{
	...
	if(GetWindowThreadProcessId(hwnd, &processID))
	{
		...
		if(GetWindowText_UTF8(hwnd, &pTitle))
		{
			...
			{
				FoundWindowWithImportantName(processID, pTitle);
			}

			if(strStartsWith(pTitle, "Crashed Application"))
			{
				iErrorID = atoi(pTitle + 19);
				if (iErrorID == 0)
				{
					iErrorID = ERRORID_UNKNOWN;
				}
			} 
			else if (strEndsWith(pTitle, ".exe") ||
				strStartsWith(pTitle, "Microsoft Visual C"))
			{
				int crashed=0;
...
void procGetList()
{
	...
	estrStackCreate(&pNarrowExeName);

	while((retval=Process32Next(hSnapshot,&pe)))
	{
		UTF16ToEstring(pe.szExeFile, 0, &pNarrowExeName);
		...
		for(i=0;i<eaSize(&process_list.ppProcessInfos);i++)
		{
			...
			if (process_list.ppProcessInfos[i]->process_id == pe.th32ProcessID)
			{
				...
				{
					if (!strstri(pNarrowExeName,process_list.ppProcessInfos[i]->exeNameForTracking))
					{
						continue;
					}
				}
		...
		if (i >= eaSize(&process_list.ppProcessInfos))
		{
			if (pe.th32ProcessID)
			{
				...
				pi->container_id = -1;
				pi->container_type = 0;
				strcpy(pi->exename, ExtractShortExeName(pNarrowExeName));
...
void KillProcessFromTypeAndID(int container_id,int container_type)
{
	...
	for(i=0;i<eaSize(&process_list.ppProcessInfos);i++)
	{
		...
		{
			if (process_list.ppProcessInfos[i]->childPID)
			{
				ProcessInfo *pChild = FindProcessByPID(process_list.ppProcessInfos[i]->childPID);

				if (pChild && strstri(pChild->exename, process_list.ppProcessInfos[i]->childExeName))
				{
					kill(process_list.ppProcessInfos[i]->childPID);
				}
