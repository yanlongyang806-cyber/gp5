...
void StartProcessFromRequest(ServerLaunchRequest *pRequest)
{
	...
	if (UtilitiesLib_GetOverlordName())
	{
		estrConcatf(&pFullCommandLine, " -SetOverlord %s ", UtilitiesLib_GetOverlordName());
	}
	...
	if (!pid)
	{
		...
		AssertOrAlert("LAUNCH_FAILURE", "Couldn't launch <<%s>> : error <<%s>>", pFullCommandLine, pErrorBuf);
...
static void handleSetDirectoryContents(Packet *pPak)
{
	...
	pCurrentFileList = TPFileList_ReadDirectory_OnlyIfPresentInPreexistingFileList(pDirName, pNewFileList);
...
static void handleRequestManualDump(Packet *pak)
{
	...
	estrPrintf(&pSystemString, "c:\\night\\tools\\bin\\CrypticError%s.exe -ForceAutoClose -pid %u -manualDump \"%s\"",
...
{
	ONCE(strcpy(gServerLibState.logServerHost, pktGetStringTemp(pPak)); SetDontDoBlockingLogConnect(true); svrLogInit();)
}
...
static void LauncherListenHandleMsg(Packet *pak,int cmd, NetLink *link,void *pUserData)
{
	...
	xcase TO_LAUNCHER_PROCESS_REPORTING_CHILD_PROCESS:
		{
			U32 iPid = pktGetBits(pak, 32);
			GlobalType eType = GetContainerTypeFromPacket(pak);
			ContainerID iID = GetContainerIDFromPacket(pak);
			U32 iCookie = pktGetBits(pak, 32);
			char *pChildProcName = pktGetStringTemp(pak);
			U32 iChildProcPid = pktGetBits(pak, 32);
			...
			else
			{
				...
				if (pProcess)
				{
					printf("%s(%d) now owns child process %s(PID: %d)\n",
						GlobalTypeToName(eType), iID, pChildProcName, iChildProcPid);

					if (pProcess->childPID)
					{
						AssertOrAlert("TWO_CHILD_PROCS", "%s(%d) already owns child proc %s(PID: %d), now wants to own %s(PID:%d)",
							GlobalTypeToName(eType), iID,
							pProcess->childExeName, pProcess->childPID, 
							pChildProcName, iChildProcPid);
					}

					strcpy(pProcess->childExeName, pChildProcName);
					pProcess->childPID = iChildProcPid;
				}
			}
		}
		break;

	xcase TO_LAUNCHER_CLIENT_REQUESTS_OPEN_MONITORING_URL:
		{
			char *pUrl = pktGetStringTemp(pak);
			Packet *pOutPack;
			assert(gpControllerLink);

			pOutPack = pktCreate(gpControllerLink, LAUNCHER_ANSWER_CLIENT_REQUESTS_OPEN_MONITORING_URL);
			pktSendString(pOutPack, pUrl);
			pktSend(&pOutPack);
		}
...
/*
	...
	*/
	...
	if (isProductionMode())
	{
		BeginPeriodicFreeDiskSpaceCheck(0, 60, 600, 3, 0, NULL, NULL);
	}
