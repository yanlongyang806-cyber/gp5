...
void HandleHereAreLastMinuteFiles(Packet *pPak)
{
	...
	for (i=0; i < eaSize(&allFilesInfo.ppFiles); i++)
	{
		...
		if (gbCopyLastMinuteFilesToXBox)
		{
			...
			sprintf(systemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbcp\" -Y -T %s xe:\\%s\\data\\%s", pProgramFilesFolder, fullFileName, GetProductName(), pFileInfo->pFileName);
...
//erase all LastMinuteFiles from previous runs.
	...
	if (bGetForXbox)
	{
		...
		sprintf(systemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbdel\" -R -S -F xe:\\%s\\data\\*.*", pProgramFilesFolder, GetProductName());
		system(systemString);
		estrDestroy(&pProgramFilesFolder);
	}
	...
	case FROM_NEWCONTROLLERTRACKER_TO_MCP_HERE_IS_SHARD_LIST:
		{
			...
			if (pStartingWindow)
			{
				...
				{
					lResult = SendMessage_AddString_UTF8(GetDlgItem(pStartingWindow->hWnd, IDC_CHOOSESERVER), gAvailableShardList.ppShards[i]->pShardName);
				...
				{	
					SendMessage_SelectString_UTF8(GetDlgItem(pStartingWindow->hWnd, IDC_CHOOSESERVER), gAvailableShardList.ppShards[0]->pShardName);
				}
...
#if STANDALONE
	pControllerTrackerHost = ORGANIZATION_DOMAIN;
#else
	if(gbQaMode)
