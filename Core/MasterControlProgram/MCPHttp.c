...
void HandleMonitoringCommandReturn(Packet *pPak)
{
	...
	CommandServingFlags eFlags = pktGetBits(pPak, 32);
	char *pReturnString = pktGetStringTemp(pPak);

	HttpServing_CommandReturn(iRequestID, iClientID, eFlags, pReturnString);
}
...
void ExecuteCommandOnOtherServer(char *pServerTypeName, U32 iServerID, CmdContext *pContext, ACMD_SENTENCE pCommandString)
{
	...
	//less dangerous than it looks... as long as CMDSRV_NORETURN is passed in, the clientID and requestID are never used 
	MCP_CommandCB(eServerType, iServerID, 0, 0, CMDSRV_NORETURN, pCommandString, pContext->access_level, spTempAuthAndIPString);
}
