	...
	case WM_CREATE:
		{
			...
			hbmLogo = LoadImage_UTF8(0, pTempFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
			...
			hLayeredWindowDll = LoadLibrary( L"user32.dll" );
...
void RegisterSplashWindow()
{
    ...
    splashwc.lpszMenuName  = L"MCP";
    splashwc.lpszClassName = L"MCP";
...
void SplashThread(void* data)
{
	...
	hlogo = CreateWindow_UTF8("MCP", windowName, WS_POPUP | WS_BORDER, 
