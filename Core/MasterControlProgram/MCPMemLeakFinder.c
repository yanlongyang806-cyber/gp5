...
void LoadMemLeakCategories(MemLeakCategory ***pppCategories)
{
	...
	HRSRC rsrc = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(MEMLEAKCATEGORIES_TXT), L"TXT");
	int iCategoryNum;
...
BOOL memLeakFinderMenuDlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam, SimpleWindow *pWindow)
{
	...
	case WM_COMMAND:
		...
		case IDC_MEMLEAK_CLEAR:
			SetWindowText_UTF8(GetDlgItem(hDlg, IDC_MEMLEAKIN), "");
			break;
		...
		case IDOK:
			{
				...
				GetWindowText_UTF8(GetDlgItem(hDlg, IDC_MEMLEAKIN), &pInString);
