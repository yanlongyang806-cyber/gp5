...
typedef struct MCPError
{
	...
	//the first, "reference" filename that this happened on
	char *pFileName_First; AST(ESTRING FORMATSTRING(NO_LIST_VIEW=1))

	//starts out equal to pFileNameFirst. Then if we get multiple filenames, gets "(many)" inserted at the beginning
	char *pFileName_Display; AST(NAME(FileName) ESTRING )

	char **ppOtherFileNames; AST(FORMATSTRING(NO_LIST_VIEW=1))
...
MCPError *ParseErrorFromRawErrorString(const char *pInString)
{
	...
	for (iLineNum = eaSize(&ppLines)-1; iLineNum >= 0; iLineNum--)
	{
		...
		{
			estrCopy2(&pRetVal->pFileName_First, pCurLine + 6);
			estrCopy(&pRetVal->pFileName_Display, &pRetVal->pFileName_First);
			free(pCurLine);
...
static void MergeFileName(MCPError *pError, char *pNewFileName)
{
	if (!pNewFileName || !pNewFileName[0])
	{
		return;
	}

	if (stricmp_safe(pError->pFileName_First, pNewFileName) == 0)
	{
		return;
	}

	if (eaFindString(&pError->ppOtherFileNames, pNewFileName) != -1)
	{
		return;
	}

	
	pError->bMine |= gimmeDLLQueryIsFileMine(pNewFileName);
	...
	if (eaSize(&pError->ppOtherFileNames) == 0)
	{
		estrInsertf(&pError->pFileName_Display, 0, "(Many)");
	}

	eaPush(&pError->ppOtherFileNames, strdup(pNewFileName));
}
...
void ReportMCPError(const char *pRawString)
{
	...
	{
		MergeFileName(pPreExisting, pError->pFileName_First);
	...
	if (pError->pFileName_First)
	{
		pError->bMine = gimmeDLLQueryIsFileMine(pError->pFileName_First);
		if (pError->bMine && !sbAtLeastOneOfMine)
		{
			sbAtLeastOneOfMine = true;
...
void CopyErrorsToClipBoard(void)
{
	...
	for (i=0; i < eaSize(&sppMCPErrors); i++)
	{
		...
		estrConcatf(&pFullString, "%s%s\t%s\t%s",
			...
			pError->pBlamee, pError->pStr, pError->pFileName_Display);
	}
...
void OpenFileCB(ListView *lv, MCPError *pError, void *data)
{
	...
	if (pError->pFileName_First && pError->pFileName_First[0])
	{
		fileLocateWrite(pError->pFileName_First, fixedUpFile);
...
void CopySelectedUniqueCB(ListView *lv, MCPError *pError, void *data)
{
	if (eaSize(&pError->ppOtherFileNames))
	{
		estrConcatf(&spAllSelectedString, "%s\t%s", 
			pError->pStr, pError->pFileName_First);

		FOR_EACH_IN_EARRAY_FORWARDS(pError->ppOtherFileNames, char, pName)
		{
			estrConcatf(&spAllSelectedString, ", %s", pName);
		}
		FOR_EACH_END;
	}
	else
	{
		CopySelectedCB(lv, pError, data);
	}
}
	...
	case WM_INITDIALOG:
		{
			...
			if (!sBlameBanner)
			{
				...
				sNoBlameBanner = LoadImage_UTF8(0, pTempFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);

				assertmsgf(sNoBlameBanner, "Couldn't load %s", pTempFileName);
				DeleteFile_UTF8(pTempFileName);
				...
				sBlameBanner = LoadImage_UTF8(0, pTempFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
				...
				sNoErrorsBanner = LoadImage_UTF8(0, pTempFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
			...
			case WM_TIMER:
			{
				...
				if (pMCPErrorLV)
				{
					...
					if (spCurSelectedError)
					{
						...
						SetTextFast(GetDlgItem(hDlg, IDC_SELECTED_FILENAME), spCurSelectedError->pFileName_Display);
	...
	case WM_COMMAND:
		...
		case IDC_COPYSELECTED:
			if (pMCPErrorLV)
			{
				...
				if (listViewCountSelected(pMCPErrorLV) == 1)
				{
					listViewDoOnSelected(pMCPErrorLV, CopySelectedUniqueCB, NULL);
				}
				else
				{
