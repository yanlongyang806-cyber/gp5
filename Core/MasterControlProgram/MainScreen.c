	...
	case WM_INITDIALOG:
		{
			...
			if (gGlobalDynamicSettings.mapNameForGameServer[0])
			{
				LRESULT lResult = SendMessage_AddString_UTF8(GetDlgItem(hDlg, IDC_STARTINGGSMAP), gGlobalDynamicSettings.mapNameForGameServer);
				SendMessage(GetDlgItem(hDlg, IDC_STARTINGGSMAP), CB_SETITEMDATA, (WPARAM)lResult, (LPARAM)0);
				SendMessage_SelectString_UTF8(GetDlgItem(hDlg, IDC_STARTINGGSMAP), gGlobalDynamicSettings.mapNameForGameServer);
			...
			for (j=0; j < NUM_EXTRA_GS_NAMES; j++)
			{
				...
				{
					LRESULT lResult = SendMessage_AddString_UTF8(GetDlgItem(hDlg, siExtraGameServerMapIDs[j]), GetNthExtraGameServerNameFromDynamicSettings(j));
					SendMessage(GetDlgItem(hDlg, siExtraGameServerMapIDs[j]), CB_SETITEMDATA, (WPARAM)lResult, (LPARAM)0);
					SendMessage_SelectString_UTF8(GetDlgItem(hDlg, siExtraGameServerMapIDs[j]), GetNthExtraGameServerNameFromDynamicSettings(j));				
				}
			...
			if (!bgb1)
			{
				...
				bgb1 = LoadImage_UTF8(0, pTempFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
				...
				bgb2 = LoadImage_UTF8(0, pTempFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
			...
			for (i=0; i < GetNumContainerTypes(); i++)
			{
				...
				{
					SetWindowText_UTF8(GetDlgItem(hDlg, gContainerButtonIDs[i].iGoButtonID), GlobalTypeToName(GetContainerStaticInfoFromSlot(i)->eContainerType) );
				}

				SetWindowText_UTF8(GetDlgItem(hDlg, gContainerButtonIDs[i].iEditCommandLineButtonID), IsCommandLineNonEmpty(GetContainerDynamicInfoFromSlot(i)->commandLine)
				...
				SetWindowText_UTF8(GetDlgItem(hDlg, gContainerButtonIDs[i].iEditSharedCommandLineButtonID), IsCommandLineNonEmpty(GetContainerDynamicInfoFromSlot(i)->sharedCommandLine) ? "!!!" : "...");
			}

			SetWindowText_UTF8(GetDlgItem(hDlg, IDC_GLOBAL_EDITCOMMANDLINE), IsCommandLineNonEmpty(gGlobalDynamicSettings.globalCommandLine) ? "!!!" : "...");
			...
			case IDC_STARTINGGSMAP:
				{
					...
					if (!sbFilledInStartingMapNames)
					{
						...
						for (i=0; i < eaSize(&ppFilteredNames); i++)
						{
							...
							lResult = SendMessage_AddString_UTF8(GetDlgItem(hDlg, IDC_STARTINGGSMAP), ppFilteredNames[i]);
					...
					if (LOWORD(wParam) == siExtraGameServerMapIDs[i])
					{
						...
						if (!sbFilledInExtraMapNames[i])
						{
							...
							for (j=0; j < eaSize(&ppFilteredNames); j++)
							{
								...
								lResult = SendMessage_AddString_UTF8(GetDlgItem(hDlg, siExtraGameServerMapIDs[i]), ppFilteredNames[j]);
							...
							{
								SendMessage_SelectString_UTF8(GetDlgItem(hDlg, siExtraGameServerMapIDs[i]), GetNthExtraGameServerNameFromDynamicSettings(i));
							}
						}
