...
void UpdateArtToolPrintfBuffer(bool bForce)
{
	...
	if (gbPrintfBuffersChanged[giActiveArtToolPrintBuffer] || bForce)
	{
		...
		if (gPrintfBuffers[giActiveArtToolPrintBuffer])
		{
			...
			SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_TOOLOUTPUT), pTempSmallString);
		...
		{
			SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_TOOLOUTPUT), pEmptyRTFString);
		}
	}
}
...
void LaunchArtistTool(int iWhichTool, bool bReLaunchIfRunning)
{
	...
	assert(iWhichTool >= 0 && iWhichTool < MAX_ARTIST_TOOLS);

	estrCopy2(&pTemp, gGlobalDynamicSettings.artCommandLines[iWhichTool]);
	estrTruncateAtFirstOccurrence(&pTemp, '\n');
	estrTruncateAtFirstOccurrence(&pTemp, '\r');

	estrConcatf(&pCommandLine, " - %s - ", pTemp);
	estrDestroy(&pTemp);
...
void ResetArtToolButtonText(HWND hWnd, int iButtonNum)
{
	...
	SetWindowText_UTF8(GetDlgItem(hWnd, gArtistToolButtonIDs[iButtonNum].iLaunchButtonID), tempString);
}
