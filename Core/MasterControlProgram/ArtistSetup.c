...
BOOL ArtistSetupDlgFunc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam, SimpleWindow *pWindow)
{
	...
	case WM_INITDIALOG:
		{
			...
			if (!pArtistSetupMCSM)
			{
				...
				for (i=0; i < MAX_ARTIST_TOOLS; i++)
				{
					...
					MultiControlScaling_AddChild(pArtistSetupMCSM, gArtistToolButtonIDs[i].iCommandLineID, SCALE_FULLRESIZE, SCALE_LOCK_TOP);
			...
			for (i=0; i < GetNumArtistTools(); i++)
			{
				...
				SetWindowText(GetDlgItem(hDlg, gArtistToolButtonIDs[i].iCommandLineID), 
					IsCommandLineNonEmpty(gGlobalDynamicSettings.artCommandLines[i])
					 ? L"!!!" : L"...");
	...
	case WM_COMMAND:
		...
		for (i=0; i < GetNumArtistTools(); i++)
		{
			...
			else if (LOWORD(wParam) == gArtistToolButtonIDs[i].iCommandLineID)
			{
				SimpleWindowManager_AddOrActivateWindow(MCPWINDOW_COMMANDLINE, gArtistToolButtonIDs[i].iFakeCommandLineID, IDD_EDITCOMMANDLINE, false,
