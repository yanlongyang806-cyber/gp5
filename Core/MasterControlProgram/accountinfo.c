...
BOOL MCPAccountInfoDlgFunc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam, SimpleWindow *pWindow)
{
	...
	case WM_COMMAND:
		...
		if (LOWORD(wParam) == IDC_USERNAME || LOWORD(wParam) == IDC_PASSWORD)
		{
			...
			estrStackCreate(&pPassword);

			GetWindowText_UTF8(GetDlgItem(hDlg, IDC_USERNAME), &pUserName);			
			GetWindowText_UTF8(GetDlgItem(hDlg, IDC_PASSWORD), &pPassword);
			...
			bValid = CheckAccountNameIsValid(pUserName, &pErrorMessage);

			if (bValid)
			{
				bValid = CheckPasswordIsValid(pPassword, &pErrorMessage);
			}
		...
		if (LOWORD(wParam) == IDOK)
		{
			...
			GetWindowText_UTF8(GetDlgItem(hDlg, IDC_USERNAME), &pTemp);
			strcpy_trunc(gGlobalDynamicSettings.accountName, pTemp);

			GetWindowText_UTF8(GetDlgItem(hDlg, IDC_PASSWORD), &pTemp);
			strcpy_trunc(gGlobalDynamicSettings.password, pTemp);
	...
	else
	{
		...
		if (sAccountState == ACCOUNTSERVERSTATE_NONE && gbLoginImmediately)
		{
			...
			if (!bOnce)
			{
				...
				GetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_USERNAME), &pTemp);
				strcpy_trunc(gGlobalDynamicSettings.accountName, pTemp);
				
				GetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_PASSWORD), &pTemp);
				strcpy_trunc(gGlobalDynamicSettings.password, pTemp);
