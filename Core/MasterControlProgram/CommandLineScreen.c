...
char *GetContainerTypeCommandLine(int iIndex)
{
	...
	if (iIndex == FAKE_GLOBALTYPE_ANIMLIBRARY)
	{
		return gGlobalDynamicSettings.artCommandLines[ANIMATIONLIBRARY];
	}
	if (iIndex == FAKE_GLOBALTYPE_TEXLIBRARY)
	{
		return gGlobalDynamicSettings.artCommandLines[TEXTURELIBRARY];
	}
	if (iIndex == FAKE_GLOBALTYPE_OBJLIBRARY)
	{
		return gGlobalDynamicSettings.artCommandLines[OBJECTLIBRARY];
	}
	if (iIndex == FAKE_GLOBALTYPE_CHARLIBRARY)
	{
		return gGlobalDynamicSettings.artCommandLines[CHARACTERLIBRARY];
	}
...
BOOL EditCommandLineDlgFunc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam, SimpleWindow *pWindow)
{
	...
	case WM_INITDIALOG:
		{
			...
			if (pWindow->iUserIndex == GLOBALTYPE_NONE)
			{
				...
				SetWindowText_UTF8(GetDlgItem(hDlg, IDC_EDITCOMMANDLINE_CAPTION), captionString);
				sprintf(captionString, "Edit GLOBALLY SHARED command line");
				SetWindowText_UTF8(hDlg, captionString);
			...
			else if (pWindow->iUserIndex == FAKE_GLOBALTYPE_ANIMLIBRARY)
			{
				sprintf(captionString, "Specify command line parameters for Animation Library. Only the first line of text is used, so you can store alternatives in other lines");
				SetWindowText_UTF8(GetDlgItem(hDlg, IDC_EDITCOMMANDLINE_CAPTION), captionString);
				sprintf(captionString, "Edit command line - Anim Library");
				SetWindowText_UTF8(hDlg, captionString);
			...
			else if (pWindow->iUserIndex == FAKE_GLOBALTYPE_TEXLIBRARY)
			{
				sprintf(captionString, "Specify command line parameters for Texture Library. Only the first line of text is used, so you can store alternatives in other lines");
				SetWindowText_UTF8(GetDlgItem(hDlg, IDC_EDITCOMMANDLINE_CAPTION), captionString);
				sprintf(captionString, "Edit command line - Texture Library");
				SetWindowText_UTF8(hDlg, captionString);
			...
			else if (pWindow->iUserIndex == FAKE_GLOBALTYPE_OBJLIBRARY)
			{
				sprintf(captionString, "Specify command line parameters for Object Library. Only the first line of text is used, so you can store alternatives in other lines");
				SetWindowText_UTF8(GetDlgItem(hDlg, IDC_EDITCOMMANDLINE_CAPTION), captionString);
				sprintf(captionString, "Edit command line - Object Library");
				SetWindowText_UTF8(hDlg, captionString);
			...
			else if (pWindow->iUserIndex == FAKE_GLOBALTYPE_CHARLIBRARY)
			{
				sprintf(captionString, "Specify command line parameters for Character Library. Only the first line of text is used, so you can store alternatives in other lines");
				SetWindowText_UTF8(GetDlgItem(hDlg, IDC_EDITCOMMANDLINE_CAPTION), captionString);
				sprintf(captionString, "Edit command line - Character Library");
				SetWindowText_UTF8(hDlg, captionString);
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			GetWindowText_UTF8_FixedSize(GetDlgItem(hDlg, IDC_EDITCOMMANDLINE), GetContainerTypeCommandLine(pWindow->iUserIndex), COMMAND_LINE_SIZE);				
		
			GetWindowText_UTF8_FixedSize(GetDlgItem(hDlg, IDC_OVERRIDEEXE), GetContainerTypeOverrideExeName(pWindow->iUserIndex), CRYPTIC_MAX_PATH);				
			GetWindowText_UTF8_FixedSize(GetDlgItem(hDlg, IDC_OVERRIDELAUNCHDIR), GetContainerTypeOverrideLaunchDir(pWindow->iUserIndex), CRYPTIC_MAX_PATH);				
