...
{
	static char *pTempFolder = NULL;
	static char *pProgramFilesFolder = NULL;
	char *pIPString = gAvailableShardList.ppShards[iServerNum]->pShardLoginServerAddress;
	char *pPatchString = gAvailableShardList.ppShards[iServerNum]->pPatchCommandLine;
	...
	GetEnvironmentVariable_UTF8("temp", &pTempFolder);
	...
	estrPrintf(&pSystemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbcp\" -Y -T %s\\patchclientxbox.exe xe:\\patchclient\\patchclientxbox.exe", pProgramFilesFolder, pToolsBinDirToUse);

	if(system(pSystemString))
	{
		MessageBox_UTF8(NULL, pSystemString, "This command failed!", MB_OK);
	...
	estrConcatf(&pSystemString,  " > \"%s\\tempCmdLine.txt\"", pTempFolder);
	if(system(pSystemString))
	{
		MessageBox_UTF8(NULL, pSystemString, "This command failed!", MB_OK);
		estrDestroy(&pSystemString);
	...
	estrPrintf(&pSystemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbcp\" -Y -T %s\\tempCmdLine.txt xe:\\%s\\cmdline.txt", pProgramFilesFolder, pTempFolder, pShard->pProductName);
	if(system(pSystemString))
	{
		MessageBox_UTF8(NULL, pSystemString, "This command failed!", MB_OK);
		estrDestroy(&pSystemString);
...
//	estrPrintf(&pUnescapedSnippet, "xe:\\%s\\GameClientXBox.exe -PatchLogin -PatchServerAndProject {PatchServer} {Project}", pShard->pProductName);
	...
	estrPrintf(&pSystemString, "\"%s\\microsoft xbox 360 sdk\\bin\\win32\\xbreboot\" xe:\\patchclient\\patchclientxbox.exe -root devkit:\\%s -verbose 2 %s -superesc executable %s",
		pProgramFilesFolder, pShard->pProductName, pPatchStringToUse, pEscapedSnippet);
	...
	case WM_INITDIALOG:
		{
			...
			SetWindowText_UTF8(hDlg, tempString);
			
			SetWindowText_UTF8(GetDlgItem(hDlg, IDC_PATCHPC_COMMANDLINE), 
				IsCommandLineNonEmpty(gGlobalDynamicSettings.patchedPCClientCommandLine) ? "!!!" : "...");

			SetWindowText_UTF8(GetDlgItem(hDlg, IDC_PATCHXBOX_COMMANDLINE), 
