...
BOOL ServerMonitorControlScreenDlgFunc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam, SimpleWindow *pWindow)
{
	...
	case WM_INITDIALOG:
			...
			SetWindowText_UTF8(hDlg, STACK_SPRINTF("Monitoring port %d", pWindow->iUserIndex));
		
			SetWindowText_UTF8(GetDlgItem(hDlg, IDC_DISABLE_ENABLE), "DISABLE");
			break;
	...
	case WM_TIMER:
		{
			...
			if ((U32)((uintptr_t)pWindow->pUserData) != iCurTime)
			{
				...
				SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_STATS_TOTAL), pStatsTotalString);

				estrMakePrettyBytesString(&pSizeString, pStats->iBytesSentLast15Secs);
				estrPrintf(&pStatsTotalString, "Last 15 seconds:\r\n%s", pSizeString);
				SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_STATS_LAST15), pStatsTotalString);
				...
				SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_CONNECTED_TOTAL), pStatsTotalString);
				SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_CONNECTED_LAST15), pStats15String);
...
/*			HttpStatsReport *pReport = HttpServing_GetStatsReport();
			...
			SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_CONNECTED_IPS), pIPString);
			...
			SetWindowText_UTF8(GetDlgItem(pWindow->hWnd, IDC_STATS), pStatsString);
	...
	case WM_COMMAND:
		...
		case IDC_DISABLE_ENABLE:
			...
			else
			{
				...
				SetWindowText_UTF8(GetDlgItem(hDlg, IDC_DISABLE_ENABLE), "ENABLE");
			}
			break;
