	...
	case WM_INITDIALOG:
		{
			...
			{
				lResult = SendMessage_AddString_UTF8(GetDlgItem(hDlg, IDC_PICKCONFIG), gGlobalStaticSettings.ppServerTypeConfigurations[i]->pName);
				SendMessage(GetDlgItem(hDlg, IDC_PICKCONFIG), CB_SETITEMDATA, (WPARAM)lResult, (LPARAM)i);
			}

			SendMessage_SelectString_UTF8(GetDlgItem(hDlg, IDC_PICKCONFIG), gGlobalDynamicSettings.serverTypeConfig);
