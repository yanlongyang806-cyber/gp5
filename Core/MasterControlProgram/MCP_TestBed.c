...
void testBinaryDiff(void)
{
	...
	StructWriteTextDiff(&estr, parse_DifTestStruct, NULL, ts, "", 0, 0, TEXTIDFFFLAG_EXCLUDE_FLAG_WORKS_AS_INCLUDE_FLAG_UNTIL_TRAVERSE_INTO_STRUCT);
...
Pet *CreatePet(enumPetType eType)
{
	switch (eType)
	{
	xcase PETTYPE_DOG:
	{
		...
		pDog->iBreed = randomIntRange(1,50);
	...
	xcase PETTYPE_CAT:
	{
		...
		pCat->fWhiskerLength = randomPositiveF32();

		return (Pet*)pCat;
	}
	xcase PETTYPE_IGUANA:
	{
		Iguana *pIguana = StructCreate(parse_Iguana);
		pIguana->pet.eType = PETTYPE_IGUANA;

		pIguana->iNumLittleIguanaBabies = randomIntRange(1,50);
...
AUTO_RUN;
void petDiffTest(void)
{
	char *pStr = NULL;
	PetList *pList = StructCreate(parse_PetList);

	pList->pDog = (Dog*)CreatePet(PETTYPE_DOG);
	pList->pIguana = (Iguana*)CreatePet(PETTYPE_IGUANA);
	pList->pOptionalPet = CreatePet(PETTYPE_DOG);

	StructTextDiffWithNull(&pStr, parse_PetList, pList, NULL, 0, 0, 0, 0);
...
DefineContext *pTestColorContext;

AUTO_RUN_LATE;
void colorExtensionTest(void)
{
	char **ppKeys = NULL;
	int *piValues = NULL;
	int iTest;
	...
	DefineFillAllKeysAndValues(enumTestColorEnum, &ppKeys, &piValues);

	eaDestroy(&ppKeys);
	ea32Destroy(&piValues);

	pTestColorContext = DefineCreate();
	DefineAddInt(pTestColorContext, "WHITE", 10);
	DefineAddInt(pTestColorContext, "BLACK", 11);
	DefineAddInt(pTestColorContext, "PURPLE", 12);
	DefineAddInt(pTestColorContext, "OCHRE", 13);

	StaticDefineInt_ExtendWithDefineContext(enumTestColorEnum, &pTestColorContext);

	DefineFillAllKeysAndValues(enumTestColorEnum, &ppKeys, &piValues);
	eaDestroy(&ppKeys);
	ea32Destroy(&piValues);

	iTest = StaticDefineInt_FastStringToInt(enumTestColorEnum, "WHITE", -1);
	pTest = StaticDefineInt_FastIntToString(enumTestColorEnum, 12);

	DefineAddInt(pTestColorContext, "ORANGE", 15);
	...
	iTest = StaticDefineInt_FastStringToInt(enumTestColorEnum, "ORANGE", -1);
	pTest = StaticDefineInt_FastIntToString(enumTestColorEnum, 15);
...
void ParseTest(void)
{
	...
	ParseLogFile(&ppLogs, "c:\\temp\\AccessLevelCommands_Jan2010_Mar2011.txt", 
		&restrictions, NULL, 0);
}
*/

//generically useful utility function for parsing through a bunch of logs of AccessLevelCommands and generating a report. Could
//end up moved into its own .exe at some point

AUTO_STRUCT;
typedef struct PerCommandCount
{
	const char *pCommandName; AST(POOL_STRING KEY)
	int iCount;
} PerCommandCount;

AUTO_STRUCT;
typedef struct MultipleCommandCounts
{
	PerCommandCount **ppCounts;
} MultipleCommandCounts;

AUTO_STRUCT;
typedef struct ALReport_PerUser
{
	char *pUserName;
	U32 iAccountID;
	U32 iFirstCommandTime;
	U32 iLastCommandTime;
	int iTotalCount;

	MultipleCommandCounts countsPerAL[12]; AST(AUTO_INDEX(CountsPerAL))
} ALReport_PerUser;

static MultipleCommandCounts sCountsPerAL[12];
static StashTable sPerUserReports = NULL;

static void CountCommand(MultipleCommandCounts *pCounts, const char *pCommandName)
{
	PerCommandCount *pCount = eaIndexedGetUsingString(&pCounts->ppCounts, pCommandName);
	if (pCount)
	{
		pCount->iCount++;
	}
	...
	{
		pCount = StructCreate(parse_PerCommandCount);
		pCount->pCommandName = pCommandName;
		pCount->iCount = 1;
		eaPush(&pCounts->ppCounts, pCount);
	}
}

static void AddALReportCommand(const char *pAccountName,  U32 iAccountID, const char *pCommandName, U32 iTime,
	int iAccessLevel)
{
	ALReport_PerUser *pUserReport;
	assert(iAccessLevel >= 0 && iAccessLevel < 12);

	CountCommand(&sCountsPerAL[iAccessLevel], pCommandName);

	if (!sPerUserReports)
	{
		sPerUserReports = stashTableCreateWithStringKeys(256, StashDefault);
	}

	if (!stashFindPointer(sPerUserReports, pAccountName, &pUserReport))
	{
		pUserReport = StructCreate(parse_ALReport_PerUser);
		pUserReport->pUserName = strdup(pAccountName);
		pUserReport->iAccountID = iAccountID;
		pUserReport->iFirstCommandTime = pUserReport->iLastCommandTime = iTime;
		stashAddPointer(sPerUserReports, pUserReport->pUserName, pUserReport, false);
	}

	if (iTime < pUserReport->iFirstCommandTime)
	{
		pUserReport->iFirstCommandTime = iTime;
	}

	if (iTime > pUserReport->iLastCommandTime)
	{
		pUserReport->iLastCommandTime = iTime;
	}

	pUserReport->iTotalCount++;
	
	CountCommand(&pUserReport->countsPerAL[iAccessLevel], pCommandName);

}

static void ProcessAccessLevelLog(ParsedLog *pLog)
{
	static char *spFixedUpCommandName = NULL;
	
	if (pLog->pObjInfo->pAccessLevelCommand->iAccessLevel <= 2)
	{
		return;
	}

	estrPrintf(&spFixedUpCommandName, "%s/%s", GlobalTypeToName(pLog->eServerType), pLog->pObjInfo->pAccessLevelCommand->pCmdString);
	estrTruncateAtFirstOccurrence(&spFixedUpCommandName, ' ');

	if (pLog->pObjInfo->pOwnerName && pLog->pObjInfo->pOwnerName[0])
	{
		AddALReportCommand(pLog->pObjInfo->pOwnerName, pLog->pObjInfo->iownerID, allocAddString(spFixedUpCommandName), pLog->iTime, 
			pLog->pObjInfo->pAccessLevelCommand->iAccessLevel);
	}
	else
	{
		char tempOwner[256];
		sprintf(tempOwner, "UNKNOWN_%s", pLog->pObjInfo->pAccessLevelCommand->pHowCalled);

		AddALReportCommand(tempOwner, 0, allocAddString(spFixedUpCommandName), pLog->iTime, 
			pLog->pObjInfo->pAccessLevelCommand->iAccessLevel);
	}
...
AUTO_RUN;
void AccessLevelCommandProcessInit(void)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(sCountsPerAL); i++)
	{
		StructInit(parse_MultipleCommandCounts, &sCountsPerAL[i]);
	}
}

AUTO_COMMAND;
void ProcessAccessLevelCommandFile(char *pFileName)
{
	ParsedLog **ppLogs = NULL;
	LogParsingRestrictions restrictions = {0};
	const char *pActionName = allocAddString("AccessLevelCommand");

	printf("About to read logs from %s\n", pFileName);
	ParseLogFile(&ppLogs, pFileName, 
		&restrictions, NULL, 0);
	printf("Read %d logs\n", eaSize(&ppLogs));

	FOR_EACH_IN_EARRAY_FORWARDS(ppLogs, ParsedLog, pLog)
	{
		if (pLog->pObjInfo && pLog->pObjInfo->pAccessLevelCommand)
		{
			ProcessAccessLevelLog(pLog);
		}
	}
	FOR_EACH_END;
	...
	eaDestroyStruct(&ppLogs, parse_ParsedLog);
}

static int SortCount(const PerCommandCount **ppCount1, const PerCommandCount **ppCount2)
{
	return (*ppCount2)->iCount - (*ppCount1)->iCount;
}

static void DumpCounts(FILE *pOutFile, 
	MultipleCommandCounts *pCounts)
{
	int iAccessLevel;

	for (iAccessLevel = 11; iAccessLevel >= 0; iAccessLevel--)
	{
		if (pCounts[iAccessLevel].ppCounts)
		{
			PerCommandCount **ppUnindexedCounts = NULL;
			FOR_EACH_IN_EARRAY(pCounts[iAccessLevel].ppCounts, PerCommandCount, pCount)
			{
				eaPush(&ppUnindexedCounts, pCount);
			}
			FOR_EACH_END;

			eaQSort(ppUnindexedCounts, SortCount);

			if (eaSize(&ppUnindexedCounts))
			{
				fprintf(pOutFile, "\tAccessLevel %d (%d unique commands)\n", iAccessLevel, eaSize(&ppUnindexedCounts));

				FOR_EACH_IN_EARRAY_FORWARDS(ppUnindexedCounts, PerCommandCount, pCount)
				{
					fprintf(pOutFile, "\t%d - %s\n", pCount->iCount, pCount->pCommandName);
				}
				FOR_EACH_END;
				fprintf(pOutFile, "\n");
			}
...
static int SortUsersByCount(const ALReport_PerUser **ppUser1, const ALReport_PerUser **ppUser2)
{
	return (*ppUser2)->iTotalCount - (*ppUser1)->iTotalCount;
}

static int SortUsersByUsername(const ALReport_PerUser **ppUser1, const ALReport_PerUser **ppUser2)
{
	return strcmp((*ppUser2)->pUserName, (*ppUser1)->pUserName);
}


static int SortUsersByAccountID(const ALReport_PerUser **ppUser1, const ALReport_PerUser **ppUser2)
{
	return (*ppUser1)->iAccountID - (*ppUser2)->iAccountID;
}



AUTO_COMMAND;
void GenerateAccessLevelCommandReport(char *pOutFileName)
{
	FILE *pOutFile;
	ALReport_PerUser **ppAllUsers = NULL;

	if (!sPerUserReports)
	{
		printf("No access level command logs have been read... run ProcessAccessLevelCommandFile on one or more files\n");
		return;
	}
	...
	fprintf(pOutFile, "-----All Commands-----\n");
	DumpCounts(pOutFile, &sCountsPerAL[0]);

	FOR_EACH_IN_STASHTABLE(sPerUserReports, ALReport_PerUser, pPerUserReport)
	{
		eaPush(&ppAllUsers, pPerUserReport);
	}
	FOR_EACH_END;

	eaQSort(ppAllUsers, SortUsersByCount);

	FOR_EACH_IN_EARRAY_FORWARDS(ppAllUsers, ALReport_PerUser, pPerUserReport)
	{
		fprintf(pOutFile, "\nUser %s(Account %u) - %d total commands\n", pPerUserReport->pUserName, 
			pPerUserReport->iAccountID, pPerUserReport->iTotalCount);
		fprintf(pOutFile, "\tFrom %s to ", timeGetLocalDateStringFromSecondsSince2000(pPerUserReport->iFirstCommandTime));
		fprintf(pOutFile, "%s\n", timeGetLocalDateStringFromSecondsSince2000(pPerUserReport->iLastCommandTime));

		DumpCounts(pOutFile, &pPerUserReport->countsPerAL[0]);
	}
	FOR_EACH_END;
	
	fprintf(pOutFile, "\n\nAll usernames/accountIDs, sorted by username\n");

	eaQSort(ppAllUsers, SortUsersByUsername);

	FOR_EACH_IN_EARRAY_FORWARDS(ppAllUsers, ALReport_PerUser, pPerUserReport)
	{
		if (pPerUserReport->iAccountID)
		{
			fprintf(pOutFile, "\t%s %u\n", pPerUserReport->pUserName, pPerUserReport->iAccountID);
		}
	}
	FOR_EACH_END;
	
	fprintf(pOutFile, "\n\nAll usernames/accountIDs, sorted by account ID\n");

	eaQSort(ppAllUsers, SortUsersByAccountID);
...
AUTO_RUN_FILE;
void weirdBugTest(void)
{
	char *pFileName = "C:/night/data/bin/geobin/maps/Adventure Zone/Az_Dr/Az_Dr.Zone/Mapsnap_0000_56.wtex";
	struct _stat32i64 buf = {0};

	cryptic_stat32i64_utc(pFileName, &buf);
}
