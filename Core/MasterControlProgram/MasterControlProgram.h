...
typedef struct GlobalMCPDynamicSettings
{
	...
	char artCommandLines[MAX_ARTIST_TOOLS][COMMAND_LINE_SIZE]; AST(INDEX(0, animLibraryCommandLine) INDEX(1, TexLibraryCommandLine) INDEX(2, ObjLibraryCommandLine) INDEX(3, CharLibraryCommandLine))
...
#define FAKE_GLOBALTYPE_ANIMLIBRARY (COMMAND_LINE_WINDOW_SHARED_OFFSET - 1)
#define FAKE_GLOBALTYPE_TEXLIBRARY (COMMAND_LINE_WINDOW_SHARED_OFFSET - 2)
#define FAKE_GLOBALTYPE_OBJLIBRARY (COMMAND_LINE_WINDOW_SHARED_OFFSET - 3)
#define FAKE_GLOBALTYPE_CHARLIBRARY (COMMAND_LINE_WINDOW_SHARED_OFFSET - 4)
...
static __forceinline char *GetTheFileName(void) { return __FILE__; }