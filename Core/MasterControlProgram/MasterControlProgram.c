...
void ReadSettingsFromFile()
{
	...
	{
		HRSRC rsrc = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(MCP_SERVER_SETUP_TXT), L"TXT");
		if (rsrc)
		{
...
void UpdateMasterControlProgramTitle(void)
{
	...
	char *pResult = fileGetcwd(cwd, ARRAY_SIZE(cwd));
...
//http://localhost/viewxpath?xpath=client[0].globObj.Exprfuncs

static void HandleOpenUrlForMonitoring(Packet *pak)
{
	char *pStr = pktGetStringTemp(pak);
	U32 **ppiMonitoringPorts;
	...
	{
		//need to find the first forward slash that is not a double forward slash, and insert :81 right before it
		char *pFirstDoubleSlash, *pFirstSingleSlash;
		char *pCopy = NULL;

		estrCopy2(&pCopy, pStr);

		pFirstDoubleSlash = strstr(pCopy, "//");
		if (pFirstDoubleSlash)
		{
			pFirstSingleSlash = strchr(pFirstDoubleSlash + 2, '/');
		}
		else
		{
			pFirstSingleSlash = strchr(pCopy, '/');
		}

		if (pFirstSingleSlash)
		{
			estrInsertf(&pCopy, pFirstSingleSlash - pCopy, ":%d", (*ppiMonitoringPorts)[0]);
		}
...
void MasterControlProgramHandleMsg(Packet *pak,int cmd, NetLink *link,void *userdata)
{
	...
	xcase FROM_CONTROLLER_OPEN_URL_FOR_MONITORING:
		HandleOpenUrlForMonitoring(pak);
	...
	case FROM_CONTROLLERTRACKER_LISTOFPATCHEDCONTROLLERS_WITH_AUTOCLIENTCOMMANDLINES:
		{
			...
			if (pStartingWindow)
			{
				...
				for (i=0; i < iNumServers; i++)
				{
					...
					lResult = SendMessage(GetDlgItem(pStartingWindow->hWnd, IDC_CHOOSESERVER), pServerName);
				...
				SendMessage_SelectString_UTF8(GetDlgItem(pStartingWindow->hWnd, IDC_CHOOSESERVER), (LPARAM)curServerName);
			}
