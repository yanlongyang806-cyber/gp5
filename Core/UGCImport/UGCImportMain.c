...
int wmain(int argc, WCHAR** argv_wide)
{
	...
	if(UGCExportImport_InitPython())
	{
		...
		if(0 == strlen(s_hoggfilename) && 0 == strlen(s_machine))
		{
			...
			printf("\t-version 1|0\n");
			printf("\t   Displays the patch version of the shard being imported to. Default is 1 (true).\n");
		...
		hogg = hogFileRead(s_hoggfilename, NULL, PIGERR_PRINTF, &err_return, HOG_NOCREATE|HOG_MUST_BE_WRITABLE);
		if(!hogg)
		{
		...
		if(s_delete_all_ugc)
		{
			printfColor(COLOR_RED | COLOR_BRIGHT, "Are you sure you want to DELETE ALL UGC CONTENT on %s before importing? ", s_machine);
			if(!UGCExportImport_InputTest("Type 'yes' to delete all UGC content (anything else cancels the import)", "yes"))
				return 1;
		}
					...
					if(bReady)
					{
						...
						for(index = 0; index < count; index++)
						{
							...
							if(s_import_projects && pUGCSearchResult->eaResults[index]->iUGCProjectID)
							{
								...
								char journal_filename[256];
								HogFileIndex hfIndex;
								char filename[256];

								sprintf(journal_filename, "Imported/%s/UGCProject/%d", s_machine, pUGCSearchResult->eaResults[index]->iUGCProjectID);
								hfIndex = hogFileFind(hogg, journal_filename);
								if((U32)-1 == hfIndex)
								{
									...
									if(PARSERESULT_ERROR != ParserReadTextFileFromHogg(filename, parse_UGCProject, pUGCProject, hogg))
									{
										...
										if(!s_do_patching || pUGCProjectVersionPublished || pUGCProjectVersionSaved)
										{
											if(pUGCProjectVersionPublished)
											{
												...
												if(hfIndex != HOG_INVALID_INDEX)
												{
													bool checksum_valid;
													U32 size = 0;
													void *data = hogFileExtract(hogg, hfIndex, &size, &checksum_valid);
													estrCreate(&estrUGCProjectDataPublished);
													estrSetSize(&estrUGCProjectDataPublished, size+1);
													memcpy(estrUGCProjectDataPublished, data, size);
											...
											if(pUGCProjectVersionSaved)
											{
												...
												if(hfIndex != HOG_INVALID_INDEX)
												{
													...
													estrSetSize(&estrUGCProjectDataSaved, size+1);
													memcpy(estrUGCProjectDataSaved, data, size);
											...
											if(!s_do_patching || estrUGCProjectDataPublished || estrUGCProjectDataSaved)
											{
												...
												// do the import
												{
													...
													{
														hogFileModifyUpdateNamedSync(hogg, journal_filename, strdup(""), (U32)strlen(""), time(NULL), NULL);
							...
							if(s_import_series && pUGCSearchResult->eaResults[index]->iUGCProjectSeriesID)
							{
								...
								sprintf(journal_filename, "Imported/%s/UGCProjectSeries/%d", s_machine, pUGCSearchResult->eaResults[index]->iUGCProjectSeriesID);
								hfIndex = hogFileFind(hogg, journal_filename);
								if((U32)-1 == hfIndex)
								{
