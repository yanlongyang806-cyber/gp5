...
TransactionTracker *FindTransactionTrackerEx(TransactionServer *pServer, const char *pTransName, bool bNewOnebegan, const char *pFileName, int iLineNum)
{
	...
	pRetVal->pPacketTracker = PacketTrackerFind("TransTracker", 0, allocAddString(pTransName));
	if (bNewOnebegan)
	{
...
AUTO_COMMAND;
char *BeginFailingTransactions(char *pTransactionName_in, int iSet, char *pConfirm)
{
	const char *pPoolName = allocAddString(pTransactionName_in);
	...
	pTracker->bFailAll = iSet;

	if (iSet)
	{
		estrPrintf(&pRetVal, "All %s transactions WILL FAIL INSTANTLY", pPoolName);
	}
	else
	{
		estrPrintf(&pRetVal, "Fail mode TURNED OFF for %s", pPoolName);
	}
	return pRetVal;
}
...
void VersionMismatchReported(char *pVersionString, GlobalType eContainerType)
{
	if (!sVersionMismatches[eContainerType])
	{
		sVersionMismatches[eContainerType] = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
	}
