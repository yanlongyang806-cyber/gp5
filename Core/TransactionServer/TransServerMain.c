...
int TransServerClientDisconnect(NetLink* link,TransactionServerClientLink *pTscLink)
{
	...
	if (pTscLink->iIndexOfLogicalConnection == -1)
	{
		servLog(LOG_TRANSSERVER, "multiplexerDisconnected", "link 0x%p error %u reason \"%s\" ip+port %s", link, linkGetDisconnectErrorCode(link),
			pDisconnectReason, ipBuf);
	}
...
int wmain(int argc, WCHAR** argv_wide)
{
	...
	listenInitReceiveStats(net_links, "Main trans server listen", TransClientPacketTypeEnum);
