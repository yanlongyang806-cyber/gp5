...
AUTO_STRUCT;
typedef struct ClusterCommandTracker_PerShard
{
	const char *pShardName; AST(KEY POOL_STRING)
	int iCount;
	U64 iByteCount; AST(FORMATSTRING(HTML_BYTES = 1))
} ClusterCommandTracker_PerShard;

AUTO_STRUCT AST_FORMATSTRING(HTML_DEF_FIELDS_TO_SHOW = "Count, ByteCount");
typedef struct ClusterCommandTracker
{
	const char *pName; AST(KEY POOL_STRING)
	int iCount;
	U64 iByteCount; AST(FORMATSTRING(HTML_BYTES = 1))
	ClusterCommandTracker_PerShard **ppPerShard;
} ClusterCommandTracker;

static StashTable sClusterCommandTrackers_Send = NULL;
static StashTable sClusterCommandTrackers_Receive = NULL;

void TransactionServer_ShardCluster_ReportClusterCommand(bool bSend, char *pCommandName, char *pShardName, int iBytes)
{

	ClusterCommandTracker *pTracker;
	StashTable sTable;
	ClusterCommandTracker_PerShard *pPerShard;

	if (!sClusterCommandTrackers_Send)
	{
		sClusterCommandTrackers_Send = stashTableCreateWithStringKeys(16, StashDefault);
		sClusterCommandTrackers_Receive = stashTableCreateWithStringKeys(16, StashDefault);

		resRegisterDictionaryForStashTable("InterShardCmds_Send", RESCATEGORY_OTHER, 0, sClusterCommandTrackers_Send, parse_ClusterCommandTracker);
		resRegisterDictionaryForStashTable("InterShardCmds_Receive", RESCATEGORY_OTHER, 0, sClusterCommandTrackers_Receive, parse_ClusterCommandTracker);
	}

	if (bSend)
	{
		sTable = sClusterCommandTrackers_Send;
	}
	else
	{
		sTable = sClusterCommandTrackers_Receive;
	}

	if (!stashFindPointer(sTable, pCommandName, &pTracker))
	{
		pTracker = StructCreate(parse_ClusterCommandTracker);
		pTracker->pName = allocAddString(pCommandName);
		stashAddPointer(sTable, pTracker->pName, pTracker, false);
	}

	pTracker->iCount++;
	pTracker->iByteCount += iBytes;

	pPerShard = eaIndexedGetUsingString(&pTracker->ppPerShard, pShardName);
	if (!pPerShard)
	{
		pPerShard = StructCreate(parse_ClusterCommandTracker_PerShard);
		pPerShard->pShardName = allocAddString(pShardName);
		eaPush(&pTracker->ppPerShard, pPerShard);
	}

	pPerShard->iCount++;
	pPerShard->iByteCount += iBytes;
}
...
bool TransactionServer_ShardCluster_SendRemoteCommandPacket(PacketTracker *pTracker, Packet *pPacket, char *pCommandName, char *pShardName, GlobalType eContainerType, ContainerID iID,
	S64 iErrorCB, S64 iErrorUserData1, S64 iErrorUserData2, int iReturnConnectionIndex, int iReturnConnectionID)
{
	...
	pOutPack = pktCreateWithTracker(pShard->pLink, SHARDCLUSTER_TRANSACTIONSERVER_HERE_IS_COMMAND, pTracker);
	pktSendString(pOutPack, GetShardNameFromShardInfoString());
	...
	iBytesSent = pktCopyRemainingRawBytesToOtherPacket(pOutPack, pPacket);
	pktSend(&pOutPack);

	TransactionServer_ShardCluster_ReportClusterCommand(true, pCommandName, pShardName, iBytesSent);
