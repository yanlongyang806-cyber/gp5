...
typedef struct Transaction
{
	...
	struct Transaction *pNextTransaction; //this means one of two things:
	...
	struct Transaction *pNextBlockedByMe; //the next transaction blocked by me, or the next
		//transaction blocked by the same transaciton as me
	struct Transaction *pLastBlockedByMe; //for the active transaction that is blocking things,
		//pointer to the end of the linked list of things it is blocking, so we can add on the end
