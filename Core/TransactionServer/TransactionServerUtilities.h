...
typedef struct TransactionTracker
{
	...
	bool bFailAll; //if true, then immediately fail all transactions of this sort (for testing, or due to a netops
		//emergency)
	...
	AST_COMMAND("Fail all", "BeginFailingTransactions $FIELD(TransName) $INT(1 to set, 0 to clear) $STRING(Type yes to begin (or end) failing all $FIELD(TransName) transactions)")
