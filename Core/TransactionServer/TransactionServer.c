...
//__CATEGORY Settings related to the Transaction Server
//when sending a packet larger than this to a multiplexer, alert
U32 siAlertCutoffSizeForMultiplexerPackets = 64000000;
AUTO_CMD_INT(siAlertCutoffSizeForMultiplexerPackets, AlertCutoffSizeForMultiplexerPackets) ACMD_AUTO_SETTING(TransServer, TRANSACTIONSERVER);

//when sending a packet larger than this, log some info about the transaction
U32 siPacketSizeCutoffForTransServerLogging = 2000000;
AUTO_CMD_INT(siPacketSizeCutoffForTransServerLogging, PacketSizeCutoffForTransServerLogging) ACMD_AUTO_SETTING(TransServer, TRANSACTIONSERVER);



#define TransServerPacketSend(ppak, pTransaction) TransServerPacketSendDbg(ppak, pTransaction, __FUNCTION__)
void TransServerPacketSendDbg(Packet **ppPak, Transaction *pTransaction, char *pCallingFunction)
{
	NetLink *pLink;
	U32 iSize = pktGetSize(*ppPak);

	pLink = (*ppPak)->link;
	if (pLink && pLink->trans_server_link_to_multiplerxer)
	{
		if (iSize > siAlertCutoffSizeForMultiplexerPackets)
		{
			char *pSizeString = NULL;
			PacketTracker *pTracker = (*ppPak)->pTracker;
			estrMakePrettyBytesString(&pSizeString, iSize);
			CRITICAL_PROGRAMMER_ALERT("BIG_MPLEX_PACKET", "Sending a packet of %s to multiplexer at %s from tracker %s, this may cause multiplexer disconnection",
				pSizeString, makeIpStr(linkGetIp(pLink)), pTracker ? pTracker->pDescriptiveName : "(UNKNOWN)");
		}
	}

	if (iSize > siPacketSizeCutoffForTransServerLogging)
	{
		...
		if (!pTransaction)
		{
			...
			estrStackCreate(&pFullSize);
			estrMakePrettyBytesString(&pFullSize, iSize);
			servLog(LOG_TRANSSERVER, "BigPacket_NoTrans", "The trans server is sending a %s packet from %s, not associated with a transaction",
				pFullSize, pCallingFunction);

			estrDestroy(&pFullSize);
		}
		else
		{
			static SimpleEventThrottler *pLogThrottler = NULL;
			if (!pLogThrottler)
			{
				//if we do 10 full logs in a minute, stop doing them for 10 minutes
				pLogThrottler = SimpleEventThrottler_Create(10, 60, 600);
			}

			servLog(LOG_TRANSSERVER, "BigPacket_Trans", "TransName %s size %u func %s",
				pTransaction->pTransactionName, iSize, pCallingFunction);

			if (SimpleEventThrottler_ItHappened(pLogThrottler, timeSecondsSince2000()) == SETR_OK)
			{
				...
				estrStackCreate(&pFullSize);
				estrMakePrettyBytesString(&pFullSize, iSize);
				DumpTransactionIntoEString(&gTransactionServer, pTransaction, &pFullString);

				servLog(LOG_TRANSSERVER, "BigPacket_Trans_Verbose", "Sending a %s packet from %s: %s",
					pFullSize, pCallingFunction, pFullString);
...
#define TRUNC_LEN 1000

char *MaybeTruncString(char *pInString)
{
	int iLen;
	...
	estrSetSize(&pRetVal, TRUNC_LEN);
	memcpy(pRetVal, pInString, TRUNC_LEN);

	estrConcatf(&pRetVal, "(Truncated... full length %d chars)", iLen);
...
void DumpBaseTransactionInfoIntoEString(BaseTransactionInfo *pBase, char **ppEString)
{
	...
	estrConcatf(ppEString, "Data: %s\n", MaybeTruncString(pBase->transaction.pData));
	...
	estrConcatf(ppEString, "Return string: %s\n", MaybeTruncString(pBase->returnString));
	
	estrConcatf(ppEString, "db update data, %d bytes of data, string: %s",
		pBase->databaseUpdateData.iDataSize, MaybeTruncString(pBase->databaseUpdateData.pString1));
	if (pBase->databaseUpdateData.pString2)
	{
		estrConcatf(ppEString, " string2: %s", MaybeTruncString(pBase->databaseUpdateData.pString2));
	}
	estrConcatf(ppEString, "\ntrans server update string: %s\n", MaybeTruncString(pBase->transServerUpdateString));
...
void CheckCounts(TransactionServer *pServer)
{
	...
	int iNumBlockedByActive = 0;
	int iNumBlockedByBlockedByNothing = 0;
	int iTotalNumBlocked = 0;
	Transaction *pNext;
	Transaction *pTrans;

	static char *spLastStatusString = NULL;
	char *pNewStatusString = NULL;
	...
	for (i=0; i < gMaxTransactions; i++)
	{
		...
		if ( pTransaction->eType != TRANS_TYPE_NONE)
		{
			...
			{
				pNext = pTransaction->pNextBlockedByMe;
	...
	while (pTrans)
	{
		assert(pTrans->eBlockedStatus == BLOCKED_BY_NOTHING);
		iNumBlockedByNothing++;

		pNext = pTrans->pNextBlockedByMe;
		while (pNext)
		{
			assert(pNext->eBlockedStatus == BLOCKED_BY_SOMETHING);
			iNumBlockedByBlockedByNothing++;
			pNext = pNext->pNextBlockedByMe;
		}

		pTrans = pTrans->pNextTransaction;
	}

	estrPrintf(&pNewStatusString, "%d unblocked, %d blocked by active, %d blocked by nothing, %d blocked by blockedByNothing, %d total blocked, %d active\n",
		iNumUnblocked, iNumBlockedByActive, iNumBlockedByNothing,iNumBlockedByBlockedByNothing,  iTotalNumBlocked, pServer->iNumActiveTransactions);

	if (!spLastStatusString || stricmp(pNewStatusString, spLastStatusString) != 0)
	{
		printf("%s\n", pNewStatusString);
		estrDestroy(&spLastStatusString);
		spLastStatusString = pNewStatusString;
		pNewStatusString = NULL;
	}
	else
	{
		estrDestroy(&pNewStatusString);
	}

	assert(iNumBlockedByActive + iNumBlockedByNothing + iNumBlockedByBlockedByNothing  == iTotalNumBlocked);
...
void GetAllConnectionIndicesForType(TransactionServer *pServer, const char *pTransactionName, GlobalType eType, U32 **ppOutRecipients)
{
	StashTableIterator iter;
	int iIndex = -1;
	StashElement element;

	stashGetIterator(pServer->objectDirectories[eType].directory, &iter);

	while (stashGetNextElement(&iter, &element))
	{
		ea32Push(ppOutRecipients, stashElementGetInt(element));
	}
}
...
Packet *NewTransactionPacket(TransactionServer *pServer, int iLogicalConnectionIndex, int ePacketType, PacketTracker *pTracker)
{
...
//tracks all the logical connections in CreateMultipleNewTransactionPackets associated with a single netlink (ie,
//going to a single multiplexer)
AUTO_STRUCT;
typedef struct LogicalConnectionConglomerator
{
	U32 iLinkID; AST(KEY)
	NetLink *pLink; NO_AST
	U32 *pMultiplexConnectionIndices;
} LogicalConnectionConglomerator;


//create one packet for each connection, EXCEPT that if there are multiple logical connections on the same physical connection,
//use CreateMultiplexedNetLinkListPacket_MultipleRecipients to avoid sending redundant packets on that link
void CreateMultipleNewTransactionPackets(TransactionServer *pServer, U32 **ppiLogicalConnectionIndices, int ePacketType, PacketTracker *pTracker, 
	Packet ***pppOutPackets)
{
	int i;
	LogicalConnection *pConnection;
	LogicalConnectionConglomerator **ppConglomerators = NULL;

	eaStackCreate(&ppConglomerators, 128);
	eaIndexedEnable(&ppConglomerators, parse_LogicalConnectionConglomerator);

	for (i = 0; i < ea32Size(ppiLogicalConnectionIndices); i++)
	{
		int iLogicalConnectionIndex = (*ppiLogicalConnectionIndices)[i];
		...
		if (pConnection->iMultiplexConnectionIndex == -1)
		{
			eaPush(pppOutPackets, pktCreateWithTracker(pConnection->pNetLink, ePacketType, pTracker));
		}
		else
		{
			U32 iLinkID = linkID(pConnection->pNetLink);
			LogicalConnectionConglomerator *pConglomerator = eaIndexedGetUsingInt(&ppConglomerators, iLinkID);

			if (!pConglomerator)
			{
				pConglomerator = StructCreate(parse_LogicalConnectionConglomerator);
				pConglomerator->iLinkID = iLinkID;
				pConglomerator->pLink = pConnection->pNetLink;
				eaPush(&ppConglomerators, pConglomerator);
			}
			
			ea32Push(&pConglomerator->pMultiplexConnectionIndices, pConnection->iMultiplexConnectionIndex);
			
			
		}
	}

	FOR_EACH_IN_EARRAY(ppConglomerators, LogicalConnectionConglomerator, pConglomerator)
	{
		if (ea32Size(&pConglomerator->pMultiplexConnectionIndices) > 1)
		{
			eaPush(pppOutPackets, CreateMultiplexedNetLinkListPacket_MultipleRecipients(pConglomerator->pLink,
				pConglomerator->pMultiplexConnectionIndices, ePacketType, pTracker));

		}
		else
		{
			eaPush(pppOutPackets, CreateMultiplexedNetLinkListPacket(pConglomerator->pLink, 
				pConglomerator->pMultiplexConnectionIndices[0], ePacketType, pTracker));
		}
	}
	FOR_EACH_END;

	eaDestroyStruct(&ppConglomerators, parse_LogicalConnectionConglomerator);
...
void AddTransactionToBlockedByNothingList(TransactionServer *pServer, Transaction *pTransaction)
{
	...
	pTransaction->pNextTransaction = NULL;
	...
	{
		pServer->pLastTransactionBlockedByNothing->pNextTransaction = pTransaction;
...
//take the first thing that is blocked by me, and put it on the blocked by nothing list... the remainder of the list of things
//blocked by me are now assumed to be blocked by it
void AddAllMyBlockedTransactionsToBlockedByNothingList(TransactionServer *pServer, Transaction *pTransaction)
{
	Transaction *pFirstToUnblock;

	if ((pFirstToUnblock = pTransaction->pNextBlockedByMe))
	{
		TransactionAssert(pFirstToUnblock->eBlockedStatus == BLOCKED_BY_SOMETHING, pFirstToUnblock, "Blocked transaction has wrong status");
		
		pFirstToUnblock->eBlockedStatus = BLOCKED_BY_NOTHING;
		if (pTransaction->pLastBlockedByMe == pFirstToUnblock)
		{
			TransactionAssert(pFirstToUnblock->pNextBlockedByMe == NULL
				&& pFirstToUnblock->pLastBlockedByMe == NULL, pFirstToUnblock, "Blocked transaction list corruption");
		}
		else
		{
			pFirstToUnblock->pLastBlockedByMe = pTransaction->pLastBlockedByMe;
		}
	
		if (pServer->pLastTransactionBlockedByNothing)
		{
			pServer->pLastTransactionBlockedByNothing->pNextTransaction = pFirstToUnblock;
			pServer->pLastTransactionBlockedByNothing = pFirstToUnblock;
		}
		else
		{
			pServer->pFirstTransactionBlockedByNothing = pFirstToUnblock;
			pServer->pLastTransactionBlockedByNothing = pFirstToUnblock;
		}

		pFirstToUnblock->pNextTransaction = NULL;
	
		pTransaction->pNextBlockedByMe = NULL;
		pTransaction->pLastBlockedByMe = NULL;
	}
...
void CleanupTransaction(TransactionServer *pServer, Transaction *pTransaction)
{
	...
	pTransaction->pNextBlockedByMe = pTransaction->pLastBlockedByMe = pTransaction->pNextTransaction = NULL;
	...
	{
		pServer->pLastEmptyTransaction->pNextTransaction = pTransaction;
...
Transaction *GetNextBlockedByNothingTransaction(TransactionServer *pServer)
{
	...
	if (pServer->pFirstTransactionBlockedByNothing)
	{
		...
		pServer->pFirstTransactionBlockedByNothing = pRetVal->pNextTransaction;
		if (!pServer->pFirstTransactionBlockedByNothing)
		{
			pServer->pLastTransactionBlockedByNothing = NULL;
		}

		pRetVal->pNextTransaction = NULL;
...
void AddBlockedTransaction(TransactionServer *pServer, Transaction *pTransaction)
{
	...
	if (pTransaction->iWhoBlockedMe)
	{
		...
		if (pWhoBlockedIt && (pWhoBlockedIt->eBlockedStatus == IS_NOT_BLOCKED))
		{
			...
			if (pWhoBlockedIt->pLastBlockedByMe)
			{
				pWhoBlockedIt->pLastBlockedByMe->pNextBlockedByMe = pTransaction;
				pWhoBlockedIt->pLastBlockedByMe = pTransaction;

			}
			else
			{
				pWhoBlockedIt->pNextBlockedByMe = pTransaction;
				pWhoBlockedIt->pLastBlockedByMe = pTransaction;
			}
...
Transaction *GetEmptyTransaction(TransactionServer *pServer)
{
	...
	{
		pServer->pNextEmptyTransaction = pServer->pNextEmptyTransaction->pNextTransaction;
	}
...
bool SendNewTransactionMessage(TransactionServer *pServer, Transaction *pTransaction, int iTransIndex, bool bRequiresConfirm, bool bSucceedAndConfirmIsOK)
{
	...
	pPacket = NewTransactionPacket(pServer, iConnectionIndex, TRANSSERVER_REQUEST_SINGLE_TRANSACTION, pTransaction->pTracker->pPacketTracker);
	...
	TransServerPacketSend(&pPacket, pTransaction);
...
void HandleNewTransactionRequest(TransactionServer *pServer, Packet *pPacket, int eServerType, U32 iServerID, int iConnectionIndex)
{
	...
	if (pTransaction->pTracker->bFailAll)
	{
		TRANS_VERBOSE_LOG(pTransaction, TVL_FAILED, "bFailAll set", 0, 0, 0);
		GenerateTransactionResponse(pServer, pTransaction, &pTransaction->responseHandle, TRANSACTION_OUTCOME_FAILURE);
		CleanupTransaction(pServer, pTransaction);
		PERFINFO_AUTO_STOP();
		return;
...
*/
	...
	pPacket = NewTransactionPacket(pServer, iConnectionIndex, eMessageType, pTransaction->pTracker->pPacketTracker);
...
void ProcessAndSendUpdateStrings(TransactionServer *pServer, Transaction *pTransaction, char *pComment)
{
	...
	static PacketTracker *pPacketTracker;

	ONCE(pPacketTracker = PacketTrackerFind("ProcessAndSendUpdateStrings", 0, NULL);)
	...
	if (eaSize(&ppDataBlocks))
	{
		...
		pPacket = NewTransactionPacket(pServer, pServer->iDatabaseConnectionIndex, TRANSSERVER_TRANSACTION_DBUPDATE, pPacketTracker);
...
void GenerateTransactionResponse(TransactionServer *pServer, Transaction *pTransaction, TransactionResponseHandle *pHandle, enumTransactionOutcome eOutcome)
{
	...
	if (pHandle->iReturnValID)
	{
		...
		if (iConnection != -1)
		{
			...
			pPacket = NewTransactionPacket(pServer, iConnection, TRANSSERVER_TRANSACTION_COMPLETE, pTransaction->pTracker->pPacketTracker);
...
void UpdateTransactionServer(TransactionServer *pServer)
{
	...
	if ((iNumToVerboseLog = ea32Size(&pServer->pTransactionsToVerboseLog)))
	{
		...
		for (i = 0; i < iNumToVerboseLog; i++)
		{
			pTransaction = GetTransactionFromIDIfExists(pServer, pServer->pTransactionsToVerboseLog[i]);
			...
			//if the transaction either doesn't exist, or is older than our cutoff, remove it. We always just
			//remove a block of transactions from the beginning of the array.
			iNumToRemove++;
		...
		if (iNumToRemove)
		{
			ea32RemoveRange(&pServer->pTransactionsToVerboseLog, 0, iNumToRemove);
		}
	}
...
void InitTransactionServer(TransactionServer *pServer)
{
	...
	for (i=0; i < gMaxTransactions; i++)
	{
		...
		pServer->pTransactions[i].pNextTransaction = (i == gMaxTransactions-1 ? NULL : &pServer->pTransactions[i+1]);
	}
...
void GenerateConnectionResult(NetLink *pLink, enumTransServerConnectResult eResult)
{
	...
	TransServerPacketSend(&pak, NULL);
}
...
void HandleNormalConnectionRegisterClientInfo(TransactionServer *pServer, Packet *pPacket, NetLink *pLink, TransactionServerClientLink *client)
{
	...
	if (pConnection->eServerType != GLOBALTYPE_OBJECTDB)
	{
		linkReceiveStats_AddLinkToNamedGroup(pLink, "NonMultiplexed_NonObjectDB");
	}
...
void HandleMultiplexConnectionRegisterClientInfo(TransactionServer *pServer, Packet *pPacket, NetLink *pLink, int iSenderMultiplexIndexFull, TransactionServerClientLink *client)
{
	...
	if (iMultiplexConnectionIndex == -1)
	{
		...
		pLink->trans_server_link_to_multiplerxer = 1;
	...
	linkReceiveStats_AddLinkToNamedGroup(pLink, "AllMultiplexed");
...
*/
	...
	case BASETRANS_STATE_POSSIBLE_WAITFORCONFIRM:
		...
		//first send a message telling them to cancel the transaction, then clear our connection handle so
		//we will ignore anything else we get from them
		SendSimpleMessage(pServer, pTransaction, iIndex, TRANSSERVER_CANCEL_TRANSACTION, SENDSIMPLEFLAG_ONLY_SEND_IF_CONNECTION_ALREADY_EXISTS);
		TransClearConnectionHandle(pServer, pTransaction, iIndex);
		break;
...
void SendSimpleDBUpdateString(TransactionServer *pServer, Transaction *pTransaction, char *pDatabaseUpdateString, char *pComment)
{
	...
	if (pTransaction)
	{
		pPacketTracker = pTransaction->pTracker->pPacketTracker;
	}
	else
	{
		static PacketTracker *pPacketTrackerWithNoTransaction;
		ONCE(pPacketTrackerWithNoTransaction = PacketTrackerFind("SendSimpleDBUpdateString_NoTrans", 0, NULL));
		pPacketTracker = pPacketTrackerWithNoTransaction;
	}
...
void HandleLocalTransactionUpdates(TransactionServer *pServer, Packet *pPacket)
{
	...
	if (eaSize(&ppDataBlocks))
	{
		...
		TransServerPacketSend(&outPacket, NULL);
		eaDestroyEx(&ppDataBlocks, TransDataBlockDestroy);
	}
	...
	S64 iErrorCB, S64 iErrorUserData1, S64 iErrorUserData2)
{
	...
	ONCE(pPacketTracker = PacketTrackerFind("HandleGotFailureFromOtherShard", 0, NULL));
	...
	{
		Packet *pOutPack = NewTransactionPacket(&gTransactionServer, iActualIndex, TRANSSERVER_SIMPLE_PACKET_ERROR, pPacketTracker);
		pktSendBits64(pOutPack, 64, iErrorCB);
		pktSendBits64(pOutPack, 64, iErrorUserData1);
		pktSendBits64(pOutPack, 64, iErrorUserData2);
		TransServerPacketSend(&pOutPack, NULL);
		return;
	}
}
...
void HandleSendPacketSimple(TransactionServer *pServer, Packet *pPak, int iReturnConnectionIndex)
{
	...
	while (1)
	{
		...
		if (recip.containerID == SPECIAL_CONTAINERID_ALL)
		{
			GetAllConnectionIndicesForType(pServer, pTransName, recip.containerType, &pConnectionIndices);
		}
		else
		{
	...
	if (iErrorCB)
	{
		...
		if (!ea32Size(&pConnectionIndices))
		{
			pOutPack = NewTransactionPacket(pServer, iReturnConnectionIndex, TRANSSERVER_SIMPLE_PACKET_ERROR, 
				PacketTrackerFind("HandleSendPacketSimple_Error", 0, NULL));
			pktSendBits64(pOutPack, 64, iErrorCB);
	...
	DirectlyUpdateSimpleRemoteCommandStats(pTransName, 
		ea32Size(&pConnectionIndices) * ((int)strlen(pTransName) + pktGetNumRemainingRawBytes(pPak)), 
		&pPacketTracker);

	if (ea32Size(&pConnectionIndices) > 1)
	{
		Packet **ppOutPackets = NULL;
		CreateMultipleNewTransactionPackets(pServer, &pConnectionIndices, TRANSSERVER_SEND_PACKET_SIMPLE, pPacketTracker, &ppOutPackets);

		FOR_EACH_IN_EARRAY(ppOutPackets, Packet, pOutPackFromArray)
		{
			pktSendString(pOutPackFromArray, pTransName);


			pktCopyRemainingRawBytesToOtherPacket(pOutPackFromArray, pPak);
			TransServerPacketSend(&pOutPackFromArray, NULL);


		}
		FOR_EACH_END;

		eaDestroy(&ppOutPackets);
	}
	else if (ea32Size(&pConnectionIndices))
	{
		pOutPack = NewTransactionPacket(pServer, pConnectionIndices[0], TRANSSERVER_SEND_PACKET_SIMPLE, 
			pPacketTracker);
...
void HandleSendPacketSimpleOtherShard(TransactionServer *pServer, Packet *pPak, int iReturnConnectionIndex)
{
	...
	static StashTable sPacketTrackers_SameShard = NULL;
	static StashTable sPacketTrackers_OtherShard = NULL;

	PacketTracker *pFoundTracker;
	...
	if (stricmp(pShardName, GetShardNameFromShardInfoString()) == 0)
	{
		...
		if (!sPacketTrackers_SameShard)
		{
			sPacketTrackers_SameShard = stashTableCreateWithStringKeys( 32, StashDeepCopyKeys_NeverRelease );
		}

		if (!stashFindPointer(sPacketTrackers_SameShard, pTransName, &pFoundTracker))
		{
			pFoundTracker = PacketTrackerFind("OtherShard_BackToMe", 0, pTransName);
			stashAddPointer(sPacketTrackers_SameShard, pTransName, pFoundTracker, true);
		}
		...
		if (iConnectionIndex == -1)
		{
			if (iErrorCB)
			{
				Packet *pOutPack = NewTransactionPacket(pServer, iReturnConnectionIndex, TRANSSERVER_SIMPLE_PACKET_ERROR, pFoundTracker);
				pktSendBits64(pOutPack, 64, iErrorCB);
				pktSendBits64(pOutPack, 64, iUserData1);
				pktSendBits64(pOutPack, 64, iUserData2);
				TransServerPacketSend(&pOutPack, NULL);
		...
		{
			Packet *pOutPack = NewTransactionPacket(pServer, iConnectionIndex, TRANSSERVER_SEND_PACKET_SIMPLE, pFoundTracker);
	...
	{
		if (!sPacketTrackers_OtherShard)
		{
			sPacketTrackers_OtherShard = stashTableCreateWithStringKeys( 32, StashDeepCopyKeys_NeverRelease );
		}

		if (!stashFindPointer(sPacketTrackers_OtherShard, pTransName, &pFoundTracker))
		{
			pFoundTracker = PacketTrackerFind("OtherShard", 0, pTransName);
			stashAddPointer(sPacketTrackers_OtherShard, pTransName, pFoundTracker, true);
		}

		if (!TransactionServer_ShardCluster_SendRemoteCommandPacket(pFoundTracker, pPak, pTransName, pShardName, recip.containerType, recip.containerID,
...
void HandleRequestContainerOwner(TransactionServer *pServer, Packet *pPak, int iReturnConnectionIndex)
{
	...
	ONCE(pTracker = PacketTrackerFind("HandleRequestContainerOwner", 0, NULL));
	...
	pOutPack = NewTransactionPacket(pServer, iReturnConnectionIndex, TRANSSERVER_HERE_IS_OWNER, pTracker);
	if (pOutPack)
	{
...
{
	char *pRemoteShardName = pktGetStringTemp(pPack);
	...
	pOutPack = NewTransactionPacket(&gTransactionServer, iConnectionIndex, TRANSSERVER_SEND_PACKET_SIMPLE, 
		PacketTrackerFind("RemoteCommandFromOtherShard", 0, allocAddString(pTransName)));
	
	pktSendString(pOutPack, pTransName);

	iBytesSent = pktCopyRemainingRawBytesToOtherPacket(pOutPack, pPack);
	TransServerPacketSend(&pOutPack, NULL);

	TransServerShardCluster_SendReceipt(pLink, iReceiptIndex, true);

	TransactionServer_ShardCluster_ReportClusterCommand(false, pTransName, pRemoteShardName, iBytesSent);
