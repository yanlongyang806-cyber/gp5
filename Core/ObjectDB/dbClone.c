...
int MasterObjectDBDisconnect(NetLink* link, void *user_data)
{
	...
	errorCode = linkGetDisconnectErrorCode(link);
	linkGetDisconnectReason(link, &reason);

	if(gDatabaseConfig.bEnableDisconnectAlerts)
		ErrorOrCriticalAlert("CLONE_DISCONNECT_FROM_MASTER", "CloneObjectDB disconnected from master with error code %u and reason: %s", errorCode, reason ? reason : "UNKNOWN");

	SERVLOG_PAIRS(LOG_ERRORS, "ObjectDBDisconnect", 
...
static void MasterObjectDBHandleMsg(Packet *pak,int cmd, NetLink *link,void *user_data)
{
	switch (cmd)
	{
		...
		break;
		case DBTOCLONE_REOPENOFFLINEHOG:
		{
			FlushGenericDatabaseThreads(true);
			FlushOfflineHogg();
			ReOpenOfflineHoggForRestores();
			TellCloneToReOpenOfflineHog(); // In case there is a cloneofclone
		}
		break;
		case DBTOCLONE_DEFRAGOFFLINEHOG:
		{
			// initiate defrag process
			LaunchOfflineDefrag(true);
		}
		break;
	}
}
