...
F32 gObjectDBAllocationTimeAlertThreshold = 0.010;
AUTO_CMD_FLOAT(gObjectDBAllocationTimeAlertThreshold, ObjectDBAllocationTimeAlertThreshold) ACMD_AUTO_SETTING(ObjectDB, OBJECTDB, CLONEOBJECTDB, CLONEOFCLONE) ACMD_CALLBACK(ChangeObjectDBAllocationTimeAlertThreshold);

void ChangeObjectDBAllocationTimeAlertThreshold(CMDARGS)
{
	SetAllocationTimeAlertThreshold(gObjectDBAllocationTimeAlertThreshold);
}
...
static U32 gOfflineStatusPrintfThrottle = 10;
AUTO_CMD_INT(gOfflineStatusPrintfThrottle, OfflineStatusPrintfThrottle) ACMD_CMDLINE;
...
void DBInitialize(void)
{
	...
	EnableAllocationTimeAlert(true);
}
...
AUTO_CMD_INT(gDatabaseState.bDefragOfflineHog,DefragOfflineHog) ACMD_COMMANDLINE;
AUTO_CMD_INT(gDatabaseState.bCloneDefragOfflineHog,CloneDefragOfflineHog) ACMD_COMMANDLINE;
...
AUTO_CMD_INT(gDatabaseConfig.bEnableOfflineDefrag,EnableOfflineDefrag) ACMD_COMMANDLINE;
AUTO_CMD_INT(gDatabaseConfig.bEnableOfflineAppendMode,EnableOfflineAppendMode) ACMD_COMMANDLINE;
AUTO_CMD_INT(gDatabaseConfig.bEnableDisconnectAlerts,EnableDisconnectAlerts) ACMD_COMMANDLINE;
...
AUTO_CMD_INT(gDatabaseConfig.iOfflineCleanupThrottle,OfflineCleanupThrottle) ACMD_COMMANDLINE;
AUTO_CMD_INT(gDatabaseConfig.iOfflineFrameThrottle,OfflineFrameThrottle) ACMD_COMMANDLINE;
AUTO_CMD_INT(gDatabaseConfig.iOfflineMaxTransactionsInFlight,OfflineMaxTransactionsInFlight) ACMD_COMMANDLINE;
AUTO_CMD_INT(gDatabaseConfig.bOnlyCleanupDeletedCharacters,OnlyCleanupDeletedCharacters) ACMD_COMMANDLINE;
...
char* dbCreateSnapshotLocal(S32 delaySeconds, bool verbose, char *dumpwebdata, bool hideMerger, bool defrag)
{
	...
	estrConcatf(&estr, " -OldSnapshotMinutes %u", gContainerSource.snapshotMinutesToKeep);
	estrConcatf(&estr, " -OldSnapshotHours %u", gContainerSource.snapshotHoursToKeep);
	estrConcatf(&estr, " -OldSnapshotDays %u", gContainerSource.snapshotDaysToKeep);
	estrConcatf(&estr, " -OldOfflineDays %u", gContainerSource.offlineDaysToKeep);
	estrConcatf(&estr, " -OldJournalHours %u", gContainerSource.journalHoursToKeep);
	estrConcatf(&estr, " -OldPendingHours %u", gContainerSource.pendingHoursToKeep);
...
{
	StashTable headerTable = stashTableCreateWithStringKeys(8, StashDeepCopyKeys_NeverRelease);
...
void dbInit_RunOnce(void)
{
	...
	if (gDatabaseState.bDefragOfflineHog || gDatabaseState.bCloneDefragOfflineHog)
	{
		gDatabaseState.databaseType = DBTYPE_OFFLINEDEFRAG;
	}
	...
	else if (gDatabaseState.databaseType == DBTYPE_OFFLINEDEFRAG)
	{
		printf("Starting in OfflineDefrag mode.\n");
	}
	else
	{
...
//		gLevel_idx = objAddContainerStoreIndexWithPaths(playerStore,".pChar.iLevelExp", ".myContainerId", 0);
	...
	{
		GSM_SwitchToSibling(DBSTATE_OFFLINEDEFRAG, false);
	}
	else
	{
...
void ObjectDBDumpStats(void)
{
	...
	if (iCurTime > iNextStatsTime)
	{
		...
		gStatus.iAverageCommandLatency = timerSeconds64(GetGenericDatabaseThreadLatency()) * 1000;
		gStatus.iAverageCommandsPerSecond = GetGenericDatabaseThreadLastMinuteCount()/60;
...
void dbDefragOfflineHog(void)
{
	// Find the most current offline hog
	...
	// Cleanup temp files
	CleanupTempOfflineHogFiles();

	oldFilename = GetCurrentOfflineHogFilename();

	if(!oldFilename)
	{
		GSM_SwitchToSibling(DBSTATE_CLEANUP, true);
		return;
	}

	// defrag it into a new file

	if(errorValue = hogDefragEx(oldFilename, 0, HogDefrag_NoRename|HogDefrag_RunDiff|HogDefrag_SkipMutex, SAFESTR(tempFilename)))
	{
		ErrorOrCriticalAlert("FAILED_OFFLINE_DEFRAG", "Attempted to defrag %s, but failed with error %d", oldFilename, errorValue);
		estrDestroy(&oldFilename);
		GSM_SwitchToSibling(DBSTATE_CLEANUP, true);
		return;
	}

	// Sleep for one second to make sure the newFilename is distinct from oldFilename
	Sleep(1000);
	timeSecondsSince2000Update();
	// If the new file matches, give it a new name with the current time stamp
	objMakeOfflineFilename(&newFilename, timeSecondsSince2000());

	fileMove(tempFilename, newFilename); 
	estrDestroy(&oldFilename);
	estrDestroy(&newFilename);
	GSM_SwitchToSibling(DBSTATE_CLEANUP, true);
}
...
void initOutgoingIntershardTransfer()
{
	...
	gOutgoingIntershardTransferNetLinks = stashTableCreate(10, StashDeepCopyKeys_NeverRelease, StashKeyTypeStrings, 0);
...
static void ReceiveIntershardTransfer_CB(TransactionReturnVal *returnVal, ISTComplete *completion)
{
	...
	if (returnVal->eOutcome == TRANSACTION_OUTCOME_SUCCESS)
	{
		...
		{
			Packet *pak = pktCreate(link, 0);	//the command doesn't matter here since we're overriding the handler
...
static void ReceiveIntershardTransfer(Packet *pak, NetLink *link)
{
	...
	if (stricmp(prodname, GetShortProductName()))
	{
		...
		{
			Packet *pkt = pktCreate(link, 0);	//the command doesn't matter here since we're overriding the handler
...
void dbMasterMoveToOffline_BeginFrame(void)
{
	...
	{
		static U32 timeOfLastPrintf = 0;
		U32 currentTime = timeSecondsSince2000();
		ProcessOffliningQueue();
		if(timeOfLastPrintf + gOfflineStatusPrintfThrottle < currentTime)
		{
			fprintf(fileGetStderr(),"\r%u/%u containers offlined. %u failures.", EntityPlayersOfflined(), TotalEntityPlayersToOffline(), OfflineFailures());
			timeOfLastPrintf = currentTime;
		}
	}
}
...
void LaunchOfflineDefrag(bool force)
{
	if(gDatabaseConfig.bEnableOfflineDefrag && (force || EntityPlayersOfflined() > 0 || ContainersCleanedUp() > 0))
	{
		char *estr = NULL;
		estrStackCreate(&estr);
		estrPrintf(&estr, "%s", getExecutableName());
		if(IsThisMasterObjectDB())
			estrConcatf(&estr, " -DefragOfflineHog");
		else if(IsThisCloneObjectDB())
			estrConcatf(&estr, " -CloneDefragOfflineHog");
	
		estrConcatf(&estr, " -LogServer %s", gServerLibState.logServerHost);
		estrConcatf(&estr, " -ControllerHost %s", gServerLibState.controllerHost);
		estrConcatf(&estr, " -SetProductName %s %s", GetProductName(), GetShortProductName());
		estrConcatf(&estr, " -SetErrorTracker %s", getErrorTracker());
		system_detach(estr,0,!gDatabaseConfig.bShowSnapshots);
		estrDestroy(&estr);
		servLog(LOG_CONTAINER, "OfflineDefragLaunched", "");
		printf("OfflineDefragLaunched\n");
		// The clone will also call this in case there is a cloneofclone. 
		// There is an internal check to make sure that there is a clone link
		TellCloneToDefragOfflineHog(); 
	}
}
...
void dbMasterCleanUpOffline_BeginFrame(void)
{
	if(IsCleaningUpOfflineHoggDone())
	{
		if (gDatabaseConfig.bEnableOfflining)
		{
			...
			TellCloneToReOpenOfflineHog();

			LaunchOfflineBackupScript();
			// initiate defrag process
			LaunchOfflineDefrag(false);
...
void dbMasterHandleRequests_Enter(void)
{
	...
	dbLogin2_EnableLoginRemoteCommands(true);
}
...
char *dbDataDir(void)
{
	...
	if (IsThisCloneObjectDB() || gDatabaseState.bCloneCreateSnapshot || gDatabaseState.bCloneDefragOfflineHog)
		isClone = true;
...
void dbInitStates(void)
{
	...
	GSM_AddGlobalState(DBSTATE_OFFLINEDEFRAG);
	GSM_AddGlobalStateCallbacks(DBSTATE_OFFLINEDEFRAG,NULL, dbDefragOfflineHog, NULL,NULL);
...
int dbLoadEntireDatabase(void)
{
	...
	if(gDatabaseConfig.bEnableOfflining)
	{
		...
		hogfile = OpenOfflineHogg(true, gDatabaseConfig.bEnableOfflineAppendMode && gDatabaseConfig.bEnableOfflineDefrag, &bCreated);
...
void dbSendGlobalInfo(TimedCallback *callback, F32 timeSinceLastCallback, UserData userData)
{
	...
	info.iDBThreadQueueLatencyMSecs = gStatus.iAverageCommandLatency;
	info.iDBThreadQueueOperationsPerSecond = gStatus.iAverageCommandsPerSecond;
	...
	info.iTransfersPerSec = GetDBUpdateTransferAverage();
