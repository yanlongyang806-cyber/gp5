...
static void CopyPacketContents(CachedSubscriptionCopy *copy, Packet *pak, int start)
{
	...
	copy->pak_data = malloc_special_heap(total, CRYPTIC_SUBSCRIPTION_HEAP);
...
typedef struct SubscribePerf {
	char*			reason;
	char			reasonBuffer[100];
	PERFINFO_TYPE*	pi;
} SubscribePerf;

TSMP_DEFINE(SubscribePerf);

StashTable stSubscribeReasonToPerf;

static SubscribePerf *CreateSubscribePerf(void)
{
	ATOMIC_INIT_BEGIN;
	TSMP_SMART_CREATE(SubscribePerf, 256, TSMP_X64_RECOMMENDED_CHUNK_SIZE);
	ATOMIC_INIT_END;

	return TSMP_CALLOC(SubscribePerf);
}
...
static void CopyZippedDataForSubscription(Container *con, ParseTable **pti, void **data, int *packed_len, int *unpacked_len, int *header_size)
{
	...
	*data = malloc_special_heap(size_to_use, CRYPTIC_CHURN_HEAP);
...
void dbContainerOwnerChangeNotify(GWTCmdPacket *packet, GlobalType conType, ContainerID conID, GlobalType ownerType, ContainerID ownerID, bool getLock)
{
	...
	// This needs to happen inside EnterCriticalSection(&gSubscriptionTrackingCS);
...
static ContainerSubscription *AddContainerIndexedSubscription(MonitoredContainer *pMonitored, GlobalType conType, ContainerID conID, GlobalType ownerType, ContainerID ownerID, const char* reason)
{
	...
	if (!pSubscription)
	{
		...
		if(dbAlwaysPerfSubscriptions || PERFINFO_RUN_CONDITIONS)
		{
			...
			if(!sp){
				sp = CreateSubscribePerf();
				estrBufferCreate(&sp->reason, sp->reasonBuffer, sizeof(sp->reasonBuffer));
				estrCopy(&sp->reason, &ownerAndReason);
												...
												const char* reason)
{
	...
	info.conID = conID;
	info.reason = strdup_special(reason, CRYPTIC_CHURN_HEAP);
...
AUTO_COMMAND_REMOTE;
int IsContainerSubscribed(GlobalType containerType, ContainerID containerID)
{
	MonitoredContainer *pMonitored;
	int subscriptions;
	...
	PERFINFO_AUTO_START_FUNC();	
	EnterCriticalSection(&gSubscriptionTrackingCS);
	pMonitored = dbAddOrGetMonitoredContainer(containerType, containerID);
	subscriptions = pMonitored ? pMonitored->refCount : 0;
	LeaveCriticalSection(&gSubscriptionTrackingCS);
	PERFINFO_AUTO_STOP();
...
int GetDBUpdateTransferAverage(void)
{
	int retVal;
	EnterCriticalSection(&gSubscriptionTrackingCS);
	retVal = CountAverager_Query(gDatabaseState.DBUpdateTransferAverager, AVERAGE_MINUTE);
	LeaveCriticalSection(&gSubscriptionTrackingCS);
	return retVal;
}
