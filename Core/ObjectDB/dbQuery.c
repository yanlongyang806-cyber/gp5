...
// Before hooking this up again, consider removing the FlushGenericDatabaseThreads call and rewriting dbRunQuery not to 
// use ContainerIterator. Instead, it should use a more safe technique for walking the containerStore such as 
// ForEachContainerOfType.
...
bool dbQueryXpathProcessingCB(char *pLocalXPath, UrlArgumentList *pArgList, int iAccessLevel, StructInfoForHttpXpath *pStructInfo, GetHttpFlags eFlags)
{
	...
	if (queryString)
	{
		//Flush the background threads so that there cannot be contention
