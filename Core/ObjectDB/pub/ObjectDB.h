...
typedef enum 
{
	...
	DBTYPE_REPLAY,
	DBTYPE_OFFLINEDEFRAG, //Defrag the offline hog
} DatabaseType;
...
typedef struct DatabaseState
{
	...
	bool	bDefragOfflineHog; // Defrag the offline hog
	bool	bCloneDefragOfflineHog; // Defrag the offline hog
...
typedef struct DatabaseConfig
{
	...
	bool	bEnableOfflineDefrag;				AST(DEFAULT(1))//Automatically defrags the offline.hogg after a restart
	bool	bEnableOfflineAppendMode;			AST(DEFAULT(1))//Writes to the offline.hogg file in append mode for improved speed.
	bool	bEnableDisconnectAlerts;			AST(DEFAULT(1))//Alert when the main or cloneObjectDBs disconnect
	...
	U32		iTargetDefragWindowStart;			AST(DEFAULT(18))//Automated defrags will happen after start and before start + duration.
	...
	U32		iOfflineCleanupThrottle;			AST(DEFAULT(120000))
	U32		iOfflineFrameThrottle;				AST(DEFAULT(100))
	U32		iLowLevelDCCThreshold;				AST(DEFAULT(2*24*60*60))
	U32		iLowLevelStaleThresholdHours;		AST(DEFAULT(5))
	U32		iOfflineMaxTransactionsInFlight;	AST(DEFAULT(1000))
	bool	bOnlyCleanupDeletedCharacters;		AST(DEFAULT(false))
...
#define DBSTATE_OFFLINEDEFRAG "dbOfflineDefrag"
...
typedef struct ObjectDBStatus
{
	...
	F32 iAverageCommandLatency;
	S64 iAverageCommandsPerSecond;
