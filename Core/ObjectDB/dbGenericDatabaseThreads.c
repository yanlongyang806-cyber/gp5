...
void QueueOfflineEntityPlayerTransactionComplete(GWTCmdPacket *packet, OffliningThreadData *threadData);
...
void dbOfflineQueueProcessingCompletion(void *user_data, void *data, GWTCmdPacket *packet)
{
	OffliningThreadData *threadData = (OffliningThreadData*)data;
	if(threadData->success)
		SendOfflineEntityPlayerTransactionRequest(threadData->request, threadData->ppRefs);
	else
		DecrementOfflineTransactionsInFlight();

	objDestroyTransactionRequest(threadData->request);
}

void dbOfflineQueueProcessingThreaded(void *user_data, void *data, GWTCmdPacket *packet)
{
	OffliningThreadData *threadData = (OffliningThreadData*)data;
	// Running in the Generic Threads, so it will already have the container locked
	if(CreateOfflineEntityPlayerTransactionRequest(threadData->conID, threadData->request, &threadData->ppRefs, false))
		threadData->success = true;
	else
		threadData->success = false;
	QueueOfflineEntityPlayerTransactionComplete(packet, threadData);
}

void dbCleanupQueueProcessingThreaded(void *user_data, void *data, GWTCmdPacket *packet)
{
	assertmsg(0, "Not yet implemented.");
}
...
void InitGenericDatabaseThreads(void)
{
	...
	gwtRegisterCmdDispatch(GenericDatabaseThreadManager, CMD_DB_OFFLINE_QUEUE_PROCESSING, dbOfflineQueueProcessingThreaded);
	gwtRegisterCmdDispatch(GenericDatabaseThreadManager, CMD_DB_CLEANUP_QUEUE_PROCESSING, dbCleanupQueueProcessingThreaded);
	...
	gwtRegisterMsgDispatch(GenericDatabaseThreadManager, MSG_DB_OFFLINE_QUEUE_COMPLETION, dbOfflineQueueProcessingCompletion);
...
void QueueCreateOfflineEntityPlayerTransactionRequest(OffliningThreadData *threadData)
{
	//objGetObjectLock will always return a valid struct
	ObjectLock *objectLock;
	PERFINFO_AUTO_START_FUNC();
	objectLock = objGetObjectLock(GLOBALTYPE_ENTITYPLAYER, threadData->conID);
	assert(objectLock);
	queueGenericDatabaseRequest_Struct(CMD_DB_OFFLINE_QUEUE_PROCESSING, *threadData, OffliningThreadData, objectLock);
	PERFINFO_AUTO_STOP();
}
...
void QueueDBLogin2UnpackPetsOnGenericDatabaseThreads(DBGetCharacterDetailState *detailState, ContainerRef **puppetRefs, ContainerRef **petRefs)
{
	ObjectLock **objectLocks = NULL;
	int i;
	int size;
	PERFINFO_AUTO_START_FUNC();
	for (i = 0, size = eaSize(&puppetRefs); i < size; ++i)
	{
		eaPush(&objectLocks, objGetObjectLock(puppetRefs[i]->containerType, puppetRefs[i]->containerID));
	}
	for (i = 0, size = eaSize(&petRefs); i < size; ++i)
	{
		eaPush(&objectLocks, objGetObjectLock(petRefs[i]->containerType, petRefs[i]->containerID));
	}
...
{
	PERFINFO_AUTO_START_FUNC();
	gwtQueueMsgStruct(packet, MSG_DB_OFFLINE_QUEUE_COMPLETION, *threadData, OffliningThreadData);
	PERFINFO_AUTO_STOP();
}
...
S64 GetGenericDatabaseThreadLatency(void)
{
	return gwtGetAverageCommandLatency(GenericDatabaseThreadManager);
}

S64 GetGenericDatabaseThreadLastMinuteCount(void)
{
	return gwtGetLastMinuteCount(GenericDatabaseThreadManager);
}
