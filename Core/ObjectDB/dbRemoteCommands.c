...
int objDBUpdateApplyOperations(char **broadcastDiff, Container *object, ObjectPathOperation **pathOperations, char **errorString)
{
	...
	for (i = 0; i < eaSize(&pathOperations) && success; ++i)
	{
		...
		void *structptr = NULL;
		int index = 0;
		int pathFlags = OBJPATHFLAG_DONTLOOKUPROOTPATH | OBJPATHFLAG_WRITERESULTIFNOBROADCAST | OBJPATHFLAG_INCREASEREFCOUNT | OBJPATHFLAG_TREATIGNOREASSUCCESS;
...
static int gDiffApplyAlertInterval = 3600;
AUTO_CMD_INT(gDiffApplyAlertInterval, DiffApplyAlertInterval);

static int gDiffApplyAlertThreshold = 1000;
AUTO_CMD_INT(gDiffApplyAlertThreshold, DiffApplyAlertThreshold);
...
int objDBModifyContainer(char **broadcastDiff, Container *object, char *diffString)
{
	...
	else
	{
		...
		{
			static TimingHistory *diffApplyHistory = NULL;
			static U32 lastTimeReset = 0;
			int countInInterval = 0;
			static CRITICAL_SECTION diffApplyHistoryCS = {0};

			ATOMIC_INIT_BEGIN;
			if(gDiffApplyAlertThreshold > 100000)
				gDiffApplyAlertThreshold = 100000; // To limit how much RAM can be allocated for the timing history
			diffApplyHistory = timingHistoryCreate(10*gDiffApplyAlertThreshold);
			InitializeCriticalSection(&diffApplyHistoryCS);
			ATOMIC_INIT_END;

			TriggerAlertf("OBJECTDB.DATA_CORRUPTION", ALERTLEVEL_WARNING, ALERTCATEGORY_NETOPS, 0, object->containerType, object->containerID, GetAppGlobalType(), GetAppGlobalID(), getHostName(), 0,
			...
			EnterCriticalSection(&diffApplyHistoryCS);
			timingHistoryPush(diffApplyHistory);
			if((timeSecondsSince2000() - gDiffApplyAlertInterval > lastTimeReset) && 
				(countInInterval = timingHistoryInLastInterval(diffApplyHistory, gDiffApplyAlertInterval) > gDiffApplyAlertThreshold))
			{
				TriggerAlertf("OBJECTDB.MASS_DATA_CORRUPTION", ALERTLEVEL_CRITICAL, ALERTCATEGORY_NETOPS, 0, object->containerType, object->containerID, GetAppGlobalType(), GetAppGlobalID(), getHostName(), 0,
					"The ObjectDB has had %d diff apply errors in the last %d seconds. Details can be found in warning alerts. Escalate immediately.", countInInterval, gDiffApplyAlertInterval);
				lastTimeReset = timeSecondsSince2000();
			}

			LeaveCriticalSection(&diffApplyHistoryCS);
		}
	}
...
int dbUpdateContainer(U32 containerType, U32 containerID, ACMD_SENTENCE diffString)
{
	...
	info.containerID = containerID;
	info.containerType = containerType;
	info.diffString = strdup_special(diffString, CRYPTIC_CHURN_HEAP); 
	if(GenericDatabaseThreadIsActive() && !OnBackgroundGenericDatabaseThread())
	{
...
AUTO_COMMAND_REMOTE;
void RemoteHandleDatabaseSyntheticLogDataString(const char *cmd, U64 sequence, U32 timestamp)
{
	static U32 lastTime = 0;

	U32 currentTime = timerCpuSeconds();

	if(!lastTime || currentTime > (lastTime + 5))
	{
	...
	if(nullStr(cmd) || !strStartsWith(cmd, "dbUpdateContainer "))
		return;

	dbHandleDatabaseUpdateString(cmd, 0, 0);
}
...
AUTO_COMMAND ACMD_CATEGORY(DEBUG) ACMD_ACCESSLEVEL(7);
...
void GetDebugContainerLocStringCB(ContainerID iMCPID, int iRequestID, int iClientID, CommandServingFlags eFlags, char *pMessageString, void *pUserData)
{
...
void PrintfSlowCmdReturnCallbackFunc(ContainerID iMCPID, int iRequestID, int iClientID, CommandServingFlags eFlags, char *pMessageString, void *pUserData)
{
