...
static void HandleCloneDisconnect(NetLink *link, void *user_data)
{
	...
	if(gDatabaseConfig.bEnableDisconnectAlerts)
		ErrorOrCriticalAlert("MASTER_DISCONNECT_FROM_CLONE", "Main ObjectDB disconnected from clone with error code %u and reason: %s", errorCode, reason ? reason : "UNKNOWN");
...
void TellCloneToReOpenOfflineHog(void)
{
	Packet *pak = NULL;
	if(dbConnectedToClone())
	{
		pak = pktCreate(sLinkToClone, DBTOCLONE_REOPENOFFLINEHOG);
		pktSend(&pak);
	}
}

void TellCloneToDefragOfflineHog(void)
{
	Packet *pak = NULL;
	if(dbConnectedToClone())
	{
		pak = pktCreate(sLinkToClone, DBTOCLONE_DEFRAGOFFLINEHOG);
		pktSend(&pak);
	}
}
