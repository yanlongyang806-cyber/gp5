...
typedef struct GraphCategory
{
	...
	bool bShouldCheckForZeroPoint; NO_AST
} GraphCategory;

AUTO_STRUCT;
typedef struct OtherGraphWantsMyCategory
{
	Graph *pOtherGraph; NO_AST
    char *pLocalCategoryName;
	char *pRemoteCategoryName;
	float fScale;
} OtherGraphWantsMyCategory;
...
//will be non-NULL. Otherwise, pOnly Category will be non-NULL
	...
	OtherGraphWantsMyCategory **ppOtherGraphsWantMyCategories; AST(NO_WRITE)

	AST_COMMAND("WriteCSV", "WriteCSVCmd $FIELD(GraphName) $STRING(Output filename)")
} Graph;
