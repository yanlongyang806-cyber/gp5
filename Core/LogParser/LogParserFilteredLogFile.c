...
{
	PurgeDirectoryOfOldFiles_Secs("c:\\LogParser\\Downloads", 5 * 24 * 60 * 60, NULL, false);
}


void InitFilteredLogFile(bool bCompressed, const char *pPrefixName)
{
	static int i = 0;
	...
	char *pPrefix = NULL;
	char *pPrefixNameFixedUp = NULL;

	AbortFilteredLogFile();
	siLogCount = 0;

	estrCopy2(&pPrefixNameFixedUp, pPrefixName);

	//just alphanum characters from prefix, as it's purely for convenience, and we don't want
	//punctuation or numbers cluttering it up
	for (i = estrLength(&pPrefixNameFixedUp) - 1; i >= 0; i--)
	{
		if (!isalpha(pPrefixNameFixedUp[i]))
		{
			estrRemove(&pPrefixNameFixedUp, i, 1);
		}
	}
	...
	do
	{
		...
		{
			i++;
			sprintf(filename, "c:\\LogParser\\Downloads\\FilteredLogs_%s_%d.txt", pPrefixNameFixedUp, i);
			sprintf(filenameGz, "c:\\LogParser\\Downloads\\FilteredLogs_%s_%d.txt.gz", pPrefixNameFixedUp, i);
		}
