...
void LogParserBinning_AddLog(ParsedLog *pInLog)
{
	...
	if (eaSize(&sLogsForBinning.ppList) == MAX_LOGS_PER_FILE)
	{
		...
		ParserWriteBinaryFile(fileName,NULL, parse_ParsedLogList, &sLogsForBinning, NULL, NULL, NULL, NULL, 0, 0, NULL, 0, 0);
...
void LogParserBinning_ParserAllLogsFromBin(CmdContext *pContext)
{
	int i, j;
	...
	if (gStandAloneOptions.bCreateFilteredLogFile)
	{
		InitFilteredLogFile(gStandAloneOptions.bCompressFilteredLogFile,
			(pContext && pContext->pAuthNameAndIP) ? pContext->pAuthNameAndIP
			: "NO_AUTH");
	}
