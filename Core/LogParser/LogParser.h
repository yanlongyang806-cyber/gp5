...
//a graph can have one or more CategoriesFromOtherGraph, which is basically the entire line from the other
//graph overlaid on top of this graph
//
//If SourceCateogryName is set, then grab only that category, and either have it keep its name, or give it
//the TargetCategoryName. If SourceCategoryName is not set, then grab all categories. If there is only one,
//name it TargetCategoryName. If there are multiple, prefix them with TargetCategoryName_. If TargetCategoryName
//is not set, use OtherGraphName
AUTO_STRUCT;
typedef struct CategoryFromOtherGraph
{
	char *pOtherGraphName;
	char *pSourceCategoryName;
	char *pTargetCategoryName;
	float fScale; //if non-zero, multiply the value by this amount
} CategoryFromOtherGraph;
...
typedef struct GraphDefinition
{
	...
	char *pFileName;						AST(CURRENTFILE USERFLAG(TOK_LOGPARSER_MUTABLE))
...
*/
	...
	CategoryFromOtherGraph **ppCategoryFromOtherGraph;
...
extern char *GetLongTermLogsFileName(void);