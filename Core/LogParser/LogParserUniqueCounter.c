...
void SetFirstCompletionTime(LogParserUniqueCounter *pCounter, U32 iCurTime);
...
static void UniqueCounter_AddLog(LogParserUniqueCounter *pCounter, ParsedLog *pLog, ParseTable *pSubTableTPI, void *pSubObject)
{
	...
	if (!pCounter->iNextCompletionTime)
	{
		SetFirstCompletionTime(pCounter, pLog->iTime);
	}
...
void UniqueCounter_CheckAllForLogsToEmit(U32 iCurTime)
{

	FOR_EACH_IN_EARRAY(spCounters->ppUniqueCounters, LogParserUniqueCounter, pCounter)
	{
		if (pCounter->iNextCompletionTime && pCounter->iNextCompletionTime < iCurTime)
		{
...
void LogParserUniqueCounter_InitAllFromConfig(void)
{
	...
	if (fileExists(pFileName) && !gbStandAlone)
	{
	...
	UniqueCounter_CheckAllForLogsToEmit(timeSecondsSince2000());
...
void LogParserUniqueCounter_Tick(U32 iCurTime)
{
	...
	{
		UniqueCounter_CheckAllForLogsToEmit(iCurTime);
...
void LogParserUniqueCounter_ForceEmitAllAndReset(void)
{
	FOR_EACH_IN_EARRAY(spCounters->ppUniqueCounters, LogParserUniqueCounter, pCounter)
	{
		if (pCounter->iNextCompletionTime)
		{
			UniqueCounter_EmitLog(pCounter, pCounter->iNextCompletionTime);
			pCounter->iNextCompletionTime = 0;
		}
	}
	FOR_EACH_END;
}
