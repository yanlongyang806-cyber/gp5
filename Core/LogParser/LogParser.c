...
//comment set for standalones so we can track who launched them
char *pLaunchComment = NULL;
AUTO_CMD_ESTRING(pLaunchComment, LaunchComment);
...
static void CalculateLoadByCallbackName()
{
	...
	sReportingData = stashTableCreate(10, StashDeepCopyKeys_NeverRelease, StashKeyTypeStrings, 0);
...
{
	sFilesProcessedTable = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
	sLogFileGroupsByName = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
...
{
	U32 iTime = pLog->iTime;
	ProcessSingleLog(pFileName, pLog, false);
	ProcessAllProceduralLogs();
	LogParserUniqueCounter_Tick(iTime);
}
...
AUTO_COMMAND ACMD_ACCESSLEVEL(0);
void BeginDirectoryScanning(CmdContext *pContext)
{
	int i, j;
	...
	giTotalLogsScanned = 0;
	giLogsThatPassedFiltering = 0;
	...
	else if (gStandAloneOptions.bCreateFilteredLogFile)
	{
		InitFilteredLogFile(gStandAloneOptions.bCompressFilteredLogFile,
			(pContext && pContext->pAuthNameAndIP) ? pContext->pAuthNameAndIP : "NOAUTH");
	}
...
void UpdateDirectoryScanning(void)
{
	...
	if (ContinueChunkedLogProcessing(spBucketList, 1.0f, &giDirScanningPercent, &gpDirScanningStatus))
	{
		...
		LogParserUniqueCounter_ForceEmitAllAndReset();
	}
	else
	{
		...
		{
			printf("%I64d out of %I64d logs passed initial filtering\n",
				giLogsThatPassedFiltering, giTotalLogsScanned);
		}
	}
...
void AddActionRestriction(char *pName)
{
	...
	{
		gStandAloneOptions.parsingRestrictions.sAction = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
	}
	...
	switch (eType)
	{
		xcase FIXUPTYPE_POST_TEXT_READ:
		{
			if (eaSize(&pOptions->parsingRestrictions.ppActions))
			{
				...
				{
					pOptions->parsingRestrictions.sAction = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
				}
				else
				{
...
char *GetLongTermLogsFileName(void)
{
	static char *spRetVal = NULL;
	
	if (!spRetVal)
	{
		estrPrintf(&spRetVal, "%s/LongTermLogs.log", fileLogDir());
	}
...
#define STANDALONE_STATUS_INTERVAL 60
...
int wmain(int argc, WCHAR** argv_wide)
{
	...
	U32 iStartupTime = 0;
	U32 iNextStandaloneStatusTime = 0;
	char **argv;
	...
	gImplicitRestrictions.sAction = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);

	if(gbLogCountingMode)
	{
		sEventTypeLogCount = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
		sActionLogCount = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
	}
	...
	if (!gbStandAlone)
	{
		LoadLogsFromFile(GetLongTermLogsFileName());
	...
	iStartupTime = timeSecondsSince2000();
	...
	if (gbStandAlone)
	{
		...
		iNextStandaloneStatusTime = timeSecondsSince2000() + STANDALONE_STATUS_INTERVAL;
	
	}

	if(gStandAloneOptions.bWebFilteredScan)
		BeginDirectoryScanning(NULL);
	...
	while(!gbCloseLogParser)
	{
		...
		if(gbStandAlone)
		{
			...
			if (timeSecondsSince2000() > iNextStandaloneStatusTime)
			{
				char *pRunningTime = NULL;
				char *pInactiveTime = NULL;
				iNextStandaloneStatusTime = timeSecondsSince2000() + STANDALONE_STATUS_INTERVAL;
				timeSecondsDurationToPrettyEString(timeSecondsSince2000() - iStartupTime, &pRunningTime);
				timeSecondsDurationToPrettyEString(timeSecondsSince2000() - GenericHttpServer_GetLastActivityTime(), &pInactiveTime);

				printfColor(COLOR_BLUE | COLOR_GREEN | COLOR_BRIGHT, "Standalone launched %s ago because <<", 
					pRunningTime);
				printfColor(COLOR_HIGHLIGHT, "%s", pLaunchComment);
				printfColor(COLOR_BLUE | COLOR_GREEN | COLOR_BRIGHT, ">> Inactive for %s. ",
					pInactiveTime);

				if (giInactivityTimeOut)
				{
					timeSecondsDurationToPrettyEString(giInactivityTimeOut, &pInactiveTime);
					printfColor(COLOR_BLUE | COLOR_GREEN | COLOR_BRIGHT, "Timeout: %s\n", pInactiveTime);
				}
				else
				{
					printfColor(COLOR_BLUE | COLOR_GREEN | COLOR_BRIGHT, "NOTIMEOUT\n");
				}

				estrDestroy(&pRunningTime);
				estrDestroy(&pInactiveTime);
			}
		...
		else
		{
			...
			LogParserUniqueCounter_Tick(timeSecondsSince2000());
		}
		...
		LogParserScheduledTasks_OncePerFrame();
...
void FrontEndTest(int iUID)
{
	...
	LogParserLaunching_LaunchOne(120, pCmdLine, 0, iUID, "FrontEndTest");
}
...
int BeginFrontEndFiltering(FrontEndFilterRequest *pRequest, CmdContext *pContext)
{
	char *pCmdLine = NULL;
	char *pTemp = NULL;
	char *pSuperEsc = NULL;
	...
	iRetVal = LogParserLaunching_LaunchOne(120, pCmdLine, 0, pRequest->iUID, 
		STACK_SPRINTF("BeginFrontEndFiltering called by %s", pContext->pAuthNameAndIP));
