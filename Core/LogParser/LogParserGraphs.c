...
//tries to sort as ints. If that doesn't work, sort as floats. If that doesn't work, sort as strings
static int SortNames2(const char** a, const char** b)
{
	bool AIsFloat, BIsFloat;
	float fA, fB;
...
AUTO_COMMAND;
void TestForceEndAllGraphLifetimes(void)
{
	FOR_EACH_IN_STASHTABLE(sGraphsByGraphName, Graph, pGraph)
	{
		if (pGraph->pDefinition->iGraphLifespan)
		{
			pGraph->iGraphClearTime = timeSecondsSince2000() - 10;
		}
	}
	FOR_EACH_END;

	Graph_CheckLifetimes(timeSecondsSince2000());
}


static void CheckCategoryForAddingZero(Graph *pGraph, GraphCategory *pCategory, U32 iTime)
{
	GraphDataPoint *pMostRecent;
	U32 iMostRecentTime;


	if (!pCategory->bShouldCheckForZeroPoint)
	{
		return;
	}

	pMostRecent = eaTail(&pCategory->ppDataPoints);
	if (pMostRecent->fSum == 0.0f)
	{
		return;
	}
	...
	{
		if (iMostRecentTime < iTime - pGraph->pDefinition->iSecondsOfInactivityBeforeSetToZero)
		{
			Graph_AddDataPointToCategoryInGraph(pGraph, NULL, pCategory->pCategoryName, 
				STACK_SPRINTF("%u", iTime), 0.0f, false, true);
		}
	}
}
...
{
	if (pGraph->pDefinition->iSecondsOfInactivityBeforeSetToZero)
	{
		if (pGraph->pOnlyCategory)
		{
			CheckCategoryForAddingZero(pGraph, pGraph->pOnlyCategory, iTime);
		}
		else
		{
			FOR_EACH_IN_EARRAY(pGraph->ppCategories, GraphCategory, pCategory)
			{
				CheckCategoryForAddingZero(pGraph, pCategory, iTime);
			}
			FOR_EACH_END;
		}
	}
	...
	if (pGraph->iGraphClearTime && pGraph->iGraphClearTime < iTime )
	{
		...
		if (pGraph->pDefinition->bLongTermGraph && pGraph->iTotalDataCount)
		{
			...
			file_servLogWithStruct(GetLongTermLogsFileName(), "LongTermData", &longTermData, parse_LogParserLongTermData);
...
{
	int iStartingSize = eaSize(pppDataPointNames);
	int iResult;
	int iCurIndex;
	...
	//check if it goes at the end to begin with, as that's far and away the most
	//common case

	iResult = SortNames2(&pDataPointNamePooled, &(*pppDataPointNames)[iStartingSize - 1]);
	...
	//now do a binary insertion

	iCurIndex = (int)eaBFind( *pppDataPointNames, SortNames2, pDataPointNamePooled );
		
	if ((*pppDataPointNames)[iCurIndex] == pDataPointNamePooled)
	{
		return;
	}
	
	eaInsert(pppDataPointNames, DECONST(char *, pDataPointNamePooled), iCurIndex);
}
...
static bool GraphHasCategoriesNoMatterWhat(Graph *pGraph)
{
	...
	if (eaSize(&pGraph->pDefinition->ppCategoryFromOtherGraph))
	{
		return true;
	}
...
static void MaybeAddDataPointToOtherGraphs(Graph *pGraph, ParsedLog *pLog, const char *pCategoryName, char *pDataPointName, float fCurVal)
{
	FOR_EACH_IN_EARRAY(pGraph->ppOtherGraphsWantMyCategories, OtherGraphWantsMyCategory, pOtherGraphCategory)
	{
		if (!pOtherGraphCategory->pLocalCategoryName || stricmp(pOtherGraphCategory->pLocalCategoryName, pCategoryName) == 0)
		{
			char *pCategoryNameToUse = NULL;
			estrStackCreate(&pCategoryNameToUse);

			if (pGraph->pOnlyCategory)
			{
				if (pOtherGraphCategory->pRemoteCategoryName)
				{
					estrCopy2(&pCategoryNameToUse, pOtherGraphCategory->pRemoteCategoryName);
				}
				else if (pCategoryName)
				{
					estrCopy2(&pCategoryNameToUse, pCategoryName);
				}
				else
				{
					estrCopy2(&pCategoryNameToUse, pGraph->pDefinition->pGraphName);
				}
			}
			else
			{
				if (pCategoryName)
				{
					if (pOtherGraphCategory->pRemoteCategoryName)
					{
						estrPrintf(&pCategoryNameToUse, "%s_%s", pOtherGraphCategory->pRemoteCategoryName, pCategoryName);
					}
					else
					{
						estrCopy2(&pCategoryNameToUse, pCategoryName);
			...
			if (estrLength(&pCategoryNameToUse))
			{

				Graph_AddDataPointToCategoryInGraph(pOtherGraphCategory->pOtherGraph, pLog, pCategoryNameToUse, pDataPointName,
					fCurVal * pOtherGraphCategory->fScale, false, false);
			}

			estrDestroy(&pCategoryNameToUse);
		}
	}
	FOR_EACH_END;
}
	...
	char *pDataPointName, float fCurVal, bool magic, bool bIsInternallyAddedPoint)
{
	...
	{
		pCategory->bShouldCheckForZeroPoint = true;
/*		MaybeAddZeroCache *pCache = malloc(sizeof(MaybeAddZeroCache));
	...
	if (!magic && !bIsInternallyAddedPoint)
	{
		MaybeAddDataPointToOtherGraphs(pGraph, pLog, pCategoryName, pDataPointName, fCurVal);
	}
		...
		if (pGraph->pDefinition->pCategoryField && pGraph->pDefinition->pCategoryField[0] || GraphHasCategoriesNoMatterWhat(pGraph))
		{
			...
			else
			{
				if(!pCategoryNameEString || !pCategoryNameEString[0])
					estrCopy2(&pCategoryNameEString, pGraph->pDefinition->pGraphName);
			}
...
static void ClearAllGraphLists(void)
{
	...
	sGraphsByGraphName = stashTableCreateWithStringKeys(4, StashDeepCopyKeys_NeverRelease);
...
static void InitCategoryFromOtherGraph(Graph *pGraph, CategoryFromOtherGraph *pCategory, Graph *pOtherGraph)
{
	OtherGraphWantsMyCategory *pOtherGraphCategory = StructCreate(parse_OtherGraphWantsMyCategory);
	eaPush(&pOtherGraph->ppOtherGraphsWantMyCategories, pOtherGraphCategory);

	pOtherGraphCategory->pOtherGraph = pGraph;
	pOtherGraphCategory->pLocalCategoryName = strdup(pCategory->pSourceCategoryName);
	pOtherGraphCategory->pRemoteCategoryName = strdup(pCategory->pTargetCategoryName);
	pOtherGraphCategory->fScale = pCategory->fScale ? pCategory->fScale : 1.0f;
...
//			estrPrintf(&pGraph->pImageLink, "<span><a href=\"/viewimage?imageName=LOGPARSER_0_GRAPH_%s.jpg\" target=\"_blank\">%s</a></span>\n", gLogParserConfig.ppGraphs[i]->pGraphName, gLogParserConfig.ppGraphs[i]->pGraphName);
	...
	//now process all CategoryFromOtherGraph settings
	FOR_EACH_IN_STASHTABLE(sGraphsByGraphName, Graph, pGraph)
	{
		if (eaSize(&pGraph->pDefinition->ppCategoryFromOtherGraph))
		{
			FOR_EACH_IN_EARRAY(pGraph->pDefinition->ppCategoryFromOtherGraph, CategoryFromOtherGraph, pCategoryFromOtherGraph)
			{
				Graph *pOtherGraph;

				if (!stashFindPointer(sGraphsByGraphName, pCategoryFromOtherGraph->pOtherGraphName, &pOtherGraph))
				{
					Errorf("Graph %s wants to get a CategoryFromOtherGraph from %s, which doesn't exist",
						pGraph->pDefinition->pGraphName, pCategoryFromOtherGraph->pOtherGraphName);
				}
				else
				{
					InitCategoryFromOtherGraph(pGraph, pCategoryFromOtherGraph, pOtherGraph);
				}
			}
			FOR_EACH_END;
		}
...
void Graph_InitSystem(void)
{
	...
	{
		char fileName[CRYPTIC_MAX_PATH];
		char *pGraphsDestroyed = NULL;

		gbLogParserGraphReadHasAlerted = false;
		GetGraphStatusFileName(SAFESTR(fileName));

		loadstart_printf("Loading preexisting graph data... ");
		...
		for (i=eaSize(&sAllNonLongTermGraphList.ppGraphs) - 1; i >= 0; i--)
		{
			...
			{
				printf("Adding graph %s\n", sAllNonLongTermGraphList.ppGraphs[i]->pGraphName);
				FindOldDataForGraph(sAllNonLongTermGraphList.ppGraphs[i]);
				AddGraphToLists(sAllNonLongTermGraphList.ppGraphs[i]);
			}
			else
			{
				estrConcatf(&pGraphsDestroyed, "%s%s", estrLength(&pGraphsDestroyed) ? ", " : "", sAllNonLongTermGraphList.ppGraphs[i]->pGraphName);
				printf("Destroying graph %s\n", sAllNonLongTermGraphList.ppGraphs[i]->pGraphName);
				StructDestroy(parse_Graph, sAllNonLongTermGraphList.ppGraphs[i]);
				eaRemove(&sAllNonLongTermGraphList.ppGraphs, i);
			}
		}

		if (estrLength(&pGraphsDestroyed))
		{
			WARNING_NETOPS_ALERT("GRAPH_LOAD_FAILURE", "One or more graphs had changed definitions and thus could not load the existing data at logparser restart. This does NOT cause any permanent data loss, since everything is still in the log files, but will cause a discontinuity. This is due to (a) corruption of logparser_graphstatus.txt, (b) an intentional change to the graph definition (in which case this is expected), or (c) an unintentional change to the graph definition, or the graph definition struct. Graphs affected: %s",
				pGraphsDestroyed);
			estrDestroy(&pGraphsDestroyed);
		}
	}
