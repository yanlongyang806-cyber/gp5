...
bool bWaitForDebuggerStandalone = false;
AUTO_CMD_INT(bWaitForDebuggerStandalone, WaitForDebuggerStandalone);
...
char LogDirectoryForLaunched[CRYPTIC_MAX_PATH];
...
void InitStandAloneListening()
{
	...
	for (;;)
	{
		...
		if (StandAloneLogParserLinks)
			break;
		delay(DEFAULT_SERVER_SLEEP_TIME);
	}
}
...
static U32 LogParserLaunching_LaunchInternal(int iTimeOut, const char *pExtraArgs, U32 index, int iUID, char *pComment)
{
	char *pCommandLine = NULL;
	int iPid;
	
	char *pEscapedShardInfoString = NULL;

	assert(index < giNumLaunchedLogParsers);

	estrPrintf(&pCommandLine, "%s -standalone -SetAccountServer %s -LaunchComment \"%s\" -ConnectToLiveLogParser -StartingPortNum %d -SetDirectoriesToScan_Precise %s -InactivityTimeOut %d -SetProductName %s %s - %s - %s", 
		getExecutableName(),
		getAccountServer(),
		pComment,
		STARTING_LOGPARSER_PORT + index, LogDirectoryForLaunched, iTimeOut, GetProductName(), 
		GetShortProductName(), pExtraArgs ? pExtraArgs : "",
		bWaitForDebuggerStandalone ? "-WAITFORDEBUGGER" : "");
	...
	if (GetShardInfoString())
	{
		estrSuperEscapeString(&pEscapedShardInfoString, GetShardInfoString());
		estrConcatf(&pCommandLine, " -EarlySuperEsc SetShardInfoString %s ", pEscapedShardInfoString);
		estrDestroy(&pEscapedShardInfoString);
	}
	...
	spData[index].pHandle = StartQueryableProcessEx(pCommandLine, NULL, false, false, false, NULL, &iPid);

	if (!spData[index].pHandle)
	{
		log_printf(LOG_MISC, "Tried to launch a standalone logparser because %s, StartQueryableProcess failed", pComment);
		return 0;
	}

	log_printf(LOG_MISC, "Launched standalone logparser on %s with pid %d, port %d, because %s", makeIpStr(getHostLocalIp()), iPid, STARTING_LOGPARSER_PORT + index, pComment);
...
U32 LogParserLaunching_LaunchOne(int iTimeOut, const char *pExtraArgs, U32 iForcedPort, int iUID, char *pLaunchComment)
{
	...
	if(iForcedPort)
	{
		...
		{
			return LogParserLaunching_LaunchInternal(iTimeOut, pExtraArgs, index, iUID, pLaunchComment);
		}
	}
	else
	{
		for (i=0; i < giNumLaunchedLogParsers; i++)
		{
			...
			{
				return LogParserLaunching_LaunchInternal(iTimeOut, pExtraArgs, i, iUID, pLaunchComment);
			}
		}
	}
...
char *GetLaunchComment(char *pFunction, CmdContext *pContext)
{
	static char *spRetVal = NULL;

	estrPrintf(&spRetVal, "Launched in %s via %s. Auth: %s",
		pFunction, GetContextHowString(pContext), pContext->pAuthNameAndIP);
...
char *LaunchStandaloneAndReturnLink(char *pUserName, CmdContext *pContext)
{
	...
	if(isProductionMode() && iCurrentTime < giLaunchingInitTime + STANDALONE_LOGPARSER_TIMEOUT)
	{
		sprintf(tempLink, "Just started up, still waiting for (possible) pre-existing standalone logparsers. (%u seconds left)", giLaunchingInitTime + STANDALONE_LOGPARSER_TIMEOUT - iCurrentTime);
		return tempLink;
	}

	iPort = LogParserLaunching_LaunchOne(120, NULL, 0, 0, GetLaunchComment(__FUNCTION__, pContext));
...
AUTO_COMMAND;
void LaunchNoTimeoutStandAloneSpecifyPort(int iPortNum, CmdContext *pContext)
{
	LogParserLaunching_LaunchOne(0, NULL, iPortNum, 0, GetLaunchComment(__FUNCTION__, pContext));
}
