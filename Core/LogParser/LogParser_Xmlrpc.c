...
AUTO_COMMAND ACMD_CATEGORY(XMLRPC);
RunFilteredScanResponse *RunFilteredScan(CmdContext *pContext, RunFilteredScanInfo *pInfo)
{
	...
	// Run scan.
	BeginDirectoryScanning(pContext);
...
AUTO_COMMAND ACMD_CATEGORY(XMLRPC);
LaunchWebFilterStandAloneResponse *LaunchWebFilterStandAlone(RunFilteredScanInfo *pInfo, CmdContext *pContext)
{
	...
	iPort = LogParserLaunching_LaunchOne(0, pExtraArgs, 0, 0, STACK_SPRINTF("Launched by LaunchWebFilterStandAloneResponse, auth: %s",
		pContext->pAuthNameAndIP));
