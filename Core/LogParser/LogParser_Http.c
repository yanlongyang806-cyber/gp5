...
typedef struct LogParserOverview
{
	...
	char *pSchedulerInfoLink; AST(ESTRING, FORMATSTRING(HTML=1, HTML_NO_HEADER=1))
...
LogParserOverview *GetLogParserOverview(void)
{
	...
	estrPrintf(&gpLogParserOverview->pSchedulerInfoLink, "<input type=\"hidden\" id=\"defaultautorefresh\" value=\"1\"><a href=\"/viewxpath?xpath=%s[%u].scheduler\">Scheduled Task info and downloads</a>",
