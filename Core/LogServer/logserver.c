...
static __forceinline char *GlobalTypeToName_ForLogServer(GlobalType eType)
{
	if (eType == GLOBALTYPE_LAST)
	{
		return "Unknown";
	}
	else
	{
		return GlobalTypeToName(eType);
	}
}
...
void MaybeAlertBadSourceType(NetLink *pLink, GlobalType eSourceType)
{
	static U32 *piAlreadyAlertedIPs = NULL;
	 
	U32 iCurIP = linkGetIp(pLink);
	if (ea32Find(&piAlreadyAlertedIPs, iCurIP) != -1)
	{
		return;
	}

	if (ea32Size(&piAlreadyAlertedIPs) == 0)
	{
		CRITICAL_NETOPS_ALERT("OBSOLETE_LOGSERVER", "Logserver getting logs from %s with global type %d. This is nearly certainly because that server is newer than the logserver and has a global type the logserver has never heard of. You should remedy this IMMEDIATELY (by updating logserver.exe). Logs are logged as 'unknown', should be renamed",
			makeIpStr(iCurIP), eSourceType);
	}
	else
	{
		CRITICAL_NETOPS_ALERT("OBSOLETE_LOGSERVER", "Logserver getting logs from %s with global type %d. This is nearly certainly because that server is newer than the logserver and has a global type the logserver has never heard of. You are getting unkown-server-type alerts from TWO DIFFERENT SOURCES now, both are being logged as unknown. Please please please fix this (by updating logserver.exe) ASAP!!!!!!!",
			makeIpStr(iCurIP), eSourceType);
	}

	ea32Push(&piAlreadyAlertedIPs, iCurIP);
}
...
void handleLogPrintf(NetLink *pLink, Packet *pak)
{
	...
	if (eSourceContainerType >= GLOBALTYPE_LAST)
	{
		MaybeAlertBadSourceType(pLink, eSourceContainerType);

		eSourceContainerType = GLOBALTYPE_LAST;
	}
	...
	while((iTimeStamp = pktGetBits(pak, 32)))
	{
...

		{
			...
			{
				printf("Oldest message: %s/%s: %d seconds\n", GlobalTypeToName_ForLogServer(eSourceContainerType), 
		...
		else if (eCategory >= LOG_LAST + EXTRA_LOG_CATEGORIES)
		{
...
#pragma warning(suppress:6001) // /analyze "Using uninitialized memory '*pMessageString'"
		...
		else
		{
			if (sbMessagesAreAlreadySortedByTime)
			{
				...
				sprintf(fileName, "%s/%s", GlobalTypeToName_ForLogServer(eSourceContainerType), GetLogCategoryName(eCategory));
				logDirectWriteWithTime(fileName,pMessageString, 0, iTimeStamp);
			...
			else
			{
				...
				if (!pCategory->name[0])
				{
					sprintf(pCategory->name, "%s/%s", GlobalTypeToName_ForLogServer(eSourceContainerType), 
		...
		if (gTypeIsInUse[eGlobalType])
		{
			for (eCategory = 0; eCategory < LOG_LAST + EXTRA_LOG_CATEGORIES; eCategory++)
			{
				...
				while (eaSize(&pCategory->ppLists) && pCategory->ppLists[0]->iTime <= iWriteCutoffTime)
				{
					...
					if (!fileName[0])
					{
						sprintf(fileName, "%s/%s", GlobalTypeToName_ForLogServer(eGlobalType), GetLogCategoryName(eCategory));
					}
...
#pragma warning(suppress:6001) // /analyze "Using uninitialized memory '*pBlock'"
						for (i=0; i < pBlock->iNumMessages; i++)
						{
							...
							logDirectWriteWithTime(fileName,pBlock->pMessages[i], 0, pCategory->ppLists[0]->iTime);
...
void InitLogServerStatus(void)
{
	...
	estrPrintf(&gGlobalInfo.pGenericInfo, "<input type=\"hidden\" id=\"defaultautorefresh\" value=\"1\"><a href=\"/viewxpath?xpath=%s[%u].generic\">Generic ServerLib info for the %s</a>",
		GlobalTypeToName_ForLogServer(GetAppGlobalType()), gServerLibState.containerID,GlobalTypeToName_ForLogServer(GetAppGlobalType()));
...
int wmain(int argc, WCHAR** argv_wide)
{
	...
	logSetDir(GlobalTypeToName_ForLogServer(GetAppGlobalType()));
...
//	sMessagesTable = stashTableCreate(128, StashDefault, StashKeyTypeStrings, 0 );
	...
	{
		BeginPeriodicFreeDiskSpaceCheck(pLogDir[0], 60, 600, 3, 0, NULL, NULL);
	}
