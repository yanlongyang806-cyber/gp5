...
bool gbNoAudioForClientBinnerClients = false;
AUTO_CMD_INT(gbNoAudioForClientBinnerClients, NoAudioForClientBinnerClients) ACMD_AUTO_SETTING(Ugc, CLIENTBINNER);
		...
		if (!bDone)
		{
			...
			case CC_NOT_RUNNING:
				if (!sbAlreadyStartedOne)
				{
					...
					estrPrintf(&pExtraCommandLine, "-SetTestingMode -ProductionEdit -notimeout -windowed -loadUserNameSpaces %s -makeBinsAndExitForNamespace %s -ReportMakebinsToTestClient",
						nameSpaceForClient, nameSpaceForClient);

					if( gbNoAudioForClientBinnerClients ) {
						estrConcatf( &pExtraCommandLine, " -noaudio" );
					}
