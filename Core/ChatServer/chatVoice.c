...
#define VIVOX_API_DEF		"http://" ORGANIZATION_DOMAIN "/api2"
#define VIVOX_ADMIN_USERID	"admin"
#define VIVOX_ADMIN_PWD		"password"
#define TUNNEL_HOST_DEF		"QAGlobalChat"

// WARNING: If you change these values, you must also update the following:
//   stunnel.conf on TUNNEL_HOST_DEF
//   The auto setting comment below on VoiceTunnelPort, which shows up in the server monitor.
// Note: These are here, rather than being data-defined or in gConf, because they MUST remain in sync with the auto setting comment below.
#define TUNNEL_PORT_DEF		4432
#define TUNNEL_PORT_DEF_FC	4434
#define TUNNEL_PORT_DEF_ST	4433
#define TUNNEL_PORT_DEF_NW	4435
...
int g_VoiceLogNonErrors = 1;

int g_VoiceEnabledRegion = 1;
...
// WARNING: If you change the below comment, you must also change the values for TUNNEL_PORT_DEF.

// Port to use for voice chat secure tunnel proxy (defaults: FightClub 4434, StarTrek 4433, Night 4435, otherwise 4432)
...
AUTO_CMD_INT(g_VoiceLogNonErrors, VoiceLogNonErrors) ACMD_AUTO_SETTING(Chat, CHATSERVER);

// Set this to 1 to allow the chat server & game clients to use Vivox's voice chat service, otherwise set it to 0
AUTO_CMD_INT(g_VoiceEnabledRegion, VoiceEnabledRegion) ACMD_AUTO_SETTING(Chat, CHATSERVER);
...
{
	return	(	g_VoiceEnabledRegion		||
				g_VoiceChatState.signed_in	||
				g_VoiceChatState.signing_in	)
			&&
			(	g_VoiceOverride  ||
				(	gShardChatServerConfig.bVoiceEnabled &&
...
void cvAcctDisableAdsCB(ResponseChain *rc, const char* xmldoc, int len)
{
	...
	char *status = NULL, *code = NULL, *msg = NULL;

	if(!st) {
		st = stashTableCreateWithStringKeys(10, StashDeepCopyKeys_NeverRelease);
	}
	...
	stashAddPointer(st, "msg", NULL, true);

	cvExtractKeysValues(xmldoc, len, &state);
	stashFindPointer(st, "status", &status);
	stashFindPointer(st, "code", &code);
	stashFindPointer(st, "msg", &msg);

	if (status && !strcmpi(status, "ERR")) {
		servLog(LOG_VOICE_CHAT, "Acct_Disable_Ads", "%s %s %s", status, code, msg);
	} else if (g_VoiceLogNonErrors) {
		servLog(LOG_VOICE_CHAT, "Acct_Disable_Ads", "Success");
	}
...
void cvAcctRecreateCB(VivoxAccountMapping *map, const char* xmldoc, int len)
{
	...
	if(!user)
	{
		servLog(LOG_VOICE_CHAT, "Acct_Recreate", "Failed due to no user");
		StructDestroy(parse_VivoxAccountMapping, map);
		return;
	}
	...
	{
		servLog(LOG_VOICE_CHAT, "Acct_Recreate", "Failed with %s %s %s on user %s", status, code, msg, map->username);
	...
	if (g_VoiceLogNonErrors) {
		servLog(LOG_VOICE_CHAT, "Acct_Recreate", "Success, attempting to join channels");
	}
...
void cvAcctResetGetIDCB(VivoxAccountMapping *map, const char* xmldoc, int len)
{
	static StashTable st = NULL;
	XMLExtractState state;
	char *acct = NULL, *status = NULL, *code = NULL, *msg = NULL;
	...
	if(!acct)
	{
		servLog(LOG_VOICE_CHAT, "Acct_Reset_GetID", "Failed with %s %s %s due to no accountid for user %s", status, code, msg, map->username);
	...
	if (status && !strcmpi(status, "ERR")) {
		servLog(LOG_VOICE_CHAT, "Acct_Reset_GetID", "%s %s %s for user %s, attempting to reset password", status, code, msg, map->username);
		// should we return here ?
	} else if (g_VoiceLogNonErrors) {
		servLog(LOG_VOICE_CHAT, "Acct_Reset_GetID", "Success, attempting to reset password");
	}
...
void cvAcctCreateCB(VivoxAccountMapping *map, const char* xmldoc, int len)
{
	static StashTable st = NULL;
	XMLExtractState state;
	char *auth = NULL, *acct = NULL, *dbcode = NULL, *status = NULL, *code = NULL, *msg = NULL;
	ChatUser *user = objGetContainerData(GLOBALTYPE_CHATUSER, map->crypticID);
	bool showError = false;

	if(!user)
	{
		servLog(LOG_VOICE_CHAT, "Acct_Create", "Failed due to no user with name %s", map->username);
		StructDestroy(parse_VivoxAccountMapping, map);
		return;
	}
				...
				if(dbcode) 
				{
					...
					switch(d)
					{
						xcase 1203: {	// Account already exists, have to get id then change pwd
							if (g_VoiceLogNonErrors) {
								servLog(LOG_VOICE_CHAT, "Acct_Create", "Account already exist, attempting reset instead");
							}
							cvAccountReset(map);
							return;
						}
	...
	if(!auth) {
		servLog(LOG_VOICE_CHAT, "Acct_Create", "Failed with %s %s %s due to no auth_token for user %s", status, code, msg, map->username);
		return;
	} else if (!acct) {
		servLog(LOG_VOICE_CHAT, "Acct_Create", "Failed with %s %s %s due to no accountid for user %s", status, code, msg, map->username);
		return;
	}
	...
	if (showError) {
		servLog(LOG_VOICE_CHAT, "Acct_Create", "%s %s %s on user %s, attempting to join channels", status, code, msg, map->username);
		// should we return here ?
	} else if (g_VoiceLogNonErrors) {
		servLog(LOG_VOICE_CHAT, "Acct_Create", "Success, attempting to join channels");
	}
...
void cvAcctVerifyCB(VivoxAccountMapping *map, const char* xmldoc, int len)
{
	static StashTable st = NULL;
	XMLExtractState state;
	char *auth = NULL, *acct = NULL, *status = NULL, *code = NULL, *msg = NULL;
	ChatUser *user = objGetContainerData(GLOBALTYPE_CHATUSER, map->crypticID);

	if(!user)
	{
		servLog(LOG_VOICE_CHAT, "Acct_Verify", "Failed due to no user with name %s", map->username);
		StructDestroy(parse_VivoxAccountMapping, map);
		return;
	}
	...
	{
		if (g_VoiceLogNonErrors) {
			servLog(LOG_VOICE_CHAT, "Acct_Verify", "No accountid for user %s, attempting to create new one", map->username);
		}
	...
	if (status && !strcmpi(status, "ERR")) {
		servLog(LOG_VOICE_CHAT, "Acct_Verify", "%s %s %s on user %s, attempting to reset password", status, code, msg, map->username);
		// should we return here ?
	} else if (g_VoiceLogNonErrors) {
		servLog(LOG_VOICE_CHAT, "Acct_Verify", "Success, attempting to reset password");
	}
...
void cvChanFindCB(ChannelRef *cref, const char* xmldoc, int len)
{
	...
	XMLExtractState state;
	ChatChannel *chan;
	char *chanid = NULL, *chanuri = NULL, *code = NULL, *dbcode = NULL, *status = NULL, *msg = NULL;
	...
	if(!chan) {
		if (g_VoiceLogNonErrors) {
			servLog(LOG_VOICE_CHAT, "Channel_Find", "Failed to find chan by id, attempting to find by name instead");
		}
	...
	if(!chanid) {
		servLog(LOG_VOICE_CHAT, "Channel_Find", "Failed with %s %s %s due to no chanid", status, code, msg);
		// should we destroy cref here ?
		return;
	} else if (!chanuri) {
		servLog(LOG_VOICE_CHAT, "Channel_Find", "Failed with %s %s %s due to no chanuri", status, code, msg);
		// should we destroy cref here ?
		return;
	}
	...
	if(chan)
	{
		...
		if (status && !strcmpi(status,"ERR")) {
			servLog(LOG_VOICE_CHAT, "Channel_Find", "%s %s %s, attempting to add %d users", status, code, msg, ea32Size(&chan->online));
			// should we return here ?
		} else if (g_VoiceLogNonErrors) {
			servLog(LOG_VOICE_CHAT, "Channel_Find", "Success, attempting to add %d users", ea32Size(&chan->online));
		}
...
void cvChanCreateCB(ChannelRef *cref, const char* xmldoc, int len)
{
	...
	if(!chan) {
		if (g_VoiceLogNonErrors) {
			servLog(LOG_VOICE_CHAT, "Channel_Create", "Failed to find chan by id, attempting to find by name instead");
		}
		chan = channelFindByName(cref->name);
	}

	if(!chan) {
		servLog(LOG_VOICE_CHAT, "Channel_Create", "Failed due to no chan");
		StructDestroy(parse_ChannelRef, cref);
		return;
	}
				...
				if(dbcode) 
				{
					...
					switch(d)
					{
						xcase 1500: {	// Channel already exists
							if (g_VoiceLogNonErrors) {
								servLog(LOG_VOICE_CHAT, "Channel_Create", "Already exist, attempting find instead");
							}
	...
	if(!chanid) {
		servLog(LOG_VOICE_CHAT, "Channel_Create", "Failed with %s %s %s due to no chanid", status, code, msg);
		return;
	} else if (!chanuri) {
		servLog(LOG_VOICE_CHAT, "Channel_Create", "Failed with %s %s %s due to no chanuri", status, code, msg);
		return;
	}
	...
	if(chan)
	{
		...
		if (showError) {
			servLog(LOG_VOICE_CHAT, "Channel_Create", "%s %s %s, attempting to add %d users", status, code, msg, ea32Size(&chan->online));
			// should we return here ?
		} else if (g_VoiceLogNonErrors) {
			servLog(LOG_VOICE_CHAT, "Channel_Create", "Success, attempting to add %d users", ea32Size(&chan->online));
		}
...
void cvChannelJoinCB(UserChannel *data, const char* xmldoc, int len)
{
	...
	if(!chan) {
		if (g_VoiceLogNonErrors) {
			servLog(LOG_VOICE_CHAT, "Channel_Join", "Failed to find chan by id, attempting to find by name instead");
		}
	...
	if(!chan) {
		servLog(LOG_VOICE_CHAT, "Channel_Join", "Failed with %s %s %s due to no chan", status, code, msg);
		return;
	} else if (!user) {
		servLog(LOG_VOICE_CHAT, "Channel_Join", "Failed with %s %s %s due to no user", status, code, msg);
		return;
	}
	...
	{
		if (status && !strcmpi(status, "ERR")) {
			servLog(LOG_VOICE_CHAT, "Channel_Join", "%s %s %s, now sending user voice enabled", status, code, msg);
			// should we return here ?
		} else if (g_VoiceLogNonErrors) {
			servLog(LOG_VOICE_CHAT, "Channel_Join", "Success, now sending user voice enabled");
		}
...
void cvChannelLeaveCB(UserChannel *data, const char* xmldoc, int len)
{
	static StashTable st = NULL;
	XMLExtractState state;
	char *status = NULL, *code = NULL, *msg = NULL;
	...
	if (status && !strcmpi(status, "ERR")) {
		servLog(LOG_VOICE_CHAT, "Channel_Leave", "%s %s %s", status, code, msg);
		// should we return here ?
	} else if (g_VoiceLogNonErrors) {
		servLog(LOG_VOICE_CHAT, "Channel_Leave", "Success");
	}
}
...
void chatVoiceLeave(ChatChannel *channel, ChatUser *user)
{
	...
	if(!channel->voiceId) {
		// Still in process of being created, no need to leave then
		return;
	}
	...
	devassert(objGetContainerData(GLOBALTYPE_CHATUSER, user->id));
	chatVoiceProcessPage(&args, cvChannelLeaveCB, NULL, true);
	StructDeInit(parse_UrlArgumentList, &args);
}
...
void cvHttpTimeout(HttpFormRequest *req)
{
	...
	{
		servLog(LOG_VOICE_CHAT, "Request_Timeout", "Timeout during attempt %d", req->attempt_count);

		// we don't want to duplicate this request when we attempt to process it again
		eaFindAndRemoveFast(&g_VoiceChatState.active_reqs, req); 
	...
	else
	{
		...
		servLogWithStruct(LOG_VOICE_CHAT, "Request_Timeout", req, parse_HttpFormRequest);
...
void chatVoiceProcessRequest(HttpFormRequest *req)
{
	...
	if(isHTTPS)
	{
		...
		if (!tunnelPort)
		{
			// See comment on TUNNEL_PORT_DEF above.
			if (!stricmp(GetShortProductName(), "FC"))
				tunnelPort = TUNNEL_PORT_DEF_FC;
			else if (!stricmp(GetShortProductName(), "ST"))
				tunnelPort = TUNNEL_PORT_DEF_ST;
			else if (!stricmp(GetShortProductName(), "NW") || !stricmp(GetShortProductName(), "NNO"))
				tunnelPort = TUNNEL_PORT_DEF_NW;
			else
				tunnelPort = TUNNEL_PORT_DEF;
		}
...
void chatVoiceLoginCB(void* unused, const char* xmldoc, int len)
{
	static StashTable st = NULL;
	XMLExtractState state;
	char* status = NULL, *auth = NULL, *acct = NULL, *disp = NULL, *code = NULL, *msg = NULL, *title = NULL;
	...
	cvExtractInit(&state, st);
	stashAddPointer(st, "title", NULL, true);
	stashAddPointer(st, "auth_token", NULL, true);
	...
	cvExtractKeysValues(xmldoc, len, &state);
	stashFindPointer(st, "title", &title);
	stashFindPointer(st, "auth_token", &auth);
	...
	if (title && strstri(title, "404"))
	{
		g_VoiceChatState.signed_in = 0;

		printfColor(COLOR_RED, "Failed to sign into Vivox - %s - Check Chat Server Auto Settings for STunnel Setup\n", title);

		servLog(LOG_VOICE_CHAT,
			"Voice_Login", 
			"Login failed - %s - Check Chat Server Auto Settings for STunnel Setup - chat_db: %d users, %d channels",
			title,
			stashGetCount(chat_db.user_names), 
			stashGetCount(chat_db.channel_names));

		CRITICAL_NETOPS_ALERT("VOICE_LOGIN_FAILURE", "Failed to sign into Vivox - %s - Check Chat Server Auto Settings for STunnel Setup", title);
	}
	else if(stricmp(status, "Ok")==0)
	{
		...
		servLog(LOG_VOICE_CHAT,
				"Voice_Login", 
				"Login successful - about to begin processing %d users, %d channels", 
				stashGetCount(chat_db.user_names), 
				stashGetCount(chat_db.channel_names));

		FOR_EACH_IN_STASHTABLE(chat_db.user_names, ChatUser, user)
		{
			// is this really a good idea ? how do we know they're still around after a crash ?
		...
		{
			// is this really a good idea ? how do we know they're still around after a crash ?
			if (chan->voiceEnabled) { // please check that it's a voice chat channel & not a zone or global text chat channel
	...
	else
	{
		...
		printfColor(COLOR_RED, "Failed to sign into Vivox - status %s\n", status);

		servLog(LOG_VOICE_CHAT,
				"Voice_Login", 
				"Login failed - %s %s %s - chat_db: %d users, %d channels",
				status, code, msg,
				stashGetCount(chat_db.user_names), 
				stashGetCount(chat_db.channel_names));

		CRITICAL_NETOPS_ALERT("VOICE_LOGIN_FAILURE", "Failed to sign into Vivox - %s %s %s", status, code, msg);
	}
...
U32 cvLoginTimeout(void* userdata)
{
	...
	servLog(LOG_VOICE_CHAT, "Voice_Login", "Login failed - Timeout");

	WARNING_NETOPS_ALERT("VOICE_LOGIN_TIMEOUT", "Failed to sign into Vivox - timeout");
...
void cvSignOut(void)
{
	if (!g_VoiceEnabledRegion)
	{
		servLog(LOG_VOICE_CHAT,
				"Voice_Signout",
				"Voice system turned off to do region-based auto-setting");
	}
	else
	{
		servLog(LOG_VOICE_CHAT,
				"Voice_Signout",
				"Entering failure mode - errors %d, active conns %d, request queue %d",
				g_VoiceChatState.failuresSinceLastSuccess,
				eaSize(&g_VoiceChatState.active_reqs),
				eaSize(&g_VoiceChatState.http_requests));

		CRITICAL_NETOPS_ALERT(	"VOICE_CHAT_DOWN", 
								"Voice chat entering failure mode - errors %d, active conns %d, request queue %d", 
								g_VoiceChatState.failuresSinceLastSuccess,
								eaSize(&g_VoiceChatState.active_reqs),
								eaSize(&g_VoiceChatState.http_requests));
	}
...
{
	static int inited = 0;
	static int voiceEnabledRegionOld;

	// this lets live ops turn voice on/off at runtime & propogates that information down to the clients
	// this may require extra code to get the channels to be setup correctly if the value changes at runtime
	if (voiceEnabledRegionOld != g_VoiceEnabledRegion) {
		crManager_UpdateVoiceRegionEnabled(g_VoiceEnabledRegion);
		voiceEnabledRegionOld = g_VoiceEnabledRegion;
	}
		...
		g_VoiceChatState.signed_in)
	{
		if(eaSize(&g_VoiceChatState.active_reqs)<g_VoiceMaxActiveRequests)
		{
			...
			for(i=0; i<numOpen && i<eaSize(&g_VoiceChatState.http_requests); i++)
			{
				...
				//printfColor(COLOR_RED, "%s : %s : ", req->url, req->args->pBaseURL);
				//FOR_EACH_IN_EARRAY(req->args->ppUrlArgList, UrlArgument, pArg) {
				//	printfColor(COLOR_RED, "%s %s : ", pArg->arg, pArg->value);
				//} FOR_EACH_END;
				//printf("\n");
