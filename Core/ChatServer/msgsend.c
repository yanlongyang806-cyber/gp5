...
bool sendChatSystemTranslatedNotify(ChatUser *user, NotifyType eType, const char *msgKey, ...)
{
	Language eLanguage;
	char *estrBuffer = NULL;
	va_list	ap;
	if (!user->uChatRelayID || !user->pPlayerInfo)
		return false;
	eLanguage = user->pPlayerInfo->eLanguage;
	va_start(ap, msgKey);
	langFormatMessageKeyV(eLanguage, &estrBuffer, msgKey, ap);
	va_end(ap);
	RemoteCommand_crReceiveNotify(GLOBALTYPE_CHATRELAY, user->uChatRelayID, user->id, eType, estrBuffer);
	estrDestroy(&estrBuffer);
	return true;
}
...
void ChatServerForwardEmailWrappers(int accountID, ChatUser *user, EmailWrappers *pEmailWrappers)
{
	if (userOnline(user))
	{
		if (userCharacterOnline(user) && user->pPlayerInfo->onlineCharacterID)
			RemoteCommand_gslMail_ReceiveMailWrapperForTransfer(GLOBALTYPE_ENTITYPLAYER, user->pPlayerInfo->onlineCharacterID, 
				accountID, user->pPlayerInfo->onlineCharacterID, pEmailWrappers);
	}
}