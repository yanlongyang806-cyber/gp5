...
extern int g_VoiceEnabledRegion;
	...
	if (gbGlobalChatResponse)
	{
		...
		if (user)
		{
			...
			if (userOnline(user) && user->uChatRelayID)
			{
				...
				RemoteCommand_crLoginSucceeded(GLOBALTYPE_CHATRELAY, user->uChatRelayID, user->id, g_VoiceEnabledRegion);
...
AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void ChatServerLeaveTradeChannel(char * zone_channel_name, ChatUserBatch *userList)
{
	int i;
	ChatChannel *channel;
	if (!strStartsWith(zone_channel_name, TRADE_CHANNEL_NAME))
		return;
	channel = channelFindByName(zone_channel_name);
	if (!channel)
		return;
...
AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void ChatServerCreateTradeChannel(char * zone_channel_name)
{
	if (!strStartsWith(zone_channel_name, TRADE_CHANNEL_NAME))
		return;
	if (!stashFindPointer(chat_db.channel_names, zone_channel_name, NULL))
	{
		NOCONST(ChatChannel) *channel = StructAllocNoConst(parse_ChatChannel);
		channelInitialize(channel, zone_channel_name, "", CHANNEL_SPECIAL_TRADE);
		stashAddPointer(chat_db.channel_names, channel->name, channel, false);
	}
}

AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void ChatServerDestroyTradeChannel(char * zone_channel_name)
{
	ChatChannel *channel = NULL;
	if (!strStartsWith(zone_channel_name, TRADE_CHANNEL_NAME))
		return;
...
AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void ChatServerLeaveLFGChannel(char * zone_channel_name, ChatUserBatch *userList)
{
	int i;
	ChatChannel *channel;
	if (!strStartsWith(zone_channel_name, LFG_CHANNEL_NAME))
		return;
	channel = channelFindByName(zone_channel_name);
	if (!channel)
		return;
...
AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void ChatServerCreateLFGChannel(char * zone_channel_name)
{
	if (!strStartsWith(zone_channel_name, LFG_CHANNEL_NAME))
		return;
	if (!stashFindPointer(chat_db.channel_names, zone_channel_name, NULL))
	{
		NOCONST(ChatChannel) *channel = StructAllocNoConst(parse_ChatChannel);
		channelInitialize(channel, zone_channel_name, "", CHANNEL_SPECIAL_LFG);
		stashAddPointer(chat_db.channel_names, channel->name, channel, false);
	}
}

AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void ChatServerDestroyLFGChannel(char * zone_channel_name)
{
	ChatChannel *channel = NULL;
	if (!strStartsWith(zone_channel_name, LFG_CHANNEL_NAME))
		return;
...
void ChatServerPrivateMesssage_Shard(ChatMessage *pMsg)
{
	...
	if (userIsPrivateMessageRestricted(user))
	{
		...
		if (!bCanTalkToUser)
		{
			sendChatSystemTranslatedNotify(user, kNotifyType_Failed, "Chat_NoTrialTells", STRFMT_END);
			return;
		}
	}
...
static bool channelMessage_CleanupName(SA_PARAM_NN_VALID ChatUser *user, SA_PARAM_NN_VALID ChatMessage *pMsg)
{
	...
	xcase kChatLogEntryType_Team:
		if (!pMsg->pchChannel || !*pMsg->pchChannel || stricmp(pMsg->pchChannel, CHAT_TEAM_SHORTCUT) == 0)
		{
			...
			{
#ifdef USE_CHATRELAY
				sendChatSystemTranslatedNotify(user, kNotifyType_Failed, "Chat_TeamError", STRFMT_END);
#endif
				return false;
			}
		}
...
AUTO_COMMAND_REMOTE;
int ChatServerForwardEmailWrappersFromGlobal(CmdContext *context, ContainerID accountID, EmailWrappers *pEmailWrappers)
{
	ChatUser *user = userFindByContainerId(accountID);
	if (!userOnline(user) || !userCharacterOnline(user))
	{
		char *commandString = NULL;
		char *wrapperString = NULL;

		ParserWriteTextEscaped(&wrapperString, parse_EmailWrappers, pEmailWrappers, 0, 0, 0);

		estrStackCreate(&commandString);
		estrPrintf(&commandString, "ChatServerForwardEmailWrappersFromGlobal %d %s", accountID, wrapperString);
		localChatAddCommmandWaitQueue(accountID, 0, 0, 0, commandString);
		estrDestroy(&commandString);
		estrDestroy(&wrapperString);
	}
	else
	{
		ChatServerForwardEmailWrappers(accountID, user, pEmailWrappers);
	}
	return CHATRETURN_NONE;
}