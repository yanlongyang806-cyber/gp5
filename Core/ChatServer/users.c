...
bool userIsPrivateMessageRestricted(ChatUser *user)
{
	if (user)
	{
		if (UserIsAdmin(user))
			return false;
		if (user->pPlayerInfo)
			return (user->pPlayerInfo->eGamePermissionInfo & CHAT_GAME_PERMISSION_INFO_TELL) == 0;
	}
	return true;
}
...
void userLocalLogout(SA_PARAM_NN_VALID ChatUser *user)
{
	...
	if (user->uGatewaySessionRefCount == 0)
		userDelete(user->id);
	else
		user->uChatRelayID = 0;
}
...
void userLocalPlayerUpdate(SA_PARAM_NN_VALID ChatUser *user, SA_PARAM_NN_VALID const PlayerInfoStruct *pPlayerInfo, ChatUserUpdateEnum eForwardToGlobalFriends)
{
	...
	else
	{
		...
		executeUserCommandWait(user->id, user->handle);
	}
...
// Gateway Chat Feature Commands
AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER) ACMD_IFDEF(CHATSERVER);
void gatewayChat_LoginUser(U32 uAccountID)
{
	...
	if (!user)
	{
		...
		estrPrintf(&commandString, "gatewayChat_LoginUser %d", uAccountID);
		localChatAddCommmandWaitQueue(uAccountID, 0, 0, 0, commandString);
		estrDestroy(&commandString);
	}
	else
	{
		user->uGatewaySessionRefCount++;
	}
	// Always send fetch request to update user
	eaiPush(&eaiAccountList, uAccountID);
	getAccountListFromGlobalChatServer(eaiAccountList);
	eaiDestroy(&eaiAccountList);
}

AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void gatewayChat_LogoutUser(U32 uAccountID)
{
	ChatUser *user = userFindByContainerId(uAccountID);
	if (user)
	{
		if (user->uGatewaySessionRefCount > 0)
			user->uGatewaySessionRefCount--;
		if (user->uGatewaySessionRefCount == 0 && user->uChatRelayID == 0)
			userDelete(user->id);
	}
}