...
{
	sendCmdAndParamsToGlobalChat("chatCommandRemote_DeleteAllShardMail", "%d \"%s\"", accountID, GetShardNameFromShardInfoString());
}

AUTO_COMMAND_REMOTE ACMD_IFDEF(GAMESERVER);
void chatCommandRemote_TransferAndDeleteShardMail(int accountID)
{
	sendCmdAndParamsToGlobalChat("chatCommandRemote_TransferAndDeleteShardMail", "%d \"%s\"", accountID, GetShardNameFromShardInfoString());
}

//For emailv3
AUTO_COMMAND_REMOTE;
int ChatServer_OutgoingMailCanBeSent(const char* pchAccount, const char* pchBody, const char* pchSubject)
{
	ChatUser *user = userFindByHandle(pchAccount);
	if (!user)
	{
		Errorf("Could not find user with account name '%s'", pchAccount);
		return 0;
	}
	if (UserIsAdmin(user))
		return 1;
