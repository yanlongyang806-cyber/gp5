...
typedef struct ChatRelayManagerUser
{
	...
	U32 uRelayID;
	U32 uAccessLevel;
	ChatGamePermissionInfoEnum eGamePermissionFlags;
...
static ChatRelayManagerUser *crManager_AssignUser(U32 uAccountID, U32 uAccessLevel, ChatGamePermissionInfoEnum eGamePermissionFlags, ChatRelayServer *server)
{
	...
	relayUser->eGamePermissionFlags = eGamePermissionFlags;
...
static ChatRelayConfig *spConfig = NULL;
static ChatRelayConfig * crManager_GetChatRelayConfig(void)
{
	if (!spConfig)
	{
		spConfig = StructCreate(parse_ChatRelayConfig);
		spConfig->uZoneRateLimit = gShardChatServerConfig.uZoneRateLimit;
		spConfig->uZoneRateLimitPeriod = gShardChatServerConfig.uZoneRateLimitPeriod;
		spConfig->uZoneSilenceTime = gShardChatServerConfig.uZoneSilenceTime;
	}
	return spConfig;
}

void crManager_ResetChatRelayConfig(void)
{
	if (spConfig)
	{
		StructDestroy(parse_ChatRelayConfig, spConfig);
		spConfig = NULL;
	}
	EARRAY_FOREACH_BEGIN(seaChatRelays, i);
	{
		RemoteCommand_chatRelay_UpdateConfig(GLOBALTYPE_CHATRELAY, seaChatRelays[i]->uID, crManager_GetChatRelayConfig());
	}
	EARRAY_FOREACH_END;
}
...
int crManager_RegisterRelay(U32 uRelayID, const char *ipStr, U32 uPort, PlayerInfoList *pAccounts)
{
	...
	RemoteCommand_chatRelay_UpdateConfig(GLOBALTYPE_CHATRELAY, uRelayID, crManager_GetChatRelayConfig());
...
void crManager_UserLogout(U32 uAccountID)
{
	ChatRelayManagerUser *relayUser = crManager_GetRelayUser(uAccountID);
	ChatUser *user = userFindByContainerId(uAccountID);
	...
	{
		ChatRelayServer *relayServer = crManager_GetRelayServer(user->uChatRelayID);
		RemoteCommand_chatRelayUserLogout(GLOBALTYPE_CHATRELAY, user->uChatRelayID, uAccountID);
		user->uChatRelayID = 0;
		ChatServerLogoutEx(user->id, user->pPlayerInfo ? user->pPlayerInfo->onlineCharacterID : 0);
		if (relayServer)
			eaiFindAndRemove(&relayServer->eaiUsers, uAccountID);
...
void crManager_GetUserSecretHash(GlobalType eRequestServerType, U32 uRequestServerID, ChatAuthRequestData *pRequestData)
{
	...
	sprintf(chatpString, "%d", pRequestData->eGamePermissionInfo);
	...
	servLogWithPairs(LOG_CHATPERMISSIONS, "ChatRelayAuthStart",
		...
		"charID", charIDString,
		"accessLevel", accessLevelString,
		"chatpermissions", chatpString,
		NULL);

	if (relayServer)
	{
		relayUser = crManager_AssignUser(pRequestData->uAccountID, pRequestData->uAccountAccessLevel, pRequestData->eGamePermissionInfo, relayServer);
		if (relayUser)
		{
...
void crManager_UpdateVoiceRegionEnabled(U32 uVoiceRegionEnabled)
{
	EARRAY_FOREACH_BEGIN(seaChatRelays, i);
	{
		RemoteCommand_chatRelay_UpdateVoiceRegionEnabled(GLOBALTYPE_CHATRELAY, seaChatRelays[i]->uID, uVoiceRegionEnabled);
	}
	EARRAY_FOREACH_END;
}
