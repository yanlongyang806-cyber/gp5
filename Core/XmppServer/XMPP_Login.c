...
void XMPP_ValidateLogin(XmppClientState *state, char *login, char *password)
{
	...
	xmpp_unescape_jid(&loginUnescape, login);
	accountValidatorRequestTicket(state->validator, loginUnescape, password);
	eaPush(&seaValidatorQueue, state);
	estrDestroy(&loginUnescape);
}
