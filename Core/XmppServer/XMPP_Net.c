...
static void xmpp_TlsSessionClosed(TlsSession *pSession, void *pUserData)
{
	...
	if (link->link)
		xmpp_Disconnect(link, "The TLS session has been completed.");
...
static int xmpp_ServingConnectTls(NetLink *link, XmppClientLink *client)
{
	...
	if (!success)
		xmpp_Disconnect(client, "Failed TLS");
	return success;
}
...
static U32 suLastCertificateCheck = 0;
...
void XMPP_NetTick(F32 elapsed)
{
	...
	if (xmppCertificate)
	{
		...
		if (uTime - suLastCertificateCheck > SECONDS_PER_DAY)
		{
			CheckCertificateValidity(xmppCertificate);
			suLastCertificateCheck = uTime;
		}
	}
	PERFINFO_AUTO_STOP_FUNC();
}
...
bool XMPP_NetBegin()
{
	...
	if (*xmppCertificateName)
	{
		...
		suLastCertificateCheck = timeSecondsSince2000();
	}
...
void XMPP_NetStartTls(XmppClientLink *client)
{
	...
	if (!success)
		xmpp_Disconnect(client, "Failed TLS"); // This occurs inside the msg handler and must be a flush and close
}
