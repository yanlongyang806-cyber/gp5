...
static void XMPP_ChannelProcessDiscoInfo(XmppClientState *state, const char *to, const char *from, const char *id)
{
	...
	{
		char *room = NULL;
		char *roomEscaped = NULL;
		estrStackCreate(&room);
		estrStackCreate(&roomEscaped);
		MakeNodeFromJid(&room, to);
		estrAppendEscaped(&roomEscaped, room);
		XMPP_SendGlobalChatQuery( GetXmppGlobalChatLink(), "XMPP_ChannelProcessChannelDiscoInfo", 
			"%d %d \"%s\" %s", 
			state->uStateID, domain, roomEscaped, id);
		estrDestroy(&room);
		estrDestroy(&roomEscaped);
	}
...
void XMPP_EndStream(XmppClientState *state, const char *reason)
{
	if (!state->stream_closed)
	{
		...
		// for preventing truncation attacks through TLS's resume feature which is not supported
...
AUTO_COMMAND_REMOTE;
void XMPP_NotifyFriendRequest(XMPP_NotifyFriendResponse *response)
{
	char *fromBareJid = NULL;
	int i, size;
	
	if (!response->fromHandle || !*response->fromHandle || !response->stateList)
		return;	
	size = eaiSize(&response->stateList->eaiStateIDs);
	if (size == 0)
		return;

	estrStackCreate(&fromBareJid);
	estrPrintf(&fromBareJid, "%s@%s", response->fromHandle, XMPP_Domain());
	for (i=0; i<size; i++)
	{
		XmppClientState *state = XMPP_FindClientById(response->stateList->eaiStateIDs[i]);
		if (XMPP_IsLoggedIn(state))
...
AUTO_COMMAND_REMOTE;
void XMPP_NotifyNewFriend(XMPP_NotifyFriendResponse *response)
{
	char *fromBareJid = NULL;
	int i, size;
	...
	for (i=0; i<size; i++)
	{
		...
		if (XMPP_IsLoggedIn(state))
		{
			...
			// Send roster push.
			XMPP_RosterPush(state, fromBareJid, response->fromHandle, true, NULL, false);
		}
	}
	estrDestroy(&fromBareJid);
}
...
AUTO_COMMAND_REMOTE;
void XMPP_NotifyFriendRemove(XMPP_NotifyFriendResponse *response)
{
	char *fromBareJid = NULL;
	int i, size;
...
AUTO_COMMAND_REMOTE;
void XMPP_NotifyChatJoin (U32 uStateID, XMPP_ChannelInfoData *room, XMPP_ChatOccupant *occupant, XMPP_ClientStateList *roster)
{
	XmppClientState *state = XMPP_FindClientById(uStateID);
	int i, size;
	...
	for (i=0; i<size; i++)
	{
		...
		if (XMPP_IsLoggedIn(memberState))
		{
			...
			xmpp_s_sendroompresence(memberState->generator, XMPP_RoomDomainName(room->eDomain), room->room, 
