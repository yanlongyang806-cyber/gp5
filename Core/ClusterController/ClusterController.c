...
void OVERRIDE_LATELINK_GetCustomServerInfoStructForHttp(UrlArgumentList *pUrl, ParseTable **ppTPI, void **ppStruct)
{
	...
	FOR_EACH_IN_STASHTABLE(gSimpleMonitoringStatusByName, SimpleMonitoringStatus, pSimpleStatus)
	{
		...
		pStatus->iConnected = (pSimpleStatus->eConnectionState == SIMPLEMONITORING_CONNECTED);
		...
		else if (pSimpleStatus->pAssumedMachineName)
		{
			pAddressString = pSimpleStatus->pAssumedMachineName;
		}
...
int wmain(int argc, WCHAR** argv_wide)
{
	...
	SimpleStatusMonitoring_Begin(0, 0);
...
//for the clustercontroller doing simple status reporting to the overlord, include patch statuses
int OVERRIDE_LATELINK_StatusReporting_GetBehaviorFlagsForExtraStatusReporting(char *pName, int iPort)
{
	if (iPort == OVERLORD_SIMPLE_STATUS_PORT)
	{
		return EXTRAREPORTING_PATCHING_STATUSES;
	}
