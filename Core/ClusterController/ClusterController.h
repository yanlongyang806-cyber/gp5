...
typedef struct Shard
{
	...
	AST_COMMAND("Run this shard in emergency restart mode", "RunShard_EmergencyRestartMode $FIELD(ShardName) $STRING(Type yes to run this shard in emergency restart mode)")
	AST_COMMAND("Run this shard in NON-EMERGENCY startup mode", "RunShard $FIELD(ShardName) $STRING(Type yes to run this shard NOT in emergency restart mode)")
} Shard;
