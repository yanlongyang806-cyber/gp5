...
AUTO_COMMAND;
char *RunShard_EmergencyRestartMode(char *pShardName, char *pConfirm)
{
	Shard *pShard;
	...
	SentryServerComm_RunCommand_1Machine(pShard->pMachineName, STACK_SPRINTF("%s -AppendCommandLine ObjectDB QdashEmergencyRestart", pShard->pRestartBatchFile));
