...
//				printf("about to send it\n");
		...
		else
		{
			...
			xcase MULTIPLEX_COMMAND_I_WANT_TO_REGISTER_WITH_SERVER:
				{
			...
			xcase MULTIPLEX_COMMAND_SEND_PACKET_TO_MULTIPLE_RECIPIENTS:
				{
					int iSenderIndex;
					int iNumDestIndexes;
					int iCmd;
					...
					iSenderIndex = pktGetBits(pak, 32);
					iNumDestIndexes = pktGetBits(pak, 32);

					ea32StackCreate(&piDestIndexes, iNumDestIndexes);

					for (i = 0; i < iNumDestIndexes; i++)
					{
						ea32Push(&piDestIndexes, pktGetBits(pak, 32));
					}
					...
					iStartingPktIndex = pktGetIndex(pak);

					for (i = 0; i < iNumDestIndexes; i++)
					{
						pDestConnection = FindConnectionFromIndex(spMultiplexer, piDestIndexes[i]);
						...
						if (pDestConnection && pDestConnection->pLink)
						{
							...
							pktCreateWithCachedTracker(pNewPacket, pDestConnection->pLink, SHAREDCMD_MULTIPLEX);
							pktSendBits(pNewPacket, 1, 0);
							pktSendBits(pNewPacket, 32, iSenderIndex);
							pktSendBits(pNewPacket, 32, piDestIndexes[i]);
							pktSendBitsPack(pNewPacket, 4, iCmd);
							pktAppend(pNewPacket, pak, -1);
...
int MultiplexerDestroyCB(NetLink* link,void *pUserData)
{
	...
	if (iDeadIndex < MULTIPLEX_CONST_ID_MAX)
	{
		...
		servLog(LOG_ERRORS, "multiplexerDisconnection", "multiplex index %d link 0x%p error %u reason \"%s\" ip %s", 
			pConnection->iIndex, link, linkGetDisconnectErrorCode(link),
				pDisconnectReason, makeIpStr(linkGetIp(link)));

		filelog_printf("Disconnections.log", "multiplex index %d link 0x%p error %u reason \"%s\" ip %s", 
			pConnection->iIndex, link, linkGetDisconnectErrorCode(link),
			pDisconnectReason, makeIpStr(linkGetIp(link)));
