...
void makeManifold(GMesh *mesh_in, GMesh *mesh_out)
{
	...
	for (i = 0; i < tmp_mesh.tri_count; i++)
	{
		...
		tris = polyGridFindTris(&tmp_mesh.grid, min, max);

		changed = 0;
		for (j = 0; j < eaiSize(&tris); j++)
		{
	...
	for (i = 0; i < mesh_in->tri_count; i++)
	{
		...
		for (j = 0; j < eaiSize(&trilists[i].intersections); j++)
		{
	...
	ZeroStruct(mesh_out);
	gmeshMerge(mesh_out, &tmp_mesh, 1, 0, false, false);
