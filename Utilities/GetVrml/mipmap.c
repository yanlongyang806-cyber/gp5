#include "../../libs/GLRenderLib/thread/ogl.h"
									 ...
									 GLenum typeout, void *dataout )
{
	...
	GLfloat sx, sy;
	GLint unpackrowlength=0, unpackalignment=4, unpackskiprows=0, unpackskippixels=0;
	GLint packrowlength=0, packalignment=4, packskiprows=0, packskippixels=0;
...
// this function is never called, and I assume is a reimplementation of gl.h's version - a microsoft header.  that function is public, this is static - they conflict. leaving for posterity.
#if 0
/*
* WARNING: This function isn't finished and has never been tested!!!!
*/
static GLint APIENTRY gluBuild1DMipmaps( GLenum target, GLint components,
...
// this function seems to actually be used, so I'm turning off this warning
#pragma warning(suppress:4211) // nonstandard extension used : redefined extern to static
{
	int		i,x,y,y_in[2];
...
#endif
/*
GLint gluBuild2DMipmapsSRC( GLenum target, GLenum dst_format,
...
GLint gluBuild2DMipmapsRGBFadeInMem(
									GLint width, GLint height, GLenum format,
									GLenum type, unsigned char *data,F32 close_mix,F32 far_mix )
{
	int		w,h,maxsize,i,size,level,bpp,maxdepth;
	U8		*pix = NULL;
	F32		ratio = 0.2,grey = 128;
	unsigned char *store_data=data;

	bpp = bytes_per_pixel( format, type );
	maxsize = 4096;
	//glGetIntegerv( GL_MAX_TEXTURE_SIZE, &maxsize );
	w = MIN(round2(width),maxsize);
	h = MIN(round2(height),maxsize);
	maxdepth = ilog2(MIN(w,h));
	for(level=0;w && h;level++)
	{
		size = w * h * 4;
		pix = malloc(size);
		gluScaleImage( format, width, height, type, (U32*)data, w, h, type, (U32*)pix );
		ratio = 1.f/256.f * (close_mix - (SQR(level) * (256-far_mix) / SQR(maxdepth)));
		ratio = MINMAX(ratio,0,1);
		for(i=0;i<size;i++)
		{
			if ((i&3) == 3)
				continue;
			pix[i] = qtrunc(pix[i] * ratio + grey * (1-ratio));
		}
		// Copy pix to mem here
		memcpy(store_data, pix, size);
		store_data+=size;
