				...
				for(size_t indx = 0; indx < 3 ; indx ++)//for each vert in that tri
				{
					...
					if( theVerts[ m_Triangles[tID].indices[indx] ].pos  == workingPosition)
					{
						...
						if(otherGroupsIndices.find( m_Triangles[tID].indices[indx] ) != otherGroupsIndices.end() )
						{
							//then we need to make a new vertex
							Vertex newVert;
							newVert = theVerts[ m_Triangles[tID].indices[indx] ];
							newVert.tangent = groupTangentVectors[i];
							size_t oldIndex = m_Triangles[tID].indices[indx];
							size_t newIndex = theVerts.size();

							Vertex &oldVert = theVerts[oldIndex];
							D3DVECTOR *oldTan = &oldVert.tangent;
							D3DVECTOR *newTan = &newVert.tangent;
							// Need to make these two tangents symmetrical over the split line, if this is from mirrored texture coords
							if (0) {
							if (D3DXVec3Dot(&oldVert.tangent, &newVert.tangent) < 0.0) {

								if (newTan->x<0 != oldTan->x<0) {
									if (ABS(ABS(newTan->x) - ABS(oldTan->x)) < 0.1)
										newTan->x = -oldTan->x;
								}
								if (newTan->y<0 != oldTan->y<0) {
									if (ABS(ABS(newTan->y) - ABS(oldTan->y)) < 0.1)
										newTan->y = -oldTan->y;
								}
								if (newTan->z<0 != oldTan->z<0) {
									if (ABS(ABS(newTan->z) - ABS(oldTan->z)) < 0.1)
										newTan->z = -oldTan->z;
								}
								if (ABS(newTan->x - oldTan->x) < 0.1)
									newTan->x = oldTan->x;
								if (ABS(newTan->y - oldTan->y) < 0.1)
									newTan->y = oldTan->y;
								if (ABS(newTan->z - oldTan->z) < 0.1)
									newTan->z = oldTan->z;

							} else {
								// why are we splitting?!
								printf("");
								if (ABS(newTan->x - oldTan->x) < 0.2)
									newTan->x = oldTan->x;
								if (ABS(newTan->y - oldTan->y) < 0.2)
									newTan->y = oldTan->y;
								if (ABS(newTan->z - oldTan->z) < 0.2)
									newTan->z = oldTan->z;
							}
							}
							if (0)
							{ // this code hacks up values, snapping to axese to test things...
							if (newTan->x > 0.7) {
								newTan->x = 1;
								newTan->y = newTan->z = 0;
								newVert.binormal.x = newVert.binormal.z = 0;
								newVert.binormal.y = -1;
								newVert.normal.x=newVert.normal.y=0;
								newVert.normal.z=1;
							}
							if (newTan->x < -0.7) {
								newTan->x = -1;
							...
							if (oldTan->x > 0.7) {
								oldTan->x = 1;
								oldTan->y = oldTan->z = 0;
								oldVert.binormal.x = oldVert.binormal.z = 0;
								oldVert.binormal.y = -1;
								oldVert.normal.x=oldVert.normal.y=0;
								oldVert.normal.z=-1;
							}
							if (oldTan->x < -0.7) {
								oldTan->x = -1;
							...
							theVerts.push_back(newVert);
							AppendToMapping( oldIndex , m_originalNumVerts , mappingNewToOldVert);
							UpdateIndices(oldIndex,newIndex,curGroup);

							assert(m_Triangles[tID].indices[indx] == newIndex);
							//printf("vert %d affected from tri %d\n", m_Triangles[tID].indices[indx], tID);
							didit=true;
						}
						else if (!didit)
						{
...
//							printf("Smoothed tan[%d] = {%1.6f, %1.6f, %1.6f} from tri %d\n", m_Triangles[tID].indices[indx],
//								groupTangentVectors[i].x, groupTangentVectors[i].y, groupTangentVectors[i].z, tID);
//							printf("vert %d affected from tri %d\n", m_Triangles[tID].indices[indx], tID);
						}
				  ...
				  const NormalCalcOption computeNormals)
{
	...
	for(i = 0; i < theIndices.size() ; i += 3)
	{
		...
		//printf("tri[%d] tan: {%1.6f,%1.6f,%1.6f}\n", i/3, t.tangent.x, t.tangent.y, t.tangent.z);
