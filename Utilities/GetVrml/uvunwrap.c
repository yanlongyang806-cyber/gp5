...
__inline static void dvaAdd(DynVec3Array *dva, Vec3 v)
{
	...
	vnew = dynArrayAdd(dva->vecs, sizeof(Vec3), dva->count, dva->max, 1);
	copyVec3(v, vnew);
}
...
#endif
		{
			DVec2 m0, m1, tvecd, tempvec, tempvec2;
			...
			*angle = acos(dotVec2(fvec, svec));
			...
			mulDVecMat2(m0, m1, fvec, tvecd);
			if (!nearSameDVec2(tvecd, svec))
			{
				*angle = -*angle;
				m1[0] = -m1[0];
				m0[1] = -m0[1];
...
{
	return calloc(1,sizeof(VertexGraph));
}
...
{
	VertexGraphNode *ret = calloc(1,sizeof(VertexGraphNode));
...
{
	return (distance3Squared(i, k) + distance3Squared(j, k) - distance3Squared(i, j)) / triArea3(i, j, k);
}

float genConstant( Vec3 i, Vec3 j, Vec3 k1, Vec3 k2 )
{
	float distijSq = distance3Squared(i, j);
	return (distance3Squared(i, k1) + distance3Squared(j, k1) - distijSq) / triArea3(i, j, k1) +
		(distance3Squared(i, k2) + distance3Squared(j, k2) - distijSq) / triArea3(i, j, k2);
}
...
{
	Prim **prims = 0;
	MergeOperation **mergeQueue = NULL, *newMergeOp;
	int i, j, size = eaSize(primitives);
	static int calls = 0;
