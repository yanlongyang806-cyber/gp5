	...
	else
	{
		if ( f )
			fprintf(f, UV_STRING(0.3),	
				p->uv[0][0], p->uv[0][1], p->uv[1][0], p->uv[1][1], p->uv[2][0], p->uv[2][1]);
		else 
...
static int tryPopulateTextureMap2(CombinedPoly ***cp_list, int try_size)
{
	...
	Vec2D **horiz=NULL, **cp_bottom_horiz=NULL, **cp_top_horiz=NULL;//, *newP = NULL; 
	Vec2 point, point2;
...
//	}
//
//	eaDestroyEx(&availableRects, rectFree);
//
//	return ret;
//}
