...
namespace Excel.Core
{
	public class ZipWorker : IDisposable
	{
		...
		private const string FILE_sharedStrings = "sharedStrings.xml";
		private const string FILE_styles = "styles.xml";
		private const string FILE_workbook = "workbook.xml";
		private const string FILE_sheet = "sheet{0}.xml";
		...
		{
			return GetStream(Path.Combine(_xlPath, FILE_sharedStrings));
		}
		...
		{
			return GetStream(Path.Combine(_xlPath, FILE_styles));
		}
		...
		{
			return GetStream(Path.Combine(_xlPath, FILE_workbook));
		}
		...
		public Stream GetWorksheetStream(int sheetId)
		{
			return GetStream(Path.Combine(
				...
				string.Format(FILE_sheet, sheetId)));
		}
