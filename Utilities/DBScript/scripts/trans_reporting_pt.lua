...
local currencies = {
	...
	["nw_escrow_steam"] = "paidsteamneverwinterescrow",
	["nw_escrow_paid"] = "paidneverwinterescrow",
	["nw_escrow_promo"] = "promoneverwinterescrow",
	["nw_claim_steam"] = "paidsteamneverwinterclaim",
	["nw_claim_paid"] = "paidneverwinterclaim",
	["nw_claim_promo"] = "promoneverwinterclaim",
};
...
local currency_codes = {
	...
	[currencies.nw_escrow_steam] = 48,
	[currencies.nw_escrow_paid] = 49,
	[currencies.nw_escrow_promo] = 50,
	[currencies.nw_claim_steam] = 51,
	[currencies.nw_claim_paid] = 52,
	[currencies.nw_claim_promo] = 53,
};
...
function Begin()
	...
	if start_date then
		...
		min_timestamp = min_timestamp + 28800;
	else
	...
	if end_date then
		...
		max_timestamp = max_timestamp + 28800;
	end
...
function ProcessTransaction(id, path)
	...
	if stamp < min_timestamp then
		return;
	end
	if stamp > max_timestamp then
		return;
	end
	...
	if t_type ~= "Micropurchase" then
		...
		for _, p in ipairs(changes) do
			...
			f:write(a_id, id, t_id, t_prov, mt_id, mo_id, p.action, c_code, TimestampFromSS2000(stamp), p_name, p_game, p_cat, t_type, reason, p.diff, p.new);
		end
	else
		for j in xindices(path..".Eapurchaseditems") do
			...
			for _, p in ipairs(spend) do
				local c_code = currency_codes[p.key:lower()];
				f:write(a_id, id, t_id, t_prov, mt_id, mo_id, p.action, c_code, TimestampFromSS2000(stamp-28800), p_name, p_game, p_cat, t_type, reason, p.diff, p.new);
			end
		end
