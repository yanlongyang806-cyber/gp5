...
bool FileListLoader::LoadFileList(char *pListFileName)
{
	...
	if (!(eType == TOKEN_IDENTIFIER && strcmp(token.sVal, "SVNBRANCH") == 0))
	{
		return false;
	}
	...
	if (!(strcmp(token.sVal, SVNBranch) == 0))
	{
		return false;
	}
	...
	do
	{
		...
		if (eType == TOKEN_STRING)
		{
			ASSERT(&tokenizer, m_iNumFiles < MAX_FILES_IN_PROJECT, "Too many files");
			...
			hFile = CreateFile(m_FileNames[m_iNumFiles], FILE_READ_ATTRIBUTES, FILE_SHARE_READ,
