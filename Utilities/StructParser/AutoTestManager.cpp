...
StringTree *spAutoTestReservedWordTree = NULL;
...
bool AutoTestManager::LoadStoredData(bool bForceReset)
{
	...
	tokenizer.SetExtraReservedWords(sAutoTestReservedWords, &spAutoTestReservedWordTree);
	...
	do
	{
		...
		STATICASSERT(eType != TOKEN_NONE, "AutoTest data corruption");
...
void AutoTestManager::GetAtestFileName(char outName[MAX_PATH], char *pInName)
{
	...
	sprintf(outName, "%sAutoGen%s_atest.c", m_pParent->GetProjectPath(), workName);
...
void AutoTestManager::FoundMagicWordAutoTest(char *pSourceFileName, Tokenizer *pTokenizer)
{
	...
	ASSERT(pTokenizer, m_iNumAutoTests < MAX_AUTOTESTS, "Too many AUTO_TESTs");
	...
	{
		ASSERT(pTokenizer,eType == TOKEN_IDENTIFIER && strlen(token.sVal) < MAX_AUTOTEST_COMMAND_LENGTH, "Expected setup func name after AUTOTEST(");
		strcpy(pAutoTest->setupName, token.sVal);
...
void AutoTestManager::FoundMagicWordAutoTestChild(char *pSourceFileName, Tokenizer *pTokenizer)
{
	...
	{
		ASSERT(pTokenizer,eType == TOKEN_IDENTIFIER && strlen(token.sVal) < MAX_AUTOTEST_COMMAND_LENGTH, "Expected setup func name after AUTO_TEST_CHILD(");
		strcpy(pAutoTest->setupName, token.sVal);
...
void AutoTestManager::FoundMagicWordAutoTestBlock(char *pSourceFileName, Tokenizer *pTokenizer)
{
	...
	ASSERT(pTokenizer,parentName[0] != 0, "No parent for AUTO_TEST_BLOCK... require previous AUTO_TEST_GROUP or specified parent name");
	...
	do
	{
		...
		{
			ASSERT(pTokenizer,m_iNumAutoTests < MAX_AUTOTESTS, "Too many AUTOTESTs");
			AUTO_TEST_STRUCT *pAutoTest = &m_AutoTests[m_iNumAutoTests++];
			memset(pAutoTest, 0, sizeof(AUTO_TEST_STRUCT));
			...
			ASSERT(pTokenizer,strlen(pAutoTest->pAutoAssertString) < TOKENIZER_MAX_STRING_LENGTH, "AUTO_TEST_BLOCK expression length overflow inside ASSERT");
		...
		{
			ASSERT(pTokenizer,0, "Expected ) or ASSERT inside AUTO_TEST_BLOCK, found something unknown");
		}
	}
	while (1);
...
void AutoTestManager::PrependParentGroupNames(AUTO_TEST_STRUCT *pAutoTest)
{
	...
	while (pParent)
	{
		...
		STATICASSERTF(strlen(workName) < MAX_AUTOTEST_COMMAND_LENGTH, "Name length overflow: %s", workName);
...
AutoTestManager::AUTO_TEST_STRUCT *AutoTestManager::FindTestByName(char *pName)
{
	...
	STATICASSERTF(0, "Couldn't find AUTO_TEST or group named %s", pName);
	return NULL;
}
