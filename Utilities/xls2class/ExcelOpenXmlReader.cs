...
namespace Excel
{
	public class ExcelOpenXmlReader : IExcelDataReader
	{
		...
		private void ReadSheetGlobals(XlsxWorksheet sheet, int filenameindex)
		{
			_sheetStream = _zipWorker.GetWorksheetStream(filenameindex);
		...
		private bool ReadSheetRow(XlsxWorksheet sheet)
		{
				...
				_xmlReader.ReadToFollowing(XlsxWorksheet.N_row))
			{
				...
				while (_xmlReader.Read())
				{
					...
					if (_xmlReader.NodeType == XmlNodeType.Element && _xmlReader.Name == XlsxWorksheet.N_c && !_xmlReader.IsEmptyElement)
					{
						...
						else if (null != a_s)
						{
							...
							if (xf.ApplyNumberFormat && IsDateTimeStyle(xf.NumFmtId) && (string)o != "")
							{
