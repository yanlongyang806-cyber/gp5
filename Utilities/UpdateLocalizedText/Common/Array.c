...
void initArray(Array* array, int size){
	array->storage = calloc(size, sizeof(void*));
	array->maxSize = size;
	array->size = 0;
}

void arrayPushBack(Array* array, void* element){
	if(0 == array->maxSize)
		initArray(array, 4);

	if(array->size == array->maxSize){
		resizeArray(array, array->maxSize * 2);
	}
...
void setAt(Array* array, int arrayIndex, void* element){
...
void resizeArray(Array* array, int newSize){
	array->storage = (void**) realloc(array->storage, newSize * sizeof(void*));
 ...
 *
 */
void arrayCopy(Array* source, Array* destination, ItemCopy func){
	int i;

	// If the destination array is not large enough, resize it.
	if(destination->maxSize < source->size){
		resizeArray(destination, source->size);
	}
