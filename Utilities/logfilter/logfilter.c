...
// cl /Ic:\src\utilitieslib\components /Ic:\src\utilitieslib\utils /Ic:\src\utilitieslib /D "USEZLIB" /D "_DEBUG" /D "_CRTDBG_MAP_ALLOC=1" /W4 /MTd /Ot /Gd /ZI kernel32.lib user32.lib gdi32.lib winspool.lib comdlg32.lib advapi32.lib shell32.lib ole32.lib oleaut32.lib uuid.lib odbc32.lib odbccp32.lib C:\src\UtilitiesLib\UtilitiesLib_md.lib logfilter.c /link /NODEFAULTLIB:"LIBCMT.lib"
...
#define CHECK(pos)  if(!(pos))  { printf("Error: Malformed log file at line %d. Attempting to recover...\n   <%20.20s>\n", iLine, pchInCur); SkipToEol(&pchInCur); continue; }
...
static int DoIt(char *pchInFile, char *pchOutFile)
{
	...
	*pchInEnd = '\0';
	...
	while(FillBuff(pchIn, &pchInCur, &pchInEnd, fpIn))
	{
		...
		if(*pchInCur!='0') // Sanity check good until 2010.
		{
			if(stricmp(tag,"chat")!=0)
			{
				printf("Error: Malformed log file at line %d. Attempting to recover...\n   <%20.20s>\n", iLine, pchInCur);
			}
			SkipToEol(&pchInCur);
			continue;
		}
