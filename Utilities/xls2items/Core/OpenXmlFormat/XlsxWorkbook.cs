...
namespace Excel.Core.OpenXmlFormat
{
	internal class XlsxWorkbook
	{
		...
		public XlsxWorkbook(Stream workbookStream, Stream sharedStringsStream, Stream stylesStream)
		{
			...
			using (XmlReader reader = XmlReader.Create(xmlFileStream))
			{
				...
				while (reader.Read())
				{
					...
					if (reader.NodeType == XmlNodeType.Element && reader.Name == N_t)
					{
                        if (concatLast)
                        {
                            _SST[_SST.Count - 1] += reader.ReadElementContentAsString();
                            concatLast = false;
                        }
                        else
