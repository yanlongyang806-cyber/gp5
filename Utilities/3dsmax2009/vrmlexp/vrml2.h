...
#define SCENE_EXPORT_CVERTICES (1<<0)
...
public:
	...
	//int   DoExport(const TCHAR *name, Interface *i, VRBLExport* exp, int options);
	int   DoExport(const TCHAR *name, Interface *i, VRBLExport* exp);
#ifdef _LEC_
	//int   DoFBExport(const TCHAR *name, Interface *i, VRBLExport* exp, int frame, TimeValue time, int options);
    ...
    inline void SetDigits(int n) { mDigits = n * 2; }
...
//    inline void SetPolygonType(int type)    { mPolygonType = type; }
	...
	inline BOOL GetExportVertexColors() { return mExportVColors; }
	inline void SetExportVertexColors(BOOL evc) { mExportVColors = evc; }
	inline BOOL GetExportTex2() { return mExportTex2; }
	inline void SetExportTex2(BOOL evc) { mExportTex2 = evc; }
...
private:
	...
	BOOL HasVisibleChildren(INode * node);
	...
	BOOL			mExportVColors; // Export vertex colors
	BOOL			mExportTex2; // Export 2nd texture coordinates
