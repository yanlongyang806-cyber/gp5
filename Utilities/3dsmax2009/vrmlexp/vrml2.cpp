...
TSTR Format(Point3 value)
{
	...
	sprintf_s(fmt, "%s\t%s\t%s", szFmtStr, szFmtStr, szFmtStr);
	sprintf_s(buf, fmt, value.x, value.y, value.z);
...
VRML2Export::point(Point3& p)
{
    ...
    TCHAR format[64];
    sprintf_s(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
    if (mZUp)
        sprintf_s(buf, format, round(p.x), round(p.y), round(p.z));
    else
        sprintf_s(buf, format, round(p.x), round(p.z), round(-p.y));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::color(Color& c)
{
    ...
    TCHAR format[64];
    sprintf_s(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
    sprintf_s(buf, format, round(c.r), round(c.g), round(c.b));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::color(Point3& c)
{
    ...
    TCHAR format[64];
    sprintf_s(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
    sprintf_s(buf, format, round(c.x), round(c.y), round(c.z));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::floatVal(float f)
{
    static TCHAR buf[150];
    TCHAR format[64];
    sprintf_s(format, _T("%%.%dg"), mDigits);
    sprintf_s(buf, format, round(f));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::texture(UVVert& uv)
{
    static TCHAR buf[150];
    TCHAR format[64];
    sprintf_s(format, _T("%%.%dg %%.%dg"), mDigits, mDigits);
    sprintf_s(buf, format, round(uv.x), round(uv.y));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::axisPoint(Point3& p, float angle)
{
    ...
    static TCHAR buf[150];
    TCHAR format[64];
    sprintf_s(format, _T("%%.%dg %%.%dg %%.%dg %%.%dg"),
...
static TCHAR * VRMLName(TCHAR *name)
{
    ...
    if (firstCharacter) {       // if empty name, quick, make one up!
        *cPtr++ = '_';
        *cPtr++ = '_';
        sprintf_s(cPtr, 256 - (cPtr - buffer), "%d", seqnum++);
    }
                             ...
                             int textureNum, BOOL pMirror)
{
    ...
    if (mExportVColors == 1) {
        if (!mCPVSource) {  // 1 if MAX, 0 if we calc
			...
			if (numAOVerts) {
				...
				Indent(level);
				width = CurrentWidth();
				fprintf(mStream, _T("ambientOcclusion AmbientOcclusion { ambientOcclusion [\n"));
				Indent(level+1);
				...
				for (i = 0; i < numAOVerts; i++) {
					...
					if (i == numAOVerts - 1)
						width += fprintf(mStream, _T("%s "), floatVal(vAmbientOcclusion.x));
					else
						width += fprintf(mStream, _T("%s, "), floatVal(vAmbientOcclusion.x));
					width = MaybeNewLine(width, level+1);
				}
	...
	if (mExportTex2)
	{
		...
		if (numTex2Verts) {
			...
			Indent(level);
			width = CurrentWidth();
			fprintf(mStream, _T("tex2Coord Tex2Coord { point [\n"));
			Indent(level+1);
			mesh.setVCDisplayData(-1); // sets vertColArray to point to the ambient occlusion channel
	...
	if ( mod )
	{
		...
		for(int idx = 0 ; idx < skinMC->GetNumPoints(); idx++)  
		{
			...
			{	 
				...
				if(weight)
				{
					width += fprintf(mStream, "%s ", nodename.data());
					width += fprintf(mStream, "%f ", weight);
				}
			}
			width += fprintf(mStream, ") ");
			width = MaybeNewLine(width, level+1);					
...
///////////////////////////////////////////////////end mm
	...
	if (mExportVColors == 1&& (mesh.getNumVertCol() || mesh.getNumMapVerts(-1)))
	{
		...
		{
			Indent(level);
			fprintf(mStream, _T("ambientOcclusionIndex [\n"));
			width = CurrentWidth();
			Indent(level+1);
	...
	if (mExportTex2)
	{
		...
		{
			Indent(level);
			fprintf(mStream, _T("tex2CoordIndex [\n"));
			width = CurrentWidth();
			Indent(level+1);
...
bool strStartsWith(TSTR &str, char *s) {
	if (_strnicmp(str.data(), s, strlen(s))==0)
		return true;
	return false;
}
...
VRML2Export::GetMtlTex(Mtl* mtl, BOOL& isWire)
{
	...
	// Check for x_name
	if (strStartsWith(sm->GetName(), "x_")) {
		bFoundFile = true;
	...
	if (strStartsWith(tm->GetName(), "x_")) {
		bFoundFile = true;
...
//        fprintf(mStream, _T("specularColor %s\n"), color(col));
		...
		Indent(level+1);
		fprintf(mStream, _T("url \"../maps/white\"\n"));
		Indent(level);
		fprintf(mStream, _T("}\n"));
		Indent(--level);
        fprintf(mStream, _T("}\n"));
                                           ...
                                           int level, BOOL pMirror)
{
    ...
    Indent(level);
    sprintf_s(name, _T("%s-COORD-INTERP"), mNodes.GetNodeName(node));
        ...
        for(int j = 0; j < tso->TimeSensorObjects.Count(); j++) {
         ...
         // skip invalid nodes (ex. user create the list then delete the node from the scene.)
            if (anim && NodeIsChildOf(node, anim)) {
                ...
                sprintf_s(timer, _T("%s"),mNodes.GetNodeName(l->GetNode()));
                if (tso->needsScript)
    ...
    for(i = 0; i < mAnimRoutes.Count(); i++) {
        ...
        if (ts != 0 || toObj->ClassID() == TimeSensorClassID) {
            if (toObj->ClassID() == AudioClipClassID)
                sprintf_s(to, _T("%s.startTime"), toName);
            else
                sprintf_s(to, _T("%s-TIMER.startTime"), toName);
            
            if (fromObj->ClassID() == ProxSensorClassID)
                sprintf_s(from, _T("%s.enterTime"), mAnimRoutes[i].mFromName);
            else
                sprintf_s(from, _T("%s-SENSOR.touchTime"), mAnimRoutes[i].mFromName);
    ...
    while (type) {
        ...
        case KEY_POS:
            ...
            if (newKeys == 0)
                return;
            timeVals = posTimes;
            sprintf_s(name, _T("%s-POS-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
        ...
        case KEY_ROT:
            ...
            if (newKeys == 0)
                return;
            timeVals = rotTimes;
            sprintf_s(name, _T("%s-ROT-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
        ...
        case KEY_SCL:
            ...
            if (newKeys == 0)
                return;
            timeVals = sclTimes;
            sprintf_s(name, _T("%s-SCALE-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
            fprintf(mStream, _T("DEF %s PositionInterpolator {\n"), name);
            break;
        case KEY_SCL_ORI:
            timeVals = sclTimes;
            sprintf_s(name, _T("%s-SCALE-ORI-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
            fprintf(mStream, _T("DEF %s OrientationInterpolator {\n"), name);
            break;
        case KEY_COLOR:
            ...
            if (newKeys == 0)
                return;
            timeVals = posTimes;
            sprintf_s(name, _T("%s-COLOR-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
...
VRML2Export::VrmlOutFileInfo()
{
	...
	fprintf(mStream, _T("# modified by CS to include skin info (bones, and vertex weights)\n"));
	fprintf(mStream, _T("# modified by CS to include pivot point\n"));
    fprintf(mStream, _T("# Produced by 3D Studio MAX VRML97 exporter, Version %.5g, Revision %.5g Cryptic Version 16\n"),
        vernum, betanum);
...
VRML2Export::GenerateUniqueNodeNames(INode* node)
{
	...
	if (!nList->hasName) {
	 // take mangled name and get a unique name
		...
		{
			TCHAR newname[512];
			strcpy_s(newname, node->GetName());
			strcat_s(newname, "_HIDDEN");
