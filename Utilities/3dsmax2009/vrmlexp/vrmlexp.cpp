...
VRBLExport::texture(UVVert& uv)
{
    static TCHAR buf[150];
    TCHAR format[20];
    sprintf_s(format, _T("%%.%dg %%.%dg"), mDigits, mDigits, mDigits);
...
VRBLExport::VrblOutFileInfo()
{
    ...
    if (fn && _tcslen(fn) > 0) {
        Indent(1);
        fprintf(mStream, _T("Info { string \"MAX File: %s\" }\n"), fn, time); // , Date: %s
    } else {
        Indent(1);
		 ...
		 // checkout name for "_0xxx" that is our tag
			...
			if (matchStr) {		// possible additional duplicate names
				if (matchStr[1] == '0') {		// assume additional duplicate names
					...
					strncpy_s(buf, name, strlen(name) - strlen(matchStr)); // first part
					buf[strlen(name) - strlen(matchStr)] = '\0';	// terminate
					//sprintf_s(newName.name, "%s_0%d", buf, matchVal+1);	// add one
                    TSTR newName(buf);
                    sprintf_s(buf, "_0%d", matchVal+1);   // add one
                    newName.Append(TSTR(buf));
					return AddName(newName.data()); // check for unique new name
				}
			}
			//sprintf_s(newName.name, "%s_0", name);	// first duplicate name
            TSTR newName(name);
