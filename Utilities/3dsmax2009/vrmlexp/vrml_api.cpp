...
CallbackTable::GetCallbackMethods(Interface* ip) {
    ...
    sprintf_s(lpINIFileName, _T("%s\\%s"), maxPlugCfgPath, "vrmlexp.ini"); 
 ...
 // get dllKeys
    ...
    TCHAR* tmpPtr = lpBuf;
    while (tmpPtr[0] != '\0') {
		size_t dllKey_size = strlen(tmpPtr) * sizeof(TCHAR);
        TCHAR* dllKey = (TCHAR*) malloc(dllKey_size);
        strcpy_s(dllKey, dllKey_size, tmpPtr);
        AddKey(dllKey);
 ...
 // get dlls
    for (i = 0; i < GetKeyCount(); i++) {
		...
		size_t dllPtr_size = strlen(lpBuf) * sizeof(TCHAR);
        TCHAR* dllPtr = (TCHAR*) malloc(dllPtr_size);
        strcpy_s(dllPtr, dllPtr_size, lpBuf);
        AddDll(dllPtr);
    }
	    ...
	    if (libInst)
	    {
         ...
         // Export Lib support for post-scene export
            if (PostSceneCallback & CallbackType) {
                strcpy_s(lpStr, "PostSceneExport");
         ...
         // Export Lib support for post-node export
            ...
            TCHAR buf[256];
            sprintf_s(buf, "Lib [%s] Load Failed", GetDll(i));
	        MessageBox(GetActiveWindow(), buf, "Export Lib", MB_OK|MB_TASKMODAL);
        }
    }
