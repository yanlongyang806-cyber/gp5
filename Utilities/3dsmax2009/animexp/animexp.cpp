...
#define OUTPUT_LINE_ANIM(a) fwrite(a, 1, strlen(a), pStream); 

static char cFirstFrame[32];
static char cVersion[32];
...
int AnimExp::DoExport(const TCHAR *name,ExpInterface *ei,Interface *i, BOOL suppressPrompts, DWORD options) 
{
	...
	// Current version of danim: 150
	sprintf_s(cVersion, "%d\n", 150);
	...
	sprintf_s(cFirstFrame, "%d\n", s);
	//const char* pcBuf = "DAnim\n{\n";
	//const char* pcBuf2 = "}";
	//fwrite(pcBuf, 1, strlen(pcBuf), pStream);
	
	// Startup the progress bar.
	ip->ProgressStart("Exporting file...", TRUE, fn, NULL);
...
void AnimExp::SampleController(INode *n, Control *c) {
	...
	for (int f = s; f <= e; f++) {
		const char* pcA = "\tTransform\n\t{\n";
		const char* pcBX = "\t\tAxis ";
		const char* pcBY = "\n\t\tAngle ";
		const char* pcC = "\n\t\tTranslation ";
		const char* pcD = "\n\t\tScale ";
		const char* pcE = "\n\t}\n\n";
		OUTPUT_LINE_ANIM(pcA);
		t = f*tpf;
		ivalid = FOREVER;
		...
		OUTPUT_LINE_ANIM(pcBX);
		for (int j=0; j<3; j++) {
			writeFloat(aax.axis[j]);
		}
		OUTPUT_LINE_ANIM(pcBY);
		writeFloat(aax.angle);
		OUTPUT_LINE_ANIM(pcC);
		writeFloat(trans.x);
		writeFloat(trans.y);
		writeFloat(trans.z);
		OUTPUT_LINE_ANIM(pcD);
		writeFloat(scale.x);
		writeFloat(scale.y);
		writeFloat(scale.z);
		OUTPUT_LINE_ANIM(pcE);
	}
}
...
void AnimExp::dumpAnimNode(INode *node)
{
	...
	const char* pcC = "\n\tVersion ";
	const char* pcD = "}\n\n";
	strncpy_s(namebuf, sizeof(namebuf), pcA, strlen(pcA));
	strncat_s(namebuf, sizeof(namebuf), name, strlen(name));
	strncat_s(namebuf, sizeof(namebuf), pcB, strlen(pcB));
	strncat_s(namebuf, sizeof(namebuf), cFirstFrame, strlen(cFirstFrame));
	strncat_s(namebuf, sizeof(namebuf), pcC, strlen(pcC));
	strncat_s(namebuf, sizeof(namebuf), cVersion, strlen(cVersion));
	OUTPUT_LINE_ANIM(namebuf);
	printf("Object %s:\n", name);
	c = node->GetTMController(); //->GetPositionController();
	SampleController(node, c);
	strncpy_s(namebuf, sizeof(namebuf), pcD, strlen(pcD));
	OUTPUT_LINE_ANIM(namebuf);
}
