...
#define OUTPUT_LINE_ANIM(a) _fputts(a, pStream); 

static TCHAR cFirstFrame[32];
static TCHAR cVersion[32];
...
int AnimExp::DoExport(const TCHAR *name,ExpInterface *ei,Interface *i, BOOL suppressPrompts, DWORD options) 
{
	...
	ip->ProgressStart(_T("Exporting file..."), TRUE, fn, NULL);
...
void AnimExp::dumpAnimNode(INode *node)
{
	...
	const TCHAR* pcC = _T("\n\tVersion ");
	const TCHAR* pcD = _T("}\n\n");
	const size_t buf_size = sizeof(namebuf) / sizeof(TCHAR);
	_tcsncpy_s(namebuf, pcA, _tcslen(pcA) + 1);
	_tcsncat_s(namebuf, buf_size, name, _tcslen(name));
	_tcsncat_s(namebuf, buf_size, pcB, _tcslen(pcB));
	_tcsncat_s(namebuf, buf_size, cFirstFrame, _tcslen(cFirstFrame));
	_tcsncat_s(namebuf, buf_size, pcC, _tcslen(pcC));
	_tcsncat_s(namebuf, buf_size, cVersion, _tcslen(cVersion));
	OUTPUT_LINE_ANIM(namebuf);
	_tprintf(_T("Object %s:\n"), name);
	c = node->GetTMController(); //->GetPositionController();
	SampleController(node, c);
	_tcsncpy_s(namebuf, pcD, _tcslen(pcD) + 1);
	OUTPUT_LINE_ANIM(namebuf);
}
