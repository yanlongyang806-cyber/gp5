   ...
   HWND hlist = GetDlgItem(hDlg,IDC_URL_LIST);
    ...
    case WM_INITDIALOG: {
      ...
      for (int i = 0; i < bookmarks.Count(); i++) {
         ...
         item.lParam = (LPARAM)bookmarks[i];
            item.pszText = bookmarks[i]->url.ToBSTR();
            item.cchTextMax = static_cast<int>(_tcslen(item.pszText)+1);   // SR DCAST64: Downcast to 2G limit.
            rc = ListView_InsertItem(hlist, &item);
            ListView_SetItemText(hlist, i, 1, bookmarks[i]->desc.ToBSTR());
      }
    ...
    case WM_COMMAND:
      ...
      case IDC_ADD: {
         ...
         long int len = SendDlgItemMessage(hDlg, IDC_URL, WM_GETTEXTLENGTH, 0, 0);
         TSTR temp;
         temp.Resize(len+1);
