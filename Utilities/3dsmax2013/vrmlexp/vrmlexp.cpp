...
VRBLExport::texture(UVVert& uv)
{
    static TCHAR buf[150];
    TCHAR format[20];
    _stprintf_s(format, _T("%%.%dg %%.%dg"), mDigits, mDigits, mDigits);
...
{
    if (node->IsRootNode()) {
        _ftprintf(mStream, _T("Separator {\n"));
        return;
    }
    Indent(level);
    if (obj->SuperClassID() == CAMERA_CLASS_ID ||
     ...
     // Lights and cameras need different top-level names for triggers
        _ftprintf(mStream, _T("DEF %s_TopLevel Separator {\n"), mNodes.GetNodeName(node));
    } else {
        _ftprintf(mStream, _T("DEF %s Separator {\n"), mNodes.GetNodeName(node));
        if (IsMirror(node)) {
            Indent(level+1);
            _ftprintf(mStream, _T("ShapeHints {\n"));
            Indent(level+2);
            _ftprintf(mStream, _T("vertexOrdering CLOCKWISE\n"));
            Indent(level+1);
            _ftprintf(mStream, _T("}\n"));
        }
    }
...
VRBLExport::OutputNodeTransform(INode* node, int level)
{
    ...
    if (isIdentity) {
        p = tm.GetTrans();
        Indent(level);
        _ftprintf(mStream, _T("Translation { translation %s }\n"), point(p));
    } else {
        ...
        if (ang != 0.0f && ang != -0.0f) {
            ...
            _ftprintf(mStream, _T("Rotation { rotation %s }\n"),
                    axisPoint(axis, -ang));
        }
        ...
        if (s.x != 1.0f || s.y != 1.0f || s.z != 1.0f) {
            Indent(level);
            _ftprintf(mStream, _T("Scale { scaleFactor %s }\n"), scalePoint(s));
        }
    }
}
...
NormalTable::PrintStats(FILE* mStream)
{
    ...
    _ftprintf(mStream,_T("# slots = %d, buckets = %d, avg. chain length = %.5g\n"),
...
VRBLExport::OutputNormalIndices(Mesh& mesh, NormalTable* normTab, int level)
{
    ...
    for (i = 0; i < numfaces; i++) {
        ...
        for(v = 0; v < 3; v++) {
            ...
            assert (index != -1);
            _ftprintf(mStream, _T("%d, "), index);
        }
        _ftprintf(mStream, _T("-1,\n"));
    }
    Indent(level);
    _ftprintf(mStream, _T("]\n"));
}
...
VRBLExport::OutputNormals(Mesh& mesh, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("NormalBinding { value PER_VERTEX_INDEXED }\n"));
                            ...
                            BOOL twoSided, int level)
{
   ...
   if (isMulti) {
        Indent(level);
        _ftprintf(mStream, _T("MaterialBinding { value PER_FACE_INDEXED }\n"));
    }

    // Output the vertices
    Indent(level);
    _ftprintf(mStream, _T("Coordinate3 { point [\n"));
        
    for(i = 0; i < numverts; i++) {
        Point3 p = mesh.verts[i];
        Indent(level+1);
        _ftprintf(mStream, _T("%s"), point(p));
        if (i == numverts-1) {
            _ftprintf(mStream, _T("]\n"));
            Indent(level);
            _ftprintf(mStream, _T("}\n"));
    ...
    if (numtverts > 0 && td) {
        Indent(level);
        _ftprintf(mStream, _T("TextureCoordinate2 { point [\n"));

        for(i = 0; i < numtverts; i++) {
            UVVert p = mesh.getTVert(i);
            Indent(level+1);
            _ftprintf(mStream, _T("%s"), texture(p));
            if (i == numtverts-1) {
                _ftprintf(mStream, _T("]\n"));
                Indent(level);
                _ftprintf(mStream, _T("}\n"));
    ...
    if (twoSided) {
        ...
        _ftprintf(mStream, _T("ShapeHints {\n"));
        Indent(level+1);
        _ftprintf(mStream, _T("shapeType UNKNOWN_SHAPE_TYPE\n"));
        Indent(level);
        _ftprintf(mStream, _T("}\n"));
    }
    // Output the triangles
    Indent(level);
    _ftprintf(mStream, _T("IndexedFaceSet { coordIndex [\n"));
    for(i = 0; i < numfaces; i++) {
        if (!(mesh.faces[i].flags & FACE_HIDDEN)) {
            Indent(level+1);
            _ftprintf(mStream, _T("%d, %d, %d, -1"), mesh.faces[i].v[0],
    ...
    if (numtverts > 0 && td) {
        Indent(level);
        _ftprintf(mStream, _T("textureCoordIndex [\n"));
        for(i = 0; i < numfaces; i++) {
            if (!(mesh.faces[i].flags & FACE_HIDDEN)) {
                Indent(level+1);
                _ftprintf(mStream, _T("%d, %d, %d, -1"), mesh.tvFace[i].t[0],
    ...
    if (isMulti) {
        Indent(level);
        _ftprintf(mStream, _T("materialIndex [\n"));
        for(i = 0; i < numfaces; i++) {
            if (!(mesh.faces[i].flags & FACE_HIDDEN)) {
                Indent(level+1);
                _ftprintf(mStream, _T("%d"), mesh.faces[i].getMatID());
                if (i != numfaces-1)
                    _ftprintf(mStream, _T(", \n"));
            }
        }
...
VRBLExport::OutputMultiMtl(Mtl* mtl, int level)
{
    ...
    Indent(level+1);
    _ftprintf(mStream, _T("ambientColor [ "));
    for(i = 0; i < num; i++) {
        sub = mtl->GetSubMtl(i);
        ...
        if (i == num - 1)
            _ftprintf(mStream, _T("%s "), color(c));
        else
            _ftprintf(mStream, _T("%s, "), color(c));
    }
    _ftprintf(mStream, _T("]\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("diffuseColor [ "));
    for(i = 0; i < num; i++) {
        sub = mtl->GetSubMtl(i);
        if (!sub)
            continue;
    ...
    Indent(level+1);
    _ftprintf(mStream, _T("specularColor [ "));
    for(i = 0; i < num; i++) {
        sub = mtl->GetSubMtl(i);
        if (!sub)
            continue;
    ...
    Indent(level+1);
    _ftprintf(mStream, _T("shininess [ "));
    for(i = 0; i < num; i++) {
        sub = mtl->GetSubMtl(i);
        if (!sub)
            continue;
        ...
        if (i == num - 1)
            _ftprintf(mStream, _T("%s "), floatVal(f));
        else
            _ftprintf(mStream, _T("%s, "), floatVal(f));
    }
    _ftprintf(mStream, _T("]\n"));
    ...
    Indent(level+1);
    _ftprintf(mStream, _T("emissiveColor [ "));
    for(i = 0; i < num; i++) {
        sub = mtl->GetSubMtl(i);
        if (!sub)
            continue;
        ...
        if (i == num - 1)
            _ftprintf(mStream, _T("%s "), color(p));
        else
            _ftprintf(mStream, _T("%s, "), color(p));
    }
    _ftprintf(mStream, _T("]\n"));
...
VRBLExport::OutputMaterial(INode* node, BOOL& twoSided, int level)
{
    ...
    _ftprintf(mStream, _T("ambientColor %s\n"), color(c));
    Indent(level+1);
    ...
    _ftprintf(mStream, _T("Texture2 {\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("filename \"%s\"\n"), td->url);
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
    ...
    Indent(level);
    _ftprintf(mStream, _T("Texture2Transform {\n"));
    ...
    if (uScl != 1.0f || vScl != 1.0f) {
        ...
        _ftprintf(mStream, _T("scaleFactor %s\n"), texture(p));
    }
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
...
VRBLExport::VrblOutCylinder(INode* node, Object *obj, int level)
{
    ...
    Indent(level+1);
    if (mZUp) {
        _ftprintf(mStream, _T("Rotation { rotation 1 0 0 %s }\n"),
                floatVal(float(PI/2.0)));
        Indent(level+1);
        _ftprintf(mStream, _T("Translation { translation 0 %s 0 }\n"),
...
VRBLExport::VrblOutCube(INode* node, Object *obj, int level)
{
    ...
    if (mZUp) {
        _ftprintf(mStream, _T("Cube { width %s "),
                floatVal(float(fabs(width))));
        _ftprintf(mStream, _T("height %s "),
                floatVal(float(fabs(length))));
        _ftprintf(mStream, _T(" depth %s }\n"),
                floatVal(float(fabs(height))));
    } else {
        _ftprintf(mStream, _T("Cube { width %s "),
                floatVal(float(fabs(width))));
...
VRBLExport::VrblOutCamera(INode* node, Object* obj, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s_Animated PerspectiveCamera {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("position 0 0 0\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("heightAngle %s\n"), floatVal(vp.fov));
    if (!mZUp) {
        Indent(level+1);
        _ftprintf(mStream, _T("orientation 1 0 0 %s\n"),
                floatVal(float(-PI/2.0)));
    }
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
...
VRBLExport::VrblOutSpotLight(INode* node, LightObject* light, int level)
{
    ...
    Indent(level+1);
    _ftprintf(mStream, _T("dropOffRate %s\n"),
...
{
    Indent(level);
    _ftprintf(mStream, _T("WWWInline {\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("name %s\n"), obj->GetUrl().data());
...
VRBLExport::VrblOutTarget(INode* node, int level)
{
    ...
    if (id != Class_ID(SPOT_LIGHT_CLASS_ID, 0) &&
        ...
        return TRUE;
    Indent(level);
    _ftprintf(mStream, _T("AimTarget_ktx_com {\n"));
    if (mGenFields) {
        Indent(level+1);
        _ftprintf(mStream, _T("fields [ SFString aimer ]\n"));
    }
    Indent(level+1);
	if ( (id == Class_ID(LOOKAT_CAM_CLASS_ID, 0)) && IsEverAnimated(lookAt))
		_ftprintf(mStream, _T("aimer \"%s_Animated\"\n"), mNodes.GetNodeName(lookAt));
	else
		_ftprintf(mStream, _T("aimer \"%s\"\n"), mNodes.GetNodeName(lookAt));
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
    return TRUE;
}
                         ...
                         int type, int level)
{
    ...
    for (int i=0; i<ikeys->GetNumKeys(); i++) {
        ...
        if (i == 0 || TCBIsDifferent(k, oldK)) {
            Indent(level);
            _ftprintf(mStream, _T("AnimationStyle_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
                _ftprintf(mStream, _T("fields [ SFBool loop, SFBitMask splineUse, SFFloat tension, SFFloat continuity, SFFloat bias, SFFloat easeTo, SFFloat easeFrom, SFVec3f pivotOffset ]\n"));
            Indent(level+1);
            _ftprintf(mStream, _T("splineUse ("));
            
            // Write flags
            BOOL hadOne = FALSE;
            if (k->tens   != 0.0f) {
                _ftprintf(mStream, _T("TENSION"));
                hadOne = TRUE;
            }
            if (k->cont   != 0.0f) {
                if (hadOne)
                    _ftprintf(mStream, _T(" | "));
                _ftprintf(mStream, _T("CONTINUITY"));
                hadOne = TRUE;
            }
            if (k->bias   != 0.0f) {
                if (hadOne)
                    _ftprintf(mStream, _T(" | "));
                _ftprintf(mStream, _T("BIAS"));
                hadOne = TRUE;
            }
            if (k->easeIn != 0.0f) {
                if (hadOne)
                    _ftprintf(mStream, _T(" | "));
                _ftprintf(mStream, _T("EASE_TO"));
                hadOne = TRUE;
            }
            if (k->easeOut!= 0.0f) {
                if (hadOne)
                    _ftprintf(mStream, _T(" | "));
                _ftprintf(mStream, _T("EASE_FROM"));
                hadOne = TRUE;
            }
            _ftprintf(mStream, _T(")\n"));
            ...
            if (k->tens   != 0.0f) {
                Indent(level+1);
                _ftprintf(mStream, _T("tension %s\n"), floatVal(k->tens));
            }
            if (k->cont   != 0.0f) {
                Indent(level+1);
                _ftprintf(mStream, _T("continuity %s\n"), floatVal(k->cont));
            }
            if (k->bias   != 0.0f) {
                Indent(level+1);
                _ftprintf(mStream, _T("bias %s\n"), floatVal(k->bias));
            }
            if (k->easeIn != 0.0f) {
                Indent(level+1);
                _ftprintf(mStream, _T("easeTo %s\n"), floatVal(k->easeIn));
            }
            if (k->easeOut!= 0.0f) {
                Indent(level+1);
                _ftprintf(mStream, _T("easeFrom %s\n"), floatVal(k->easeOut));
            }
            ...
            Indent(level+1);
            if (type != KEY_POS) _ftprintf(mStream, _T("pivotOffset %s\n"), point(po));
            Indent(level);
            _ftprintf(mStream, _T("}\n"));
        ...
        case KEY_SCL: {
            ...
            mHadAnim = TRUE;
            Indent(level);
            _ftprintf(mStream, _T("ScaleKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
                _ftprintf(mStream,
        ...
        case KEY_COLOR:
            ...
            mHadAnim = TRUE;
            Indent(level);
            _ftprintf(mStream, _T("ColorKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
                _ftprintf(mStream,
        ...
        case KEY_POS:
            ...
            mHadAnim = TRUE;
            Indent(level);
            _ftprintf(mStream, _T("PositionKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
                _ftprintf(mStream,
        ...
        case KEY_ROT: {
            ...
            mHadAnim = TRUE;
            Indent(level);
            _ftprintf(mStream, _T("RotationKey_ktx_com {\n"));
            Indent(level+1);
            if (mGenFields)
                _ftprintf(mStream,
                             ...
                             int type, int level)
{
    ...
    Indent(level+1);
    if (mGenFields)
        _ftprintf(mStream, _T("fields [ SFVec3f pivotOffset ]\n"));
    Indent(level+1);
...
VRBLExport::WritePositionKey0(INode* node, TimeValue t, int level, BOOL force)
{
    ...
    Indent(level+1);
    if (mGenFields)
        _ftprintf(mStream, _T("fields [ SFLong frame, SFVec3f translation ]\n"));
    Indent(level+1);
...
{ 
    ...
    Indent(level+1);
    if (mGenFields)
        _ftprintf(mStream, _T("fields [ SFLong frame, SFVec3f scale ]\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("frame %d\n"), (t-mStart)/GetTicksPerFrame());
    Indent(level+1);
    _ftprintf(mStream, _T("scale %s\n"), scalePoint(s));
...
VRBLExport::VrmlOutTopLevelCamera(int level, INode* node, BOOL topLevel)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s PerspectiveCamera {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
...
VRBLExport::VrblOutFileInfo()
{
    ...
    Indent(1);
    _ftprintf(mStream, _T("Info { string \"Produced by 3D Studio MAX VRML 1.0/2.0 exporter, Version %.5g, Revision %.5g\" }\n"), vernum, betanum);
    
    
    time_t ltime;
    time( &ltime );
	char time[1024];
    ctime_s(time, sizeof(time), &ltime);
    // strip the CR
    time[strlen(time)-1] = '\0';
	TCHAR* fn = mIp->GetCurFileName().dataForWrite();
    if (fn && _tcslen(fn) > 0) {
        Indent(1);
        _ftprintf(mStream, _T("Info { string \"MAX File: %s\" }\n"), fn, time); // , Date: %s
    } else {
        Indent(1);
        _ftprintf(mStream, _T("Info { string \"Date: %s\" }\n"), time);
    }
    Indent(1);
    _ftprintf(mStream, _T("ShapeHints {\n"));
    Indent(2);
    _ftprintf(mStream, _T("shapeType SOLID\n"));
    Indent(2);
    _ftprintf(mStream, _T("vertexOrdering COUNTERCLOCKWISE\n"));
    Indent(2);
    _ftprintf(mStream, _T("faceType CONVEX\n"));
    Indent(1);
    _ftprintf(mStream, _T("}\n"));
}
...
VRBLExport::VrblOutAnimationFrames()
{
    if (mType == Export_VRBL && mHadAnim) {
        Indent(1);
        _ftprintf(mStream, _T("AnimationFrames_ktx_com {\n"));
        Indent(2);
        if (mGenFields)
            _ftprintf(mStream, _T("fields [ SFLong length, SFLong segmentStart, SFLong segmentEnd, SFLong current, SFFloat rate ]\n"));
        Indent(2);
        int frames = (mIp->GetAnimRange().End()-mIp->GetAnimRange().Start())/
                      GetTicksPerFrame() + 1;
        _ftprintf(mStream, _T("length %d\n"), frames);
        Indent(2);
        _ftprintf(mStream, _T("rate %d\n"), GetFrameRate());
        Indent(1);
        _ftprintf(mStream, _T("}\n"));
    }
}
...
GetAppData(Interface * ip, int id, TCHAR* def, TCHAR* val, int len)
{
	...
	if (!ad)
        ...
        //mbstowcs(val, (char*) ad->data, len);
		_tcscpy_s(val, len, (TCHAR*) ad->data);
	}
}
...
WriteAppData(Interface* ip, int id, TCHAR* val)
{
    ...
    node->RemoveAppDataChunk(Class_ID(VRBL_EXPORT_CLASS_ID,0),
                             SCENE_EXPORT_CLASS_ID, id);
	int len = _tcslen(val) + 1;
	int buf_size = sizeof(TCHAR) * len;
    TCHAR* buf = (TCHAR*) MAX_malloc(buf_size);	//MAX_malloc data is managed&freed by the system.
    _tcscpy_s(buf, len, val);
...
VrblExportDlgProc(HWND hDlg, UINT msg, WPARAM wParam, LPARAM lParam) 
{
    ...
    case WM_INITDIALOG: {
		...
		gen = 1; // JE: Removing unused options for Cryptic
        ...
        gen = _tcscmp(text, _T("yes")) == 0;
		gen = 0; // JE: Removing unused options for Cryptic
...
#ifdef _LEC_
		// JE: Removing unused options for Cryptic
    ...
    case WM_COMMAND:
        ...
        case IDC_EXP_HELP: {
            ...
            _tcscpy_s(helpFile, helpDir);
            _tcscat_s(helpFile, _T("\\vrmlout.hlp"));
        ...
        case IDOK: {
			...
			exp->SetExportVertexColors(IsDlgButtonChecked(hDlg, IDC_EXPORT_VCOLORS));
// 			WriteAppData(exp->mIp, EXPORT_VCOLOR_ID, exp->GetExportVertexColors() ?
// 						_T("yes"): _T("no"));
			exp->SetExportTex2(IsDlgButtonChecked(hDlg, IDC_EXPORT_TEX2));
			...
			WriteAppData(exp->mIp, URL_PREFIX_ID, exp->GetUrlPrefix().dataForWrite());
...
VRBLExport::DoExport(const TCHAR *filename, ExpInterface *ei, Interface *i, BOOL suppressPrompts, DWORD options) 
{
	...
	//if (suppressPrompts)
		//initializeDefaults();
	//else
	if (!suppressPrompts && !DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_VRBLEXP), 
    ...
    if (IsVRML2()) {
     // generate the callback table of third party dlls
        ...
        if (this->GetFlipBook()) {
            ...
            FILE* fio;
			_tfopen_s(&fio, rfName.data(), _T("w"));
            _ftprintf(fio, _T("%s\n"), filename);
...
/*
				if (suppressPrompts) {
					int val = vrml2.DoFBExport(filename, i, this, frame, mStart, SCENE_EXPORT_CVERTICES);
					if (!val) return val;
				} else {
					int val = vrml2.DoFBExport(filename, i, this, frame, mStart, 0);
					if (!val) return val;
				}
*/
        ...
        VRML2Export vrml2;
		int val;
		val = vrml2.DoExport(filename, i, this);
/*
		if (suppressPrompts) {
			val = vrml2.DoExport(filename, i, this, SCENE_EXPORT_CVERTICES);
		} else {
			val = vrml2.DoExport(filename, i, this, 0);
		}*/
...
#if IS_CRYPTIC_EXPORTER	
	// need to set this before initialization
	mIp = GetCOREInterface();
	// moved here so that we can override before calling DoExport
	initializeDefaults();
#endif
}
...
// Node Name lookup
TSTR NodeTable::GetNodeName(INode* node)
{
    DWORD hash = HashNode(node, NODE_HASH_TABLE_SIZE);
    NodeList* nList;
...
TSTR NodeTable::AddName(const TCHAR* name)
{
		 ...
		 // checkout name for "_0xxx" that is our tag
			...
			if (matchStr) {		// possible additional duplicate names
				if (matchStr[1] == '0') {		// assume additional duplicate names
					...
					_tcsncpy_s(buf, name, _tcslen(name) - _tcslen(matchStr)); // first part
					buf[_tcslen(name) - _tcslen(matchStr)] = _T('\0');	// terminate
					//_stprintf_s(newName.name, "%s_0%d", buf, matchVal+1);	// add one
                    TSTR newName(buf);
                    _stprintf_s(buf, _T("_0%d"), matchVal+1);   // add one
                    newName.Append(TSTR(buf));
					return AddName(newName.data()); // check for unique new name
				}
			}
			//_stprintf_s(newName.name, "%s_0", name);	// first duplicate name
            TSTR newName(name);
            newName.Append(zeroSuffix);
			return AddName(newName.data()); // check for unique new name
		}
	}
