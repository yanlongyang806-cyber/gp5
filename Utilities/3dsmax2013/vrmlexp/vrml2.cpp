...
#include <maxstring.h>
////////////////////////////////////////mm for skin info
#include "FindSkin.h" //FindSkinModifier() is here
////////////////////////////////////////end mm
#include "maxclasses.h"
...
Matrix3
GetLocalTM(INode* node, TimeValue t, bool node_transform)
{
    Matrix3 tm;
	if (node_transform)
		tm = node->GetNodeTM(t);
	else
		tm = node->GetObjectTM(t); //GetObjTMAfterWSM(t);
#ifdef TEMP_TEST
    if (getworldmat)
        return tm;
#endif
    if (!node->GetParentNode()->IsRootNode()) {
        Matrix3 ip = node->GetParentNode()->GetObjectTM(t); //GetObjTMAfterWSM(t);
        tm = tm * Inverse(ip);
    }
    return tm;
}
...
////////////////////////////////////////////////////////////////mm
//added these funcs so I cound use them below for skin
#define CTL_CHARS  31
...
TSTR Format(float value)
{
	...
	_stprintf_s(buf, szFmtStr, value);
	CommaScan(buf);
	return TSTR(buf);
}
...
TSTR Format(Point3 value)
{
	...
	_stprintf_s(fmt, _T("%s\t%s\t%s"), szFmtStr, szFmtStr, szFmtStr);
	_stprintf_s(buf, fmt, value.x, value.y, value.z);
...
VRML2Export::point(Point3& p)
{
    ...
    TCHAR format[64];
    _stprintf_s(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
    if (mZUp)
        _stprintf_s(buf, format, round(p.x), round(p.y), round(p.z));
    else
        _stprintf_s(buf, format, round(p.x), round(p.z), round(-p.y));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::color(Color& c)
{
    ...
    TCHAR format[64];
    _stprintf_s(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
    _stprintf_s(buf, format, round(c.r), round(c.g), round(c.b));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::color(Point3& c)
{
    ...
    TCHAR format[64];
    _stprintf_s(format, _T("%%.%dg %%.%dg %%.%dg"), mDigits, mDigits, mDigits);
    _stprintf_s(buf, format, round(c.x), round(c.y), round(c.z));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::floatVal(float f)
{
    static TCHAR buf[150];
    TCHAR format[64];
    _stprintf_s(format, _T("%%.%dg"), mDigits);
    _stprintf_s(buf, format, round(f));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::texture(UVVert& uv)
{
    static TCHAR buf[150];
    TCHAR format[64];
    _stprintf_s(format, _T("%%.%dg %%.%dg"), mDigits, mDigits);
    _stprintf_s(buf, format, round(uv.x), round(uv.y));
    CommaScan(buf);
    return buf;
}
...
VRML2Export::axisPoint(Point3& p, float angle)
{
    ...
    static TCHAR buf[150];
    TCHAR format[64];
    _stprintf_s(format, _T("%%.%dg %%.%dg %%.%dg %%.%dg"),
...
static TCHAR * VRMLName(const TCHAR *name)
{
    ...
    if (firstCharacter) {       // if empty name, quick, make one up!
        *cPtr++ = '_';
        *cPtr++ = '_';
        _stprintf_s(cPtr, 256 - (cPtr - buffer), _T("%d"), seqnum++);
    }
...
{
	TCHAR *nodnam = mNodes.GetNodeName(node).dataForWrite();
    Indent(level);
    _ftprintf(mStream, _T("DEF %s Transform {\n"), nodnam);
    ...
    for(int i=0; i < numNotes; i++) {
        ...
        for (int j = 0; j < nt->keys.Count(); j++) {
            ...
            if (note.Length() > 0) {
                Indent(level+1);
                _ftprintf(mStream, _T("#Info { string \"frame %d: %s\" }\n"),
...
VRML2Export::OutputNodeTransform(INode* node, int level, BOOL mirrored)
{
	...
	Quat q;
	float ang;
    Matrix3 tm = GetLocalTM(node, mStart, false);
	Point3 p = GetLocalTM(node, mStart, true).GetTrans();
    ...
    if (isIdentity) {
		////////////////mm added pivot point to the the object.
		Indent(level);
		_ftprintf(mStream, _T("translation %s\n"), point(p + node->GetObjOffsetPos()));
		Indent(level);
		_ftprintf(mStream, _T("pivot %s\n"), point(p));
		///////////////end mm
        return FALSE;
    }
    AffineParts parts;
 ...
 //   p = parts.t;
    ...
    if (ang != 0.0f && ang != -0.0f) {
        Indent(level);
        _ftprintf(mStream, _T("rotation %s\n"), axisPoint(axis, -ang));
    }
    ...
    if (!(AEQ(s.x, 1.0)) || !(AEQ(s.y, 1.0)) || !(AEQ(s.z, 1.0))) {
        Indent(level);
        _ftprintf(mStream, _T("scale %s\n"), scalePoint(s));
        q = sv.q;
        AngAxisFromQa(q, &ang, axis);
        if (ang != 0.0f && ang != -0.0f) {
            Indent(level);
            _ftprintf(mStream, _T("scaleOrientation %s\n"), axisPoint(axis, -ang));
        }
    }
    return parts.f < 0.0f;
}
...
static Point3 getNormal(Mesh &mesh, int index, int corner)
{
	int j, norCnt;
	MeshNormalSpec *meshNorm = mesh.GetSpecifiedNormals();
	int meshNormIndex = meshNorm?meshNorm->GetNormalIndex(index, corner):-1;
	int smGroup = mesh.faces[index].getSmGroup();
	int cv = mesh.faces[index].v[corner];
	RVertex * rv = mesh.getRVertPtr(cv);
	if (meshNormIndex != -1 && meshNorm->GetNormalExplicit(meshNormIndex)) {
//		MeshNormalFace &f = meshNorm->Face(index);
//		int normID = f.GetNormalID(corner);
//		return meshNorm->Normal(normID).Normalize();
		return meshNorm->GetNormal(index, corner);
	}
		...
		if (norCnt == 1)
			...
			if (rv->ern[j].getSmGroup() & smGroup) {
				return rv->ern[j].getNormal();
			}
		}
	} // else
	return mesh.getFaceNormal(index);
}
                                 ...
                                 int textureNum)
{
    ...
    _ftprintf(mStream, _T("normalIndex [\n"));
    Indent(level+1);
    for (i = 0; i < numfaces; i++) {
        ...
        if (textureNum == -1 || id == textureNum) {
            ...
            for(v = 0; v < 3; v++) {
				n = getNormal(mesh, i, v);
                int index = normTab->GetIndex(n);
                assert (index != -1);
                width += _ftprintf(mStream, _T("%d, "), index);
                width = MaybeNewLine(width, level+1);
            }
            width += _ftprintf(mStream, _T("-1, "));
            width = MaybeNewLine(width, level+1);
        }
    }
    _ftprintf(mStream, _T("]\n"));
...
VRML2Export::OutputNormals(Mesh& mesh, int level)
{
    ...
    for(int index = 0; index < numfaces; index++) {
        for(i = 0; i < 3; i++) {
			normTab->AddNormal(getNormal(mesh, index, i));
        }
    }
    ...
    NormalDesc* nd;
    Indent(level);
    _ftprintf(mStream, _T("normal "));
    _ftprintf(mStream, _T("Normal { vector [\n"));
    int width = CurrentWidth();
    Indent(level+1);
    ...
    for(i = 0; i < NORM_TABLE_SIZE; i++) {
        for(nd = normTab->Get(i); nd; nd = nd->next) {
            ...
            width += _ftprintf(mStream, _T("%s, "), normPoint(p));
            width = MaybeNewLine(width, level+1);
        }
    }
    _ftprintf(mStream, _T("] }\n"));
    ...
    Indent(level);
    _ftprintf(mStream, _T("normalPerVertex TRUE\n"));
                             ...
                             int textureNum, BOOL pMirror)
{
    ...
    _ftprintf(mnfile, "Vertices:\n");
    for (i = 0; i < mnmesh.VNum(); i++)
        _ftprintf(mnfile, "  %3d)  %8.3f, %8.3f, %8.3f\n", i,
            mnmesh.P(i).x, mnmesh.P(i).y, mnmesh.P(i).z);
    _ftprintf(mnfile, "Faces:\n");
    ...
    Indent(level++);
    if (isWire)
        _ftprintf(mStream, _T("geometry DEF %s-FACES IndexedLineSet {\n"),
            mNodes.GetNodeName(node));
    else
        _ftprintf(mStream, _T("geometry DEF %s-FACES IndexedFaceSet {\n"),
                mNodes.GetNodeName(node));

    if (!isWire) {
        Indent(level);
        _ftprintf(mStream, _T("ccw %s\n"), pMirror ? _T("FALSE") : _T("TRUE"));
        Indent(level);
        _ftprintf(mStream, _T("solid %s\n"), twoSided ? _T("FALSE") : _T("TRUE"));
        Indent(level);
        _ftprintf(mStream, _T("convex %s\n"), concave ? _T("FALSE") : _T("TRUE"));
    }
 ...
 // color-------
    if (0 || mPreLight) {
        if (!mCPVSource) {   // 1 if MAX, 0 if we calc
            ...
            Color c;
            Indent(level);
            _ftprintf(mStream, _T("colorPerVertex TRUE\n"));
            Indent(level);
            width = CurrentWidth();
            _ftprintf(mStream, _T("color Color { color [\n"));
            Indent(level+1);
            cfaces = vxColDiffTab.Count();
            for (i = 0; i < cfaces; i++) {
                c = *((Color*) vxColDiffTab[i]);
                if (i == cfaces - 1) width += _ftprintf(mStream, _T("%s "), color(c));
                else width += _ftprintf(mStream, _T("%s, "), color(c));
                width = MaybeNewLine(width, level+1);
            }
            Indent(level);
            _ftprintf(mStream, _T("] }\n"));
            ...
            if (numCVerts) {
                ...
                for (i = 0; i < nVerts; i++) {
                    ...
                    */
                    ...
                    if (i == nVerts - 1)
                        width += _ftprintf(mStream, _T("%s "), color(vColor));
                    else
                        width += _ftprintf(mStream, _T("%s, "), color(vColor));
                    width = MaybeNewLine(width, level+1);
                }
	...
	if ((0 || mPreLight) && mesh.getNumVertCol())
	{
		Indent(level);
		_ftprintf(mStream, _T("colorIndex [\n"));
		width = CurrentWidth();
		Indent(level+1);
 ...
 // output coordinate---------
    if (textureNum < 1) {
        Indent(level);
        _ftprintf(mStream, _T("coord DEF %s-COORD Coordinate { point [\n"),
                mNodes.GetNodeName(node));
        width = CurrentWidth();
        Indent(level+1);
        for (i = 0; i < polyMesh.GetVertexCnt(); i++) {
            ...
            if (pMirror)
                p = - p;
#endif
            width += _ftprintf(mStream, _T("%s"), point(p));
        ...
        Indent(level);
        _ftprintf(mStream, _T("coord USE %s-COORD\n"),
                mNodes.GetNodeName(node));
    }
    Indent(level);
	_ftprintf(mStream, _T("coordIndex [\n"));
	Indent(level+1);
    width = CurrentWidth();
 ...
 // Output Texture coordinates
    if (numtverts > 0 && (td || textureNum == 0) && !isWire) {
        if (textureNum < 1) {
            Indent(level);
            _ftprintf(mStream, _T("texCoord DEF %s-TEXCOORD TextureCoordinate { point [\n"),
                mNodes.GetNodeName(node));
            width = CurrentWidth();
            Indent(level+1);
            for (i = 0; i < polyMesh.GetTVertexCnt(); i++) {
                UVVert t = polyMesh.GetTVertex(i);
                width += _ftprintf(mStream, _T("%s"), texture(t));
            ...
            Indent(level);
            _ftprintf(mStream, _T("texCoord USE %s-TEXCOORD\n"),
                    mNodes.GetNodeName(node));
        }
    }
    if (numtverts > 0 && td && !isWire) {
        Indent(level);
	    _ftprintf(mStream, _T("texCoordIndex [\n"));
	    Indent(level+1);
        width = CurrentWidth();
...
/*
        for (i = 0; i < polyMesh.GetPolygonCnt(); i++) {
            ...
            normTab->AddNormal(n);
            width += _ftprintf(mStream, _T("%s, "), normPoint(n));
			width  = MaybeNewLine(width, level+1);
		}
*/
                             ...
                             int textureNum, BOOL pMirror)
{
    ...
    if (mExportVColors == 1) {
        if (!mCPVSource) {  // 1 if MAX, 0 if we calc
			...
			int numAOVerts = mesh.getNumMapVerts(-1);
			if (numAOVerts) {
				VertColor vAmbientOcclusion(1,1,1);
				Indent(level);
				width = CurrentWidth();
				_ftprintf(mStream, _T("ambientOcclusion AmbientOcclusion { ambientOcclusion [\n"));
				Indent(level+1);
				mesh.setVCDisplayData(-1); // sets vertColArray to point to the ambient occlusion channel
				for (i = 0; i < numAOVerts; i++) {
					vAmbientOcclusion = mesh.vertColArray[i];
					if (i == numAOVerts - 1)
						width += _ftprintf(mStream, _T("%s "), floatVal(vAmbientOcclusion.x));
					else
						width += _ftprintf(mStream, _T("%s, "), floatVal(vAmbientOcclusion.x));
					width = MaybeNewLine(width, level+1);
				}
	...
	if (mExportTex2)
	{
		int numTex2Verts = mesh.getNumMapVerts(2);
		if (numTex2Verts) {
			VertColor v(1,1,1);
			Indent(level);
			width = CurrentWidth();
			_ftprintf(mStream, _T("tex2Coord Tex2Coord { point [\n"));
			Indent(level+1);
			mesh.setVCDisplayData(-1); // sets vertColArray to point to the ambient occlusion channel
			UVVert *vertArray = mesh.mapVerts(2);
			for (i = 0; i < numTex2Verts; i++) {
				UVVert p = vertArray[i];
				width += _ftprintf(mStream, _T("%s"), texture(p));
				if (i == numTex2Verts-1) {
					_ftprintf(mStream, _T("]\n"));
					Indent(level);
					_ftprintf(mStream, _T("}\n"));
	...
	//mm////////////////////////////////////////////////////////////////////////////////////////////////////////
//This code gets the Bones and Vertex Weights for the node and prints them to the VRML file
//FindSkinModifier() is in FindSkin.h; FixupName() and Format() are at the top of this file; ISkin and the rest comes 
//from the new COM_Skin COM object which is the new the skin modifier.
//Finally, "node" is the node currently being printed to VRML file.
	...
	if ( mod )
	{
		...
		//Begin Skin section
		Indent(level);
		_ftprintf(mStream, _T("skin ["));
		
		// vertex assignment information
		for(int idx = 0 ; idx < skinMC->GetNumPoints(); idx++)  
		{
			width += _ftprintf(mStream, _T("( "));
			for(int j = 0 ; j < skinMC->GetNumAssignedBones(idx) ; j++)
			{	 
				//get the bone and its name
				TSTR nodename;
				INode* bone = skin->GetBone(skinMC->GetAssignedBone(idx,j));
				nodename = bone->GetName();
				
				//get the weight
				float weight = skinMC->GetBoneWeight(idx,j);
				
				//write the data to the file stream
				if(weight)
				{
					width += _ftprintf(mStream, _T("%s "), nodename.data());
					width += _ftprintf(mStream, _T("%f "), weight);
				}
			}
			width += _ftprintf(mStream, _T(") "));
			width = MaybeNewLine(width, level+1);					
...
///////////////////////////////////////////////////end mm
    ...
    for(i = 0; i < numfaces; i++) {
        ...
        if (textureNum == -1 || id == textureNum) {
            if (!(mesh.faces[i].flags & FACE_HIDDEN)) {
                width += _ftprintf(mStream, _T("%d, %d, %d, -1"),
	...
	if (mExportVColors == 1&& (mesh.getNumVertCol() || mesh.getNumMapVerts(-1)))
	{
		TVFace *faces = mesh.vcFace;
		if (faces)
		{
			Indent(level);
			_ftprintf(mStream, _T("colorIndex [\n"));
		...
		faces = mesh.mapFaces(-1);
		if (faces)
		{
			Indent(level);
			_ftprintf(mStream, _T("ambientOcclusionIndex [\n"));
			width = CurrentWidth();
			Indent(level+1);
	...
	if (mExportTex2)
	{
		...
		{
			Indent(level);
			_ftprintf(mStream, _T("tex2CoordIndex [\n"));
			width = CurrentWidth();
			Indent(level+1);
...
bool strStartsWith(TSTR &str, TCHAR *s) {
	if (_tcsnicmp(str.data(), s, _tcslen(s))==0)
		return true;
	return false;
}
...
VRML2Export::GetMtlTex(Mtl* mtl, BOOL& isWire)
{
	...
	// Check for x_name
	if (strStartsWith(sm->GetName(), _T("x_"))) {
		bFoundFile = true;
		bitmapFile = sm->GetName();
	}
	...
	if (strStartsWith(tm->GetName(), _T("x_"))) {
		bFoundFile = true;
		bitmapFile = tm->GetName();
	}
	...
	if (!bFoundFile || bitmapFile.data() == NULL)
		bitmapFile = bm->GetMapName();
    if (bitmapFile.data() == NULL)
        return NULL;
    int l = _tcslen(bitmapFile)-1;
    if (l < 0)
        return NULL;
                            ...
                            int level, int textureNum)
{
    ...
    Indent(level++);
    _ftprintf(mStream, _T("appearance Appearance {\n"));
    ...
    if (!mtl || (mtl->ClassID() != Class_ID(DMTL_CLASS_ID, 0) &&
        ...
        _ftprintf(mStream, _T("Material {\n"));
        Indent(level+1);
        _ftprintf(mStream, _T("diffuseColor %s\n"), color(col));
//        Indent(level+1);
//        _ftprintf(mStream, _T("specularColor .9 .9 .9\n"));
//        _ftprintf(mStream, _T("specularColor %s\n"), color(col));
        Indent(level);
        _ftprintf(mStream, _T("}\n"));
		Indent(level);
		_ftprintf(mStream, _T("texture ImageTexture {\n"));
		Indent(level+1);
		_ftprintf(mStream, _T("url \"../maps/white\"\n"));
		Indent(level);
		_ftprintf(mStream, _T("}\n"));
		Indent(--level);
        _ftprintf(mStream, _T("}\n"));
    ...
    sm->Update(0, i);
    Indent(level);
    _ftprintf(mStream, _T("material Material {\n"));
    Color c;
	...
	if (ambin >= difin)
		_ftprintf(mStream, _T("ambientIntensity 1.0\n"));
	else
		_ftprintf(mStream, _T("ambientIntensity %s\n"), floatVal(ambin/difin));
    Indent(level+1);
    ...
    sh = sh * 0.95f + 0.05f;
    Indent(level+1);
    _ftprintf(mStream, _T("shininess %s\n"), floatVal(sh));
    Indent(level+1);
    _ftprintf(mStream, _T("transparency %s\n"),
    ...
    if (si > 0.0f) {
        ...
        _ftprintf(mStream, _T("emissiveColor %s\n"), color(p));
    }
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
    ...
    Indent(level+1);
    _ftprintf(mStream, _T("url \"%s\"\n"), td->url);
    // _ftprintf(mStream, _T("repeatS TRUE\n"));
    // _ftprintf(mStream, _T("repeatT TRUE\n"));
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
    ...
    Indent(level);
    _ftprintf(mStream, _T("textureTransform TextureTransform {\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("center 0.5 0.5\n"));
...
VRML2Export::VrmlOutSphere(INode * node, Object *obj, int level)
{
    ...
    _ftprintf(mStream, _T("geometry "));
    
    _ftprintf(mStream, _T("Sphere { radius %s }\n"), floatVal(radius));
                                  ...
                                  BOOL mirrored)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("Transform {\n"));
    if (mZUp) {
        Indent(level+1);
        _ftprintf(mStream, _T("rotation 1 0 0 %s\n"),
                floatVal(float(PI/2.0)));
        Indent(level+1);
        _ftprintf(mStream, _T("translation 0 0 %s\n"),
...
VRML2Export::VrmlOutCylinder(INode* node, Object *obj, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("geometry "));
    _ftprintf(mStream, _T("Cylinder { radius %s "), floatVal(radius));
    _ftprintf(mStream, _T("height %s }\n"), floatVal(float(fabs(height))));
...
VRML2Export::VrmlOutCone(INode* node, Object *obj, int level)
{
    ...
    _ftprintf(mStream, _T("Cone { bottomRadius %s "), floatVal(radius1));
    _ftprintf(mStream, _T("height %s }\n"), floatVal(float(fabs(height))));
...
VRML2Export::VrmlOutCube(INode* node, Object *obj, int level)
{
    ...
    _ftprintf(mStream, _T("geometry "));
    if (mZUp) {
        _ftprintf(mStream, _T("Box { size %s "),
                floatVal(float(fabs(width))));
        _ftprintf(mStream, _T("%s "),
...
VRML2Export::VrmlOutCamera(INode* node, Object* obj, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s Viewpoint {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("position 0 0 0\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("fieldOfView %s\n"), floatVal(vp.fov));
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
...
VRML2Export::VrmlOutSound(INode* node, SoundObject* obj, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s Sound {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("direction %s\n"), point(dir));
    Indent(level+1);
    _ftprintf(mStream, _T("intensity %s\n"), floatVal(intensity));
    Indent(level+1);
    _ftprintf(mStream, _T("location 0 0 0\n"));
    Indent(level+1);
    _ftprintf(mStream, _T("maxBack %s\n"), floatVal(maxBack));
    Indent(level+1);
    _ftprintf(mStream, _T("maxFront %s\n"), floatVal(maxFront));
    Indent(level+1);
    _ftprintf(mStream, _T("minBack %s\n"), floatVal(minBack));
    Indent(level+1);
    _ftprintf(mStream, _T("minFront %s\n"), floatVal(minFront));
    Indent(level+1);
    _ftprintf(mStream, _T("priority %s\n"), floatVal(priority));
    Indent(level+1);
    _ftprintf(mStream, _T("spatialize %s\n"),
            spatialize ? _T("TRUE") : _T("FALSE"));
    if (obj->audioClip) {
        Indent(level+1);
//        _ftprintf(mStream, _T("source USE %s\n"), VRMLName(obj->audioClip->GetName()));
        _ftprintf(mStream, _T("source\n"));
...
VRML2Export::VrmlOutTouchSensor(INode* node, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s-SENSOR TouchSensor { enabled %s }\n"),mNodes.GetNodeName(node),
    ...
    for(int i=0; i < size; i++) {
        ...
        if (top != otop) {
			if (_tcscpy_s(toObjName, sizeof(toObjName) / sizeof(TCHAR), ob->name.data()) == EINVAL)
				toObjName[0] = _T('\0');
            AddAnimRoute(vrmlObjName, toObjName, node, top);
                                ...
                                int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s ProximitySensor {\n"),mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("enabled %s\n"),
            enabled ? _T("TRUE") : _T("FALSE"));
    Indent(level+1);
    Point3 center(0.0f, 0.0f, height/2.0f);
    _ftprintf(mStream, _T("center %s\n"), point(center));
    Indent(level+1);
    Point3 size(width, length, height);
    _ftprintf(mStream, _T("size %s\n"), scalePoint(size));
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
    ...
    for(int i=0; i < numObjs; i++) {
        ...
        if (top != otop) {
			if (_tcscpy_s(toObjName, sizeof(toObjName) / sizeof(TCHAR), ob->name.data()) == EINVAL)
				toObjName[0] = _T('\0');
            AddAnimRoute(vrmlObjName, toObjName, node, top);	//was passing ob->name.data() which is a const as of 2013.
            AddCameraAnimRoutes(vrmlObjName, node, top);
            otop = top;
        }
    }
...
VRML2Export::VrmlOutBillboard(INode* node, Object* obj, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s Billboard {\n"), mNodes.GetNodeName(node));
    if (screenAlign) {
        Indent(level+1);
        _ftprintf(mStream, _T("axisOfRotation 0 0 0\n"));
    } else {
        Point3 axis(0, 0, 1);
        Indent(level+1);
        _ftprintf(mStream, _T("axisOfRotation %s\n"), point(axis));
    }
    Indent(level+1);
    _ftprintf(mStream, _T("children [\n"));
...
VRML2Export::VrmlOutTimeSensor(INode* node, TimeSensorObject* obj, int level)
{
    ...
    duration = end - start;
    Indent(level);
    _ftprintf(mStream, _T("DEF %s-TIMER TimeSensor {\n"),mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("cycleInterval %s\n"),
            floatVal(float(duration) / float(TIME_TICKSPERSEC)));
    Indent(level+1);
    _ftprintf(mStream, _T("loop %s\n"), loop ? _T("TRUE") : _T("FALSE"));
    Indent(level+1);
    if (startOnLoad)
        _ftprintf(mStream, _T("startTime 1\n"));
    else
        _ftprintf(mStream, _T("stopTime 1\n"));
    _ftprintf(mStream, _T("}\n"));
    if (obj->needsScript) {
        _ftprintf(mStream, _T("DEF %s-SCRIPT Script {\n"),mNodes.GetNodeName(node));
        Indent(1);
        _ftprintf(mStream, _T("eventIn SFFloat fractionIn\n"));
        Indent(1);
        _ftprintf(mStream, _T("eventOut SFFloat fractionOut\n"));
        Indent(1);
        _ftprintf(mStream, _T("url \"javascript:\n"));
        Indent(2);
        _ftprintf(mStream, _T("function fractionIn(i) {\n"));
        Indent(2);
        float fract = (float(end) - float(start))/
                       (float(animEnd) - float(mStart));
        float offset = (start - mStart) /
                       (float(animEnd) - float(mStart));
        _ftprintf(mStream, _T("fractionOut = %s * i"), floatVal(fract));
        if (offset != 0.0f)
            _ftprintf(mStream, _T(" + %s;\n"), floatVal(offset));
        else
            _ftprintf(mStream, _T(";\n"));
...
VRML2Export::VrmlOutPointLight(INode* node, LightObject* light, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s-LIGHT PointLight {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("intensity %s\n"),
            floatVal(light->GetIntensity(mStart, FOREVER)));
    Indent(level+1);
    Point3 col = light->GetRGBColor(mStart, FOREVER);
    _ftprintf(mStream, _T("color %s\n"), color(col));
    Indent(level+1);
    _ftprintf(mStream, _T("location 0 0 0\n"));

    Indent(level+1);
    _ftprintf(mStream, _T("on %s\n"), ls.on ? _T("TRUE") : _T("FALSE"));
    Indent(level+1);
    _ftprintf(mStream, _T("radius %s\n"), floatVal(ls.attenEnd));
    if (ls.useAtten) {
        Indent(level+1);
        _ftprintf(mStream, _T("attenuation 0 1 0\n"));
    }
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
    return TRUE;
...
VRML2Export::VrmlOutDirectLight(INode* node, LightObject* light, int level)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s-LIGHT DirectionalLight {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("intensity %s\n"),
            floatVal(light->GetIntensity(mStart, FOREVER)));
    Indent(level+1);
...
VRML2Export::VrmlOutSpotLight(INode* node, LightObject* light, int level)
{
    ...
    light->EvalLightState(mStart, iv, &ls);
    Indent(level);
    _ftprintf(mStream, _T("DEF %s-LIGHT SpotLight {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("intensity %s\n"),
            floatVal(light->GetIntensity(mStart,FOREVER)));
    Indent(level+1);
    ...
    _ftprintf(mStream, _T("direction %s\n"), normPoint(dir));
    Indent(level+1);
    _ftprintf(mStream, _T("cutOffAngle %s\n"),
            floatVal(DegToRad(ls.fallsize)));
    Indent(level+1);
    _ftprintf(mStream, _T("beamWidth %s\n"), floatVal(DegToRad(ls.hotsize)));
    Indent(level+1);
    _ftprintf(mStream, _T("on %s\n"), ls.on ? _T("TRUE") : _T("FALSE"));
    Indent(level+1);
    _ftprintf(mStream, _T("radius %s\n"), floatVal(ls.attenEnd));
...
VRML2Export::VrmlOutTopPointLight(INode* node, LightObject* light)
{
    ...
    _ftprintf(mStream, _T("DEF %s PointLight {\n"), mNodes.GetNodeName(node));
    Indent(1);
    _ftprintf(mStream, _T("intensity %s\n"),
            floatVal(light->GetIntensity(mStart, FOREVER)));
    Indent(1);
    ...
    _ftprintf(mStream, _T("location %s\n"), point(p));
...
VRML2Export::VrmlOutTopDirectLight(INode* node, LightObject* light)
{
    ...
    _ftprintf(mStream, _T("DEF %s DirectionalLight {\n"), mNodes.GetNodeName(node));
    Indent(1);
    _ftprintf(mStream, _T("intensity %s\n"),
            floatVal(light->GetIntensity(mStart, FOREVER)));
    Indent(1);
...
VRML2Export::VrmlOutTopSpotLight(INode* node, LightObject* light)
{
    ...
    light->EvalLightState(mStart, iv, &ls);
    _ftprintf(mStream, _T("DEF %s SpotLight {\n"), mNodes.GetNodeName(node));
    Indent(1);
    _ftprintf(mStream, _T("intensity %s\n"),
            floatVal(light->GetIntensity(mStart,FOREVER)));
    Indent(1);
    ...
    if (ls.useAtten) {
        ...
        Indent(1);
        _ftprintf(mStream, _T("attenuation 0 %s 0\n"), floatVal(attn));
    }
    _ftprintf(mStream, _T("}\n"));
    return TRUE;
}
...
{
    TCHAR url[256];
	if (_tcscpy_s(url, sizeof(url) / sizeof(TCHAR), obj->GetUrl().data()) == EINVAL)
		url[0] = _T('\0');
    Indent(level);
    _ftprintf(mStream, _T("Inline {\n"));
    Indent(level+1);
    if (url && url[0] == '"')
        _ftprintf(mStream, _T("url %s\n"), url);
    else
    ...
    if (obj->GetUseSize())
    {
        ...
        Indent(level+1);
        _ftprintf(mStream, _T("bboxSize %s\n"),
...
//    if (numLod > 1) {
        Indent(level);
        _ftprintf(mStream, _T("LOD {\n"));
        Indent(level+1);
        Point3 p = node->GetObjTMAfterWSM(mStart).GetTrans();
        _ftprintf(mStream, _T("center %s\n"), point(p));
        Indent(level+1);
        _ftprintf(mStream, _T("range [ "));
        for(i = 0; i < numLod-1; i++) {
            if (i < numLod-2)
                _ftprintf(mStream, _T("%s, "), floatVal(lodObjects[i]->dist));
            else
...
VRML2Export::isVrmlObject(INode * node, Object *obj, INode* parent)
{
    ...
    */
	...
	// Adding recursive check to see if children are visible
 ...
 // LEC uses dummies as place holders and need dummy leaves written.
    ...
    return (obj->IsRenderable() ||
        ...
        (mExportHidden || !node->IsHidden()) && (!exportSelected || node->Selected());
        
}

BOOL VRML2Export::HasVisibleChildren(INode * node)
{
	if(node->NumberOfChildren() > 0)
	{
		for(int c = 0; c < node->NumberOfChildren(); c++)
		{
			...
			if((!child->IsHidden() || (!exportSelected || child->Selected()) || HasVisibleChildren(child)))
			{
				return TRUE;
			}
		}
                                           ...
                                           int level, BOOL pMirror)
{
    ...
    Indent(level);
    _stprintf_s(name, _T("%s-COORD-INTERP"), mNodes.GetNodeName(node));
    _ftprintf(mStream, _T("DEF %s CoordinateInterpolator {\n"), name);
	AddInterpolator(name, KEY_COORD, mNodes.GetNodeName(node).dataForWrite());
        ...
        for(int j = 0; j < tso->TimeSensorObjects.Count(); j++) {
         ...
         // skip invalid nodes (ex. user create the list then delete the node from the scene.)
            if (anim && NodeIsChildOf(node, anim)) {
                ...
                _stprintf_s(timer, _T("%s"),mNodes.GetNodeName(l->GetNode()));
                if (tso->needsScript)
    ...
    for(int i = start; i < end; i++) {
        ...
        if (isTriMesh && obj->IsRenderable()) {
            ...
            if (ob->objectUsed && i == -1) {
                ...
                // We have an instance
                Indent(level);
                _ftprintf(mStream, _T("geometry USE %s-FACES\n"),
                        ob->instName.data());
            } else {
                ob->objectUsed = TRUE;
...
int my_assert1( /*char *expr,*/ int line, char *file )
{
...
VRML2Export::VrmlParent(INode* node)
{
    ...
    */
    assert (node);
    _tcscpy_s(buf, mNodes.GetNodeName(node));
    return buf;
}
                              ...
                              int level)
{
    ...
    if (action == HyperLinkJump || action == MrBlueMessage ||
        ...
        switch (action) {
        case MrBlueMessage:
            _ftprintf(mStream, _T("%s \"signal:\"\n"), name);
            break;
        case HyperLinkJump:
            camera = obj->GetCamera();
            if (camera.Length() == 0)
                _ftprintf(mStream, _T("%s \"%s\"\n"), name, obj->GetURL());
            else
                _ftprintf(mStream, _T("%s \"%s#%s\"\n"), name, obj->GetURL(),
        ...
        case SetViewpoint:
            ...
            if (trigType == MouseClick) {
                ...
                if (desc.Length() > 0) {
                    Indent(level+1);
                    _ftprintf(mStream, _T("description \"%s\"\n"), desc);
                }
            }
            break;
        default:
        ...
        switch (trigType) {
        case MouseClick:
            _ftprintf(mStream, _T("DEF %s-SENSOR TouchSensor {}\n"), vrmlObjName);
            break;
        case DistProximity:
            break;
        case BoundingBox:
                                 ...
                                 INode* top)
{
    for(int i = 0; i < top->NumberOfChildren(); i++) {
        ...
        if (sid == CAMERA_CLASS_ID)
			AddAnimRoute(vrmlObjName, mNodes.GetNodeName(child).dataForWrite(),
                fromNode, child);
                          ...
                          INode* toNode  )
{
    ...
    if (!from || *from == '\0') {
        _tcscpy_s(fromStr, mNodes.GetNodeName(fromNode));
        from = fromStr;
    }
    if (!to || *to == '\0')
		to = mNodes.GetNodeName(toNode).dataForWrite();
...
VRML2Export::WriteAnimRoutes()
{
    ...
    for(i = 0; i < mAnimRoutes.Count(); i++) {
		...
		TCHAR *toName = mNodes.GetNodeName(toNode).dataForWrite();
        ...
        if (ts != 0 || toObj->ClassID() == TimeSensorClassID) {
            if (toObj->ClassID() == AudioClipClassID)
                _stprintf_s(to, _T("%s.startTime"), toName);
            else
                _stprintf_s(to, _T("%s-TIMER.startTime"), toName);
            
            if (fromObj->ClassID() == ProxSensorClassID)
                _stprintf_s(from, _T("%s.enterTime"), mAnimRoutes[i].mFromName);
            else
                _stprintf_s(from, _T("%s-SENSOR.touchTime"), mAnimRoutes[i].mFromName);
            _ftprintf(mStream, _T("ROUTE %s TO %s\n"), from, to);
        }
    }
}
                          ...
                          int* level, BOOL fromParent)
{
    ...
    if (obj->GetBBoxEnabled() && !fromParent) {
        ...
        _ftprintf(mStream, _T("point [ %s, "), point(p0));
        _ftprintf(mStream, _T(" %s ]\n"), point(p1));
        (*level)++;
        hadHeader = TRUE;
    }
    ...
    if (obj->GetLosEnabled()) {
        if (obj->GetLosType() == CanSee) {
            ...
            _ftprintf(mStream, _T("distance %s\n"),
                    floatVal(GetLosProxDist(node, mStart)));
            Indent(*level+1);
            _ftprintf(mStream, _T("sightAngle %s\n"),
                    floatVal(DegToRad(obj->GetLosVptAngle())));
            Point3 p = GetLosVector(node, mStart);
            Indent(*level+1);
            _ftprintf(mStream, _T("vector %s\n"), normPoint(p));
            Indent(*level+1);
            _ftprintf(mStream, _T("vectorAngle %s\n"),
    ...
    for(i = 0; i < mInterpRoutes.Count(); i++) {
        ...
        if (mInterpRoutes[i].mType == KEY_TIMER)
            _ftprintf(mStream,
                    ...
                    (TSTR)mInterpRoutes[i].mNode, (TSTR)mInterpRoutes[i].mInterp);
        else if (mInterpRoutes[i].mType == KEY_TIMER_SCRIPT) {
            _ftprintf(mStream,
                    _T("ROUTE %s-TIMER.fraction_changed TO %s-SCRIPT.fractionIn\n"),
                    (TSTR)mInterpRoutes[i].mNode, (TSTR)mInterpRoutes[i].mNode);
            _ftprintf(mStream,
                    _T("ROUTE %s-SCRIPT.fractionOut TO %s.set_fraction\n"),
                    (TSTR)mInterpRoutes[i].mNode, (TSTR)mInterpRoutes[i].mInterp);
        } else if (!mTimer.isNull())
            _ftprintf(mStream, _T("ROUTE %s.fraction_changed TO %s.set_fraction\n"),
                    (TSTR)mTimer, (TSTR)mInterpRoutes[i].mInterp);
        Indent(level);
        TCHAR* setType = NULL;
        ...
        if (mInterpRoutes[i].mType != KEY_TIMER &&
            ...
            assert(setType);
            if (isCamera)
                _ftprintf(mStream, _T("ROUTE %s.value_changed TO %s.%s\n"),
                        (TSTR)mInterpRoutes[i].mInterp, (TSTR)mInterpRoutes[i].mNode,
                        setType);
            else if (mInterpRoutes[i].mType == KEY_COLOR)
                _ftprintf(mStream, _T("ROUTE %s.value_changed TO %s-LIGHT.%s\n"),
                        (TSTR)mInterpRoutes[i].mInterp, (TSTR)mInterpRoutes[i].mNode,
                        setType);
            else if (mInterpRoutes[i].mType == KEY_COORD)
                _ftprintf(mStream,_T("ROUTE %s.value_changed TO %s-COORD.%s\n"),
                        (TSTR)mInterpRoutes[i].mInterp, (TSTR)mInterpRoutes[i].mNode,
                        setType);
            else
                _ftprintf(mStream, _T("ROUTE %s.value_changed TO %s.%s\n"),
    ...
    while (type) {
        ...
        case KEY_POS:
            ...
            if (newKeys == 0)
                return;
            timeVals = posTimes;
            _stprintf_s(name, _T("%s-POS-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
            _ftprintf(mStream, _T("DEF %s PositionInterpolator {\n"), name);
            break;
        case KEY_ROT:
            eps = float(1.0e-5);
            ...
            if (newKeys == 0)
                return;
            timeVals = rotTimes;
            _stprintf_s(name, _T("%s-ROT-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
            _ftprintf(mStream, _T("DEF %s OrientationInterpolator {\n"), name);
            break;
        case KEY_SCL:
            eps = float(1.0e-5);
            ...
            if (newKeys == 0)
                return;
            timeVals = sclTimes;
            _stprintf_s(name, _T("%s-SCALE-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
            _ftprintf(mStream, _T("DEF %s PositionInterpolator {\n"), name);
            break;
        case KEY_SCL_ORI:
            timeVals = sclTimes;
            _stprintf_s(name, _T("%s-SCALE-ORI-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
            _ftprintf(mStream, _T("DEF %s OrientationInterpolator {\n"), name);
            break;
        case KEY_COLOR:
            ...
            if (newKeys == 0)
                return;
            timeVals = posTimes;
            _stprintf_s(name, _T("%s-COLOR-INTERP"), mNodes.GetNodeName(node));
            Indent(level);
            _ftprintf(mStream, _T("DEF %s ColorInterpolator {\n"), name);
            break;
        default: return;
        }

		AddInterpolator(name, type, mNodes.GetNodeName(node).dataForWrite());
        // Now check to see if a TimeSensor references this node
        INodeList* l;
        for(l = mTimerList; l; l = l->GetNext()) {
            TimeSensorObject* tso = (TimeSensorObject*)
        ...
        for (i=0; i < timeVals.Count(); i++) {
            ...
            case KEY_COLOR:
                mHadAnim = TRUE;
                p = posKeys[i];
                width += _ftprintf(mStream, _T("%s, "), color(p));
                break;
            ...
            case KEY_SCL:
                mHadAnim = TRUE;
                s = sclKeys[i].s;
                width += _ftprintf(mStream, _T("%s, "), scalePoint(s));
                break;
            case KEY_SCL_ORI:
                mHadAnim = TRUE;
                q = sclKeys[i].q;
...
#define AVOID_NEG_SCALE
            ...
            if (parts.u.x * oldu.x < -eps || parts.u.y * oldu.y < -eps ||
                ...
                if (hight < t) {
                    if (!isCamera)
                        mat = GetLocalTM(node, hight, true);
                    else
...
VRML2Export::WriteVisibilityData(INode *node, int level) {
	...
	vis = node->GetVisibility(t) <= 0.0f ? FALSE : TRUE;
        if (vis != lastVis) {
            mHadAnim = TRUE;
            Indent(level);
            _ftprintf(mStream, _T("HideKey_ktx_com {\n"));
            if (mGenFields) {
                Indent(level+1);
                _ftprintf(mStream, _T("fields [ SFLong frame] \n"));
            }
            Indent(level+1);
            _ftprintf(mStream, _T("frame %d\n"), i);
            Indent(level);
            _ftprintf(mStream, _T("}\n"));
        }
        lastVis = vis;
...
VRML2Export::VrmlOutTopLevelCamera(int level, INode* node, BOOL topLevel)
{
    ...
    _ftprintf(mStream, _T("DEF %s Viewpoint {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("position %s\n"), point(p));
    Indent(level+1);
    _ftprintf(mStream, _T("orientation %s\n"), axisPoint(axis, -ang));
    Indent(level+1);
    _ftprintf(mStream, _T("fieldOfView %s\n"), floatVal(vp.fov));
    Indent(level + 1);
    _ftprintf(mStream, _T("description \"%s\"\n"), mNodes.GetNodeName(node));
...
VRML2Export::VrmlOutTopLevelNavInfo(int level, INode* node, BOOL topLevel)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s NavigationInfo {\n"),mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("avatarSize [%s, "), floatVal(collision));
    _ftprintf(mStream, _T("%s, "), floatVal(terrain));
    _ftprintf(mStream, _T("%s]\n"), floatVal(step));
    Indent(level+1);
    _ftprintf(mStream, _T("headlight %s\n"),
            headlight ? _T("TRUE") : _T("FALSE"));
    Indent(level+1);
    _ftprintf(mStream, _T("speed %s\n"), floatVal(speed));
    Indent(level+1);
    if (type < 0 || type > 3)
        type = 0;
    _ftprintf(mStream, _T("type \"%s\"\n"), navTypes[type]);
    Indent(level+1);
    _ftprintf(mStream, _T("visibilityLimit %s\n"), floatVal(visLimit));
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
}
...
VRML2Export::VrmlOutTopLevelBackground(int level, INode* node, BOOL topLevel)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s Background {\n"),mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("skyColor ["));
    for(i = 0; i < numColors + 1; i++)
        _ftprintf(mStream, _T("%s, "), color(col[i]));
    _ftprintf(mStream, _T("]\n"));
    
    if (numColors > 0) {
        Indent(level+1);
        _ftprintf(mStream, _T("skyAngle ["));
        _ftprintf(mStream, _T("%s, "), floatVal(angle2));
        if (numColors > 1)
            _ftprintf(mStream, _T("%s, "), floatVal(angle3));
        _ftprintf(mStream, _T("]\n"));
    }
    ...
    Indent(level+1);
    _ftprintf(mStream, _T("groundColor ["));
    for(i = 0; i < numColors + 1; i++)
        _ftprintf(mStream, _T("%s, "), color(col[i]));
    _ftprintf(mStream, _T("]\n"));
    ...
    if (numColors > 0) {
        Indent(level+1);
        _ftprintf(mStream, _T("groundAngle ["));
        _ftprintf(mStream, _T("%s, "), floatVal(angle2));
        if (numColors > 1)
            _ftprintf(mStream, _T("%s, "), floatVal(angle3));
        _ftprintf(mStream, _T("]\n"));
    ...
    if (bg->back.Length() > 0) {
        ...
        _ftprintf(mStream, _T("backUrl \"%s\"\n"), url.data());
    }
    if (bg->bottom.Length() > 0) {
        Indent(level+1);
        url = PrefixUrl(bg->bottom);
        _ftprintf(mStream, _T("bottomUrl \"%s\"\n"), url.data());
    }
    if (bg->front.Length() > 0) {
        Indent(level+1);
        url = PrefixUrl(bg->front);
        _ftprintf(mStream, _T("frontUrl \"%s\"\n"), url.data());
    }
    if (bg->left.Length() > 0) {
        Indent(level+1);
        url = PrefixUrl(bg->left);
        _ftprintf(mStream, _T("leftUrl \"%s\"\n"), url.data());
    }
    if (bg->right.Length() > 0) {
        Indent(level+1);
        url = PrefixUrl(bg->right);
        _ftprintf(mStream, _T("rightUrl \"%s\"\n"), url.data());
    }
    if (bg->top.Length() > 0) {
        Indent(level+1);
        url = PrefixUrl(bg->top);
        _ftprintf(mStream, _T("topUrl \"%s\"\n"), url.data());
    }
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
}
...
VRML2Export::VrmlOutTopLevelFog(int level, INode* node, BOOL topLevel)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s Fog {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("color %s\n"), color(p));
    Indent(level+1);
    _ftprintf(mStream, _T("fogType \"%s\"\n"), type == 0 ? _T("LINEAR") :
            _T("EXPONENTIAL"));
    Indent(level+1);
    _ftprintf(mStream, _T("visibilityRange %s\n"), floatVal(visLimit));
    Indent(level);
    _ftprintf(mStream, _T("}\n"));
}
...
VRML2Export::VrmlOutAudioClip(int level, INode* node)
{
    ...
    Indent(level);
    _ftprintf(mStream, _T("DEF %s AudioClip {\n"), mNodes.GetNodeName(node));
    Indent(level+1);
    _ftprintf(mStream, _T("description \"%s\"\n"), ac->desc.data());
    Indent(level+1);
    _ftprintf(mStream, _T("url \"%s\"\n"), ac->url.data());
    Indent(level+1);
    _ftprintf(mStream, _T("pitch %s\n"), floatVal(pitch));
    Indent(level+1);
    _ftprintf(mStream, _T("loop %s\n"), loop ? _T("TRUE") : _T("FALSE"));
    Indent(level+1);
    if (start)
...
VRML2Export::VrmlOutFileInfo()
{
	...
	_ftprintf(mStream, _T("# modified by CS to include skin info (bones, and vertex weights)\n"));
	_ftprintf(mStream, _T("# modified by CS to include pivot point\n"));
    _ftprintf(mStream, _T("# Produced by 3D Studio MAX VRML97 exporter, Version %.5g, Revision %.5g Cryptic Version 16\n"),
        vernum, betanum);
    ...
    time( &ltime );
	char time[1024];
    ctime_s(time, 1024, &ltime);
    // strip the CR
    time[strlen(time)-1] = '\0';
    TSTR fn = mIp->GetCurFileName();
    if (fn && _tcslen(fn) > 0) {
        _ftprintf(mStream, _T("# MAX File: %s\n\n"), fn, time); // , Date: %s
    } else {
        _ftprintf(mStream, _T("# Date: %s\n\n"), time);
    }
}
...
VRML2Export::VrmlOutWorldInfo()
{
    ...
    _ftprintf(mStream, _T("WorldInfo {\n"));
    if (mTitle.Length() != 0) {
        Indent(1);
        _ftprintf(mStream, _T("title \"%s\"\n"), mTitle.data());
    }
    if (mInfo.Length() != 0) {
        Indent(1);
        _ftprintf(mStream, _T("info \"%s\"\n"), mInfo.data());
    }
    _ftprintf(mStream, _T("}\n"));
}
...
VRML2Export::StartAnchor(INode* node, int& level)
{
    ...
    for(l = sb->mSensors; l; l = l->GetNext()) {
        ...
        if (obj->ClassID() == AnchorClassID) {
            ...
            if (type == 0) {
                Indent(level);
                _ftprintf(mStream, _T("parameter \"%s\"\n"), ao->parameter.data());
                Indent(level);
                _ftprintf(mStream, _T("url \"%s\"\n"), ao->URL.data());
            } else {
                if (ao->cameraObject) {
...
VRML2Export::VrmlOutNode(INode* node, INode* parent, int level, BOOL isLOD,
 ...
 // Don't gen code for LOD references, only LOD nodes
    ...
    if (mEnableProgressBar) SendMessage(hWndPDlg, 666, 0,
		(LPARAM) mNodes.GetNodeName(node).dataForWrite());
    ...
    if (node->IsRootNode()) {
     ...
     // Also output top-level objects
        ...
        if (mHasLights && !mHasNavInfo) {
            _ftprintf(mStream, _T("NavigationInfo { headlight FALSE }\n"));
        }
    }
 ...
 // give third party dlls a chance to write the node
    if (!node->IsRootNode()) {
        ...
        if (!mStream)
            _tfopen_s(&mStream, mFilename, _T("a"));
    }
                        ...
                        int frame, TimeValue time)
{
	...
	mExportVColors	 = exp->GetExportVertexColors();
	mExportTex2		 = exp->GetExportTex2();
//  Leaving code in place commented out for now as other options are looked at for exporting vertex colors while prompt is suppressed.
/*
	if (options & SCENE_EXPORT_CVERTICES) {
		mExportVColors = true;
	} else {
		mExportVColors	 = exp->GetExportVertexColors();
	}*/
 ...
 // Write out the VRML header
    _ftprintf(mStream, _T("#VRML V2.0 utf8\n\n"));
...
VRML2Export::GenerateUniqueNodeNames(INode* node)
{
	...
	if (!nList->hasName) {
	 // take mangled name and get a unique name
		if ((mExportHidden || !node->IsHidden()) && !(exportSelected && node->Selected() == FALSE))
			nList->name    = mNodes.AddName(VRMLName(node->GetName()));
		else// Hidden or unselected, in which case we treat as hidden
		{
			TCHAR newname[512];
			_tcscpy_s(newname, node->GetName());
			_tcscat_s(newname, _T("_HIDDEN"));
			nList->name    = mNodes.AddName(VRMLName(newname));
		}
