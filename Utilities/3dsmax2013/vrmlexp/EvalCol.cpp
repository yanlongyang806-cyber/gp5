...
class CheckFileNames: public AssetEnumCallback {
	public:
		...
		void RecordName(TCHAR *name);	//deprecated as of max sdk 2013 - see plug-in upgrade guide in sdk documentation
...
void CheckFileNames::RecordName(TCHAR *name) // DEPRECATED - use CheckFileNames::RecordAsset()
{														
	if (name) {											
		if (name[0]!=0) {								
...
{
	if (asset.GetType() == kBitmapAsset) {
		if (missingMaps->FindName(asset.GetFileName()) == -1) {
