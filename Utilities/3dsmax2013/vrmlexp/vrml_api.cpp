...
CallbackTable::GetCallbackMethods(Interface* ip) {
    ...
    _stprintf_s(lpINIFileName, _T("%s\\%s"), maxPlugCfgPath, _T("vrmlexp.ini")); 
 ...
 // get dllKeys
    ...
    TCHAR* tmpPtr = lpBuf;
    while (tmpPtr[0] != '\0') {
		size_t dllKey_size = _tcslen(tmpPtr);			// was strlen(tmpPtr) * sizeof(TCHAR) and used as argument for allocation and as string length;
        TCHAR* dllKey = (TCHAR*) malloc(dllKey_size * sizeof(TCHAR));
        _tcsncpy_s(dllKey, dllKey_size, tmpPtr, _tcslen(tmpPtr));
        AddKey(dllKey);
 ...
 // get dlls
    for (i = 0; i < GetKeyCount(); i++) {
		...
		size_t dllPtr_size = _tcslen(lpBuf); // was strlen(lpBuf) * sizeof(TCHAR) and used as argument for allocation and as string length;
        TCHAR* dllPtr = (TCHAR*) malloc(dllPtr_size * sizeof(TCHAR));
        _tcsncpy_s(dllPtr, dllPtr_size, lpBuf, _tcslen(lpBuf));
        AddDll(dllPtr);
    }
 ...
 // load the dlls
    for (i = GetDllCount(); i--; ) {
	    ...
	    if (libInst)
	    {
            ...
            lpCallbackType = GetProcAddress((HMODULE) libInst, (LPCSTR) _T("ExportLibSupport")); 
            if (lpCallbackType) {
         ...
         // Export Lib support for pre-scene export
            if (PreSceneCallback & CallbackType) {
                _tcscpy_s(lpStr, _T("PreSceneExport"));
                DllPreScene lpProc = (DllPreScene) GetProcAddress((HMODULE) libInst, (LPCSTR) lpStr);
                if(!lpProc) {
                   TCHAR buf[256];
                    _stprintf_s(buf, _T("method %s not implimented"), lpStr);
         ...
         // Export Lib support for post-scene export
            if (PostSceneCallback & CallbackType) {
                _tcscpy_s(lpStr, _T("PostSceneExport"));
         ...
         // Export Lib support for pre-node export
            if (PreNodeCallback & CallbackType) {
                _tcscpy_s(lpStr, _T("PreNodeExport"));
         ...
         // Export Lib support for post-node export
            if (PostNodeCallback & CallbackType) {
                _tcscpy_s(lpStr, _T("PostNodeExport"));
            ...
            TCHAR buf[256];
            _stprintf_s(buf, _T("Lib [%s] Load Failed"), GetDll(i));
	        MessageBox(GetActiveWindow(), buf, _T("Export Lib"), MB_OK|MB_TASKMODAL);
        }
    }
