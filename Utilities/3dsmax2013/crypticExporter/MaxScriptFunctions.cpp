...
//Was #include "definsfn.h" - is no longer in the SDK... 
...
{ 
	String* success = new String(_T("Export successful."));
	...
	if((count == 1 && is_string(arg_list[0])) || (count == 4 && (is_string(arg_list[0]) && is_bool(arg_list[1]) && is_bool(arg_list[2]) && is_bool(arg_list[3]))))
	{
		...
		{
			throw RuntimeError(_T("Error: Problem during export!"));
		}
	}
	else
	{
		throw RuntimeError(_T("Invalid arguments. Syntax is \"ExportVRML (string filename)\" or \"ExportVRML (string filename) (bool exportSelected) (bool exportHidden) (bool exportVertexColors)\"."));
	}
...
{ 
	...
	{
		throw RuntimeError(_T("Invalid arguments. Syntax is \"ExportDANIM (string filename)\"."));
	}
...
{ 
	...
	{
		throw RuntimeError(_T("Invalid arguments. Syntax is \"ExportVRML (string filename)\"."));
	}
}