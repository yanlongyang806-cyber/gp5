...
public:
	...
	const TCHAR *	ClassName() { return _T("AnimImport");}
...
LibDescription() { return _T(".anim import"); }
...
AnimImport::LongDesc() {			// Long ASCII description (i.e. "Targa 2.0 Image File")
	return _T("3ds max Animation Importer");
}

const TCHAR *
AnimImport::ShortDesc() {			// Short ASCII description (i.e. "Targa")
	return _T("Animation Import");
}
...
AnimImport::CopyrightMessage() {	// ASCII Copyright message
	return _T("Copyright 2004 Cryptic Studios");
}
...
int AnimImport::loadAnimFile(const TCHAR *name)
{
	...
	numFrames=0;
	fread(&v, 4, 1, pStream);	//in animexp.cpp the sizeof(header) must be 4
	numFrames = v / 160;
	...
	if (remainder) {
		MessageBox(ip->GetMAXHWnd(),_T("Corrupt .anim file?","Error"), _T("Error"), MB_OK);
		fclose(pStream);
		return 0;
	}
...
AnimRecord *AnimImport::findAnimRecord(const TSTR name)
{
	for (int i=0; i<numAnimRecords; i++) {
		if (stricmp(records[i].name, name.ToCStr())==0) {
			return &records[i];
		}
	}
	return NULL;
