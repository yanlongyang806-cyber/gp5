...
#ifdef _XBOX
#include "wininclude.h" // If this gets included in the Extern c section, conflicts happen
#endif

#define CLIENT

//#define PSDK_PERF_TIMING
...
#define DEFAULT_MAX_DYNAMIC_ACTOR_COUNT 1000
...
PSDKScene*				psdkScene[MAX_NX_SCENES]; // max scenes for now
...
void nwDebugDumpVerts( const char* name, const float* verts, U32 uiVertCount, U8 uiStride)
{
	U32 uiIndex;
	char cFileName[512];
...
void nwDebugDumpTris( const char* name, const U32* puiTris, U32 uiTriCount, U8 uiStride)
{
	U32 uiIndex;
	char cFileName[512];
...
U8 nwDeinitializeNovodex( )
{
	...
	if(actorGuidTable)
	{
		if ( stashGetCount(actorGuidTable) > 0 )
...
{
#ifdef CLIENT 
	if (nx_state.initialized && nxScene[NX_CLIENT_SCENE])
		return true;
#elif SERVER
	if ( nx_state.initialized )
		return true;
...
#endif
	psdkSceneDestroy(&psdkScene[iScene]);
...
int nwCreateScene()
{
...
#else
		nxSceneConstants.gravity = -32.15f;
		nxSceneConstants.staticFriction = 0.7f;
		nxSceneConstants.dynamicFriction = 0.5f;
		nxSceneConstants.shapeSkinWidth = 0.025f;
		nxSceneConstants.restitution = 0.4f;
#endif
	...
	for (iNewScene=0; iNewScene<MAX_NX_SCENES; ++iNewScene)
	{
		if (!nxScene[iNewScene])
			break;
	}
	
	if ( iNewScene == MAX_NX_SCENES )
	{
		PERFINFO_AUTO_STOP();
	...
	if ( !nx_state.softwareOnly && psdkHasHardwareSupport() )
	{
		sceneDesc.simType = NX_SIMULATION_HW;
		sceneDesc.hwSceneType = NX_HW_SCENE_TYPE_RB;
		psdkSceneCreate(&psdkScene[iNewScene], &sceneDesc, NULL, NULL);
		psdkGetNxScene(psdkScene[iNewScene], &nxScene[iNewScene]);
	}
...
static void processActorCreationQueue(int iSceneNum, bool bCreateActor)
{
	...
	while (pNewActorDesc)
	{
			...
			if ( bCreateActor )
			{
				...
				if ( pNewActor )
				{
					...
					if ( pEmissary->jointDesc )
					{
						...
						SAFE_DELETE(pEmissary->jointDesc);
					}
		...
		SAFE_DELETE(pNewActorDesc->body);
...
static void removeActor( NxActor* pActor, int iSceneNum )
{
	...
	if ( pEmissary )
	{
		...
		SAFE_DELETE(pEmissary);
	}
	else if ( pActor->isDynamic() )
	{
		Errorf("Dynamic actors should always have an NwEmissary!\n");
	}
	if ( pActor->isDynamic())
...
void* nwCookAndLoadMeshShape(const char* name, const void* vertArray, int vertCount, int vertStride, const void* triArray, int triCount, int triStride )
{
	...
	psdkCreateTriangleMesh(stream, &mesh);
	PERFINFO_AUTO_STOP();
...
void nwFreeMeshShape(void* shapeParam)
{
	...
	if(!shapeParam || !nx_state.initialized)
	{
		return;
	}
	...
	psdkReleaseTriangleMesh(shapeDesc->meshData);
...
{
	if ( !nx_state.maxDynamicActorCount || nx_state.dynamicActorCount < nx_state.maxDynamicActorCount )
		return true;
	return false;
}
...
void nwCookAndStoreConvexShape( const char* name, const float* verts, int vertcount, NxGroupType group, const FxGeo* geoOwner, int iSceneNum )
{
	...
	stream.resetByteCounter();
	if ( !psdkCreateConvexMesh(stream, &mesh) )
	{
...
{
	if (stashFindPointer(convexShapeTable, name, convexShapeDesc) )
		return true;
	return false;
}
...
void* nwCreateConvexActor(const char* name, Mat4 convexTransform, const float* verts, int vertcount, NxGroupType group, float fDensity, int iSceneNum)
{
	...
	if ( !status )
		return NULL;
	stream.resetByteCounter();
	psdkCreateConvexMesh(stream, &mesh);
...
// -------------------------------------------------------------------------------------------------------------------
void* nwCreateMeshActor(Mat4 meshTransform, void* shape, NxGroupType group, int iSceneNum)
{
	NxScene* pScene = nxScene[iSceneNum];
	assert( pScene );
...
{
	SAFE_DELETE_ARRAY(triMesh.points);
	SAFE_DELETE_ARRAY(triMesh.triangles);
}
// -------------------------------------------------------------------------------------------------------------------
...
void  nwAddCCDSkeletonForActor(void* actor)
{
	...
	for (int i=0; i< nbShapes; i++)
	{
		...
		NxCCDSkeleton* pCCDSkel;
		
		psdkCreateCCDSkeleton(&CCDMesh, &pCCDSkel);
...
{
	NxMat34* pNewMat = new NxMat34;
	nwSetNxMat34FromMat4(*pNewMat, (Vec3*)mat4);

	if ( nx_state.threaded )
	{
		AFQOneParam* pAFQObject = (AFQOneParam*)malloc(sizeof(AFQOneParam));
		pAFQObject->guidActor = guidActor;
		pAFQObject->pFunc = nwInternalSetActorMat4;

		pAFQObject->paramOne = (U32)pNewMat;

		nwEnqueueActorFunctionOneParam(pAFQObject);
	}
	else
	{
		nwInternalSetActorMat4(guidActor, (U32)pNewMat);
	}
...
static void  nwDebugRender(const NxDebugRenderable* pDebugData)
{
	psdkSetParameter(NX_VISUALIZATION_SCALE, 1);

	psdkSetParameter(NX_VISUALIZE_COLLISION_AABBS, 0);
	psdkSetParameter(NX_VISUALIZE_COLLISION_SHAPES, 1);
	psdkSetParameter(NX_VISUALIZE_ACTOR_AXES, 0);
	psdkSetParameter(NX_VISUALIZE_BODY_REDUCED, 0);
	psdkSetParameter(NX_VISUALIZE_BODY_SLEEP, 0);

	psdkSetParameter(NX_VISUALIZE_JOINT_WORLD_AXES, 0);
	psdkSetParameter(NX_VISUALIZE_JOINT_LIMITS, 0);
