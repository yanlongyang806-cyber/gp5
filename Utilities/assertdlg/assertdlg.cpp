...
TCHAR externalbuf[] = 
"Champions Online has encountered an unexpected crash\n";
...
LRESULT CALLBACK ExternalAssert(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{
			...
			SetWindowText(GetDlgItem(hDlg, IDC_STATIC_EXTERNALPRODUCT), externalbuf);

			SetWindowText(hDlg, "Champions Online Program Crash");
	...
	case WM_COMMAND:
		...
		{
			GetDlgItemText(hDlg, IDC_CRASHDESCRIPTION, externalReturnBuffer, 256);
                     ...
                     int       nCmdShow)
{
	...
	//ret = (int)DialogBox(hInst, (LPCTSTR)IDD_ABOUTBOX, NULL, (DLGPROC)Assert);
	ret = (int) DialogBox(hInst, (LPCTSTR)IDD_EXTERNAL, NULL, (DLGPROC)ExternalAssert);
	system("pause");
	return 0;
}
