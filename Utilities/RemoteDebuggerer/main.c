...
void getNameList() {
	...
	for(name_count=0; ; name_count++) {
		sprintf(key, "NameList%d", name_count);
...
void getBytes(char *buffer, int len) {
	...
	int origlen = len;
	char buf[256];
	sprintf(buf, "Downloading %d bytes...", len);
	status(buf);
	while (len) {
		...
		if (in_get_pdb_mode) {
			...
			sprintf(buf, "%d%%", pos*100/origlen);
			SetWindowText(hPdbButton, buf);
		} else {
			sprintf(buf, "Recv'd %d / %d bytes (%d%%)", pos, origlen, pos*100/origlen);
			status(buf);
		}
		printf("Read %d bytes\n", i);
	}
	printf("Read complete\n");
	sprintf(buf, "Recv complete (%d bytes)", origlen);
	status(buf);
}
...
void doAttach() {
	...
	{
		char temp[256];
        sprintf(temp, "Connected on port %d, waiting for call stack\n", port);
		status(temp);
	}
	sockSetDelay(s, 0);
