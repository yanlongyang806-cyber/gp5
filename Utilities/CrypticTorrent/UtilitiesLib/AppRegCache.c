...
static signed char app_registry_name_set = 0;

signed char regAppNameSet(void)
{
...
const char *regGetAppKey(void)
{
	...
	strcpy(buf, CRYPTIC_REG_KEY"\\");
	strcat(buf, regGetAppName());
...
void regRefresh(void){
	...
	memset(&RegRefreshFlags, 0, sizeof(RegRefreshFlags));
}
...
void regSetInstallationDir(const char* installDir){
	...
	strcpy(subkey, "SOFTWARE\\Cryptic\\");
	strcat(subkey, regGetAppName());
...
{
	static signed char gotPatchValue=0;
	static char patchValue[1024];

	if (!gotPatchValue) {
		regGetAppString("PatchValue", "", patchValue, sizeof(patchValue));
		gotPatchValue = 1;
	}
	return patchValue;
}
...
{
	static signed char gotPatchBandwidth=0;
	static char patchBandwidth[1024];

	if (!gotPatchBandwidth) {
		regGetAppString("TransferRate", "", patchBandwidth, sizeof(patchBandwidth));
		gotPatchBandwidth = 1;
	}
