			...
			// get available configs from server
			{
				...
				if ( msg == BUILDCOMM_SENDCONFIGS )
				{
					...
					for ( ; i > 0; --i )
					{
						...
						ParserSetTableInfo(tpiBuildConfig, sizeof(BuilderizerConfig), "BuildConfig", NULL, __FILE__, false);
						bcfg = StructAlloc(tpiBuildConfig);
						//ZeroStruct(bcfg);
						ParserRecv(tpiBuildConfig, pak, bcfg, 0);
				...
				if ( !buildIsInProgress )
				{
					for ( i = 0; i < eaSize(&loadedConfigs); ++i )
					{
						if ( listViewIsSelected(lvConfigs, loadedConfigs[i]) )
						{
							...
							ParserSend(tpiBuildConfig, pak, NULL, loadedConfigs[i], SENDDIFF_FLAG_FORCEPACKALL, 0, 0, NULL);
							pktSend(&pak);
							linkFlush(buildServerLink);
							buildIsInProgress = 1;
							buildFailed = 0;
				...
				if ( wParam == 0 )
				{
					...
					if ( waitForServerMsg(&pak) )
					{
						...
						buildIsInProgress = pktGetBitsPack(pak, 1);
						{
							...
							ParserSetTableInfo(tpiBuildStatus, sizeof(BuildStatus), "BuildStatus", NULL, __FILE__, false);
							bstats = StructAlloc(tpiBuildStatus);
							ZeroStruct(bstats);
							ParserRecv(tpiBuildStatus, pak, bstats, 0);
