...
void buildserverSendStatus(NetLink *link)
{
	...
	ParserSend(tpiBuildStatus, pak, NULL, &lastBuildStatus, SENDDIFF_FLAG_FORCEPACKALL, 0, 0, NULL);
	pktSend(&pak);
	linkFlush(link);
	if (!sentLastStatus)
		sentLastStatus = 1;
...
void buildserverRunStep(NetLink *link, Packet *pak)
{
	if ( !buildThread )
	{
		...
		ZeroStruct(step);
		ParserRecv(tpiBuildStep, pak, step, 0);
...
void buildserverHandleClientMsg(Packet *pak,int cmd,NetLink* link,void *user_data)
{
	BuildClientLink	*client = user_data;
