...
void loadAllChecksums()
{
	int count;
	int i;
	char **file_names = fileScanDir("C:/lists");
    count = eaSize( &file_names );
	for (i=0; i<count; i++)
	{
...
void patchTo(NewPigEntry **file_list)
{
	...
	if (fileExists(hog_file_name) && !delete_it) {
		...
		// Create
		assertmsg(0, "just call update?");
		//hogCreateFromMem(entries, count, hog_file_name, PIGERR_ASSERT);
	}
	SAFE_FREE(entries);
}
