...
typedef int (*uncompressDeltasFunc)(void *dst, const NxU8 *src, int stride, int count, int elemSize, bool isFloat);
...
class NxPhysicsSDK
	...
	*/
	virtual NxTriangleMesh* createTriangleMesh(const NxStream& stream, uncompressDeltasFunc uncompressDeltas) = 0;
