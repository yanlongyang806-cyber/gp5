...
namespace crnd
{
   ...
   // pHeader must point to a buffer holding the crn file header.
   // pData may point to a buffer holding the actual compressed data. If NULL, the data is assumed to
   // be located immediately after pHeader.
   ...
   crnd_unpack_context crnd_unpack_begin(const void *pHeader, const void* pData, uint32 data_size);
...
namespace crnd
{
	inline unsigned int crn_make_uint(const unsigned char (&buf)[1]) {
		return buf[0];
	}

	inline unsigned int crn_make_uint(const unsigned char (&buf)[2]) {
        return (buf[0] <<  8U) |  buf[1];
	}

	inline unsigned int crn_make_uint(const unsigned char (&buf)[3]) {
        return (buf[0] << 16U) | (buf[1] <<  8U) | (buf[2]);
	}

	inline unsigned int crn_make_uint(const unsigned char (&buf)[4]) {
...
#pragma pack(1)
   ...
   enum crn_header_flags
   {
	  ...
	  cCRNHeaderFlagReversedLevels = 2,
   };
   ...
   struct crn_header
   {
      ...
      crn_packed_uint<4>    m_level_ofs[cCRNMaxLevels];
      crn_packed_uint<4>    m_level_sizes[cCRNMaxLevels];
   };

   const unsigned int cCRNHeaderMinSize = (unsigned int)sizeof(crn_header);
...
#ifdef _PREFAST_
#define CRND_ANALYSIS_ASSUME(x)	__analysis_assume(x)
#else
#define CRND_ANALYSIS_ASSUME(x) ((void)0)
#endif

#ifdef NDEBUG
#define CRND_ASSERT(x) CRND_ANALYSIS_ASSUME(x)
#undef  CRND_ASSERTS_ENABLED
#else
#define CRND_ASSERT(_exp) do { if(!(_exp)) crnd::crnd_assert(#_exp, __FILE__, __LINE__); CRND_ANALYSIS_ASSUME(_exp); } while(0)
...
#define CRND_FOURCC(a, b, c, d) ((a) | ((b) << 8U) | ((c) << 16U) | ((d) << 24U))
   ...
   bool crnd_validate_file(const void* pData, uint32 data_size, crn_file_info* pFile_info)
   {
      ...
      if (pFile_info)
      {
         ...
         {
            pFile_info->m_level_compressed_size[i] = pHeader->m_level_sizes[i];
         }
   ...
   const void* crnd_get_level_data(const void* pData, uint32 data_size, uint32 level_index, uint32* pSize)
   {
      ...
      if (pSize)
      {
         *pSize = pHeader->m_level_sizes[level_index];
      }
...
namespace crnd
{
   ...
   public:
      ...
      bool init(const void *pHeader, const void* pData, uint32 data_size)
      {
         m_pHeader = crnd_get_header(m_tmp_header, pHeader, data_size);
         if (!m_pHeader)
            return false;
		 ...
		 if (pData)
			 ...
			 m_pData = static_cast<const uint8*>(pHeader) + sizeof(crn_header);
         ...
         uint32 level_index)
      {
		 ...
		 uint32 cur_level_size = m_pHeader->m_level_sizes[level_index];

         return unpack_level(m_pData + cur_level_ofs, cur_level_size, pDst, dst_size_in_bytes, row_pitch_in_bytes, level_index);
      }
         ...
         uint32 level_index)
      {
         ...
         bool status = false;
         switch ((uint)m_pHeader->m_format)
         {
         case cCRNFmtDXT1:
   ...
   crnd_unpack_context crnd_unpack_begin(const void *pHeader, const void* pData, uint32 data_size)
   {
      ...
      if (!p->init(pHeader, pData, data_size))
      {
         crnd_delete(p);
         return NULL;
      }
