...
namespace crnlib
{
   ...
   bool crn_comp::create_comp_data()
   {
	  ...
	  if (m_pParams->m_flags & cCRNCompFlagReverseLevels) {
		  m_crn_header.m_flags = crnd::cCRNHeaderFlagReversedLevels;
	  }

      m_comp_data.clear();
      m_comp_data.reserve(2*1024*1024);
      append_vec(m_comp_data, &m_crn_header, sizeof(m_crn_header));
	  uint start_ofs = sizeof(m_crn_header);
      ...
      if (m_packed_color_endpoints.size())
      {
         ...
         m_crn_header.m_color_endpoints.m_ofs = m_comp_data.size() - start_ofs;
      ...
      if (m_packed_color_selectors.size())
      {
         ...
         m_crn_header.m_color_selectors.m_ofs = m_comp_data.size() - start_ofs;
      ...
      if (m_packed_alpha_endpoints.size())
      {
         ...
         m_crn_header.m_alpha_endpoints.m_ofs = m_comp_data.size() - start_ofs;
      ...
      if (m_packed_alpha_selectors.size())
      {
         ...
         m_crn_header.m_alpha_selectors.m_ofs = m_comp_data.size() - start_ofs;
         append_vec(m_comp_data, m_packed_alpha_selectors);
      }

      m_crn_header.m_tables_ofs = m_comp_data.size() - start_ofs;
	  ...
	  uint level_sizes[cCRNMaxLevels];

	  for (uint i = 0; i < m_mip_groups.size(); i++)
	  {
		 uint lvl;
		 if (m_pParams->get_flag(cCRNCompFlagReverseLevels))
			lvl = m_mip_groups.size() - i - 1;
		 else
			lvl = i;

		 level_ofs[lvl] = m_comp_data.size();
		 append_vec(m_comp_data, m_packed_chunks[lvl]);
		 level_sizes[lvl] = m_comp_data.size() - level_ofs[lvl];
	  }
      ...
      for (uint i = 0; i < m_mip_groups.size(); i++)
	  {
         dst_header.m_level_ofs[i] = level_ofs[i] - start_ofs;
		 dst_header.m_level_sizes[i] = level_sizes[i];
	  }
	  
      const uint actual_header_size = sizeof(crnd::crn_header);

      dst_header.m_sig = crnd::crn_header::cCRNSigValue;

      dst_header.m_data_size = m_comp_data.size() - start_ofs;
