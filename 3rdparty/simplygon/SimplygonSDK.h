...
namespace SimplygonSDK
{
	...
	enum ErrorCodes 
		{
		SG_ERROR_NOERROR = 0,
		SG_ERROR_NOLICENSE = -1, // no license was found 
	...
	enum FeatureFlags
		...
		SG_FEATUREFLAGS_NONMANIFOLD			=	0X00000020,
		...
		SG_FEATUREFLAGS_ALL					=	0xffff01ff,
		};
	...
	/** 
	 * Deprecated values, use SG_MATERIAL_CHANNEL_[...] instead
	 */
	 ...
	 */
	static const unsigned int SG_NUM_SUPPORTED_TEXTURE_CHANNELS = 8;
	static const unsigned int SG_NUM_SUPPORTED_COLOR_CHANNELS = 8;

	/**
	 * The maximum number of supported bones per vertex in Simplygon
	 */
	static const unsigned int SG_BONES_PER_VERTEX = 4;
	...
	public:
		 ...
		 * No range checking is done in  release mode @param id - the id of the tuple 
		 */
		 ...
		 * needed) and then use  SetItem / SetRealTuple to set the items directly. @param id
		 * - the id of the tuple  @param tuple - the value(s) of the tuple 
		 */
	...
	public:
		 ...
		 * old tuples will be set to 0.  @param add_missing_fields if set to true, missing fields
		 * will be added to this field data object.
		 */
	 ...
	 * of a vertex, expressed with a 1-4 components real field. By convension, the existing
	 ...
	 * 'DiffuseColors'		The diffuse colour of the vertex.	The  colour is specified in a
	 * 4-component  (RGBA) uchar field.  'SpecularColors'	The specular colour component
	 * of the  vertex. The colour value is specified  in a 4-component (RGBA) uchar field.
	...
	public:
		...
		/**
		 * Adds/Removes/Gets the DiffuseColors field in the triangles. 
		 */
		virtual	void AddDiffuseColors(  ) = 0;
		...
		/**
		 * Adds/Removes/Gets a user-specified triangle-vertex field. To be able to add a field,
		 * it must have a name that does not  conflict with the existing triangle-vertex fields.
		 */
		virtual	void AddUserTriangleVertexField( IValueArray *field ) = 0;

		/**
		 * Adds/Removes/Gets the BoneWeights field in the vertices. Must have same tuplesize
		 * as BoneIds
		 */
		...
		/**
		 * Adds/Removes/Gets the SpecularColors field in the triangles. 
		 */
		virtual	CountedPointer<IRealArray> GetSpecularColors(  ) = 0;

		/**
		 * Adds/Removes/Gets the GroupIds field in the triangles.
		 */
		...
		/**
		 * Copies all fields in the specified triangle-vertex in the source geometry to this
		 * geometrys' triangle-vertex. CopyCombineTriangleVertices combines two triangles by
		 * linear interopolation into the destination triangle. CopyCombine3TriangleVertices
		 * combines three triangles via barycentric coordinates.
		 */
		virtual	void CopyTriangleVertex( IGeometryData *source , rid dest_id , rid src_id ) = 0;
		 ...
		 * combines three triangles via barycentric coordinates.
		 */
		virtual	void CopyCombine3TriangleVertices( IGeometryData *source , rid dest_id , rid src_id_1 , rid src_id_2 , rid src_id_3 , real alpha_1 , real alpha_2 ) = 0;
		...
		virtual	void RemoveBitangents( rid level ) = 0;

		/**
		 * Adds/Removes/Gets the BoneIds field in the vertices. Must have same tuplesize as
		 * BoneWeights
		 */
		virtual	void RemoveBoneIds(  ) = 0;

		/**
		 * Adds/Removes/Gets the SpecularColors field in the triangles. 
		 */
		virtual	void RemoveSpecularColors(  ) = 0;

		/**
		 * Convenience method to create, add, set name & tuple size of a user or custom field
		 * 		 */
		...
		/**
		 * Copies all fields in the specified vertex in the source geometry to this geometrys'
		 * vertex. CopyCombineVertices combines two vertices by linear interopolation into the
		...
		/**
		 * Adds/Removes/Gets the MaterialIds field in the triangles.
		 */
		 ...
		 * Convenience method to create, add, set name & tuple size of a user or custom field
		 * 		 */
		virtual	CountedPointer<IValueArray> AddBaseTypeUserTriangleVertexField( rid base_type , const char * name , unsigned int tuple_size ) = 0;

		/**
		 * Adds/Removes/Gets the BoneIds field in the vertices. Must have same tuplesize as
		 * BoneWeights
		 */
		virtual	void AddBoneIds( unsigned int tuplesize ) = 0;

		/**
		 * Copies all fields in the specified triangle in the source geometry to this geometrys'
		...
		/**
		 * Adds/Removes/Gets a TexCoords field in the triangles.  The valid id range of the
		 * level parameter is 0-15, which equals fields  'TexCoords0' - 'TexCoords15'
		 */
		 ...
		 * to the TriangleVertices field. The triangles will be remapped to use the new vertices
		 * correctly. Expanding the vertex field is required to be able to extract a range of
		 * the geometry, and to be able to stream it.  Note! This will lead to increased memory
		 * requirement, as vertex  data is not shared among neigbouring triangles. It also removes
		 * the connectivity information of the triangles, and effectively  detaches all triangles
		 * from one another. This call may replace  fields in the Vertices field data. Requery
		 * any previously cached  field pointer after the call. All user fields are copied as
		 * well.
		 */
		virtual	void ExpandVertices(  ) = 0;

		/**
		 * Tests if the extensts of the geometry fully contain all vertex coordinates. Note!
		 * ExtentsContainCoords checks all vertices in the geometry, regerdless to whether the
		 ...
		 * it must have a name that does not  conflict with the existing triangle-vertex fields.
		 */
		virtual	void RemoveUserTriangleVertexField( const char * name ) = 0;

		/**
		 * Gets/Sets the number of triangles in the geometry. The triangle-vertices count is
		 * always (TriangleCount*3)  AddTriangles increases the triangle count by the specified
		 * value.
		 */
		 ...
		 * combines three triangles via barycentric coordinates.
		 */
		virtual	void CopyCombineTriangleVertices( IGeometryData *source , rid dest_id , rid src_id_1 , rid src_id_2 , real alpha ) = 0;

		virtual	CountedPointer<IRealArray> GetTangents( rid level ) = 0;

		/**
		 * Adds/Removes/Gets a Color field in the triangles.  The valid id range of the level
		 * parameter is 0-15, which equals fields  'Colors0' - 'Colors15'
		 */
		...
		/**
		 * Retrieves the ReFieldData object that contains the triangle-vertex fields
		 */
		virtual	CountedPointer<IFieldData> GetTriangleVertices(  ) = 0;
		...
		virtual	void AddBitangents( rid level ) = 0;
		 ...
		 * it must have a name that does not  conflict with the existing triangle-vertex fields.
		 */
		virtual	CountedPointer<IValueArray> GetUserTriangleVertexField( const char * name ) = 0;
		...
		/**
		 * Adds/Removes/Gets the Normals field in the triangles. 
		 */
		 ...
		 * Adds/Removes/Gets the SpecularColors field in the triangles. 
		 */
		virtual	void AddSpecularColors(  ) = 0;

		/**
		 * Creates a "packed" RePackedGeometryData object, where all fields in the TriangleVertex
		 * field data object is moved to  the vertex field data object. Please note that the
		 * vertex field data object will be enlarged to accommodate for data in a vertex  shared
		 * by mutiple triangles, where the corner data differs between the triangles. The method
		 * will fail if a TriangleVertices field is named the same as an existing Vertices field,
		 * please make sure to remove any such field before calling the method.
		 */
		 ...
		 * and Bitangent fields of the TriangleVertices
		 */
		 ...
		 * Adds/Removes/Gets the DiffuseColors field in the triangles. 
		 */
		virtual	void RemoveDiffuseColors(  ) = 0;

		/**
		 * Adds/Removes/Gets the DiffuseColors field in the triangles. 
		 */
		virtual	CountedPointer<IRealArray> GetDiffuseColors(  ) = 0;
	...
	public:
		...
		/**
		 * Adds/Removes/Gets the SpecularColors field in the vertices. 
		 */
		virtual	CountedPointer<IUnsignedCharArray> GetSpecularColors(  ) = 0;
		...
		/**
		 * Adds/Removes/Gets the Normals field in the vertices. 
		 */
		virtual	void RemoveNormals(  ) = 0;
		...
		/**
		 * Convenience method to create, add, set name & tuple size of a user field 
		 */
		virtual	CountedPointer<IValueArray> AddBaseTypeUserVertexField( rid base_type , const char * name , unsigned int tuple_size ) = 0;
		 ...
		 * parameter is 0-15, which equals fields  'TexCoords0' - 'TexCoords15'
		 */
		virtual	void AddTexCoords( rid level ) = 0;
		...
		/**
		 * Adds/Removes/Gets the DiffuseColors field in the vertices. 
		 */
		virtual	void AddDiffuseColors(  ) = 0;
		 ...
		 * Adds/Removes/Gets the DiffuseColors field in the vertices. 
		 */
		virtual	CountedPointer<IUnsignedCharArray> GetDiffuseColors(  ) = 0;
	...
	public:
		...
		/**
		 * Set/Get the vertex color channel used for the material channel.  @param channel_name
		 ...
		 * channel, or -1 to not use the current vertex color channel @returns the vertex color
		 * channel, or -1 if no channel is set for the material channel
		 */
		...
		/**
		 * Set/Get the blend type of a texture in a layered texture. Note that the SetTextureBlendType
		 ...
		 * layer_id the layer id (0->size-1) @param blend_type the blend type to set @returns
		 * the blend type for the texture
		 */
		virtual	void SetLayeredTextureBlendType( const char * channel_name , rid layer_id , rid blend_type ) = 0;

		/**
		 * Set/Get a named texture texture coordinate level The valid values are 0->15 @param
		 * channel_name the name of the material channel. Default channels are defined in the
		 * SG_MATERIAL_CHANNEL_[...] defines @param level the texture coordinate level @returns
		 * the texture coordinate level
		 */
		...
		/**
		 * Set/Get a named texture filename in a layered texture. Note that the SetTexture always
		 * sets the first texture (layer id 0) @param channel_name the name of the material
		 * channel. Default channels are defined in the SG_MATERIAL_CHANNEL_[...] defines @param
		 * layer_id the layer id (0->size-1) @param texture_filename the texture file name @returns
		...
		/**
		 * Set/Get a named texture filename. If a texture image is set using SetTextureImage/SetLayeredTextureImage
		 ...
		 * defines @param texture_filename the texture file name @returns the texture file name
		 */
		virtual	rstring GetTexture( const char * channel_name ) = 0;

		/**
		 * Set/Get the base color of the material channel.  @param channel_name the name of
		 * the material channel. Default channels are defined in the SG_MATERIAL_CHANNEL_[...]
		 * defines @param r the red value  @param g the green value  @param b the blue value
		 * @param a the alpha value  @returns the value of the component
		 */
		...
		/**
		 * Set/Get a named texture image data. @param channel_name the name of the material
		 * channel. Default channels are defined in the SG_MATERIAL_CHANNEL_[...] defines @param
		 * image the image object @returns the texture object
		 */
		...
		/**
		 * Set/Get the texture coordinate level of a texture in a layered texture. Note that
		 * the SetTextureLevel always sets the first texture (layer id 0) @param channel_name
		 * the name of the material channel. Default channels are defined in the SG_MATERIAL_CHANNEL_[...]
		 * defines @param layer_id the layer id (0->size-1) @param level the texture coordinate
		 * level @returns the texture coordinate level
		 */
		...
		/**
		 * Set/Get a named texture image data in a layered texture. Note that the SetTextureImage
		 * always sets the first texture (layer id 0) @param channel_name the name of the material
		 * channel. Default channels are defined in the SG_MATERIAL_CHANNEL_[...] defines @param
		 * image the image object @returns the texture object
		 */
		...
		/**
		 * Set/Get the number of textures maps in a layered texture @param channel_name the
		 * name of the material channel. Default channels are defined in the SG_MATERIAL_CHANNEL_[...]
		 * defines @param size the number of textures in the layered texture (>=1) @returns
		 * the number of textures in the layered texture (>=1)
		 */
	...
	public:
		 ...
		 * the table. @param the material to add into the table @return the material id of the
	...
	public:
		...
		/**
		 * Concatenates a scaling transform to the current transform @param scaling the scaling
		 * vector
		 */
		 ...
		 * angle around the axis @param axis the rotation axis
		 */
		...
		/**
		 * Concatenates a translation transform to the current transform @param translation
	...
	public:
		 ...
		 * the MappingImage object. The image data will contain two fields: <br/> * ReRidArray
		 * - "TriangleIds" - TupleSize:1 - The id of the triangle  <br/> * ReUnsignedShortArray
		 * - "BarycentricCoords" - TupleSize:2 - Items A & B of the barycentric coordinate on
		 * the triangle. <br/> The user can control the size of the mapped texture using the
		 * values MappingTextureWidth and MappingTextureHeight
		 */
	...
	public:
		...
		virtual	void RunSymmetryPostProcess(  ) = 0;
	...
	public:
		 ...
		 * node the node to evaluate @param time the animation time to evaluate @param global_transform
		 * the transformation matrix that receives the global transformation 
		 */
	...
	inline const rchar *GetInterfaceVersionHash()
		{
		return "B6797378945380DB06E4F5BDBEEE0364E94D0CF2B8D319BF4072E26339B1093F";
		}
