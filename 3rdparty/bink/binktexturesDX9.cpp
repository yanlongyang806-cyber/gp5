...
S32 Create_Bink_shaders9( LPDIRECT3DDEVICE9 d3d_device )
{
  HRESULT hr;
  ID3DBlob *buffer = 0;
  ...
  if ( YCrCbToRGBNoPixelAlpha == 0 )
  {
	hr = D3DCompile(StrYCrCbToRGBNoPixelAlpha, sizeof(StrYCrCbToRGBNoPixelAlpha),
		0, 0, 0, "main", PIXEL_SHADER_TARGET, 0, 0, &buffer, NULL);
    if ( SUCCEEDED( hr ) )
    {
...
void Free_Bink_textures9( LPDIRECT3DDEVICE9 d3d_device,
                         BINKTEXTURESET * set_textures )
{
  BINKFRAMETEXTURES9 * abt[] = { &set_textures->textures9[ 0 ], &set_textures->textures9[ 1 ], &set_textures->tex_draw9 };
  BINKFRAMETEXTURES9 * bt;
  int i;
...
S32 Create_Bink_textures9( LPDIRECT3DDEVICE9 d3d_device,
                          BINKTEXTURESET * set_textures )
{
  BINKFRAMEBUFFERS * bb;
  BINKFRAMETEXTURES9 * bt;
  ...
  for ( i = 0; i < set_textures->bink_buffers.TotalFrames; ++i )
  {
    ...
    if ( bb->Frames[ i ].YPlane.Allocate )
    {
      if ( !make_texture( d3d_device,
                          ...
                          0, D3DFMT_A8, D3DPOOL_SYSTEMMEM, 1,
                          &bt->Ytexture,
...
fail:
  ...
  Free_Bink_textures9( d3d_device, set_textures );
  return( 0 );
}
...
void Lock_Bink_textures9( BINKTEXTURESET * set_textures, S32 lock_both_for_writing )
{
  BINKFRAMETEXTURES9 * bt;
...
void Unlock_Bink_textures9( LPDIRECT3DDEVICE9 d3d_device, BINKTEXTURESET * set_textures, HBINK Bink )
{
  BINKFRAMETEXTURES9 * bt;
  BINKFRAMEPLANESET * bp;
  int i;
  ...
  S32 num_rects;
  BINKFRAMETEXTURES9 * bt_dst;
  BINKFRAMETEXTURES9 * bt_src;

  bt_src = &set_textures->textures9[ set_textures->bink_buffers.FrameNum ];
  bt_dst = &set_textures->tex_draw9;
...
void Draw_Bink_textures9( LPDIRECT3DDEVICE9 d3d_device,
                         BINKTEXTURESET * set_textures,
                         U32 width,
                         U32 height,
                         F32 x_offset,
                         ...
                         S32 is_premultiplied_alpha )
{
  ...
  bt_draw = &set_textures->tex_draw9;
  ...
  vertices[ 0 ].sx = x_offset - 0.5f;
  vertices[ 0 ].sy = y_offset - 0.5f;
  vertices[ 0 ].sz = 0.0f;
  vertices[ 0 ].rhw = 1.0f;
  vertices[ 0 ].tu = 0.0f;
  vertices[ 0 ].tv = 0.0f;
  ...
  vertices[ 1 ].sx = x_offset + ( ( (F32)(S32) width ) * x_scale ) - 0.5f;
  vertices[ 1 ].tu = 1.0f;
#ifdef USE_PS_1_x
  vertices[ 1 ].tu1 = 1.0f;
  vertices[ 1 ].tu2 = 1.0f;
  ...
  vertices[ 2 ].sy = y_offset + ( ( (F32)(S32) height ) * y_scale ) - 0.5f;
  vertices[ 2 ].tv = 1.0f;
#ifdef USE_PS_1_x
  vertices[ 2 ].tv1 = 1.0f;
  vertices[ 2 ].tv2 = 1.0f;
...
#else
int symbol_to_make_linker_happy_7;
