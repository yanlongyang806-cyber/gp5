...
static ID3D10Device * d3d_device = 0;
static ID3D10RenderTargetView * d3d_render_target_view = 0;
static IDXGISwapChain * d3d_swap_chain = 0;
...
static int Init_d3d( HWND window, U32 width, U32 height )
{
  ...
  if ( FAILED( D3D10CreateDeviceAndSwapChain( NULL, D3D10_DRIVER_TYPE_HARDWARE , NULL,
                  0, D3D10_SDK_VERSION, &sd, &d3d_swap_chain, &d3d_device ) ) )
    return 0;
  ...
  if ( FAILED( d3d_swap_chain->GetBuffer( 0, __uuidof( ID3D10Texture2D ), (LPVOID*) &back_buffer ) ) )
    return 0;
 
  hr = d3d_device->CreateRenderTargetView( back_buffer, NULL, &d3d_render_target_view );
  back_buffer->Release();
  
  if ( FAILED( hr ) )
    return 0;

  d3d_device->OMSetRenderTargets( 1, &d3d_render_target_view, NULL );
...
static void Show_frame( HBINK Bink, BINKTEXTURESET * texture_set )
{
  ...
  F32 clear[4] = { 0.0f, 0.125f, 0.3f, 1.0f }; 
  d3d_device->ClearRenderTargetView( d3d_render_target_view, clear );
...
static void start_next_frame( HBINK Bink, BINKTEXTURESET * texture_set )
{
  ...
  Lock_Bink_textures( texture_set, 0 );
                    ...
                    int cmd_show )
{
  ...
  Bink = BinkOpen( cmd_line, BINKSNDTRACK | BINKNOFRAMEBUFFERS | BINKALPHA );
  if ( !Bink )
  {
...
// @cdep pre $set( c8sysincludes, -I$dx10path\include $c8sysincludes ) 
// @cdep pre $set( csysincludes64EMT, -I$dx10path\include $csysincludes64EMT ) 
// @cdep pre $requires(binktexturesdx10.cpp)
// @cdep pre $requiresbinary(d3d10.lib)
// @cdep pre $requiresbinary(advapi32.lib)
// @cdep pre $requiresbinary(winmm.lib)
// @cdep pre $requiresbinary(uuid.lib)
// @cdep pre $set( STD8LIBs, -LIBPATH:$dx10path\lib\x86 $STD8LIBs )
// @cdep pre $set( STDLIBs64EMT, -LIBPATH:$dx10path\lib\x64 $STDLIBs64EMT )
// @cdep pre $requiresbinary($builddir\binkw$platnum.lib)
// @cdep post $if( $exists($dx10path\include\d3d10.h), $BuildWin3264EXE, )
