 ...
 *
 * Created on 08/07/07 at 17:31:18.
 */
...
#define XONLINE_E_LOGON_PPLOGIN_FAILED                  _HRESULT_TYPEDEF_(0x80151103L)
#define XONLINE_E_LOGON_SPONSOR_TOKEN_INVALID           _HRESULT_TYPEDEF_(0x80151104L)
#define XONLINE_E_LOGON_SPONSOR_TOKEN_BANNED            _HRESULT_TYPEDEF_(0x80151105L)
#define XONLINE_E_LOGON_SPONSOR_TOKEN_USAGE_EXCEEDED    _HRESULT_TYPEDEF_(0x80151106L)
#define XONLINE_E_LOGON_FLASH_UPDATE_NOT_DOWNLOADED     _HRESULT_TYPEDEF_(0x80151107L)
#define XONLINE_E_LOGON_UPDATE_NOT_DOWNLOADED           _HRESULT_TYPEDEF_(0x80151108L)
...
#define XPROFILE_SETTING_MAX_AUDIO_DEVICE_DESC  (100 * sizeof(WCHAR))
...
// Just reserving IDs, server will never see these because they are client side only they can be changed if need be.
#define XPROFILE_OPTION_VOICE_AUTOMIX                       XPROFILEID( XUSER_DATA_TYPE_INT32,      sizeof(DWORD),          60)
#define XPROFILE_OPTION_VOICE_MICBOOST                      XPROFILEID( XUSER_DATA_TYPE_INT32,      sizeof(DWORD),          61)
#define XPROFILE_OPTION_VOICE_RECORDDEVICE_DESC             XPROFILEID( XUSER_DATA_TYPE_UNICODE,    XPROFILE_SETTING_MAX_AUDIO_DEVICE_DESC, 62)
#define XPROFILE_OPTION_VOICE_PLAYBACKDEVICE_ID             XPROFILEID( XUSER_DATA_TYPE_BINARY,     sizeof(GUID),           63)
#define XPROFILE_OPTION_VOICE_PLAYBACKDEVICE_DESC           XPROFILEID( XUSER_DATA_TYPE_UNICODE,    XPROFILE_SETTING_MAX_AUDIO_DEVICE_DESC, 64)
#define XPROFILE_OPTION_VOICE_PLAYBACKLEVEL                 XPROFILEID( XUSER_DATA_TYPE_FLOAT,      sizeof(FLOAT),          65)
#define XPROFILE_OPTION_VOICE_RECORDLEVEL                   XPROFILEID( XUSER_DATA_TYPE_FLOAT,      sizeof(FLOAT),          66)
#define XPROFILE_OPTION_VOICE_VADHIGH                       XPROFILEID( XUSER_DATA_TYPE_INT32,      sizeof(DWORD),          67)
#define XPROFILE_OPTION_VOICE_VADNORMAL                     XPROFILEID( XUSER_DATA_TYPE_INT32,      sizeof(DWORD),          68)
...
XOnlineGetServiceInfo(
    __in DWORD dwServiceID,
    __out PXONLINE_SERVICE_INFO pServiceInfo
    );
...
XStorageBuildServerPath(
    __in DWORD dwUserIndex,
    __in XSTORAGE_FACILITY StorageFacility,
    __in_bcount_opt(dwStorageFacilityInfoSize) CONST void *pvStorageFacilityInfo,
    __in_opt DWORD dwStorageFacilityInfoSize,
    __in LPCWSTR pwszItemName,
    __inout_ecount_opt(*pdwServerPathLength) WCHAR *pwszServerPath,
    __inout DWORD *pdwServerPathLength
    );
...
XStorageDownloadToMemoryGetProgress(
    __inout XOVERLAPPED *pXOverlapped,
    __out_opt DWORD *pdwPercentComplete,
    __out_opt ULONGLONG *pqwNumerator,
    __out_opt ULONGLONG *pqwDenominator
    );
...
XUserWriteProfileSettings(
    __in DWORD                          dwUserIndex,        // User index of player whose settings are written
    __in DWORD                          dwNumSettings,      // Count of settings in pSettings
    __in_ecount(dwNumSettings)          const PXUSER_PROFILE_SETTING   pSettings,      // Pointer to array of settings to set
...
XUserReadProfileSettings(
    __in DWORD                      dwTitleId,          // A title in your family or 0 for the current title
    __in DWORD                      dwUserIndex,        // User index of requesting user
    __in DWORD                      dwNumSettingIds,    // Count of setting ids in pdwSettingIds
    __in_ecount(dwNumSettingIds)    const DWORD*           pdwSettingIds,      // Pointer to array of settings to retrieve
    __inout DWORD*                  pcbResults,         // Size of pResults buffer.  If *pcbResults is 0 then required size is returned.
    __inout_bcount(*pcbResults)     PXUSER_READ_PROFILE_SETTING_RESULT pResults,
    __inout_opt PXOVERLAPPED        pXOverlapped
    );
...
XUserReadProfileSettingsByXuid(
    __in DWORD                    dwTitleId,          // A title in your family or 0 for the current title
    __in DWORD                    dwUserIndexRequester,   // User index of requesting user.  Used for permission checking.
    __in DWORD                    dwNumFor,           // Count of XUIDs in pxuidFor
    __in_ecount(dwNumFor)         const XUID*              pxuidFor,           // Pointer to array of XUIDs to request settings for
    __in DWORD                    dwNumSettingIds,    // Count of setting ids in pdwSettingIds
    __in_ecount(dwNumSettingIds)  const DWORD*             pdwSettingIds,      // Pointer to array of settings to retrieve
    __inout DWORD*                pcbResults,         // Size of pResults buffer.  If *pcbResults is 0 then required size is returned.
    __inout_bcount(*pcbResults)   PXUSER_READ_PROFILE_SETTING_RESULT pResults,
...
XSessionCreate(
    ...
    __inout ULONGLONG*          pqwSessionNonce,
    __inout PXSESSION_INFO      pSessionInfo,
    __inout_opt PXOVERLAPPED    pXOverlapped,
    __out HANDLE*               ph
    );
...
XSessionJoinLocal(
    __in HANDLE                 hSession,
    __in DWORD                  dwUserCount,
    __in_ecount(dwUserCount)    const DWORD*         pdwUserIndexes,
    __in_ecount(dwUserCount)    const BOOL*          pfPrivateSlots,
    __inout_opt PXOVERLAPPED    pXOverlapped
    );
...
XSessionJoinRemote(
    ...
    __in_ecount(dwXuidCount)    const XUID*          pXuids,
    __in_ecount(dwXuidCount)    const BOOL*          pfPrivateSlots,
    __inout_opt PXOVERLAPPED    pXOverlapped
    );
...
XSessionArbitrationRegister(
    __in HANDLE                 hSession,
    __in DWORD                  dwFlags,
    __in ULONGLONG              qwSessionNonce,
    __inout DWORD*              pcbResultsBuffer,
    __inout_bcount(*pcbResultsBuffer) PXSESSION_REGISTRATION_RESULTS pRegistrationResults,
    __inout_opt PXOVERLAPPED    pXOverlapped
    );
...
WINAPI
XSessionSearch(
    __in DWORD                  dwProcedureIndex,
    __in DWORD                  dwUserIndex,
    __in DWORD                  dwNumResults,
    __in WORD                   wNumProperties,
    __in WORD                   wNumContexts,
    __in_ecount(wNumProperties) PXUSER_PROPERTY      pSearchProperties,
    __in_ecount(wNumContexts)   PXUSER_CONTEXT       pSearchContexts,
    __inout DWORD*              pcbResultsBuffer,
    __inout_bcount(*pcbResultsBuffer) PXSESSION_SEARCHRESULT_HEADER pSearchResults,
    __inout_opt PXOVERLAPPED    pXOverlapped
    );
...
XSessionGetDetails(
    __in HANDLE                 hSession,
    __inout DWORD*              pcbResultsBuffer,
    __inout_bcount(*pcbResultsBuffer) XSESSION_LOCAL_DETAILS*  pSessionDetails,
    __inout_opt PXOVERLAPPED    pXOverlapped
    );
...
XSessionMigrateHost(
    __in HANDLE                 hSession,
    __in DWORD                  dwUserIndex,
    __inout XSESSION_INFO*      pSessionInfo,
    __inout_opt PXOVERLAPPED    pXOverlapped
    );
...
XSessionModifySkill(
    ...
    __in_ecount(dwNumXuids)     CONST XUID*             rgXuid,
    __inout_opt  PXOVERLAPPED   pXOverlapped
    );
...
XSessionCalculateSkill(
    __in      DWORD             dwNumSkills,
    __in_ecount(dwNumSkills)    double* rgMu,
    __in_ecount(dwNumSkills)    double* rgSigma,
    __out_ecount(dwNumSkills)   double* pdblAggregateMu,
    __out_ecount(dwNumSkills)   double* pdblAggregateSigma
    );
...
XSessionWriteStats(
    __in    HANDLE                          hSession,
    __in    XUID                            xuid,                       // Remote XUID to update
    __in    DWORD                           dwNumViews,                 // Number of structs in pViews
    __in_ecount(dwNumViews) CONST XSESSION_VIEW_PROPERTIES* pViews,                     // List of views to update
    __inout_opt XOVERLAPPED*                pXOverlapped
    );
...
XTitleServerCreateEnumerator(
    __in_opt      LPCSTR                      pszServerInfo,
    __in          DWORD                       cItem,
    __out_opt     PDWORD                      pcbBuffer,
...
XFriendsCreateEnumerator(
    ...
    __in DWORD dwFriendsToReturn,
    __in_ecount(dwFriendsToReturn) DWORD *pcbBuffer,
    __out HANDLE *ph
    );
...
XInviteSend(
    __in DWORD dwUserIndex,
    __in DWORD cInvitees,
    __in_ecount(cInvitees) const XUID *pXuidInvitees,
    __in_opt const WCHAR *pszText,
    __inout_opt XOVERLAPPED *pXOverlapped
    );
...
XPresenceCreateEnumerator(
    __in DWORD dwUserIndex,
    __in DWORD cPeers,
    __in_ecount(cPeers) const XUID *pPeers,
    __in DWORD dwStartingIndex,
    __in DWORD dwPeersToReturn,
    __out_ecount(dwPeersToReturn) DWORD *pcbBuffer,
    __out HANDLE *ph
    );
...
XStorageDelete(
    __in DWORD dwUserIndex,
    __in const WCHAR *wszServerPath,
    __inout_opt XOVERLAPPED *pXOverlapped
    );
...
XStorageDownloadToMemory(
    ...
    __in_bcount(dwBufferSize) const BYTE *pbBuffer,
    __in DWORD cbResults,
    __out_bcount(cbResults) XSTORAGE_DOWNLOAD_TO_MEMORY_RESULTS *pResults,
    __inout_opt XOVERLAPPED *pXOverlapped
    );
...
XStorageEnumerate(
    ...
    __in const WCHAR *wszServerPath,
    __in DWORD dwStartingIndex,
    __in DWORD dwMaxResultsToReturn,
    __in DWORD cbResults,
    __out_bcount(cbResults) XSTORAGE_ENUMERATE_RESULTS *pResults,
    __inout_opt XOVERLAPPED *pXOverlapped
    );
...
XStringVerify(
    __in DWORD dwFlags,
    __in const CHAR *szLocale,
    __in DWORD dwNumStrings,
    __in_ecount(dwNumStrings) const STRING_DATA *pStringData,
    __in DWORD cbResults,
    __out_bcount(cbResults) STRING_VERIFY_RESPONSE *pResults,
    __inout_opt XOVERLAPPED *pXOverlapped
    );
...
XUserEstimateRankForRating(
    __in DWORD dwNumRequests,
    __in_ecount(dwNumRequests) const XUSER_RANK_REQUEST *pRankRequests,
    __in DWORD cbResults,
    __out_bcount(cbResults) XUSER_ESTIMATE_RANK_RESULTS *pResults,
    __inout_opt XOVERLAPPED *pXOverlapped
    );
