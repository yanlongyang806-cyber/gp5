...
*
* The XML format of this request can be found here: \ref Account_AuthToken_Login_1
*
 ...
 * \ingroup login
 */
VIVOXSDK_DLLEXPORT void vx_req_account_authtoken_login_create(vx_req_account_authtoken_login_t ** req);
#endif
...
/**
* Used to start and stop session group file based recordings, and used to flush loop mode recordings to file.
* 
...
typedef struct vx_req_sessiongroup_control_recording {
    ...
    */
    VX_HANDLE sessiongroup_handle;
    /**
     * Number of delta frames per control frame. 
     * 0 means only the starting control frame. Valid only on START.
     */
    int delta_frames_per_control_frame;
    /**
     * Set to 1 to enable recording audio events.
     * Valid only on START
     */
    int enable_audio_recording_events;
    /**
     * The VXR File to write to. 
     * This can be NULL for any command except VX_SESSIONGROUP_RECORDING_FLUSH_TO_FILE 
     * or VX_SESSIONGROUP_RECORDING_CONTROL_START.
     */
    char *filename;
    /**
     * DEPRECATED: Set this when creating the session group.
     * @deprecated
     */
    int loop_mode_duration_seconds;
...
typedef struct vx_req_sessiongroup_set_playback_options {
   /**
    ...
    /**
    * Playback mode. 
    * Used to control the play mode (normal, silence skipping, etc).
    */
    ...
    */
    VX_HANDLE sessiongroup_handle;
    /**
    * Used speed up playback speed. 
    * Valid values are between 1.0 and 3.0. 
...
typedef struct vx_req_aux_set_vad_properties {
    ...
    vx_req_base_t base;
    /**
    * The 'Hangover time' - the time (in seconds) that it takes for the VAD to 
