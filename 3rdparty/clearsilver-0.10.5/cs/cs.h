...
/* CSFILEFREE is a callback function to handle freeing data allocated for
 * files in a CSFILELOAD callback. Don't set this up unless you set up
 * a CSFILELOAD callback. */
typedef void (*CSFILEFREE)(void *data);
...
/*
 * Function: cs_register_filefree - register a filefree function
 * Description: cs_register_filefree registers a filefree function that 
 *              overrides the default, which is simply free().
 * Input: parse - a pointer to an initialized CSPARSE structure
 *        filefree - a CSFILEFREE function
 * Output: None
 * Return: None
 *
 */

void cs_register_filefree(CSPARSE *parse, CSFILEFREE filefree);
