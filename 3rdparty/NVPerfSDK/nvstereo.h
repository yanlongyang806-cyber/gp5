...
namespace nv {
    namespace stereo {
        ...
        struct D3D9Type
        {
            ...
            static void UpdateTextureFromStaging(Device* pDevice, DeviceContext* pDevContext, Texture* tex, StagingResource* staging)
            {
                RECT stereoSrcRect;
                stereoSrcRect.top = 0;
		...
		// The D3D11 "Driver" for stereo updates, encapsulates the logic that is Direct3D10 specific.
		struct D3D11Type
		{
			...
			static const NV_STEREO_REGISTRY_PROFILE_TYPE RegistryProfileType = NVAPI_STEREO_DX11_REGISTRY_PROFILE;
			...
			static void UpdateTextureFromStaging(Device* pDevice, DeviceContext* pDevContext, Texture* tex, StagingResource* staging)
			{
				...
				pDevContext->CopySubresourceRegion(tex, 0, 0, 0, 0, staging, 0, &stereoSrcBox);
			}
		};
        ...
        public:
            ...
            NvAPI_Status Init(Device* dev)
			{
				// Set that we've initialized regardless--we'll only try to init once.
				mInitialized = true;

				return NvAPI_Stereo_CreateHandleFromIUnknown(dev, &mStereoHandle);
            }
			...
			{
				NvU8 stereoActive = 0;
				if (!mStereoHandle || NVAPI_OK != NvAPI_Stereo_IsActivated(mStereoHandle, &stereoActive)) {
					return false;
				}
			...
			void SetSurfaceCreationStereoMode(NVAPI_STEREO_SURFACECREATEMODE surfaceCreationMode)
			{
				if (mStereoHandle)
					NvAPI_Stereo_SetSurfaceCreationMode(mStereoHandle, surfaceCreationMode);
			}

            float GetConvergenceDepth() const { return mActive ? mConvergence : 1.0f; }


            void UpdateStereoTexture(Device* dev, DeviceContext* devContext, Texture* tex, bool deviceLost)
            {
                if (!mInitialized) {
                    Init(dev);
                }

                if (!mStereoHandle || !RequiresUpdate(deviceLost)) {
                    return;
                }

                StagingResource *staging = D3DType::CreateStagingResource(dev, mEyeSeparation, mSeparation, mConvergence);
                if (staging) {
                    D3DType::UpdateTextureFromStaging(dev, devContext, tex, staging);
                    staging->Release();
                }
            }
