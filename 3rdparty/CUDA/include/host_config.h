...
#include <bits/c++config.h> /* get _GLIBCXX_ATOMIC_BUILTINS */
...
namespace std
{
  ...
  {
    template<class T> inline typename T::char_type *copy_s(typename T::char_type*, size_t, const typename T::char_type*, size_t, _Secure_char_traits_tag);
    template<class T> inline typename T::char_type *move_s(typename T::char_type*, size_t, const typename T::char_type*, size_t, _Secure_char_traits_tag);
  }
}

namespace stdext
{
  template<class U, class V, class W> inline V unchecked_uninitialized_copy(U, U, V, W&);
}
